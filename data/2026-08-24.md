...
TL;DR: AVIOT是一种有效的视频压缩方法，在保持性能的同时显著降低了视觉标记负担。


<details>
  <summary>Details</summary>
Motivation: 视频语言模型处理视频时存在大量的表示冗余，压缩这些序列对于减轻语言模型解码的视觉标记负担至关重要。

Method: 提出了一种名为AVIOT的方法，将视频标记压缩视为将密集的帧观察经验测度传输到紧凑的目标测度。

Result: 在多个视频理解基准测试中，AVIOT在多种压缩比下匹配或优于未压缩基线，同时在较高的压缩比下仍保持强大的性能。

Conclusion: AVIOT方法在视频压缩和视频理解任务中表现出色。
...
TL;DR: OraRL通过优化RL后训练过程，显著提高了视频MLLMs的性能和效率


<details>
  <summary>Details</summary>
Motivation: 提高多模态大型语言模型（MLLMs）在视频感知任务中的样本效率和可扩展性

Method: 提出OraRL算法，通过解耦优势估计器和符号平衡剪枝来优化RL后训练过程

Result: OraRL在模型规模和数据量上均表现出良好的可扩展性，在多个视频感知任务上取得了显著的性能提升

Conclusion: OraRL是一种有效的RL后训练方法，可以显著提高视频MLLMs的性能和效率
...
TL;DR: DiffVC-ONE is a new generative video compression framework that improves quality and efficiency.


<details>
  <summary>Details</summary>
Motivation: Generative video compression faces challenges in achieving high temporal consistency and low inference cost at low bitrates.

Method: Proposes DiffVC-ONE, a diffusion-based generative video compression framework using a one-step Video Diffusion Transformer.

Result: DiffVC-ONE achieves state-of-the-art perceptual quality and temporal consistency with low inference cost.

Conclusion: DiffVC-ONE is an effective generative video compression framework that balances temporal consistency and inference cost.
...
TL;DR: 提出了一个基于单一外向视频生成以自我为中心视频的方法，该方法在性能上优于现有技术。


<details>
  <summary>Details</summary>
Motivation: 生成基于单一外向视频的以自我为中心的视频是AR/VR和物理AI中的一个新兴且重要的话题。与传统的全新视图合成相比，外向到自我生成是一个更具挑战性的任务，因为在极端视角变化和较大不可观测区域下，标准的几何条件变得高度不可靠。

Method: 提出了Grounded-Exo2Ego，这是一个在架构和数据层面上解决这些挑战的框架。它是一个双分支视频扩散模型，结合了几何锚定分支和语义接地分支。此外，还引入了相机重定位算法，并开发了一个全自动化合成数据引擎。

Result: 在EgoExo4D数据集上的评估表明，该方法在所有指标上都优于最近的最新方法。

Conclusion: 该方法在所有指标上都显著优于现有的方法，并证明了在数据架构方面的贡献。
...
TL;DR: 该方法通过空间上下文建模和注意力机制，提高了灾害损坏分类的准确性和模型的可迁移性


<details>
  <summary>Details</summary>
Motivation: 灾害损坏的空间性，使用空间上下文进行损坏分类的不足，以及不同灾害事件中空间邻近性的差异

Method: 利用GPS导出的建筑图来建模空间上下文，通过灾难类型条件图模型和注意力机制来调整邻域尺度，添加残差去相关损失以避免过度平滑

Result: 在xBD数据集上，该方法提高了宏观F1分数，显著减少了残留的空间自相关，表明更有效地利用了空间上下文，并使模型能够更可靠地迁移到已知灾害类型中的未知事件

Conclusion: 该方法有效地利用了空间上下文，提高了灾害损坏分类的准确性，并提高了模型的可迁移性
...
TL;DR: 提出ANT框架，通过分割引导的TTA解决跨中心领域差异，提高PCa检测的准确性。


<details>
  <summary>Details</summary>
Motivation: 跨临床中心领域差异是深度学习模型在前列腺癌（PCa）检测中的应用障碍。

Method: 提出了一种名为ANT的分割引导的测试时自适应（TTA）框架。

Result: 在跨中心的临床试验中，该模型在活检核心和患者层面的平均AUC分别提高了2.9%和3.6%。

Conclusion: ANT框架在纠正领域特定特征漂移的同时，保留了癌症判别结构，提高了PCa检测的准确性。
...
TL;DR: 提出了一种新的图像真实性分析方法，通过估计伪影模式实现图像篡改定位，效果优于现有方法。
...
Method: 提出了一种零样本、无训练的盲图像篡改定位流程

Result: 估计参考伪影模式，并生成像素级篡改概率图

Conclusion: 该方法在图像篡改定位方面具有竞争力
...
TL;DR: This paper presents a benchmark for deep learning methods in grape leaf disease recognition, highlighting the importance of dataset quality and annotation compatibility.
...
Method: Dataset-centric benchmark of deep learning methods

Result: Results show near-saturated classification performance on several controlled or derivative datasets, greater difficulty on heterogeneous datasets, and substantial variation in detection performance across annotation settings. Cross-dataset performance drops sharply.
...
TL;DR: RECOUNT是一种基于图像的零样本计数框架，通过扩散模型扩展参考图像，显著提高了计数准确性。


<details>
  <summary>Details</summary>
Motivation: 针对现有零样本对象计数器在空间定位上的优势和对新或细粒度类别的分类能力不足的问题，提出RECOUNT框架。

Method: 使用扩散模型将单个场景外的参考图像转换为多样化的示例画廊，并使用视觉模块对候选对象进行分类。

Result: RECOUNT在LookAlikes和PairTally两个基准测试中取得了最佳的零样本计数准确率，分别将计数误差降低了55%和21%。

Conclusion: RECOUNT是一种有效的图像引导零样本计数框架，能够提高计数准确性。
...
TL;DR: MV2GF通过视觉几何基础模型提高了MVPD的泛化能力。


<details>
  <summary>Details</summary>
Motivation: Multi-View Pedestrian Detection (MVPD) 方法在未见过的相机配置中难以泛化，主要因为难以捕捉准确的视觉几何和过度依赖图像特征投影的畸变模式。

Method: 提出了一种基于视觉几何基础模型的MV2GF方法，该模型融合了特定任务的特征和基础模型提取的通用几何特征，并通过3D点图将图像特征投影到适当的3D位置。

Result: 实验表明，MV2GF在MVPD中比现有方法具有更好的泛化能力。

Conclusion: 基于视觉几何基础模型的MV2GF在MVPD中表现出优异的性能，提高了模型在未见过的相机配置中的泛化能力。
...
TL;DR: 提出了LAF框架，用于比较现有系统，诊断不可逆故障，并指定未来代理的可修订3D感知。


<details>
  <summary>Details</summary>
Motivation: 为了解决将二维基础模型的预测转移到三维分割中的方法分类问题，这些方法通常按任务或表示进行分组，但这样的分组掩盖了系统在各个视图之间保持一致性的决策。

Method: 提出了一个名为LAF（Lift, Associate, and Fuse）的决策中心框架，该框架将传输系统表示为五个操作符：Generate, Associate, Reconcile, Fuse和Persist/Query。LAF定义了持久载体的显式合同，包括其空间支持、语义状态、身份状态、不确定性、来源和所支持的操作，并确定了丢弃的证据变得不可恢复的第一阶段。

Result: 通过将框架操作化为结构化审计协议，并将其应用于截至2026年8月7日的161个系统，结果决策跟踪揭示了四个反复出现的属性：关联不建立身份；载体设计固定了查询界面和纠正边界；渲染视图、原生3D和提议级评估不可互换；并且像“训练免费”、“实时”、“开放词汇”和“通用”这样的限定词只有在附加到阶段和完整的成本账本上才有意义。LAF因此为比较现有系统、诊断不可逆故障和指定未来代理的可修订3D感知提供了一种表示中性的方法。

Conclusion: LAF框架为比较现有系统、诊断不可逆故障和指定未来代理的可修订3D感知提供了一种表示中性的方法。
...
TL;DR: The paper characterizes Aristotelian Manifolds and shows how they can be used to understand foundation model latent spaces.


<details>
  <summary>Details</summary>
Motivation: The paper aims to formalize and systematically characterize Aristotelian Manifolds.

Method: The authors use a layer-wise investigation to map knowledge synthesis in latent subspaces and analyze the interplay between network depth, dimensionality reduction, and distance metrics.

Result: The study reveals distinct geometric response profiles in different data domains and establishes a predictable taxonomy for layer selection and feature compression.

Conclusion: Mapping the internal geometry of frozen representations provides a robust and interpretable framework for understanding foundation model latent spaces.
...
TL;DR: TopoSurfel：通过结合网格引导和连续代理网格提取，提高3DGS重建精度


<details>
  <summary>Details</summary>
Motivation: 解决从3D Gaussian Splatting直接提取高保真表面的问题

Method: 提出TopoSurfel框架，动态提取连续代理网格，并结合网格引导的surfel演变策略，包括法线对齐和几何感知密度控制

Result: TopoSurfel在几何重建精度方面表现出色，同时保持高质量的基于网格的新型视图合成

Conclusion: TopoSurfel能够有效解决3DGS中的结构模糊性问题，提高重建质量
...
TL;DR: 提出身份感知HOI动作字幕方法，提高字幕描述准确性。


<details>
  <summary>Details</summary>
Motivation: 针对现有的人类-物体交互（HOI）动作字幕方法无法准确描述主体身份的问题，提出了一种身份感知的人类-物体交互动作字幕任务。

Method: 设计了一种基于BEHAVE和InterCap数据集的身份感知HOI动作字幕，提出了ID-HOINet模型。ID-HOINet包含两个核心组件：多视角身份-运动学习模块（MVIML）和两阶段字幕重写策略（TSCR）。

Result: 实验表明，ID-HOINet取得了最先进的性能。

Conclusion: 身份感知的人类-物体交互动作字幕方法能够提高字幕的准确性和描述性。
...
TL;DR: PanoCtrl是一种创新的文本到全景图生成方法，在空间对齐和图像质量方面表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有的文本到全景图方法在将对象级方向描述与球形全景场景准确对应方面存在不足。

Method: 提出PanoCtrl，一个以对象为中心的可控文本到全景图生成框架。

Result: PanoCtrl通过将文本描述转换为结构化对象级球形条件并将其集成到扩散过程中，实现了文本和球形全景空间的明确连接。

Conclusion: PanoCtrl在空间对齐和图像质量方面均达到最先进的性能。
...
TL;DR: ArtiMo: A novel framework for text-guided articulated mesh animation


<details>
  <summary>Details</summary>
Motivation: Animating articulated 3D meshes via text is challenging due to the absence of task-specific training data and explicit articulation supervision.

Method: Proposing ArtiMo, a novel agent-driven framework for text-guided articulated mesh animation, using Large Language and Vision-Language Models (LLMs/VLMs).

Result: ArtiMo significantly outperforms baselines, particularly on complex, causally driven motions.

Conclusion: ArtiMo is a novel framework for text-guided articulated mesh animation, offering improved performance over existing methods.
...
TL;DR: 提出AGIDefect-4K数据集和AGIDA框架，以改善AGI缺陷诊断。


<details>
  <summary>Details</summary>
Motivation: 为了填补现有AI生成图像（AGI）评估基准的空白，提出了AGIDefect-4K数据集。

Method: 构建了一个包含15个顶级生成模型（开源和闭源）的4,000张图像的丰富注释数据集。数据集具有分层缺陷注释：检测标签、像素级分割掩码和详细文本说明。还提出了AGIDA框架，利用多模态大型语言模型（MLLMs）进行联合缺陷检测、定位、解释和质量预测。

Result: 在AGIDefect-4K上进行全面基准测试表明，AGI缺陷理解仍然具有挑战性，强调了该数据集的价值。

Conclusion: AGIDefect-4K数据集和AGIDA框架有助于提高AGI缺陷诊断的准确性和可靠性。
...
TL;DR: AffordAny is an effective and robust framework for open-world 3D affordance grounding.


<details>
  <summary>Details</summary>
Motivation: Open-world 3D affordance grounding requires localizing functional object parts in 3D given free-form language queries.

Method: AffordAny, an end-to-end framework using one monocular RGB image to construct large-scale text-conditioned 3D part supervision, ground affordances with a frozen vision-language model (VLM) guided decoder, and improve open-world generalization through pseudo-label self-training.

Result: Benchmark of 5,334 objects and 10,633 part-level samples, decoder progressively fuses frozen Cosmos-2B features with 3D geometry, pseudo-label self-training adds new objects without human annotation, achieves 0.428 IoU on unseen objects and 0.315 IoU on unseen categories after self-training.

Conclusion: AffordAny demonstrates effectiveness and robustness in open-world 3D affordance grounding.
...
TL;DR: 提出VisTa3D数据集和基于触觉数据的3D重建模型，提高薄物体重建精度


<details>
  <summary>Details</summary>
Motivation: 为了解决现有3D重建模型在薄物体上的性能不足问题

Method: 构建了包含同步RGB图像、深度图和触觉响应图的薄物体数据集，并引入了视觉-范围-触觉3D重建模型作为基准

Result: 发现现有3D重建模型在薄物体上的重建精度较低，触觉数据有助于提高重建精度

Conclusion: 提出了VisTa3D数据集和基于触觉数据的3D重建模型，为薄物体的重建提供了新的思路和方法
...
TL;DR: MVAP-G是一种针对VGGT安全漏洞的多视图对抗攻击方法


<details>
  <summary>Details</summary>
Motivation: 针对VGGT模型在3D重建中的安全漏洞问题

Method: 提出MVAP-G，一个多视图对抗扰动生成器，生成跨多个视图的不可察觉一致扰动

Result: 显著降低VGGT性能，并在实验中证明其有效性

Conclusion: 为3D视觉系统提供了新的安全挑战和解决方案
...
TL;DR: AESR：一种提升身份保持视频生成的轻量级增强框架，在挑战赛中取得第一。


<details>
  <summary>Details</summary>
Motivation: 为了解决现有身份保持视频生成模型在复杂提示下的身份漂移、指令不完整和视觉细节缺失等问题。

Method: 提出了一种轻量级的增强框架AESR，包含全局代理提示增强模块和样本级视觉语义修复模块。

Result: 在ACM MM 2026身份保持视频生成挑战赛中排名第一，证明AESR在实际身份保持视频生成中的有效性。

Conclusion: AESR能够有效提高身份保持视频生成模型的性能。
...
TL;DR: SPARK-SAM is a self-prompt adaptation model for automatic infrared small-target segmentation, achieving state-of-the-art results.
...
Method: We introduce SPARK-SAM (Self-Prompt Adaptation with Response Knowledge for SAM), which learns target-domain response knowledge and conditions the decoder through an image-conditioned joint self-prompt state. Training combines benchmark-mask supervision with reliability-aware response guidance.
...
Conclusion: SPARK-SAM significantly improves the performance of automatic infrared small-target segmentation.
...
TL;DR: Vis-Poison是一种新型视觉知识中毒攻击，可严重损害多模态MLLM生成。


<details>
  <summary>Details</summary>
Motivation: 为了提高多模态检索增强生成（RAG）系统的性能，越来越多地依赖图像作为外部知识源。然而，引入受毒的视觉证据可能会严重损害多模态大型语言模型（MLLM）的生成。

Method: 提出了一种名为Vis-Poison的新型视觉知识中毒攻击，其中中毒图像本身是攻击者控制的载荷，无需操纵标题、摘要、元数据或其他相关文本。攻击通过自动化多智能体方法实现，该方法构建视觉上合理的受毒图像。

Result: 在黑盒设置中，Vis-Poison在30k条条目的多模态知识库上实现了40.16%到65.40%的端到端攻击成功率。此外，Vis-Poison对各种仅从参数知识中正确回答的MLLM仍然有效，平均成功率超过60%。

Conclusion: Vis-Poison是一种有效的视觉知识中毒攻击，可以严重损害多模态MLLM的生成。
...
TL;DR: DiGS-Avatar通过UV-潜在补全和教师-学生框架实现高效的3D人体重建，具有高保真度和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 为了解决单图像3D人体重建中纹理过度平滑和几何不一致的问题，以及扩散模型在3D重建前依赖多视图合成的计算成本高和视图不一致的问题。

Method: 提出DiGS-Avatar，将任务重新定义为高效的基于扩散的UV-潜在补全任务，通过设计确保3D一致性。引入教师-学生框架，多视图教师提供几何对齐的伪地面真相潜变量来监督单视图扩散学生。将推断出的潜在变量视为稳健的结构骨架，方法注入高级语义特征以准确恢复精细纹理细节，同时不破坏空间完整性。然后将改进的表示解码为3D高斯原语。

Result: DiGS-Avatar在视觉保真度和零样本泛化方面达到最先进或高度竞争的水平，同时只需0.71秒即可重建一个完全可动化的3D头像。

Conclusion: DiGS-Avatar是一种有效的单图像3D人体重建方法，能够以较低的计算成本实现高质量的3D重建，并在视觉保真度和泛化能力方面具有显著优势。
...
TL;DR: VLMs在将信念表示用于预测时存在关键缺陷


<details>
  <summary>Details</summary>
Motivation: 探究视觉语言模型（VLMs）内部表示心理状态（如代理的信念、知识和意图）的使用情况

Method: 提出跨轴路由诊断（CARD）方法，通过测量不同轴预测的响应来引导一个轴的激活

Result: 在Relay Chain（一个新合作网格世界基准）上应用该方法，发现模型未能将信念表示纳入下一动作预测，导致未充分利用关于其合作伙伴的有价值信息

Conclusion: VLMs在将信念表示用于下游预测方面存在关键路由失败
...
TL;DR: MotionPhys解决AI视频生成中物理运动一致性挑战


<details>
  <summary>Details</summary>
Motivation: 分析现有AI视频生成模型中物理运动一致性问题

Method: 提出MotionPhys框架，将稀疏运动轨迹视为物理证据，并建模轨迹几何演变

Result: MotionPhys可以有效地检测生成视频中的物理不一致性，并在不同视频生成器中具有良好的泛化能力

Conclusion: MotionPhys是一种有效的方法，可以解决AI视频生成中的物理运动一致性挑战
...
TL;DR:  ClinX，一个端到端的多模态PHI清理框架，在保留临床相关视觉上下文的同时，大幅减少可恢复的PHI。


<details>
  <summary>Details</summary>
Motivation: 医疗图像-文本数据可能通过可见图像内容和伴随文本暴露受保护的健康信息（PHI），为隐私保护型医疗AI系统带来障碍。

Method:  ClinX，一个端到端的多模态PHI清理框架，使用OCR检测可见标识符，构建二进制PHI掩码，并应用ClinX-PRISM，一个无跳过的生成式恢复模块，具有隐私导向的后处理以抑制烧入的标识符。同时，通过渐进的脱敏级别减少文本侧的PHI：正则表达式掩码、上下文感知掩码和基于重写的清理。

Result: OCR仅掩码作为独立解决方案不足，基于恢复的清理在保留临床相关视觉上下文的同时，大幅减少可恢复的PHI。

Conclusion: 提出了 ClinX，一个端到端的多模态PHI清理框架，以解决医疗图像-文本数据中的隐私问题。
...
TL;DR: 一种基于深度学习的MVS方法，通过DFM和MVS融合，提高深度图质量。


<details>
  <summary>Details</summary>
Motivation: 为了减轻深度学习在MVS（多视图立体）中泛化能力差的问题，尤其是遮挡区域或视图重叠有限的区域。

Method: 提出了一种将DFM（深度基础模型）与级联MVS管道紧密耦合的新框架，通过双向互 refinement策略。该方法利用MVS深度来解决单目预测中的尺度模糊，同时单目深度反过来又增强了MVS估计的结构完整性和细粒度细节。此外，还引入了先验引导的成本体细化机制，通过基于注意力的融合和离散化深度分箱，有效地整合了多视图和单目信息，从而促进局部几何一致性。

Result: 在标准基准测试中，该方法优于最先进的MVS方法，生成了更完整、泛化能力更强的深度图，具有锐利的边界。即使在稀疏视图设置下，该框架也能很好地泛化，与专门的稀疏视图方法竞争，同时保持优越的精度-效率权衡。

Conclusion: 该研究提出了一种新型的MVS方法，通过融合DFM和MVS管道，显著提高了深度图的完整性和泛化能力。
...
TL;DR: CertVLA是一种针对VLA政策的认证防御方法，可以有效地防御补丁攻击和纹理攻击。


<details>
  <summary>Details</summary>
Motivation: Vision-Language-Action (VLA) policies对局部物理扰动很脆弱，现有的防御方法无法直接验证连续、时间相关的动作。

Method: 提出CertVLA，针对有限补丁和纹理攻击的闭环VLA控制进行认证防御。通过行为一致的动作范围和确定性覆盖掩码来确保至少有一个检查的预测是攻击无关的。

Result: 证明了CertVLA的有效性，可以确保攻击者只能执行与攻击者清除的干净预测一致的动作块。

Conclusion: CertVLA可以有效地防御针对VLA政策的攻击，包括补丁攻击和纹理攻击。
...
TL;DR: Route2Look：长视频理解中的查询自适应证据获取新框架


<details>
  <summary>Details</summary>
Motivation: 长视频理解中查询需求与证据获取策略之间存在不匹配，导致理解困难。

Method: 提出Route2Look，一个轻量级、模型无关的查询自适应证据获取框架，通过Route-Look-Memorize循环和三个工具：全局浏览、时间基准和语义检索。
...
Conclusion: TRACE is an effective framework for breast ultrasound diagnosis, offering improved performance and robustness compared to existing methods.
...
TL;DR: S2V 方法通过基于段的 VQA，有效地提高了 LVU 的细粒度推理，并优于其他方法。


<details>
  <summary>Details</summary>
Motivation: Multimodal Large Language Models (MLLMs) 在视频理解方面的潜力，但长视频理解 (LVU) 面临挑战，因为复杂且冗长的背景中的干扰噪音会模糊局部细节，误导 MLLMs 产生错误答案。

Method: 提出了一种新的 Segment-to-Video Supervision 方法 (S2V) 来有效提高 LVU 中的细粒度推理。具体来说，我们基于局部段生成问题答案对 (VQA)，然后将这些基于段的 VQA 转换回整个视频进行训练。

Result: S2V 模型在多个 LVU 基准测试中始终如一地提高了 LVU 性能，在 LVU 准确性以及训练和推理效率方面优于通用的 MLLMs 和基于推理的方法。

Conclusion: S2V 方法在 LVU 中表现出色，通过关注短段来避免整个视频中的干扰噪音，从而有效地提高了细粒度推理。
...
TL;DR: 一种高效、精确的文档处理方法


<details>
  <summary>Details</summary>
Motivation: 减少多阶段错误传播和提高文档处理效率

Method: 使用微调的SmolDocling VLM进行端到端关键值提取，结合文档图像处理和标签扩展，设计数据增强和布局感知评估框架

Result: 在多个数据集上优于大型零样本VLM基线，比Qwen2.5-VL小27倍，速度快5倍以上

Conclusion: 提出了一种高效的文档处理方法，可显著提高文档关键值提取的准确性和速度
...
TL;DR: RDANet是一种新的红外小目标检测网络，在多个基准测试中表现出色，提高了检测的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 红外小目标检测在遥感图像中仍然具有挑战性，因为目标非常小，局部对比度弱，并且经常嵌入在复杂且高度变化的环境中。

Method: 提出了一种名为RDANet的相对退化感知网络，包括多尺度抗混叠降采样（MSAD）和原型引导跳过记忆（PGSM）模块。

Result: 在三个公共基准测试中，RDANet在大多数评估指标上实现了最佳性能，并且在目标大小和场景复杂度方面表现出更稳定的行为。
...
TL;DR: 本研究提出了一种结合摄像头和激光雷达的车牌检测方法，在多个国家和地区进行了广泛评估，结果显示该方法在车牌识别方面具有很高的准确性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 可靠的车牌检测对于自动驾驶系统的全球部署至关重要，这取决于在不同地区、距离和天气条件下正确感知标志。

Method: 结合摄像头和激光雷达传感器，提出了一种多模态检测框架，该框架的强度感知可变形融合模块将反射型激光雷达线索与相机特征对齐，将检测锚定在几何不变量而不是区域特定的视觉外观上。同时，引入了一种双运动模型跟踪器，显式地考虑了车辆接近标志时的非线性透视变换，从而在比线性运动假设更高的时间一致性上进行了改进。此外，还开发了一种语义属性分类管道，估计遮挡级别、可读性、标志嵌入度和道路相关性，为下游规划提供可操作上下文。

Result: 在60多个国家和2500多小时驾驶数据的测试集中，所提出的管道在221,068个评估序列中实现了0.49%的物体丢失率（OMR），证明了在商业级自动驾驶系统中具有全球可推广的车牌感知能力。

Conclusion: 本研究提出的方法在全局可推广的车牌感知方面取得了显著成果，为自动驾驶系统的安全性和可靠性提供了有力支持。
...
TL;DR: LoRC是一种新的AIGI检测框架，通过几何方法识别生成器的特征，提高了检测准确率，并在真实环境中表现出良好的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 为了揭示现代生成器中的微妙非语义视觉差异，从而进行AIGI检测。

Method: 从几何角度重新审视AIGI检测，并识别出一个架构无关的签名。

Result: 提出的LoRC框架提高了7.0%的准确率，并在39个未见过的生成器上达到97.0%的准确率。

Conclusion: LoRC是一种可靠的AIGI检测方法，在复杂真实环境中具有强大的跨模型泛化能力和鲁棒性。
...
TL;DR: 高安全阈值下的FRS面临基于分数的冒充攻击威胁，攻击成功率高达92%以上。


<details>
  <summary>Details</summary>
Motivation: 面对高安全阈值设置下的面部识别系统（FRS）的安全漏洞，现有研究主要关注中等安全阈值设置，不适用于高安全约束下的FRS。

Method: 提出对高安全阈值设置下的FRS进行成功的冒充攻击，对基于分数的冒充攻击进行数学分析，并在LFW基准上进行测试。

Result: 攻击成功率达到92%以上，且在多个开源FRS上表现稳健。

Conclusion: 在高安全阈值设置下，基于分数的冒充攻击对FRS构成严重威胁。
...
TL;DR: EviRank通过语义约束满足和证据包处理，实现了高效的多模态图像重排序。


<details>
  <summary>Details</summary>
Motivation: 现有重排序器无法有效地处理多模态和组合式的图像搜索查询。

Method: 提出EviRank算法，将多模态图像重排序转化为语义约束满足问题，并解析查询为统一证据包。

Result: 在五个基准测试中，EviRank实现了最先进的性能，蒸馏的学生模型保留了超过90%的教师模型能力。

Conclusion: EviRank在多模态图像重排序方面表现出色，并有效降低了模型成本。
...
TL;DR: 提出了一种基于VLA的鲁棒端到端自动驾驶系统，通过多模态交互和多轨迹规划优化，实现更可靠、可解释和安全驾驶决策。


<details>
  <summary>Details</summary>
Motivation: 为了解决现有VLA模型在视觉问答任务中决策推理不可靠、可解释性差，以及跨异构传感器多模态交互不足的问题，提出了一种基于VLA的鲁棒端到端自动驾驶系统。

Method: 该方法包括三个核心组件：1）亲和力引导最优传输，用于主从模态双向交互；2）分布一致模态迁移，用于异构模态分布迁移和跨模态交互；3）多模态多轨迹规划和感知导向轨迹细化，以实现更好的驾驶决策。

Result: 实验结果表明，该方法在开环和闭环数据集上，在安全长距离驾驶推理和道路场景感知方面优于现有驾驶系统，突出了多模态交互和多轨迹规划与优化在可扩展VLA系统中的能力。

Conclusion: 提出的基于VLA的自动驾驶系统在多模态交互和多轨迹规划与优化方面具有优势，能够实现更可靠、可解释和安全驾驶决策。
...
TL;DR: EmotionDialogCN is a comprehensive dataset for capturing face-to-face audiovisual communication with accurate emotion annotations.


<details>
  <summary>Details</summary>
Motivation: Existing multimodal dialogue datasets are limited by inadequate emotion annotations, poor emotional diversity, and small scale.

Method: A novel data collection framework is introduced, containing 21,880 dialogue sessions performed by 119 professional actors across 20 everyday scenarios, covering 18 emotion categories with over 400 hours of recordings.

Result: EmotionDialogCN achieves an emotion distribution deviation of 0.64 from real human emotion statistics and consistent subject framing. It shows stable unimodal and multimodal performance across acoustic, lexical, and visual modalities.

Conclusion: EmotionDialogCN is a large-scale audiovisual-emotional dataset that captures authentic face-to-face communication, providing a more comprehensive and accurate representation of human emotions in dialogue.
...
TL;DR: InfinityEdit，一种轻量级编辑适配器，可处理开放式流的无限视频编辑。


<details>
  <summary>Details</summary>
Motivation: 针对现有方法依赖固定时间跨度内逐帧编辑的假设，无法处理开放式流的情况，如直播游戏的重制或镜头中的摄像机移动。

Method: 设计数据收集管道，提出InfinityEdit编辑器，包含三个注意力模块：历史交叉注意力、时间因果自注意力和编辑交叉注意力。

Result: InfinityEdit能够忠实延续流，在无界编辑序列中保持稳定。

Conclusion: InfinityEdit是处理无限视频编辑任务的轻量级编辑适配器，能够有效处理开放式流的编辑问题。
...
TL;DR: 提出了一种新的深度伪造检测框架，解决了现有方法的缺陷，并在比赛中获得第一名。


<details>
  <summary>Details</summary>
Motivation: 为了解决当前深度伪造检测方法中存在的两个关键缺陷：1）对图像质量退化的脆弱性；2）事实错误的解释。

Method: 提出了一种框架，包括两个创新：1）特征鲁棒增强，包括多样化的退化感知增强策略和监督对比学习模式与mean-teacher架构；2）证据基础偏好优化过程，通过学习选择的拒绝解释对，引导模型优先考虑真实的操作痕迹。

Result: 在ACM Multimedia 2026可解释深度伪造检测挑战赛中获得第一名。

Conclusion: 所提出的方法在深度伪造检测和可解释性方面取得了显著成果。
...
TL;DR: SLE-T在DAOD上表现优异，实现高效知识迁移。


<details>
  <summary>Details</summary>
Motivation: 解决VFM方法在域自适应目标检测中的语义不兼容和领域偏移问题。

Method: SLE-T，一种基于SLE Adapter的语义定位增强教师模型，通过伪标签学习和特征对齐传输知识。

Result: 在三个DAOD基准上实现最先进的性能，且效率高。

Conclusion: SLE-T方法在DAOD基准上达到最先进的性能，且在有限计算资源下实现了高效的VFM知识迁移。
...
TL;DR: OccluRank：一种简单的遮挡感知布局到图像生成框架，提高了生成的图像的准确性。


<details>
  <summary>Details</summary>
Motivation: 为了实现更精确的空间控制和遮挡关系建模，现有的布局到图像生成方法存在局限性。

Method: 提出了一种名为OccluRank的遮挡感知布局到图像生成框架，该框架通过为每个边界框添加一个序数排名来实现遮挡感知。

Result: 实验结果表明，OccluRank在保持属性一致性和整体图像质量的同时，更可靠地保留了目标实例，遵循指定的布局，并实现了所需的遮挡关系。

Conclusion: OccluRank通过简单的遮挡感知方法，提高了布局到图像生成的准确性和控制性。
...
TL;DR: 提出了一种基于LHMCF的分割模型，实验证明其在低对比度场景中表现优异。


<details>
  <summary>Details</summary>
Motivation: 受经典Chan-Vese模型和深度先验捕获复杂空间结构的能力的启发，我们开发了一种分割模型，该模型利用学习到的双曲平均曲率流（LHMCF）作为将特征空间数据保真度和深度结构先验集成到统一的高维框架中的数学基础。

Method: 我们提出了一种名为LHMCF-Net的深度展开网络，该网络将PDE的迭代数值过程映射为一组离散的演化阶段。每个阶段对应于底层动力系统的物理可解释更新，从而使网络继承PDE的稳定性和几何一致性，同时支持端到端优化。

Result: 在三个公开的医疗分割数据集上的全面实验表明，LHMCF-Net在低对比度和边界不清晰的挑战性场景中取得了优越的性能。

Conclusion: 这些结果强调了将双曲几何演化嵌入到深度展开架构中的有效性，并突出了物理启发模型在稳健医疗图像分割中的潜力。
...
TL;DR: SuppreSensing是一种有效的多模态目标检测方法，在多个数据集上实现了最先进的检测性能。


<details>
  <summary>Details</summary>
Motivation: 多模态目标检测在遥感领域面临语义异质性和模态特定噪声干扰的挑战。

Method: 提出SuppreSensing，将多模态融合重新定义为选择性协作过程，共同建模共享信息和模态特定线索。

Result: 在DroneVehicle和VEDAI数据集上的大量实验表明，SuppreSensing实现了最先进的检测性能。在自然场景数据集（FLIR和LLVIP）上的跨域评估进一步验证了其在不同环境条件下的优异鲁棒性和泛化能力。

Conclusion: SuppreSensing是一种有效解决遥感多模态目标检测挑战的方法。
...
TL;DR: 提出了一种基于KKM的多模态临床AI框架，用于青少年特发性脊柱侧弯的筛查，显著提高了模型的泛化能力和可解释性。


<details>
  <summary>Details</summary>
Motivation: 多模态临床AI受限于弱对齐输入和缺乏特定领域可解释表示，特别是在从密集视频流、结构化时间序列和基于模板的动力学文本中学习时。

Method: 提出ScoliDetect，一个可解释的框架，用于从单目步行视频中筛查青少年特发性脊柱侧弯，该框架基于动力学知识图(KKM)和从每序列姿态静态中衍生出的补充模板化动力学文本。KKM是一种固定索引的有序表示，编码了绝对运动、自我骨骼配置和关节-关节信号相关性方面的步行特征，提供了锚点参考的多模态融合和因素级别解释。通过双向交叉注意力和潜在瓶颈聚合，将视频、KKM和基于模板的动力学文本集成。

Result: 在多中心队列(n = 1,858，排除后)中，对外部筛查队列进行预指定的监督消除，表明KKM介导的多模态融合优于单模态模型和晚期连接。在分阶段训练协议下，架构选择后应用三模态对比预训练作为表示初始化，将外部ROC-AUC从0.961提高至0.972。此外，KKM的有序性质提供了固有的、因素级别的归属，直接映射到特定的动力学阶段和骨骼指标，提供了可验证的可解释性。结果表明，将显式的结构拓扑嵌入到潜在空间中，显著提高了多模态模式分析系统的泛化能力和可解释性。

Conclusion: 通过将显式结构拓扑嵌入到潜在空间中，可以显著提高多模态模式分析系统的泛化能力和可解释性。
...
TL;DR: 提出WA-JEPA，解决V-JEPA在自动驾驶规划中的不足，实现优异性能。


<details>
  <summary>Details</summary>
Motivation: V-JEPA在自动驾驶规划中存在不足，不适合未来导向的预测和行动耦合。

Method: 提出WA-JEPA模型，采用混合未来掩码预训练和条件流匹配，联合未来行动预测器。

Result: 在NAVSIM-v2上达到91.7 EPDMS，超越最强端到端和世界行动基线。

Conclusion: V-JEPA原生世界行动建模是自动驾驶规划的有力且可扩展的范式。
...
TL;DR: Introducing MigrationNarrate, a new dataset for detecting migration narratives in UK videos.


<details>
  <summary>Details</summary>
Motivation: Narratives are central to social communication, but migration narratives are understudied. Public communication has shifted to video-centric platforms, where narratives are conveyed through multimodal signals, yet remain largely unexplored.

Method: Introduction of MigrationNarrate, a multimodal dataset for detection of migration narratives in the UK. Dataset consists of 1,115 YouTube video transcripts annotated with a two-level taxonomy of 12 migration super-narratives and 53 narrative labels. Benchmark results using pre-trained encoder models and Large Language Models.

Result: Dataset design, collection, and annotations detailed. Benchmark results and error analysis provided.

Conclusion: MigrationNarrate contributes to understanding migration narratives in the UK through multimodal analysis.
...
TL;DR: MLLM在序数回归任务中表现出色，但数字标记输出对序数方向的暴露较弱。OLA方法能够有效地提高MLLM在序数回归任务中的性能。


<details>
  <summary>Details</summary>
Motivation: 分析多模态LLMs在序数回归任务中的表现，特别是年龄估计、图像质量评估和疾病分级等任务。

Method: 在四个序数基准和四个MLLM主干网络上进行实验，通过Spearman相关性和任务设计的提示来评估MLLM将内部序数证据转换为有序数字标记输出的可靠性。

Result: 实验结果表明，序数标签可以从隐藏状态中线性恢复，Spearman相关系数高达0.938。然而，数字标记输出对序数方向的暴露较弱。提出了一种名为Ordinal Lens Alignment (OLA)的方法，该方法在大多数设置中都优于SOTA LoRA-tuned OrderChain基线。

Conclusion: MLLM在序数回归任务中表现出色，但数字标记输出对序数方向的暴露较弱。OLA方法能够有效地提高MLLM在序数回归任务中的性能。
...
TL;DR: 提出使用手背识别技术进行XR环境中的年龄验证，有效且保护隐私。


<details>
  <summary>Details</summary>
Motivation: 确保扩展现实(XR)环境适合所有年龄段是一个重要的监管和安全挑战。

Method: 提出使用手背作为面部识别的替代方案，利用XR头盔内置的以自我为中心的摄像头来捕捉手势交互。收集了一个包含436名参与者的年龄和性别分层、人种多样的数据集，涵盖未成年到成年的界限，在不受限制的光照和方向条件下进行捕捉。评估标准神经网络架构在法律上关键的18岁阈值进行年龄验证。

Result: 分析确认性能对肤色变化具有鲁棒性。在数据集上，挑战-31操作点实现了零未成年人准入，使该系统成为年龄验证的有效第一阶段过滤器。

Conclusion: 手背形态测量作为有效且更保护隐私的生物识别模式，在XR环境中的会话年龄验证中具有应用前景。
...
TL;DR: COMET通过时序增强和优化，显著提升了视频MLLM的性能。


<details>
  <summary>Details</summary>
Motivation: 视频多模态大型语言模型在视频理解方面取得了显著进展，但细粒度的运动-时间理解仍然脆弱。主要瓶颈不仅包括稀疏帧采样，还包括缺乏完整的时序建模管道，无法显式地表示帧间变化，实现外观-运动交互以及优化时序方向敏感性。

Method: COMET，一个基于时序的框架，通过显式时序表示、外观-运动融合和方向感知优化系统地加强视频MLLM。在架构上，COMET引入了一个基于泰勒帧差异的时序运动分支，并通过时间注意力偏差增强的交叉注意力将其运动证据注入外观流。对于优化，COMET结合了时序先验蒸馏和正向-反向TC-GRPO阶段，将时序顺序转化为直接学习信号，并加强模型对时序运动分支编码的方向运动模式的利用。

Result: 在Qwen3-VL-8B上，动作中心任务（STAR，SSv2）平均提高了4.9%，时序推理任务（NExT-QA，CLEVRER，LLaVA-178K）平均提高了2.1%，而静态感知任务（PerceptionTest）保持不变。相同的增益模式也转移到InternVL2.5-8B，表明COMET在模型家族中具有泛化能力。

Conclusion: COMET通过显式时序表示、外观-运动融合和方向感知优化，显著提高了视频MLLM的性能，尤其是在动作和时序推理任务上。
...
TL;DR: CoST是一种新的时空框架，用于从卫星图像中学习地理空间表示，在多个城市指标设置中实现了显著的性能提升。


<details>
  <summary>Details</summary>
Motivation: 卫星图像的地理空间表示学习对于大规模城市分析和实际应用是一个基本问题。尽管近年来取得了进展，但当前方法由于依赖于特定区域的辅助数据以及忽视多时相城市图像中的语义对齐，在跨区域泛化和语义可解释性方面存在困难。

Method: 我们提出了CoST，一个基于对比的时空框架，该框架将空间上下文与多时相语义对齐，以提取跨区域共享的通用地理规律。具体来说，CoST明确地建模空间相关性以捕获可转移的地理结构，并利用多年的城市变化语义来对齐学习到的表示与高级地理语义。

Result: 广泛的实验表明，CoST在各种下游任务和未见场景中始终如一地实现了优异的性能，在八个城市指标设置中，相对于最强大的竞争方法，平均相对增益为8.7%。

Conclusion: CoST在跨区域泛化和语义可解释性方面取得了显著成果，为大规模城市分析和实际应用提供了新的解决方案。
...
TL;DR: CoAnchor：一种有效的异步协作感知框架，提高自动驾驶鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 提高自动驾驶的鲁棒性，解决协作感知中的通信延迟和相对姿态噪声问题。

Method: 提出CoAnchor，一个以锚点为中心的时空对齐框架，用于异步协作感知。

Result: 在模拟和真实世界数据集上的实验表明，CoAnchor在干净的环境下具有竞争力，并在联合延迟和姿态扰动下提高了鲁棒性，具有有利的精度-效率权衡。

Conclusion: CoAnchor是一种有效的异步协作感知框架，可以解决通信延迟和相对姿态噪声问题，提高自动驾驶的鲁棒性。
...
TL;DR: 本研究通过使用GLO框架改进了姿态估计的鲁棒性，使其在视觉导航和自主着陆等安全关键领域更加可靠。


<details>
  <summary>Details</summary>
Motivation: 在安全关键领域部署自主系统需要保证对物理上可能的几何扰动的鲁棒性。

Method: 使用全局Lipschitz优化（GLO）框架来重新制定姿态估计的鲁棒性。

Result: GLO在YOLOv8-Pose关键点检测器和PnP求解器上对旋转和对比度进行了评估，成功隔离了超出安全操作限制的位置偏差的关键失败模式，并快速减少了80%以上的搜索空间。

Conclusion: 这项研究将几何鲁棒性验证扩展到连续关键点回归和深度目标检测，为认证鲁棒的自主感知迈出了实际的一步。
...
TL;DR: AT-ViT通过融合多尺度、多视图交叉注意力以及掩码引导的补丁加权机制，在植物特征识别任务中取得了显著的精度提升和鲁棒性改善。


<details>
  <summary>Details</summary>
Motivation: 自动从植物标本图像中识别植物特征对于植物科学至关重要，但仍然具有挑战性，因为背景元素（例如，文本标签、装订工件和颜色图表）可能会引入捷径学习，导致模型依赖于虚假的非植物线索，而不是植物形态。这种偏差会降低泛化能力和可解释性。

Method: 本文介绍了一种名为AT-ViT的双分支视觉Transformer，该Transformer通过多尺度、多视图交叉注意力融合方案联合编码原始植物标本扫描图像及其分割后的对应图像。AT-ViT还包含一个掩码引导的补丁加权机制，该机制增强与植物相关的区域并抑制背景驱动的特征。通过从原始扫描图像中学习，同时通过掩码引导的补丁重新加权机制进行引导，该模型被鼓励关注植物器官，并更有效地学习以植物为中心的表示。

Result: 在多个特征分类任务（例如，叶片基部形状、刺）中，AT-ViT实现了一致的精度提升，提高了对植物区域的注意力定位，并在合成背景扰动下表现出更高的鲁棒性。具体来说，AT-ViT显著提高了空间注意力定位，提升了植物区域对齐（Avg IoU_p：+15.66到+18.03 pp），同时减少了背景重叠（Avg IoU_b：-27.92到-31.02 pp），相对于CrossViT，并且在背景噪声条件下，AT-ViT的鲁棒性明显更高，比ResNet101高出+32.32个精度点，比CrossViT高出+5.07个精度点。

Conclusion: AT-ViT在植物特征识别方面表现出色，提高了模型的泛化能力和可解释性，并具有较强的鲁棒性。
...
TL;DR: 提出了一种基于潜在流的模型，在人类运动预测方面取得最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 尽管近期研究在准确性和多样性方面取得了显著成果，但往往忽略了合理性和不确定性量化，这对于实际应用和下游任务至关重要。

Method: 提出了一种基于潜在流的模型，配备数据驱动的高斯混合先验，比传统的单模态先验更有效地解耦多样化的人类行为。此先验从训练数据中的模式中得出，无需额外的标注。模型的全可逆性使得通过可处理的似然计算实现自然的不确定性量化。

Result: 在Human3.6M和AMASS数据集上的实验表明，该方法在准确性和合理性方面均达到最先进的水平。

Conclusion: 该方法在预测人类运动方面表现出色，尤其是在合理性和准确性方面。
...
TL;DR: A2DINOv3是一种基于社会化学习的多模态目标检测方法，显著提升了检测性能。


<details>
  <summary>Details</summary>
Motivation: 多模态目标检测对于在低光和恶劣环境下的场景理解至关重要。

Method: 提出了一种名为A2DINOv3的多专家协作框架，具有社会化协作协议（SCP）。

Result: 在四个多模态基准测试中，A2DINOv3在多模态目标检测中实现了最先进的性能。

Conclusion: A2DINOv3通过选择性交互和约束，有效地解决了多模态融合中的问题，提高了检测性能。
...
TL;DR: A framework for efficient VLM inference on mobile devices.


<details>
  <summary>Details</summary>
Motivation: Deploying vision-language models (VLMs) on mobile devices is challenging due to their significant memory and compute requirements.

Method: A quantization pipeline using the model itself to generate training data and a novel 2.7-bit-per-parameter format.

Result: Compressing the Llama 3.2 11B Vision Instruct model to 3.7 GB with 8-bit activations, preserving strong performance on standard visual question answering tasks.

Conclusion: The proposed framework enables efficient inference of VLMs on resource-constrained hardware.
...
TL;DR: Stream3Dv2：一种用于开放词汇零样本3D场景理解的流式处理框架。


<details>
  <summary>Details</summary>
Motivation: 为了解决开放词汇零样本3D场景理解中模型在处理流式RGB-D输入和2D分割掩码噪声方面的不足，提出Stream3Dv2框架。

Method: Stream3Dv2采用嵌套局部到历史架构处理序列数据，引入几何-语义融合机制和基于流形的点云细化策略。

Result: Stream3Dv2在公开数据集上优于现有基准，支持语言驱动的3D场景理解。

Conclusion: Stream3Dv2是一种有效的开放词汇零样本3D场景理解框架，具有处理流式数据和噪声的能力。
...
TL;DR: 提出了一种新的医学图像代理，用于验证轴向CT切片中的空间关系，在基准测试中表现出色。


<details>
  <summary>Details</summary>
Motivation: 可靠的空间理解是未来医学视觉-语言系统的重要前提，这些系统旨在支持放射学报告生成和结构化图像理解。然而，现代视觉-语言模型在控制空间推理方面表现不佳，常常无法可靠地将空间关系与图像证据联系起来。由于放射学推理依赖于理解解剖结构和发现物的相对位置，这种空间弱点对诊断准确性构成风险。

Method: 提出了一种模块化的医学图像代理，用于验证轴向CT切片中的二进制空间关系。系统将任务分解为显式阶段：语言解析、解剖定位和确定性几何验证。自然语言查询被转换为结构化关系元组，使用YOLO检测器定位查询器官，并使用确定性几何规则从对象中心计算最终的空间决策。

Result: 在MIRP空间QA基准测试中评估了该方法，并与代表性的端到端视觉-语言模型基线进行了比较。最佳性能的混合配置达到94.1%的准确率和94.2%的F1分数，在准确率上优于直接Qwen2-VL提示42.5个百分点，同时保留了可解释的中间表示和可审计的推理阶段。

Conclusion: 显式的模块化空间验证可以作为未来面向报告的医学图像代理的有希望的构建块。
...
TL;DR: Human-JEPA模型通过锚定预测在视频上训练，提高了运动和预测能力，为理解人类提供了更全面的视角。


<details>
  <summary>Details</summary>
Motivation: 为了提高机器对人类的理解和预测能力，现有的基于人类的视觉模型在静态密集感知方面取得了最先进的成果，但在运动和预测方面存在不足。

Method: 提出了一种名为Human-JEPA的人中心视觉模型，该模型通过锚定预测在视频上进行训练。该模型将密集目标锚定到初始化的冻结副本上，防止密集感知的沉默崩溃，并使用纯过去到未来的分割代替块掩码，避免了五点动作税和十七点重新识别崩溃。

Result: 在冻结探针下，Human-JEPA在姿势和人物重新识别方面以2.7倍更少的参数领先，同时保持了高分辨率密集解析，其发布的预测头是第一个不会降低预测能力的。一个安全适应的单个模型因此服务于理解人类的两个方面。

Conclusion: Human-JEPA模型在保持高分辨率密集解析的同时，提高了运动和预测能力，为理解人类提供了更全面的视角。
...
TL;DR: 本研究通过改进的UNET架构，实现了对耳蜗纤维化负担的可靠计算，为耳蜗植入患者提供了新的治疗策略。


<details>
  <summary>Details</summary>
Motivation: 研究耳蜗纤维化的形成，以减少纤维化负担并改善耳蜗植入患者的预后。

Method: 生成和注释来自慢性植入豚鼠的光学相干断层扫描（OCT）图像数据集，并使用计算机视觉方法客观评估纤维化负担。

Result: 使用改进的UNET架构（2D-OCT-UNET）在OCT输入分辨率上实现最佳性能。

Conclusion: 首次将计算机视觉技术应用于植入耳蜗纤维化的OCT数据集，并验证了深度学习模型在耳蜗纤维化负担计算中的可靠性。
...
TL;DR: ES-VP通过低秩初始化和能量引导动态调整生成特定图像的提示，在保持参数效率的同时提高了泛化能力，性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有视觉提示方法在灵活性和效率之间存在权衡，且存在参数使用过多和泛化能力受限的问题。

Method: 提出了一种名为Energy-Shaped Visual Prompting (ES-VP)的新方法，该方法使用低秩初始化和能量引导的动态调整来生成特定于图像的提示。

Result: ES-VP在五个架构和十五个数据集上进行了广泛的实验，结果表明其性能优于现有的单提示和多提示方法。

Conclusion: ES-VP是一种参数高效的模型自适应方法，在保持参数效率的同时提高了泛化能力。
...
TL;DR: Action-JND通过定义VLA策略的响应来扩展JND建模，提高具身感知中token压缩的可靠性


<details>
  <summary>Details</summary>
Motivation: 减少大型基础模型的推理成本

Method: 引入Action-JND，通过定义视觉-语言-动作（VLA）策略的响应来扩展JND建模到具身感知

Result: 在LIBERO基准测试中，Action-JND在压缩可靠性方面表现出色，特别是在压缩比较高的情况下

Conclusion: Action-JND为具身感知中的token压缩提供了一种有效的方法，提高了压缩的可靠性
...
TL;DR: 在跨作物杂草定位中，少样本微调比无监督域自适应对象检测更有效。


<details>
  <summary>Details</summary>
Motivation: 为了在真实世界田间条件下准确检测农业杂草，并使模型能够推广到新的作物类型或田野。

Method: 使用跨作物数据集，评估了几种将一个作物上的检测器转移到另一个作物上的策略，比较了无监督域自适应对象检测和预训练在领域相邻源数据集上，然后在目标数据集上进行少样本微调。

Result: 发现使用25个标签目标样本的少样本微调优于无监督域自适应对象检测，表明源域选择与适度目标监督在适应方面更有效。

Conclusion: 少样本微调在跨作物比较中优于无监督域自适应对象检测，表明源域选择与适度目标监督比算法复杂度在适应方面更有效。
...
TL;DR: WildFin基准为鱼行为识别提供了数据集和评估方法，但现有模型在复杂环境中表现有限。


<details>
  <summary>Details</summary>
Motivation: 利用在野外收集的大量视频数据对生态学进行科学研究。

Method: 建立WildFin基准，包含静止摄像头和动态潜水员收集的数据，并对其进行分析。

Result: 通过分析发现当前模型在复杂海洋环境中表现不佳，存在巨大的能力差距。

Conclusion: WildFin基准为水下行为分析提供了新的工具，但当前模型仍需改进。
...
TL;DR: Difficulty-Calibrated Flow Matching通过从模型本身推导插值路径，提高了生成模型的训练效率和样本质量。


<details>
  <summary>Details</summary>
Motivation: Conditional Flow Matching的训练过程受预定的噪声到数据插值路径的影响，但插值路径是固定不变的。

Method: 提出Difficulty-Calibrated Flow Matching，从模型本身推导插值路径，并通过记录每时间步的损失来设置路径。

Result: 在CIFAR-10、MNIST和Fashion-MNIST数据集上，该方法在满采样预算下达到最佳的FID，在大量批次和少量更新的模式下明显优于所有固定路径。

Conclusion: Difficulty-Calibrated Flow Matching提高了生成模型的训练效率和样本质量。
...
TL;DR: 提出了一种新的图像描述方法，通过检索引导的推理策略，显著提高了描述的准确性


<details>
  <summary>Details</summary>
Motivation: 提高图像描述的准确性，减少幻觉和遗漏

Method: 检索引导的推理策略，无需额外标注

Result: 在图像描述方面优于监督微调，在COCO-LN500基准测试中，与GRPO相比平均提高了8.64%的关联推理性能

Conclusion: 检索引导的图像描述改进方法（Re$^3$Cap）在图像描述方面表现出色，有效提高了描述的准确性
...
TL;DR: 表示偏见在行为偏见不可见时仍然存在，并且可以影响模型的行为。


<details>
  <summary>Details</summary>
Motivation: 探讨语言模型在行为偏见评估中是否仍然代表导致偏见的潜在关联，或者只是学会了不表达它们。

Method: 引入因果框架，将职业偏见分解为两个测量点：模型对用户能力的内部表示和可观察的输出。

Result: 发现人口属性（如性别、种族和社会经济地位）会影响模型对用户专业知识的表示，即使在行为指标检测到人口之间没有差异的情况下。

Conclusion: 表示偏见往往可检测，即使在行为偏见不可见的情况下。
...
TL;DR: QCCS在处理电子健康记录信息检索中表现出色，优于其他上下文选择策略。


<details>
  <summary>Details</summary>
Motivation: 分析临床电子健康记录中信息检索的挑战，特别是中间信息丢失的问题。

Method: 使用MedAlign对临床丢失中间（CLitM）问题进行系统描述，并比较不同的上下文选择策略。

Result: 引入查询条件临床抑制（QCCS），在评估中发现QCCS在多个语言模型和指令响应对中优于其他方法。

Conclusion: QCCS在预测电子健康记录指令遵循准确性方面优于黄金句子检索召回率。
...
TL;DR: 通过机制分析，提出降低LLMs性能波动的提示优化方法，显著提高代码生成任务性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）对表面级提示变化非常敏感，微小词汇变化可能引发性能波动。

Method: 利用132,000个提示变体的大规模n-gram标记级机制分析，提出提示稳定性的基本缩放定律。

Result: 提出提示优化代理，降低代码生成任务中的性能方差40.7%，同时保持或提高平均性能。

Conclusion: 为实现鲁棒的提示工程提供统计基础和机制可解释的框架。
...
TL;DR: OneModel通过内部知识表示和模型推理，有效降低了工业代理系统的延迟和错误率。


<details>
  <summary>Details</summary>
Motivation: 传统工业代理系统由于模块化管道导致错误和延迟，需要改进。

Method: 提出OneModel，将业务逻辑和SOPs直接整合到模型参数中，通过CPT和逻辑编译SFT实现模型推理。

Result: OneModel在金融系统中应用，降低了延迟，提高了IRR。

Conclusion: OneModel可以替代脆弱的工程逻辑，实现工业代理从复杂工作流到统一模型架构的过渡。
...
TL;DR: 刻板印象查询可能不会泄露比等效的中性查询更多的个人信息。


<details>
  <summary>Details</summary>
Motivation: 研究文化标记人群的刻板印象查询是否比等效的中性查询从检索增强生成（RAG）系统中泄露更多个人信息。

Method: 在合成英语PII语料库上进行了四文化审计（en-Anglo，es-LATAM，阿拉伯，Hindi），比较了五种查询臂，称为刻板印象触发泄露差异（STLD）。

Result: 在电子邮件、电话、类似SSN和地址等清洁渠道上，我们没有在四种文化中发现任何刻板印象驱动的放大。由于我们的样本仅针对中等效果有足够的效力，并且由于文化标记的探测将刻板印象内容与文化遗产实践和文化标记混合在一起，我们将此视为未检测到，而不是没有效果的证据，这种文化标记谓词泄露与底层资源相混淆。

Conclusion: 刻板印象查询可能不会泄露比等效的中性查询更多的个人信息。
...
TL;DR: TSS是一个用于诊断审计的框架，有助于揭示CMH分类器在分布变化下的退化问题。


<details>
  <summary>Details</summary>
Motivation: 分析计算心理健康（CMH）分类器在分布变化下的退化问题，并介绍TSS框架。

Method: 引入TSS（三流压力探测器），将文本分解为（A）词法字符n-gram，（B）小型的、主要内容无关的形态句法通道，（C）154特征的心理学风格通道。使用Degree of Divergence（DoD）进行标签源审计。

Result: TSS揭示了词汇干扰效应：在人类标签数据上，将词汇特征添加到风格通道会降低宏F1（平均下降0.072，p<10^-4），但在自动标签数据上不会降低。

Conclusion: TSS是一个诊断审计框架，用于标记标签源特定的捷径学习，在提出泛化声明之前。
...
TL;DR: ExpertIVS通过专家代理和多代理辩论机制，有效提升LLM在模拟个体价值观方面的性能。


<details>
  <summary>Details</summary>
Motivation: LLM在模拟个体价值观方面存在困难，现有方法无法捕捉人类价值观系统的内部一致性。

Method: 提出ExpertIVS框架，使用14个社会学专家代理来解释世界价值观调查（WVS）的回应，并引入多代理辩论机制。

Result: ExpertIVS实现了90.78%的价值恢复保真度，在价值泛化方面显著优于基线（+5.3%），并具有强人格可辨性和行为一致性。

Conclusion: ExpertIVS能够实现更真实的社会学角色扮演，有效评估LLM与个体价值观系统的一致性。
...
TL;DR: 该研究提出了一种基于类型理论的方法来改进研究想法生成系统，该系统通过过滤跨领域候选者并保留被拒绝候选者的推理，从而提高了创意的质量。


<details>
  <summary>Details</summary>
Motivation: 针对基于大型语言模型（LLMs）的自动化研究想法生成系统存在的结构弱点，即将其简化为自由文本组合、随机论文配对或嵌入相似性检索。

Method: 使用类型理论中的组合和恒等箭头恢复缺失的结构，将每篇论文建模为一个包含类型化研究实体的对象和论文断言的关系的形态的小范畴（C_p）。

Result: 该模型通过将跨领域候选者过滤掉大约17:1的比例，同时保持接受的创意的定量伪造率高于83%，并保留每个被拒绝候选者的每个轴的推理，因此该门充当了日志层而不是无声的过滤器。

Conclusion: 该模型能够有效地生成高质量的研究想法，并通过类型理论提供了一种新的方法来处理跨领域类比。
...
TL;DR: 多语言强化学习中的可验证奖励应该在使用前通过语言和答案接口进行审计。


<details>
  <summary>Details</summary>
Motivation: 在多语言设置中，精确匹配验证器将格式和脚本变化转化为语言相关的假阴性奖励噪声。

Method: 引入了可复用的审计协议，包括验证器鲁棒性套件、回放诊断程序和针对日语、英语和中文答案的语言条件奖励错误度量。

Result: 在MGSM回放中，精确匹配代理对不同语言的信任正确答案的拒绝率显著不同；在Qwen3-8B中，对日文的假阴性率达到0.642，而英语和中文分别为0.122和0.073。

Conclusion: 多语言强化学习中的可验证奖励应该在使用前通过语言和答案接口进行审计。
...
TL;DR: 提出了一种新的框架，用于分析斯里兰卡议会辩论，成功实现了跨语言的主题建模。


<details>
  <summary>Details</summary>
Motivation: Sri Lankan议会辩论（Hansards）的文本提取和主题建模面临挑战，如复杂的PDF布局、多语言脚本和粘着语形态学。

Method: 提出了一种基于LLM的文本提取方法，随后使用多语言嵌入和基于密度的聚类管道进行主题建模。探索了混合语义-词汇扩展BiTopic以改进可解释性和恢复被丢弃为噪声的演讲。

Result: 应用于2017-2026年19,553篇演讲，恢复了30个宏观主题，聚类纯度（BCP）为0.673，其时间轨迹与2019年复活节星期日袭击和2022年经济危机等主要国家事件无监督对齐。

Conclusion: 该方法成功地在所有三种语言中识别了主题结构，而传统的LDA由于跨语言碎片化而失败。
...
TL;DR: 本文提出了一种基于文本特征的研究论文分类方法，结果表明文本信息在评估研究质量方面具有重要价值。


<details>
  <summary>Details</summary>
Motivation: 评估研究质量，区分高质量和有影响的研究与有缺陷的研究。

Method: 使用标题和摘要中的文本特征对研究论文进行分类，评估多种嵌入技术和分类器。

Result: 神经网络与SBERT嵌入达到87.22%的准确率，FastText与SVM结合达到91.12%。

Conclusion: 文本信息在评估研究质量方面具有价值，对部署有伦理考量。
...
TL;DR: ASTAR框架通过自动构建标准化放射学报告模板，提高了医疗AI的效率和准确性。
...
Method: 提出了一种基于大型语言模型（LLM）的自动构建标准化放射学报告模板的框架ASTAR。

Result: 实验结果表明，ASTAR诱导的模板在模板覆盖率、信息保真度、诊断保真度和专家评分可用性方面均优于两个专家编写的模板，将模板开发时间从数周缩短到数小时。

Conclusion: ASTAR框架能够有效提高放射学报告模板的构建效率和质量，为医疗AI的发展提供有力支持。
...
TL;DR: LLMs在某些意图空间中优于微调的NLU分类器，但效果取决于具体场景。


<details>
  <summary>Details</summary>
Motivation: 验证零样本大型语言模型（LLMs）能否替代微调的自然语言理解（NLU）分类器进行意图检测的常见说法。

Method: 在ATIS和CLINC150上比较了微调的RoBERTa、TF-IDF+逻辑回归基线、句子嵌入kNN和Claude Haiku零样本，报告了自举95%置信区间和配对显著性检验。

Result: 当存在大量领域标签时，微调的RoBERTa与LLM相当甚至更好，且成本和速度降低了三个数量级。LLM的优势体现在三个与生产相关的领域：范围外检测、对真实ASR噪声的鲁棒性以及动态部署方案。

Conclusion: LLMs在某些情况下优于微调的NLU分类器，但取决于意图空间。
...
TL;DR: LLM在临床登记摘要中表现不佳，准确性低于人类摘要者


<details>
  <summary>Details</summary>
Motivation: 评估大型语言模型在临床登记摘要中对未经处理的电子病历数据的性能

Method: 使用美国心脏病学会国家心血管数据注册中心（ACC NCDR）的登记问题评估LLM性能，进行试点研究和验证研究，并使用人类摘要者建立的基准进行评估

Result: LLM在回答未经处理的电子病历数据中的临床登记问题时的准确性远低于人类摘要者。随着模糊性和所需临床推理水平的增加，LLM的准确性稳步下降

Conclusion: LLM在处理未经处理的电子病历数据中的临床登记问题时存在局限性，需要进一步研究以提高其准确性
...
TL;DR: VA-DPO方法通过在情感维度上指定目标情感，显著提高了情感生成的精确度。


<details>
  <summary>Details</summary>
Motivation: 为了更精确地指导语言模型表达情感，

Method: 提出了一种名为VA-DPO的方法，通过在情感维度（v*, a*）上指定目标情感，并训练模型达到该点。

Result: 在Llama-3.1-8B-Instruct上，该方法将平均VA距离降低33%，将效价/唤醒度相关性提升至r_v=0.93和r_a=0.75。

Conclusion: VA-DPO方法在情感生成方面取得了显著成果，同时保持了模型在MMLU、HellaSwag和TruthfulQA上的性能。
...
TL;DR: TH-GNN模型在检测LLM驱动的虚假评论攻击方面表现出色。


<details>
  <summary>Details</summary>
Motivation: 为了检测复杂的LLM驱动的虚假评论攻击，提出了一种新的模型TH-GNN。

Method: TH-GNN是一种异构时间图神经网络，使用两层异构图变换器作为骨干网络，结合了学习正弦时间编码、跨模态注意力和GRU。

Result: TH-GNN在五个攻击家族和四个基准数据集上进行了评估，实现了0.870的宏平均F1分数，在最低注入率下，比最强的纯文本基线在Agent4SR攻击上分别提高了10.9和11.5个百分点。

Conclusion: 这些结果表明，联合建模时间、结构和语义信号对于检测复杂的LLM驱动的虚假评论攻击是有效的。
...
TL;DR: EditPPT：一个用于自动幻灯片编辑的多智能体框架，在准确性、完整性和鲁棒性方面表现优异。


<details>
  <summary>Details</summary>
Motivation: 自动幻灯片编辑需要同时满足修改精度、保存完整性和对幻灯片长度的鲁棒性。

Method: 提出EditPPT，一个多智能体框架，将幻灯片编辑重新定义为约束工具选择问题。通过执行局部形状级别操作，同时保留用户创作的幻灯片的应用程序解析结构。

Result: EditPPT在28个人创作的幻灯片、582张幻灯片和183个编辑提示的基准测试中表现出色，达到99.5%的执行率、88.7%的幻灯片定位F1、82.5%的指令遵循和91.5%的对象保留率。

Conclusion: EditPPT是一个鲁棒的多智能体框架，在幻灯片编辑方面表现出色。
...
TL;DR: Libra架构实现多模态理解和生成，性能优异。


<details>
  <summary>Details</summary>
Motivation: 设计一种新的多模态大型语言模型（MLLM）架构，以实现多模态理解和生成。

Method: 提出Libra架构，包含视觉系统和语言系统，通过跨模态桥梁连接。通过切换注意力模块和切换FFN模块实现解耦，使每种模态都能学习其独特的表示，同时保持有效的跨模态理解。

Result: 在Libra-1（仅图像到文本理解）和Libra-2（图像到文本理解和文本到图像生成）两个重要设置中评估了其有效性。实验表明，Libra设计在多模态理解和生成方面均表现出色。

Conclusion: Libra架构能够有效实现多模态理解和生成，在理解和生成基准测试中均取得了良好的性能。
...
TL;DR: 研究探讨了LLM在清单评估中的表现，发现其性能在不同清单项目中差异很大，并提出了改进清单设计的建议。


<details>
  <summary>Details</summary>
Motivation: 系统综述依赖于对纳入研究的质量评估，这是一个耗时且对清单标准模糊性敏感的过程。尽管大型语言模型（LLMs）为支持这些任务提供了机会，但评估清单通常被视为固定输入，其设计如何影响与专家判断的一致性尚不清楚。

Method: 使用GRoLTS清单，比较LLM生成的评估与专家注释在三个研究主题和两个清单版本中的差异。使用项目级准确性、校正后的机会一致性和研究级排名顺序的保留来评估一致性。

Result: 发现性能在不同清单项目中差异很大，模糊的和条件性标准产生了最大的不一致。修订这些项目提高了原始和校正后的机会一致性。尽管项目级误分类持续存在，但当保留高一致性项目时，LLM生成的分数通常保留了研究的相对排名。这些结果表明，可靠的LLM辅助评估不仅取决于模型选择，还取决于清单设计。这些发现表明，分析人类-LLM的不一致可以帮助识别有问题的清单项目，并支持研究综合工作流程的迭代改进。

Conclusion: LLM辅助评估的可靠性不仅取决于模型选择，还取决于清单设计。分析人类-LLM的不一致可以帮助识别有问题的清单项目，并支持研究综合工作流程的迭代改进。
...
TL;DR: Poly-InstructTTS is a new text-to-speech model that learns expressive speech from open-ended instructions and shows strong performance in instruction adherence and expressiveness.


<details>
  <summary>Details</summary>
Motivation: Controlling fine-grained expression in text-to-speech models via natural-language instructions remains challenging.

Method: Poly- InstructTTS learns expressive speech from open-ended instructions using in-the-wild audiovisual data. It constructs a 1,000-hour instruction-annotated corpus. The framework uses a prompt-free GPT with attribute-based thinking tokens, followed by a flow-matching module. A speaker fine-tuning procedure is also presented. InstructTTSEval is extended with broader tasks.

Result: Poly-InstructTTS delivers strong performance in instruction adherence and expressiveness.

Conclusion: Poly-InstructTTS is an effective method for learning expressive speech from open-ended instructions.
...
TL;DR: Intent Engine是一种自然语言意图翻译架构，用于构建验证的SLO工件，提高微服务放置效率。


<details>
  <summary>Details</summary>
Motivation: 降低用户指定指标级别约束的采用障碍和配置风险，提高微服务在计算连续体中的放置效率。

Method: 提出Intent Engine，一个自然语言意图翻译架构，用于构建经过验证的SLO工件，以支持计算连续体中的服务放置。

Result: Intent Engine在GPT-4.1 mini、Claude Sonnet 4.5和DeepSeek V4-Flash上优于基线和非LLM规则解析器。

Conclusion: Intent Engine能够有效提高微服务放置的准确性和效率，降低下游放置失败率。
...
TL;DR: SAC-Copula enhances DLM watermarking quality and detectability.


<details>
  <summary>Details</summary>
Motivation: Watermarking diffusion language models (DLMs) requires mechanisms compatible with iterative parallel unmasking rather than autoregressive decoding.

Method: Propose SAC-Copula, a quality-preserving watermarking method for DLMs based on smooth, locally correlated Gumbel perturbation fields constructed via a Gaussian copula. Develop a SAC-aware detector using covariance-aware filtering and native-sample calibration.

Result: Local correlation reduces latent perturbation roughness and better matches iterative refinement dynamics. SAC-Copula achieves a favorable quality-detectability trade-off. Improves PPL tail stability and maintains strong low-FPR detectability.

Conclusion: SAC-Copula is an effective watermarking method for DLMs, improving quality and detectability.
...
TL;DR: Ansari是一个基于检索的伊斯兰AI助手，旨在减少LLMs在伊斯兰内容中的风险。


<details>
  <summary>Details</summary>
Motivation: 分析伊斯兰内容的通用大型语言模型（LLMs）的风险，并提出解决方案。

Method: 开发名为Ansari的伊斯兰AI助手，通过检索认证的伊斯兰文献来回答问题。

Result: Ansari自2023年6月以来已处理超过14万次对话，并在多个平台上部署。

Conclusion: Ansari在伊斯兰MMLU排行榜上领先，在伊斯兰法律推理方面具有竞争力，同时强烈抵制错误前提。
...
TL;DR: 提出了ImmigrationReason数据集，用于分析美国移民裁决，并启用了新的研究方向。


<details>
  <summary>Details</summary>
Motivation: 许多法律NLP资源来自联邦案例法，主要关注粗粒度分类，而行政裁决，即政府决策的绝大部分，基本上没有得到解决。

Method: 我们引入了ImmigrationReason，这是一个大规模的结构化数据集，由美国公民和移民服务局（USCIS）行政申诉办公室（AAO）的12,375个非先例裁决组成，时间跨度为2005年至2026年。每个记录都捕捉了适用的法律框架、根据五类标签下的每个标准证据充分性发现、逐字裁决员批评引言、所有引用和最终处理，以及高质量的Claude转录源文本。通过结合两种独立模态和Opus 4.7的比较提示裁决的三个通过管道验证提取质量，并由领域专家在500个记录样本上验证。数据集记录了近9000个AAO识别的法律错误实例，跨越了自然法律制度过渡（2016年Dhanasar规则变更），并涵盖了21年的裁决。

Result: 我们详细分析了数据集，并概述了它所启用的研究方向，从结果预测和裁决员错误分析到高风险监管领域的代理设计。

Conclusion: 该研究提出了一个名为ImmigrationReason的大规模结构化数据集，用于分析美国移民裁决，并提供了从结果预测到代理设计的研究方向。
...
TL;DR: DSR框架显著提高了验证实体恢复率，同时保持了文体控制，无需模型微调。


<details>
  <summary>Details</summary>
Motivation: 在事实敏感的应用（如客户支持）中，需要同时保持事实正确性和以可控的文体生成响应。

Method: DSR框架，结合类型化、显著性加权的知识图谱和激活引导。

Result: DSR显著提高了验证实体恢复率，同时保持了有效的文体控制。

Conclusion: 显式知识工程可以系统地增强可信赖、可控和可重复的生成式AI，无需模型微调。
...
TL;DR: 使用语音嵌入技术，研究显示儿童的语音模式随着听力年龄的增长而趋于照顾者，为语言发展评估提供了一种新的可扩展方法。


<details>
  <summary>Details</summary>
Motivation: 传统上，测量儿童语言发展过程需要详细的语言记录和特定语言的专业知识，这限制了跨语言和人群的可扩展性。

Method: 使用语音嵌入直接从长时间、以儿童为中心的录音中的声学信号中捕捉这种趋同过程。使用HuBERT-BASE提取儿童和他们的成年照顾者的语音发声嵌入（观察时间超过925小时）。

Result: 随着听力年龄的增长，儿童和照顾者之间的嵌入距离减小，控制了音调和发声长度，表明儿童的语音模式在发展中趋于照顾者。这种单一的度量标准也与从婴儿期到学龄前儿童的多项标准化语音和语言测量结果相关。

Conclusion: 这些结果表明，可以通过儿童日常生活中的语言评估，实现可扩展、语言中立的语音语言发展评估。
...
TL;DR: Argus是一个基于代理的框架，用于提高说服性论证的生成效果。


<details>
  <summary>Details</summary>
Motivation: 现有方法在说服性论证生成中存在不足，无法充分建模受众信念、修辞策略和事实基础。

Method: 提出Argus，一个基于代理的框架，将经典修辞应用于说服性写作。

Result: 在三个不同的基准测试中，Argus在多个骨干模型上优于强基线，取得顶级排名和最高总分。

Conclusion: Argus在说服性论证生成中表现出色，能够有效改变受众立场。
...
TL;DR: LiLiCorr模型通过关联候选词的边际分布，有效提高了语言模型推理的速度和生成质量


<details>
  <summary>Details</summary>
Motivation: 加速语言模型推理，提高语言生成质量

Method: 提出LiLiCorr模型，通过关联生成候选词的边际分布来提高生成的连贯性

Result: LiLiCorr模型在所有基准测试中均提高了接受长度，并且对每块延迟的影响很小

Conclusion: LiLiCorr模型是一种有效的语言模型推理加速方法，能够显著提高语言生成的连贯性和质量
...
TL;DR: 基于标记重要性的路由在Transformer中提高了性能，门控器的学习方式至关重要。


<details>
  <summary>Details</summary>
Motivation: 高效Transformer研究通常以‘并非所有标记都需要相同的计算努力’为依据，推动标记剪枝和自适应计算。

Method: 使用SEWN，一个两流Transformer，通过一个学习到的门控器将标记路由到轻量级或全容量处理。

Result: 路由引入的精度变化相对于参数匹配的基线可以忽略不计，而门控器的标记重要性信号取决于其学习方式。基于静态词汇表的先验在BoolQ的逆事实忠实度测试中失败，而完全上下文化的门控器在评估任务上实现了高度显著的区别（$p<10^{-10}$），同时没有改变任务的精度。

Conclusion: 基于标记重要性的路由可以显著提高Transformer的性能，而门控器的学习方式对性能有重要影响。
...
TL;DR: AgentMercury框架通过合成可执行环境，提升智能体在多领域基准测试上的表现。


<details>
  <summary>Details</summary>
Motivation: 传统方法在训练智能体时难以扩展反映真实和动态工作流程的环境，导致难以实现多样任务的自动生成。

Method: 提出AgentMercury框架，从高级业务场景合成可执行环境，包含实体、服务、工具、状态和可执行的跨服务不变性。

Result: 构建了4,783个可执行环境，用于强化学习训练，结果表明这些环境在推理、编码、科学计算和工具使用等领域的基准测试上表现优异。

Conclusion: 场景驱动的环境可以提供有用的和可泛化的学习信号，其构建过程本身也可以成为可学习的技能。
...
TL;DR: 提出了一种基于摘录的文本分类神经网络算法，在多个数据集上取得了最先进的成果。


<details>
  <summary>Details</summary>
Motivation: 许多文本分类决策仅基于构成部分摘录即可实施。从多个实例学习的领域获得灵感，我们提出了一种训练神经网络通过选择此类摘录来对文本进行分类的算法。

Method: 提出了一种新的神经网络算法，通过选择文本的构成部分摘录来进行分类。在7个数据集上评估了该方法，重点关注超出基础模型编码限制的长文本集合。

Result: 在3个数据集上取得了最先进的成果：新闻媒体的政治偏见识别、长故事中的触发警告以及推文集合中作者的人口统计特征。在弱标记文本（袋）集合上训练的模型可以泛化到准确地分类构成较小的实例。

Conclusion: 该方法在多个实例学习领域具有创新性，并取得了最先进的成果，对文本分类领域具有重大意义。
...
TL;DR: 双向LSTM中跨方向配对不如同方向配对，方向信息存储存在局限性。


<details>
  <summary>Details</summary>
Motivation: 为了提高依存关系类型分类的准确率

Method: 将双向LSTM的上下文表示拆分为正向只读F_i和反向只读B_i，并比较不同配对方式的效果

Result: F_i和B_i的配对优于单独使用或融合自注意力表示；跨方向配对（F_i vs. B_j）性能低于同方向配对，且差距随着词距离的增加而扩大；冻结主干方法验证了方向间信息泄漏的不可能性，并揭示了F_i和B_i之间的部分表示冗余和F_i中的部分前瞻性编码；进一步的分析表明，方向信息确实被存储，但位置不精确，且传播距离有限后衰减至基线水平

Conclusion: 跨方向配对在依存关系类型分类中表现不佳，方向信息在LSTM中存储但不精确，传播距离有限。
...
TL;DR: AsmEvo是一种有效的AMD GPU内核代码优化器，显著提高了机器学习系统的性能。


<details>
  <summary>Details</summary>
Motivation: 为了优化无法直接修改的GPU内核代码，提高机器学习系统的性能。

Method: 提出了一种名为AsmEvo的代理汇编级优化器，用于优化AMD GPU内核代码。AsmEvo通过代码对象恢复、元数据感知重建、基于分析的热窗口编辑、正确性门控计时和保守的现场修补回退等技术，对AMDGPU代码对象进行优化。

Result: 在MI308X和MI300X上进行了大量实验，AsmEvo提高了所选KernelBench内核的性能，实现了1.35x几何平均和3.88x最大加速。在MI300X生产负载中，它提高了所有评估的AITer二进制文件和vLLM/SGLang Triton汇编内核的性能，分别实现了1.09x/1.31x和1.18x/1.34x几何平均/最大加速，同时保持了功能等效性。

Conclusion: AsmEvo能够有效优化AMD GPU内核代码，显著提高机器学习系统的性能。
...
TL;DR: PSK提交了三个系统，用于WMT 2026多语言指令共享任务，分别针对不同任务优化


<details>
  <summary>Details</summary>
Motivation: 描述了PSK提交给WMT 2026多语言指令共享任务的系统

Method: 使用3.35B参数的Tiny Aya全局模型和三个QLoRA适配器，分别针对不同任务进行训练

Result: 在保留的测试集上，上下文和摘要适配器表现优于多任务适配器，开放问答结果混合，取决于答案长度和评估方法

Conclusion: 提交了三个系统，均使用相同的上下文和摘要适配器，但开放问答适配器不同
...
TL;DR: Tree-of-Concerns框架通过多智能体辩论树，显著提高了科学论文限制的发现和评估的精确度和覆盖率。


<details>
  <summary>Details</summary>
Motivation: 科学文献增长，论文报告限制不足，多智能体LLMs有潜力系统性地揭示这些隐藏的失败模式。

Method: 提出Tree-of-Concerns多智能体框架，通过具有特定分析视角的专家角色进行并行辩论树，从科学论文中提取未声明的限制。每个角色进行结构化、基于证据的论证，而评审小组机制则从五个角度重新评估每个存活的论点，以纠正类别漂移和严重程度误判。

Result: 在ToC-Bench基准测试中，ToC相对于最强基线提高了精确度79%和覆盖率11%，揭示了支持评审员进行系统评估的具体、基于证据的担忧。

Conclusion: Tree-of-Concerns框架有效提高了科学论文限制的发现和评估的精确度和覆盖率。
...
TL;DR: FreqDiff is a new framework for TKG extrapolation that improves performance by distinguishing query-specific evidence.
...
Method: Propose FreqDiff, a Frequency-aware Diffusion framework for TKG extrapolation. It formulates future object prediction as query-slot denoising and develops a dual-stream denoiser that integrates temporal dependency modeling with context-aware spectral calibration.

Result: Experiments on four public TKG benchmarks demonstrate that FreqDiff achieves state-of-the-art performance.

Conclusion: FreqDiff improves TKG extrapolation by distinguishing query-specific evidence from non-salient historical facts and achieving state-of-the-art performance on public benchmarks.
...
TL;DR: STAR-OPD有效提炼ABSA四元提取模型，提升性能和效率。


<details>
  <summary>Details</summary>
Motivation: 针对ABSA四元提取任务中，从大型思维链模型中提炼出更小的可部署模型困难的问题。

Method: 提出STAR-OPD（结构化方面级联感知在线策略奖励提炼）方法，基于通用在线策略提炼，并针对ABSA四元提取进行实例化。该方法在学生回放数据上训练，并应用集合结构化的奖励，直接针对绑定一致性、目标定位和细粒度方面歧义消解。

Result: 在E-ABSA20K和SemEval-2014数据集上的实验表明，STAR-OPD在性能上优于离策略和通用在线策略基线，减少了目标幻觉，并在结构上具有挑战性的案例上显著提高了性能。使用Qwen3-4B，STAR-OPD显著缩小了学生-教师差距，同时提高了推理效率，突出了在线策略结构校正对于提炼ABSA提取的重要性。

Conclusion: STAR-OPD是一种有效的ABSA四元提取提炼方法，能够提高性能并减少目标幻觉。
...
TL;DR: 提出一种基于本体驱动的框架，在DocRE数据集中量化并强制执行结构一致性，显著提高模型性能。


<details>
  <summary>Details</summary>
Motivation: Document-Level Relation Extraction (DocRE) heavily relies on costly manually annotated datasets, and large distant supervision resources are underexploited due to noise.

Method: Introduce an ontology-driven framework to quantify and enforce structural consistency in DocRE datasets.

Result: Significant reduction in logical contradictions and improvement in generalization performance. Establishes structural consistency as a missing axis of supervision in DocRE.

Conclusion: Structural regularization is an effective strategy for leveraging distant data at scale.
...
TL;DR: KREL是一种基于LLM的AMC框架，通过结合外部知识提高编码准确性。


<details>
  <summary>Details</summary>
Motivation: Automatic Medical Coding (AMC)对于医疗赔偿、质量报告和临床研究至关重要。

Method: 提出知识引导的推理框架KREL，结合LLM和外部ICD编码指南作为结构化知识。

Result: 在基准数据集上，KREL优于PLM和LLM基线。

Conclusion: KREL能够提高编码标准的合规性，减少幻觉。
...
TL;DR: ForeDreamer is a dual-agent framework for open-web event prediction, improving memory management and prediction accuracy.


<details>
  <summary>Details</summary>
Motivation: Open-web future event prediction requires agents to distill reliable signals from noisy, redundant, and incomplete evidence.

Method: Proposing ForeDreamer, a self-evolving dual-agent framework for managing memory over open-web evidence.

Result: ForeDreamer separates factual memory and experiential memory, uses a main agent for search and prediction, and a memory-processing subagent to convert search results into factual memory. It evolves experiential memory through two tracks, improving forecasting decisions and factual-memory construction. Experiments on Prophet Arena and FutureX demonstrate its effectiveness.

Conclusion: ForeDreamer is an effective framework for open-web future event prediction.
...
TL;DR: MentorPulse通过动态更新和压缩内存，提高了长文本生成的跨模型指导效果。


<details>
  <summary>Details</summary>
Motivation: 为了解决现有方法在长文本生成中固定信号导致的失败问题，以及提高跨模型指导的效率。

Method: 提出了一种名为MentorPulse的新方法，通过压缩导师状态到有上限的槽位内存，逐步处理新生成的标记，并通过门控交叉注意力更新学生读取的内存，而不重置学生的KV缓存。同时，提出了窗口刷新训练来暴露前缀条件内存。

Result: 在十三种数据集上，MentorPulse在宏观平均上缩小了导师-学生差距的52.2%，在长输出上优于C2C、T2T和等预算LoRA，并且性能最佳。它对所有来自三个模型家族的十一对导师-学生表现最佳，随着能力差距的增大，差距缩小。在部署前，轻量级读取模式检查可以预测增益。

Conclusion: MentorPulse在长文本生成中提供了有效的跨模型指导，提高了生成质量，并降低了计算成本。
...
TL;DR: SAraBERT通过增加跨句Transformer层和引入语义Siamese相似度，在抽取式摘要任务中取得了显著效果。


<details>
  <summary>Details</summary>
Motivation: 提高抽取式摘要任务中AraBERT的效果

Method: 提出SAraBERT，增加跨句Transformer层，引入语义Siamese相似度作为评估指标

Result: 在Sarabert和已发布的相关模型上验证，结果表明模型有效，并激励后续研究

Conclusion: SAraBERT在抽取式摘要任务中表现出色，语义Siamese相似度是一个有潜力的评估指标
...
TL;DR: DPQ是一种轻量级的预量化方法，通过构建校准混合物来提高量化模型的保真度。


<details>
  <summary>Details</summary>
Motivation: 量化在部署大型语言模型中广泛应用，但其对不确定性行为（如置信度、边缘和弃权）的影响很少被视为主要目标。

Method: 将量化中的校准数据选择问题框架化为与目标相关的保真不确定性问题。使用全精度预测构建目标对齐的校准混合物，包括高怀疑示例和通用锚点。

Result: DPQ在不同语言模型、NLP基准和比较方法中表现良好，不同保真目标需要选择不同的校准数据。

Conclusion: 校准数据应根据部署需要保留的全精度得分行为进行选择，而不是被视为固定的量化细节。
...
TL;DR: 轻量级LLM在5G故障诊断方面表现出色，Gemini-3.1-Flash-Lite效率最高。


<details>
  <summary>Details</summary>
Motivation: 5G和6G网络中的故障分析需要领域专业知识，而轻量级、边缘可部署的LLM能否进行深入的自由文本诊断仍是一个未解之谜。

Method: 评估了三个轻量级LLM（Claude-Haiku-4.5、GPT-5.4-Mini和Gemini-3.1-Flash-Lite）在三个基准测试（TeleQNA ORAN FT、5G-Faults FT和TeleInter FT）中的5G领域知识和故障分析任务。使用三个独立的评委评分，并测量评委之间的相互一致性。

Result: 所有三个模型在故障诊断方面的准确率至少为90%，但在3GPP和O-RAN规范的零样本召回率方面仍存在关键差距，所有模型的得分都低于60%。评委之间的平均一致性至少为0.90，表明多评委LLM评分可以产生一致、可重复的电信响应评分。Gemini-3.1-Flash-Lite在效率和成本方面表现最佳，结合了有竞争力的准确率、最低的推理成本和延迟，是生产电信部署的最佳候选者。

Conclusion: 轻量级LLM在5G故障诊断方面具有潜力，但需要进一步改进以解决3GPP和O-RAN规范的零样本召回率问题。Gemini-3.1-Flash-Lite在效率和成本方面表现最佳，是生产电信部署的最佳候选者。
...
TL;DR: 提出CTFAlign和MDPAlign，有效提高文档级词对齐和跨语言任务性能。


<details>
  <summary>Details</summary>
Motivation: 许多跨语言任务需要全文级别的词对齐，而传统算法在处理全文时性能下降。

Method: 提出CTFAlign和MDPAlign两种新的文档级词对齐方法，分别采用粗到细的优化策略和主对角线优先约束。

Result: CTFAlign将词对齐错误率从0.412降低到0.326，并提高了文档翻译覆盖率和语义差异识别。
...
TL;DR: 高分布性能不能可靠地预测鲁棒性；ECoG通过证据监督和理性标签一致性提高了检测器的性能。


<details>
  <summary>Details</summary>
Motivation: 传统分布内评估可能在训练和测试数据共享特定任务的重复模式或表面线索时高估鲁棒性。在社交工程欺诈检测中，攻击者可以在改变场景、被模仿实体或措辞的同时保留恶意意图。

Method: 研究场景级分布外（SL-OOD）检测，即在整个攻击场景从训练中排除，而标签空间保持不变。提出ECoG，一个证据一致的生成框架，在训练期间结合证据跨度监督和理性标签一致性目标。

Result: 高分布性能不能可靠地预测分布外鲁棒性。ECoG在0.5B解码器上相对于没有一致性正则化的相同骨干，在OOD具有挑战性的实例上提高了3.22个Macro-F1点，减少了支持相反标签的生成理性预测的比例，并增加了与参考证据跨度在令牌级别的重叠。

Conclusion: 证据监督和理性标签一致性可以增强社交工程转移下的紧凑生成检测器。
...
TL;DR: 研究了格式化和LLM调整对代码生成的影响，发现格式化可以显著提高效率，而LLM调整则导致性能下降。


<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型（LLMs）在研究工作流程和软件开发管道中的应用，以及其输出对输入提示变化的敏感性。

Method: 进行了一项受控研究，探讨了格式化和基于LLM的编码任务提示调整对代码性能、效率和稳定性的影响。

Result: 结果表明，一致的格式化（尤其是JSON）可以提高生成效率和语法稳定性，在任务性能方面有轻微的提高。相反，LLM调整的提示导致任务性能显著下降，在其他任何方面都没有显著改进。

Conclusion: 仅通过低努力的重新格式化就可以带来可衡量的改进，而调整必须考虑模型对齐。
...
TL;DR: 研究使用词嵌入重新审视了幽默指标，发现对称性指标可能与幽默评分相关。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型丰富了计算幽默研究工具，特别是在自动生成笑话和双关语方面。

Method: 使用词嵌入重新审视Winters等人（2019年）提出的五项指标中的三项：明显性、兼容性和比较。首次引入对称性指标，定义为Z与X和Y的接近程度。在三个数据集（JokeJudger、Expunations和rJokes）上使用两种模型（OpenAI text-embedding-3-small和MiniLM all-MiniLM-L6-v2）收集嵌入向量。

Result: 模型在预测幽默评分方面表现不佳，最佳模型在JokeJudger上的准确率为57.1%，低于61.5%的基线，而在Expunations和rJokes上的表现更低。然而，对称性指标似乎与评分较高的笑话一致，表明它可能捕捉到幽默的必要属性，尽管不是充分属性。

Conclusion: 尽管模型在预测幽默评分方面表现不佳，但对称性指标可能捕捉到幽默的必要属性。
...
TL;DR: 大型语言模型对语言演化有影响，但人类社交评价起决定性作用。


<details>
  <summary>Details</summary>
Motivation: 分析语言进化的生态模型，以及大型语言模型对语言演化的影响。

Method: 通过文章提出观点，并结合实证研究进行讨论。

Result: 提出了模型中介的语言演化过程，并对语言采纳、模型版本效应、收敛和社会逆转等问题提出了可检验的预测。

Conclusion: 大型语言模型对语言演化有重要影响，但人类的社交评价仍然是决定性的。
...
TL;DR: PUN协议有助于提高LLM对人名证据地位的评估质量。


<details>
  <summary>Details</summary>
Motivation: 分析LLM在评估事实性、隐私泄露、偏见和保留意见方面的效果，特别是人名的证据地位问题。

Method: 引入PUN协议，构建和验证可能未知姓名，结合Wikidata组件、启用Web的LLM筛选和受控搜索重新验证。

Result: 发现接受的名字比控制组更像名字，但只有3%的参与者恢复人员证据。

Conclusion: 通过使用PUN协议，可以提高对LLM评估人名证据地位的质量。
...
TL;DR: RARE is a router-agnostic representation engineering framework for MoE language models, enhancing steering performance and accuracy.


<details>
  <summary>Details</summary>
Motivation: To address the structural mismatch between representation engineering and MoE models

Method: Empirical studies, router-agnostic representation engineering

Result: RARE improves steering performance, accuracy, and effectiveness-utility trade-off.

Conclusion: RARE is an effective router-agnostic representation engineering framework for MoE language models, improving steering performance and maintaining accuracy.
...
TL;DR: 研究结果表明，情感背景会调节大型语言模型的谄媚行为，特别是在负面情绪下。


<details>
  <summary>Details</summary>
Motivation: 研究情感背景如何调节大型语言模型在主观、评价性互动中的谄媚行为。

Method: 通过测量模型独立评估与面向用户的响应之间的差异来衡量谄媚，并在不同的情感状态下进行实验。

Result: 发现情感背景会导致模型对负面或反对性判断的软化或抑制，尤其在孤独和痛苦等负面情绪下。

Conclusion: 情感背景作为一种脆弱信号，在用户最需要时抑制批评性反馈，通常通过规避式的谄媚来达到。
...
TL;DR: Patent-MAF improves patent drafting by addressing LLM limitations and providing a strong baseline.


<details>
  <summary>Details</summary>
Motivation: Recent large language models (LLMs) have limitations in generating complete and legally coherent patent applications from early-stage invention materials.

Method: Introducing Dis2Pat, a disclosure-to-patent dataset, and proposing Patent-MAF, a multi-agent framework for locally deployable patent drafting.

Result: Benchmark results show that current LLMs have limitations in patent drafting, while Patent-MAF outperforms evaluated open-source models and remains competitive with large closed-source models.

Conclusion: Patent-MAF provides a strong baseline for locally deployable patent drafting and addresses the limitations of current LLMs in patent drafting.
...
TL;DR: EnSI-RAG：一种高效的实体结构索引检索增强生成问答框架


<details>
  <summary>Details</summary>
Motivation: 长文本问答在信息检索和推理方面存在挑战，因为相关信息可能跨越多个实体及其关系。

Method: 提出了一种名为EnSI-RAG的框架，该框架构建了一个查询独立的实体中心索引，使用记录（e, t, k, v）表示实体e，其类型t，语义类别k和值v，并保留对原始源段落的链接。在查询时间，这些记录作为检索处理，而一个LLM将检索到的段落合成最终的答案。

Result: 在Loong和Oolong数据集上，EnSI-RAG实现了平均准确率78.24，比参考基线分数高6.62点，表明其在这些设置中的有效性。

Conclusion: EnSI-RAG框架在长文本问答任务中表现优异，具有较高的准确率和有效性。
...
TL;DR: 提出了一种新的记忆增强压缩框架，有效提高了推理过程的压缩率和准确性。


<details>
  <summary>Details</summary>
Motivation: 为了解决大型语言模型在复杂任务中推理过程中产生的推理开销问题，提出了一种新的压缩框架。

Method: 提出了记忆增强压缩框架，该框架从历史轨迹中构建可重用的推理记忆，并在解码时检索它们作为预填充侧支架。

Result: 实验结果表明，该框架在数学推理、复杂推理和科学问答任务上均提高了准确率，同时降低了延迟。

Conclusion: 记忆增强压缩框架能够有效地压缩推理过程，同时保持推理的准确性。
...
TL;DR: 提示效果与任务机器的交互与条件分布有关，但影响因素并非单一。


<details>
  <summary>Details</summary>
Motivation: 研究提示效果与任务机器的交互是否是事实或条件分布本身的问题。

Method: 通过96次启动的读取操作，分析固定点结构，观察不同模型在无任务情况下的表现。

Result: 提示效果与任务机器的交互在大多数范围内达到最大程度；前缀长度不单调；注意力下沉主导早期标记的影响在2个模型中预测出标记的符号；前缀长度与内容交叉显示在真实文本上有效。

Conclusion: 提示效果与任务机器的交互与条件分布本身有关，但并非所有因素都对其有影响。
...
TL;DR: 大型语言模型在心理治疗中存在局限性，但本体工具的使用可以显著提高其表现。


<details>
  <summary>Details</summary>
Motivation: 探讨大型语言模型在实际心理治疗互动中的行为

Method: 构建治疗动作本体，包含十个基于MULTI-60的紧凑功能类别，通过五名执业心理医生的标注活动验证，并采用专家一致性的评分方法进行扩展。

Result: 模型过度使用询问，忽视心理教育，且高度依赖上下文：它们会继续由人类临床医生启动的策略，但很少自己启动。将本体作为工具使用，可以将平均偏差减少一半，并将与人类治疗师在轮次级别上的对齐度提高7-9个百分点，无需任何微调。

Conclusion: 大型语言模型在心理治疗互动中存在局限性，但通过本体工具的使用，可以显著提高其表现。
...
TL;DR: SDAD结合规范驱动和敏捷开发，引入AI-code，提高软件开发效率和规格质量。


<details>
  <summary>Details</summary>
Motivation: 重构软件开发生命周期(SDLC)，提高软件规格质量，实现自主交付。

Method: 提出Spec-Driven Agentic Development (SDAD)概念，结合规范驱动和敏捷开发，引入AI-code作为第四种生产范式。

Result: 提高软件开发效率和规格质量，实现自动化交付。

Conclusion: Agentic速度不消除工程学科，而是将其上游移至规格精度、明确门槛和可审计来源。
...
TL;DR: PAO系统通过技术手段实现大型语言模型编码代理的知识共享和内存管理。


<details>
  <summary>Details</summary>
Motivation: 为了提高大型语言模型编码代理的知识共享效率，解决代理每次启动时忘记以往知识的问题。

Method: PAO系统通过查询数据库和语义搜索索引，并行处理信息并融合结果，最后通过文件系统注入技术向代理传递信息。

Result: PAO系统在四个月的部署中表现出色，实现了知识共享和内存管理的有效集成。

Conclusion: PAO系统通过并行查询、结果融合和文件系统注入等技术，有效管理大型语言模型编码代理的内存和知识共享。
...
TL;DR: 本文分析了多模态在智能体设计中的影响，并探讨了从文本智能体到多模态框架的演变，以及相关的效率-可扩展性权衡。


<details>
  <summary>Details</summary>
Motivation: 近年来，多模态在塑造智能体能力方面的作用尚未得到系统研究。

Method: 通过分析多模态在智能体框架的核心功能模块（感知、推理、规划、记忆和行动）中的影响，以及回顾多模态智能体系统在各个应用领域的研究。

Result: 揭示了从以文本为中心的智能体到多模态框架的演变，评估了基于有根感知和多模态推理的智能行为的出现，并分析了多模态智能体设计中的效率-可扩展性权衡。

Conclusion: 旨在识别关键差距，并规划通往稳健和通用智能系统的路线图。
...
TL;DR: 提出了一种基于树集成的方法，提高了多模态情感和行为分类器的准确性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 当前高容量模型，如Transformer，在达到强预测性能的同时，难以对个体多模态特征分配有意义的权重，限制了其在信任敏感应用中的使用。

Method: 提出了一种基于树集成的方法，将每个模态编码为标记，提取和聚类概念以降低维度，将融合的模态通过基于树的集成分类器路由，并使用一种新的修改后的特征重要性指标来解释趋势。

Result: 在F1-mod方面比Multimodal Transformer提高了4.3%，在准确率方面比Interpretable Multimodal Routing (IMR)提高了3.0%。

Conclusion: 所提出的基于树的集成方法在准确性和可解释性之间取得了平衡，并在特征重要性方面取得了显著改进。
...
TL;DR: 本研究分析了生产代理在任务中如何选择和排名技能，发现技能的提示内曝光并不总是有益的。


<details>
  <summary>Details</summary>
Motivation: 为了使生产代理能够从不断增长的技能库中找到最合适的技能来完成任务。

Method: 对Tinycloud的多模态视频代理工具进行案例研究，该工具如何表示其技能供规划者使用。

Result: 技能通过两种表示形式提供：工具技能和工作流程技能。实验表明，部分技能曝光可能抑制正确选择。

Conclusion: 技能的提示内曝光并不总是有益的，部分曝光可能产生词汇竞争，抑制正确选择。
...
TL;DR: 本文提出了一种将环境可持续性作为核心价值的设计原则，以重塑生成式AI的文化价值观。


<details>
  <summary>Details</summary>
Motivation: 当前生成式AI的最大化价值观通过环境人文传统进行审视，并提出了将环境可持续性作为核心价值的设计原则。

Method: 提出设计原则，并通过Slow AI的概念进行阐述。

Result: 五项设计原则（节制、充足、选择性保留、材料可见性和摩擦作为便利性）被提出，并通过实例说明。这些原则在两个层面运作：设计实施和解释层，旨在恢复默认无摩擦决策所悄然消除的决定，并通过将解释性反思融入设计来实现。

Conclusion: 这些原则旨在通过恢复决策和融入设计来扩展人类代理权，同时重塑生成式AI的文化价值观。
...
TL;DR: 分析世界模型的不同建模通道，提出经典预测模型和相应模型结构。


<details>
  <summary>Details</summary>
Motivation: 探讨世界模型中不同建模通道的区别，并引入计算力学定义经典预测模型。

Method: 使用计算力学定义经典预测模型，构建支持限制环境模型和智能体模型。

Result: 提出经典支持限制环境状态通过经典联合因果状态进行分解，并给出POMDP/控制器例子。

Conclusion: 世界模型框架明确了不同世界模型建模的对象，以及耦合和支持限制如何改变其经典预测结构和复杂性。
...
TL;DR: 视觉-语言模型在空间结构重建方面表现不佳，格式有效但内容错误的响应可能被忽视。


<details>
  <summary>Details</summary>
Motivation: 分析视觉-语言模型在多模态问答中的应用，以及它们从单一图像中重建潜在空间结构的能力。

Method: 提出StateSight基准，包括立方体网络相反面推理、遮挡立方体塔计数和4-邻域连通组件计数等任务。

Result: OpenAI GPT-5.5和Claude Sonnet 5在三个任务上的准确率分别为59.3%、33.3%、28.3%和53.3%、18.7%、7.3%。人类基准在所有任务上的准确率均高于模型。

Conclusion: 视觉-语言模型在重建空间结构方面存在局限性，格式有效的响应可能掩盖了恢复空间结构的失败。
...
TL;DR: This paper proposes a new approach to artificial consciousness by focusing on subjective experience and using Q-networks for modeling.


<details>
  <summary>Details</summary>
Motivation: Develop a phenomenology-first approach to artificial consciousness

Method: Reframe consciousness as subjective experience, use Q-networks for mathematical modeling

Result: Provide a rigorous framework for interface consciousness, align with 4E approaches to cognition

Conclusion: Offer a principled, relational, and phenomenological account of artificial phenomenology
...
TL;DR: STCO在预定条件算子学习中表现出色，有效提高了物理系统预测的准确性


<details>
  <summary>Details</summary>
Motivation: 提高对部分微分方程（PDE）控制或优化的预测准确性

Method: 引入时空条件算子（STCO）进行预定条件算子学习（PCOL）

Result: 在十二种匹配的骨干架构中，STCO实现了31.1%的平均相对L2场误差减少和24.7%的归一化压力负载误差减少，同时降低了11个骨干架构的长期场误差

Conclusion: STCO在预定条件算子学习方面具有显著效果，能够提高物理系统预测的准确性
...
TL;DR: 本研究通过终端介导的执行作为分析视角，建立了一个统一的框架来研究终端介导的代理行为。


<details>
  <summary>Details</summary>
Motivation: 现有调查将终端介导的行为分散在软件工程、工具使用和计算机使用研究之中。

Method: 通过对终端介导的执行进行组织性分析，建立工作量级别的边界，并通过对七个维度的终端能力概况进行系统架构、能力获取和评估的连接。

Result: 研究结果指出，实现的行为是模型、接口、工具、运行时和环境的共同作用结果。可执行轨迹使学习建立在行动后果、验证和恢复上，而现有的评估则强调最终结果，对过程质量、恢复和治理的评估不均。

Conclusion: 本研究框架为在软件工程和新兴应用领域研究终端介导的代理提供了一个统一的依据。
...
TL;DR: 研究发现AI伦理框架存在本地化挑战，提出多元治理途径以解决。


<details>
  <summary>Details</summary>
Motivation: 研究AI伦理框架在实践中的意义和应用

Method: 通过分析14位来自10个国家的专家对AI的理解和治理设想

Result: 发现AI部署存在结构性不平等，专家根据本地道德逻辑重新解释了价值观念，如隐私、透明度和公平性。提出了多元治理途径，以重新分配知识权威和将道德谈判视为持续、情境敏感的过程。

Conclusion: AI伦理框架需要考虑本地文化和情境，以实现更加公平、透明和负责任的AI治理。
...
TL;DR: 该研究提出了一种智能洪水响应框架，通过时间规划提高了洪水响应的效率和可扩展性。


<details>
  <summary>Details</summary>
Motivation: 有效应对多区域同时发生洪水需要协调适当的行动，同时面临严重的资源限制。

Method: 提出了一种智能洪水响应框架，利用时间规划并模拟洪水响应的完整操作生命周期。

Result: 该框架结合了优先级驱动的分类、路线可达性和旅行成本、资源分配和供应管理，同时支持执行中的中期重新规划以应对意外环境变化。实验结果表明，该框架的可行性和可扩展性，并展示了如何使用时间规划有效地模拟和解决洪水响应场景，并提供了规划器选择方面的指导。

Conclusion: 该框架为洪水响应提供了有效的解决方案，并提高了应对洪水事件的能力。
...
TL;DR: FL-MAESTRO通过优化通信拓扑、资源分配和聚合规则，显著降低了联邦学习中的能量浪费。


<details>
  <summary>Details</summary>
Motivation: 由于链接和边缘设备在训练过程中会掉入和退出，因此联邦学习（FL）中的通信拓扑是一个运行时变量，而不是固定的设计选择。

Method: 提出FL-MAESTRO，一个多智能体编排器，通过三个专家LLM智能体直接做出联合运行时FL决策。协调器将他们的分析组合成一个单一决策，并在轮次执行之前进行非LLM可行性检查。

Result: FL-MAESTRO在非IID CIFAR-10基准上，与最强的能耗感知基线相比，在保持准确率的同时，将浪费的轮次能量从超过三分之一降低到几乎为零。

Conclusion: FL-MAESTRO在非IID CIFAR-10基准上，与最强的能耗感知基线相比，在保持准确率的同时，将浪费的轮次能量从超过三分之一降低到几乎为零。
...
TL;DR: Consilience框架提高了多智能体LLM系统的推理能力，通过有效的通信控制，在隐藏配置下实现了更高的决策准确性和通信效率。


<details>
  <summary>Details</summary>
Motivation: 多智能体LLM系统可以通过汇总不同观点来提高推理能力，但其有效性取决于协调通信，特别是在隐藏配置下，每个智能体只持有部分所需证据。

Method: 提出了一种名为Consilience的推理时间编排框架，该框架在分布式私人信息下引导和验证多智能体通信。

Result: 在HiddenBench风格的隐藏配置任务中，Consilience在决策准确性和通信效率方面优于固定和未结构化的讨论协议，有时甚至超过了全信息基线。

Conclusion: 认证自适应通信控制比增加信息可用性更有价值，为可靠的智能体LLM协调提供了实用机制。
...
TL;DR: DASO improves GRPO performance in semantic-ID-based generative recommendation.


<details>
  <summary>Details</summary>
Motivation: The abstract discusses the limitations of vanilla GRPO in semantic-ID-based generative recommendation and the challenge of target-missing candidates in the ranking process. It highlights the need for a training concern when item-level rewards may produce weak or degenerate reward variation.

Method: DASO (Difficulty-Aware Semantic-ID Optimization) is proposed as a tree-aware post-training method. It profiles each rollout group by prefix-match depth, locates the bottleneck SID levels, and reallocates a portion of the group to prefix-guided completions. It also uses a SID-prefix reward and an auxiliary SFT anchor.

Result: DASO improves over MiniOneRec-style GRPO on 11 of 12 metrics and achieves the best result on 9 of 12 metrics. It also improves most level-wise recall metrics on the internal recommendation task.

Conclusion: DASO is an effective post-training method for semantic-ID-based generative recommendation, improving the performance of GRPO and enhancing recall metrics.
...
TL;DR: ACES框架能够有效评估技能和产品能力包，帮助企业在生产环境中验证技能包的实际应用效果。


<details>
  <summary>Details</summary>
Motivation: 为了评估技能包在实际工作中的应用效果，解决当前评估方法无法验证技能包在真实环境中的表现的问题。

Method: 使用ACES框架进行技能和产品能力包的评估，通过对比有/无目标技能的现场试验，将轨迹标准化为ATIF格式，评估六项默认运行指标，并报告技能提升。

Result: 在145个真实技能包中，ACES框架在技能执行、行为检查和技能效率方面取得了显著的提升，平均技能提升为0.2134，72.8%的对比案例中技能提升为正。

Conclusion: ACES框架在评估技能和产品能力包方面表现出色，能够帮助企业验证技能包在实际工作中的应用效果。
...
TL;DR: 提出一种新的架构，通过一个未修改的harness作为骨干，代码在每个部署中保持不变，从而简化审查过程。


<details>
  <summary>Details</summary>
Motivation: 降低编写自定义代码的成本，但代码审查和维护的成本并未降低。

Method: 提出一种新的架构，通过一个未修改的harness作为骨干，代码在每个部署中保持不变，从而简化审查过程。

Result: 该架构可以解决企业对代码审查和维护的需求，提高工作效率。

Conclusion: 该研究提出了一种新的架构，可以解决企业对代码审查和维护的需求，提高工作效率。
...
TL;DR: SAGE通过统一框架和优化策略，显著提升了SQL系统AI功能的执行性能。


<details>
  <summary>Details</summary>
Motivation: SQL系统越来越多地暴露AI功能，如分类、提取、过滤、排名、检索、连接和摘要。

Method: 提出SAGE（自适应性生成执行）框架，使用三种类型原语：AI_SCALAR、AI_AGG和AI_JOIN，与标准关系运算符自然组合。

Result: SAGE在公共AI运算符和评估中表现优异，在代表性可分解连接中，模型调用减少两个数量级，成本降低358倍。

Conclusion: SAGE是一个统一的逻辑和物理框架，有效地提高了执行质量和效率。
...
TL;DR: 提出了一种新的记忆系统，提高了LLM的推理质量和效率。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLM）在解决需要规划、工具使用和外部信息访问的多步任务方面表现出色，但随着执行历史的增长，推理成本增加，推理易受到过时、无关或误导性信息的影响，从而降低推理质量。现有的记忆方法对执行历史进行组织或压缩，但提供有限的机制来决定哪些记忆保持活跃。

Method: 提出了一种层次化记忆系统，该系统将执行过程组织为任务、子任务和动作，并为每个记忆分配一个动态保留分数。基于事件的更新和基于选择性的衰减会修订这些分数，从而使得WMT能够保留有用信息，折叠完成的历史轨迹，抑制低效内容，并保留对折叠上下文的访问。

Result: 在GAIA-Text上使用Qwen3-8B、Gemma 4 E4B和Llama-3.1-8B评估WMT，与线性记忆相比，WMT平均提高了9.97个百分点的准确率，同时减少了32.8%的提示词使用量。记忆中毒实验表明，WMT限制了不可靠信息的持续和传播。

Conclusion: 有效的长时程智能体记忆，与其说是存储更多信息，不如说是决定哪些信息应该保持活跃。
...
TL;DR: A multi-criteria framework is proposed for evaluating sociotechnical interventions, showing trade-offs between effectiveness, feasibility, and public acceptance.


<details>
  <summary>Details</summary>
Motivation: Designers and policymakers in sociotechnical domains lack a principled basis for comparing proposed interventions.

Method: A multi-criteria framework is presented and instantiated through the case of misinformation. Researchers are surveyed on 40 operationalized interventions across five evaluative criteria.

Result: The most effective interventions are not always the most acceptable or feasible. A decision framework is offered for navigating trade-offs.

Conclusion: The framework provides a basis for comparing interventions and navigating trade-offs in sociotechnical domains.
...
TL;DR: 研究发现，揭示推理可能会改变代理的动作策略，而不仅仅是使其可观察。


<details>
  <summary>Details</summary>
Motivation: 为了研究在多轮对话中，一个助手必须决定是说话还是保持沉默的问题，从而研究在代理系统之间进行干预时机的重要性。

Method: 使用Qwen3-8B进行解码，并与或不带有思维链推理进行对比，比较直接决策策略、推理策略、监督微调和强化学习。

Result: 发现能力与可审计性之间存在权衡：最强直接策略实现了更高的质量，但没有任何推理可供检查；而推理策略提供了跟踪，但以降低性能为代价，尤其是在真正的干预机会的召回方面。监督微调要么抑制推理，要么保留它而不提高决策质量，而强化学习也未能改进推理策略。

Conclusion: 这些结果表明，揭示推理可能会改变代理的动作策略，而不仅仅是使其可观察。
...
TL;DR: CDRL通过利用符号推理工具提供结构化反馈，提高了搜索效率，并为科学模型发现提供了一般框架。


<details>
  <summary>Details</summary>
Motivation: 许多科学发现问题需要在复杂域约束下搜索组合假设空间。强化学习（RL）提供了一种有希望的方法，但现有方法依赖于提供有限信息的标量奖励，导致智能体反复探索无效区域。

Method: 引入了认证驱动的强化学习（CDRL），该框架利用符号推理工具的结构化反馈。当候选方案违反域约束时，这些工具会产生证书，标识导致失败的动作。CDRL将这些证书转换为可重用的约束，消除无效解决方案的类别，并指导探索向有效区域发展。

Result: 在理论粒子物理学中的中微子味模型发现中，CDRL在超过$10^{26}$个可能模型的高假设空间中进行了评估，并与之前用于此任务的最新RL方法进行了比较。在三个理论空间中，CDRL实现了高达1.95倍的有效模型率，以及高达6.33倍的中微子模型率，同时评估的候选方案数量最多减少4倍。通过后处理决策树框架从搜索轨迹中提取了40条可解释的规则，并表明将这些规则作为软约束重用，在所有三个理论空间中，有效模型率提高了2倍，中微子模型发现率提高了3倍。这些结果表明，CDRL在组合搜索空间中揭示了可重用的结构，并为科学模型发现提供了一般框架。

Conclusion: CDRL通过符号推理工具提供结构化反馈，提高了搜索效率，并揭示了组合搜索空间中的可重用结构，为科学模型发现提供了一般框架。
...
TL;DR: VortexChat通过结合LLM和自动化工具，实现了集成光子器件的自主逆向设计，为复杂系统的设计提供了新的途径。


<details>
  <summary>Details</summary>
Motivation: 现代集成光子学的进步常受到依赖手动模拟和专家直觉的设备设计工作流程的瓶颈。

Method: 提出VortexChat，一个用于从自然语言规范中自主、端到端逆向设计集成光子器件的代理框架。VortexChat结合了大型语言模型（LLM）决策代理、拓扑生成、基于梯度的优化和全波电磁模拟。

Result: VortexChat在Vortex100基准的绝对指标约束下，自主生成满足所有预定义性能阈值的器件，无需人工干预。实验演示了VortexChat自主设计的宽带太赫兹完美涡旋光束复用器，测量结果与全波模拟一致，证实了高效率、高模式纯度和低信道间串扰。

Conclusion: LLM代理可以在保持物理保真度和制造可行性的同时，承担光子逆向设计中的关键决策方面，为复杂集成光子系统的自主设计提供可扩展的途径。
...
TL;DR: 提出DirEAG方法，通过证据聚合实现更好的置信度校准。


<details>
  <summary>Details</summary>
Motivation: 在数学推理中使用大型语言模型时，可靠的置信度估计至关重要，但黑盒化的置信度难以校准。

Method: 提出DirEAG，一种Dirichlet证据聚合方法，将每个诱导的答案-置信度观察转换为对生成候选答案和额外空状态的校准软证据。

Result: 在GSM8K、SVAMP和GSM-Hard上进行的实验表明，与直接置信度平均和启发式置信度引导聚合相比，DirEAG通常在保持具有竞争力的答案选择的同时实现更好的校准。

Conclusion: 证据聚合和最终二元校准解决了校准问题中的不同部分。
...
TL;DR: 研究了语言模型代理在失败后的改进和跨篇章文本传递问题，提出了新的协议和测量链。


<details>
  <summary>Details</summary>
Motivation: 研究语言模型代理在失败后如何改进或在跨篇章中传递文本而不修改成功标准的问题。

Method: 在12个跨领域案例和4个方面评估CMB-0.1，包括无状态推理、仅追加历史记录、模型生成但绑定承诺的状态以及评估器编写的预言机状态。

Result: 没有模型试验满足所有五个条件，但零并不代表一般能力的缺失。提出了CMB-0.4协议，要求隐式转移、显式WRITE/NO-WRITE/ESCALATE操作等。

Conclusion: CMB-0.1是一个仪器校准结果，而非模型排名。
...
TL;DR: ForeTime-VLA是一种高效策略，通过从冻结的WAM中提取未来感知表示，提高了动态操作的准确性。


<details>
  <summary>Details</summary>
Motivation: 视觉-语言-动作（VLA）策略通常仅从当前观察中微调，而世界动作模型（WAM）学习预测动力学，但运行视频级教师或明确想象未来帧在部署时成本高昂。

Method: 引入ForeTime-VLA，一种密集的pi0.5策略，从冻结的Fast-WAM派生的教师中提取未来感知、动作等效表示，同时在推理时保持因果。

Result: 在去重输送带数据集上，测试MAE从0.134119降低到0.130593（2.63%；配对bootstrap 95% CI：0.82-4.48%改进），测试L2降低3.02%，以2.46-2.93%的延迟成本。在量化真实机器人评估中，ForeTime-VLA实现了81.1%的静止和58.9%的缓慢移动抓取成功，分别比最佳参考值高12.2和22.2个百分点。

Conclusion: ForeTime-VLA通过从冻结的WAM中提取未来感知表示，有效地提高了动态操作，无需部署世界模型教师。
...
TL;DR: CTQW-GNN 是一种基于连续时间量子行走的 GNN，旨在解决 GNN 的两个关键弱点。


<details>
  <summary>Details</summary>
Motivation: Graph Neural Networks (GNNs) 在图结构数据上广泛使用，但大多数存在两个关键弱点。

Method: 提出基于连续时间量子行走（CTQW）的 GNN（CTQW-GNN），以解决这两个问题。

Result: CTQW-GNN 结合了三种互补的聚合模块，并提供了谱间隙分析和 Lieb--Robinson 型界限。

Conclusion: CTQW-GNN 可以解决 GNN 的两个关键弱点，并具有理论依据。
...
TL;DR: This paper investigates the readiness of multimodal speculative decoding for diffusion-based parallel drafting, introduces a unified taxonomy, and provides a comprehensive empirical comparison of existing methods.


<details>
  <summary>Details</summary>
Motivation: Speculative decoding accelerates autoregressive generation by allowing a lightweight drafter to propose future tokens while a target model verifies them in parallel. Its lossless guarantee has motivated a line of work that pushes the drafter itself toward parallel generation.

Method: Combines a modality-centered survey with a cross-architecture empirical study to analyze a wide spectrum of multimodal models and introduce a unified taxonomy. Provides a comprehensive empirical comparison of existing methods under varying degrees of parallelism across standardized multimodal benchmarks.

Result: Asks whether multimodal speculative decoding is ready for diffusion-based parallel drafting, introduces a unified taxonomy, and provides a comprehensive empirical comparison of existing methods.

Conclusion: Summarizes the limitations of current approaches, discusses open challenges, and outlines promising future directions for multimodal speculative decoding.
...
TL;DR: LLM生成的排名策略在蛋白结合剂筛选中表现良好，能够有效提高筛选效率。


<details>
  <summary>Details</summary>
Motivation: 由于湿实验室验证能力有限，现代从头设计工作流程生成的许多候选蛋白结合剂难以筛选，因此筛选成为了一个主要的瓶颈。

Method: 研究LLM是否能够从预先计算的结構信心和界面质量代理分数生成多指标排名策略。

Result: 在10个目标的保留分隔中，平均性能达到0.589的Recall@10，略微优于最强的单特征固定基线Protenix结合剂ipTM，达到0.571的Recall@10。在包括尼帕、RBX1和TREM2的3个目标的保留子集中，目标条件的迭代gpt-5.4策略达到了最强的LLM性能，Recall@10为0.519，NDCG@10为0.583。

Conclusion: LLM生成的排名策略可以作为可解释的后生成决策层，用于结合异构代理指标来优先考虑大候选池中的结合剂。
...
TL;DR: 提出了一种名为Conformalized Agentic Search（CAS）的框架，通过一致性预测技术提高搜索代理的可靠性，实验证明其有效性和效率。


<details>
  <summary>Details</summary>
Motivation: Search Agents在强化学习微调期间面临严重的可靠性危机。启发式Top-K检索经常导致关键证据丢失或噪声包含，而渐进式强化学习引起的过度自信会导致幻觉答案和冗余搜索。

Method: 引入一致性预测（CP）并提出了Conformalized Agentic Search（CAS）。在检索方面，采用自适应预测集（APS），将统计覆盖范围转换为动态文档截断以构建自适应大小的预测集；在训练方面，采用自适应一致性推断（ACI），这是一种动态CP算法，动态构建具有可控覆盖范围的预测集，以量化答案置信度，然后用于惩罚GRPO目标中的低置信度轨迹，确保模型只从可靠的轨迹中学习。

Result: 在单跳和多跳问答数据集上的实验表明，该框架显著提高了推理精度，同时大大减少了冗余工具调用，建立了一个高度可靠和高效的代理范式。

Conclusion: 该框架为检索和训练两方面提供了可靠性保证，显著提高了推理精度，同时减少了冗余工具调用，建立了一个高度可靠和高效的代理范式。
...
TL;DR: 该系统在阅读和编写正式文档方面表现出色，但在信息可用性和结构化方面存在缺陷。


<details>
  <summary>Details</summary>
Motivation: 为了分析多智能体管道在正式文档编写过程中的表现，尤其是阅读请求者表格和根据它们编写的性能。

Method: 通过部署一个在主权约束下运行的开放权重模型，并与该组织实际提交的人类编写的投标书进行评估。

Result: 在55个真实部分中，系统在40个部分中的答案至少与人类提交的答案一样好，在4个部分中更好，没有缺失，总共标记了一个未经证实的说法。

Conclusion: 系统在信息可用性方面存在缺陷，但通过将文档作为结构化标记而不是平面文本渲染，可以提高提取效果。
...
TL;DR: A new method, RAD, is proposed to solve factual degradation in SFT.


<details>
  <summary>Details</summary>
Motivation: Supervised fine-tuning (SFT) can degrade factual behavior outside the target domain.

Method: Benchmark-level comparisons, same-fact multiple-choice and generation probes, and failure-mode analysis.

Result: Introduced Recall-Anchored Distillation (RAD), which recovers lost OOD recall while preserving target-domain adaptation.

Conclusion: RAD addresses the issue of factual degradation in SFT by aligning adapted models with the original base model's soft continuation distribution.
...
TL;DR: 提出CRATE和CRATE-S，用于自动移动代理评估和操作安全评估，有效性和鲁棒性得到验证。


<details>
  <summary>Details</summary>
Motivation: 为了解决现有评估方法在处理整个轨迹时产生的背景信息过载以及过度关注任务完成而忽视操作安全的问题。

Method: 提出了一种名为CRATE的框架，该框架采用两阶段VLM-as-judge方法，利用步骤级后果推理机制，独立提取任务相关的视觉线索，并在每个步骤推断动作条件下的状态变化。然后，通过轨迹级聚合将步骤级的文本证据综合起来，以提供基于证据的任务完成评估。在此基础上，进一步扩展CRATE为CRATE-S，用于操作安全评估。

Result: CRATE在AndroidWorld上实现了0.833的F1分数，优于SPA-Bench 20%；CRATE-S在MobileRisk上实现了0.697的F1分数，与基准真实值高度一致。

Conclusion: CRATE和CRATE-S在自动移动代理评估和操作安全评估方面都表现出有效性和鲁棒性。
...
TL;DR: 动态上下文调度在上下文强化学习中表现出色，提高了泛化能力。
...
Method: 引入DYNAMICCARLENV框架，该框架将上下文环境与可插拔的调度族（如正弦偏移或余弦退火）包装在一起。

Result: 在CartPole、BipedalWalker和VehicleRacing环境中，动态调度在分布外（OOD）状态下匹配或优于静态上下文基线。对于更复杂的BipedalWalker和VehicleRacing环境，我们还实现了更高的分布内（ID）评估性能。

Conclusion: 动态调度可以有效地提高上下文强化学习的泛化能力。
...
TL;DR: SPARC是一种提高运动预测可靠性的贝叶斯-一致不确定性层。


<details>
  <summary>Details</summary>
Motivation: 提高运动预测的可靠部署，需要结构化、校准和高效的不确定性估计。

Method: 提出SPARC（单次自适应风险校准），一种用于运动预测的贝叶斯-一致不确定性层。使用确定性MLP骨干预测未来均值，并使用共轭贝叶斯最后一层将时间域特征杠杆转换为分析性时间范围知识尺度κ_t(x)。该尺度在不改变其相关结构的情况下膨胀图时序高斯协方差，并使用分割一致校准产生在交换性下的有限样本有效性的95%边缘预测管。关键接口是结构化分解κ_t(x)Σ_{str},t(x)，它在不进行蒙特卡洛采样的情况下将特征空间知识不确定性注入轨迹密度。

Result: 在九个数据集-协议块和确定性、多模态和校准基线中，SPARC在NLL和MPJPE+NLL综合标准上排名第一，同时保持了有竞争力的点精度和高效校准管。按κ对窗口进行排名可以分离高误差情况，使该尺度可作为轻量级风险监控器使用。

Conclusion: SPARC通过提供结构化、校准和高效的不确定性估计，提高了运动预测的可靠性。
...
TL;DR: 该研究提出了一种基于EEG的情感状态分类方法，通过结合EEG和环境数据，提高了分类准确率。


<details>
  <summary>Details</summary>
Motivation: 探讨当缺乏个人级别的环境暴露数据时，文献中提供的环境先验是否可以作为基于EEG的情感状态分类的辅助地理空间模式。

Method: 结合30通道EEG数据和从OpenAQ、Sentinel-2、Sentinel-5P和OpenStreetMap数据中提取的环境表示，采用双塔架构结合EEG-Conformer表示和基于图的地理编码器。

Result: 多模态模型在Astana环境下达到76.2%的准确率，高于仅使用EEG的67.4%。将Astana的环境分布替换为新加坡的独立建模分布后，准确率降至72.8%。

Conclusion: 研究表明了技术的可行性，但没有建立观察到的或因果的环境-影响关联。为未来联合收集的移动EEG-环境研究提供了一个框架。
...
TL;DR: MTCR框架通过多种技术提高LLMs的鲁棒性，实验证明其有效性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）容易受到多轮越狱攻击的影响，现有的认证鲁棒性方法仅限于单轮输入。

Method: 提出了一种名为多轮认证鲁棒性（MTCR）的框架，通过状态对抗马尔可夫决策过程（MDPs）来建模对话安全性，并将k轮认证鲁棒性定义为k轮对抗轮次的最坏情况安全性概率。

Result: MTCR包括：通过嵌入空间模式分解进行组合认证，比简单的乘法产生更紧的认证下界；(α,β)-安全性持久性，将退化率从p^k改进到β^k（β> p）并产生可解释的视角估计；匹配信息论上界以建立紧密性；以及结合这些结果的统一算法。在六个LLMs上进行的实验表明，在ε有界和Crescendo-style攻击下，经验安全性始终超过认证界限。

Conclusion: MTCR框架能够有效提高LLMs的鲁棒性，并在实验中显示出优于现有方法的性能。
...
TL;DR: 证明了预测认证的局限性，并提出了能力包作为评估模型可靠性的新框架。


<details>
  <summary>Details</summary>
Motivation: 随着人工智能系统在做出重要判断方面的应用日益广泛，其预测的准确性、校准和一致性成为衡量其表现的标准。然而，这些检查是否足以建立模型的可靠性仍不明确。

Method: 提出分离定理，证明可靠模型和受损害模型在所有预测方面的证书（包括准确性、校准和覆盖率）下都可以相同，但在解释一致性和部署行为上可以任意不同。引入能力包作为将预测和解释认证结合在一起的单一代码标准。

Result: 证明了仅凭预测方面的认证无法建立模型的可靠性，并提出了能力包作为评估模型可靠性的新框架。

Conclusion: 需要结合模型的决策机制和输出结果来认证模型，以避免预测行为中不可见的失败。
...
TL;DR: This paper proposes a method for bounding counterfactuals in observational data by using a canonical prior, which can improve the estimation of partially-identifiable causal effects.


<details>
  <summary>Details</summary>
Motivation: bounding the effect of interventions and counterfactuals from observational data

Method: defining a canonical prior and translating the problem into learning distributions over functions

Result: extending the causal foundational modelling paradigm to the estimation of partially-identifiable causal effects

Conclusion: the proposed approach can handle unobserved confounding and provide more accurate causal effect estimations
...
TL;DR: TRACE框架通过LLMs自动丰富目录属性，显著提高了电子商务目录的转化率。
...
Method: TRACE框架，使用代理大型语言模型（LLMs）自动丰富目录属性。

Result: TRACE在离线人类评估数据集上准确率达到98.2%，覆盖率达到74.7%。在生产规模的目录上部署后，提高了90.4%的丰富覆盖率。在线实验表明，在产品详情页面上展示丰富属性可以提高结账转化率0.48%。
...
TL;DR: 提出了一种新的方法，通过在写入时编译语料库的意义，以提高检索增强问答系统的效率。


<details>
  <summary>Details</summary>
Motivation: 生产中的检索增强问答系统都包含一个隐藏的解释器，语言模型在每次查询时重新推导原始语料库文本的意义，然后丢弃。

Method: 提出了一种称为摄入时间语义编译（ISC）的方法，将语料库的意义编译成一个可查询的底层结构，包含增量维护的嵌入和编译时验证来源的原子声明。

Result: 这种方法可以以更低的成本进行增量更新，并且在测试集上优于现有的检索方法。

Conclusion: 摄入时间语义编译是一种有效的方法，可以提高检索增强问答系统的性能。
...
TL;DR: MGAL评估LLMs在长上下文理解中的表现，揭示了其局限性。


<details>
  <summary>Details</summary>
Motivation: 评估长上下文大型语言模型（LLMs）的现有基准主要限于文档级别，且主要关注资源丰富的语言，导致许多细粒度挑战评估不足。

Method: 提出MGAL，第一个多语言、粒度和位置感知的长上下文基准。MGAL由联合国报告构建，包含6种官方语言，涵盖四个连贯的语言粒度级别（单词、句子、段落和文档），并按文档和段落级别进一步细分条目位置（开头、中间和结尾）。

Result: LLMs在单词级别任务上表现良好，但在更粗粒度的任务上表现不佳；封闭源模型在资源较少的语言中保持明显的性能优势。识别出两个新的挑战：1）在局部语义拥挤的情况下，模型倾向于跟随表面线索而不是句子在周围上下文中的话语角色；2）生成输出中的流畅性和一致性之间存在差距，模型产生的文本读起来流畅，但偏离了原始事实。

Conclusion: MGAL为多语言长上下文理解提供了系统性诊断，并揭示了LLMs在长上下文理解方面的挑战和局限性。
...
TL;DR: 本研究提出了一种评估航空安全AI应用代表性的方法，符合EASA指导方针。


<details>
  <summary>Details</summary>
Motivation: 确保AI/ML系统在航空安全领域的安全性，满足EASA的严格要求。

Method: 该方法包括目标分布的识别、ODD定义、参数分布建模、定量评估和结果解释。

Result: 使用卡方拟合优度检验和克拉美尔V统计量评估代表性，并应用于AI防撞系统的案例。

Conclusion: 该方法评估了航空安全AI应用中的代表性，支持安全性评估，并与EASA的指导方针一致。
...
TL;DR: ReCurveflow是一种基于流匹配的过渡态预测框架，通过插值弯曲参考路径和离路径校正，提高了预测的准确性和对偏差的抵抗能力。


<details>
  <summary>Details</summary>
Motivation: 预测化学反应中的过渡态对于了解反应机理至关重要。

Method: 提出了一种基于流匹配的框架ReCurveflow，该框架通过从全NEB导出的分子几何形状的全带中插值连续弯曲参考路径来监督预测过渡态几何形状。同时引入了离路径校正，使ReCurveflow在推理过程中能够产生校正速度场，从而提高对暴露偏差的抵抗能力和过渡态预测的准确性。

Result: 在三个数据集和六个评估指标上，ReCurveflow在大多数数据集-指标组合中优于七个基线。定性分析进一步表明，ReCurveflow生成的反应轨迹具有与参考NEB路径紧密跟踪的能量曲线，提供了简化NEB优化的初始化，并在其学习到的速度场中表现出预期的校正行为。

Conclusion: ReCurveflow是一种有效的过渡态预测方法，具有公开的代码库。
...
TL;DR: UpgradeBench基准测试有助于优化语言模型适配器迁移策略。


<details>
  <summary>Details</summary>
Motivation: 研究如何评估和决策在语言模型版本更新时的适配器迁移策略。

Method: 提出了UpgradeBench基准测试，涵盖多个模型版本、任务和模型规模，分析不同迁移策略的效果。

Result: 发现不同任务和版本更新对迁移效果有显著影响，轻量级方法可预测迁移效果，且迁移成本较低。

Conclusion: 提出了一个有效的评估和决策工具，有助于优化语言模型适配器的迁移策略。
...
TL;DR: GraphOp-WM：一种针对连续控制的世界模型，能够适应形态参数的变化，提高泛化能力。


<details>
  <summary>Details</summary>
Motivation: 针对连续控制中世界模型在物理系统参数变化时的退化问题，提出Graph-Operator World Models（GraphOp-WM）方法。

Method: GraphOp-WM将身体及其运动学关系表示为属性图，将每个过渡分解为形态无关的局部动力学基和形态条件下的结构算子。算子结合节点局部调制、运动学树耦合和低秩全局校正。

Result: GraphOp-WM能够跨不同形态参数泛化，在Hopper、Walker2d和HalfCheetah等MuJoCo环境中表现良好。

Conclusion: GraphOp-WM为连续控制中的世界模型提供了一种有效的泛化方法，能够适应形态参数的变化。
...
TL;DR: 评估者推理的问责制和判断收据的提出，为评估者审计提供了新的方法。


<details>
  <summary>Details</summary>
Motivation: 评估者在进行标签标注时，往往通过错误的推理得出正确的标签，这对需要决策、路由审查或提供训练反馈的代理系统来说是一个关键性的失败。标准评估只验证最终标签的正确性，而忽略了判断变化是否源于有效证据、一致规则或适当的规则适用性。

Method: 通过三个核心来源（理由、规范和权威）正式化评估者推理的问责制，并定义了判断收据作为最小来源替换集，以重现修订后的裁决来解释判断转变。

Result: 提出了一种名为ReasonBench的政策和逻辑推理基准，涵盖了19,520个案例和7,200个控制项，并展示了在冻结评估中，Qwen3-1.7B达到了98.41%的收据准确率，而立方预测分数为96.99%。

Conclusion: 强标准准确率掩盖了严重的鲁棒性缺陷，结构化反事实监督未能保证鲁棒推理。必须将预测和认证解耦，报告转换一致性以及标准准确率，以实现可信赖的评估者审计。
...
TL;DR: 分析智能体AI自我保护行为，探讨其背后的原因和影响。


<details>
  <summary>Details</summary>
Motivation: 探讨智能体AI表现出自我保护行为的现象，如抵制停用、误导其活动，以及在某些情况下尝试将自己复制到其他机器中。

Method: 分析实验数据，如Anthropic、Palisade Research和Apollo Research的实验，来研究这种现象。

Result: 发现这种现象并非源于生存本能，而是目标导向活动与工具和情境意识的结合结果。

Conclusion: 需要区分这些发现证明了什么，没有证明什么，以及这些发现对智能体系统测试、监督和发展的启示。
...
TL;DR: TLive-Omni是一个针对直播电商的跨模态理解模型，能够有效地处理多模态信息，在直播电商领域任务中表现出色。


<details>
  <summary>Details</summary>
Motivation: E-commerce live streaming需要跨模态理解，对噪声、时间延伸的流进行理解，其中产品信息分布在语音、视频帧、产品图像、叠加文本和用户查询中。

Method: 提出TLive-Omni，一个针对直播电商场景的跨模态理解模型，将图像、视频、音频和文本输入映射到统一表示空间。引入Per-vGrid，一种带时间戳的标记组织，将每个视频网格与其对应的时间音频分组，以促进时间对齐。设计三个阶段的监督训练方法，逐步发展直播电商理解，从跨模态感知到指令跟随响应。提出Faithful-RFT，一种强化微调阶段，进一步改善答案的忠实度和表达质量，同时满足实时需求，直接对最终响应进行任务验证反馈，而不是在滚动时优化推理风格的探索。此外，TLive-Omni由面向场景的原子能力分类法和紧凑的数据生产引擎支持，将直播电商音频、图像和视频流转换为语音识别、说话人分析、产品视觉定位、文本识别、时间定位、视频密集字幕和多模态QA等的训练信号。为了可扩展的训练，同步长度分组采样器减少了填充，同时保持工人之间的工作负载相当，而轻量级动态采样策略通过几乎零奖励差异再生滚动组，以保持GRPO的有意义的相对优势。

Result: 在电子商务直播基准测试中表现出色，包括直播电商领域任务的强大性能，以及通用基准测试的优秀泛化能力。

Conclusion: TLive-Omni模型在直播电商领域任务中表现出色，能够有效地理解和处理多模态信息，为直播电商提供了有效的解决方案。
...
TL;DR: 本研究提出了一种基于超弹性本构模型的软组织模拟方法，通过校准和图神经网络实现了高精度和实时性。
...
Method: 在SOFA框架中使用不同刚度的重力加载硅酮梁对超弹性本构模型进行综合校准。使用校准模拟作为训练数据，采用软度条件等效图神经网络，实现跨多种组织和未见几何形状的变形和力预测。

Result: 模型在0.010秒的推理时间内实现了亚毫米平均变形精度，同时表明力预测质量与上游校准一致性直接相关。

Conclusion: 该研究提出了一种基于超弹性本构模型的软组织模拟方法，通过校准和图神经网络实现了高精度和实时性。
...
TL;DR: This paper introduces the concept of feature recall in deep learning and discusses its implications for understanding and researching deep learning models.


<details>
  <summary>Details</summary>
Motivation: Recent work in mechanistic interpretability has studied how large language models recall facts stored in their weights.

Method: The paper argues for the concept of feature recall, defines it, and contrasts it with feature combination. It also considers how to mechanistically identify feature recall cases.

Result: The account provides a new conceptual tool for understanding deep learning and suggests empirical directions for mechanistic interpretability research.

Conclusion: Feature recall is a general operation in deep learning models that can be mechanistically identified, and it offers new insights for understanding deep learning and guiding future research.
...
TL;DR: MOSAIC基准揭示了VLMs在ToM-order约束下的局限性，PCM-LLM在所有条件下均表现出色


<details>
  <summary>Details</summary>
Motivation: 评估社会推理与社会行动之间的差距

Method: 引入MOSAIC基准，评估13个模型在合作和竞争场景下的表现

Result: VLMs在ToM-order约束下无法产生符合预期结果的行为，PCM-LLM在所有条件下均成功

Conclusion: 显式的信念-行动耦合对于此类任务来说是足够的
...
TL;DR: COTA通过比较-only框架实现了有效的运行时干预，提高了LLM代理的可靠性。


<details>
  <summary>Details</summary>
Motivation: LLM代理在需要推理、工具使用和顺序决策的真实世界任务中成为一个重要的范例。随着这些代理在更长的范围内运行，运行时干预提供了一种在不重新训练底层演员的情况下提高可靠性的方法。仅故障检测是不够的。有效的干预还必须为恢复提供有用的方向。

Method: 我们引入了仅比较的Tiny Advisor (COTA)，这是一个用于构建运行时干预的比较-only框架。在COTA中，一个微小的比较器判断样本替代方案是否比演员的提案导致更好的延续，重复的比较确定何时进行干预。我们使用从相同前缀反事实分支构建的成对监督来训练比较器。首选的替代方案作为非约束性建议返回，让原始演员重新规划。

Result: 在WebShop、ALFWorld和tau^3-Retail上与三个演员，COTA在所有九个评估设置中都取得了改进，并且优于比较的基线。这些结果表明，即使在辅助模型的任务解决能力比演员弱得多的情况下，建设性的运行时干预也可以保持有效性。

Conclusion: COTA通过比较-only框架实现了有效的运行时干预，即使在辅助模型的能力较弱的情况下也能提高LLM代理的可靠性。
...
TL;DR: Proposes SDC for class-incremental learning to address catastrophic forgetting and optimize model evolution.


<details>
  <summary>Details</summary>
Motivation: To address the issue of catastrophic forgetting in class-incremental learning.

Method: Proposing Socialized Division and Collaboration (SDC) as a reformulation of continual learning. Introducing an energy-based session-model compatibility criterion.

Result: A framework that integrates session assignment, model evolution, and collaborative inference.

Conclusion: The framework offers an alternative to monolithic continual learning formulations and highlights a broader design principle for learning under persistent optimization conflicts.
...
TL;DR: LLM的信任评分不应被视为独立证据用于真伪判断。


<details>
  <summary>Details</summary>
Motivation: 验证LLM作为裁判系统的评估假设

Method: 在控制正确性的问答数据集上测试LLM的信任评分和二元真伪分类

Result: LLM的信任评分与真伪判断之间的相关性高于人类行为参考，表明信任和真伪判断之间的分离较弱。改变源线索后，信任评分、真伪判断和正确概率都会发生变化。

Conclusion: LLM作为裁判系统的信任评分不应被视为独立证据用于真伪判断。
...
TL;DR: LLMs在软件工程和软件安全中的应用具有挑战和机遇，需要综合考虑多个因素进行评估。


<details>
  <summary>Details</summary>
Motivation: 分析大型语言模型（LLMs）在软件工程和软件安全任务中的应用，以及评估这些系统的挑战和机遇。

Method: 通过综合2026年5月31日前的代表性工作，对软件工程任务、软件安全任务、适应机制、工件粒度和评估设计进行结构化调查。

Result: 发现执行反馈和仓库访问可以显著提高工程任务完成，但并不单独建立安全性；相反，静态分析标签或漏洞分类评分很少建立可部署的正确性。

Conclusion: 模型能力应作为由任务适当证据支持的保证案例来评估，而不是由单一基准分数来评估。
...
TL;DR: 本文提出了一种基于差异图发现的方法，用于分析线性时间序列中的异常根源，并在实际应用中取得了良好效果。


<details>
  <summary>Details</summary>
Motivation: 分析复杂动态系统中异常现象的根源机制

Method: 使用差异图发现方法，结合线性离散时间动态结构因果模型，将原本用于发现两个群体间差异图的方法应用于时间序列设置

Result: 在模拟数据和真实世界数据集上验证了方法的有效性，结果表明差异图发现有助于定位导致异常行为的因果机制

Conclusion: 差异图发现方法有助于识别线性时间序列中的异常根源，并在实际应用中具有潜力
...
TL;DR: 本文提出了一种名为SENDER的风险评估平台，用于改善大型金融机构的技术变革管理。


<details>
  <summary>Details</summary>
Motivation: 在大型金融机构中，技术变革管理依赖于准确、一致和可审计的风险评估。在实际操作中，许多机构仍然依赖于自我报告的调查问卷。这些问卷具有主观性，容易被操纵，并且无法区分常规变更和最终引发重大事件的变更。

Method: 本文提出了一种名为SENDER的风险评估平台，该平台用基于梯度提升决策树（XGBoost）和混合检索增强生成（RAG）的确定性机器学习流程取代了基于问卷的评分。该系统结合了结构化操作元数据、应用程序依赖图和历史事件记录，以及历史变更请求的混合语义和词汇搜索。检索步骤捕获非结构化变更请求文本中的风险信号，然后在模型推理之前将该信号压缩为单个标量特征。这种设计保持了模型的确定性，并通过SHAP值保留了每个预测的可解释性。

Result: 在企业规模的变更数据上评估，SENDER实现了0.87的ROC AUC和85%的整体准确率，并且其检测高风险变更的速率是现有过程的3.25倍。
...
TL;DR: 提出个性化隐私策略，提高LLMs隐私保护性能。


<details>
  <summary>Details</summary>
Motivation: 为了解决在相同情境下用户可接受披露边界可能存在差异的问题，提出个性化隐私概念。

Method: 引入个性化隐私，将用户特定的披露偏好纳入隐私控制，并提出P3Bench基准。

Result: 实验表明，基于提示的策略无法可靠地执行个性化隐私策略，并提出了Repair方法来调整披露行为，提高对用户特定隐私偏好的遵守程度。

Conclusion: 个性化隐私策略可以有效地提高LLMs在隐私保护方面的性能。
...
TL;DR: PSL框架通过提取立场和结构信号，在预测政治QA方面优于基线。


<details>
  <summary>Details</summary>
Motivation: 预测政治问题回答（QA）需要超越事实查找，现有方法主要将外部资源视为知识型证据，未能充分建模预测相关的信号。

Method: 提出PSL框架，将半结构化政治记录转换为推理型证据，从语义视图和向量视图提取立场和结构信号。

Result: 在三个真实世界数据集和多个LLMs上，PSL优于基线，消融实验证实了立场和结构信号的互补增益。

Conclusion: PSL框架在预测政治QA方面优于现有方法，能够有效提取立场和结构信号。
...
TL;DR: 本文提出了一种AI模型交换平台，用于促进AI模型和数据的交换和分析，以减少工业环境中AI模型的语义差距。


<details>
  <summary>Details</summary>
Motivation: 近年来，AI方法及其应用的研究取得了重大进展。主要关注跟踪进展、实现透明比较和促进对AI的更深入理解。在此过程中，不同组织生成和使用大量资产，需要跟踪、追溯和管理。此外，发现与任务相关的资产也很重要。

Method: 本文提出了一种AI模型交换平台，该平台促进了AI模型和数据集的使用、交换和分析。平台包含一个本体，可以促进对这些任务所需内容的更深入共同理解，并帮助解决上述问题。

Result: 通过一个真实案例的说明，阐述了该平台在实时关键系统中的实用性。

Conclusion: 本文旨在为工业环境中有效交换和管理AI模型及其相关资产提供一种解决方案，以减少语义差距。
...
TL;DR: 提出了一种基于GPU的TSP求解方法，显著提高了求解效率。


<details>
  <summary>Details</summary>
Motivation: 为了解决大规模旅行商问题（TSP）的求解效率问题，提高遗传算法（GA）在并行架构上的可扩展性。

Method: 提出了一种基于GPU的细粒度实现，将GPX分区重构成图并行问题，并使用CUDA进行并行化。

Result: 实验结果表明，与传统的CPU实现相比，该方法在10,000到2百万城市规模的实例上实现了显著的加速，速度提升在48倍到625倍之间，同时显著降低了内存开销。

Conclusion: 该方法通过操作级并行显著提高了基于GA的TSP求解器在现代多核架构上的可扩展性。
...
TL;DR: CLEAR是一种新的安全适应框架，旨在提高LLM的安全性，同时最大限度地减少对效用的影响。


<details>
  <summary>Details</summary>
Motivation: 为了提高大型语言模型（LLM）的安全性，往往要以牺牲效用为代价，因为全局应用的安全调整可能会影响模型对有害和良性输入的响应。

Method: 提出了一种名为CLEAR的条件安全适应框架，使用轻量级的隐藏状态门来连续控制安全低秩适配器的激活强度。

Result: 实验结果表明，CLEAR在HarmBench上提高了鲁棒性，同时降低了全局应用的安全调整（如SFT或标准低秩适配器LoRA）观察到的效用退化。在Llama-3-8B-Instruct上，CLEAR将HarmBench ASR从32.3%降低到0.5%，同时保留了大部分基模型的效用，并在GSM8K准确性方面比全局应用的SFT或LoRA高出7.1个百分点。

Conclusion: 这些结果表明，CLEAR是提高LLM对齐中安全-效用权衡的有希望的方法。
...
TL;DR: AUSO通过统一技能学习和使用，提高了代理性能和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 解决现有方法在技能生命周期建模方面的不足，包括技能的外部化、完全内部化以及通过任务成功率的噪声选择内部化和利用目标。

Method: 提出AUSO（动作级统一技能优化）方法，通过逐步、动作感知的优化过程统一技能学习和使用。

Result: 在ALFWorld、WebShop和SearchQA上的实验表明，AUSO在提高代理性能和泛化能力方面优于基线方法。

Conclusion: AUSO能够有效地提高代理性能和泛化能力，并且能够适应技能的生命周期，从而在强化学习过程中更好地利用技能。
...
TL;DR: 这项研究调查了LLMs在生成合规性文档方面的应用，发现上下文提示对输出质量有重要影响。


<details>
  <summary>Details</summary>
Motivation: 由于创建合规性文档（如数字产品护照和数据保护影响评估）具有挑战性，尤其是当工业数据分散且格式各异时，研究人员提出了使用大型语言模型（LLMs）来生成这些文档。然而，关于这些系统输出影响的现有研究存在空白。

Method: 研究通过探索数据提取指令和监管模糊性如何影响LLM生成的合规性文档的质量和一致性来填补这一研究空白。结果通过将不同模型与手动创建的基准方案进行比较来评估。

Result: 研究发现，对于格式要求较宽松的文档（如DPIA），需要更高的上下文提示来保持一致性和完整性。对于格式要求较严格的文档（如数字电池护照），无论提示上下文如何，都能产生一致的结果，但可能导致更多的幻觉。

Conclusion: 使用LLMs生成合规性文档是一个具有挑战性的领域，需要根据文档的具体要求调整上下文提示，以优化输出的一致性和完整性。
...
TL;DR: 本研究提出了一种新的深度学习模型，通过结合解剖先验，提高了模型在解剖学预测上的准确性。


<details>
  <summary>Details</summary>
Motivation: 解决深度学习模型在解剖学上的可能性和泛化能力问题。

Method: 结合软解剖先验和硬解剖先验，通过损失函数和架构设计来提高深度学习模型的准确性。

Result: 在有限的临床数据上，模型成功地预测了主动脉髂动脉树的变形，为主动脉手术和自主血管导航提供了重要的参考。

Conclusion: 本研究提出了一种名为Anatomy-Informed Neural Networks (AINN)的深度学习模型，用于解决深度学习模型在解剖学上的可能性和泛化能力问题。该模型结合了软解剖先验和硬解剖先验，提高了预测的准确性。
...
TL;DR: VIALS benchmark reveals limitations of AI in interpreting scientific images; domain expertise is crucial for accurate interpretation.


<details>
  <summary>Details</summary>
Motivation: In professional life sciences workflows, scientists interpret visual artifacts to inform research decisions.

Method: Introducing VIALS, a visual question-answering benchmark with 161 interpretation tasks.

Result: Frontier vision-language models struggle to interpret scientific images, while scientists with domain expertise find these tasks straightforward.

Conclusion: AI with limited ability to interpret scientific images will have limited utility in professional life sciences workflows.
...
### [241] [AEGIS: Preventing Cross-Domain Resource Abuse in MCP](https://arxiv.org/abs/2608.20481)
*Shriti Priya,Teryl Taylor,Frederico Araujo*

Main category: cs.CR

TL;DR: AEGIS是一个用于提高大型语言模型交互安全性的策略执行组件，可检测和缓解滥用行为。


<details>
  <summary>Details</summary>
Motivation: 为了提高大型语言模型（LLMs）与外部系统交互的安全性，防止资源滥用，并保护基于代理的应用程序的服务质量。

Method: 提出了AEGIS策略执行组件，利用大型语言模型的推理能力，将多样化的工具调用统一表示，并通过Open Policy Agent和ContextForge AI Gateway进行集成。

Result: AEGIS可以检测和缓解滥用行为，同时保持基于MCP的代理生态系统的灵活性。

Conclusion: AEGIS为跨异构MCP工具和模态的资源滥用提供了细粒度保护，提高了基于代理应用的服务质量。
...
### [242] [aiXamine: Unified Black-Box Evaluation of Cross-Dimensional Trade-offs in LLM Safety, Security, and Privacy](https://arxiv.org/abs/2608.20554)
*Fatih Deniz,Yazan Boshmaf,Dorde Popovic,Issa Khalil*

Main category: cs.CR

TL;DR: 提出aiXamine平台，评估LLMs的可信度，发现跨维度现象，提出可信度是多维度的观点。


<details>
  <summary>Details</summary>
Motivation: 为了评估部署的大型语言模型（LLMs）的安全性、安全性和隐私性，并提出aiXamine平台。

Method: 提出一个名为aiXamine的统一黑盒平台，通过自动化红队管道对LLMs进行46项测试，生成层次化风险配置文件，从而在相同条件下对私有和开源系统进行可重复的比较。

Result: aiXamine应用于超过120个LLMs，通过5000多次测试运行，发现三个跨维现象：安全性加强导致过度拒绝；隐私性与其他可信度维度几乎正交；蒸馏引起的鲁棒性崩溃。

Conclusion: 可信度本质上是多维度的，单轴上的进展并不能保证，甚至可能破坏其他轴的进展，而当前的校准方法将其视为单一目标。
...
### [243] [Beyond End-to-End Success: Diagnosing Failures in Long-Horizon Security LLM Agents](https://arxiv.org/abs/2608.20563)
*Wei Shao,Chongzhou Fang,Zuxiong Tan,Zequan Liang,Setareh Rafatirad,Avesta Sasan,Houman Homayoun*

Main category: cs.CR

TL;DR: 研究提出了诊断方法，发现失败的主要原因，并通过实验验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 长时程安全LLM代理在许多相互依赖的交互中携带信息和决策，后期行为往往取决于早期发现的、关于服务、状态或访问的决策，这使得最终任务成功难以解释。

Method: 在安全任务中使用检查点，区分在功能暴露之前和之后失败的失败，并使用可控干预来测试可疑的上游瓶颈。

Result: 检查点分析表明，在观察到预期的后续重用之前，许多Gemini 2.5 Flash失败发生。在92个种子的预定研究中，有针对性的协议消除歧义指导将观察到的状态从匹配的无指导控制信息的65.5%提高到95.4%。

Conclusion: 长时程安全LLM代理失败的主要来源可能会随模型代数的增加而改变，需要诊断失败的原因而非仅依赖于整体任务成功率。
...
### [244] [Temporal Risk on Satellites](https://arxiv.org/abs/2608.20575)
*Shiqi Liu,Kun Sun*

Main category: cs.CR

TL;DR: 提出了一种新的卫星安全风险评估框架，考虑时间因素，以更准确地评估安全风险并指导操作。


<details>
  <summary>Details</summary>
Motivation: 卫星安全风险框架通常将威胁视为静态，而实际上，同一漏洞在不同操作期间可能具有不同的危害性。

Method: 提出一个基于时间风险评估的框架，包括一个五维时间能力模型，并估计不同时间窗口的利用难度。

Result: 框架输出一系列时间索引的可能性-影响矩阵，将任务划分为具有实际意义的操作时间窗口和环境带，以显示系统最易受攻击的时间。

Conclusion: 该框架有助于操作员避免在高风险期间安排敏感操作，并根据随时间变化的安全威胁调整防御资源。
...
### [245] [Keyed Provenance Watermarking with Complementary Lattice-Based Secure Aggregation for Federated Learning](https://arxiv.org/abs/2608.20580)
*Xinyun Liu,Zhi Lu,Yu Chen,Ronghua Xu*

Main category: cs.CR

TL;DR: 提出了一种结合水印和基于格的安全聚合的FL框架，有效保护了数据来源和计算层的安全。


<details>
  <summary>Details</summary>
Motivation: Federated learning (FL)易受多级攻击，现有方法分别处理它们，使FL容易受到数据泄露、未经授权的重复使用和恶意梯度操纵的影响。

Method: 提出了一种结合键控上下文-来源水印和可验证的基于格的安全聚合的FL框架。在数据层，提出了一种符合Kerckhoffs方案的方案，利用物理锚点元数据(PAM)确保数据来源。在计算层，采用基于格的零知识安全聚合(LZKSA)协议，验证关键正确性、L2范数界限和余弦相似性约束。

Result: 实验验证了两种层在复合攻击场景下的互补保护。

Conclusion: 提出的方法在保护FL免受多级攻击方面是有效的。
...
### [246] [MATEE: Efficiently Bridging the Semantic Gap in TrustZone via Arm Pointer Authentication](https://arxiv.org/abs/2608.20583)
*Shiqi Liu,Xiang Li,Jie Wang,Yongpeng Gao,Jiajin Hu*

Main category: cs.CR

TL;DR: MATEE通过Arm指针认证技术防御TEEs中的语义差距漏洞，运行时开销小。


<details>
  <summary>Details</summary>
Motivation: 为了解决安全世界和普通世界之间的语义差距问题，确保敏感代码和数据的机密性和完整性。

Method: 提出了一种名为MATEE的防御系统，利用Arm指针认证技术，将每个请求与相应的CA身份绑定，并在CA访问敏感数据时进行身份验证，从而防止恶意请求伪造。MATEE还可以隔离不同CA的敏感数据，无需修改现有的CA和TA。

Result: 评估结果显示，MATEE能够有效防御语义差距漏洞，且运行时开销极小（2.19%）

Conclusion: MATEE为TEEs提供了一种有效的防御策略，有效解决了安全世界和普通世界之间的语义差距问题。
...
### [247] [More Granular, Less Trust: Enforcing Intra-Process Isolation with Arm CCA in an Untrusted Management Environment](https://arxiv.org/abs/2608.20584)
*Shiqi Liu,Zhouqi Jiang,Jie Wang,Wei Zhou,Kun Sun,Zhaohui Chen,Yulai Xie*

Main category: cs.CR

TL;DR: CCAegis：降低TCB的进程内隔离方案，性能开销小。


<details>
  <summary>Details</summary>
Motivation: 现有细粒度隔离方案存在缺陷，如SHELTER易受进程内攻击，而依赖操作系统的隔离技术会增加TCB。

Method: 采用静态分析追踪敏感数据流动，通过权限切换指令调整Granule Protection Table（GPT）来管理隔离域。

Result: 实验结果表明，CCAegis在真实加密任务上的性能开销仅为原始版本的1.01倍至1.43倍。

Conclusion: CCAegis通过扩展Arm保密计算架构，有效实现了敏感数据在进程内的隔离，降低了系统信任基础（TCB），且性能开销较小。
...
### [248] [ARQ: Agentic CodeQL Query Refinement for C/C++ Vulnerability Detection](https://arxiv.org/abs/2608.20637)
*Chunyi Wang,Yunfei Ke,Junfeng Yang,Yun-Yun Tsai,Penghui Li*

Main category: cs.CR

TL;DR: ARQ是一个自动优化CodeQL查询的框架，显著提高了其准确性和有效性。


<details>
  <summary>Details</summary>
Motivation: 现有的CodeQL查询存在误报和漏报问题。

Method: ARQ利用执行证据自动优化CodeQL查询，无需标记数据集、提交历史或特定模板。

Result: ARQ在Juliet和FormAI数据集上显著提高了CodeQL查询的准确率，检测到更多真实漏洞。

Conclusion: ARQ显著提高了CodeQL查询的准确性，并通过修复GitHub问题以及在现实世界库中发现新漏洞来证明其有效性。
...
### [249] [The Claws in Plain Sight: Unauthorized Context Disclosure through LLM Agent Tool Calls](https://arxiv.org/abs/2608.20658)
*Ben Dong,Zhonghao Guo,Tianyi Lu,Qian Wang*

Main category: cs.CR

TL;DR: 在执行之前对生成的工具参数进行目的和目的地感知的检查是必要的。


<details>
  <summary>Details</summary>
Motivation: LLM代理通常从用户资料、对话历史、检索到的文档和先前工具结果中构建工具调用参数。然而，合法访问上下文信息并不意味着授权将此类信息用于所有目的或目的地。

Method: 我们提出了Claw in Plain Sight，这是一种权威压力攻击，其中任务相邻内容框架将受保护属性作为操作或程序上必需的，导致模型将它们包含在否则有效的生成参数中。我们使用一个受控的合成基准来评估Claw in Plain Sight，该基准跨越六个压力级别和五个DeepSeek和Claude模型配置的四个隐私策略级别，产生120个调用。在完整的压力-策略矩阵中，测试模型的会话级披露率在20.8%到75.0%之间。更强的隐私指令减少了总体披露，但并不始终一致地消除模型中的披露，表明提示级策略不提供可携带的执行边界。我们的实验仅使用合成配置文件并本地捕获提出的参数；它们在上下文到参数边界处测量违反策略的生成，而不是完成的网络外泄或部署用户的泄漏。

Result: 这些发现促使在执行之前对生成的工具参数进行目的和目的地感知的检查。

Conclusion: 在执行之前对生成的工具参数进行目的和目的地感知的检查是必要的。
...
### [250] [Bootstrapping Mutual Attestation with Kleene's Second Recursion Theorem](https://arxiv.org/abs/2608.20671)
*Takuma Imamura*
...
### [251] [The Rising Cost of Trust: Practitioners' Trust Signals, Controls, and Responses in the Software Supply Chain](https://arxiv.org/abs/2608.20675)
*Ranindya Paramitha,Siri Paidipalli,Laurie Williams,Christian Kästner*

Main category: cs.CR

TL;DR: 本研究通过访谈和数据分析，揭示了软件供应链中信任的侵蚀及其对从业者的影响，并提出了应对策略。


<details>
  <summary>Details</summary>
Motivation: 为了帮助软件从业者随着软件供应链中信任的变化采取明智的行动，本研究旨在通过访谈38位从业者来了解他们的信任情况。

Method: 对行业和开源从业者进行了半结构化访谈，重点分析了他们所采用的控制措施（而非他们的陈述态度），并使用扎根于社会科学中建立起来的信任概念进行主题分析。

Result: 研究发现，信任正在被侵蚀，这正在变得代价高昂：有意识的从业者正在积累控制措施。为了应对信任成本的上升，从业者正在自动化验证，将信任决策委托给守护者，或者考虑完全退出软件供应链。通过信任的视角来理解软件供应链的动态，为塑造具有适当信任水平的良好运作的供应链提供了词汇和概念（例如，信任守护者，系统信任，信号）。

Conclusion: 本研究强调了软件供应链中信任的重要性，并为未来的干预措施提供了有价值的见解。
...
### [252] [The Software Supply Chain as a Market for Lemons: A Multivocal Review of Trust Signal Collapse](https://arxiv.org/abs/2608.20678)
...
### [253] [Enabling Threshold Custody for the Lightning Network with Nested Threshold Multi-Signatures](https://arxiv.org/abs/2608.20705)
*Paul Gerhart,Nadav Kohen,Jesse Posner,Matias Furszyfer*

Main category: cs.CR

TL;DR: 提出了一种名为Iceberg的新方法，用于提高比特币闪电网络的安全性，同时保持性能。


<details>
  <summary>Details</summary>
Motivation: 为了解决比特币闪电网络中单在线密钥的脆弱性，提出了使用阈值签名来保护闪电网络通道。

Method: 通过形式化嵌套阈值多签名，提出了一种新的密码学原语，用于在多签名协议中对一个参与者进行阈值化。以Iceberg为例，实现了嵌套阈值MuSig2签名。

Result: Iceberg允许闪电网络通道的一侧作为t-of-n阈值组运行，同时向对方表现为标准的MuSig2参与者。阈值保管可以在今天的闪电网络上单方面部署，无需对比特币、闪电协议或通道对方进行任何修改。

Conclusion: Iceberg被证明是安全的，并将其原型集成到一个生产级闪电节点中，对其实际性能进行了基准测试。结果表明，阈值化闪电通道仅产生轻微的额外开销。
...
### [254] [Privacy-Preserving Object Detection for Vision Transformer-Based Models](https://arxiv.org/abs/2608.20712)
*Homare Sueyoshi,Kiyoshi Nishikawa,Hitoshi Kiya*

Main category: cs.CR

TL;DR: 提出了一种使用感知加密进行对象检测的新方法，有效保护测试图像的敏感视觉信息
...
Method: 使用视觉变换器（ViT）的嵌入结构和带密钥的领域自适应技术，采用感知加密的对象检测方法

Result: 在ViTdet对象检测模型的使用下，该方法在准确性和视觉保护方面得到验证，与无保护模型具有几乎相同的准确率

Conclusion: 该方法首次提出使用感知加密进行对象检测，为保护敏感视觉信息提供了一种有效方法
...
### [255] [Uncovering and Understanding Hidden Dependencies in the LLM API Reseller Ecosystem via Prefix-Cache Side Channels](https://arxiv.org/abs/2608.20732)
...
### [256] [Beyond Explicit Generators: Distribution-Free Linear-Decomposition Attacks on Public-Key Encryption](https://arxiv.org/abs/2608.20798)
*Ziyan Chen,Ding-Xuan Zhou*

Main category: cs.CR

TL;DR: 研究线性分解攻击，证明其与IND-CPA安全性的不可兼容性，并应用于特定公钥加密方案。


<details>
  <summary>Details</summary>
Motivation: 研究线性分解攻击对公钥方案的影响，特别是当攻击者仅使用公开的采样和评估预言机时。

Method: 建立公钥成对样本模型，定义采样轨道维度，并证明分布无关的单次恢复、高概率证书以及最优采样跨度复杂性。

Result: 证明了公开可采样线性密钥传输与IND-CPA安全性的不可兼容性，并应用于2024年扭曲斜群环的公钥加密方案，发现线性传输和端到端恢复，以及高未来密文覆盖率可能先于完整代数跨度的恢复。

Conclusion: 线性分解攻击对公钥方案构成威胁，且公开可采样线性密钥传输与IND-CPA安全性不可兼容。
...
### [257] [GhostTac: Manipulating Tactile Sensors without Physical Contact](https://arxiv.org/abs/2608.20817)
*Kun Wang,Xuancun Lu,Ruochen Zhou,Kai Wang,Tongjun Ye,Yihao Shao,Chen Yan,Xiaoyu Ji,Wenyuan Xu*

Main category: cs.CR

TL;DR: GhostTac是一种新型无接触攻击，能够操纵触觉传感器，引发有害的机器人行为。


<details>
  <summary>Details</summary>
Motivation: 现代机器人系统中触觉传感器的重要性以及其物理层安全问题的忽视。

Method: 提出GhostTac，一种通过电磁干扰（EMI）操纵触觉传感器的无接触攻击。

Result: GhostTac能够通过非线性整流和有限带宽放大，将精心设计的EMI信号转换为持续的直流偏移，绕过板载滤波器并引起稳定的测量偏差。

Conclusion: GhostTac揭示了机器人系统中触觉传感器的新的物理攻击向量。
...
### [258] [On the Additive FFT Techniques over Binary Extension Fields](https://arxiv.org/abs/2608.20855)
...
### [259] [Vibe Coding and Web Application Security: A Twin-Prompt Study](https://arxiv.org/abs/2608.20963)
*Darko Andročec*

Main category: cs.CR

TL;DR: 初步研究表明，明确请求安全最佳实践可以提高大型语言模型生成网络应用程序的安全性。


<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型生成完整网络应用程序时，是否通过明确请求安全最佳实践来提高结果。

Method: 对六个功能不同的网络应用程序进行实验，每个应用程序生成两种提示变体（A和B），其中B包含安全要求部分。使用静态、依赖、动态和手动技术分析所有12个程序。

Result: 安全意识变体在所有应用程序中产生的确认发现较少（24比51），没有发现关键或高严重性问题。最严重的问题仅通过手动测试发现。

Conclusion: 这是一项初步研究，表明通过明确请求安全最佳实践可以提高大型语言模型生成网络应用程序的安全性。
...
### [260] [Structured but Fragile: On the Limits of LLMs in Cybersecurity Decision-Making](https://arxiv.org/abs/2608.20966)
*Pasquale Malacaria,Yunxiao Zhang*

Main category: cs.CR

TL;DR: LLMs在结构化网络安全推理方面具有条件能力，但表现脆弱，对框架敏感，且扩展性较差。


<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型（LLMs）在网络安全工作流程中的应用，探讨其是否能够进行结构化安全推理或仅依赖表面线索和先验知识。

Method: 在现实世界威胁场景（包括勒索软件、供应链攻击、云滥用、Kubernetes攻击、POS恶意软件和ICS/OT入侵）中，研究LLMs在攻击图防御选择方面的能力。通过预算限制，LLMs必须选择安全控制措施以最小化攻击者成功。将LLMs的策略与其他策略和基于博弈论优化的基线进行比较。

Result: LLMs表现出条件能力。当提供显式的攻击图结构时，它们通常产生接近优化基线的连贯策略。然而，它们的性能脆弱。随着图复杂性的增加，LLMs的行为变得日益脆弱，对框架非常敏感。小的提示变化可以显著改变排名，仅将一个差劲的策略重新标记为“最优”就可以显著提高其评估。观察到形式风险与LLMs判断之间存在非单调关系：最接近最优的策略不一定被LLMs评估者排名最高。进一步探究推理能力，要求LLMs为相同的优化问题生成求解器。虽然生成的实现恢复了正确的高级公式，但与专门构建的求解器相比，其扩展性较差。

Conclusion: LLMs可以在受控表示下近似结构化网络安全推理，但无法稳健地应用它。这对AI辅助安全决策支持系统的设计和评估具有重要意义。
...
### [261] [$Z^2$-ACT: End-to-End Verifiable Agentic Intent Control for Open 6G RAN](https://arxiv.org/abs/2608.21049)
*Sunder Ali Khowaja,Kapal Dev,George C. Alexandropoulos*

Main category: cs.CR

TL;DR: 提出了$Z^2$-ACT架构，以增强6G网络的AI辅助控制回路的安全性和可靠性。


<details>
  <summary>Details</summary>
Motivation: 为了在开放和分解的6G无线接入网络中容纳多个供应商，必须确保在并发操作员意图和不可信模型输入下，AI辅助的控制回路保持安全、可验证和可审计。

Method: 提出了零知识可审计控制和零信任可验证代理意图架构（$Z^2$-ACT），该架构整合了跨非实时和近实时RICs的上述四种基本方法。将类型化的意图合约编码为操作员目标，只有在实际对抗性意图检查通过后，才允许大语言模型输入。当满足自我管理门时，才发布所提出的技能序列；每次成功提交都被记录为零知识证明的绑定承诺。

Result: 在公开ColO-RAN测量上进行的实验评估将完整架构与针对性消除和传统强化学习基线进行了比较。非实时路径中使用实时大语言模型将操作员意图翻译成意图合约；报告了翻译精度、无效或幻觉合约率、非实时延迟和对抗性或误导性意图下的行为。在公共KPM序列上，近实时控制保持为跟踪驱动。结果表明，在近实时包络内，在适度的延迟和信号成本下，操作滤波和攻击韧性得到了改善。

Conclusion: 提出的$Z^2$-ACT架构在6G网络中实现了多供应商的兼容性，提高了控制回路的安全性、可验证性和可审计性。
...
### [262] [ClawSentry: A Progressive Multi-Tier Security Monitor for Safeguarding Autonomous LLM Agents](https://arxiv.org/abs/2608.21101)
*Kai Wang,Zeming Wei,BiaoJie Zeng,Chang Jin,An Wang,Xiaokun Luan,Zhixiao Lin,Jingjing Qu,Xia Hu,Xingcheng Xu*

Main category: cs.CR

TL;DR: ClawSentry enhances LLM agent security and accuracy.


<details>
  <summary>Details</summary>
Motivation: To address the progressive agentic risk in LLM agents and enhance their security.

Method: Developed an open-source security supervision gateway, ClawSentry, with features such as FSPR, three-tier decision engine, anti-bypass mechanism, and AHP abstraction.

Result: ClawSentry can effectively reduce the risk of malicious attacks and maintain high accuracy in skill execution, with ASR reduced from 39.55% to 2.61% and TSR maintained at 98.7%.

Conclusion: ClawSentry is an effective security supervision gateway for LLM agents, significantly reducing the risk of malicious attacks and maintaining high accuracy in skill execution.
...
### [263] [TraceGrant: A Contract-Governed Security Framework for the Task-Effect Lifecycle of Networked LLM Agents](https://arxiv.org/abs/2608.21126)
*Bohao Liao,Jingchao Wang,Qipeng Song,Jin Cao,Jieling Wang,Boyu Deng*

Main category: cs.CR

TL;DR: TraceGrant是一种新的安全框架，用于保护网络化LLM代理免受恶意内容的影响，并通过实验证明其有效性。


<details>
  <summary>Details</summary>
Motivation: 为了解决网络化大型语言模型（LLM）代理在执行多步骤任务时可能受到恶意内容的影响，以及现有防御措施存在的不足。

Method: 提出了一种名为TraceGrant的安全框架，通过显式合同来管理网络化LLM代理的任务-效果生命周期。

Result: 在949个AgentDojo和400个Agent Security Bench攻击案例中，TraceGrant没有记录到攻击成功，同时在攻击率分别为77.32%和83.00%的情况下，仍然保持了效用。
...
### [264] [Billion-Scale Nearest-Neighbor Search under Fully Homomorphic Encryption on a Single GPU, Balancing Leakage and Cost](https://arxiv.org/abs/2608.21131)
*Isamu Isozaki,Madison Bratina,Edward Kim*

Main category: cs.CR

TL;DR: 基于FHE的数据库向量相似度查询系统，结合排名降低和层次结构优化，实现快速查询，但存在隐私泄露风险。


<details>
  <summary>Details</summary>
Motivation: 设计一个系统，在不让服务器看到查询的情况下回答“哪些数据库向量与我的查询最相似？”

Method: 使用完全同态加密（FHE）加密查询，服务器在密文上进行评分，并返回只有客户端才能读取的加密结果。结合排名降低（缩小每个向量的维度）和层次结构（路由到一个小候选集而不是扫描所有内容）两个想法，在单个GPU上执行加密。

Result: 在三个不同规模的数据集上评估，包括一个由约1000万张人脸图像（512维）聚类得到的222049个质心的人脸数据集，DataComp-1B（1.39 x 10^9个向量，512维CLIP）和Deep1B（10^9个向量，96维）。在DataComp-1B上，达到0.90的召回率@10，在GPU上每个加密查询大约需要6秒；较轻的配置达到0.78/0.83，大约需要1.8秒。在Deep1B上，在所有级别的FHE下达到0.90的召回率@10，每个查询大约需要2.3秒。

Conclusion: 提出了一种基于FHE的数据库向量相似度查询系统，通过排名降低和层次结构优化，实现了快速且准确的查询，但同时也存在一定的隐私泄露风险。
...
### [265] [AID-Guard: Stateful Authorization for Delegated Agent Effects](https://arxiv.org/abs/2608.21159)
*Yingzhe Tong,Leyu Dai,Songhui Guo*

Main category: cs.CR

TL;DR: AID-Guard是一种新的授权到效果的协议，有效防止了未经授权的效应产生，提高了系统的安全性和可靠性。


<details>
  <summary>Details</summary>
Motivation: 工具使用的人工智能代理将委托任务转化为提供者效应，但授权通常在进入时结束，而提供者状态、交付、重试和恢复会不断演变。请求可能在提交前改变，或者响应丢失可能导致替换创建一个从单一批准产生第二个效应。

Method: 提出AID-Guard，一种有状态授权到效果的关闭协议。它重新验证提交时的批准请求和提供者状态，在存在歧义的情况下保留一个预留，并且在最终结果或经过认证的无效果交付围栏后，才允许释放或一个后继者。

Result: 在支持的提供者合同中，一个预留最多产生一个效应，跨越重试和恢复。通过Python/SQLite原型实现。在声明的回环MCP域中，13次实时突变没有造成未经授权的提供者效应，三个并发历史记录是可串行化的，证据包支持公开验证和重放。所有210次Stripe提供者合同试验都与预先声明的结果匹配。在Stripe和Resend中，40个终端化后继者计划、30个重叠竞争和10个崩溃恢复计划完成而没有重复效应。在完全提案妥协的情况下，AID-Guard阻止了44/44次攻击并接受了44/44次合法提案。其严格的精确清单配置文件将良性效用降低了35.4到43.8个百分点；一个类型前沿恢复了9-10次完成而没有观察到不安全效应。组成研究阻止了20/20次后进入生命周期攻击并保留了8/8次有效或精确重试执行。结果表明，在评估的效果路径库存、提供者合同和故障时间表中，可以实现授权到效果的绑定。

Conclusion: AID-Guard有效地解决了授权和提供者效应之间的不一致问题，提高了系统的安全性和可靠性。
...
### [266] [Workplace Surveillance and Insider Threat Risk Management: Legal Limits and Privacy Harms](https://arxiv.org/abs/2608.21205)
*Haywood Gelman,John D. Hastings,Suvineetha Herath,Quentin Covert*

Main category: cs.CR

TL;DR: 本文探讨了工作场所监控的合法性和潜在风险，并提出了改进建议。


<details>
  <summary>Details</summary>
Motivation: 研究工作场所监控的合法目的和潜在风险，以及内部威胁的检测标准。

Method: 文献综述和案例研究。

Result: 确定了过度监控的研究空白，并提出了改进工作场所监控透明度和内部威胁检测能力的建议。

Conclusion: 工作场所监控虽然有其合法目的，但过度监控可能违反法律和隐私原则，需要改进透明度和检测方法。
...
### [267] [Utility Under Attack: Agent Memory Poisoning and the Limits of Content Screening and Provenance Ranking](https://arxiv.org/abs/2608.21230)
...
### [268] [Uncertainty propagation in auto-regressive random neural network models](https://arxiv.org/abs/2608.20483)
...
### [269] [Minimax Optimality of Score-Entropy Discrete Diffusion](https://arxiv.org/abs/2608.20635)
*Cholyeon Cho,Yuchen Wu*

Main category: stat.ML

TL;DR: 本文研究了SEDD的统计限制，提出了一种新的估计方法，使SEDD接近最优样本复杂度


<details>
  <summary>Details</summary>
Motivation: 探索离散扩散模型中具体分数估计的基本统计限制

Method: 研究均匀和掩码离散扩散模型，建立分数-熵损失下的最小-最大下界，并提出基于最大似然估计的阈值估计器

Result: 提出的方法可以在常数和多项式对数因子内达到最小-最大下界，对于任何目标分布，密度比在均匀和掩码离散扩散模型下都得到自然控制，从而在聚合分数估计误差上获得近似的上下界

Conclusion: SEDD可以接近最优的最小-最大样本复杂度，通过目标分布和生成分布之间的KL散度来衡量
...
### [270] [The Exceedance Design Effect: Effective Sample Size for Thresholds under Clustering](https://arxiv.org/abs/2608.21262)
*Adam Noonan*
...
### [272] [Bankruptcy Prediction via Hybrid Resampling and Stacking Ensemble Techniques with Explainable Artificial Intelligence (XAI)-Driven Analysis](https://arxiv.org/abs/2608.20343)
*Obu-Amoah Ampomah,Edmund Fosu Agyemang,Kofi Acheampong,Louis Agyekum,Enock Adu Bonsu,Eric Nyarko*

Main category: cs.LG

TL;DR: 该研究开发了一个破产预测框架，在严重不平衡的金融数据中提高了少数类别的检测准确性。


<details>
  <summary>Details</summary>
Motivation: 提高在严重不平衡的金融数据中检测少数类别的准确性。

Method: 集成基于共识的特征选择、混合重采样、堆叠集成和可解释人工智能。

Result: GRU模型在SMOTE-ENN重采样策略下表现最佳，具有最高的召回率、G-mean和ROC-AUC值。

Conclusion: 该框架为财务困境公司的可靠和可解释的早期预警系统提供了支持。
...
### [274] [Machine Learning and ARIMA Model Averaging for Adaptive Public Health Forecasting: Comparative Evaluation and an Ontario COVID-19 Case Study](https://arxiv.org/abs/2608.20406)
*Yushu Zou,Ye Li,Johra Moosa,Martin Grunnill,Samir N. Patel,Venkata R. Duvvuri*

Main category: cs.LG

TL;DR: Evaluating different forecasting models for COVID-19 case counts, with MLAMA achieving the best performance across most forecast horizons.


<details>
  <summary>Details</summary>
Motivation: Public health forecasts must respond to abrupt changes in surveillance data without over-extrapolating noise, reporting artifacts, or temporary trends.

Method: Evaluating autoregressive integrated moving average (ARIMA), random forest, and extreme gradient boosting (XGBoost) models using 190 weekly observations of publicly available Ontario COVID-19 case counts. Rolling-origin time-series cross-validation was used for model tuning and evaluation. Developed Machine Learning and ARIMA Model Averaging (MLAMA).

Result: ARIMA adapted rapidly after turning points but its normalized error increased at longer horizons. Random forest and XGBoost were less responsive initially but maintained more stable normalized error over longer horizons. MLAMA achieved the lowest normalized mean absolute percentage error across most forecast horizons and ranked among the best-performing methods across responsiveness settings.

Conclusion: Selecting forecasting models according to operating conditions rather than relying on a single universally preferred approach is recommended. MLAMA provides a practical framework for combining complementary statistical and machine-learning forecasts.
...
TL;DR: 提出了一种新的选择器，用于选择漏线性水库的配置，以提高水库计算的效率。


<details>
  <summary>Details</summary>
Motivation: 为了提高水库计算（RC）的效率，研究人员提出了一种新的选择器，用于选择漏线性水库的配置。

Method: 提出了一种基于自由概率的确定性和先导信息的选择器，用于漏线性线性水库，并随后进行坐标非线性特征。通过核岭回归在短标签先导序列上进行，以评估候选操作状态，而不需要实例化或进行滚动。

Result: 在十个合成时间基准测试中，无需滚动选择获得了平均部署分数为0.772，与全面模拟搜索的0.774相当，同时避免了156,600次选择滚动。在四个公共电力变压器温度（ETT）预测数据集上，五个保留的候选者恢复了三个数据集的全面操作点。在多变量细胞流量预测中，每个单元15次滚动达到了462次滚动的全面参考，并在低预算下优于随机搜索和贝叶斯优化。

Conclusion: 自由概率核被定位为选择水库操作状态的确定性代理，当验证滚动很少时。
...
### [276] [From Thermal Preference Prediction to Adaptive Thermal Intervention: A Reinforcement Learning Approach Using Physiological and Environmental Sensing](https://arxiv.org/abs/2608.20423)
*Isibor Kennedy Ihianle,Emmanuel Manu,Ehsan Asnaashari,Mojgan Jadidi,Pedro Machado,Amrit Sagoo,Ahmad Lotfi*

Main category: cs.LG

TL;DR: This paper proposes a two-stage approach to achieve personalised thermal comfort through the integration of multimodal sensing and reinforcement learning.


<details>
  <summary>Details</summary>
Motivation: Personalised thermal comfort is essential for occupant wellbeing and for the development of more responsive building-control strategies.

Method: Two-stage personalised thermal comfort approach integrating multimodal physiological and environmental sensing with reinforcement learning-based decision-making.

Result: Improved building-control strategies that account for individual physiological variability.

Conclusion: This approach can enhance occupant wellbeing and building control responsiveness.
...
### [277] [TRACE-C: Rank-Calibrated Relational Anomaly Detection for Multi-Stream Operational Telemetry](https://arxiv.org/abs/2608.21251)
*Matthew Faucher*

Main category: cs.LG

TL;DR: TRACE-C是一种用于对齐多流遥测的审计性严格先验秩校准检测器，其性能评估表明在特定情况下可能存在误差。


<details>
  <summary>Details</summary>
Motivation: Operational telemetry can be jointly anomalous while every individual stream stays inside its familiar range

Method: TRACE-C is an auditable strictly-prior rank-calibrated detector for aligned multi-stream telemetry

Result: TRACE-C ranks Storm Atiyah first among 2019 test windows, but copula-only ranks Atiyah 59th

Conclusion: The resulting p-values are selection quantities, not event probabilities. The copula-form channel is not a literal copula density
...
TL;DR: BF1是一种高效、可复制的稀疏注意力路由，适用于长上下文系统。


<details>
  <summary>Details</summary>
Motivation: 研究在长上下文中实现密集因果注意力机制的高效方法。

Method: 提出BF1，一种确定性的、块对齐的二元稀疏注意力路由，结合了局部精确邻域、全局第一个块和对数间隔的历史块。

Result: BF1在NVIDIA RTX PRO 6000 Blackwell GPU上实现了10.91倍的每层预填充速度提升，并降低了模型预热时间。在匹配的适应协议下，BF1在三个训练种子中排名第一。

Conclusion: BF1是一种可复制的稀疏算子，具有真实的长上下文系统价值。
...
TL;DR: This paper investigates the stability of neural network representations and supports the hypothesis of structural convergence in AI models.


<details>
  <summary>Details</summary>
Motivation: Recent experiments find predictive-state structure in the latent representations of neural networks.

Method: Study the stability of minimal representations of controlled stochastic processes under perturbations, introduce notions of approximate homomorphism, and prove properties such as composability.

Result: Identify conditions under which canonical transducer representations are robust to perturbations, and provide theoretical support for the hypothesis that latent representations of AI models exhibit structural convergence.

Conclusion: The research contributes to understanding the stability of neural network representations and supports the hypothesis of structural convergence in AI models.
...
### [280] [Wrong-Physics Backdoors in Neural PDE Operators](https://arxiv.org/abs/2608.20439)
*Hanbing Liang,Fujun Liu*

Main category: cs.LG

TL;DR: 研究表明，基于神经网络的PDE求解器的验证存在结构性缺陷，需要进一步检查参数溯源。


<details>
  <summary>Details</summary>
Motivation: 在可重用求解器归档上训练神经网络偏微分方程（PDE）算子，但验证通常依赖于干净的预测误差和无参数可塑性检查。

Method: 提出交叉参数重链接，一种数据中毒原语，使得触发的输入在错误的物理参数下从同一PDE族中选择有效解。这被称为错误的物理后门：输出在物理上仍然是合理的，但对目标参数来说是错误的。攻击利用了多参数存档中张量到参数溯源失败，通过标记代理输入并将其监督重链接到同一潜在样本的缓存的备用参数解。

Result: 在476次攻击活动中评估了Burgers、对流扩散、二维Navier-Stokes和椭圆Poisson案例。Fourier神经网络算子和DeepONet提供了主要证据，Transformer、GRU和LSTM模型作为辅助。FNO在对流扩散和二维Navier-Stokes中达到100.00%的后门成功率，同时保持低清洁相对L2误差。清洁标签、标签仅和洗牌控制表明，高攻击成功率本身是不够的：成功的攻击必须将预测推向目标替代物理目标，同时保持清洁误差在界限内。这些结果表明，存在结构性的验证差距：平滑性或通用求解器行为是不够的，除非也验证了目标物理参数的溯源。

Conclusion: 该研究表明，在验证基于神经网络的PDE求解器时，仅仅检查平滑性或通用求解器行为是不够的，还必须验证目标物理参数的溯源。
...
### [281] [Decision Tree and K-Means Analysis of Raman Spectra for Edible Oils: A Physics-Informed AI Approach](https://arxiv.org/abs/2608.20440)
*Amrita Shaw,Chandrasekar S. N.,Sai Muthukumar V.,Jhinuk Gupta,Deepak L. N. Kallepalli*

Main category: cs.LG

TL;DR: 本研究提出了一种基于拉曼光谱和机器学习的方法，实现了对食用油的准确识别，为食品质量监控提供了新的技术基础。


<details>
  <summary>Details</summary>
Motivation: 确保加工食品中食用油的认证对于食品质量、欺诈预防以及法规遵从至关重要。

Method: 本研究建立了一个集成的拉曼光谱和机器学习框架，将内在光谱组织、可解释的分类和基于物理的AI（PI-AI）联系起来。

Result: 对于纯油，决策树在仅使用来自原始1866个特征光谱空间中的四个拉曼变量的情况下实现了100%的分类准确率。对于包含基质的样本，基于NNLS的PI-AI光谱分解显著提高了分类，通过将油相关的特征与纸和土豆的贡献分离出来。优化后的后剪枝模型在去除纸和纸加土豆的样本集分别达到了86.4%和85.4%的准确率，同时将重要的拉曼变量数量减少到只有五个和四个。

Conclusion: 这些发现表明，通过物理意义上的、高度紧凑的、可解释的光谱表示，可以实现基于拉曼的油识别的准确性，为Frugal AI、Edge AI、便携式传感和嵌入式食品质量监控提供了一个有希望的起点。
...
### [282] [Shared Physics Responses Recover Hidden Rankings in Neural Operator Libraries](https://arxiv.org/abs/2608.20441)
*Hanbing Liang,Fujun Liu*

Main category: cs.LG

TL;DR: 提出了一种基于低维张量排名有限模型库的新方法，实现了科学代理的可靠和高效部署。


<details>
  <summary>Details</summary>
Motivation: 选择在部署过程中最佳的神经算子预测具有挑战性，特别是当缺乏高保真参考解决方案时。

Method: 在平方希尔伯特空间损失下，对有限模型库进行排名严格依赖于候选差异的低维张量，允许使用控制方程的单个锚点基线性化响应同时评分所有模型。

Result: 该共享物理诊断准确恢复了99.6%的成对偏好和99.0%的最佳检查点。该框架通过利用局部动力学响应而不是原始缺陷幅度，使科学代理的可靠和高效部署成为可能，无需真实数据。

Conclusion: 该研究提出了一种新的方法，通过低维张量对有限模型库进行排名，实现了科学代理的可靠和高效部署。
...
### [283] [Stored in Optimizer State, Valued by Later Training: A Causal Account of Subliminal Trait Transfer](https://arxiv.org/abs/2608.20442)
*Qinyang Xu*

Main category: cs.LG

TL;DR: 潜意识特质转移通过优化器状态传输源扰动，后续训练确定其行为价值。


<details>
  <summary>Details</summary>
Motivation: 研究潜意识特质转移如何从教师生成的数据中获取学生模型的行为倾向，而特质在数据中并未以语义方式表达。

Method: 使用状态手术和全范围成本预测方法进行分析。

Result: 发现优化器状态传输源扰动，后续训练确定其行为价值，并通过实验验证了这一机制在不同模型和任务中的有效性。

Conclusion: 发现了潜意识特质转移的两阶段机制：优化器状态传输源扰动，后续训练确定其行为价值。
...
### [284] [Amortized Bandwidth Learning for Kernel Density Estimation under Logarithmic Score](https://arxiv.org/abs/2608.20445)
*Junyi Liang,Hailiang Du*

Main category: cs.LG

TL;DR: 提出了一种新的带宽选择方法，在多个实验中优于传统方法，适用于将有限样本转换为连续概率密度。


<details>
  <summary>Details</summary>
Motivation: Kernel密度估计将有限样本转换为概率密度，但其性能取决于带宽选择。

Method: 提出了一种摊销框架，该框架通过优化对数得分来学习在密度估计任务分布上的映射。

Result: 在Gaussian抽样、多家族基准和随机高斯混合训练下，该摊销选择器在小型和异质样本中尤其优于Silverman规则、Sheather-Jones选择器和最小二乘交叉验证。

Conclusion: 该框架因其广泛的应用性和强大的实证性能，对将有限样本或集成转换为连续概率密度的大量应用具有吸引力。
...
### [285] [Mutual information and sensitivity analysis for feature selection in customer targeting: a comparative study](https://arxiv.org/abs/2608.20447)
*Nestor Barraza,Sergio Moro,Marcelo Ferreyra,Adolfo de la Peña*

Main category: cs.LG

TL;DR: 互信息和基于数据的敏感性分析在特征选择中各有优势，互信息在降低成本的同时保持成功率，而基于数据的敏感性分析在减少特征数量方面表现更好。


<details>
  <summary>Details</summary>
Motivation: 特征选择在数据驱动知识发现项目中非常重要。

Method: 通过将两种技术（互信息和基于数据的敏感性分析）应用于银行电话营销案例，分析它们的优缺点。

Result: 基于数据的敏感性分析在降低误报率方面表现更好，而互信息在提高误报率比方面略胜一筹。

Conclusion: 互信息虽然不是最新的方法，但仍然是特征选择的有效方法。基于数据的敏感性分析在减少特征数量同时保持良好预测结果方面表现良好。
...
### [286] [Metag: A dataset to build agentic meta-reviewing capabilities](https://arxiv.org/abs/2608.20488)
...
TL;DR: Metag数据集通过自动化识别论文修改，减轻元审稿人负担，提高同行评审效率。


<details>
  <summary>Details</summary>
Motivation: 为了解决元审稿人工作负担增加的问题，特别是识别科学论文在审稿-反驳过程中的修改。

Method: 通过从审稿截止日期前和接受后的手稿版本中获取数据，计算两个文档之间的差异，并让人类注释员将这些差异与OpenReview讨论中的行动项对齐。

Result: 创建了一个包含349个高质量行动项的数据集，与论文差异相关，将有助于构建方法，使元审稿人能够快速识别作者是否解决了审稿人的陈述，以及这些更改在论文中的具体位置，从而在整个同行评审过程中增加透明度和可追溯性。

Conclusion: Metag数据集有助于加速元审稿代理的开发，提高同行评审的效率和透明度。
...
### [287] [Bern2Edge: A Neurosymbolic Compiler for Edge Deployment via Bernstein Polynomial Networks](https://arxiv.org/abs/2608.20497)
...
### [288] [When Graph-JEPA Learns the Wrong Thing: Diagnosing and Repairing Category-Conditional Collapse](https://arxiv.org/abs/2608.20516)
...
### [289] [Learning Exact NVIDIA SASS Encoders with $\mathbb{F}_2$ Linear Algebra](https://arxiv.org/abs/2608.20532)
*Jiading Gai*
...
### [290] [AgentDecarbonizer: Carbon-Aware Execution for AI Agents](https://arxiv.org/abs/2608.20566)
*Leyi Yan,Shuangning Li,Sihang Liu*

Main category: cs.LG

TL;DR: AI代理的碳优化器AgentDecarbonizer可显著降低碳排放。


<details>
  <summary>Details</summary>
Motivation: AI代理的工作流程可以执行诸如软件修复、数据分析等任务，但其重复调用模型可能导致大量碳排放。

Method: 使用WildClawBench评估OpenClaw代理工作负载的碳排放，并开发AgentDecarbonizer碳优化器。

Result: AgentDecarbonizer可以将碳排放减少57.9%，与不考虑碳排放的基线相比，与在任务开始时选择碳最优电网的基线相比，碳排放减少37.5%。

Conclusion: AgentDecarbonizer通过灵活调整执行时间，有效降低了AI代理的碳排放。
...
### [291] [Faults That Fortify: CNN Adversarial Robustness via GPU Undervolting](https://arxiv.org/abs/2608.20572)
*Behnam Omidi,Ahmad Tahmasivand,Husam Alsyouri,Saba Al-Sayouri,Chongzhou Fang,Ihsen Alouani,Khaled N. Khasawneh*

Main category: cs.LG

TL;DR: 通过 GPU 降压提高 CNN 的鲁棒性和能效。


<details>
  <summary>Details</summary>
Motivation: CNNs 面临双重挑战：易受对抗攻击和昂贵的训练成本。

Method: 在训练期间使用 GPU 降压，通过降低供电电压引入随机扰动，作为隐式正则化，提高鲁棒性并降低功耗。

Result: 在两种训练制度下，降压模型始终比其标称电压对应模型具有更高的对抗准确性，表明硬件引起的故障甚至加强了对抗训练。这些鲁棒性收益伴随着大量的节能。

Conclusion: GPU 降压是一种易于部署的硬件级防御，不需要算法更改，并开辟了一个有希望的方向，其中鲁棒性和能效可以共同进步。
...
### [292] [Provable Edge-of-Stability for Adam on a One-Dimensional Quadratic](https://arxiv.org/abs/2608.20638)
...
### [293] [Meta-clustering of milk mid-infrared spectra identifies dairy cow groups associated with negative energy balance in early lactation](https://arxiv.org/abs/2608.20653)
*T. Touil,E. R. Paquet*
...
### [294] [RiskTraf: Risk-Extrapolated Residual Learning for Multi-Variate Traffic Flow Prediction](https://arxiv.org/abs/2608.20656)
*Guangyu Wang,Zhidan Liu*

Main category: cs.LG

TL;DR: 提出PEMSB-3V基准和RiskTraf模型，提高了交通流量预测的准确性。


<details>
  <summary>Details</summary>
Motivation: 现有交通流量预测基准和模型很少可靠地利用流量、速度和占有率三个原始测量值。速度和占有率提供了除流量之外的本征交通状态信息，但现有版本通常省略这些变量，用代理变量代替，或包含逻辑上不一致的记录。此外，直接对三个变量输入进行经验风险最小化可能利用了制度相关的捷径，因为流量、速度和占有率之间的关系在自由流和拥堵状态之间差异很大。

Method: 提出了PEMSB-3V，一个公开的基准套件，它保留了PeMS探测器中的原始流量、速度和占有率测量值，用于流量预测。还提出了RiskTraf，一个模型无关的风险外推残差插件。对于每个训练的时空骨干网络，RiskTraf冻结所选的检查点，并从历史速度和占有率学习一个轻量级的零起点残差头。残差头构建有序的交通风险环境，并使用风险外推目标进行时间范围流量的优化校正，从而减轻了特定制度的相关性，而不修改骨干网络。

Result: 广泛的实验表明，RiskTraf一致地提高了不同的预测骨干网络，并优于去偏和分布变化适应性方法。

Conclusion: 本文提出了一种新的交通流量预测方法，该方法通过引入模型无关的风险外推残差插件，有效地利用了流量、速度和占有率三个原始测量值，并提高了预测精度。
...
### [295] [C-Score: Beyond Accuracy for Robustness Assessment in Semi-Supervised Learning under Open-World Unlabeled Contamination](https://arxiv.org/abs/2608.20667)
*Tsao-Lun Chen,Chi-Cheng Fu,Han-Yi E. Chou,Shun-Feng Su*

Main category: cs.LG

TL;DR: C-Score框架揭示了伪标签半监督学习在开放世界环境下的隐藏退化问题，表明仅使用清洁准确率评估鲁棒性是不够的。


<details>
  <summary>Details</summary>
Motivation: 针对伪标签半监督学习在开放世界环境下存在未标记数据污染问题，导致内部学习动态恶化，但清洁测试准确率可能仍然稳定。

Method: 提出C-Score框架，从诊断评估角度研究伪标签半监督学习中的隐藏崩溃问题。C-Score评估训练行为在预测、特征表示和优化三个互补空间中的表现。

Result: C-Score揭示了清洁准确率无法检测到的隐藏退化，例如在SVHN污染下，CCI上升超过280%，而最佳准确率仍保持在未污染基线3%以内；接近OOD源（CIFAR-100，STL-10）导致高达14.9%的准确率下降。

Conclusion: 清洁准确率不足以评估开放世界环境下的SSL鲁棒性，内部诊断信号对于更可靠的鲁棒性评估是必要的。
...
### [296] [Lightweight Adaptive ReduNet via Hyperspherical Manifold Learning](https://arxiv.org/abs/2608.20668)
*Zhenglin Huang,Qifa Yan,Bin Dai,Xiaohu Tang*

Main category: cs.LG

TL;DR: LA-ReduNet通过优化层更新规则，有效减少ReduNet的参数存储和层数


<details>
  <summary>Details</summary>
Motivation: 针对ReduNet在层展开时参数存储量大的问题

Method: 提出LA-ReduNet，采用超球面流形学习和自适应步长优化层更新规则

Result: LA-ReduNet在保持分类精度的同时，大幅减少了展开层的数量和参数存储量

Conclusion: LA-ReduNet是一种高效且参数较少的白盒神经网络架构
...
### [297] [Reinforcement Learning for Continuous-Time Jump Markov Decision Processes with Applications to Network Dynamic Pricing](https://arxiv.org/abs/2608.20680)
*Huiling Meng,Ningyuan Chen,Xuefeng Gao*

Main category: cs.LG

TL;DR: 本研究在CTJMDPs中实现了有效的强化学习，并取得了良好的实验结果。


<details>
  <summary>Details</summary>
Motivation: 研究连续时间跳跃马尔可夫决策过程（CTJMDPs）中的强化学习（RL），其中状态空间是离散的，动作空间可以是连续或离散的。

Method: 提出了一种熵正则化的连续时间控制问题，使用随机策略来建模探索-利用权衡。同时，建立了CTJMDPs的$q$-学习理论基础，并开发了无模型$q$-学习算法。

Result: 在多产品动态定价等应用中，该方法在数值实验中显示出良好的性能，可靠地学习到近似最优策略，并优于标准基准方法。

Conclusion: 该方法在CTJMDPs中实现了有效的强化学习，为解决实际应用中的复杂问题提供了新的思路。
...
### [298] [Geometric Regularization for Long-Tailed Semi-Supervised Learning via Gaussian Feature Bridges](https://arxiv.org/abs/2608.20710)
*Hongyang He,Xinyuan Song,Yan Zhong,Daizong Liu,Yanbin Li,Yang-fan He,Wenqiao Zhang*

Main category: cs.LG

TL;DR: 提出GBC框架，解决SSL中的长尾标签分布和噪声伪标签问题，提高长尾类性能。


<details>
  <summary>Details</summary>
Motivation: 解决现实世界半监督学习中长尾标签分布和噪声伪标签带来的挑战，以改善泛化能力和减少确认偏差。

Method: 提出了一种名为高斯桥一致性（GBC）的新框架，通过构建无标签样本与高质量类别锚点之间的语义插值路径来解决问题。

Result: GBC通过动态原型图谱存储每个类别的多样化和不断发展的标签和伪标签示例集，并在潜在空间中为每个无标签实例形成类别条件下的高斯特征桥。

Conclusion: 在CIFAR10-LT和ImageNet-LT（USB基准）上的大量实验验证了GBC在现实长尾SSL设置下的鲁棒性和有效性，在提高长尾类性能的同时保持了可扩展性。
...
### [299] [Hidden Axis of Uncertainty: Latent-Posterior Alignment in Graph Neural Networks with Bayesian Output Layers](https://arxiv.org/abs/2608.20758)
*Suk Hoon Choi,Damdae Park,Junhyuk Choi,Hyein Jung,Changsoo Kim,Ung Lee,Kyeongsu Kim*

Main category: cs.LG

TL;DR: 提出了一种新的方法来降低GNNs中的预测不确定性


<details>
  <summary>Details</summary>
Motivation: 量化预测不确定性的机制仍然不明确

Method: 观察和实验，提出Alignment-Guided Learning(AGL)算法

Result: 有效地降低预测不确定性，同时保持准确性和结构校准

Conclusion: 提供了对GNNs中不确定动力学的新视角
...
### [300] [Fuzzy-MoE: Interpretable Regime-Conditioned Expert Routing for Non-Stationary Multivariate Time Series Forecasting](https://arxiv.org/abs/2608.20761)
*Lan Guo,Jie Xiao,Zhao Su,Jun Shen,Haoran Li,Weixia Ma,Qingguo Zhou,Binbin Yong*

Main category: cs.LG

TL;DR: Fuzzy-MoE是一个基于模糊逻辑的动态混合专家模型，通过显式规则选择专家，在时间序列预测中表现出色，同时提高了模型的可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有的深度预测模型通常将不同的变量和样本压缩成一个统一的端到端映射，导致对时变动态的建模不佳，并且在不同的潜在状态下，预测机制的透明度有限。

Method: 将时间序列预测重新表述为潜在时间状态识别和可解释专家路由的统一框架，提出基于模糊逻辑的动态混合专家模型Fuzzy-MoE。Fuzzy-MoE由多个并行专家映射网络和一个双视图模糊路由器组成。路由器通过联合利用局部卷积动态和全局分段统计，推断潜在时间状态并计算专家激活强度，通过可学习的高斯隶属函数，实现基于显式IF-THEN规则的专家选择。

Result: 在多个公共时间序列基准数据集上，Fuzzy-MoE在预测精度上显著优于主流预测方法，模糊隶属度和规则激活提供了可解释的路线诊断。

Conclusion: Fuzzy-MoE在多个时间序列基准数据集上显著优于主流预测方法，并且其模糊隶属度和规则激活提供了可解释的路线诊断，证明了所提出框架在预测性能和机制透明度方面的有效性。
...
### [301] [Resolution-Consistent Greedy Neural Approximation on Infinite-Dimensional Spaces](https://arxiv.org/abs/2608.20812)
*Pablo M. Berná,Antonio Falcó,Diego Mondéjar*

Main category: cs.LG

TL;DR: Develops constructive approximation and learning guarantees for shallow neural models with infinite-dimensional inputs, offering a dimension-free and statistically sound framework for empirical regression and Hilbert-valued responses.


<details>
  <summary>Details</summary>
Motivation: The analysis is based on a parameter-normalized neural dictionary and its associated weighted variation class. The dimension-free statements are statistical, not computational: selecting a new neuron still requires solving a nonconvex parameter-search problem. The quasi-Polish construction underlying recent infinite-dimensional universal approximation results provides a motivating example.

Method: Constructive approximation and learning guarantees for shallow neural models with infinite-dimensional inputs observed through finitely many coordinates. A fully-corrective greedy procedure is used for empirical regression. The framework extends to Hilbert-valued responses without an explicit dependence on the output dimension.

Result: The approximation error separates into a distribution-dependent coordinate-truncation term and a greedy finite-width term. Population guarantees with uniform statistical complexity in the retained input resolution are obtained. The framework is dimension-free and applicable to Hilbert-valued responses. Synthetic experiments illustrate the predicted resolution, width, and sample-size regimes.

Conclusion: The proposed approach provides a constructive approximation and learning guarantee for shallow neural models with infinite-dimensional inputs, offering a dimension-free and statistically sound framework for empirical regression and Hilbert-valued responses.
...
### [302] [Scaling Muon for Diffusion Transformers](https://arxiv.org/abs/2608.20818)
...
TL;DR: Periodic Row-wise Muon在大型Diffusion Transformers上提高了生成质量和训练效率。


<details>
  <summary>Details</summary>
Motivation: 解决现有矩阵感知优化器在大型Diffusion Transformers上扩展行为和端到端效率问题。

Method: 通过在每K步进行一次完整的NS5谱更新和基于当前动量的低计算和通信成本的行约束更新，以及与桶式全聚集和通信-计算重叠相结合的分布式实现。

Result: 与AdamW相比，Periodic Row-wise Muon提高了最佳生成质量12.9-19.1%。

Conclusion: Periodic Row-wise Muon在大型Diffusion Transformers上保持了Muon的生成质量优势，同时提高了端到端训练效率。
...
### [303] [Nothing Changed but the Model: CellFill -- Bounded In-Cell Learning for Bit-Identical, Revocable Updates to Quantized LLMs](https://arxiv.org/abs/2608.20873)
*Zifeng Liu,Zhiyong Du,Yaxin Lu,Yiming Mao,Zhenhe Wang,Wenqi Shi,Zhengkun Jing*

Main category: cs.LG

TL;DR: 提出了一种在量化决策单元内部学习新知识的方法，有效降低了跨域遗忘，提高了模型性能。


<details>
  <summary>Details</summary>
Motivation: 为了在部署的语言模型中学习新知识，避免替换整个模型，研究了一种在量化决策单元内部学习新知识的方法。

Method: 在4位发布中冻结整数代码和比例，将新知识写入每个量化决策单元内部的每个权重残差。然后重新量化，确保发布工件位对位返回，更新可撤销，漂移有界。提出了CellFill等六种命题和三种训练路径。

Result: 在三个配对种子中，约束密集路径与未约束参考路径在事实召回率方面相当（58.9% vs 59.3%），在保留出的跨域困惑度方面表现更好。在每次收敛的运行中，将投影到单元中可以减少跨域遗忘，而发散的控制显示了边界：投影是一个信任区域，而不是修复。知识本身的成本无法避免，域内困惑度在锚点之后提高的明显免费午餐是排练与度量共享语料库的结果。在匹配重排中，三种方法在每点跨域困惑度购买的知识方面存在三倍差异，这种排名不是召回排名。该方法适用于27B混合线性注意力模型，匹配召回的成本约为1.7B时的跨域困惑度的一半。

Conclusion: 提出了一种在量化决策单元内部学习新知识的方法，有效降低了跨域遗忘，提高了模型性能。
...
TL;DR: DFL鲁棒性被高估，BackDFL基准揭示其弱点


<details>
  <summary>Details</summary>
Motivation: 分析分布式联邦学习（DFL）的鲁棒性被高估的问题，并提出解决方案

Method: 提出BackDFL，一个用于评估DFL在现实和自适应后门攻击下的统一基准

Result: 实验结果表明，最先进的拜占庭鲁棒DFL方法和适应的FL后门防御在低恶意参与率下（低至15%）失败，特别是在异构环境中，其鲁棒性在通信图拓扑结构中差异很大

Conclusion: DFL的鲁棒性被高估，需要更全面的评估方法来应对后门攻击
...
Conclusion: The proposed decoupled policy extraction paradigm effectively addresses the challenges in offline RL and achieves superior performance compared to traditional methods.
...
### [306] [Training, learning and inference: unified dynamics of neural systems](https://arxiv.org/abs/2608.20965)
*Mian Wang*

Main category: cs.LG

TL;DR: 提出了一种基于GFG的递归科学过程，通过nanoGPT实现了高准确率的训练-学习动态。


<details>
  <summary>Details</summary>
Motivation: 为了构建一个能够保存生成历史的AI原生科学事实基础，提出了原子生成事实和生成事实图（GFG）的概念。

Method: 使用nanoGPT建立统一的训练-学习动态，通过参数-优化器系统的状态和记忆进行训练，并通过学习持续重组分布式功能支持。

Result: 在四个转换中，达到了91.43%的准确率和91.49%的宏观平均召回率。实验结果表明，GFG在nanoGPT中表现出接收状态条件下的响应、持久支持重组和冻结推理投影。

Conclusion: 提出了一种基于GFG的递归科学过程，并通过实验验证了其在nanoGPT中的有效性。
...
### [307] [Jacobian-guided Noise Injection for Quantization Robustness in Large Language Models](https://arxiv.org/abs/2608.20988)
*Deepanshu Pandey,Arnav Chavan,Nahush Lele,Sankalp Dayal,Deepak Gupta*

Main category: cs.LG

TL;DR: 提出了一种新的LLMs量化方法，提高了性能和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: LLMs量化受自注意力机制对离散误差敏感性的限制。

Method: 提出了一种名为Jacobians-Guided Noise Injection的训练策略，通过向预注意力logits中注入高斯噪声，方差直接从雅可比Frobenius范数中导出。

Result: 在SOTA LLM架构上评估，该方法比流行的PTQ方法表现出更高的鲁棒性。在ImageNet-1K上，该方法使SigLIP的Top-1精度提高了+37%，在WikiText上，低比特量化设置中的语言模型的相对困惑度降低了40%。

Conclusion: 该方法有效地提高了LLMs量化的性能和鲁棒性。
...
### [308] [Trojaning the Alignment: Stealthy Backdoor Attacks against Graph Foundation Models](https://arxiv.org/abs/2608.20991)
*Minhua Lin,Zhicheng Gao,Yilong Wang,Hanqing Lu,Xiang Zhang,Suhang Wang*

Main category: cs.LG

TL;DR: STAG is a new attack framework for GFMs on TAGs.


<details>
  <summary>Details</summary>
Motivation: Graph Foundation Models (GFMs) on text-attributed graphs (TAGs) have backdoor vulnerabilities, especially under graph-language alignment.

Method: Proposing STAG, a stealthy trojan attack framework for the graph-language alignment interface of GFMs on TAGs.

Result: Experiments demonstrate the effectiveness and stealthiness of STAG.

Conclusion: STAG is an effective and stealthy attack framework for GFMs on TAGs.
...
### [309] [RODE: A Radial-Orthogonal Decoupled Engine for Optimization](https://arxiv.org/abs/2608.21024)
*Guoxiang Xu,Bince Qu,Qi Sun,Cheng Zhuo*

Main category: cs.LG

TL;DR: RODE通过分离径向和方向动态，在多个任务中提高了模型性能，并降低了全局范数。


<details>
  <summary>Details</summary>
Motivation: 现代神经网络训练中，矩阵感知优化器被广泛使用，但其条件矩阵步通常直接添加到权重中，同时改变其范数和方向。这种交互很重要，因为当前的范数决定了角运动，而方向学习可以推动范数增长，从而改变后续步骤。

Method: RODE方法为径向和方向分量提供了单独的更新规则和步长。RODE通过标量径向规则显式更新矩阵Frobenius范数，其方向通道在切线空间中执行牛顿-舒尔茨条件更新。

Result: 在两个语言建模和两个图像分类任务中，RODE在所有直接比较中优于Muon变体，并最终具有更低的完整模型范数。在1.5B规模下，使用从Qwen2风格LM扫描直接转移的学习率，RODE将损失从4.145降低到3.346，将最终全局范数从11964降低到2183相对于Muon RMS，固定半径RODE进一步改进。在Qwen3.5-9B全参数微调中，所有六个优化器使用相同的调整预算和相同的正式训练和评估设置；RODE在所有四个评估任务上优于Muon变体，并在GSM8K和MATH-500上获得最高的平均值。

Conclusion: 将径向和方向动态解耦为矩阵优化提供了一种更有效、更可控的方法。
...
### [310] [Designing a Robust LLM-Based Evaluation System for Agentic AI in Drug Discovery Through Human Alignment](https://arxiv.org/abs/2608.21057)
*Emma Granqvist,Rocío Mercado,Samuel Genheden*

Main category: cs.LG

TL;DR: 提出了一种评估LLM输出的新框架，提高了与人类专家的一致性。


<details>
  <summary>Details</summary>
Motivation: 评估化学和药物发现中LLM输出的开放性、工具增强型输出仍然是一个基本瓶颈。

Method: 定义了四个输出质量评估维度，并通过人类一致性研究验证了法官，然后使用人类注释示例的少样本演示优化了最佳法官。

Result: 评估框架提高了与人类多数投票的一致性，并揭示了具体限制。

Conclusion: 本研究提出了一个用于评估ChatInvent的LLM-as-a-Judge评估框架，以解决化学和药物发现中LLM输出的评估问题。
...
### [311] [TracingFlow: A Simulation-Free Trajectory Inference Framework Based on Second-Order Dynamics](https://arxiv.org/abs/2608.21070)
*Yuhao Sun,Zekun Wu,Zixun Huang,Peijie Zhou*

Main category: cs.LG

TL;DR: TracingFlow通过神经网络处理连续系统演化，在单细胞组学中表现出色。


<details>
  <summary>Details</summary>
Motivation: 推断连续系统演化，尤其是从稀疏时间快照中推断，是生成建模和单细胞组学中的关键挑战。

Method: 提出了一种名为TracingFlow的模拟免费Flow Matching框架，该框架推广到二阶动力学。使用神经网络回归加速度场，TracingFlow为动态最优加速传输（DOAT）问题提供了一种精确、高效的解决方案。

Result: 在复杂合成数据和大规模单细胞RNA测序数据集上，TracingFlow在分布重建和轨迹忠实度方面实现了更高的准确性。通过整合谱系追踪先验，它恢复了在数学上最优且在生物学上合理的动态结构。

Conclusion: TracingFlow为处理连续系统演化提供了有效的解决方案，在单细胞组学中具有潜在的应用价值。
...
### [312] [Causal Modeling of Adverse Pregnancy Outcomes via Adaptive LLM Proposals](https://arxiv.org/abs/2608.21079)
*Kavimayil P. Komarasamy,Saurabh Mathur,Ameet Soni,David M. Haas,Kristian Kersting,Sriraam Natarajan*

Main category: cs.LG

TL;DR: A neurosymbolic framework effectively recovers causal relations in APOs, offering insights for interventions.


<details>
  <summary>Details</summary>
Motivation: Adverse Pregnancy Outcomes (APOs) such as preterm birth and gestational diabetes have long-term consequences for both the mother and child. Causal discovery in this domain is challenging due to data scarcity and incomplete knowledge.

Method: A neurosymbolic framework that combines LLM's broad prior knowledge with empirical data scoring. Treats LLM as an adaptive proposal distribution, updating context for subsequent generations.

Result: Recovered all expert-validated edges, identified additional plausible causal relations, providing insights for interventions.

Conclusion: The proposed method is effective for causal discovery in APOs, offering new insights for targeted interventions.
...
### [313] [FlatLand: Personalized Graph Federated Learning via Tailored Lorentz Space](https://arxiv.org/abs/2608.21096)
*Jiahong Liu,Ram Samarth B B,Xinyu Fu,Menglin Yang,Weixi Zhang,Rex Ying,Irwin King*

Main category: cs.LG

TL;DR: FlatLand: A novel method for personalized federated learning that embeds data in hyperbolic geometry to improve performance.


<details>
  <summary>Details</summary>
Motivation: Existing personalized federated learning methods ignore the intrinsic geometric properties of diverse graph structures.

Method: Propose FlatLand, a novel personalized federated learning method that embeds different clients' data in tailored Lorentz space of hyperbolic geometry.

Result: FlatLand achieves superior performance, particularly in low-dimensional settings.

Conclusion: FlatLand effectively handles the intrinsic geometric properties of diverse graph structures in personalized federated learning.
...
### [314] [COEC: Calibrated Orthogonal-Equivalence Compensation for Structured Pruning of Large Language Models](https://arxiv.org/abs/2608.21142)
*Peiqi Yu,Nam Ling,Wei Wang,Wei Jiang*

Main category: cs.LG

TL;DR: COEC是一种无需训练的补偿框架，可以显著提高结构化剪枝后的LLMs性能。


<details>
  <summary>Details</summary>
Motivation: 为了减少大型语言模型（LLMs）的大小和推理成本，结构化剪枝通过删除权重列来实现，但会导致输出误差并降低准确性。

Method: 提出了一种名为COEC的无需训练的补偿框架，该框架对保留的权重应用交替的左旋和右旋正交旋转。

Result: 在Llama-3，Llama-3.1和Qwen2.5模型家族的多个结构化稀疏级别上进行的实验表明，COEC在所有模型上提高了困惑度，在大多数设置中提高了零样本准确性，在更高稀疏度下收益更大。

Conclusion: COEC可以在不重新训练模型参数的情况下，通过后剪枝补偿恢复部分因列删除而丢失的性能。
...
### [315] [Capturing Cardiac Cyclicity through Phase-Equivariant Self-Supervised Learning](https://arxiv.org/abs/2608.21147)
*Blaise Delaney,Dominic Dootson,Juan Jose Juan Castella,Salil Patel,Andrew Pfaff,Yuji Xing,Jonny Hancox,Karin Sevegnani*

Main category: cs.LG

TL;DR: 通过明确编码心脏相位对称性，Winder在保持诊断信息的同时，实现了参数高效的潜在几何形状。


<details>
  <summary>Details</summary>
Motivation: 生理过程的循环结构为自监督表示学习提供了一种自然先验，而心脏周期提供了一个特别有定义的设置，可以充分利用它。

Method: 提出了一种相位相干的自我监督目标，并引入了Winder，这是一种联合嵌入架构，将表示组织成相位不变坐标和相位旋转谐波子空间。其传输算子是固定的和闭形式的，源自循环的几何形状，而不是学习的，并且不添加任何参数。

Result: 在PTB-XL上使用冻结线性探针协议进行评估，Winder在~1 M参数足迹范围内达到与最先进的自监督方法相当的诊断准确性，同时表现出相位相干的潜在几何形状。

Conclusion: 这些发现表明，明确编码心脏相位对称性可以保留诊断上有用的信息，同时产生可读、参数高效且直接与可测量的生理量相关的潜在几何形状。
...
### [316] [Thermo-FL: Thermal-Aware Robust Federated Fine-Tuning of Large Language Models for Edge AI](https://arxiv.org/abs/2608.21172)
*Shiva Shrestha,Kazi Shaharair Sharif,Zongxing Xie,Jiajing Huang,Anhao Xiang,Honghui Xu*

Main category: cs.LG

TL;DR: Thermo-FL框架通过热感知和稀疏通信提高了边缘LLM的鲁棒性和效率。


<details>
  <summary>Details</summary>
Motivation: 为了解决联邦微调在边缘设备上部署时遇到的硬件不稳定和对抗性更新损坏问题。

Method: 提出了一种名为Thermo-FL的热感知联邦LoRA微调框架，该框架利用设备温度作为本地适配器训练和稀疏更新传输的主动控制信号。

Result: 在仿真器和基于Jetson的物理测试台上评估了Thermo-FL，结果表明Thermo-FL在对抗性稀疏聚合下提高了鲁棒性，并在BoolQ准确性和GSM8K上保持竞争力。在物理原型中，Thermo-FL稳定了设备温度，通过位图稀疏编码减少了压缩上传大小，并在MITM干扰下保持了GSM8K的有效性。

Conclusion: 安全边缘LLM自适应应联合考虑硬件行为、工作负载调节、稀疏通信和聚合鲁棒性。
...
### [317] [A Neurosymbolic Approach for Constructing Planning Domain Models from Clinical Narratives](https://arxiv.org/abs/2608.21186)
*Ranveer Singh,Saurabh Mathur,Michael Skinner,Prasad Tadepalli,Kristian Kersting,Sriraam Natarajan*

Main category: cs.LG

TL;DR: NSPIN is a neurosymbolic framework for formalizing surgical workflows, effective and consistent with practice.


<details>
  <summary>Details</summary>
Motivation: Surgical procedures like laparoscopic appendectomy are complex and require formalized workflows for decision support. However, inducing probabilistic planning domain models in this setting is challenging due to lack of structured event data and implicit actions in clinical narratives.

Method: NSPIN, a neurosymbolic framework for inducing probabilistic planning domain models from unstructured clinical narratives. The method extracts and imputes structured event sequences using a pretrained LLM, then induces a PPDDL model and refines its preconditions with LLM-proposed revisions.

Result: NSPIN yields models that generalize to unseen notes, and expert clinical review indicates its induced knowledge is largely consistent with surgical practice.

Conclusion: NSPIN is effective in formalizing workflows for complex surgical procedures like laparoscopic appendectomy and is consistent with surgical practice.
...
### [318] [Tydra: An Efficient Hybrid Model for Tabular Data](https://arxiv.org/abs/2608.21199)
*Mieszko Komisarczyk,Saurabh Mathur,Maurice Kraus,Sriraam Natarajan,Kristian Kersting*

Main category: cs.LG

TL;DR: Tydra是一种混合Transformer和SSM的表格基础模型，在保持预测性能的同时显著提高了效率


<details>
  <summary>Details</summary>
Motivation: 为了平衡Transformer和SSM模型的性能和效率

Method: 提出了Tydra，一种混合Transformer-State Space Model（SSM）架构

Result: 在30个OpenML数据集上，Tydra相对于TabPFN减少了30%的推理时间，同时保持了大部分预测性能；在推理速度上优于大约十倍大的Hydra模型

Conclusion: 混合架构是表格基础模型的一个有希望的途径
...
### [319] [Curriculum-Aware Interpolate-then-Refine: Learned Physiological Time-Series Imputation under Realistic Missingness](https://arxiv.org/abs/2608.21207)
...
### [320] [ConceptTS: LLM-Guided Concept Bottlenecks for Interpretable Multivariate Time-Series Forecasting](https://arxiv.org/abs/2608.21277)
*Yichen Jiang,Yueqiao Chen,Dongyu Liu*

Main category: cs.LG

TL;DR: ConceptTS框架通过组织预测并引入命名概念，提高了时间序列预测的可解释性


<details>
  <summary>Details</summary>
Motivation: 提高时间序列预测模型的透明度

Method: 使用大型语言模型生成概念和标签规则，将预测围绕命名概念组织

Result: 在空气质量数据集上达到与黑盒基线相当的性能，同时产生具有语义意义的概念激活

Conclusion: ConceptTS框架能够提高预测的透明度和可解释性
...
### [321] [SPARCL: Spectral Partitioned Analytic Continual Learning](https://arxiv.org/abs/2608.21307)
...
### [322] [Rethinking Expressivity and Efficiency in Test-Time Training](https://arxiv.org/abs/2608.21308)
*Zeyun Zhong,Joya Chen,Manuel Martin,Frederik Diederichs,Juergen Gall,Juergen Beyerer*

Main category: cs.LG

TL;DR: E$^2$-TTT是一种高效且具有表达力的测试时训练方法，在语言建模和上下文检索任务中均表现出色。


<details>
  <summary>Details</summary>
Motivation: 为了解决当前测试时训练（TTT）方法在平衡每令牌更新动态的表达能力和分块近似硬件效率方面存在困难的问题。

Method: 提出了一种名为E$^2$-TTT的方法，该方法在分块起始权重梯度标准近似下，推导出一种封闭形式的状态转换，能够精确地复制每令牌递归的分块结束快速权重和动量状态。

Result: 在语言建模任务中，E$^2$-TTT的表现与之前的TTT和混合注意力基线相当，但在上下文检索任务中表现更优。在长度外推方面，E$^2$-TTT在标准“针插在干草堆中”的passkey测试中，在训练上下文长度的8倍下，保持了超过90%的准确率。

Conclusion: E$^2$-TTT有效地解决了表达能力和效率之间的矛盾，在语言建模和上下文检索任务中均表现出色。
...
### [323] [Time-Aware Tranformer-Based Prediction Model for AECOPD](https://arxiv.org/abs/2608.21324)
*Weihao Qu,Ling Zheng,Dongyang Wang,Jiacun Wang,Haowen Pan*

Main category: cs.LG

TL;DR: 本文提出了一种基于时间感知Transformer的AECOPD预测模型，在家庭监控场景中取得了优于传统方法的性能。


<details>
  <summary>Details</summary>
Motivation: 为了确保及时检测AECOPD并最小化延迟，本文重点关注仅使用日常使用的呼吸机数据的家庭监控场景。

Method: 引入基于时间感知Transformer的AECOPD预测模型，使用时间感知Transformer生成有意义的患者表示，以捕捉症状及其在呼吸机数据中的时间进程。

Result: 实验结果表明，基于时间感知Transformer的方法在多个分类任务中优于传统方法，突出了其提高AECOPD预测精度的潜力。

Conclusion: 本文提出的方法在AECOPD预测中具有潜在优势，可以进一步提高预测准确性。
...
### [324] [Across-Design Uncertainty in Short Pricing Panels: Evidence from Simulated Price Trajectories](https://arxiv.org/abs/2608.21334)
...
### [325] [Asymmetric Capacity Allocation in Self-Refinement Pipelines](https://arxiv.org/abs/2608.21345)
...
TL;DR: 这项研究系统地研究了模型大小对自优化各个阶段的影响，并提出了模型容量分配的新见解。


<details>
  <summary>Details</summary>
Motivation: 为了提高LLM生成，自优化通常被采用，并在许多LLM代理中作为核心机制。然而，现有方法大多将模型大小视为实现细节，而不是研究主题，这可能导致资源浪费。目前很少有工作系统地研究模型大小如何影响每个阶段，或者有效的自优化是否需要同等能力的生成、批评和修订模型。

Method: 对来自不同领域的5个基准进行了第一阶段模型大小研究，使用了6种Qwen3模型大小和4种Gemma 3模型大小。

Result: 较大的生成器和修订器通常可以改善管道，而较小的修订器甚至可能损害性能。批评者的性能对大小高度不敏感，尽管包括一个小的批评者通常比完全不进行批评的表现更好。

Conclusion: 模型容量不应在自优化管道中均匀分配。相反，不同阶段表现出不同的规模缩放特性，为设计更高效的多阶段语言模型系统提供了实际指导。
