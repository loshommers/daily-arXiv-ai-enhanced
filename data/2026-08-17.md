...
TL;DR: 本研究针对妇科图像分割中的持续学习问题，揭示了遗忘现象与网络深度中更新位置的关系。


<details>
  <summary>Details</summary>
Motivation: 在临床实践中，医学图像数据通常是按顺序到达的，需要模型能够持续适应不断变化的数据分布。本研究针对妇科图像分割问题，探讨了在数据分布发生较大变化时，现有持续学习方法难以保留先前学习知识，导致灾难性遗忘的问题。

Method: 通过块级消融分析，研究了不同编码器-解码器区域对分割性能和遗忘的影响。同时，通过受控适应实验，探讨了遗忘在持续学习过程中与网络深度之间的关系。

Result: 研究发现，遗忘在编码器-解码器架构中受到网络深度中更新位置的影响很大。限制更新到瓶颈附近区域时，遗忘仍然有限，但一旦较浅的编码器和解码器变得可训练，即使只更新一小部分参数，遗忘也会急剧增加。

Conclusion: 本研究揭示了妇科图像分割中持续学习中的遗忘现象，为改进持续学习方法提供了新的思路。
...
TL;DR: Multiphase-Diff方法在处理高对比度、尖锐界面多相场时表现出优越性，为科学样本生成提供了有效解决方案。


<details>
  <summary>Details</summary>
Motivation: 为了解决高对比度、尖锐界面多相场中物理约束扩散所面临的三个耦合难题。

Method: 提出了一种名为Multiphase-Diff的方法，包括：保守通量残差、解析双射表示和雅可比预处理的似然函数。

Result: 在三个互补的多相基准测试中，Multiphase-Diff在物理和分布保真度方面优于七个基线，并表现出跨相对比度和组成的鲁棒性，证明了其在科学样本生成中的有效性。

Conclusion: Multiphase-Diff方法在处理高对比度、尖锐界面多相场时表现出优越性，为科学样本生成提供了有效解决方案。
...
TL;DR: PROVE是一种无需训练的、黑盒提示反转攻击，在图像相似度和文本图像对齐方面优于现有方法，为版权保护提供了新的解决方案。


<details>
  <summary>Details</summary>
Motivation: 现代文本到图像模型可以从自然语言提示中生成高度逼真的图像，而最近的提示反转进展使得从生成输出中恢复这些提示变得更加可行，引发了新的版权保护和内容所有权问题。

Method: PROVE（基于验证证据的提示恢复）是一种无需训练的、黑盒提示反转攻击，通过组合可验证的场景描述来重建提示，而不是优化标记序列，针对原始版权作品和AI生成内容。

Result: 在MS-COCO、Flickr30K和Lexica上，使用最先进的文本到图像生成器，PROVE在图像相似度（DINO、LPIPS）和文本图像对齐（CLIP）方面，始终优于基于优化的、基于字幕的以及基于强化学习的基线，无需任何训练、生成器访问或微调，证明了更强的、更实用的提示反转攻击。

Conclusion: PROVE是一种有效的提示反转攻击，可以重建提示，同时确保每个恢复的声明都有明确的图像证据支持，为版权保护和内容所有权提供了新的解决方案。
...
TL;DR: MedPlex通过将文本指导与视觉表示学习相结合，实现了医学图像分割性能的突破。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割通常被视为仅涉及视觉的问题，但临床解释往往依赖于解剖学、位置、外观和周围环境的文本知识。现有的基于视觉-语言模型（VLM）的文本引导分割方法通常只将语言用作晚期条件信号，限制了其对视觉表示学习的影響。

Method: MedPlex（医学plexus视觉和语言）是一个端到端的VLM框架，将文本引导作为分割学习的持续、基于临床的组件。通过Bi-Fusion（双向融合），视觉和文本表示在编码层次结构中共同演变。MedPlex还引入了类级别和区域级别的概念对齐，以在互补粒度上组织共享表示。类级别对齐将每个解剖目标锚定到一个聚合的临床概念配置文件，而区域级别对齐通过类特定的视觉证据保留个别概念，如形状、位置、外观和纹理。这样，语言在整个编码器中提供结构化监督，而不仅仅是在后期阶段作为提示。

Result: MedPlex在CT和MR基准测试中实现了多器官、心脏亚结构和肿瘤分割的最新性能，包括具有真实自由文本临床监督的设置。

Conclusion: MedPlex通过结合视觉和文本信息，显著提高了医学图像分割的性能。
...
TL;DR: 生成模型在缓解视觉任务数据稀缺方面显示出潜力，但在稀疏、高方差领域中的有效性有限。


<details>
  <summary>Details</summary>
Motivation: 为了解决视觉任务中的数据稀缺问题，基于扩散的生成模型取得了进展，这激发了使用合成图像生成的方法。然而，这种方法在像ImageNet这样的自然图像基准数据集上的有效性仍然不清楚，其在稀疏、高方差的真实世界领域中的有效性仍然不明确。本文关注那些图像与常见图像数据集差异很大的领域，并且在这些领域中获取额外数据成本很高。与不可生成的数据增强基线相比，我们评估了两种生成稀疏数据扩展方法的下游分类器性能提升：分布建模和样本扰动。在五个基于受试者划分的train--validation的创伤分类任务中，没有一种生成方法能够始终优于强大的非生成基线。特征空间分析揭示了反复出现的失败模式：记忆或崩溃，分布偏移，以及生成视觉上合理但简化的典型实例，这些实例比真实数据更容易分类。

Method: 我们评估了两种生成稀疏数据扩展方法的下游分类器性能提升：分布建模和样本扰动。通过五个创伤分类任务进行实验，对比了非生成基线的方法。

Result: 没有一种生成方法能够始终优于强大的非生成基线。特征空间分析揭示了失败模式：记忆或崩溃，分布偏移，以及生成简化典型实例。

Conclusion: 生成模型在稀疏、高方差的真实世界领域中的有效性有限，需要进一步研究以解决特征空间中的失败模式。
...
TL;DR: CAST是一种无需训练、无需图像的零样本学习框架，在标准基准测试中表现出色。


<details>
  <summary>Details</summary>
Motivation: 在许多领域，由于数据不可用，将大型预训练模型应用于新类别通常需要目标分布的示例。

Method: CAST（闭式分析语义迁移）是一种无需训练、无需图像的框架，通过权重注入将预训练分类器扩展到先前未见过的类别。

Result: 实验表明，CAST在标准零样本学习基准测试中与现有无图像方法相匹配或超过，并接近少量样本适应方法的性能，同时不需要迭代优化或目标分布的示例。

Conclusion: CAST为无样本学习提供了一种有前景的方法，通过利用辅助语义信息，可以有效地扩展预训练模型到新类别。
...
TL;DR: ChartProbe框架通过监督简单技能提升VLMs的复杂推理能力。


<details>
  <summary>Details</summary>
Motivation: 分析VLMs在图表问题上的推理能力不足，并探讨其根本原因。

Method: 提出ChartProbe框架，通过分析图表渲染代码来诊断模型在感知、定位、简单推理等技能上的缺陷。

Result: 在三个VLMs上，仅监督简单技能就能在未训练的复杂推理问题上取得显著提升。

Conclusion: 复杂视觉推理可以在没有复杂推理监督的情况下得到提升。
...
TL;DR: Kolmogorov-Arnold网络在土地分类中优于多层感知器，与随机森林相当。


<details>
  <summary>Details</summary>
Motivation: 土地分类对于土地管理、环境监测和城市规划至关重要。

Method: 使用Landsat 8影像评估Kolmogorov-Arnold网络在土地分类中的应用，并与随机森林和多层感知器模型进行比较。

Result: 在Calgary数据集上，Kolmogorov-Arnold网络与随机森林的精度相当，优于多层感知器，同时需要更少的可训练参数，并提供更高的可解释性。

Conclusion: Kolmogorov-Arnold网络在土地分类中具有潜在的应用价值。
...
TL;DR: XSA-MAD是一种基于CLIP的MAD方法，通过建模语义不一致性提高了变形攻击检测能力。


<details>
  <summary>Details</summary>
Motivation: Morphing attacks对人脸识别系统构成严重威胁，现有基于图像的MAD方法泛化能力差。

Method: 提出XSA-MAD，一个基于CLIP的多模态框架，显式地建模真实和变形人脸之间的语义不一致性。将变形概念分解为四个可解释的属性，包括身份、面部几何、纹理和一致性，并编码为结构化和属性感知的文本表示。图像编码器逐步与这种判别性文本空间对齐，从而产生统一的语义表示，捕捉真实和变形图像之间的生成不变和概念级差异。

Result: 在MAD22和MorDIFF上进行了实验，并在SMDD上进行训练，证明了XSA-MAD在多种变形原则上的强大泛化能力。特别是在基于GAN的变形上，XSA-MAD达到了2.92%的等错误率，并且在高保真生成攻击下始终优于现有方法。

Conclusion: XSA-MAD在MAD任务上表现出色，具有强大的泛化能力和对高保真攻击的鲁棒性。
...
TL;DR: 这项研究挑战了将可检测性视为单一属性的假设，并提出了一个两阶段模型来解释AI编辑的可检测性。


<details>
  <summary>Details</summary>
Motivation: 随着AI生成图像编辑的普及，旨在遏制由此产生的虚假信息的平台将可检测性视为一个单一、无差别的属性：编辑要么收到警告，要么不收到。

Method: 进行了一项控制眼动追踪研究（N=59，拉丁方设计，四个条件交叉编辑区域和语义合理性），混合效应分析显示，编辑是否被注意到以及是否被正确判断为虚假是两个不同的阶段，受不同因素的影响：编辑区域驱动注意力捕获（p<0.001），而语义合理性驱动判断准确性和看但未看到（LBFS）错误率（p<0.001）。

Result: 这种分离在纠正多重比较后仍然存在；两个因素之间的次要交互作用不存在。这种两阶段解释将视觉注意力研究中的长期区分（预注意捕获和努力识别）扩展到AI编辑可检测性的新领域。然后测试是否可以生成眼动模型来计算操作注意力捕获阶段：一个训练有素的Transformer用于生成扫描路径，对每张图像的注意力进行跟踪，具有强大的判别力（在保留的刺激上，皮尔逊r=0.77-0.82），并且在预测LBFS发生率的更困难的任务上，即使没有访问合理性标签，也适度优于两个参数的线性基线（r=0.52 vs. r=0.48）。

Conclusion: 这项研究挑战了将可检测性视为单一属性的假设，并提出了一个两阶段模型来解释AI编辑的可检测性。
...
TL;DR: 一致投票的LLM面板在有限的标记数据预算下发现最优网络架构。


<details>
  <summary>Details</summary>
Motivation: 在有限的标记数据预算下优化神经网络架构。

Method: 使用LLM进行架构辩论、代码实现、交叉审查和自动化验证训练循环。

Result: 发现了一个425K参数的编码器-解码器架构，在Thebe断层数据集上达到最高的F1和IoU值。

Conclusion: 一致投票的LLM面板是发现特定领域架构的实用且低成本的途径。
...
TL;DR: OpenBelief-Nav通过保留观察级别信息和独立性表示，在语言引导的导航任务中提高了成功率。


<details>
  <summary>Details</summary>
Motivation: 为了在语言引导的导航中提供紧凑的语义记忆，同时保留观察级别的短语、可靠性线索和帧掩码来源，同时保持单独的聚合几何和视觉表示。

Method: 提出了OpenBelief-Nav，一个保留观察级别短语、可靠性线索和帧掩码来源的同时维护独立聚合几何和视觉表示的物体记忆。

Result: 在ScanNet200和Replica场景上，全信念投影实现了mIoU分数分别为0.2742和0.2912，与早期提交读出的0.2393和0.2701相比。在78次HM3D-YCB导航试验中，共识和早期提交检索各达到60/78的成功率，而信念加权检索为58/78，双映射为55/78。在20次Unitree G1运行中，允许最多两个已验证候选尝试的纠正策略将目标确认成功率从6/10提高到8/10。

Conclusion: OpenBelief-Nav在保持语义记忆的同时，提高了导航任务的成功率。
...
TL;DR: 提出RGBX-Next，一种基于生成模型的3D渲染方法，提高渲染精度和控制能力


<details>
  <summary>Details</summary>
Motivation: 为了解决扩散模型在3D渲染中缺乏精确控制的问题

Method: 使用生成模型作为条件渲染器，基于传统渲染的G缓冲区进行学习

Result: 提出RGBX-Next，一个统一的正向和反向渲染生成框架，能够从图像、视频和流中估计G缓冲区，并从G缓冲区渲染出逼真的图像、视频和流

Conclusion: 该设计原则将有助于未来可控生成正向和反向渲染的研究
...
TL;DR: CoANeRV是一种高效的视频表示方法，通过坐标感知的token空间框架提高了重建和编码性能。


<details>
  <summary>Details</summary>
Motivation: 提高视频表示的效率和可扩展性，避免昂贵的视频优化和特定权重生成。

Method: 提出CoANeRV，一个坐标感知的token空间框架，用于视频表示。

Result: CoANeRV在多个视频数据集上表现出色，提高了重建质量，减少了内存峰值，并提供了高效的编码。

Conclusion: CoANeRV是一个有效的视频表示方法，具有高效的重建和编码性能。
...
TL;DR: 提出STN数据集和CMCNet模型，提高甲状腺结节诊断的准确性和数据效率。


<details>
  <summary>Details</summary>
Motivation: 为了解决深度学习方法在甲状腺结节诊断中多类预测和特征级别监督的不足，以及标注数据有限的问题。

Method: 提出STN数据集，包含600个甲状腺结节的横纵面超声图像、边界框注释和五个TI-RADS特征类别的完整标签，并引入CMCNet模型进行嵌入对齐。

Result: 实验结果表明，这种嵌入对齐策略比直接的多任务学习更高效、更稳健，并且在不平衡设置中始终优于InfoNCE、中心损失、一个强大的多任务基线和VQA风格的跨模态模型。

Conclusion: 通过引入STN数据集和CMCNet模型，提高了甲状腺结节诊断的准确性和数据效率。
...
TL;DR: 提出了一种基于几何分解和低秩特征的高速隐式光场表示方法，提高了重建质量和效率。


<details>
  <summary>Details</summary>
Motivation: 快速重建密集光场面临挑战，因为光场是具有强空间-角冗余和结构差异变化的维数高的信号。

Method: 提出了一种基于几何分解和多分辨率低秩特征的高速隐式光场表示方法。

Result: 在公共光场数据集上的实验表明，该方法在重建质量方面具有竞争力，同时提供了更好的模型参数、训练时间和推理效率之间的权衡。

Conclusion: 该方法为快速光场重建提供了一种有效解决方案，具有较好的性能和效率。
...
TL;DR: 提出了一种新的颜色恒常性框架，在纯色场景中显著提高了性能。


<details>
  <summary>Details</summary>
Motivation: 纯色场景中的颜色恒常性具有挑战性，因为大多数像素共享狭窄的色调范围，导致基于色度学的线索都汇聚到一个点上，标准估计器变得模糊不清。

Method: 提出了一种紧凑的框架，结合了两种创新：(i) SAFE，一种场景感知特征调制网络，将照明线索组织成一个结构化的四元组表示，然后根据场景复杂度特征进行选择性重新加权；(ii) 学习到的颜色空间（LCS），一种场景相关的色度归一化，直接解决纯色场景中的色度崩溃问题。

Result: 实验结果表明，SAFE在纯色场景中始终提高了性能。与每个指标上表现最佳的基线相比，它将平均角度误差降低了10%，将最佳25%误差降低了20%，将最差25%误差降低了5.8%。

Conclusion: SAFE在纯色场景中提高了颜色恒常性的性能，优于现有基线。
...
TL;DR: PPOM通过减少对补丁网格对齐的敏感性，提高了基于提示的学习性能。


<details>
  <summary>Details</summary>
Motivation: Prompt tuning的预测对冻结视觉Transformer的空间采样敏感，特别是非重叠补丁分块令预测依赖于图像与补丁格子的对齐（相位）。

Method: 提出Patch-Phase Orbit Marginalization (PPOM)，一种无需训练的推理算子，将相位偏移视为一个干扰变量。

Result: PPOM提高了主机的性能，无需重新训练。

Conclusion: PPOM提供了提示适应和补丁网格敏感性之间的确定性接口。
...
TL;DR: 提出了一种新的辅助条件增强框架，显著提高了RGB图像的零样本异常检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有的基于RGB图像的零样本异常检测方法在感知几何变形、深度变化或细微表面变化等异常方面存在局限性。

Method: 提出了一种可插入的辅助条件增强框架，用于零样本异常检测。该框架使用辅助观察作为条件信号来细化RGB特征，并通过轻量级元学习模块生成自适应的低秩残差更新。此外，还构建了不确定性感知的空间调制，以确定局部残差更新的加强或抑制。

Result: 在MVTec 3D-AD和Eyecandies数据集上进行了大量实验，表明该框架在多模态零样本异常检测方面取得了最先进的性能。

Conclusion: 该框架有效地提高了基于RGB的零样本异常检测器的性能，实现了多模态零样本异常检测的新突破。
...
TL;DR: ProFocus通过逐步视觉聚焦建模艺术图像情感，实现更精准的情感解读。


<details>
  <summary>Details</summary>
Motivation: 情感智力中图像情感反应的解读至关重要，而艺术图像的情感解读更具挑战性，因为现有方法依赖于通用的视觉嵌入模型，无法捕捉艺术情感中的细微线索。

Method: 提出了一种名为ProFocus的新型框架，该框架通过逐步视觉聚焦来建模艺术图像中的情感体验。它包含两个核心组件：分层艺术评论员（HAC）和逐步提示融合（PHF）模块。HAC利用多模态大型语言模型生成三个认知层次的有序语言先验，PHF则通过将分层提示逐步注入视觉特征来促进融合，实现逐步聚焦过程，使模型能够捕捉细微的情感线索，并产生更忠实于人的解释。

Result: 在ArtEmis v1.0和v2.0数据集上的大量实验表明，ProFocus在情感识别和情感解释方面均优于现有方法。

Conclusion: ProFocus是一种有效的艺术图像情感解读框架，能够实现更精细的情感捕捉和解释。
...
TL;DR: FIRM是一种用于精细MLLM分割的掩码内标记表示方法，在遥感图像分割基准测试中表现出色。


<details>
  <summary>Details</summary>
Motivation: 为了解决将隐式指令翻译成精确像素级掩码的问题，需要多模态大型语言模型（MLLMs）。在遥感图像中，小目标、细结构和相邻实例可能占据相同视觉标记的不同部分，导致单一掩码标签丢失内部空间结构，使附近目标合并，物体边界变得粗糙。

Method: 提出了FIRM，即掩码的精细内标记表示。FIRM为每个视觉标记预测一个掩码代码，该代码指定一个r×r二进制子单元格模式，而不是单个前景/背景标签。然后使用固定查找将预测的代码转换为离散的子单元格掩码，并通过边缘化代码分布获得软结构场。此外，引入了一个轻量级的连续渲染器，使用预合并的视觉特征和图像细节细化该场，以恢复每个子单元格内的精细边界。

Result: 在五个推理和引用分割基准测试中，FIRM在卫星和无人机图像上实现了领先的结果，包括LaSeRS上的70.5/80.5 gIoU/cIoU和EarthReason上的3.0点平均增益。

Conclusion: 这些结果表明，对于精细的MLLM分割，显式表示内标记掩码模式具有价值。
...
TL;DR: 提出了一种新的手术视频理解方法，在多个数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 理解手术视频需要长时序推理，现有方法处理不佳，我们提出了一种新的方法。

Method: 构建一个分离推理和感知的智能体，通过进化上下文来提高性能。

Result: 该方法在多个数据集和评估维度上优于现有方法，尤其是在长时序神经外科视频中。

Conclusion: 提出的方法在理解手术视频方面取得了显著成果，为长时序推理提供了新的思路。
...
TL;DR: Developed a system for esports-style commentary on gameplay recordings


<details>
  <summary>Details</summary>
Motivation: Live game commentary is scarce

Method: Content-based video narration system using VLM and text-to-speech

Result: Esports-style commentary for arbitrary gameplay recordings

Conclusion: System supports cloud TTS or on-device TTS model, released as a reproducible baseline
...
TL;DR: ForgeWM是一种高效的、可控的多步视频生成框架。


<details>
  <summary>Details</summary>
Motivation: 为了实现低延迟因果生成和可靠的游戏原生控制响应。

Method: ForgeWM通过领域适应、教师强制因果训练、因果一致性蒸馏和双向教师策略匹配来实现。

Result: ForgeWM在Minecraft轨迹上优于其他系统，同时实现了最低的参考LPIPS。

Conclusion: ForgeWM是一个有效的可控多步视频生成的框架。
...
TL;DR: 提出 SSP 评估框架，以更准确地评估 VLA 模型在自动驾驶中的应用。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法往往使用独立选择的合成、模拟和物理数据，因此测量的性能差距可能受到场景内容变化的影响，而不是真正的领域敏感性。

Method: 提出了一种名为 SSP (Synthetic-Simulation-Physical) 的事件匹配 Syn2Sim2Phy 评估框架，将跨领域比较锚定到相同的安全关键交互。从合成长尾视频开始，SSP 建立了一个经过验证的事件规范，该规范保留了道路拓扑、参与者角色、相对运动、冲突演变、超车顺序、响应约束和事件阶段。然后在 CARLA 和封闭试验场构建平台特定的实现，并在转移审计确认保留必需事件属性后进行评估。SSP 将来自 OpenEMMA、LLaViDA 和 Alpamayo-R1 的异构输出映射到公共语义槽和 1 秒轨迹窗口，以评估输出有效性、语义准确性、关键交互识别、轨迹质量和风险响应。

Result: 在合成、模拟和物理领域，宏观平均集成 VLA 能力得分分别为 0.259、0.291 和 0.325，而最佳领域因场景而异。Alpamayo-R1、OpenEMMA 和 LLaViDA 分别获得 0.405、0.338 和 0.131 的分数。SSP 提供了一个可重复的场景迁移链和未经假设物理领域普遍优越的证据合格评估 VLA 行为。

Conclusion: SSP 提供了一种新的评估框架，可以更准确地评估 VLA 模型的性能，并有助于提高自动驾驶系统的安全性。
...
TL;DR: E-S2Feat框架通过脉冲神经网络优化局部特征学习，在保证精度的同时，显著提高了能耗效率。


<details>
  <summary>Details</summary>
Motivation: 为了解决事件驱动局部特征方法在稀疏性、噪声和纹理限制下的鲁棒性不足，以及在不具备丰富资源平台（如无人机）上的精度和能耗平衡问题。

Method: 提出了一种名为E-S2Feat的基于脉冲神经网络的框架，用于事件驱动的局部特征检测和描述。该框架从特征表示和选择的角度联合优化局部特征学习。首先，通过模块特定的脉冲激活机制，在低比特、节能的推理下保留细粒度结构线索和判别信息，从而提高整体表示保真度。其次，通过语义引导的特征调制机制，利用语义先验来细化关键点响应分布，增强局部描述符的判别能力，从而引导模型提取具有更高几何稳定性和更强判别能力的局部特征。

Result: 在ECD和EDS数据集上的实验表明，该方法在姿态估计精度方面显著优于SuperEvent等基线方法。同时，在理论计算能耗效率方面，该方法实现了大约4.8倍的提升。在TUM-VIE数据集上的视觉惯性里程计实验进一步验证了该方法在完整SLAM系统中的有效性和实际应用潜力。

Conclusion: E-S2Feat框架在事件驱动的局部特征检测和描述方面表现出色，在保证精度的同时，显著提高了能耗效率。
...
TL;DR: 本文提出了一种新的视频生成方法，通过将MLLM与DiT结合，提高了视频生成的语义对齐和时间一致性。


<details>
  <summary>Details</summary>
Motivation: 为了填补将MLLM与DiT集成到视频生成中的空白，本文系统地研究了如何将MLLM与DiT集成，以回答三个问题：什么中间表示应该连接MLLM和DiT，MLLM应该如何生成它，以及DiT如何在扩散渲染中结合它。

Method: 本文提出了BiVidGen，一个混合框架，其中MLLM首先生成语义视觉标记，DiT通过多层交叉注意力在文本和这些标记的条件下渲染视频。

Result: 实验结果表明，BiVidGen在VBench-Long上提高了语义对齐和时间一致性，比微调的DiT基线性能更强。

Conclusion: 本文的研究表明，基于MLLM的显式视觉规划为文本到视频生成提供了一种有效的中间接口，超越了仅文本条件。
...
TL;DR: 提出了一种新的图像到图像转换方法，在真实感和忠实度方面优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 提出一种源无关框架，在反向扩散过程中动态细化二值掩码。

Method: 计算预训练扩散模型在每个潜在时间步的预测差异，引入基于预测差异的经验均值和标准差的时变统计阈值方案。

Result: 在AFHQ和Celeba-HQ数据集上，该方法在真实感（FID，KID）和忠实度（SSIM，LPIPS）方面优于最先进的无监督图像到图像方法。

Conclusion: 该方法通过仅需要目标域的预训练模型，实现了精确、自动化的定位和跨不同源分布的无缝转换。
...
TL;DR: Identified and addressed limitations in existing methods for understanding spurious correlations in deep learning models.


<details>
  <summary>Details</summary>
Motivation: Deep learning models trained on datasets with spurious correlations may rely on shortcut features that do not generalize.

Method: K-means and non-negative matrix factorisation for grouping shortcut and task contribution maps, visualization through contribution maps and representative examples.

Result: Discovered shared and distinct spatial patterns, enabled targeted inspection, proposed an intervention approach.

Conclusion: Existing methods may mask recurring spatial shortcut patterns, our approach can address this limitation.
...
TL;DR: 本研究提出了一种三维地震体相分割的新方法，通过比较和基准测试，为未来研究提供了基线。


<details>
  <summary>Details</summary>
Motivation: 地质体相分割在地球物理学中是一项重大挑战，需要稳健的方法和系统来有效地识别地质上相似的体相，同时数据标签有限。

Method: 本研究提出了一种基于体素的三维地震体相分割的比较和可重复基准，该基准基于公开可用的地震数据集，包括荷兰F3和Parihaka数据集，具有标准化的数据拆分和评估指标。通过评估三种代表性的现代三维分割架构，建立了强大的基线结果，突出了该领域未来研究的潜力和挑战。

Result: 建立了强大的基线结果，突出了该领域未来研究的潜力和挑战。

Conclusion: 三维地震体相分割是一个具有挑战性的领域，需要进一步研究以解决现有方法的局限性。
...
TL;DR: InstructVVT：一种基于Diffusion Transformer的视频虚拟试穿框架，无需空间先验，提高试穿效果。


<details>
  <summary>Details</summary>
Motivation: 针对视频虚拟试穿任务中存在的挑战，如对空间先验的依赖、视觉上下文压缩以及重建目标无法完全捕捉试穿特定的人类偏好。

Method: 提出了一种基于Diffusion Transformer的指令驱动和参考引导的视频虚拟试穿框架InstructVVT，该框架无需推理时间空间先验。通过双级参考条件化方案直接从输入三元组（源视频、参考服装和指令）中恢复细粒度控制。

Result: 在ViViD-S和TripVVT-Bench数据集上进行了大量实验，结果表明InstructVVT在服装保真度、结构保留和时间一致性方面优于现有开源方法，尽管需要更少的推理时间控制。

Conclusion: InstructVVT是一种有效的视频虚拟试穿框架，能够提高服装保真度、结构保留和时间一致性，同时减少对推理时间空间先验的依赖。
...
TL;DR: Owner3D：一种无训练的3D局部风格化框架，有效减少外观泄漏。


<details>
  <summary>Details</summary>
Motivation: 针对大型重建模型中3D风格化的挑战，提出了一种无训练框架Owner3D。

Method: Owner3D通过所有权引导的风格写入，将局部外观控制直接集成到LRM重建过程中，并引入边界双重槽以解决外观模糊问题。

Result: 实验结果表明，Owner3D在目标区域风格保真度和非目标外观保持方面优于现有方法，降低了外观泄漏。

Conclusion: Owner3D是一种有效的3D局部风格化框架，能够提高风格化效果并减少泄漏。
...
TL;DR: CoDS enhances perception by combining multi-agent info, joint training, and advanced feature extraction techniques.


<details>
  <summary>Details</summary>
Motivation: Collaborative perception breaks through single-view limitations via multi-agent information exchange.

Method: Joint training of detection and BEV segmentation; Collaborative Reliability Map (CoRM); Semantic Mixture-of-Experts (S-MoE) module; Bidirectional Task Complementary Interaction (BTCI).

Result: Our CoDS surpasses existing baselines on both tasks and maintains stable robustness under multi-source noise.

Conclusion: Proposes a robust Collaborative perception framework with expert-driven Detection and BEV Segmentation (CoDS) to enhance feature quality and perception performance.
...
TL;DR: 提出了一种新的标量场编码方法，通过使用各向异性的高斯基元和有效的预算分配，显著提高了编码效率和压缩性能。


<details>
  <summary>Details</summary>
Motivation: 科学模拟中，标量体积的产生速度通常快于存储、传输和加载的速度，而就地减少必须使用有限的模拟资源。

Method: 将标量场编码为各向异性的高斯基元，并在固定预算下进行。从局部场结构中分析地分配完整的基元集，包括位置、方向和形状，然后直接针对标量场进行细化，而不进行致密化、修剪或计数更改。选定的预算决定了细化前的编码存储，以及与迭代调度一起提供了可控制的细化时间预算。

Result: 在受控基准测试中，截断感知场评估将编码时间减少了高达51倍；在单个桌面GPU上，最多用四分钟将1.4百万个高斯函数编码到一个十亿体素体积中，减少迭代细化在一分钟内完成。在跨越2.1百万到11亿评估体素的五个数据集中，压缩有用的配置在压缩比从2.2倍到超过40,000倍的情况下实现了15.0-38.7 dB PSNR。预编码结构统计描述了单次分配从额外容量中获得有限收益的字段。由于基元保留了标量属性而不是烘焙外观，单个紧凑模型为每个后续可视化状态服务 - 支持后处理转移函数、颜色图、光照和视点更改，而无需重新编码。

Conclusion: 该方法有效地提高了标量场的编码效率，并实现了高压缩比和快速处理速度。
...
TL;DR: HiCo-GS 提高了城市尺度新型视图合成的质量


<details>
  <summary>Details</summary>
Motivation: 城市尺度新型视图合成的可扩展表示

Method: HiCo-GS，包含两个互补模块：跨层级上下文聚合（CLCA）和深度-法线几何一致性（DNGC）正则化

Result: 在真实世界和合成城市基准测试中实现最先进的渲染质量和更干净的几何形状

Conclusion: HiCo-GS 是一种高保真重建框架，有效解决了现有方法的跨层级特征隔离问题，提高了城市尺度新型视图合成的质量
...
TL;DR: SPARGen：一种统一的多模态框架，提高了空间感知和推理的性能。


<details>
  <summary>Details</summary>
Motivation: 为了解决现有方法在空间感知和推理中独立处理几何结构、建立对应关系和理解空间关系的问题，以及限制互补表示之间的知识迁移。

Method: 提出SPARGen，一个统一的多模态框架，将3D重建、密集对应和空间推理视为指令条件生成任务。将紧凑的结构化和语言输出序列化，同时以图像对齐的形式生成密集几何场，使空间监督能够在原生多模态生成模型中共同塑造共享表示。

Result: 在3D重建、对应和空间推理的基准测试中，SPARGen在单一原生多模态生成框架内实现了跨异构空间任务的竞争优势。

Conclusion: SPARGen是一种有效的方法，能够提高空间感知和推理的性能，同时促进互补表示之间的知识迁移。
...
TL;DR: PISA显著提高了OVOD-TTA方法的性能，在COCO-C上的AP@50%超越了现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有源无关的OVOD-TTA方法在遇到图像域变化时，性能下降严重，且初始预测较差时，精度会显著降低。同时，现有的源域估计方法在分类任务中恢复的抽象、稀疏表示不适合检测任务，无法捕捉到密集、具体的特征。

Method: 提出了一种新的源无关的OVOD-TTA方法PISA，通过 Corruption-Invariant Feature Extractor (CIFE)，Feature Alignment Module (FAM) 和多尺度对齐框架 (BAA) 等技术来提高检测性能。

Result: PISA在VOC-C，COCO-C和LVIS-C等基准测试中取得了显著的性能提升，特别是在COCO-C上的AP@50%达到了最先进的水平，超越了现有方法。

Conclusion: PISA显著提高了原始模型的定位精度和类别识别准确率，且无需访问源域数据，在COCO-C上的AP@50%达到了最先进的水平，超越现有方法3.92%.
...
TL;DR: S$^2$VOPD is a novel method for visual on-policy distillation, achieving significant performance improvement


<details>
  <summary>Details</summary>
Motivation: Visual on-policy distillation heavily relies on informative teacher-student asymmetry, but where can this asymmetry come from when no privileged information is available?

Method: Self-Supervised Visual On-Policy Distillation (S$^2$VOPD) that constructs on-policy learning signals from asymmetric augmented views

Result: S$^2$VOPD improves Qwen3.5-4B from 70.7% to 77.4%, surpasses GPT-5.4, and recovers 96% of the improvement achieved by methods with privileged information

Conclusion: S$^2$VOPD is a simple yet effective method for visual on-policy distillation, achieving significant performance improvement
...
TL;DR: CSG-Mamba是一种高效的息肉分割模型，在多个数据集上取得了优异的性能。


<details>
  <summary>Details</summary>
Motivation: 准确分割息肉对于计算机辅助结肠镜检查至关重要，但内镜图像往往包含低对比度边界、黏膜纹理干扰、镜面反射和高设备依赖性外观变化。

Method: 提出了一种名为CSG-Mamba的卷积评分门控视觉状态空间网络，用于内镜息肉分割。该网络基于VM-UNet风格的非对称U型编码器-解码器，在语义丰富的瓶颈处插入了一个卷积评分门控（CSG）模块。CSG通过点卷积和大核深度卷积生成局部空间评分图，并通过乘法门控重新校准状态空间特征。

Result: 在Kvasir-SEG上，CSG-Mamba实现了0.9220 Dice和15.87 HD95，在CVC-ColonDB上实现了0.7418 Dice和0.6570 mIoU，在大多数重叠和召回率指标上优于基线，同时保持了有竞争力的边界精度。

Conclusion: CSG-Mamba在息肉分割任务中表现出色，提高了分割精度和效率。
...
TL;DR: SCVIB场景下，TT-VG有效提升了视觉实例绑定定位的准确率。


<details>
  <summary>Details</summary>
Motivation: 提高多轮个性化定位中查询方同实例定位的准确性。

Method: TT-VG（转换树视觉定位）方法，结合目标状态转换树（TSTT）和视觉证据定位适应性（VEGA）。

Result: TT-VG在匹配的目标分辨率下，比最强的比较方法高出16.20个百分点。

Conclusion: TT-VG方法有效提高了SCVIB场景下的视觉实例绑定定位的准确率。
...
TL;DR: 提出CoG方法，提高文本到图像扩散模型的性能和可靠性。


<details>
  <summary>Details</summary>
Motivation: 解决文本到图像扩散模型的两个主要缺点：缺乏连续、概念特定的指导机制和缺乏高局部一致性的可靠性。

Method: 引入概念互信息的新概念，并利用网络中不同层的概念依赖性差异，增强概念相关层在概念引导（CoG）中的作用。CoG首先量化每层的概念特定影响，然后使用跳过概念相关层的预测的加权组合进行去噪。

Result: 在各种目标和流行模型（如PixArt-alpha、SD3、SD3.5和FLUX.1-dev）上实现性能提升。

Conclusion: CoG方法提高了文本到图像扩散模型在精确控制和局部一致性方面的性能。
...
TL;DR: RankT2I automates semantic discovery in T2I models, enhancing efficiency and effectiveness.


<details>
  <summary>Details</summary>
Motivation: Identifying semantics in T2I models is challenging and time-consuming.

Method: RankT2I, a novel, training-free, and model-agnostic framework for automating semantic discovery in T2I models.

Result: Efficiently identifies a wide range of semantics for T2I editing models across domains, outperforming existing methods.

Conclusion: RankT2I provides a novel approach to automate the discovery of editable semantics in T2I models, improving efficiency and effectiveness.
...
TL;DR: 本研究提出AppleScab-LT数据集，用于监测苹果黑星病进展，为作物健康监测提供可靠资源。


<details>
  <summary>Details</summary>
Motivation: 有限的长序列数据集限制了在自然田间条件下疾病进程的监测。

Method: 构建了AppleScab-LT纵向实地数据集，通过重复观察个别跟踪的感染叶片来监测苹果黑星病进展。

Result: 数据集包含21个纵向叶片序列、2,101张高分辨率图像和264个重复监测的感染叶片的渐进时间样本。

Conclusion: AppleScab-LT为时间疾病智能、疾病进程建模、精准农业和未来作物健康监测提供了一个可靠资源。
...
TL;DR: 提出了一种新的零样本基于骨骼的动作预测任务，为现实系统中新型动作泛化提供了新的解决方案。


<details>
  <summary>Details</summary>
Motivation: 解决现有基于骨骼的动作预测方法在真实场景中遇到的新动作识别问题

Method: 提出了一种新的零样本基于骨骼的动作预测任务（ZS-SkAA），包括：
1. 一个由时空特征提取器和互信息估计与最大化模块组成的基线模型。
2. 使用NTU RGB+D数据集的基准协议，用于严格的ZS-SkAA评估。

Result: 实验表明，该模型在NTU RGB+D数据集上实现了高零样本准确率，为ZS-SkAA提供了一种强大的基线方法，并确立了ZS-SkAA作为现实系统中新型动作泛化的重要研究方向。

Conclusion: ZS-SkAA是现实系统中新型动作泛化的重要研究方向。
...
TL;DR: 通过Endo-C6基准测试，我们发现轻量级的小样本调整可以显著提高TVLMs在特定内镜腐蚀下的鲁棒性，并促进临床视觉语言系统的标准化鲁棒性报告。
...
Method: We introduce Endo-C6, a compact corruption benchmark of six endoscopy-realistic perturbations evaluated at a fixed high severity, and apply it to public Gastrointestinal (GI) endoscopy and laparoscopic cholecystectomy videos. Under a standardized prompt protocol, we benchmark 3 recent surgical TVLM baselines and analyze robustness in both mean and worst-case settings, spanning 294 dataset-level evaluations.

Result: Our findings show that off-the-shelf TVLMs can exhibit severe worst-case collapse under endoscopy-specific corruptions, whereas lightweight few-shot adaptation can substantially improve corrupted performance and robustness without changing the prompt-based interface. We expect Endo-C6 to support standardized robustness reporting and promote more reliable clinical vision-language systems.

Conclusion: Lightweight few-shot adaptation can significantly improve the robustness of TVLMs under endoscopy-specific corruptions, and Endo-C6 can promote standardized robustness reporting for clinical vision-language systems.
...
TL;DR: 本研究分析了VLMs在处理文本图像时的视觉风格偏差，发现其对VLMs的解释有显著影响。


<details>
  <summary>Details</summary>
Motivation: 研究视觉语言模型（VLMs）如何处理视觉和文本信息，以及视觉风格偏差的影响。

Method: 引入Stealth Visual Prompts，控制文本的视觉风格，并测量对VLMs分析的影响。

Result: 发现VLMs对文本中存在的负面词汇处理不当，且视觉风格偏差会影响VLMs的语义理解。

Conclusion: 视觉风格对VLMs的解释有指导作用，可能导致与人类语义理解不同的结果。
...
TL;DR: MAGneT-3D：一种针对单目视频的域泛化3D目标检测新方法。


<details>
  <summary>Details</summary>
Motivation: 针对基于查询的3D检测器在单目视频中的应用局限性，如难以泛化到未见过的数据集和环境。

Method: 提出MAGneT-3D方法，包括DRAG和TRIM策略，以及跨数据集基准测试。

Result: 在零样本域迁移下，MAGneT-3D优于所有基线，NDS从12.1%提升到18.6%，同时域内准确率也提高。

Conclusion: MAGneT-3D是一种有效的域泛化单目时序3D目标检测方法。
...
TL;DR: Developed a neural network model for efficient floor plan design and energy-efficient building design.


<details>
  <summary>Details</summary>
Motivation: Enable efficient floor designing and determine the number and type of lights needed per floor.

Method: Developed a neural network-based model using Mask RCNN, with images annotated and converted into Coco data format for training.

Result: Achieved bbox_mAP and segm_mAP values of 0.7596 and 0.7111, respectively. Model performs well at different IoU thresholds. Helps various industries, such as architecture and construction, to improve design time and create efficient workflows.

Conclusion: The developed model aids in efficient floor plan design and energy-efficient building design by automatically detecting MEP objects from floor plans.
...
TL;DR: TRIAGE是一种基于半监督学习的OCT扫描分类框架，在少量标记数据的情况下，实现了高准确率。


<details>
  <summary>Details</summary>
Motivation: 为了解决高级视网膜疾病诊断成像模态OCT在自动化方面的不足，以及由于专家标注成本高昂导致的标注不足问题。

Method: 提出了一种名为TRIAGE的风险控制半监督框架，该框架结合了患者级符合性风险控制器、不对称成本矩阵、分层分类器、患者分组符合性风险控制器、原-对偶覆盖控制和上下文感知Transformer教师。

Result: 在Noor Eye Hospital数据集上，TRIAGE在仅使用20%标记数据的情况下，实现了89.66%的扫描级准确率、0.8805的宏-F1、0.9641的宏-AUC和8.34%的下级评分率。在仅使用5%标记数据的情况下，TRIAGE保持了76.88%的准确率和0.1656的下级评分率。与六种最先进的半监督方法相比，TRIAGE在降低下级评分率方面表现显著，提高了42.7%。在OCT-C8数据集上，使用1%标记数据进行3类分类时，TRIAGE实现了98.00%的准确率，使用10%标记数据进行8类分类时，实现了95.94%的准确率。

Conclusion: TRIAGE是一种有效的半监督框架，可以显著提高OCT扫描分类的准确性和效率。
...
TL;DR: CREST是一种新的弱监督语义分割框架，在极地低压检测和分割任务中表现出色。


<details>
  <summary>Details</summary>
Motivation: Polar lows的快速检测和像素级分割是高纬度地区海洋气旋研究中的挑战。

Method: 提出了一种名为Constrained Region Erasing with Soft Targets (CREST)的弱监督语义分割(WSSS)框架，该框架仅从图像级标签生成掩码。

Result: 在Sentinel-1 SAR数据上，CREST比标准AER更接近气旋结构，并返回一个多类掩码，其中类表示分配给每个区域的可靠性。在BUS-UCLM乳腺超声和PASCAL VOC人体数据集上，CREST在相同设置下优于等效的AER流程。

Conclusion: CREST在检测和分割极地低压方面表现出色，优于传统的AER方法。
...
TL;DR: 提出GBU-Palm，解决现有PAD数据集的限制，进行多模态视频数据集和基准测试，发现环境变化对架构性能影响显著，提供跨环境条件下的鲁棒PAD方法基准。


<details>
  <summary>Details</summary>
Motivation: 针对现有棕榈呈现攻击检测（PAD）数据集的限制，提出GBU-Palm数据集和基准。

Method: 构建包含21,326个视频的大规模多模态视频数据集，包括105个主体的210个棕榈，在不同环境下进行采集，建立泄漏控制的协议，对四种代表性的视频架构进行基准测试。

Result: 发现环境变化对架构性能有显著影响，RGB-NIR融合并不总是优于RGB-only输入。通过模型行为分析，揭示了不同的失败模式和证据利用。

Conclusion: GBU-Palm提供了一个统一且具有挑战性的基准，用于跨环境条件下开发和发展鲁棒的多模态棕榈PAD方法。
...
TL;DR: 提出IRGNN，一种新的雷达点云目标检测方法，在保持高性能的同时，降低计算成本和内存使用。


<details>
  <summary>Details</summary>
Motivation: 针对雷达点云数据在恶劣天气条件下可靠性降低的问题，以及雷达点云数据相较于激光雷达数据在稀疏性、无序性和信息量方面的不足，提出了一种新的雷达点云目标检测方法。

Method: IRGNN，一种不变雷达图神经网络，通过将雷达点云重构为图表示，并采用改进的消息传递神经网络和虚拟节点层，增强局部特征传播和全局上下文建模，最后应用特定任务的头进行目标分类和边界框预测。

Result: 在RadarScenes数据集上，IRGNN优于现有的雷达目标检测方法，并取得了具有竞争力的性能。同时，IRGNN在推理过程中显著降低了计算成本和内存使用，证明了其在自动驾驶中高效雷达感知的有效性和实际潜力。

Conclusion: IRGNN是一种有效的雷达点云目标检测方法，在保持高性能的同时，降低了计算成本和内存使用，具有实际应用潜力。
...
TL;DR: CRAFT: A novel framework for subject-driven image personalization achieving state-of-the-art performance without composed-target supervision.


<details>
  <summary>Details</summary>
Motivation: Subject-driven image personalization is a foundational capability for modern visual content creation.

Method: CRAFT (Constrained Reward via Attention Fine-Tuning), a single-step ReFL framework fine-tunes a pre-trained reference-aware MMDiT via LoRA adapters using compact reference-only data construction.

Result: State-of-the-art performance on XVerseBench with no composed-target supervision.

Conclusion: CRAFT is a novel approach for subject-driven image personalization, achieving high performance with compact reference-only data construction and without composed-target supervision.
...
TL;DR: 研究发现，使用冻结图像嵌入进行艺术史风格分类时，艺术家断开评估比传统随机分割评估更能反映对风格的真正理解。


<details>
  <summary>Details</summary>
Motivation: 研究冻结图像嵌入模型（如CLIP）在艺术史风格分类中的准确性是否反映了对风格的真正理解还是对个别艺术家的识别。

Method: 采用艺术家断开协议重新评估风格分类，在平衡的20世纪四个运动320幅画作数据集上测试，比较不同图像编码器的风格准确率。

Result: 在艺术家断开协议下，5-NN风格准确率从0.87下降到0.77，其中表现最差的是超现实主义，准确率下降了20个百分点。视觉结构比语言的影响更大。

Conclusion: 艺术家断开评估对于测量冻结嵌入中的风格理解是必要的。
...
TL;DR: 本研究提出了一种基于深度学习的框架，用于在史前手印中归因性别，通过集成模型和不确定性分析提高了准确性。


<details>
  <summary>Details</summary>
Motivation: 确定史前手印创作者的生物学性别是一个具有挑战性的问题，因为缺乏真实信息、当代和史前群体之间的种群差异以及图像退化带来的不确定性。

Method: 提出了一种基于深度学习的性别归因框架，该框架在整个分析流程中显式地建模、传播和汇总不确定性。方法结合了双重图像处理、双重轮廓提取、结构化轮廓增强、模型架构多样性和基于集成决策聚合。

Result: 在当代数据上，集成模型实现了强大的分类性能，老年年龄组的准确率超过88%。当应用于史前图腾时，该框架产生了性别预测和内部一致性置信度度量，使形态稳定和模糊案例之间的区分成为可能。

Conclusion: 不确定性可以成为考古推理的可测量组成部分，使古代岩画的稳健和可重复解码成为可能。
...
TL;DR: MagnifiQ是一种高分辨率图像恢复框架，通过渐进式提升和自适应卷积操作，实现了高质量的图像恢复。


<details>
  <summary>Details</summary>
Motivation: 高分辨率图像从退化输入中恢复是一个挑战，因为它必须在保持全局结构一致性的同时恢复细粒度局部细节，尤其是在4K分辨率下，直接基于扩散的恢复计算成本高且容易产生重复或不一致的纹理。

Method: 引入了MagnifiQ，一个图像恢复框架，该框架可以跨分辨率逐步提升和恢复图像，例如从1024x1024提升到4096x4096。该方法利用预训练的文本到图像扩散模型（如SDXL），并通过用卷积操作替换其原始的自注意力层来适应更可扩展的高分辨率推理。此外，还提出了一种渐进式提升策略，通过多个分辨率阶段的迭代恢复图像，而不是直接生成最终的4K图像，从而提高全局一致性并减少高分辨率伪影。为了在控制内容漂移的同时增强局部细节，MagnifiQ使用特定于补丁的文本提示，在恢复过程中提供空间定位的语义指导。

Result: 在合成和真实退化图像上的大量实验表明，MagnifiQ在感知质量和人类偏好方面优于之前的基于扩散的恢复方法，产生了更清晰的纹理和更一致的4K结果，同时通过其可扩展的骨干网络和渐进式设计提供了实用的速度-质量权衡。

Conclusion: MagnifiQ是一种有效的图像恢复框架，能够在保持全局结构一致性的同时恢复细粒度局部细节，尤其是在4K分辨率下，表现出色。
...
TL;DR: CPI-Bench是一个全面、实用和智能的基准测试，用于现实世界的图像编辑，能够有效区分不同模型的性能，为图像编辑模型的优化提供指导。


<details>
  <summary>Details</summary>
Motivation: 针对现有图像编辑基准测试的局限性，如简单单图像任务、覆盖维度有限以及无法有效区分不同模型性能的问题，提出CPI-Bench，一个全面、实用和智能的基准测试，用于现实世界的图像编辑。

Method: CPI-Bench包括三个核心子集：CPI-General-Bench（全面覆盖多样化的编辑任务，包括多图像编辑评估）、CPI-Practical-Bench（关注高频真实用户应用场景）和CPI-Intelligent-Bench（专注于评估基于高度需求的推理编辑能力）。

Result: 基于CPI-Bench对主流图像编辑模型的评估结果显示，CPI-Bench增强了模型间性能差异的区分度。它提供了对一般编辑能力、实际部署效率和高级推理编辑的全面和可靠的量化，为图像编辑模型的未来优化提供了宝贵指导。

Conclusion: CPI-Bench与Arena Image Edit Leaderboard的排名分析高度一致，表明它忠实捕捉了人类评估者的偏好和感知判断，作为现实世界用户体验的稳健代理。
...
TL;DR: 语义检索不一定总是更高效，自适应路由器更适合不同任务和模型。


<details>
  <summary>Details</summary>
Motivation: 质疑语义检索在令牌效率上的优势，研究不同检索方法的效果

Method: 形式化问题、五臂消融实验、映射失败模式到可测量变量、初步研究（Python和TypeScript仓库）

Result: 在符号定位和参考完整性任务上，语义检索通常成本较高且效率不高。

Conclusion: 自适应路由器是关键，基于任务类别、模型能力和词汇噪声进行选择。
...
TL;DR: SELR：一种结合效率和可解释性的自我解释潜在推理方法。


<details>
  <summary>Details</summary>
Motivation: 为了提高计算效率，将文字推理压缩成紧凑的嵌入，但压缩推理到潜在空间会使其变得不透明，难以解释。

Method: 提出了一种名为Self-Explainable Latent Reasoning (SELR)的统一框架，该框架通过多任务训练目标同时优化两个目标：(1)答案损失，优化潜在推理轨迹以产生准确的最终答案；(2)CoT损失，显式训练模型将自身潜在表示解码回人类可理解的推理步骤。

Result: 在大型语言模型（LLMs）和视觉语言模型（VLMs）上验证了SELR的有效性，结果表明SELR与基线相比，在标记效率和准确性方面具有优势，同时提供自包含的可解释性，无需辅助模型。

Conclusion: SELR是一种有效的自我解释潜在推理方法，提高了推理的效率和可解释性。
...
TL;DR: InflationAgent通过优化路由策略解决了语言模型中的token通货膨胀问题，提高了效率和准确性。


<details>
  <summary>Details</summary>
Motivation: 为了解决语言模型中由于重试和额外token消耗导致的实际成本与预期成本之间的差距问题。

Method: 提出了一种四阶段路由器，包括系统性地测量token通货膨胀、引入CoT Branching Entropy预测通货膨胀，并选择模型以最大化语义交换率。

Result: 在GSM8K测试中，InflationAgent实现了94.7%的准确性，比FrugalGPT少用31%的token，且在将失败推理链转发到GPT-4o时，其准确性降低了34.8个百分点。

Conclusion: InflationAgent 通过测量、预测和优化路由策略有效地解决了语言模型中的token通货膨胀问题，提高了效率和准确性。
...
TL;DR: BCMT架构通过块状因果记忆有效地解决了长序列处理中的复杂度问题。


<details>
  <summary>Details</summary>
Motivation: 为了解决Transformer架构在长序列中自注意力机制的二次复杂度问题。

Method: 提出BCMT（块状因果记忆Transformer）架构，通过块状局部交互和全局上下文传播解耦，并使用指数因果记忆。

Result: 在长上下文语言建模中，BCMT在验证性能上与密集Transformer相当，同时显著提高训练吞吐量和减少内存消耗。

Conclusion: 指数因果记忆是长上下文语言建模中有效替代密集全局注意力机制的方法。
...
TL;DR: Jais 2是阿拉伯语为中心的LLMs，在多个基准测试中表现出色，支持开放研究


<details>
  <summary>Details</summary>
Motivation: 推进以阿拉伯为中心的语言建模

Method: 开发Jais 2模型家族，包括70B参数和8B参数的变体，使用定制阿拉伯语词汇和优化架构进行训练

Result: 在阿拉伯语和文化基准测试中表现出色，包括诗歌、宗教、烹饪和梦境解释等，同时在翻译和摘要等通用任务中表现良好

Conclusion: Jais 2为阿拉伯语为中心的LLMs提供开放和高效的解决方案，支持进一步的研究和开发
...
TL;DR: IterCOMP是一种有效提升多跳问答效率和准确性的提示压缩框架。


<details>
  <summary>Details</summary>
Motivation: 多跳问答需要复杂的推理，现有方法无法有效处理，导致效率和准确性下降。

Method: 提出IterCOMP，一个统一的、无需训练的提示压缩框架，结合迭代压缩循环进行多跳推理。

Result: 在MusiQue、2WikiMultiHopQA和HotpotQA上，IterCOMP在精确匹配和F1分数上取得显著提升，同时减少token预算，优于现有基线，且随着推理复杂度增加表现出鲁棒性。

Conclusion: IterCOMP是一种有效的多跳问答提示压缩框架，能够提高效率和准确性，具有较好的鲁棒性。
...
TL;DR: 提出了一种新的方法评估LALMs的公平性，有效减少了虚假的公平性发现。


<details>
  <summary>Details</summary>
Motivation: LALMs在语音理解和音频问答等任务中应用广泛，但公平性问题引起关注。

Method: 提出语义感知混合效应回归框架进行公平性评估，考虑语义变化和说话者特征等混杂因素。

Result: 在模拟数据和真实世界基准测试中，该方法显著减少了虚假的公平性发现，并产生了更稳健和可解释的群体性能差异估计。

Conclusion: 该方法为LALMs的公平性评估提供了一种有效的方法，有助于减少模型偏差带来的误导性结论。
...
TL;DR: TeachMateGPT通过多智能体系统提高教科书评估项生成质量


<details>
  <summary>Details</summary>
Motivation: 减少科学教师的负担，提高评估项生成的质量

Method: 多智能体系统，包括知识库、检索生成流程、验证协议和课程数据集

Result: 提高了评估项的真实性和答案的相关性

Conclusion: TeachMateGPT是一个有效的系统，可以生成高质量的教科书评估项，减少教师工作量
...
TL;DR: StreamHear通过半监督方法提高了流式自动语音识别在领域迁移情况下的性能。


<details>
  <summary>Details</summary>
Motivation: 传统流式自动语音识别在领域迁移的目标音频上表现不佳，因为标注的领域内数据成本高，而未标注的音频丰富。

Method: StreamHear，一个半监督流程，通过在标注训练集上微调预训练的流式学生，在未标注部分生成伪标签，并在混合数据上微调学生。此外，引入了先验正则化的动态规划重新对齐步骤，使用ASR假设锚点来固定块级单词位置。

Result: 在四个数据集上，StreamHear在金融电话、准备好的阅读语音和电话质量对话中，一致优于监督学生微调，并缩小了与离线教师之间的差距。

Conclusion: StreamHear在处理领域迁移的流式自动语音识别任务中表现出色，能够有效地利用未标注数据提高性能。
...
TL;DR: 佛罗里达大学提交了基于检索增强的多样本翻译方法，用于低资源印地语翻译


<details>
  <summary>Details</summary>
Motivation: 描述了美国佛罗里达大学在WMT26低资源印地语翻译共享任务中的提交

Method: 将检索增强的多样本翻译管道从AmericaNLP 2026系统改编为翻译英语和十一种东北印度语言的双向翻译

Result: 在推理时，使用BM25检索语言特定训练库中最相似的平行示例，并使用Gemini 2.5 Flash在这些示例的基础上进行翻译。没有涉及模型微调。训练库结合了官方WMT26数据以及Samanantar和以前WMT共享任务发布等公开语料库。通过对所有22个语言方向对中的检索计数r和发展示例计数d进行网格搜索，为每个提交选择最佳配置

Conclusion: 该研究提出了一种基于检索增强的多样本翻译方法，用于低资源印地语翻译，并在WMT26低资源印地语翻译共享任务中取得了良好的效果
...
TL;DR: 推理导向训练未能优先放大最高影响力行为，而置信度校准是正确性的强烈信号，但未得到充分放大。


<details>
  <summary>Details</summary>
Motivation: 探讨推理导向训练对推理行为的影响，以及如何量化推理行为与正确答案之间的关系。

Method: 通过15个模型和6个基准进行标注和分析，比较不同推理行为对正确性的影响。

Result: 发现推理模型强烈放大自我纠正、假设检验和不确定性认可，但最高影响力行为是置信度校准、知识对齐和自我意识。

Conclusion: 推理模型中，推理行为与正确答案相关，但推理导向的训练并未优先放大最高影响力行为。
...
TL;DR: ASSERT提高了GenAI审计的透明度和可靠性。


<details>
  <summary>Details</summary>
Motivation: 为了解决生成式AI（GenAI）系统审计中报告率的变化难以归因和解释的问题。

Method: 引入ASSERT，一个基于规范的测量流程，将每个报告率与生成该报告率的测量选择的具体规范相关联。

Result: 通过对话设置、模拟用户、法官和非合规证据条等影响报告率，观察到报告率有显著变化。这些测量选择会显著改变报告率，并可能重新排序GenAI系统排名。

Conclusion: ASSERT有助于审计的归因和解释，提高了审计的透明度和可靠性。
...
TL;DR: 使用基于执行的监督和偏好强化学习，提高了多对多编程语言翻译的质量。


<details>
  <summary>Details</summary>
Motivation: 许多编程语言的代码翻译需要保持可执行性，但神经代码翻译主要关注C++、Java和Python等少数流行语言，导致多对多设置中并行监督稀疏，产生可理解的但不可执行的翻译。

Method: 使用基于执行的监督驱动的偏好强化学习，首先将可验证的Python程序扩展到多语言执行验证代码池，然后使用池生成跨语言对的翻译候选者，并按其执行结果进行标记。使用这些偏好来训练一个评分跨语言翻译质量的奖励模型。最后，使用奖励模型作为信号，通过GRPO优化基础LLM。

Result: 在HumanEval-X++和现有基准测试中，与未训练的基线相比，该方法在所有语言上实现了平均13%的改进，在中端语言上提高了21%。

Conclusion: 本研究建立了一种可靠的数据生成、培训和基准测试方法，为许多编程语言的多对多翻译质量进一步自举铺平了道路。
...
TL;DR: A new framework expands creative writing data beyond story generation, enhancing LLMs' ability to handle diverse formats.


<details>
  <summary>Details</summary>
Motivation: High-quality creative writing data for large language models (LLMs) remains dominated by story-centric data, limiting models' ability to follow the structural and functional conventions of diverse creative formats.

Method: Proposing an attribute-guided genre expansion framework, separating thematic breadth from genre-form control, using human-authored story prompts as diverse creative seeds, and manually curated genre attributes to enforce distinct structural, stylistic, and formatting conventions.

Result: Constructing the Multi-Genre Collection, a 50K-example corpus spanning 13 creative genres, demonstrating that models fine-tuned on this data surpass not only base models and writing-specialized baselines, but also models trained on existing writing corpora.

Conclusion: Genre-count ablations indicate that controlled genre expansion, rather than story-centric scaling alone, is a key driver of robust creative writing capability.
...
TL;DR: 提出了一种新的无训练自适应剪枝方法，显著提高了LRMs在批量推理中的性能和效率


<details>
  <summary>Details</summary>
Motivation: 为了解决大型推理模型（LRMs）在批量推理中性能下降的问题

Method: 提出了一种新的无训练自适应剪枝方法，该方法特别针对批量推理中的LRMs，包括周期性top-k选择和激活记忆机制

Result: 在多个推理基准测试中，该方法在平均准确率上比之前最先进的自适应剪枝方法提高了39.7个百分点，在50%的目标稀疏度下，速度比密集推理快1.40倍

Conclusion: 该方法有效提高了LRMs在批量推理中的性能和效率
...
TL;DR: S2Dialog：一种新的多模态对话检索框架，在DailyTalk数据集上表现优异。


<details>
  <summary>Details</summary>
Motivation: 多模态对话检索旨在从多模态对话库中检索与目标对话在文本语义和语音对话风格上相似的对话。

Method: 提出S2Dialog，一个统一的多模态对话语义-风格检索框架，包括对话级文本检索器和对话级语音检索器，以及对话级文本-语音对比学习。

Result: 在DailyTalk多模态对话数据集上的实验表明，S2Dialog实现了出色的检索性能。

Conclusion: S2Dialog在多模态对话检索方面取得了显著成果。
...
TL;DR: HERMES是一种可扩展的多智能体框架，用于从超长科学文档中提取结构化数据，提高了数据密集型学科的知识集成效率。


<details>
  <summary>Details</summary>
Motivation: 科学地质知识难以通过计算访问，因为它们大多被困在传统的专著和历史文献中，这些文献中的非结构化文本和复杂的布局阻碍了计算访问。

Method: 我们介绍了一个可扩展的多智能体框架HERMES，它可以从超长科学文档中提取结构化数据。使用协调的大型语言模型，HERMES在统一的文档级提取过程中整合了领域约束、验证规则和证据追踪，该过程包括解析文本、表格、图表和标题。

Result: 应用于55卷《无脊椎动物古生物学论文集》，该系统生成了一个包含32,277个化石分类实体和451,878个属性的数据库，并已在线发布。提取性能在化石组之间保持稳定（实体平均F1分数约为0.90，属性平均F1分数约为0.91），与测试的完全手动基线相比，每卷效率提高了大约六倍。在古地磁学和地球化学方面的评估，在没有额外模型训练的情况下，证明了跨不同地质科学领域的迁移性。

Conclusion: 这项工作为将历史科学文献转化为面向FAIR的标准化数据提供了实际途径，为数据密集型学科和大规模知识集成提供了可持续的基础设施。
...
TL;DR: 在MLC-SLM挑战中，提出的方法在说话人分离和识别以及对话语音理解任务中均取得显著效果。


<details>
  <summary>Details</summary>
Motivation: 评估多语言对话语音语言模型（MLC-SLM）在完整、未分割的多语言对话中的表现，包括说话人分离和识别，以及对话语音理解。

Method: 对VibeVoice-ASR-7B进行微调，采用随机静音裁剪、一致时间戳纠正和指数移动平均（EMA）训练策略；对Qwen3-Omni-30B-A3B-Instruct进行微调，构建合成问答对，进行多模态候选生成、静音音频过滤和分布匹配增强。

Result: 在Task 1上，裁剪将tcpMER从18.30%降低到17.27%，EMA进一步降低到16.73%；在Task 2上，应用分布匹配增强和标记直接回答将准确率从83.0%提高到86.0%。

Conclusion: 所提出的方法在MLC-SLM挑战的两个任务中都取得了显著的性能提升。
...
TL;DR: OPD在长文本推理教师和短文本学生模型之间转移推理能力，提高了数学推理表现，并在其他领域表现出泛化能力。


<details>
  <summary>Details</summary>
Motivation: 研究在长文本推理教师和短文本学生模型之间应用策略蒸馏（OPD）的挑战，并探索从长文本推理模型SU-01到短文本学生模型转移推理能力的方法。

Method: 通过在共享文本空间中进行OPD，仅对学生在教师分词器下占据相同文本跨度下的标记进行对齐；引入学生参考KL损失和掩码特殊终止标记（如</think>和<|im_end|>）的优势，以缓解生成长度过长和频繁截断的问题。

Result: 在数学推理，特别是自然语言数学证明方面，实验结果表明OPD在相同家族和不同家族的学生模型（如Qwen3、Qwen3.5、Intern-S2、GLM-4.7、Gemma-4）上表现出一致的增益。特别是，Intern-S2-Preview在ProofBench上提高了21.2分，达到55.2，超过了Gemini-2.5-Pro。

Conclusion: OPD可以转移推理能力，这些能力在数学训练领域之外也能泛化。
...
TL;DR: Envs-FORGE通过将验证器奖励转换为环境合成动作，提高了强化学习在终端智能体训练中的性能。


<details>
  <summary>Details</summary>
Motivation: 为了解决强化学习在终端智能体训练中需要可靠奖励和有用难度的可执行训练环境的问题，以及现有方法如few-shot、Self-Instruct和Evol-Instruct将相同的提示策略应用于每个种子，即使当前策略可能从更难、更容易或不同的任务中受益。

Method: 提出了一种名为Envs-FORGE的提示策略，该策略将验证器奖励转换为每个种子的环境合成动作。Envs-FORGE估计种子通过率，围绕目标学习前沿评分六个投影方向动作，并解决每个种子的混合整数线性规划（MILP）来选择条件生成动作。选定的动作驱动指令、夹具、Oracle解决方案、测试和Docker环境的同步重写；只有经过黄金验证的包才进入RL训练。

Result: 在Qwen 3.5 35B上，Envs-FORGE将tb-core的Pass@1从40.0%提高到49.2%，提高了9.2个百分点，将tb-2.0从23.0%提高到29.4%，提高了6.4个百分点，超过了最强的固定食谱基线。它在SWE-bench Verified上达到77.1%，而Base为73.4%，在评估的4B-35B模型中，tb-core提高了6.8-9.2个百分点。所有合成方法都导出100个验证环境，并使用2.27M-2.88M合成令牌，使比较处于相同的下游训练集大小和相同的操作规模。

Conclusion: Envs-FORGE在强化学习环境中提高了性能，为终端智能体的训练提供了更有效的环境合成方法。
...
TL;DR: 词汇多样性影响LLMs的内在维度，揭示了其组织原则。


<details>
  <summary>Details</summary>
Motivation: 探讨词汇多样性如何影响语言模型中内在维度的估计

Method: 分析词汇多样性对内在维度的影响，并推导出一个精确的公式

Result: 发现低词汇多样性时，较少的独特最终词产生较高的内在维度；高词汇多样性时，较多独特词产生较高的内在维度。推导出的公式与所有测试的尺度上的观察到的转变点相匹配。

Conclusion: 需要谨慎解释内在维度作为复杂性的直接线索，并揭示了语言数据在LLMs中的组织原则。
...
TL;DR: 本文对大型模型在体育领域的应用进行了综合调查，为体育智能研究与实践发展奠定基础。


<details>
  <summary>Details</summary>
Motivation: 体育在全球范围内越来越受欢迎，大型模型在体育领域的应用展现出巨大的潜力。

Method: 对大型模型在体育领域的应用进行综合调查，包括任务、应用、数据集和基准等。

Result: 为大型模型驱动的体育智能研究与实践发展奠定基础。

Conclusion: 大型模型在体育领域的应用具有巨大的潜力，但仍面临挑战，需要进一步研究。
...
TL;DR: 摘要评价指标需要改进，以更好地衡量摘要对个人的实用性。


<details>
  <summary>Details</summary>
Motivation: 目前对摘要评估的研究主要集中在摘要质量或特定需求属性上，但无法衡量摘要对个人的实用性。

Method: 对流行摘要指标的信息和角色差异敏感性进行评估，并进行了专家人工评估。

Result: 许多流行的摘要指标，包括基于LLM的指标，在信息内容的基本扰动测试中失败。传统和基于LLM的指标在信息满意度方面都不足，与人类判断的一致性较差。

Conclusion: 现有的摘要评价指标不足以衡量摘要对个人的实用性，需要改进指标和方法。
...
TL;DR: L-FNO在稀疏事件环境中优于传统神经网络操作符，为随机事件动态的神经网络操作器模型提供了有效的归纳偏差。


<details>
  <summary>Details</summary>
Motivation: 现代操作系统在常规条件下也面临不确定性，这些不确定性来自外生协变量和内生事件动态。标准神经网络操作符通常被训练为回归风格的函数到函数模型，而不是条件强度估计器，这限制了它们在稀疏事件环境中的适用性。

Method: 我们引入了洛伦兹傅里叶神经网络操作符（L-FNO），这是一种结合了FNO风格的协变量路径、洛伦兹谱核和基于似然的学习目标的随机神经网络操作符。

Result: 在八个合成点过程基准和三个真实世界数据集上评估了L-FNO，这些数据集涵盖了疾病爆发预测和半导体故障或缺陷检测。L-FNO在事件似然、校准诊断和稀有事件检测方面优于基于回归和似然的基本神经网络操作符。

Conclusion: 结构化谱记忆和基于似然的学习为随机事件动态的神经网络操作器模型提供了有效的归纳偏差。
...
TL;DR: Small benchmarks insufficient for evaluating coding ability, advocate for differentiated evaluation and sustained benchmark maintenance.


<details>
  <summary>Details</summary>
Motivation: Post-training papers, model cards, and blog posts often treat scores on a small set of coding benchmarks as evidence of broad coding capability.

Method: Case study with Django-based benchmark suite, evaluation of foundation models and checkpoints post-trained on SWE-bench trajectories.

Result: Benchmark rankings frequently fail to generalize, limited cross-task transfer, and SWE-bench optimization yields limited gains.

Conclusion: A small number of benchmarks is insufficient for evaluating diverse models under benchmark optimization pressure, encourage differentiated evaluation, and propose capability taxonomy and sustained benchmark maintenance.
...
TL;DR: 本文研究了XGBoost的鲁棒性，并提出了MM-XGBoost两步程序以改善其性能。


<details>
  <summary>Details</summary>
Motivation: 研究XGBoost的鲁棒性及其性能受垂直异常值和杠杆点的影响。

Method: 探索基于M-, S-, 和τ-估计量的稳健回归的替代损失函数。

Result: MM-XGBoost两步程序在鲁棒性和预测精度之间提供了最佳权衡。

Conclusion: XGBoost的鲁棒性可以通过使用MM-XGBoost两步程序得到提高。
...
TL;DR: APM通过跨尺度异构融合提高了小型语言模型的性能，证明了即使没有明确的语义对齐，也可以通过激活引导的方法实现有效的模型融合。


<details>
  <summary>Details</summary>
Motivation: 为了改善小型的接收方语言模型，我们研究了一种跨尺度的设置，即使存在显著的架构不匹配，也要使用更强的捐赠者模型。

Method: 我们提出了一种名为APM的激活引导框架，用于跨尺度融合。APM在捐赠者上构建任务条件激活图，选择显著的层、隐藏维度、注意力头和MLP神经元，将其修剪到接收者架构，并使用微插值系数将结果捐赠切片注入原始接收者。

Result: 在16个基准测试中，APM将原始3B接收者的总体平均准确率从55.5%提高到60.6%。RTE准确率从64.3%提高到82.3%，QNLI从52.3%提高到65.7%，BoolQ从70.8%提高到79.2%。

Conclusion: 这些结果表明，当捐赠者的贡献足够集中且经过精心选择时，跨尺度异构融合可以在没有明确语义对齐的情况下成功。
...
TL;DR: 不确定性采样在减少标签成本和提高模型性能方面是有益的，但其鲁棒性取决于数据集、预算、噪声结构和评估指标。


<details>
  <summary>Details</summary>
Motivation: 为了验证不确定性采样是否失败是因为它获取了更多被污染的标签，还是因为错误集中在困难区域特别有害。

Method: 使用基于边界的不确定性采样与随机采样在三个公共二进制表格数据集上进行了比较。在所有数据集上，不确定性采样在干净标签下提高了标准化平衡准确率的曲线下面积（AUC）1.09到1.77个百分点。

Result: 不确定性采样在标签效率方面表现出色，但其鲁棒性取决于数据集、预算、噪声结构和评估指标。

Conclusion: 不确定性采样在减少标签成本和提高模型性能方面是有益的，但需要谨慎使用并考虑具体的应用场景。
...
TL;DR: KRPRVFL模型通过结合计算效率和鲁棒性，有效解决了传统RVFL模型的局限性，提高了分类任务的性能。


<details>
  <summary>Details</summary>
Motivation: 解决传统RVFL模型对噪声标签、异常值和不平衡数据的敏感性，以提升其在实际应用中的性能。

Method: 提出基于核风险敏感均值p幂的RVFL（KRPRVFL）模型，结合RVFL的计算效率和核风险敏感均值p幂（KRP）准则的鲁棒性。通过用KRP损失替换标准最小二乘目标，KRPRVFL自适应地减少训练过程中受污染或不可靠样本的影响，从而提高稳定性和泛化能力。此外，引入协作学习机制，使模型组件之间能够进行自适应交互，进一步增强在复杂和噪声环境中的鲁棒性。利用核诱导的特征映射来捕捉非线性关系，无需显式选择隐藏层，保持效率和可扩展性。

Result: 在UCI和KEEL基准数据集上的广泛实验表明，KRPRVFL在准确性、鲁棒性和统计显著性方面始终优于基线模型，突显其作为快速、可扩展和可靠解决方案在具有挑战性的分类任务中的有效性。

Conclusion: KRPRVFL模型在保持效率和可扩展性的同时，提高了对噪声标签、异常值和不平衡数据的鲁棒性，在分类任务中表现出色。
...
TL;DR: ORCA improves anomaly detection in collider physics with enhanced sensitivity and interpretability.


<details>
  <summary>Details</summary>
Motivation: Generic event-level anomaly detection for collider physics has two recurring problems: anomaly scores are hard to interpret, and they correlate strongly with energy scale and object multiplicity.

Method: A two-stage framework that first learns an embedding space via supervised contrastive learning across a diverse set of physics processes, then runs a standard autoencoder in that space to generate event-level anomaly scores.

Result: Significant gains in both breadth and depth of sensitivity to new physics signals with respect to a baseline autoencoder architecture. Improved sensitivity and interpretability of anomalous samples. Accurate recovery of injected signal yields and characterization of signals absent from the template library.

Conclusion: ORCA is a route to interpretable anomaly detection-based searches at colliders, enhancing downstream statistical analysis.
...
TL;DR: QED通过查询引导的擦除方向，显著提高了长上下文检索性能。


<details>
  <summary>Details</summary>
Motivation: 线性注意力在长上下文中存在干扰问题，导致检索质量下降。

Method: 提出了一种名为Query-derived Erase Direction (QED)的新方法，通过查询添加第二个擦除方向，与键正交。

Result: QED在所有长度上都提高了检索质量，尤其是在S-NIAH-1数据集上，可用的上下文长度大约增加了两倍。

Conclusion: QED是一种有效的长上下文检索方法，可以显著提高检索性能。
...
TL;DR: AI模型发展迅速，能力提升显著，成本降低，应用领域不断拓展。


<details>
  <summary>Details</summary>
Motivation: AI模型从简单的BERT发展到能够解决复杂数学问题和编写软件的巨型代理。

Method: 通过分析AI模型的发展历程，特别是GPT 5.6 Luna等模型的应用，以及不同模型在特定任务上的表现。

Result: AI模型在解决实际编码问题上的能力显著提高，成本大幅下降。

Conclusion: AI模型在各个领域的应用越来越广泛，未来有望在更多领域发挥重要作用。
...
TL;DR: EEG-PRISM是一种能够将EEG基础模型的可解释性扩展到其他相关领域的通用方法，具有准确映射到频域和空间域的能力，支持瞬态事件分析和临床相关生物标志物识别。


<details>
  <summary>Details</summary>
Motivation: 当前可解释人工智能技术在EEG分析中提供的时间-通道输入空间的归因分数与临床对EEG的直觉不匹配，因此需要一种通用的方法来扩展任何基础模型的可解释性，而无需修改或重新训练底层模型。

Method: EEG-PRISM利用线性变换和已建立的反向传播规则将时间-通道归因分数映射到其他领域。通过可逆的DFT将映射到频域，通过近似可逆的EEG生成模型将映射到源域。在模拟和真实数据中评估EEG-PRISM，使用五种基础模型和四种AI解释器评估跨领域的真实现象恢复。

Result: 在模拟中，EEG-PRISM实现了几乎完美的光谱恢复和69.2%的空间准确性。在癫痫中，EEG-PRISM正确地确定delta-theta活动是最显著的，并以50%的准确性正确定位了癫痫发作区域。在自闭症中，EEG-PRISM将预测性delta-alpha生物标志物定位在额叶和颞叶区域，与先前的研究一致。

Conclusion: EEG-PRISM是一种理论上有根据的后处理归因方法，可以准确地将映射到频域和空间域。它支持瞬态事件（例如，癫痫发作）的窗口级分析以及临床相关生物标志物（例如，自闭症）的组级识别，从而推进可解释的EEG基础模型。
...
TL;DR: SAGE：一种提高Transformer-based SNN能量效率的训练方法。


<details>
  <summary>Details</summary>
Motivation: 提高稀疏事件驱动计算的能量效率，解决传统深度神经网络训练中的挑战。

Method: 提出SAGE，一种基于Transformer的SNN的确定性模ulated代理梯度机制。

Result: 在CIFAR-10/100数据集上，SAGE相对于固定代理基线提高了1-2%的准确率。

Conclusion: SAGE通过使用注意力派生的不确定性作为轻量级训练信号，提高了Transformer-based SNN的适应性代理梯度学习。
...
TL;DR: 高似然数据在微调初期更有利，但随着模型容量增加和训练时间延长，低似然数据变得更有益。


<details>
  <summary>Details</summary>
Motivation: 在推理监督微调中，相同指令的候选响应在匹配学生当前分布方面可能存在很大差异。基于似然度的响应选择方法表明，接近学生分布的响应提供更有效的监督，从而激发了对高似然响应通常更适合微调的假设。

Method: 通过在数学推理上进行的受控实验，使用从1.5B到8B参数的学生和由更强的教师模型生成的监督，观察到了明显的“快速拟合/慢速增益”模式。

Result: 高似然数据提供更快和更稳定的早期改进，尤其是对于较小的模型，但低似然数据在训练时间更长的情况下对较大模型越来越有利。

Conclusion: 有效的数据选择应考虑模型容量和计算预算，而不是基于对高似然监督的单一普遍偏好。
...
### [112] [High-dimensional nonparametric changepoint detection via low-rank degree-two density projection](https://arxiv.org/abs/2608.13922)
*Guoqing Zhang,Zhaixin Chen*

Main category: cs.LG

TL;DR: 提出了一种新的方法来检测高维数据中的分布变化，该方法在多个实验中表现出良好的性能。


<details>
  <summary>Details</summary>
Motivation: 检测高维数据中的分布变化是困难的，因为既没有指定变化前也没有指定变化后的密度是参数化的。

Method: 引入了一种基于表示的方法，该方法保留了所有度至多为二的密度信息，同时用矩阵均值估计代替密度估计。

Result: 提出的LRD估计量具有帐篷形状的总体目标和非渐近算子范数分析，其主导随机项的尺度为√(rdlog(nd))。对于多个变化，给出了种子最窄阈值过程，并通过归纳法证明了精确恢复，该归纳法为每个未检测到的变化保留了一个隔离区间。交叉拟合的标量细化在一部分学习变化低秩方向，在另一部分定位，达到∼O_P(κ^{-2})误差；匹配的Le Cam下界表明对数最优。几何β-混合扩展来自依赖矩阵Bernstein不等式。

Conclusion: 该方法在高达200的环境维度、三个变化的d=100序列和128个特征的人类活动基准测试中，在计算上仍然实用，并且可以准确地检测到均值CUSUMs无法看到的纯依赖变化。
...
TL;DR: 测试表明两个内核不可互换，需要进一步研究以确定可互换性


<details>
  <summary>Details</summary>
Motivation: 测试两个GPU内核在相同缩放INT8 GEMM接口下是否可互换

Method: 固定checkpoint、提示、硬件、推理引擎、解码和量化配置，仅交换INT8线性内核（CUTLASS与Triton）

Result: 两个内核在1.7B规模下完全相同，但在所有端到端比较中均不一致；FP8 GEMM显示不同特征；教师强制重放将层与标记相关联，预测翻转风险具有高ROC-AUC值；将发布预注册、每层预测、内核选择证据和一致性程序

Conclusion: 两个内核在相同配置下不可互换，需要进一步研究以确定可互换性
...
### [114] [QUASAR: Lowering the Loss Floor of Quantization-Aware Training with Loss-Aware Reconstruction](https://arxiv.org/abs/2608.13966)
...
### [115] [CutClean: Neural Network Pruning for Privacy-Preserving Inference](https://arxiv.org/abs/2608.13773)
*Leonardo Magliolo,Vito Paolo Pastore,Giuseppe Valenzise,Enzo Tartaglione*

Main category: cs.LG

TL;DR: 提出了CutClean方法，有效减少神经网络中的隐私信息泄露。


<details>
  <summary>Details</summary>
Motivation: 随着神经网络在高风险应用中的部署，隐私泄露问题日益突出。

Method: 提出了一种名为CutClean的隐私感知剪枝方法，通过在每个网络模块中放置辅助线性隐私头来量化信息泄露，并应用不同级别的稀疏度来去除私有属性泄露。

Result: 在合成和真实世界数据集上的实验表明，该方法有效地最小化了私有信息流，同时实现了高稀疏率和保留了分类目标精度。

Conclusion: CutClean方法为处理敏感属性的模型部署提供了有效的隐私保护手段。
...
### [116] [When Does More Correct Data Hurt? Insertion-Stability and the Limits of Dimension-Based Theory](https://arxiv.org/abs/2608.14020)
*Joseph Sankoorikal Johny*

Main category: cs.LG

TL;DR: 研究添加正确数据的安全性，引入插入稳定性概念，证明学习者在面对单调对手时的表现。


<details>
  <summary>Details</summary>
Motivation: 分析添加已知正确数据的安全性，并研究学习者在面对单调对手时的表现。

Method: 通过单调对手模型来研究学习者在面对不同数据集时的表现，并引入插入稳定性概念。

Result: 证明了在VC维大于等于2的类别中，没有学习器可以保证期望误差低于Theta((d/n)log(en/d))。同时，证明了插入稳定性学习者在面对单调对手时具有免疫性。

Conclusion: 学习者的选择与数据类别紧密相关，合适的学习者可以避免单调对手带来的影响。
...
TL;DR: PPAPlace通过结合图注意力和空间卷积，实现了时序驱动的布局优化，有效提升了芯片的PPA性能。


<details>
  <summary>Details</summary>
Motivation: 芯片布局对性能、功耗和面积有显著影响。

Method: 提出了一种基于时序驱动的可微代理PPAPlace，该代理结合了图注意力机制和空间卷积，并使用后全局布线标签进行训练。

Result: PPAPlace在五个ChiPBench测试电路中，平均WNS和TNS分别提高了22%和51%，同时保持了功率和可布线性。

Conclusion: PPAPlace是一种有效的时序驱动布局方法，可以显著提高芯片的PPA性能。
...
### [118] [Forecast Collapse in Time-Series Foundation Models](https://arxiv.org/abs/2608.14106)
*Shu Wan,Miles Ma,Hank Zhu,Guangqi Liu,Stephen Wang,Qingsong Wen,Huan Liu*

Main category: cs.LG

TL;DR: 研究发现预测结果几乎平坦，提出了一种新的平衡校准和排名的目标，提高了预测模型的性能。


<details>
  <summary>Details</summary>
Motivation: 预测美国1,000支股票的每小时回报时，发现预测结果几乎平坦，股票排名不佳。

Method: 研究了时间序列基础模型（TSFMs）、12个深度学习预测模型和97个公共基准配置，并发现这与目标可预测性密切相关。

Result: 提出了CalibRank，一种平衡校准和排名的简单目标，在Finance1K上几乎将横截面相关性提高三倍，同时保持幅度接近目标，并提高了所有测试模型的关联度。

Conclusion: 传统的时序评估存在盲点，序列指标可能隐藏了下游决策所需的横截面结构失败。
...
TL;DR: 本文全面回顾了药物-靶点结合亲和力预测的机器学习方法，指出了当前方法的局限性，并提出了未来的研究方向。


<details>
  <summary>Details</summary>
Motivation: 为了全面回顾和比较分析最近用于药物-靶点结合亲和力预测的机器学习方法，重点关注识别优势、局限性和研究差距。

Method: 对代表性深度学习方法进行综述，使用常见基准数据集和评估指标，涵盖各种神经网络架构和表示策略。分析七个广泛使用的基准数据集和常用的评估指标。

Result: 许多方法在标准基准上报告了强大的性能，但它们的有效性通常受数据集偏差和有限的评估设置的影响。大多数方法在冷启动场景中表现出降低的性能，突出了泛化方面的挑战。

Conclusion: 识别了当前方法的几个局限性，包括数据集不平衡、缺乏标准化评估、有限的现实世界适用性和冷启动场景的挑战。讨论了未来的研究方向，包括更好的数据集设计、更稳健的评估方法、改进的冷启动问题处理和多模态表示的集成。
...
### [120] [Revisiting Energy-based Tabular Anomaly Detection: Energy and Reconstruction are Complementary](https://arxiv.org/abs/2608.14186)
*Junichiro Niimi*

Main category: cs.LG

TL;DR: 作者研究了DBM在表格异常检测中的应用，发现其与自动编码器结合时效果显著，认为经典EBMs是表格异常检测的有价值工具。


<details>
  <summary>Details</summary>
Motivation: 受到深度学习中能量模型（EBMs）近期复兴的启发，例如能量基础变压器和JEPA，作者重新审视了经典的深度玻尔兹曼机（DBM）在表格异常检测任务中的应用。

Method: 作者评估了一个具有两个隐藏层的DBM，在两个表格基准数据集（UCI Bank Marketing和NSL-KDD）上与八个经典和现代基线进行对比，包括隔离森林、OCSVM、LOF、自动编码器、VAEs、COPOD、ECOD和Deep SVDD。

Result: DBM的均值场能量在Bank Marketing数据集上与最强的基线（自动编码器）相当，在NSL-KDD数据集上具有统计学上的优势，并且在两个数据集上都显著优于其他七个基线。当与自动编码器通过排名融合时，DBM能量在两个数据集上都产生了统计学上显著的改进。

Conclusion: 作者认为，经典的能量模型，如DBM，应作为表格异常检测工具箱中的一种非冗余的补充视图，与当前实践中占主导地位的基于重建的分数相结合。
...
### [121] [Dynamic Multi-Depot Vehicle Routing with Online Requests: Event-Driven Transformer--DRL and Rolling-Horizon Benchmarking](https://arxiv.org/abs/2608.13799)
*Faezeh Ardali,Gerald M. Knapp*

Main category: cs.LG

TL;DR: 提出了一种针对动态多仓库车辆路径问题的学习与基准框架，通过实验证明其有效性，但仍有改进空间。


<details>
  <summary>Details</summary>
Motivation: 提出一个针对动态多仓库车辆路径问题的学习与基准框架，解决请求逐渐揭示和车辆状态演变的问题。

Method: 使用行为克隆和近端策略优化训练Masked MLP和Transformer策略。采用确定性可行性掩码防止无效的车辆-请求分配，同时使用固定前缀/灵活后缀路线承诺保护已完成、活跃和近期决策，并分别衡量车辆重新分配和重新排序。将学习到的策略与动态插入启发式算法和时间限制滚动时域优化进行比较。

Result: 在20个场景的政策基准测试中，所有方法都完成了每个请求，没有无效操作，但最近可行的方法在平均目标函数和路由质量、等待时间、稳定性、完工时间和运行时间方面表现最佳。在五次独立的训练运行中，PPO对MLP的平均影响很小，但平均提高了Transformer的性能，尽管种子变异较大。在共同协议下，最近可行的方法在综合目标和路线中断方面表现最佳，而滚动时域在计算成本显著更高的情况下实现了最低的等待时间和完工时间。学习到的策略保留了毫秒级的决策，并在不重新训练的情况下转移到最多80个请求的实例中，但并未优于最强的启发式算法。没有一种方法在路由效率、服务响应性、稳定性和在线计算方面表现最佳。

Conclusion: 提出的方法在动态多仓库车辆路径问题中表现出色，但仍有改进空间。
...
### [122] [Designing Reinforcement Learning for Diffusion Models: A Unified Path-Space View](https://arxiv.org/abs/2608.14430)
*Yixian Xu,Yuanrui Zhang,Shengjie Luo,Liwei Wang,Di He*

Main category: cs.LG

TL;DR: 本文提出了一种新的RL算法，通过重要性抽样和策略梯度估计器统一了扩散模型中不同RL算法的设计空间。


<details>
  <summary>Details</summary>
Motivation: Reinforcement learning (RL)后训练为将扩散模型与人类偏好和特定任务奖励对齐提供了一种直接方法。然而，目前用于扩散模型的RL算法仍然分散：反向轨迹方法依赖于离散似然比，而正向匹配方法在奖励标记的噪声版本的重放样本上训练。

Method: 从正则化的扩散-RL目标开始，使用采样SDE之间的重要性抽样来获得轨迹空间上的显式策略梯度估计器。

Result: 提出了一种多样本KDE值梯度估计器，该估计器重用重放组，以及具有稳定现有配方并排除奇异配方的规模有界权重族。实验表明，这种方法在方差减少方面优于先前的扩散-RL基线。

Conclusion: 这些方法的不同损失源于一个单一路径空间原理，并通过重要性抽样和策略梯度估计器实现了统一设计空间。
...
TL;DR: HI-MGN通过分层多尺度处理器提高了网格图神经网络的效率，在基准测试中表现出色。


<details>
  <summary>Details</summary>
Motivation: 为了解决传统图神经网络在大型、高保真网格上的效率问题，提出了一种多尺度扩展的网格图神经网络（HI-MGN）。

Method: 提出了一种名为HI-MGN的网格图神经网络，它使用分层多尺度处理器，通过最远点采样和Voronoi划分来细化图，同时保留原始网格拓扑结构。

Result: 在三个结构和流体基准测试中，HI-MGN与MeshGraphNets和Bi-Stride Multi-Scale GNN相比，在提高精度的同时减少了训练时间和峰值内存使用。

Conclusion: 拓扑感知的分层消息传递和从粗到细的插值学习为可扩展的基于网格的物理代理建模提供了一个有效且实用的框架。
...
TL;DR: 本文对联邦提示学习（FPL）进行了全面调查，以回顾将联邦学习范式与大型语言模型相结合的最新进展，并讨论了安全、隐私和鲁棒性问题。


<details>
  <summary>Details</summary>
Motivation: 高计算成本、数据集中化和隐私问题阻碍了大型语言模型（LLMs）的训练和部署。联邦学习（FL）提供了一种去中心化的训练范式，允许客户端在不共享原始数据的情况下协作训练学习模型，因此是隐私保护LLMs训练和推理的有希望解决方案。

Method: 对联邦提示学习（FPL）进行了全面调查，以回顾将联邦学习范式与大型语言模型相结合的最新进展。

Result: 系统地检查了现有的FPL方法，涵盖了整个模型生命周期：预训练、微调和实际应用，同时讨论了安全、隐私和鲁棒性问题，并总结了现有的防御机制。

Conclusion: 强调了开放挑战和未来方向，旨在帮助读者了解见解如何推动FPL的研究。
...
TL;DR: 一种基于几何过滤框架的方法有效提高了LLM生成文本分类合成训练数据的质量。


<details>
  <summary>Details</summary>
Motivation: 解决LLM生成文本分类合成训练数据质量不均的问题

Method: 提出一种基于几何过滤框架的方法，通过计算LLM生成样本与真实类样本在句子嵌入空间中的欧几里得距离来评估样本，并选择几何一致的候选样本。使用软加权机制将过滤分数转换为样本权重以进行分类器训练。

Result: 在13个数据集、5个分类器、10种增强方法和超过6700种配置上进行评估，该方法在SMOTE的基础上提高了+2.61个百分点（p<0.0001，Cohen's d=0.95，88.9%胜率）。该方法无需修改过滤器即可推广到命名实体识别（+9.26个百分点，100%胜率），并且对来自4个提供商的5个LLM具有鲁棒性。一个关键发现是，最简单的基于距离的过滤器始终优于复杂的多标准替代方案。

Conclusion: 该方法有效提高了LLM生成文本分类合成训练数据的质量，并具有良好的泛化能力和鲁棒性。
...
TL;DR: VBKL框架在深度表示和利益分析方面表现出优势


<details>
  <summary>Details</summary>
Motivation: 分析深度表示复杂性与深度表示利益的关系

Method: 提出VBKL框架，将非线性递归字典构造与线性变化叠加分离

Result: 识别递归字典为布朗拉回RKHS球的并集，建立变化控制的Hölder正则性、紧致性和严格增长，以及局部非退化条件下的严格增长与深度

Conclusion: 实验表明，VBKL框架在有限数据精度-复杂度权衡方面具有优势
...
TL;DR: 提出了一种基于马尔可夫决策过程和蒙特卡洛树搜索的时尚服装组合生成方法，实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 时尚服装组合的生成是一个具有挑战性的任务，因为审美兼容性的非单调性和隐含性，以及组合搜索空间的指数级增长。

Method: 将时尚服装组合生成任务形式化为约束集成生成（CEG），并建模为有限时间窗的确定性马尔可夫决策过程。提出统一顺序组合模型（USCM）来联合建模集合级兼容性和潜在组合意图。使用LE-MCTS机制来处理组合过程中的项目检索，平衡局部审美协同和全局结构平衡。

Result: 在Polyvore Outfits数据集上进行的大量实验以及iFashion和PolyvoreU数据集上的零样本评估表明，该框架在独立人类偏好评估、自动美学代理和结构有效性指标方面实现了最先进的性能。

Conclusion: 该研究提出了一种基于马尔可夫决策过程和蒙特卡洛树搜索的时尚服装组合生成方法，实现了最先进的性能。
...
TL;DR: MedMix：一种解决联邦多模态医疗AI模态异质性的语义对齐框架。


<details>
  <summary>Details</summary>
Motivation: 针对联邦多模态医疗AI在客户端和样本级别面临模态异质性的挑战，提出了一种名为MedMix的语义对齐框架。

Method: MedMix框架使用模态上下文来协调跨客户端的路由和专家专业化，包括客户端侧的模态上下文感知路由和共识引导的路由对齐，以及客户端自适应的专家聚合。

Result: 在真实世界的多模态医疗数据集上的实验表明，MedMix在各种模态异质性和模态不完整性设置中实现了最佳的F1平均分数，特别是在严重异质性下取得了明显的收益。

Conclusion: MedMix框架有效地解决了联邦多模态医疗AI中的模态异质性挑战，提高了模型性能。
...
TL;DR: HQKAN在联邦学习生物信号数据分类中表现出色，优于MLP基准。


<details>
  <summary>Details</summary>
Motivation: 为了解决医疗保健和可穿戴设备在共享原始信号进行集中式模型训练时遇到的隐私限制，联邦学习允许在保持原始生物信号数据在其各自来源的同时进行协作模型训练。然而，由于客户端样本有限、心律失常标签不平衡以及客户端之间的非独立同分布（非-IID）数据，联邦ECG分类仍然具有挑战性。

Method: 本研究评估了混合量子启发的柯尔莫哥洛夫-阿诺德网络（HQKAN）与多层感知器（MLP）在MIT-BIH数据集上进行五类心律失常分类以及在INCART数据集上进行三类分类，在联邦平均（FedAvg）下的性能。

Result: 在多个客户端配置下，HQKAN在MIT-BIH上提高了大多数聚合和少数类指标，同时使用了37.35% fewer trainable parameters，并将通信成本降低了24.89%；在INCART上，它实现了相应的44.81%和36.41%的降低。

Conclusion: 这些结果表明，HQKAN为隐私感知的联邦学习生物信号数据提供了一种紧凑、通信高效且鲁棒的替代方案。
...
TL;DR: CForce方法通过强制早期和后期掩码预测一致性，提高了dLLMs的预测准确性和生成速度。


<details>
  <summary>Details</summary>
Motivation: 为了解决现有dLLMs在早期去噪阶段由于激进并行策略导致的不可靠预测问题，导致错误传播到后续阶段。

Method: 提出了一种名为CForce的蒸馏方法，通过在预收集的自我滚动轨迹上训练模型，强制早期阶段的掩码预测与后期阶段的预测保持一致。同时，引入了置信度自适应KL散度作为蒸馏目标，结合了前向和反向KL的优点。对一致性目标进行了理论分析，解释了为什么CForce可以近似最小化早期阶段的预测误差。

Result: 在非编辑和可编辑的LLaDA模型上进行了实验，显示出改进的速度-质量权衡，尤其是在高并行解码预算下。

Conclusion: CForce方法可以有效地提高dLLMs的预测准确性，并提高语言生成的速度和质量。
...
TL;DR: HyGenQ 通过 HCD 和 SR 解决了混合 IGMs 中的量化问题，提高了图像生成质量。


<details>
  <summary>Details</summary>
Motivation: Iterative Generative Models (IGMs) 在图像生成方面具有很高的保真度，但其迭代推理计算量大，Post-training Quantization (PTQ) 可用于加速，但直接应用于混合 IGMs 会引发模型崩溃。

Method: 提出 HyGenQ，一个针对混合 IGMs 的 PTQ 框架，包括 Hierarchical Cluster Decoupling (HCD) 和 Scaling Recalibration (SR)。HCD 通过多阶段聚类过程识别和分离异常通道，SR 通过扩展异常值超过高斯界限来避免模型崩溃。

Result: HyGenQ 成功将代表性混合 IGMs 量化为 8 位精度（W8A8），显著优于现有基线，并在不同模型家族中验证了其鲁棒性。

Conclusion: HyGenQ 是一种有效的 PTQ 框架，可以显著提高混合 IGMs 的性能和鲁棒性。
...
TL;DR: 本研究提出了一种基于MN增强的MHA-PNN模型，用于预测无侧限抗剪强度，在预测精度和不确定性量化方面表现出色。


<details>
  <summary>Details</summary>
Motivation: 为了准确预测无侧限抗剪强度(su)，本研究使用CLAY/10/7490全球数据库，开发了基于阿特堡界限和压力板锥入试验(CPTU)测量的概率间接模型。

Method: 使用多变量正态(MN)、多重插补链式方程(MICE)和miss森林(MF)三种方法填充缺失数据。将多头注意力(MHA)机制集成到人工神经网络(ANN)中构建间接模型，并与基于MN的传统预测模型进行比较。

Result: MN增强的MHA-PNN模型在预测精度和不确定性量化方面优于其他模型。

Conclusion: 本研究提出的方法在地质工程应用中具有潜在价值，尤其是在处理稀疏和不完整数据集时。
...
TL;DR: 通过跨层设计提高联邦学习的鲁棒性和效率。


<details>
  <summary>Details</summary>
Motivation: 传统的网络层处理无法有效处理联邦学习中数据量不均等的重要性，导致通信瓶颈和信道损坏问题。

Method: 提出一种基于层间极化码的联邦学习方案，利用有限块长度下极化码的不等错误保护（UEP）特性。

Result: 该方案可以显著提高联邦学习的鲁棒性和效率，在信道质量恶化时表现更为明显。

Conclusion: 跨层设计在提高联邦学习鲁棒性和效率方面是有效的。
...
TL;DR: 提出了一种新的情境学习理论，用于部分序，并探讨了开放世界学习中的关键问题。


<details>
  <summary>Details</summary>
Motivation: 在部分序上发展情境学习的理论，区分逻辑可识别性、提示教学成本、结构复杂性和形式坐标解码类的确切容量。

Method: 开发一个理论，使用版本空间语义来明确背景知识和开放世界与封闭世界的假设。

Result: 证明了有限开放世界提示中的精确完成三分法，并确定了开放世界教学数的特征和最大值。

Conclusion: 为情境学习提供了一个新的理论框架，有助于理解部分序上的学习过程。
...
TL;DR: 本研究提出了一种新的文本到图像扩散模型，通过动态调整引导缩放来提高图像质量和文本对齐。


<details>
  <summary>Details</summary>
Motivation: 现代文本到图像扩散模型依赖无分类器引导（CFG）以实现高图像保真度和文本对齐。然而，CFG通常对所有时间步长、样本和条件应用静态、全局的缩放，这种选择通常次优且可能引入伪影，因为不同的状态可能需要不同级别的引导。尽管已知时间变化计划可以提高质量，但手动设计它们是相当复杂的且与应用相关。

Method: 在本论文中，我们将引导计划作为扩散时间、条件当前噪声样本的函数来学习，以更好地对齐采样的图像与文本提示。我们将此框架为密度比率估计问题：训练判别器以估计真实和引导边缘分布之间的时间依赖性对数密度比率，同时轻量级的生成器网络预测最佳的状态相关引导缩放。

Result: 在实证研究中，我们的方法在文本到图像生成基准测试中优于启发式CFG计划和先前学习动态引导的方法。

Conclusion: 通过将引导计划作为扩散时间、条件和当前噪声样本的函数来学习，可以更好地对齐采样的图像与文本提示，并提高文本到图像生成的质量。
...
TL;DR: 提出了一种名为RAEF的检索增强扩展预测方法，它通过改进检索和聚合机制，在时间序列预测中实现了更高的准确性和更低的计算成本。


<details>
  <summary>Details</summary>
Motivation: 在特定应用中，时间序列数据通常具有较短或可忽略的历史数据，因此需要通过微调或RAG进行适应。然而，微调需要大量的计算成本。

Method: 提出了一种名为RAEF（检索增强扩展预测）的无模型方法，该模型基于RAF（检索增强预测），并对检索和聚合机制进行了关键改进。

Result: RAEF在多个基准数据集上的实证评估表明，它在准确性和推理开销方面都优于RAF。与零样本和微调的基础模型进行了全面比较，RAEF在避免微调计算负担的同时，实现了与微调相竞争或更优的性能，从而在时间序列预测的领域自适应方面建立了一种实用且可扩展的方法。

Conclusion: RAEF是一种有效且高效的时间序列预测方法，适用于特定领域应用中的数据适应性。
...
TL;DR: 该方法通过优化扩散模型的采样过程，提高了时间序列预测的准确性。


<details>
  <summary>Details</summary>
Motivation: 对时间序列预测中扩散模型的不确定性建模方法进行挑战，并研究其迭代采样过程对预测质量的影响。

Method: 通过分析反向扩散过程中预测质量的变化，提出了一种无标签的全局停止标准，并设计了一种伯努利时间步采样器。

Result: 实验结果表明，与现有方法相比，该方法在八个真实世界数据集上表现出优异的性能。

Conclusion: 扩散模型在低噪声区域进行持续细化可能导致预测质量下降，而提出的方法可以有效提高预测精度。
...
TL;DR: 提出了一种新的OWMS学习策略，提高了性能并可能适用于其他问题


<details>
  <summary>Details</summary>
Motivation: 现有的OWMS学习策略无法适应资源变化

Method: 提出资源自适应原-对偶学习框架

Result: 提高OWMS学习策略的性能

Conclusion: 该方法可能适用于其他在线学习问题
...
TL;DR: 提出了针对顺序预测问题的对数上界后悔度算法。


<details>
  <summary>Details</summary>
Motivation: 考虑对$m$进制序列的顺序预测问题。

Method: 提出针对随机和对抗环境的算法，并建立其后悔度的对数上界。

Result: 对数上界后悔度。

Conclusion: 提出了对数上界后悔度的算法。
...
TL;DR: 使用AlphaZero进行拓扑控制，优化电网管理。


<details>
  <summary>Details</summary>
Motivation: 现代电网在整合易变的可再生能源方面承受压力，需要新的方法来保持电网稳定和运行。

Method: 本研究探讨了基于模型的AlphaZero启发式方法，利用蒙特卡洛树搜索（MCTS）进行主动电网管理。

Result: 优化后的AlphaZero方法实现了98.43%的峰值生存率，显著优于近端策略优化（PPO）变体。

Conclusion: AlphaZero是拓扑控制的有力框架，但纯强化学习不足以构建有效的系统；需要领域特定启发式、二元奖励和受限的线路负载观察空间的集成。
...
TL;DR: 提出了TLO，用于从欧拉场预测到拉格朗日粒子轨迹生成的零样本泛化，在多个基准测试中表现出色。


<details>
  <summary>Details</summary>
Motivation: 由于拉格朗日轨迹不如欧拉场常见，而大多数神经网络算子主要在欧拉表示下训练和评估，这导致了一个新的学习问题：能否仅使用欧拉场观察数据训练的模型，在没有拉格朗日监督或任务特定调整的情况下，从欧拉场预测到拉格朗日粒子轨迹生成进行零样本泛化？

Method: 提出了可迁移的潜在算子（TLO），该算子学习一个统一的流表示，用于欧拉场预测和拉格朗日粒子轨迹生成。

Result: TLO在五个流体动力学基准测试中，在欧拉场预测和零样本拉格朗日粒子轨迹生成方面均优于现有神经网络算子，并在有限的拉格朗日微调后进一步提升了性能。

Conclusion: TLO在欧拉场预测和零样本拉格朗日粒子轨迹生成方面优于现有神经网络算子，并在有限的拉格朗日微调后进一步提升了性能。
...
TL;DR: XIGL通过人机交互和主动学习策略，有效地从GNN中移除了捷径，提高了模型在分布外任务中的可靠性。


<details>
  <summary>Details</summary>
Motivation: Graph Neural Networks (GNNs) 在预测任务中通过无意利用捷径（即与预测相关但不是因果关系的边、节点和特征）来解决预测任务，这影响了它们在分布外任务中的可靠性。

Method: 提出了一种名为XIGL的架构无关的人机交互策略，用于从GNN中移除这些捷径。关键洞察包括：1）通过检查GNN的解释可以检测到对捷径的依赖；2）一旦意识到这些捷径，足够专业的用户可以提供定制的纠正反馈，这有助于消除模型中的混淆。XIGL支持任何查询策略；然而，由于纠正反馈可能难以获取，我们开发了一种主动学习策略，优先考虑可能显示捷径行为的解释，以降低标注和认知成本。

Result: 在多个GNN架构上展示了XIGL的有效性，包括现有的和基于解释的策略。我们的实现可在网上找到。

Conclusion: XIGL是一种有效的策略，可以减少GNN在预测任务中对捷径的依赖，提高其在分布外任务中的可靠性。
...
TL;DR: 研究比较了不同算法在 CVRPTW 优化问题上的表现，发现深度学习和启发式方法在规模较大的问题上效率较高。


<details>
  <summary>Details</summary>
Motivation: 面对全球城市人口增长，具有现实约束条件的路线优化问题变得越来越重要。

Method: 研究了带时间窗的容量限制车辆路径问题（CVRPTW），并与精确求解器 SCIP、启发式算法 LKH、2-OPT、3-OPT、ORTools 框架和深度学习模型 JAMPR 进行比较。

Result: 在问题规模为 50 的情况下，深度学习和经典启发式解接近 SCIP 精确解，但所需时间更少。对于问题规模为 100 的情况，SCIP 精确方法比神经和经典启发式方法慢约 13 倍，在相同路径成本下，第一种可行解的效率差约 50%。

Conclusion: 开发了一个名为 Smart Routes 的平台，用于解决路线优化问题，该平台包括精确、启发式和深度学习模型，并便于集成自定义算法和数据集。
...
TL;DR: 本研究成功应用深度强化学习解决 CPDPTW 问题，为不同规模问题提供快速解决方案。


<details>
  <summary>Details</summary>
Motivation: 随着全球城市人口的增长，构建车辆最优路线以进行货物 pickups 和 deliveries 成为最具前景的任务之一。

Method: 本研究首次成功将深度强化学习（修改后的 JAMPR 模型）应用于解决具有容量和时间窗口约束的 Pickup 和 Delivery 问题（CPDPTW）。

Result: 获得了鲁棒模型，为小到中等规模的问题提供快速最优解，为大到 (> 200) 的问题提供快速次优解。

Conclusion: 该研究为解决具有容量和时间窗口约束的 Pickup 和 Delivery 问题提供了一种有效的方法，具有实际应用价值。
...
TL;DR: AATC方法有效解决KV缓存瓶颈，提升长上下文推理性能


<details>
  <summary>Details</summary>
Motivation: 解决长上下文推理中KV缓存造成的内存瓶颈问题

Method: 提出Attention-Aware Transform Coding (AATC)方法，结合信号处理和率失真理论中的工具，对缓存进行编码和位分配

Result: 在Llama-3.1-8B-Instruct和Qwen-2.5-7B-Instruct上，该方法在LongBench、RULER、GSM8K、MMLU-Pro和MATH-500等任务上实现近似无损精度，压缩比约为5.8倍，优于基线方法

Conclusion: AATC方法有效降低了长上下文推理中的内存瓶颈，提高了推理精度和效率
...
TL;DR: MINT是一种改进的框架，通过紧凑的交易嵌入实现高效的多模态推理和零样本预测


<details>
  <summary>Details</summary>
Motivation: 提高金融交易数据预测的准确性

Method: 提出了一种名为MINT的框架，将预训练的交易序列编码器与仅解码的LLM连接，并通过轻量级嵌入注入、交易语言对齐和指令调整进行连接

Result: MINT在预测问答任务中实现了最先进的性能，同时大幅降低了输入令牌、延迟和内存消耗

Conclusion: 紧凑的交易嵌入是多模态推理和零样本预测任务中优于文本序列化的交易表示方法
...
TL;DR: 提出自适应保护机制，提高进化特征构建的搜索效果和解决方案质量。


<details>
  <summary>Details</summary>
Motivation: 现有方法在进化特征构建中缺乏保护重要特征的机制，导致有效特征可能丢失。

Method: 提出了一种自适应保护机制，利用特征重要性指标有选择地保护构建的特征。

Result: 实验结果表明，该机制在回归基准数据集上提高了解决方案的质量，并在信用分类数据集上扩展了搜索有效性。

Conclusion: 自适应保护机制在进化特征构建中提高了搜索效果和解决方案质量。
...
TL;DR: FairTFM：提高TFMs公平性的新策略。


<details>
  <summary>Details</summary>
Motivation: 在关键决策中，Tabular Foundation Models (TFMs) 的公平性属性尚未得到充分探索。

Method: 将公平性约束直接纳入TFM训练，提出FairTFM，基于合成公平任务和公平感知架构。

Result: 在132个公平性任务上，FairTFM在保持竞争力的准确率的同时，公平性得到显著提升。

Conclusion: FairTFM能够有效提高TFMs的公平性，为关键决策提供了更可靠的预测方法。
...
TL;DR: 提出了一种新的连接子空间聚类算法，在测地学应用中表现优异。


<details>
  <summary>Details</summary>
Motivation: 受测地学应用启发，需要识别海平面连续区域以进行主成分分析。

Method: 提出了一种新的约束聚类问题——连接子空间聚类，使用一种高效的Lloyd风格的启发式算法来解决这个问题。

Result: 该方法能够准确地将高维点划分为k个连接区域，且在160个全球海平面时间序列配置中，基于合并的修复策略在73.75%的情况下表现最佳。

Conclusion: 该算法在测地学和其他空间嵌入式多变量时间序列分析中具有广泛的应用前景。
...
TL;DR: autoschema框架实现实时模式归一化，提高生物医学知识图谱问答准确性。


<details>
  <summary>Details</summary>
Motivation: 研究生命科学知识图谱的互操作性，以解决不同资源使用不同模式、标识符和链接的问题。

Method: 提出autoschema框架，实现实时模式归一化，无需训练。

Result: autoschema在生物医学知识图谱问答任务中提高了平均事实准确性，并在纵向BioASQ评估中给出了一致的增益。

Conclusion: autoschema可以支持实时模式归一化，为不规则和先前未记录的图提供支持，无需首先创建经过策划的模式文件。
...
TL;DR: 提出新的凸损失函数，用于SVM和神经网络，实验表明其泛化性能提升不明显。


<details>
  <summary>Details</summary>
Motivation: 提出新的凸损失函数，用于SVM和神经网络在二分类任务中的应用。

Method: 使用粒子群优化算法解决带有修改后损失的原始SVM问题。

Result: 实验结果表明，使用损失函数内的模式相关性可以在理论上提高某些数据集上的泛化性能。

Conclusion: 提出的新损失函数是一般损失的推广，但泛化性能的提升并不明显。
...
TL;DR: 提出了一种无需训练的多模态轨迹预测方法，在数据稀缺的情况下表现出色，具有广泛的应用前景。


<details>
  <summary>Details</summary>
Motivation: 提出一种无训练的多模态轨迹预测方法，以实现与57M参数的transformer相当的准确度，同时无需GPU和零学习参数。

Method: 构建历史状态到下一位置对的转换表，并使用空间接近度、方位、速度和时间上下文的产品核检索邻居。两种推理模式在共享表示上操作：多样性惩罚采样产生覆盖不同可能路线的轨迹，而束搜索找到最高概率的路径。

Result: 在TrAISformer基准（丹麦海事AIS）上，该方法在数据完全可用时达到具有竞争力的准确度，在数据稀缺的环境中显著优于transformer——在训练数据降至10%时仍保持稳定，而TrAISformer则急剧下降。这使新地理区域的部署成为可能，所需历史数据减少一个数量级，且无需GPU训练。

Conclusion: 该方法在多模态轨迹预测方面表现出色，特别是在数据稀缺的情况下，具有很高的实用价值。
...
TL;DR: SWA是一种高效的方法，可以在不重复训练的情况下提高深度学习模型的对称性。


<details>
  <summary>Details</summary>
Motivation: 设计现代深度学习解决方案时，学习任务的对称性已成为一个重要因素。数据增强是一种将对称性纳入通用神经网络的有效方法。

Method: 研究随机权重平均（SWA）作为不要求重复训练运行的替代集成技术。通过将训练结束时的随机训练轨迹近似为Ornstein-Uhlenbeck过程来分析SWA。

Result: 在无限宽度极限下，SWA在增强数据上提供了一种等变性提升，这超出了仅从SWA本身性能提升中可以期望的。通过在具有离散和连续对称性的计算机视觉和图分类模型上进行了广泛的数值实验来验证我们的结果。

Conclusion: SWA是一种有效的集成技术，可以在不重复训练的情况下提高深度学习模型的对称性。
...
TL;DR: DeaMoE通过优化MoE架构和路由策略，显著提升了实时交互应用中的解码效率。


<details>
  <summary>Details</summary>
Motivation: 为了满足实时交互应用场景中极低的响应延迟要求，研究人员通常采用小批量解码，这使得MoE推理成为内存限制的，并且严重受到专家权重加载的瓶颈。然而，这个瓶颈得到了有限的关注，现有的解决方案如后训练权重压缩或预训练期间的细粒度专家设计要么降低模型精度，要么引入额外的计算和通信开销。

Method: 提出DeaMoE，一种解码高效的MoE架构，将专家分组到几个部门，同一部门的专家由于来自同一专业领域而共享大多数参数，并且每个专家还包含一些私有参数来反映其独特性。此外，为DeaMoE设计定制的两阶段路由策略以避免冗余加载。

Result: 与vanilla MoE相比，DeaMoE将每步加载的权重减少了高达50.9%，在A40上对预训练的7B模型实现了高达1.33倍的端到端TPOT速度提升，在微基准测试中对DeepSeek-V3在A40和H100上分别实现了2.00倍和1.97倍的峰值速度提升。

Conclusion: DeaMoE通过有效的专家分组和路由策略，显著提高了MoE模型的解码效率，降低了延迟，并保持了模型精度。
...
TL;DR: CytoBERT是一种新的基础模型，用于解决流式细胞术数据异质化和非标准化问题，提高数据分析和迁移学习的能力。


<details>
  <summary>Details</summary>
Motivation: Cytometry数据高度异质化和非标准化，难以应用和跨研究迁移。

Method: 提出CytoBERT，一个公开的、开源的、开放权重的单细胞流式细胞术数据基础模型。在大型流式细胞术语料库上自监督预训练，进行样本级分类的微调。

Result: CytoBERT在异质流式细胞术数据集上的迁移学习是可行的，为可扩展、可推广的流式细胞术分析提供起点。

Conclusion: CytoBERT有助于解决流式细胞术数据异质化和非标准化的问题，提高数据分析和迁移学习的能力。
...
TL;DR: 提出了一种新的 NAS 方法，通过放松的 bilevel 优化框架，实现了高效的神经元和激活级别架构优化，并显著提高了预测性能。


<details>
  <summary>Details</summary>
Motivation: 为了解决神经架构搜索（NAS）的计算成本问题，包括离散的架构决策、指数级增长的搜索空间和候选架构的高成本训练。

Method: 提出了一种通用的 bilevel 优化框架，包括三种可扩展的公式，用连续松弛代替离散的神经元和激活级别决策，从而在不同组合架构空间上实现可微优化。这三种方法分别是 NAS-NG、NAS-MA 和 NAS-NGMA。

Result: 实验结果表明，这些方法在 MLP 和 CNN 上都成功地识别出紧凑的架构，并具有竞争力或改进的预测性能。在 MNIST 上，NAS-NGMA 实现了 98.68% 的测试准确率，而 NAS-NG 实现了 99.63% 的准确率。在 CIFAR-10 上，这些方法始终优于 vanilla DARTS。

Conclusion: 这些结果将放松的 bilevel 优化确立为离散 NAS 的可扩展替代方案，并提供了一个通用的框架，用于有效的神经元和激活级别架构优化。
...
TL;DR: LP-NAS是一种基于线性规划的NAS方法，在架构搜索和评估阶段优于标准DARTS。


<details>
  <summary>Details</summary>
Motivation: 减少对人类专家的依赖，自动化神经网络架构设计。

Method: 提出基于线性规划的NAS（LP-NAS），使用验证损失梯度和训练损失Hessian计算架构更新方向。

Result: LP-NAS在CIFAR-10和CIFAR-100数据集上优于标准DARTS，并在ImageNet数据集上验证了发现架构的可迁移性。

Conclusion: LP-NAS是一种有效的NAS方法，可以加速架构优化并提高验证性能。
...
TL;DR: 本文提出了一种名为sMuon的方法，解决了Muon优化器在PEFT中应用的限制，实现了性能提升。


<details>
  <summary>Details</summary>
Motivation: 为了解决Muon优化器在参数高效微调（PEFT）中使用较少的问题，特别是与LoRA方法的结合问题。

Method: 通过线性化和最小二乘法近似低秩设置中松弛的Muon目标函数的解，实现sMuon（小Muon）方法，该方法是使用仅matmul操作的高效实现。

Result: 在SFT和ReLoRA预训练实验中，sMuon方法表现出色。虽然结果取决于模型和评估方法，但使用Muon进行低秩微调整体上提供了适度的性能提升。

Conclusion: sMuon方法在低秩微调中提供了性能改进，为Muon优化器的更广泛应用提供了新的可能性。
...
TL;DR: 提出了一种两阶段框架，用于从多个异构表中生成高质量的合成数据。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要针对单一输入表场景，难以有效处理具有不同特征集的多个异构表。

Method: 提出一个两阶段框架，第一阶段将异构原始表转换为具有相同列的标准化统计表，第二阶段训练扩散转换器模型捕获结构模式并生成合成统计表。

Result: 实验结果表明，该方法在学习的统计表示中具有高保真度，并且在生成的合成数据中具有有利的保真度-多样性权衡。

Conclusion: 该方法有效解决了现有方法的限制，并支持生成大量真实合成异构表。
...
TL;DR: Rollplex通过分解和并行计算提高了VLMs的运行效率。


<details>
  <summary>Details</summary>
Motivation: 当前的on-policy RL执行顺序对VLMs来说是低效的，因为它无法充分利用GPU的计算能力。

Method: Rollplex使用相 Awareness的内存管理和并行度感知的权重共享来提高运行效率。

Result: 在32个H800 GPU上，Rollplex相对于串行协同定位和去聚合，在相同的GPU预算下，速度提高了1.23至1.30倍和1.57至2.24倍。

Conclusion: Rollplex实现了参考和训练阶段的分解，并将前缀计算移动到滚动解码窗口中，显著提高了VLM的运行速度。
...
TL;DR: RecipeNet是一种用于配方数据表示学习的层次Transformer架构，在性能上优于现有表格模型。


<details>
  <summary>Details</summary>
Motivation: 在材料合成、制药配方和工业制造等领域，配方数据以有序步骤序列的形式出现，其中包含异构的有序字段。现有的表格学习方法通常将这种结构扁平化为固定模式的表示，限制了它们捕捉字段交互和程序依赖的能力。

Method: 提出RecipeNet，一种层次Transformer架构，通过堆叠的Transformer编码器对每个步骤中的字段交互和步骤之间的顺序依赖进行编码。

Result: 在多个配方数据集和任务上的实验表明，RecipeNet在性能上始终优于现有的表格模型，突出了层次和顺序建模对配方表示学习的重要性。

Conclusion: RecipeNet在配方数据表示学习方面表现出色，优于现有表格模型，证明了层次和顺序建模的价值。
...
TL;DR: 该研究通过层间敏感性分析，揭示了MoE架构中不同层级的相对重要性，为模型压缩提供了新的思路。


<details>
  <summary>Details</summary>
Motivation: 为了更好地理解混合专家（MoE）架构中各个层级的相对重要性，特别是对于模型压缩的影响。

Method: 使用基于大小值的专家掩码对Qwen3.6-35B-A3B模型进行层间敏感性分析，并在XLCoST跨语言代码翻译基准上进行测试。

Result: 发现层敏感性强烈依赖于深度，早期和中间层对专家掩码非常敏感，而晚期层可以容忍对低幅度专家的强烈掩码。

Conclusion: 这些发现为深度感知MoE专家掩码提供了经验基础，并为物理权重手术、基于激活的专家评分和基于训练的恢复提供了实际途径。
...
TL;DR: 研究发现，LLM展现出与人类大脑相似的模块化架构，表明模块化可能是智能系统的基本属性。


<details>
  <summary>Details</summary>
Motivation: 探讨人类大脑功能特化的模块化组织是否是智能系统构建的基本原则，或仅仅是生物大脑的进化偶然事件。

Method: 使用电路分析，在涵盖四个认知领域（语言、形式推理、社会推理、物理推理）的N=46个任务中，对大型语言模型（LLM）进行测试。

Result: LLM展现出与人类大脑相似的模块化架构，即同一网络的任务在LLM中激活重叠的神经元，而不同网络的任务则激活不同的神经元。

Conclusion: 大脑和神经网络中模块化的趋同出现表明，这可能是一个智能系统的基本属性。
...
TL;DR: 本文研究了LLM服务的真实负载，并发布了完整的追踪数据，支持后续研究。


<details>
  <summary>Details</summary>
Motivation: 研究真实世界LLM服务的负载

Method: 对一年的生产追踪进行全局描述和纵向研究

Result: 揭示负载演化和用户-模型结构，支持未来研究

Conclusion: 本文通过对Chutes一年的生产追踪进行全局描述和纵向研究，更深入地理解了真实世界的LLM服务负载，并支持未来的研究工作。
...
TL;DR: 本文提出了Agentao，一种用于工具使用型LLM代理的受控本地第一运行时，以增强其安全性。


<details>
  <summary>Details</summary>
Motivation: LLM agents在工具使用方面作为执行系统，具备调用工具、修改本地状态、使用持久内存和与外部协议交互的能力。这些能力使代理有用，但也引入了与过度授权动作、审计性弱、提示注入、工具中毒和无控制的副作用相关的风险。本文提出了Agentao，这是一种为工具使用型LLM代理提供的受控本地第一运行时。

Method: Agentao采用分层架构，包括面向主机的界面、主机合约、运行时核心、权限介质的工具系统以及内存、回放、插件、技能、子代理和协议集成的支持子系统，将模型生成的动作建议与主机授权执行分离。

Result: Agentao不提供正式的安全性保证，而是展示了如何将权限、状态、协议边界和执行轨迹作为显式运行时抽象，构建更可治理、可检查且适合主机控制的本地环境中的代理。

Conclusion: Agentao通过分层架构和权限管理，提高了LLM代理的安全性、可审计性和可控性。
...
TL;DR: 转向评估人-人工智能团队的表现，以实现更好的社会成果。


<details>
  <summary>Details</summary>
Motivation: AI 评估的现有范式关注超人类自主性能，这隐含地目标是取代人类。

Method: 提出新的评估范式，即评估人-人工智能团队的表现。

Result: 促进人工智能系统作为人类能力的真正补充，从而带来比当前过程更好的社会成果。

Conclusion: AI 社区应转向评估人-人工智能团队的表现，以实现更好的社会成果。
...
TL;DR: 研究高置信度错误，发现稳定误校准的可能性，并通过审计和内部敏感性探测验证了局部稳定性。


<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型中高置信度错误的不同可能性：稳定误校准，即在高置信度错误在微小扰动下保持局部稳定。

Method: 结合两种诊断方法：标签感知的输出级审计分数和内部敏感性探测。

Result: 审计分数追踪了自我批评减少决策损失的地方，内部自我批评提示在三个开放权重模型中一致地减少了隐藏状态敏感性。

Conclusion: 支持提示诱导的局部稳定，而不是纯粹输出级的弃权模式，但并不表示校准：审计定义的过度自信错误并不比自信正确的答案更具有局部敏感性，因此一些高置信度错误可能是稳定的和误校准的，而不是简单的脆弱。
...
TL;DR: 开发了一种新指标来衡量代理行为的一致性，发现不同系统在不同任务上的一致性存在差异。


<details>
  <summary>Details</summary>
Motivation: 目前代理评估主要依赖于结果指标，如成功率，这些指标只捕获代理是否成功，而不反映其行为的一致性。

Method: 开发并训练一个模型来预测任务成功，从代理执行轨迹的行为特征中推导出每个轨迹的特征归因向量，并测量这些向量在一个代理系统内的平均成对相似度。

Result: 在从六个软件工程任务中的九千个轨迹中进行分析后，发现BCM是一个补充结果指标的过程级可靠性信号，可以帮助观察到跨任务和任务内一致性的分离。

Conclusion: 引入行为一致性指标(BCM)来衡量代理行为的一致性，并发现跨任务和任务内一致性是不同的维度，一些系统在本地可重复性上表现出色，但在不同任务间表现出不稳定性，而其他系统在两个尺度上都保持一致。
...
TL;DR: 该论文提出了一种基于主动感知的框架，用于解决具身环境中的目标模糊性问题，通过结合物理信息获取和用户意图澄清，提高了目标辨识的准确性和效率。


<details>
  <summary>Details</summary>
Motivation: 为了解决目标模糊性问题，该论文提出了一种基于主动感知的框架。

Method: 该框架利用主动观察获取信息，并使用视觉语言模型来决定是否继续观察、请求澄清或完成目标选择。

Result: 实验表明，该框架在统一的目标辨识过程中结合了物理信息获取和用户意图澄清。

Conclusion: 该方法有效地提高了在具身环境中的目标辨识准确性和效率。
...
TL;DR: MobileMem是一个用于研究设备上长期记忆的基准和框架，旨在解决AI代理在真实移动环境中的长期记忆问题。


<details>
  <summary>Details</summary>
Motivation: 为了解决现有基准在真实移动环境中的不足，以及AI代理需要长期记忆来积累和利用用户特定经验的问题。

Method: 提出MobileMem，一个基于一年规模移动体验的基准和框架，使用知识基础的合成流程从用户-app会话中构建连贯和时序一致的长时轨迹。

Result: MobileMem提供互补的文本和多模态设置，涵盖多跳和时序推理、知识更新和隐式偏好推理，使代理能够记住过去、理解现在、适应未来。

Conclusion: MobileMem将记忆从信息检索扩展到经验智能，以实现持续的个人学习。
...
TL;DR: 研究了预测模型更新导致的样本级回归的方法，发现信号有效性取决于任务，并提出了一种选择性回退机制。


<details>
  <summary>Details</summary>
Motivation: 研究如何从推理时间可用的信号中预测模型更新导致的样本级回归。

Method: 比较单模型信号和跨版本信号在统一的增值测试下的效果，测试内容包括选择题回答、数学推理和代码生成等三个任务家族的六个基准和六个模型更新对。

Result: 发现信号有效性取决于任务，且没有信号在所有模型更新中都是最佳。一些跨版本信号在置信度失效时仍具有信息性，支持一种概念验证选择性回退，将高风险样本路由回旧模型。

Conclusion: 根据不同任务选择合适的回归信号，并考虑使用跨版本信号作为置信度回退的辅助。
...
TL;DR: 提出了一种无需标记基准的评估学习 harness 的框架，验证了教师相对提升作为真实 harness 提升的代理。


<details>
  <summary>Details</summary>
Motivation: 在网络安全领域，评估具有检索或记忆功能的LLM结合的“持续学习 harness”系统（Agentic 'Continual Learning Harness' systems）的价值，传统上是通过与标记基准的增益来衡量，但这种做法在操作安全环境中往往失败。

Method: 提出一个无需标记基准的端到端评估学习 harness 的框架，基于扩展假设。一个更强的教师模型向一个带有持续学习 harness 的小学生提供稀疏样本的纠正。通过 harness 的学生随时间收敛到教师的情况来评分。

Result: 在安全任务、模型家族和 harness 设计方面，我们表明相对于教师模型的改进与相对于保留的黄金标准的改进相关，验证了教师相对提升作为无标签时真实 harness 提升的代理。

Conclusion: 这些结果表明，当人类提供相同类型的稀疏、高精度纠正时，教师大小的模型可以通过相同的 harness 进行改进。
...
TL;DR: 四种架构各有优势，正确性和效率之间存在权衡。


<details>
  <summary>Details</summary>
Motivation: 自然语言接口在企业数据中的应用需要将不明确的请求转换为受控的可执行行为，同时控制无效查询、策略失败、成本和非确定性。

Method: 使用SemPlan基准评估了四种架构：直接SQL生成、工具-代理基线、结构化语义请求生成、澄清/状态语义计划变体，并比较了它们的正确性和效率。

Result: A3架构在正确性方面表现最好，A1架构在策略正确率方面表现最佳，A4架构在API成本和拒绝率方面表现最佳。

Conclusion: 不同的架构在正确性和效率方面存在权衡，没有一种架构能够全面胜出。
...
### [184] [Handover of In-Context Learning State Across Session Boundaries](https://arxiv.org/abs/2608.14528)
*Masahiro Kato,Taka Kato*

Main category: cs.AI

TL;DR: 研究了会话切换的方法论和理论属性，提出了一个三部分记录，为决定保留内容和内存需求提供了理论和方法。


<details>
  <summary>Details</summary>
Motivation: 研究大语言模型应用中会话切换的方法论和理论属性。

Method: 将切换定义为任务相关的上下文学习状态的转移，并区分了早期材料的精确恢复和目标分布的保留。

Result: 分析了内存约束、作者和继续程序的影响，并量化了在已知下游查询之前写入的成本。提出了一个三部分记录，用于存储决策和约束，使用任务证明的统计信息进行重复证据，并保留原始观察结果。

Conclusion: 为决定会话切换必须保留的内容及其内存需求如何依赖于继续任务提供了理论和方法。
...
TL;DR: 通过语义归一化和模糊推理系统，该研究有效地验证了临床护理是否符合循证方案，并揭示了抗生素给药时间等关键问题。


<details>
  <summary>Details</summary>
Motivation: 验证临床护理是否符合循证方案是一个自然神经符号问题，但安全关键的环境使任何单一范式都无效。

Method: 提出一个专家引导的流程，将大型语言模型严格约束在语义归一化上，将混乱的药物和微生物字符串映射到固定的临床词汇，同时使用Sugeno模糊推理系统对归一化事件进行推理。模糊层编码了8个生存脓毒症运动包规则，并用[0,1]的分级评分代替二元判断。

Result: 应用于2,438个MIMIC-IV v3.1脓毒症病例，揭示了抗生素给药时间作为最关键的失败点（平均0.24，13%在一小时内），一小时内的表现不佳（平均36.7%），51%的乳酸下降，以及合规组之间ICU住院时间的描述性差异（3.8天与5.1天）。

Conclusion: 该研究提出了一种基于专家引导的流程，通过语义归一化和模糊推理系统，有效地验证了临床护理是否符合循证方案，并揭示了抗生素给药时间等关键问题。
...
TL;DR: 责任规则可能引发人工智能的不平等使用，导致投资扭曲和决策失衡。


<details>
  <summary>Details</summary>
Motivation: 人工智能在临床决策中的广泛应用引发了对算法准确度差异的关注，以及由此带来的法律责任问题。

Method: 研究人工智能算法设计决策和医生在医疗实践中使用人工智能的决策如何受到法律责任的影响。

Result: 发现责任规则可能引发人工智能的不平等使用，医生可能会减少人工智能的整体使用，并且在中等程度的责任压力下，可能会减少对弱势群体使用人工智能。随着责任压力的增加，医生对弱势群体使用人工智能的行为可能会先下降，然后随着公司重新分配投资以减少差异或转向平等准确度设计而上升。强制要求算法在患者群体中具有相同的准确度可能无意中伤害到两组人群，因为统一准确度要求扭曲了公司的投资激励和医生的平衡人工智能使用决策。

Conclusion: 责任规则可能会扭曲人工智能的投资激励和医生的平衡人工智能使用决策，导致不平等使用人工智能。
...
TL;DR: 时间JEPA具有HMM和PIB-VJEPA的计算结构，在序列学习方面具有优势


<details>
  <summary>Details</summary>
Motivation: 研究HMM和PIB-VJEPA的对应关系

Method: 提出Markov-Chain JEPA模型，进行实验验证

Result: 发现时间JEPA具有原则上的状态空间解释

Conclusion: 时间JEPA在处理序列学习方面具有优势
...
TL;DR: 提出了一种基于自动机的STL控制合成方法，提高了鲁棒性和满意度。


<details>
  <summary>Details</summary>
Motivation: 现代自主和AI系统缺乏准确和完整的系统模型，使得基于优化的合成方法不适用，从而推动了基于学习的控制的发展。

Method: 提出了一种基于自动机的创新方法，该方法通过构建时序交替自动机，从给定的STL规范中提供有效的内存机制和相关的马尔可夫奖励，适用于强化学习框架。

Result: 实验表明，该方法学习到的策略在鲁棒性和满意度方面优于使用基于鲁棒性奖励的现有方法学习到的策略。

Conclusion: 该方法为基于STL的鲁棒控制合成提供了一种有效的方法，适用于现代复杂系统。
...
TL;DR: 研究了隐藏状态信号的可解码性和因果可用性，结果受限于特定的预训练模型和测试条件。


<details>
  <summary>Details</summary>
Motivation: 研究隐藏状态信号的可解码性和因果可用性，以及它们在没有可重用动作映射支持下的行为。

Method: 通过证据格组织测试，在已知的仿射S_5载体上验证几何分支，并使用随机干预数据集进行实验。

Result: 在测试的载体中，状态可用性、因果使用、局部几何和可重用闭合是可区分的。结果仅限于一个预训练模型、样本最终标记层、两个有限世界以及测试的仿射或诊断功能类别。

Conclusion: 在测试的载体中，状态可用性、因果使用、局部几何和可重用闭合是可区分的。结果仅限于一个预训练模型、样本最终标记层、两个有限世界以及测试的仿射或诊断功能类别。
...
TL;DR: ConceptFlow可视化Eurovision Song Contest的投票模式与音乐特征之间的关系


<details>
  <summary>Details</summary>
Motivation: Construct and render nested line diagrams from many-valued formal contexts

Method: Conceptual scaling, factor lattices computation, subdirect product identification, interactive visualization

Result: Nested line diagram of Eurovision Song Contest winners, revealing voting pattern-musical characteristic relationships

Conclusion: ConceptFlow can effectively visualize the relationship between voting patterns and musical characteristics in the Eurovision Song Contest
...
TL;DR: MOOSEDev是一种追踪软件项目代码变更原因的系统，在性能上优于基准工具。


<details>
  <summary>Details</summary>
Motivation: 为了追踪软件项目中代码变更的原因，提出了一种名为MOOSEDev的系统。

Method: MOOSEDev系统通过知识图谱记录架构决策、经验教训、约束和理由，并通过MCP接口向代理提供。

Result: 与基准向量内存工具相比，MOOSEDev在超替、集合完备性和否定问题上的预期答案集几乎完整（0.98-1.00），而基准的前k个检索结果介于6%到27%之间。

Conclusion: MOOSEDev在处理代码变更原因追踪方面优于基准工具。
...
TL;DR: Second Thought框架通过并行推理提高了LLM代理的推理效率。


<details>
  <summary>Details</summary>
Motivation: 为了提高LLM代理在ReAct范式中的推理效率，研究者们提出了Second Thought框架。

Method: Second Thought框架在Thought阶段结束后立即启动四个辅助分支进行推理，并与主循环并发解码，然后将生成的推理结果合并。

Result: Second Thought框架在三个代理基准和三个推理LLM上降低了平均回合数，并在六个基准上减少了主线程解码时间。

Conclusion: Second Thought框架有效地提高了LLM代理的推理效率。
...
TL;DR: 研究通过自然语言传达语义约束在组装任务中更有效。


<details>
  <summary>Details</summary>
Motivation: 在部署后，智能体遇到结构上的语义约束，这些约束在训练期间不可用，且智能体最初不知道相关的结构和组件概念。

Method: 在模拟的玩具卡车组装领域中研究智能体通过用户交互获得并利用这种知识。

Result: 通过自然语言（例如，“自卸车有斗”）传达语义约束比仅依靠任务演示和/或仅通过自然语言命名部件具有更高的数据效率。

Conclusion: 该研究证明了通过自然语言交流语义约束在数据效率方面优于仅依赖任务演示或仅命名部件的方法。
...
### [195] [Regime-Conditional Verification: Correctness Estimation for Adapting and Monitoring Safety Classifiers](https://arxiv.org/abs/2608.14089)
*Thiago Sandoval,Ufuk Topcu*

Main category: cs.AI

TL;DR: RCV通过估计预测与部署者策略不一致的概率，选择性地纠正可能错误的预测，从而提高了安全分类器的性能和准确性。


<details>
  <summary>Details</summary>
Motivation: 安全分类器在部署时经常失败，原因有两个：它们的决策反映了训练期间学习的策略，而不是部署者的期望策略，并且随着部署流量的演变，其性能会下降。

Method: 提出了一种名为Regime-Conditional Verification (RCV)的轻量级包装器，该包装器可以适应现成的安全分类器，而无需重新训练。RCV从分类器的内部表示中估计每个预测与部署者策略不一致的概率，并选择性地纠正可能错误的预测。相同的正确性估计还可以提供无标签的信号，用于检测分布偏移，从而实现一个维护循环，该循环更新正确性估计层，仅在必要时才进行分类器微调。

Result: 在三个现成的安全分类器和两个基准数据集上，RCV在每种分类器-数据集组合中提高了对部署者策略的遵守，在不需要修改底层分类器的情况下，捕捉到多达0.81的先前未检测到的危险内容。在针对十个攻击活动的部署研究中，每个活动都是RCV训练中保留的损害类别，RCV在专门的注入面板中检测到每个活动；在维护普查中，大多数漂移事件在没有更新分类器的情况下得到修复，微调仅用于修复无法恢复的残留事件。

Conclusion: RCV是一种有效的轻量级包装器，可以显著提高安全分类器的性能和准确性，同时减少对分类器微调的需求。
...
TL;DR: 提出了一种自适应的失败发现方法，提高了失败发现的效率。


<details>
  <summary>Details</summary>
Motivation: 在有限的测试预算下，自主系统可能会以罕见和异质的方式失败，这使得在实际世界中发现失败变得困难。

Method: 提出了一种自适应的失败发现方法，该方法结合了代理评估和有限的靶系统结果，以指导靶系统测试的场景选择。通过使用控制变量启发的残差建模来纠正代理失败信号，该方法学习了一个靶风险的局部预测器。为了找到既可能又多样化的失败，该方法将预测器与一个支持感知的互信息目标相结合，该目标有利于现实、有支持的区域，同时扩展了覆盖范围。

Result: 在自动驾驶、操作和四足速度跟踪任务中，该方法发现的失败数量是随机采样和主动学习基线的2倍，包括被竞争方法遗漏的严重和多样化的失败。

Conclusion: 该方法有效地利用代理系统信息，准确预测严重靶系统失败，提高了失败发现的效率。
...
### [197] [QuaSAR: Quantization Compensation via Stable Activation-Aware Rank Truncation](https://arxiv.org/abs/2608.14149)
*Lin-Fa Lee,Yi-Yu Chang,Kuo-Hei Yeh*

Main category: cs.AI

TL;DR: 提出了一种新的训练后量化方法，该方法通过改进残差补偿和门控机制，在保证模型精度的同时，显著减少了模型大小。


<details>
  <summary>Details</summary>
Motivation: 为了提高模型精度并限制额外的模型存储开销，最近的一些训练后量化方法通过闭式残差补偿来恢复模型精度。

Method: 现有的方法通过拟合优度来选择门控层，仅保留那些补偿产生正残差拟合分数的层，并丢弃其余层。然而，在低比特W4A4设置下，这种门控机制无法区分难以预测的量化误差和数值求解器失败。为了解决这个问题，提出了一种参数免费的截断伪逆求解器，该求解器在求逆之前删除了折叠方向。

Result: 在ViT-B上，该方法在W4A4设置下达到了81.42%的top-1精度，优于之前的方法和基于微调的基线。结合联合低秩和量化压缩，该方法在54.7 MB的模型大小下达到了80.26%的精度，在模型大小和精度之间提供了一个良好的平衡。

Conclusion: 该方法有效地提高了模型精度，同时减少了模型大小，为模型部署提供了新的解决方案。
...
TL;DR: 机制可解释性证据在多种分析条件下表现不稳定。


<details>
  <summary>Details</summary>
Motivation: 研究欧盟AI法案要求高风险系统提供商提供技术文档，描述系统如何做出决策。

Method: 进行交叉网格分析，对GPT-2小模型和间接宾语识别任务进行15,840个预注册规格的分析。

Result: 发现证据在可接受的容差范围内无法通过可提交性标准，电路结构几乎不重叠，功能上不相关。

Conclusion: 机制可解释性证据在多种分析条件下表现不稳定，需要进一步研究。
...
TL;DR: 一种新的模拟感知LLM多智能体框架在模拟电路布局优化中提高了效率。


<details>
  <summary>Details</summary>
Motivation: 模拟驱动的设计过程在布局优化阶段仍然需要大量的迭代和专家干预，而现有的基于贝叶斯优化和大型语言模型的方法存在效率和实用性问题。

Method: 提出了一种模拟感知的LLM多智能体框架，通过迭代更新布局优化参数，并使用行为-观察-反思循环在紧凑的结构化布局表示上执行上下文策略改进（ICPI）。

Result: 实验表明，该方法在真实世界模拟电路上的表现优于现有的方法，减少了后布局模拟次数并提高了性能。

Conclusion: 通过在上下文中迭代更新布局优化参数，我们的方法在仅进行几十次后布局模拟的情况下，提高了后布局性能，超过了生成器的内置启发式方法和基于贝叶斯优化的调优方法。
...
TL;DR: FLARE MCMC是一种基于低保真度近似计算的MCMC方法，可提高混合速度和计算效率。


<details>
  <summary>Details</summary>
Motivation: MCMC方法在复杂模型推断中应用广泛，但存在混合速度慢、计算成本高等问题。

Method: FLARE MCMC是一种多保真度分层MCMC方法，利用低保真度近似计算真实似然值，提高混合速度，降低计算成本。

Result: FLARE MCMC在不同科学领域（如水文和宇宙学）中，在相同计算时间内获得更大的有效样本量。

Conclusion: FLARE MCMC是一种有效的MCMC方法，可以显著提高计算效率。
...
TL;DR: SocialRL方法在多个领域提升AI代理的社交推理能力，达到或超过现有模型的水平。


<details>
  <summary>Details</summary>
Motivation: AI代理在代表用户执行任务时，可能面临与对方利益冲突的情况，而友好的助手可能会无意中泄露用户信息或轻易放弃。

Method: 提出SocialRL方法，直接训练社交推理，并在6个领域（ Deal-or-No-Deal, CaSiNo, Craigslist, Job Interview, Calendar, Marketplace）进行应用和评估。

Result: 在领域内训练达到前沿水平，跨领域迁移遵循游戏结构，两种策略（级联RL和多头教师在线策略蒸馏）将各个领域的专家整合为一个统一的4B模型，达到0.627的平均效用，与GPT-4.1、GPT-5.1和GPT-5.2相当。

Conclusion: SocialRL方法能够有效训练AI代理进行社交推理，提高其在复杂任务中的表现。
...
TL;DR: SDO通过子空间去冲突技术，有效提升了多适配器组合的性能


<details>
  <summary>Details</summary>
Motivation: 为了解决多适配器组合中身份混淆、属性泄漏和场景组成不稳定的问题

Method: 提出了一种名为SDO的子空间去冲突算子，用于多适配器组合

Result: SDO通过重构层级的低秩更新、提取紧凑的子空间特征、测量成对冲突和施加置换等变变换，提高了身份保真度和组成稳定性

Conclusion: SDO在多适配器组合中表现良好，特别是在适配器数量增加时效果更明显
...
TL;DR: 本文提出了一种基于内存和计算频率联合优化的DNN推理方法，显著降低了能耗并提高了性能。


<details>
  <summary>Details</summary>
Motivation: 为了实现高效的DNN推理，大多数现有研究集中于动态电压和频率缩放（DVFS）来调整计算频率，而忽略了内存频率对推理性能的影响。

Method: 基于现实推理时间模型，我们建立了一个优化问题，以最小化所有移动设备在截止日期约束下的能耗。对于本地推理，我们通过凸优化推导出一个近最优的闭式解，而对于边缘推理，在给定带宽的情况下，我们获得了一个最优的闭式解。此外，我们还提出了一种低复杂度的启发式算法，以有效地解决整体问题，具有多项式时间复杂度。

Result: 仿真结果表明，与最优解相比，提出的近最优解可以在严格的截止日期约束下实现最佳性能，性能差距高达2.5%。同时，与现有方法相比，我们的算法将设备的能耗降低了高达10.4%。

Conclusion: 本文提出的方法在降低能耗和提高推理性能方面取得了显著成果。
...
TL;DR: 提出了一种新的架构模式来解决LLM文本到SQL系统在部署中的可靠性问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在自然语言接口数据库（NLIDB）中的应用，但LLM文本到SQL系统在部署中存在问题：幻觉列或错误汇总导致流畅的错误答案，与正确答案在应用时无法区分。

Method: 提出了一种基于可信内核和生成外壳的架构模式，该模式基于一个不变量：可以制造组件可能影响系统回答的问题，而不是返回的值。生成外壳解释不明确的输入并表达回复；确定性内核将完全指定的查询与有限的答案形状集进行匹配，并通过确定性执行将它们编译为查询。

Result: 实现了模式，给出了五个决策配方，并在三个领域进行了工作，将不变量从返回值扩展到代理系统的行为，并报告了两年生产案例研究和两种生成替代方案：微调解析器和工具检索代理。

Conclusion: 提出了一种新的架构模式，用于解决LLM文本到SQL系统在部署中的可靠性问题，并通过案例研究证明了其有效性。
...
TL;DR: AI研究偏好模型（RPMs）通过预测最有价值的候选解决方案，提高了机器学习研究任务的性能。


<details>
  <summary>Details</summary>
Motivation: AI研究代理（AIRA）在提出、实施和评估自己的机器学习实验方面取得了进展，但前沿任务的进展受到成本的限制。

Method: 引入AI研究偏好模型（RPMs），通过预训练语言模型构建，包括推理模型和代理模型。

Result: 两种模型将平均标准化分数从0.684提高到0.711和0.729，并在约15小时内达到无指导代理的24小时性能，使用不到三分之二的执行预算。

Conclusion: AI研究偏好模型（RPMs）有效提高了机器学习研究任务的性能。
...
TL;DR: HELIX 通过 harness 进化提高了智能体能力，实现了 harness、模型和数据之间的反馈系统。


<details>
  <summary>Details</summary>
Motivation: 提高智能体能力主要关注改进模型，但交互式智能体通过运行时 harness 进行操作，该 harness 中介了上下文、工具、控制流和停止。harness 形塑了模型可以完成的事情以及学习轨迹。这种耦合促使模型-harness 共同进化以实现递归自我改进：为固定模型构建 harness，从经过验证的兄弟轨迹中更新模型，并根据模型能力的变化重建 harness。实现这个循环需要一种受控的方式来进化 harness，同时保持干预身份和效果。

Method: 提出 HELIX，一种可追溯的 harness 进化基础。HELIX 将智能体系统分解为类型化的端口、可重用的原子、配方、产品外壳和运行时策略。它使干预明确且可审计，同时保留轨迹、测试结果和来源。harness 进化因此承担两个相互关联的角色：提高固定模型执行和产生匹配的成功、回归、近错和替代解决方案作为后续模型改进的数据。

Result: 在代码修复的进化一轮中评估 HELIX。一个包含 65 个候选者的投资组合发现了一个固定的 harness，将任务覆盖率提高了 4.0%。完整的投资组合通过互补的兄弟行为揭示了高达 58.0% 的更多验证覆盖率。选定的候选者通过重复运行和 SWE-bench 评估器进行评估。200 个插槽的兄弟切片产生了 438 个验证 SFT、评论家、过滤器和偏好记录。这些结果表明 harness、模型和数据如何形成一个反馈系统：harness 进化扩展了当前能力并为下一个模型创造了学习信号；模型更新激励了下一轮 harness 进化。HELIX 为研究这个递归过程提供了一个可审计的接口。代码可在 https://github.com/HKUDS/HELIX 获取。

Conclusion: HELIX 通过提供一种可审计的接口来研究 harness 进化的递归过程，实现了 harness、模型和数据之间的反馈系统，从而提高了智能体能力。
...
TL;DR: 研究探讨了使用形式化法律方法治理不可完全检验推理的AI系统的可能性


<details>
  <summary>Details</summary>
Motivation: 如何治理我们无法完全检验推理的AI系统

Method: 使用形式化方法，例如Reified Input/Output Logic，并测试GPT-4在法律语句翻译中的表现

Result: 证明了计算法可以作为治理工具，并提出了将法律形式化以实现程序执行的目标

Conclusion: 通过形式化规则，AI系统可以按照法律进行治理，这为AI的透明度和可审计性提供了保障
...
TL;DR: Nanbeige4.2-3B模型通过循环Transformer提高参数效率，但需修复漏洞和优化以实现可靠评估。


<details>
  <summary>Details</summary>
Motivation: 为了提高参数效率，构建了基于循环Transformer的Nanbeige4.2-3B模型。

Method: 通过在第二个前向传递中重用一层层，增加了有效深度而无需额外参数。

Result: 发现并修复了五个独立漏洞，提高了模型在Apple Silicon上的运行效率。引入了分块预填充策略，将允许的上下文宽度提高了2.7倍。

Conclusion: 尽管修复了漏洞，但Nanbeige4.2-3B在执行代理任务时仍需进一步优化。
...
TL;DR: 提出内容深度评分指标，评估短视频推荐系统，发现现有系统存在局限性


<details>
  <summary>Details</summary>
Motivation: 针对短视频推荐系统偏好浅内容视频的问题，提出了一种新的内容深度评分指标（CDS）

Method: 提出内容深度评分指标（CDS），构建了第一个短视频内容深度评估基准SCOPE-Bench，并使用该基准评估了13个代表性的推荐系统

Result: 发现现有推荐系统普遍偏好浅内容视频，推荐认知深度内容的效果仅略好于随机选择

Conclusion: 现有推荐系统存在推荐认知深度内容方面的局限性，需要进一步改进
...
TL;DR: 提出了一种基于模拟的方法，通过使用虚拟传感器增强数据集来解决稀疏数据下机器学习模型泛化问题，并在实际城市中得到验证。


<details>
  <summary>Details</summary>
Motivation: 城市交通管理依赖于受部署成本和隐私法规限制的空间覆盖有限的传感器网络。在稀疏数据上训练的机器学习模型无法泛化到未监控的位置，并且必须在使用传感器基础设施更改时重新训练。

Method: 提出了一种基于模拟的方法，通过生成增强的交通计数数据集来解决此问题，其中每个物理传感器都被放置在道路网络中替代位置上的虚拟传感器所取代。虚拟传感器是通过一种图搜索启发式算法选择的，该算法共同最大化原始位置和替代位置之间的车辆流量连续性和交通指标相似性，同时强制执行最小空间位移以确保观察到的交通状况的多样性。

Result: 在两个比利时城市：布鲁塞尔和那慕尔上验证了该方法。增强的数据集保留了观察位置上每日需求的双峰分布和交通的动态。

Conclusion: 该方法可以有效地生成增强的交通计数数据集，以解决稀疏数据下机器学习模型泛化问题，并为城市交通管理提供更全面的数据支持。
...
TL;DR: 通过分析发现，Transformer-based sequential recommenders在推理时对最近一次交互有高度依赖，并揭示了残差优势现象，通过调整残差强度可以控制这种依赖


<details>
  <summary>Details</summary>
Motivation: 研究Transformer-based sequential recommenders with causal self-attention在推理时对最近一次交互的依赖行为

Method: 结合预测时间诊断和基于范数的全注意力块分析

Result: 发现SASRec-style模型表现出高度局部化的最后一项依赖，并揭示了残差优势现象，通过调整残差强度来控制这种依赖

Conclusion: 提供了将极端的最后一项依赖与推理时的残差优势联系起来的结构解释
...
TL;DR: 本文探讨了在芯片设计中使用AI的可能性，提出将超级智能体建模为AI组织的新方法。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）和工具使用智能体的最新发展，鼓励人们探索在芯片设计中使用智能体的潜力。核心问题是我们在这样一个复杂的行业中真正需要什么样的AI。

Method: 将芯片设计超级智能体建模为庞大的AI组织。

Result: 对芯片设计领域中使用AI的方法进行了探讨。

Conclusion: 通过将芯片设计超级智能体建模为AI组织，为该行业在AI应用方面提供了新的思路。
...
TL;DR: 通过控制实验，研究了技能在LLM代理推理时的作用，发现技能在稳定动作和提供程序锚点方面有效，但在检索和适应方面存在瓶颈。


<details>
  <summary>Details</summary>
Motivation: 通过结构化的知识包增强LLM代理在推理时的技能已成为一种实用且有效的方法。然而，现有的评估主要衡量技能是否提高了聚合任务的成功率，而一个更基本的问题却很少被探讨：技能何时有帮助，为什么它们有效，以及它们在哪里失败？

Method: 通过在各种基准、代理和LLM上进行控制实验，我们隔离了表示、结果注释、检索难度和技能的跨框架鲁棒性的影响。为了进一步回答这个问题，我们设计了一项对比研究，将控制定量实验与配对轨迹分析相结合。

Result: 我们标准化了来自控制实验的8,135个试验记录，并从240个开放编码记录中保留了238个有效的唯一标签。我们将这些观察结果整合为三个高级类别和十二种技能使用模式的分类法：技能在嘈杂轨迹成为程序锚点以稳定执行时起作用。在匹配比较中，技能在Workflow Memory上的提高为6.06分。程序锚定占技能案例的65.7%，而显式知识注入仅占4.5%，这表明技能稳定了动作而不是注入缺失的事实。检索是一个独立的瓶颈：随着池的大小从5增加到100，实际使用精度从29.6%下降到3.3%。可混淆的干扰项损害了离线识别，但下游成功保持稳定；精确的地面实况调用既不充分也不必要。技能在脆弱的假设、不兼容的上下文或不足的适应下失败。这些发现将评估超越了聚合成功率，并指导可靠的自我进化代理。

Conclusion: 这些发现将评估超越了聚合成功率，并指导可靠的自我进化代理。
...
TL;DR: 比较了多种机器学习方法在材料建模中的应用，没有明确的最优方法，强调了每种方法的优缺点


<details>
  <summary>Details</summary>
Motivation: 为了比较不同机器学习方法在材料建模中的应用效果

Method: 使用Treloar的经典实验数据对超弹性模型进行基准测试，比较了多种框架，包括广义不变人工神经网络、物理增强神经网络、自适应材料指纹识别和高效无监督本构定律识别与发现

Result: 所有方法都能很好地再现基准数据，没有明确的最优方法，而是强调了每种方法的优缺点，并提供了实际应用的指导

Conclusion: 机器学习方法在材料建模中具有巨大潜力，但需要根据具体应用选择合适的方法
...
TL;DR: 研究发现，在固定TPP下，最优重复次数与模型大小正相关，且与领域验证损失负相关。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型的规模扩大，其训练令牌预算也必须增加以保持适当的每参数令牌数（TPP）。然而，高质量领域数据比通用网络数据更难扩展。随着模型大小和训练令牌预算的增加，其在训练混合中的比例往往会降低。重复现有高质量数据是有效对抗这种稀释的方法，但过度重复可能导致过拟合。我们研究了在实用的大型语言模型扩展下的这种权衡，其中训练令牌预算与模型大小成比例增长。对于固定的领域，我们发现，在固定的TPP下，最优重复次数与模型大小呈轻微正相关。在不同领域之间，我们发现最优重复次数与领域的最终验证损失呈强烈负相关：损失较低的领域通常可以从更多的重复中受益。相比之下，独特领域数据量与最优重复次数只有弱相关性。这些发现表明，在具有相同TPP的小型代理模型上调整的重复次数可以为大型模型提供一个实用的估计。

Method: 我们研究了在实用的大型语言模型扩展下的这种权衡，其中训练令牌预算与模型大小成比例增长。对于固定的领域，我们首先发现，在固定的TPP下，最优重复次数与模型大小呈轻微正相关。在不同领域之间，我们发现最优重复次数与领域的最终验证损失呈强烈负相关：损失较低的领域通常可以从更多的重复中受益。相比之下，独特领域数据量与最优重复次数只有弱相关性。

Result: 我们发现，在固定的TPP下，最优重复次数与模型大小呈轻微正相关。在不同领域之间，最优重复次数与领域的最终验证损失呈强烈负相关：损失较低的领域通常可以从更多的重复中受益。相比之下，独特领域数据量与最优重复次数只有弱相关性。这些发现表明，在具有相同TPP的小型代理模型上调整的重复次数可以为大型模型提供一个实用的估计。

Conclusion: 这些发现表明，在具有相同TPP的小型代理模型上调整的重复次数可以为大型模型提供一个实用的估计。
...
TL;DR: Mandato是一种基于协议级别的治理代理，通过数字签名命令确保AI代理的授权透明性和可审计性。


<details>
  <summary>Details</summary>
Motivation: AI代理越来越多地通过标准化的工具调用协议（如MCP）对外部系统进行操作，但没有基础设施层限制其行为在主要方已验证授权的范围内：授权逻辑存在于应用程序代码中，既未签名也无可独立审计，生成的日志缺乏证据价值。

Method: 提出Mandato，一种治理代理，在协议级别强制执行对代理行为的数字签名命令。命令是一个机器可读的、加密签名的授权工件，指定代理可以调用哪些工具，在哪些参数约束和上下文条件下，多长时间，代表谁；代理评估每个工具调用与适用的命令链，在线阻止不符合的调用，并将每个决策（允许、拒绝以及每个的证据）记录在只追加、哈希链式审计日志中，该日志设计用于证据使用，并通过合格时间戳定期锚定。

Result: Mandato通过在协议级别强制执行对代理行为的数字签名命令，提高了AI代理的治理水平，确保了授权的透明性和可审计性。

Conclusion: Mandato为AI代理的治理提供了一种新的解决方案，有助于提高AI代理的安全性和可靠性，并符合相关法律法规要求。
...
### [221] [MathForm: Scaling Mathematical Autoformalization with Knowledge Retrieval and Verification-Guided Refinement](https://arxiv.org/abs/2608.14221)
*Lushi Pu,Weiming Zhang,Xinheng Xie,Zixuan Fu,Bingxiang He,Hengyu Zhao,Hongya Lyu,Xin Li,Jie Zhou,Yudong Wang*

Main category: cs.AI

TL;DR: MathForm框架通过Mathlib知识检索和迭代改进，实现了高效的自动形式化，并在基准测试中表现出色。


<details>
  <summary>Details</summary>
Motivation: 为了解决现有自动形式化方法在忠实形式化方面的挑战，如模型对库特定知识的依赖和缺乏反馈驱动的修订机制。

Method: 引入MathForm框架，通过Mathlib知识检索和验证引导的迭代改进来构建经过验证的训练数据。

Result: 构建了FormalVerse数据集，包含约367K个经过验证的示例，并在六个基准测试中取得了优异的性能。

Conclusion: MathForm框架在自动形式化方面取得了显著成果，为数学领域提供了有效的工具。
...
### [222] [Grounding Without Corrective Control: Truth-Tracking Profiles for Large Language Models](https://arxiv.org/abs/2608.14252)
*Brett Reynolds*
...
TL;DR: RGLT通过连接潜在推理轨迹与检索改进，提高了推理密集型检索的性能。


<details>
  <summary>Details</summary>
Motivation: 为了提高推理密集型检索的性能，需要文本表示不仅捕获语义相似性，还要捕获确定相关性的推理。

Method: 提出了一种名为RGLT的潜在推理框架，该框架通过指令条件下的潜在推理轨迹在隐藏空间中执行非自回归推理。它结合了过程监督的显式到隐式的蒸馏和基于检索的监督，使用阶段性的CoT重建来塑造中间潜在状态，并使用检索效果信用来优化潜在推理轨迹上的增量检索收益。

Result: 在推理密集型检索基准测试中，RGLT在保持高效嵌入推理的同时，始终优于强大的基线。

Conclusion: RGLT通过连接中间潜在转换与检索改进，有效地提高了推理密集型检索的性能。
...
TL;DR: 提出了一种基于图的框架，用于检测和恢复自主LLM代理的运行时行为漂移。


<details>
  <summary>Details</summary>
Motivation: 为了解决自主LLM代理在复杂现实工作流程中的运行时行为漂移问题，以及现有方法在提示级别处理漂移但缺乏结构化机制的问题。

Method: 提出了一种基于图的框架，其中单个小型语言模型通过强化学习在每个恢复图节点上进行训练，以专门处理漂移分类、操作检测、风险评估或最终决策。模型学习生成适应其角色的结构化XML格式推理。训练结合基于规则的奖励与LLM作为判断的语义质量信号。

Result: 在公共AppWorld基准测试中，该方法通常利用有关疑似漂移开始的信息，使用小型语言模型发布正确的恢复决策。训练的小型语言模型可靠地遵循规定的输出模式，并根据其分配的节点角色在每个字段中产生语义适当的内容。

Conclusion: 该研究提出了一种基于图的框架，用于检测和恢复自主LLM代理的运行时行为漂移，通过使用小型语言模型实现了有效的恢复决策和语义适当的内容生成。
...
### [225] [AnchorBench: A Multi-Pathway Benchmark for the Anchoring Effect in LLMs](https://arxiv.org/abs/2608.14320)
...
TL;DR: 本研究通过强化学习在复杂制造环境中进行生产调度，实现了关键性能指标的平衡改进。


<details>
  <summary>Details</summary>
Motivation: 生产调度在复杂的制造环境中具有挑战性，需要同时解决序列依赖的设置时间、随机干扰和截止日期限制。

Method: 使用强化学习方法，利用开源数字模型游乐场（DMPG）模拟场景并训练强化学习代理，并与传统调度规则进行比较。

Result: 基于强化学习的调度在关键性能指标方面取得了平衡的改进，其中Proximal Policy Optimization（PPO）表现最稳健。

Conclusion: 这项工作的主要贡献是通过在一个现实且可共享的场景中验证基于强化学习的调度，并提供了一个可重用的开源框架，以弥合学术界研究与企业实践之间的差距。
...
TL;DR: 提出了一种新的视觉世界模型Traj-LeWM，通过引入LTC解决了LeWM的局限性，并在多个任务上取得了显著的性能提升。


<details>
  <summary>Details</summary>
Motivation: 为了解决LeWM的局限性，如未评估完整轨迹相对于任务目标的相对位置，以及仅根据预测终点距离对候选序列进行排序的问题。

Method: 提出Traj-LeWM，保留LeWM的局部动力学目标和终点得分，同时引入基于目标条件的潜在轨迹成本（LTC），以聚合轨迹级信息作为补充信号。在训练期间，基于LTC的轨迹偏好监督补充了下一步预测，以塑造共享表示。在规划期间，将LTC与终点距离相结合，将中间路径信息纳入候选排序。

Result: 在Push-T、OGBench-Cube、Reacher和Two-Room任务上，Traj-LeWM分别比LeWM提高了3、14、7和7个百分点的性能。

Conclusion: Traj-LeWM通过引入LTC有效地解决了LeWM的局限性，并在多个任务上取得了显著的性能提升。
...
### [229] [LLMs Don't Pay for the Jump](https://arxiv.org/abs/2608.14397)
*Paras Balani,Subhrakanta Panda*

Main category: cs.AI

TL;DR: 机器推理缺乏认知错误与物理成本的耦合，导致无法进行有效的归纳和演绎。


<details>
  <summary>Details</summary>
Motivation: 提出数学理论，预测有限测量量的无限能量，无法被物理接受。

Method: 通过热力学耦合形式化区分，展示固定权重变压器推理缺乏这种耦合。

Result: 输出熵在任务因果难度急剧增加的情况下几乎保持不变，即使准确率从100%下降到17%。

Conclusion: 机器归纳和演绎无法产生假设，其采用需要认知错误与物理成本的耦合。
...
TL;DR: 提出了一种基于VideoLLaMA2的多模态核心意见提取框架，有效提高了科技情报领域的意见提取准确率。


<details>
  <summary>Details</summary>
Motivation: 针对科技情报领域中的意见提取（OE）需求，现有模型难以从大量信息流中过滤噪声并生成结构化输出。

Method: 提出一个多模态核心意见提取框架，使用VideoLLaMA2和VideoLLaMA2.1作为基础模型，在2,194个多语言和多模态样本的精选数据集上进行QLoRA微调。

Result: 在图像增强设置下，微调后的VL2.1生成结构化JSON核心意见输出，达到64.98%的精确度、42.15%的召回率、51.14%的F1分数和74.00%的样本级准确率。与零样本VL2.1设置相比，西班牙语和俄语的F1分数分别从4.83%和0.45%提高到46.05%和51.93%。框架还集成了基于模糊累积前景理论的提取后三级模块，用于案例级价值评估，为下游科技情报筛选提供案例级价值信号。

Conclusion: 该框架有效地提高了多语言和多模态意见提取的准确性和效率，为科技情报领域提供了有效的工具。
...
### [231] [Split the Labor: Separating Evidence Interpretation from Decision Aggregation](https://arxiv.org/abs/2608.14509)
*Zhelun Wu*
...
### [232] [Removing Temporal Note Redundancy Improves Multimodal Reinforcement Learning for Medicine](https://arxiv.org/abs/2608.14157)
*Chenran Weng,Joo Seung Lee,Malini Mahendra,Anil Aswani*

Main category: cs.AI

TL;DR: 本文提出了一种去除冗余文本的方法，显著提高了基于强化学习的机械通气决策支持。


<details>
  <summary>Details</summary>
Motivation: 为了解决强化学习在机械通气中的应用问题，本文提出了一种新的状态表示框架。

Method: 提出了一种冗余感知的多模态状态表示框架，用于去除冗余文本。

Result: 该框架显著提高了基于强化学习的临床决策支持的性能。

Conclusion: 通过隔离新临床信息，可以显著提高状态表示的质量，并直接改善强化学习性能。
...
### [233] [BiasTrace: Linking Reasoning Behaviours to Biased Outputs in LLMs](https://arxiv.org/abs/2608.14161)
*Varsha Ramineni,Hossein A. Rahmani,Jerome Ramos,Karin Sevegnani,Emine Yilmaz*

Main category: cs.AI

TL;DR: 提出BiasTrace，用于分析LLMs中的偏见，发现偏见通常源于微妙的推理行为。


<details>
  <summary>Details</summary>
Motivation: LLMs存在社会偏见，可能导致不准确和歧视性的推断，在高风险应用中存在风险。

Method: 提出BiasTrace，一种用于标注模型生成痕迹中的推理行为并将其与偏见结果关联的方案。

Result: 分析表明，偏见输出通常源于微妙的推理行为，而不是明确带有偏见的语言，并且推理级别的标注可以改善偏见检测。

Conclusion: 这些发现强调了检查更广泛的推理模式以更好地理解LLMs中的偏见的重要性。
...
### [234] [Can Language Models Understand mmWave Data? Benchmarking Large Language Models for mmWave Radar-Based Human Understanding](https://arxiv.org/abs/2608.14179)
*Jeongwan Shin,Jaehyeon Kim,Donguk Ko,Jaeho Choi*

Main category: cs.AI

TL;DR: 提出mmWave-QA基准，评估LLMs在毫米波感知中的推理能力。


<details>
  <summary>Details</summary>
Motivation: 探索将大型语言模型（LLMs）与毫米波（mmWave）模式相结合，以利用LLMs的推理和生成能力进行感知。

Method: 提出了一种最小化文本化接口，将每个毫米波点云序列化为简洁的自然语言，使现成的LLMs能够在问答（QA）环境中运行。构建了mmWave-QA，这是第一个用于语言条件毫米波人类感知的基准。

Result: 建立了mmWave-LLM集成的可扩展研究的基础，并突出了LLMs在雷达感知中的零样本推理潜力以及它们在视觉退化下的鲁棒性。

Conclusion: mmWave-QA为毫米波-LLM集成研究提供了标准化评估平台，有助于推动该领域的进一步发展。
...
### [235] [FreeBalance: Pre-Routing Online Moe Load Balancing via Residual Workload Prediction](https://arxiv.org/abs/2608.14205)
*Pengfei Chen,Yize Wu,Shouxu Kuang,Ke Gao,Ling Li*

Main category: cs.AI

TL;DR: FreeBalance通过预测路由分布，在路由决策可用之前进行主动专家迁移规划，显著减少了负载平衡开销。


<details>
  <summary>Details</summary>
Motivation: 负载不平衡是MoE模型分布式推理中专家并行效率的主要瓶颈。

Method: 提出FreeBalance，一个无损耗的在线负载平衡框架，通过残差工作量预测将专家迁移与先前的计算阶段重叠。

Result: FreeBalance减少了最大到平均排名负载比32.8%，并减少了端到端预填充延迟13.1%。

Conclusion: FreeBalance通过预测路由分布，在路由决策可用之前进行主动专家迁移规划，从而显著减少了负载平衡开销。
...
### [236] [APTER: Adaptive Post-Training with Expert-Grounded Rubrics](https://arxiv.org/abs/2608.14212)
*Xukai Wang,Liangqi Li,Zhiyue Xu,Jingang Zhou,Xiaoyu Shi,Jiansheng Cai,Bo Zhang,Zhe Li,Xu-Yao Zhang*

Main category: cs.AI

TL;DR: APT器是一种有效提高大型语言模型在专业领域推理能力的方法。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在专业领域的应用需要满足领域约束、包含关键证据和提供完整推理，而非仅仅生成流畅的回应。

Method: APT器（自适应后训练与专家构建的评分标准）框架，将结构化领域知识整合到细粒度评估、优化和诊断中。专家构建的评分标准从领域专家构建的专家标准框架开始，每个标准代表一个稳定的职业技能。对于每个查询，APT器选择相关标准并将它们实例化为查询级别的评分标准，与它们的源标准相关联，将可重用的专家标准转换为无参考答案的可执行查询级别的监督。自适应后训练使用评分标准作为优化和标准级别的诊断信号。通过聚合按标准ID的低分评分揭示持续缺陷并触发强化学习中的有针对性的监督微调更新。

Result: 在数学推理和医学问答上的实验表明，APT器在两个领域都取得了持续的提高。在三个模型代之间，APT器将数学和医学的平均值分别提高了15.86和8.04个百分点。

Conclusion: APT器在数学推理和医学问答两个领域都取得了显著效果，证明了其有效性和实用性。
...
### [237] [A Generalized Parallelogram Rule for Proportional Analogies on Riemannian Manifolds](https://arxiv.org/abs/2608.14220)
*Pierre-Alexandre Murena,Marcelo Hartmann*

Main category: cs.AI

TL;DR: 在Riemannian域中引入比例类比关系，扩展非欧几里得空间中的应用


<details>
  <summary>Details</summary>
Motivation: 研究比例类比在非欧几里得空间中的应用受限

Method: 引入Riemannian域中的比例类比关系，扩展欧几里得空间中的平行四边形法则

Result: 在球面、形状空间和概率分布流形等不同流形上展示了所引入的类比关系

Conclusion: 在Riemannian域中引入比例类比关系，有助于在非欧几里得空间中应用类比推理
...
### [238] [Attributing Preprocessing Invariance in Spectral Foundation Models](https://arxiv.org/abs/2608.14227)
*Dongjun Wei,Hongyi Wu,Yinuo Zou*

Main category: cs.AI

TL;DR: 预处理不变性验证需要更深入的分析，模型性能不能简单衡量其有效性。


<details>
  <summary>Details</summary>
Motivation: 为了验证预处理不变性在光谱基础模型中的有效性，使用拉曼基础模型作为案例研究。

Method: 通过在不同预处理管道下训练和测试分类器，评估模型的预处理不变性。

Result: 模型在六个拉曼评估数据集上没有显著优于其自身的归一化。归一化可以单独提高原始光谱的性能。训练可以提高编码器性能，但归一化本身也能实现。大多数已发布的系统已经去除了这种形式的变换，并声称实现了不变性。

Conclusion: 预处理不变性不能简单地通过模型性能来衡量，需要更深入的分析。
...
### [239] [Polaris : Multi Agentic System for Conversational Enterprise Analytics](https://arxiv.org/abs/2608.14246)
*Varuni H K,Soham Sarkar,Jay Kumar,Goutham Krishnan,Tanvi Johari,Avinash Bharadwaj,Santosh Hegde*

Main category: cs.AI

TL;DR: Polaris是一个多智能体框架，用于对话式企业分析，通过动态任务协调和智能体编排，提高了数据分析和洞察力，实现了端到端的商业智能。


<details>
  <summary>Details</summary>
Motivation: 在当今快节奏的环境中，快速访问、理解和采取行动的能力变得至关重要。然而，大多数组织仍然数据丰富但洞察力不足，受限于查询、解释和解释企业规模信息的复杂性。

Method: 提出Polaris，一个由主管领导的对话式企业分析多智能体框架，以弥合这一差距。Polaris引入了动态任务协调（DTC），这是一个决策理论编排层，将智能体-任务分配建模为自适应二分匹配，从而实现跨专业智能体的实时协调、恢复和优化，用于查询、可视化和推理。通过将DTC与以推理为先、ReAct风格的智能体相结合，Polaris将自然语言查询转换为连贯的分析工作流程，不仅检索和可视化数据，还解释了背后的“为什么”。

Result: 在结构化企业数据集上的评估显示，语义保真度高，答案相关性强，突显了多智能体编排在大规模提供可信、端到端商业智能方面的潜力。

Conclusion: Polaris通过引入动态任务协调和多智能体编排，有效地解决了企业数据分析和洞察力不足的问题，为企业提供了更强大的商业智能支持。
...
### [240] [TimeSage-EV: A Live Benchmark for Agentic Time Series Analysis in Evolving Environments](https://arxiv.org/abs/2608.14270)
*Qingren Yao,Yaxuan Kong,Yuqi Nie,Yichen Li,Stefan Zohren,Anna Vettoruzzo,Qingsong Wen,Ming Jin,Joaquin Vanschoren*

Main category: cs.AI

TL;DR: TimeSage-EV是一个实时基准，用于评估动态环境中代理时间序列分析的性能。


<details>
  <summary>Details</summary>
Motivation: 在高风险领域的时间序列分析依赖于重复的数据发布，新的观测值可能会改变证据基础和后续结论的有效性。现有的时间序列QA基准大多依赖于固定的快照，导致时间有效性和截止日期感知的证据使用未得到评估。

Method: 我们引入了TimeSage-EV，一个用于动态环境中代理时间序列分析的实时基准。它跟踪了6个领域的60个真实机构场景，包括从2023年2月到2026年5月的1,485个场景-时期QA对，涵盖了每月、每周、每日和不规则的发布周期。在每个时期，大型语言模型（LLM）代理接收时间序列数据和源报告，而保留的目标发布提供真实情况。TimeSage-EV评估状态识别、数据总结和预测推理。

Result: 与前沿LLM代理和TimeSage-1.0（一个具有可重复分析技能库的新颖自演化代理）的实验表明，在模型层级之间存在显著的性能差距，在时间有效性、外生情境使用和适应性方面存在重复的失败。我们发布了TimeSage-EV作为研究资源，包括每月更新、代码、排行榜和故障模式分析。

Conclusion: TimeSage-EV是一个用于动态环境中代理时间序列分析的实时基准，它评估了状态识别、数据总结和预测推理，并揭示了在模型层级之间存在显著的性能差距。
...
### [241] [Intern-S2-Mobius: Foundation Model with Decoupled Knowledge and Reasoning](https://arxiv.org/abs/2608.14290)
...
TL;DR: Mobius-v0架构通过知识-推理分离提高了模型性能和推理速度。


<details>
  <summary>Details</summary>
Motivation: 提出Mobius-v0架构，以提高知识压缩和推理效率。

Method: 设计一个全局共享内存来存储知识向量，并使用多个推理器进行迭代式组合推理。

Result: 1) 7B模型在62.6%的基线训练数据下达到与7B Transformer基线相似的下游评分；2) Intern-S2-Mobius在Qwen3.5-35B上持续预训练，达到相似的下游评分，同时实现近4倍的端到端推理速度提升。

Conclusion: Mobius-v0架构有效提高了知识压缩和推理效率，并实现了显著的性能提升。
...
### [242] [Sensor-Driven Mission Synthesis for UAV/UGV Swarms: A TB-CSPN Coordination Architecture with Hardware-Enforced Safety](https://arxiv.org/abs/2608.14306)
*Uwe M. Borghoff,Paolo Bottoni,Remo Pareschi*

Main category: cs.AI

TL;DR: 该研究提出了一种基于TB-CSPN的无人机/地面车辆群协调架构，提高了在不确定环境下的鲁棒性和安全性。


<details>
  <summary>Details</summary>
Motivation: 提出一种协调架构以支持异构无人机/地面车辆群在不确定、多模态传感器证据下合成任务动作，同时保持执行边界上的硬件强制安全性。

Method: 结合雷达、RF、声学和视觉观察，使用基于主题的通信空间Petri网（TB-CSPN）编排，将传感器输出转换为时间有限的语义令牌，并使用监督代理提供授权和政策控制的任务转换。

Result: 实现了可审计的决策路径，通过守卫和同步约束协调层中的非确定性，并使异构证据的有限时间集成成为可能。

Conclusion: 该架构提高了在争议环境中，包括网络妥协、欺骗、干扰和通信丢失等情况下的鲁棒性。
...
### [243] [Program-space Diffusion for Morphology-to-Transcriptomics Prediction](https://arxiv.org/abs/2608.14330)
*Ruyter Swann,Dorent Reuben,Racoceanu Daniel*

Main category: cs.AI

TL;DR: 提出了一种新的方法来从组织学数据中预测转录组表达，通过利用协调转录变异降低了任务的维度。


<details>
  <summary>Details</summary>
Motivation: Spatial transcriptomics (ST) 的成本高且可扩展性有限，这促使研究者直接从常规组织学预测空间表达。尽管现有方法取得了令人鼓舞的结果，但大多数方法在基因水平上操作，没有利用已建立的转录组建模实践，并依赖于异质基因选择策略，这增加了方法间公平比较的复杂性。

Method: 将形态到转录组预测重新定义为转录程序空间中的条件生成，利用协调转录变异，而不是独立预测基因。使用共识非负矩阵分解（cNMF）提取捕获训练数据中协调表达变异的低维转录程序集，并训练条件扩散模型从组织学中生成程序激活。

Result: 这种方法利用了协调转录变异，并显著降低了条件生成任务的维度。

Conclusion: 提出的方法可以有效地从组织学中预测转录组表达，为空间转录组学提供了一种新的解决方案。
...
### [244] [Clearing the Fog: Towards Installing and Refining Proactive Exploration Capabilities in LLM Agents](https://arxiv.org/abs/2608.14339)
...
### [245] [ScienceFlow: A long-horizon agent for ML research, scientific discovery and beyond](https://arxiv.org/abs/2608.14354)
...
TL;DR: ScienceFlow框架通过有效的状态管理、自适应探索和目标一致执行，提高了长期研究的效率。


<details>
  <summary>Details</summary>
Motivation: 为了解决自主机器学习和科学发现中，长期目标下持续高效研究的挑战。

Method: 提出ScienceFlow框架，将研究工作划分为可执行工作空间的研究段，以可恢复的可执行状态表示研究进度，并采用ESTRA进行研究段的转换控制，以及基于资源可用性和预算的证据感知执行控制器进行资源分配。

Result: 在机器学习、科学建模和数学优化任务上，ScienceFlow在24小时内达到了70.22%的Any-Medal分数，超过先前报道的结果。

Conclusion: 有效的状态管理、自适应探索和目标一致执行对于扩展自主研究至关重要。
...
### [246] [Disentangled Shared Representations Improve Morpho-Transcriptomic Integration](https://arxiv.org/abs/2608.14355)
*Julian Ostermaier,Swann Ruyter,Reuben Dorent,Daniel Racoceanu*

Main category: cs.AI

TL;DR: 通过解耦共享和特定模态信息，可以改善空间转录组学的多模态表示学习。


<details>
  <summary>Details</summary>
Motivation: 为了提高多模态表示学习，特别是针对空间转录组学数据，研究者们希望明确分离共享和特定模态的潜在成分。

Method: 研究者比较了基于VAE和对比的方法，包括标准版本和解耦版本，在两个癌症队列中进行了实验。使用跨模态重建、下游探测和跨模态探测迁移来评估表示。

Result: 对比目标在下游探测性能上优于基于VAE的模型。解耦版本在所选重建和探测指标上有所改善，但收益取决于模型家族、任务、方向和解耦强度。

Conclusion: 明确分解共享和特定模态信息可以提高空间转录组学的多模态表示学习，并为未来的基础模型提供了一个有用的评估框架。
...
### [247] [Designing Sustainable Federated Learning as a Service using Neural Architecture Search](https://arxiv.org/abs/2608.14359)
*Keya Patel,Sajib Mistry,Sheik Fattah,Deepak Kanneganti,Aneesh Krishna,Mufti Mahmud,Monowar Bhuyan*

Main category: cs.AI

TL;DR: 提出了一种新的框架，以解决FLaaS消费者在可持续性方面的限制，从而提高碳可行的联邦训练。


<details>
  <summary>Details</summary>
Motivation: FLaaS消费者在可持续性方面的限制对保持碳可行的联邦训练构成了重大挑战。

Method: 提出了一种可持续的联邦学习即服务（SFLaaS），一种针对异构可持续性约束的碳约束神经架构搜索（NAS）框架。

Result: 引入了需求驱动的搜索空间，将消费者可持续性配置文件转换为在联邦执行之前的可行架构区域。开发了一种消费者级碳可行性估计机制，以评估在动态碳条件下的候选架构。提出了可持续的消费者调度策略，自适应地选择可行的消费者，并将本地工作负载分配给以保持消费者参与和统计数据覆盖。提出了一种进化搜索策略，在硬碳约束下联合优化预测性能、消费者可行性和参与覆盖。

Conclusion: 实验表明，所提出的方法在真实世界数据集和模拟环境中是有效的。
...
### [248] [AgentRewind: Recoverable Execution for Long-Horizon LLM Agents](https://arxiv.org/abs/2608.14380)
*Yu Zhuang,Kefei Chen,Yitong Duan,Shuxin Zheng,Jian Li,Xu-Yao Zhang*

Main category: cs.AI

TL;DR: AgentRewind通过回滚和MettleBench提高了长期任务的成功率。


<details>
  <summary>Details</summary>
Motivation: 许多现实世界任务需要LLM代理在长时间执行范围内与环境交互，现有方法在发生错误后提供很少支持。

Method: 提出AgentRewind，一种运行时恢复框架，记录代理上下文和控制环境的对齐检查点，允许代理回到早期状态并使用先前尝试的信息继续执行。还构建了MettleBench，用于评估长时间工程任务中的任务完成和部分进度。

Result: 实验表明，AgentRewind提高了与比较基线相比的任务成功率和平均清单进度。

Conclusion: AgentRewind在长期执行中提高了任务成功率，并通过MettleBench提供了对长时任务的评估方法。
...
### [249] [Tripwire: Triggering Aligned Refusal via Statistically Certified Safety Neurons](https://arxiv.org/abs/2608.14392)
...
### [250] [The Past and Future of AI Scientists](https://arxiv.org/abs/2608.14407)
*Ross D. King*

Main category: cs.AI

TL;DR: AI科学家有望自动化科学，推动科学快速发展，并促进全球变革。


<details>
  <summary>Details</summary>
Motivation: 自动化科学的需求和挑战

Method: 综述研究

Result: AI科学家的发展和应用前景

Conclusion: AI科学家有潜力彻底改变科学，推动科学快速发展，并促进全球变革。
...
### [251] [Knowing When to Stop: Bayesian Optimal Stopping for LLM Evaluations](https://arxiv.org/abs/2608.14425)
*Toby D. Pilditch*
...
### [252] [The Dynamics of Intelligence Explosions](https://arxiv.org/abs/2608.14426)
*Toby Ord*

Main category: cs.AI

TL;DR: 研究AI研发中的智能爆炸，发现生成时间对智能爆炸行为至关重要。


<details>
  <summary>Details</summary>
Motivation: 探讨AI研发中反馈循环的数学原理，以及驱动这些动态的因素。

Method: 数学建模和理论分析

Result: 揭示了指数增长模型可能高估了智能爆炸的可能性，并强调了生成时间（反馈循环的周期）在智能爆炸行为中的关键作用。

Conclusion: 生成时间的快速减少是实现智能爆炸的关键，而指数增长模型可能低估了这一过程。
...
### [253] [PACE-Bench: Benchmarking Physics Adaptation via Code Evolution in Dynamic Environments](https://arxiv.org/abs/2608.14441)
*Yuhao Zhan,Bingxiang He,Zecong Tang,Chaojun Xiao*

Main category: cs.AI

TL;DR: PACE-Bench基准评估了自演化代理在变化条件下的适应性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 针对现有评估方法在固定执行条件下优化且不测试条件变化后的恢复能力的问题，提出PACE-Bench模拟器基准。

Method: 通过引入PACE-Bench模拟器基准，包含144个源到目标适应对，比较了四种范式下的十种自演化方法。

Result: 模拟器基准下的反射比未经验证的自我修订更可靠，而记忆锚定将代理锚定到早期设计，而广泛的树搜索没有收敛。

Conclusion: 模拟器基准有助于评估自演化代理的适应性和鲁棒性，为未来的研究提供了参考。
...
### [254] [Wyvern: An Agentic Framework for Generating Grounded Multimodal Reports](https://arxiv.org/abs/2608.14446)
...
TL;DR: Wyvern框架：一种多智能体技术报告生成框架，提高了信息基础和多模态内容质量。


<details>
  <summary>Details</summary>
Motivation: 在人工智能驱动创新时代，知识增长速度加快，难以跟上。生成模型在内容合成中越来越受欢迎，但往往缺乏信息基础。

Method: 提出Wyvern，一个多智能体框架，用于自动生成基于信息基础的、多模态的技术报告。Wyvern允许生成多模态输出，将图像、表格和文本以及支持性参考文献集成到统一报告中。此外，特别关注内容的基础性，实现了主张自动修订阶段。进行人工评估研究以评估所提出框架的质量。

Result: 结果表明，87%的情况下，所提出的框架的图表信息感知优于最近基线。此外，Wyvern的报告在63%到100%的情况下被评为比三种替代方法更有用。还进行自动评估，显示与基线相比，Wyvern在引用召回率上提高了2.3倍，在引用精确度上提高了1.6倍。

Conclusion: Wyvern框架能够生成高质量、信息丰富且有用的技术报告，在多模态内容和信息基础方面优于现有方法。
...
### [255] [SheetCompass: Hierarchical Relation Graphs for Agentic Spreadsheet Reasoning](https://arxiv.org/abs/2608.14452)
*Panjing He,Mingyue Cheng,Yucong Luo,Li Li,Xiaohan Zhang*

Main category: cs.AI

TL;DR: SheetCompass is a new framework for LLMs to reason and automate spreadsheets by modeling structural relationships and maintaining task information.


<details>
  <summary>Details</summary>
Motivation: Spreadsheets are widely used for organizing, analyzing, and manipulating semi-structured data, but automated spreadsheet reasoning remains challenging for large language models (LLMs).

Method: Proposes SheetCompass, a graph-guided and memory-driven agentic framework for spreadsheet reasoning and automation.

Result: SheetCompass explicitly models structural relationships within and across worksheets while maintaining task-relevant information in memory, enabling agents to reason more effectively over complex workbooks.

Conclusion: SheetCompass improves the reasoning and automation capabilities of LLMs for spreadsheets by explicitly modeling structural relationships and maintaining task-relevant information in memory.
...
### [256] [Shift Aware Transfer Learning with Adaptive Dual-Encoder Fusion for PM Forecasting in Data-Limited Environments](https://arxiv.org/abs/2608.14456)
*Shahab Band,Hamed Mohammadi*

Main category: cs.AI

TL;DR: 本研究提出了一种新的迁移学习框架，用于提高PM2.5短期预测的准确性，并取得了显著的性能提升。


<details>
  <summary>Details</summary>
Motivation: 在目标域观测数据有限且源域和目标域的统计性质不同时，短时预测细颗粒物（PM2.5）仍然困难。在这些设置中，仅在本地数据上训练的模型可能无法捕捉复杂的时序动态，而直接迁移学习可能导致负迁移。

Method: 本研究开发了一种感知转移的双编码器迁移框架，该框架结合了源域知识和目标特定的表示学习。源编码器使用来自10个美国监测地点的小时观测数据进行了预训练。然后，使用按时间顺序的培训-验证-测试协议，使用来自台湾77个站点的两年小时观测数据对该框架进行了调整和评估。

Result: 在四个主要基线中，冻结源双编码器模型实现了最佳性能，MSE = 21.8960，MAE = 3.1597，R^2 = 0.8725。这相当于相对于TL-v1的MSE降低了约7.1%，相对于TL-v2的MSE降低了4.1%。消融分析表明，删除台湾特定分支导致性能下降最大。允许源编码器进行适应产生了最佳的整体结果，MSE = 21.6575，MAE = 3.1383，R^2 = 0.8739。SHAP分析表明，预测主要受最近的PM2.5观测值和相关污染物传输和扩散的气象变量的驱动。

Conclusion: 这些结果表明，当目标特定信息得到保留，并且允许在目标监督下适应转移表示时，源域知识最有效。
...
### [257] [Twin: Playing an Unknown Game with a Test-Time Digital Twin](https://arxiv.org/abs/2608.14490)
*Alexy Skoutnev,Kirill Acharya,Gaston Longhitano,Madeleine Udell,Kevin Ellis,Iddo Drori*

Main category: cs.AI

TL;DR: Twin系统通过自动构建游戏规则和目标，在持续学习任务中取得了显著成果。


<details>
  <summary>Details</summary>
Motivation: 为了完成持续学习任务，如ARC-AGI-3游戏，研究人员提出了一种测试时世界模型推理（Twin）系统。

Method: 该系统使用前沿编码代理编写可执行的世界模型，通过模拟和交互来构建游戏规则和目标。它使用归纳先验知识来恢复游戏的真正转换和目标。系统在双世界模型中进行回放验证，并确保在程序重现每个先前观察到的游戏转换之前不会采取任何行动。如果世界模型预测与实际动作结果不匹配，则将其作为反例用于修复世界模型。

Result: Twin在183个级别中完成了179个（97.8%），在179个级别中有88.3%的效率高于人类。在156个级别上，系统在获得任何奖励之前推断出目标（87.2%），在剩余的级别中自动通过搜索发现目标。基准分数在0到100之间，与人类首次玩游戏时的完成和动作效率进行比较。直接玩游戏，基线模型得分仅为7.8%；现成的工具将其提高到61.1%，而我们的双世界模型将相同的基线模型提高到93.3%，完成了25个游戏中的23个。构建可用的世界模型比预期的要简单，而更困难的问题是推断正确的目标。

Conclusion: Twin系统在持续学习任务中表现出色，特别是在游戏规则的自动发现和目标推断方面。
...
### [258] [Participatory Moral AI Is Not Neutral: The Invisible Hand of Developers](https://arxiv.org/abs/2608.14522)
*Taenyun Kim,Edyta Bogucka,Daniele Quercia*

Main category: cs.AI

TL;DR: 道德AI诱导的每个阶段都可能影响结果，需要审计和披露。


<details>
  <summary>Details</summary>
Motivation: 为了解决人工智能系统在社会中做出道德决策的问题，研究人员采取了道德偏好诱导的方法。

Method: 研究人员通过假设性困境对参与者进行投票，并使用汇总的选票来训练AI模型，然后在大规模上应用该政策。

Result: 研究发现，特征范围、投票者抽样和问题框架的每个选择都可能塑造道德AI诱导产生的偏好。在三个部署环境中，研究人员考察了道德AI诱导管道的三个主要阶段。

Conclusion: 基于投票的对齐不能仅通过汇总来提供公平或透明的AI；至少，道德AI诱导管道的每个阶段都应该接受审计和披露。
...
### [259] ["I Thought You Were The Uncensored Place": Norms, Rules, and Moderation in AI-Generated Sexual Content Communities](https://arxiv.org/abs/2608.13659)
*Lucy Qin,Jaron Mink,Elissa M. Redmiles*

Main category: cs.CR

TL;DR: 研究分析了AIG-SC在线社区的治理，发现社区治理复杂，需多方面治理减轻滥用内容。


<details>
  <summary>Details</summary>
Motivation: 为了了解社区治理尝试在防止滥用同时支持自由表达方面的作用和效果。

Method: 对大型AIG-SC在线社区（10,000+成员）的24名成员和版主进行了访谈。

Result: 揭示了社区治理的复杂性，包括创作者和社区对滥用内容的个人界限、社区规范、内容监管规则以及社区价值观与监管之间的紧张关系。

Conclusion: 技术、社区和法律治理可能最有效地减轻滥用内容的产生。
...
### [260] [Weird Machines in Transport Layer Security](https://arxiv.org/abs/2608.13685)
*Michael Collins,Jada Cumberland,Brianne Dunn,Ross Gore,Samuel Jackson,Sachin Shetty,Jonathan Takeshita*

Main category: cs.CR

TL;DR: 怪异机器理论被应用于TLS，揭示了其潜在的计算能力和安全风险。


<details>
  <summary>Details</summary>
Motivation: 将怪异机器理论扩展到新的领域：传输层安全性（TLS）握手及其两个主导实现，OpenSSL和BoringSSL。

Method: 通过分析TLS原语，包括会话缓存条目、重协商逻辑、扩展解析和证书验证步骤，构建了Turing完备系统，其计算与认证和信任决策相关联。

Result: 验证了怪异机器理论，并通过两个基于实际OpenSSL代码路径的工作演示。第一个演示是一个哨兵系统，将标准TLS原语组合成一个检测异常握手行为的防御机制。第二个演示是一个身份验证绕过攻击，通过中间连接重协商，在不造成任何内存损坏或外部恶意软件的情况下，通过加密强度策略检查。

Conclusion: 怪异机器理论可以应用于TLS，并揭示了其潜在的计算能力和安全风险。
...
### [261] [A Reproducibility Protocol for Cross-Implementation Evaluation of Post-Quantum ACVP Test Vectors](https://arxiv.org/abs/2608.13784)
*Christopher M. Frost*

Main category: cs.CR

TL;DR: 测试了NIST ML-KEM实现的一致性，发现实际标准存在差距


<details>
  <summary>Details</summary>
Motivation: 验证不同实现之间的加密标准一致性

Method: 定义产品中立的重复性协议，对三种NIST ML-KEM实现进行测试

Result: 所有测试用例均符合预期，但存在实际标准差距

Conclusion: 加密标准实现需要精确的协议定义和验证，以实现独立验证的可靠性
...
### [262] [The ack3 H1 2026 DeFi Incident Dataset: Audit Scope Across 135 Security Incidents](https://arxiv.org/abs/2608.13792)
...
### [263] [Mobile Apps vs. Web Browsers: A User Perception Study with Android Apps and Google Chrome](https://arxiv.org/abs/2608.13803)
*Harel Berger*

Main category: cs.CR

TL;DR: 研究分析了用户对移动应用与网页浏览器的偏好，旨在提升用户体验。


<details>
  <summary>Details</summary>
Motivation: 探究用户对移动应用和网页浏览器的使用偏好，尤其是安全性、隐私性和易用性方面。

Method: 结合实验和调查方法，对Android智能手机用户进行研究。

Result: 确定影响用户选择的关键担忧和偏好，为开发者提供改善平台安全、隐私和易用性的见解。

Conclusion: 研究结果对提升移动应用和网页浏览器的用户体验具有重要意义。
...
### [264] [Vaulted Passkeys: A Device-Bound Proposal for Authenticated Credential Export and Import](https://arxiv.org/abs/2608.13806)
*Pol Henarejos*

Main category: cs.CR

TL;DR: 本文提出了一种新的设备绑定架构，通过导出受保护的凭证状态来解决硬件认证器的恢复差距问题。


<details>
  <summary>Details</summary>
Motivation: 硬件认证器故意抵抗私钥提取，但替换、灾难恢复和受控迁移产生了可移植性的合法需求。现有的设备绑定凭证指南通常通过在故障之前注册一个额外的认证器来降低恢复风险。这创建了一个独立的凭证注册，需要提前存在备用硬件；这是冗余，而不是原始凭证的备份。本文通过在源可用时导出受保护的凭证状态，并在以后获得的硬件上恢复它，来解决由此产生的恢复差距，而不需要克隆完整的认证器或将明文私钥暴露给常规桌面软件。

Method: 我们提出了Vaulted Passkeys，这是一种设备绑定架构，其中256位的随机Kvault通过HKDF分隔的密钥和四个明确的AEAD配置文件保护认证的PKV1凭证封装。设计将注册与导出/导入分离，并将所需的保险库与可选的身份分离。我们贡献了一个角色分离的系统模型、线格式、威胁分析、实现映射和可验证的评估计划。

Result: 原型证明了可行性，但既不是正式的安全证明，也不是建议的最终标准。

Conclusion: 本文提出了一种新的设备绑定架构，通过导出受保护的凭证状态来解决硬件认证器的恢复差距问题。
...
### [265] [TLF: Rapid Characterization of RF Transceiver Parameters in Embedded Systems via Bus-Level Interception](https://arxiv.org/abs/2608.13815)
...
### [266] [Verified Pythagorean Composition for Adaptive Cryptographic Games: Noise Flooding in Homomorphic Encryption](https://arxiv.org/abs/2608.13846)
*Yi Lee,Alexandru Cojocaru,Junyi Liu,Xiaodi Wu*

Main category: cs.CR

TL;DR: 提出了一种新的近似同态加密安全证明方法，并进行了机器验证。


<details>
  <summary>Details</summary>
Motivation: 为了提高近似同态加密在解密攻击中的安全性

Method: 使用噪声注入和条件KL散度累积的方法

Result: 成功证明了一种新的安全证明方法，并使用Rocq和SSProve进行了机器验证

Conclusion: 该方法能够有效地提高近似同态加密的安全性，为后续研究提供了新的思路。
...
### [267] [CipherSight: Robust Website Fingerprinting via Record-Resource Semantic Supervision under Distribution Shifts](https://arxiv.org/abs/2608.13905)
*Runhan Song,Qiqi Liu,Chuanzhou Pan,Zhenquan Ding,Youquan Xian,Chongru Fan,Lei Cui,Wei Wang,Zhiyu Hao*

Main category: cs.CR

TL;DR: CipherSight是一种基于TLS记录的HTTPS网站指纹识别方法，在多种条件下均表现良好


<details>
  <summary>Details</summary>
Motivation: 识别加密流量中的网站，但现有方法在实际情况下降性能

Method: CipherSight，基于TLS记录的分层框架，联合编码多个记录级属性，引入分层架构，使用掩码记录建模（MRM）任务

Result: 在2000多个网站类别中达到95.41%的准确率，在时间和地理漂移下仍保持超过90%的准确率

Conclusion: CipherSight在HTTPS网站指纹识别中表现优异
...
### [268] [Characterizing the Variance Envelope: A Multi-Dimensional Analysis of Spectre Telemetry Across Architectures and Workloads](https://arxiv.org/abs/2608.13920)
...
### [269] [CoSA: Context-Aware Severity Assessment via Context Analysis with Large Language Models](https://arxiv.org/abs/2608.13928)
*Jinfeng Jiang,Yikun Li,Chengran Yang,Ting Zhang,Wen Bin Leow,Yide Yin,Eng Lieh Ouh,Lwin Khin Shar,David Lo*

Main category: cs.CR

TL;DR: CoSA通过上下文感知和存储库感知评估，显著提高了CVSS基本指标的推断准确性和自动化严重性评估的可靠性。


<details>
  <summary>Details</summary>
Motivation: 准确评估漏洞严重性对于优先修复至关重要，但手动评估通用漏洞评分系统（CVSS）基本指标仍然劳动密集。现有的自动化方法往往无法捕获评估许多CVSS基本指标所需的存储库级别的证据。这种存储库感知评估具有挑战性，因为相关证据散布在整个存储库中，且噪声很大。

Method: 提出了一种名为CoSA的上下文感知漏洞严重性评估方法，该方法从存储库工件中推断CVSS基本指标。CoSA构建了一个代码属性图（CPG），并应用了两阶段存储库剪枝策略：轻量级静态剪枝以保留结构上邻近的上下文，然后是代理大型语言模型（LLM）引导的剪枝步骤，以保留CVSS相关上下文并收集支持证据。LLM然后将检索到的存储库上下文合并为紧凑的、CVSS指标级别的文本摘要，这些摘要被输入到轻量级变压器预测器中。我们还构建了一个高质量的存储库级别数据集，包括6,816个CVSS标记实例，涵盖90种常见弱点枚举（CWE）类型。

Result: 在真实世界漏洞上的实验表明，CoSA在预测准确性和宏F1方面均优于函数级和纯LLM基线。与表现最佳的基线相比，它将预测准确率提高了14.4%，宏F1提高了15.3%，表明显式、指标导向的存储库上下文检索对于实际和可靠的自动化严重性评估至关重要。

Conclusion: CoSA方法通过上下文感知和存储库感知评估，提高了CVSS基本指标的推断准确性和自动化严重性评估的可靠性。
...
### [270] [Extracting and Verifying Illicit Bitcoin Addresses from Underground Forum Discussions](https://arxiv.org/abs/2608.13930)
*Abdoul Nasser Hassane Amadou,Arnaud Legout,Imane Fouad,Konstantin Avrachenkov,Anas Motii*

Main category: cs.CR

TL;DR: 提出了一种从HackForums论坛构建证据支持的比特币标签的方法，以识别非法交易相关的比特币地址，并发布了相关数据集和流程。


<details>
  <summary>Details</summary>
Motivation: 为了构建一个可复制的流程，从HackForums论坛中获取证据支持的比特币标签，以识别与论坛上讨论的非法交易相关的比特币地址。

Method: 结合LLM辅助筛选、专家审查和链上验证来识别与论坛上讨论的非法交易相关的比特币地址。

Result: 构建了一个包含2,438个手动验证的非法比特币地址的数据集，涵盖2010-2024年以及LLM筛选期间分配的十二个网络犯罪类别。

Conclusion: 发布的数据集、时间元数据和完整的提取流程支持对加密货币促进的网络安全犯罪的可重复研究。
...
### [271] [Vectorized SQIsign Implementation Using AVX-512](https://arxiv.org/abs/2608.13948)
...
### [272] [Structural Leakage in Graph Encryption: Attacks and Defenses](https://arxiv.org/abs/2608.13981)
*Hua Shen,Renzhi Chen,Ge Wu,Willy Susilo,Jing Chen,Mingwu Zhang*

Main category: cs.CR

TL;DR: 分析了图加密方案中的结构泄露问题，提出了改进方案


<details>
  <summary>Details</summary>
Motivation: 分析单对最短路径查询中的结构泄露问题

Method: 对PathGES、Fragment Tree攻击和BlindGES进行综合分析

Result: 揭示了PathGES的不足，提出了Fragment Tree攻击，并介绍了BlindGES方案

Conclusion: 对图加密方案中的结构泄露问题进行了深入研究，为安全设计提供了参考
...
### [273] [P2Skill: Privacy Preserving Skill Distillation for Cloud-Local LLM Inference Systems](https://arxiv.org/abs/2608.14094)
*Myunghoon Ryu,Geunpyo Park,Sungjoon Lee,XinYu Piao,Jong-Kook Kim*

Main category: cs.CR

TL;DR: P2Skill：一种基于提示的技能蒸馏方法，提高了云本地LLM推理系统的隐私保护推理质量。


<details>
  <summary>Details</summary>
Motivation: 为了在保护个人设备上的敏感用户数据的同时利用大型云模型的推理能力，提出了一种基于提示的技能蒸馏方法P2Skill。

Method: P2Skill方法通过云LLM对执行失败进行迭代细化，使本地SLM能够泛化到记忆中的PII模式之外，无需隐私特定的微调或学习辅助检测器。

Result: 在四个领域的基准测试中，P2Skill比以前的基线实现了更高的隐私保护推理质量，分别是1.69倍和3.66倍。

Conclusion: P2Skill是一种有效的隐私保护方法，能够提高云本地LLM推理系统的隐私保护推理质量。
...
### [274] [BGA: A noise-immune neural distillation framework for malicious signature extraction in high-entropy encrypted flows](https://arxiv.org/abs/2608.14126)
*Sheng Hong,Yixuan Huang,Weiwei Jiang,Junyuan Zhang,Jiacheng Wang,Ruijian Jiao*

Main category: cs.CR

TL;DR: BGA是一种有效的噪声免疫神经蒸馏框架，在加密威胁情报检测中表现出色，具有实际应用潜力


<details>
  <summary>Details</summary>
Motivation: 缓解高熵TLS 1.3流中的注意力稀释

Method: 提出BGA，一种针对加密威胁情报的噪声免疫神经蒸馏框架，使用方差分析（ANOVA）分离高判别性控制平面特征，并集成Wasserstein GAN with Gradient Penalty（WGAN-GP）模块解决数据不平衡问题，使用BiLSTM和自适应门控多头注意力机制进行时间依赖性提取和动态抑制加密伪影

Result: 在CIC-IDS-2018和Edge-IIoT基准测试中，BGA在所有关键指标上均超过95.2%，在噪声注入压力测试中，性能优于Transformer，具有低延迟，适用于异构工业边缘网关

Conclusion: BGA是一种有效的噪声免疫神经蒸馏框架，在加密威胁情报检测中表现出色，具有实际应用潜力
...
### [275] [MazeRunner: Nonlinear Task and Clue Orchestration for LLM-driven Black-Box Automated Penetration Testing](https://arxiv.org/abs/2608.14216)
*Zhenyuan Li,Yi Jiang,Junjie Cheng,Yaokun Li,Jing Qiu,Shouling Ji*

Main category: cs.CR

TL;DR: MazeRunner是一种基于三代理的自主渗透测试系统，在HTB目标上表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有渗透测试代理在非线性场景中表现不佳，导致误诊和证据遗忘。

Method: 基于三代理的任务和线索编排框架，进行自主渗透测试。

Result: MazeRunner在HTB目标上完成了47.7%的标注子任务，并在六个目标上实现了用户级或更高的访问权限。

Conclusion: MazeRunner在渗透测试方面表现出色，在HTB目标上的表现优于现有模型。
...
### [276] [A Four-Axis Trustworthiness Benchmark for LLM-as-Judge in Principle-Based Regulation](https://arxiv.org/abs/2608.14329)
*Dipankar Sarkar*
...
### [277] [A Hybrid LLM-Based Framework for Automated Security Annotation Generation in Business Process Models](https://arxiv.org/abs/2608.14370)
*Md Kamrul Islam,Tiphaine Henry,Mattia Salnitri,Julius Köpke,Sami Souihi*

Main category: cs.CR

TL;DR: 提出了一种自动生成安全注释的混合框架，该框架在精确性和效率方面优于人工分析。


<details>
  <summary>Details</summary>
Motivation: 将安全注释纳入业务流程模型对建模和分析安全业务流程至关重要。尽管存在用于此目的的BPMN扩展，如SecBPMN2，但从自然语言规范中推导出准确和完整的安全注释仍然是一项手动、专家密集且易出错的任务。

Method: 提出了一种混合框架，该框架以BPMN流程模型和安全需求文档作为输入，并自动生成符合SecBPMN2规范的安全注释。该方法结合了基于大型语言模型（LLM）的语义提取、模式约束映射、基于规则的归一化和确定性验证。

Result: 在27个来自不同领域的流程模型的精选数据集上对框架进行了全面评估。结果表明，该框架能够持续产生结构有效的SecBPMN2注释，具有高模式完整性。与人类安全分析师相比，该系统在保持可比较的召回率（0.52比0.50）的同时，实现了显著更高的精度（0.58比0.29），并减少了近50%的错误或位置不当的注释。此外，注释生成速度远快于手动注释。

Conclusion: 混合LLM和基于规则的自动化可以减少建模工作量，同时提高一致性和可靠性，从而为安全设计BPM提供了一个可扩展的基础。
...
### [278] [STINER: Automated Extraction of Strategic Cyber Threat Intelligence from X](https://arxiv.org/abs/2608.14418)
*Yasir Ech-Chammakhy,Oussama Azrara,Jaafar Chbili,Anas Motii*

Main category: cs.CR

TL;DR: STINER和DarkBERT提高社交媒体战略情报提取，优化自动化防御系统。


<details>
  <summary>Details</summary>
Motivation: 传统命名实体识别模型难以处理社交媒体的非正式和高度不规则的语言，导致自动化防御系统存在盲点。

Method: 提出STINER，一个用于从社交媒体流中提取战略情报的词汇表和专家注释语料库。构建一个包含2100个真实警报的高质量、专家注释数据集，并提出以威胁行为者、部门、位置等为中心的八个实体类型的细粒度分类法。在12个评估配置中对九个模型进行基准测试，涵盖通用和领域自适应编码器、开放架构提取和生成型LLM（零样本和微调设置）。

Result: 领域自适应编码器如DarkBERT在严格F1分数上达到89.33%，优于通用基线和微调的大型语言模型，后者推理延迟显著更高。利用STINER-DarkBERT进行2025年上半年的欧洲威胁景观分析。结果与官方报道的主要目标一致，同时突出了西班牙攻击的可见性特征，并说明了社交媒体驱动的提取如何揭示SafePay勒索软件活动的早期信号，这比供应商威胁景观报告中的回顾性描述要早。

Conclusion: STINER和DarkBERT在提取社交媒体中的战略情报方面表现出色，有助于提高自动化防御系统的有效性。
...
### [279] [Lower Bounds on Black-Box Constructions of Pseudorandom Functions](https://arxiv.org/abs/2608.14501)
*Bar Alon,Itai Dinur,Muthuramakrishnan Venkitasubramaniam*

Main category: cs.CR

TL;DR: 本研究证明了从PRG到PRF的完全黑盒构造的不可行性，即使构造允许对PRG进行自适应查询，对于具有足够长输出的弱PRF也成立。


<details>
  <summary>Details</summary>
Motivation: 考虑从PRG到PRF的完全黑盒构造，其中构造和还原都必须是黑盒，并且还原与敌手交互的次数与敌手在其底层函数中每次交互对函数的查询次数无关。

Method: 证明了一个下界，即使构造允许对PRG进行自适应查询，对于具有足够长输出的弱PRF也成立。

Result: 证明了不存在这样的构造，它对PRG的调用次数小于n/logn和o(in/login)，其中in是PRF的输入长度。

Conclusion: 对于具有一个位输出的弱PRF，即使敌手被限制为进行独立同分布的均匀随机查询，该不可行性也成立。
...
### [280] [Trust Without Boundaries: An Architectural Analysis of Satellite Flight Software](https://arxiv.org/abs/2608.14532)
*Jack Vanlyssel,Gruia-Catalin Roman,Kendra Cook,Sazzadur Rahaman,Afsah Anwar*

Main category: cs.CR

TL;DR: 本文分析了NASA的核心飞行软件，发现了安全弱点，并讨论了加强安全边界的机制。


<details>
  <summary>Details</summary>
Motivation: 随着航天器变得更加软件驱动和互联，机载飞行软件成为一个越来越重要的安全边界。

Method: 我们分析了NASA的核心飞行软件(cFS)，使用NASA的飞行代表性NOS3模拟器，通过五个实验来验证这些弱点，并与其他模块化飞行软件框架进行比较。

Result: 结果显示，单个受感染的组件可以以难以与合法行为区分的方式利用广泛共享的权限。

Conclusion: 我们讨论了加强未来飞行软件系统内部信任边界的机制。
...
### [281] [Finding Vulnerabilities via LLM-Augmented Semantics-Aware Type-Checking](https://arxiv.org/abs/2608.14533)
*Ruizhe Wang,Meng Xu,N. Asokan*

Main category: cs.CR

TL;DR: SETYPE是一种基于语义的漏洞检测方法，在Python Web应用程序中实现了高精度的漏洞检测。


<details>
  <summary>Details</summary>
Motivation: 传统的漏洞检测方法依赖于安全专家将不安全的编码模式编码成算法规则，但这种方法往往只关注语法模式，而忽略了代码中的更深层次的语义信息，如变量和函数名的含义。随着软件系统的日益复杂，仅使用语法规则来模拟漏洞变得越来越具有挑战性。

Method: 提出了一种基于语义的漏洞检测方法，称为SETYPE，该系统可以从源代码中直接推导出来，仅基于符号和表达式的自然语言含义。在SETYPE类型系统中，类型推理和检查都由大型语言模型（LLMs）执行，类型检查失败表示存在潜在漏洞。

Result: 通过原型PYSETYPE演示了SETYPE在检测Python Web应用程序中的漏洞方面的可行性。在真实世界的应用评估中，达到了87%的检测精度和88%的检测准确率。使用PYSETYPE，我们发现了15个潜在零日漏洞，其中9个被开发人员确认。

Conclusion: SETYPE在检测软件漏洞方面具有可行性和有效性，尤其是在Python Web应用程序中。
...
### [282] [Learning Unsteady Aneurysm Hemodynamics with Physics-Informed DeepONets](https://arxiv.org/abs/2608.13629)
*Oscar L. Cruz-Gonzalez,Valérie Deplano,Badih Ghattas*

Main category: stat.ML

TL;DR: 提出了一种新的深度学习模型，用于预测腹主动脉瘤中的血流动力学，提高了推理速度并降低了误差。


<details>
  <summary>Details</summary>
Motivation: 在腹主动脉瘤（AAA）中，对患者特异性血流动力学评估至关重要，特别是壁面剪切应力、涡流结构和压力分布。

Method: 提出了一种改进的多输入多输出PI-DeepONet（M3PI-DeepONet），用于预测理想化腹主动脉瘤几何形状中的非稳态流动。

Result: M3PI-DeepONet同时预测非稳态3D流速和压力场，与参考CFD模拟相比，在可用条件分支输入的情况下，实现了约36倍的保守保留周期推理加速。

Conclusion: 这项工作推进了深度学习在心血管疾病建模中的应用，是向实时、无创临床诊断迈出的一步。
...
### [283] [On the Brittleness of Maximum Likelihood Estimation for Gaussian Process Hyperparameter Optimization](https://arxiv.org/abs/2608.13793)
*Tyler R. Johnson,Kian Ben-Jacob,Christopher P. Muller,Ramin Bostanabad*

Main category: stat.ML

TL;DR: 本文评估了MLE在训练GPs时的脆弱性，并提出了解决方案，以构建准确且鲁棒的GPs。


<details>
  <summary>Details</summary>
Motivation: 机器学习（ML）已成为现代工程设计流程中不可或缺的一部分。训练机器学习模型的关键步骤之一是选择损失函数，这可以通过各种技术系统地制定，例如最大似然估计（MLE）和交叉验证。虽然MLE是训练机器学习模型中最受欢迎、最有效和最直观的机制之一，但它很脆弱：如果支撑它的假设没有得到满足，训练的机器学习模型可能泛化能力差。这种脆弱性甚至影响了广泛用于工程设计的高斯过程（GPs），它们通常（错误地）被认为对过拟合具有很强的鲁棒性。

Method: 在本文中，我们对MLE在训练高斯过程（GPs）进行概率回归或分类任务时的脆弱性进行了根本性评估。我们将基于理论的指标与MLE进行比较，并提出实际解决方案。我们广泛的研究表明，我们的解决方案在下游设计任务（如贝叶斯优化）中的有效性，并为从业者提供了一份蓝图，以构建准确且鲁棒的GPs，这些GPs在预测精度、不确定性量化以及推理成本方面甚至可以优于表格基础模型。

Result: 我们的贡献可以通过GitHub在https://github.com/Bostanabad-Research-Group/GP-vs-TabPFN-vs-GPyTorch上公开获取。

Conclusion: 本文评估了MLE在训练GPs时的脆弱性，并提出了解决方案，以构建准确且鲁棒的GPs。
...
### [284] [Deep Vision in Smart Manufacturing: MODERN Framework for Intelligent Quality Monitoring and Diagnosis](https://arxiv.org/abs/2608.13937)
*Yicheng Kang,Yuling Jiao,Xin Geng,Mahesh Nagarajan*

Main category: stat.ML

TL;DR: 我们提出了MODERN，一个用于质量监测和故障隔离的深度学习框架，它在工业质量控制中表现出色。


<details>
  <summary>Details</summary>
Motivation: 为了提高工业质量控制，我们需要一个能够监测质量并隔离故障的深度学习框架。

Method: 提出了一个名为MODERN的深度学习框架，该框架集成了智能制造中的增强功能，并使用改进的残差神经网络架构。

Result: MODERN框架能够有效地监测产品缺陷的可能性，并识别故障区域。在缺乏训练数据的情况下，该框架也能发挥作用。

Conclusion: 该研究提供了新的管理启示，即制造商不总是需要不断升级其监控设备，并展示了其方法在实际数据和模拟实验中的优越性能。
...
### [285] [Extending Occam's inversion with lasso fusion, overcomplete dictionaries, and isotropic total variation regularisation](https://arxiv.org/abs/2608.14225)
*Anandaroop Ray*

Main category: stat.ML

TL;DR: 本文将l1正则化应用于Occam反演，提高了非线性地球物理反演的效率和准确性，并推荐了最佳的求解器。


<details>
  <summary>Details</summary>
Motivation: 为了提高非线性地球物理反演的鲁棒性，减少地质过度解释，本文研究了Occam反演算法。

Method: 通过将l1正则化应用于Occam框架，并结合不同的求解器，如坐标下降、迭代加权最小二乘和分割Bregman，解决了一维和二维的地球物理问题。

Result: 结果表明，对于一维问题，坐标下降是最佳选择；对于二维问题，迭代加权最小二乘比分割Bregman更优，且需要的平方最小二乘求解次数少一个数量级。

Conclusion: 本文通过展示l1正则化反演的各种形式，鼓励地球物理学家采用Occam反演。
...
### [286] [Offline Deep Q* Estimation with Diffusion Models](https://arxiv.org/abs/2608.14401)
*Xiaohong Chen,Yuling Jiao,Lican Kang,Jerry Zhijian Yang,Chen Zhong*

Main category: stat.ML

TL;DR: 提出了一种新的离线强化学习框架，通过条件扩散模型估计奖励律和转移核，从而获得Q*的深度估计器，并在理论和实验上证明了其有效性。


<details>
  <summary>Details</summary>
Motivation: 在离线强化学习中，估计最优动作值函数Q*可以通过仅基于离线观察来解决最优贝尔曼方程。一个基本挑战是奖励函数和转移核未知，因此最优贝尔曼算子不能直接从数据中观察到。

Method: 提出了一种新的框架，将算子估计与值函数学习解耦。首先，通过条件扩散模型估计奖励律和转移核，从而诱导最优贝尔曼算子的数据驱动近似。然后，将这些估计器插入贝尔曼方程，通过最小化神经网络的贝尔曼残差来获得Q*的深度估计器。

Result: 理论上，首先通过总变差距离对条件扩散估计进行端到端分析，建立了学习最优贝尔曼算子的锐利非渐近收敛率。然后，建立了超贝尔曼残差风险的oracle值-阶段率。最后，在集中性条件下，将此残差界限转化为Q*的深度估计器的L^2收敛率。

Conclusion: 该方法在理论上有效，并在大量数值实验中证明了其有效性和强大的经验性能。
...
### [287] [Online Inference in Distributional Temporal-Difference Learning](https://arxiv.org/abs/2608.14408)
*Yang Peng,Liangyu Zhang*

Main category: stat.ML

TL;DR: 研究在线统计推断，证明收敛性和bootstrap推断的有效性


<details>
  <summary>Details</summary>
Motivation: 研究在线统计推断在固定策略下对回报分布函数的函数

Method: 非参数分布时序差异学习从单个马尔可夫轨迹估计回报分布，证明Polyak-Ruppert平均估计器的根-T误差收敛到Cramér空间中的中心高斯随机元素，以及条件于观察轨迹，根-T差异收敛到相同的Gaussian极限

Result: 为平滑统计函数提供bootstrap推断，包括方差、CVaR、预期损失和expectiles；为非平滑统计函数开发局部渐近理论，包括回报分位数

Conclusion: 证明了在线统计推断的收敛性和bootstrap推断的有效性
