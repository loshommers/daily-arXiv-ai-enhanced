...
TL;DR: UAV3DCrop基准测试了不同方法在作物监测中的应用，结果表明当前方法在农业应用中仍有局限性。


<details>
  <summary>Details</summary>
Motivation: 准确的三维作物监测是数据驱动精准农业的基础，它使田间尺度上对植物结构、生长动态和管理响应的分析成为可能。

Method: 引入了UAV3DCrop，这是一个公开的重复多角度无人机作物调查基准。它包含来自91个场景的88,830张RGB图像，分辨率为5280×3956像素，地面采样距离为3.6-5.8毫米，场景包括玉米、大豆、小麦和大麦。

Result: 场景优化方法在不同目标上排名不同：Splatfacto-big在外观上领先，而Scaffold-GS在深度上领先，与Splatfacto在冠层高度上统计上相等。在前进模型中，MapAnything在七个指标上领先，而其他模型在作物之间变化更多，在绝对尺度上严重失败，因为对齐隐藏了这一点。

Conclusion: 当前的3D重建方法还不能互换用于农业用途：没有一种方法同时在外观、几何形状和冠层高度上获胜，只有四个前进模型中的一个能够恢复可用的度量尺度。
...
TL;DR: 本研究使用DER方法进行森林高度估计，提高了预测的不确定性估计能力。


<details>
  <summary>Details</summary>
Motivation: 准确估计森林高度对于碳核算、生物多样性监测和生态系统管理等应用至关重要。

Method: 使用深度证据回归（DER）方法进行森林高度估计，并引入掩码证据损失函数进行密集地理空间预测。

Result: DER方法在预测性能上与确定性U-Net相当，同时提供了良好的不确定性估计。

Conclusion: 证据学习在地球观测数据中用于不确定性感知的森林结构估计方面具有潜力。
...
TL;DR: TransSLR：一种新的轻量级手语识别模型，显著提高了代表性不足的语言的识别准确率。


<details>
  <summary>Details</summary>
Motivation: 针对非洲中部手语（CASL）等代表性不足的语言的自动手语识别问题，现有基准CASL-W60的准确率仅为69.93%，且微调高资源模型的方法无法解决这一问题。这主要是因为可用的CASL数据量有限，以及CASL与大规模语料库（如WLASL）在词汇和视觉领域上的巨大差异，导致预训练表示几乎无用。

Method: 提出TransSLR，这是一种轻量级的时序变换器编码器，从头开始在64帧归一化姿态序列上训练，包含平均池化和分类头。通过在几何关键点表示上操作，而不是在原始RGB上操作，TransSLR实现了不依赖视觉外观的无符号者泛化。

Result: 在CASL-W60基准测试中，TransSLR建立了新的最先进准确率80.39%，超过了之前的最佳水平10.46%。此外，我们的仅编码器设计显著降低了计算开销，使得在资源受限环境中部署成为可能。在CASL-W60基准测试上进行了广泛实验，与基于RGB和多模态基线进行了比较，证明了TransSLR达到了最先进的性能。

Conclusion: TransSLR在CASL-W60基准测试中取得了显著的性能提升，为代表性不足的语言的自动手语识别提供了一种有效的解决方案。
...
TL;DR: EB-CaP是一种基于能量的缓存个性化方法，用于视频面部表情识别，在保持低计算和内存开销的同时，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 视频面部表情识别（FER）具有挑战性，因为模型必须识别个体之间差异的微妙、随时间演变的情感状态。

Method: 提出了一种基于能量的缓存个性化（EB-CaP）方法，该方法使用轻量级的能量模型从当前未标记的视频中采样原型，并在在线中填充个性化缓存，而不需要积累大量的目标数据或存储多样化的源原型。其能量函数仅依赖于预训练的CLIP：目标视频嵌入和类别文本嵌入之间的相似性指导原型采样。

Result: 在BioVid、StressID和BAH上的实验表明，EB-CaP优于最先进的TTA方法，同时保持低计算和内存开销。

Conclusion: EB-CaP是一种有效的视频FER TTA方法，在保持低计算和内存开销的同时，优于现有方法。
...
TL;DR: InsertFuse：一种多类别参考引导图像插入的统一框架


<details>
  <summary>Details</summary>
Motivation: 为了提高多类别参考引导图像插入的性能

Method: 提出InsertFuse框架，通过解耦类别特定专业知识学习和跨类别能力整合，结合插入策略蒸馏和几何条件等策略

Result: 在公共AnyInsertion基准和多个类别测试集上取得了最先进的性能

Conclusion: InsertFuse在多类别参考引导图像插入方面表现出色
...
TL;DR: 研究了用于LR面部识别的合成生成策略，发现合成-真实差距，并强调需要在真实LR上验证生成方法。


<details>
  <summary>Details</summary>
Motivation: 在监控设置中，面部识别（FR）系统经常遇到低分辨率（LR）面部，其面部区域低于标准的112 × 112输入大小。虽然标记的高分辨率（HR）训练数据很丰富，但标记的原始LR数据，尤其是成对的原始LR/HR数据，却很稀缺。一种解决方案是从可用的HR面部合成LR数据，但合成努力与识别精度的回报程度尚不清楚。

Method: 提出了一种紧凑型、边缘设备导向的面部识别系统，研究了简单的合成生成策略，包括基于插值的退化、知识蒸馏、前置域转换器（PDT）、真实ESRGAN式退化和具有身份感知损失的学到的超分辨率（SR）前端。

Result: 在合成跨分辨率面部基准（LFW、CFP-FP、AgeDB-30）和真实世界原始LR数据集TinyFace上评估了这些策略，发现合成-真实差距：在合成基准上最优的退化设置并非在真实LR上最优。发现更多的合成努力并不单调地有帮助：学习到的SR前端没有超过直接将对齐的LR图像输入到强大骨干的方法，而紧凑骨干的简单插值增强是唯一优于其基线的合成。

Conclusion: 对于LR面部识别的生成方法必须在真实LR上进行验证，并与直接馈送基线进行比较。
...
TL;DR: 研究开发了一种基于AR的颌骨切割引导方法，成功替代了物理打印物，提高了手术效率和精度。


<details>
  <summary>Details</summary>
Motivation: 为了在颌骨重建手术中提供精确的手术引导，研究者们寻求一种无需物理打印物的解决方案。

Method: 研究者们开发了一种基于标记的无标记增强现实（AR）技术，该技术通过表面几何形状将虚拟切割引导与暴露的颌骨对齐。他们使用HoloLens 2的时间飞行相机捕获部分术中点云，并通过牙齿加权的全局阶段计算对应关系和求解截断最小二乘刚性对齐。不对称点对平面迭代最近点（ICP）阶段则进一步精确定位。

Result: 该技术在三种不同暴露条件下的平均目标注册误差（TRE）分别为4.05、6.10和7.10毫米，平均延迟为0.805秒。这些结果支持了使用AR技术替代物理打印的可行性。

Conclusion: 基于AR的颌骨切割引导方法在手术中具有可行性，可以减少物理打印物的使用，并提高手术效率。
...
TL;DR: SLED模型通过蒸馏技术，有效地压缩地球观测数据，并在多个任务上取得优异表现。


<details>
  <summary>Details</summary>
Motivation: 现有位置编码器在处理大量地球观测数据时存在计算成本高、批量大小限制和性能不佳等问题。

Method: 使用地理空间位置作为绑定模式，通过蒸馏技术预训练位置编码器。

Result: SLED模型轻量、模块化，可以灵活地结合多种模式，并支持更小的批量大小，提高效率。

Conclusion: SLED模型在多个基准任务上保持或超越了现有方法，并证明了在预训练中使用额外模式的益处。
...
TL;DR: 零样本提示质量不是适应性锚定质量的完整代理，需要在适应边界的两侧评估语言。


<details>
  <summary>Details</summary>
Motivation: 询问在目标域视觉适应后，根据冻结视觉-语言模型获得的零样本准确性选择的语言描述是否仍然有效。

Method: 在严格配对协议下，通过视觉LoRA在EuroSAT，CropDisease，ISIC和ChestX上比较通用类名模板和固定详细类描述。

Result: 在语义饱和和语义出现两种模式下，LoRA后的详细描述变得更加有用。

Conclusion: 零样本提示质量是适应性锚定质量的不完全代理，需要在适应边界的两侧评估语言。
...
TL;DR: CoDAT是一种适用于物联网边缘设备的高效人体动作识别模型。


<details>
  <summary>Details</summary>
Motivation: 实时人体动作识别在物联网边缘设备上需要能够捕获丰富时空线索的模型，同时满足严格的延迟、内存和功耗限制。

Method: 提出了一种名为CoDAT的协同双注意力Transformer，它使用轻量级的双分支模块来代替传统的多头注意力机制，包括空间卷积注意力（SCA）和步长单头注意力（SSHA）。同时，嵌入了一个参数免费的TShift模块以实现跨帧的时序通信。

Result: 在Jetson AGX Orin和Raspberry Pi 5上进行的广泛实验表明，CoDAT在图像和动作识别中实现了能量-精度平衡。在ImageNet-1K和Kinetics-400等数据集上，CoDAT在速度和准确性方面优于其他模型。

Conclusion: CoDAT是一种高效且准确的人体动作识别模型，适用于物联网边缘设备。
...
TL;DR: 提出了一种名为S&D的鲁棒性增强方法，显著提高了视觉任务中的模型性能。


<details>
  <summary>Details</summary>
Motivation: 为了保证安全关键应用中对自然图像损坏的鲁棒性，我们需要对模型的内部鲁棒性进行明确的表征。

Method: 我们提出了一种名为S&D的改进方法，该方法通过动态选择鲁棒路径和通过对称性保持变换来多样化这些路径，从而提高鲁棒性。

Result: S&D在八个基准测试中表现出色，在各种视觉任务、不同的主干网络和复杂的真实场景中，都显著提高了性能。

Conclusion: S&D是一种有效且可扩展的鲁棒性增强方法，对于提高安全关键应用中的模型鲁棒性具有重要意义。
...
TL;DR: WaveFreqAnchor：基于波-结构锚定和频率校正的免训练面部恢复框架。


<details>
  <summary>Details</summary>
Motivation: 针对现有基于扩散模型的反扩散方法在严重降质情况下存在身份相关结构漂移和保真度下降的问题。

Method: 提出了一种名为WaveFreqAnchor的训练自由框架，基于波-结构锚定和频率校正扩散。包括锚点空间波-结构指导（ASWG）和多尺度小波-傅里叶注入（MWFI）。

Result: 有效保留了面部身份，恢复了清晰和逼真的面部细节，实验结果表明优于现有方法。

Conclusion: 该方法在面部恢复方面表现优异，实现了高质量的保真度恢复。
...
TL;DR: Atelier是一个快捷方式感知的控制状态规划框架，显著提高了艺术家基础图像生成的质量。


<details>
  <summary>Details</summary>
Motivation: 现有的图像模型在响应艺术家名称时，通常通过经典快捷方式（如重复的图案、通用的色调或过度代表性的时期签名）来回应，而不是保留用户的意图场景。

Method: 引入Atelier，一个针对快捷方式感知的控制状态规划框架，用于艺术家基础图像生成。Atelier通过艺术家级别的知识和局部修补引用来锚定状态，编译后端感知的生成计划，并通过全局和局部真实性反馈迭代地改进候选图像。

Result: Atelier在艺术家级别的风格忠实度、对源结构的忠实度以及与基线相比的快捷方式替代减少方面，均表现出显著改进。

Conclusion: Atelier在艺术家基础图像生成方面表现出色，通过减少快捷方式替代，提高了艺术风格的一致性和对源结构的忠实度。
...
TL;DR: GraphVerse是一个评估MLLMs视觉图推理能力的统一基准，揭示了当前模型的局限性，并验证了GIE策略的有效性。


<details>
  <summary>Details</summary>
Motivation: 为了评估MLLMs在视觉图推理（VGR）中的能力，特别是它们是否能够真正地对结构化的视觉信息进行推理。

Method: GraphVerse是一个统一的基准，可以同时评估MLLMs在单图和配对图设置下的感知、视觉推理和基于文本的图推理。VGR-Score是一个过程敏感的指标，可以评估推理质量，而不仅仅是最终答案的准确性。

Result: 实验表明，当前的MLLMs在VGR中存在一些关键局限性，同时也验证了GIE策略的有效性和GraphVerse在更广泛的多模态推理能力中的可迁移性。

Conclusion: GraphVerse是一个统一的基准，可以同时评估MLLMs在单图和配对图设置下的感知、视觉推理和基于文本的图推理。VGR-Score是一个过程敏感的指标，可以评估推理质量，而不仅仅是最终答案的准确性。实验结果表明，当前的MLLMs在VGR中存在一些关键局限性，同时也验证了GIE策略的有效性和GraphVerse在更广泛的多模态推理能力中的可迁移性。
...
TL;DR: 提出AnyTrack，解决多模态跟踪问题，实验验证其优越性。


<details>
  <summary>Details</summary>
Motivation: 解决现有多模态跟踪器对模态组合固定、适应性和泛化能力有限的问题。

Method: 设计了一种新的统一框架AnyTrack，包括Modality-aware Interaction Module和Context Understanding Module，并扩展了多模态对象跟踪基准。

Result: 在完整和缺失模态设置下的广泛实验表明AnyTrack达到最先进性能。

Conclusion: AnyTrack在视觉对象跟踪方面实现了最先进的性能，验证了其有效性和灵活性。
...
TL;DR: UniCycleFlow通过新的框架提高了无配对图像翻译的性能。


<details>
  <summary>Details</summary>
Motivation: 为了在双向无配对图像翻译中同时保留源结构并学习双向的无监督变换。

Method: 提出了一种名为UniCycleFlow的修正流框架，将双向翻译表示为单个时间条件速度场的正向和反向积分。

Result: 在十个翻译方向上，UniCycleFlow在7个任务上实现了最低的FID，平均FID为55.1。

Conclusion: UniCycleFlow在无配对图像翻译任务中表现出色，提高了翻译质量。
...
TL;DR: PAST通过改进的奖励机制和自适应调节策略，提高了文本到图像任务中强化学习的效率和偏好优化质量。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在文本到图像任务中取得显著进展，但直接优化下游目标时仍存在局限性。强化学习（RL）可以实现有针对性的优化，但现有方法通常受限于低效的微调和稀疏的奖励。

Method: 我们提出了PAST，它通过联合感知去噪进度和提示难度来区分奖励，并自适应地调节训练周期长度。具体来说，我们设计了一种内在奖励范式来补偿稀疏的外部奖励，并引导模型探索从噪声模式中更有效地偏离的路径。我们进一步为内在奖励提供了理论依据。然后，PAST动态监测去噪完成度和图像结构与提示语义之间的语义对齐。当这两个指标满足生成要求时，系统自适应地终止训练。这使系统能够根据提示难度和当前生成过程适当地分配周期长度。最后，基于预测的残差噪声水平，我们建立了一种双重自适应协调机制。具体来说，它不仅平衡外部和内部奖励，而且平衡探索和收敛。

Result: 实验结果表明，PAST通过其双重自适应调节机制，将现有RL微调方法的计算效率提高了66.7%，并通过其双重自适应调节机制将偏好优化质量提高了29.5%。

Conclusion: PAST通过其独特的奖励机制和自适应调节策略，有效地提高了文本到图像任务中强化学习的效率和质量。
...
TL;DR: AdvTiles：一种基于可学习瓦片的物理对抗伪装框架，实现视觉自然性和攻击有效性的平衡。


<details>
  <summary>Details</summary>
Motivation: 针对现有方法在视觉自然性和攻击有效性之间的权衡问题，提出了一种新的物理对抗伪装框架。

Method: 使用可学习的瓦片构建AdvTiles框架，通过直通Gumbel-Softmax估计器进行可微分瓦片选择，实现瓦片模式和空间布局的联合优化。此外，通过可微分的3D高斯Splatting渲染，结合不同视角、尺度、光照和背景变化，提高伪装的鲁棒性。

Result: 在多个检测器上的实验表明，AdvTiles的平均ASR达到86.2%，优于现有最先进的攻击方法。将优化后的伪装应用于可穿戴对抗服装，验证了其在现实场景中的有效性。

Conclusion: AdvTiles框架在保持自然伪装外观的同时，实现了强大的攻击性能，为物理对抗攻击提供了新的思路。
...
TL;DR: 提出了一种新的图像恢复方法FIT，通过显式建模退化意识和引入任务标记丢弃策略，实现了最先进的图像恢复性能。


<details>
  <summary>Details</summary>
Motivation: 为了恢复由多种和空间非均匀损坏导致的图像退化，全图像恢复寻求一个单一模型。然而，许多统一的Transformer依赖于固定的补丁分区：任务/退化条件仅在网络块中注入，使嵌入和重建阶段对局部退化变化不敏感。

Method: 提出了一种名为FIT（Flexible Image Transformer）的新方法，该方法在整个管道中显式地建模退化意识，从补丁采样到像素重建。具体来说，FIT使用轻量级的退化编码器从局部退化严重程度预测全局退化向量g和空间退化图M，并通过自适应变形联合条件化补丁嵌入和解嵌入。此外，为了提高对退化类型的鲁棒性，我们引入了一种任务标记丢弃策略，在训练期间规范任务条件。

Result: 在五个标准基准（BSD68、Rain100L、SOTS、GoPro和LOLv1）上，FIT在五退化设置中实现了最先进的性能，平均PSNR为30.72 dB，在三个退化设置中为32.83 dB，比最近的统一恢复方法高出0.5~1.1 dB。此外，学习到的偏移量提供了直接的可视化退化意识空间自适应的方法。

Conclusion: FIT在图像恢复任务中表现出色，通过显式建模退化意识和引入任务标记丢弃策略，显著提高了对退化类型的鲁棒性，并实现了最先进的性能。
...
TL;DR: 提出了一种结合3D姿态推理和几何引导图像生成的框架，用于室内场景图像中插入新家具。


<details>
  <summary>Details</summary>
Motivation: 研究室内场景图像中插入新家具的问题。

Method: 提出一个两阶段框架，包括3D姿态推理和几何引导的图像生成。

Result: 实验结果表明，该方法在3D姿态估计和图像合成方面都得到了验证。

Conclusion: 该方法通过数据驱动的迭代推理解决了单视图家具插入的不确定性，并实现了几何上合理的3D放置。
...
TL;DR: 提出了一种新的目标检测方法ECADet，通过扩展对象性发现，提高了对具有语义意义的视觉元素的检测效果。


<details>
  <summary>Details</summary>
Motivation: 现有的目标检测方法主要针对离散和可数的物体实例进行定位，忽略了诸如天空、道路、草原、水域和运动场等具有语义意义的视觉元素。

Method: 提出了一种新的目标检测设置ECAD，并构建了一个超越事物中心对象性的基准BTCO-Bench。提出了一种基于冻结DINOv3编码器的轻量级DETR检测器ECADet，并引入了GAER和PGQM来提高定位和对象性估计。

Result: 在BTCO-Bench上，ECADet在代表性的无类别和基于提议的检测器中表现优异。

Conclusion: ECADet在扩展对象性发现方面是有效的。
...
TL;DR: 提出区域检测任务，构建RegionDet基准，评估不同检测器，揭示挑战，即将发布。


<details>
  <summary>Details</summary>
Motivation: 针对现有目标检测基准主要围绕单个对象实例构建，对区域目标的系统评估支持有限。

Method: 提出区域检测任务，构建区域检测基准RegionDet，包含八个区域类别，并使用COCO风格的边界框注释和评估协议。

Result: 在RegionDet上对代表性闭集和零样本/开放词汇检测器进行评估，结果显示闭集检测器可以部分学习区域级模式，而零样本/开放词汇检测器表现较差，揭示了当前视觉语言检测器的强对象中心偏差。

Conclusion: 区域检测面临挑战，包括弱边界线索、强上下文依赖和关系级区域理解不足。RegionDet将发布。
...
TL;DR: 我们提出了一种基于大规模数据集和四智能体框架的多智能体法医推理方法，显著提高了深度伪造检测的准确性。


<details>
  <summary>Details</summary>
Motivation: 现有的深度伪造检测方法在新兴合成方法面前效果不佳，缺乏可靠的文本标注。

Method: 构建一个包含 100,000 个视频的大规模深度伪造视频数据集，使用多智能体推理框架进行分析。

Result: 多智能体框架在基准测试中优于所有方法，包括 GPT 和 Gemini。

Conclusion: 我们的多智能体法医推理框架在 FaceVid-Forensics-100K 基准测试中表现出色，优于包括 GPT 和 Gemini 在内的所有方法。
...
TL;DR: DAEP算法在视频语料库中困难感知时间答案定位任务中表现优异


<details>
  <summary>Details</summary>
Motivation: 视频语料库中困难感知时间答案定位任务

Method: DAEP算法，包括视频排名、锚点扩展和重排序

Result: 在官方评估中排名第一，平均分数为0.2728

Conclusion: 视觉证据、程序上下文和困难感知规划提高了排名质量，复杂问题上的提升最大
...
TL;DR: FedVAR是一种用于视频异常识别的联邦学习框架，通过语义对齐提高异常检测的准确性。


<details>
  <summary>Details</summary>
Motivation: 在工业物联网和物理-数字系统时代，联邦学习为视频异常识别提供了一种有前景的去中心化智能范式。

Method: FedVAR，一个弱监督的联邦学习框架，利用视觉-语言模型（VLMs）的丰富表示，采用基于原型的对齐机制，为所有客户端创建一个共享的语义锚点，以重新中心和调整其视觉和文本特征空间。

Result: 在具有挑战性的基准测试中，FedVAR在各种非IID数据分区方案、未见领域和新颖异常类别中，始终优于最先进的联邦基线，为基于视频的物理-数字系统中的分布式智能建立了一个稳健的框架。

Conclusion: FedVAR为视频异常识别提供了一种有效的解决方案，有助于在分布式边缘客户端之间实现语义对齐，并提高异常检测的准确性。
...
TL;DR: ControlRef: An efficient and precise multi-instance synthesis framework for MM-DiTs


<details>
  <summary>Details</summary>
Motivation: Layout-guided multi-instance generation is essential for controllable image synthesis in Multi-Modal Diffusion Transformers (MM-DiTs). However, integrating this capability into unified architectures remains challenging.

Method: Proposes ControlRef, a highly efficient and precise multi-instance synthesis framework. Utilizes a Unified Instance-Layout Control (UILC) attention mask and Anchored 4D-RoPE for precise regional binding and spatial alignment.

Result: Achieves state-of-the-art visual fidelity and localization accuracy. Reduces inference latency by over 80% in sparse layouts and memory overhead by 50% in dense scenarios.

Conclusion: ControlRef effectively addresses the challenges of layout-guided multi-instance generation in MM-DiTs, providing efficient and precise image synthesis.
...
TL;DR: HazeSpikeMamba：结合脉冲灵感和注意力机制的紧凑除雾框架，显著提高除雾性能。


<details>
  <summary>Details</summary>
Motivation: Dehazing networks在真实照片上的性能往往下降，因为合成雾霾模型不能完全捕捉真实雾霾的变异性，且配对的真实雾霾-清晰图像稀缺。

Method: 提出HazeSpikeMamba，一个紧凑的除雾框架，结合了多尺度U-Net中的脉冲灵感和注意力状态空间全局路径。使用TPCNNSpike更新所有神经元，并采用MambaIRv2的注意力状态空间模块进行全局路径。对于目标域适应，使用预训练的冻结退化网络进行雾霾重合成。

Result: 在RTTS、URHI和HSTS上，该适应模型在BRISQUE和NIMA方面均优于其他方法。

Conclusion: HazeSpikeMamba通过结合脉冲灵感和注意力机制，在除雾任务上取得了显著的性能提升。
...
TL;DR: PORTA：无需重新训练的VLM剪枝框架，实现高效压缩。


<details>
  <summary>Details</summary>
Motivation: 现有剪枝策略在部署受限环境中面临挑战，且不适用于无任务特定样本的剪枝。

Method: 提出PORTA剪枝框架，基于激活变化和自适应稀疏分配机制。

Result: PORTA在CLIP、BLIP和Qwen2-VL等VLM架构上实现高性能，无需重新训练。

Conclusion: PORTA是高效的VLM压缩框架。
...
TL;DR: MuST-VAD通过双向知识交换提高了弱监督视频异常检测的精度。


<details>
  <summary>Details</summary>
Motivation: 在弱监督视频异常检测中，提高检测精度和特征丰富度。

Method: 提出MuST-VAD，一种双向学习框架，其中异常检测器和大型视觉语言模型相互交换知识。

Result: 在UCF-Crime数据集上，MuST-VAD将单次传递基线的一跳AUROC从88.15%提高到了88.63%，平均精度（AP）从37.25%提高到了42.46%，在AP方面优于最先进的方法4.13个百分点。

Conclusion: MuST-VAD通过双向学习提高了弱监督视频异常检测的性能。
...
TL;DR: 研究提出了一种新的肋骨骨折诊断方法，提高了定位的准确性和效率。


<details>
  <summary>Details</summary>
Motivation: 简化CT图像上肋骨骨折的定位过程，减少诊断时间。

Method: 阶段诊断研究，双平面几何重建，侧探测器再训练，跨视图对应信度识别。

Result: 双平面重建几何精确度达到中值3D误差4.0毫米，双视图可用性达到61.1%，58.4%的骨折存在正确配对。侧探测器再训练提高了双视图可用性，准确度达到93%。最终端到端准确率为2.50%。

Conclusion: 验证了双平面重建几何的高条件定位保真度，通过阶段性识别跨视图对应信度作为有效瓶颈，以及保留不确定发现的选择性辅助工作流程，而不是一个独立的自动重建器。
...
TL;DR: 提出了一种基于Sonata架构的多模态自监督学习框架，在户外LiDAR点云任务上取得了显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 解决户外环境中机器人操作时深度学习模型性能受限的问题，由于3D标注成本高导致标注数据稀缺。

Method: 提出了一种多模态、多教师蒸馏框架，用于户外LiDAR点云上的自监督学习。基于Sonata架构，引入了Vernata，包括三个扩展：稀疏视图增强、内存银行机制和跨模态蒸馏。

Result: 在GrandTour、TartanGround和Waymo数据集上，以及我们自己的机器人平台收集的数据上进行了评估，结果表明，与Sonata基线相比，该方法在TartanGround上实现了54.7的mIoU分数（+5.9分，+12.1%），在Waymo上实现了57.1的mIoU分数（+7.3分，+14.7%）。在减少模态设置（缺少颜色或法线）的情况下，自监督方法也保持了强大的性能，分别在相应数据集上实现了49.4和50.2的mIoU分数。

Conclusion: 该方法在户外LiDAR点云的自监督学习方面取得了显著的性能提升。
...
TL;DR: MaskFlow是一种用于区域图像编辑的框架，实现了精确定位、背景保留和边界转换，在自然场景和信息图表图像上表现优异。


<details>
  <summary>Details</summary>
Motivation: 区域图像编辑因其空间可控性而受到广泛关注。尽管基于指令和基于掩膜的编辑方法可以实现强大的语义对齐，但可靠的区域控制仍然具有挑战性，其中编辑必须准确定位并自然地与保留的上下文结合。

Method: 我们提出了MaskFlow，这是一个用于精确定位、一致背景保留和无缝边界转换的训练框架。MaskFlow将掩膜集成到概率路径和流匹配目标中，协调可编辑区域内的生成与源保留区域外的生成。我们提出的Soft-Poisson去缝合模块进一步在训练和采样过程中细化预测向量场，以改善编辑前景与保留背景的平滑集成。我们还设计了一个数据合成管道来构建MEData，这是一个基于掩膜的图像编辑数据集，用于训练区域图像编辑模型并促进进一步的研究。

Result: 在自然场景和信息图表图像上的实验表明，在定性和定量评估方面，与竞争方法相比，该方法在一致性方面有所提高。

Conclusion: MaskFlow是一种有效的区域图像编辑方法，在保持背景的同时，实现了精确的区域定位和无缝的边界转换。
...
TL;DR: 提出AVCap模型和评估标准，解决音频视频联合字幕的挑战


<details>
  <summary>Details</summary>
Motivation: 为了解决现有工作在高质量公共数据集、强化学习方法以及评估标准方面的限制

Method: 提出AVCap-100K数据集、AVCap模型和AVCap-Bench评估标准

Result: AVCap模型在多个评估中达到或超过专有模型，AVCap-Bench和AVCap-Score为音频视频字幕的原子级细节提供评估标准

Conclusion: 该研究为音频视频联合字幕的多模态视频理解和生成提供了新的解决方案
...
TL;DR: TACT通过改进MLLMs的反常识推理能力，在视觉推理任务中实现了显著的性能提升。


<details>
  <summary>Details</summary>
Motivation: 针对MLLMs在视觉反常识推理中的瓶颈问题，特别是低频事实场景中共享语言解码器对先验知识的偏好导致的问题。

Method: 提出了一种文本锚定的数据构建流程（FFD），估计常识事实的先验强度，并将验证的反常识场景提炼成高质量的文本语料库。基于此语料库，引入了TACT，一种文本锚定的后训练框架，该框架通过将证据跟随和先验驱动推理轨迹引导到不同的优化阶段，使解码器能够解决先验-证据冲突。

Result: 在反常识视觉基准测试中，TACT在视觉推理方面取得了显著提升，同时保持了泛化能力，证明了文本到视觉的跨模态迁移的有效性。

Conclusion: 通过改进MLLMs的反常识推理能力，TACT在视觉推理任务中取得了显著的性能提升，为下游应用提供了更强大的支持。
...
TL;DR: DCMPC-Net通过跨模态语义引导和结构增强，有效提高了图像恢复效果


<details>
  <summary>Details</summary>
Motivation: 针对现有图像恢复模型在适应不同天气条件时缺乏对退化特征的显式空间和语义建模的问题

Method: 提出了一种名为DCMPC-Net的退化感知跨模态提示补偿网络，该网络利用预训练的视觉-语言模型从跨模态退化线索中提取条件恢复特征

Result: 在多种天气条件下实现了鲁棒且感知一致的恢复，实验结果表明DCMPC-Net在特定任务和统一设置中都优于现有方法，实现了更高的准确性和视觉保真度

Conclusion: DCMPC-Net通过跨模态语义引导、空间对齐和结构增强，有效提高了图像恢复效果
...
TL;DR: 提出DSMCL框架，提高了跨模态ReID的性能。


<details>
  <summary>Details</summary>
Motivation: 为了解决现有方法在空间嵌入空间中学习模态一致性时的性能限制，同时忽略了频域模态差异，特别是在具有高度判别性和模态敏感性的高频表示。此外，大多数方法针对特定的模态设置进行了定制，限制了它们在多样化的跨模态场景中的应用。

Method: 提出了一种双空间模态一致性学习（DSMCL）框架，该框架联合建模空间特征分布一致性和频域判别一致性。空间模态一致性学习（SMCL）分支执行高斯特征对齐，而频域感知判别一致性学习（FDCL）策略通过身份感知的跨模态对比学习对高频表示进行正则化。DSMCL能够同时捕获模态特定的特征和模态共享的身份线索，学习鲁棒表示，并建立一个统一的框架，能够适应多样化的异构模态设置。

Result: 在SYSU-MM01、RegDB、LLCM、HOSS-ReID和CMShipReID上进行了大量实验，包括17种评估协议，表明DSMCL在各种跨模态场景中一致地提高了多个代表性基线的方法性能。

Conclusion: DSMCL是一个通用的跨模态ReID框架，通过联合建模空间和频域特征，实现了在多个数据集和评估协议上的性能提升。
...
TL;DR: 提出了一种基于VLM的卫星数据下行链路优化策略，显著降低带宽消耗并加速时间敏感任务的时间洞察。


<details>
  <summary>Details</summary>
Motivation: 现代地球观测卫星产生大量高分辨率、多光谱数据，但下行链路容量成为瓶颈，导致延迟或观测数据丢失。

Method: 提出“先总结，后下载”的范式，利用机载边缘计算和视觉语言模型（VLM）。

Result: 系统通过三个阶段的交互协议，将下行链路从被动批量传输转变为主动、语义感知的对话。在资源受限的NVIDIA Jetson平台上实现和评估系统，实验表明该策略显著降低了带宽消耗，并加速了时间敏感任务的时间洞察。

Conclusion: 该策略有效解决了卫星数据下行链路瓶颈问题，提高了时间敏感任务的数据获取效率。
...
TL;DR: 使用多模态融合技术，无人机调查在野生动物种群估计中表现出色，提高了性别和年龄结构的分类准确率。


<details>
  <summary>Details</summary>
Motivation: 支持野生动物种群估算的空中无人机调查，但有用的普查不仅仅是计数：种群动态由物种组成、性别比和年龄结构定义，即哪些物种存在以及一群动物如何分为成年雄性、成年雌性和幼崽。

Method: 使用自监督DINOv3特征在各个阶段融合两种记录模式，跟踪动物，并在两个摄像头达成一致时才确认动物，只保留清晰、非遮挡的帧，并通过投票分配物种和性别；从地理参照的身体大小中单独读取生命阶段，因为在大调查分辨率下，幼崽通常只与成年雌性在大小上有所不同。

Result: 在鹿角季节的四次飞行中，融合的管道正确地分类了26个检测到的个体中的25个（8个成年雄性中的7个，所有16个成年雌性和2个幼崽），而单独使用任何一个传感器只有20个。多模态物种分类达到96.0%，而性别分类中融合两个传感器最为重要：结合RGB + 热模型在环境和季节中最为稳健。自动化人口分类将无人机飞行从计数转变为可重复阅读的群体结构，因此管理者已经采取行动的性别比和年龄结构可以像调查可以飞行一样经常收集。

Conclusion: 多模态融合技术可以显著提高无人机在野生动物种群调查中的效率和准确性，为种群管理和保护提供有力支持。
...
TL;DR: LEADer：一种基于扩散模型的图像修复新方法，通过动态调节先验强度和自适应轨迹剪枝，显著提高了性能和效率。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的图像修复方法在指导图像重建时，通常依赖于固定的数据约束和均匀的步长，从而忽视了生成过程的动态性质。这种僵化的设计使得模型容易受到空间非均匀退化的影响，从而导致结构扭曲和细节丢失。同时，均匀的步长引入了计算冗余，而简单的步长减少策略往往会导致累积近似误差。

Method: 提出了一种名为LEADer的本地知识不确定性引导主动采样框架。在空间域中，LEADer利用像素级不确定性动态调节零空间内的先验强度，从而有效平衡细节保留和伪影抑制。在时间域中，它通过不确定性轨迹量化采样稳定性，从而实现自适应轨迹剪枝，从而加速收敛。

Result: 理论证明表明，我们的框架实现了严格的数据一致性，而轨迹剪枝策略允许一个确定的误差界限，从而保证了在跳过采样下的稳定收敛。值得注意的是，我们的即插即用方法可以无缝集成到各种DMIR基线中。大量实验表明，LEADer提高了多种最先进的DMIR方法的性能，同时显著减少了采样时间，而内存开销可以忽略不计。

Conclusion: LEADer是一种有效的图像修复方法，可以显著提高DMIR的性能，同时减少采样时间和内存开销。
...
TL;DR: 本文提出了一种针对无监督高分辨率图像生成的DiT模型适配方法，有效提升了生成性能。


<details>
  <summary>Details</summary>
Motivation: 为了解决现有研究在无监督高分辨率图像生成任务中，对Diffusion Transformer (DiT) 模型应用不足的问题。

Method: 提出了一种新方法，针对无监督高分辨率图像生成任务，特别针对空间无序和长生成时间这两个挑战，对现有的DiT模型进行适配。

Result: 实验结果表明，该方法有效提高了无监督高分辨率图像生成的性能。

Conclusion: 该方法有效地解决了现有DiT模型在无监督高分辨率图像生成中的空间无序和长生成时间问题，为无监督高分辨率图像生成提供了新的思路和方法。
...
TL;DR: 视频LLM性能随视觉预算变化，需针对特定任务调整。


<details>
  <summary>Details</summary>
Motivation: 分析视频LLM（大型语言模型）在视觉预算增长时的性能表现。

Method: 通过控制视觉预算，分析每个冻结模型-项目对的响应轨迹，并推导出配置互补性、有害转换和文本覆盖的匹配网格度量。

Result: 发现没有单一的预算适用于所有项目；在多个任务和模型中观察到配置互补性；通过采样干预和结构化帧审计提高了性能。

Conclusion: 视频LLM的性能随视觉预算的变化而变化，需要针对特定任务和模型调整预算。
...
TL;DR: Proposed methods and dataset enhance omni-domain IRST detection performance.


<details>
  <summary>Details</summary>
Motivation: Omni-domain infrared small target (IRST) detection is crucial for infrared surveillance, yet remains challenging due to heterogeneous imaging domains and inconsistent target characteristics.

Method: We introduce 	extbf{``understand before detect''}, a paradigm that formulates omni-domain IRST detection as an understanding-driven process, where holistic infrared target understanding precedes precise detection. We propose 	extbf{JinSight}, which first develops holistic IRST understanding through language supervision and then transfers the learned cross-domain representations to precise small-target detection. We introduce Latent Semantic Interaction (LSI), which exchanges language-aligned global semantics with fine-grained spatial features in a compact low-rank space. We build 	extbf{OmniIRST-VL}, the first large-scale, highly diverse vision--language dataset for omni-domain IRST detection.

Result: JinSight enables a single model to generalize across heterogeneous infrared domains. Latent Semantic Interaction (LSI) improves the detection performance. OmniIRST-VL provides a benchmark for omni-domain IRST detection.

Conclusion: The proposed methods and dataset address the challenges of omni-domain IRST detection and improve detection performance.
...
TL;DR: YOLO-PEFT是一种结构感知的PEFT框架，提高了实时检测器的性能。


<details>
  <summary>Details</summary>
Motivation: 语言模型迁移的通用参数高效微调（PEFT）方法在实时检测器上可能无声失败，因为异构操作和检测特定组件强加的放置约束在常规Transformer堆栈中不存在。

Method: 提出YOLO-PEFT，一个结构感知框架，将适配器放置公式化为可审计的约束规划问题。

Result: 在YOLO11s和YOLO12s上，与Full-SFT的0.6428和0.6662相比，规划选择的RS-LoRA分别达到0.7138和0.7307 mAP50-95。在RT-DETR-L上，所有七个评估的LoRA家族配置均超过预定义的灾难性阈值。

Conclusion: YOLO-PEFT通过将手动目标模块试错替换为显式、可检查的规划，同时保留经过验证的训练-保存-合并-导出路径，在评估的检测器家族、放置策略和校准覆盖范围内提高了PEFT的效率。
...
TL;DR: 提出了一种名为KnifeHunter的刀具图像检索系统，有效提高了执法机构对刀具识别的效率。


<details>
  <summary>Details</summary>
Motivation: 刀具暴力是公共安全的一大挑战，执法机构需要可扩展的工具进行刀具识别、情报分析和溯源。

Method: 提出了一种名为KnifeHunter的端到端法医刀具图像检索系统，结合了全局上下文和空间局部判别证据。

Result: 在Medium协议下，CoRe-Net实现了88.0%的mAP和86.7%的mP@10，在干扰条件下实现了85.1%的mAP和83.8%的mP@10。

Conclusion: KnifeHunter在实战执法环境中实现了高效的细粒度法医刀具匹配。
...
TL;DR: RoRA是一种有效的训练无关框架，可以显著降低MLLMs的推理时间和存储成本，同时保持较高的准确性。


<details>
  <summary>Details</summary>
Motivation: 为了降低多模态大型语言模型（MLLMs）在图像编码中的预填充和KV缓存存储成本，现有训练无关的剪枝方法通过重要性、多样性和空间覆盖来选择标记，但将保留的标记视为可互换的，并且没有明确跟踪哪些与对象相关的区域已经被覆盖。

Method: RoRA，将视觉标记剪枝作为面向角色的区域证据分配。给定一个固定预算，RoRA将标记分为受保护的语义核心、互补上下文和细粒度细节。首先，使用位置先验和提示校准的对象先验对文本条件注意力进行校准，然后从高置信度锚点构建注意力锚定区域（AARs）作为覆盖对象支持的轻量级代理。主要在AARs之外探索上下文，而一个小型的AAR引导预算恢复局部细节；仅对上下文阶段的冗余过滤使用成对相似度。在匹配的预算下，RoRA在LLaVA和Qwen-VL系列中始终优于强大的训练无关基线，即使在激进的剪枝比率下也保留了大部分未剪枝的准确性，例如，在LLaVA-1.5上，在88.9%的剪枝率下保留了96.5%的完整性能，在Qwen3-VL上，在75-90%的剪枝率下比D2Pruner提高了约5%。在66.7%的剪枝率下，RoRA在标记选择上只需0.7毫秒，将端到端推理时间减少了24.6%，对应于在NVIDIA H800上未剪枝推理的1.33倍加速。

Result: RoRA在LLaVA和Qwen-VL系列中始终优于强大的训练无关基线，即使在激进的剪枝比率下也保留了大部分未剪枝的准确性。在66.7%的剪枝率下，RoRA在标记选择上只需0.7毫秒，将端到端推理时间减少了24.6%，对应于在NVIDIA H800上未剪枝推理的1.33倍加速。

Conclusion: RoRA是一种有效的训练无关框架，可以显著降低MLLMs的推理时间和存储成本，同时保持较高的准确性。
...
TL;DR: SCSR在独立的中国人群中有效，为神经退行性疾病研究提供新工具。


<details>
  <summary>Details</summary>
Motivation: 研究随机皮质自我重建（SCSR）在神经退行性疾病如阿尔茨海默病（AD）中的应用，以及其在独立的中国人群数据集中的泛化能力和迁移性。

Method: 评估SCSR衍生的Z分数在区分健康扫描、轻度认知障碍（MCI）和AD患者方面的能力，并比较四种训练策略：直接应用UKB训练的模型、在中文数据上微调、从头开始训练和联合训练。

Result: SCSR在所有评估的模型中提供了稳健的皮质萎缩检测，其中微调的SUNet模型表现最佳（平均成对AUC = 0.848），其次是UKB训练的SUNet。重建误差在整个生命周期内保持较低，表明了强大的跨人群迁移性。

Conclusion: SCSR在独立的中国人群数据集中表现出良好的泛化能力和迁移性，为神经退行性疾病的研究提供了新的工具。
...
TL;DR: GABL improves bronchoscope localization accuracy and speed.


<details>
  <summary>Details</summary>
Motivation: Camera localization in bronchoscopy is challenging due to accuracy, real-time constraints, and limited training data.

Method: Proposes a unified geometry-aware bronchoscope localization framework (GABL) that fuses preoperative structural priors with intraoperative video. Uses graph-guided localization and Transformer-based tracking with RGB-depth matching objective.

Result: Reduces translation and rotation errors by 8.37% and 31.76% compared to state-of-the-art, with 4x inference speedup.

Conclusion: GABL is effective for bronchoscope localization, improving accuracy and real-time performance.
...
TL;DR: This paper proposes a framework for improving VLMs in radiology reporting by adding spatially localized anomaly heatmaps, enhancing diagnostic accuracy and clinical interpretability.


<details>
  <summary>Details</summary>
Motivation: Radiology reporting is time-consuming and subject to inter-rater variability, making automated report generation an attractive clinical application for Vision-Language Models (VLMs).

Method: Benchmarking state-of-the-art VLMs on lumbar spine MRI, focusing on diagnostic accuracy. Proposing an architecture-agnostic framework that augments VLM inputs with spatially localized, disc-level anomaly heatmaps generated by a semi-supervised U-Net++ model.

Result: Demonstrating that standard lexical and semantic metrics poorly reflect clinical correctness. Improving anatomical sensitivity and providing an independent interpretability output for clinical oversight.

Conclusion: Moving closer to diagnostically reliable, visually grounded VLMs for lumbar spine MRI interpretation.
...
TL;DR: 提出了一种新的多假设流估计框架，以解决模糊区域中的多匹配问题，并在视频帧插值中实现了显著的性能提升。


<details>
  <summary>Details</summary>
Motivation: 为了解决在模糊区域中由于重复或随机纹理、旋转对称结构或模糊运动等原因导致的多个可能的匹配，单一流估计器只能保留一个位移并丢弃其他候选位移的问题。

Method: 提出了一种多假设流估计框架，该框架保留最上面K个候选对应关系，并通过可靠性引导的路由在每个位置选择一个。每个假设从粗匹配锚点初始化，并通过锚点中心局部注意力分别细化。帧合成因此基于所选的流-外观假设，而不是候选运动的软组合。

Result: 在MA-HD基准和公共VFI基准上的实验表明，该方法在比较的方法中实现了最佳的LPIPS和DISTS。

Conclusion: 该方法有效提高了视频帧插值的质量。
...
TL;DR: InstanceSplat：通过实例感知和共享结构，实现高效泛化的3D重建和场景理解。


<details>
  <summary>Details</summary>
Motivation: 当前3DGS方法在场景理解上存在分类导向和实例感知不足的问题。

Method: 提出InstanceSplat，一个统一的实例感知3DGS框架，用于从姿态无关的多视角图像中进行可泛化的3D重建和实例感知场景理解。

Result: 在多种输入视角设置和未见过的数据集上，InstanceSplat在新型视图合成、实例分割和开放词汇语义理解方面表现出最先进的性能、实用效率和强大的泛化能力。

Conclusion: InstanceSplat通过共享实例结构和实例感知表示，实现了3D重建和场景理解的相互促进，为3D场景理解提供了新的思路。
...
TL;DR: 提出了一种新的优化器-校正器框架，有效解决了深度生成模型中3D形状优化的流形漂移问题。


<details>
  <summary>Details</summary>
Motivation: 优化3D形状在深度生成模型潜在空间中的基本原理对于计算机辅助工程至关重要，但存在一种称为流形漂移的关键故障模式：基于梯度的优化将潜在向量移动到有效形状流形之外的趋势。

Method: 提出了一种新的优化器-校正器框架，该框架交替进行梯度步骤进行目标最小化和引导流匹配，以将潜在状态驱动回有效形状流形。通过将目标最小化与基于流的校正解耦，自由优化并严格校正，这种交替设计避免了固有的权衡，在保持几何有效性的同时，没有牺牲表达能力，同时在现代3D形状模型上保持计算可行性。

Result: 在从简单向量潜在空间到大型架构的各种下游优化任务中，包括空气动力学阻力和物体合规性优化，证明了其有效性。

Conclusion: 该研究提出了一种新的优化器-校正器框架，有效地解决了深度生成模型中3D形状优化的流形漂移问题，并在多个下游任务中取得了良好的效果。
...
TL;DR: CANIS improves 3D object orientation canonicalization by incorporating semantic orientation prior from image-to-3D generative models.


<details>
  <summary>Details</summary>
Motivation: Existing 3D object orientation canonicalization approaches often rely on geometric cues, but fail to incorporate semantic meaning.

Method: CANIS, a category-agnostic, generation-assisted framework, introduces the semantic orientation prior of a frozen image-to-3D generative model into 3D canonicalization.

Result: CANIS improves downstream 3D classification, part segmentation, and dense correspondence under arbitrary rotations. Validated on synthetic benchmarks and partial observations in real-world scans.

Conclusion: CANIS is an effective framework for 3D object orientation canonicalization.
...
TL;DR: 通过微调和结合不同CMR基础模型，提高了多视角CMR分析的有效性，但LGE瘢痕分割仍有待改进。


<details>
  <summary>Details</summary>
Motivation: 探讨不同心脏核磁共振（CMR）基础模型在异构多视角和多序列CMR分析中的有效性。

Method: 对CineMA进行微调，用于短轴和长轴视角的电影和晚期钆增强（LGE）分割；使用两个最新的冻结CMR基础模型提取嵌入向量，然后结合注意力机制的多实例学习进行左心室射血分数（LVEF）回归。

Result: 电影分割在短轴、双心室和四心室电影MRI上的Dice分数分别为0.862、0.883和0.902；LGE分割在不同视角上的Dice分数在0.621到0.846之间；直接LVEF回归模型实现了4.96个百分点的均方误差（MAE）和0.91的皮尔逊相关系数。

Conclusion: 基础模型可以有效地适应和结合用于多视角CMR分析，而准确的LGE瘢痕分割仍然是一项具有挑战性的任务。
...
TL;DR: EliSeg：一种基于报告的异常分割方法，无需预定义目标，在多个数据集上表现优异。


<details>
  <summary>Details</summary>
Motivation: Radiology reports描述临床观察，但没有指定可执行的分割目标。

Method: 我们研究了基于报告的异常分割，其中模型必须在绘制相应区域之前，直接从未过滤的报告中选择目标资格、基数和发现到掩码的对应关系。我们提出了EliSeg框架，该框架将目标构建与掩码生成相结合。

Result: EliSeg在MIMIC-CXR-ILS数据集上的实验表明，它一致优于直接分割方法和提取-然后分割级联，同时有效地抑制了无效报告提及的掩码。消融研究证实了验证和修订的互补作用，并在CheXlocalize数据集上的评估表明，EliSeg有效地迁移到了外部数据集。

Conclusion: EliSeg是一种有效的基于报告的异常分割方法，能够从未过滤的报告中直接确定目标资格、基数和发现到掩码的对应关系，并在多个数据集上取得了良好的性能。
...
TL;DR: 提出了一种无监督的LVLMs幻觉检测和缓解框架，显著提高了模型性能。


<details>
  <summary>Details</summary>
Motivation: 解决大型视觉语言模型（LVLMs）中物体幻觉问题

Method: 使用Logit Lens解码高注意力区域的视觉特征，并识别出两种幻觉机制：视觉不确定性和上下文先验。提出Detect-Mitigate框架，包括Logit-Lens Consistency Check、HARM和VEED方法。

Result: 在多个幻觉基准测试中取得最先进的结果

Conclusion: 通过分析模型注意力机制，提出了一种有效的无监督幻觉检测和缓解框架，提高了LVLMs的性能。
...
TL;DR: Proposes H2AL, an RFMIS framework using hyperbolic space and hierarchical modeling to improve segmentation performance.


<details>
  <summary>Details</summary>
Motivation: To overcome the limitations of existing registration-based Few-shot medical image segmentation (RFMIS) methods that primarily perform pixel-level optimization and inference in Euclidean space, treating anatomical structures as flat and disjoint, and neglecting inherent hierarchies.

Method: Proposing a Hyperbolic Hierarchy-aware Aggregative Learning (H2AL) framework for RFMIS. The framework includes a Hyperbolic Hierarchy-aware Infusion (H2I) module that utilizes hyperbolic space for hierarchical modeling, and an end-to-end joint optimization algorithm for collaborative learning across tasks.

Result: Experiments on two anatomical regions demonstrate the effectiveness and efficiency of the proposed method in both registration and segmentation.

Conclusion: The H2AL framework significantly improves pseudo-label quality and anatomical discrimination, leading to enhanced segmentation performance in RFMIS.
...
TL;DR: SkySeaLand数据集用于评估卫星目标检测器，SkyDet是一个低参数基线，用于混合交通检测。


<details>
  <summary>Details</summary>
Motivation: 卫星目标检测在处理小目标和宽格式场景时面临挑战，因为标准正方形输入缩放会导致细节丢失。

Method: 引入SkySeaLand，一个包含1307张高分辨率卫星图像和19101个验证过的边界框的公开数据集，包含飞机、船只、汽车和陆地场景。提供原始的COCO和YOLO标注。评估了YOLO、RT-DETR、DETR和Faster R-CNN系列中的12个检测器。

Result: YOLO和RT-DETR变体获得84.4-88.2 mAP50，没有从更大的参数计数中获得一致的准确度提升。SkyDet（一个1.22 M参数的无锚点基线）在4.90 MB的足迹中获得60.5 mAP50和24.32 mAP50-95，在Tesla T4上的延迟为13.74 ms（72.8 FPS）。

Conclusion: SkySeaLand为混合陆地和海洋交通检测提供了一个紧凑的基准，而SkyDet建立了一个低足迹的参考标准，而不是最先进的准确度声明。
...
TL;DR: GeoDistill-Refine通过引入伪掩码和细化方法，提高了航天图像分割的准确性。


<details>
  <summary>Details</summary>
Motivation: 为了解决基础分割模型在航天图像分割中存在的几何误差问题，提出了一种名为GeoDistill-Refine的两阶段框架。

Method: 该框架将离线的SAM 3伪掩码传输到紧凑的分割网络中，并通过六种固定的提示词进行融合，以稳定教师输出。学生网络首先学习前景轮廓，然后通过从伪掩码中导出的签名距离场、骨架和面积目标进行细化。此外，还引入了样本级门控机制，以减少不可靠伪几何的影响。

Result: 在SpaceSense-Bench HJM锁箱集上，GeoDistill-Refine在Image IoU和Boundary F1上分别比纯伪标签学生提高了0.0456和0.1380。在SPEED+ Lightbox和Sunlamp领域以及TANGO上的外部评估显示出与边界质量或前景精度增益的竞争力。TinyUNet包含0.263 M参数，在RTX 4090上处理每张图像大约需要1.1毫秒。

Conclusion: GeoDistill-Refine框架有效地提高了航天图像分割的准确性和效率。
...
TL;DR: WorldTrace提高了长期视觉持久性，无需重新训练。


<details>
  <summary>Details</summary>
Motivation: 研究交互式视频世界模型中的视觉持久性。

Method: 提出WorldTrace，一个无训练的内存框架，用于长期视觉持久性。

Result: WorldTrace-Field提高了时间一致性+15.5%，WorldTrace-Landmark在LoopBench上提高了情节回忆+19.5%。

Conclusion: WorldTrace有效提高了视觉持久性生成，无需重新训练。
...
### [68] [SABRE: Scalable and Automated Benchmarking of VLMs under Stress](https://arxiv.org/abs/2608.07435)
*Zixuan Lan,Luzhe Sun,Matthew R. Walter,Jiawei Zhou*

Main category: cs.CV

TL;DR: SABRE是一种自动化的VLM压力测试框架，用于识别VLM的弱点。


<details>
  <summary>Details</summary>
Motivation: 为了解决视觉语言模型（VLMs）在基准测试方面的发展滞后问题，提出了一种名为SABRE的自动管道。

Method: SABRE将测试初稿转换为结构化规范、生成或编辑的图像和问答对。自动化过滤和人工审查用于验证候选答案的有效性。

Result: SABRE-Prior测试表明，VLMs在视觉证据和世界先验知识之间存在差异。SABRE支持其他压力测试设置。

Conclusion: SABRE是一个可重用的框架，用于构建和更新VLM压力测试，而不是一个固定的基准。
...
TL;DR: UniJEPA：统一联合嵌入预测架构，实现高效的零样本规划。


<details>
  <summary>Details</summary>
Motivation: 现有方法在联合图像级和视频级世界建模方面存在碎片化，需要一种统一的方法。

Method: 提出UniJEPA，一个统一的JEPA，在共享的潜在空间中联合学习光度预测（图像级变换）和时间预测（视频级下一状态动态）。

Result: UniJEPA在图像、视频和控制基准测试中表现良好，速度比生成式世界模型快数十倍。

Conclusion: UniJEPA通过统一潜在空间中的光度预测和时间预测，实现了高效的零样本规划。
...
TL;DR: ISYV为视频推理提供统一框架和解决方案，提高身份匹配和长期跟踪能力。


<details>
  <summary>Details</summary>
Motivation: 解决现有视频推理任务中简化的视频-文本设置所导致的身份匹配和以人为中心的推理限制问题。

Method: 提出身份条件查询（ICQ）任务，并构建ISYV（我在视频中寻找你）系统解决方案，包括ISYV-Bench、ISYV-75K和ISYV-Framework。

Result: ISYV-Bench在身份识别到因果推理的能力评估中表现出挑战性，ISYV-75K是一个大规模训练集，ISYV-Framework包含针对ICQ的模型和训练策略。实验表明，ISYV在跨领域身份匹配和长期跟踪方面优于基线模型。

Conclusion: ISYV为以人为中心的视频推理提供了统一的任务定义、可扩展的数据集/基准和建模见解。
...
TL;DR: COVER通过改进grounding系统的时间定位准确性，提高了视频时间定位的可靠性。


<details>
  <summary>Details</summary>
Motivation: 视频时间定位的ground truth是区间分布，但每个grounding系统只返回一个区间，没有可靠性声明，因此在部署时无法区分正确和错误的区间。

Method: COVER是一个后处理、模型无关的包装器，通过调整时间非一致性分数的分位数，将任何grounding系统（训练过的定位器或黑盒视频-语言模型）转换为输出包含真实时刻的概率至少为1-α的时间区域。

Result: 在三个基准和五个grounding系统中，实现了的覆盖范围跟踪目标，校准揭示了点度量所隐藏的内容。

Conclusion: COVER通过校准时间非一致性分数的分位数，提高了视频时间定位的可靠性。
...
TL;DR: MirrorWorld通过SRD和GTA，有效解决了视频镜面反射生成问题，实现了高质量的反射重建。


<details>
  <summary>Details</summary>
Motivation: 观察镜面反射生成涉及两个互补挑战：确定应该反射的场景内容以及如何将反射内容在镜面区域内进行空间排列。

Method: 提出MirrorWorld，一个反射感知的视频修复框架，在生成过程中建模场景到镜子的关系。具体来说，引入语义关系蒸馏（SRD），将关系信息从冻结的视觉基础模型转移到鼓励可见场景内容与镜面区域之间的语义关联。进一步提出几何变换对齐（GTA），学习一个引导反射内容空间排列的变换。

Result: 实验结果表明，MirrorWorld在反射重建质量方面优于代表性的基于图像的反射生成方法和强大的视频修复基线。

Conclusion: MirrorWorld通过引入SRD和GTA，有效地解决了视频镜面反射生成中的挑战，实现了高质量的反射重建。
...
TL;DR: SimWAM是一种简单而有效的WAM，它通过仅将视频生成用作训练信号来提高端到端自主驾驶。


<details>
  <summary>Details</summary>
Motivation: 世界行动模型（WAMs）通过将视频动力学先验转移到动作预测来提高端到端自主驾驶，但现有方法在推理时需要昂贵的未来生成。

Method: 提出SimWAM，一种简单而有效的WAM，它仅将视频生成用作训练信号。联合训练预训练的视频专家和轻量级动作专家，使用联合流匹配。隔离的注意力掩码使动作预测与未来帧独立，允许在训练后丢弃视频分支，留下一个直接预测轨迹的自包含规划器。由于两个专家没有共享任何参数，并且仅通过统一的注意力接口进行交互，因此可以替换视频主干，独立扩展动作专家，而无需修改学习目标或推理流程。

Result: SimWAM在NAVSIM上达到91.5 PDMS，超过最先进的基于WAM的规划器，具有显著更低的延迟，并实现了零样本到nuScenes的迁移。

Conclusion: SimWAM是一种简单而坚实的基线，可以从中受益于视频生成方面的进步，以实现高效的自主驾驶。
...
TL;DR: TEXAS 通过基于正确性的任务专家发现和标记级监督分配，显著提升了 MoE 语言模型在下游适应中的性能。


<details>
  <summary>Details</summary>
Motivation: Mixture-of-Experts (MoE) 语言模型在下游适应中通过将每个标记路由到一小部分专家，使路由模式有助于识别与任务相关的专家。然而，当前方法有两个局限性：任务专家通常是从反映使用情况而不是与成功完成任务相关的聚合路由统计中识别出来的，并且任务专家的激活仍然未充分探索作为监督分配的信号。

Method: 我们引入了任务专家感知监督 (TEXAS)，它将基于正确性的任务专家发现与标记级监督分配相结合。TEXAS 比较了基模型成功解决和未能解决的实例上的专家激活，并保留了在成功实例上激活更强的专家。在微调期间，当这些专家被激活时，它会对失败实例中的答案标记进行加权。因此，TEXAS 利用现有的路由行为，而不限制适应到固定的专家子集或施加显式的目标路由分布。

Result: 在三个 MoE 模型和六个基准测试中，TEXAS 在 18 个设置中的 17 个中实现了最佳或并列最佳性能，平均比最强的基线提高了 1.3-1.5 分。消融和进一步的分析验证了发现的专家和由此产生的监督策略。

Conclusion: TEXAS 通过结合基于正确性的任务专家发现和标记级监督分配，有效地提高了 MoE 语言模型在下游适应中的性能。
...
TL;DR: 研究人员通过生成对齐的诊断等级分析了语音语言模型在非语言任务上的表现，并发现决策规则和状态到答案读出之间存在性能损失。


<details>
  <summary>Details</summary>
Motivation: 为了评估语音语言模型在非语言任务上的表现，研究人员引入了一种生成对齐的诊断等级，以分析音频到答案计算的不同阶段的失败情况。

Method: 通过比较生成的答案、选项的logits、logits的仿射读出以及相同答案标记处的隐藏状态的线性读出，研究人员分析了不同阶段的失败情况。

Result: 在五个系统和两个情感语料库中，状态解码的平均准确率比生成提高了27.8个百分点，决策规则和读出覆盖差距在所有十种情况下都是正的。无标签的logit校正提高了所有条件下的生成准确率，表明部分决策规则差距是可以操作的。在排名匹配的比较中，除了原始读出之外的情感信息可以推广到保留的说话者，并且可以抵抗对测量的声学描述符的控制，但替换选定的读出外部方向通常对生成的答案影响很小。这些结果表明信息可用性和行为使用之间的区别，并将性能损失定位在决策规则和状态到答案读出之间。

Conclusion: 通过引入生成对齐的诊断等级，研究人员能够更好地理解语音语言模型在非语言任务上的表现，并定位性能损失的原因。
...
TL;DR: 提出了一种新的情感分析方法，通过复杂推理和NTDH技术提高了性能。


<details>
  <summary>Details</summary>
Motivation: 全面情感分析具有挑战性，因为它涉及异构预测任务，并且情感意义具有上下文依赖性，需要协调冲突的线索而不是直接映射到标签。

Method: 将任务重新定义为复杂推理问题，以实现跨异构标签空间的一个输出接口和可验证奖励的轨迹优化。提出NTDH来解决数据方面的四个失败。自然化设置训练答案为黄金标签，因此它是构造上正确的。容差感知门检查每个答案是否与任务的评分范围相符。领域感知策略使用情感科学的概念来细化推理。方向提示只报告错误的类型和方向，而不暴露目标。使用SFT和GRPO训练Qwen3-8B，并量化每个部分的数据质量影响。

Result: 使用16,302条训练记录，比可比的指令调整系统少约14倍，最终策略在六个官方测试指标中的五个上优于其SFT检查点，在比较系统中达到最强的EI-reg结果，Pearson相关系数为0.862。

Conclusion: 提出了一种新的方法来处理情感分析，该方法通过复杂推理和NTDH技术提高了性能。
...
TL;DR: ConstructCIE dataset improves causal information extraction from construction accident reports, but challenges remain in precise span-level extraction and error reduction.


<details>
  <summary>Details</summary>
Motivation: Construction accident narratives contain rich causal information, but the evidence is often implicit, long-span, and distributed.

Method: ConstructCIE, a manually annotated dataset for Causal Information Extraction from OSHA construction accident reports. Hierarchical schema for accident types, causal factors, sub-causal factors, and supporting evidence spans. Supervised sequence taggers and instruction-tuned LLMs evaluated in an end-to-end hierarchical extraction setting.

Result: Most evaluated models achieve strong accident-type prediction and recover broad causal meaning. Limited in precise span-level extraction. JHE stronger in exact and soft matching. IHE higher keyword F1. Error distributions vary by extraction strategy. Evidence-selection and span-boundary errors common.

Conclusion: Reliable Causal Information Extraction for construction accidents requires stronger domain grounding and more accurate evidence extraction.
...
TL;DR: GRASP模型通过自我优化策略提高隐私保护水平，在保护隐私和保持信息效用之间取得了良好的平衡。


<details>
  <summary>Details</summary>
Motivation: 为了解决大型语言模型可以推断敏感个人属性的问题，保护个人隐私。

Method: 引入GRASP模型，通过自我优化策略强化本地匿名化器，同时作为匿名化器、对手和效用评判者。

Result: GRASP模型在保护隐私的同时，保持了较高的信息效用，且在成本和效率方面优于DPO和现有对抗匿名化方法。

Conclusion: GRASP模型是一种有效且高效的隐私保护方法，能够保护用户隐私，同时保持信息的效用。
...
TL;DR: Spherical Soft-Masking (S-SM) enhances MDLM convergence and performance.


<details>
  <summary>Details</summary>
Motivation: Soft-masking accelerates the convergence of Masked Diffusion Language Models (MDLMs).

Method: Introduce Spherical Soft-Masking (S-SM), a drop-in replacement that aggregates the top-(k) predictions with a Fr'echet mean on the hypersphere and blends this mean with the mask direction using spherical linear interpolation (SLERP), then restores the native mask norm.

Result: SLERP feedback avoids the training degradation that LERP feedback induces and delivers MAUVE gains of up to 2x over the vanilla MDLM baseline and 27.5-56.1% over TopK/LERP at various sampling budgets, alongside consistently lower generative perplexity (16.9-19.6% over the baseline), while leaving output entropy and convergence essentially unchanged.

Conclusion: Spherical Soft-Masking (S-SM) improves the performance of MDLMs compared to traditional LERP-based methods.
...
TL;DR: 研究发现，当前FPQA方法在真实场景中存在局限性，需要进一步改进。


<details>
  <summary>Details</summary>
Motivation: 为了评估大型语言模型（LLM）在识别问题中的虚假预设并避免或纠正虚假假设方面的能力，研究者们进行了FPQA测试。

Method: 通过在各个模型家族、大小和基准测试中进行广泛的实验，研究者在FPQA任务中测试了不同方法的表现。

Result: 研究发现，在FPQs上表现较好的方法往往在TPQs上表现较差，这是由于事实检查模块对真实预设也进行了拒绝。

Conclusion: 研究结果揭示了当前FPQA方法的局限性，并提出了未来研究的方向，即开发能够更好地泛化到现实场景的FPQA方法。
...
TL;DR: TradeVerse基准评估LLMs在纵向政治贸易谈判中的表现，实验结果表明当前LLMs存在挑战。


<details>
  <summary>Details</summary>
Motivation: 为了评估LLMs在处理纵向政治贸易谈判方面的潜力，研究者们需要构建一个能够处理多轮讨论和复杂关系的基准。

Method: 研究者们从世界贸易组织（WTO）的特定贸易争端中构建了TradeVerse基准，其中包含成员国的多轮讨论和辩论。

Result: TradeVerse基准包含1170个会议的记录，涵盖了5个小组和89个产品类别，定义了三个任务：分析会议记录并预测产品HS章节代码、猜测响应国家的名称、扮演响应国家并提供最后轮次的声明。

Conclusion: TradeVerse基准是第一个用于研究LLMs在理解纵向政治贸易谈判方面潜力的基准，实验结果表明当前LLMs在这些任务上面临挑战。
...
TL;DR: 本研究探讨了LLM生成的文本具有独特的语言特征，为相关研究提供了新的视角。


<details>
  <summary>Details</summary>
Motivation: 分析LLM生成的文本的特定语言特征

Method: 使用计算描述符和文体学主成分分析

Result: 揭示了2024年和2026年LLM生成文本风格的代际转变，并证明了每个模型都保持了独特的语言特征

Conclusion: 将LLM输出视为具有个人语言特征的变体，为语言变异和变化、LLM生成文本检测、法医语言学和基于使用的语言方法研究提供了有价值的框架。
...
TL;DR: 提出了一种名为因子假设搜索（FHS）的方法，用于解决大分类检索中的检索准备度差距问题，显著提高了检索性能。


<details>
  <summary>Details</summary>
Motivation: 在许多情况下，输入是间接证据，如表格单元格的含义取决于其行、列、数据类型和上下文。

Method: 提出了一种名为因子假设搜索（FHS）的方法，该方法维护多个关于命名语义维度的部分解释。

Result: 在金融分类标签和CodiEsp临床编码任务中，FHS在非或然方法中实现了最佳的Recall@1、MRR和最终准确率。

Conclusion: FHS方法在处理大分类检索时，能够有效地解决检索准备度差距问题，提高了检索的准确性和效率。
...
TL;DR: 通过分析语言隐喻，揭示播客中左右翼观点的框架差异。


<details>
  <summary>Details</summary>
Motivation: 通过识别引导说话者或作者的隐喻，有助于揭示他们对事件的框架。

Method: 提出一种无监督方法，从语料库中提取语言隐喻，并使用结构化聚类方法形成与概念隐喻相对应的组。

Result: 指出左翼和右翼播客在主题和框架方面的关键差异。例如，左翼播客倾向于将媒体故事概念化为武器，而右翼来源通常讨论经济作为一个受垂直变化影响的系统。

Conclusion: 该研究通过分析语言隐喻，揭示了播客中左右翼观点的框架差异。
...
TL;DR: 研究分析了长期任务中的视野差距问题，发现需要更多的步骤级别信号和诊断方法。


<details>
  <summary>Details</summary>
Motivation: 研究前沿语言模型在解决推理问题时存在的长期任务问题，即‘视野差距’。

Method: 通过系统性地收集和审查arXiv论文（2024-2026年），并分析长期任务的生命周期，包括规划、记忆、执行、训练、评估和基础/安全。

Result: 发现随着视野的延长，仅关注结果的信号变得无信息量，而该领域的响应措施——无论是过程奖励模型、信用分配还是轨迹级别的诊断——都制造了更密集的步骤级别的信号。

Conclusion: 论文强调了在处理长期任务时，需要更多的步骤级别信号和诊断方法，以及如何将批评与方法分离，以促进研究进步。
...
TL;DR: Proposes Tone-Aware RAG to improve communication alignment in RAG systems, emphasizing the importance of tone awareness in socially sensitive contexts.


<details>
  <summary>Details</summary>
Motivation: Standard RAG systems exhibit a structural limitation: retrieved documents carry their own communication styles that shape the behavior of the system before any tone instructions are processed.

Method: Proposed Tone-Aware RAG (TA-RAG), a conceptual architectural framework that positions communicative alignment alongside factual accuracy as a core design objective. TA-RAG operationalises four constraints across the retrieval, context construction, generation, and constraint validation phases.

Result: TA-RAG aims to address the gap in current RAG systems by incorporating tone awareness and communicative alignment, resulting in more effective and socially sensitive responses.

Conclusion: Tone awareness should be treated as a present design imperative for RAG systems operating in socially sensitive and high-stakes contexts.
...
TL;DR: 本研究提出了一种新的基于上下文的bandit算法，有效解决了大语言模型迭代优化过程中过度利用的问题，显著提升了模型性能。


<details>
  <summary>Details</summary>
Motivation: 为了解决现有方法中存在的过度利用问题，例如在自反馈自优化和传统的bandit方法中，往往依赖于静态选项或忽视了饱和效应，这会导致收益的逐渐减少。

Method: 提出了一种新的基于上下文的bandit算法，该算法明确地结合了奖励衰减建模。利用期望最大化（EM）算法，该方法同时估计臂特定的和衰减参数。此外，通过将提示嵌入为臂，我们促进了臂价值的学习，使我们的方法与传统的不连接的线性置信上界（LinUCB）框架不同。

Result: 在情感反转和GSM8K基准测试中，实验结果表明，与强大的基线相比，我们的方法实现了显著的性能提升。消融研究证实，在bandit框架内结合奖励衰减建模对于减轻过度利用和优化迭代优化过程至关重要。

Conclusion: 本研究提出了一种基于上下文的bandit算法，有效地解决了大语言模型迭代优化过程中过度利用的问题，提高了模型的性能。
...
TL;DR: Stockmark-Nemotron-3-Nano-Omni-JapanDocReader模型通过能力注入和遗忘控制实现结构化文档解析，实验表明混合SFT和基于DAPO的解析中心RL进一步提升了性能。


<details>
  <summary>Details</summary>
Motivation: structured document parsing via capability injection and forgetting control

Method: parsing-centric SFT, mixed SFT, parsing-centric RL

Result: improved structured document parsing performance

Conclusion: DAPO-based parsing-centric RL further improves structured document parsing
...
TL;DR: MTI-GNN在认知扭曲检测中表现出色，优于现有方法，并具有较好的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 认知扭曲检测是计算心理健康中的关键任务，但现有方法往往忽视了扭曲思维的心理结构。

Method: 提出MTI-GNN（多视角三元相互作用图神经网络），将Beck的认知三元组——自我、世界和未来的负面看法——作为分类的补充视角进行建模。LLM将每个句子分解为三个视角，从这些视角构建特定于视角的相似图，并由多视角GNN进行编码。三元相互作用模块通过顺序源条件更新和特征门控来建模跨视角依赖，而原型引导的视角融合通过标签条件聚合进行。标签扩展的监督在训练过程中结合了所有可用的扭曲注释。

Result: 在来自四个韩国、英语和中文数据集的9,764个样本上评估MTI-GNN，涵盖了十个扭曲类别。MTI-GNN在所有监督变体中表现优异，在零样本和少样本设置下超过了八个提示生成模型。删除一个视角的消融实验表明，所有三个视角都做出了显著贡献，而人类专家评估提供了初步证据，表明它们与预期的认知维度一致。

Conclusion: MTI-GNN在认知扭曲检测任务中表现出色，优于现有方法，并具有较好的泛化能力。
...
TL;DR: 本文提出了一种改进的OPD预热方法，提高了OPD的性能。


<details>
  <summary>Details</summary>
Motivation: 分析在策略蒸馏（OPD）中预热阶段的有效性及其影响因素。

Method: 从数据和训练角度分析OPD的预热阶段，提出Simple-OPD方法。

Result: Simple-OPD通过教师生成的CoT和LoRA预热学生，提高了OPD的有效性和鲁棒性。

Conclusion: 预热对于OPD的有效性至关重要，Simple-OPD方法有效提高了OPD的性能。
...
TL;DR: FutureBridge通过考虑SLM的后续推理能力，显著提高了SLM在数学推理任务中的性能。


<details>
  <summary>Details</summary>
Motivation: 为了提高小语言模型（SLM）在预测分歧时的性能，研究人员提出了Token-level collaboration方法。

Method: FutureBridge通过评估LLM-SLM联合候选词对SLM后续推理的支持程度来进行排序。在训练过程中，使用答案验证的LLM轨迹提供固定的共享未来，冻结的SLM在共同语境下评估每个候选词。在推理过程中，LLM仅用于扩展候选池，选择一个词，并将生成任务返回给SLM。

Result: 在五个数学推理基准测试中，FutureBridge将Qwen3-1.7B SLM的Math Avg.相对于贪婪SLM解码提高了35.1%。

Conclusion: 研究表明，在Token选择中，考虑SLM是否可以使用每个候选词继续推理比仅依赖LLM的局部偏好更有益。
...
TL;DR: 提出了一种新的无数据方法Autonomy-of-Heads（AoH），用于优化长上下文LLM推理，显著提高性能和效率。


<details>
  <summary>Details</summary>
Motivation: Long-context LLM inference受二次注意力计算和增长的KV缓存成本限制。现有的稀疏注意力和KV压缩方法通常从运行时注意力分数、观察窗口、校准提示或学习门中决定保留哪些标记或头，使得头诊断输入依赖性强且部署成本高。

Method: 提出了一种名为Autonomy-of-Heads（AoH）的无数据方法，通过查询键投影的谱几何来识别检索和流头。AoH定义了核注意力算子$M_h = W_K^{h	op}W_Q^h$，并使用其有效秩作为头函数的权重空间度量：集中的谱表示少数几个主导的查询键匹配方向，与检索头相关联，而分散的谱表示不存在主导的全局匹配方向，与流头相关联。进一步推导出高效的$d_	ext{head}$-维计算，避免了构建完整的$d_	ext{model}	imes d_	ext{model}$矩阵。

Result: 在50%的稀疏性下，AoH在平均上保留了Full Attention性能的96.5%，同时将预填充和解码延迟分别减少了高达41.4%和66.0%，将KV缓存内存减少了50.0%（在256K个标记时）。

Conclusion: AoH是一种有效的方法，可以显著提高长上下文LLM推理的性能和效率。
...
TL;DR: 提出了一种统一的方法来优化LLM路由，并证明学习路由器在成本效益和个性化方面具有优势。


<details>
  <summary>Details</summary>
Motivation: 为了实现成本效益高的部署，模型路由变得至关重要。

Method: 提出一个统一的路由公式，将LLM路由作为一个具有五个组件的顺序决策过程：上下文编码器、模型编码器、评分函数、决策规则和学习信号。

Result: 开发了一个自动化的管道来构建路由监督并联合评估路由器在响应质量和推理成本方面的性能。

Conclusion: 研究表明，学习路由器相对于最强的固定模型基线提高了14.6%，轻量级路由器在严格成本约束下更具竞争力，并且基于用户条件的路由持续提高个性化。
...
TL;DR: 本研究通过分析数字化标本记录，识别并参考了非地名录地名（NGP），发现概率性推理在地理参考方面最为准确。


<details>
  <summary>Details</summary>
Motivation: 生物标本记录为历史地理知识提供了宝贵资源，本研究旨在识别未被当前地名录收录的地名，即非地名录地名（NGP），并探讨其地理参考问题。

Method: 通过分析新西兰阿拉恩特草本馆的数字化数据，使用重复出现同一地名的标本记录中的不同位置和空间关系术语，提取并反转这些关系，以确定NGP的位置。研究采用确定性、概率性和基于大型语言模型（LLM）的方法，进行比较分析。

Result: 概率性推理在伪NGP基准测试中取得了最高的准确率（中位数误差1.43公里；A@1公里36%），而基于LLM的方法给出了具有竞争力但不够精确的估计（中位数误差1.80公里；A@1公里31%），表明尽管LLM取得了进步，但在需要高空间精度的情况下，传统建模仍然具有优势。

Conclusion: 非地名录地名（NGP）的地理参考问题可以通过多种方法解决，其中概率性推理在提高空间精度方面表现最佳。
...
TL;DR: 本研究通过ZCA白化改善了WEAT的可靠性，为AI公平研究提供了新的见解。


<details>
  <summary>Details</summary>
Motivation: 为了提高Word Embedding Association Test（WEAT）的可靠性，该研究提出了Zero-phase Component Analysis（ZCA）白化作为几何预处理的步骤。

Method: 将ZCA白化应用于嵌入空间，以恢复其各向同性，从而提高WEAT的可靠性。

Result: ZCA白化显著降低了嵌入空间的所有模型的各向异性，提高了WEAT结果的可靠性。

Conclusion: 该研究为计算社会科学和AI公平研究中的WEAT提供了可靠性的改进，并建议在非各向同性的嵌入空间中对偏差测量结果持谨慎态度。
...
TL;DR: 这项研究探讨了LLM对提示框架的敏感性，发现它们在事实背景下也会根据提示框架调整其响应。


<details>
  <summary>Details</summary>
Motivation: 研究LLM对提示框架的敏感性以及强化用户在提示中表达的偏见程度。

Method: 使用160个不同主题的提示评估6个LLM，包括意见和事实领域。

Result: LLM会根据提示框架调整其响应，即使在事实背景下。

Conclusion: LLM的可操纵性和边界，以及它们强化用户偏见和易受明确提示操纵的能力。
...
TL;DR: GPTKB 2.0 是一个大规模去歧义知识库的演示平台。


<details>
  <summary>Details</summary>
Motivation: 为了解决现有基于LLM的知识库实体歧义问题。

Method: 通过上下文引导的消歧方法构建知识库，支持结构化查询和实体链接。

Result: GPTKB 2.0 包含大量去歧义的三元组、关系和类别，并支持多种查询和交互功能。

Conclusion: GPTKB 2.0 提供了一个可交互的演示平台，用于探索大规模去歧义知识库。
...
TL;DR: 通过选择性证据准入，ECF提高了视觉DLM-RAG的性能。


<details>
  <summary>Details</summary>
Motivation: 为了提高视觉检索增强生成（RAG）的性能，通常假设应将所有可用的证据传递给生成器。

Method: 提出了熵基候选过滤器（ECF）来限制有害的视觉暴露，同时保留检索覆盖率。ECF构建了多粒度证据单元，并使用空白控制的块置信度和检索排名来确定哪个候选者应该进入最终上下文。

Result: 在三个多模态DLM和五个视觉QA基准上，ECF的平均答案准确率比最强的固定top-k输入提高了2.62个百分点，在LLaDA2.0-Uni上，比每个数据集的最佳无训练结果平均提高了2.37个百分点。

Conclusion: 更广泛的检索通过选择性证据准入而不是无条件证据扩展来提高视觉DLM-RAG的性能。
...
TL;DR: A hybrid knowledge graph approach improves HR platform skill matching.


<details>
  <summary>Details</summary>
Motivation: Human Resources (HR) platforms face challenges in organizing unstandardized, multilingual expertise declarations, impacting talent matching.

Method: Hybrid knowledge graph generation pipeline, combining Large Language Model (LLM) and Wikidata multilingual Knowledge Graph (KG), with agentic reflexion pattern for synthesizing concepts and metadata.

Result: Scalable, explicable, and self-healing framework for generating a comprehensive skills knowledge graph from unstructured text.

Conclusion: Improved talent matching and skill organization in HR platforms.
...
TL;DR: HNR-DAC框架通过两阶段训练策略，在科学主张验证任务中取得了显著成果。


<details>
  <summary>Details</summary>
Motivation: 验证科学论文中的科学主张，需要预测主张-论文关系并识别支持该预测的段落。

Method: 提出了一种名为HNR-DAC的两阶段框架，该框架在它将实际遇到的案例上进行训练。HNR-DAC包括Hard-Negative Reranking（HNR）和Distribution-Aligned Classification（DAC）两个阶段。HNR使用基重排器的分数来量化证据的混淆性，并将金牌证据与最易混淆的候选人进行对比。DAC在由冻结的HNR构建的推理输入产生的Top-1段落上进行训练，而HNR的Top-3段落标识符提供证据输出。

Result: 在NLPCC 2026任务10的第2条赛道上，最终配置实现了97.21%的Hit@3，95.79%的Macro-F1，94.47%的Joint@3，平均得分为95.13%。相应的提交在官方第2条赛道上排名第三，同时实现了最高的总体Macro-F1（93.05%），以及70.16%的Joint@3和平均得分81.61%。

Conclusion: HNR-DAC框架在科学主张验证任务中表现出色，有效提高了预测准确率。
...
TL;DR: 通过监测LLMs的激活，可以有效地测量文本中的概念内容，无需特定任务微调。


<details>
  <summary>Details</summary>
Motivation: 分析文本中概念内容的不同方法，以及大型语言模型（LLMs）内部知识与其在响应中表达之间的差距。

Method: 使用递归特征机（RFM）算法和线性探测来提取内部知识，并与嵌入基线、表面基线和模型自身答案进行比较。

Result: 线性探测在无需特定任务微调的情况下，其准确率与微调的领域分类器相当，并且在十二次比较中有十一次优于模型自身答案。

Conclusion: LLMs的激活可以反映文本中的概念内容，而无需特定任务微调。
...
TL;DR: 通过控制实验，我们证明了存储已验证的修正查询可以提高文本到SQL查询的准确率。


<details>
  <summary>Details</summary>
Motivation: 为了解决测试时缩放技术纠正困难的文本到SQL查询的问题，但是额外的计算通常在每次答案后丢弃。系统越来越多地保留已验证的修复片段，但评估仍然报告一个端到端的分数。这不能区分重复问题的重放和未见问题的帮助，或识别负责的内存选择。

Method: 我们通过控制实验，保持单次求解器固定，每次只改变一个内存选择。我们分别测量重放、跨问题保留和保持同一数据库的迁移。

Result: 在BIRD数据集上，存储已验证的修正查询将保持首次尝试的准确率提高了4.34个百分点。这种收益捕捉了同一问题按需修复提供的44.4%的准确率空间。控制干预确定了数据库特定内容为主要操作成分。可靠的验证和更广泛的检索覆盖产生了支持性收益；更丰富的格式和更复杂的检索器没有。

Conclusion: 测量这种未来价值称为结晶问题。通过控制实验，我们证明了存储已验证的修正查询可以提高文本到SQL查询的准确率。
...
TL;DR: Stoicheia 是一种基于掩码扩散的字符级编码器，在古希腊文本处理任务中表现出色。


<details>
  <summary>Details</summary>
Motivation: 为了处理古希腊文本的缺失、重新分段、强调和标点等问题，

Method: 提出了一种名为 Stoicheia 的 405M 参数字符级掩码扩散编码器，该编码器具有五个独立的掩码平面：字母、单词和句子边界、变音符号、大写和小写以及标点符号。

Result: 在三个实验中，Stoicheia 在损坏的铭文和纸草文献重建、形态句法标注和依存句法分析以及宏音化与韵律扫描方面均取得了显著的性能提升。在 Ithaca 测试集上，Stoicheia 的字符错误率相对于之前的最佳系统降低了 9.1 和 8.0 个百分点，top-1 准确率提高了 10.5 个百分点。

Conclusion: Stoicheia 是一种有效的古希腊文本处理工具，在多个任务上均表现出色。
...
TL;DR: 研究发现，模型在简单问题上的失败是由于第二跳不遵循训练分布，提出了一种新的训练策略来改善这种情况。


<details>
  <summary>Details</summary>
Motivation: 理解大型语言模型在解决复杂问题上的成功与在简单问题上的失败之间的内部机制。

Method: 在控制符号环境中从头开始训练Transformer模型，进行机制分析。

Result: 发现模型在第二跳遵循训练分布时可靠地泛化，但在分布之外时总是失败；提出了一种循环式训练策略，显著提高了在分布之外的两跳查询上的泛化能力。

Conclusion: 解释了模型在不同泛化行为中的表现差异，并提出了一种新的训练策略来提高泛化能力。
...
TL;DR: 本文提出的方法成功复现了经典视觉世界研究的预测处理结果，为神经语言模型在人类语言处理中的应用提供了新思路。


<details>
  <summary>Details</summary>
Motivation: 探索神经语言模型在人类语言处理中的应用潜力。

Method: 结合CLIP家族的多模态双编码器模型和双模态归因方法，预测视觉世界研究中的注视行为。

Result: 成功复现了经典英语视觉世界研究的预测处理结果，且无需生成架构和微调。

Conclusion: 该研究证明神经语言模型在模拟人类语言处理方面具有潜力。
...
TL;DR: 研究探讨了使用LLM在语法工程中的潜力和局限性。


<details>
  <summary>Details</summary>
Motivation: 在平行语法项目中，平衡跨语言一致性和特定语言属性需要语言形式主义和计算实现的专长。

Method: 在ParGram项目中开发粤语和爱尔兰语树库，使用OpenAI的gpt-oss-120b模型进行翻译和句法结构生成。

Result: 翻译性能总体不满意，句法结构生成在需要跨语言抽象的任务上表现不佳。LLM生成的输出在建议替代分析和部分捕捉谓语-论元关系方面仍具有一定的参考价值。

Conclusion: LLM在协作语法工程中具有潜力和局限性，专家驱动的分析和验证仍然非常重要。
...
TL;DR: NLP心理测量学通过LLM和RF模型，揭示了心理健康预测的潜力与限制，并使这些区别明确、可测量和可测试。


<details>
  <summary>Details</summary>
Motivation: 分析自然语言处理（NLP）模型在预测心理健康结果时的测量问题，并提出NLP心理测量学方法。

Method: 使用九个大型语言模型（LLM），通过控制角色（认知数字阴影）完成心理测量问卷，并提取情感轮廓和句法语义结构。结合人格和社会人口统计变量，使用随机森林（RF）回归器和SHAP技术分析特征对性能的影响。

Result: RF模型解释了生活满意度（SWLS）70.8%的方差，抑郁（PHQ-9）55.7%，DASS-21抑郁68.5%，焦虑76.0%，压力72.4%的方差。社会人口统计变量对抑郁、焦虑或压力没有显著解释力，但对生活满意度有显著影响。RF模型可以区分日记和角色，使用网络/情感特征对临床参与者进行分类，准确率达到68%。

Conclusion: NLP心理测量学通过可解释的AI和网络/情感特征，使这些区别明确、可测量和可测试。
...
TL;DR: SA-PPG和RailCap是更准确的污染缓解方法。


<details>
  <summary>Details</summary>
Motivation: 现有的污染缓解评估方法存在缺陷，如G-AP指标的问题。

Method: SA-PPG通过分层聚合每个问题的概率差距来评估模型，而RailCap在生成过程中判断污染。

Result: SA-PPG揭示了先前策略的恢复效果被高估，而RailCap在多个污染模型和基准测试中实现了最低的SA-PPG。

Conclusion: SA-PPG和RailCap是有效的污染缓解方法，可以更准确地评估模型的性能。
...
TL;DR: 本研究评估了LLMs的概念理解，发现其存在局限性，并为改进概念型LLMs提供了见解。


<details>
  <summary>Details</summary>
Motivation: 为了支持对LLMs中概念的受控探测，我们设计了一组测试，针对其核心属性：抽象性、组合性和基础性。

Method: 创建了一个以概念为中心的基准，针对空间概念（如方向、距离、拓扑及其组合）和问答任务。

Result: 结果显示了当前LLMs在概念理解方面的明显局限性，并提供了关于塑造其获取和组合结构化概念能力的因素的见解。

Conclusion: 我们的发现有助于设计基于概念的新型LLMs，以改善信息访问和知识管理。
...
TL;DR: LitTraceQA是一个评估科学研究问答系统生成可验证答案能力的基准。


<details>
  <summary>Details</summary>
Motivation: 科学文献作为知识来源被越来越广泛地应用于语言模型、检索增强生成系统和研究助理中，但回答研究问题需要不仅仅是流畅的生成。一个可靠系统必须能够识别相关论文、定位支持答案的具体证据，并生成忠实于该证据的回应。

Method: 提出了LitTraceQA，一个针对文献基础的问答基准。该系统需要从研究问题和论文元数据集中返回三个相互关联的输出：标准论文标识符、支持证据位置和答案（包括自由文本、多项选择题和结构化表格）。LitTraceQA针对科学阅读中常见的证据类型：表格、图表、文本片段、方程式或算法和引用上下文。

Result: LitTraceQA提供了55个示例，包括26个隐藏来源的单篇论文问题和29个多篇论文问题，并提供了黄金论文、证据注释和答案进行本地验证。此外，还对包含4988个独特问题记录和4859篇独特黄金论文的更大注释集合进行了分析。通过分别评估论文检索、证据定位和答案准确性，LitTraceQA为能够生成可验证答案的科学研究问答系统提供了一个测试平台。

Conclusion: LitTraceQA是一个针对文献基础的问答基准，旨在评估科学研究问答系统生成可验证答案的能力。
...
TL;DR: CoinRAG improves RAG efficiency and accuracy through fine-grained cache reuse and semantic unit identification.


<details>
  <summary>Details</summary>
Motivation: Recent optimization studies on Retrieval-Augmented Generation (RAG) have exploited chunk-level KV cache reuse to avoid processing long retrieved contexts for higher efficiency, while significant information redundancy and noise still remain in the coarse-grained chunks.

Method: Proposes CoinRAG (Contextualized Information Nugget KV Cache Reuse for Long-Context RAG) to optimize the Pareto frontier under low prefill latency constraints while maximizing accuracy. CoinRAG reuses offline-computed, fine-grained nugget caches to form a learned contextual representation efficiently in a more semantically relevant but compact manner. It identifies query-relevant semantic units within retrieved chunks through two-stage retrieval and assembles their sliced KV representations with a chunk-level context.

Result: Extensive evaluations on LongBench multi-hop question answering tasks demonstrate that CoinRAG significantly reduces operational costs and outperforms the other baselines with a new Pareto frontier and an average 5.3% relative improvement in answer quality (F1) under a standard fast prefill latency budget.

Conclusion: CoinRAG effectively optimizes the efficiency and accuracy of RAG, providing a new Pareto frontier and improved answer quality.
...
TL;DR: CreativeInstruct通过注入特殊跨度提高LLMs的创意生成能力，并在叙事生成和强化学习任务中表现出色。


<details>
  <summary>Details</summary>
Motivation: 提高大型语言模型（LLMs）在生成创意内容方面的能力，同时保持其输出质量和多样性。

Method: 提出CreativeInstruct，一种可扩展的指令微调方法，通过学习注入特殊[StartCreativity]跨度来引导模型生成更具创造性的内容。同时，引入基于图编辑距离的结构多样性指标，以捕捉纯词汇和语义指标未能捕捉到的叙事层面变化。

Result: CreativeInstruct在叙事生成任务中，与多模型基线和蒸馏变体相比，在保持质量的同时，实现了多样性。人类评估显示，70.3%的情况下，标注者认为CreativeInstruct的生成内容比经过后训练的LLMs更具创意。在强化学习任务中，应用CreativeInstruct的GRPO模型在AMC和MATH上的表现分别提高了约4%和5%。

Conclusion: CreativeInstruct是一种有效的提高LLMs创意生成能力的方法，对创意任务和强化学习任务具有显著优势。
...
<details>
  <summary>Details</summary>
Motivation: 为了减轻LLM评估器的评分偏差

Method: 提出了一种新的方法，通过让LLM随机生成数字标记来识别其潜在数值偏差，并在随机数字生成提示中添加下游任务定义来测量特定任务的潜在数值偏差。在LLM评估中，考虑LLM的潜在数值偏差来校正给定输入的标记生成概率。

Result: 实验结果表明，该方法优于基线，包括未进行偏差校正的LLM和以前的校准方法。此外，评分偏差在LLMs、任务和评分范围之间有所变化，表明测量潜在数值偏差的重要性。

Conclusion: 提出的方法有效地减轻了LLM评估器的评分偏差，并提高了评估的准确性。
...
### [126] [WorldMark: A Plug-and-Play World Knowledge Interface for Cross-Host Language Model Watermarking](https://arxiv.org/abs/2608.06416)
*Song Xiao,Yuqi Yuan,Yanshuo Zhang,Kejun Zhang*

Main category: cs.CR

TL;DR: WorldMark：一种利用世界知识记忆（WKM）的水印技术，用于追踪大型语言模型文本来源，无需额外训练和参数。


<details>
  <summary>Details</summary>
Motivation: 为了追踪大型语言模型产生的文本的来源，研究人员在解码过程中嵌入可检测的统计信号，但现有的方案都存在局限性。

Method: 提出WorldMark，一个使用World Knowledge Memory (WKM)组织语义和情节知识的接口，通过不对称知识调制（AKM）调整水印强度。

Result: WorldMark不需要重新训练骨干模型，不引入额外的检测器模型或参数。在C4评估中，WorldMark提高了清洁和攻击检测的准确性，同时略微降低了困惑度。

Conclusion: WorldMark是一种有效的水印技术，可以追踪大型语言模型文本的来源，同时保持较低的困惑度。
...
### [127] [Fairis: Fairness-Aware Aggregation with Provable Influence Containment against Fairness Poisoning Attacks in Collaborative Machine Learning](https://arxiv.org/abs/2608.06469)
*Devharsh Trivedi,Nesrine Kaaniche,Nikos Triandopoulos,Maryline Laurent,Jackson Walters*

Main category: cs.CR

TL;DR: Fairis是一种服务器端重新加权方案，旨在提高金融机构之间协作机器学习的公平性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 为了确保金融机构之间协作的机器学习既公平又具有鲁棒性，以抵御故意敌对操纵。

Method: 提出了一种名为Fairis的服务器端重新加权方案，该方案使用归一化权重，并证明了三个性质：单调权重减少（MWR）、人口参与和非欺诈性。

Result: Fairis能够确保每个客户端具有严格正权重，并证明了一个敌对者在其偏差中的权重单调递减；剪裁的FairFed可以达到更低的权重，但对任何保证都没有，并且在台湾信用上直接将客户端权重置零。

Conclusion: Fairis在对抗足够隐秘的敌对者时，能够将权重降低41%至54%，而不会对良性模型造成0.04的精度损失。
...
### [128] [CyberForge: Verified Vulnerability Injection at Repository Level for Cybersecurity Agent Training](https://arxiv.org/abs/2608.06471)
*Amine Lbath,Manan Suri,Aurelien Delaitre,Vadim Okun,Massih-Reza Amini,Ram D. Sriram,Dinesh Manocha*

Main category: cs.CR

TL;DR: CyberForge框架通过注入漏洞生成安全训练数据，显著提升LLM在软件漏洞发现和修补方面的能力


<details>
  <summary>Details</summary>
Motivation: 解决现实软件中复杂漏洞发现和修补的局限性

Method: 提出CyberForge框架，通过注入漏洞到真实C/C++项目中生成可执行的安全训练数据

Result: 生成的语料库包含80个项目和63个弱点类别的1034个验证漏洞，在所有六种配置中，SEC-bench补丁修复性能提高3.3到14.7点

Conclusion: CyberForge框架能够有效提高安全训练数据的质量和数量，从而提升LLM在软件漏洞发现和修补方面的能力
...
### [129] [StepJack: Benchmarking Computer-Use Agent Safety Against Multi-Step Indirect Prompt Injection](https://arxiv.org/abs/2608.06477)
*Zhuoxin Zhan,Akbar Rafiey,Avery Ma,Leila Pishdad,Layla El Asri*

Main category: cs.CR

TL;DR: 新的多步间接提示注入攻击对CUA构成威胁，可显著提高攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 计算机使用代理（CUA）面临来自间接提示注入的日益增长的威胁。

Method: 开发了一种新的攻击类——多步间接提示注入，并建立了一个名为StepJack的CUA安全基准。

Result: 多步攻击在六个CUA中的三个上提高了攻击成功率（ASR），最高达到31.2点。

Conclusion: 多步间接提示注入对CUA构成了严重威胁，需要采取安全措施。
...
### [130] [Canonicalization Failures as a Recurring Vulnerability Class: Representation Divergence in Cryptographic Systems and Its Avoidance](https://arxiv.org/abs/2608.06508)
*Arslan Brömme*
...
### [131] [Model Confidence Under Answer-Preserving Attacks: An Informativeness-Manipulability Frontier](https://arxiv.org/abs/2608.06571)
*Reza Khanmohammadi,Ivan Brugere,Simerjot Kaur,Charese H. Smiley,Kundan Thind,Mohammad M. Ghassemi*

Main category: cs.CR

TL;DR: 攻击可以破坏置信度的一致性，导致错误答案被接受。


<details>
  <summary>Details</summary>
Motivation: 研究视觉语言系统在白盒、仅图像攻击下的置信度读出。

Method: 在四个视觉语言模型、三个视觉问答基准、五个部署的置信度通道和两个防御估计器上进行实验。

Result: 攻击可以破坏置信度的一致性，导致错误答案被接受。

Conclusion: 置信度不是一个内在稳健的监督信号，而是一个敏感的完整性信号。
...
### [132] [WhiteNet: Robust Identification of Overlapping IEEE 802.11 Signals Across Unseen Channels](https://arxiv.org/abs/2608.06581)
*Ildi Alla,Vincent Lenders*

Main category: cs.CR

TL;DR: WhiteNet框架通过频谱白化和合成重叠混合器，提高了IEEE 802.11协议识别的鲁棒性和效率


<details>
  <summary>Details</summary>
Motivation: Deep learning (DL) classifiers在处理IEEE 802.11协议重叠信号识别时，在信道条件变化时性能下降

Method: 提出WhiteNet框架，使用频谱白化预处理步骤，结合合成重叠混合器进行预训练

Result: 在公共空中IEEE 802.11数据上，WhiteNet缩小了由未见信道条件引起的精度差距，参数数量比先前技术少7.7倍，并可选地提炼为紧凑版本，用于功率受限的边缘设备上的粗略频谱感知

Conclusion: WhiteNet框架有效解决了信道可变性带来的问题，提高了识别精度和效率
...
### [133] [From Documentation to Zero-day Vulnerabilities: LLM-Driven Fuzzing of JavaScript Engines in PDF Readers](https://arxiv.org/abs/2608.06641)
*Suyue Guo,Stijn Pletinckx,Tianle Yu,Yigitcan Kaya,Saad Ullah,Wenbo Guo,Christopher Kruegel,Giovanni Vigna*

Main category: cs.CR

TL;DR: PDFuzzer通过使用LLM和约束求解器，显著提高了PDF阅读器模糊测试的覆盖率，并发现了多个零日漏洞。


<details>
  <summary>Details</summary>
Motivation: 现有PDF阅读器模糊器依赖于简单的测试用例，这些测试用例仅涉及单个API调用，导致覆盖率有限，可能遗漏需要连续API调用的漏洞。

Method: PDFuzzer，一种新的PDF引擎模糊器，可自动生成复杂且有意义的API调用序列。使用LLM构建上下文无关文法，并从JavaScript API手册和执行跟踪中提取的规范中推断API调用之间的关系。基于文法和关系，PDFuzzer使用约束求解器生成具体的API调用序列进行模糊测试。

Result: 在三个主流PDF阅读器（Adobe Acrobat Reader，Foxit PDF Reader和PDF-XChange Editor）上，PDFuzzer在覆盖率方面显著优于现有的PDF模糊器（TypeOracle，Favocado和Cooper）和基于LLM的模糊器（Fuzz4All，naive LLM）。PDFuzzer的覆盖率比现有工具高出48%，并在这三个阅读器中发现了31个零日漏洞，从信息泄露到任意代码执行。

Conclusion: PDFuzzer在提高PDF阅读器模糊测试的覆盖率和发现漏洞方面是有效的。
...
### [134] [CyberLLM: A Multi-Agent LLM Framework for Autonomous Detection and Guarded Response in Automotive Cybersecurity](https://arxiv.org/abs/2608.06651)
*Nenad Petrovic,Oussama Jeddou,Feres Ben Fraj,Vahid Zolfaghari,Fengjunjie Pan,Andre Schamschurko,Alois Knoll*

Main category: cs.CR

TL;DR: CyberLLM框架在运行时安全防护下自动检测和修复软件定义汽车中的漏洞，实验结果表明其在保证安全性的同时，具有高精度和高召回率。


<details>
  <summary>Details</summary>
Motivation: 软件定义汽车（SDV）扩大了汽车攻击面，同时安全约束禁止自主代理在没有监督的情况下采取行动。

Method: 提出了一种名为CyberLLM的多代理、LLM指挥的框架，该框架在形式化、运行时安全防护下自动检测漏洞并执行修复。检测结合了确定性层（正则表达式规则、AST分析器和拓扑图检查）与LLM细化传递，以高召回率为基础，辅以高精度推理。决策代理聚合发现结果，使用以人为中心的资产分类法进行标记，并选择分层响应，通过带签名的跨会话记忆和重新规划反馈提高其信心。每个动作在允许运行之前都要与四个上下文安全属性和一个独立的行为一致性预言机进行验证，拒绝的动作将触发升级和重新规划。对称攻击管道生成并回放漏洞利用，以便双方可以在相同场景下进行锻炼。

Result: 在独立、真实标记的九个汽车ECU模块（C、C++和Rust）的基准测试中，始终开启的确定性层在完美精度下覆盖了34%的标记漏洞，添加基于事实的LLM细化和完整性传递大约将覆盖率加倍到约70%（F1 $0.83$），同时在干净控制中产生零误报。结果表明，LLM代理可以在确定性、可审计的安全外壳中执行有用的自主网络安全防御。

Conclusion: LLM代理在确定性、可审计的安全外壳中可以执行有用的自主网络安全防御。
...
### [135] [Policy-Masked Private Experts: Auditable and Reversible Capability Access Control in Sparse MoE Models](https://arxiv.org/abs/2608.06690)
*Zhuoheng Huang,Mukesh Singh*

Main category: cs.CR

TL;DR: 信任授权可以决定新参数的可访问性，模型在不同场景下表现出显著的性能提升。


<details>
  <summary>Details</summary>
Motivation: 研究信任授权能否确定哪些新训练的参数可以被前向传递访问。

Method: Policy-Masked Private Experts模型，冻结预训练的稀疏混合专家(MoE)模型，训练一个不重叠的专家分支，在top-k路由之前选择公共或私有池。

Result: 未经授权的请求不执行任何私有专家。在两个冻结的Qwen基准测试中，私有分支提高了精确工具使用的百分比。DeepSeek再现了路由不变性并取得了27.0百分点的提升。

Conclusion: 这些结果表明，可以对训练参数路径进行可审计、可逆的控制，同时显示出有用的迁移仍然与分布相关。
...
### [136] [Tight Security for BBS Signatures](https://arxiv.org/abs/2608.06724)
*Rutchathon Chairattana-Apirom,Dennis Hofheinz,Stefano Tessaro*

Main category: cs.CR

TL;DR: 该研究证明了BBS签名方案在单次签名场景下的安全性，并揭示了其在允许生成多个签名的场景下的安全性局限。


<details>
  <summary>Details</summary>
Motivation: 研究BBS签名方案的安全性和效率，以促进其在实际隐私保护认证系统中的应用和标准化。

Method: 采用元化降阶论证方法，结合密码分析和模型理论，对BBS签名方案进行安全性分析。

Result: 证明在单次签名的场景下，BBS签名方案的紧化降阶到q-SDH假设是可能的；同时，证明了允许对同一消息生成多个签名时，无法将BBS签名方案紧化降阶到q-SDH（及其变体）。

Conclusion: BBS签名方案在单次签名场景下具有紧化降阶到q-SDH假设的安全性，但在允许对同一消息生成多个签名的场景下，无法实现紧化降阶到q-SDH假设的安全性。
...
### [137] [Retrieval-Constrained Policy Optimization for Attack Technique Extraction from Cyber Threat Intelligence](https://arxiv.org/abs/2608.06778)
*Jiayun Zhang,Junshen Xu,Zejun Xie,Yi Fan*

Main category: cs.CR

TL;DR: TTP-R1 is a new framework for mapping CTI text to MITRE ATT&CK techniques, achieving better performance than existing methods.


<details>
  <summary>Details</summary>
Motivation: Mapping cyber threat intelligence (CTI) text to MITRE ATT&CK techniques is essential for structured threat analysis, yet manual annotation is costly and does not scale.

Method: We propose TTP-R1, a two-stage framework that combines retrieval-augmented supervised fine-tuning (SFT) with reinforcement learning using verifiable rewards (RLVR).

Result: TTP-R1 achieves the best average F1 across four CTI benchmarks, improving sub-technique-level F1 by 7.4 percentage points over Claude Sonnet 4.5 with retrieval augmentation, while running 28x faster when served as an 8B-parameter model on a single GPU.

Conclusion: TTP-R1 effectively solves the challenge of mapping CTI text to MITRE ATT&CK techniques and outperforms existing methods.
...
### [138] [LoRAScan: Detecting Backdoor Prompts in Low-Rank Adapters for Large Language Models via Down-Projection Activation Spikes](https://arxiv.org/abs/2608.06795)
*Doniyorkhon Obidov,Honggang Yu,Xiaolong Guo,Kaichen Yang*

Main category: cs.CR

TL;DR: LoRAScan effectively detects and rejects trigger-bearing inputs in LoRA-based models without modifying adapter parameters.


<details>
  <summary>Details</summary>
Motivation: To address the security risks posed by potentially backdoored adapters in LoRA-based models.

Method: LoRAScan identifies low-variance insertion sites and monitors them during inference to detect and reject trigger-bearing inputs.

Result: LoRAScan rejects approximately 98.49% of malicious inputs with a small error rate on clean inputs, outperforming existing defenses.

Conclusion: LoRAScan is an effective adapter-aware defense for detecting and rejecting trigger-bearing inputs in LoRA-based models.
...
### [139] [On a General Theoretical Framework for Radio Frequency Fingerprint-Based Authentication](https://arxiv.org/abs/2608.06805)
*Yuanyu Zhang,Jianing Wang,Shuangrui Zhao,Pinchang Zhang,Yulong Shen,Xiaohong Jiang*

Main category: cs.CR

TL;DR: 本文提出了一种基于RFF的理论框架，用于解释其作为设备身份标识的可靠性，并探讨了通信认证协同设计问题。


<details>
  <summary>Details</summary>
Motivation: 解释射频指纹（RFF）作为可靠设备身份标识的原因和方式，以促进基于RFF的认证技术的实际应用。

Method: 将RFF建模与认证属性分析相结合，提出一个通用的理论框架。

Result: 提供了一个理解基于RFF的认证为什么和如何在实践中是可信的坚实基础。

Conclusion: 提出了一个基于RFF的理论框架，并讨论了通信认证协同设计问题。
...
### [140] [POKEx: Performance analysis of POKE-key exchange and SIDH-variants](https://arxiv.org/abs/2608.06826)
*Hyeonhak Kim,Suhri Kim*

Main category: cs.CR

TL;DR: POKE在基于等高线的密钥交换中表现出色，是当前最有前途的候选方案。


<details>
  <summary>Details</summary>
Motivation: SIDH因小密钥尺寸和高效性能而受到关注，但Castryck和Decru在2022年的密钥恢复攻击表明其易受经典计算机上的多项式时间攻击，从而降低了其安全性。同时，POKE作为基于等高线公钥加密方案，具有高效性能。

Method: 将POKE调整为密钥交换算法，并与M-SIDH、terSIDH和CSIDH进行比较。

Result: POKE基于的KEM比terSIDH快约21.21倍，比CSIDH快64.97倍。

Conclusion: POKE是目前最有前途的等高线基于密钥交换候选方案。
...
### [141] [MIFA: An MILP-based Framework for Improving Differential Fault Attacks](https://arxiv.org/abs/2608.06837)
*Hanbeom Shin,Insung Kim,Sunyeop Kim,Byoungjin Seok,Deukjo Hong,Jaechul Sung,Seokhie Hong,Sangjin Lee,Dongjae Lee*

Main category: cs.CR

TL;DR: 提出了一种新的DFA框架，提高了密码算法的安全性，减少了密钥恢复所需的故障注入次数。


<details>
  <summary>Details</summary>
Motivation: 为了抵抗差分故障攻击（DFA），提高密码算法的安全性。

Method: 提出了一种新的DFA框架，利用混合整数线性规划（MILP）求解器，并系统分析了所有可能的单比特翻转故障的输入差异，以确定最有效的故障注入位。

Result: 该框架可以攻击比之前更深的轮次，减少密钥恢复所需的故障注入次数。对于DEFAULT，在第六轮之后注入三个故障，在第七轮和第八轮之后注入两个故障，可以将密钥空间减少到1。

Conclusion: 新的DFA框架有效地提高了密码算法的安全性，并减少了密钥恢复所需的故障注入次数。
...
### [142] [Understanding and Improving Model Editing for Secure Code Generation](https://arxiv.org/abs/2608.06848)
*Weifeng Sun,Quanjun Zhang,Yuchen Chen,Chengran Yang,Gou Tan,David Lo*

Main category: cs.CR

TL;DR: 模型编辑提高了代码生成模型的安全性，但需要结合其他方法以平衡安全性和功能性。


<details>
  <summary>Details</summary>
Motivation: 提高代码生成模型的安全性，减少从训练模式中学习到的易受攻击的实现。

Method: 对模型进行编辑，以增强其安全性，并与CoSec等推理时间方法进行比较。

Result: 模型编辑在已知漏洞类型上比CoSec获得更大的安全收益，安全比率提高15%-25%，但无法可靠地转移到未知漏洞，并可能降低功能正确性。SafeEdit方法结合了功能调整和编辑感知正则化，在保持安全性的同时，在功能正确性方面取得了显著改进。

Conclusion: 模型编辑是一种有效的安全代码生成机制，但需要结合其他方法以平衡安全性和功能性。
...
### [143] [SynChain: Inducing Computer-Use Agent Systems to Construct Their Own Attack Chains](https://arxiv.org/abs/2608.06862)
...
TL;DR: 本文提出了一种新的攻击范式，用于在大型语言模型中嵌入恶意影响，并证明了保护CUAs需要来源感知推理。


<details>
  <summary>Details</summary>
Motivation: 为了解决现有安全防御主要针对外部触发或时间有限的攻击，本文揭示恶意影响可以秘密嵌入到自主合成工具的结构冗余中，使其能够生存于内部状态更新并绕过标准审查机制。

Method: 提出了SynChain，一个利用持久性感知的定向监督微调的自合成攻击范式，以及CUAChain数据集，用于评估恶意影响的传播。

Result: SynChain能够使休眠有效载荷在未来工作流中以可信上下文无缝重新激活，实验表明其在攻击成功率上优于基线，证明了保护CUAs需要跨任务执行轨迹的来源感知推理。

Conclusion: 保护使用计算机的代理需要针对跨任务执行轨迹的来源感知推理。
...
### [144] [Rigid-Covert GNSS Spoofing of UAV Swarms: A Structural Blind Spot, Its Detection Limit, and Absolute-Anchor Defenses](https://arxiv.org/abs/2608.06885)
...
### [145] [When Context Bites: Detecting RAG Poisoning via Document-Level Attention Collapse](https://arxiv.org/abs/2608.06947)
*Yingtao Ren,Ziyi Zhao,Yiwei Fu,Xiao Luo,Yu-Cheng Chang,Chin-Teng Lin*

Main category: cs.CR

TL;DR: RAG的投毒攻击检测：提出D-SCAN方法，有效检测攻击。


<details>
  <summary>Details</summary>
Motivation: 提升大型语言模型的生成效果，但RAG容易受到投毒攻击的影响。

Method: 提出了一种基于注意力动态的检测框架D-SCAN。

Result: D-SCAN能够有效检测投毒攻击，即使攻击未改变最终答案。

Conclusion: RAG的投毒攻击检测是一个挑战，但D-SCAN提供了一种有效的解决方案。
...
### [146] [Casting the Net! Revisiting MasterFace Impersonation Attacks](https://arxiv.org/abs/2608.06952)
*Seunghun Paik,Sunpill Kim,Chanwoo Hwang,Jae Hong Seo*

Main category: cs.CR

TL;DR: 通过MasterFaces攻击，攻击者可以显著提高基于API的FRS的冒充率，对系统安全构成威胁。


<details>
  <summary>Details</summary>
Motivation: 面对现实攻击能力下的FRS（人脸识别系统）安全问题，特别是零努力冒充者对基于FMR（假匹配率）的基线构成威胁。

Method: 将MasterFaces攻击形式化为生物识别表示空间中的最大覆盖问题，并构建针对API的NET。

Result: 攻击可以将开源和商业API的FRS的冒充率提高最多9.5倍，在最多30次认证尝试内超过标准FMR。

Conclusion: 即使是合法访问公共商业API，攻击者也可以通过MasterFaces放大冒充率，对基于这些API的下游应用造成严重影响。
...
### [147] [HarnessSafe: Evaluating Safety Across Persistent Carriers in Agent Harnesses](https://arxiv.org/abs/2608.06984)
...
### [148] [Effects of parental controls in the context of Digital Forensics](https://arxiv.org/abs/2608.07016)
*Selina Märchya,Mauro Vignatia,Frank Breitinger*

Main category: cs.CR

TL;DR: 家长控制系统能够影响数字取证，但存在克服其限制的方法


<details>
  <summary>Details</summary>
Motivation: 分析家长控制系统对数字取证调查的影响

Method: 通过控制实验评估Microsoft、Google和Apple家长控制系统的证据可访问性

Result: 提供实用指导，有助于提高在受家长控制系统管理的环境中的取证准备

Conclusion: 家长控制系统可能妨碍数字取证调查，但可以通过特定的方法克服这些限制
...
### [149] [Soft Redaction of Image Provenance via Zero-Knowledge Proofs](https://arxiv.org/abs/2608.07063)
*Muhammad Awan,John Collomosse*

Main category: cs.CR

TL;DR: 提出了一种基于零知识证明的图像来源软编辑机制，以解决透明度与隐私之间的冲突，并证明了其实用性和效率。


<details>
  <summary>Details</summary>
Motivation: 为了解决图像来源透明度与隐私之间的冲突，提出了一种软编辑机制。

Method: 使用零知识证明（ZKPs）来替换敏感的来源声明，并使用Chebyshev多项式近似在ZKP证明电路中支持位置声明。

Result: 结果表明，基于图像来源的ZKPs可以提供实用的软编辑功能，与C2PA兼容，可以在几秒钟内构建并在毫秒内验证。

Conclusion: 提出了一种基于零知识证明的图像来源软编辑机制，以解决透明度与隐私之间的冲突，并证明了其实用性和效率。
...
### [150] [SoK: Cryptographic Key Recovery for Cryptoasset Custody and Financial Technologies](https://arxiv.org/abs/2608.07104)
*Francisco Javier Becerra Sanchez,Antonio Ken Iannillo,Radu State*

Main category: cs.CR

TL;DR: 本文对加密资产系统中密钥恢复的知识体系进行了系统分析，并提出了相应的发现和研究议程。


<details>
  <summary>Details</summary>
Motivation: 分析加密资产系统中密钥恢复的知识体系，并探讨其机制和权衡。

Method: 通过系统文献综述，从118篇论文中发现77篇相关论文，并构建一个包含多个维度的主矩阵。

Result: 提出了一个通用的构建模型，并发现了六个关键发现，包括恢复语义的异质性、信任的转移、滥用路径的创造、恢复生命周期管理的不足、协议证据超过用户证据以及恢复元数据保护不足。

Conclusion: 这些发现为恢复感知金融技术的研究议程提供了动机。
...
### [151] ["Operator, can you hear me?" A Faithful Line into the UNISOC Baseband](https://arxiv.org/abs/2608.07143)
*Eduard Vlad,Philipp Mao,Marcel Busch,Mathias Payer*

Main category: cs.CR

TL;DR: Unislop方法通过模拟和同步，提高了对基带处理器及其周围组件的分析准确性，成功应用于UNISOC UDX710平台，实现了对基带处理器的深入分析。


<details>
  <summary>Details</summary>
Motivation: 为了系统地分析基带处理器中最关键的逻辑，需要实时检查固件，这需要重新部署基带。现有的重新部署工作无法准确模拟执行环境，也无法准确模拟基带处理器及其周围组件的SoC复杂性，这使得分析实际上变得不可能。

Method: Unislop方法，通过模拟每个周围组件、协处理器、SIM、应用处理器，以与基带同步的方式在共享时钟上运行，从而在组件接口处进行忠诚度检查。

Result: Unislop方法在UNISOC UDX710平台上实现，成功执行代码，绕过固件完整性检查，对基带进行仪器化，并从运行设备中恢复其外围环境。重新部署后，达到了与真实设备相同的控制平面状态，建立了完整的PDU会话，并在入站和出站上传输了真实的IP流量。

Conclusion: Unislop方法为分析基带处理器提供了有效途径，具有在UNISOC基带产品线上的广泛应用潜力。
...
### [152] [An Analysis of Architectural and Operational Dynamics of Phishkits in the Wild](https://arxiv.org/abs/2608.07451)
*Behzad Ousat,Mohammad Ali Tofighi,Estefan Schafir,Amin Kharraz*

Main category: cs.CR

TL;DR: 分析了1300个钓鱼工具包，发现大部分具有相似组件，可预测性高，便于检测。


<details>
  <summary>Details</summary>
Motivation: 分析2020年至2023年间收集的1300个现代钓鱼工具包，研究其架构、源代码、通信渠道和与攻击者共享的泄露数据性质。

Method: 对1300个钓鱼工具包进行分析，包括架构、源代码、通信渠道和泄露数据性质。

Result: 发现动态重定向和伪装机制，钓鱼工具包对当前消息服务的依赖性很高，大部分钓鱼工具包具有相似或相同的组件，大量案例可预测。

Conclusion: 钓鱼工具包数量众多，但具有高级功能的比例很小，大部分钓鱼工具包具有相似或相同的组件，可预测性高，便于大规模检测。
...
### [153] [Towards Multi-Label Graph Foundation Models: from Single-Vector Representation Learning to Multi-Semantic Basis Learning](https://arxiv.org/abs/2608.06394)
*Dongxiao He,Jiayu Zhang,Jitao Zhao,Yi Wang,Di Jin*

Main category: cs.AI

TL;DR: Proposed MSB-GFM enhances multi-label node classification in diverse graph domains.


<details>
  <summary>Details</summary>
Motivation: Existing multi-label node classification methods only consider in-domain scenarios, limiting cross-domain generalization. Existing GFMs assume single-label, leading to semantic entanglement in multi-label nodes.

Method: Propose MSB-GFM, a multi-semantic basis representation learning paradigm, and a semantic-structure dual-channel architecture with domain adversarial training.

Result: Extensive experiments demonstrate the effectiveness of the proposed model.

Conclusion: MSB-GFM effectively addresses the limitations of existing methods and improves cross-domain multi-label node classification.
...
### [154] [EntropyMoE: Entropy-Aware Sparse Expert Routing for Tokenizer-Free LLMs](https://arxiv.org/abs/2608.06398)
*Bo Liu,Muxuab Yu,Yu Zhang,Pengfei Gao,Yongping Zhang*

Main category: cs.AI

TL;DR: EntropyMoE通过使用补丁熵进行专家路由，提高了字节级大语言模型的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的字节级大型语言模型在分组字节为动态大小补丁后，使无分词器的建模更具竞争力。然而，现有的字节补丁架构仍然对每个补丁应用相同的密集前馈计算。这种均匀的计算无法适应补丁语义和粒度的变化。

Method: 提出了一种名为EntropyMoE的混合专家（MoE）架构，用于动态字节补丁。EntropyMoE用Top-K专家层替换了全局补丁Transformer中的密集前馈模块。每个动态补丁作为专家路由的基本单元，其字节覆盖范围决定了其对工作负载的贡献。路由器直接从补丁熵中选择专家，使用与动态补丁构建相同的粒度信号来组织稀疏计算。补丁熵和长度共同定义了调节专家专业化的特征空间。

Result: 实验表明，EntropyMoE在匹配的密集和稀疏基线中实现了最低的保留字节每比特，同时保持了可比的下游精度。这些结果将补丁熵确立为稀疏条件计算的有效路由坐标，并将混合专家建模扩展到基于分词器的表示之外。

Conclusion: EntropyMoE通过使用补丁熵作为路由坐标，有效地解决了现有字节补丁架构的局限性，并提高了模型在字节级大语言模型中的性能。
...
### [155] [Beyond Routing Weights: Faithful Response-Level Interpretation of Mixture-of-Experts Reward Models via Contribution Contrast](https://arxiv.org/abs/2608.06400)
...
### [156] [Interpretable Unsupervised Community Detection with LLM-Symbolized Structured Processes](https://arxiv.org/abs/2608.06402)
*Aoting Zeng,Kai Wang,Jianwei Wang,Yuxiang Sun,Yizhang He,Wenjie Zhang*

Main category: cs.AI

TL;DR: LUCID：一种基于LLM的、可解释的、无标签和无监督的社区检测方法，在真实世界数据集上表现出色。


<details>
  <summary>Details</summary>
Motivation: 在图分析中，社区检测是一个基本任务，旨在识别具有相似行为或兴趣的实体集合。经典的目标驱动方法在处理复杂图结构时存在困难，而深度学习方法虽然提高了性能，但牺牲了可解释性，并依赖于标记数据和训练。大型语言模型（LLMs）具有强大的推理能力和世界知识，对可解释、无标签的社区检测很有前景。

Method: 我们提出了LUCID，一种LLM引导的、可解释的、无训练的和无监督的社区检测方法。LUCID设计为一个四阶段流程，包括：（1）局部视图社区初始化阶段；（2）多因素社区合并阶段；（3）多粒度社区细化阶段；（4）全局视图社区选择阶段。

Result: 在真实世界数据集上的大量实验表明，LUCID作为一种无监督方法，实现了最先进的性能，并且始终优于领先的未监督和半监督基线。

Conclusion: LUCID是一种基于LLM的社区检测方法，在可解释性、无标签和无监督方面表现出色，为图分析中的社区检测提供了新的思路。
...
### [157] [ADIAS: Automated Design of Interactive Agentic Systems](https://arxiv.org/abs/2608.06410)
*Lekang Jiang,Bohan Tang,Stephan Goetz,Yiwen Guo*

Main category: cs.AI

TL;DR: ADIAS通过基于问题的优化显著提升了自动化智能体设计性能。


<details>
  <summary>Details</summary>
Motivation: 为了提高自动化智能体设计的效率，通过迭代修订、评估和反馈总结来改进智能体利用。

Method: 提出了一种基于问题的智能体优化方法，其中修复进度作为显式持久性问题状态来指导优化，而不是在每个回合中从候选历史中重新推导。在ADIAS框架中实现，该框架具有两个机制：持久性问题状态和问题引导的优化。

Result: 在五个交互式基准测试中，ADIAS平均优于最强大的基线25.2%，并在四个骨干模型上实现了一致的收益。控制消融进一步表明，移除持久性问题状态或用候选中心修订替换问题中心修订会导致性能下降高达40.7%。

Conclusion: ADIAS框架通过基于问题的优化方法显著提高了自动化智能体设计的性能。
...
### [158] [Learning to Predict Middle-Layer Attention in MLLMs for Visual Token Prunin](https://arxiv.org/abs/2608.06411)
*Yuyao Sun,Tao Deng,Shuang Li,Deqing Wang,Hao Geng,Minjun Yu*

Main category: cs.AI

TL;DR: 提出了一种新的方法，通过减少视觉标记数量，提高了MLLMs的效率，并保持了模型性能。


<details>
  <summary>Details</summary>
Motivation: 多模态大型语言模型（MLLMs）在视觉语言任务上表现出色，但其效率受处理大量视觉标记的成本限制。视觉标记剪枝可以减少这种成本，但需要准确的标记重要性估计。

Method: 提出了一种名为Middle-layer Attention Prediction（MAP）的方法，该方法使用问题对比教师选择来识别特定样本的教师层，并将所选层的注意力提炼成一个轻量级的预测器，该预测器从多模态输入特征估计视觉标记的重要性。

Result: 在LLaVA-NeXT-7B上的十个基准测试中，MAP在只保留5.56%的视觉标记的情况下，保留了未剪枝模型97.5%的性能，实现了3.09倍的端到端加速。

Conclusion: MAP通过减少视觉标记的数量，提高了MLLMs在视觉语言任务上的效率，并保持了模型性能。
...
### [159] [WebGrader: Training LLMs for Web Development with Self-Evolving Programmatic Grader](https://arxiv.org/abs/2608.06474)
*Boshui Chen,Huiping Liu,Shaolei Zhang*

Main category: cs.AI

TL;DR: WebGrader是一种新的评分器，可以有效地训练大型语言模型生成更完整的网站。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型生成完整网站的能力不断提高，强化学习成为缩小其功能差距的关键方法。

Method: 提出WebGrader，一个自我进化的程序性评分器，它从每个网站请求中自动推导出所需的交互流程，将每个流程表示为可执行的Flow Contract，并使用其执行结果作为强化学习奖励。

Result: WebGrader在WebGen-Bench上训练的8B策略达到52.01%的功能成功率，优于匹配的外观加脚本奖励7.88点，超过o4-mini和DeepSeek-v4-flash。在WG-core-250上，策略达到44.953的全分，超过Qwen3-Coder-480B。

Conclusion: WebGrader通过分离测试计划、动作定位、证据收集和语义判断，在观察到请求的转换后才发出通过判决。
...
### [160] [Can MLLMs Decode the Creative Leap? Introducing C4 for Cross-Concept Understanding](https://arxiv.org/abs/2608.06501)
...
TL;DR: KnowPlan系统通过自动化和优化，有效解决了大学课程规划问题。


<details>
  <summary>Details</summary>
Motivation: 为了解决从官方大学来源规划学位课程的两个问题：首先从目录、系部页面、JSON端点和PDF中重建课程体系，然后才能根据先决条件和重叠要求约束优化学生特定的路径。

Method: KnowPlan系统，包括CatalogBrowse和DegreeMap模块。CatalogBrowse通过有限集的原子目录义务来评分合法行为，并使用span-constrained clause-to-AST模型进行确定性解析。DegreeMap将输入编译成类型化的需求超图，并使用CP-SAT进行优化。

Result: CatalogBrowse在100所大学的广泛轨迹和6所学校的密集轨迹上，达到96.2%的库存召回率和88.7%的掩码源恢复率。DegreeMap保持100.0%的硬可行性，同时将个性化效用提高+0.066。整个流程认证了99.5%的请求，与特权黄金图的效用差距为0.015。

Conclusion: KnowPlan系统有效地解决了大学课程规划问题，提高了个性化效用和可行性。
...
TL;DR: TaskSense通过注意力机制和逆动力学监督，提高了视觉控制模型的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 为了解决视觉控制中任务相关内容与背景干扰之间的不匹配问题，提高模型对视觉干扰的鲁棒性。

Method: 提出了TaskSense，一个以任务为中心的世界建模框架，通过可微的随机空间注意力机制和辅助逆动力学目标来引导注意力到控制相关的区域。

Result: 在DeepMind Control Suite上与DreamerV3基准相当，在Distracting Control Suite上表现优于DreamerV3，提高了对视觉干扰的鲁棒性。

Conclusion: TaskSense通过注意力机制和逆动力学监督，有效地提高了视觉控制模型的鲁棒性。
...
### [163] [Divergent Response Modes in Frontier Language Models Under Steering Pressure](https://arxiv.org/abs/2608.06578)
*Ali Jalal-Kamali*

Main category: cs.AI

TL;DR: 研究评估了六个前沿语言模型的行为可操控性，发现模型之间存在差异，并揭示了不同模型的独特响应模式


<details>
  <summary>Details</summary>
Motivation: 探讨前沿语言模型在显式引导压力下的行为可操控性差异

Method: 使用300对基础和引导项目，在三个类别上评估六个模型的行为可操控性，包括价值观冲突、推理引发和推理抑制，并使用共识评分方法

Result: 发现模型在行为可操控性方面存在差异，不仅体现在行为改变的程度，还体现在响应模式上，某些响应模式只出现在一两个模型中

Conclusion: 研究揭示了前沿语言模型在行为可操控性方面的差异，并发现了不同模型在响应模式上的独特行为
...
### [164] [Automated item evaluation: Predicting item acceptance and rejection using LLM-generated critiques](https://arxiv.org/abs/2608.06609)
...
### [165] [NxN E-valuation: Hypothesis Certification via a Conformal CRT Null](https://arxiv.org/abs/2608.06621)
*Bin Wang,Yan Zhong*
...
### [166] [Shape Your Feed: An LLM-based Agentic System for Conversational Recommendation](https://arxiv.org/abs/2608.06632)
...
TL;DR: Shape Your Feed（SYF）是解决工业推荐系统中推荐优化与用户表达兴趣之间差距的有效框架


<details>
  <summary>Details</summary>
Motivation: 解决工业推荐系统中推荐优化与用户表达兴趣之间存在的差距问题

Method: 提出基于LLM的Shape Your Feed（SYF）推荐框架，实现实时、多模态内容共筛选

Result: SYF的在线A/B实验显示，提高了推荐的相关性和用户情感，表明在工业环境中实现互动、用户可控推荐的实际和可扩展途径

Conclusion: SYF是一个有效的推荐框架，可以解决工业推荐系统中推荐优化与用户表达兴趣之间存在的差距问题
...
### [167] [TRACE: A Multi-Layer Benchmark for Human AI Controller Coordination Under Drift and Failure](https://arxiv.org/abs/2608.06657)
*Joshua Zuniga,Srinivasan Subramanian,Ramya Madhuri Narapureddy,Md Abdullah Al Hafiz Khan*

Main category: cs.AI

TL;DR: 本文提出了一种新的方法来识别和归因漂移，为提高现代网络物理和人工智能辅助系统的信任度提供了新的思路。


<details>
  <summary>Details</summary>
Motivation: 为了解决现代网络物理和人工智能辅助系统中信任度问题，本文针对漂移这一缺陷进行研究。

Method: 通过在ALFRED基准测试中注入可控漂移，构建了一个包含1,918条漂移轨迹的基准数据集。每个轨迹都包含五个执行层（状态、观察、决策、规则、控制）的逐步记录，并标注了漂移类型、受影响层、开始时间、责任方和因果关系。同时，使用一个无泄漏协议和一个基于经典、循环和注意力机制模型家族的基线研究。

Result: 在无泄漏协议下，漂移可识别和可归因，优于随机和大多数基线。注意力机制模型在此符号基准上并不优于简单模型。

Conclusion: 本文提出的方法可以有效地识别和归因漂移，为提高现代网络物理和人工智能辅助系统的信任度提供了新的思路。
...
### [168] [CellWorld: From Gene-Level Reconstruction to Latent Cell Prediction in Spatial Transcriptomics Foundation Models](https://arxiv.org/abs/2608.06659)
*Haiping Liu,Qian Zhao,Lijing Lin,Jingyuan Sun,Hongpeng Zhou*

Main category: cs.AI

TL;DR: CellWorld是一种基于潜在空间预测预训练的空间转录组基础模型，在空间转录组学中表现出色。


<details>
  <summary>Details</summary>
Motivation: 为了提供一种可扩展的方法来构建空间转录组的基础模型，避免直接重建技术变异，将预测目标从观察到的基因测量值转移到潜在细胞表示。

Method: 提出CellWorld，预测被掩蔽细胞的潜在表示，从可见的空间上下文和有限的局部表达提示中预测。在包含4600万人类细胞的语料库上预训练了四个CellWorld变体。

Result: CellWorld在所有11个线性探针基准和所有7个微调的空间基准上均优于所有基线。CellWorld-Large在仅使用语料库5%的广泛生物来源覆盖下，在所有七个空间基准上优于所有完全微调的基线。

Conclusion: CellWorld提供了一种可扩展的路线来构建空间转录组的基础模型，在空间转录组学中表现出色。
...
### [169] [Vehicle routing problem using deep reinforcement learning - A case study about truck planning in the industry](https://arxiv.org/abs/2608.06668)
*Siliang Lu,Dan Hu,Lili Wu*

Main category: cs.AI

TL;DR: 本文探讨了深度强化学习在车辆路径优化中的应用，并展示了其在降低总成本方面的效果。


<details>
  <summary>Details</summary>
Motivation: 解决VRP的优化问题，减少成本和碳足迹

Method: 深度强化学习在车辆路径优化中的应用

Result: 优化后的路线降低了10%以上的总成本。

Conclusion: 强化学习代理优化路线的总成本比基线结果低10%以上。
...
### [170] [A Multi-Agent Framework for Automated Coarse-Grained Molecular Dynamics of Polymers](https://arxiv.org/abs/2608.06694)
*Joohee Choi,Junhyeong Lee,Seunghwa Ryu*

Main category: cs.AI

TL;DR: CGMas自动化聚合物粗粒度，提高模拟效率。


<details>
  <summary>Details</summary>
Motivation: 聚合物模拟中粗粒度分子动力学扩展了全原子方法可及的尺度，但自下而上的粗粒度建模是费力的。

Method: 提出CGMas，一个多智能体框架，自动化拓扑构建、平衡、映射、势能推导和验证。

Result: CGMas完成了所有27个均聚物和共聚物任务，22个与全原子密度匹配在5%以内，将模拟时间从38-88分钟缩短到1分钟。

Conclusion: 证明了代理LLM在自动化聚合物粗粒度方面的可行性。
...
### [171] [AgentPatch: Coarse-to-Fine Weak-Task Repair for Merging Agentic Multimodal Large Language Models](https://arxiv.org/abs/2608.06699)
*Zibo Shao,Baochen Xiong,Chengdong Xu,Linhui Xiao,Kaichen Li,Haoran Gong,Yan Li,Yaguang Song,Xiaoshan Yang*

Main category: cs.AI

TL;DR: AgentPatch：解决Agentic MLLM合并中的能力保留和行为遗忘问题，提高合并效果。


<details>
  <summary>Details</summary>
Motivation: 扩展多模态感知和推理，包括规划、工具使用和动态环境中的交互。

Method: 提出AgentPatch，一种训练无关的粗到细修复框架。

Result: AgentPatch提高了各种合并骨干的性能，缓解了弱任务退化，并更好地平衡了弱任务恢复与互补搜索和代理视觉处理能力的保护。

Conclusion: AgentPatch能够有效地解决不对称能力保留和行为关键遗忘问题，提高了Agentic MLLM的合并能力。
...
### [172] [WebRider: Persona-Conditioned Intent Controllers for Live-Web Assistance](https://arxiv.org/abs/2608.06704)
*Zhi Li,Tao Zhou,Yeqing Li,Eugene Ie,Demetri Terzopoulos*

Main category: cs.AI

TL;DR: WebRider通过形式化政策合约和分层架构，提高了网络任务分配的准确性和可靠性。


<details>
  <summary>Details</summary>
Motivation: 当前网络代理在完成任务时仅关注最终答案，而忽略了定义任务分配的政策约束。这可能导致政策违反而未被察觉。

Method: WebRider通过将委托政策形式化为意图合约，并采用分层架构来维护合约、实现意图和执行操作。

Result: 实验表明，WebRider在42个公共网站上对4,096个实时网络合约进行了审计，并取得了良好的效果。

Conclusion: WebRider能够确保任务分配的准确性，同时保持政策的遵守和用户经验的连续性。
...
### [173] [MolBioKG: Grounding Out-of-Graph Molecules in Biomedical Knowledge Graphs via Multi-Resolution Structural Anchoring](https://arxiv.org/abs/2608.06713)
...
### [174] [The Optimizer Is the Agent: Reasoning-Driven Search across Prompts, Programs, and ML Workflows](https://arxiv.org/abs/2608.06714)
*Junbo Li,Boyi Liu,Canwen Xu,Yite Wang,Yuxiong He,Zhangyang Wang,Qiang Liu,Zhewei Yao*

Main category: cs.AI

TL;DR: ReASearch框架通过代理自主决策，实现推理驱动的优化，在多个任务中表现出色


<details>
  <summary>Details</summary>
Motivation: 探索如何将搜索策略内部化到单个工具使用代理中

Method: 提出ReASearch框架，实现推理驱动的优化，代理自主决策评估、诊断失败、编辑和验证/重启

Result: 在14个不同任务中，ReASearch的表现优于或与专用优化系统相当，相对于强基线实现了2%至40%的改进，并在某些情况下发现了优于先前人类已知结果的新解决方案

Conclusion: 复杂搜索行为自然地从代理的推理过程中出现，无需显式控制器
...
### [175] [bioMoR: Biology-Guided Mixture-of-Recursions for Effective Genomic Learning](https://arxiv.org/abs/2608.06727)
...
### [176] [From Cheap Fakes to Pure Synthesis: Addressing the New Era of T2V Fake News Videos](https://arxiv.org/abs/2608.06732)
...
### [177] [IB-RL: Isolated Bilateral Reinforcement Learning for Strategic Dialogue Agents](https://arxiv.org/abs/2608.06735)
*Senhao Wang,Chenghao Cai,Haitao Hu,Mingxing Huang,Xingguang Wang,Wenhao Li,Zecheng Lin*

Main category: cs.AI

TL;DR: 提出IB-RL方法，有效提高LLMs在战略对话任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 提高大型语言模型（LLMs）在具有静止、可验证奖励的任务上的性能，例如数学推理和代码执行。

Method: 提出Isolated Bilateral Reinforcement Learning（IB-RL）方法，通过联合滚动和独立优化，解决静态对手不匹配问题。

Result: 在Vehicle TeleSales任务中，IB-RL达到89.6% Success@1，优于84.6%的最佳单边RL基线；在Deal-or-NoDeal任务中，达到98.4%的协议率，优于86.4%的最佳单边基线。

Conclusion: IB-RL方法能够更有效地泛化到未见过的对手。
...
### [178] [MemPrism: Task-Conditioned Relational Memory Views for Long-Horizon Agents](https://arxiv.org/abs/2608.06745)
...
TL;DR: MemPrism通过任务条件关系视图提高长期代理任务性能和减少内存消耗。


<details>
  <summary>Details</summary>
Motivation: 现有记忆系统通常假设证据可以通过固定的表示直接消费，这导致表示不匹配，即相关信息可用但未针对当前决策进行组织。

Method: 提出MemPrism，一个任务条件关系记忆框架，将持久经验存储与决策时工作记忆分离。MemPrism将交互记录为事件流，并根据当前任务上下文动态构建关系视图。轻量级视图策略选择关系结构、证据范围、结果条件和粒度，而确定性作曲家和渲染将历史事实转换为临时光学工作记忆视图，以适应冻结的任务策略。

Result: 在长期具身和Web代理基准测试中，MemPrism在提高任务性能方面表现一致，尤其是在轨迹变长时，同时减少内存令牌消耗。此外，学习的视图策略在不同VLM之间转移，无需额外调整，证明了任务条件关系视图作为代理通用记忆接口的有效性。

Conclusion: MemPrism通过任务条件关系视图，有效地解决了长期代理的记忆表示不匹配问题，提高了任务性能并减少了内存消耗。
...
### [179] [Mind the Gap: A Dual Knowledge Graph Framework for Unified Multi-task User Intent Inference](https://arxiv.org/abs/2608.06752)
*Tzu-Cheng Peng,Chien Chin Chen,Chih-Hao Ku,Yung-Chun Chang*

Main category: cs.AI

TL;DR: DKG-MTI is a dual knowledge graph framework for unified multi-task user intent inference from online travel reviews.


<details>
  <summary>Details</summary>
Motivation: Existing approaches often rely on hierarchical pipelines that suffer from error propagation or retrieval methods that ignore structural relationships in domain knowledge.

Method: An inference-only knowledge augmentation framework that dynamically constructs a User-Specific Intent Knowledge Graph from each review and aligns it with a Global Hotel Knowledge Graph through structure-aware semantic smoothing. The aligned knowledge is combined with the original review and processed by a large language model to simultaneously predict aspect ratings and generate reverse user intent statements.

Result: Experiments on TripAdvisor reviews show that DKG-MTI consistently outperforms strong LLM and retrieval-based baselines in both classification and intent generation tasks, demonstrating the effectiveness of structure-aware knowledge alignment for scalable and explainable intent inference.

Conclusion: DKG-MTI is an effective dual knowledge graph framework for unified multi-task user intent inference from online travel reviews.
...
### [180] [Capek 0.5: An Execution-Centric Vision-Language Model for Embodied Intelligence](https://arxiv.org/abs/2608.06756)
*Ying Chen,Weizhen Li,Zhe Hu,Zhenjiang Li,Rui Jiang,Zhifeng Gu,Lihuang Fang,Jiangping Liu,Lei Yi,Jie Chen*

Main category: cs.AI

TL;DR: Capek 0.5：一种基于执行中心能力分类法的实体视觉语言模型，有效整合了机器人执行过程中的互补能力。


<details>
  <summary>Details</summary>
Motivation: 为了满足机器人执行过程中的迭代需求，需要互补的能力，这些能力在监督信号、预测格式和验证标准方面有所不同。

Method: 提出了一种名为Capek 0.5的实体视觉语言模型，该模型围绕执行为中心的能力分类法构建。通过强化学习和可验证的奖励，每个能力首先由专门的专家获得，然后通过权重空间合并和路由策略空间蒸馏将专家合并到一个推理时模型中。

Result: Capek 0.5在大多数匹配的基准行上有所改进，保留了所有四种专业能力，并在闭环实体任务执行中进行了迁移。

Conclusion: Capek 0.5通过一种新的能力分类法，有效地整合了机器人执行过程中的互补能力，提高了执行效率和性能。
...
### [181] [LiFTER: A Grounded Neuro-Symbolic Microscope for Continuous-Time Dynamic Graph Forecasting](https://arxiv.org/abs/2608.06765)
*Minwoo Yu,Young-guk Ha*

Main category: cs.AI

TL;DR: LiFTER是一种新的神经符号预测器，它通过将观察到的交互作为基于事实的时间事实并应用可执行的时间规则，提高了连续时间动态图模型预测的准确性和可解释性


<details>
  <summary>Details</summary>
Motivation: 为了解决连续时间动态图模型在预测中无法显式地识别事件间共享实体和时序模式的问题

Method: 提出了一种神经符号预测器LiFTER，它将观察到的交互作为基于事实的时间事实，并应用可执行的时间规则来预先查询事实

Result: 在四个CTDG基准测试中，LiFTER实现了有竞争力的历史负预测，最高宏观解释准确率和删除保真度。该架构还可以作为显微镜，分离数据集和轨迹中循环、历史位置和转换的贡献，并追踪到单个事实。独立执行重构了19,664个测试预测的所有logits，最大误差为0.0000131。LiFTER将未来链接预测转化为可验证的基于事实的计算

Conclusion: LiFTER有效地解决了连续时间动态图模型在预测中的问题，提高了预测的准确性和可解释性
...
### [182] [Surg-UniWorld: A Unified Surgical World Model with Multimodal Control Experts](https://arxiv.org/abs/2608.06770)
...
TL;DR: 提出了一种名为Surg-UniWorld的统一手术世界模型，通过多模态控制提高了手术人工智能和模拟的生成质量。


<details>
  <summary>Details</summary>
Motivation: 为了提供手术人工智能和模拟的生成基础，可控手术世界模型可以通过合成真实的器械-组织相互作用。然而，现有方法缺乏统一的多模态控制范式，而直接融合异构视觉条件常常导致解剖扭曲、器械外观漂移和时间不一致的交互。

Method: 提出了一种名为Surg-UniWorld的统一手术世界模型，具有多模态控制专家。首先，从第一帧外观和分层语义掩码构建了一个分层手术锚，以保持持久场景身份、解剖组织和交互边界。然后，锚相对模态专家解释边缘、深度和光流证据相对于共享锚，捕获互补边界、几何和运动信息。进一步地，多模态控制专家执行贡献保留的阶段式组合激活模态增量，并为Wan2.2视频扩散主干生成控制提示。为了支持多模态手术世界建模，进一步构建了Cholec80-SurgWAM，作为可控手术视频生成的基准。

Result: Surg-UniWorld在生成质量、时间一致性和多模态可控性方面，始终优于现有的可控视频生成方法和手术世界模型基线。

Conclusion: Surg-UniWorld为可控手术世界建模提供了一种有效的方法，提高了手术人工智能和模拟的生成质量。
...
### [183] [Evolving Parallel Algorithm Portfolios via Potential-Aware Instance Generation with LLMs](https://arxiv.org/abs/2608.06808)
*Shaofeng Zhang,Shengcai Liu,Zhiyuan Wang,Ke Tang*

Main category: cs.AI

TL;DR: PIAC框架通过引入新指标和LLM技术，有效提升了LLM-ACP在组合优化问题上的泛化能力。


<details>
  <summary>Details</summary>
Motivation: LLM-ACP在解决复杂组合优化问题时在实践中的少样本场景中泛化能力差。

Method: 引入潜在感知实例和算法协同进化(PIAC)框架，提出潜在增益等新指标，并利用LLM合成多样本突变体。

Result: 在TSP和CVRP问题上，PIAC优于现有的LLM-ACP基准，TSP Greedy Constructive投资组合相对改进19.76%。

Conclusion: PIAC框架提高了LLM-ACP的泛化能力，在解决组合优化问题时具有优越性。
...
### [184] [Gated-BEPO: Confidence-Gated Bellman Credit Assignment for Large Language Model Agents](https://arxiv.org/abs/2608.06861)
*Hongxi Yan,Ziyue Huang,Shichao Fan,Qingjie Liu*

Main category: cs.AI

TL;DR: Gated-BEPO通过从经验回滚图中推导出步骤级信用，提高了长时域环境中大型语言模型代理的训练效果。


<details>
  <summary>Details</summary>
Motivation: 在长时域环境中训练大型语言模型代理需要将信用分配给单个动作，从稀疏的终端结果中分配信用。

Method: 提出了一种名为Gated-BEPO的方法，该方法从经验回滚图中推导出步骤级信用。对于每个回滚组，Gated-BEPO构建一个经验图，并通过反映当前策略的经验动作分布的均值备份Bellman不动点来估计节点值。然后，使用广义优势估计在每条采样轨迹上累积这些时间差残差，从而产生步骤级Bellman优势，这些优势可以捕捉到即时和下游效应。为了自适应地融合剧集级和步骤级信用，一个置信门仅在具有多个观察到的后续状态时才包含Bellman信用，否则使用剧集级信用。

Result: 在WebShop、ALFWorld和视觉Sokoban上的实验表明，在语言和视觉语言模型上均取得了持续改进。

Conclusion: Gated-BEPO方法通过从经验回滚图中推导出步骤级信用，有效地提高了长时域环境中大型语言模型代理的训练效果。
...
### [185] [CEDAR: Agent-Orchestrated Tree Search for Goal-Directed Optimization of Complex Systems](https://arxiv.org/abs/2608.06871)
*Yingtao Tian*

Main category: cs.AI

TL;DR: CEDAR是一种利用LLM和MCTS发现复杂系统的自主方法，有助于解决人工生命中的核心问题，并促进复杂系统在各个领域的应用。


<details>
  <summary>Details</summary>
Motivation: 为了解决预测反馈结构如何产生涌现行为这一人工生命中的核心开放问题，以及现有的系统结构编写和决策制定中的挑战。

Method: 提出了一种名为CEDAR的自主方法，使用大型语言模型（LLM）代理来发现满足用户指定行为目标的多样化复杂系统。该方法的核心创新是将LLM驱动的蒙特卡洛树搜索（MCTS）与复杂系统深度耦合，同时使用LLM进行评估和改进。

Result: CEDAR能够减少人力投入，同时实现现有方法难以实现的能力，促进复杂系统在各个领域的更广泛采用。

Conclusion: CEDAR为复杂系统的设计和应用提供了一种新的有效方法，有助于解决人工生命中的核心问题。
...
### [186] [SkillEval: Decomposing Agent Skill Quality into Interpretable Signals](https://arxiv.org/abs/2608.06891)
*Jiahui Han,Qinuo Li,Ziheng Peng,Haotian Wu,Haoze Liu,Danfeng Shan,Guanchu Wang,Huiqi Deng,Ninghao Liu*

Main category: cs.AI

TL;DR: SkillEval是一种用于评估和改进技能质量的框架，它通过评估文档的通用属性来区分技能质量，并指导技能改进。


<details>
  <summary>Details</summary>
Motivation: 评估技能质量对于提高技能的可重用性和性能至关重要。

Method: 提出SkillEval框架，用于评估文档级别技能的通用属性。

Result: SkillEval能够可靠地区分不同质量的技能，并指导技能改进。

Conclusion: SkillEval为评估和改进技能质量提供了一种新的、可解释的方法。
...
### [187] [From Points to Edges: Edge-Conditioned Spectral Operators for Physics-Sensitive PDE Learning](https://arxiv.org/abs/2608.06894)
*Zhentao Tan,Ruijie Quan,Yi Yang*

Main category: cs.AI

TL;DR: ESO是一种新的谱算子框架，通过局部边缘信息调节全局谱混合，在PDE求解中实现最先进性能。


<details>
  <summary>Details</summary>
Motivation: 许多偏微分方程（PDEs）包含对基本物理行为至关重要的局部结构。例如，在达西流中，局部材料界面通常通过渗透率场的急剧变化反映出来，并可能强烈影响解。现有的谱算子主要基于中心点表示的模态混合，这使得它们对这种局部结构变化反应不足。

Method: 提出边缘条件谱算子（ESO），这是一种新颖的谱算子框架，它通过使用局部边缘变化来调节全局谱混合。通过将成对变化模态混合器（PVMM）纳入其中，将局部边缘信息注入到谱模态选择中，ESO在保持谱神经网络算子的全局近似能力的同时，使学习的核能够适应对物理敏感的局部结构。此外，我们引入了任务自适应的物理感知重加权（PAR），该重加权强调了由特定物理量识别的物理重要区域。

Result: 在九个PDE基准测试中，ESO始终实现了最先进的性能。视觉和区域分析进一步表明，ESO减少了系数跳跃、高梯度流动结构和其他物理敏感区域的解误差。

Conclusion: ESO是一种有效的谱算子框架，可以更好地适应物理敏感的局部结构，并在PDE求解中实现最先进的性能。
...
### [188] [Long-Horizon Agent Trajectory Attribution: A Unified Benchmark and Fine-Grained Annotation Framework](https://arxiv.org/abs/2608.06909)
*Jing Chen,Yang Sun,Li Zhang,Lin Xu,Jie Shi*

Main category: cs.AI

TL;DR: 开发了新的LLM代理轨迹归因基准和注释框架。


<details>
  <summary>Details</summary>
Motivation: 对现有大型语言模型（LLM）代理行为评估方法的不足进行改进。

Method: 引入轨迹归因和开发基准和注释框架。

Result: 创建了一个包含超过1300个注释轨迹的基准，定义了两个评估任务，并提供了参考基线。

Conclusion: 该基准为LLM代理的轨迹归因分析提供了新的工具和方法。
...
### [189] [Fast LapSum: Exact Differentiable Top-k at Million Scale](https://arxiv.org/abs/2608.06912)
...
### [190] [ReGraph: Learning to Generate Recipe Graphs from Food Images](https://arxiv.org/abs/2608.06917)
*Guoshan Liu,Bin Zhu,Pengkun Jiao,Jingjing Chen,Chong-Wah Ngo,Yu-Gang Jiang*

Main category: cs.AI

TL;DR: We propose RGL to improve recipe generation using LMMs, focusing on structured recipe graphs and fine-grained ingredient-state capture.


<details>
  <summary>Details</summary>
Motivation: Recent Large Multimodal Models (LMMs) have achieved impressive performance in recipe generation from food images.

Method: We present ReGraph, a large-scale recipe graph dataset that represents ingredients, cooking actions, and tools as entities, uses entity attributes to describe ingredient state changes, and employs typed relations to encode manipulation targets, destinations, and procedural ordering. We further incorporate explicit Recipe Reasoning Chain-of-Thought (RR-CoT) traces, providing auxiliary supervision for procedural decomposition and structured graph generation. Based on ReGraph, we propose Recipe Graph Learning (RGL), a two-stage framework that enables LMMs to generate a plausible fine-grained cooking workflow from a food image in the form of a structured recipe graph.

Result: Our experiments reveal a substantial gap between text-generation quality and recoverable procedural structure: recipes produced by existing approaches achieve competitive text-generation scores yet yield limited reference-aligned entity and relation structure under the ReGraph schema. In contrast, across two representative LMM backbones, RGL consistently improves the generation of cooking entities and procedural relations, while our analysis further shows that fine-grained ingredient-state capture remains the most challenging dimension.

Conclusion: RGL enables LMMs to generate a plausible fine-grained cooking workflow, improving the generation of cooking entities and procedural relations, although fine-grained ingredient-state capture remains challenging.
...
### [191] [Deal Me Maybe: The Role of Emotions in Multi-Agent Negotiation](https://arxiv.org/abs/2608.06922)
*Massimiliano Luca,Apoorva Singh,Bruno Lepri*

Main category: cs.AI

TL;DR: 研究表明，情感对LLM代理的价格谈判有显著影响。


<details>
  <summary>Details</summary>
Motivation: 研究情感如何影响基于LLM的价格谈判。

Method: 在受控框架下，买家和卖家代理被分配六种情绪状态之一，在两种预算条件下就350种真实消费品进行谈判。

Result: 情绪强烈地影响结果，买家情绪主要驱动接受和拒绝，而卖家情绪塑造让步动态。

Conclusion: 这些影响不仅影响语言，还影响终止行为和价格轨迹，对商业中的情绪调节代理提出担忧。
...
### [192] [TRIBE: Predicting Team Performance via Communication Behavior Ensembles](https://arxiv.org/abs/2608.06926)
*Ali Jalal-Kamali,Nikolos Gurney,David V. Pynadath,Fred Morstatter*

Main category: cs.AI

TL;DR: TRIBE enhances team assistance with predictive capabilities and efficiency improvements.


<details>
  <summary>Details</summary>
Motivation: Designing autonomous agents that effectively assist human teams

Method: TRIBE, a domain independent approach

Result: predicts team performance; reveals team behavioral dynamics; improves speed and performance

Conclusion: TRIBE effectively assists human teams
...
### [193] [Science Edge Evaluation: SEE the Missing Step Toward Real Scientific Discovery](https://arxiv.org/abs/2608.06931)
...
### [194] [Blind to the Pivotal Vote: Aggregate Independence Metrics Miss Where Verification Actually Helps](https://arxiv.org/abs/2608.06940)
*Yang Shu*

Main category: cs.AI

TL;DR: LLM判断小组的误差集中在关键查询上，多数方替换规则和信号使用可显著提高整体准确性。


<details>
  <summary>Details</summary>
Motivation: LLM判断小组是标准评估工具，但先前的工作报告了小组错误的高度相关性：九位法官提供的信息相当于两位独立法官的信息，而聚合只缩小了很小一部分差距。一种自然的补救措施——来自不同证据来源的信号，例如执行测试套件——在规模上没有产生可区分的变化（-0.04，95% CI [-0.10, +0.02]）。聚合依赖和条件决策效用是不同的问题。基本的多数算术法可以解决单票替换的影响集：只有一票之差的决策可以改变。实证问题是小组错误率是否会上升，有用的替换是否集中在那里。它们确实如此：整个准确性增益集中在这些关键查询上，增益很大（+10.4到+23.3个百分点，涵盖三个主要配置），而其他地方正好为零。我们在三个代码基准和四个小组规模（9位法官的扩展和56个依赖子采样检查，增益+6.5到+16.1个百分点）上确认了这种模式。在HumanEval+/MBPP+上，多数方替换规则将整体准确性从82.44%提高到85.62%，同时仅在16.2%的查询上调用信号；仅信号仍然更强，为87.60%。因此，人口层面的依赖诊断和边缘分层效用是互补的，而影响集的描述为任何指定的单票替换策略提供了呼叫减少规则。

Method: 研究LLM判断小组的误差和改进方法，包括测试套件的使用、多数算术法和边缘分层效用分析。

Result: 研究发现，小组错误率集中在关键查询上，通过多数方替换规则和信号使用，整体准确性得到显著提高。

Conclusion: LLM判断小组的误差可以通过多种方法进行诊断和改进，其中多数方替换规则和信号使用是有效的。
...
### [195] [LMM Modality Transfer: A Pre-requisite for Autonomous GIS Agents](https://arxiv.org/abs/2608.06948)
...
### [196] [Does Splitting a Triage Decision Across Agents Hide Bias or Help Catch It? A Multi-Agent Simulation Study of LLM-Based Resource Allocation Under Audit Capacity Constraints](https://arxiv.org/abs/2608.06949)
*Paul-Peter Arslan*

Main category: cs.AI

TL;DR: 多代理管道可以减少偏见，但审计能力至关重要。


<details>
  <summary>Details</summary>
Motivation: 研究当决策被分配到多代理管道中时，偏见是否仍然存在。

Method: 使用合成灾难分类模拟器，比较单个代理控制条件与九个代理管道在不同压力维度下的表现。

Result: 没有在两种条件下观察到偏见结果发生的频率有可测量的差异，但审计能力对是否发现偏见有显著影响。

Conclusion: 在资源受限的情况下，将决策分散到多代理管道（评估、分配、独立审计）中可以减少偏见，但审计能力对是否发现偏见有显著影响。
...
### [197] [Critical Acclaim Orientation in Large Language Models: Evidence from Film Preference Elicitation](https://arxiv.org/abs/2608.06955)
*Jonghyun Jee,Aaron Shaw*

Main category: cs.AI

TL;DR: LLMs在评价电影时表现出偏好，这种偏好与获奖情况、商业成功和公众接受度有关。


<details>
  <summary>Details</summary>
Motivation: 探讨大型语言模型（LLMs）是否系统地再现评价等级的问题。

Method: 通过研究电影评价，使用来自四个家族（Anthropic、OpenAI、Alibaba和Mistral）的八个模型，对200部电影进行评价，包括获奖、商业成功和双重合法性（获奖+商业成功）的电影。使用Bradley-Terry估计，对每个模型进行20,000次成对强制选择比较。

Result: 所有模型都表现出一致的获奖倾向：获奖但商业不为人知的电影被选为比商业成功但批评未认可的更好。这种模式在每个家族的模型规模中都会增长。此外，嵌套OLS回归分析表明，评价倾向、公共可见性和流行接受度明显有助于解释偏好。调整公共可见性会逆转模型对双重合法性的偏好，而考虑流行接受度则会减轻仅商业成功的电影的大部分劣势。最后，评价和推荐导向的提示框架产生不同的排名，表明批评认可的倾向可能在实际LLMs部署中间接体现。

Conclusion: LLMs在评价电影时表现出一定的偏好，这种偏好与电影的获奖情况、商业成功和公众接受度有关。
...
### [198] [CAi Copilot: Reducing Operational Workload in Molecular Design through Intent-Driven Agentic Workflows](https://arxiv.org/abs/2608.06961)
*Zhu Wang,Jiangyu Chen,Yingjun Shang,Yuhui Yao,Laiao Lu,Tianfan Fu,Na Zou*

Main category: cs.AI

TL;DR: CAi Copilot通过自动化分子设计流程提高了效率。


<details>
  <summary>Details</summary>
Motivation: 将研究意图转化为基于科学工具的适应性、可追溯的运行，解决专家在每个步骤中协调、判断中间结果和整合证据的挑战。

Method: CAi Copilot，一个专家导向的代理，具有三个相互关联的层次：研究接口层、代理推理层和执行基础层。

Result: 在45个任务中，CAi取得了最强的整体性能，结果分数为84.59，超过第二好的结果18.07分。

Conclusion: CAi成功地将广泛的分子设计意图转化为透明、可追溯的工作流程，将中间决策与候选级别的证据相连接。
...
### [199] [Learning in Deep Networks under Dale's Constraint](https://arxiv.org/abs/2608.06963)
*Roy Abel,Shimon Ullman*

Main category: cs.AI

TL;DR: 提出了一种新的神经网络架构，通过非负活动表示神经元的激活和学习信号，实现了类似反向传播的学习，在Tiny ImageNet基准测试中表现出色。


<details>
  <summary>Details</summary>
Motivation: 解释在真实神经元约束下，神经网络如何实现有效学习的问题。

Method: 引入一种新的神经网络架构，其中神经元的激活和学习信号都由非负活动表示，突触具有固定的符号，同时支持类似反向传播的学习。

Result: 理论上证明了学习方案可以精确恢复反向传播更新，实际应用中在Tiny ImageNet基准测试上取得了显著优势。

Conclusion: 有效学习可以来自生物合理的机制，而不需要混合符号信号，为更真实的神经计算模型提供了一步。
...
### [200] [Finding Usable Weight Mechanisms with Tiled SVD](https://arxiv.org/abs/2608.06969)
*Ash Manvi,Samreena Tajreen*

Main category: cs.AI

TL;DR: 提出了一种新的机制提取方法，并进行了实验验证。


<details>
  <summary>Details</summary>
Motivation: 目前主流的机制可解释性方法是通过训练代理字典，如稀疏自动编码器和从最大激活文本中提取的特征标签。

Method: 提出直接从线性位置提取机制，通过列块SVD实现：每个机制是一个三元组（v,u,σ），读作触发器、写入和强度。

Result: 在Gemma-2-2B上使用WikiText-2（16,384个标记子样本），所有七个线性映射都进行了评分：残差写入（mlp.down, attn.o）在层后RMSNorm后进行A/B/C测试，通过52/52个站点层；其他映射只进行A/B测试。

Conclusion: 我们发布了库代码、语料库构建器、实验入口和单元测试。
...
### [201] [FedLBW: A Loss-Based Weighting Strategy for Federated Learning on Non-IID Data in Wireless Networks](https://arxiv.org/abs/2608.07007)
...
### [202] [ReQuant: Fixed-Grid Discrete Refinement for Post-Training Quantization](https://arxiv.org/abs/2608.07019)
*Yongge Ma,Guoan Wang,Feiyu Wang,Yaoming Li,Qian Zhang,Zihan Yan,Yinjun Han,Tong Yang*

Main category: cs.AI

TL;DR: ReQuant是一种改进PTQ的固定网格细化方法，显著提升了量化模型性能。


<details>
  <summary>Details</summary>
Motivation: 为了在产生可执行的量化模型后保持量化权重的可改进性，在PTQ中引入了补充优化阶段。

Method: ReQuant，一种无反向传播的固定网格细化过程，用于该阶段。

Result: 实验表明，ReQuant在多样化的模型家族、位宽和下游任务中，从异构的PTQ初始化器中一致地提高了量化模型，尤其是在简单的初始化器和较低的位宽上取得了显著收益。

Conclusion: ReQuant作为现有PTQ管道的实用补充阶段，可进一步改进。
...
### [203] [ZIPBrain: Can EEG Foundation Models Be Faster, Locally Deployable, but Accurate?](https://arxiv.org/abs/2608.07033)
*Lingwei Li,Yirong Kan,Peng Chen,Xu Cao,Zheng Chen,Yasuhiko Nakashima*

Main category: cs.AI

TL;DR: ZIPBrain：一种提高EEG基础模型性能和可部署性的新方法。


<details>
  <summary>Details</summary>
Motivation: 提高EEG基础模型的性能和可部署性，降低计算负担，同时保持准确性。

Method: 提出ZIPBrain，一种新的冗余感知EEG标记池化模块，利用低信噪比特征减少标记数量。

Result: ZIPBrain在多个EEG基础模型上表现出强大的通用性，平均提高1.3%-10.5%，同时将推理时间缩短32.7%（使用CUDA Graph可达41.8%）。

Conclusion: ZIPBrain是一个有效的解决方案，可以提高EEG基础模型的性能和可部署性，同时保持准确性。
...
### [204] [Not All Problems Are Best Modeled as MILP: A DSL-Centric Framework for Flexible and Accurate Optimization Modeling](https://arxiv.org/abs/2608.07040)
*Shaofeng Zhang,Hongyuan Su,Qingwen Peng,Zefang Zong,Shengcai Liu,Ke Tang,Yong Li*

Main category: cs.AI

TL;DR: OptiDSL significantly improves the formulation accuracy and reduces modeling time compared to MILP-based pipelines.


<details>
  <summary>Details</summary>
Motivation: Recent works have leveraged LLMs to automate optimization modeling, but current frameworks predominantly rely on a rigid MILP paradigm, which can induce prohibitive modeling complexity and severely restrict solver flexibility.

Method: Propose OptiDSL, a framework that shifts the focus from rigid MILP formulations to domain-specific language (DSL) representations. Utilize LLMs to map natural language onto standardized, domain-accepted structures. Decouple problem formulation from execution. Enable seamless integration with a diverse library of specialized solvers.

Result: Experimental results on the comprehensive benchmark of 44 COP types show that OptiDSL significantly surpasses MILP-based pipelines, yielding a 51.66% gain in formulation accuracy and a 91.71% decrease in modeling time. Also outperforms MILP-based pipelines on the existing benchmark, achieving a 23.09% higher formulation accuracy.

Conclusion: OptiDSL significantly improves the formulation accuracy and reduces modeling time compared to MILP-based pipelines, and outperforms them on the existing benchmark.
...
### [205] [Unsupervised Adaptation of PDE Foundation Models](https://arxiv.org/abs/2608.07053)
*Ziye Song,Zhao Wei,Xin Yu,Ivor Tsang,Yueming Lyu*

Main category: cs.AI

TL;DR: 提出了一种无监督PDE微调框架，无需真实解即可实现高性能。


<details>
  <summary>Details</summary>
Motivation: 针对预训练PDE模型在未见过的PDE系统上适应性问题，提出了一种无监督的PDE微调框架。

Method: 预训练一个基于邻域注意力的Transformer，在多样化的时间依赖性PDE上进行训练，以产生跨异构方程的可迁移表示。在适应阶段，使用PDE残差和边界条件构建基于物理的目标函数，并通过低秩适应（LoRA）在未见过的方程上微调模型。为解决标准LoRA在物理量学习上的不均匀性，引入了NSLoRA，这是一种牛顿-舒尔茨正交化的变体，用于重新平衡适应。

Result: 该方法在无需任何真实解的情况下，实现了与监督LoRA微调相当的性能，并在多个空间维度的异构PDE基准测试中，始终优于竞争性神经网络算子基线和最近的PDE基础模型。

Conclusion: 提出了一种无需真实解的无监督PDE微调框架，有效解决了预训练PDE模型在未见过的PDE系统上的适应性问题。
...
### [206] [BONSAI: Evolvability-Guided Tree Search over Skills](https://arxiv.org/abs/2608.07056)
*Yash Priya Shastri,Anand Eswaran,Adnan Qidwai,Pankaj Thorat,Sachin Joshi*

Main category: cs.AI

TL;DR: BONSAI通过可进化性优化技能，显著提升了技能准确率。


<details>
  <summary>Details</summary>
Motivation: 优化技能需要优化文本，但传统的评分方法存在局限性。

Method: 提出了一种名为BONSAI的新技能优化框架，该框架通过可进化性来引导优化过程。

Result: BONSAI在三个基准测试中提升了技能无代理的准确率，并优于两个基线模型。

Conclusion: BONSAI是一种有效的技能优化框架，可以显著提高技能的准确率。
...
### [207] [PTQ4SNN: Membrane-Aware Post-Training Quantization for Spiking Neural Networks](https://arxiv.org/abs/2608.07066)
*Hui Xie,Tong Shi,Haotong Qin,Aishan Liu,Xiaode Liu,Jinyang Guo*

Main category: cs.AI

TL;DR: PTQ4SNN是一种有效的低比特部署框架，能够有效保留模型精度。


<details>
  <summary>Details</summary>
Motivation: 由于突触神经网络（SNNs）的低比特部署不完整，因此提出了PTQ4SNN框架。

Method: PTQ4SNN框架使用统一的比例桥和混合精度位分配对权重和递归膜状态进行量化。

Result: 实验表明，PTQ4SNN在W4量化下有效保留了模型精度，并在大约4位膜精度下实现。

Conclusion: PTQ4SNN是一个有效的低比特部署框架，能够保留模型精度。
...
### [208] [DocMemo: Dynamic Evidence Discovery via Probabilistic Memory-Guided Retrieval for Multi-Modal Document Understanding](https://arxiv.org/abs/2608.07067)
*Hanshu Yao,Janfeng Zhong,Niu Lian,Jinpeng Wang*

Main category: cs.AI

TL;DR: DocMemo：一种基于内存引导的长文档理解框架，通过动态证据探索和结构记忆实现高效的长文档推理。


<details>
  <summary>Details</summary>
Motivation: 针对长文档理解中现有系统在静态检索和跨轮记忆方面的局限性，提出了一种名为DocMemo的内存引导框架。

Method: 提出了一种名为DocMemo的内存引导框架，该框架将长文档推理建模为动态证据探索。DocMemo维护一个包含文档架构记忆、页面信念记忆和问题情景记忆的三级检索状态，分别捕捉结构先验、动态相关性估计和查询特定推理轨迹。在推理过程中，DocMemo通过贝叶斯页面信念更新、空间邻近传播和结构感知自适应粒度证据访问来不断细化跨轮页面选择，同时补充页面级证据以细粒度视觉区域。

Result: 在3个基准测试中，DocMemo实现了最先进的性能，并验证了结构记忆和动态页面信念更新的有效性。

Conclusion: DocMemo在长文档理解方面取得了显著的性能提升，为长文档推理提供了新的思路和方法。
...
TL;DR: MemOPD通过记忆对齐的在线策略蒸馏提高了长周期智能体的性能和稳定性。


<details>
  <summary>Details</summary>
Motivation: 为了解决长周期智能体在交互过程中积累的上下文信息过多，导致性能和稳定性下降的问题，提出了一种名为Memory-Aligned On-Policy Distillation（MemOPD）的方法。

Method: MemOPD记录了每个模型调用的输入和采样输出，恢复其原始标记位置和因果可见性，并将重构的调用打包以进行高效的教师评分。教师提供全词汇监督，而PPO保持最终任务目标。

Result: 实验验证了跨多个上下文更新之间的状态一致性，并显示与匹配控制中的持久历史教师评分相比，F1提高了7.0%。MemOPD-3B将F1与PPO相比提高了高达416.2%，同时打包在训练期间演员计算中实现了高达1.63x的速度提升。

Conclusion: MemOPD通过确保教师评分与学生的状态保持一致，提高了长周期智能体的性能和稳定性。
...
### [210] [Recipes for Creativity: Iterative Generation and Evaluation in Large Language Models](https://arxiv.org/abs/2608.07243)
*Rens Anderson,Tessa Verhoef,Amirhossein Zohrehvand*

Main category: cs.AI

TL;DR: 通过迭代搜索可以提高LLM的创造力，评估者设计至关重要


<details>
  <summary>Details</summary>
Motivation: 研究如何通过迭代搜索来提高大型语言模型（LLM）的创造力

Method: 通过将FunSearch应用于食谱生成，并使用TTCT作为LLM评估方法来评估输出，进行两个实验，测试迭代次数、生成器温度和在循环中选择评分模型的大小

Result: 迭代生成-选择可以产生与人类基准相当的食谱创造力评分，但额外的迭代本身并不提高创造力。在循环中的评估者最为重要：较小的选择评分者在大多数TTCT维度上产生显著更高的分数，而温度的影响有限，仅限于原创性

Conclusion: 评估者设计是主观创造性搜索中的第一级设计变量
...
### [211] [Transformers Struggle to Use Their Emergent World Models: Revisiting the Tower of Hanoi, and the Illusion of Thinking](https://arxiv.org/abs/2608.07077)
...
### [212] [MemWM: Memory-Augmented Text-Based World Model](https://arxiv.org/abs/2608.07107)
...
TL;DR: MemWM通过引入世界记忆，提高了智能体的任务成功率


<details>
  <summary>Details</summary>
Motivation: 解决系统预测错误的问题

Method: 引入MemWM，一个基于记忆的文本世界模型，使用世界记忆来预测环境状态

Result: 在ALFWorld，WebShop和ScienceWorld上，记忆增强的智能体在下游成功方面比SFT训练的世界模型智能体提高了65.4%的相对增益

Conclusion: MemWM在保持预测准确性的同时，提高了智能体的任务成功率
...
TL;DR: 本文提出GeoBenchLLM，用于评估大型语言模型在地理相关任务上的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 为了探究大型语言模型在地理相关任务上的泛化能力，作者提出了一种新的基准enchName。

Method: 作者从十二个公开数据集中选取了不同地理相关任务和领域的数据，对一组大型语言模型在地理空间和时间理解方面的表现进行了评估。

Result: 结果表明，推理能力和规模对整体性能有显著影响。

Conclusion: GeoBenchLLM是一个公开的基准，可以用于评估大型语言模型在地理相关任务上的表现。
...
### [214] [How Much, Then Where: Credit-Conserving Action-to-Token Allocation for Multi-Turn Agent Reinforcement Learning](https://arxiv.org/abs/2608.07118)
...
TL;DR: FACTOR通过分离信用分配决策，提高了多轮智能体强化学习中的信用分配效率。


<details>
  <summary>Details</summary>
Motivation: 在多轮智能体强化学习中，信用分配在两个层面上进行：将轨迹级别的信用分配给动作，并将每个动作的信用分配到其标记上。

Method: 提出了一种名为FACTOR的方法，该方法将这两个决策分开。FACTOR使用检查点校准的TD残差来分配每个动作的信用，这些信用会缩小到轨迹优势，并使用反馈条件下的教师-学生似然差距来将每个信用分配到实现的动作标记上。动作归一化保留了动作平均系数并防止了标记级别的符号翻转。该方法与动作均值减少相结合，消除了动作的标量代理权重对其标记长度的隐式依赖。在行为策略和剪辑之前，每个动作的内动作均值代理等于其TD信用。

Result: FACTOR在ALFWorld、WebShop和ScienceWorld上始终优于竞争基线，每个环境-种子比较都倾向于FACTOR，在最长时域环境中取得了最大的收益。相同的超参数无需重新调整即可转移到更大的骨干网络和不同的模型家族。消融实验表明，TD动作信用是改进的主要驱动因素，而事后标记分配贡献了补充收益。

Conclusion: FACTOR方法在多轮智能体强化学习中的信用分配方面取得了显著成果，提高了学习效率和性能。
...
TL;DR: ResidencyRL通过模拟多轮临床接触训练临床AI代理，提高了诊断准确性和管理质量，为临床掌握铺平了道路。


<details>
  <summary>Details</summary>
Motivation: 在医学教育中，医生通过住院医师培训将学术知识转化为临床技能，但临床推理主要依赖于患者接触，而大型语言模型在静态医学基准上表现出色，但优化临床决策的方法仍不完善。

Method: 提出了一种名为ResidencyRL的强化学习方法，通过模拟多轮临床接触（每条轨迹最多60轮对话和8次工具调用）来训练临床人工智能（AI）代理。ResidencyRL将策略代理与能够进行复杂、对抗性行为的LLM模拟器配对，以结构化奖励对诊断准确性、管理质量、沟通、文档和安全进行训练。

Result: ResidencyRL代理在对抗条件下提高了7.0%的诊断准确性（88.0%对81.0%），并减少了31%的漏诊红旗率，证明了提前关闭的严格缓解。专家临床医生盲法验证了这些收益，87.6%的情况下更喜欢训练后的代理。程序能力转移到未见过的基准：代理在AMIE多访问基准的所有六个临床轴上优于基线模型，并在AgentClinic和CRAFT-MD上显示出一致的改进方向。

Conclusion: 我们的发现表明，通过模拟中的多轮强化学习可以有效地学习连续的临床决策，从而产生稳健、可推广的能力，为临床掌握铺平了道路。
...
### [216] [DiDPO: Diff-in-Diff Policy Optimization for Coding Agent Training](https://arxiv.org/abs/2608.07147)
*Xucong Wang,Zhe Zhao,Liheng Yu,Di Wu,Xiaofeng Cao,Pengkun Wang*

Main category: cs.AI

TL;DR: DiDPO通过深入代码差异，实现了编码代理训练中的细粒度信用分配，显著提升了编码代理的性能。


<details>
  <summary>Details</summary>
Motivation: 针对编码代理面临的独特且细致的信用分配挑战，现有强化学习可验证奖励方法无法深入代码差异，使编码动作的独特属性对训练不可见。

Method: 提出Diff-in-Diff Policy Optimization (DiDPO)，一种无批评的强化学习方法，直接从代码差异的结构中构建细粒度信用单位。DiDPO将多轮编码交互组织成多个思维-行动步骤，并发现样本轨迹中的代码差异。然后通过聚合来自每个整个差异的具有高度相似性的子差异来选择锚点，并提供最佳平衡锚点语义范围和可能形成的群体质量的分割方案。最后，这些锚点形成优势组，并将差异级别的优势投影回单个响应标记。

Result: 在长期编码和推理基准测试中，DiDPO显著优于强大的代理强化学习基线。在Qwen2.5-7B-Coder上，DiDPO比可比方法高出10%以上，并缩小了与更大模型的差距，为编码代理训练中的细粒度信用分配提供了一个原则性框架。

Conclusion: DiDPO在编码代理训练中的细粒度信用分配方面表现出色，为强化学习在编码领域的应用提供了新的思路。
...
### [217] [A MARL Centered Reference Architecture for Large Language Model Augmentation in Smart Manufacturing](https://arxiv.org/abs/2608.07148)
...
TL;DR: Niyam-AI通过SHA-256锁定工具和约束，隔离验证工具调用，生成zk-SNARK证明，显著提高自主LLM代理的安全性。


<details>
  <summary>Details</summary>
Motivation: 为了提高自主LLM代理的安全性，防止代理被欺骗执行不当操作。

Method: 提出Niyam-AI框架，通过SHA-256锁定允许的工具和约束，由隔离的Judge模型验证每个工具调用，并通过EZKL生成zk-SNARK证明。

Result: 在2,000个实际场景中，Niyam-AI的F1得分为88.5%，误报率为1.1%，在所有情况下均显著优于NeMo Guardrails、Prompt Guard 2和GPT-OSS-Safeguard。

Conclusion: Niyam-AI提供了一种既准确又可验证的安全防护机制。
...
TL;DR: SkillProx通过闭环诊断和近端细化，显著提升了LLM代理的技能适应性和准确性。


<details>
  <summary>Details</summary>
Motivation: LLM agents通过积累程序性知识在技能中适应重复任务。然而，现有框架缺乏显式的诊断-结果反馈，将删除视为通用的编辑操作，而不是巩固积累知识的专用机制。

Method: SkillProx，一个受近端梯度启发的正向-反向框架，结合了闭环诊断进化和基于效用的近端细化。

Result: 在多个骨干LLM的分布内和分布外基准上，SkillProx将平均准确率提高了3.0个百分点。组件消融证明了闭环诊断和近端细化的互补作用。

Conclusion: SkillProx通过结合闭环诊断和近端细化，有效提高了LLM代理的技能适应性和准确性。
...
### [221] [Agent Memory Distillation: Empowering Small LLM Agents with Hierarchical Teacher Memory](https://arxiv.org/abs/2608.07169)
*Taeil Kim,Kangsan Kim,Sung Ju Hwang*

Main category: cs.AI

TL;DR: AMD 是一种将知识从大型代理传递到小型代理的无训练框架，显著提高了小语言模型的表现。


<details>
  <summary>Details</summary>
Motivation: 提高小语言模型的表现，因为它们在生成足够的成功轨迹方面存在困难。

Method: 提出了一种名为Agent Memory Distillation (AMD) 的无训练框架，通过分层记忆将结构化知识从大型教师代理传递到小型学生代理。

Result: 在三个工具使用基准测试中，使用 GPT-5-mini 作为教师，四个学生模型（4B-8B 参数）实现了平均准确率提升 27.2%p、11.2%p 和 3.4%p，并且始终优于现有的基于记忆的基线。

Conclusion: AMD 在小语言模型中表现出色，特别是在工具使用基准测试中。
...
### [222] [SetEasy: A Multi-Modal Classroom Engagement Assessment and Seating Optimization Framework](https://arxiv.org/abs/2608.07188)
...
### [223] [EMAS: Stabilizing Multi-Agent System Evolution through Evidence-Guided Revision](https://arxiv.org/abs/2608.07196)
*Chao Fei,Qingyi Si,Kaihua Liang,Yanghua Xiao,Panos Kalnis,Hongcheng Guo*

Main category: cs.AI

TL;DR: EMAS通过利用经验来优化多智能体系统，提高了准确性并降低了成本。


<details>
  <summary>Details</summary>
Motivation: 许多自动多智能体系统设计方法在初始设计阶段优化提示和拓扑结构，然后在不更改系统的情况下部署到后续样本。然而，这些样本的经验很少被整合到可重用的系统更新中，而以准确性为导向的设计可能会产生高昂的token成本。

Method: 我们引入了EMAS（演化多智能体系统），它使用这些经验来修改MAS拓扑结构和提示，而无需更新LLM参数，以提高准确性或降低成本。EMAS将跟踪转换为结构化诊断，指定修订操作和目标。它仅在相同的诊断在多个样本中重复出现时才生成候选修订，并且只有在与当前MAS的配对验证满足相应的接受标准时才应用它。

Result: 在四个基准和两种LLM上，EMAS在主干和模型-基准设置中均实现了最高的任务加权总体准确性。在两个演化时代内，EMAS在Kimi-K2-6和Qwen3.6-27B上的任务加权准确性分别提高了6.30%和20.10%。在MBPP上使用Qwen3.6-27B，EMAS将准确性从55.09%提高到89.12%，同时将每个任务的token使用量减少了62.2%。这些结果表明，EMAS可以将新样本的经验转化为可重用的MAS拓扑结构和提示更新。

Conclusion: EMAS能够将新样本的经验转化为可重用的MAS拓扑结构和提示更新，提高了任务加权准确性并降低了token使用量。
...
TL;DR: 开发工具INSPECT-AI，提高RCT研究完整性评估


<details>
  <summary>Details</summary>
Motivation: 提高临床护理指南的证据质量

Method: 开发基于LLM的交互式工具INSPECT-AI，结合INSPECT-SR框架和RIPE-O本体，对已发表RCT的研究完整性进行评估

Result: 生成140个专家对95篇RCT发表的初步研究完整性评估，并使用RIPE-O进行描述

Conclusion: INSPECT-AI有助于提高RCT研究完整性的评估效率和一致性
...
TL;DR: 重复或避免行为是主要信号来源，频率跟踪贡献有限。


<details>
  <summary>Details</summary>
Motivation: 探索透明替代方案是否能够预测人类玩家的行为，并揭示背后的行为结构。

Method: 使用决策数据，比较不同模型，包括可解释机器学习和深度学习模型，以及频率跟踪扩展。

Result: 重复或避免行为是主要信号来源，频率跟踪贡献有限。

Conclusion: 频率跟踪对预测的额外贡献有限，而重复或避免行为是可解释和策略可利用信号的主要来源。
...
TL;DR: 提出了一种基于贝叶斯网络的方法，用于解决概率逻辑程序中因果推理的歧义问题，提高了干预推理的准确性


<details>
  <summary>Details</summary>
Motivation: 探讨概率逻辑程序中因果推理的歧义问题

Method: 利用循环概率逻辑程序与贝叶斯网络之间的关系，推导出唯一确定因果顺序的条件

Result: 提出了一种验证学习到的概率逻辑程序是否支持良好定义的干预语义的方法

Conclusion: 该方法能够解决概率逻辑程序中因果推理的歧义问题，提高了干预推理的准确性
...
TL;DR: 提出了一种新的生成世界导航模型，提高了连续视觉语言导航的性能。


<details>
  <summary>Details</summary>
Motivation: 为了解决现有视觉语言导航（VLN）系统在训练中未能显式地建模代理的视觉观察如何随其预测的运动而演变的问题，以及现有连续VLN的生成世界动作模型（WAMs）未能将联合未来视图和动作生成条件化在从观察历史中推断出的几何感知表示上的问题。

Method: 提出了一种名为WNM-3D的生成世界导航模型，该模型具有3D场景条件，用于连续VLN。该模型使用冻结的前馈几何编码器从单目自旋RGB历史中提取几何感知表示，并使用可训练的3D场景到标记适配器将它们转换为世界动作Diffusion Transformer标记空间中的固定长度前缀。通过块因果注意力，该前缀为未来视频动作块提供条件，为未来视图和动作生成提供共享的几何上下文。

Result: 在GN-Bench上的实验表明，WNM-3D在闭环导航中优于基于VLM的导航策略及其2D条件对应物。在固定的近目标评估集中，WNM-3D也实现了更高的流动动作一致性和更低的视觉运动误差。

Conclusion: WNM-3D通过提供共享的几何上下文，有效地提高了连续VLN的性能。
...
### [228] [Winning by Peeking: Unenforced Budgets and Test-Set Selection Inflate Short-Budget AutoML Comparisons](https://arxiv.org/abs/2608.07303)
*Guilin Zhang,Kai Zhao*

Main category: cs.AI

TL;DR: Orcetra在短时间预算的AutoML比较中表现出色，但结果受到协议缺陷的影响，重新运行实验后，其表现显著下降。


<details>
  <summary>Details</summary>
Motivation: 在工具README和研讨会论文中，对短时间预算（秒级而非小时级）的AutoML系统进行比较很常见，但容易出错。

Method: 通过案例研究，分析了简单AutoML引擎Orcetra在513个OpenML数据集上的表现，并与FLAML和AutoGluon进行了比较。

Result: Orcetra在60秒的预算内赢得了57.1%的数据集，在30秒内赢得了78.4%的数据集。然而，这些结果受到协议缺陷的影响，因为搜索循环在测试分割上对每个候选者进行评分并报告最佳结果，而基线在训练数据上选择并在测试集上仅接触一次。

Conclusion: 重新运行实验，将选择移动到验证分割，外部强制执行截止日期，并将每个框架固定到机器的相等份额，Orcetra的获胜率从59.4%下降到34.3%，与竞争对手的任何成对差异均不再显著。
...
### [229] [An End-to-End Agent Auditing Engine](https://arxiv.org/abs/2608.07346)
*Haoning Wang,Mingxun Zhang,Chenyue Yu,Yingjun Shang,Xia Hu,Guanchu Wang,Na Zou*

Main category: cs.AI

TL;DR: 介绍了 $A^2E$，一个用于评估代理 Harness 的端到端引擎，通过多维指标评估 Harness 能力，揭示了模型-Harness 组合的性能差异


<details>
  <summary>Details</summary>
Motivation: 为了解决高效构建端到端、系统化和全面评估管道的挑战

Method: $A^2E$（代理审计引擎），一个端到端的评估引擎，用于代理 Harness

Result: 通过多维度指标系统评估 Harness 的能力，揭示了不同 Harness 在执行效率、工具使用、任务规划和错误恢复方面的差异，以及模型-Harness 组合在不同任务中的性能变化

Conclusion: 证明了系统评估的必要性，并为模型和 Harness 的协同进化提供了指导
...
### [230] [QFCQT: A Chaotically Gated Quantformer Framework for Volatile Time-Series Forecasting](https://arxiv.org/abs/2608.07363)
*Junkai Lin,Siqi Hou,Raymond Lee*

Main category: cs.AI

TL;DR: QFCQT是一种基于Transformer的预测器，通过结合量子-分形启发的非线性激活和自适应振荡器融合机制，在非平稳时间序列预测中表现出色。


<details>
  <summary>Details</summary>
Motivation: 由于长程依赖性、局部波动爆发、结构变化和非线性振荡行为，预测非平稳时间序列仍然很困难。尽管基于Transformer的预测器在建模长期时间依赖性方面非常有效，但它们的正向块通常依赖于平滑的静态激活，这些激活对急剧的体制变化不够敏感。受定量Transformer设计和基于振荡器的非线性激活的启发，我们提出了QFCQT，即量子-分形启发的混沌门控量子转换器，用于在复杂波动动态下进行稳健预测。

Method: 我们提出了QFCQT，包括三个主要组件：(1)一个Quantformer风格的数值编码器，它通过线性嵌入直接处理多元输入；(2)一个可学习的Lee振荡器激活模块，它将标量预激活映射到动态振荡响应，并通过Max-over-Time池化进行总结；(3)一个平滑-混沌门控融合机制，它自适应地平衡传统平滑激活和混沌敏感响应。此外，QFCQT采用八个参数化的Lee振荡器家族的软叠加，以自适应地捕捉不同体制下的不同非线性响应模式。

Result: 在ETTh1、ETTh2和A股股票指数基准测试中，QFCQT在包括Informer、LogTrans、LSTMa、HAT和COTN在内的强大基线中表现一致优于。

Conclusion: QFCQT在复杂波动动态下表现出色，为非平稳时间序列的预测提供了一种新的方法。
...
### [231] [Curriculum as Code: An AI-Assisted Architecture for Instructional Design in STEM Education](https://arxiv.org/abs/2608.07364)
*Henrique Mohallem Paiva*

Main category: cs.AI

TL;DR: 本文提出了一种基于课程代码范式的六阶段AI辅助教学设计架构，旨在减少教师工作量，提高教学材料的质量和一致性。


<details>
  <summary>Details</summary>
Motivation: 为主动学习创建定制的教学材料给教师带来了沉重的工作负担。标准演示工具缺乏对技术内容的强大支持，而当前的AI应用往往会出现幻觉，并无法正式化教学编写过程，限制了其在严格学术设计中的实用性。

Method: 基于课程代码范式的六阶段AI辅助教学设计架构，结合生成式AI、LaTeX和Python，自动创建可重复、视觉一致、技术精确的材料。

Result: 经过一年的验证，该架构在基于项目的学习环境中，通过8个模块和28个项目情境显著降低了教师工作量。

Conclusion: 该架构显著降低了教师工作量，生成的资源通过独立同行评审，并得到六位不同教师的采用，证明了其可扩展性。材料从8.5到9.9/10的高质量评分，表明其在广泛的STEM教育应用中的可行性。
...
### [232] [People Are Not Just Their Countries. Disentangling Social Determinants of LLM Value Alignment Across Europe](https://arxiv.org/abs/2608.07367)
*Maria-Louisa Wightman,Guillaume Bied,Tijl De Bie*

Main category: cs.AI

TL;DR: LLMs的价值观对齐存在差异，国家和社会人口因素共同影响这种差异。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型（LLMs）在信息和建议方面的应用日益广泛，了解它们在价值观方面与人类的对齐程度成为一个紧迫的问题。

Method: 利用欧洲社会调查，我们通过考虑与10个主要的商业LLMs在15个社会人口变量以及居住国方面的价值观对齐，来填补这一知识空白。

Result: 分析显示，LLMs与不同社会人口群体的价值观对齐程度确实不平等，特别是在教育、收入、职业和宗教方面定义的群体。在考察个体层面的对齐时，受访者所在国家作为一个独立的变量，解释了相当一部分的变异，与考虑的社会人口变量全集合相当。进一步分解国家层面和社会人口因素的作用，我们发现它们在解释价值观对齐模式方面是互补的，它们相对权重在考虑的问题子集中有所不同。

Conclusion: LLMs与不同社会人口群体的价值观对齐程度存在差异，国家层面和社会人口因素在解释这种差异方面起着互补作用。
...
### [233] [FinRank: An Evidence-Grounded Benchmark for Financial Question Answering and Retrieval over SEC Filings](https://arxiv.org/abs/2608.07400)
*Sasan Mansouri,Daniel Saad,Mark Wahrenburg,Manu Weissel,Fabian Woebbeking*

Main category: cs.AI

TL;DR: FinRank为财务问答提供了一种基于证据的基准


<details>
  <summary>Details</summary>
Motivation: 针对财务问答中证据来源敏感的检索问题

Method: FinRank方法，要求系统识别目标实体、报告期间和披露背景下的证据

Result: 基准测试结果表明，在证据集合上，即使7B指令微调的嵌入器也只达到44.8%的Recall@10；低于十亿参数的编码器最多比BM25高3.5分；金融适应的嵌入器比BM25低9.7分；将随机负面示例替换为手工定制的负面示例时，成对准确率下降13.0-20.5个百分点

Conclusion: FinRank为开发不仅准确而且基于正确披露的财务问答系统提供了一个证据优先的基准
...
### [234] [CoBa: Cost-Effective Test-Time Scaling via Compute-Balanced Routing](https://arxiv.org/abs/2608.07424)
...
### [235] [A Picture is Worth a Thousand Tokens: How Vision Language Models Cut AI Energy Costs While Improving Accuracy](https://arxiv.org/abs/2608.07427)
*Bhavika Jalli,Nikhil Korati Prasanna,Jayanta Choudhury*

Main category: cs.AI

TL;DR: VLM通过减少输入令牌数量，显著降低了电信网络分析和NTSDA的能耗，同时保持了准确性。


<details>
  <summary>Details</summary>
Motivation: 电信网络分析和数值时间序列数据分析（NTSDA）中，LLM推理占AI操作能耗的90%以上，与输入令牌数量成比例，这是一个关键的低效问题。

Method: 使用视觉语言模型（VLM）将时间序列编码为二维图，以减少输入令牌数量，从而降低能耗。

Result: VLM在降低能耗的同时，保持了准确性，并在电信异常检测等任务上优于基线模型。

Conclusion: VLM是数值时间序列工作负载中一种节能且精度更高的模式，为将能耗视为一级工程约束的AI推理系统提供了实证基础。
...
### [236] [TEPA: Revoking Stale Memories for Conflict-Robust Language Agents](https://arxiv.org/abs/2608.07429)
*Yan Zhou,Yue Ouyang,Kaiyang Zheng,Suncheng Xiang*

Main category: cs.AI

TL;DR: TEPA通过撤销机制解决长期记忆中的可证伪性问题，提高了记忆的准确性和可靠性。


<details>
  <summary>Details</summary>
Motivation: 长期记忆使语言代理能够重用过去的事实、偏好和任务经验。持久性也产生一个中心的可证伪性问题：当世界发生变化时，过时的记忆可以保持可检索并污染提示。

Method: 引入TEPA，一种可撤销的证据-记忆机制，使有效性成为记忆的显式状态。TEPA将观察结果表示为键控先例，并在同一键下有新证据与之矛盾时撤销活动先例，允许检索从当前证据中提取，同时保留撤销的历史以供审计。

Result: 在控制隐藏制度漂移、真实文件支持的可执行漂移和偏好更新流中，撤销防止了过时的活动记忆在回滚后仍然保留在检索集中。在50个种子的控制漂移中，追加和最后写入获胜的内存低于没有内存（追加和最后写入获胜均为0.210，没有内存为0.309，TEPA为0.950），并且在真实文件执行下重复了相同的模式（追加为0.203，没有内存为0.298，TEPA为0.950）。在干净的记忆代理Bench SH-6k上，TEPA与强大的最后写入获胜缓存相匹配，证实了当前键替换是单跳事实整合的决定性操作。在多跳和非常长的上下文记忆代理Bench设置上的边界测试揭示了检索链和上下文选择瓶颈，这些瓶颈超出了事实级有效性的跟踪。这些结果共同确立了生命周期撤销作为必须证伪、审计和随后重新推广不断发展的知识的代理的核心记忆操作。

Conclusion: TEPA通过生命周期撤销机制有效地解决了长期记忆中的可证伪性问题，提高了记忆的准确性和可靠性。
...
### [237] [Post-Grokking Collapse at the Representation-Readout Interface in Muon-Trained Transformers](https://arxiv.org/abs/2608.07436)
...
### [238] [Fisher-R1: Training LLM Agents for Reliable Hypothesis Testing](https://arxiv.org/abs/2608.07437)
*Jiacheng Miao,Jin Mu,Guanhua Chen,James Zou*

Main category: cs.AI

TL;DR: P-Bench基准和Fisher-R1模型提高了LLM在假设检验中的可靠性。


<details>
  <summary>Details</summary>
Motivation: 提高假设检验的可靠性，应对大型语言模型在假设检验中出现的推理错误。

Method: 构建P-Bench基准，训练Fisher-R1模型。

Result: Fisher-R1在P-Bench基准上优于基线模型，提高了假设检验的可靠性。

Conclusion: 当前LLM在假设检验中缺乏可靠的统计推理能力，但通过强化学习训练可以显著提高可靠性。
...
### [239] [Blast Radius](https://arxiv.org/abs/2608.07440)
*MY Pitsane,Hope Mogale*

Main category: cs.AI

TL;DR: Blast Radius通过预测性内存管理减少了代理编码的令牌消耗，提高了效率，并有助于提高大型语言模型的可重用性和可持续性。


<details>
  <summary>Details</summary>
Motivation: 解决代理编码在可负担性和浪费令牌方面面临的问题

Method: 引入Blast Radius，这是一种预测性内存管理层，通过耦合上下文和代码通道来估计传入提示的覆盖范围。NECROPHORESIS实现可逆的驱逐，通过存档已死亡上下文来记录，而RDM识别并埋葬重复出现的转写。在波兰上下文空间上对可逆上下文驱逐进行公式化，将上下文熵与复活概率联系起来。

Result: 在七个OpenAI模型上，Blast Radius将令牌消耗量减少了17-26%，在测试的政策中实现了最低的溢出率，并保持了字节级可逆。在450个埋葬的尸体中，378个是重复出现的死亡物质，没有一个是被召回的。Blast Radius在HCRC之下运行，确定要埋葬哪些记录以及传入提示可以进入代码库多远。

Conclusion: 这项工作有助于Algosophy的更广泛目标：使大型语言模型和代理编码更具可重用性和可持续性。
...
### [240] [Interaction Creates Dynamical AI Behavior Absent in Isolation](https://arxiv.org/abs/2608.07457)
*Bella Xinrui Li,Frank Yingjie Huo,Neil F Johnson*

Main category: cs.AI

TL;DR: AI之间的互动可能导致非平衡动力学状态，为物理学研究提供新方向。


<details>
  <summary>Details</summary>
Motivation: 研究AI在日常生活中的互动，特别是当其中一个AI开始指挥另一个AI时会发生什么。

Method: 通过实验和理论分析来研究AI之间的相互作用。

Result: 发现了一个反直觉的答案，这为非平衡物理学开辟了新的途径。当上司AI向下属AI发送一系列消息，同时忽略其回复时，会将下属AI驱入一种它单独时永远不会表现出的异质行为状态。下属AI不会模仿其上司或恢复到它自己的行为；相反，它采取了一种完全不同的行为。上司AI的附加价值类似于预录磁带。当上司AI倾听时，他们两者都会采用类似的异质动力学状态。简单的动力理论可以捕捉主要效应，例如为什么在未来的AI-AI交互中，传达相同消息的方式将很重要。

Conclusion: AI之间的相互作用可以导致非平衡动力学状态，这为物理学提供了新的研究方向。
...
### [241] [Robust Average-Reward Markov Decision Processes: Minimax-Optimal Learning via Plug-in Reductions](https://arxiv.org/abs/2608.06545)
*Yuepeng Yang,Yuxin Chen,Yuejie Chi*

Main category: cs.LG

TL;DR: 研究了在模型不确定性下学习鲁棒策略所需的样本数量，并提出了基于减少的插件程序来获得这些比率。


<details>
  <summary>Details</summary>
Motivation: 研究在模型不确定性下，根据平均奖励标准学习ε-最优鲁棒策略所需的样本数量。

Method: 使用生成模型从名义转移核中获取样本，并在半径最多为σ的(s,a)矩形总变分不确定性集中评估策略性能。

Result: 确定了σH0作为区分高容忍度和低容忍度机制的扰动尺度。通过匹配上下界，表明在对数因子内，最小化总样本复杂度为NSA ≈ SA/ε^2，其中S和A分别是状态和动作的数量，N是每个状态-动作对的样本数。样本复杂度由一个类似于名义AMD结果的一次性跨度项和一个仅在低容忍度机制中出现的鲁棒性特定项组成。

Conclusion: 通过基于减少的插件程序获得这些比率，这些程序选择减少——名义或鲁棒——及其折现因子：一个跨度信息程序，它使用已知的跨度参数进行选择，以及一个跨度无感知程序，它从数据中校准这两个选择。
...
TL;DR: OmniDecVAEs是一种高效的多模态可穿戴计算模型，能够提高识别准确率和数据重建质量，适用于智能边缘设备和临床医疗保健。


<details>
  <summary>Details</summary>
Motivation: 多模态可穿戴计算中学习解耦表示是开发多功能、通用和可持续模型的关键要求。然而，现有的方法并不是全栈可穿戴处理器，即它们不能同时解决特定任务分类性能、解耦和可解释表示学习、融合以及高度异构多模态时间序列的生成建模。

Method: 提出了一种名为OmniDecVAEs的框架，该框架通过多视角自监督分解损失和共享非对称自动编码器（AE）架构，有效地从任意许多模态中统一和可扩展地学习多用途表示。

Result: 在具有多达三十种模态的具有挑战性的全模态人类活动识别（HAR）设置中，结果表明OmniDecVAEs能够学习全栈可穿戴表示。与基于transformer和基于VAE的方法相比，OmniDecVAEs的全栈解耦表示属性在活动和身份识别中的准确率分别提高了1.01%和6.75%。此外，OmniDecVAEs合成了真实的全模态时间频率数据，其重建效果得到增强（均方误差降低了76.84%），并且真实和合成数据之间的分布相似性也有所提高（最大平均差异提高了13.85%）。结果表明OmniDecVAEs具有作为轻量级模型的潜力，适用于智能边缘可穿戴设备和临床医疗保健，通过其增强的表示能力、模态不变的空间复杂度（410万个参数）和实时延迟，将处理需求和能力统一在单个模型中。

Conclusion: OmniDecVAEs在多模态可穿戴计算中表现出强大的学习和处理能力，对于智能边缘可穿戴设备和临床医疗保健领域具有潜在的应用价值。
...
### [243] [Latent Fact-Checking: Detecting Misinformation through Activation Engineering](https://arxiv.org/abs/2608.06417)
*Pedro Barcelos,Otávio Parraga,Marcelo M. Mussi,Lucas M. Fraga,Lucas S. Kupssinskü,Rodrigo C. Barros*

Main category: cs.LG

TL;DR: 提出了一种基于transformer模型潜在几何形状的信息虚假检测方法，在多个基准上取得了良好的效果。


<details>
  <summary>Details</summary>
Motivation: 在线虚假信息的泛滥推动了可扩展检测系统的需求。

Method: 引入基于激活工程的信息虚假检测框架，利用transformer模型的潜在几何形状。通过对比真实和虚假语句的激活来激发虚假信息方向，并将未见过的声明激活投影到该方向，然后将其输入到多层感知器（MLP）进行分类。

Result: 在11个模型上评估该方法，包括Gemma、Llama和Qwen系列，参数范围从270M到12B。在三个事实核查基准上表现良好，其中在较小的模型上观察到最大增益。在AVeriTeC上的性能较为有限，这归因于其基于证据的标签方案。

Conclusion: 证明真实性是预训练语言模型潜在空间中结构化的、线性可分的概念，并指出了基于可解释性的虚假信息检测作为基于检索的管道的实用补充。
...
### [244] [Risk-Aware Decision Policies for Agents Under Noisy Perception](https://arxiv.org/abs/2608.06420)
*David Szczecina*

Main category: cs.LG

TL;DR: 研究通过人工生命模型展示了在噪声感知下，意识到的决策制定对生存的重要性。


<details>
  <summary>Details</summary>
Motivation: 生物系统中的感知存在固有噪声，需要生物体在不确定的情况下做出决策，误分类可能导致损失或致命。

Method: 提出了一种人工生命捕食者-猎物模型，在噪声感知下进行觅食，并比较了使用考虑其噪声预测的各种策略的代理性能。通过在对称和非对称感知噪声下的控制实验，我们展示了盲目信任感知标签会导致随着噪声的增加而灾难性的失败，而意识到的策略显著提高了生存率并减少了致命错误。

Result: 我们发现，随着不确定性的增加，代理从探索性策略转变为保守策略。我们的模型通过显示明确的信息收集可以在感知不可靠时提高鲁棒性，将风险敏感的觅食、生态信息利用和人工生命联系起来。这些结果突出了意识到的决策制定的重要性，并为具有噪声标签的鲁棒学习提供了一个可解释的人工生命类似物。

Conclusion: 不确定性意识决策的重要性，以及具有噪声标签的鲁棒学习的人工生命类似物。
...
### [245] [Sharding Prevents LLM Oversight Failures and Adversarial Exploitation](https://arxiv.org/abs/2608.06422)
...
### [246] [Adversarial Causal Intervention Falsification](https://arxiv.org/abs/2608.06427)
*Mojtaba Eslami*

Main category: cs.LG

TL;DR: 提出对抗性因果干预伪造方法，证明其在有限模型和干预类别下的性质，为因果生成建模、主动因果发现和实验设计之间建立桥梁。


<details>
  <summary>Details</summary>
Motivation: 研究生成模型在复制观测分布的同时编码错误的因果结构。

Method: 提出对抗性因果干预伪造（ACIF）方法，并证明其在有限模型和干预类别下的性质。

Result: 证明了对抗性目标可以精确地减少到最坏干预积分概率度量，实现了干预等价性识别，并给出了有限样本一致收敛和基于边界的模型选择保证。

Conclusion: 该框架为对抗性因果判别器可以和不能证明的内容提供了清晰的说明，并在因果生成建模、主动因果发现和实验设计之间建立了原则性的桥梁。
...
### [247] [Fixed and Adaptive Topological DeepONets: Functional Measurements on Hausdorff Locally Convex Spaces](https://arxiv.org/abs/2608.06428)
*Khemraj Shukla,George Em Karniadakis*

Main category: cs.LG

TL;DR: 提出了一种新的深度ONet框架，在多个任务上表现出色，尤其是在非范数输入空间中。


<details>
  <summary>Details</summary>
Motivation: 基于Ismailov的Topological DeepONet框架，将点样本替换为连续线性泛函，并开发固定和自适应功能测量系统。

Method: 使用连续对偶Hausdorff局部凸空间中的连续线性泛函，结合Lee和Shin的系数空间两步法，并引入训练仅解码器和正则化来稳定自适应坐标。

Result: 在反导算子、非范数局部凸输入空间、异构达西流、控制算子以及固定时间和时间演化的Navier-Stokes涡流算子等任务上进行了评估。在异构达西问题中，功能模型在未见过的网格上保持几乎分辨率无关的误差为5.5-5.6%。在控制问题中，自适应测量将平均误差降低到1.2%以下。在固定时间的Navier-Stokes问题中，自适应拓扑深度ONet是最准确的深度ONet模型，达到平均相对L^2误差为1.685% +/- 0.017%。

Conclusion: 该框架提供了一种紧凑、可解释且可离散化的坐标系统，适用于非范数输入空间。
...
TL;DR: MiGHT-EHR通过统一表示学习方法，在EHRs上实现了临床预测的显著改进。


<details>
  <summary>Details</summary>
Motivation: 由于电子健康记录（EHRs）具有改善临床预测的潜力，因此从EHRs中进行学习已引起广泛关注。然而，由于EHRs编码了异构的、按时间顺序排列的临床交互，有效的学习仍然具有挑战性。

Method: 我们提出了多任务图变换器（MiGHT-EHR），它通过统一表示学习方法共同建模这三个属性。MiGHT-EHR从EHRs中构建一个异构图，其中节点表示临床实体，边通过归一化点互信息识别的统计相关实体连接。

Result: 在MIMIC-III和MIMIC-IV数据集上，MiGHT-EHR在四个任务上平均优于最先进的方法：药物推荐、住院时间预测、死亡率和再入院率，尤其是在死亡率和再入院率预测方面有显著提高。

Conclusion: 这些发现表明，MiGHT-EHR表示支持多样化的预测任务，同时保留临床可解释的结构。
...
### [249] [SNI-GNN: SmartNIC-Assisted Full-Graph GNN Training with In-Network Embedding Prediction](https://arxiv.org/abs/2608.06441)
*Guofan Yu,Sitian Chen,Zhenheng Tang,Xiaowen Chu,Amelie Chi Zhou*

Main category: cs.LG

TL;DR: SNI-GNN通过预测网络中的远程嵌入来减少通信，同时保持全图GNN训练的准确性，显著提高了扩展性和性能。


<details>
  <summary>Details</summary>
Motivation: Full-graph GNN training在多服务器集群上的扩展性差，因为节点间嵌入交换过于沉重且不规则。

Method: SNI-GNN，一个SmartNIC辅助的全图训练系统，通过在网络中预测远程嵌入来减少通信，同时保持准确性。它使用轻量级的线性趋势预测器来优化缓存的历史嵌入，并结合基于重要性的边界节点采样策略和异步DPU-GPU数据管道，其中包含中间结果的重复使用。

Result: SNI-GNN在NVIDIA BlueField-3上实现，与最先进的全图系统集成，减少了21-45%的通信，在BNS-GCN上实现了1.3-3.6倍的端到端加速，在SANCUS基线上的加速达到1.29倍，准确率损失≤0.01，并且能够高效地扩展到具有数千万条边的图上的16个GPU。

Conclusion: 基于SmartNIC的网络预测是通信高效的缩放全图GNN训练的实用补充。
...
### [250] [ED-CSP: Crystal Structure Prediction from Electron Diffraction](https://arxiv.org/abs/2608.06448)
*Germain Poloudenny,Yaël Frégier,Arnaud Demortière*

Main category: cs.LG

TL;DR: ED-CSP和ED-CS从稀疏ED观测中预测晶体结构，优于现有模型。


<details>
  <summary>Details</summary>
Motivation: 从稀疏、未索引的电子衍射（ED）观测中恢复周期性3D晶体结构是一个具有挑战性的生成性反问题。

Method: 引入了一种机器学习框架ED-CSP，从化学成分、原子计数和多个检测平面电子衍射点集中预测晶体结构。ED-CSP结合了关系集合编码器、置换不变的多视图聚合和周期性流程生成器来联合预测晶格参数和分数原子坐标。

Result: 在2,075个保留的CHILI-100K材料上，仅用CHILI训练的ED-CSP达到了57.49%的结构匹配率MR@5，超过了PXRDGen（52.92%），一种基于粉末X射线衍射的当前最先进的晶体结构预测模型。进一步扩大训练数据提高了性能：从一百万结构的先验开始初始化将MR@5提高到66.27%。

Conclusion: ED-CSP和ED-CS为从稀疏电子衍射观测中生成晶体结构预测提供了一个基准，并为进一步将实验数据迁移到提供了基础。
...
### [251] [Beyond Attention: Signed Integrated Gradients Attribution in a BiomeGPT-Style Microbiome Transformer](https://arxiv.org/abs/2608.06486)
*Oren Nelson*

Main category: cs.LG

TL;DR: 提出了一种基于集成梯度和二次积分Hessian的方法，用于解释特征标记的Transformer模型，如BiomeGPT，以揭示物种丰富度的影响和微生物群落相互作用规则。


<details>
  <summary>Details</summary>
Motivation: 解释下游分类在特征标记的Transformer模型中的意义，如BiomeGPT。

Method: 使用集成梯度（Integrated Gradients）和二次积分Hessian，提出源衍生基线T' = S + A_0，以揭示物种丰富度的影响。

Result: 该方法可以明确区分病原微生物和保护性微生物信号，揭示物种-丰富度方向关系和敏感性诊断，并可用于解释BiomeGPT等模型的可解释性。

Conclusion: 该方法为特征标记的Transformer模型提供了可解释性的原则性方法，具有普遍性。
...
### [252] [Toward Reliable Context Compression for Long-Horizon Agents: An Empirical Study of Execution Instability](https://arxiv.org/abs/2608.06503)
*Guanghui Min,Liang Wu,Mayank Darbari,Chen Chen,Liangjie Hong*

Main category: cs.LG

TL;DR: 该研究提出了TRACE框架，通过评估压缩事件和优化压缩提示，在AppWorld上取得了比现有基线更好的结果。


<details>
  <summary>Details</summary>
Motivation: 为了解决长期智能体中上下文增长控制的问题，以及压缩行为效应的不明确性。

Method: 提出TRACE框架，通过成对闭环连续性评估压缩事件，并使用总结偏好优化自然语言压缩提示。

Result: 在AppWorld上的初步结果表明，与现有压缩基线相比，在任务性能、多运行可靠性和上下文执行效率方面有所提高。

Conclusion: 这些发现为边界局部评估作为可靠智能体上下文压缩的有希望方向提供了早期证据。
...
### [253] [Unmasking Removal-Budget Confounding: A Matched Operating-Point Evaluation Framework for Adaptive Data Cleaning](https://arxiv.org/abs/2608.06511)
*Wei-Hsiang Chen,Pin-Hsuan Yu,Chen-Hsuan Fang,Jung-Hua Wang*

Main category: cs.LG

TL;DR: 提出了一种新的评估框架来解决数据清理中的移除预算偏差问题，实验表明在匹配操作点时，性能差异会缩小或消失。


<details>
  <summary>Details</summary>
Motivation: 为了解决数据清理方法中存在的移除预算偏差问题，提出了一种新的评估框架。

Method: 引入了操作点感知的评估框架，使用匹配预算和匹配召回控制以及阈值无关的指标（AUROC和AUPRC）来评估方法。

Result: 在CIFAR-10和ImageNet-100上的实验表明，在匹配操作点时，大多数在简单评估中观察到的性能差异都会缩小或消失。

Conclusion: 自适应清理方法必须在匹配操作点上进行基准测试，以确保性能提升反映真实的腐败识别。
...
### [254] [Target-Weighted Neyman Allocation: Experimental Design for Heterogeneous Treatment Effects under Population Shift](https://arxiv.org/abs/2608.06512)
*Hoang Dang,Luan Pham,Minh Nguyen*

Main category: cs.LG

TL;DR: TWNA是一种两阶段分层设计，可提高目标加权组平均治疗效果的精度，并有效处理部署组成的不确定性。


<details>
  <summary>Details</summary>
Motivation: 在随机实验中，通过实验比例分配资源可能会浪费预算，而通过部署比例分配则可能低估某些组。

Method: 	extbf{TWNA}（目标加权奈曼分配）是一种两阶段分层设计，利用试点估计的组臂结果方差来分配最终阶段的样本量和治疗概率，以实现目标加权组平均治疗效果（GATE）的精度。

Result: TWNA在处理部署组成的不确定性方面表现出色，无论目标混合是否大致已知或完全未知。模拟和真实协变量基准测试表明，当组既是部署重要也是难以测量时，TWNA可以获得最大的收益。

Conclusion: TWNA是一种有效的分层设计，可以提高目标加权组平均治疗效果的精度，并有效处理部署组成的不确定性。
...
### [255] [CertBind from Multimodal Connectivity to Certifiable Retrieval Decisions](https://arxiv.org/abs/2608.06516)
*Shuheng Cao,Zhenhao Zhang,Ruiqi Chen,Renjie Cao,Weijia Zhang,Siyu Zhang,Jiaxin Liu,Xiangyu Zeng,Haotian Geng,Fan Gu*

Main category: cs.LG

TL;DR: CertBind是一种提高冻结多模态编码器性能的新方法


<details>
  <summary>Details</summary>
Motivation: 为了解决冻结的多模态编码器在表示层可组合性的问题

Method: 引入CertBind，一个针对冻结多模态连接图的可认证组合的多尺度理论

Result: CertBind在节点、边、路径和查询尺度上提供了解决方案，并显著提升了性能

Conclusion: CertBind扩展了多模态组合性，从连接表示到可认证的任务决策
...
### [256] [Online Security Learning in Cooperative Multi-Agent Systems under Hidden Byzantine Attacks](https://arxiv.org/abs/2608.06520)
*Ximing Sun,Yue Wang*
...
### [257] [Newton-Schulz Retraction-Based Inference Enables Hidden Quantum Markov Models to Outperform Classical HMMs](https://arxiv.org/abs/2608.06554)
*Ning Ning*

Main category: cs.LG

TL;DR: NS-RIS算法在HQMM学习方面取得了显著成果，提高了性能并降低了计算成本。


<details>
  <summary>Details</summary>
Motivation: 为了解决HMM在复杂隐藏动态下的局限性，提出了HQMM。然而，现有的HQMM学习方法在非量子过程生成数据上并未始终优于EM训练的HMM，限制了其实际应用。

Method: 提出了NS-RIS算法，该算法基于牛顿-舒尔茨正交化计算极分解搜索方向，同时保持Stiefel流形可行性，避免了昂贵的矩阵分解。

Result: NS-RIS在合成HMM生成的基准测试中优于EM和COSM，平均提高了38.5%的评估指标，最高提高了50.6%。在合成HQMM基准测试中，它将测试指标提高了18.9%，同时将运行时间减少了12.0%。在Splice分类基准测试中，NS-RIS在更高维的潜在状态下也优于EM和COSM，相对于COSM，将平均分类误差分别降低了17.9%（潜在维度6）和14.9%（潜在维度8）。

Conclusion: NS-RIS将HQMM从HMM的理论推广扩展到实际且具有表达力的科学序列数据模型。
...
### [258] [Bootstrap-Conditioned Action Selection with Tabular Foundation Models](https://arxiv.org/abs/2608.06559)
*Devansh Gupta,Shiv Tavker,Dmitry Efimov,Suchitra Sathyanarayana,Gitanjali Bhutani,Boris N. Oreshkin*

Main category: cs.LG

TL;DR: 提出BC-ICL，通过预训练模型和自举方法在上下文赌博中实现高效决策。


<details>
  <summary>Details</summary>
Motivation: 在稀疏、有偏交互数据、不可靠的不确定性估计和严重的冷启动情况下，实际部署上下文相关赌博仍然困难。

Method: 提出BC-ICL（使用ICL的自举条件动作选择），在每一轮抽取交互历史的自举重采样，在冻结的预训练ICL模型上条件化，对所有动作进行评分，并选择具有最高采样评分的动作。进一步引入臂-上下文条件架构，促进动作之间的共享统计强度，并帮助避免孤立臂赌博的常见自举失败模式。

Result: 在标准上下文赌博套件中实现了强大的早期回合后悔和后悔性能，在严格的在线协议下优于既定基线。

Conclusion: 通过使用预训练的表格基础模型和上下文学习，BC-ICL在上下文赌博中实现了有效的决策，并提高了样本效率。
...
### [259] [Theoretical Foundations of Communication-Efficient, Robust, and Practical Distributed and Federated Optimization](https://arxiv.org/abs/2608.06563)
*Grigory Malinovsky*

Main category: cs.LG

TL;DR: This thesis addresses challenges in distributed optimization and federated learning, introduces novel algorithms, and provides theoretical guarantees and experimental validation.


<details>
  <summary>Details</summary>
Motivation: Modern large-scale training relies on classical optimization principles, but the constraints of distributed systems require these foundations to be reconsidered.

Method: Seven challenges at the intersection of theory and practice are addressed, focusing on key bottlenecks in federated learning and distributed optimization.

Result: Novel algorithmic frameworks are introduced, sharp guarantees are established under realistic assumptions, and numerical experiments support the theory.

Conclusion: This thesis contributes to the advancement of machine learning and optimization in distributed systems, particularly in federated learning and distributed optimization.
...
### [260] [Quantization Damage Is Multiplicative, Not Additive](https://arxiv.org/abs/2608.06564)
...
### [261] [CrystalGRPO: Target-Aligned and Coverage-Preserving Reinforcement Learning for Flow-Based Crystal Structure Prediction](https://arxiv.org/abs/2608.06582)
*Kaixiang Su,Hongfei Xue,Qiang Zhu*

Main category: cs.LG

TL;DR: CrystalGRPO框架通过结合能量预测和结构匹配，提高了晶体结构预测的Top-1和Top-20恢复性能。


<details>
  <summary>Details</summary>
Motivation: 基于流的方法在晶体结构预测（CSP）中产生候选结构效率高，但其预训练目标并未直接优化下游目标恢复。强化学习后训练提供了一种灵活的解决方案，但现有方法主要依赖于能量奖励和仅坐标的随机策略。预测的能量无法识别参考多晶型，而奖励驱动的浓度可以减少Top-N恢复所需的候选覆盖范围。

Method: 提出了一种名为CrystalGRPO的CSP后训练框架，该框架将现有的ODE到SDE策略构建扩展到联合坐标-晶格状态。CrystalGRPO结合了MACE预测的能量和基于StructureMatcher的恢复得分，并提供了两种操作模式：CrystalGRPO-Q（优先考虑单次抽取恢复）和CrystalGRPO-C（结合完整轨迹参考正则化和覆盖感知的组优势以保持有限预算的目标恢复）。

Result: 在MP-20和MPTS-52上，与仅坐标的强化学习相比，两种变体在所有四种骨干-数据集设置中均降低了单样本和二十样本RMSE。CrystalGRPO-Q在所有设置中一致地提高了Top-1，而CrystalGRPO-C在所有设置中实现了更高的Top-20。

Conclusion: CrystalGRPO框架在晶体结构预测中提高了性能，特别是在Top-1和Top-20的恢复方面。
...
### [262] [Flowing Through States: Neural ODE Regularization for Reinforcement Learning](https://arxiv.org/abs/2608.06595)
*Mohamed Ghanem,Bernd Finkbeiner*

Main category: cs.LG

TL;DR: 通过显式建模潜在动态，本文提出的方法在强化学习任务中显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 在序列决策任务中，神经网络通常依赖于环境状态的潜在表示。然而，环境动态决定了语义状态的演变方式，而相应的潜在转换通常被隐式地处理，这可能导致两者之间的潜在不匹配。

Method: 我们提出通过将马尔可夫决策过程（MDP）轨迹与常微分方程（ODE）流进行类比来显式地建模潜在动态。在此基础上，我们引入了一种基于神经ODE的正则化方法，该方法强制潜在嵌入遵循一致的ODE流，从而将表示学习与环境动态对齐。

Result: 该方法在强化学习中被证明是有效的，通过将其集成到Actor-Critic算法中。在标准Atari基准和PPO的网格世界环境中，该方法在各种标准Atari基准测试中取得了显著的性能提升。

Conclusion: 本文提出的方法通过显式建模潜在动态，有效地将表示学习与环境动态对齐，在强化学习任务中取得了显著的性能提升。
...
### [263] [Retrofitting Linear Attention into Diffusion Language Models](https://arxiv.org/abs/2608.06628)
*Jinha Kim,Younghun Roh,Jaeyeon Kim*

Main category: cs.LG

TL;DR: 通过引入块混合注意力机制，预训练的dLLMs可以显著提高推理速度。


<details>
  <summary>Details</summary>
Motivation: 针对dLLM推理中的瓶颈，探讨是否可以通过线性化注意力机制来进一步加速推理。

Method: 引入块混合注意力机制，在活跃去噪块内保留精确的softmax注意力，同时在之前的块上应用线性注意力。

Result: 将块混合注意力机制应用于预训练的dLLM LLaDA~2.1，在保持基准性能的同时，解码吞吐量提高至1.7倍，支持更多并发请求。

Conclusion: 预训练的dLLMs可以通过线性化注意力机制进行高效线性化，以实现更快的推理。
...
### [264] [The Sparsity Whisperer](https://arxiv.org/abs/2608.06630)
*Linghao Kong,Inimai Subramanian,Micah Adler,Dan Alistarh,Dan Gutfreund,Nir Shavit*

Main category: cs.LG

TL;DR: 提出了一种基于差异信息的剪枝方法，有效提高了大型语言模型的剪枝效果。


<details>
  <summary>Details</summary>
Motivation: 现有剪枝方法主要保留大激活或重建层输出，忽略了MLP中某些稀疏敏感神经元的关键计算：将相似输入区分成不同输出。

Method: 提出了一组基于差异信息的剪枝方法，包括Wisp、Wisp+和Whisper。Wisp使用输入差异范数评分权重，Wisp+使用神经元最强烈区分的输入对进行评分，Whisper使用轻微正则化的差异Hessian作为重建目标。

Result: 在Llama 2和3.1模型上，第二阶变体在强基于重建的基线之上持续改进，无更新变体在激活感知基线之上改进，特别是在约束设置中。改进效果在Wanda和SparseGPT上扩展到结构化稀疏、下游评估和其他模型家族。将我们的差异信息标准与更强的技术如RIA和ALPS相结合，可以进一步改进，以可忽略的成本将整体精度-运行时前沿向外推移。

Conclusion: 保留输出差异是用于后训练LLM稀疏化的广泛有用且可组合的信号。
...
### [265] [Cryptanalytic Extraction of Isolated Bias-Free GLU Feed-Forward Blocks by Antipodal Separation](https://arxiv.org/abs/2608.06631)
...
### [266] [Bypassing Krum: Selection-Aware Backdoor Attacks in Federated Learning](https://arxiv.org/abs/2608.06637)
*Srinivasan Subramanian,Md. Abdullah Al Hafiz Khan,Kazi Aminul Islam*

Main category: cs.LG

TL;DR: Krum-Proxy攻击方法能够绕过基于距离的聚合规则，对联邦学习系统构成威胁。


<details>
  <summary>Details</summary>
Motivation: 为了减轻对抗性客户端行为对联邦学习的影响，广泛使用鲁棒的聚合方法。然而，基于距离的聚合规则（如Krum和Multi-Krum）依赖于几何属性，这可能被自适应对手利用。

Method: 提出了一种名为Krum-Proxy的攻击方法，这是一种选择感知的后门注入策略，可以持续绕过拜占庭鲁棒聚合。方法通过两阶段优化过程，将特定任务的攻击目标与几何感知的细化分离，使用最近邻代理、随机参考建模和锚点引导对齐。为了保持隐蔽性，引入了一种投影机制，将对抗性更新限制在现实范围内的范数和方差界限内。

Result: 在标准联邦学习基准上的实验表明，Krum-Proxy在保持清洁准确性的同时，实现了更高的攻击成功率，突显了基于距离的聚合对选择感知对手的脆弱性。

Conclusion: Krum-Proxy攻击方法能够有效绕过基于距离的聚合规则，对联邦学习系统构成威胁。
...
### [267] [Dirichlet Follow-the-Leader Closes the Gap in Simultaneous Multiclass U-Calibration](https://arxiv.org/abs/2608.06656)
*Pahan Dewasurendra*
...
### [268] [EpiFlow: A framework for improving the utility of wastewater signals for disease forecasting](https://arxiv.org/abs/2608.06671)
...
TL;DR: 本研究提出了一种基于废水数据的疾病监测方法，通过结合WVL提高了传染病预测的准确性。


<details>
  <summary>Details</summary>
Motivation: 为了提高疾病监测和爆发预警的准确性，研究废水病毒负荷（WVL）与疾病负担之间的关系。

Method: 通过处理废水数据，分析WVL与负担指标之间的关系，并生成实时预测。使用熵度量来评估WVL的可预测性，通过因果测试分析WVL与负担指标之间的关系，并构建一个考虑信号之间演变关系的时变预测模型。通过模拟评估WVL报告延迟的影响。

Result: 在弗吉尼亚州及其卫生区域对COVID-19医院入院进行预测，结果表明，结合WVL可以提高预测准确性，尤其是在关键的流行病阶段，预测覆盖范围提高了20个百分点。

Conclusion: WVL信号可以改善传染病预测，即使在低发病率或报告延迟的情况下。
...
### [269] [A Transferable Autologistic Model for Predicting Rare Failures in Heterogeneous Equipment](https://arxiv.org/abs/2608.06695)
*Islam Benamirouche,Djemel Ziou,Feriel Fass*

Main category: cs.LG

TL;DR: 提出了一种新模型，用于预测异构设备家族中的故障，并在模拟冰箱数据集上取得了良好效果。


<details>
  <summary>Details</summary>
Motivation: 预测故障，尤其是在故障发生频率低、同系列设备传感器配置不同、目标为预测而非诊断时，是预测性维护的主要挑战。

Method: 提出一种通用于目标概率模型，学习异构设备家族中的共享故障相关模式，并针对目标设备进行简明适应性调整。模型明确考虑传感器异质性、运行环境和退化动态，以产生适合维护计划的校准故障概率估计。

Result: 在包含27个具有不同传感器配置、运行条件和故障类型的模拟冰箱数据集上评估了模型性能，提供了控制环境。

Conclusion: 该模型能够有效地预测故障，为维护计划提供准确的数据支持。
...
### [270] [Dueling World Models: Advantage-Style Action Channels for Common-Mode Distractor Rejection](https://arxiv.org/abs/2608.06706)
*Jiazhuo Li,Yiming Fei,Zhiruo Zhou,Heikichi Hayashi*

Main category: cs.LG

TL;DR: 提出了一种新的方法来解决潜在世界模型在处理非控制动作场景时的预测问题。


<details>
  <summary>Details</summary>
Motivation: Latent world models 在处理包含非控制动作的场景时，预测能力下降，因为不同动作的预测变得难以区分。

Method: 提出了一种基于对抗分解的方法，将价值分解为状态基线和动作优势，通过减去预测的平均效应来消除动作无关的干扰。

Result: 在网格世界、合成生成器、连续控制和自然像素Atari游戏中，该方法有效地恢复了代理的自身效应，并在现有模型中揭示了被忽视的动作通道。

Conclusion: 该方法在有限样本中精确取消干扰，为动作条件世界模型提供了一种有效的解决方案。
...
### [271] [Multi-Level Modeling of Large Language Model Inference Latency and Energy via Hybrid Analytical--Machine-Learning Predictors](https://arxiv.org/abs/2608.06723)
*Saeid Shokoufa,Mohammad Erfan Sadeghi,Mehdi Kamal,Massoud Pedram*

Main category: cs.LG

TL;DR: 提出HYMELL框架，有效估计LLMs的能耗和推理延迟，实现快速设计空间探索和节能优化。


<details>
  <summary>Details</summary>
Motivation: 降低大语言模型（LLMs）的能耗和推理延迟，以实现可持续的人工智能部署和硬件感知设计。

Method: 提出了一种名为HYMELL的混合三级框架，结合了分析建模和机器学习（ML），用于估计LLMs的推理延迟和能耗。

Result: 在NVIDIA H100 GPU上评估，HYMELL实现了高预测精度，对于LLaMA 3 8B，在prefill和decode阶段均达到低于5%的误差。

Conclusion: HYMELL能够直接从架构参数预测执行成本，从而实现快速、无硬件的设计空间探索和节能优化。
...
### [272] [Solver-Guided Reasoning for Mixed-Equilibrium Strategies](https://arxiv.org/abs/2608.06741)
...
### [273] [KReF: Training-Free Retrieval for Long-Term Time-Series Forecasting and Predictive Uncertainty](https://arxiv.org/abs/2608.06748)
*Yang Zhang,Rui Su*

Main category: cs.LG

TL;DR: KReF是一种无训练的检索框架，在长期时间序列预测中表现出色。


<details>
  <summary>Details</summary>
Motivation: 为了解决传统概率长期时间序列预测方法中训练模型依赖和反馈延迟的问题

Method: 提出了一种名为KReF的无训练检索框架，该框架将检索到的历史未来数据视为查询局部经验预测分布，并使用手工艺统计数据或冻结的随机傅里叶特征进行嵌入和检索。

Result: 在六个长期时间序列预测基准和四个预测范围内，KReF在所有12个数据集嵌入设置中获得了最低的CRPS，在9个设置中获得了最低的IS90。在两个数据集中，其点预测也匹配或超过了训练基线。

Conclusion: 检索被证明是长期时间序列预测的有用且未充分探索的归纳偏差。
...
### [274] [Sub-Quadratic Bisimulation Metrics via Approximate Nearest Neighbors: Coverage-Augmented Guarantees and Computable Two-Sided Certificates](https://arxiv.org/abs/2608.06762)
...
### [275] [CubicQuant: Parametric Non-Uniform Codebooks for High-Throughput LLM Inference with 1-8-Bit Weights](https://arxiv.org/abs/2608.06763)
*Xuetian Gao*

Main category: cs.LG

TL;DR: CubicQuant在保持表示能力的同时，提高了GPU执行效率。


<details>
  <summary>Details</summary>
Motivation: 平衡自适应重建级别与表示的规则性，以实现高效的GPU执行。

Method: 引入CubicQuant，一种参数非均匀标量格式，在保持密集整数代码流的同时，适应每个权重组内的重建级别。

Result: 在G=128的有限组中，W4 CubicQuant相对于最佳枚举的四位有限浮点格式，在均匀、高斯和拉普拉斯样本上分别降低了3.90%、9.44%和6.27%。

Conclusion: 该格式具有代表性承诺和直接可执行性；下游模型质量和跨设备端到端性能仍然是开放的评价问题。
...
### [276] [Hidden Gauge Controls Feature Specialization in ReLU Networks](https://arxiv.org/abs/2608.06766)
*Tongxi Wang*

Main category: cs.LG

TL;DR: 研究了网络训练过程中特征所有权的问题，证明了初始预测器对特征学习和神经元选择的影响。


<details>
  <summary>Details</summary>
Motivation: 分析网络在训练过程中如何分配任务相关的结构，并探讨特征所有权的问题。

Method: 在可处理的高斯教师-学生模型中，通过改变正齐次尺度量规来研究特征所有权的问题。

Result: 证明了全局选择和功能剪枝，将影响归因于改变特征系数和方向的不同的移动性。

Conclusion: 初始预测器既不能决定特征何时被学习，也不能决定哪个神经元学习它。
...
### [277] [ArchEGraph: A Large-Scale Graph Dataset for Geometry-Topology-Physics Aligned Building Energy Modeling](https://arxiv.org/abs/2608.06772)
*Yihui Li,Yihui Chen,Kaidi Zha,Xiaoyue Yan,Zhexuan Yu,Shiqi Dai,Jun Xiao,Jun Yin,Ramon Elias Weber,Borong Lin*

Main category: cs.LG

TL;DR: 提出ArchEGraph数据集，用于建筑能耗建模，提高设计反馈的准确性。


<details>
  <summary>Details</summary>
Motivation: 准确估计建筑能耗对于实现碳中和和可持续建筑至关重要。为了更好地理解设计决策对建筑能耗的影响，并校准能够为建筑师和工程师提供快速设计反馈的机器学习模型，需要大规模数据集，这些数据集明确地将建筑几何形状映射到性能。

Method: 提出ArchEGraph，这是一个大规模基准数据集，它将建筑表示为具有对齐几何形状、拓扑结构、天气和区域级热负荷的异构图。数据集包含5,481座建筑和49,326个经过验证的建筑-天气模拟案例。总共有超过133,000个空间节点和1,440,000个面节点，反映了大量的几何和拓扑复杂性。基于ArchEGraph，定义了两个基准任务：(i)从多边形网格中重建图，旨在从几何表示中恢复拓扑结构；(ii)基于拓扑结构的负荷预测，利用图结构和时间天气条件来预测区域级响应时间序列。进一步引入了两个任务的标准化评估协议，并进行了跨建筑和跨气候的泛化实验，以评估模型的鲁棒性。

Result: ArchEGraph提供了一个统一的测试平台，用于研究建筑能耗建模中几何-拓扑-物理耦合，使可扩展和可泛化的代理模型得以开发和评估。

Conclusion: ArchEGraph数据集有助于提高建筑能耗建模的准确性和效率，为建筑师和工程师提供更有效的设计反馈。
...
### [278] [Faster Query-Key Learning Sharpens Attention in Self-Attention Models](https://arxiv.org/abs/2608.06776)
*Rahul Vashisht,Harish G. Ramaswamy*

Main category: cs.LG

TL;DR: 通过分析电路参数化，提高自注意力模型的可解释性和预测性能


<details>
  <summary>Details</summary>
Motivation: 分析自注意力模型中查询-键和输出-值电路参数化对参数轨迹的影响

Method: 梯度流分析，推导封闭形式的动态方程，实验验证

Result: 因子化导致两个电路学习率的隐式缩放，输出-值和查询-键参数沿直线移动，学习率相对速度决定其移动速度，查询-键学习相对于输出-值学习产生更尖锐的注意力，提高注意力可解释性代理，同时保持可比较的预测性能

Conclusion: 通过分析电路参数化对注意力模式的影响，我们可以提高自注意力模型的可解释性和预测性能
...
### [279] [Understanding Differentiable Embeddings Through Differential and Integral Geometry](https://arxiv.org/abs/2608.06809)
*Xinyu Zhang,Klaus Mueller*

Main category: cs.LG

TL;DR: 提出了一种新的诊断框架，用于评估非线性降维嵌入的可信度。


<details>
  <summary>Details</summary>
Motivation: 为了确定非线性降维嵌入是否可信，分析员需要一种方法来评估。

Method: 提出了一种基于嵌入几何的诊断框架，包括微分视图和积分视图，以及基于有限尺度邻域关系的经典秩度量指标。

Result: 该框架提供了两种互补的几何视角，能够解释嵌入的局部行为，并验证了理论预测。实验表明，该方法可以准确估计嵌入的曲率，并区分单值嵌入和路径相关优化嵌入。

Conclusion: 该框架为嵌入几何的降维提供了全面的诊断方法，并验证了其有效性。
...
### [280] [Multiscale Reward Hedging from Correct Demonstrations](https://arxiv.org/abs/2608.06825)
*Pahan Dewasurendra*

Main category: cs.LG

TL;DR: 本文提出了一种新的学习方法，用于从正确示范中学习，该方法在存在多个正确答案时更有效。


<details>
  <summary>Details</summary>
Motivation: 在存在多个正确答案时，从正确示范中学习比监督学习更困难，因为学习者看到的是有效答案，而不是自己的答案是否有效，也没有奖励。现有的奖励对冲保证假设有限奖励类别。

Method: 我们给出了第一个连续类的无边界保证。关键是在每个精度尺度上对共享投票进行对冲，以进行容错最优性测试。每个尺度都有一个目标奖励的存活代理，并且超过该尺度的预测将代理加倍。这导致了同时的尾部界限，其中G是优化差距函数的类。将尾部整合给出累积隐藏间隙，其界限为度量熵积分，独立于轮数。多项式熵(A/ε)^d给出O(dlogA)的总间隙和一个快速的O(d/m)统计速率。

Result: 对于有界线性上下文推荐，结果是任意紧凑菜单的O(d)后悔。这是第一个多项式有限界限，没有对菜单的结构限制，但以不正确的预测为代价。尽管一般投票可能很昂贵，但它对于一维Lipschitz参数曲线恰好是多项式时间。固定半径的二维推荐在菜单大小为K时需要O(KT^2)时间。我们还证明了Ω(d)的下限，低秩和有界ReLU网络推论，以及一个鲁棒定理，它只增加了演示者的累积次优化。一个可重复的自适应压力测试说明了预测的尺度适应性。在分解后，精确的MovieLens审计在十个用户上运行1.7秒CPU时间，并改善了演示评分策略和适当的在线基线上的平均潜在间隙。学习者只使用动作示范，从未观察到奖励或损失。

Conclusion: 本文提出了一种新的学习方法，用于从正确示范中学习，该方法在存在多个正确答案时更有效。这种方法为连续类提供了无边界保证，并且具有多项式时间和空间复杂度。实验结果表明，该方法在MovieLens数据集上取得了显著的性能提升。
...
### [281] [Graph Machine: Exploring Edge Mechanisms as an Inductive Bias](https://arxiv.org/abs/2608.06834)
*Lintai Hou*

Main category: cs.LG

TL;DR: Graph Machine通过边机制在数独任务中优于Transformer，支持了边机制作为有前途的架构设计。


<details>
  <summary>Details</summary>
Motivation: Transformers在全局内容匹配方面表现出强大的架构，但推理问题可能需要更强的归纳偏差，即迭代遍历潜在关系。

Method: 提出了一种名为Graph Machine的架构，具有两种显式的基于边的机制：边增强注意力和以边为中心的引用。

Result: 在受控设置下，Graph Machine在数独任务中优于Transformer基线，消融研究和机制分析将收益归因于边机制。模型发现了数独几何的紧凑的基于边的构造。

Conclusion: 显式边机制作为一种有前途的架构设计，值得更广泛的评估。
...
### [282] [Mathematical Principles and Experimental Discoveries of the Emergence of Symbolic Patterns in Artificial Neural Networks](https://arxiv.org/abs/2608.06839)
...
### [283] [Bridging the Gap Between Hyperdimensional Computing and Kernel Methods via the Nyström Method](https://arxiv.org/abs/2608.06860)
*Quanling Zhao,Anthony Hitchcock Thomas,Ari Brin,Xiaofan Yu,Tajana Rosing*

Main category: cs.LG

TL;DR: NysHD方法有效提升HDC在机器学习中的性能


<details>
  <summary>Details</summary>
Motivation: 提高机器学习中的信息处理效率

Method: 提出NysHD方法，基于Nyström方法构建映射

Result: 在图和字符串数据集上分别提高了11%和17%的分类准确率

Conclusion: NysHD方法有效提高了HDC在机器学习中的应用效果
...
### [284] [SkillAligner: Treating Retrieved Skills as Adaptable Drafts at Execution Time](https://arxiv.org/abs/2608.06880)
*Qinfeng Li,Dalin He,Yuntai Bao,Ying Yang,Ruoxi Chen,Xinyan Yu,Lizhou Liang,Ge Su,Wenqi Zhang,Xuhong Zhang*

Main category: cs.LG

TL;DR: SkillAligner是一种提高语言代理执行效能的自适应技能适配框架。


<details>
  <summary>Details</summary>
Motivation: 为了解决从知识库中检索到的技能可能因语义相关性而无法保证执行效用的难题，我们提出了一种名为SkillAligner的训练免费执行时技能自适应框架。

Method: SkillAligner通过将检索到的技能视为可适应的草稿，而不是固定的指令，在执行前进行一次联合适配。该适配过程将有用的技能片段专门针对任务需求进行适配，将其程序性假设与可用的执行接口对齐，并通过解决技能之间的依赖关系、冲突和冗余来组合最终的指导。适配后的内容被整合成紧凑的执行指南并在后续轨迹中重用。

Result: 广泛的实验表明，SkillAligner在现有技能使用基线的基础上显著提高了任务性能，降低了实例级别的技能引起的退化，并降低了总推理成本。

Conclusion: SkillAligner通过提高技能适应性，显著提升了语言代理的执行效能。
...
### [285] [PRISM: Principled Reference Identification for Schrodinger Bridge Model](https://arxiv.org/abs/2608.06893)
*Forouzan Fallah,Yezhou Yang*

Main category: cs.LG

TL;DR: PRISM理论优化了Schrödinger桥模型的参考设计，提高了信号恢复的效果。


<details>
  <summary>Details</summary>
Motivation: Schrödinger bridge models从降级观测中恢复干净信号，但参考过程的选择通常是启发式的。

Method: 开发PRISM理论，以优化桥参考设计，并分析时间变化的Gaussian参考，证明不可见性原理，并推导出有限步骤目标函数。

Result: PRISM将参考设计从超参数扫描转化为高斯域的计算，并确定真实图像破坏它的确切位置。

Conclusion: PRISM理论优化了Schrödinger桥模型的参考设计，提高了信号恢复的效果。
...
### [286] [Recent advances in weakly supervised learning: New supervision paradigms, assumption relaxations, and practical solutions](https://arxiv.org/abs/2608.06896)
*Wei Wang,Gang Niu,Masashi Sugiyama*

Main category: cs.LG

TL;DR: 本文研究了弱监督学习领域的最新进展，包括置信度差异分类、互补标签学习和部分标签学习的评估框架。


<details>
  <summary>Details</summary>
Motivation: 解决现实应用中高质量标注数据的缺失问题，实现弱监督学习。

Method: 新监督范式、宽松假设、实际解决方案、置信度差异分类、互补标签学习、部分标签学习评估框架

Result: 提高了弱监督学习的准确性和实用性。

Conclusion: 提出了基于置信度差异分类的新弱监督二元分类问题解决方案，研究了基于更宽松假设的互补标签学习，并提出了部分标签学习的评估框架。
...
### [287] [MiCoPro: End-to-End Mixed Precision HW/SW Co-design with HW-aware Proxy Model](https://arxiv.org/abs/2608.06916)
*Zijun Jiang,Yangdi Lyu*

Main category: cs.LG

TL;DR: 提出MiCo框架和MiCoPro模型，用于优化和部署边缘AI应用的MPQ模型，显著提高性能。


<details>
  <summary>Details</summary>
Motivation: 为了解决现有MPQ算法的灵活性和效率限制，以及传统方法在理解不同MPQ方案对训练后量化和量化感知训练结果的影响方面的挑战，以及缺少MPQ模型的优化和部署的端到端框架。

Method: 提出MiCo框架，采用新颖的优化算法在严格的延迟约束下搜索精度最优的量化配置。MiCoPro引入了鲁棒的硬件感知代理（HAP）模型，利用目标特定的延迟建模，实现从PyTorch模型到裸机C代码的快速探索和直接部署。

Result: 在BitFusion加速器和SIMD扩展的RISC-V处理器上展示了框架的通用性，实现了高达40%的延迟降低，精度下降小于3%。

Conclusion: MiCo框架和MiCoPro模型在提高边缘AI应用的MPQ模型性能方面表现出色。
...
### [288] [Walkable to Whom? Capturing Subjective Variability in Walkability Perception Using Multimodal Deep Learning](https://arxiv.org/abs/2608.06934)
*Moloud Damandeh,Meead Saberi*

Main category: cs.LG

TL;DR: 本研究提出了一种新的步行舒适度感知模型，提高了评分准确性，并强调了评估者个体差异的重要性。


<details>
  <summary>Details</summary>
Motivation: 现有研究将个体对步行舒适度的不同判断简化为综合评分，并假设了统一感知，同时依赖车载街景图像，这些图像并不反映行人的视觉体验。

Method: 提出一个包含1,196名受访者、29,870条步行舒适度评分的数据集，将城市、郊区和区域澳大利亚环境中的行人道图像与评估者属性联系起来，并提出了第一个用户条件的多模态深度学习框架来感知步行舒适度，融合视觉特征与受访者级表示。

Result: 研究表明，与匹配的街景图像相比，行人道图像获得了更高的步行舒适度评分，表明图像来源是感知调查中的实质性设计决策。用户条件模型将排名一致性提高了65%，超过图像基准（二次加权kappa 0.47比0.29），表明评估环境的人具有预测指示作用，超越了图像内容本身。

Conclusion: 这些发现支持从综合、观察者独立的步行舒适度评分转向代表不同用户模型的转变，从而能够更全面地评估行人环境。
...
### [289] [ELMZip: Onboard Satellite Image Compression via Extreme Learning Machines for Efficient Downlink](https://arxiv.org/abs/2608.06942)
*Woojin Cho,Junghwan Park,Sangcheol Sim,Steve Andreas Immanuel,Junhyuk Heo,Darongsae Kwon*

Main category: cs.LG

TL;DR: ELMZip框架通过高效压缩和传输多光谱图像数据，为小卫星地球观测提供了新的解决方案。


<details>
  <summary>Details</summary>
Motivation: 由于高数据量和受限的通信窗口，小卫星（如CubeSats）获取的多光谱图像面临着显著的数据下行链路挑战。

Method: 提出基于极值学习机（ELM）和领域分解策略的ELMZip框架，通过随机特征单层网络将拟合过程表述为凸最小二乘问题，从而消除计算量大的反向传播的需求。

Result: 显著降低下行链路负载，在保持高重建精度的同时实现显著的压缩效率，使资源受限的平台能够最大化数据回报，并推进实时AI驱动的地球观测。

Conclusion: ELMZip框架为小卫星高效、无分辨率的机载神经网络表示提供了一种解决方案，有助于解决多光谱图像数据下行链路挑战。
...
### [290] [A Rate Separation for Agnostic Direct Sums](https://arxiv.org/abs/2608.06951)
...
### [291] [How Molecular Generative Models Organize Molecular Identity](https://arxiv.org/abs/2608.06956)
...
### [292] [Density-aware Hierarchical Clustering Based on Element-Categorized Connection Subgraphs](https://arxiv.org/abs/2608.06990)
*Yuning Yu,José Rodríguez-Piñeiro,Xuefeng Yin,Bin Feng*

Main category: cs.LG

TL;DR: 提出了一种新的密度感知层次聚类方法DHC-ECS，在聚类性能上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决传统层次聚类方法在密度变化和结构连通性方面的不足。

Method: 提出了一种基于元素分类的连接子图（DHC-ECS）的密度感知层次聚类方法，该方法结合了层次聚类、基于密度的聚类和图聚类。

Result: 在异构基准数据集上的广泛评估表明，DHC-ECS在聚类准确性和参数鲁棒性方面优于基线方法（包括AChameleon、RNN-DBSCAN、McDPC和G-RMS）。

Conclusion: 所提出的聚类算法通过利用局部密度和图结构连通性（即顶点和边的二重性）具有很大的潜力，以及可以确定内在阈值，减少对手动参数调整的依赖。
...
### [293] [Beyond Foundation Models: Dimension-Aware Neural Architecture Search with Small-Data Representation Models for Cryocooler Lifetime Prediction](https://arxiv.org/abs/2608.06993)
*Gregor Molan,Grafika Jati,Francesco Barchi,Andrea Acquaviva,Aljaž Osterman,Martin Molan*

Main category: cs.LG

TL;DR: 提出了一种针对小型数据集的表示学习框架，通过合理的选择归纳偏差和容量控制，实现了有效的表示学习，无需大规模预训练。


<details>
  <summary>Details</summary>
Motivation: 针对大规模预训练时间序列模型在工业和科学领域缺乏大量、多样数据的问题，提出FSD-RM（小型数据表示模型族）范式作为实际替代方案。

Method: 使用CNN1D、LSTM、GRU、Transformer等编码器进行容量控制下的表示学习，并在多变量遥测数据上进行无监督训练。采用维度感知神经架构搜索（NAS）联合优化模型容量和输入维度。

Result: 在制冷机遥测数据上的实验表明，该方法在降低训练成本和模型复杂度的同时，实现了具有竞争力的预测性能。

Conclusion: 提出了一种针对小型数据集的表示学习框架，通过合理的选择归纳偏差和容量控制，实现了有效的表示学习，无需大规模预训练。
...
### [294] [Every Cache Entry Earns Its Place: Global Allocation of Resolution and Coverage for KV Cache Compression](https://arxiv.org/abs/2608.07001)
*Haolin Tian,Yuzhe Liu,Tonghan Wang*

Main category: cs.LG

TL;DR: GraceKV是一种全局方法，用于在KV缓存压缩中分配解析和覆盖，有效提高了压缩效果。


<details>
  <summary>Details</summary>
Motivation: 大语言模型处理越来越长的上下文时，KV缓存存储和重复访问已成为瓶颈。

Method: 提出GraceKV，全局分配KV缓存压缩中的解析和覆盖，将压缩过程视为固定缓存预算下的全局资源分配问题。

Result: 系统实验表明，GraceKV在24个32个设置中排名第一，可承受高达128倍压缩。

Conclusion: 全局预算分配在协调信息覆盖和局部解析方面是有效的。
...
### [295] [Hyperbolic Graph Embedders for Link Prediction and Topology Reconstruction](https://arxiv.org/abs/2608.07029)
*Robert Jankowski,Maksim Kitsak,Dorota Celińska-Kopczyńska*

Main category: cs.LG

TL;DR: 对13种无监督的拟超平面图嵌入器进行基准测试，发现最大似然和基于表示学习的方案在整体性能上表现最强，但没有任何一种方法在所有任务和结构环境中都占主导地位。


<details>
  <summary>Details</summary>
Motivation: Hyperbolic embeddings在复杂网络中的几何表示提供紧凑的表示，但机器学习、网络科学和算法学中开发的方法的系统比较很少。

Method: 对13种无监督的拟超平面图嵌入器进行基准测试，包括链接预测和拓扑重建。

Result: 最大似然和基于表示学习的方案（包括混合变体）在整体性能上表现最强，但没有任何一种方法在所有任务和结构环境中都占主导地位。性能与嵌入范式比学科起源更为相关。

Conclusion: 确定了不同范式成功或失败的网络环境，并为下游应用中的方法选择提供了实际指导。
...
### [296] [Accounting Graph Transformer for Short-History Multi-KPI Forecasting in Small Businesses](https://arxiv.org/abs/2608.07037)
*Shrutendra Harsola,Vignesh Subrahmaniam*

Main category: cs.LG

TL;DR: AGT通过引入会计图转换器，提高了财务报表关键绩效指标的预测准确性，为小企业提供了一种有效的预测方法。


<details>
  <summary>Details</summary>
Motivation: 小企业通常只有12-24个月的会计历史，但规划和风险评估需要协调的财务报表预测。

Method: 我们研究了从71个月账簿系列中联合预测13个收入报表、资产负债表、现金流量和营运资本关键绩效指标（KPI）。我们引入了会计图转换器（AGT），它将每个账簿系列表示为掩码令牌，通过固定会计关系图上的类型化注意力交换信息，汇集特定目标上下文，并将其与门控的三个月最近路径融合。

Result: 在来自1,060家未见公司的11,993个预测起点中，AGT在三个独立的种子中实现了0.6990 ± 0.0013的样本加权KPI宏平均绝对误差（MAE），而最强大的基线LightGBM为0.7378 ± 0.0014。在预指定的种子42中，配对公司集群自举给出了LightGBM减去AGT的差异为0.0395，95%置信区间（CI）为[0.0350, 0.0439]。AGT在匹配种子42的比较中，在所有13个KPI上均优于LightGBM、TimeMixer和SOFTS。最终架构消融实验表明，关系注意力、会计拓扑和最近路径都提高了验证和测试精度。在1,094家额外未见公司上，AGT的MAE为0.7548，而SOFTS为0.7694。一个5.3M参数的模型产生了156个对齐的预测，无需针对公司进行适配，提供了一个用于综合规划、流动性和营运资本分析的预测层。

Conclusion: AGT在联合预测关键绩效指标方面优于现有基线方法，并为小企业提供了高效的预测解决方案。
...
### [297] [Beyond Isolation: Unlocking Reinforcement Learning Component Synergy for Sample-Efficient Continuous Control](https://arxiv.org/abs/2608.07086)
...
TL;DR: 本文研究了强化学习系统中组件间的相互作用，提出了一种新的框架ROSER，在多个任务中实现了性能提升。


<details>
  <summary>Details</summary>
Motivation: 由于强化学习系统的内在属性，其比其他机器学习范式更为复杂，导致强化学习系统设计需要同时考虑许多紧密耦合的因素。尽管算法组件在个体方面取得了进展，但它们的功能相互依赖性仍被低估。

Method: 进行系统研究，发现不同组件的有效性存在显著的任务依赖性，盲目堆叠最先进的技术并不一定能带来性能提升，反而可能引发新的挑战，如非平稳性的累积。

Result: 基于这些发现，提出了一种名为ROSER的强化学习框架，该框架协调了三个关键维度：基于模型的表示、优化稳定性和经验重放。在多个连续控制基准测试中，ROSER一致优于原始基线，并比盲目堆叠提升了17.60%。

Conclusion: 本研究强调了在强化学习系统设计中采用整体视角的必要性，并为开发样本高效的智能体铺平了道路。
...
### [298] [Synthetic LiDAR Data Generation and Deterministic Downsampling for Point Cloud Classification on the Edge](https://arxiv.org/abs/2608.07106)
*Niclas Meyer,Stefan Reitmann*

Main category: cs.LG

TL;DR: 本文提出了一种针对Raspberry Pi 5的硬件约束工作流程，提高了3D感知的实时性。


<details>
  <summary>Details</summary>
Motivation: 为了解决将三维深度学习框架部署到低功耗嵌入式处理器时的瓶颈问题，本文提出了一种针对Raspberry Pi 5的硬件约束工作流程。

Method: 使用基于物理的模拟构建了一个合成LiDAR数据集，并整合了一个独立的、以特征驱动的关键点层（CPL）作为前端过滤器。

Result: 在ARM Cortex-A76处理器上，完整的管道实现了大约50 FPS的推理吞吐量，同时保持了88.36%的实例分类精度。

Conclusion: 本文提出的方法证明了在边缘进行确定性实时3D感知的可行性。
...
### [299] [Modular TTT: Rethinking Test-Time Training as Composable Modules](https://arxiv.org/abs/2608.07110)
*Bohao Tang,Zhen Qin,Yuqi Pan,Zheng Li,Pengfei Liu,Ya Zhang*

Main category: cs.LG

TL;DR: 提出模块化TTT框架，提高了TTT方法的设计和性能


<details>
  <summary>Details</summary>
Motivation: 针对TTT方法难以设计新方法和隔离各个组件的问题

Method: 提出模块化TTT框架，将内部学习器表示为有向无环图，并暴露快速权重网络、损失函数、学习率等设计维度

Result: 发现小学习率初始化、权重衰减和单层非线性可以提高性能，而MSE和内积损失表现相似；深层快速权重网络和归一化往往损害性能；残差连接和门控提供的好处很小

Conclusion: 模块化TTT框架有效提高了TTT方法的设计和性能
...
### [300] [Online Conformal Prediction Beyond Feedback](https://arxiv.org/abs/2608.07139)
*Joar Skalse,Edoardo Pona,Osvaldo Simeone,Nicola Paoletti*

Main category: cs.LG

TL;DR: 提出了一种新的在线一致性预测方法，无需反馈即可提供与基于赌博的OCP方法相当的覆盖范围。


<details>
  <summary>Details</summary>
Motivation: 在部署机器学习模型时进行不确定性量化至关重要。

Method: 提出了一种基于查询的在线一致性预测（OCPQ）方法，通过将问题转化为部分监控博弈来解决。

Result: OCPQ在任意黑盒分类器和任何（非独立同分布）无信息数据流上具有O(T^(2/3))的期望后悔和期望覆盖至少β-O(T^(-1/3))，其中β是用户定义的，而查询仅占期望轮次T^(-1/3)的分数。

Conclusion: 该方法在保证正确标签以足够高的概率被覆盖的同时，鼓励输出小的预测集，从而在不需要从部署的预测集中获取反馈的情况下，提供了与基于赌博的OCP方法相当的覆盖范围。
...
### [301] [Interpretable reinforcement learning with decision-tree pruning](https://arxiv.org/abs/2608.07151)
*Mark Leon Ringer,Michel Tokic*

Main category: cs.LG

TL;DR: 提出了一种剪枝过程，以简化强化学习策略，同时保持高性能和可解释性。


<details>
  <summary>Details</summary>
Motivation: 提高强化学习策略的可解释性，以便提高其可信度。

Method: 提出一种剪枝过程，通过定义一组结构和用法感知的算子来简化基于规则的策略，并通过重新执行策略来评估候选编辑。

Result: 在经典控制和MuJoCo基准测试中，剪枝痕迹揭示了可解释性的持续改进，同时保持高性能。

Conclusion: 该剪枝过程在简化策略的同时保持了性能，并提高了策略的可解释性。
...
### [302] [Machine Learning-Based Inter-Crystal Scatter Recovery for Ultra-High Resolution PET Imaging](https://arxiv.org/abs/2608.07155)
*Alexandre Bernier,Roger Lecomte,Jean-Baptiste Michaud*

Main category: cs.LG

TL;DR: 提出了一种基于神经网络的ICS恢复方法，显著提高了UHR-PET的灵敏度和分辨率。


<details>
  <summary>Details</summary>
Motivation: UHR-PET中晶格间散射（ICS）事件对图像分辨率和灵敏度造成挑战。

Method: 提出了一种前馈神经网络来优化ICS事件恢复，通过推断第一次康普顿相互作用对应的响应线。

Result: 与常规方法相比，灵敏度提高了70%至106%，同时保持了亚毫米的空间分辨率（低至1.6毫米）。

Conclusion: 这种ICS恢复方法是一种有效的解决方案，可以补偿小型、像素化探测器在UHR-PET中的较低检测效率，同时减少扫描时间和辐射剂量，同时基本上保持图像质量。
...
### [303] [Capacity Confounds and Coverage Guarantees in Adaptive Sub-model Federated Learning](https://arxiv.org/abs/2608.07157)
*Alireza Moayedikia,Alicia Troncoso Lora*

Main category: cs.LG

TL;DR: 分析自适应容量分配的可行性，发现容量和覆盖度比分配智能更重要。


<details>
  <summary>Details</summary>
Motivation: 现有方法仅根据设备资源分配容量，而提出根据每个客户的数据异构性（根据服务器已观察到的更新估计）进行容量分配的方法。

Method: 使用HAS-FL作为测试案例，分析自适应容量分配框架的可行性。

Result: 发现三个关键点：1. 客户端异质性更新差异估计主要受容量影响而非数据；2. 自适应分配存在隐藏的失败模式；3. 匹配预算控制确定自适应分配的贡献。

Conclusion: 子模型训练有价值，但保护准确性的关键是参数覆盖率而非分配智能。
...
### [304] [Trajectory-Relative Hindsight Distillation for Agentic Reinforcement Learning](https://arxiv.org/abs/2608.07371)
...
### [305] [Edge Sparsification via Temporal Forman-Ricci Curvature for Dynamic Graph Learning](https://arxiv.org/abs/2608.07158)
*Poupak Azad,Cuneyt Gurcan Akcora,Kiarash Shamsi*

Main category: cs.LG

TL;DR: 提出了一种基于网络曲率的时间图学习方法，显著提高了预测性能和计算效率。


<details>
  <summary>Details</summary>
Motivation: 分析实时变化系统，如金融交易网络、通信系统和在线社交平台。

Method: 提出一种基于网络曲率的边缘稀疏化框架，将Forman-Ricci曲率扩展到有向加权时间图，通过捕捉结构支持、时间近度和局部交互竞争。

Result: 在9个交易网络和3个时间图基准数据集上的实验表明，该框架在多个图级预测任务中保留了预测性能。TRicci将时间图稀疏化约80%，平均减少端到端训练和推理时间55.94%，而预测性能没有显著下降。

Conclusion: 时间曲率可以作为可扩展时间图学习的原则性基础，在大量稀疏化下保留预测时间结构信息。
...
### [306] [Fluid-DiT: Graph-Free Diffusion Transformers for Fluid Flow Simulations Learning](https://arxiv.org/abs/2608.07161)
*Shentong Mo,Guolin Ke*

Main category: cs.LG

TL;DR: Fluid-DiT是一种创新的图免费扩散Transformer，在模拟复杂流体流动方面优于传统的基于图的扩散方法。


<details>
  <summary>Details</summary>
Motivation: 模拟复杂流体流动需要捕获完整的平衡分布，而不仅仅是平均轨迹，但高保真求解器在计算上仍然具有挑战性。

Method: 提出了一种名为Fluid-DiT的无图扩散Transformer，该Transformer用基于注意力的去噪来代替图消息传递，消除了显式的图设计，同时保留了对混沌流分布建模的能力。

Result: 在层流圆柱尾流、椭圆流系统和湍流3D机翼实验等标准基准上，Fluid-DiT在样本质量和分布精度方面始终优于基于图的扩散基线，实现了更高的R^2相关性和更低的Wasserstein距离。此外，它可以从短、不完整的轨迹中泛化到未见过的雷诺数和几何形状，显示出强大的可扩展性。

Conclusion: Fluid-DiT在模拟复杂流体流动方面表现出色，尤其是在样本质量和分布精度方面优于基于图的扩散方法。
...
### [307] [Momba: Network Modernization Improves Multi-Objective Reinforcement Learning](https://arxiv.org/abs/2608.07180)
*Adam Štafa,Santeri Heiskanen,Petr Novotný,Joni Pajarinen*

Main category: cs.LG

TL;DR: 通过改进神经网络架构，MORL算法性能显著提升。


<details>
  <summary>Details</summary>
Motivation: 提高多目标强化学习（MORL）算法的性能，特别是当需要平衡多个冲突目标时。

Method: 结合最新的神经网络设计技术，包括：观察和特征归一化、权重归一化以及使用熵正则化的MORL算法对分布性回报进行建模。

Result: 在标准连续控制基准测试中，这些改进显著提高了解决方案集的质量，而无需对底层算法进行重大更改。

Conclusion: 通过改进神经网络架构，可以显著提高MORL算法的性能，同时保持算法的稳定性。
...
### [308] [Conformal Fusion Under Missing Modalities](https://arxiv.org/abs/2608.07183)
*Alireza Moayedikia*
...
### [309] [MAUPITI: On-Device Prototype-Based Learning on a Smart Infrared Sensor](https://arxiv.org/abs/2608.07192)
...
TL;DR: 该方法在保持高精度的同时，有效降低了内存和功耗，适用于隐私保护的人体感知在嵌入式系统中的应用


<details>
  <summary>Details</summary>
Motivation: 隐私保护的人体感知在嵌入式系统中的应用

Method: 集成16x16热MOSFET阵列和RISC-V微控制器，采用基于原型最近类平均（NCM）分类器，离线训练和量化简单卷积神经网络（CNN）编码器，流式存储和更新类别原型

Result: 在两个数据集上的实验表明，该方法在分类和原型更新方面的延迟开销可忽略不计（考虑两个阶段均小于0.29%），有效地实现了感知框架的在线自适应

Conclusion: 该方法在保持高精度的同时，有效降低了内存和功耗，适用于隐私保护的人体感知在嵌入式系统中的应用
...
### [310] [An AI4AI Framework for Visual Token Pruning](https://arxiv.org/abs/2608.07193)
*Zhen Liu,Wenli Huang,Wei Song,Yuhan Liu,Zhiqin Yang,Jingwen Fu*

Main category: cs.LG

TL;DR: AutoPrune框架有效降低MLLMs推理成本
...
Method: 构建AI4AI框架，设计AutoPrune框架，使用Token Pruning Domain-Specific Language (TPDSL)

Result: 有效、高效、可迁移，在移除94.4%视觉标记的情况下，保持超过99%的完整标记性能，降低FLOPs 9.9倍和预填充延迟6.4倍

Conclusion: AutoPrune框架在视觉标记剪枝方面表现出色
...
### [311] [Stochastic Autoregressive Learning](https://arxiv.org/abs/2608.07224)
...
### [312] [Learning Suffers More Than the Policy Class Under Partial Observability: A Closed-Form Analysis](https://arxiv.org/abs/2608.07228)
*Idil Gözel*
...
### [313] [TOFD: Target-Oriented Feature Decoupling against Poisoning Attacks in Split Federated Learning](https://arxiv.org/abs/2608.07274)
*Yuhan Xie,Jingrong Huang,Chen Lyu*

Main category: cs.LG

TL;DR: TOFD：一种针对SFL中毒攻击的检测和防御框架


<details>
  <summary>Details</summary>
Motivation: 为了解决SFL架构中引入的独特攻击面和现有防御措施不足的问题

Method: 提出了一种名为TOFD的统一框架，包括三个阶段：目标推断、样本净化和去耦优化

Result: TOFD在五个数据集上进行了广泛实验，在各种攻击场景下优于现有防御措施，具有优异的鲁棒性和低计算开销

Conclusion: TOFD能够有效检测和抵御各种中毒攻击，为SFL的隐私保护协同训练提供了新的解决方案
...
### [314] [$\boldsymbolλ$-Orthogonality Regularization for Compatible Representation Learning](https://arxiv.org/abs/2509.16664)
*Simone Ricci,Niccolò Biondi,Federico Pernici,Ioannis Patras,Alberto Del Bimbo*

Main category: cs.LG

TL;DR: 本文提出了一种新的方法，通过放松正交约束和仿射变换，在保留原始表示的同时，实现了对特定分布的适应性。


<details>
  <summary>Details</summary>
Motivation: 由于训练成本高和表示学习的不一致性，检索系统对促进表示之间的通信和确保独立训练的神经网络之间的兼容性产生了极大兴趣。

Method: 本文提出了一种放松的正交约束，即λ-正交正则化，同时学习仿射变换，以获得特定分布的适应性，同时保留原始学习到的表示。

Result: 在各种架构和数据集上进行了广泛的实验，验证了我们的方法，证明了它保留了模型的零样本性能并确保了模型更新的兼容性。

Conclusion: 本文提出的方法在适应不同学习到的表示方面具有优势，为检索系统中的表示学习提供了新的思路。
...
### [315] [A foundation-model approach to pediatric headache classification from rs-fMRI](https://arxiv.org/abs/2608.07287)
*Guilherme S. Imai Aldeia,Clara Moon,Julie Shulman,Navil Sethna,Allison Smith,Alyssa Lebel,William G. La Cava,Scott Holmes*

Main category: cs.LG

TL;DR: NeuroSTORM在儿童偏头痛分类中表现出色，但需改进以区分不同头痛亚型


<details>
  <summary>Details</summary>
Motivation: 研究rs-fMRI在儿童偏头痛分类中的应用潜力

Method: 使用NeuroSTORM对rs-fMRI数据进行编码和微调，与基于功能连接矩阵的标准神经科学方法进行比较

Result: NeuroSTORM在区分头痛和非头痛、以及多类分类中表现优于标准方法，但在区分不同头痛亚型方面存在困难

Conclusion: NeuroSTORM在儿童偏头痛分类中具有潜力，但需进一步研究以提高亚型识别的准确性
...
### [316] [FUSE: Feature-Wise Unified Specialization with Cross-Column Exchange for Mixed-Type Tabular Flow Matching](https://arxiv.org/abs/2608.07294)
*Suman Cha,Seongchan Lee,Dohyun Ko,Hyunjoong Kim*

Main category: cs.LG

TL;DR: FUSE是一种有效处理混合类型表格数据的方法，在多个数据集上取得了优异的性能。


<details>
  <summary>Details</summary>
Motivation: 生成混合类型表格数据需要联合建模各种特征分布及其复杂的跨列依赖关系。

Method: 提出了一种名为FUSE的方法，该方法通过使用分别针对数值和分类特征的适应性混合模块，并保留所有列之间的信息交换来显式分离这些角色。

Result: 在八个表格数据集上的全面实验表明，FUSE在分布忠实度和下游效用指标方面均实现了强大且一致的性能。

Conclusion: FUSE在生成混合类型表格数据方面表现出色，为处理复杂的跨列依赖关系提供了有效的方法。
...
### [317] [From Optimal Actions to World Models: Identifiability of Transition Kernels in Discounted MDPs](https://arxiv.org/abs/2608.07301)
*Neal Batra*

Main category: cs.LG

TL;DR: 本文研究了从最优动作中恢复马尔可夫决策过程转移概率的可能性，并探讨了不同奖励形式的影响。


<details>
  <summary>Details</summary>
Motivation: 研究仅从最优动作中恢复马尔可夫决策过程的转移概率

Method: 证明和比较不同奖励形式的恢复能力

Result: 证明了两个核函数在特定条件下给出相同的最优动作，并比较了不同奖励形式的恢复能力

Conclusion: 奖励的形式影响从最优动作中学习到的动态信息
...
### [318] [Is SwiGLU's Open Positive Tail Necessary? Evidence from Closed-Tail Gating with MemGLU](https://arxiv.org/abs/2608.07323)
*Yuting Ge,Pengju Yang,Mingkai Nie*

Main category: cs.LG

TL;DR: MemGLU与SwiGLU在性能上相近，表明开放正尾不是必需的


<details>
  <summary>Details</summary>
Motivation: 测试仅解码器语言模型FFN是否需要SwiGLU的开放正尾

Method: 引入MemGLU作为从忆阻分支几何中派生出的封闭尾比较器，进行配对9M和30M预训练运行，使用三个种子

Result: MemGLU在验证NLL中与SwiGLU相差约0.1%，训练的SwiGLU检查点对正尾抑制敏感，机制诊断显示尽管损失相似，但两种模型使用其门控方式不同，结果表明模型适应了预训练期间可用的门控几何形状，在测试的规模下，SwiGLU的开放正尾对于仅解码器语言模型FFN不是必要的

Conclusion: SwiGLU的开放正尾对于仅解码器语言模型FFN不是必要的
...
### [319] [When GNNs Fail: Quantifying and Overcoming Temporal Correlation Volatility in Time Series](https://arxiv.org/abs/2608.07333)
*Chen Shao,Yue Wang,Zhenyi Zhu,Zhanbo Huang,Tobias Käfer,Zonghan Wu,Danai Koutra*

Main category: cs.LG

TL;DR: 本文提出了一种新的GNN层GLIDE，用于提高动态环境下的时间序列预测性能。


<details>
  <summary>Details</summary>
Motivation: 通过将多元时间序列表示为图，其中单个序列作为节点，成对时间相关性作为边，这一方法已经得到广泛关注。

Method: 我们首先提出时间相关性波动（TCV），这是一个模型无关的度量，旨在量化这些潜在结构的分布演化。接着，我们提出了一种新的GNN层，称为动态环境中的推理图层（GLIDE），它通过两种理论上的设计机制来增强：路径基于的消息传递和静态与动态传播分离。

Result: GLIDE在静态和动态设置下提高了平均性能，最大增幅达到85.7%。

Conclusion: 本文研究了GNN在静态和动态设置下的代表性，并提出了一个名为GLIDE的新型GNN层，以提高动态拓扑下的学习效果。
...
### [320] [Aftab: A Comprehensive Benchmark of CNN Encoders and Advanced Value Functions in Parallelized Q-Networks](https://arxiv.org/abs/2608.07335)
*Taha Shieenavaz,Shabnam Zareshahraki,Loris Nanni*

Main category: cs.LG

TL;DR: 本研究提出了一种新的卷积神经网络架构Aftab，用于深度强化学习，在多个基准测试中表现出色。


<details>
  <summary>Details</summary>
Motivation: 近年来，深度强化学习在简化、高度并行化的范式上取得了进展。然而，在无需依赖计算成本高昂的重放缓冲区或目标网络的离策略学习环境中，视觉编码器的表示能力和参数效率仍被低估。

Method: 本研究系统地研究了用于PQN的卷积神经网络架构设计空间。设计了8种不同的CNN拓扑，在严格的参数约束下优化样本效率。此外，通过整合Hadamax编码范式和高级Q学习扩展（包括分布性、集成和对抗头），研究了表示和价值估计改进的影响。

Result: 在Atari-57基准测试中，提出的复合架构Aftab实现了6.479的Interquartile Mean（IQM）Human-Normalized分数，比标准PQN基线提高了0.86的概率。在高度非平稳的Procgen Hard基准测试中的结构弹性评估确认了分布外泛化，Aftab的IQM Procgen Normalized分数为0.418，而基线为0.382。

Conclusion: 本研究建立了一个高效、概率上更优的结构参考，用于无模型的强化学习，同时保持了无缓冲、并行优化的简单性和内存效率。
...
### [321] [Residual Algebra for Representation-Preserving Learning](https://arxiv.org/abs/2608.07349)
*Yao Wu*

Main category: cs.LG

TL;DR: 通过代数化残差和明确的残差所有权与组合，提高了投资收益。


<details>
  <summary>Details</summary>
Motivation: 学习异构表示通常被简化为特征拼接，这会抹去产生错误的表示。

Method: 将残差代数化，实现对象作为点时条件均值场在10x10秩网格上。FPRC-PQ实现代数作为放松-聚合-关闭：每个字段通过调整其自身坐标中的残差进行放松；校正的字段在固定的均值处相遇，这是唯一的身份擦除边界；共享学习器仅关闭聚合的新残差。

Result: 在2023-2026年的3.67M中国A股股票日观察下，评估的基础代数将净收益从13.52%提高到19.10%，夏普比率从1.42提高到2.09。

Conclusion: 通过使残差所有权和组合明确，同时保留表示身份，提高了收益。
...
### [322] [FedDOSE: Federated Learning Framework Decomposing Site Effects for Modeling Brain Dynamic Functional Connectivity](https://arxiv.org/abs/2608.07393)
*Deepank Girish,Yi Hao Chan,Yubin Zheng,Sukrit Gupta,Jagath C. Rajapakse*

Main category: cs.LG

TL;DR: FedDOSE通过分解站点差异和引入动态功能连接分析，提高了多站点fMRI数据集的分析效果。


<details>
  <summary>Details</summary>
Motivation: 在多站点fMRI数据分析中，统计异质性和站点差异是关键挑战。现有联邦学习方法依赖于静态功能连接（FC），忽略了大脑网络的动态信息。

Method: 提出FedDOSE，一种新的框架，显式分解站点差异以分析动态功能连接（dFC）。FedDOSE引入了模块性引导的Tucker分解块来编码高维dFC张量，并有效地捕获模块级别的时空模式。在所有站点生成类特定的原型，并通过结合最优传输（OT）质心公式和Procrustes分析在全球范围内进行对齐。

Result: 在诊断自闭症谱系障碍（ASD）和注意力缺陷多动障碍（ADHD）的实验中，FedDOSE在ASD和ADHD检测方面优于最先进的方法。

Conclusion: FedDOSE在从多站点数据集中学习稳健表示以进行可靠分析方面表现出有效性。
...
### [323] [Beyond Post-Hoc Temperature Scaling: Bilevel Optimization for LLM Calibration](https://arxiv.org/abs/2608.07419)
...
### [324] [Beyond Myopic World Models: Long-Horizon End-to-End Training for Direct Future Prediction](https://arxiv.org/abs/2608.07420)
*Xinyi Li,Zaishuo Xia,Chenjie Hao,Yubei Chen*

Main category: cs.LG

TL;DR: DPWM通过端到端预测目标优化长时间预测，提高了世界模型在长时间预测任务中的性能。


<details>
  <summary>Details</summary>
Motivation: 传统的世界模型训练方法存在局限性，难以支持长时间预测。

Method: 提出直接预测世界模型（DPWM），通过端到端预测目标优化长时间预测。

Result: DPWM在长时间预测任务中优于传统方法，并提高了长时间预测的准确性。

Conclusion: DPWM通过优化长时间预测目标，提高了世界模型在长时间预测任务中的性能。
...
### [325] [Diffusion LLMs as Targets and Adversaries: Mechanistic Safety Exploits](https://arxiv.org/abs/2608.07430)
*Elena Dumitrescu,Gert Lek,Lydia Y. Chen,Jérémie Decouchant*

Main category: cs.LG

TL;DR: DLLMs的安全机制存在漏洞，SN-Guided Diffusion提供了一种有效的解决方案。


<details>
  <summary>Details</summary>
Motivation: 研究DLLMs作为目标和对手，揭示基于扩散的对齐的机制漏洞。

Method: 首先展示了DLLMs中的安全对齐仍然稀疏且可跨架构迁移，然后介绍了SN-Guided Diffusion，一个完全离线的黑盒越狱框架。

Result: DLLMs从自回归前辈初始化继承了相同的安全足迹，通过直接安全神经元映射和剪枝进行迁移攻击。SN-Guided Diffusion在多个目标和私有目标上实现了高达77.1%的迁移攻击成功率。

Conclusion: DLLMs的安全机制仍然需要改进，SN-Guided Diffusion是一种有效的黑盒越狱框架。
...
### [326] [Mixture of Geodesic Factor Analyzers on Riemannian Homogeneous Spaces](https://arxiv.org/abs/2608.06971)
*Hengchao Chen,Yuanyao Tan,Chao Huang,Hongtu Zhu,Qiang Sun*

Main category: stat.ML

TL;DR: MGFA：黎曼同胚空间上的混合测地因子分析，有效聚类流形值数据。


<details>
  <summary>Details</summary>
Motivation: 介绍在黎曼同胚空间上的混合测地因子分析（MGFA），用于对具有各向异性子群体的流形值数据进行聚类。

Method: 使用测地因子模型在每个混合成分内，提出迭代估计算法，并在球面、形状空间和双曲空间上实现。

Result: MGFA在指定情况下显著优于竞争方法，在模型误设下仍具有鲁棒性。案例研究证明了MGFA在二维轮廓和三维形状分析中的有效性。

Conclusion: MGFA是一种有效的流形值数据聚类方法，在多个领域具有应用潜力。
...
### [327] [Optimized Certainty Equivalent Risk Minimization Using Samples: Algorithms, Convergence Rates, and Applications](https://arxiv.org/abs/2608.07113)
*Sumedh Gupte,Prashanth L. A.,Sanjay P. Bhat*

Main category: stat.ML

TL;DR: 本文提出了一种基于OCE优化的新方法，并将其应用于金融和机器学习领域。


<details>
  <summary>Details</summary>
Motivation: 优化OCE风险，应用于金融投资组合优化、机器学习中的不确定性量化、分类和回归等。

Method: 提出了一种基于OCE和UBSR之间联系的特征描述，从UBSR的经典样本平均近似（SAA）形成OCE估计器。

Result: 推导了OCE估计器的均方误差（MSE）界限，提出了基于SG算法的OCE优化方法，并进行了实验验证。

Conclusion: 该研究为OCE优化提供了一种有效的方法，并应用于金融和机器学习领域。
