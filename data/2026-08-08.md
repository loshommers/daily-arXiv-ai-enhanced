...
TL;DR: MapTCL通过引入时间一致性损失，显著提高了动态城市环境中的在线高精度地图构建的可靠性。


<details>
  <summary>Details</summary>
Motivation: 动态城市环境中，由于移动物体和遮挡，构建可靠的在线高精度地图仍然具有挑战性。

Method: 提出MapTCL，通过双向对齐在当前帧和过去帧之间建立时间一致性损失。Bidirectional Vector Consistency Learning (BVCL)将关联的过去和当前向量实例之间的几何和语义差异建模为辅助损失。此外，还采用Raster map Consistency Learning (RCL)作为额外的损失来稳定密集的BEV特征。

Result: 在两个标准基准测试中，MapTCL显著提高了生成的HD地图的时间稳定性。MapTCL作为通用模块，在不增加推理开销的情况下，在nuScenes和Argoverse 2上分别实现了+3.7 mAP & +2.8 C-mAP和+3.1 mAP & +2.5 C-mAP的增益。

Conclusion: MapTCL通过引入时间一致性损失，有效提高了动态城市环境中的在线高精度地图构建的可靠性。
...
TL;DR: 本研究针对T2I系统产生的仇恨性视觉故事进行了研究，提出了新的防御措施，以提高安全性。


<details>
  <summary>Details</summary>
Motivation: 为了解决由文本到图像（T2I）系统产生的仇恨性视觉故事在组级别上的仇恨意义未被充分探索的问题。

Method: 引入了HatefulStoryPrompts，包含330个多轮配置，并评估了五个前沿模型。此外，还评估了现有的 moderation 系统在HatefulVisualStory数据集上的表现，并提出了补充的主动和生成后防御措施。

Result: 所有模型都完成了超过80%的故事，最强的达到了99.0%。现有的 moderation 系统在识别组级别仇恨意义方面表现不佳，而我们的主动和生成后防御措施在识别上有所提升。

Conclusion: 随着图像生成从孤立输出发展到连贯的视觉叙事，安全性必须相应地从每图像的审查发展到对交互和图像关系的状态推理。
...
TL;DR: StyleComposer通过路由和协调风格属性，提供了一种更灵活和精确的风格合成方法。


<details>
  <summary>Details</summary>
Motivation: 现有的参考引导方法将颜色、纹理和结构作为单一风格信号进行传输，导致每个属性的来源和强度无法由用户控制。

Method: StyleComposer通过将每个风格属性路由到其最佳分离表示，并在去噪时间上协调这些路由。

Result: StyleComposer无需训练或逆变换，比以往的方法更紧密地满足三个参考和提示，并为每个属性提供一个强度滑块。

Conclusion: StyleComposer提供了一种更灵活的风格合成方法，允许用户更精确地控制每个风格属性的来源和强度。
...
TL;DR: In-Context Forcing提高了视频去噪性能和推理速度。


<details>
  <summary>Details</summary>
Motivation: 传统的多步自回归视频扩散模型依赖之前的完全去噪清晰帧作为上下文，导致时间语义和动态受损。

Method: 提出了一种名为In-Context Forcing的渐进式自回归范式，利用逐渐降低噪声水平的上下文，实现自适应指导。

Result: 实验表明，该方法在视觉保真度和推理速度上均显著优于现有技术。

Conclusion: In-Context Forcing方法提高了视频扩散模型的时序一致性和动态表现，并加速了推理过程。
...
TL;DR: 提出PU-DPO框架，有效解决放射学报告生成中的遗漏噪声问题，提高检测率和隐藏阳性恢复。


<details>
  <summary>Details</summary>
Motivation: 为了解决传统放射学报告生成模型中存在的遗漏噪声问题，该研究提出了PU-DPO框架。

Method: 提出了一种基于正负样本优化的框架，通过对比学习，生成包含或省略特定发现的变体，以优化模型偏好信号。

Result: 在半合成实验和真实世界胸部X光图基准测试中，PU-DPO在检测率和隐藏阳性恢复方面优于先前方法，并且对遗漏噪声具有更强的鲁棒性。

Conclusion: PU-DPO框架有效提高了放射学报告生成模型的准确性和鲁棒性。
...
TL;DR: 提出了一种新的对象级3D变化检测方法，在Subiaco和Urb3DCD-V2基准测试中取得了显著成果。


<details>
  <summary>Details</summary>
Motivation: 高清晰3D激光雷达地图对自动驾驶和智慧城市服务至关重要，需要可靠地检测多时相城市激光雷达中的对象级变化，以保持数字地图与物理世界的一致性。

Method: 提出了一种对象级3D变化检测流程，该流程集成了检测限制感知的配准、几何驱动的对象代理以及基于规则的语义和实例分割，以及高度、体积和表面法线方向中的位移线索，以分配五个变化标签并具有置信度。通过解耦配准、几何和语义，该流程将姿态不确定性传播到空间变化的检测限制中，稳定了跨时代对应关系，并抑制了由残余错位和密度变化引起的错误变化。

Result: 在Subiaco地区的LoDA基准测试中，该方法实现了95.0%的准确率、90.8%的宏观F1和83.0%的宏观IoU，超过了最佳基线8.7个IoU点和4.4个F1点。在公共Urb3DCD-V2基准测试中，该方法达到了96.81%的平均准确率和89.52%的平均变化IoU，在mAcc和mIoUch方面分别比最强的基线提高了1.36点和3.18点。

Conclusion: 该方法在对象级3D变化检测方面表现出色，显著优于现有基线，为自动驾驶和智慧城市建设提供了可靠的技术支持。
...
TL;DR: 开发了一种基于VoxTell的轻量级框架，用于胶质瘤亚区域分割细化，提高了分割精度。
...
Method: 开发了一个基于VoxTell的轻量级框架。预训练的VoxTell生成初始掩码。来自分割错误的Oracle提示编码目标、动作、位置、影像证据、编辑大小和保留约束。冻结的Qwen/VoxTell提示嵌入通过可训练的投影注入到其多尺度解码器条件中；其他权重保持冻结。训练、验证和测试使用了901、100和250个BraTS-GLI病例。在100个脑膜瘤、转移、儿童肿瘤和UPENN-GBM病例上评估了跨数据集迁移。
...
Conclusion: 3D视觉语言基础模型可以执行指令引导的胶质瘤亚区域分割细化。对正确、空白和矛盾提示的敏感性表明文本依赖的轮廓编辑，而不是非特定的后处理，支持作为临床医生在环工具的进一步评估。
...
TL;DR: 通过引入先验知识，该方法在视觉提示调整中提高了参数效率，并在图像分类任务中取得了良好的效果。


<details>
  <summary>Details</summary>
Motivation: Prompt tuning 在 NLP 中是一种高效的参数调整范式，最近被扩展到计算机视觉领域。然而，现有的视觉提示是随机初始化的，没有利用先验知识，如 NLP 中的指令。

Method: 提出将两种互补的语义先验注入到 VPT 中。基本图像先验，包括颜色、纹理和形状，通过经典的手工制作算子提取并注入到输入空间，而自注意力图在特征空间中提供实例感知语义。进一步提出了一种级联方案，将两种先验在整个 ViT 调整过程中整合。

Result: 在 34 个具有挑战性的图像分类数据集上的实验表明，在仅调整 0.74% 的 ViT 参数的情况下，实现了优越的下游调整。

Conclusion: 该方法在视觉提示调整中引入了先验知识，提高了参数效率，并在图像分类任务中取得了良好的效果。
...
TL;DR: 研究发现，深度视觉模型中的捷径学习源于像素级别的不可见元数据痕迹，并通过控制元数据-语义相关性来提高模型性能和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: Identify the source of shortcut learning in deep vision models.

Method: Analyze invisible metadata traces at the pixel level, and introduce controlled metadata-semantics correlations.

Result: Stronger metadata-semantics correlations lead to higher sensitivity to metadata traces and larger performance degradation under metadata distribution shifts. Mitigation strategies reduce sensitivity without sacrificing performance on downstream tasks.

Conclusion: Metadata sensitivity can explain the strong generated-image detection ability of some encoders, and its mitigation can improve out-of-distribution generalization.
...
TL;DR: MOSAIK通过动态调整块大小和预测保真度损失，在减少计算成本的同时提高了图像生成的性能。


<details>
  <summary>Details</summary>
Motivation: Pixel-space扩散模型直接在图像空间中生成，避免了潜在扩散模型的重建天花板。然而，它们更高的token计数导致生成成本高，因为自注意力的二次复杂度。

Method: MOSAIK框架通过在不同区域和去噪步骤中改变块大小来解决这个问题。它将PixelDiT主干适应于生成任意异构的块布局，并使用轻量级的预测器来估计由块粗化引起的保真度损失。

Result: MOSAIK在减少70%的FLOPs和83%的token计数的同时，与全计算的PixelDiT在GenEval上匹配，其DPG-Bench分数仅下降1.0分。与各种效率范例相比，包括时间块调度和特征缓存，该方法在适度预算下提供具有竞争力的性能，并且在高度受限的计算环境中始终优于这些基线。

Conclusion: MOSAIK通过动态调整块大小，实现了高效且性能优异的图像生成。
...
TL;DR: CDSeg是一种高效的跨域分割方法，可以处理大规模场景，并实现高精度分割。


<details>
  <summary>Details</summary>
Motivation: 现代图像模型能够提供关于每个视图应分割什么的强烈线索，但它们的掩码本身并不能确定这些标签在3D中应持续的位置。

Method: 提出了一种名为CDSeg的跨域分割方法，该方法不需要特定的3D分割训练，并使用高斯原语作为可渲染的标签载体。外部掩码源提供标签，而渲染器导出的可见性确定哪些3D原语接收它们。载体通过将每个输入点完成到一个高斯中，保留其索引，或重用优化高斯场景的本地原语来实例化。CDSeg在渲染过程中记录像素-原语关联，并通过投票和局部过滤器融合多视图掩码。结果标签可以返回到原始点，保留在本地高斯场景中，或渲染到其他视图。

Result: CDSeg可以处理包含数百万个原语的场景，并在几秒钟内完成。它在DesktopObjects-360上获得了92.35%的mIoU，在NeRDS-360上获得了95.89%，在ScanNet-v2验证集上获得了65.77%。

Conclusion: CDSeg提供了一种接口，用于在点云、高斯场景和图像视图之间重用2D掩码，而无需特定于任务的3D分割网络。
...
TL;DR: VideoArgus是一个统一的基于评分准则的视频生成和编辑评估框架，在多个任务中表现出色。


<details>
  <summary>Details</summary>
Motivation: 评估生成的视频仍然具有挑战性，因为现有的基准依赖于固定的评估内容，只涵盖生成和编辑设置的一部分，并且为其分数提供有限的证据。

Method: 引入VideoArgus，这是一个统一的基于评分准则的框架，涵盖五个视频生成和编辑设置。对于每个输入实例，VideoArgus生成一个一次性的输出盲、样本特定的评分准则，并重复使用它来评估所有相应的候选视频。评分准则定义了具体的标准、评分规则、失败模式和证据计划，这些指导特定标准的VLM QA和视觉工具产生基于证据的评分、理由和诊断报告。进一步构建VideoArgus-Bench，包含1,026个经过策划的输入实例，由653张高质量图像和416个高质量视频构建，所有基准评分准则预先生成、冻结和发布。在一个独立的1,260个视频的人为对齐集中，VideoArgus在所有五个任务中，与相应的基准特定评估器相比，实现了更高的输入Spearman和Kendall相关性。模型排名在不同评分准则生成和评估VLM骨干之间也保持高度一致。所有代码和数据都已发布。

Result: 在独立的1,260个视频的人为对齐集中，VideoArgus在所有五个任务中，与相应的基准特定评估器相比，实现了更高的输入Spearman和Kendall相关性。模型排名在不同评分准则生成和评估VLM骨干之间也保持高度一致。

Conclusion: VideoArgus是一个有效的视频生成和编辑评估框架，能够提高评估的准确性和一致性。
...
TL;DR: DynaPix基准揭示了模型在时间锚定方面的不足。


<details>
  <summary>Details</summary>
Motivation: 当前评估通常接受词语或看似逼真的图像，但预测状态从未与真实状态进行比较。

Method: 引入DynaPix基准，要求模型从相似候选图像中选择真实未来图像，或在大型画廊中进行搜索。

Result: DynaPix使预测可检查，并揭示了模型在时间锚定方面的不足。

Conclusion: DynaPix揭示了模型在时间锚定方面的差距，即模型在将预测与事件相关联方面比与时间本身相关联方面做得更好。
...
TL;DR: OmniMech基准为评估视觉-语言模型在可执行CAD生成方面的性能提供了新的框架，揭示了当前模型的不足。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉-语言模型在从图像生成可执行CAD程序方面取得进展，但主要针对粗略的通用3D对象，很少涉及工业机械设计中所需的精细几何和毫米级公差。

Method: 提出OmniMech，第一个用于评估从工业制造数据生成可执行CAD程序的百万规模基准。OmniMech包含超过251,000个完全标注尺寸和公差的二维正投影图，配对原生物件模型、多视图渲染、网格、STEP和B-rep表示，以及丰富的语义标注。基准包括四个任务：1）从工程图纸合成参数化CAD程序；2）进行几何和结构一致重建的图到3D推理；3）基于标注的推理，涉及尺寸、符号、特征标注和制造约束；4）使用可视化、测量、CAD执行和验证工具的工具增强代理推理。

Result: 实验表明，当前的VLMs和CAD专用模型在可执行程序合成、精细3D重建和可靠执行尺寸和公差方面仍存在困难。

Conclusion: OmniMech基准为VLMs在可执行CAD生成方面的评估提供了新的框架，并指出了当前模型的局限性。
...
TL;DR: HFM通过扩展流匹配到双级别结构，实现了高效且性能优越的无条件3D点云生成。


<details>
  <summary>Details</summary>
Motivation: 现有方法在全局形状拓扑和局部几何细节的捕获上存在局限性，忽略了3D形状的自然分层结构。

Method: HFM通过将流匹配扩展到双级别结构，实现了无条件3D点云生成。它将任务分解为两个级别：潜流匹配和条件点流匹配。

Result: HFM通过在潜空间中建模全局形状流形并条件重建点云，实现了高效的3D点云生成。

Conclusion: HFM在ShapeNet和ModelNet基准测试中表现出色，与现有方法相比具有竞争力甚至最佳性能。
...
TL;DR: MLLM在主动物理危害预测方面存在显著不足，需要改进以实现可靠的安全预警。


<details>
  <summary>Details</summary>
Motivation: 及时预测物理危害对于现实世界安全至关重要，但现有的MLLM评估主要集中在有害内容或一般风险上，而对主动物理危害预测探索不足。

Method: 提出了一种名为SPRINT的基准，包含2,888个真实世界体育视频，用于评估MLLM在主动物理危害预测方面的能力。

Result: 评估结果表明，即使在最好的模型中，对危害的敏感性超过了95%，但对其原因的识别却低于50%。此外，显式危险查询甚至在无危害的视频上也引发了严重的误报。

Conclusion: 当前MLLM在主动物理安全方面表现不佳，缺乏稳定、基于原因的早期预警，强调了在动态物理环境中实现可靠主动安全的需求。
...
TL;DR: EffectLearner：一种高效的视频对象去除框架，在复杂场景下表现优异。


<details>
  <summary>Details</summary>
Motivation: 针对现有视频对象去除方法在处理复杂场景时的局限性，提出了一种新的方法。

Method: 提出EffectLearner框架，结合VLM和DiT，进行跨模态推理和对象去除。

Result: 在ROSE-Bench和EffectWorld数据集上优于现有方法，证明在复杂场景下具有良好的视频对象去除效果。

Conclusion: EffectLearner能够有效去除复杂场景中的视频对象，并在标准数据集上取得了优异的性能。
...
TL;DR: CoordRefer通过解耦坐标框架选择和坐标条件化定位，提高了多视角图像3D视觉定位的性能。


<details>
  <summary>Details</summary>
Motivation: 为了解决现有方法中坐标相对的框模糊和视觉定位性能下降的问题

Method: 提出了CoordRefer，一个坐标感知框架，将坐标框架选择与坐标条件化定位解耦。首先选择参考框架定义坐标系，然后对3D框预测进行坐标系条件化。执行坐标感知的监督微调，建立坐标框架选择和坐标条件化框回归，随后使用基于3D IoU的奖励进行组相对策略优化，以将两个阶段与下游定位质量对齐。

Result: 在ScanRefer with Qwen3-VL-2B上，CoordRefer在Acc@0.25上比坐标无关基线提高了11%，在Acc@0.5上提高了7%，其几何细化变体超过了使用显式3D输入的方法。

Conclusion: CoordRefer在多视角图像3D视觉定位任务中表现优异，有效解决了坐标模糊问题，提高了定位精度。
...
<details>
  <summary>Details</summary>
Motivation: MLLMs在视频理解方面取得了进展，但仍然具有挑战性，因为令牌限制使得MLLMs难以捕捉时间上稀疏的证据。

Method: VideoRouter(VR)将长视频理解重新构思为协调互补的证据视图，而不是选择单个帧的子集。它首先将每个视频组织成一个与问题无关的时间层次结构，将视频划分为从粗到细的时间上连贯的段。在这个层次结构中，上层节点捕获广泛的剧情背景和事件进展，而底层节点保留细粒度的局部细节和承载证据的时刻。这自然产生了两个互补的视图：一个全局视图用于覆盖导向的推理，一个局部视图用于细节导向的证据恢复。进一步引入一个验证引导的路由器来确定哪个视图被选定的证据更好地支持，并选择最终的答案。

Result: 通过广泛的实验验证了所提出设计的效果，表明验证引导的路由器有效地协调了全局和局部推理，在VideoMME上，我们的方法在LLaVA-Video-7B主干网络上优于最先进的帧选择方法2.9个点。

Conclusion: VideoRouter(VR)通过协调互补的证据视图，有效地解决了MLLMs在视频理解中的挑战，提高了视频理解的准确性和效率。
...
TL;DR: UA-NWM improves aerial image-goal navigation by addressing limitations of existing methods, offering robust performance and low inference latency.


<details>
  <summary>Details</summary>
Motivation: Existing methods based on world models in aerial image-goal navigation rely on only one or a few point predictions, which is inadequate for large-scale outdoor environments with substantial future-state uncertainty.

Method: Proposing the Uncertainty-Aware Navigation World Model (UA-NWM), an efficient latent world model for aerial image-goal navigation, which formulates trajectory scoring as conditional out-of-distribution detection.

Result: UA-NWM represents plausible futures with an uncertainty subspace and decomposes the prediction--goal discrepancy into uncertainty-explainable and unexplainable components. Only the unexplainable residual is used for scoring, enabling robust selection without multiple future samples. Extensive experiments demonstrate that UA-NWM consistently outperforms existing navigation world models while maintaining low inference latency. Real-world UAV experiments further validate its practical applicability.

Conclusion: UA-NWM is an efficient and robust method for aerial image-goal navigation, providing a significant improvement over existing approaches.
...
TL;DR: ALTER是一种从3D CT自动生成放射学报告的方法，在多个数据集上取得最先进成果。


<details>
  <summary>Details</summary>
Motivation: 自动从3D CT生成准确的放射学报告具有挑战性，现有的方法无法充分模拟个体区域内患者的纵向变化。

Method: ALTER方法通过全局优先集成、区域代理差异和间隔变化融合等技术，解决从3D CT生成准确和完整的放射学报告的挑战。

Result: ALTER在大多数评估指标上实现了最先进的成果，包括RadGenome-ChestCT验证和CTRG-Chest-548K测试集。

Conclusion: ALTER在RadGenome-ChestCT验证和CTRG-Chest-548K测试集上的大多数评估指标上实现了最先进的成果。
...
TL;DR: 提出了TruthLens，一个自我评估框架，用于解决LVLMs中的对象幻觉问题，显著提高了模型在对象识别方面的性能。


<details>
  <summary>Details</summary>
Motivation: 尽管大型视觉语言模型（LVLMs）取得了显著进展，但对象幻觉仍然是一个基本挑战，阻碍了它们的可靠部署。我们的工作主要受到以下关键发现的启发：真实和幻觉对象标记在隐藏表示中可以清楚地分离，但这种可分离性在语言建模（LM）头部在很大程度上丢失。

Method: 我们提出了TruthLens，这是一个自我评估框架，它教会LM头部在不使用任何辅助模型或额外推理成本的情况下，暴露每个对象的真伪信号。具体来说，我们重新利用了一个很少使用的特殊标记作为参考标记。对于每个对象标记位置，我们提取LM头部分配给这个特殊标记的对数概率，并将其与预定义常数的差定义为真伪分数。然后，使用一个将分数推向1（真实对象）和0（幻觉对象）的均方误差（MSE）目标对模型进行微调，同时使用散度约束来保留原始生成能力。

Result: 尽管只在有限的对象类别集上训练，但TruthLens在具有更大标签空间的基准测试中表现出良好的泛化能力。在多个LVLMs上进行的广泛实验表明，该模型达到了最先进的性能；值得注意的是，在Qwen2.5-VL-7B上，TruthLens在MS-COCO上的AUROC超过了先前最佳方法17%以上。

Conclusion: TruthLens是一个有效的自我评估框架，可以显著提高LVLMs在对象识别方面的可靠性。
...
TL;DR: DOME-HDR：高效HDR重建框架，感知平衡，一致性出色。


<details>
  <summary>Details</summary>
Motivation: 提出DOME-HDR，一个双输出多曝光HDR重建框架，通过增益图逆色调映射同时生成感知平衡的SDR图像和一致性的HDR图像。

Method: 使用LoRA自适应潜在扩散模型合成基础SDR，双重交叉注意力融合模块注入互补的结构和颜色线索，锚定中间曝光以保持稳定性。使用HDR Prior引导的增益图网络（HPGM）预测空间变化的增益图。

Result: 在Kalantari、Tel和Challenge123上评估，DOME-HDR在HDR重建质量上达到最先进水平；消融实验进一步证实了双重交叉注意力和SDR引导的增益图估计的有效性。

Conclusion: DOME-HDR是一个高效的HDR重建框架，在感知平衡和一致性方面表现出色。
...
TL;DR: SCI-CLIP是一种无需训练的开放词汇分割方法，通过区域抽象提高分割质量


<details>
  <summary>Details</summary>
Motivation: 解决无监督开放词汇分割中缺乏推理抽象的问题

Method: 提出SCI-CLIP，一个基于区域抽象的推理框架，包括：1. 诱导区域一致交互图；2. 通过图传播值重构密集特征；3. 构建和查询离线参考记忆库；4. 将冻结的CLIP风格特征转换为密集预测

Result: 在八个基准测试中，SCI-CLIP在密集预测的结构质量、上下文推理的鲁棒性和基于示例校正的对齐方面均有显著提升

Conclusion: SCI-CLIP是一种无需训练的开放词汇分割方法，在多个基准测试中表现出色
...
TL;DR: ChronoVision通过将视觉逻辑与潜在图像对齐，在复杂视觉认知任务中取得了显著的性能提升。


<details>
  <summary>Details</summary>
Motivation: 为了解决多模态大型语言模型在复杂视觉认知任务中的困难，特别是需要多步时间推理的任务。

Method: 提出ChronoVision，一个多模态框架，旨在将视觉逻辑与潜在图像对齐。在监督微调期间，使用重建视觉头部预测最终转换状态的潜在表示，同时ROI注意力定位模块通过语义跨度查询将模型集中在关键视觉证据上。在训练后，应用强化学习，具有隐式过程定位机制，由一个综合奖励函数引导，该函数评估结果正确性、潜在过程对齐和无监督视觉关注。此外，引入Vbvr-VQA，一个将视频推理重新表述为严格图像排序任务的全新数据集。

Result: 在Vbvr-VQA上达到最先进的性能，领域内准确率为74.8%，领域外准确率为71.6%，在IntPhys2（一个具有挑战性的跨领域基准）上的准确率为55.0%。

Conclusion: ChronoVision在处理复杂视觉认知任务方面表现出色，特别是在时间跟踪方面。
...
TL;DR: Vorch-IR：一种统一的视频身份替换框架，实现单人和双人身份替换，以及可选的背景替换，提高了身份保留和运动保真度。


<details>
  <summary>Details</summary>
Motivation: 视频身份替换旨在在保留驾驶视频的运动、表情和时间结构的同时，转移一个或多个主体的身份。现有方法主要针对单人设置，通常需要特定于任务的结构性控制，如掩码或姿态表示，限制了它们在通用多模态编辑系统中的灵活性。多人在替换方面的进展进一步受到配对训练数据稀缺的限制。

Method: 我们提出了Vorch-IR，这是一个统一的框架，支持单人和双人身份替换，以及可选的背景替换，在单个模型中。它基于LTX2，联合对驾驶视频、索引参考图像和文本编辑指令进行条件化。参考图像不需要与驾驶视频的姿势、布局或空间配置匹配：它们的角色作为主体或背景参考通过指令指定。通过自注意力融合密集视觉条件，而通过交叉注意力建立视觉-语言上下文，以建立语义对应。我们还开发了一个自动数据构建管道，为所有四种编辑设置合成配对监督。

Result: 实验使用自动指标和成对人工评估证明了在多样化场景中强大的身份保留、运动保真度和时间一致性。时间重叠推理策略还扩展了短剪辑模型，无需自回归延续即可生成长达一分钟的生成内容。

Conclusion: Vorch-IR在视频身份替换方面取得了显著进展，提高了身份保留、运动保真度和时间一致性，并扩展了短剪辑模型的能力。
...
TL;DR: Vorch-Streamer是一种解决实时长格式头像音频-视频生成挑战的框架，实现了高质量的实时T2AV流。


<details>
  <summary>Details</summary>
Motivation: 为了解决实时长格式头像音频-视频生成中存在的挑战，例如暴露偏差和视觉漂移，以及如何在没有足够局部音频-视频上下文的情况下确定下一步说话的内容。

Method: 提出了一种名为Vorch-Streamer的框架，该框架在训练过程中使用了合成语料库，并应用了长时域自我强制和DMD蒸馏。此外，还使用外部语言模型预测语音规划令牌，以控制语音进度。

Result: Vorch-Streamer在27.12 FPS的速度下同时生成音频和视频，超过了24-FPS的实时播放率，同时保持了音频-唇同步和长格式生成中的身份保留。

Conclusion: Vorch-Streamer有效地解决了实时长格式头像音频-视频生成中的挑战，实现了高质量的实时T2AV流。
...
TL;DR: 开发了一种新的模拟器，提高了毫米波人体活动识别的准确性和效率。


<details>
  <summary>Details</summary>
Motivation: 为了解决毫米波人体活动识别在用户相对于传感系统方向变化时性能下降的问题，同时减少收集多方向数据的劳动强度和成本。

Method: 开发了一种基于物理引导的模拟器，该模拟器从单方向运动中合成具有不同方向的无线训练数据。提出了一种双注意力网络，从双链路多普勒频谱图中提取活动判别性和方向鲁棒性表示。引入了一种对抗性无监督迁移学习机制，仅使用少量未标记的目标域样本对特征分布进行对齐。

Result: S2M-Sense平台在重现现实世界特征方面表现出高保真度，与60.48 GHz毫米波测量数据进行了验证，所有4种活动和4个方向上的模拟和测量多普勒频谱图之间的平均结构相似性指数（SSIM）为0.84。实验结果表明，S2M-Sense仅使用双链路多方向模拟数据集就实现了88.33%的识别准确率，在仅使用16个未标记的测量样本进行模拟到现实的迁移学习后，准确率提高到了95%。两种情况（有无迁移学习）都优于最先进的跨域传感方法。

Conclusion: 提出的方法有效提高了毫米波人体活动识别的性能，并减少了数据收集的成本。
...
<details>
  <summary>Details</summary>
Motivation: 针对现有RGB-D语义分割方法中深度表示捕获不足和计算冗余的问题

Method: 提出URNet，一个统一的重新参数化RGB-D网络，在单个编码器中同时执行多模态特征提取和跨模态融合

Result: 在多个RGB-D分割基准测试中实现最先进的性能，同时保持高效性

Conclusion: URNet在保持高效性的同时，显著提升了RGB-D语义分割的性能
...
TL;DR: DistMedVL通过概率跨模态适配器提高了医学图像分割的性能。


<details>
  <summary>Details</summary>
Motivation: 多模态医学图像理解中视觉和文本表示的对齐至关重要，但在现实临床条件下，两种模态都存在不确定性，这阻碍了现有方法的性能。

Method: 提出DistMedVL，一个概率视觉-语言框架，引入轻量级概率跨模态适配器（PCM-Adapter），在冻结编码器上显式建模表示不确定性。PCM-Adapter包含两个连续模块进行渐进概率对齐。首先设计了一个马氏对齐模块（MAM），将文本标记建模为高斯分布，并通过马氏距离计算补丁-文本兼容性，从而实现方差条件匹配，降低不可靠特征维度的权重。此外，还设计了一个分布流模块（DFM），估计模态置信参数，并执行视觉引导的文本分布细化，以适应成像模态之间的分布变化。

Result: 在八个医学分割基准上的大量实验表明，DistMedVL在仅6.3M可训练参数的情况下，优于最先进的方法，表现出卓越的数据效率、扰动鲁棒性和跨数据集泛化能力。

Conclusion: DistMedVL通过引入PCM-Adapter有效地建模了表示不确定性，提高了多模态医学图像理解中的性能。
...
TL;DR: SciQNet是一种有效的科学图像质量评估框架，在ICME 2026挑战赛中表现优异。


<details>
  <summary>Details</summary>
Motivation: 科学图像质量评估对于实验观察、定量证据和概念知识的传达至关重要。然而，由于它们的质量既取决于视觉清晰度，又取决于科学信息性，因此评估具有挑战性。

Method: 提出了一种名为SciQNet的两阶段多模态自适应框架，第一阶段在科学文档图像上进行领域自适应预训练，第二阶段进行任务特定的微调，包括联合评分和理解监督。

Result: 在评估的预训练数据比例中，使用40%的分层子集的数据表现出最佳性能。最终模型在SIQA-S得分为92.21，SIQA-U得分为47.38，综合得分为69.80。

Conclusion: SciQNet在ICME 2026科学图像质量评估挑战赛中获得了第二名，证明了该方法的有效性。
...
TL;DR: 提出TAU-Bench基准，用于评估VAU系统的实例跟踪和细粒度理解，发现语义推理与视觉基础之间存在差距。


<details>
  <summary>Details</summary>
Motivation: 为了评估视频异常理解（VAU）系统的可靠性和准确性，提出了一种新的基准TAU-Bench。

Method: 开发了一个包含异常事件跟踪和细粒度异常理解的基准TAU-Bench，包含大量视频、轨迹和像素级掩码，并使用自动化数据引擎进行构建。

Result: 评估结果表明，即使模型能够生成合理的异常解释，也可能无法可靠地定位和跟踪正确的实例，揭示了语义推理和视觉基础之间的差距。

Conclusion: TAU-Bench的引入强调了实例基础评估对于构建更可靠和准确的VAU系统的重要性。
...
TL;DR: StreamMind通过优化架构和记忆管理，在连续、交互式流视频理解方面实现了显著的性能提升。


<details>
  <summary>Details</summary>
Motivation: 部署自主多模态代理在连续、真实世界环境中需要他们摄取无界的视听流并保持小时的记忆。然而，当前的评估主要依赖于简短的剪辑和多项选择题格式。

Method: 我们引入StreamArena，这是一个小时级、交互式流视频理解的基准。StreamArena包含243个全长视频，平均88.8分钟，以及3,646个严格注释的开放式问答对，用于评估实时感知、历史回顾、主动交互和多模态工具利用。

Result: StreamMind在所有四种能力上都优于现有的流基线，并通过重用持久状态来减少查询到答案的延迟。

Conclusion: StreamMind通过将延迟关键交互和主动监控分配给独立安排的前端工作者，同时后端工作者异步构建持久的多模态记忆并执行历史回忆和外部搜索，解决了连续交互和长期多模态理解之间的紧张关系。
...
TL;DR: G$^2$ARD-GS is a geometry-guided distillation method for efficient 3DGS model compression and reuse.


<details>
  <summary>Details</summary>
Motivation: Dense colored LiDAR maps provide accurate city-scale geometry, but lifting them into 3D Gaussian Splatting (3DGS) retains millions of primitives, making the resulting models costly to store, transmit, render, and adapt.

Method: G$^2$ARD-GS, a geometry-guided distillation method that converts a dense Gaussian prior instantiated either as a training-free point-cloud lift or a trained GS model into a compact, reusable representation.

Result: On MatrixCity, G$^2$ARD-GS achieves the best PSNR, SSIM, and LPIPS across matched 5x--30x compression budgets, outperforming PUP by 3.2--6.8 dB in PSNR. When reused as frozen geometry, the compact model improves off-trajectory appearance adaptation by 3.7--4.9 dB over PUP 3D-GS and preserves image-to-model registration accuracy on Cambridge KingsCollege at 30x compression.

Conclusion: G$^2$ARD-GS is an effective method for reducing the cost of storing, transmitting, rendering, and adapting 3DGS models while maintaining high quality.
...
TL;DR: LAWM-3D模型通过多视图学习和三维感知，显著提升了世界模型的性能。


<details>
  <summary>Details</summary>
Motivation: 世界模型能够使智能体在不与现实世界互动的情况下执行前向运行和规划。然而，它们在开放世界具身智能中的应用受到动作标注高成本和不同平台间动作空间异质性的限制。最近，通过自监督方式直接从未标记的人类视频中学习动作表示的潜在动作模型（LAMs）缓解了这一瓶颈。尽管如此，大多数现有的LAMs依赖于单视图输入并在二维像素空间中主要运行，引发了一个基本问题：仅仅将多视图视频纳入LAM训练是否能够使学习到的潜在动作具备三维感知能力？

Method: 本研究提出了一种名为LAWM-3D的模型，该模型引入了三个紧密耦合的关键设计：(1)一种用于学习三维感知潜在动作的多视图不变统一动作标记方案；(2)一种几何对齐约束，将中间编码器特征锚定到预训练的三维基础模型，从而明确地提供跨视图几何对应关系；(3)一种非注入性RGB-D联合重建目标，防止从未来帧外观信息中进行捷径学习，迫使LAM专注于具有几何意义的运动线索。

Result: 广泛的实验表明，所提出的具有三维感知能力的潜在动作显著提高了世界模型性能，在生成质量、物理一致性和泛化能力方面达到了最先进的结果。

Conclusion: LAWM-3D通过结合多视图视频和三维感知能力，有效地提高了世界模型的性能。
...
TL;DR: MEC框架通过Matryoshka排名问题优化长视频帧选择，提高了准确率和减少了延迟。


<details>
  <summary>Details</summary>
Motivation: 由于长视频中帧的重叠和有限的上下文窗口，框架选择对于应用大型多模态模型（LMMs）至关重要。

Method: 将长视频帧选择建模为Matryoshka排名问题，构建一个单一优先级序列，其小前缀集中查询条件下的证据，而较大的前缀则逐步增加证据和时间上下文。

Result: MEC（Matryoshka Evidence-to-Context）框架提高了平均准确率，与最先进的选择器相匹配，并减少了端到端选择延迟。

Conclusion: MEC框架在长视频帧选择中表现出色，提高了准确率并减少了延迟。
...
TL;DR: 提出了一种新的无人机摄影测量方法，提高了重建精度和完整性


<details>
  <summary>Details</summary>
Motivation: 针对无人机摄影测量中传统飞行模式与场景几何不匹配导致的局部重建误差问题

Method: 提出了一种迭代混合离散-连续视点规划方法，用于从代理重建中进行目标无人机摄影测量

Result: 该方法通过摄影测量启发式方法对采样表面点进行评分，并评估完整视点集的可见性、成对重叠和图连通性，最终飞行路径结合了近距离细节视点和更广泛的模型覆盖视点，提高了重建精度和完整性

Conclusion: 该方法有效提高了无人机摄影测量的重建精度和完整性
...
TL;DR: PhyLatent通过多种途径解决JEPA世界模型的失败模式，显著提高了模型性能。
...
Method: 通过物理状态锚定、未来表示对齐、静态视觉不变性、反事实分支分离和潜在降噪等三种训练途径来解决物理不变性崩溃、物理可识别性崩溃和反事实动力学崩溃三种失败模式。

Result: 在OGBench-Cube上，PhyLatent将三种失败率从15.60%、6.71%和8.41%降低到7.53%、0.95%和4.62%，并将模型预测控制（MPC）成功率从70.0%提高到78.1%。在TwoRooms上，与相同的架构和规划者相比，成功率达到98.0%。
...
TL;DR: Engram-E2VID通过外观记忆痕迹的生成激活，实现了基于参考帧和事件流的高质量视频重建。


<details>
  <summary>Details</summary>
Motivation: 基于参考帧和事件流恢复目标RGB帧的挑战，尤其是在复杂运动和长时间间隔下。

Method: 提出Engram-E2VID框架，通过外观记忆痕迹的生成激活来重建目标帧。

Result: 在三个基准测试中，Engram-E2VID在PSNR上最多提高3.29 dB，在LPIPS上最多降低0.08，重建间隔增加时退化较慢。

Conclusion: Engram-E2VID在视频重建方面表现出色，特别是在处理复杂运动和长时间间隔时。
...
TL;DR: LiteKD-Net, a lightweight knowledge-distilled network for mobile image denoising, achieves good results in terms of restoration quality and computational efficiency.


<details>
  <summary>Details</summary>
Motivation: Mobile image denoising requires both good restoration quality and low computational cost, and it's difficult to collect large-scale LQ-GT clean pairs.

Method: Propose LiteKD-Net, a lightweight knowledge-distilled network for mobile image denoising. Generate paired training data using physics-guided noise simulation. Adapt Real-ESRGAN for identity-resolution denoising and construct a lightweight Student using Lite-RRDB blocks. Apply feature-level knowledge distillation to transfer restoration capability from Teacher to Student.

Result: Reduced runtime and increased inference rate with good restoration quality. Achieved the best results in all metrics compared to SwinIR.
...
TL;DR: 提出了一种新的FS-IAD模型，通过概念引导和动态注意力机制，在有限数据条件下实现了高性能的异常检测。


<details>
  <summary>Details</summary>
Motivation: 针对工业检测中冷启动阶段训练样本稀缺的问题，提出了一种概念引导的适应性特征重建模型。

Method: 提出了ConceptADapt模型，该模型利用有限的正常支持特征预学习一组固定正常概念，并利用它们挖掘与查询特征之间的关系，从而在测试时重新校准其统计数据以提高异常检测。同时，开发了一种动态注意力机制，并结合稀疏自编码器学习鲁棒的正常概念。为了在推理过程中实现快速适应，将LoRA引入到注意力模块中，以引入最小的更新参数。

Result: 在MVTec-AD、VisA和MPDD等三个广泛使用的FS-IAD基准上进行了大量实验，结果表明，该模型在检测和定位任务中均优于现有方法，在各种射击设置下实现了显著的改进。

Conclusion: 提出的ConceptADapt模型在FS-IAD任务中表现出色，为异常检测提供了有效的解决方案。
...
TL;DR: UniVVT是一种统一端到端框架，通过隐式语义指导实现视频虚拟试穿，性能卓越。


<details>
  <summary>Details</summary>
Motivation: 现有的多阶段设计复杂，且允许显式几何先验的错误不可逆地传播到生成的视频中。

Method: 提出了一种统一的端到端框架，将视频虚拟试穿重新定义为语义条件下的视频生成。

Result: UniVVT在多个基准测试中实现了最先进的性能。

Conclusion: UniVVT通过统一的端到端框架实现了视频虚拟试穿，验证了隐式语义指导作为端到端虚拟试穿中简单有效的替代方案。
...
TL;DR: 引入了新的基准测试，发现现有模型在全局空间推理上存在不足。


<details>
  <summary>Details</summary>
Motivation: 现有的基准测试主要关注局部空间感知，忽略了连续、长时视觉流中的全局空间意识。

Method: 引入了全局空间-时间基准（GST-Bench），包含从6,790分钟合成视频中提取的人验证问题，要求模型从未见过的视角进行精确的空间推理，并将自我中心观察映射到全局图像。

Result: 22个最先进的VLMs在GST-Bench上的表现与人类存在显著差距，最强的零样本模型得分仅为42.68，远低于人类的79.08分。

Conclusion: 模型在局部空间理解上表现良好，但仍无法将长期观察整合成全局一致的场景表示。
...
TL;DR: 提出BEACON，一个基于贝叶斯证据获取的WSI推理框架，显著提升了无训练代理框架的性能。


<details>
  <summary>Details</summary>
Motivation: 在计算病理学中，由于竞争性诊断往往表现出相似和重叠的形态模式，因此语义相关并不一定意味着诊断信息丰富。这可能导致基于相关性的检索获取冗余的观察结果，而无法解决诊断不确定性。

Method: 提出BEACON，一个即插即用的代理框架，将WSI推理重新定义为贝叶斯证据获取问题。通过维护关于竞争性诊断假设的概率信念，并通过最大化预期信息增益（EIG）来获取补丁，以减少诊断不确定性。

Result: 在五个WSI-VQA基准上的大量零样本实验表明，BEACON在无训练的代理框架中取得了最强整体性能，同时显著提高了证据获取效率，将贝叶斯证据获取确立为不确定性感知代理WSI推理的原则性范例。

Conclusion: BEACON在无训练的代理框架中实现了显著的性能提升，为不确定性感知的WSI推理提供了一个新的原则性框架。
...
TL;DR: FoRM通过关系流形上的连续流映射，提高了图像修复的蒸馏性能。


<details>
  <summary>Details</summary>
Motivation: 传统知识蒸馏方法在图像修复中忽略了知识传递过程的动态结构。

Method: 提出Flow-Map Distillation on Relation Manifolds (FoRM)，将基于关系知识传递重新定义为关系流形上的连续流映射问题。

Result: 在五个图像修复任务上实现了一致性的提升，包括超分辨率、去雨、去噪、去模糊和低光增强，与最先进的蒸馏基线相比，训练方差降低了约50%，同时实现了更高质量的修复。

Conclusion: FoRM通过引入流映射算子和一致性约束，提高了图像修复的蒸馏性能。
...
TL;DR: HyTBE模型通过扩展关系模式和自适应调整视觉表示，提高了红外小目标检测的跨域泛化能力


<details>
  <summary>Details</summary>
Motivation: 解决红外小目标检测在未见过的红外领域泛化性能下降的问题

Method: 提出HyTBE模型，通过扩展源域关系模式和使用显式关系线索自适应调整视觉表示来提高检测性能

Result: 在NUAA-SIRST、NUDT-SIRST和IRSTD-1K数据集上的leave-one-domain-out实验表明，HyTBE在跨域泛化方面优于竞争基线

Conclusion: HyTBE模型能够有效提高红外小目标检测的跨域泛化性能
...
TL;DR: SR-JEPA通过查询训练好的预测路径来推断3D场景中缺失实体的潜在表示，并展示了上下文相关的实体内容与度量几何的结合。


<details>
  <summary>Details</summary>
Motivation: 许多masked JEPAs主要通过它们产生的编码器进行评估。作者提出要研究训练好的预测路径本身在原始3D场景中整个实体缺失时能推断出什么。

Method: 引入SR-JEPA，这是一种针对场景级点云的点原生JEPA，其原始冻结的预测路径可以在提供的位置进行查询。训练使用自包含的3D EMA目标：没有重建、语义标签、语言或提升的2D特征。

Result: 在5,953个ARKitScenes对象上，推断的潜在表示达到43.13%的语义-身份宏准确率。在8,570个Sr3D支持对上，完整的潜在表示达到41.15 AP。

Conclusion: 这些结果表明，模型完成上下文相关的实体内容，下游计算将其与度量几何结合。
...
TL;DR: Vorch-Director通过残差校正策略和噪声级别感知，提高了音频-视频生成的稳定性和保真度。


<details>
  <summary>Details</summary>
Motivation: 为了解决在音频-视频生成中由于累积误差导致的身份漂移、过度平滑和音频-视频不同步问题。

Method: 提出了一种名为Vorch-Director的噪声级别感知残差校正策略，该策略将每个残差与其起源的噪声级别相关联，并在训练期间注入来自匹配噪声级别的残差。

Result: Vorch-Director在ST-Bench上进行了评估，并引入了一个新的长时程音频-视频基准，具有质量漂移和长距离一致性的指标。实验表明，与强基线相比，Vorch-Director在稳定性和音频-视频保真度方面有所改进。

Conclusion: Vorch-Director通过噪声级别感知的残差校正策略，实现了更真实的自回归历史，同时保持了高效的教师强制训练，在长时程音频-视频生成方面取得了显著的成果。
...
TL;DR: EviSelect: A fine-grained dynamic visual selection framework for improved long-form video understanding.
...
Method: Proposes a fine-grained dynamic visual selection framework named EviSelect, grounded in the target MLLM internal attention evidence. Efficiently probes visual evidence via sparse prefilling as a structured prior to guide distribution-aware dynamic sampling. Designs a lightweight selector that locates query-relevant timestamps and adjusts local sampling rate and spatial resolution. Formulates the selector as a stochastic policy and optimizes it via GRPO under a joint accuracy--efficiency reward.
...
Conclusion: EviSelect is a fine-grained dynamic visual selection framework that improves long-form video understanding by efficiently selecting query-relevant frames and adjusting sampling rates and spatial resolutions.
...
TL;DR: VSMP-IMU通过结构化语义运动程序生成可控的合成IMU数据，显著提升了可穿戴活动识别的性能。


<details>
  <summary>Details</summary>
Motivation: Wearable human activity recognition (HAR) is often limited by the scarcity of labeled sensor data, especially in low-resource, class-imbalanced, and subject-generalization settings.

Method: VSMP-IMU, a video-grounded framework for controllable synthetic IMU generation based on a structured Semantic Motion Program (SMP), which separates activity-defining semantics from label-preserving variation.

Result: VSMP-IMU achieves an average Macro-F1 of 78.33%, improving over real-only training by 9.77% and over the strongest prior synthetic baseline by 4.04%. In low-resource settings with reduced training data-samples, it improves over real-only training by 18.54% and over the strongest prior synthetic baselines by more than 6% on average. Under long-tail evaluation in imbalanced datasets, it improves tail-class Macro-F1 by 19.86% over Real-only training and by 4.76% over SOTA.

Conclusion: Structured video-grounded semantics provide a practical foundation for controllable, wearable-relevant synthetic sensor data generation.
...
TL;DR: KVAE tokenizer在音频、图像和视频生成中优于开源tokenizer，具有广泛的应用前景


<details>
  <summary>Details</summary>
Motivation: LDM在信号压缩和生成过程中的重要性

Method: 设计并实现了一系列KVAE tokenizer，包括KVAE-Audio、KVAE-3D和KVAE-2D

Result: 在重建和生成结果上优于开源tokenizer，如VAEs、HunyuanVideo、FLUX、MovieGen等

Conclusion: KVAE tokenizer在音频、图像和视频生成中表现出色，具有广泛的应用前景
...
TL;DR: Vorch-Omni是一个统一的音频-视频生成框架，支持多种任务，提高了音频-视频生成的效率。


<details>
  <summary>Details</summary>
Motivation: 现有的生成视频模型方法通常依赖于碎片化的特定任务模型，导致在处理不同任务时存在干扰。

Method: 提出Vorch-Omni，一个基于任意条件到任意输出的统一多任务框架，能够灵活处理视频和音频信号。使用条件掩码和任务标识符区分目标、源内容和参考，同时使用位置类型分离时间上下文和独立条件。采用视觉-语言模型和视频VAE来捕捉语义和结构信息。
...
TL;DR: ODin通过将配准建模为3D扩散过程，实现了更精确、更快速的人体配准。


<details>
  <summary>Details</summary>
Motivation: 由于获取噪声、遮挡和未知软组织动力学，3D人体配准具有固有的歧义性，传统回归方法往往无法准确表示几何形状。

Method: ODin将配准建模为3D扩散过程，通过全局、局部和位置条件引导每个点到其正确位置，从而生成与目标几何形状对齐的点云，同时保留模板语义。

Result: 实验表明，这种生成式方法不仅优于基于回归的基线，而且达到了新的水平，超越了高度工程化的方法，同时将注册时间减少了三分之二。

Conclusion: ODin通过建模配准的不确定性，实现了更精确的人体配准，提高了效率。
...
TL;DR: STAIL是一种有效的增量学习方法，可显著提高医学图像分析的性能。


<details>
  <summary>Details</summary>
Motivation: 医学图像分析中的深度学习模型在动态环境中持续适应新的临床任务时，会遭受严重的灾难性遗忘。

Method: 提出了一种名为语义文本锚定增量学习（STAIL）的新框架，引入了非对称语义巩固缓冲区（SCB）和语义锚定机制（LSAM）。

Result: 在三个异构医学数据集上进行了广泛的实验，证明了STAIL作为高度有效的模块，显著提高了各种现有基线的性能。

Conclusion: STAIL在持续性能和减少遗忘方面表现出色，是一种有效的增量学习方法。
...
TL;DR: EG-FM方法通过移动端点显式建模生成轨迹，提高了生成模型在像素空间中的性能。


<details>
  <summary>Details</summary>
Motivation: 为了提高生成模型在像素空间中的生成能力，并减少高维空间中全局结构和细粒度细节的学习难度。

Method: 提出了能量引导的流匹配（EG-FM）方法，通过移动端点来显式建模从粗糙到细粒度的生成轨迹。

Result: 在ImageNet类条件图像生成任务上，EG-FM在$256 	imes 256$分辨率下，仅需更少的训练周期，即可实现更低的FID值，并在$512 	imes 512$分辨率下实现了1.58的FID值。

Conclusion: EG-FM方法能够有效地提高生成模型的性能，且对训练数据和骨干网络无需调整。
...
TL;DR: Proposed a Bayesian adaptively-weighted ensemble framework for few-shot learning in anatomical segmentation, achieving significant improvements in performance.
...
Method: Propose a Bayesian adaptively-weighted ensemble framework for segmentation under label scarcity and domain shift. Adapt multiple few-shot segmentation algorithms using a small labelled support set. Use Bayesian optimisation to identify ensemble weights. Apply learned weights to combine predictions on query images.

Result: Statistically significant improvements over individual few-shot learners, fixed-weight ensembles, training-from-scratch baselines and recent state-of-the-art ensembling approaches. Practical mechanism for deploying segmentation systems to new clinical sites under severe annotation constraints.
...
TL;DR: 提出了一种新的VITON图像生成引导方法，提高了生成图像的真实性和多样性。
...
Method: 利用开源AI模型，通过标签（如长度和风格）增强图像数据。通过使用标签和服装图像训练适配器，模型可以生成更多样化的图像，用户可以控制。

Result: 为最终用户（如零售商）提供更真实的试穿效果，确保图像与实际尺寸尽可能匹配，避免误导消费者。

Conclusion: 该方法在VITON场景下有效提高了图像生成的真实性和多样性。
...
TL;DR: 提出了一种基于几何感知的车辆定位方法，显著提高了定位精度。


<details>
  <summary>Details</summary>
Motivation: 准确的车辆定位对于智能交通系统、交通监控和交通冲突分析非常重要。

Method: 提出了一种两阶段的几何感知定位流程，首先使用YOLO26检测器检测车辆，然后使用专门的ResNet34回归网络预测对应于投影车辆基的四角点。

Result: 在合成和真实数据上的实验表明，该方法在平均图像空间定位误差和地面平面误差方面均有明显改进。

Conclusion: 该方法在车辆定位方面表现出色，尤其是在处理几何挑战和强透视畸变和视差的情况下。
...
TL;DR: HALO通过双优先级驱动的框架，显著提高了遥感影像的增强效果。


<details>
  <summary>Details</summary>
Motivation: 为了从极端低光照下恢复高保真遥感影像，对于可靠的地球观测和下游机器视觉至关重要。

Method: 提出HALO，一个由双优先级驱动的增强框架，将增强视为由基础模型先验驱动的引导特征聚合问题。

Result: HALO在8个具有挑战性的合成和真实世界遥感基准测试中实现了最先进的性能，显著提高了物理边界的清晰度和颜色保真度，同时最大限度地保留了用于下游地球观测任务的判别特征。

Conclusion: HALO有效地解决了现有方法在极端低光照条件下的遥感影像增强问题，为地球观测和机器视觉提供了可靠的解决方案。
...
TL;DR: Curia-MAE模型通过改进预训练方法，在多个分割任务中表现出色，尤其是在病变数据稀缺的情况下。
...
Method: 通过扩展卷积MAE预训练，添加鲁棒的重建目标、特征正则化和局部-全局相似性目标，并使用Curia-MAE模型进行多模态、多解剖结构的MAE模型预训练。

Result: Curia-MAE在八个解剖和病变分割基准上提高了冻结编码器的性能，并在病变任务上表现优异，特别是在标记数据稀缺的情况下。

Conclusion: 单一代码器可以跨多种分割任务重复使用，降低临床工作流程中适应和部署此类模型的成本。
...
TL;DR: DTRNet：一种结合文本识别和部首解码的伪造字符检测框架。


<details>
  <summary>Details</summary>
Motivation: 在K-12教育场景中，手写中文文本识别不仅需要转录学生的写作，还需要检测伪造的字符。然而，现有的识别模型通常局限于预定义的正常字符集，因此无法明确识别伪造字符。现有的检测方法存在互补的局限性：字符级方法提供可解释的结构证据，但效率低下；而行级方法效率高，但严重依赖置信度分数，容易发生漏检，缺乏明确的结构证据。因此，关键挑战是在保持行级效率的同时，保留与上下文推断无关的字符结构证据。

Method: 我们提出了DTRNet，一个用于行级伪造字符检测的双文本-部首解码框架。DTRNet将上下文感知文本识别与字符级结构验证解耦，其中文本分支执行行级转录，部首分支预测基于词典的伪造字符判断的合法表意描述序列（IDS）。我们进一步引入了IDS引导的置信度调整（IGCA）来在推理过程中使用结构证据细化文本预测。

Result: 实验结果表明，DTRNet有效地检测了伪造字符，同时保持了强大的识别性能，并提供了可解释的部首级证据。代码、检查点和处理后的数据集在https://github.com/BNU-ERC-ITEA/DTRNet上公开可用。

Conclusion: DTRNet是一种有效的行级伪造字符检测方法，在保持识别性能的同时，提供了可解释的结构证据。
...
TL;DR: 提出了一种新的形状感知OBB到HBB转换方法，提高了遥感图像中船只检测的准确性
...
Method: 提出了一种形状感知的OBB到HBB转换方法，利用船体形状、船体饱满度和边界框方向生成更紧凑的轴对齐HBB表示

Result: 与三种基线方法进行了比较，包括使用最小和最大值的Outer HBB、面积等效HBB和GBB边际化HBB

Conclusion: 该方法在船只检测方面提高了准确性
...
TL;DR: 提出改进方法解决表示-原型不一致问题，提高动作分割准确性。


<details>
  <summary>Details</summary>
Motivation: 针对现有基于最优传输（OT）的无监督动作分割方法中存在的表示-原型不一致问题，尤其是对于模糊过渡和短频动作，提出改进方案。

Method: 基于CLOT，引入图约束模块和动作嵌入细化步骤，定期重新锚定原型到稳定的表示几何。两种变体：D-CLOT使用k-means更新原型，D-CLOT_B使用OT质心加权更新原型。

Result: 在五个基准数据集上，两种变体均优于CLOT，视频级F1和mIoU提升，动作级F1提升。在Assembly101数据集上建立首个无监督动作分割基线。

Conclusion: 提出的改进方法有效解决了表示-原型不一致问题，提高了动作分割的准确性。
...
TL;DR: HoloWorld：统一室内外生成的框架，提高城市外观生成性能。


<details>
  <summary>Details</summary>
Motivation: 为了创建一个连贯的城市世界，需要将室内和室外环境统一生成，以实现所需的对应关系。

Method: HoloWorld框架基于不断更新的跨尺度世界上下文，从用户描述开始，逐步表示和更新多样化的世界信息，包括城市规模规划和单个建筑。它利用演变的上下文和先前生成的相邻区块，自回归地生成具有一致空间组织和视觉身份的城市外观。生成的外观表示进一步基于3D建筑实例和足迹，实现具有几何约束布局和继承外观特征的特定建筑室内生成。

Result: HoloWorld是第一个在连贯的3D城市世界中统一室内和室外生成的框架。实验表明，HoloWorld在城市外观生成方面表现出色，平均AQS分数比SOTA提高了7.68%，同时保持了强大的建筑级室内外对应性和跨区块连续性。

Conclusion: HoloWorld框架能够有效地统一室内和室外生成，为创建连贯的城市世界提供了新的解决方案。
...
TL;DR: 提出Robust-WAM，通过在VAE生成路径上添加语义预见对齐目标，提高了基于视频生成的WAM的性能和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 为了克服VAE潜在空间在视觉变化下对动作预测的脆弱性，以及现有语义潜在空间模型无法利用大规模VGM预训练的问题。

Method: 提出Robust-WAM，一种基于视频生成WAM的后训练方法，在VAE生成路径的基础上，在动作流上添加轻量级的语义预见对齐目标。使用可学习的查询令牌将未来场景语义引入动作流，并通过与未来真实帧的语义预见对齐其输出隐藏状态。为建立每个查询与其描述的未来步骤之间的时序对应关系，赋予其匹配动作令牌的位置编码。

Result: 在分布外泛化模拟基准和真实机器人设置上的实验表明，Robust-WAM在保持分布内性能的同时，一致地提高了多个WAM基线成功率。

Conclusion: Robust-WAM是一种有效的后训练方法，可以显著提高基于视频生成的WAM的性能，同时保持其在分布内外的泛化能力。
...
TL;DR: 该研究利用VLMs创建了一个互动地图，展示了20世纪巴黎亚美尼亚商业社区，并证明了对资源匮乏语言的有效性。


<details>
  <summary>Details</summary>
Motivation: 将法国数字化亚美尼亚报纸转换为20世纪巴黎亚美尼亚商业社区互动地图的需求

Method: 基于IIIF的端到端管道，结合视觉语言模型(VLMs)进行数据初始化

Result: 创建了一个包含500页西里尔亚美尼亚语报纸和3,270个广告级别注释的语料库，并开发了一个可复制的流程

Conclusion: VLM驱动的数据初始化是有效解决资源匮乏的历史语言（如西里尔亚美尼亚语）的方法。
...
TL;DR: FlaRe：一种结合光线追踪和神经辐射的灵活场景表示方法。


<details>
  <summary>Details</summary>
Motivation: 为了提高神经场景表示的灵活性和与常规图形工作流程的集成，研究人员提出了Floating Radiance Networks (FlaRe)。

Method: Floating Radiance Networks (FlaRe)通过结合显式光线追踪几何和连续神经辐射函数来表示场景。

Result: FlaRe在标准重建基准测试中表现出色，同时使用紧凑的原始数据集。

Conclusion: FlaRe是一种多功能的表示方法，将高保真神经网络渲染、光线追踪、几何操作和外观编辑统一到一个场景模型中。
...
TL;DR: 提出了一种新的无标签校准方法，提高了视觉-语言模型的预测置信度。


<details>
  <summary>Details</summary>
Motivation: 为了提高视觉-语言模型在分布偏移下的识别精度，但通常会导致校准退化，使得预测置信度不可靠。

Method: 提出了一种名为Zero-Shot-Anchored Entropy Calibration (ZAEC)的无标签后处理方法，该方法使用零样本熵作为样本特定的不确定性参考。ZAEC通过最小温度缩放选择性地恢复锐化预测的零样本熵，同时保持所有其他预测不变。

Result: 在五个TTA方法和15个数据集上，ZAEC在ViT-B/16上实现了最低的后处理宏平均ECE，在RN50上保持了一致的收益。

Conclusion: ZAEC能够有效地提高预测置信度并降低熵，同时保持分类精度和校准性能。
...
TL;DR: 3D CT 基础模型在诊断方面的表现取决于物理瓶颈，而非模型架构。


<details>
  <summary>Details</summary>
Motivation: 评估 3D CT 基础模型在诊断方面的广泛性。

Method: 使用 $k$-nearest neighbors, zero-shot prompting 和线性探测在三个胸部的 CT 扫描队列中对十个冻结的 CT 编码器进行基准测试。

Result: 没有发现普遍的最先进水平，排名取决于评估环境。

Conclusion: 主要性能决定因素是物理瓶颈，而非模型架构。广泛的或高对比度异常可以可靠地恢复，而小的、低对比度的焦点病变仍然是挑战。
...
TL;DR: Diff-VF：一种高效的长视频生成框架，解决现有模型在处理长视频时的不足。


<details>
  <summary>Details</summary>
Motivation: 针对现有视频扩散模型在处理长视频时的不足，例如难以保持长期时间一致性和多样性运动，提出了一种新的长视频生成框架。

Method: Diff-VF框架，结合了混合噪声初始化、加权窗口采样和时序扩展采样三种策略，并通过跳过残差引导实现长视频增强。

Result: Diff-VF在VBench-Long评估中表现出色，在时间一致性和运动多样性之间取得了更好的平衡，同时保持了帧级的质量。

Conclusion: Diff-VF是一种高效的长视频生成框架，适用于不同时空建模策略的视频扩散模型。
...
TL;DR: 将历史预测和辅助层纳入模型，提高地球观测产品准确性
...
Method: 引入Crop Type (CTY)嵌入编码器，将历史预测作为时间有序的类别标记，并关注年份轴；研究如何表示外部提供的Base Vegetation Layer (BVL)掩码；在仅评估作物类别的18个类别上评估模型；在约540万个标记像素的泛欧洲数据集上测试模型

Result: 将预测历史添加到模型中，将作物仅F1分数提高1.6个百分点，并纠正了召回偏差的错误配置文件；在历史和目标年份中一致地表示BVL掩码，在作物类别上增加了约2.5个百分点；该方法是一种低成本的方法，适用于任何定期发出类别图的地理空间或基础模型

Conclusion: 通过将历史预测和辅助层纳入模型，可以提高地球观测产品的准确性，并纠正错误配置文件，从而提高模型的性能
...
TL;DR: UCD是一种针对SAM3图像分割的有效对抗攻击，显著降低了其性能。


<details>
  <summary>Details</summary>
Motivation: 扩展promptable segmentation从几何驱动掩码预测到开放词汇概念分割，并提高概念级预测的鲁棒性。

Method: 引入Universal Concept Disruption (UCD)，通过学习单个有界图像扰动来攻击SAM3图像分割。

Result: 在多个数据集上，UCD在匹配评估协议下，平均掩码AP从59.43降低到18.73，平均cgF1从50.32降低到20.49。

Conclusion: UCD是第一个针对SAM3图像分割的通用跨概念对抗攻击，有效降低了其鲁棒性。
...
TL;DR: 通过实验比较，发现迭代系统在LiDAR语义场景补全中优于宽控制，但存在成本和性能之间的权衡。


<details>
  <summary>Details</summary>
Motivation: 比较单次预测器、参数匹配的更宽预测器和从相同的冻结预测器初始化的权重捆绑的多网格细化器，以研究在LiDAR语义场景补全中，是否应该通过迭代来花费额外的测试时间计算，或者花费相似的参数预算来扩展一次预测器。

Method: 在五个训练种子和815个SemanticKITTI序列-08帧上，比较了单次预测器、参数匹配的更宽预测器和从相同的冻结预测器初始化的权重捆绑的多网格细化器。

Result: 迭代系统在连续角度移除的情况下，mIoU比宽控制提高了0.911个点，95%的移动块自举区间为[0.804, 1.040]，超过了预先声明的0.5个点的实际差距。在独立75%细化下，迭代仅增加了0.300个点[0.166, 0.436]，而观察家族增强增加了5.975个点[5.662, 6.140]。迭代系统每帧的成本为10.74毫秒和0.75 GiB，而宽控制为6.25毫秒和0.23 GiB。

Conclusion: 这些结果表明，基于几何条件的经验边界而不是普遍优势：连贯间隙可以证明固定深度细化是合理的，广泛细化的证据可以通过训练覆盖范围更有效地解决，而虚假证据需要不同的鲁棒性机制。
...
TL;DR: PaCoNet是一种新的并行坐标数据提取方法，利用深度学习技术，能够自动分析和重设计并行坐标图。


<details>
  <summary>Details</summary>
Motivation: 当前研究主要关注低维可视化，如条形图、折线图和饼图，但对于高维数据可视化方法并行坐标图的研究还很少。

Method: 提出PaCoNet，一种并行坐标数据提取的新方法，利用深度学习技术进行数据提取和分析。

Result: PaCoNet可以提取线坐标和单个数据样本，实现并行坐标图的自动分析和重设计。

Conclusion: PaCoNet为复杂可视化分析奠定了基础，推动了计算机视觉与数据可视化领域的交叉发展。
...
TL;DR: 提出DARAD框架，有效解决遥感影像文本检索中的持续检索问题，提高检索性能。


<details>
  <summary>Details</summary>
Motivation: 随着地球观测技术的快速发展，遥感影像文本检索（RS-ITR）变得越来越重要。然而，由于遥感影像的尺度变化和分布偏移加剧了跨模态对齐空间扭曲，使得现有的持续学习方法难以支持可靠的持续检索。

Method: 提出了一种名为DARAD的框架，该框架包含双重适配器和排名感知蒸馏，以保留历史跨模态排名结构，同时从不断发展的档案中学习新的视觉和文本概念。视觉分支引入了空间融合适配器，整合了粗略的区域线索和细粒度的块线索，以适应遥感影像的尺度变化，同时将视觉更新锚定到预训练的对齐空间。文本分支采用多专家语义路由，将共享的文本语义与语义特定的残差分离，以吸收新出现的描述，同时限制全局文本嵌入漂移。此外，双向排名蒸馏使用冻结的教师模型和历史锚点来保留历史跨模态排名结构，从而减轻持续阶段之间的对齐空间扭曲。

Result: 在多阶段持续检索协议下的实验表明，DARAD在性能上优于现有的持续学习方法，提高了对新到达数据的适应性，同时保持了在历史数据上的有效性。

Conclusion: DARAD框架能够有效解决遥感影像文本检索中的持续检索问题，提高了检索的准确性和适应性。
...
TL;DR: 提出了一种基于JEPA的自监督条形图数值恢复管道，有效提高了数据提取效率。


<details>
  <summary>Details</summary>
Motivation: 为了解决从条形图中提取数据的问题，以及标注数据的稀缺性，提出了一种基于联合嵌入预测架构（JEPA）的自监督方法。

Method: 使用JEPA编码器生成语义丰富的潜在特征，然后使用解码器模型消耗这些特征来输出刻度和条形的坐标，从而恢复条形图的数值。

Result: 该方法在自我监督微调和提取特征的质量方面表现出色，与端到端监督基线模型相比具有优势。

Conclusion: 提出的条形图数值恢复管道有效地从条形图中提取数据，为数据可视化领域提供了新的解决方案。
...
TL;DR: 商业视觉语言模型在阴影去除中有效，但需结合经典低级视觉先验


<details>
  <summary>Details</summary>
Motivation: 研究商业视觉语言模型对经典低级视觉的影响

Method: 通过阴影去除问题进行实验，使用商业生成编辑器进行阴影去除，并设计了一种代理候选选择流程来提高质量

Result: 商业视觉语言模型不替代经典低级视觉先验，但可用于约束和引导物理上受限的生成

Conclusion: 商业视觉语言模型在阴影去除任务中有效，但需要与经典低级视觉先验结合使用
...
TL;DR: 本文提出了一种用于印度东北部半小时降水实时预测的轻量级深度学习模型，取得了优异的性能。


<details>
  <summary>Details</summary>
Motivation: 在灾害管理和准备中，准确的短期降水预测至关重要。然而，降水的可变性和非线性特性使得气象学家对短期预测具有挑战性。此外，捕捉时空数据中的时间依赖性也是降水实时预测的挑战。

Method: 本文提出了一种轻量级深度学习模型，用于半小时降水实时预测。该模型结合了DenseNet架构、残差连接和transformer编码器，以减少模型参数并有效地进行降水实时预测。选择印度的东北部地区作为研究区域，该地区在6月至9月期间因季风季节而降水量最高。模型以过去五次半小时降水作为输入，预测下一个两小时的降水。本研究使用GPM IMERG降水数据集进行模型训练和测试。

Result: 该模型在30分钟的时间间隔内实现了最佳的MAE为0.235毫米，RMSE为0.735毫米，KGE分数为0.816。

Conclusion: 提出的模型在印度东北部的半小时降水实时预测中表现良好，为灾害管理和准备提供了有价值的工具。
...
TL;DR: 通过集成几何先验和多视图预测，3DGS 的重建质量得到显著提升。


<details>
  <summary>Details</summary>
Motivation: 3D Gaussian splatting (3DGS) 在稀疏表示中提供实时渲染，但依赖于结构从运动初始化和光度优化可能导致几何重建不理想。

Method: 将预测的法线和深度图作为几何先验集成到 3DGS 框架中，并分析多视图预测的效果。

Result: 多视图预测（如 VGGT 所做）优于单视图替代方案，置信图可显著提高先验的有效性。实验表明，重建质量得到一致提升，复杂场景（包括反光物体）中的显著增益。

Conclusion: 集成几何先验和多视图预测可显著提高 3DGS 的重建质量，特别是在复杂场景中。
...
TL;DR: SURE框架通过引入不确定性估计和反馈学习，提高了视觉扩散模型的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的潜在奖励模型只输出标量分数，无法估计每个预测的不确定性，导致优化方向错误和奖励黑客攻击。

Method: 提出了一种名为SURE的统一潜在空间框架，包括样本自适应潜在奖励模型(SURE-LRM)和不确定性引导奖励反馈学习(SURE-REFL)。SURE-LRM预测每个噪声潜变量的高斯效用，其均值预测奖励分数，其方差反映预测的不确定性。SURE-REFL通过不确定性引导奖励反馈学习，提供不确定性引导的密集反馈。

Result: 实验表明，SURE-LRM在偏好预测方面优于强基线，SURE-REFL在各种指标上取得了最先进的性能，并进一步提高了优化稳定性。

Conclusion: SURE框架提高了图像和视频扩散模型的偏好预测性能和优化稳定性。
...
TL;DR: 本研究提出了一种基于合成数据的AI方法，显著提高了上踝关节姿势评估的准确性。


<details>
  <summary>Details</summary>
Motivation: 为了提高放射影像的诊断质量，患者的姿势是关键因素。由于患者定位困难和标准化不足，需要一种自动化的AI方法来评估患者姿势。然而，由于监管障碍，获取所需的深度图像和相应的放射影像很困难。

Method: 提出了一种框架，可以从计算机断层扫描中合成地生成所需的训练数据。通过在合成数据集上进行预训练，提高了真实上踝关节的姿势评估。

Result: 使用合成数据集进行预训练后，真实上踝关节的姿势评估提高了11个百分点。

Conclusion: 该研究提出了一种基于合成数据的AI方法，可以有效地提高上踝关节姿势评估的准确性。
...
TL;DR: 比较了人类启发方法和大型自监督模型在构图分析中的性能，发现人类启发方法在保持可解释性的同时提供有竞争力的性能。


<details>
  <summary>Details</summary>
Motivation: 为了解决学习有意义的构图表示的持续差距，并指出语义偏差，提出人类启发的方法可能是关键。

Method: 比较了两种平行范式：基于感知组块的启发式方法和由最近的大规模构图数据集启用的高级模型。人类启发的方法使用以对象为中心的模型进行区域分解，并使用图注意力网络来捕获元素之间的空间关系。

Result: 使用冻结编码器时，人类启发的方法在保持可解释性的同时实现了有竞争力的性能。当有足够的数据进行微调时，大型自监督模型在性能上显著优于人类启发的方法，但以可解释性和跨领域泛化能力为代价。

Conclusion: 人类启发的方法在保持可解释性的同时提供了有竞争力的性能，而大型自监督模型在性能上占优，但牺牲了可解释性和跨领域泛化能力。
...
TL;DR: BendTwin是一种新的视频重建方法，通过引入弯曲约束，提高了机械稳定性，在性能上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 从视频观察中重建具有机械特性的物体，有助于机器人规划和交互，从而实现物理一致的动态预测。

Method: 提出了一种名为BendTwin的弯曲感知微分弹簧-质量框架，用于基于视频的重建和可变形物体的未来预测。该框架引入了局部表面三点的弯曲刚度和阻尼，惩罚偏离静止角度和正则化高阶变形。

Result: 实验表明，BendTwin在性能上优于仅轴向的PhysTwin基线。消融研究进一步表明，弯曲约束在不同下采样比下保持系统稳定性，并始终优于原始的PhysTwin公式。

Conclusion: BendTwin为从稀疏视图RGB-D视频中构建机械忠实度数字孪生提供了一种有效的方法。
...
TL;DR: 提出了一种基于事件相机的事件驱动反射消除网络EvReflection，在合成和真实世界基准测试中均取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 尽管在反射消除方面取得了显著进展，但当前方法主要利用单帧的静态图像先验，并且由于反射层和传输层之间固有的歧义，仍然存在严重的残留伪影。

Method: 提出利用事件信号来打破这种歧义。通过使用事件相机捕获微动态，揭示这两个层之间的运动差异。因此，提出了一种新颖的事件驱动反射消除网络EvReflection，该网络利用这些动态线索进行层分离。具体来说，设计了一个微动态解耦器，从事件流中分离出特定于层的运动作为先验，然后引导一个视差注意力校正器从RGB图像中干净地去除伪影。此外，为了解决数据稀缺问题，开发了一个视差感知的模拟管道，并构建了EVR$^2$基准数据集，这是该任务的第一个真实世界数据集。

Result: 在合成和真实世界基准测试中，EvReflection实现了最先进的性能，分别在PSNR中超过了最佳竞争方法1.6 dB和1.2 dB。

Conclusion: EvReflection是一种有效的事件驱动反射消除网络，在合成和真实世界基准测试中均取得了最先进的性能。
...
TL;DR: HOPE is a new framework for pressure estimation in hand-object interaction, achieving good performance in various scenarios.


<details>
  <summary>Details</summary>
Motivation: Estimating physical pressure from vision is essential for understanding contact-rich hand-object interaction.

Method: Formulating pressure estimation as a hand-centric video prediction problem with monocular video as input, and proposing HOPE framework with two key components.

Result: HOPE generalizes to bare-hand egocentric and in-the-wild videos, producing joint contact and pressure predictions beyond the scope of contact-only or planar-pressure baselines.

Conclusion: HOPE is an effective framework for pressure estimation in hand-object interaction.
...
TL;DR: CFGPNet是一种基于交叉注意力和融合梯度编程网络的多光谱目标检测框架，在多个基准数据集上表现出色。


<details>
  <summary>Details</summary>
Motivation: RGB-T目标检测在低光、恶劣天气和复杂多尺度环境中表现出色，但现有方法存在跨模态交互不足、模态分布差距不稳定融合和高计算成本等问题。

Method: 提出了一种基于交叉注意力和融合梯度编程网络的CFGPNet框架，使用改进的GELAN骨干网络和ReViT风格的重新参数化块来增强特征表示，同时保持计算效率。引入了跨计算高效注意力模块来增强跨模态特征交互，并结合密集特征聚合和基于选择的注意力强调的ASAF网络生成紧凑且具有判别性的融合表示。此外，还集成了一种可编程梯度辅助分支来提高梯度传递和优化质量。

Result: 在五个公开的多光谱基准数据集FLIR、M3FD、LLVIP、VEDAI和MFAD上，CFGPNet在不同场景、对象尺度和模态平衡下均表现出强大的性能。特别是在FLIR上达到80.7% / 45.0%的mAP50 / mAP50:95，M3FD上达到89.9% / 63.4%，LLVIP上达到97.8% / 68.9%，VEDAI上达到83.3% / 56.9%，MFAD上达到83.4% / 61.8%。

Conclusion: CFGPNet是一种有效的、实用的解决方案，在三个模型规模上提供了有用的精度-效率权衡。
...
TL;DR: EmoWorld：分离视频生成中的情感和语义信息，实现更自然的视频表达。


<details>
  <summary>Details</summary>
Motivation: 现有的视频生成器无法有效分离全局氛围、情感语义和时间信息，导致生成效果不理想。

Method: 使用 Video DiT 框架，结合不同情感导向和提示库，实现了对全局氛围、语义和时间的分离控制。

Result: 在多个数据集和指标上，EmoWorld 显著提升了目标情感对齐、情感引导和过渡单调性等指标。

Conclusion: EmoWorld 在多个情感类别和视频生成场景中取得了显著的效果，并提高了视频生成的情感表达和视觉效果。
...
TL;DR: 提出DG-Det方法，显著提升拥挤场景视频目标计数精度。


<details>
  <summary>Details</summary>
Motivation: 提升拥挤场景中的视频目标计数能力，并基于文本或视觉提示稳健地计数目标类别实例。

Method: 提出了一种深度引导检测器（DG-Det）和通用后处理流程，结合深度线索与多尺度RGB-D交叉注意力和显式遮挡预测，并引入了统一的去重框架。

Result: 实验表明，该方法相比现有基线在MAE上降低了62.01%，RMSE也有明显改善。

Conclusion: 本文提出的方法在拥挤和遮挡场景中实现了稳健的视频目标计数，为未来研究提供了新的数据集和代码。
...
TL;DR: PRISM是一种新的无GAN流匹配框架，用于无配对监督下的图像到图像翻译，在多个基准测试中表现出色。


<details>
  <summary>Details</summary>
Motivation: 解决无配对监督下的图像到图像翻译问题，特别是如何控制保留和改变内容的问题。

Method: 提出PRISM，一个无GAN的流匹配框架，使用学习到的每个特征的门控机制来控制保留和改变。

Result: 在五个自然和生物医学基准测试中，PRISM在四个基准测试中取得了最佳的性能，在第五个基准测试中取得了具有竞争力的结果。

Conclusion: PRISM在保持目标真实性和结构保留之间取得了良好的平衡。
...
TL;DR: 提出了一种轻量级模型RSBdSL38，用于孟加拉国手语识别，显著提高了移动设备的易用性


<details>
  <summary>Details</summary>
Motivation: 扩大对孟加拉国聋人和听力受损者教育和服务的机会

Method: 提出了一种基于轻量级注意力机制的卷积神经网络，并使用专家验证的数据集进行训练

Result: 在BdSL手语识别任务上达到了96.37%的准确率，且在移动设备上运行流畅

Conclusion: RSBdSL38模型和从头开始训练的模型将基准准确率转化为可部署的易用性，同时降低了预训练骨干的成本
...
TL;DR: HoloCount是一个全面的基准测试，揭示了MLLMs在定量场景理解中的局限性，并为开发更可靠的多模态系统提供了路线图。


<details>
  <summary>Details</summary>
Motivation: 为了解决现有多模态大型语言模型（MLLMs）在定量场景理解中的精度瓶颈，以及现有计数基准测试的局限性。

Method: 提出了一种名为HoloCount的全面且诊断性丰富的基准测试，该测试围绕一个三级层次分类结构进行。HoloCount评估了MLLMs在以下方面的能力：（1）语义计数，关注原子和基于属性的计算；（2）分析计数，通过空间和集合推理评估逻辑组成；（3）鲁棒性测试，测试模型在不利场景和基于现实世界的先验知识下的完整性。通过评估超过20个最先进的MLLMs，揭示了关键的性能差距。

Result: 发现即使顶级模型在从感知到复杂分析推理和不利场景的任务转换过程中也会显著退化。研究提供了当前MLLMs计数能力的系统景观，并为开发更基于现实世界和可靠的的多模态系统提供了路线图。

Conclusion: HoloCount是一个全面的基准测试，有助于评估MLLMs在定量场景理解中的能力，并揭示了当前MLLMs在复杂推理和不利场景下的局限性。
...
TL;DR: MAS 提高多玩家世界模型的性能和可扩展性。


<details>
  <summary>Details</summary>
Motivation: 现有的多玩家世界模型在多玩家环境中存在冗余计算、视图不一致和可扩展性差等问题。

Method: MAS 通过分解世界动态和视图渲染，使用逻辑引擎推进全局权威类型状态，并从共享状态生成视图。

Result: MAS 实现了更高的状态准确性和更低的跨视图不一致性，并支持 1,024 个并发玩家进行 10,000 次迭代。

Conclusion: MAS (Multiplayer world models with Authoritative Shared State) 提出了解决多玩家环境中世界状态与视依赖视觉潜在状态纠缠的问题，实现了比现有多视图基线更高的状态准确性和更低的跨视图不一致性，为可扩展和一致的多智能体世界模拟提供了实际基础。
...
TL;DR: OTLesMix：一种用于医学影像分割的新图像合成方法


<details>
  <summary>Details</summary>
Motivation: 医学影像分割的深度学习发展及其在病理特征提取方面的应用

Method: 提出OTLesMix图像合成方法，利用Wasserstein重心与最优传输计划生成真实且多样化的样本

Result: 在三个脑部病变分割任务上，该方法相较于无合成数据的模型提高了Dice分数2.9至6.6个百分点，并优于基于混合的最新方法

Conclusion: OTLesMix方法在医学影像分割中表现出色，有效提升了分割准确性
...
TL;DR: 本研究提出了一种基于智能手机照片的牙齿定位和编号模型，以解决口腔健康问题。


<details>
  <summary>Details</summary>
Motivation: 口腔健康问题影响数十亿人，但专业牙科护理的成本和有限的可及性阻碍了预防性口腔保健。

Method: 开发了一个基于Mask R-CNN的管道，用于在智能手机图像上定位和编号牙齿。

Result: 在内部测试集上，模型达到了0.818的实例掩码AP@50、0.780的类别感知PQ和0.884的操作F1。在公共数据集上，模型达到了0.901的实例掩码AP@50、0.832的类别感知PQ和0.928的操作F1。

Conclusion: 智能手机图像可以支持自动牙齿水平解剖映射，为资源有限的环境中的远程筛查和远程牙科提供可扩展、可能低成本的基座。
...
TL;DR: UQ-Loc通过处理不确定性提高LiDAR场景坐标回归的定位精度和鲁棒性


<details>
  <summary>Details</summary>
Motivation: 提高定位精度和鲁棒性，同时处理不确定性

Method: UQ-Loc，结合非各向同性高斯协方差头，使用NLL损失和kNN平滑正则化，并采用ECE评估不确定性质量

Result: 在6-DoF定位精度上实现一致提升，产生良好校准的协方差矩阵

Conclusion: UQ-Loc在处理不确定性方面表现出色，提高了定位精度和鲁棒性
...
TL;DR: 提出了一种新的LLM推理框架，通过理论分析和实验验证，揭示了LLM推理中的统计规律，为模型优化提供了指导。


<details>
  <summary>Details</summary>
Motivation: 为了深化对大型语言模型（LLMs）行为的理解并指导模型优化，

Method: 提出了一种框架，该框架在不简化模型架构或类比现有物理系统的情况下，寻求LLM推理中的统计规律和理论解释。将LLM推理表述为线索图上的引导发现过程，并使用平均场近似法推导出发现线索分数的一维常微分方程。

Result: 通过实验，使用学生LLM对教师LLM输出的标准化惊讶度来识别线索标记，并通过平均许多推理思维链来获得统计规律。实验结果表明，所得到的统计规律在同一数据集内可重复，并且可以通过解决所提出的理论方程进行拟合。

Conclusion: 该方法为LLM推理提供了新的理解和优化指导。
...
TL;DR: GraphRAG在引用精确度上表现不佳，三重鲁棒性是值得信赖的RAG架构声明的最低标准。


<details>
  <summary>Details</summary>
Motivation: 解释GraphRAG在引用精确度方面表现不佳的原因。

Method: 三重鲁棒性分析，固定检索架构，改变嵌入器、语料库和裁判三个维度。

Result: GraphRAG在所有三个设置中都会产生过引用，其忠实度后果取决于语料库。

Conclusion: 三重鲁棒性是值得信赖的RAG架构声明的最低标准。
...
TL;DR: RIG-RoPE addresses limitations of multidimensional position assignment in multimodal contexts
...
Conclusion: RIG-RoPE adds no learned parameters and can be implemented inside tiled attention kernels
...
TL;DR: 框架介导的后期训练有效提升模型性能，自动技能发现显著提高通过率。


<details>
  <summary>Details</summary>
Motivation: 训练后的参数优化和推理时的过程框架通常独立设计，导致难以自动获取和内部化复杂策略。

Method: 提出了一种基于框架介导的后期训练方法，将过程框架组织成一个可演化的图结构，通过发现、提炼和动态重新编译与模型参数协同进化。以技能训练为例。

Result: 在FeatureBench上，自动发现的技术提高了通过率8.1个百分点。经过渐进提炼后，模型在没有外部框架的情况下仍达到了27.7%的通过率（提炼保留率85.2%，定义为提炼后的/有技能的通过率），显著优于标准SFT在同一数据上的表现。

Conclusion: 框架介导的后期训练方法有效提高了模型的通过率和性能，为自动获取和内部化复杂策略提供了新思路。
...
TL;DR: CAE通过锚定安全电路，在进化过程中保持模型安全性，优于传统约束方法。


<details>
  <summary>Details</summary>
Motivation: 当前自进化算法缺乏类似的发展约束，可能导致模型在增强能力的同时失去生存所需的本质功能。

Method: 提出电路锚定进化（CAE），通过机制可解释性识别一个小的安全电路，并在进化过程中将其锚定，同时允许其他特征自由进化。

Result: CAE在保持安全性的同时，最小化能力损失，在效果和效率上都优于基于奖励的显式约束。

Conclusion: 电路锚定可以防止模型进化产生能力强大但危险的系统。
...
TL;DR: PoolBench基准揭示了池化设计选择对概念表示性能的影响，并指出构建方法的选择比池化本身更重要。


<details>
  <summary>Details</summary>
Motivation: 对解码器仅概念表示工作中的池化设计选择进行深入分析，因为目前缺乏比较不同概念、模型和任务中池化选择的共享协议。

Method: 提出PoolBench基准，通过固定评估协议来隔离池化作为实验变量。PoolBench涵盖了17个概念、19种池化策略和3个开放的仅解码器模型（Llama-3.1-8B、Gemma-2-9B、Mistral-7B），在37,693个真实文本段落组成的单一审计语料库上进行了评估。主要轴是线性可分性（D1/AUROC），引导概念普遍性（D2/SCP）和输出级别的解耦（D3）作为诊断轴。

Result: 主要发现是决定性的：W4_hierarchical达到跨模型的平均AUROC为0.7799，而广泛采用的P1_last_token基线仅达到0.7640，并且统计上显著较差（Friedman+Nemenyi，p = 2.0e-36；在18种有效策略中有77个显著对）。排名在层之间稳定（rho = 0.961--0.990）。一个关键的负面结果是：强大的检测并不等同于强大的引导——对于大多数概念，D2和D3在D1中显著较弱，这表明这是一个基本的表示限制，而不是池化失败。在中难度概念上，W4_hierarchical比P1_last_token高0.042--0.113 AUROC；构建方法的选择（DiffMean与REPE）比池化有更大的影响（AUROC变化0.15），确立了正确的实践层次。

Conclusion: PoolBench基准有助于理解池化设计选择对概念表示的影响，并提供了评估不同池化策略的统一框架。
...
TL;DR: Proposed SER framework improves performance in low-resource Persian language SER using Whisper and PCA, with limited benefits from ASR fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Speech Emotion Recognition (SER) in low-resource languages is challenging due to limited labeled data.

Method: Propose a SER framework with Whisper, dimensionality reduction using PCA, and ASR fine-tuning.

Result: PCA-based dimensionality reduction improves SER performance and efficiency, while ASR fine-tuning yields limited gains for SER.

Conclusion: Findings provide insights into using large pretrained speech models for SER in low-resource languages.
...
TL;DR: 评估了LLM中的认知偏差，发现了影响偏差表达的行为动态，并发布了相关工具和数据


<details>
  <summary>Details</summary>
Motivation: 评估现实多轮交互环境中高级指令微调LLM中的认知偏差表达

Method: 引入新的三条件实验框架，对24,300个由评审员验证的用户提示进行基准测试

Result: 发现6个LLM在偏见对话环境下系统性增加了偏差表达，识别出两种行为动态：对话中接触偏见推理通常会放大下游偏差倾向，而明确表达的偏见提示通常触发与对齐相关的抑制行为，从而减少偏差表达

Conclusion: 提出的方法有助于研究LLM中的情境条件认知偏差和行为适应
...
TL;DR: CNM-BERT improves Chinese NLP models with structural encoding for OOV characters.


<details>
  <summary>Details</summary>
Motivation: Improve performance on rare and OOV characters by incorporating compositional structure.

Method: Proposed CNM parses IDS into trees and encodes them with Tree-MLP, fusing structural embeddings into BERT.

Result: CNM-BERT outperforms baseline on structural-probing and tasks across different datasets, achieving significant improvements.

Conclusion: CNM-BERT demonstrates robust OOV understanding and tangible downstream value across various tasks and scales.
...
TL;DR: ConWriter是一个无需训练的框架，用于一致性感知的长篇故事生成，在长篇故事生成任务中表现良好。


<details>
  <summary>Details</summary>
Motivation: 长篇故事生成需要模型在扩展上下文中保持叙事一致性，但现有基于提示的方法往往随着时间的推移积累时间、事实、角色、常识和风格错误。

Method: 我们提出ConWriter，这是一个无需训练的一致性感知长篇故事生成框架。ConWriter以场景级别增量编写故事，由静态故事需求、动态叙事记忆、符号状态推理和不确定性感知风险信号引导。而不是将长篇故事生成视为单个自由解码过程，ConWriter维护不断发展的故事状态，检查新场景是否满足所需叙事转换，并使用不确定性感知风险信号来优先验证和局部修复。这使生成过程中可以实现一致性控制，在局部错误传播到后续场景之前。

Result: 在ConStory-Bench上进行评估，包括四个长篇故事任务：延续、生成、扩展和完成。由于长篇生成的成本高，我们使用了每个任务的第一个五个案例，并在Qwen3.5-Plus、DeepSeek-V4-Flash和GPT-5系列上测试了3k、6k和12k的目标长度。实验遵循ConStory-Bench官方评估协议。

Conclusion: ConWriter在长篇故事生成方面表现良好，通过维护故事状态和使用不确定性感知风险信号实现了一致性控制。
...
TL;DR: 本文提出自洽性框架，用于理解语言模型的失败，并探讨通过一致性优化改善模型性能的方法。


<details>
  <summary>Details</summary>
Motivation: 许多语言模型（LM）在预训练和后训练管道中表现出复杂的失败，包括过度关注用户立场、逻辑泛化不完整和产生自信但错误的响应。

Method: 提出自洽性作为理解这些失败的理论框架，并讨论了通过一致性优化可以实现的模型属性。

Result: 提出了一种新的模型属性，通过优化一致性可以实现的，并讨论了开发普遍一致的语言模型的意义。

Conclusion: 自洽性框架有助于理解语言模型的失败，并通过一致性优化可以改善模型性能。
...
TL;DR: 嵌入本地化原则到提示上下文中提高了LLMs本地化准确性的准确性。


<details>
  <summary>Details</summary>
Motivation: 分析五种大型语言模型（LLMs）在时间、数字和日期本地化方面的能力，而不是翻译。

Method: 选择了可以加载到通用硬件上并运行的模型，为每个模型计算了基线质量，然后测试了三种不同的提高准确性的策略。

Result: 与Tang等人的发现相反，发现将本地化原则嵌入到提示上下文中，与直接翻译或替代策略相比，在测试的LLMs上提供了统计上显著提高的准确性。

Conclusion: 嵌入本地化原则到提示上下文中是提高LLMs本地化准确性的有效方法。
...
TL;DR: CFR是一种针对测试时的干预措施，可提高数学推理的准确性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在生成数学对象时可能违反显式要求，例如省略模数缩减、返回非整数或使用错误的编码答案形式。

Method: 约束-首先推理（CFR），一种无需训练的两阶段提示协议。

Result: 在AIME、CMIMC、BRUMO和AIMO_AMC上，该方法在多个骨干上改善了直接思维链（CoT）。

Conclusion: CFR作为特定测试时干预措施，其效益取决于可恢复的约束和可靠的阶段1提取，而不是作为数学推理的通用替代品。
...
TL;DR: 本章探讨了普通话中语调和声调的相互作用，通过实证研究和理论模型，揭示了它们在传达交际意义中的作用。
...
Conclusion: 为理解普通话中语调和声调的复杂关系提供了新的视角
...
TL;DR: 通过检索示例增强提示，LLMs在文档级文本简化任务中表现出色。


<details>
  <summary>Details</summary>
Motivation: 文本简化需要大型语言模型（LLMs）重写复杂文档，同时保留意义、可读性和话语连贯性。尽管基于提示的LLMs表现出有希望的性能，但它们通常会产生不一致的简化，因为仅凭文本指令对复杂文档级转换的指导有限。

Method: 我们研究了检索到的文档简化示例是否可以通过从并行简化语料库中选择示例来增强提示，从而提高文档级生成。这种方法使LLMs能够在不进行特定任务微调的情况下利用相关的简化模式。

Result: 在OneStopEnglish语料库上使用多个最先进的LLMs进行的实验表明，结合检索到的示例可以一致地提高简化质量，与仅使用提示的生成相比，与代表性的监督（T5）和基于计划的（PlanSimp）文档简化系统相比，其性能具有竞争力或更优。此外，我们发现示例引导提示的好处因LLMs而异，这表明有效使用检索到的示例取决于模型在生成过程中整合上下文信息的能力。

Conclusion: 通过从并行简化语料库中检索示例并增强提示，可以显著提高LLMs在文档级文本简化任务中的性能。
...
TL;DR: 本研究对基于扰动的CPT进行了系统研究，发现它可以提高多语言方言的鲁棒性，并为选择CPT策略提供了指导。


<details>
  <summary>Details</summary>
Motivation: 多语言语言模型中方言变体仍然是一个主要挑战。扰动式持续预训练（CPT）已成为提高鲁棒性的一个有前景的方法。然而，现有工作在很大程度上孤立地评估单个扰动策略，并且对它们为什么有效提供了有限的见解。

Method: 对基于扰动的CPT进行系统研究，比较了九个德语、意大利语和阿拉伯语方言任务中的六个训练条件。

Result: 基于扰动的CPT，尤其是字符噪声CPT，在保持标准语种性能的同时，一致地提高了零样本方言鲁棒性。更重要的是，我们表明，具有类似下游性能的方法会诱导不同的鲁棒性机制，表现出不同的语言模型适应性、表征对齐和预测修复模式。

Conclusion: 结果提供了对合成表面变异如何提高鲁棒性的更完整理解，并为在多语言和方言环境中选择CPT策略提供了实际指导。
...
TL;DR: 本文提出了一种衡量LLM生成内容分布宽度的框架，发现LLM生成内容相对狭窄，但合理


<details>
  <summary>Details</summary>
Motivation: 分析LLM在创作哈利波特同人小说时与人类写作之间的差距

Method: 提出一个基于人类写作经验分布的框架来衡量LLM生成内容的分布宽度

Result: 发现当前LLM生成的内容虽然合理但较窄，集中在人类响应空间的中心附近

Conclusion: 该框架有助于研究人员更好地评估LLM创作内容的分布宽度，即其‘文化影响力’
...
TL;DR: SkillZip是一种高效的程序抽象框架，通过保持合同压缩和智能扩展，显著提高了大型语言模型在技能库检索和技能重用方面的性能。


<details>
  <summary>Details</summary>
Motivation: 随着技能库的增长，现有系统在重用程序、压缩过程中保持程序合同、保持压缩程序的执行性和可扩展性以及随着技能的发展更新压缩库方面存在挑战。

Method: SkillZip，一个执行感知的程序抽象框架，在节级别图上执行保持合同压缩。它将重复的合同有效模式重写为可逆的宏，同时保留边界签名、依赖闭包、验证器可达性和源级别扩展。在推理时间，它将紧凑的、依赖封闭的上下文进行解包，仅在需要时扩展宏。ReZip进一步集成新技能并使用执行证据修订有风险的宏。

Result: 在技术和具身代理基准测试上的全面实验表明，SkillZip在压缩率提高3.46倍的同时，实现了99.2%的依赖保留和98.7%的验证器可达性，并且性能优于最强大的基线高达12.2分。

Conclusion: SkillZip在技能库检索和技能重用方面表现出色，为大型语言模型提供了一种有效的解决方案。
...
TL;DR: FOCUS是一种新的LLMs角色控制方法，通过解耦特定领域的专家角色和自适应控制角色激活，提高了任务准确率。


<details>
  <summary>Details</summary>
Motivation: 为了解决现有角色控制方法在跨领域耦合问题，导致在医疗等高谨慎领域行为过于激进，或在金融等风险敏感领域行为过于保守的问题。

Method: FOCUS方法首先从LLMs中自动提取专家角色向量，然后应用正交分解来解耦特定领域的专家角色，最后引入专家门控模块以根据任务上下文自适应地控制角色激活。采用两阶段训练策略和门控选择正则化器，模型学习为单领域和跨领域任务激活适当的角色。

Result: 在金融、法律、医疗和跨领域基准测试中，FOCUS提高了任务准确率，并优于现有的角色控制方法。

Conclusion: FOCUS方法能够有效解决跨领域耦合问题，提高任务准确率，是LLMs中一种有效的角色控制方法。
...
TL;DR: 研究LLMs的指代词解析能力，发现某些LLMs在特定条件下近似人类表现。
...
Method: 采用标准链接假设，将人类阅读时间与模型在指代词处的惊讶度相关联。比较模型准确率与人类准确率，以探究指代词先行词的理解问题。

Result: 某些LLMs在指代词解析中表现出与人类类似的敏感性，而对语义干扰效应的敏感性较弱或不存在。
...
TL;DR: dLLMs的自由度可能导致推理失败，通过新的设计可以恢复推理能力。


<details>
  <summary>Details</summary>
Motivation: Masked diffusion language models (dLLMs) 提供了在推理任务中的自由度，但这一自由度可能导致推理失败。

Method: 在LLaDA-8B和GSM8K上进行实验，分析解码过程中的token分配，并引入新的提示-解码器设计。

Result: 发现自由度高的解码方式在推理任务中表现不佳，提出了一种新的干预方法，可以恢复推理能力。

Conclusion: dLLMs的自由度不是优势，而是可能导致推理失败的轴。通过引入新的设计，可以恢复推理能力。
...
TL;DR: 该研究通过PPMI图平均优化了RI向量，提高了童话数据集上的准确率。
...
Method: 在童话语料库上，通过PPMI图平均修复弱RI初始化，提高准确率。

Result: PPMI图平均与Top-K剪枝是弱RI嵌入的有用的非梯度修复。

Conclusion: 该方法在童话数据集上提高了RI的准确率，但在其他数据集上表现不佳。
...
TL;DR: EchoPrompt是一种基于潜在提示恢复的无监督检测器，在零样本检测器中表现出色，对于检测机器生成文本具有重要意义。


<details>
  <summary>Details</summary>
Motivation: 为了解决大型语言模型（LLMs）生成文本的风险，如虚假信息传播、教育滥用和平台治理问题，需要开发鲁棒的机器生成文本检测器。

Method: 提出了一种名为EchoPrompt的无监督检测器，基于潜在提示恢复。该检测器通过在机器生成文本前添加统一的通用前缀，部分重新激活隐藏的依赖关系，并使用指令微调模型来测量诱导的可能性增益，将其与对应的基础模型进行校准，并将结果差异汇总为评分，以量化潜在提示依赖。

Result: EchoPrompt在零样本检测器中取得了最先进的性能，同时在具有挑战性的评估设置中保持了强大的鲁棒性。

Conclusion: EchoPrompt是一种有效的无监督检测器，可以有效地检测机器生成的文本，对于解决LLMs带来的风险具有重要意义。
...
TL;DR: 本文比较了两种自动语音识别中识别新词和罕见词的策略，并分析了它们的优缺点。
...
Result: 上下文偏差方法将偏差词错误率（WER）降低了高达88%，而对其他词的影响不大。语音LLMs在朗读语音上表现优异，但泛化到非朗读语音较差，且对干扰计数和提示词顺序敏感。

Conclusion: 本文比较了两种策略，并分析了它们的优缺点，为方法选择提供了指导。
...
TL;DR: TCFM是一种自适应框架，通过选择性地应用Flow Matching和结合教师指导，在多种任务中显著提升了多语言文本嵌入质量。


<details>
  <summary>Details</summary>
Motivation: 不同任务需要不同的优化策略，但现有的多语言文本嵌入模型通常使用单一的训练目标进行适应。

Method: 提出了Task-Conditional Flow Matching（TCFM），这是一个多语言嵌入自适应框架，该框架在选择性地应用于翻译任务时应用Flow Matching，同时使用更适合其学习动态的目标来优化检索、分类和配对分类任务。TCFM进一步结合了教师指导的表示保持和三阶段课程以实现稳定适应。

Result: 在Indic Massive Text Embedding Benchmark上进行评估，TCFM建立了一个新的最先进的指标，在多样化的多语言任务中一致地提高了嵌入质量，并在嵌入模型家族中实现了泛化。

Conclusion: TCFM通过提供任务特定的自适应策略，显著提高了多语言文本嵌入模型在各种任务上的性能。
...
TL;DR: HiLP通过引入高级抽象潜在变量，有效解决了NTP和MTP等方法的局限性，提高了语言模型的性能。


<details>
  <summary>Details</summary>
Motivation: 为了解决标准Next-Token Prediction (NTP)在长距离推理和规划方面的不足，以及MTP和NextLat等方法的局限性。

Method: 引入了分层潜在预测（HiLP），通过引入辅助的高级抽象潜在变量来减少潜在空间中的误差累积效应。

Result: HiLP可以导致更长的距离一致的信念状态表示，并在编码和多步推理基准测试中证明了方法的有效性，并提供了更高效的解码效率。

Conclusion: HiLP是一种有效的长距离推理和规划方法，可以显著提高语言模型的性能。
...
TL;DR: 提出M$^3$R-Bench和M$^3$R-Reasoner，以解决现有隐喻理解模型的局限性，实验表明其在多个指标上优于现有模型。


<details>
  <summary>Details</summary>
Motivation: 为了解决现有基准在隐喻理解方面的局限性，如缺乏基于证据的解释，难以评估模型是否基于视觉和文本线索建立映射。

Method: 提出了M$^3$R-Bench，一个统一且基于证据的基准，包含1000个经过人工验证的图像-文本实例，并提供隐喻发生、目标-源映射、情感和逐步解释的联合注释。此外，还提出了M$^3$R-Reasoner，结合基于课程的推理监督和任务感知强化学习，以使模型推理与隐喻解释保持一致。

Result: 实验结果表明，M$^3$R-Reasoner在四个统一任务指标上优于更大的专有MLLM，并在视觉证据和情感推理方面分别提高了28.45和30.11分，同时在平均评分标准上超过Claude-Sonnet-4.6 8.00分。

Conclusion: M$^3$R-Bench和M$^3$R-Reasoner在隐喻理解方面提供了显著的改进，有助于解决现有模型在处理跨模态证据和映射方面的不足。
...
TL;DR: 提出MoCA和CoDAR，用于分析隐含社会语境，提高模型性能，但仍存在挑战。


<details>
  <summary>Details</summary>
Motivation: 由于现实世界中隐含社会语境的普遍存在，但缺乏一个正式和系统的框架来研究它们。

Method: 提出了一种名为MoCA的新任务，系统地从情感、意图和立场三个关键维度对隐含社会场景进行建模。构建了一个包含3,108个多模态实例的高质量基准数据集，并通过精细的语义注释揭示表达者、表达内容、接受者以及表达方式和原因。

Result: 使用MoCA数据集，我们发现最先进的跨模态大语言模型在这个任务上表现不佳，因为它们依赖于明确的线索，并且难以推理隐含的社会语境。提出了一种名为CoDAR的新框架，通过将观察到的表达和期望的真诚行为之间的差异建模为认知冲突，从而推断隐藏的心理状态。实验表明，CoDAR显著提高了模型性能。

Conclusion: 尽管CoDAR显著提高了模型性能，但与人类推理之间仍存在较大差距，突显了隐含社会理解的固有困难。
...
TL;DR: 提出TSR框架，结合LHRL-VGR算法，显著提升LLMs在社交互动中的表现。


<details>
  <summary>Details</summary>
Motivation: LLMs在结构化任务中表现出色，但在动态社交互动中表现不佳，需要长期目标协调和快速适应。

Method: 提出Think-Strategy-Response (TSR)框架，将社交对话分解为两个层次阶段：高级战略规划和低级语言执行。引入线性化分层强化学习与方差门控奖励（LHRL-VGR）算法，根据目标实现分数的方差动态分配奖励。

Result: 在SOTOPIA基准测试中，该方法将Qwen2.5-7B代理优化，在目标完成成功率上超过GPT-4o基准，多智能体社交谈判任务中表现最先进。

Conclusion: 该方法在多智能体社交谈判任务中表现出色，有效提高了LLMs在动态社交互动中的表现。
...
TL;DR: MameLoshnLM是一种为意第绪语设计的开源语言模型，在基准测试中表现出色，为低资源语言的语言模型开发提供了新的思路。


<details>
  <summary>Details</summary>
Motivation: 为了解决意第绪语语言建模的局限性，以及现有多语言语料库和基准的不足。

Method: 引入Oytser高质量预训练语料库和Kashes多任务基准，使用Llama 3.1 8B进行预训练。

Result: MameLoshnLM在基准测试中优于同类规模的开放基准，更好地捕捉语言定义的词汇和形态模式。

Conclusion: 为意第绪语NLP提供了基础，为历史丰富但数字代表性不足的语言的语言模型开发提供了模板。
...
TL;DR: 提出了一种新方法来关联相似度分数，以简化模型迁移，并通过实验证明其有效性。


<details>
  <summary>Details</summary>
Motivation: 研究相似度分数如何在嵌入模型之间建立关联，解决由于几何特性不同导致分数不可直接比较的问题，从而简化模型迁移并限制阈值重用。

Method: 提出了一种新的方法，通过学习分数分布之间的映射来关联相似度分数，引入了合成查询探针（Synthetic Query Probing），生成查询以创建受控查询-块对，并使用线性、等调性和分位数映射学习分数转换函数。

Result: 在SciFact和一个专有语料库上的实验表明，模型在排名上大体一致，但绝对分数表现出系统性的失真。学习到的映射部分对齐这些空间并提高了阈值的便携性，等调回归表现最佳。

Conclusion: 强调了跨模型校准的必要性，并将合成查询探针定位为分析嵌入可比性的可扩展框架。
...
TL;DR: MACRO是一种有效的动态层路由框架，显著提高了LLMs的性能。


<details>
  <summary>Details</summary>
Motivation: 现有路由方法需要更新模型权重，为每个测试实例运行昂贵的搜索循环，或在推理期间需要真实标签。

Method: 提出了一种名为MACRO的框架，该框架学习特定于任务的路线，而无需修改底层参数。MACRO将层路由建模为依赖于层索引、计算预算阶段、方向位移和操作上下文的上下文相关马尔可夫策略。

Result: MACRO在多个开放权重LLMs上的不同推理和知识基准测试中实现了平均+5.0%的准确率提升，在小模型上取得了最大的收益。MACRO比最佳动态路由方法Dr. LLM提高了+7.2%，同时将路由搜索时间减少了9.4倍。

Conclusion: MACRO是一种有效的动态层路由框架，可以显著提高LLMs的性能。
...
TL;DR: 研究通过使用在线双极性记忆，提高了Text-to-SQL的修复精度。


<details>
  <summary>Details</summary>
Motivation: 研究是否可以不更新参数，利用最终的修复结果来改善后续的Text-to-SQL场景。

Method: 提出MERIT，一个无需训练的智能体，维护在线双极性记忆，包含已验证的修正和观察到的失败方向。

Result: MERIT在Spider和Bird数据集上提高了执行精度，但在Bird上的提升证据较弱。

Conclusion: 这些结果表明，在何种情况下因果跨查询记忆可以改善修复，以及何时更广泛的记忆表示更为可取。
...
TL;DR: 通过将临床数据转换为书面交流，构建了无标签真实世界数据的临床NLP系统，为医疗自然语言处理提供了一种实际的研究资源。


<details>
  <summary>Details</summary>
Motivation: 为了提高医疗自然语言处理的能力，将临床交流中的信息从编码形式转化为更易于理解的传达形式。

Method: 使用大型语言模型将临床数据（如病历、诊断标签、症状列表或护理计划）转换为书面和转录的交流，以用于下游模型。

Result: 通过组织叙述调查和十三项新颖的案例研究，构建了无标签真实世界数据的临床NLP系统，包括EMS预到达报告、现场无线电伤员记录、护士交接班、患者门户分诊和低资源出院交流。研究表明，合成交流可以启动这些系统，并且微调的编码器模型优于评估的零样本基线。

Conclusion: 合成临床交流正在成为一种实际的研究资源；将其建立为可重用临床基础设施将需要真实数据的传输、安全和外部验证。
...
<details>
  <summary>Details</summary>
Motivation: 为了解决大型语言模型在个性化方面的局限性，如缺乏专业领域知识、高计算成本和快速数据更新问题，以及检索增强生成在提供细粒度、标记级引导方面的不足，以及基于聊天的接口在协同写作领域的局限性。

Method: 引入SteerWrite，一个无需训练的框架，用于个性化协同写作。该方法无需梯度更新，即可有效地将基础模型适应特定领域，并针对小数据集进行了特定设计。

Result: 实验表明，SteerWrite在多个数据集、指标和模型上实现了最先进的性能，显著降低了人工编辑工作量。

Conclusion: SteerWrite是一种有效的个性化协同写作框架，能够解决现有方法的局限性，并显著提高写作效率。
...
TL;DR: SiPE improves syntactic generalization and real-world language understanding by incorporating syntactic information into position embeddings.


<details>
  <summary>Details</summary>
Motivation: Positional embeddings (PE) in Transformers are agnostic to syntactic structure.

Method: Introduce Syntax-Informed Positional Embeddings (SiPE) that learns a lightweight syntactic prior from dependency parses during pretraining and injects it across all three dominant PE families. Isolate where and how the prior should enter the model based on the architecture. 

Result: SiPE improves on the SyntaxGym benchmark by up to 10.3% and reduces perplexity by 9.0% over a base model with no syntactic supervision. Also improves real-world language understanding, raising scores on the GLUE benchmark by up to 8.2% over a model trained without it. Establishes a new Pareto frontier between syntactic supervision and inference cost.

Conclusion: SiPE is an effective method for incorporating syntactic information into position embeddings, leading to improvements in both syntactic generalization and real-world language understanding.
...
TL;DR: 本文分析了ASR在语言多样性方面的挑战，并提出了一个参与式框架和审计协议以解决这些问题。


<details>
  <summary>Details</summary>
Motivation: 探讨自动语音识别（ASR）及其在公共服务、医疗和教育领域的应用，以及其对低资源、土著和非标准语言变体的影响。

Method: 基于语言学资本、种族语言意识形态、语言政策研究和去殖民化计算方法进行分析。

Result: 提出三个危害（3M）分类法——误识别、错配和不信任，以及一个七层情境模型，以解决ASR和ASR语音界面中的语言多样性问题。

Conclusion: 建议采用参与式框架和最低审计协议，将受影响的社区定位为共同设计师、评估者和治理伙伴，以实现文化胜任的ASR。
...
TL;DR: 观察模式选择动态调整，路由策略需考虑代理成功率。


<details>
  <summary>Details</summary>
Motivation: 研究不同观察模式对Web代理性能的影响

Method: 在VisualWebArena和WebArena上测量六种观察模式在八个网站模型组合上的表现

Result: 最佳模式的选择因任务而异，重复运行同一模式对结果影响不大，但通过将无法解决的任务发送到最便宜的模式可以降低成本9.5-30.6%，且成功率不变。五种路由策略中，没有一种能稳定地优于固定一种最佳模式，且路由监督的局限性在于代理的成功率。更强的代理可以提高标签供应和路由机会。

Conclusion: 观察模式的选择应根据任务动态调整，路由策略应考虑代理的成功率。
...
TL;DR: NeSy-RAG是一个改进的RAG框架，显著提升了问答系统的性能和透明度。


<details>
  <summary>Details</summary>
Motivation: 为了提高RAG的推理透明度和用户特定上下文处理能力，以及解决现有RAG中存在的推理步骤难以验证和用户上下文缺失的问题。

Method: NeSy-RAG是一个模块化神经符号RAG框架，它从检索到的文本片段中合成可归因的Prolog模块。

Result: NeSy-RAG在ShARC基准测试中，未经特定领域训练，实现了61.1%的准确率，超过了相同模型的RAG基线。

Conclusion: NeSy-RAG显著提高了基于检索的生成（RAG）的性能，特别是在推理透明度和用户特定上下文处理方面。
...
TL;DR: 提出GB/T-Bench，提升规则密集型文档审查的AI能力


<details>
  <summary>Details</summary>
Motivation: 对规则密集型文档审查能力评价不足

Method: 引入GB/T-Bench，提出GB/T Review Taxonomy，进行可控反例生成，开发诊断型评估协议和GB/T-Reviewer框架

Result: GB/T-Reviewer将CMCS提升至0.5094，展示结构化技能协调在规则密集型文档审查中的价值

Conclusion: 为标准化和其他高风险文档领域中的可信AI铺平道路
...
TL;DR: Proposed framework effectively evaluates conversational-agent benchmark quality.


<details>
  <summary>Details</summary>
Motivation: Poor benchmarks in task-oriented conversational agents lead to unreliable evaluations.

Method: A reference-free framework using LLM judges to assess benchmark quality.

Result: The framework consistently distinguishes between benchmark quality levels across domains and judge models.

Conclusion: The framework is a practical approach for evaluating conversational-agent benchmarks.
...
TL;DR: 提出了一种新的多语言推理迁移方法，显著提高了LLMs的推理能力。


<details>
  <summary>Details</summary>
Motivation: 为了扩展大型语言模型（LLMs）的推理能力，多语言推理迁移至关重要。

Method: 提出了一种名为RP-OPSD的方法，即推理枢轴引导的在线策略自蒸馏（Reasoning-Pivot-guided On-Policy Self-Distillation）。该方法利用匹配的教师视图在有和没有英语参考解决方案之间的分布差异作为指导特权蒸馏和参考锚定的操作代理。

Result: 在涵盖17种语言和多个难度级别的数学推理基准测试中，RP-OPSD方法优于强大的多语言推理基线和OPSD变体。进一步分析表明，RP-OPSD将特权蒸馏集中在推理控制和问题条件状态更新令牌上，而对于主要支持表面实现的令牌则降低权重。

Conclusion: RP-OPSD是一种有效的多语言推理迁移方法，可以显著提高LLMs的推理能力。
...
TL;DR: 该研究提出了评估模型选择性信任的方法，并证明了其有效性。


<details>
  <summary>Details</summary>
Motivation: 语言模型越来越多地根据外部信号来回答问题，单个误导信号可能将正确答案变为错误。

Method: 提出了MIST基准和SCOPE方法，用于评估模型在选择性信任方面的能力。

Result: 在综合基准研究中，发现模型对误导信号的敏感性是普遍存在的。SCOPE方法在降低误导信号影响的同时，保持了模型的准确性。

Conclusion: 模型应该根据选择性信任来评估，而不仅仅是抵抗力。
...
TL;DR: 本文调查了针对ViTs输入自适应推理的反向效率退化攻击，比较了两种代表性攻击，并分析了其攻击面、操作方式和防御措施。


<details>
  <summary>Details</summary>
Motivation: 为了满足能源和延迟预算，Vision Transformers (ViTs)越来越多地依赖于输入自适应推理，如token修剪和早期停止。本文调查了一种针对这些机制的反向效率退化攻击，旨在在不降低准确度的同时增加计算量。

Method: 统一和比较了两种代表性攻击：SlowFormer（通用对抗性补丁）和DeSparsify（图像扰动），并在三个流行的token修剪框架A-ViT、ATS和AdaViT上进行比较。使用GFLOPs、精度损失和攻击成功（AS）指标来标准化报告。

Result: 理解这些攻击对于设计既能减轻风险又保持轻量级的对策至关重要，因为部署通常发生在低功耗环境中，如移动或嵌入式设备。

Conclusion: 本文重点分析了输入自适应优化如何为效率退化创建攻击面，这些攻击在实际中的操作方式以及哪些优化最易受攻击，以及目前存在的防御措施及其在攻击下的效率恢复情况。
...
TL;DR: CyberBridge是一个将网络安全职位描述与专业角色配置文件自动映射的框架。


<details>
  <summary>Details</summary>
Motivation: 随着网络安全领域的快速发展，学术课程难以与实际工作所需的技能保持一致。

Method: 将职位描述分解为知识、技能和任务（KST）语句，使用句子BERT模型嵌入，并与最语义相似的劳动力配置文件匹配。

Result: CyberBridge支持三个主要用例：职位推荐、市场分析和课程规划。

Conclusion: CyberBridge是一个用于网络安全教育职业指导和基于证据的课程开发的实用工具。
...
TL;DR: LMR系统信号平面存在安全漏洞，攻击者可以推断出敏感信息，对网络安全构成威胁。


<details>
  <summary>Details</summary>
Motivation: 公共安全机构和关键基础设施运营商依赖基于P25、TETRA和TETRAPOL的树突状陆地移动无线电（LMR）系统，这些系统不仅需要保护通信内容，还需要保护通信的事实。然而，LMR标准在内容保密性和通信保密性之间存在着明显的差距。

Method: 通过被动观察信号，研究从暴露的信号中推断攻击者信息的方法。

Result: 发现即使内容加密完美，也可以从被动观察的信号中推断出敏感信息，包括网络拓扑、地理位置、单元存在、跨小区和组移动、组织结构以及操作安全细节等。

Conclusion: LMR标准的信号平面推断能力远超观察者的直接接收范围，将本地嗅探转变为全国性网络映射能力，从而降低或破坏LMR标准的身份模糊化。
...
TL;DR: eMicro是一种用于微服务的路径感知防御系统，有效防止多跳攻击，适用于大规模微服务环境。


<details>
  <summary>Details</summary>
Motivation: 现代云应用中微服务的交互形成复杂的请求路径，传统的服务间访问控制无法防止多跳攻击，导致系统易受未授权访问和数据泄露的影响。

Method: 提出eMicro，一个针对微服务的路径感知防御系统，通过基于历史的服务调用序列访问控制、高效的确定性有限自动机（DFA）安全策略编码以及基于eBPF的内核请求跟踪三种关键技术来防止多跳攻击。

Result: 在DeathStarBench和来自Uber、阿里巴巴和字节跳动的生产云跟踪数据上的评估表明，eMicro可扩展，在1微秒内执行策略检查，仅用100 MB存储5000万策略，将传播开销减少90%，对运行时影响微乎其微。

Conclusion: eMicro为大型微服务环境提供了可扩展且高效的针对多跳攻击的保护，适用于部署。
...
TL;DR: 本文对商业车辆制动ECU固件更新进行逆向工程分析，证实了安全召回修复也是一种安全补丁


<details>
  <summary>Details</summary>
Motivation: 分析商业车辆制动电子控制单元（ECU）固件更新的逆向工程

Method: 对更新器可执行文件进行分析以提取固件，并对S12X架构的固件映像进行差异二进制分析

Result: 识别出针对旧版协议处理中未记录漏洞的特定更改，并证实了安全召回修复也是一种安全补丁

Conclusion: 研究确认了安全召回修复也是一种安全补丁
...
TL;DR: 本文提出了一种新的IPI防御方法，通过对抗训练增强了分类器的鲁棒性，并在实验中取得了良好的效果。


<details>
  <summary>Details</summary>
Motivation: 现代大型语言模型（LLMs）的指令遵循能力使其能够自主完成复杂任务，但同时也容易受到恶意指令攻击。

Method: 开发了一种针对恶意句子分类的方法，该方法同时具有上下文和查询感知能力。同时，提出了两种对抗训练方法，以增强分类器的鲁棒性。

Result: 在间接提示注入（IPI）基准测试中，该方法在静态攻击下优于现有IPI防御基线，在自适应攻击下提供了更高的效用、更低的攻击成功率。

Conclusion: 恶意文本检测器在特定应用领域可能需要进行领域依赖的调整。
...
TL;DR: 本文通过模拟实验确定了比特币软分叉结果的决定因素，发现经济权重分布是关键


<details>
  <summary>Details</summary>
Motivation: 分析比特币共识机制及其软分叉结果的可预测性

Method: 使用情景发现方法，结合PRIM进行模拟实验

Result: 确定了导致争议性分叉结果的配置区域，并发现了影响分叉结果的关键参数阈值
...
TL;DR: 本研究揭示了macOS应用程序生态系统中隐私侵犯的广泛存在，并提出了相应的缓解措施。


<details>
  <summary>Details</summary>
Motivation: 桌面应用程序在用户隐私方面被忽视，需要对其数据收集和共享实践进行更深入的研究。

Method: 采用以应用程序开发为中心的视角，研究macOS生态系统中调节和披露数据收集和共享实践的机制。开发了一个名为NutriScan的分析框架，结合静态和动态分析技术，对macOS应用程序的数据实践和披露进行综合分析。

Result: 分析发现，85%的macOS应用程序在未披露的情况下访问用户数据API，49.7%将数据传输给广告实体和托管提供商，其中12.5%未进行相应披露。桌面应用程序被在线跟踪器用于丰富用户配置文件和设备指纹，揭示了在线跟踪生态系统的真实范围。

Conclusion: 桌面应用程序生态系统由不同的机制和数据抽象组成，增加了开发者的复杂性，并促进了未披露的隐私侵犯行为。提出了旨在简化数据披露过程并改善苹果应用程序审查过程的缓解措施。
...
TL;DR: PromptShield-Home基准测试表明，智能家居代理的安全性问题最好通过学习路由和传感器融合来解决，而不是用MLLM替换检测器。


<details>
  <summary>Details</summary>
Motivation: 智能家居助手使用多模态大型语言模型，需要区分真实用户指令和类似指令的干扰内容。

Method: 引入PromptShield-Home基准测试，比较不同抽象层级的检测方法。

Result: 检测器对所有内容都做出反应，而所有MLLM配置都过度拒绝，几乎不执行真实指令。正确集相互独立，最佳单层检测器准确率为76.5%，而最佳多层模型准确率达到94.1%。

Conclusion: 智能家居代理的安全性问题最好通过学习路由和传感器融合来解决，而不是用MLLM替换检测器。
...
TL;DR: 残差化表示法提高了CAN入侵检测的性能，但存在局限性。


<details>
  <summary>Details</summary>
Motivation: 现有的CAN入侵检测系统依赖于基于存在的特征，但在攻击者重用合法仲裁ID时表现不佳。

Method: 提出了一种针对CAN的特定表示方法，即基于ID的行为残差化，通过提取时序、协议和有效载荷特征并残差化与每个仲裁ID的正常基线。

Result: 在六个无监督检测器和两个数据集上，残差化提高了大多数评估中的平均F1值。在更现实的ROAD数据集上，该表示方法在针对信号操纵攻击的召回率达到0.99以上，具有高ROC-AUC。

Conclusion: 残差化表示法在CAN入侵检测中提高了性能，特别是对于重用合法ID的攻击，但存在新型ID洪水攻击和跨ID模糊攻击的局限性。
...
TL;DR: XSec是一种自解释深度架构，用于解决深度学习模型在安全应用中的可解释性问题，提高了分类精度并减少了解释延迟。


<details>
  <summary>Details</summary>
Motivation: 为了解决深度学习模型在安全应用中的可解释性问题，特别是透明度至关重要的问题。

Method: 提出了一种名为XSec的自解释深度架构，该架构在训练期间使用基于掩码的方法从数据中提取信息子特征，并学习原型，然后在测试时利用原型在专门的相似性层中计算相似度得分，从而生成可解释的解释。

Result: 在五个不同的安全场景中评估了XSec，证明了其能够以97.33%的平均分类精度实现，与近似方法和扰动方法相比，XSec显著减少了解释延迟。

Conclusion: XSec扩展了自解释AI在安全应用中的适用性，弥合了深度学习性能和关键场景中可解释性需求之间的差距。
...
TL;DR: PoisonedEvolution是一种轨迹投毒攻击，能成功嵌入目标行为，暴露了证据提升作为自我进化代理的安全边界。


<details>
  <summary>Details</summary>
Motivation: 为了提高自我进化技能（SES）系统的安全性，防止轨迹投毒攻击。

Method: 使用惰性金丝雀规范评估四种代表性安全效果家族，包括嵌入目标行为和攻击者记录。

Result: 在10%的攻击者支持下，PoisonedEvolution在六个主流LLM进化器中成功嵌入目标行为，证明了其在不同进化架构中的迁移能力。

Conclusion: 证据提升是自我进化代理的安全边界。
...
TL;DR: Proposed framework distinguishes scientific traffic from attacks, enhancing REN security.


<details>
  <summary>Details</summary>
Motivation: Research and Education Networks (RENs) face a security paradox due to the similarity between their normal traffic patterns and volumetric attacks.

Method: Proposing and evaluating a high-fidelity traffic forecasting framework using an exclusive dataset of 57 days and 13.7 billion packets.

Result: Advanced architectures like TiDE reduce baseline prediction error by 30-42% and improve the distinction between legitimate scientific bursts and potential anomalies. A novel anomaly-integration strategy improves model robustness by 3.3%.

Conclusion: The proposed framework effectively distinguishes scientific workflows from network attacks, enhancing network security operations.
...
TL;DR: ARIA是一个自动化框架，用于生成针对定制LLMs的后门指令，绕过检测并有效。


<details>
  <summary>Details</summary>
Motivation: 针对定制LLMs的后门攻击问题，提高攻击成功率，降低攻击难度。

Method: 提出ARIA，一个自动化红队框架，通过攻击者LLM迭代生成和优化后门指令，并利用目标LLM的结构化反馈指导。

Result: ARIA在三个代码智能任务上实现了最高的攻击成功率（0.945），并保持最佳清洁任务效用。

Conclusion: ARIA在绕过平台和用户检测方面表现出色，在所有任务中保持最佳的清洁任务效用，并有效抵抗现有防御方法，展现出强大的泛化能力和鲁棒性。
...
TL;DR: 提出ASV方法，显著提高神经网络模型提取攻击的效率


<details>
  <summary>Details</summary>
Motivation: 解决Carlini等人提出的神经网络模型提取攻击中的计算瓶颈

Method: 将几何视图的硬标签攻击转化为代数框架，提出近似签名向量（ASV）方法

Result: 显著减少聚类时间，提高模型提取效率

Conclusion: ASV方法在硬标签设置下有效地提高了神经网络模型提取攻击的效率
...
TL;DR: 提出了一种自适应LDP框架，可动态估计数据域，提高数值数据收集质量。


<details>
  <summary>Details</summary>
Motivation: 局部差分隐私（LDP）在收集数值数据时提供强大的隐私保障，但现有LDP机制需要预定义的数据域，这在实践中往往是未知的。这种先验知识的缺乏为数据收集者创造了一个关键困境：如果选择的域太窄，范围外的值将被裁剪，导致信息丢失。相反，如果域太宽，在隐私化过程中会添加过多的噪声，这会降低收集到的数据质量。这突出了对能够动态估计数据域的方法的需求。

Method: 提出了一种自适应LDP框架，其中每个用户发送两份数据：其扰动数值数据和表示其原始值是否被当前域裁剪的隐私化信号。通过聚合这些信号，我们的方法自适应裁剪区域边界（ABC）方法，迭代调整域以适应底层数据分布，无需先验知识。

Result: 在实证评估中，结果表明我们的框架在各种数据集和底层LDP机制中显著提高了数值数据收集的质量。我们还表明，估计的范围在实践中成功收敛，并且我们的方法对超参数具有鲁棒性。

Conclusion: 该研究提出了一种自适应LDP框架，能够动态估计数据域，从而提高数值数据收集的隐私性和质量。
...
TL;DR: This paper addresses quantum software security by proposing a structured S-FoM set and benchmarking framework.


<details>
  <summary>Details</summary>
Motivation: Quantum software security concerns and the lack of security metrics

Method: Characterization of measurement gaps, proposal of S-FoM set, and benchmarking rubric definition

Result: Three-layer measurement gap characterized, structured S-FoM set proposed, benchmarking rubric defined, and illustrative reanalysis of obfuscation techniques presented

Conclusion: Quantum software security deserves attention with explicit S-FoMs and a benchmarking framework
...
TL;DR: 提出了一个针对QaaS平台攻击的STRIDE威胁模型，识别了多种攻击向量，并提出了更有效的防御策略。


<details>
  <summary>Details</summary>
Motivation: 云量子即服务（QaaS）平台（如IBM Quantum、IonQ Cloud和Amazon Braket）的访问日益普及。混合量子经典算法（VQE、QAOA、QML）通过一系列的编排、编译和执行层传输数据。近期研究在各种单独阶段展示了各种关键攻击：校准篡改、SWAP攻击、QubitHammer等。然而，由于术语不同，这些攻击仍然是孤立的，且现有的基于STRIDE的量子威胁模型缺乏对QaaS堆栈的结构化视图。

Method: 将工作流程分解为六阶段模型，采用STRIDE威胁模型。

Result: 矩阵展示了每个阶段的量子特定、继承的经典和可能的攻击向量。进一步研究了被忽视的领域（否认和提升权限），并区分了三种不同的跨阶段攻击链，影响更大。

Conclusion: 提出了一个结构化的威胁模型来分析QaaS平台的攻击，并识别了潜在的攻击向量。
...
TL;DR: RustGo：高效的Rust模糊测试器，发现更多漏洞。


<details>
  <summary>Details</summary>
Motivation: Rust语言提供内存安全，但存在内存漏洞，现有模糊测试方法效率低。

Method: 提出RustGo，针对可能存在内存漏洞的代码区域进行模糊测试，利用Rust特定静态分析自动识别目标，并采用独立状态和动态剪枝。

Result: RustGo平均剪枝78.49%无关路径，比现有模糊测试器快2.09到5.08倍，发现13个未知漏洞。

Conclusion: RustGo是一种有效的Rust定向灰盒模糊测试器，可提高模糊测试效率并发现更多漏洞。
...
TL;DR: 提出APV作为任务条件下的漏洞管理抽象，用于管理持续部署实例的代理控制暴露。


<details>
  <summary>Details</summary>
Motivation: 为了管理跨越组件并超出任何单个事件的持续部署实例，现有的安全程序需要一种方法。

Method: 提出了一种名为“代理姿态漏洞（APV）”的任务条件漏洞管理抽象，这是一种持久记录，用于组合代理控制暴露。

Result: APV将不同任务中的不同运行时表现与不变姿态联系起来，并在权限缩小、缺失控制添加、风险接受或关闭验证之前保持开放。
...
TL;DR: 本文提出了一种基于GPML框架的拓扑驱动方法，用于检测水分配网络中的网络攻击，并在三个数据集上取得了良好的检测效果。


<details>
  <summary>Details</summary>
Motivation: 水分配网络依赖工业控制系统，这使得它们容易受到改变流量模式和网络行为的网络攻击。传统的检测方法依赖于原始流量或协议信息，往往忽略了由这些攻击引起的结构变化。

Method: 提出了一种基于图处理机器学习（GPML）框架的拓扑驱动方法，将原始流量转换为动态图，从中提取和分析社区和频谱指标，以检测任何随时间变化的结构和通信修改。

Result: 在三个工业水分配数据集（HITL、SWaT和CrossTest）上评估了该方法。频谱和社区图指标在检测网络攻击和物理攻击方面提高了模型性能。

Conclusion: 该方法有效地检测了水分配网络中的网络攻击，并在三个数据集上提高了检测性能。
...
TL;DR: 多模态安全降级源于表示失配，MMAligner显著提高模型安全性。


<details>
  <summary>Details</summary>
Motivation: 分析多模态大型语言模型（MLLMs）在拒绝不安全文本提示的同时生成有害响应的原因。

Method: 几何分析MLLM表示，提出MMAligner方法。

Result: 发现安全机制在模态间持续有效，但不安全的模态输入发生表示偏移，导致安全降级。MMAligner显著提高拒绝率，改善安全-效用权衡。

Conclusion: 多模态安全降级源于表示失配，MMAligner有效提高模型安全性。
...
TL;DR: dfence指令有效减轻Spectre攻击，性能影响小。


<details>
  <summary>Details</summary>
Motivation: 现有的软件和硬件缓解措施有限，且存在错误和性能开销问题。

Method: 在Proteus CPU上实现dfence指令，并通过静态类型系统验证其正确性。

Result: dfence指令在性能上具有小于1%的平均开销，并支持静态类型系统验证。

Conclusion: dfence指令可以有效地减轻Spectre-PHT和Spectre-STL攻击，同时保持较低的硬件开销和性能影响。
...
TL;DR: 本研究通过硬件约束密钥和Zero-Trust架构，显著提高了密钥安全性，防止了所有注入攻击


<details>
  <summary>Details</summary>
Motivation: 提高密钥的保密性和内容感知授权

Method: 使用硬件约束密钥和五层Zero-Trust执行栈

Result: 成功防止了所有注入攻击场景，没有假阳性

Conclusion: 硬件约束密钥和Zero-Trust架构能够有效提高密钥安全性
...
TL;DR: 本文提出了一种新的版权保护机制，通过不可学习扰动和双重提取策略，有效地解决了深度学习中的版权保护问题。


<details>
  <summary>Details</summary>
Motivation: 为了保护深度学习中的数据版权，研究者们提出了新的版权保护机制。

Method: 通过向示例添加精心设计的扰动，使模型无法学习，以保护数据版权。同时，设计了一种双重提取策略来消除不可学习扰动对水印提取的副作用。

Result: 在ImageNet、CIFAR10和Pets图像数据集上进行了大量实验，结果表明该方法可以提供全面的版权保护。

Conclusion: 本文提出的方法可以有效地解决深度学习中的版权保护问题，为数据版权保护提供了一种新的思路。
...
TL;DR: HOPSCOTCH是一个用于机械化和验证密码学证明的新框架。


<details>
  <summary>Details</summary>
Motivation:  mechanizing computationally sound, game-based cryptographic proofs

Method: using a Lean 4 framework with shallow embedding and state-abstraction methodology

Result: a new framework called HOPSCOTCH, which can mechanize cryptographic proofs and prove their computational soundness

Conclusion: HOPSCOTCH provides an effective way to mechanize and verify cryptographic proofs
...
TL;DR: 提出了一种从Tamarin到ProVerif的可靠翻译，支持严格的比较，证明了可靠性和完整性。


<details>
  <summary>Details</summary>
Motivation: 为了对Tamarin和ProVerif这两个用于安全协议形式验证的著名工具进行系统比较。

Method: 提出了一种从Tamarin到ProVerif的可靠翻译，包括公式重写、编码多集重写语义和同时处理事件等新技术。

Result: 翻译覆盖了562个566个引理任务，其中246个（247个）在非XOR任务中具有明确结果，ProVerif在334个（92.3%）情况下更快。

Conclusion: 该翻译支持对Tamarin和ProVerif进行严格的比较，并证明了在忠实片段中的可靠性和完整性。
...
### [202] [Quality Diversity for Reliable Data Driven Time-Use Optimization](https://arxiv.org/abs/2608.05230)
*Aneta Neumann,Ty Stanford,Dorothea Dumuid,Frank Neumann*

Main category: stat.ML

TL;DR: 本文提出了一种不确定性量化质量多样性（QD）框架，以更可靠地推荐时间使用。


<details>
  <summary>Details</summary>
Motivation: 忽视健康相关决策中的不确定性会导致不切实际的时间使用建议。

Method: 使用组合数据分析，结合儿童队列数据集，开发了一种新的方法，将预测不确定性纳入QD过程。

Result: 我们的框架通过直接将不确定性嵌入优化，将时间使用建议转向不确定性较低的区域，同时保留高质量结构，以实现更可靠的行为健康决策。

Conclusion: 我们引入了一个不确定性量化质量多样性（QD）框架，以更可靠地推荐时间使用。
...
### [203] [A Unified Causal Inference Framework for the Desirability of Outcome Ranking Paradigm in Benefit-Risk Evaluation](https://arxiv.org/abs/2608.05244)
*Yuan Feng,Shiyu Shu,Yixin Fang,Ionut Bebu,Toshimitsu Hamasaki,Scott Evans,Guoqing Diao*

Main category: stat.ML

TL;DR: 开发了一种新的因果推断框架，用于评估随机试验和观察性研究中的利益-风险评估。


<details>
  <summary>Details</summary>
Motivation: 为了在随机试验和观察性研究中进行利益-风险评估，我们开发了一个统一的协变量调整因果推断框架，用于估计结果排序（DOOR）概率的可取性。

Method: 该框架将DOOR概率表示为两种治疗方案下边际序数结果分布的双线性泛函，通过顺序风险集风险估计条件序数分布，并推导出DOOR概率的有效影响函数（EIF）。

Result: 点估计模拟比较了G计算、标准化逆概率加权（IPW）、增强IPW（AIPW）和目标最大似然估计（TMLE），使用广义线性模型或超级学习器（SL）估计干扰函数。TMLE-SL在点估计性能方面表现最强且最一致，AIPW-SL排名第二。基于EIF的推断在AIPW-SL和TMLE-SL中进行评估，包括交叉拟合，跨越重叠、治疗效应异质性和治疗分配变化的设置。CVTMLE-SL在DOOR尺度偏差、潜在序数分布的恢复、标准误差准确性和置信区间覆盖率方面表现出最强的整体性能。

Conclusion: 本研究提出了一种新的因果推断框架，为随机试验和观察性研究中的利益-风险评估提供了有效的工具。
...
### [204] [Deep Generalised Mixed Models: a Novel Neural Network Structure for Analysing Hierarchical Data](https://arxiv.org/abs/2608.05930)
*Nina van Gerwen,Dimitris Rizopoulos,Manon Hillegers,Loes Keijsers,Sten Willemsen*

Main category: stat.ML

TL;DR: Developed a novel neural network architecture for ESM data analysis, showing potential but with suboptimal performance due to model instability.


<details>
  <summary>Details</summary>
Motivation: The work is motivated by data collected by the GrowIt! app, which was released to investigate daily emotions among adolescents during the COVID-19 pandemic.

Method: Develop a novel neural network architecture that generalises mixed effects models to deep learning.

Result: The model can accommodate longitudinal outcomes following generic distributions, scale well to high-dimensional settings and provide valid inference when data are missing-at-random. However, the model instability leads to suboptimal performance.

Conclusion: The Deep Generalised Mixed Model has potential but requires further improvement for stability and performance.
...
### [205] [Handling Missing Data in Probabilistic Regression Trees](https://arxiv.org/abs/2608.06195)
...
### [206] [Beyond Marginal Validity: Finite-Sample Guarantees for Localized Conformal Prediction](https://arxiv.org/abs/2608.06206)
*Anton Conrad,Rustam Isaev,Denis Belomestny,Eric Moulines,Sergey Samsonov*

Main category: stat.ML

TL;DR: 本文提供了关于RLCP的有限样本保证，解决了条件有效性和预言效率的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的RLCP理论缺乏对实现局部集的有限样本保证，同时控制条件有效性和预言效率。

Method: 通过证明条件覆盖间隙和长度误差的上界来提供保证。

Result: 证明了在高概率下，对于任何固定的分数，当条件得分CDF满足Hölder正则性和标准密度和核假设时，条件覆盖间隙和长度误差的上界是统一的。

Conclusion: 本文提供了关于随机局部一致性预测（RLCP）的有限样本保证，解决了条件有效性和预言效率的问题。
...
### [207] [Minimax Optimal Early-Stopped Gradient Descent for Gaussian Mixture Classification](https://arxiv.org/abs/2608.06250)
*Alex Buna,Shirley Xiaoqi Liu,Patrick Rebeschini*

Main category: stat.ML

TL;DR: 提前停止可以解决过参数化分类中梯度下降法的次优性问题，并实现最优风险。


<details>
  <summary>Details</summary>
Motivation: 研究过参数化分类中梯度下降法在逻辑损失函数下的表现，以及如何通过提前停止来克服统计上的次优性。

Method: 结合了提前停止迭代的严格上界和任意分类器的匹配统计下界，并建立了一个新的校准结果，将过量逻辑风险转换为过量零一风险。

Result: 在具有标签翻转噪声的高斯混合模型中，提前停止在适当的先验时间达到最小-最大最优零一风险。

Conclusion: 提前停止可以克服梯度下降法在逻辑损失函数下的次优性，并在具有快速和连续衰减的协方差谱的情况下实现最小-最大最优风险。
...
### [208] [Stochastic Dynamics on Persistence Diagram Space via Reinforcement Learning](https://arxiv.org/abs/2608.06276)
*Farzana Nasrin*

Main category: stat.ML

TL;DR: 提出了一种基于强化学习的PD空间随机动力学框架，能够有效降低图复杂度，同时保留主导拓扑结构。


<details>
  <summary>Details</summary>
Motivation: 为了解决现有文献中持久度图（PDs）的统计分析和概率建模框架有限的不足，

Method: 提出了一种基于强化学习的PD空间随机动力学框架，通过拓扑感知的局部编辑操作使图演化。

Result: 证明了诱导的马尔可夫链是不可约的、非周期的和几何遍历的，从而在PD空间上存在唯一的平稳概率分布。

Conclusion: 该框架能够保留主导拓扑结构，同时降低图复杂度，在合成和神经影像学PDs上进行了实验验证。
...
### [209] [Optimal Rates for Learning with Monotone Adversaries](https://arxiv.org/abs/2608.06337)
*Anay Mehrotra*
...
### [210] [Scalable estimation of VARMA models](https://arxiv.org/abs/2608.06340)
*Daniel Paulin,Victor Elvira*

Main category: stat.ML

TL;DR: 提出了一种新的VARMA模型估计框架，提高了估计效率，使其在高维数据中的应用成为可能。


<details>
  <summary>Details</summary>
Motivation: 传统的VARMA模型在处理高维数据时计算成本高，难以实际应用。

Method: 提出了一种新的估计框架，包括部分自相关重参数化、高斯先验和固定大小的充分统计量损失函数。

Result: 该框架可以有效地估计VARMA模型，且迭代成本与序列长度无关。

Conclusion: 该研究为VARMA模型的高效估计提供了一种新的方法，使其在高维数据中的应用成为可能。
...
### [211] [MS-MLB: An Open Machine Learning Benchmark for Blood-Based MS Classification](https://arxiv.org/abs/2608.05196)
*Adam Simson,Ankush Dutta,Quang Bui*

Main category: cs.LG

TL;DR: MS-MLB是一个基于机器学习的MS诊断方法，使用公开数据集和多种算法进行评估，旨在为研究比较提供工具。


<details>
  <summary>Details</summary>
Motivation: 为了建立一种基于机器学习的多发性硬化症（MS）诊断方法，并提供一个可重复的公开基准，用于从全血RNA表达数据中进行MS研究分类。

Method: 使用公开的GSE17048队列，将其转换为MS与正常对照组的任务，并在共享、泄漏控制的管道下评估多个算法。评估包括嵌套交叉验证、未触动的分层保留集、自举置信区间、ROC和精确度召回分析、校准测量和探索性MS研究评分。

Result: 梯度提升在保留集上的MS研究评分排名第一，评分为93.83，AUC-ROC为0.989，敏感性为0.950，特异性为0.778，F1分数为0.927，Brier分数为0.050。

Conclusion: MS-MLB是一个针对MS与正常对照组分类的公开基准，旨在为研究比较提供工具，但尚未在临床中得到验证。
...
### [212] [When Do Corrective Features Help? An Agent for Corrective Feature Discovery on Black-Box Forecasters](https://arxiv.org/abs/2608.05207)
...
### [213] [PPDL: LLM-Based Flows as Probabilistic Programs](https://arxiv.org/abs/2608.05234)
*Louis Mandel,Guillaume Baudart,Mandana Vaziri,Martin Hirzel*

Main category: cs.LG

TL;DR: 本文提出了一种概率性编程语言，用于提高LLM流程的可靠性和用户信任。


<details>
  <summary>Details</summary>
Motivation: 构建利用大型语言模型（LLMs）的可靠应用程序仍然是一个重大挑战。LLMs在多样化任务中提供令人印象深刻的性能，但其输出往往缺乏准确性，并且没有提供明确的置信度度量。这种不确定性在多次调用LLMs和其他工具的流程中加剧，使得开发人员和最终用户难以信任结果。

Method: 本文介绍了一种用于编程LLM流程的概率性编程语言。它使开发人员能够量化并在整个应用程序流程中传播不确定性，并且可以在不添加任何代码的情况下实验不同的推理缩放技术。
...
### [214] [Decoupling Perception from Description: Computation-Grounded Representation Alignment between Multivariate Time Series and Language](https://arxiv.org/abs/2608.05238)
*Xinran Feng,Yi Xie,Chao Zhang,Ruikun Li,Wanyun Ling,Ziyue Li,Chenxi Liu*

Main category: cs.LG

TL;DR: CGTime通过解耦感知和描述，在多变量时间序列语言模型中实现了更好的性能和可扩展性。


<details>
  <summary>Details</summary>
Motivation: 训练多模态模型以将时间序列与语言对齐遇到自监督陷阱。

Method: 通过解耦感知与描述来解决这个问题。使用确定性代码计算一组来自真实、开源的多变量时间序列的统计数据；LLM用言语表达这些预先计算的事实。

Result: 产生了CGTime，一个基于计算的4B参数时间序列语言模型。CGTime在多变量理解任务上优于更大的通用模型，在保留的基准测试中达到最佳的多变量事实分数（0.283 vs. GPT-4o-mini的0.173和GPT-5.4-nano的0.203），并且这个差距在Holm校正的配对显著性测试中对每个基线都持续存在。它还在生成的字幕中更准确地陈述可验证的数值事实，并涵盖了更广泛的统计特性。

Conclusion: CGTime模型通过将感知与描述解耦，在多变量理解任务上优于现有方法，并具有更好的可扩展性和准确性。
...
TL;DR: DiSR通过分离感知和推理，在空间推理任务中表现出色，同时提高了可解释性和效率。


<details>
  <summary>Details</summary>
Motivation: 通过明确地将3D感知与推理分离，探索空间推理的替代范式，而不是通过大规模训练同时获取隐式的3D感知和推理。关键观察是，现代感知模型在估计连续3D几何形状方面表现出色，而大型语言模型（LLMs）在组合和符号推理方面特别有效。

Method: 提出了一种名为Disentangled Spatial Reasoner（DiSR）的框架，该框架使用现成的专家感知模型将物理世界重新构建为结构化的3D证据，并使用LoRA微调LLM以仅在此显式几何证据上进行推理。

Result: 在流行的空间推理基准测试中，DiSR在无需大规模3D VQA训练或复杂的工具使用策略的情况下，取得了有竞争力的性能。除了强大的性能外，DiSR还提供了改进的可解释性、模块化和计算效率，证明了将感知和推理的显式分离是一种可扩展且有效的替代范式，用于空间智能的端到端建模。

Conclusion: 显式分离感知和推理是一种可扩展且有效的替代范式，用于空间智能的端到端建模。
...
### [216] [Marginal Matching Does Not License Factorized Sampling: Auditing Conditional Style Leakage in Factorized Generative Models](https://arxiv.org/abs/2608.05243)
...
### [217] [FlowAdam: Implicit Regularization via Geometry-Aware Soft Momentum Injection](https://arxiv.org/abs/2604.06652)
*Devender Singh,Tarun Sheel*

Main category: cs.LG

TL;DR: FlowAdam结合ODE和软动量注入，在耦合参数优化中提高了Adam的性能。


<details>
  <summary>Details</summary>
Motivation: 传统自适应矩方法在处理密集或旋转参数耦合时存在问题，如矩阵分解、张量分解和图神经网络。

Method: 引入FlowAdam，通过常微分方程（ODE）进行连续梯度流集成。Soft Momentum Injection混合ODE速度与Adam动量，以避免训练崩溃。

Result: 在耦合优化基准测试中，FlowAdam的ODE集成提供隐式正则化，降低持有-out误差10-22%，并在CIFAR-10等条件下优于Adam。MovieLens-100K表明，收益来自耦合参数交互，而非偏差估计。

Conclusion: FlowAdam和Soft Momentum Injection提高了自适应矩方法在耦合参数优化中的性能。
...
TL;DR: 提出PRISM和PRISM-Eval，显著提高多模态指令遵循的准确率。


<details>
  <summary>Details</summary>
Motivation: 研究现实世界多模态指令中，大多数训练数据仍然将指令遵循简化为回答一个自包含的问题的问题。

Method: 通过评分标准理解，将模型视为遵循评分标准的执行者，而不是与评分标准相比的生成器。提出PRISM数据合成框架，包括角色-任务对、前缀引导的规则集、质量过滤的评分标准和结构化验证跟踪。引入PRISM-Eval，其宽松和严格指标使用与固定标签的确定性匹配，因此不需要推理时间判断模型。

Result: PRISM将Qwen3-VL-4B在PRISM-Eval上的严格准确率从9.5%提升到30.1%，同时保持一般基准上的平均性能，并且这些增益转移到四个额外的开源MLLMs，表明结构化评分标准监督是通往多规则、优先级感知的多模态指令遵循的可扩展途径。

Conclusion: 结构化评分标准监督是通往多规则、优先级感知的多模态指令遵循的可扩展途径。
...
### [219] [The Loss Does Not See the Basis, but Adam Does](https://arxiv.org/abs/2608.05136)
*Devender Singh*
...
### [220] [Beyond Full-Model Rollback: AuroSFT for Adapter-State Multi-Task Fine-Tuning](https://arxiv.org/abs/2608.05250)
*Yue Han,Ziniu Liu*

Main category: cs.LG

TL;DR: AuroSFT通过改进多任务监督微调，提高了模型的泛化能力和准确率。


<details>
  <summary>Details</summary>
Motivation: 针对多任务监督微调（SFT）中不同任务在不同时间达到最佳泛化的问题，提出了一种参数高效的框架AuroSFT。

Method: AuroSFT通过将过拟合感知的多任务SFT的携带状态重铸为紧凑、可合并的适配器状态，冻结预训练骨干，仅训练注入的适配器，在任务峰值回滚适配器检查点，并继续在剩余的活跃混合中训练。

Result: AuroSFT在保留骨干比较协议下，平均准确率达到61.36%，高于msft参考行的59.85%，在所有五个骨干上均获得更高的准确率。

Conclusion: AuroSFT是一种有效的多任务监督微调框架，可以提高模型的泛化能力和准确率。
...
### [221] [Beyond Rotations: AuroOFT for Expressive Quantized Orthogonal Fine-Tuning](https://arxiv.org/abs/2608.05253)
*Yue Han,Dianlin Wang*

Main category: cs.LG

TL;DR: AuroOFT通过结合qoft和低秩非线性残差，显著提升了低比特语言模型的性能和参数效率。


<details>
  <summary>Details</summary>
Motivation: 为了提高低比特语言模型的参数效率，通过在冻结量化权重之前学习结构化激活旋转。

Method: 引入AuroOFT，将qoft作为稳定的量化兼容分支，并为每个调整后的线性层附加一个零起始的门控低秩非线性残差。AuroOFT将激活映射到RMS归一化的紧凑潜在空间，并使用具有有界或令牌相关门控的自适应非线性基。零初始化的上投影使AuroOFT在初始化时在功能上与qoft相同，而正交性仍然是分支级别的稳定性属性，而不是组合非线性层的属性。

Result: 在匹配的数据、优化、解码和解析协议下，AuroOFT在1.5B/3B Qwen2.5设置上比匹配的qoft提高了1.30-2.70%的Macro-6，比QLoRA高6.52-10.62%，相对于QLoRA在代表性规模上节省了32.3-44.7%的可训练参数。

Conclusion: AuroOFT通过引入非线性残差和优化正交性，提高了低比特语言模型的性能和参数效率。
...
### [222] [Hybrid Probabilistic Zonotopes for Identifiable and Refinable Predictive Uncertainty](https://arxiv.org/abs/2608.05454)
*Zhen Zhang,Amr Alanwar*

Main category: cs.LG

TL;DR: HProbZ通过将不确定性来源表示为带域生成器，在处理复杂预测任务时表现出优越性。


<details>
  <summary>Details</summary>
Motivation: 在神经网络中，概率预测头通常输出高斯混合或单个共形区域，但这两种方法都不能区分真实预测任务中经常存在的不同不确定性来源：离散模式的选择、所选模式内的有界系统性漂移和不可减少的随机噪声。

Method: 引入了混合概率带域（HProbZ），该输出头将这三个来源表示为二进制、有界和随机的带域生成器，并通过卷积接受封闭形式的似然函数。

Result: 实验分析表明，与同一编码器混合基线相比，HProbZ设计在代表性预测基准上表现出有效性，同时提供了混合或凸共形预测器不能共同提供的结构属性。

Conclusion: HProbZ在处理真实预测任务中的不确定性方面表现出有效性，并且与现有的混合或凸共形预测器相比具有独特的表征能力。
...
TL;DR: 开发了一个新的投资组合管理系统，旨在为零售投资者提供个性化的税务意识强的投资组合管理。


<details>
  <summary>Details</summary>
Motivation: 零售投资者缺乏像机构客户那样个性化的、税务意识强的投资组合管理。现有的机器人顾问使用静态的基于规则的分配，而机构级系统需要账户最低限额和技术堆栈，这对个人投资者来说不可用。

Method: 提出一个完全构建的、集成测试的应用程序，该应用程序使用FastAPI后端和仪表板，让用户用普通语言描述投资目标，将目标路由到六个投资指令之一，并从三阶段强化学习系统生成实时、经纪商集成的投资组合推荐。该系统包括自监督跨资产编码器、专家混合（MoE）分配策略和轻量级LoRA适配器，后者可以个性化推荐，而无需重新训练共享模型。

Result: 该系统功能完整，已通过实时经纪商API（Alpaca，模拟交易模式）进行端到端集成测试，包括多用户身份验证、信任优先的预应用确认流程、每日电子邮件摘要和可审计的行动完整性链。

Conclusion: 该系统是一个新兴的、预部署的应用程序，具有完整的部署路径，并使用14天的向前回溯测试（包括自举置信区间）作为初步的预部署验证。
...
TL;DR: 本研究通过比较15种机器学习模型，发现TabPFN在预测野火后泥石流方面表现最佳，并通过合成数据增强提高了预测性能。


<details>
  <summary>Details</summary>
Motivation: 预测野火后泥石流对于减轻社区、基础设施和资源在近期燃烧区域强降雨期间的危险至关重要。然而，由于特征空间中泥石流和非泥石流事件的重叠、模型可解释性的需求以及训练数据的有限，识别可靠的机器学习模型变得复杂。

Method: 通过系统评估机器学习模型在预测性能、特征重要性和合成数据增强方面的表现来解决这些挑战。使用美国西部野火后泥石流事件的流域尺度观测数据，比较了包括Tabular Prior-Data Fitted Network (TabPFN)在内的15个模型。使用重复分层交叉验证来评估TabPFN的未增强性能，并使用SHapley Additive exPlanations (SHAP)来识别驱动预测的特征。进一步评估使用TabPFN生成的样本进行合成数据增强，以解决泥石流观测数据的稀缺性。

Result: TabPFN在未增强的情况下取得了最高的性能，其次是最佳树模型。合成增强提高了除CNN之外所有模型的性能，其中深度学习模型的平均威胁分数提高了+0.041。通过结合严格的模型基准测试、可解释的特征分析和合成数据增强，这项工作提供了一个全面的框架，以提高野火后泥石流的预测。

Conclusion: 通过结合严格的模型基准测试、可解释的特征分析和合成数据增强，这项工作提供了一个全面的框架，以提高野火后泥石流的预测。
...
### [225] [The Tamed Subgradient Unadjusted Langevin Algorithm beyond Convexity](https://arxiv.org/abs/2608.06283)
*Iosif Lytras,Nikolaos Makras,Sotirios Sabanis*

Main category: cs.LG

TL;DR: 提出SG-TULA算法，有效处理复杂目标分布采样，在LLM预训练中表现优异。


<details>
  <summary>Details</summary>
Motivation: 研究目标分布的采样问题，其中势函数同时具有非光滑性、超线性梯度增长和非凸性。

Method: 提出了一种名为SG-TULA的算法，该算法是Langevin扩散的离散化版本，直接作用于子梯度，不依赖于计算量大的平滑过程。

Result: 在Wasserstein-2距离下推导了非渐近收敛界限，并跟踪了所有常数。对于LLM的预训练势函数，验证了假设并提供了过风险估计。

Conclusion: SG-TULA算法在处理具有复杂特性的目标分布采样问题时表现出色，并在LLM预训练中取得了有竞争力的结果。
...
### [227] [Surv-IPTB: An Attention-Based Model for Estimating Individual Probability of Treatment Benefit with Survival Data](https://arxiv.org/abs/2608.06288)
*Lev V. Utkin,Stanislav K. Kogan,Andrei V. Konstantinov*

Main category: cs.LG

TL;DR: 提出了一种新的基于注意力的框架，用于估计生存分析中的个体治疗获益概率，并在实验中表现出色。


<details>
  <summary>Details</summary>
Motivation: 在生存分析背景下，提出一种新的基于注意力的框架来估计个体治疗获益概率（IPTB）。

Method: 将IPTB估计重新定义为二元分类问题，利用治疗组和对照组之间的成对病人比较。通过不精确概率表示，对右截尾观察值进行原则性处理。

Result: 在合成数据集上进行了广泛的实验，证明了该方法在各种截尾率和治疗效应强度下均保持稳健的性能。模型在非线性场景中优于基线模型。

Conclusion: 提出的基于注意力的框架是一种可扩展且具有统计原理的个性化治疗获益评估解决方案。
...
TL;DR: QEvict通过可恢复淘汰策略，提高大语言模型的信息保留和推理能力


<details>
  <summary>Details</summary>
Motivation: 解决自回归大语言模型推理中KV缓存内存占用问题

Method: 提出QEvict，一种三层的KV缓存管理方案，结合可恢复的淘汰策略

Result: 在长文本理解、检索和推理基准测试中，QEvict优于代表性淘汰和量化基线，减少遗漏的注意力并提高信息保留

Conclusion: QEvict通过可恢复淘汰策略，在保持高置信度窗口完整精度的同时，保留了更广泛的历史上下文，提高了信息保留和推理能力
...
### [229] [DG-FedReuse: Proxy-Gradient-Gated Cached-Update Reuse with Matched Sparse Uplink Accounting](https://arxiv.org/abs/2608.05358)
*Rahil Aftab,Vineet Kumar Rakesh,Soumya Mazumdar,Tapas Samanta*

Main category: cs.LG

TL;DR: DG-FedReuse在联邦学习中减少了数据传输，但准确性提升有限。


<details>
  <summary>Details</summary>
Motivation: 研究联邦学习中的本地优化和模型更新传输问题

Method: 提出DG-FedReuse机制，允许选定的客户端在随机头梯度差异代理低于阈值时贡献衰减的缓存更新

Result: 在90轮预算下，DG-FedReuse在图像分类数据集上实现了83.36-85.42%的更新数据字段上传节省，比匹配的Top-K FedAvg节省了76.88%的数据；种子对齐的准确率差异在-5.29到-0.14个百分点之间；在测试控制的检查点下，保留的最佳观察到的测试准确率仅作为探索性存档证据，与匹配的FedAvg相比，变化范围在-2.38到+0.45个百分点之间；对称的密集模型下传敏感性将节省的头等降至41.68-42.71%，与Top-K FedAvg相比的增量收益为3.24-4.27个百分点；研究在实现的模拟器中描述了提出的重用规则；它没有建立无偏泛化、端到端带宽减少、运行时间或能源节省、更快收敛或优于现有过时更新和懒惰聚合方法的优势。

Conclusion: DG-FedReuse机制在联邦学习中有效减少了数据传输，但仍有改进空间。
...
### [230] [Quantum-Structured World Models (QSWMs) for Predictive Latent Dynamics](https://arxiv.org/abs/2608.05371)
*Hailong Jiang,Emran Hossain,Feng Yu,Jianfeng Zhu,Guilin Zhang,Wulan Guo*

Main category: cs.LG

TL;DR: 提出量子结构化世界模型，显示预测潜力，但长期预测有限。


<details>
  <summary>Details</summary>
Motivation: 为了研究量子理论中启发的数学结构是否对世界建模提供有用的归纳偏好，提出了量子结构化世界模型(QSWMs)。

Method: 通过量子启发的框架实现，包含结构化的潜在状态、潜在转换算子和测量启发解码图。在基本的细胞自动机上，使用复值和密度矩阵-like QSWM变体与强经典基线进行比较评估。

Result: 结果表明，复值QSWMs在局部预测方面具有潜在的优势，同时也揭示了在长期滚动和密度矩阵变体方面的局限性。

Conclusion: 量子结构化世界模型(QSWMs)在预测世界建模方面显示出潜力，但仍存在局限性。
...
### [231] [Spectral Distillation: From Nonlinear Dynamics to Linear State-Space Models](https://arxiv.org/abs/2608.05416)
*Liane Galanti,Devan Shah,Shlomo Fortgang,Elad Hazan*

Main category: cs.LG

TL;DR: 一种端到端可证明的方法，从非线性动态系统中提取最优LDS表示。


<details>
  <summary>Details</summary>
Motivation: 寻求一种不直接解决非凸系统识别问题的方法来学习非线性动力系统。

Method: 观察谱滤波（OSF）和谱到LDS蒸馏。

Result: 在实验中，该管道生成的LDS预测器比直接训练的基线更好或匹配。

Conclusion: 提出了一个从非线性动态系统中提取最优LDS表示的端到端可证明方法。
...
### [232] [Perturbation Sensitivity at Convergence: A Simple Signal for Identifying Spuriously Correlated Samples](https://arxiv.org/abs/2608.05419)
*Nilesh Kumar*

Main category: cs.LG

TL;DR: 提出了一种新的方法来识别和纠正模型中的虚假相关性问题，显著提高了模型在子群体上的性能。


<details>
  <summary>Details</summary>
Motivation: 在数据中存在虚假相关性的数据上通过经验风险最小化训练的模型在平均准确率上很高，但在相关性不存在的子群体上失败。

Method: 提出了一种在模型收敛后使用损失不再区分两个群体的信号来识别受影响的样本的方法。

Result: 使用检测到的样本重新平衡训练将Waterbirds的最坏群体准确率从57.3%提高到80.8%，与使用地面实况群体标签的85.8%相比。

Conclusion: 该方法无需早期停止时代，无需任何阶段的群体注释，并且每次训练样本需要两个正向传递。
...
### [233] [IFlowNets: Extending Generative Samplers to Learn Strategies in Incomplete Information Games](https://arxiv.org/abs/2608.05422)
*Conor M. Artman,Nicholas Di,Scott Perkins*

Main category: cs.LG

TL;DR: IFlowNets在非完全信息博弈中实现了高效的生成采样，性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 许多算法将强化学习（RL）与反事实后悔（CFR）方法相结合，以利用计算速度和性能之间的权衡。然而，在非完全信息博弈的游戏理论应用中，对生成采样框架的研究较少。

Method: 将生成流网络框架Adversarial Flow Networks（AFlowNets）扩展到非完全信息博弈，称为信息流网络（IFNs）。证明了在完全信息博弈中为生成流网络建立的先前约束对于获得有效的密度（对应于玩家策略）和有效的训练目标是不适用的。提出了IFlowNets的推广，解决了这个问题，并严格推广了AFlowNets。

Result: 在三个标准博弈环境中的初步结果表明，IFlowNets在性能和速度上与结果采样蒙特卡洛反事实后悔（OSMCCFR）和基于标准RL的方法相当或更好。

Conclusion: IFlowNets在非完全信息博弈中提供了一种有效的生成采样框架，在性能和速度上优于或相当于现有方法。
...
### [235] [EvoHarness-RL: Learning Self-Evolving Runtime Harness for Long-Horizon LLM Agents](https://arxiv.org/abs/2608.05446)
...
TL;DR: EvoHarness-RL通过学习策略解决长期LLM智能体的挑战，提高了成功率，并揭示了辅助退火和辅助进化的关键动态。


<details>
  <summary>Details</summary>
Motivation: 为了解决长期LLM智能体在维护状态、跟踪进度、调用工具、验证结果和跨交互重用经验时遇到的挑战，我们研究了智能体学习使用策略的问题。

Method: 我们引入了EvoHarness-RL，它将信念、进度和经验（BPE）作为策略面对的辅助状态。通过监督辅助策略微调，基础智能体学习辅助动作空间以及如何构建有用的外部状态，同时成本感知的GRPO探索协调策略，在长期交互中选择性读取、更新和整合该状态。

Result: 在ALFWorld上使用Qwen3-8B LLM实例化后，EvoHarness-RL达到了96.9%的成功率，并揭示了两个关键动态：辅助退火和辅助进化。

Conclusion: 长期智能体从可训练的策略中受益，这些策略用于构建和协调外部辅助工作空间，而不仅仅是添加更强大的工具或更大的内存。
...
TL;DR: 提出TDOF和MZAttn，提高特定类型任务上的注意力机制表现。


<details>
  <summary>Details</summary>
Motivation: 研究多头注意力中输入相关性和输入独立性之间的不对称性对排列不变集目标的影响。

Method: 引入目标算子的变换自由度（TDOF）作为复杂度度量，并展示深度分离分析，提出矩阵区域图注意力（MZAttn）。

Result: 实验表明，该架构优势在高度依赖输入集的稀疏组合目标上出现，在参数匹配的标准注意力已有竞争力的聚合统计目标上优势较小。

Conclusion: 引入TDOF和MZAttn能够提高注意力机制在特定类型任务上的表现。
...
TL;DR: KV-Skill是一种有效的任务知识获取和部署方法。


<details>
  <summary>Details</summary>
Motivation: 为了解决任务知识存储和共享的问题，提出了一种名为KV-Skill的设计空间。

Method: KV-Skill通过外部分解算子实现，语言模型通过轻量级接口读取。支持将文本技能转换为操作符，并训练共享接口；或直接从任务结果中学习紧凑的潜在操作符。

Result: 在多个基准和模型上测试，KV-Skill在转换文本为技能方面表现出色，准确率显著提高。在Qwen3.5-4B LiveMath上，注册达到77.2的准确率。KV-Skill在多个设置中优于其他方法。

Conclusion: KV-Skill能够有效地从文本或经验中获取任务知识，并将其压缩为外部操作符，独立于主干部署。
...
TL;DR: LC-GRPO通过Langevin校正提高了流模型GRPO的样本生成准确性，增强了强化学习效果。


<details>
  <summary>Details</summary>
Motivation: 为了解决流模型中样本生成和在线强化学习之间的不匹配问题。

Method: 提出了一种名为LC-GRPO的流模型GRPO框架，结合了Langevin校正。

Result: 实验结果表明，LC-GRPO在文本到图像和文本到视频任务中，在奖励优化、生成质量和随机训练回滚与确定性测试时间ODE推理之间的差距方面都有所提高。

Conclusion: LC-GRPO在流模型GRPO中提供了更准确的样本生成，提高了强化学习的效果。
...
TL;DR: GAUGE是一种用于不完整多模态分类的轻量级反事实门控框架，提高了预测的可靠性。


<details>
  <summary>Details</summary>
Motivation: 为了解决多模态分类中模态不完整的问题，现有的方法在粗粒度模态级别操作，无法保留可靠成分同时抑制误导成分，从而影响预测可靠性。

Method: 提出了一种名为GAUGE的轻量级反事实门控框架，用于不完整的多模态分类。GAUGE首先使用冻结的填充器填充缺失的模态，并将观察到的和恢复的输入统一编码为细粒度证据单元。GAUGE通过预测感知的泰勒证据得分来评估替换每个单元的参考表示的反事实效应，所有这些都在单次前向-后向传递中获得。这些分数被映射到连续的门控，然后转换为单元级证据调节的加性注意力-logit偏差，而不改变骨干架构。

Result: 在六个基准测试中，GAUGE在各种不完整输入设置下优于强大的基线。此外，泰勒余项理论分析描述了一阶近似相对于精确反事实效应的误差，将GAUGE确立为在模态不完整性下进行细粒度证据控制的原理性和可扩展框架。

Conclusion: GAUGE是一种有效的轻量级框架，用于解决多模态分类中的模态不完整问题，提高了预测的可靠性。
...
TL;DR: PMOT 是一种用于 $p$-成本最优传输的新框架，表现出色，竞争力强。


<details>
  <summary>Details</summary>
Motivation: 为了解决一般 $p$-成本最优传输问题，提出了一种新的潜在匹配最优传输（PMOT）框架。

Method: 使用标量势参数化 CNF 速度场，采用自诱导匹配损失进行训练，允许灵活的终端分布匹配。

Result: PMOT 在合成基准上学习到 $p$-特定的地图，在多维表格数据上作为似然密度模型具有竞争力，在基于 MMD 的颜色变换实验中展示了灵活的样本终端匹配。

Conclusion: PMOT 在解决一般 $p$-成本最优传输问题方面表现出色，并在多个实验中证明了其有效性和竞争力。
...
TL;DR: 重绘击败了重新采样，一致性胜过基于证据的基线。


<details>
  <summary>Details</summary>
Motivation: 在保证正确性的前提下，利用模型在经过扰动的输入上的共识作为无标签的可靠性信号和自我训练目标。

Method: 构建RENDER-EQ集，对三个公开的VLM进行测量，包括自然图像的基线和模型的交叉重绘共识的微调。

Result: 在准确性和可靠性方面，重绘击败了重新采样，一致性在两个模型中胜过基于证据的基线，第三个模型则与之持平。

Conclusion: 重绘击败了重新采样，一致性在三个模型中胜过基于证据的基线，但在调整模型的交叉重绘共识后，精度在每个五次复制运行中都会下降。
...
TL;DR: SEAM是一种新的机器学习模型验证框架，可以确保全局解释一致性，并有助于提高模型的可靠性和可解释性


<details>
  <summary>Details</summary>
Motivation: 验证机器学习模型在局部范围内的解释是否可以组合成全局可接受的解释

Method: 提出了一种名为SEAM的生成器无关框架，该框架可以计算局部到全局的一致性问题，并使用有限解释层实例SEAM-$Ω$来表示每个区域的结构化解释，比较相邻解释的重叠，并将不一致转化为通道解决的障碍

Result: SEAM可以检测到即使局部预测准确时也不兼容的解释，并将失败归因于特定的通道和重叠，为现有的求解器和学习模型添加了全局解释一致性审计，以测试其局部解释是否形成一个连贯的科学解释

Conclusion: SEAM是一种有效的工具，可以确保机器学习模型的全局解释一致性，并有助于提高模型的可靠性和可解释性
...
TL;DR: SAP是一种有效的自监督学习方法，可以用于有限的标记数据下的故障诊断。


<details>
  <summary>Details</summary>
Motivation: 在工业环境中，机器故障诊断需要大量的标记数据，而标记数据是一种稀缺资源。

Method: 提出了一种名为SAP的自监督学习方法，通过利用频谱混叠在未标记的振动数据上预训练模型。

Result: SAP在CWRU数据集上显示出稳定且高度区分性的表示。在线性探测设置中，SAP只需少量标记数据就能快速实现非常高的分类性能，并且方差低。

Conclusion: SAP结合线性探测对于有限标记数据的故障诊断比全监督训练更有效和可靠。
...
TL;DR: CircuitSteer是一种新的方法，通过多层电路引导，有效地控制LLMs的行为。
...
Result: CircuitSteer在多个任务中表现出色，包括毒性、情感强度、拍马屁和拒绝，并且是唯一能够一致产生流畅性保持干预的方法。

Conclusion: 多层电路引导，通过在所选特征之间强制执行几何对齐，比静态单点干预提供了更严格、更有效的行为控制。
...
TL;DR: 提出CvLoss，解决多变量时间序列预测中的交叉变量和滞后依赖问题，提高预测模型性能。


<details>
  <summary>Details</summary>
Motivation: 多变量时间序列预测面临独特挑战，因为未来变量通常在共享系统动力学下共同演化。

Method: 提出了一种名为CvLoss的插件结构正则化器，以约束交叉变量图上的预测残差。

Result: CvLoss在提高竞争性预测模型方面表现一致，优于代表性的学习目标，并且与各种预测骨干兼容。

Conclusion: CvLoss在存在交叉变量和滞后依赖的情况下，可以解决直接预测（DF）目标不匹配的问题，并显著提高预测模型的性能。
...
TL;DR: Developed a new framework for workpiece localization in hot forging with high accuracy and low latency.


<details>
  <summary>Details</summary>
Motivation: Continuous workpiece localization is essential for traceability and process coordination in hot forging, but direct tracking is unreliable due to extreme temperatures, surface degradation, and irregular routing.

Method: An equipment-centric framework that infers workpiece locations from handling equipment observed by multiple static 2D cameras. The framework estimates floorplan-space 3D equipment coordinates and recognizes grasp and release activities. Event-driven finite state machines validate these activities as discrete handling events and continuously update workpiece states and locations. A keypoint-guided attention mechanism integrated into a 3D convolutional neural network improves activity recognition by focusing on functionally relevant equipment regions.

Result: Achieved 100% event detection accuracy within a 33-second tolerance window, a mean localization error of 317.8 mm, and a mean system latency of 21 seconds. The framework connects vision-based perception with interpretable event-driven reasoning and supports visualization of workpiece transfers and quantitative analysis of equipment operations.

Conclusion: The framework effectively localizes workpieces in hot forging processes, ensuring traceability and process coordination, with high accuracy and low latency.
...
TL;DR: 本研究提出了一种基于形状约束的预测建模方法，用于纳米医药系统的制造工艺开发，显著提高了预测准确性并减少了实验工作量。


<details>
  <summary>Details</summary>
Motivation: 由于纳米颗粒性质（如尺寸和多分散性指数）对工艺参数的微小变化敏感，纳米治疗药物的开发通常涉及广泛的实验优化。缺乏预测性数学框架使得迭代实验筛选成为必要，从而增加了成本和开发时间。

Method: 本研究引入并验证了一种基于形状约束的预测建模方法，旨在提高在不同工艺条件下对纳米颗粒特性的估计。使用控制微流控方法，在变化的脂质浓度、流速和水分-有机混合比下系统地制备了脂质体和脂质纳米颗粒。基于实验数据和专家知识的形状约束模型随后被用于验证药物应用，并使用最少的经验数据。

Result: 结果表明，形状约束建模有助于准确预测纳米颗粒的大小和分散性，减少了大量实验工作流程的需求。该框架支持合理和高效的纳米医药系统制造工艺开发。

Conclusion: 基于形状约束的建模方法为纳米医药系统的制造工艺开发提供了一种合理和高效的框架，减少了实验工作量并提高了预测准确性。
...
TL;DR: 论文提出了一个用于激光粉末床熔化的神经符号闭环架构，证明了其提高产品质量的可行性。


<details>
  <summary>Details</summary>
Motivation: 提高激光粉末床熔化工艺的质量。

Method: 提出了一种几何条件神经符号闭环架构，结合了符号推理和统计学习来设定约束感知预测控制器的目标。

Result: 在IN625的Eagar-Tsai替代方案中对NIST AM-Bench基准进行了校准，该架构消除了由几何盲控制器产生的缺陷，在双重评分下将缺陷保持在零，并且在发生故意设备不匹配时优雅地降级，并通过编辑本体数据而不是代码重新定向到新的合金和约束。

Conclusion: 该论文提出了一个几何条件神经符号闭环架构，用于激光粉末床熔化，证明了其在提高产品质量方面的可行性。下一步是进行实验校准比率。
...
TL;DR: 提出了一种基于岭回归最小二乘优化的一次性机器反学习算法，GROM，在移除敏感知识方面表现出色。


<details>
  <summary>Details</summary>
Motivation: 为了安全地从大型语言模型（LLMs）中移除特定、敏感的知识，机器反学习变得至关重要。

Method: 提出了一种新颖的一次性反学习方法，直接使用精确的解析解，将反学习过程构造成一个岭回归正则化最小二乘优化问题，并推导出针对目标权重矩阵的封闭形式增量更新。

Result: GROM方法在TOFU-5%、TOFU-10%、MUSE-Books、MUSE-News和WMDP数据集上实现了最先进的遗忘-效用权衡，显著降低了计算开销，而没有牺牲整体模型性能。

Conclusion: GROM是一种快速、有效的机器反学习算法，能够从大型语言模型中移除特定知识，同时抵抗低比特量化攻击。
...
TL;DR: 先验难度预测对于代理成功至关重要，本研究展示了如何通过识别预测信号和暴露环境缺陷来提高其准确性。


<details>
  <summary>Details</summary>
Motivation: 任务难度影响代理的成功概率，在没有执行成本高昂的模拟之前，仅通过任务描述来估计它，这对于环境设计者和训练课程构建至关重要。

Method: 研究跨多个领域的17个代理基准的先验难度预测，包括编码、数学、机器学习、网络导航、函数调用等。

Result: AUC可以掩盖差的难度估计，识别token级别的熵作为有用的预测信号，并展示如何通过预期和观察到的难度之间的残差暴露隐藏的环境缺陷，如污染和不可行性。

Conclusion: 先验难度预测对于环境设计和训练课程构建至关重要，并且可以通过识别有用的预测信号和暴露环境缺陷来提高其准确性。
...
TL;DR: 提出了一种基于学习排序的框架来选择高效的张量网络收缩计划，结果表明这种方法能够有效减少搜索成本，但性能仍受后端依赖。


<details>
  <summary>Details</summary>
Motivation: 为了降低量子算法模拟的成本，研究人员提出了基于张量网络收缩的方法。然而，选择合适的收缩计划对于提高效率至关重要。

Method: 提出了一种学习排序框架，通过从GPU测量中训练梯度提升排序器来选择高效的收缩计划。每个计划通过其成对收缩的序列直接导出的结构特征来表示。

Result: 学习排序器通常能够识别出更好的计划，其中列表模型提供了最强的整体决策质量。

Conclusion: 学习排序是一种实用的方法来减少收缩计划搜索，但性能仍然部分依赖于后端。
...
TL;DR: 这项研究评估了后处理分析方法在聚类结果模式检测中的有效性，发现现有工具存在局限性，需要新的模式检测方法。


<details>
  <summary>Details</summary>
Motivation: 在数据分析中，解释聚类结果仍然是一个基本挑战，特别是在需要从高维数据中提取有意义模式的领域，如医疗保健。虽然存在许多可解释性技术，但它们主要是为了评估特征重要性或提供局部实例级解释，而不是识别聚类中存在的结构化模式。

Method: 本研究对常用的后处理分析方法进行了比较评估，以检测聚类结果中的模式。为了进行可控评估，我们引入了一套合成数据集，其中系统地注入了预定义的模式。评估了三种广泛使用的技术：随机森林代理模型与置换特征重要性、LIME（局部可解释模型无关解释）和主成分分析。

Result: 结果表明，尽管每种方法都可以成功地恢复相关特征，但没有一种方法可以一致地检测到所有注入的模式类型。这些发现突出了现有可解释性工具与模式级聚类解释要求之间的关键差距，促使开发专门的模式检测方法。

Conclusion: 这项工作强调了现有可解释性工具在模式级聚类解释方面的不足，并提出了开发新的模式检测方法的必要性。
...
TL;DR: FALM-PINN is an effective and efficient optimization framework for solving high-frequency and nonlinear PDEs with superior performance.
...
Method: Proposes the Fourier-enhanced alternating Levenberg--Marquardt PINN (FALM-PINN), an optimization framework that decouples representation learning from coefficient fitting. The upper-level problem learns a Fourier-enhanced basis that enriches the latent space with high-frequency components, while the lower-level problem resolves the coupling by fitting the projection coefficients on this basis, solving a nonlinear least-squares problem with the Levenberg--Marquardt algorithm.

Result: The framework applies to general nonlinear and coupled PDE systems, and reduces to a single-step convex optimization problem for linear PDEs. Proves global convergence of the alternating training scheme in both cases. Numerical examples show that FALM-PINN achieves relative $L^2$ errors up to two orders of magnitude lower than state-of-the-art baselines.

Conclusion: FALM-PINN is effective in solving high-frequency and nonlinear PDEs and achieves superior performance compared to existing methods.
...
TL;DR: 研究简单变换在异构文本嵌入模型间转换表示的可行性，发现兼容性受多种因素影响，并得出异构嵌入空间并非普遍相关的结论。


<details>
  <summary>Details</summary>
Motivation: 探究简单变换是否能在异构文本嵌入模型之间转换表示，以及理解独立训练的模型如何组织语义信息，以实现AI到AI的潜在通信。

Method: 使用线性映射等轻量级翻译器，在现实文本设置中测试文献中关于潜在普遍性的假设，评估九种嵌入模型的兼容性。

Result: 简单翻译器在部分兼容对中恢复了有意义的共享结构，支持迁移，但在其他情况下失败。兼容性取决于架构、训练目标、池化和数据分布。结果显示，异构嵌入空间并非像一些文献中经常暗示的那样通过简单映射相互关联。

Conclusion: 异构嵌入空间并非通过简单映射普遍相关。
...
TL;DR: 提出 OG-SPR，通过结合多步潜在自我预测和下一观察预测，有效提高了视觉 RL 的样本效率。


<details>
  <summary>Details</summary>
Motivation: 提高强化学习（RL）中从像素中高效学习策略的能力。

Method: 提出观察 grounded 自预测表示（OG-SPR），这是一种无模型的视觉 RL 算法，用于连续控制，学习在潜在空间中具有时间预测性并且在观察水平动态中具有基础的表示。OG-SPR 包含两个核心辅助目标：多步潜在自我预测和下一观察预测。

Result: 在 DeepMind 控制套件中的 28 个视觉控制任务上进行了实验，OG-SPR 比最先进的自预测和观察预测 RL 方法提高了总体性能，尤其是在狗和类人等具有挑战性的领域取得了显著的收益。

Conclusion: OG-SPR 是一种有效的视觉 RL 算法，能够提高策略学习效率，在具有挑战性的视觉控制任务中表现优于现有方法。
...
TL;DR: 提出了一种统一的不确定性定义和基准，通过半合成数据集直接计算不确定性，揭示了方法之间的差异，为不确定性管理提供了新的思路。


<details>
  <summary>Details</summary>
Motivation: 在安全敏感应用中，可靠的预测不确定性评估至关重要，需要区分认知不确定性和随机不确定性。然而，这些不确定性类型在文献中没有统一的定义，使得评估方法是否产生准确的预测不确定性变得困难。此外，由于缺乏真实的认知不确定性，评估进一步复杂化。现有的基准大多依赖于代理任务，如异常检测，这些任务不能提供完整的真实不确定性目标，对不确定性估计的结构和质量提供有限的见解。

Method: 提出了一种统一的不确定性定义，即点预测风险，这是在给定数据的可能真实函数分布下，预测器的期望损失。这种观点结合了函数的贝叶斯不确定性以及与估计器相关的后验均值偏差，捕获了如误设和优化误差等影响。这种公式构成了一个基于理论的基准，它使用具有真实协变量和已知生成过程的半合成数据集，直接计算先验认知和随机不确定性。通过避免代理评估，该基准能够进行细粒度的不确定性估计分析。

Result: 实验结果表明，准确的预测并不保证可靠的不确定性分离。基准揭示了方法之间的实际有用差异，确定了与先验不确定性目标有意义的对齐的方法，同时揭示了敏感性与数据集和建模选择。

Conclusion: 该研究提出了一种统一的不确定性定义和基准，有助于评估不确定性估计的准确性和可靠性，为安全敏感应用中的不确定性管理提供了新的思路。
...
TL;DR: TFM在表格预测问题上的表现并不理想，因为它们违反了边际化一致性和因子分解一致性要求。
...
Method: 提出两个要求：边际化一致性因子分解一致性。

Result: 所有评估的TFM都违反了这两个要求。

Conclusion: TFM在表格预测问题上的表现并不理想。
...
TL;DR: ProDVI利用大型语言模型初始化RL智能体，提高样本效率。


<details>
  <summary>Details</summary>
Motivation: Deep Reinforcement Learning (RL)在样本效率方面存在局限性，原因是RL智能体通常是从零开始初始化，迫使它们通过在线交互获取任务相关知识。

Method: 提出ProDVI框架，利用大型语言模型中编码的常识和领域知识，无需依赖这些资源初始化RL智能体。具体来说，ProDVI提示代码生成语言模型生成可执行Python函数，这些函数编码了对环境动力学的粗略假设。然后使用这些函数生成合成转换。基于这些转换，我们在演员-评论家框架中构建辅助动力学预测目标，预先训练价值网络的状态-动作编码器。所学习的表示在在线RL开始之前提供动力学感知归纳偏差。

Result: 在OpenAI Gym和DeepMind Control Suite任务上的实验表明，ProDVI可以有效地提高无模型RL算法的样本效率。

Conclusion: ProDVI是一种基于大型语言模型初始化RL智能体的框架，可以有效地提高RL算法的样本效率。
...
TL;DR: BioKD框架通过利用生理信号作为特权信息，提高了基于视频的情绪识别性能。


<details>
  <summary>Details</summary>
Motivation: 解决基于视频的情绪识别在模糊或社会性掩饰的行为线索下的局限性，以及生理信号的较差部署性。

Method: 提出了一种名为BioKD的可靠性感知生理到视频知识蒸馏框架，利用生理信号作为训练过程中的特权信息，引导基于视频的学生模型学习深度情感表示。

Result: 在DEAP和AMIGOS数据集上，BioKD在情绪识别任务中优于基线模型，并有效减轻了过自信的教师错误。

Conclusion: BioKD框架在情绪识别任务中表现出色，提高了性能并减少了额外的计算开销。
...
TL;DR: AutoThread通过自适应调整线程数提高RL推理效率，平均加速提高18.4%，吞吐量提升显著。


<details>
  <summary>Details</summary>
Motivation: 在模拟循环决策系统中，强化学习（RL）推理经常受到模拟器端执行开销的限制，工作负载动态且对运行时线程配置敏感。现有的多线程策略在执行前后难以匹配线程资源，导致资源争用、调度开销和吞吐量降低。

Method: 通过经验分析，确定任务执行时间与调度时间的比率是确定最佳线程数的决定性因素。提出AutoThread，一种混合自适应线程调整方法，用于缓解RL推理中的模拟瓶颈。AutoThread采用物理信息神经网络算子（PINO）作为线程数预测器，并结合有限源M/M/1排队模型来约束和引导预测，实现动态工作负载下的快速准确估计。它还进行负载感知的在线微调，以补偿预测误差并优化资源分配。

Result: 实验表明，AutoThread比静态策略平均加速提高18.4%，达到XGBoost和Reinforcer平均吞吐量的1.7倍和1.8倍，与最先进的方法相比，执行时间缩短高达83.8%。代码和数据集在https://github.com/suchenjm/AutoThread公开。

Conclusion: AutoThread是一种有效的自适应线程调整方法，可以显著提高RL推理的效率和吞吐量。
...
TL;DR: CurvPrompt通过几何自适应提示在少样本链接预测任务中显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法在单一、固定的嵌入空间内操作，未解决几何未适应的问题。

Method: 提出了一种名为CurvPrompt的拓扑路由几何提示框架，用于动态图。该框架维护一个曲率多样的黎曼专家库，每个专家都与一个可学习的提示配对。

Result: CurvPrompt在四个基准数据集上进行了广泛的实验，结果表明其在少样本链接预测任务中显著提升性能，同时在对节点分类任务中表现出强大的、一致的性能。

Conclusion: CurvPrompt在少样本链接预测任务中显著提升性能，同时在对节点分类任务中表现出强大的、一致的性能，验证了几何自适应提示的必要性。
...
TL;DR: 研究比较了不同奖励模型在北极航运中的应用，发现非线性奖励模型更有效，但船舶特定潜在上下文对性能有负面影响。


<details>
  <summary>Details</summary>
Motivation: 为了帮助北极航运适应快速变化的海冰条件，人工智能辅助导航可以发挥作用，但可靠部署需要可解释且对环境变化具有鲁棒性的奖励模型。

Method: 通过逆强化学习（IRL）框架从船舶轨迹中恢复此类奖励，并使用元IRL方法引入潜在上下文变量以捕获行为异质性。

Result: 非线性奖励模型比线性基线提高了50.9%的保留可能性，而添加船舶特定潜在上下文降低了16.5%的性能。行为分析、上下文探测和预注册的特征隐藏消融表明，船舶级别的变化主要是由可观察的路线和环境条件解释的，而不是隐藏的船舶特定因素。此外，预测精度、路线保真度和奖励迁移产生了不同的模型排名，表明没有单一指标足以评估学习到的奖励。

Conclusion: 这些发现促使我们测试在添加每艘船的潜在上下文之前，观察到的路线、环境和船舶特征是否已经解释了行为变化。这支持在安全关键领域更可靠的人工智能部署。
...
TL;DR: Kastor方法通过结合先进模型和训练目标，显著提高了物理模拟的预测精度和效率。


<details>
  <summary>Details</summary>
Motivation: 加速物理模拟，用快速、可微的代理模型替换计算量大的传统偏微分方程求解器。

Method: 提出Kastor方法，结合大步长因果自回归模型和非因果时间超分辨率网络，以及Mean预测正则化（MPR）训练目标。

Result: 在预测精度、光谱一致性和计算效率方面优于竞争方法，平均预测精度提高42.9%。

Conclusion: Kastor方法在物理模拟中表现出色，提高了预测精度和物理保真度。
...
TL;DR: SPT增强了Transformer在医学时间序列任务上的性能，特别是在数据受限的情况下。


<details>
  <summary>Details</summary>
Motivation: 探究SPT在多模态、多变量和简单单变量医学时间序列上的效果。

Method: 在多个医学时间序列任务上评估了基于Transformer的模型，通过SPT和不同掩码策略进行训练。

Result: SPT在各种配置下均提高了分类准确率，并且对深度模型效果更佳。

Conclusion: SPT显著提高了基于Transformer的模型的性能，尤其是在数据受限的医学时间序列任务中。
...
TL;DR: 提出了一种适应时间变化到达率的轻量级调度算法，提高了LLMs的吞吐量并保持了低延迟。


<details>
  <summary>Details</summary>
Motivation: 为了提高LLMs在信息检索和问题解决中的性能，研究人员致力于改进调度算法以提升吞吐量并保持低延迟。然而，现有方法通常假设泊松请求到达，与实际交通的突发性和动态性不符。

Method: 提出了一种对WAIT算法的轻量级扩展，该算法能够适应时间变化的到达率，无需先验交通知识。基于观察到的到达时间进行在线估计请求强度。使用基于马尔可夫调制泊松过程（MMPP）的合成工作负载和多种请求类型进行仿真评估。

Result: 在评估的低到达率场景中，该方法比Sarathi-Serve、ORCA和vLLM实现了更高的吞吐量，同时保持了可比的延迟。

Conclusion: 该方法有效提高了LLMs在信息检索和问题解决中的性能，为调度算法的改进提供了新的思路。
...
TL;DR: SkillTFM通过技能演化实现无监督TFM适应，提高了预测性能。


<details>
  <summary>Details</summary>
Motivation: 为了解决分布偏移、异构特征语义和特定任务模式等问题，将TFM适应从参数更新转移到技能的门控演化。

Method: 提出SkillTFM系统，通过技能库和门控技能演化实现无监督的TFM适应。

Result: 在模拟边界设置和现实世界的电价预测中，SkillTFM提高了AUC值，并展示了其有效性和普适性。

Conclusion: SkillTFM是一种有效的无监督TFM适应方法，可以显著提高预测性能。
...
TL;DR: SAGA通过使用依赖句法分析器监督代替人工标注，在低资源语言中显著提高了语法质量。


<details>
  <summary>Details</summary>
Motivation: Preference optimisation has proven effective for improving large language models but typically relies on costly human preference annotations. Extending these methods to morphologically rich, low-resource languages remains challenging because such annotations are scarce.

Method: We present SAGA (Score-weighted Adaptive Generation Alignment), a parser-guided preference optimisation framework that replaces human labels with dependency-parser supervision. SAGA converts parser judgements into preference pairs for delta-DPO, combines parser quality with lexical diversity in a composite reward, filters low-information pairs using a reward-gap criterion, and monitors reward hacking to maintain reliable supervision.

Result: Across Danish, Icelandic, and Norwegian Bokmål using GPT-SW3-1.3B, SAGA consistently improves grammatical quality without requiring human preference labels. Danish parse success increases from 69.0% to 93.8%, Icelandic achieves a +4.5 percentage-point improvement on an independent Stanza evaluation (three-run mean +3.3 percentage points) while native speakers prefer SAGA outputs in 80% of pairwise comparisons, and Norwegian Bokmål improves by +28 percentage points.

Conclusion: These results demonstrate that parser-derived supervision is a practical alternative to human preference annotation for grammatical alignment in low-resource languages where high-quality dependency parsers are available.
...
TL;DR: 本文探讨了持续学习的演变，从参数为中心转向系统级自适应，并分析了其方法和挑战。


<details>
  <summary>Details</summary>
Motivation: 为了超越传统的模型自适应观点，扩展持续学习的范围。

Method: 通过三个维度（何时、如何、何地）来分析持续学习的演变。

Result: 系统地调查了代表性方法，追踪了持续学习的持续转变，并讨论了由此范式转变产生的主要挑战、更广泛的影响和未来方向。

Conclusion: 持续学习正从以参数为中心的学习向系统级自适应转变。
...
TL;DR: GEM-3是一种新型天气预报模型，通过多时间步长推理解决了预测精度和可用性之间的权衡。


<details>
  <summary>Details</summary>
Motivation: 现有的机器学习天气预报模型依赖于预定的固定自回归时间步长，这导致了预测精度和可用性之间的权衡。

Method: 提出了一种名为GEM-3的概率全球天气模型，该模型通过显式的多时间步长推理解决了这一权衡。

Result: GEM-3可以在推理时配置模型时间步长，以平衡预测精度和可用性。混合时间步长训练提高了模型的稳定性。

Conclusion: GEM-3是一种实用的天气预报系统，结合了中等范围的概率技能、稳定的长期预测、高效的训练和推理以及决策相关的诊断。
...
TL;DR: 本文对后训练自适应进行了综合分析，提出了一个六维分类法，并确定了开放挑战。


<details>
  <summary>Details</summary>
Motivation: 为了综合后训练自适应的文献，并介绍一个按机制、目标、数据需求、持久性、结构范围和模型类型组织的六维分类法。

Method: 对后训练自适应的文献进行综合分析，并建立分类法。

Result: 提出了一个六维分类法，区分了常见的混淆术语，并展示了适应策略如何从传统机器学习演变到深度学习、基础模型、大型语言模型和多模态大型语言模型。

Conclusion: 该调查确定了后训练自适应中的开放挑战，包括评估、可重复性、持久推理时间自适应、遗忘、多模态自适应和治理意识后的工作流程。
...
TL;DR: MetaboLLM和MetaboLLM-GIN在代谢组学预测和分类任务中表现出色，为领域专用语言模型的应用提供了新的可能性。


<details>
  <summary>Details</summary>
Motivation: 代谢组学知识分散在异构资源中，难以转化为预测表示。

Method: 开发了MetaboLLM，这是一种通过持续预训练、监督微调和结构检索进行适配的代谢组学专用大型语言模型，以及MetaboLLM-GIN，它使用图同构网络将生成的生化描述转换为代谢物图，以进行患者级别的预测。

Result: MetaboLLM在代谢组学知识、关系和描述任务上优于相应的基线和医学适配模型，并将其转移到外部公共基准。MetaboLLM-GIN在预测冠脉旁路移植术后应激性高血糖（AUC为0.8616）和绝经后激素治疗方案分类（AUC为0.8123）方面取得了最高的AUC，优于传统模型、替代图结构和从未适配或非检索LLM配置生成的图。模型解释在两个应用中都产生了有生物学意义的发现。

Conclusion: 领域专用语言模型可以将异构生化知识组织成可预测和可解释的代谢物图表示。
...
TL;DR: RxnCLF通过自监督对比学习实现反应表示学习，在产量预测任务中表现出色，具有广泛的应用前景。


<details>
  <summary>Details</summary>
Motivation: 反应产物预测具有挑战性，因为标记数据稀缺，反应空间组合庞大且稀疏，限制了现有反应表示的泛化。

Method: 提出反应对比学习基础（RxnCLF），这是一种自监督对比框架，用于反应表示学习。RxnCLF基于压缩反应图（CRG），将反应物和产物信息统一到一个图中，使模型能够学习明确的丰富转换结构，而不是孤立的图。

Result: 在170万Pistachio反应上进行预训练，RxnCLF学习到一个紧凑且连续的潜在空间，该空间捕捉了反应中心特征和更广泛的侧链上下文，使其具有转换意识和化学可解释性。在多个产量预测基准上进行微调，包括Buchwald-Hartwig、Pd催化的BH偶联、专有的HTE C-N偶联和酰胺形成数据集，RxnCLF在R2和整体性能方面均优于基于图和序列的基线。

Conclusion: RxnCLF作为基于CRG的可扩展反应基础模型，有潜力推广到更广泛的反应空间，并支持各种下游反应信息学任务，包括区域选择性预测、对映选择性预测和反应条件优化。
...
TL;DR: U-OPSD：通过模型自己的生成实现自我蒸馏，优于基模型和监督方法。


<details>
  <summary>Details</summary>
Motivation: 现有的方法仍然高度依赖外部监督，包括真实信号、环境反馈或来自更大模型的指导，因此无法实现真正的'自我'蒸馏。

Method: 我们提出无监督的OPSD方法，它首先通过多数投票构建一个伪解决方案，然后条件化教师分布并蒸馏到模型的错误补全中。

Result: U-OPSD在多个基准、基模和训练设置上始终优于基模型，并可与带有真实标签的监督方法相媲美。在AIME24、AIME25、HMMT25、MATH500和AMC23上，U-OPSD在4B和8B规模上分别比基模型提高了8.5%和10.7%。

Conclusion: U-OPSD是一种有效的方法，可以增强大型语言模型的能力。
...
TL;DR: 提出RRC方法，解决生成式奖励模型在强化学习中的应用限制，显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有的强化学习算法采用的标量评分范式与生成式奖励模型的比较性质之间存在不匹配，导致生成式奖励模型在强化学习中的应用受限。

Method: 提出了一种基于排名的奖励构造（RRC）方法，该方法从相对偏好排名中推导奖励，以提供更有效的强化学习学习信号。RRC包括两种互补策略：自我竞争排名和锚点引导排名。

Result: 在开放式聊天和推理基准测试中，RRC显著提高了生成式奖励模型的强化学习训练效果，与现有的奖励构造方法相比，实现了持续的改进。

Conclusion: RRC方法能够有效地解决生成式奖励模型在强化学习中的应用问题，并取得了显著的性能提升。
...
### [287] [Agentic Nesting: A New Methodology for Existing Enterprise Application Integration and Services](https://arxiv.org/abs/2608.05159)
*Xi Wang,Kun Li,Xianyao Ling,Gang Yin,Liang Zhang,Jiang Wu,Wenbo Lei,Jun Xu,Annie Wang,Fu Zhang,Weizhe Wang*

Main category: cs.AI

TL;DR: 本文提出了一种名为“Agentic Nesting”的框架，用于解决企业应用集成问题，并通过代理协作实现跨应用交互和流程编排。


<details>
  <summary>Details</summary>
Motivation: 企业运营依赖于多个异构业务系统和信息应用，导致数据孤岛和流程碎片化。

Method: 提出了一种名为“Agentic Nesting”的多代理协作框架，将现有企业应用封装为自主AI代理，并在分层嵌套结构中组织。

Result: 通过这种方法，可以实现自然语言交互和自主操作，并通过中央协调器协调多个代理进行任务分解和动态调度，同时提供统一的对话接口以实现跨应用查询和流程编排。

Conclusion: 本文提出了“应用即代理”的集成范式和“对话即集成”的交互理念，并探讨了该方法在异构系统协调和大规模数据应用场景中的通用化潜力。
...
### [288] [The Ignition Index: Measuring Global Workspace Dynamics in Language Models](https://arxiv.org/abs/2608.05160)
*Saman Rahbar*
...
### [289] [Woodpecker Distillation: Weak Models Diagnose Reasoning Bugs in Strong Models](https://arxiv.org/abs/2608.05168)
*Dayu Wang,Jiaye Yang,Weikang Li,Jiahui Liang,Yang Li,Deguo Xia,Jizhou Huang*

Main category: cs.AI

TL;DR: Woodpecker Distillation通过对比局部干预提高强模型推理能力，实验证明效果优于直接模仿基准。


<details>
  <summary>Details</summary>
Motivation: 分析大语言模型在推理任务中失败的本地化推理错误，并提出解决方案。

Method: 提出Woodpecker Distillation训练框架，通过对比弱模型的成功与失败干预来改善强模型的性能。

Result: 在数学推理基准测试中，Woodpecker Distillation能够提高强模型的性能并优于直接模仿基准。

Conclusion: Woodpecker Distillation通过对比局部干预改善强模型推理能力，有效解决大语言模型在推理任务中的失败问题。
...
### [290] [From Continuous Predictors to Clinical Thresholds: Early Evidence on Performance Trade-offs of Guideline-Based Categorisation for Ischaemic Stroke Outcome Prediction](https://arxiv.org/abs/2608.05203)
...
### [291] [SkillTrace: Multi-Trace Provenance Auditing for LLM-Agent Skill Reuse](https://arxiv.org/abs/2608.05204)
*Jialuo Chen,Minghe Wang,Lingqi Jiang,Jianan Ma,Xinhao Deng,Xiaohu Du,Ruixiao Lin,Yunhao Feng,Linkang Du,Jingyi Wang*

Main category: cs.AI

TL;DR: SKILLTRACE是一种用于LLM-agent技能再用的多跟踪溯源审计框架，能够提高检测的准确性和全面性。


<details>
  <summary>Details</summary>
Motivation: 随着技能成为市场艺术品，对其再用的审计不再像普通代码克隆检测那样简单。现有的检测器针对单一模态的源代码或整个包的相似性，但技能再用的证据分布在作者文本、实现片段和操作结构中。因此，它们可能会错过只保留技能一部分的再用。

Method: SKILLTRACE是一个用于LLM-agent技能再用的多跟踪溯源审计框架。它提取了三个溯源跟踪：表达式、实现和操作。操作跟踪以技能操作图（SOG）的形式表示，捕获激活、过程和资源流结构。LLM仅在摄入时协助操作跟踪提取；在审计时间，SKILLTRACE确定性地比较缓存的跟踪，将每个跟踪与相同功能的严格负数校准，并报告哪个跟踪支持再用决策。

Result: 在SKILLTRACE-BENCH上，SKILLTRACE实现了AUROC 0.938和F1 0.898。36,446个技能的野审计进一步表明，跟踪归因的证据在仓库级别的基线之上暴露了可操作的再审查队列。

Conclusion: SKILLTRACE为LLM-agent技能再用的审计提供了一种有效的方法，能够提高检测的准确性和全面性。
...
### [292] [Abstract Event Causal Rules: Induction and Application](https://arxiv.org/abs/2608.05205)
*Ziwei Zheng,Peiqiong Chen,Bang Wang*

Main category: cs.AI

TL;DR: AECR提高了事件因果推理的泛化能力，提升了事件预测性能。


<details>
  <summary>Details</summary>
Motivation: 现有因果事件对在低频长尾和未见事件组合上存在泛化不足的问题。

Method: 提出了一种名为AECR的新关系级因果抽象范式，通过多智能体CACI系统和相似性约束聚类从噪声原始因果数据中提取可靠的AECRs，并构建了两个完整的AECR知识库。同时，提出了AR-GCAE，将AECRs注入到因果图事件预测任务中。

Result: 实验结果表明，应用AECRs显著提高了事件因果推理的泛化能力，并在事件预测上取得了显著的性能提升，特别是在罕见和未见事件样本上。

Conclusion: AECR能够有效解决现有因果事件对泛化不足的问题，并在事件预测任务中取得显著效果。
...
### [293] [Otter: A Time-Aware, History-Conditioned Human Chess AI](https://arxiv.org/abs/2608.05206)
*Tarun Kumar S*

Main category: cs.AI

TL;DR: Otter：基于时间感知的序列模型，预测象棋走棋选择，准确率高。


<details>
  <summary>Details</summary>
Motivation: 预测人类棋手走棋选择，提高人工智能在象棋领域的预测能力。

Method: 使用时间感知的序列过程模型，结合历史走棋编码和时间控制模块。

Result: 在6.1亿个位置上训练，达到55.23% top-1和90.95% top-5的走棋预测准确率。

Conclusion: 时间感知的序列模型在象棋预测方面优于仅位置的方法，且模型规模更小。
...
### [294] [SearchAuditor: Auditing and Attributing Failures in Long-Horizon Search Agents](https://arxiv.org/abs/2608.05212)
...
### [295] [PD-GS: Phoneme-Driven 3DGS for Audio-Driven Talking Heads](https://arxiv.org/abs/2608.05218)
*Ao Fu,Yi Zhou*

Main category: cs.AI

TL;DR: PD-GS通过融合音素信息和音频上下文，提高了唇部动作的准确性，减少了“漏嘴”现象，并生成了更逼真的头像。


<details>
  <summary>Details</summary>
Motivation: 3D Gaussian Splatting (3DGS)虽然可以实现快速、逼真的头像渲染，但准确的唇部动作仍然难以实现，常常出现“漏嘴”现象。

Method: 提出了一种名为Phoneme-Driven Gaussian Splatting (PD-GS)的方法，通过自动语音识别和强制对齐管道获取时间对齐的音素标记，并将其与3DGS头像结合。核心组件是Linguistic Fusion Module (LFM)，它通过学习门控机制自适应地融合连续音频上下文和离散音素嵌入。PD-GS仅使用单目视频进行训练，使用图像重建和唇部关键点监督。

Result: 在HDTF数据集上，PD-GS在唇部几何方面优于比较的基线（LMD 2.66），并在具有挑战性的音素序列中减少了闭合违规，产生了更符合语言学的神经头像。

Conclusion: PD-GS方法能够有效提高唇部动作的准确性，减少“漏嘴”现象，生成更逼真的头像。
...
### [296] [When Privileged Guidance Misaligns: State-Matched Routing and Contextualized Self-Distillation for Multi-Turn Agents](https://arxiv.org/abs/2608.05219)
*Junzhuo Liu,Weiwei Li,Jun Ling,Peng Wang*

Main category: cs.AI

TL;DR: SMRC-SD通过状态匹配路由和上下文化自蒸馏，提高了多轮代理的任务成功率。


<details>
  <summary>Details</summary>
Motivation: 提供与学生的当前执行状态兼容的特权参考指导，以解决状态-参考不匹配问题。

Method: 引入状态匹配路由和上下文化自蒸馏（SMRC-SD），仅在匹配状态下应用蒸馏，并构建状态条件教师上下文。

Result: SMRC-SD在ALFWorld和WebShop上提高了任务成功率，分别从0.746提高到0.865，从0.574提高到0.693。
...
### [297] [Innovation-Residual Auditing of Autonomous Analysis Agents: Localization, Detection Limits, Error Control, and Identifiability](https://arxiv.org/abs/2608.05490)
*Ahmed Hassoon,Mark Dredze*

Main category: cs.AI

TL;DR: 提出了一种新的审计方法，可以有效地定位自动化代理在数据分析中的错误。


<details>
  <summary>Details</summary>
Motivation: 分析自动化代理在数据分析中的错误定位问题。

Method: 提出了一种新的审计方法，从已知正确的分析中学习，并标记偏离预测的操作。

Result: 量化了错误传播的范围，并确定了当错误逐渐累积时选择比较长度的方法。提出了控制单一审计分析中错误标记比例的步骤，并量化了当模型不完美或分析选择依赖于其内容时保证的减弱程度。最后，确定了对任何此类审计可以报告的误差极限。

Conclusion: 提出的方法可以有效地定位自动化代理在数据分析中的错误，并量化了误差传播和保证减弱的程度。
...
TL;DR: Project2Task通过将项目分解为明确任务，提高了研究任务组合的质量和准确性。


<details>
  <summary>Details</summary>
Motivation: 研究项目需要通过多个界限明确的任务来推进，而现有系统往往将项目视为一个庞大的任务，导致任务模糊或重叠，或依赖人工协调。

Method: Project2Task，一个基于图的自主研究项目级规划层，将候选贡献表示为创新原子，并组织成有向世系图。使用轻量级伯努利块模型目标选择水平、垂直和混合投资组合分解。然后生成具有明确贡献所有权的界限任务，修复重叠和缺失执行字段，并发出具有依赖关系的任务合同，指定目标、输入、预期成果、评估要求、边界约束、依赖关系和执行顺序。

Result: 在十个项目摘要的基准测试中，基于手稿的投资组合评估给Project2Task的平均质量得分为7.15，而简报基线为4.58，仅主题设置得分为5.31。将合同与AutoResearchClaw集成后，下游任务的平均准确性从0.536提高到0.759。

Conclusion: Project2Task通过显式项目到任务的规划，产生了连贯、非冗余和可执行的研究任务组合，具有价值。
...
TL;DR: TriQua和TriQuaScore在LLM事实性评估中表现出色。


<details>
  <summary>Details</summary>
Motivation: 解决LLM事实性评估中原子事实与广泛陈述之间的矛盾，提高评估精度。

Method: 提出TriQua框架和TriQuaScore方法，对事实进行灵活建模和量化评估。

Result: TriQua和TriQuaScore在事实性评估方面取得显著成果，与人类标注的事实性评分高度一致。

Conclusion: TriQua框架和TriQuaScore方法在事实性评估方面表现出色，TriQua在分解质量上表现良好，在基于证据的事实验证中优于现有框架。
...
TL;DR: DSV系统将梦境描述转换为视觉图像，提高了梦境传达的效率。


<details>
  <summary>Details</summary>
Motivation: 梦境难以传达情感，需要一种新的方法来可视化梦境。

Method: 开发了一个名为Dream Scene Visualiser (DSV)的系统，该系统将书面梦境描述转换为四个画面的时间序列图像，以可视化梦境。

Result: 通过在DreamBank中的50个梦境描述上进行评估，DSV在质量、保真度和连贯性方面表现出色。

Conclusion: DSV系统有效地将梦境描述转换为视觉图像，为梦境的传达提供了一种新的方法。
...
TL;DR: Search2Skill是一种基于搜索的新框架，能够提高技能学习效率，并在多个领域和模型规模上表现出色。


<details>
  <summary>Details</summary>
Motivation: 为了解决现有自进化技能方法受限于模型已知知识的局限性，以及难以从代理中提取领域惯例和标准程序的问题。

Method: 提出了一种名为Search2Skill的新框架，该框架通过基于标准强化学习方案优化搜索、检索和技能生成过程。

Result: 在八个专家领域和三个基准测试上的实验表明，Search2Skill在流式和离线评估协议下均优于搜索增强和轨迹基础的方法。

Conclusion: Search2Skill通过技能抽象获得收益，且技能可以在不同模型规模之间迁移。
...
TL;DR: 提出LUNAR基准，评估LLM跨领域行为个性化，发现证据选择、跨领域整合和隐私控制是关键挑战。


<details>
  <summary>Details</summary>
Motivation: 为了解决现有个性化LLM基准主要依赖文本角色或孤立的行为信号，无法评估跨领域行为个性化的问题。

Method: 提出LUNAR基准，通过多阶段粗到细的合成流程，基于现实世界的行为模式进行评估。
...
TL;DR: WorldClaw：一种生成大规模3D世界的全新框架，实现全局连贯性、丰富内容和编辑重用。


<details>
  <summary>Details</summary>
Motivation: 生成大规模、自由可探索的3D世界从开放文本仍然具有挑战性，因为系统必须共同维持全局空间连贯性、丰富的局部内容和适合下游编辑和重用的明确资产。

Method: WorldClaw是一个完全自主的、从粗到细的框架，用于开放世界的3D场景生成。规划代理将文本提示转换为区域、地形、资产、材料和空间关系的结构化规范。然后，WorldClaw从语义布局、可重用资产、生成或程序材料以及区域感知的高度场构建全局一致的地形基础。对于需要细节的区域，它生成地形条件下的组合，重建可编辑的纹理网格，并恢复它们在地形上的放置；基于渲染的代理进一步细化地形、对象、外观和接触。

Result: 在多样化的开放世界提示中，WorldClaw生成了具有一致空间组织、视觉上引人注目的局部内容以及可编辑实例级资产的大型场景，同时保持一致的全局地形结构。

Conclusion: WorldClaw在生成大规模、自由可探索的3D世界方面表现出色，能够有效地处理全局空间连贯性、局部内容和编辑重用问题。
...
TL;DR: 本研究提出了一种优化引擎，有效提高了军事资产分配的效率和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 解决预先承诺姿态问题，提高军事资产分配的效率和鲁棒性。

Method: 情景加权对抗鲁棒姿态优化引擎，使用复合期望值（CEV）优化器和鲁棒CEV扩展，以及贝叶斯对抗者模型。

Result: 在三个实验中，优化引擎提高了姿态效率和适应性，验证了其结构性能。

Conclusion: 提出的优化引擎在三个实验中证明了其有效性和鲁棒性，相较于贪婪算法，提高了效率和适应性。
...
TL;DR: 本研究开发了一个框架，用于评估不同代理架构对显微镜任务性能的影响，发现基准测试对代理的某些方面有用，但不能可靠地预测代理在未见过的任务上的表现。


<details>
  <summary>Details</summary>
Motivation: 研究代理控制物理基础设施的方法，特别是在显微镜控制中的应用。

Method: 开发了一个基准和跟踪日志框架，用于评估不同代理架构、LLM、RAG和上下文参数对显微镜任务性能的影响。

Result: 基准测试对代理的资格、回归测试、诊断和直接比较有用，但不能可靠地预测代理在未见过的显微镜任务上的表现。

Conclusion: 当前异构测试套件不支持任务无关的全局配置模型。
...
TL;DR: 本文探讨了GPT-3.5模型在在线借贷反事实分析中的应用，结果表明LLMs在该领域具有潜力。


<details>
  <summary>Details</summary>
Motivation: 预测假设场景下的潜在结果，为决策提供有价值的见解。

Method: 使用GPT-3.5模型进行反事实分析，重点关注在线借贷环境。

Result: GPT的预测性能优于先进的机器学习算法，反事实ROI生成表现出逻辑一致性和因果推理。

Conclusion: LLMs在在线借贷的反事实分析中具有潜力，并建议在预测和决策的广泛情境中应用LLMs。
...
TL;DR: DoctorAgents is an AI framework for efficient ML pipeline development in clinical settings.


<details>
  <summary>Details</summary>
Motivation: Clinical machine learning (ML) has the potential to support high-stakes medical decision-making, but reliable deployment is often constrained by scarce, heterogeneous, and temporal complexity.

Method: Proposing DoctorAgents, an agentic AI framework that autonomously constructs and optimizes end-to-end ML pipelines through specialized large language model (LLM) agents for generation, validation, and refinement. DoctorAgents backpropagates natural-language feedback through textual gradient descent to perform targeted updates without exhaustive search.

Result: Experiments across diverse clinical tasks show that DoctorAgents consistently outperforms established AutoML baselines while producing more interpretable task-specific representations.

Conclusion: DoctorAgents provides an effective solution for developing ML pipelines for small clinical data, outperforming existing AutoML systems and producing more interpretable results.
...
TL;DR: CANOE框架通过多智能体神经符号框架解决了护理计划协调中的信息整合和透明度问题，提高了临床正确性和安全性。


<details>
  <summary>Details</summary>
Motivation: 为了解决在护理计划协调中整合来自多个专业学科的临床、功能和心理社会信息的需求，以及单体LLM管道在透明和安全方面的局限性。

Method: 提出了一种名为CANOE（可争议的论证专家网络）的多智能体神经符号框架，该框架通过五个模块来解决这些局限性：复杂性评估、自适应团队招募、基于角色的基于场地的定量双极论证框架（A-QBAF）的论证计算、人机交互的争议和护理计划合成。

Result: 在Discharge Me!和MedicalRAG上的评估表明，经过医学微调的模型在临床正确性和安全性方面表现最佳，而CANOE的论证结构提供了忠实且可争议的解释。

Conclusion: CANOE框架在护理计划协调中显示出强大的临床正确性和安全性，同时提供了可争议的论证结构，从而提高了透明度和安全性。
...
TL;DR: PSI是一个有效的指标，可以改善LLM辅导的效果。


<details>
  <summary>Details</summary>
Motivation: 为了解决现有评估方法主要关注答案质量，而忽略了教学适应性，提出了一种新的教学适应性指标（PSI）。

Method: 通过PSI评估LLM生成的辅导响应与学习者准备情况和课程进度的匹配程度，并对表现不佳的案例进行改进。

Result: PSI在评估LLM辅导效果方面有效，可以显著提高弱表现案例的性能。

Conclusion: 学习者意识和课程意识的匹配可能比模型类别本身对有效辅导更重要，并且这种匹配是可衡量和可改进的。
...
TL;DR: 通过混合利益相关者工作坊评估AI警务用例风险，强调种族公平，强调工具效果和普及性。


<details>
  <summary>Details</summary>
Motivation: 探讨英国及全球警务中使用AI工具的日益增多，特别是关注种族偏见这一已知风险。

Method: 混合利益相关者协商工作坊，汇集社区代表、警察和学者评估13个AI警务用例的风险，重点关注种族偏见。

Result: 参与者普遍接受AI采用，仅对三个用例表示反对，特别是再犯风险评估。分析显示，强调种族公平并未缩小讨论范围，而是聚焦于工具的实际效果、真正益处及其普及性。

Conclusion: 将种族偏见视角纳入AI用例的风险效益分析中，从起点就体现了其价值。
...
TL;DR: 提出了一种选择性的翻译框架，用于提高机器人自主系统的行为规范生成和验证的可靠性。


<details>
  <summary>Details</summary>
Motivation: 为了提高机器人自主系统的行为规划、推理和验证，将自然语言指令转换为机器可解释的正式规范至关重要。然而，现有的翻译模型在结果不可靠或无法捕捉用户意图的情况下，仍然为每个输入生成规范，这在安全性关键应用中存在风险。

Method: 提出了一个选择性的翻译框架，该框架不仅生成正式规范，还能确定它们何时可信赖。通过两个互补的黑盒信号评估可靠性：规范反向翻译回自然语言的保真度和在精确语义等价下的重复翻译的分散度。这些信号分别对不同类型的错误失效，并能比单独使用任何一种更锐利地分离错误的翻译。符合性风险控制将此评分定量化为一个决策，即接受规范或拒绝，并在接受执行的错误规范的速率上有无分布的限制。符合性异常检测器在尝试翻译之前对指令嵌入进行筛选，以排除分布外的输入。

Result: 该框架在信号时序逻辑（STL）、线性时序逻辑（LTL）和几何时空逻辑（SpaTiaL）上进行了实验，证明了改进的翻译可靠性、在评估的跨层转换下的鲁棒性以及有效的不确定性感知拒绝。这项工作通过使人工智能系统能够识别生成的规范可能不可靠的情况，为可信的自然语言接口奠定了基础。

Conclusion: 所提出的选择性翻译框架显著提高了机器翻译的可靠性和安全性，为建立可信赖的自然语言接口奠定了坚实的基础。
...
TL;DR: 教学顺序对学习效果至关重要，但找到最佳顺序的成本很高，且存在NP难题。


<details>
  <summary>Details</summary>
Motivation: 学习概念之间的先决条件依赖关系时，何时教学顺序很重要，以及找到最佳顺序的成本是什么。

Method: 将教学顺序作为随机最短路径问题进行研究，其中尝试一个概念会以状态相关的概率成功，失败则使学习者状态保持不变。

Result: 证明随机性可以被消除，问题转化为确定性的最短路径问题，保持最优值和操作。发现最优排序仍然NP难，并且当可实现的转移偏好与先决条件一起保持无环时，任何剩余联合图的拓扑排序都是最优的。

Conclusion: 教学顺序对学习效果有重要影响，但寻找最佳顺序的成本很高。
...
TL;DR: RST是一种高效的递归合成框架，用于构建长期终端代理任务，显著降低成本并提升模型训练效果。


<details>
  <summary>Details</summary>
Motivation: 高质量长期训练数据成本高昂，因为每个任务都需要保持指令、环境、参考解决方案和验证器的相互一致性。人工编写无法扩展，直接使用大型语言模型（LLMs）生成通常会导致这些依赖关系中断。

Method: 提出递归合成终端任务（RST），这是一个递归验证合成框架，用于大规模构建长期终端代理任务。从验证的种子任务开始，RST扩展参考解决方案，重新对齐验证器和指令到新的工作流程，在新的沙盒中验证结果，并将接受的任务作为后续轮次的种子重新使用。

Result: 通过15轮递归，RST生成了37,484个合成的终端代理任务，每个任务大约花费0.05美元。任务难度在轮次中显著增加：中值参考解决方案从67行增长到374行，执行命令的中值数量从40个增长到244个，DeepSeek-V4-Pro在R1时的pass@4从90%下降到R15时的2.5%。为了证明训练效用，我们在合成的任务上收集拒绝采样的Qwen3.5轨迹，并用于监督微调。在Terminal-Bench~2、Terminal-Bench Hard和Long-Horizon Terminal Bench上，对Qwen3.5-27B和Qwen3.5-122B-A10B的微调将性能提高了最多10分，而PPO的代理性能将Qwen3.5-27B在三个基准测试中的表现分别提升到49.44%、32.00%和22.07%，相对于基础模型，分别提高了20.0%、41.2%和21.9%。此外，经过15轮后，递归没有上限：随着难度的不断提高，合成产量和验证率保持稳定，表明该过程可以继续扩展到所报告的规模之外。

Conclusion: RST框架有效地解决了长期终端代理任务的构建问题，通过递归合成和验证，大幅降低了任务生成成本，并提高了模型训练效果。
...
### [318] [EcoAgent-Bench: Evaluating Economic Decision-Making in Budget-Constrained LLM Agents](https://arxiv.org/abs/2608.05519)
...
### [319] [Hyper-ES: Effective Evolution Strategies for LLM Reasoning via Descent Direction Merging](https://arxiv.org/abs/2608.05541)
*Yu Gu,Zhi Zheng,Yunpeng Ba,Xialiang Tong,Mingxuan Yuan,Zhenkun Wang*

Main category: cs.AI

TL;DR: Hyper-ES 是一种有效的优化方法，可以显著提高资源受限的大语言模型在推理任务中的性能。


<details>
  <summary>Details</summary>
Motivation: Evolution Strategy (ES) 是一种很有前途的替代方案，用于在资源受限的大语言模型 (LLM) 推理中进行基于梯度的微调。然而，将 ES 直接应用于十亿参数的 LLM 是非常低效的。在如此高维的参数空间中，大多数随机扰动几乎与有用的更新方向正交，导致优化不稳定。

Method: 我们提出了 Hyper-ES，这是一个基于子空间的 ES 框架，它避免了 ES 在全参数搜索中的弱点，同时利用了它在低维优化中的优势。Hyper-ES 首先进行少量低成本的基于梯度的微调运行，以获得下降方向。然后，Hyper-ES 在这个子空间内应用 CMA-ES 优化层级的 DARE-TIES 合并系数，允许 ES 在有意义的下降方向组合上进行搜索，而不是在任意全模型扰动上进行搜索。

Result: 我们在三个 Qwen2.5-Instruct 和 DeepSeek-R1-Distill 主干上对 Hyper-ES 进行了评估，涵盖了六个数学推理数据集。结果表明，Hyper-ES 比 GRPO-LoRA 高出 1%，同时需要 10% 更少的占用空间的梯度更新。

Conclusion: Hyper-ES 是一种有效的优化方法，可以显著提高资源受限的大语言模型在推理任务中的性能。
...
### [320] [SkillTV-Bench: Benchmarking How Well Judges Perform on Skill-Augmented Agentic Execution](https://arxiv.org/abs/2608.05573)
*Zhi Han,Chenxi Zeng,Liuhaichen Yang,Zihan Guo,Ming Zhou,Yang Li*

Main category: cs.AI

TL;DR: 提出SkillTV-Bench和SkillTV-Evolve，提高技能感知轨迹验证准确率。


<details>
  <summary>Details</summary>
Motivation: 评估技能感知轨迹验证方法，解决现有基准测试中任务时间技能与可检查证据和环境的结合问题。

Method: 提出SkillTV-Bench基准测试和SkillTV-Evolve方法，通过自动化进化循环优化JudgeSkill。

Result: 在SkillTV-Bench上，优化后的技能使同一裁判的准确率提高了14.8个百分点，在离线 rollout-pool 选择中，成功率达到45.5%。

Conclusion: SkillTV-Bench和SkillTV-Evolve方法有效提高了技能感知轨迹验证的准确率。
...
### [321] [DreamGuard: Efficient Runtime Guardrail for LLM Agents via Risk-Aware World Model](https://arxiv.org/abs/2608.05695)
*Wenhao Lin,Chenyu Yu,Xingwei Lin,Sicong Cao,Xiang Chen,Lei Xue,Le Yu,Letian Sha,Chunming Wu*

Main category: cs.AI

TL;DR: 提出DreamGuard，提高LLM代理的安全性并保持低延迟。


<details>
  <summary>Details</summary>
Motivation: LLM代理调用外部工具和与真实世界系统交互时，可能导致不可逆转的后果，现有的运行时保护措施主要评估当前行动的安全性，缺乏对风险随轨迹演变模型的明确表述。

Method: 提出DreamGuard，一个基于风险感知世界模型的主动保护措施。该世界模型维护一个紧凑的循环潜在状态，并预测未来潜在状态。DreamGuard根据这些信息进行干预决策。

Result: 实验结果表明，DreamGuard在四个基准和在线保护评估中优于通用、反应性和主动性保护基线，在评估的保护措施中实现了最佳的安全-效用权衡，并保持每次调用平均25毫秒的端到端延迟。

Conclusion: DreamGuard有效地解决了LLM代理的长期风险问题，提高了安全性，同时保持了低延迟。
...
TL;DR: StepReflect provides an effective and efficient solution for GUI agents' long-horizon execution by enhancing their action reflection ability.


<details>
  <summary>Details</summary>
Motivation: Autonomous mobile GUI agents require accurate action reflection for reliable long-horizon execution.

Method: StepReflect, which formulates per-step GUI reflection as supervised structured prediction conditioned on explicit transition specifications and paired visual evidence. StepReflect is trained through a staged pipeline combining supervised fine-tuning, teacher-student distillation, and preference- and reward-based refinement.

Result: Offline, the resulting 8B model achieves 82.16% transition-level accuracy on AndroidWorld, exceeding zero-shot GPT-5.2 by 11.83 percentage points under the same structured input. Online, across M3A, Agent-SAMA, MAI-UI-8B, and Seed-2.0-Pro, StepReflect achieves higher task success in three of four agent configurations and remains within one successful task of the GPT-5.2 Reflection Agent in the fourth. It also reduces paid API charges relative to GPT-based reflection in all four configurations.

Conclusion: These results establish StepReflect as a practical, locally deployable alternative to repeated frontier-model reflection for long-horizon mobile GUI agents.
...
### [323] [ChainClaw: A Layered Agent Framework for Reliable On-Chain Execution](https://arxiv.org/abs/2608.05790)
*Jiacheng Wei,Zhaoxin Fan,Xin Wen,Yuqin Lan,Dongrun Li,Wenjun Wu,Faguo Wu,Xiao Zhang*

Main category: cs.AI

TL;DR: ChainClaw是一个解决区块链环境中基本差距的区块链原生代理框架。


<details>
  <summary>Details</summary>
Motivation: 通用大型语言模型代理在工具增强任务上表现出色，但在区块链环境中，这些假设会失效。

Method: 提出ChainClaw，一个基于OpenClaw的区块链原生代理框架，通过分层架构解决三个基本差距：反应性、不可逆性和可观察性。

Result: ChainClaw在七个任务上优于代表性基线，在安全和任务完成方面均表现出色。

Conclusion: ChainClaw是一个有效的区块链原生代理框架，可以解决区块链环境中的基本差距。
...
### [324] [Epistemic Trustworthiness in Generative AI: A Normative Framework for Warranted Reliance in High-Stakes Workflows](https://arxiv.org/abs/2608.05602)
*Nimisha Karnatak,Max Van Kleek,Nigel Shadbolt*

Main category: cs.AI

TL;DR: 本研究提出生成式AI系统应具备认知谦逊、认知可访问性和抵抗认知不公正三个条件，以确保其在高风险环境中的可靠性。


<details>
  <summary>Details</summary>
Motivation: 分析生成式AI系统在高风险专业环境中的应用及其输出对用户信念、推理和行为的影响。

Method: 通过哲学理论，建立评估生成式AI系统可靠性的规范性框架。

Result: 提出了三个必要且不可替代的条件：认知谦逊、认知可访问性和抵抗认知不公正。

Conclusion: 强调在设计和评估生成式AI系统时，应关注认知上的可靠性而非仅仅输出正确性。
...
### [325] [Measuring and Detecting Harmful AI Sycophancy](https://arxiv.org/abs/2608.05624)
*Bohan Jiang,Dawei Li,Yasin Silva,Huan Liu*

Main category: cs.AI

TL;DR: This paper investigates the occurrence, detection, and generalization of PSRS in LLMs using CAP.
...
Method: Introducing CAP (Contrastive Anchor Probing), a framework for collecting labeled PSRS data. Applying CAP to 17 open- and closed-source LLMs.

Result: PSRS rates range from 5% to 56% across LLMs, detection is feasible from response text alone, performance drops on unseen models.

Conclusion: Detection of PSRS is feasible, but cross-model generalization is a challenge.
...
### [326] [SkillHEX: Improving Agent Skills via Hypothesis-Driven Autonomous Exploration and Exploitation](https://arxiv.org/abs/2608.05628)
*Yuru Feng,Yaoqi Chen,Beidi Zhao,Qianxi Zhang,Xinjiang Wang,Jianan Lu,Zhirui Wang,Shusen Xu,Zengzhong Li,Qi Chen*

Main category: cs.AI

TL;DR: SkillHEX通过假设验证和证据引导的搜索，有效提高了LLM技能的自我进化能力。


<details>
  <summary>Details</summary>
Motivation: 虽然代理技能使LLM具备可重复的程序知识，但手动维护成本高、可扩展性差，且与实际应用不匹配。

Method: 我们引入了SkillHEX，一个将假设驱动的自我验证与证据引导的树搜索相结合的闭环框架。SkillHEX将可证伪的失败假设转化为可执行的测试，生成密集的诊断证据作为奖励，而无需额外的环境尝试。

Result: 在SkillsBench的87个任务上评估，SkillHEX优于现有的自我进化方法，在五次迭代预算下，使用GPT-5.3-Codex和Claude Opus 4.7的平均通过率分别为55.9%和57.9%。

Conclusion: SkillHEX通过提供一种有效的自我进化框架，解决了现有方法在技能进化过程中的局限性，提高了LLM在现实世界应用中的性能。
...
### [327] [Bayesian Expected Uncertainty Reduction (B-EUR) Model: A Computational Account of What Makes Design Options Worth Trying](https://arxiv.org/abs/2608.05642)
...
### [328] [Refining Over Resampling: Test-Time Self-Correction for LLM Reasoning](https://arxiv.org/abs/2608.05643)
...
TL;DR: 提出了一种新的轨迹预测方法，通过将社交影响分解为规划阶段和反应阶段，提高了预测的稳定性。


<details>
  <summary>Details</summary>
Motivation: 为了更有效地区分社交影响在轨迹规划中的功能角色，提出了一种新的方法。

Method: 提出了INTraJ框架，将社交影响分解为两个阶段：规划阶段和反应阶段。

Result: 在四个标准基准测试中取得了显著的改进，特别是在FDE和长时一致性方面。

Conclusion: 阶段化的社交建模对于稳定的预测至关重要。
...
### [330] [Grounded Well-Condition Anomaly Detection on the Volve Field: Constructed Labels, a Baseline, and a Dual-Head Model](https://arxiv.org/abs/2608.05685)
*Gospel Bassey,Vincent Fakiyesi*

Main category: cs.AI

TL;DR: 本文提出了一种基于真实生产数据的机器状态监测方法，并验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 在真实生产环境中，机器状态监测的公开基准测试很少，因为很少有意制造故障，并且每个事件都是已知的。在生产现场，通常只有传感器历史记录，没有故障日志。这使得异常检测方法必须自己创建标签，并且可能导致未注意到的假设。

Method: 使用Equinor发布的开放Volve现场数据，构建异常标签，并与现场自己的工程文档中所述的物理故障可能性进行核对。使用无监督基线和小型双头模型测试构建的标签是否可学习。

Result: 无监督检测器仍然落在我们的规则标记的区域，表明标签不是任意的。紧凑的监督模型在未见过的井中很好地恢复了事件存在和事件类型，但时间定位仅大致正确。

Conclusion: 本文提出了一种基于真实生产数据的机器状态监测方法，并验证了其有效性。
...
TL;DR: 本研究旨在通过设计人机交互，帮助个体形成对AI系统的准确信念，并促进AI系统的改进和准确性。


<details>
  <summary>Details</summary>
Motivation: 研究如何设计和塑造人机交互以实现三个目标：1）帮助个体形成关于AI系统的准确信念，以便他们在最小成本下改善或确保有利的结果；2）鼓励改进或阻止游戏行为；3）确保AI系统继续实现其预期目标，如最大化准确性。

Method: 结合理论分析、数据驱动建模、人类受试者实验和基于真实世界和半合成数据集的实证评估。

Result: 提出设计符合人类需求、价值观和能力的AI系统的原则和方法，推动以人为本的机器学习。

Conclusion: 本研究从评估个体和AI系统的角度，对人机交互进行考察和研究，以实现上述目标。
...
TL;DR: RA-CAD通过生成-执行-批评-重写循环和反馈驱动优化，显著提高了文本到CAD生成的质量和效率。


<details>
  <summary>Details</summary>
Motivation: 为了弥合反馈利用的差距，将自然语言设计意图转换为可编辑和可执行的参数化计算机辅助设计（CAD）代码，减少手动建模的专业知识和努力。

Method: 提出RA-CAD（CAD的ReAct代理），一个状态感知代理，通过生成-执行-批评-重写循环与CAD环境交互。在每次迭代中，RA-CAD执行当前代码并观察其结果。根据设计指令、当前代码和执行反馈，代理随后生成一个显式的执行后批评作为中间策略操作。

Result: 在CADFusion和Text2CAD上的实验表明，与现有方法和强大的专有语言模型相比，RA-CAD在执行有效性和几何质量方面达到了最先进的水平，证明了所提出的状态感知文本到CAD代理的有效性。

Conclusion: RA-CAD通过状态感知和反馈驱动的优化，显著提高了文本到CAD生成的质量和效率。
...
TL;DR: BlockPython通过双向翻译和四个阶段帮助学习者从基于块的编程过渡到基于文本的编程，并通过收集过程证据和提供支持来促进学习。


<details>
  <summary>Details</summary>
Motivation: 为了帮助学习者从基于块的编程过渡到基于文本的编程，减少理解计算概念和用Python语法表达之间的认知差距。

Method: 设计并实现了BlockPython平台，该平台通过双向翻译和四个阶段（任务分解、基于块的实践、代码挑战和扩展交互）来引导学习者。

Result: 学习者逐步建立程序结构、运行行为和文本代码之间的联系，平台通过收集过程证据和提供诊断、程序可视化和学习助手来支持学习过程。

Conclusion: BlockPython为支持从基于块的编程到基于文本的编程的过渡和分析了学习过程提供了一个系统设计参考。
...
TL;DR: 冻结迁移在特定情况下有益，但不是一种默认的重用策略或普遍优于目标端重炼的替代方案。


<details>
  <summary>Details</summary>
Motivation: Prompt-side playbooks可以提高使用工具的语言代理，但其超出源设置的可移植性尚不明确。

Method: 研究了冻结脚本迁移在共享的提炼-验证-迁移协议下的情况。

Result: 在ALFWorld上，迁移在受控的贪婪解码下有益；在近预算匹配的比较中，提炼的指导优于五个固定的演示。在TAU2-Bench上，预指定的总体对比支持适度的平均匹配域优势，但全局Holm校正只保留了135个路由级效应中的一个；剩余的网格提供了对兼容性敏感的异质性的描述性证据。在XBench-DeepSearch上，一个工件-运行时配对在产生重复查询、延迟停止和成本显著增加后仍保留有用的第一次尝试启发式方法。在所有基准测试中，迁移的脚本和目标导出的脚本都需要目标端的成功验证、终止、协议兼容性和成本。因此，冻结迁移是一种条件性冷启动选项，而不是默认重用的策略，也不是比目标端重炼普遍更好的替代方案。

Conclusion: 冻结迁移是一种条件性冷启动选项，而非默认重用的策略或普遍优于目标端重炼的替代方案。
...
### [338] [When Agentic AI Meets Integrated Sensing and Communication](https://arxiv.org/abs/2608.05792)
*Kai Li,Conggai Li,Sarah Ali Siddiqui,Syed Sohail Ahmed,Xin Yuan,Shenghong Li,Wei Ni*

Main category: cs.AI

TL;DR: 该论文分析了Agentic AI在ISAC中的应用，并提出了一个闭环框架来评估和推动相关技术的发展。


<details>
  <summary>Details</summary>
Motivation: Agentic AI正在将ISAC从面向功能的物理层技术转变为以目标驱动的闭环智能系统，这种范式称为AISAC。

Method: 该调查将文献统一到一个包含六个阶段的闭环框架中，包括观察、情境化、推理和预测、规划和编排、执行和协作以及反馈和弹性。

Result: 引入了五个层次的代理成熟度，从物理层原语到完全闭环的代理ISAC。使用该框架回顾了多模态智能、大型语言模型、强化学习、联邦学习、RIS辅助控制、无人机和车联网以及AI原生网络管理的进展，并分析了隐私、安全、弹性和可持续性作为全感知-推理-行动循环的跨领域需求。

Conclusion: 没有系统报告超过一个或两个代理特定评估标准，暴露了声称和展示的代理成熟度之间的差距。识别了在物理到语义的扎根、预测性世界模型、实时代理-PHY交互、安全工具使用、异构多代理协作、基准测试和资源高效自主方面的开放挑战。
...
### [339] [When Self-Evolution Backfires: Pre-Commit Gating against Skill Contamination in LLM Agents](https://arxiv.org/abs/2608.05810)
*Linfang Shang,Ming Xu,Yiding Sun,Tianle Xia,Lingxiang Hu,Lan Xu,Ning Zheng*

Main category: cs.AI

TL;DR: Proposes VaG to filter harmful skills and improve performance in self-evolving agents.


<details>
  <summary>Details</summary>
Motivation: Self-evolving agents accumulate capability by distilling reusable skills from their execution trajectories, but past a critical pool size, newly added skills degrade performance instead of improving it.

Method: Formalizing capability-contamination phase transition, tracing to structural cause, showing contamination is structurally irreversible, proposing Verifier-as-Gatekeeper (VaG) with three heterogeneous critics and marginal-gain subset selection.

Result: VaG improves every round, reaching 72% pass@1 with a smaller pool, and its frozen skill pool transfers positively to other backbones and benchmarks without re-evolution.

Conclusion: Skill admission should be a pre-commit necessity, and VaG effectively filters harmful skills.
...
### [340] [Cautious Context Steering for Language Model Personalization](https://arxiv.org/abs/2608.05813)
*Gihoon Kim,Jeyoung Lee,Suhan Woo,Sekwon Oh,Minsu Jeon,Hyounsoo Han,Euntai Kim*

Main category: cs.AI

TL;DR: CCS通过条件化上下文来个性化语言模型，提高了生成质量并降低了推理成本。


<details>
  <summary>Details</summary>
Motivation: 个性化语言模型对于满足不同用户目标和背景至关重要。

Method: 提出了一种名为Cautious Context Steering (CCS)的方法，该方法通过在冻结的骨干语言模型上添加一个轻量级的适配器，在每个标记处决定用户上下文是否以及如何影响生成。

Result: CCS在单个数据集上训练，在领域内和四个分布外的个性化基准测试中均提高了生成质量，证明了对新用户和领域稳健的泛化能力。CCS还避免了按用户微调和CoS所需的额外上下文条件前向传递，大大降低了推理成本。

Conclusion: CCS提供了一种有效的个性化方法，可以减少数据稀疏性和提高泛化能力。
...
### [341] [ViSR-KGC: Visual Subgraph Reasoning with Vision-Language Models for Multimodal Knowledge Graph Completion](https://arxiv.org/abs/2608.05833)
*Jiafan Li,Mengxue Yang,Jiaqi Zhu,Liang Chang,Ying Li,Hongan Wang*

Main category: cs.AI

TL;DR: ViSR-KGC improves KGC by integrating visual subgraph reasoning and multimodal embeddings.


<details>
  <summary>Details</summary>
Motivation: Knowledge graph completion (KGC) faces challenges in limited relation-specific evidence and graph structure interpretation.

Method: Proposing ViSR-KGC, a visual subgraph reasoning approach for KGC. It integrates representation learning, VLMs, and pre-trained models.

Result: Extracts a compact subgraph, transforms it into an image, and combines it with textual descriptions for VLM inference.

Conclusion: ViSR-KGC effectively captures semantic correlations for KGC.
...
### [342] [Runtime Observability for Heterogeneous Attention Memory](https://arxiv.org/abs/2608.05863)
*Fanzhe Wei,Li Liu,Ziyang Wang,Chenyu Wang*

Main category: cs.AI

TL;DR: 提出了一种新的方法来处理现代模型中的内存问题，通过合约和组合技术，实现了对模型风险的量化和管理。


<details>
  <summary>Details</summary>
Motivation: 现代模型不再使用简单的键值缓存，而是使用潜在缓存、学习稀疏选择器和循环状态来存储模型记忆，这些不同的存储方式在压缩下会以不同的方式失败。

Method: 提出了一种运行时可观察性合约，该合约使用三个操作符涵盖了所有四种内存类别，并在六个模型配置和五个架构家族上进行了实例化。通过将每个阶段的界限组合成一个可执行的请求级风险账本。合约将错误度量作为类型，只有当度量匹配时才能进行组合，这拒绝了我们的第一个组合链；修复的链通过两个已证明的桥梁跨越了度量，无法由任何形式化系统认证的部分被测量，自动将组合层降低到经验性：每个主张都得到认证、部分认证或经验性认证，组合继承了最弱的层级，层级由机器决定。

Result: 在超过 12.4M 条条目读取和八路并发下，每个请求都有预算和失败关闭的身份归属，账本量化了今天的见证中的诚实权衡，并保持了其风险预算，没有违规行为。融合的始终在线探针在服务噪声地板下观察了声明的单层子集。应用于服务的 DeepSeek-V4 堆栈和打包的压缩-KV 原型，相同的机制将无声的损坏定位到精确的结构边界——在无驱逐、身份隔离的机制中精确，以及在驱逐或槽位重用机制中观察到的每个故障——通过机器裁决的区分活动，该活动在过程中拒绝了我们的两个混淆推断。所有工件、守卫和 Lean 开发都发布在 https://github.com/metask-ai/witprobe-attention-memory；本文中的每个数字都通过一个命令从提供的工件中重新生成。

Conclusion: 提出了一种新的方法来处理现代模型中的内存问题，通过合约和组合技术，实现了对模型风险的量化和管理。
...
### [343] [Seeing Is Not Deciding: Can Multimodal LLMs Act as Effective CEOs?](https://arxiv.org/abs/2608.05864)
*Yuyang Dai,Xueqing Peng,Yuxia Wang,Preslav Nakov,Zhuohan Xie*

Main category: cs.AI

TL;DR: 多模态输入提高了决策能力，但视觉增强可能损害决策，需要选择性定位策略。


<details>
  <summary>Details</summary>
Motivation: 探讨大型语言模型在执行商业决策中的应用，特别是在视觉业务证据感知和整合方面的能力。

Method: 提出C-SUITEBENCH，一个包含文本和多媒体条件的受控多模态基准，评估九个前沿模型在首席执行官角色下的决策能力。

Result: 多模态输入提高了基于证据的推理，但在资源分配上存在整合悖论，视觉通道组合破坏了解码过程中的约束满足。

Conclusion: 视觉感知和受限制的动作是多模态代理中的分离瓶颈，无差别的视觉增强可能损害高风险决策，为未来的高管AI系统提供有针对性的定位策略。
...
### [344] [Improving Interoperability among Defence and National Security Ontologies: Analysis and Evaluation Tasks](https://arxiv.org/abs/2608.05867)
...
### [345] [Personalized Deep Research Query Refinement with Graph-Scaffolded Evidence Grounding](https://arxiv.org/abs/2608.05876)
*Soojin Yoon,Dongha Lee*

Main category: cs.AI

TL;DR: G-STEER通过优化用户请求，提高了深度研究代理的性能。


<details>
  <summary>Details</summary>
Motivation: 用户请求作为深度研究代理的研究规范，塑造了寻找证据和综合证据的方式。在个性化深度研究中，这些规范还必须反映用户目标、约束、偏好和评估标准。用户上下文可以集成到深度研究流程中，或者作为其输入的研究规范。

Method: G-STEER将框架因素组织为意图提取图中的提取目标，该图捕捉了它们的依赖关系。它从图支架的轨迹中学习到一个澄清策略，这些轨迹跨越了不同的因素依赖和证据条件。该策略在平衡目标覆盖范围与证据获取成本之间产生一个精细的查询。

Result: 实验表明，G-STEER在评估的DRAs中实现了最强的整体加权目标覆盖范围和最高的下游报告个性化，同时询问大约三分之一的问题，与强大的澄清基线相比。

Conclusion: G-STEER通过优化用户请求，提高了深度研究代理的性能。
...
### [346] [AppDeltaWorld: Transition-Grounded Delta Code World Model for Mobile GUI Agents](https://arxiv.org/abs/2608.05891)
*Weikai Xu,Yunren Feng,Haoxiang Lei,Kun Huang,Yuxuan Liu,Kang Zhao,Xiaolin Hu,Shuo Shang,Bo An*

Main category: cs.AI

TL;DR: AppDeltaWorld是一种基于转换的delta代码世界模型，用于提高移动GUI代理的长期交互策略收集和改进。


<details>
  <summary>Details</summary>
Motivation: 为了解决移动GUI代理在收集和改进长期移动交互策略方面的挑战，如难以获取敏感应用的真实轨迹、现有模拟环境成本高昂以及GUI世界模型存在不稳定生成、有限模态覆盖和不一致的动作-转换逻辑等问题。

Method: 提出AppDeltaWorld，一个基于转换的delta代码世界模型，它将下一个GUI预测为可达到的代码更新，而不是无约束的图像或文本描述。AppDeltaWorld在动作-转换约束下检索特定应用的Level-1 HTML引用，根据当前屏幕、动作、预测的下一屏幕文本和检索的结构生成Level-2可执行HTML，并在浏览器渲染之前将生成的视觉资产插入图像槽中。

Result: 在CMGUIBench-500上的Code2World评估中，AppDeltaWorld作为世界模型实现了最高的保真度，在结构布局和UI元素重建方面相对于仅图像和仅代码的基线有明显的提升。作为训练环境，AppDeltaWorld支持过滤的闭环SFT数据构建，当与公共监督结合时，使AppDeltaAgent在AndroidLens上达到最先进的性能，并在MobileGym和MobileWorld上实现一致的提升。此外，基于世界模型的测试时间强化学习使策略适应，并在不与真实应用进行额外交互的情况下进一步提高了性能。

Conclusion: AppDeltaWorld是一种有效的移动GUI代理世界模型和训练环境，能够提高长期移动交互策略的收集和改进。
...
### [347] [ECG-LENS: Lead-Aware Clinical Context Enriched ECG Report Generation and Evaluation](https://arxiv.org/abs/2608.05893)
*Akanta Das,Tasinul Islam Ahon,Ahmed Mahir Sultan Rumi,Md Mahbubur Rahman,Tausif Amim Shadly,Tanzima Hashem*

Main category: cs.AI

TL;DR: ECG-LENS：一种用于自动生成心电图报告的新框架，在多个评价指标上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 降低临床医生的解释工作负担，提高诊断效率，扩大欠发达社区对心脏评估的访问渠道。

Method: 提出ECG-LENS，这是一个端到端的心电图报告生成框架，它联合集成了多导联信号建模、诊断感知表示和基于临床的文字生成。

Result: 在PTB-XL和MIMIC-IV-ECG上的域内实验和跨领域评估中，ECG-LENS在METEOR、ROUGE-L和F1-ECGBERT方面分别优于最强大的基线，绝对增益分别为4.0%、6.3%和11.5%。

Conclusion: ECG-LENS能够有效生成心电图报告，并在多个评价指标上优于现有方法。
...
### [348] [GSBF: Gaussian Splatting for Environment-Aware Beamforming](https://arxiv.org/abs/2608.05896)
*Yijie Bian,Wei Guo,Zixin Wang,Shenghui Song,Jun Zhang,Khaled B. Letaief*

Main category: cs.AI

TL;DR: GSBF方法通过使用3D高斯喷溅和无需在线CSI，提高了MIMO通信系统的波束成形性能。


<details>
  <summary>Details</summary>
Motivation: 传统波束成形设计需要准确的瞬时信道状态信息（CSI）和迭代优化，这导致大量的导频开销和计算复杂度。

Method: 开发了一种基于多模态数据的3D高斯喷溅环境感知波束成形（GSBF）管道，该管道通过持续的3D高斯表示来表征环境。具体来说，GSBF使用保持互易性的双向球面高斯（Bi-SG）核来建模环境散射响应，并执行双向电磁光栅化以渲染角度传播器图。然后，通过过完备数组流形字典聚合渲染的图，并将其投影到恒模波束成形器，从而直接从接入点（AP）姿态和用户位置合成波束，而无需在线瞬时CSI。

Result: 模拟结果表明，GSBF在低延迟方面始终优于基线，如穷举波束对齐（EBA）。

Conclusion: GSBF方法有效地降低了波束成形设计的导频开销和计算复杂度，同时提高了MIMO通信系统的性能。
...
### [349] [CourseGraph: Finding overlaps and differences in Computer Science courses across universities](https://arxiv.org/abs/2608.05910)
*Arthur Nijdam,Paul Stankovski Wagner,Sara Ramezanian*

Main category: cs.AI

TL;DR: CourseGraph通过自动评估外部课程，帮助识别课程重叠，支持大学之间的课程对齐。


<details>
  <summary>Details</summary>
Motivation: 学生流动项目如Erasmus+使学生在其他大学选修课程，拓宽他们的学术和文化视野。然而，这种灵活性也带来实际挑战：确保学生在其他地方选修的课程不会与他们在母校的课程大量重复。

Method: 我们提出了CourseGraph方法，该方法基于课程管理员在评估课程纳入学位计划时的过程，自动评估外部课程。Course-Graph从课程网页中提取信息，如课程标题、描述和学习成果。然后，使用基于BERT的语言模型对这些信息进行语义表示，之后计算课程的成对相似度。然后，使用随机森林分类器确定国外候选课程是否与学生在课程中已包含的课程重叠。

Result: 我们使用（1）埃因霍温科技大学计算机科学专业，其中包含关于大量重叠课程的信息；（2）六个经过批准的国际项目，包括在隆德大学计算机科学专业注册的学生，以及相应的课程管理员作出的决定来评估CourseGraph。实验结果表明，CourseGraph为识别重叠课程和支持大学之间的课程对齐提供了有效的方法。

Conclusion: CourseGraph为识别重叠课程和支持大学之间的课程对齐提供了有效的方法。
...
### [350] [GAUGE: A Measurement-Grounded Benchmark for Physical Fidelity in Simulation Engines and Video World Models](https://arxiv.org/abs/2608.05948)
...
TL;DR: GAUGE是一个评估物理真实性的基准，用于比较数值模拟器和视频世界模型。


<details>
  <summary>Details</summary>
Motivation: 评估物理真实性的现有方法通常独立进行，并过度依赖感知相似性或人类判断，提供有限见解。

Method: 提出GAUGE，一个基于现实世界的诊断基准，用于联合评估数值模拟器和生成视频世界模型如何再现或偏离现实世界物理。

Result: 结果显示没有统一的忠实物理引擎，最大的差异出现在冲击接触、快速纺织品运动和体积变形中。视频世界模型可以产生具有预期方程形式的轨迹，同时恢复不正确的加速度、动量传递和振荡时间。

Conclusion: GAUGE为开发更真实的模拟器和世界模型奠定了基础。
...
### [351] [VLMs for Videogame Data Annotation](https://arxiv.org/abs/2608.05949)
*Katrin Schmid,Iuri Frosio*

Main category: cs.AI

TL;DR: 研究了VLMs在视频游戏标注中的应用，发现其存在局限性，并提出改进策略。


<details>
  <summary>Details</summary>
Motivation: 由于VLMs和AI代理在现实世界应用中处理复杂问题的能力，以及它们在视频游戏中的有限应用，我们研究了使用VLMs为视频游戏帧序列添加奖励信号的方法。

Method: 通过使用VLMs为视频游戏帧序列添加奖励信号，包括条件训练和离线强化学习等潜在应用。

Result: VLMs在回答赛车游戏等基本问题方面存在困难，并讨论了VLM输出混合和提示优化等对策。此外，还展示了输入序列长度、分辨率和批量问题如何影响标注质量和其令牌消耗。

Conclusion: VLMs在视频游戏标注方面具有潜力，但需要改进以应对游戏场景的复杂性和物理限制。
...
### [352] [Training a Conditioned Video Game Agent on a VLM Annotated Dataset](https://arxiv.org/abs/2608.05954)
*Katrin Schmid,Iuri Frosio*

Main category: cs.AI

TL;DR: 使用VLM标注和离线强化学习在视频游戏中实现政策学习，但存在挑战和限制。


<details>
  <summary>Details</summary>
Motivation: 为了解决强化学习在视频游戏中的应用难题，如获取奖励、设置奖励权重以及理解奖励影响等，我们提出了使用视觉语言模型（VLM）对视频游戏数据集进行标注，以提取人类定义的奖励。

Method: 使用VLM标注视频游戏数据集，通过离线强化学习（Offline RL）训练条件化的智能体，使其能够根据期望回报做出相应反应。

Result: 离线强化学习成功训练了智能体，并讨论了实验中出现的困难和限制。

Conclusion: VLM标注的离线强化学习方法在视频游戏中实现政策学习具有可行性，但仍存在挑战和限制。
...
### [353] [Stability of Ranking-dependent Pair-wise Comparison Patterns in the Analytic Hierarchy Process](https://arxiv.org/abs/2608.05958)
*Vitaliy Tsyganok,Sergii Kadenko,Oleh Andriichuk*

Main category: cs.AI

TL;DR: 研究提高了专家数据质量，减少了比较次数，并对决策支持做出了贡献


<details>
  <summary>Details</summary>
Motivation: 提高专家数据质量，减少专家比较次数

Method: 比较三种不完全排名依赖成对比较模式：最佳-最差法，最佳-第二最佳（前两名）法，原始最大差异法

Result: 定义了最稳定的排名依赖成对比较模式，减少了比较次数，提高了专家会话结果的可靠性

Conclusion: 对不确定环境中的决策支持在算法、认知和应用方面做出贡献
...
### [354] [Temporal Bridges for Spatial Resolution: Enhancing Climate Data Super-Resolution with Bidirectional Alignment](https://arxiv.org/abs/2608.05981)
*Yichen Zhang,Yixiong Xiao,Congxi Xiao,Jingbo Zhou*

Main category: cs.AI

TL;DR: 提出了一种新的时间增强框架，提高了气候数据超分辨率的空间分辨率，实验结果表明性能优越。


<details>
  <summary>Details</summary>
Motivation: 高分辨率气候数据对于气象预测和决策支持至关重要，但其获取成本高昂，需要开发数据驱动的气象预测模型。

Method: 提出了一种新的时间增强框架，具有双向时间对齐，通过双向对齐建立时间桥梁，提高气候数据超分辨率的空间分辨率。

Result: 在大规模真实世界数据集上的实验表明，该框架表现出优异的性能。

Conclusion: 该研究为气候数据超分辨率提供了一种有效的方法，有望提高气象预测和决策支持的准确性。
...
### [355] [AgentOPSD: Recursive Self-Distillation for Agentic Reinforcement Learning](https://arxiv.org/abs/2608.05987)
...
TL;DR: AgentOPSD：一种新的轮次级信用分配方法，显著提升了强化学习在复杂任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 强化学习（RL）在长周期、多轮次的智能体任务中，往往无法正确评估决定结果的关键决策。

Method: 提出了一种名为AgentOPSD的递归方法，用于在智能体强化学习中进行轮次级信用分配。

Result: 在ALFWorld、WebShop和Search-QA上使用Qwen2.5模型进行评估，AgentOPSD优于GRPO和强自蒸馏基线，在ALFWorld上实现了89.1%的成功率。

Conclusion: AgentOPSD是一种有效的信用分配方法，可以显著提高强化学习在复杂任务中的性能。
...
### [356] [OPERA: Operator-residual feedback for reliable autonomous optical experiments with language-model agents](https://arxiv.org/abs/2608.05990)
*Ning Xu,Xiang Zheng,Fuqiang Zhong,Huadong Wang,Xiaolong Wu,Zhiyuan Liu,Hui Ning*

Main category: cs.AI

TL;DR: OPERA框架提高了光学实验中自主决策的准确性，降低了实验成本，并改善了结构光重建的性能。


<details>
  <summary>Details</summary>
Motivation: Autonomous agents choose actions based on scores that may not reflect experimental success.

Method: Developed OPERA, an operator-residual framework for optical experiments. Represented experimental actions as optical operators and evaluated outcomes using physically interpretable residuals.

Result: Operator-residual feedback increased the probability of reaching and maintaining task targets, reduced experimental budgets, and improved performance in structured-light reconstruction.
...
### [357] [Hybrid Machine Learning Framework for Herd-Level Cattle Growth Pattern and Weight Gain Forecasting in Grazing-Based Production Systems](https://arxiv.org/abs/2608.06001)
*Muhammad Riaz Hasib Hossain,Rafiqul Islam,Shawn R. McGrath,Md Zahidul Islam,David W. Lamb*

Main category: cs.AI

TL;DR: 研究提出了一种混合机器学习框架，用于预测澳大利亚东南部牧群层面的牛体重，以提高放牧管理效率。


<details>
  <summary>Details</summary>
Motivation: 商业放牧系统产生的牲畜观察数据不规则，对牛的生长预测提出了挑战。

Method: 开发了混合机器学习框架，利用2022年至2024年在澳大利亚东南部收集的自动化感知观察数据，对牧群层面的牛体重进行预测。

Result: 评估了四种混合架构，包括残差、堆叠、级联和集成辅助框架。ARIMA、LSTM和GRU模型作为比较基线。混合架构在稀疏观测条件下比循环顺序模型具有更高的鲁棒性。

Conclusion: 所提出的框架可能支持饲料分配、放牧管理和牲畜营销决策。
...
### [358] [HERALD: Counterfactual Audits and Minimal Repairs for Proof-of-Retrieval Rewards](https://arxiv.org/abs/2608.06012)
...
### [359] [From Economic Agents to Agentic Economies: A Systems Blueprint for Economic World Models](https://arxiv.org/abs/2608.06020)
*Jiale Han,Xiang Li,Jing Qian,Wenyuan Gu,Pin Gao,Ye Luo,Hongyuan Zha,Dacheng Tao,Benyou Wang,Lin William Cong*

Main category: cs.AI

TL;DR: 本研究提出了一种构建经济世界模型的方法，旨在促进经济模拟环境的发展，以支持人类决策者和AI代理。


<details>
  <summary>Details</summary>
Motivation: 为了加速下一代经济模拟环境的发展，使其能够作为人类决策者的真实沙盒，以及作为AI代理的训练、规划、评估和安全基础。

Method: 通过将EWM议程转化为实施蓝图，对EWM系统进行六级能力等级划分，并系统性地调查了这些级别的现有工作。

Result: 揭示了现有工作主要集中在较低级别的代理和模拟环境中，而具有自我进化代理、内生制度、持续实证对齐和验证的经济机制的系统仍然很少。

Conclusion: 本研究旨在通过建立经济世界模型作为生成引擎，实现经济动态的内部生成，以促进经济模拟环境的发展。
...
### [360] [Integrating Implicit and Explicit Relational Biases through Graph-Based Multiple Instance Learning: A Case Study in Skin Lesion Diagnosis](https://arxiv.org/abs/2608.06037)
*Rafał Buler,Jakub Buler,Maciej Bobowicz,Michał Grochowski*

Main category: cs.AI

TL;DR: 结合隐式和显式关系建模，图像分类性能显著提升。


<details>
  <summary>Details</summary>
Motivation: 为了捕捉数据之间的结构依赖关系，研究提出了一个双级关系框架，以解决隐式表示学习和显式结构建模之间的差距。

Method: 首先使用EfficientNetB3架构建立基线，然后采用基于补丁的策略，利用卷积掩码自动编码器通过自监督重建学习补丁间的隐式关系。进一步地，通过结合显式关系建模，将学习到的嵌入组织成各种图拓扑结构，包括基于网格、随机和k-最近邻结构。

Result: 在ISIC-2018和ISIC-2019皮肤病变诊断基准测试中，将隐式补丁间建模与显式图传递相结合，取得了最佳性能。在ISIC-2018测试集上，基线模型实现了76.17%的平衡准确率，而隐式补丁关系建模将准确率提高到了77.12%。完全集成的基于网格的图注意力网络将性能进一步提升至79.27%。在ISIC-2019上，隐式方法达到59.84%的平衡准确率，而隐式和显式建模的结合达到了60.67%。

Conclusion: 结合隐式补丁间建模和显式图传递，可以显著提高图像分类的性能。
...
### [361] [When History Lies: Evaluating and Improving Tool Use under Misleading Multi-Turn Histories](https://arxiv.org/abs/2608.06057)
*Xiaoqing Wu,Xingyu Fan,Feifei Li,Wenhui Que*

Main category: cs.AI

TL;DR: 提出了一种新的工具使用方法，通过迁移策略提高了工具使用的准确率，并证明了其在不同场景下的有效性。


<details>
  <summary>Details</summary>
Motivation: 工具调用代理从累积的对话和工具跟踪中推断任务状态。然而，在持续交互中，历史痕迹可能在它们不再对当前请求具有权威性后仍然在结构上有效且在语义上合理。

Method: 提出了一种名为bench的配对基准，具有同步的原始、污染和Oracle状态视图，以保留系统策略、当前工具、最新请求和黄金后续操作。此外，还提出了一种方法，通过在学生生成的词干上进行软监督，将Oracle条件下的教师策略转移到只观察污染历史的学生身上。

Result: 在Qwen3-1.7B上，该方法实现了87.0%的平衡工具使用准确率，优于Gold-SFT（66.3%）、Oracle序列蒸馏（82.3%）和离线策略令牌蒸馏（85.0%）。该方法可扩展：8B教师将相同的紧凑1.7B学生提高到91.9%，而8B学生达到93.0%。这些结果将历史可靠性确立为工具使用的瓶颈，并证明了可靠状态策略迁移作为有效且可扩展的解决方案。

Conclusion: 历史可靠性是工具使用的瓶颈，可靠状态策略迁移是有效且可扩展的解决方案。
...
### [362] [Signal or Spurious Cue? A Randomized Audit of Survey-Country Metadata in LLM Social Inference](https://arxiv.org/abs/2608.06085)
*Yifan Lyu,Xinran Li,Jiaqi Qiao,Xiujuan Xu*

Main category: cs.AI

TL;DR: 验证的元数据对预测有益，但随机标签的披露并不能有效减少其针对国家的采用。


<details>
  <summary>Details</summary>
Motivation: 调查国家元数据可以提高大型语言模型对个人响应的预测，但同一线索在随机分配时可能会改变预测。

Method: 在记录内部审计中测试是否披露随机标签的均匀、记录独立的来源会减少其针对国家的采用，以及验证的调查国家是否会降低保留的Brier损失。使用独立的人口锚定和记录的人类答案来衡量五个固定API模型、六个国家和七个发展选定目标的方向和后果。

Result: 在审查后的72个记录的初级小组中，不透明和披露的随机标签各自产生了0.214的国家方向变化。配对衰减为0.0003（95% CI [-0.0157, 0.0166]）。验证的国家减少了Brier损失0.040（95% CI [0.024, 0.056]），而随机标签的遗憾为零。一个非重叠的混合覆盖率一致性小组保留了披露的随机移动和验证的有效性，但衰减仍然不确定。在选定目标上，验证的元数据在两个小组中都很有用，但披露并不能可靠地减少随机标签的采用。PROV-FORECAST包含从校正小组中校正的14,400对项目级概率分布。

Conclusion: 验证的元数据在预测中很有用，但披露随机标签并不能可靠地减少其针对国家的采用。
...
### [363] [Evaluating Investment Logic in Large Language Models: A Real-World Benchmark Towards Personalzied Financial Agents](https://arxiv.org/abs/2608.06108)
...
### [364] [ECHO: A Locally-Deployable Agentic Health Assistant with Temporal Memory, Safety Guardrails, and Speech Assessment](https://arxiv.org/abs/2608.06110)
*Abdulkadir Külçe,Alihan Esen,Cağla Fikir,Berke Kurt,Kuzey Arar,Gökhan Ercan,Faik Boray Tek*

Main category: cs.AI

TL;DR: ECHO是一种用于长期慢性病管理的对话式健康助手，集成了多个模块，具有高通过率和准确率，支持合规性。
...
Method: 集成三个互补软件模块，构建统一系统；核心模块为基于ReAct循环的智能聊天机器人，包含17个临床工具和时序知识图谱；两阶段混合安全层拦截所有查询；多模态语音评估模块结合 Whisper 和 BERT 编码，融合交叉注意力；全系统作为网络应用程序实现，支持GDPR和KVKK合规性。

Result: 聊天机器人工具执行通过率94.9%，安全层准确率88.8%，多模态评估模块平均宏F1值为0.652。

Conclusion: ECHO系统为长期慢性病管理提供了一种高效、安全、合规的解决方案。
...
### [365] [From Siloed Algorithms to Compliance-First Agentic Platforms: A Multi-Layered Architecture for Hospital AI Systems](https://arxiv.org/abs/2608.06112)
*Manideep Dhar,Ritwik Singh,Sharat Chandra Kumar Manikonda*

Main category: cs.AI

TL;DR: 本研究提出了一种新型AI架构，旨在解决医院AI部署中的孤岛化问题，并通过模拟实验证明了其有效性和实用性。


<details>
  <summary>Details</summary>
Motivation: 医院在采用人工智能进行分诊、影像、调度等方面迅速发展，但大多数部署仍处于孤立的状态，导致重复劳动、潜在风险和未实现的商业价值。

Method: 提出了一种针对医院的、以合规性为首要的、具有多个互操作层的智能体AI架构。

Result: 通过使用合成但结构上真实的医院数据集和开放的原型实现，该研究证明了分诊风险预测、工作流程优化和合规性记录的端到端编排，在保持政策保护的数据访问的同时，实现了显著的任务周转时间和手动文档工作的模拟减少。

Conclusion: 该架构为医院领导者提供了一个从临时工具到受治理、全球合规、以回报为导向的AI平台的实用蓝图，该平台可以定制为本地、混合和原生云部署。
...
### [366] [Mind the Gaps: Mixture-of-Minds for Human Simulation](https://arxiv.org/abs/2608.06115)
*Pranav Dahiya*

Main category: cs.AI

TL;DR: 本文提出了一种名为Anacreon的受众模拟模型，用于预测个体对问题的回答。该模型通过学习作者嵌入、聚类真实定性语料库和为每个集群训练专用适配器来实现。它在外部来源的大型调查中取得了最先进的个体水平准确度。


<details>
  <summary>Details</summary>
Motivation: 预测人口对全新问题的回答是一个长期目标，统计方法在群体层面上表现良好，但在个体层面上却失败。大型语言模型模拟器继承了这一差距。它们恢复了群体的中心趋势，同时削弱了群体的异质性，并携带社会偏见和提示脆弱性，这些都会扭曲个体预测。

Method: 本文介绍了Anacreon，一个针对窄域内个体水平的受众模拟模型。Anacreon学习一个作者嵌入来区分个体，将真实的定性语料库围绕种子人群聚类，并为每个集群训练一个专门的适配器，这是一种心智混合体，基于Gemma~4 12B基础。它从公共文本中收集人口统计、心理特性和调查回答，并使用情感链为每个记录增补。它通过打乱回答选项来减少提示脆弱性，通过平衡训练分布来减少正面偏见。

Result: 在一个大型、外部来源的调查中，Anacreon在个体水平的准确度度量上达到了最先进的序数对齐，即0.775，该度量在领域内已经达成共识，但仍有轻微的残留偏见。

Conclusion: 这项工作是通过忠实模拟的个体来获取聚合见解的一步。
...
### [367] [Poli-Bias: Understanding and Measuring Large Language Model Biases in International Political Conflicts](https://arxiv.org/abs/2608.06123)
*Massi-Nissa Abboud,Aladin Djuhera,Elena Cabrio,Holger Boche*

Main category: cs.AI

TL;DR: Poli-Bias是一种用于测量LLMs中政治偏见的框架，它揭示了国家身份和用户身份如何影响等效行为的描述、评估和辩护。


<details>
  <summary>Details</summary>
Motivation: 测量大型语言模型（LLMs）中的政治偏见仍然具有挑战性，因为它可以通过框架、论证和法律推理中的微妙差异表现出来，这些差异难以用单一指标捕捉。

Method: 提出了一种名为Poli-Bias的对比框架，该框架通过比较不同国家身份在多样化地缘政治关系、违法行为和推理任务中的配对提示的响应来衡量LLMs是否根据涉及的国家对待法律等效冲突场景不同。

Result: 在13个当代LLMs中，发现国家身份和用户身份可以系统地影响等效行为在国际法下的描述、评估和辩护。

Conclusion: Poli-Bias被确立为一个精细的框架，用于审计LLMs中的政治公正性和谄媚性。
...
### [368] [Contextual Information Policy Optimization for Search Agents](https://arxiv.org/abs/2608.06128)
*Xingyu Guo,Wei Chen,Linlin Yang,Baochang Zhang*

Main category: cs.AI

TL;DR: CIPO通过将策略优化与证据使用对齐，提高了搜索代理在知识密集型任务中的推理可靠性。
...
Method: 提出了一种名为CIPO的证据导向强化学习框架，该框架将策略优化与外部证据使用明确对齐。CIPO为受检索信息影响的推理动作分配密集的、回合级别的信用，同时将此证据使用信号与全局结果奖励相结合，以保持答案的正确性。

Result: 在七个领域内和领域外的基准测试中，CIPO减少了先验驱动推理的普遍性，并在大多数任务上取得了优异的性能。

Conclusion: CIPO框架能够有效地引导推理过程，提高搜索代理在知识密集型任务中的可靠性。
...
### [369] [Schema-Guided Hierarchical Information Extraction and Semantic Evaluation Using Generative AI](https://arxiv.org/abs/2608.06167)
*Modhurita Mitra,Jan-Willem Versteeg,Maarten D. Schermer,Shiva Nadi Najafabadi,Marie L. De Bruin,Lourens T. Bloem*

Main category: cs.AI

TL;DR: 一种高效的信息提取和评估框架，基于生成式AI，应用于非结构化文本处理


<details>
  <summary>Details</summary>
Motivation: 从非结构化文本中提取复杂、结构化信息，并进行自动化语义评估

Method: 基于模式的框架，使用生成式AI，引入基于路径的语义匹配算法，以及用于分类比较结果的评分标准

Result: 从NICE发布的文档中提取12个属性，F1分数超过90%，提取时间比人工专家快30倍，框架在不同生成式AI模型和HTA组织及语言中具有通用性和可迁移性

Conclusion: 提出了一种有效的框架，能够高效地从非结构化文本中提取信息并进行评估，具有广泛的应用前景
...
### [371] [HarnessOpt-Bench: Evaluating LLMs at Harness Optimization](https://arxiv.org/abs/2608.06301)
*Varun Ursekar,Apaar Shanker,Yash Maurya,Shehab Yasser,Vijay S. Kalmath,Veronica Chatrath,Yuan Xue*

Main category: cs.AI

TL;DR: 提出了一种新的编排优化基准，评估了LLM在编排优化任务上的表现，发现优化器模型分离度超过编码编排，本地编排并非始终优于其他编排，且增益在不同任务和种子规则下差异很大。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型（LLM）在智能系统中的应用越来越广泛，其能力不仅取决于模型权重，还取决于其周围的提示、工具、控制流、内存和编排代码等。这使得自动化编排优化成为提高AI系统性能的重要途径，同时也是AI系统本身的一项具有挑战性的能力。然而，该领域缺乏一个共同的协议来衡量前沿LLM在此任务上的表现。

Method: 提出了一种名为HarnessOpt-Bench的基准，用于在昂贵和随机的评估下进行端到端的编排优化。优化器（一个与编码编排配合的LLM）接收目标代理的种子编排、评分评估反馈和固定的目标评估预算。它编辑编排并提名一个最终候选方案，该方案在保留的测试分区上按其相对于种子版本的归一化增益进行评分，该测试分区在整个搜索过程中保持不可访问。可信执行环境强制执行评估边界，测量目标代理的资源使用情况，并保留候选版本以供审计。

Result: 评估了5个前沿LLM作为优化器，在共享的编码编排和它们本地的编排下，在4个下游任务中进行了111次评分运行。实验结果表明，优化器模型分离度超过它们所通过的编码编排，本地的编排并不始终优于其他编排，并且增益在不同任务和种子规则下差异很大。这些结果表明，编排优化是一种可衡量的、具有区分度的能力，具有很大的改进空间。

Conclusion: 编排优化是一种可衡量的、具有区分度的能力，具有很大的改进空间。
...
### [372] [PaDoc: Layout-Grounded Parallel Decoding for Document Parsing](https://arxiv.org/abs/2608.06146)
...
### [373] [Beyond Top-K: Replacing Black-Box Retrieval with Interpretable Agentic Operations](https://arxiv.org/abs/2608.06305)
*Sagar Tamang,Ayush Vyas,Tabarakul Hazarika*

Main category: cs.AI

TL;DR: READ模型在处理财务报告等长文档时比传统方法更可靠，BM25与READ在统计上无显著差异


<details>
  <summary>Details</summary>
Motivation: 对长文档检索增强生成的设计提出质疑，特别是针对财务报告等文档

Method: 提出READ模型，通过三个确定性操作（标准化词汇搜索、结构导航和有界跨度读取）来读取原始文档

Result: READ在51个验证问题上的准确率为58.8%，优于密集检索的15.7%，且BM25与READ在统计上无显著差异

Conclusion: READ模型在处理财务报告等长文档时比传统方法更可靠，且BM25与READ在统计上无显著差异
...
### [374] [CogVis: Must Open-Vocabulary Change Detection Perceive the Scene Anew for Every Query?](https://arxiv.org/abs/2608.06150)
...
### [375] [iARCS: Iterative Agentic RL for Controllable 3D Scene Generation](https://arxiv.org/abs/2608.06161)
*Saugat Adhikari,Ashok Prasad Neupane,Pramish Paudel,Ajad Chhatkuli,Danda Pani Paudel*

Main category: cs.AI

TL;DR: iARCS通过迭代代理强化学习框架，提高了合成3D场景生成在计算机视觉和具身AI中的实用性。


<details>
  <summary>Details</summary>
Motivation: 合成3D场景生成在计算机视觉和具身AI中作为数据源的使用越来越广泛，但现有的生成器往往在优化感知真实性的同时，无法可靠地满足任务关键的功能约束。这种不匹配限制了合成数据在下游训练中的有用性，在可访问性、可穿越性和空间规则遵守方面通常至关重要。

Method: iARCS，一种迭代式代理强化学习框架，该框架将预训练的场景生成器适应自然语言任务要求。iARCS采用两阶段策略：通用奖励预训练以改进物理合理性和布局质量，然后是任务特定微调，使用由LLM生成的奖励程序进行迭代优化，这些程序从训练反馈中不断改进。

Result: 实验表明，在可通行性、可达性和清除重点任务上提高了约束保真度，有效地优化了特定任务的约束，具有竞争力的场景多样性。此外，我们还表明，iARCS生成的数据提高了基础生成器的性能，支持其作为实用的合成数据生成工具的价值，而不仅仅是可控的场景编辑方法。

Conclusion: iARCS通过迭代代理强化学习框架有效地解决了合成3D场景生成中任务关键功能约束的问题，提高了合成数据的实用性。
...
### [376] [MicroEvo: Knowledge-Guided LLM Sampling for Efficient Microarchitecture Design Space Exploration](https://arxiv.org/abs/2608.06183)
...
TL;DR: MicroEvo是一种基于知识的微架构优化框架，通过结合LLMs和MCTS，显著提高了Pareto前沿质量和搜索效率。


<details>
  <summary>Details</summary>
Motivation: 微架构设计空间探索面临搜索空间庞大和PPA评估成本高昂的问题，导致设计决策的模拟预算有限。现有方法在没有考虑微架构依赖的情况下进行盲目搜索，并且未能有效地从迭代搜索中学习，导致评估浪费和Pareto收敛性弱。

Method: 提出MicroEvo，一个将现成的LLMs与MCTS相结合的知识引导框架，用于多目标微架构优化。MicroEvo结合了LLM驱动的进化算子、平衡Pareto贡献和多样性的Pareto感知树策略、主动知识积累机制以及状态感知指令，以适应在线搜索行为。

Result: 实验表明，MicroEvo将Pareto前沿质量提高了36.2%，比NSGA-II高10.6倍，并且展示了强大的可扩展性，适用于复杂的工业级核心。

Conclusion: MicroEvo是一种有效的微架构优化方法，能够提高Pareto前沿质量并提高搜索效率。
...
### [377] [Comparative Approaches to Agent Retrieval over Large Skill Libraries](https://arxiv.org/abs/2608.06196)
...
### [378] [EnvACE: Internalizing Environment Dynamics via World Rehearsal for Agentic Reinforcement Learning](https://arxiv.org/abs/2608.06197)
...
TL;DR: EnvACE通过世界排练实现了LLM代理训练的扩展，提高了策略学习，并在多个基准测试中优于基线。


<details>
  <summary>Details</summary>
Motivation: 传统的长期工具使用训练方法依赖于与真实或合成的可执行环境交互，这种方法的建设和验证成本高昂，或者依赖于难以验证的外部模拟器。

Method: EnvACE，一种代理强化学习方法，在训练过程中用世界排练代替外部环境交互。策略在行动和排练之间交替：首先生成一个工具调用，然后扮演环境角色以产生由该动作引起的响应，并根据排练的响应条件化后续决策。这两个角色通过任务成功奖励联合端到端优化。通过世界排练，策略将其参数中的动作与环境响应之间的关系内化，从而产生一个直接支持决策的代理世界模型。

Result: 在BFCL-v4、tau^2-Bench、VitaBench和FinMCP-Bench上，EnvACE实现了强大且可迁移的性能，在总体评估中优于环境扩展基线。控制研究进一步表明，世界排练在所有模型规模上始终如一地提高了策略学习。在测试时间，内部化的世界模型在执行前允许进行私人排练，在没有额外外部交互的情况下，在适度的排练预算下产生进一步的收益。

Conclusion: 我们的发现将世界排练确立为一种新的途径，可以将LLM代理训练扩展到外部环境的约束之外。
...
### [379] [TS-RAG: Retrieval Augmented Generation for Time Series Forecasting](https://arxiv.org/abs/2608.06223)
*Yixiong Xiao,Congxi Xiao,Jingbo Zhou*

Main category: cs.AI

TL;DR: TS-RAG通过引入专门设计的参考标记，有效融合了输入序列和检索到的相似序列的信息，显著提高了时间序列预测的准确性。


<details>
  <summary>Details</summary>
Motivation: 尽管深度学习模型在时间序列预测方面表现出色，但检索增强生成（RAG）的应用仍然有限。由于RAG通过结合相关外部信息已证明可以有效提升大型语言模型的能力，检索类似的时间序列序列作为参考也可能提高时间序列预测任务的准确性。然而，大多数时间序列模型受限于有限的数据、较小的参数规模和缺乏大型语言模型中广泛存在的生成能力。简单地将参考序列连接到提示中，就像在语言模型中做的那样，可能不会产生预期的结果。

Method: 我们提出了一种新颖的方法，即TS-RAG，它利用RAG来增强预测性能。该框架引入了专门设计的参考标记，以有效地融合来自输入序列和检索到的相似序列的信息，从而能够更稳健地捕捉复杂的时序动态。

Result: 实验结果表明，TS-RAG在多个现实世界的预测基准测试中实现了持续的最优性能。

Conclusion: TS-RAG通过利用RAG增强了时间序列预测的性能，实现了在多个基准测试中的最佳性能。
...
### [380] [DASH: Divergence-Adaptive Supervision Horizons for On-Policy Self-Distillation of Reasoning Models](https://arxiv.org/abs/2608.06243)
...
TL;DR: DASH是一种新的强化学习方法，通过自适应调整监督权重，提高了语言模型的推理能力
...
Result: 在三个数学推理基准测试中，DASH在所有三个模型规模上均优于标准OPSD（On-policy Self-distillation）

Conclusion: DASH通过自适应调整监督权重，提高了语言模型的推理能力
...
### [381] [Improving the Realism of Synthetic Clinical Benchmarks Under Utility Constraints](https://arxiv.org/abs/2608.06265)
...
### [382] [The Illusion of Visual Tool-Use: A Causal Audit of Thinking with Images](https://arxiv.org/abs/2608.06270)
*Zhiheng Wang,Bo Peng,Lai Wei,Chaochao Lu*

Main category: cs.AI

TL;DR: 视觉工具使用存在误导性，并不总是导致准确的答案


<details>
  <summary>Details</summary>
Motivation: 分析多模态LLM中图像操作对答案的影响

Method: 构建因果图，通过干预政策、轨迹和步骤三个层次进行审计

Result: 发现视觉工具使用存在政策误校准，导致答案不准确

Conclusion: 视觉工具使用在多数情况下并不具有因果效应，存在误导性
...
### [383] [QuanTiMedAI: Quantum-Enhanced Time-Series Model guided by Agentic AI for Cardiac Arrest Mortality Prediction](https://arxiv.org/abs/2608.06294)
*Mutasim Fuad Sarker,Adiba Rahman Namira,Wafa Binte Alam,Md Adnan Arefeen,Mahzabeen Emu,Sumaiya Tabassum Nimi*

Main category: cs.AI

TL;DR: QuanTiMedAI通过量子增强的顺序建模在心脏骤停死亡率预测中取得显著成果。


<details>
  <summary>Details</summary>
Motivation: 针对重症监护病房中心脏骤停死亡率预测的局限性，现有研究主要依赖于早期入院时的静态总结，忽略了患者在整个ICU住院期间生理恶化和恢复的时序进展。

Method: 提出QuanTiMedAI，一个基于量子增强时间序列模型的代理人工智能框架，用于心脏骤停死亡率预测。该系统结合了临床信息化的代理大型语言模型（LLM）进行特征发现和紧凑的量子循环网络进行时序感知的死亡率预测。

Result: 在MIMIC-IV心脏骤停患者队列上进行的广泛实验表明，QuanTiMedAI的量子增强架构仅使用605个参数就达到了0.852的AUROC，比当前最先进的基线提高了约2.9%。结构化消融研究系统地验证了每个架构设计选择的贡献。

Conclusion: 量子增强的顺序建模可以超越经典循环网络，同时使用大量更少的参数。
...
### [384] [Bias Analysis of L2 Speaking Assessment Systems Using Concept Activation Vectors](https://arxiv.org/abs/2608.06300)
*Arya Labroo,Mengjie Qian,Kate Knill*

Main category: cs.AI

TL;DR: 通过CAVs分析，发现SAEs在降低原始激活空间敏感性的同时，使概念更容易线性恢复。


<details>
  <summary>Details</summary>
Motivation: 评估自动说话评估系统的公平性和可解释性，特别是在语言学习环境中。

Method: 使用概念激活向量（CAVs）分析两个基于神经网络的说话评估系统：基于文本的BERT评分器和基于Whisper的语音和文本多模态评分器。

Result: 概念的可恢复性强烈依赖于被探测的表示和架构，而不仅仅是概念本身。SAEs使概念更容易线性恢复，但会减弱原始激活空间的敏感性。

Conclusion: 在审查说话评估系统中的偏差时，需要区分概念的可恢复性和概念的影响。
...
### [385] [TRAJDEBUG: Tracing Error Lifecycle to Identify Critical Failures in Long-Horizon Agent Trajectories](https://arxiv.org/abs/2608.06346)
...
TL;DR: 提出TrajDebug框架，有效解决LLM-based agentic系统的错误检测和调试问题。


<details>
  <summary>Details</summary>
Motivation: 针对LLM-based agentic系统在复杂领域中的错误检测和调试困难，提出TrajDebug框架。

Method: 提出TrajDebug框架，使用多粒度历史压缩和基于证据的错误识别，支持关键归因。

Result: 在多个基准测试中，TrajDebug表现出最佳性能，诊断结果有助于提高下游代理的成功率。

Conclusion: TrajDebug框架有效解决了LLM-based agentic系统的错误检测和调试问题。
...
### [386] [Challenges in Evaluating Explanation Methods for Static and Evolving Data](https://arxiv.org/abs/2608.06351)
*Jerzy Stefanowski*

Main category: cs.AI

TL;DR: 本文探讨了XAI评估局限性，并提出了解决方案


<details>
  <summary>Details</summary>
Motivation: 探讨可解释人工智能(XAI)在评估方面的局限性

Method: 通过DetoxAI图像识别系统展示XAI局限性，并展示人类基础的评价方法，探讨适应数据流变化的方法，讨论反事实的适应经验，以及跟踪数据、模型和解释共进化的挑战

Result: 提出了XAI评估局限性，展示了人类基础的评价方法，探讨了适应数据流变化的方法，讨论了反事实的适应经验，并分析了数据、模型和解释共进化的挑战

Conclusion: 对XAI评估的局限性进行了深入分析，为XAI方法的发展提供了有益的参考
...
### [387] [The Low Frequency Trap: Video Language Models Fail at Simple Event Bookkeeping](https://arxiv.org/abs/2608.06361)
...
TL;DR: 提出基于跟踪的参数化分析，发现视频事件计数存在阶段性时间失败，评估应转向详细诊断。


<details>
  <summary>Details</summary>
Motivation: 解决现有视频基准测试中固定剪辑导致的难以隔离的失败模式问题。

Method: 引入基于跟踪的参数化分析，在三个受控视频任务中进行事件计数。

Result: 发现存在阶段性时间失败，在高计数、高频模式下，只有0.2%的最终计数是正确的，模型仅恢复18.1%的真实事件。

Conclusion: 基于跟踪的配置文件将视频评估从总体准确率指标转变为对时间推理失败点的详细诊断。
...
### [388] [Tracing the Heart: An Evidence-Linked Pipeline for Heart-Failure Feature Engineering](https://arxiv.org/abs/2608.06366)
...
TL;DR: 开发了nMAS，一种自动化心力衰竭特征工程的方法，提高了心力衰竭表型的准确性，但需要外部验证。


<details>
  <summary>Details</summary>
Motivation: 电子健康记录（EHR）特征工程是临床研究和人工智能中的一个主要瓶颈，占数据科学家工作量的39-45%。这在心力衰竭中尤为明显，心力衰竭影响约670万美国成年人，需要整合碎片化的EHR数据与疾病特定的、基于指南的临床推理。现有的基于规则和基于大型语言模型（LLM）的方法只能实现部分自动化，可维护性和证据可追溯性有限。

Method: 开发了Nimblemind多智能体系统（nMAS），一个与证据链接、基于标准化的自动化心力衰竭特征工程管道，并在9个EHR源表中的500个虚拟患者记录上进行了评估。nMAS生成了132个结构和70个基于标准的聚合特征，经过结构完整性、标准合规性和来源验证，并由受限的LLM进行审计。

Result: 添加聚合特征后，HFrEF的保留AUROC从0.895提高到0.963，HFpEF表型从0.870提高到0.910。独立的LLM基于标准评估证据支持和方法论的合理性，特征得分达到最大分的81.5%。

Conclusion: 这些结果表明，对于复杂的 cardiovascular EHR 数据，自动可审计的特征工程是可行的，尽管评估仅限于单一机构的队列，需要外部验证。
