...
TL;DR: 提出了一种基于知识引导和精细动作分解的动作识别方法KDA，显著提高了动作识别的准确性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 在复杂场景中进行动作识别往往涉及多个并发精细动作，这使得对内部动作结构的建模变得具有挑战性。

Method: 我们提出了一种名为知识引导的解耦与原子动作（KDA）的方法，该方法利用精细的语义知识来增强动作表示并实现更精确的解耦。具体来说，我们使用大型语言模型（LLMs）将动作标签分解为原子动作，提供显式的时空语义。知识注入模块（KIM）首先将原子动作知识集成到视频特征中。在此基础上，知识解耦模块（KDM）进一步解耦原子动作知识，以产生更精确的语义指导以实现动作解耦。引入了知识解耦损失（KD Loss）来鼓励在KDM中更清晰地解耦知识组件。

Result: 实验表明，KDA提高了特征可辨别性，并在多标签动作识别基准测试中实现了最先进的性能。此外，KIM和KDM可以轻松集成到其他方法中，证明了其强大的通用性。

Conclusion: KDA方法通过结合知识引导和精细动作分解，有效提升了动作识别的精确度和性能。
...
TL;DR: 全身图像在BMI估计中优于半身和面部图像


<details>
  <summary>Details</summary>
Motivation: 人体特征如体重和身高是衡量其身心健康、日常生活习惯和财务状况的重要指标。BMI（身体质量指数）是一种广泛使用的指标，它编码了体重和身高的特征。BMI已被用作自我监测工具，并对个人生活有长期影响。例如，它可以帮助预测各种疾病的风险和估计寿命。然而，由于人体姿态、相机几何形状、个人外观和干扰背景的广泛变化，使用单个图像自动估计BMI是一项具有挑战性的任务。

Method: 本研究探讨了使用深度神经网络通过采用不同的模态（包括RGB、深度图、姿态亲和图和边缘图）来预测BMI、体重和身高，从而解决自动BMI估计的挑战。我们提出了一种新的数据集，包含6105张图像，具有身高、体重和BMI的地面真实标签。该数据集在野外收集，包含来自各种族群的图像，分布在不同的年龄组和性别中。数据集包括正面、背面、全身、半身、侧面姿势、带有不同背景和尺度变化的镜子自拍，可能包含隐藏部分或全部面部的伪影。我们仅使用不同CNN骨干（包括VGG、DenseNet和ResNet）的全身、半身和面部图像进行了广泛的实验。

Result: 实验结果表明，与半身和面部图像相比，全身图像在野外产生了更好的结果。

Conclusion: 全身图像在BMI估计方面优于半身和面部图像，这表明在野外场景中，全身图像可以提供更准确的健康评估。
...
TL;DR: DVPSFormer是一种高效的4D场景理解统一在线架构，提高了机器人感知的性能。


<details>
  <summary>Details</summary>
Motivation: Safe autonomous navigation requires a holistic understanding of dynamic environments, necessitating the simultaneous estimation of metric depth, semantic segmentation, and instance trajectories.

Method: We propose DVPSFormer, a unified online architecture designed for efficient 4D scene understanding. Central to our approach is explicit scene discretization (ESD), a novel mechanism that leverages segmentation queries to represent foreground and background regions, enabling a discrete-to-continuous (D2C) depth head to decode metric depth in a single pass. Furthermore, we propose an online majority voting (OMV) mechanism that exploits temporal consistency to refine classification during instance tracking.

Result: DVPSFormer establishes a new state-of-the-art on the Cityscapes-DVPS and SemKITTI-DVPS benchmarks, offering a streamlined solution for online robotic perception.

Conclusion: DVPSFormer is a unified online architecture for efficient 4D scene understanding, which improves the performance of robotic perception.
...
TL;DR: Rad-JEPA 3D通过预测遮挡视图的潜在特征来学习体积CT表示，显著提高了3D医学图像分析的性能。


<details>
  <summary>Details</summary>
Motivation: 自监督预训练在3D医学图像分析中至关重要，但现有的体积编码器无法保留下游推理所依赖的粗略空间和几何结构，限制了其在器官解耦、异常检测和空间理解方面的性能。

Method: 提出Rad-JEPA 3D，一个联合嵌入预测框架，通过预测从遮挡视图中的完整扫描的潜在特征来学习体积CT表示。核心是一个混合H-Mamba编码器，融合了Mamba状态空间分支和分组查询注意力分支，通过轻量级的按令牌路由器结合。为了提高中间表示的质量，进一步提出了隐藏状态正交正则化（HSOR），该正则化通过调整学生-教师隐藏状态和减少编码器中的特征冗余来对齐。这种层状正则化产生了更一致和具有判别性的体积表示，从而提高了器官识别和空间推理任务的性能。

Result: 在约120,000个CT扫描上预训练，Rad-JEPA 3D尽管体积紧凑（只有4.0B个总参数），但达到了最先进的性能：在封闭式VQA上与最先进的方法具有竞争力，在Spatial-Med基准测试上取得了最佳平均空间推理分数。消融研究证实，混合块和HSOR具有互补的增益，并且诱导的空间结构可以替代原始语言模型规模在体积推理任务上。

Conclusion: Rad-JEPA 3D通过引入混合编码器和HSOR，在3D医学图像分析中取得了显著的性能提升。
...
TL;DR: WildShadowRemover：一种鲁棒的视频阴影去除框架，在真实世界场景中表现出优异的性能。


<details>
  <summary>Details</summary>
Motivation: 由于复杂的光照、多样的阴影外观和有限的训练数据，在野外进行视频阴影去除仍然具有挑战性。尽管它在许多视觉和图形应用中非常重要，但在非约束的真实世界场景中，它仍然被广泛未探索。

Method: 提出WildShadowRemover框架，该框架通过LoRA微调对预训练的视频扩散模型进行适应，以实现鲁棒的视频阴影去除。引入细节注入模块和阴影掩码引导的频率分解调制模块，以选择性地恢复高频纹理同时抑制阴影伪影。利用Depth Anything 3的单目深度先验，在具有挑战性的光照条件下提供几何感知的指导。构建WildShadow，一个大规模成对视频阴影去除数据集和基准，涵盖多种合成场景。

Result: 实验表明，该方法在阴影去除质量和时间一致性方面优于现有方法，能够生成具有优越视觉质量和强大泛化能力的时序一致的无阴影视频。

Conclusion: WildShadowRemover在野外视频阴影去除方面取得了显著的成果，为相关应用提供了有效的解决方案。
...
TL;DR: 联邦物理信息神经网络在脑肿瘤诊断中显示出优异的性能，同时遵守隐私规则，并提高了诊断准确率。


<details>
  <summary>Details</summary>
Motivation: 诊断脑肿瘤如胶质瘤、脑膜瘤和垂体腺瘤时，静态成像方法无法捕捉肿瘤生长、组织位移或刚度变化。深度学习模型通常需要在一个地点汇集患者数据，这与隐私规则如GDPR和HIPAA相冲突，并限制了跨机构的泛化。

Method: 提出了一种结合联邦学习和基于线性弹性方程的物理信息损失的物理信息神经网络。三个模拟的临床地点在每个地点使用物理信息损失训练患者特定的MRI数据，并通过FedAvg协议在中央服务器上共享模型权重。

Result: 联邦模型达到91.4%的整体准确率，非联邦基线模型为90.0%，肿瘤类别的平均AUC为0.985，垂体肿瘤准确率从85.6提高到94.5%。训练产生了平滑、无发散的位移场，与预期的组织变形一致。

Conclusion: 联邦训练可以与基于物理的约束相结合，而不会对性能产生重大损失。
...
TL;DR: BG-REAL是一个针对背景操纵检测和定位的基准，旨在补充现有的图像操纵定位基准。


<details>
  <summary>Details</summary>
Motivation: 背景操纵是一个实际但未充分规定的图像取证设置，目前的评估主要强调以对象为中心的复制移动、拼接或通用合成编辑。

Method: 提出BG-REAL，一个用于背景操纵检测和定位的公共真实数据锚定基准包。该基准包从Open Images V7实例分割数据源构建，包含7000个处理样本，覆盖6个编辑家族，匹配真实控制样本，源组分割，掩码和泄漏QA，599个人辅助质量控制行，三个完成的外部基线（TruFor，MVSS-Net和HiFi-Net），以及五个种子模型评估。

Result: BG-REAL在整体准确率之外，使用匹配的真实控制诊断来衡量基线错误地将重新编码的真实图像分类为操纵的频率；误报率从0.57（TruFor，最低）到1.00（一些弱或掩码信息基线）不等，表明重新编码的伪影是基线共享的捷径风险，而不是任何特定模型的特定问题。

Conclusion: BG-REAL作为一个以背景操纵为重点的基准，是对通用图像操纵定位基准的补充，而不是一个完全基于真实数据或通用目的的基准。
...
TL;DR: 提出了一种新的图像重建方法，可以直接从图像中重建适用于模拟的几何形状，消除了计算机视觉和数值分析之间的障碍。


<details>
  <summary>Details</summary>
Motivation: 图像重建方法在恢复三维几何形状方面取得了进展，但其表示形式不适合数值模拟。

Method: 提出了一种名为FORGE-SIM的方法，直接从稀疏定位RGB图像中重建多块B样条边界表示，无需人工干预。通过优化样条表示本身，该方法产生紧凑、光滑、密封的几何形状，与计算机辅助设计和模拟工作流程兼容。此外，还引入了一种将观察到的场（如热状态和语义信息）投影到重建模型上的策略，使其能够立即用于模拟。

Result: 获得的模型质量足够高，可以进行热模拟和模态分析。通过在单个优化框架中统一图像重建和模拟准备建模，消除了计算机视觉和数值分析之间长期存在的障碍。

Conclusion: 这项工作将使模拟驱动的设计、检查和数字孪生应用的新工作流程成为可能。
...
TL;DR: LumaGuide is a new framework for HDR image and video generation, which can achieve controllable generation without retraining diffusion models.


<details>
  <summary>Details</summary>
Motivation: Pretrained diffusion models generate realistic images but are constrained by the statistical biases of their training data, limiting their ability to produce high dynamic range (HDR) content.

Method: Introduce LumaGuide, a training-free framework for distribution shaping in diffusion models. Control luminance distributions in perceptually uniform PQ space.

Result: Aligning luminance histograms is sufficient to induce HDR-consistent behavior, including coherent highlights and preserved shadow detail, while maintaining semantic fidelity. LumaGuide enables flexible specification of target distributions. Extends naturally to video generation with temporal consistency constraints. Achieve controllable generation by directly shaping output distributions at sampling time, without retraining diffusion models.

Conclusion: LumaGuide is an effective and flexible framework for HDR image generation and video generation with temporal consistency constraints, which can achieve controllable generation without retraining diffusion models.
...
TL;DR: 通过数据集扩展和教师微调，实现了轻量级鹰类物种分类，提高了召回率，降低了误分类率，并实现了实时边缘部署。


<details>
  <summary>Details</summary>
Motivation: 为了在风力涡轮机碰撞缓解中实现实时边缘部署，研究轻量级鹰类物种分类。

Method: 使用DINOv2-L作为教师，提炼出三个轻量级学生（MobileNetV4，ViT-Small和EfficientNet-B0）。通过视频帧提取增加Steller's Sea Eagle图像数量，扩大数据集至12,519张图像。在视频和源图像级别的分组中，三个学生的集成在五个蒸馏种子上实现了0.935 +/- 0.004的宏观召回率。在1,258张与之前训练图像不同的图像子集上，白尾海雕的召回率提高了38.6个百分点，将其误分类为Steller's Sea Eagle的错误率从61%降至15%。在NVIDIA Jetson Orin Nano上使用TensorRT FP16部署EfficientNet-B0，包括主机设备传输，达到3.19 ms/image（313 images/s），与FP32的argmax一致性为99.95%。在五个种子的控制比较中，蒸馏（与CE-only相比）以及从DINOv2-L到DINOv3-L教师的转变都没有带来明显的集成级改进；主要收益来自数据集扩展和教师微调。

Result: 在保持97.5%教师宏观召回率的同时，实现了参数减少到八分之一的效果。在独立测试集上，白尾海雕的召回率显著提高，误分类率降低。在NVIDIA Jetson Orin Nano上实现了高效的实时性能。

Conclusion: 数据集扩展和教师微调是提高分类性能的关键因素。轻量级模型在保持性能的同时，实现了实时边缘部署。
...
TL;DR: 基于基础模型的特征在预测头颈癌远处转移风险方面表现出色，减少了专业知识的需求。


<details>
  <summary>Details</summary>
Motivation: 预测头颈癌患者的远处转移风险，以实现及时干预，改善治疗效果。

Method: 使用RADCURE数据集中的2327名患者的术前CT图像，创建包括放射组学、基于深度学习的特征和CT基础模型提取的特征在内的三个特征集。这些特征集在多层感知器（MLP）中单独用于预测远处转移风险。

Result: 基于CT基础模型的模型优于基于放射组学和基于深度学习的模型，分别达到0.791、0.772和0.753的AUC值。基于CT基础模型的模型性能与结合使用放射组学和基于深度学习特征的模型相当，后者的AUC值为0.794。

Conclusion: 基于基础模型的特征为传统放射组学提供了一种有前景的替代方案，同时减少了领域专业知识的需求和大量标注数据集的需求。它们的最小预处理需求也使它们成为一种更易于访问和可扩展的选项。
...
TL;DR: HeteroPROMPT：一种实时、隐私保护且高效的异构协作感知框架。


<details>
  <summary>Details</summary>
Motivation: 为了提高自主系统的环境感知能力，通过共享传感器数据、中间特征和检测结果。然而，在实际部署中，协作车辆通常使用异构的传感器、感知模型、数据集和训练域，导致特征空间偏移，从而降低了下游融合和检测的性能。现有的方法通常需要重新训练融合和检测组件或引入模态特定的特征解释器。这些方法难以扩展到新加入的代理，并且通常需要访问专有元数据，从而引发隐私问题。

Method: 提出了HeteroPROMPT，一个实时且具有隐私保护功能的异构协作感知框架。HeteroPROMPT通过模块化提示和轻量级学习调整，快速将每个异构代理的特征与以自我为中心的统一特征空间对齐，同时保持代理编码器和协作融合与检测堆栈冻结。其基于视觉提示的训练和推理通过低计算开销，对通道和空间位置进行Bird's Eye View (BEV)特征的调制。为了实现无元数据部署，一个自动编码器学习紧凑的统一表示，并从共享特征中提取模态线索，使实时模态分类和路由到适当的HeteroPROMPT模块成为可能，而不会泄露专有代理信息。

Result: 在OPV2V-H和V2XSet数据集上的实验表明，HeteroPROMPT在保持与最先进的异构CP方法相当的平均精度的情况下，使用了数量级的可训练参数。这提供了一种可扩展且实用的CP解决方案。所提出的模态分类器在部署期间从紧凑特征中预测加入代理的模态，准确率超过99.99%。

Conclusion: HeteroPROMPT是一个有效且实用的异构协作感知框架，提高了自主系统的环境感知能力，同时解决了隐私问题。
...
TL;DR: Eddeep：一种快速而准确的扩散MRI涡流畸变校正方法。


<details>
  <summary>Details</summary>
Motivation: 解决扩散MRI中涡流畸变校正的问题，提高微结构分析的准确性。

Method: 提出了一种基于深度学习的Eddeep框架，包括监督图像翻译网络和无需监督的配准网络。

Result: Eddeep在多个互补指标上实现了与FSL Eddy相当的质量，同时大幅减少了推理时间。

Conclusion: Eddeep能够提供准确且高效的涡流畸变校正，支持更快的扩散MRI处理流程的发展。
...
TL;DR: MoSAIC是一种用于部分局部运动风格迁移的潜扩散框架，通过改进响应-保留权衡，提高了选择性可控部分局部运动编辑的效果。


<details>
  <summary>Details</summary>
Motivation: 为了编辑角色运动，需要从参考动作中提取手势或步态，同时保留源动作、时间、根轨迹和未选中的身体区域。然而，现有的运动数据集很少提供任意部分局部内容的参考组合配对目标，并且自重建训练可能会让扩散模型重现内容运动，但会低估已路由的参考。

Method: 提出MoSAIC，这是一种潜扩散框架，用于部分局部参考条件下的运动风格迁移。MoSAIC通过解剖区域分解内容和参考特征，通过单独的条件路径保留根轨迹，并将用户选择的参考路由到各个身体部位。其核心贡献是对齐干预监督，通过控制局部变换构建同步参考和反事实目标，使所需的区域响应和要保留的运动在训练过程中直接可观察。

Result: 在包含128个动作和896个路由条件的冻结评估中，部分掩码路由将保留区域误差从70.64 mm降低到66.45~mm，将匹配噪声的目标泄漏从18.08 mm降低到9.88 mm，相对于全身路由，同时保留正选择区域响应。匹配预算延续研究进一步表明，保留对齐干预监督可以使所选目标响应提高8.8%，所需路由影响集中度提高2.0个百分点。这些结果表明，MoSAIC改善了选择性可控部分局部运动编辑所需的有效响应-保留权衡。

Conclusion: MoSAIC通过改进响应-保留权衡，提高了选择性可控部分局部运动编辑的效果。
...
TL;DR: Zero-Fi是一种基于Wi-Fi的零样本人体活动识别方法，通过信号语言对齐扩展了Wi-Fi传感的应用范围。


<details>
  <summary>Details</summary>
Motivation: 传统的基于Wi-Fi的人体活动识别方法通常假设一组已知的活动，并且需要每个目标类别的标记Wi-Fi样本，这限制了它们识别未见过的活动的能力。

Method: Zero-Fi是一个对比信号语言对齐框架，用于零样本Wi-Fi人体活动识别。它从互补的Wi-Fi信号特征中学习统一的表示，并将它们与自然语言活动描述的语义表示对齐在共享嵌入空间中。这种跨模态对齐使Zero-Fi能够识别新的活动类别，而无需为这些类别提供标记的Wi-Fi样本或模型适应。

Result: 在大型公共基准数据集上的实验表明，Zero-Fi能够有效地进行零样本识别，突出了信号语言对齐扩展Wi-Fi传感超出预定义活动类别的潜力。

Conclusion: Zero-Fi通过信号语言对齐实现了零样本Wi-Fi人体活动识别，为识别未知活动提供了新的解决方案。
...
TL;DR: 提出了一种新的WLI/NBI病变分割方法，通过可靠性感知复域融合，提高了分割性能。


<details>
  <summary>Details</summary>
Motivation: 由于视角变化、组织变形和手持采集的顺序性，白光成像（WLI）和窄带成像（NBI）的配对观察通常在空间上错位。这导致直接WLI/NBI融合容易混合非对应区域，甚至可能降低病变边界处的分割质量。

Method: 提出了一种可靠性感知复域融合框架，用于配对但未注册的WLI/NBI病变分割。该框架首先建立拓扑正则化特征对应，然后进一步估计跨模态对应中的可靠性。在可靠性的引导下，模型选择性地在可学习的复数表示中融合WLI和NBI特征。在这个表示中，WLI衍生的线索主要提供与外观相关的幅度响应，而NBI衍生的线索提供与结构敏感的相位响应。

Result: 在配对的WLI/NBI内窥镜数据集上的实验表明，所提出的可靠性感知注册基础和复域融合一致地提高了病变分割性能。角色反转和模块消融研究进一步验证了模态角色设计和可靠性引导跨模态交互的必要性。

Conclusion: 该方法有效解决了WLI和NBI融合中的空间错位问题，提高了病变分割的准确性。
...
TL;DR: TIDE通过降低计算成本，使密集、同质目标的实时跟踪成为可能。


<details>
  <summary>Details</summary>
Motivation: 跟踪密集、同质的目标，如集群鱼，对多目标跟踪来说是一个主要挑战，因为个体间具有极端的同质性、严重的物理聚类和快速的非刚性形变。

Method: 我们提出了Tracking Identities with Dual-branch Elasticity (TIDE)。TIDE绕过昂贵的外观线索，利用自适应几何对应IoU，这是一种利用空间和结构一致性来稳健地处理复杂形态变化的关联机制。

Result: 在MFT-Edge基准测试中，我们的轻量级L分支仅使用20.47G FLOPs就实现了28.43的HOTA，与SU-T等上限相比，计算量减少了38.7倍，直接促进了实时边缘部署。我们的可扩展S分支建立了29.98的HOTA，成功弥合了高精度云分析和高效边缘跟踪之间的差距。

Conclusion: TIDE在保持高精度的同时，显著降低了计算成本，使实时边缘部署成为可能。
...
TL;DR: FAS-R1是一个两阶段的MLLM框架，提高了FAS的性能和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 为了提供更全面的对抗伪造（FAS）功能，包括真伪判断、攻击类型识别和伪造区域定位。

Method: 提出了一种两阶段的推理导向的多语言语言模型（MLLM）框架FAS-R1，结合了冷启动监督微调和特定于FAS的GRPO后训练。

Result: FAS-R1在真实度准确性、攻击类型准确性和伪造区域定位方面表现出色，同时优于对比系统在跨域真实度泛化和答案-理由质量方面。

Conclusion: FAS-R1是一个高效且准确的FAS框架，为真实度分类、攻击类型识别和伪造区域定位提供了强大的支持。
...
TL;DR: HERMES是一种针对放射治疗的三项任务的单一容器化算法，表现出良好的性能。


<details>
  <summary>Details</summary>
Motivation: 针对放射治疗靶区分割、恶性分期和无事件生存期的三个任务，提出了一种名为HERMES的单一容器化算法。

Method: 使用10折集成STU-Net小网络进行分割，预测掩码驱动下游任务。从预测掩码中提取与AJCC/UICC 7th-edition放射学N/T分期大小和数量轴对齐的紧凑几何特征。结合深度和临床风险专家进行预后分析。

Result: 在内部交叉验证中，这些特征将N期平衡准确率从0.691提高到0.720。HERMES在HECKTOR 2026验证排行榜上取得了0.6454的加权分数，并进入测试阶段。

Conclusion: HERMES在放射治疗靶区分割、恶性分期和无事件生存期方面表现良好。
...
TL;DR: 提出了一种名为SATATrack的UAV反无人机跟踪方法，该方法结合了语义感知和时间自适应机制，在多个任务中实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: UAV反无人机跟踪是一个低空安全任务，需要使用移动观察无人机上的摄像头来定位敌对无人机。由于摄像头平台和目标同时移动，与传统无人机跟踪和地面反无人机跟踪不同，这导致快速视角变化、运动模糊、尺度变化和视觉相似的干扰，使得可靠的视觉匹配变得困难。

Method: 提出了一种名为SATATrack的语义感知时间自适应框架，该框架引入了语义感知上下文传播（SACP），并使用目标描述来引导时间上下文在主干阶段的传播，以在快速外观变化下保持目标身份。在训练期间，使用辅助对比正则化器来抑制对语义相似背景区域的响应。在推理期间，使用时间感知分布对齐（TADA）在线对齐特征分布，而不更新模型参数，将最近帧估计与训练时间统计相结合以提高稳定性。

Result: SATATrack在UAV-Anti-UAV基准测试中实现了最先进的性能，同时在反无人机和无人机目标跟踪任务中保持竞争力。

Conclusion: SATATrack是一种有效的UAV反无人机跟踪方法，通过引入语义感知和时间自适应机制，提高了跟踪的准确性和鲁棒性。
...
TL;DR: EgoSafe-Bench provides a benchmark for LVLMs to improve forensic reasoning in video understanding.


<details>
  <summary>Details</summary>
Motivation: Real-world visual safety understanding requires causal reasoning under epistemic uncertainty.

Method: Introduce EgoSafe-Bench, a benchmark for forensic reasoning in egocentric safety scenarios.

Result: EgoSafe-Bench reveals significant perception-reasoning decoupling in LVLMs, showing models often achieve high descriptive scores but struggle in causal reasoning and logical closure.

Conclusion: EgoSafe-Bench challenges LVLMs and fosters development of logically robust video understanding systems.
...
TL;DR: 提出CineWeaver，实现无训练的长视频生成，具有高质量、一致性、稳定性。


<details>
  <summary>Details</summary>
Motivation: Cinematic video generation is challenging for text-to-video diffusion models due to concurrent requirements on multi-shot generation, fine-grained controllability over characters and scenes, and long-form generation across extended temporal horizons.

Method: Propose a unified framework named CineWeaver to achieve reference-controllable multi-shot long-video generation without retraining. Manipulate positional encoding and attention patterns to break temporal continuity during inference to enable clear shot transitions using pretrained video diffusion models. Extend the proposed framework with a shot-routed reference conditioning mechanism for per-shot fine-grained controllability, and develop an anchor memory mechanism to allow long-form generation with consistent global appearance cues.

Result: CineWeaver produces high-quality cinematic videos of long durations with consistent identities, stable global appearance, and clear shot transitions.
...
TL;DR: TrailNet：一种基于眼动追踪的医学图像分割新方法，显著提高了分割性能。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割高度依赖于劳动密集型和耗时耗力的像素级标注。眼动追踪提供了一种成本效益高的解决方案，可以自然地整合到临床工作中。

Method: 提出了一种名为TrailNet的轨迹引导的不确定性感知网络，该网络利用眼动监督的医学图像分割，从空间语义建模到时间上下文，通过联合利用注视点和轨迹。具体来说，所提出的轨迹引导时空编码器建模时间上下文，并与图像空间语义建立互补的交互，以增强目标感知。此外，多尺度不确定性解码器利用类别互斥约束来产生确定性的预测，并减轻由噪声引起的监督不确定性。为了实现无眼动推理，我们进一步引入了一种循环蒸馏策略，通过教师-学生网络转移特征级别的知识。

Result: 在两个公共数据集上的实验结果表明，TrailNet优于最先进的方法，分别实现了81.25%和81.85%的Dice分数。

Conclusion: TrailNet通过引入轨迹引导和不确定性感知机制，在医学图像分割任务中取得了显著的性能提升。
...
TL;DR: MedARC improves diagnostic performance by reducing visual-token overhead and inference time.


<details>
  <summary>Details</summary>
Motivation: Existing token compression methods increase the risk of removing clinically relevant regions.

Method: Proposes MedARC, a unified, training-free framework for Adaptive Redundancy Compression of visual tokens in 3D medical VLMs.

Result: MedARC preserves informative tokens and improves diagnostic performance.

Conclusion: MedARC reduces visual-token overhead and inference time while preserving or improving diagnostic performance.
...
TL;DR: 计算路径提供了广泛有用的可靠性信号，其价值由模型组织和遇到的偏移量共同决定。


<details>
  <summary>Details</summary>
Motivation: 研究最终表示是否丢弃证据，以及该证据是否可以改善OoD检测和图像分类。

Method: 保留样本身份，研究连接连续状态之间的转换，将类一致性传输与输入特定创新分离，并协调运动与关系重组。

Result: 这些路径显示出强大的样本特定连续性和架构特定深度配置文件，这些配置文件在数据集之间重复出现。

Conclusion: 计算路径提供了广泛有用的可靠性信号，其价值由模型组织和遇到的偏移量共同决定。
...
TL;DR: 3DGBGS通过自适应颗粒球组织，提高了新视图合成的效率和渲染质量。


<details>
  <summary>Details</summary>
Motivation: 3DGS和GBC在自适应表示方面具有天然兼容性，3DGS的效率部分源于GBC的生成原理。

Method: 提出3DGBGS，自适应地将SfM点云划分为3D颗粒球，使用较大球体紧凑地表示平滑和冗余区域，使用较小球体保留复杂几何和局部细节。

Result: 3DGBGS将初始和最终锚点分别减少了37.1%和10.0%，平均模型存储减少了9.8%，同时保持可比的渲染质量。

Conclusion: 3DGBGS是一种紧凑的基于锚点的框架，可以有效地进行新视图合成。
...
TL;DR: 本研究通过深度学习模型（如ResNet50）对肺部疾病进行分类，结果显示ResNet50在准确性方面表现最佳。


<details>
  <summary>Details</summary>
Motivation: 随着呼吸系统疾病病例数的增加，迫切需要早期检测和准确诊断。卷积神经网络在利用影像学检查诊断疾病时显示出有希望的结果。

Method: 本研究调查了应用深度学习算法（如VGG16、VGG19和ResNet50）基于X光图像对肺部疾病进行分类的潜力。对上述模型的性能进行了详细分析，以评估它们如何分类各种类型的肺部疾病，包括肺炎、肺结核、肺癌和正常肺部。为此，这些深度学习模型在大量X光图像上进行了训练。

Result: 结果表明，虽然所有三种模型都提供了良好的结果，但与其它模型相比，ResNet-50由于其效率和高度的准确性表现最佳。

Conclusion: 我们相信，这些深度学习模型可以在未来成功应用于肺部疾病的诊断实践中。这有助于早期疾病检测并改善患者预后。
...
TL;DR: 检测器在不同部署之间不可靠迁移，CEG可以改善性能。


<details>
  <summary>Details</summary>
Motivation: 传统的基于基准排名选择OOD检测器的假设不成立。

Method: 通过控制的可移植性审计，比较了17个分布数据集、3个视觉语言模型和7个代表性的零样本OOD检测器。

Result: 检测器排名在不同部署中会反转，每个检测器至少在一个域上超过80% FPR95。CEG减少了检测器的敏感性并提高了GL-MCM和MCM的性能。

Conclusion: 没有单一检测器能够在部署之间可靠地迁移，因此引入了互补证据守卫（CEG）来提高检测器的性能。
...
TL;DR: R-SLPR是一种基于区域的新型点云注册框架，它通过将问题分解为多个阶段来克服尺度不匹配问题，并在ModelNet40上实现了最精确的注册结果。


<details>
  <summary>Details</summary>
Motivation: 在机器人系统中，点云注册对于三维感知至关重要。然而，经典注册算法在将包含有限、不完整或模糊几何线索的源点云与参考点云对齐时失效。将小型、部分点云注册到显著更大的全局参考点云的挑战在现实世界部署中普遍存在，但现有基于学习的解决方案未能充分解决这个问题，因为它们通常假设可比的规模和显著的重叠。

Method: 我们提出了一种基于区域的点云小到大注册框架（R-SLPR），这是一种新颖的三阶段架构，它将尺度不匹配的注册问题重新定义为一系列区域提议、区域匹配和迭代细化。R-SLPR显式地识别候选区域，在估计刚性变换之前，确保即使在高尺度不匹配的情况下也能进行鲁棒的对接。该框架引入了斐波那契网格分割方法和对比学习目标，以有效地生成和匹配局部几何块。在此基础上，一个新颖的级联锚点选择和细化算法迭代地对齐源和目标区域，以最大限度地提高精度。

Result: 在ModelNet40上的广泛评估表明，R-SLPR建立了一个新的最精确的准确度标准，优于先前的方法，并将位置和旋转的均方根误差分别降低到0.009和1.104。

Conclusion: R-SLPR通过将尺度不匹配的注册问题分解为区域提议、区域匹配和迭代细化，实现了高精度的点云注册。
...
TL;DR: 提出了一种用于3DGS场景理解的多模态质量评估框架，提高了质量评估的准确性。


<details>
  <summary>Details</summary>
Motivation: 3D Gaussian Splatting在新型视图合成和3D场景重建中表现出色，但对可靠的质量评估提出了更高的要求。

Method: 开发了一个用于3DGS场景理解的多模态质量评估框架。包括：1. 引入一个3D感知质量表示学习框架，通过增强基于VGGT的编码器并添加一个专门的质量头。2. 构建一个基于Qwen的多模态大语言模型（MLLM）的 grounding 多模态推理机制。

Result: 该框架能够捕捉跨视图一致性，并通过联合建模深度和点云相关的结构信息，学习结构感知的质量表示。

Conclusion: 该研究提出的方法能够有效地进行3DGS场景的质量评估。
...
TL;DR: 提出了一种高效的训练框架DVP，通过解耦视觉处理显著降低了MLLMs视觉指令微调的计算成本。


<details>
  <summary>Details</summary>
Motivation: 为了解决多模态大型语言模型（MLLMs）在视觉指令微调中计算成本高且不必要的问题，

Method: 提出了一种名为Decoupled Visual Processing（DVP）的效率训练框架，该框架将预训练LLM的上层解码器层替换为轻量级的独立可训练的单一Transformer块，专门用于视觉标记处理。

Result: 在LLaVA-1.5框架上进行的实验表明，DVP在MME、POPE和ChartQA基准测试中实现了具有竞争力的性能，同时仅训练了总参数的一小部分，表明MLLMs中的视觉表示可以通过解耦、参数高效的路径有效地学习。

Conclusion: DVP是一种有效的训练框架，可以显著减少MLLMs视觉指令微调的计算成本，同时保持良好的性能。
...
TL;DR: JEPADepth: A self-supervised monocular depth framework that improves performance over existing methods.


<details>
  <summary>Details</summary>
Motivation: Self-supervised monocular depth estimation relies on photometric reconstruction losses that couple depth, pose, and appearance assumptions.

Method: Proposes JEPADepth, a self-supervised monocular depth framework that incorporates a complementary training objective inspired by Image Joint-Embedding Predictive Architectures (I-JEPA) for self-supervised depth learning. Augments a standard photometric pipeline with a masked prediction loss computed in the representation space of a pretrained DINOv3 Vision Transformer encoder.

Result: Consistently improves performance over the same DINOv3-based photometric baseline on KITTI. Competitive with state-of-the-art transformer-based approaches and outperforms strong CNN-based baselines on the standard benchmark. Achieves the best or near-best performance among the compared methods on both Make3D and Cityscapes across multiple metrics in zero-shot transfer.

Conclusion: JEPADepth is an effective and competitive self-supervised monocular depth estimation framework.
...
TL;DR: 研究跨尺度知识转移，发现大语言模型融合主要是语言推理的转移。


<details>
  <summary>Details</summary>
Motivation: 为了研究不同能力在不同规模之间能否转移，以及不同能力之间的转移模式。

Method: 引入了Cross-Scale Directional Parameter Injection (CDPI)方法，通过线性探针分析异构多模态大语言模型融合过程中的跨尺度知识转移。

Result: 发现跨尺度异构大语言模型融合主要是语言侧推理的转移，而非广泛的能力继承。

Conclusion: 跨尺度异构大语言模型融合中，知识转移的选择性主要受能力依赖响应的影响，而二次曲率效应限制了有效转移的范围。
...
TL;DR: FakeIDet3-DB是一个综合数据库，用于检测身份证伪造攻击，其性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统的身份证明认证方法依赖于复杂的、高频的安全模式的结构完整性。然而，先进的生成式AI模型现在可以注入局部化、高保真的操纵，创建绕过标准验证的欺骗性攻击。训练鲁棒的图像取证模型来检测这些异常受到隐私法规的限制，迫使依赖于缺乏真实身份证复杂视觉模式的合成模板。

Method: 提出FakeIDet3-DB，第一个全面数据库，包含对真实政府颁发的身份证的数字操纵。使用先进的图像细化程序来抑制视觉瑕疵。为了符合严格的数据保护法规，采用基于补丁的框架。提出PACE算法，利用积分图映射和距离驱动的非极大值抑制（NMS）。

Result: 从超过6.4K张真实/伪造身份证中提取了近5.2M个补丁。在检测和定位攻击方面，所提出的FakeIDet3-DB使用最先进的模型进行广泛评估，所有模型都难以检测和定位来自生成和经典技术的攻击（检测方面32.45%的EER和83.48%的AUC-ROC）。

Conclusion: FakeIDet3-DB数据库能够有效地检测和定位身份证伪造攻击，为图像取证领域提供了重要的工具。
...
TL;DR: FPSGen是一个灵活的户外场景生成框架，通过独立构建点源和路径直化传输，实现了高质量的生成结果。


<details>
  <summary>Details</summary>
Motivation: 现有基于点的户外场景生成方法主要关注LiDAR条件下的补全。这种训练-推理不匹配继承了部分扫描的稀疏性和可见性偏差，导致遮挡区域的稀疏远距离区域和不完整几何形状。此外，依赖于部分扫描限制了当LiDAR观测不可用或由布局线索代替时的生成。

Method: FPSGen是一个灵活的框架，独立于部分扫描构建点源。首先，从主动线索预测带有密度、高度和掩码通道的鸟瞰图（BEV）先验。然后，从密度图采样以形成BEV支持的点源，实现无条件和条件初始化。接着，使用教师-学生近似最优传输方案，利用教师预测的端点学习一个诱导更直的传输路径的速度场。通过将BEV点源构建与路径直化传输相结合，FPSGen提供了一个统一的框架，用于无条件和灵活的线索条件场景生成。

Result: 在SemanticKITTI补全任务上，FPSGen实现了最先进的JSD和体素IoU性能，同时在单个点传输步骤中保持了强大的性能。在KITTI-360无条件生成任务上，它也在比较的方法中实现了最佳的覆盖率（COV）。

Conclusion: FPSGen是一个有效的户外场景生成框架，能够提供高质量的生成结果，并具有灵活性和鲁棒性。
...
### [43] [Forensic Reproducibility Audit of a Radiology Vision-Language Model Benchmark: From Intended Protocol to Released Artifact](https://arxiv.org/abs/2607.25589)
*Mateusz Kozłowski*

Main category: cs.CV

TL;DR: 对医疗图像AI基准进行审计，发现不一致性，撤回原始性能声明，并提出改进措施。


<details>
  <summary>Details</summary>
Motivation: 对医疗图像AI基准中的数据集、DICOM渲染、提示、供应商API、自动标签、统计代码、稿件和存储库发布的一致性进行审计

Method: 对保存的胸部X光片视觉-语言模型（VLM）试点进行回顾性法医可重复性审计

Result: 发现数据集分割成员身份未保留，未验证的提取器截断五个报告至4000个字符，以及重新构建一个包含369个完整病例发现块的共同队列后，Cochran的Q值从154.73变为182.29。

Conclusion: 撤回原始性能、排名、提示效果和临床声明，并指定机器可验证的控制措施，包括队列、DICOM渲染、提示和模型身份、调用状态、注释来源、关键分析和派生工件。
...
TL;DR: Genie Sim PanoWorld是一种高效的全景3D场景重建方法。


<details>
  <summary>Details</summary>
Motivation: 从单个360度全景图中重建高保真、自由导航的3D场景，无需场景优化或多视图捕获。

Method: 提出了一种名为Genie Sim PanoWorld的两阶段前馈管道，通过显式、可控制的全景视频桥接生成和重建。

Result: 在全景视频生成和下游3D重建方面优于基于几何条件的基线，同时泛化到未见过的室内场景。

Conclusion: Genie Sim PanoWorld在全景视频生成和3D重建方面表现出色，且适用于未见过的室内场景。
...
TL;DR: AnchorSteer通过改进去噪过程，有效提高了文本到图像扩散模型的精确对齐能力。


<details>
  <summary>Details</summary>
Motivation: 文本到图像扩散模型在视觉质量上表现出色，但在与复杂构图提示的精确对齐方面存在困难。

Method: 提出了一种名为AnchorSteer的训练免费框架，对初始化和去噪轨迹进行精细控制。

Result: 在GenEval和T2I-CompBench++上进行的大量实验表明，AnchorSteer在文本-图像对齐方面优于现有基线，同时保持了高视觉质量。

Conclusion: AnchorSteer通过改进去噪过程，有效提高了文本到图像扩散模型的精确对齐能力。
...
TL;DR: 提出了一种针对物理世界OFENs的实时攻击方法，有效提高了鲁棒性测试。


<details>
  <summary>Details</summary>
Motivation: 为了测试OFENs的鲁棒性，防止安全事故，提出了一种针对物理世界OFENs的实时攻击方法。

Method: 利用红外灯的隐蔽性，预先生成大量对抗样本，实时计算并动态显示，实现对受害系统的精确和针对性攻击，直接攻击物理世界中的受害模型。

Result: 实验结果表明，该方法在不同光照条件、不同物体运动速度和不同物体放置下，能够有效攻击OFENs，损害网络准确估计光流的能力。

Conclusion: 该方法有效提高了OFENs的鲁棒性测试，为安全应用提供了重要保障。
...
TL;DR: Visko Orbis 1.0：实时交互式长视频生成系统


<details>
  <summary>Details</summary>
Motivation: 提出 Visko Orbis 1.0，一个实时、交互式长视频生成模型。

Method: 基于精炼的块级流式生成器和流式视频上采样器，支持多语言提示和提示切换，具有多尺度记忆功能。

Result: 实现实时 4K 视频生成，在长视频生成领域获得最高整体偏好和时序稳定性评分。

Conclusion: Visko Orbis 1.0 是一个高效的实时交互式长视频生成系统。
...
TL;DR: 序列感知训练提高了基于事件SNN对象检测的性能。


<details>
  <summary>Details</summary>
Motivation: 许多基于事件的对象检测器未充分利用连续事件流中的时间信息。

Method: 提出Sequence-SOD，一种序列感知的SNN对象检测器，处理包含多个时间点标签的扩展事件序列。

Result: 在Gen1 Automotive Detection Dataset上，序列感知训练将mAP从单间隔训练的23.38提高至25.30（无增强）和26.88（事件数据增强）。

Conclusion: 序列感知训练是提高基于事件SNN检测性能的有益方向。
...
TL;DR: TPD通过无监督框架提高后期场景实现，保持时序一致性和视觉保真度。


<details>
  <summary>Details</summary>
Motivation: Text-to-video扩散模型在生成自然语言描述的时序内容时，往往无法实现后期场景。

Method: 提出了一种名为Temporal Prior Decoupling (TPD)的无监督框架，通过条件化早期段来构建时序反事实，并定义全提示和反事实轨迹之间的差异作为被抑制的信号方向。

Result: TPD显著提高了后期概念实现，同时保持了时序一致性和视觉保真度。

Conclusion: TPD在标准扩散采样中运行，无需重新训练，并且是分类器无关的，因此对骨干网络是透明的。
...
TL;DR: FreeShadow：无需训练的阴影去除方法，无需训练，泛化能力强，结果逼真。


<details>
  <summary>Details</summary>
Motivation: 现有阴影去除方法在泛化能力上受限，零样本方法则产生伪影且测试时间优化耗时。

Method: 提出FreeShadow，基于预训练的扩散模型，无需训练或优化即可利用扩散先验进行阴影去除。引入照明转移注意力（ITA）进行照明恢复，分析光照变化对扩散模型中的自注意力图和潜在高频特征的影响，并选择性地保留光照不变成分以保持内容保真度。此外，提出局部纹理保留重光照（LTPR）以减轻VAE压缩引起的局部纹理错位。

Result: 实验表明，该方法具有强大的泛化能力，并生成逼真的无阴影图像。

Conclusion: FreeShadow是一种有效的阴影去除方法，无需训练，具有强大的泛化能力和逼真的结果。
...
TL;DR: CASIAL是一种几何畸变鲁棒水印框架，在多种变换下表现优异。


<details>
  <summary>Details</summary>
Motivation: 深度学习水印对非几何失真的鲁棒性很强，但对几何变换的鲁棒性有限。

Method: 提出了一种名为CASIAL的几何畸变鲁棒水印框架，该框架包含覆盖图像感知的消息传播（CAS）策略和不变性对齐学习（IAL）模块。

Result: CASIAL在六种几何变换下，其鲁棒性比11个基线方法显著更强，同时保持了高视觉质量。

Conclusion: CASIAL在六种几何变换下，其鲁棒性比11个基线方法显著更强，同时保持了高视觉质量。它还在六种信号失真和四种光度变换下保持了具有竞争力的性能。
...
TL;DR: 本研究提出了一种新的方法，通过动态时间规整和专用神经网络架构，显著提高了在线手写轨迹的重建精度。


<details>
  <summary>Details</summary>
Motivation: 研究通过在线手写轨迹重建来促进人机交互，特别是针对配备传感器的数字笔在任意表面上书写并获取数字痕迹的方法。

Method: 提出了一种新的处理流程，将笔的传感器信号映射到对应的手写轨迹。该方法利用动态时间规整对信号进行预处理，并引入一种基于时间卷积网络架构的专用神经网络来重建在线轨迹。

Result: 开发了一个新的基准数据集，用于对方法进行评估，结果表明，与最著名的竞争对手相比，该方法具有显著的改进。

Conclusion: 本研究提出的方法在重建在线手写轨迹方面表现出色，为数字笔的应用提供了新的可能性。
...
TL;DR: Dualin improves image-to-text inversion and editing capabilities.


<details>
  <summary>Details</summary>
Motivation: Prompt inversion enables text-to-image diffusion models to generate desired images without extensive prompt engineering, but existing methods have limitations.

Method: Dualin, a two-stage method that jointly recovers the semantic prompt and latent noise of the target image.

Result: Dualin generates high-quality inverted prompts and achieves state-of-the-art image fidelity, and can establish a robust foundation for precise and controllable image editing.

Conclusion: Dualin effectively addresses the limitations of existing prompt inversion methods and improves image fidelity and editing capabilities.
...
TL;DR: 本研究提出了一种结合视觉和形态信息的多模态框架，用于鸟类骨骼的分类，为人工智能在动物考古学中的应用提供了新的思路。


<details>
  <summary>Details</summary>
Motivation: 人工智能在考古学中的应用潜力巨大，但在动物考古学中的应用，尤其是鸟类骨骼遗骸的识别方面，仍有限。

Method: 本研究提出了一种多模态框架，结合卷积神经网络图像分析和骨测量数据，对鸟类骨骼进行分类。使用超过10,000张来自多个博物馆和研究收藏的数据集，研究了两个分类任务：骨骼元素识别和家族级别的分类。

Result: 模型在骨骼类型分类上达到了86%的准确率，在家族级别分类上达到了51%的最高准确率和75%的前三名准确率。

Conclusion: 这些结果表明，在统一深度学习框架中结合视觉和形态信息是可行的，并为未来人工智能辅助动物考古学识别建立了方法论基础。
...
TL;DR: StatePlay：一种新的游戏世界模型，通过状态感知提高游戏生成保真度。


<details>
  <summary>Details</summary>
Motivation: 现有游戏世界模型在生成视觉真实感和交互式环境方面表现出色，但未能充分建模游戏状态动态，导致生成的游戏流程可能违反底层规则。

Method: 提出了一种名为StatePlay的新型状态感知游戏世界模型，该模型采用混合Transformer（MoT）架构，能够同时预测视觉内容和游戏状态，并优化每个分支以适应其模态。

Result: StatePlay在状态预测的平均归一化L1距离方面达到0.06以下，与未进行显式状态建模的模型相比，在生成的游戏流程中提高了18.6%的机制保真度。

Conclusion: 强调了状态感知游戏世界建模的重要性，并超越了像素级现实，实现了完整且机制上忠实于游戏生成。
...
TL;DR: 提出了一种新的点云数据集蒸馏方法，有效提高了分类准确率。


<details>
  <summary>Details</summary>
Motivation: 解决现有点云数据集蒸馏方法忽略长尾分布不平衡的问题。

Method: 自适应合成预算和3D长尾分布匹配

Result: 在ShapeNet55上，分类准确率提高了7.0个百分点。

Conclusion: 通过设计自适应合成预算和3D长尾分布匹配，显著提高了分类准确率。
...
TL;DR: 数据增强可以提高皮肤病变分类器的鲁棒性，混合策略和光度变换对OOD泛化最有用。


<details>
  <summary>Details</summary>
Motivation: Dermoscopic皮肤病变分类器在跨成像设备、光照和捕获伪影的域偏移下通常会失去准确性。

Method: 在多源ISIC存档集合上使用Derm7pt进行搜索，采用ConvNeXt-Large骨干和ROC-AUC。在病变-ID级别进行拆分，并将HAM10000和ISIC 2019-2020保留为占主导地位的源分离OOD测试。

Result: 最大的OOD增益来自混合策略，光度变换在最有用的OOD操作中占主导地位。在相同的保留来源的扩展池中，增益为+0.053（95% CI +0.045至+0.061，p<0.001），在四个训练种子中一致（每个种子的ROC-AUC：基线0.761-0.775，混合0.806-0.829）。在一个小的独立临床集合中，单个检查点的敏感性从0.591上升到0.818，但这是基于22个恶性病例，并且在种子之间并不持续。

Conclusion: 模拟真实域偏移来源的增强可能比最大化域内准确性更重要。由于策略是在评估它所使用的相同来源上选择的，因此在将这种效应量视为无偏之前，需要一种源分离的选择方案。
...
TL;DR: See2Think评估框架揭示了多模态模型视觉推理的依赖性和挑战。


<details>
  <summary>Details</summary>
Motivation: 分析多模态大型语言模型在推理过程中对视觉状态（如草图、注释、工具和中间图像）的依赖性，以及现有基准测试的局限性。

Method: 提出See2Think，一个包含See2ThinkBench和视觉思维动作（VAoT）的统一评估框架。See2ThinkBench包含1,200个开放式、视觉依赖性问题，涵盖12个任务类别，包括2D结构化、3D场景和现实推理。VAoT记录文本思维、视觉动作、渲染状态和后续推理。

Result: 发现视觉推理在模型和环境上高度依赖，没有单一设置在所有任务中占主导地位。过程分析显示，模型通常选择相关视觉操作，而忠实渲染是最大的瓶颈，高反馈获取并不一定转化为准确率提升。在相关任务损坏的反馈下，模型的准确性下降了超过10个百分点。

Conclusion: 视觉推理对模型和环境有很强依赖性，视觉操作选择对模型表现有显著影响，而渲染质量是影响推理准确性的关键因素。
...
TL;DR: 本文提出了改进的自回归视频蒸馏方法，通过引入分布评估协议和联合蒸馏，提高了生成质量、覆盖率和多样性。


<details>
  <summary>Details</summary>
Motivation: 分析现有自回归视频蒸馏方法的设计和局限性

Method: 引入分布评估协议和联合蒸馏方法

Result: 提高生成质量、覆盖率和多样性

Conclusion: 分布对齐在自回归视频蒸馏中至关重要
...
TL;DR: Ripple：一种实时高效的音频-视频生成系统，实现高质量与实时性的平衡。


<details>
  <summary>Details</summary>
Motivation: 音频-视频生成模型虽然质量很高，但存在高延迟，不适用于实时应用。尽管已经提出了几种流式音频-视频生成方法，但它们成本高昂且无法支持长形式生成。

Method: 提出了一种名为Ripple的实时音频-视频生成系统，该系统具有跨模态循环记忆机制。Ripple结合了固定长度滑动窗口注意力和特定模态的记忆状态，以有效地进行流式推理并保留长期上下文。此外，还引入了跨模态记忆交互来增强视听同步。为了有效地学习这种记忆增强模型，我们设计了三个阶段的训练方案：1）将双向音频-视频教师调整为块状因果注意力与模拟记忆；2）通过端到端蒸馏优化记忆构建和交互流程；3）应用针对流式音频-视频生成的在线强化后训练。

Result: Ripple在480P分辨率下达到约28 FPS，比教师快，同时能够进行连贯的长形式生成。在短视频和长视频基准测试中进行了大量实验，证明我们的性能优于现有的离线和在线联合音频-视频生成方法。

Conclusion: Ripple是一种高效的实时音频-视频生成系统，在保持高生成质量的同时，实现了实时性，为实时应用提供了新的解决方案。
...
TL;DR: BATS是一种高效的3D医学图像分割模型，通过混合分辨率处理实现内存节省和性能提升。


<details>
  <summary>Details</summary>
Motivation: 许多高性能的体素分割模型维护密集的多尺度特征图，导致高激活内存和推理成本。

Method: 提出了一种名为BATS的3D医学图像分割架构，该架构集中处理预测类别边界附近的精细分辨率。

Result: 在五个公开的CT和MRI数据集上，BATS实现了最高的LiTS Dice值，平均比最强的密集基线MedNeXt-L高0.37 Dice点。

Conclusion: BATS通过混合分辨率处理提供了一致的内存节省，而运行时间和准确性的提升取决于数据集边界密度。
...
TL;DR: Sci-ImageMiner基准和竞赛在科学图像理解和推理方面取得了重要进展。


<details>
  <summary>Details</summary>
Motivation: 提高科学图像理解和推理的准确性

Method: 多模态人工智能，包括视觉感知和领域特定推理

Result: 高级多模态模型在分类和摘要任务上表现良好，但在数据提取和科学推理方面表现不佳，特别是在视觉问答方面

Conclusion: Sci-ImageMiner基准和竞赛为科学图像理解和推理研究提供了一个严格的平台，并展示了最先进方法在具有挑战性和复杂性的研究领域的潜力。
...
TL;DR: SCALPEL 是一种基于 LLM 的语义跨模态对齐框架，显著提升了医学多模态表示学习性能。


<details>
  <summary>Details</summary>
Motivation: Medical VLP 框架存在局限性，如上下文窗口有限和轻量级文本编码器表示能力浅显。

Method: 提出 SCALPEL 框架，通过 LLM 驱动的编码器学习实现语义跨模态对齐。

Result: SCALPEL 在跨模态检索、零样本疾病分类和医学视觉问答方面取得最先进的性能。

Conclusion: SCALPEL 解决了现有 VLP 框架的瓶颈问题，提高了医学多模态表示学习的效果。
...
TL;DR: CinemaTraj：通过自然语言描述自动生成高质量电影风格相机轨迹。


<details>
  <summary>Details</summary>
Motivation: 自动从自然语言描述中生成具有电影表现力的相机轨迹，在房地产广告和虚拟旅游创建等领域具有很高的实用价值。

Method: 提出CinemaTraj框架，将相机轨迹规划重新构造成语言基础的空间推理问题。

Result: 在ScanNet++环境中评估CinemaTraj，产生与提示一致、无碰撞的轨迹，在提示对齐、轨迹质量和安全性指标上优于现有方法。

Conclusion: CinemaTraj框架在生成高质量的 cinematic 相机轨迹方面表现良好。
...
TL;DR: 提出了一种改进的YOLO-Pose模型，通过引入概率扩展和校准，提高了关键点定位和不确定性量化。


<details>
  <summary>Details</summary>
Motivation: YOLO-Pose模型在关键点定位方面高效，但不能量化相关的空间不确定性。

Method: 引入轻量级的后处理概率扩展，通过训练额外的概率头，预测每个关键点的输入相关的$2	imes2$散布矩阵，并进行高斯校准或Student-$t$校准。

Result: 实验表明，学习到的不确定性估计可以有效地进行关键点级可靠性排名，Student-$t$校准最能捕捉经验残差分布，基于不确定性的剪枝可以去除不可靠的关键点。

Conclusion: 该方法提高了YOLO-Pose模型在关键点定位和不确定性量化方面的性能。
...
TL;DR: PMA是一种新的框架，用于减轻MCIT中的投影仪级别遗忘问题，提高了多模态连续指令调整的性能。


<details>
  <summary>Details</summary>
Motivation: 在多模态连续指令调整（MCIT）中，视觉分布的转移和指令语义的演变导致共享投影仪漂移，从而产生投影仪级别的遗忘问题。

Method: 提出了一种名为渐进式多模态对齐（PMA）的框架，该框架使投影仪能够持续适应，同时保留先前学习的对齐。PMA通过轻量级表示描述符检测多模态分布的转移，并在需要时逐步扩展投影仪专家。可扩展的路由器根据多模态特征整合专家输出，而原始预训练的投影仪则保留作为稳定的对齐锚点。这种渐进式机制在保持稳定性和可塑性方面取得了平衡，具有亚线性参数增长，并可作为现有MCIT方法的无缝附加组件。

Result: 在两个最近的MCIT基准测试上进行了广泛的实验，结果表明，与PMA结合时，减轻投影仪级别的遗忘可以带来一致的收益，优于以前的最先进方法。此外，PMA可以扩展到各种MLLM主干网络，证明了其在MCIT方面的鲁棒性和广泛适用性。

Conclusion: PMA是一种有效的框架，可以减轻MCIT中的投影仪级别遗忘问题，提高多模态连续指令调整的性能。
...
TL;DR: 提出了一种新的RPC精炼流程，提高了卫星图像地理定位的准确性


<details>
  <summary>Details</summary>
Motivation: 提高卫星图像地理定位的准确性

Method: 提出了一种外观感知的RPC精炼流程，结合了学习到的局部特征匹配和全局图像描述符

Result: 在季节性多样化的WorldView-3图像上，该流程在无地面控制点的情况下提高了RPC精炼的相对精度，同时显著减少了匹配时间

Conclusion: 该方法提高了RPC精炼对时间序列外观变化的鲁棒性，使多时相卫星图像的有效利用成为可能
...
TL;DR: 本研究通过视频表示正则化方法，有效解决了视频世界模型中自回归视频漂移问题，提高了长视频生成的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 为了解决滑动窗口自回归推理中严重的误差累积问题，以及长期生成的不稳定性。

Method: 研究视频世界模型的内部表示动力学，并提出视频表示正则化方法。

Result: 该方法在VBench的美学质量和成像质量指标上分别提高了38.65到55.56和44.37到72.08。

Conclusion: 视频表示正则化方法有效提高了长视频生成的鲁棒性。
...
TL;DR: 提出了一种生成大规模标注合成训练数据的方法，用于工业缺陷检测，提高了检测性能。


<details>
  <summary>Details</summary>
Motivation: 工业质量控制中自动化缺陷检测的重要性，以及标注缺陷数据的稀缺性带来的挑战。

Method: 使用BlenderProc生成大规模标注合成训练数据，包括可配置的材料外观、相机模式和领域随机化，并生成自动的COCO格式标注。

Result: 评估了四种训练策略：仅合成、仅真实、混合和从合成权重微调。结果表明，从合成权重微调优于仅真实训练，混合训练在真实数据稀缺的情况下有效恢复性能。

Conclusion: 提出的方法使缺陷检测可扩展，无需大量真实标注数据集，适用于设备上的工业检测。
...
TL;DR: VCA provides a method to analyze the effectiveness of spatial benchmarks and identifies uncredited correct decisions.


<details>
  <summary>Details</summary>
Motivation: Closed yes/no spatial benchmarks can reward a correct answer even when the image adds little support beyond no-image contexts.

Method: Visual Credit Audit (VCA) separates two estimands: whether the benchmark image gives the model's declared decision more support than text-only and blank controls, and whether the model responds to relation-specific visual evidence.

Result: 12.73-26.25% of decisions are correct yet uncredited. Matched same-split image permutation reduces D-CC by 21.25-47.80 points. Response to relation reversal spans 81.57-100.00%, while 32.11% pooled change answer.

Conclusion: VCA decomposes benchmark success into correctness, additional image support, and relation-consistent response.
...
TL;DR: SciFigQual-Bench和SFQ-Agent在科学图像质量评估方面取得了显著成果。


<details>
  <summary>Details</summary>
Motivation: 现有图像质量评估方法主要针对自然照片或AI生成内容，不适用于科学论文。

Method: 提出SciFigQual-Bench，一个全文上下文基准，评估科学图像的五个维度（清晰度、布局、标题适配、上下文相关性和误导风险）。

Result: SFQ-Agent（F3）与GPT-5.6-Sol结合，在测试子集eval1200上实现了最低的平均绝对误差（0.418）和最高的一致性率（93.4%）

Conclusion: SciFigQual-Bench和SFQ-Agent为科学图像质量评估提供了一种新的方法。
...
TL;DR: 该方法在临床环境中实现了鲁棒的AES分割。


<details>
  <summary>Details</summary>
Motivation: 现有的方法在临床环境下稳健性和泛化能力有限，难以有效利用预训练模型。

Method: 基于蒸馏的DINOv3 ViT-Small骨干网络和步骤关注特征细化模块

Result: 在内部数据集上，该方法实现了85.55%的mIoU，并且在四个未见的公共数据集上显示出最强的鲁棒性。

Conclusion: 该方法在临床环境中实现了鲁棒的AES分割，并通过设计解码器有效地适应了预训练模型的表示。
...
TL;DR: FreqForcing框架有效解决长视频生成错误累积问题，提升视频质量


<details>
  <summary>Details</summary>
Motivation: 针对长视频生成中错误累积问题，提出FreqForcing框架

Method: Spectral Self-Anchoring (SSA)，利用低频成分和高频成分保持视觉稳定性和动态运动

Result: FreqForcing在长视频生成中优于现有无训练方法，与代表性基于训练的方法具有竞争力

Conclusion: FreqForcing能够有效缓解长视频生成中的错误累积问题，提升视频质量
...
TL;DR: Veritas++通过感知增强和值感知在线蒸馏，有效提升了AI生成图像检测的性能。


<details>
  <summary>Details</summary>
Motivation: 随着图像生成模型能力的增长，合成图像在公开媒体中变得司空见惯，这使得AI生成图像（AIGI）的检测变得越来越重要。

Method: 提出了Veritas++，一个感知增强的推理框架，以可靠感知作为真实性推理的基础。

Result: 实验结果表明，Veritas++在标准、真实世界和新兴基准测试中取得了良好的泛化能力。

Conclusion: Veritas++有效地弥合了感知差距，在检测方面取得了显著提升，而VaOPD进一步实现了能力演化，而不会牺牲现有性能。
...
TL;DR: 提出了一种基于多任务微调的VQA模型，提高了答案的准确性和对视觉证据的理解。


<details>
  <summary>Details</summary>
Motivation: 从单标签分类转向视觉问答（VQA），模型必须回答关于图像的自由形式临床问题。

Method: 提出了一种简单的多任务微调方案，从现有的VQA数据集中构建辅助的定位和描述任务，最小化额外的注释。

Result: 在Kvasir-VQA-x1上，对三种小的VLM主干进行微调，结果表明，与分布内和分布外数据相比，一致地提高了准确性，并改善了答案标记与相关图像区域之间的隐式对齐。

Conclusion: 所提出的方法在VQA任务上取得了显著的性能提升，并有助于提高模型对视觉证据的理解。
...
TL;DR: SeasonStereo是一种利用合成图像对和基础模型先验进行大规模3D重建的新方法。


<details>
  <summary>Details</summary>
Motivation: 提高从卫星图像中进行精确3D重建的能力，尤其是在时间序列图像中，克服季节性和光照变化带来的挑战。

Method: 提出SeasonStereo框架，通过训练具有可控季节性外观变化的合成图像对，并利用基础模型中的零样本几何先验。

Result: SeasonStereo在匹配精度上与最先进的LiDAR监督模型相当，同时提供更清晰的几何细节，无需对齐的多日期真实训练产品或LiDAR派生的标签。

Conclusion: SeasonStereo为从异构卫星图像进行大规模3D重建提供了一种经济高效的途径。
...
TL;DR: 提出了一种轻量级框架ACA，用于改进CT基础模型在解剖学领域的视觉-语言对齐，提高了性能并降低了计算成本。


<details>
  <summary>Details</summary>
Motivation: 解决CT视觉-语言基础模型在下游任务中通常使用整个体积表示，这会稀释精细解剖信号的问题。

Method: 提出了一种名为Anatomy Contextualized Adaptation (ACA)的轻量级框架，该框架通过TotalSegmentator将CT体积分解为解剖级别嵌入，并通过transformer捕捉跨解剖关系，同时将嵌入与从放射学报告中提取的每个解剖和扫描级别的文本对齐。

Result: 在Merlin和CT-RATE上评估，ACA在零样本发现分类中始终优于冻结的基础模型基线和现有的细粒度方法，且一旦嵌入被缓存，训练时间不到一小时。

Conclusion: ACA是一种轻量级的方法，可以将CT基础模型适应于基于解剖的视觉-语言对齐，同时保留和增强全局解剖上下文。
...
TL;DR: 提出了一种结合SLAM和SfM的系统，用于提高视频姿态和校准的恢复，显著优于现有技术。


<details>
  <summary>Details</summary>
Motivation: 准确恢复任何未受约束视频的相机校准和度量姿态，以解锁大规模训练数据用于导航和场景理解。

Method: 提出一种结合SLAM的强顺序约束和离线SfM的灵活性和全局优化系统的系统，利用广泛的基线密集图像匹配，将时间排序视为可靠闭环的第一公民，并使用度量单目深度先验增强全局优化。

Result: 在具有极端运动和视觉对称性的多样化、具有挑战性的数据集上的彻底评估表明，该方法在鲁棒性和准确性方面显著优于现有SLAM和SfM，无论是经典还是学习，无论相机校准已知还是未知。

Conclusion: 提出的方法能够有效地恢复任意、长、未校准视频的度量重建，为导航和场景理解提供了强大的工具。
...
TL;DR: TurboVLA 是一种高效的视觉-语言-动作模型，显著降低了计算和内存成本，在 LIBERO 数据集上表现优异。


<details>
  <summary>Details</summary>
Motivation: 为了解决现有视觉-语言-动作（VLA）模型在每轮策略调用时产生的巨大计算和内存开销问题。

Method: 提出了一个名为 TurboVLA 的新型 VLA 范式，它将传统的 $V 	o L 	o A$ 路径重新定义为直接的 $V + L 	o A$ 映射。TurboVLA 不再使用大型语言模型作为感知和行动之间的中心接口，而是独立编码视觉观察和语言指令，通过轻量级的双向视觉-语言交互直接交换它们之间的信息，并使用紧凑的解码器预测连续的动作块。

Result: 在 LIBERO 上，TurboVLA 实现了 97.7% 的平均成功率，仅使用 0.2B 个参数，31.2 毫秒的推理延迟，以及在消费级 RTX 4090 上的 0.9 GB 推理 VRAM，与更大规模的 VLA 策略相比，性能相当或优于。

Conclusion: TurboVLA 作为一个简单且有效的替代方案，为现有的以 LLM 为中心的 VLA 范式提供了新的视角，展示了如何高效地将视觉、语言和动作联系起来以实现机器人操作。
...
TL;DR: 引入论文内部声明验证框架，使用LLM评估论文新颖性声明的支持程度，提高同行评审效率。


<details>
  <summary>Details</summary>
Motivation: 随着科学论文提交量的增加，人们开始对使用大型语言模型（LLMs）来协助同行评审产生兴趣。

Method: 我们引入了论文内部声明验证框架，该框架使用LLM从引言中提取新颖性声明，检索与声明相关的证据，并评估方法是否支持所述贡献。

Result: 人类评估表明，框架生成的评估与人类评审员关注的问题之间存在显著一致性，特别是在与新颖性相关的问题上。BERTScore进一步区分了对应的人类-LLM评审对与不匹配的控制，表明该框架捕捉了与人类评审员观察一致的关注点。

Conclusion: 该框架有助于提高同行评审的效率和准确性，特别是在评估新颖性方面。
...
TL;DR: DuplexGen通过人类校准实现场景自适应的对话转接。


<details>
  <summary>Details</summary>
Motivation: 当前模型在对话中应用单一的规范，不考虑场景差异。

Method: 提出DuplexGen框架，通过校准LLM预测与少量槽位级别的人类偏好注释来生成场景自适应的对话。

Result: DuplexGen在六项合作和竞争任务中，与人类偏好的一致性高于未校准的提示或仅基于通用人类-人类数据的训练。

Conclusion: 人类校准而非语料库规模或提示设计是使转接合成场景特定的关键。
...
TL;DR: 本研究旨在通过提出一种新的检测流程，解决GenAI诗歌检测的难题，并提取关键属性以实现准确分类。


<details>
  <summary>Details</summary>
Motivation: 随着人工智能技术的进步，生成式人工智能（GenAI）和人类撰写的文本几乎无法区分。此外，全球AI聊天机器人的标准化使得学术不端行为更加频繁。现有研究表明，即使没有进行任何修改，GenAI诗歌也最难区分，因此现代检测器自然将其视为类似人类的诗歌。然而，这些论文的客观性需要通过现代检测工具进行验证，但诗歌的主观性和现代大型语言模型（LLMs）架构的黑盒性质使得验证此类工作相当复杂。

Method: 提出了一种零样本检测流程，该流程包含一个由人类和AI诗歌组成的语料库，以验证人类和AI创作的可区分性，并提取上述关键属性以实现准确分类。

Result: 提取这些属性有两个方面的好处：首先，它减少了训练所需的边际，因为只需要基于误分类属性的诗进行训练和微调；其次，它为GenAI检测困境提供了关键见解，从而加强了现代检测流程。

Conclusion: 研究的主要目标是推导出英语诗歌的属性，这些属性有助于人类和AI诗歌的分类和误分类，并为诗歌可区分性主张提供证实或反驳的证据。
...
TL;DR: V-Steer通过编辑缓存值向量来恢复LLMs的指令层次结构，显著提高了其准确性和效率。


<details>
  <summary>Details</summary>
Motivation: 为了解决前沿大型语言模型（LLMs）违反指令层次结构的问题，提出了一种新的方法V-Steer。

Method: V-Steer是一种在推理时通过编辑提示位置的缓存值向量来恢复特权影响的无监督方法。它使用直接对数概率的贡献来识别受低优先级段控制的头部，然后通过就地乘性编辑缓存V张量来增强特权段并抑制冲突的低优先级段。

Result: 在从70亿到700亿参数的模型上，V-Steer将受控角色冲突基准测试中的主要约束准确率从不到18%提高到92%，在更广泛的指令层次结构评估中显著优于仅提示基线，在4个LLMs规模中的3个尺度上与基于训练的最先进方法相匹配或超过，同时解码速度开销可忽略不计。

Conclusion: V-Steer是一种有效的无监督方法，可以恢复LLMs的指令层次结构，并显著提高其准确性和效率。
...
TL;DR: 这项研究创建了一个大型宗教广播语料库，用于支持对该领域的描述性研究、分析和研究。


<details>
  <summary>Details</summary>
Motivation: 宗教广播在美国是一种普遍但研究不足的大众传播形式，内容层面的分析受到大规模转录数据的缺乏的限制。

Method: 对2025年7月一个月内从785个不同的直播流中捕获的英语宗教广播进行转录，并使用自动管道进行转录和说话人识别。然后，使用大型语言模型对节目格式和主题进行分段和标记。

Result: 生成一个包含流元数据、录音元数据和转录行的链接表语料库。

Conclusion: 该语料库支持对宗教广播的跨地区和传统描述性研究，分析宗教媒体中讨论的社会和政治问题，以及在不被充分代表的领域中进行语音处理研究。
...
TL;DR: Robostreet Flow架构通过联合优化，显著降低长途卡车运输成本


<details>
  <summary>Details</summary>
Motivation: 降低长途卡车运输成本

Method: 联合优化车辆、车队编队和运营模式

Result: Robostreet Flow架构，降低每吨英里成本20.5%，降低运营成本56%相对于柴油基线

Conclusion: Robostreet Flow架构能够有效降低长途卡车运输成本
...
TL;DR: AgentGUI：提升AI代理观察与引导效率


<details>
  <summary>Details</summary>
Motivation: 解决AI代理自主能力快速提升，但人类监督滞后的问题

Method: 开发AgentGUI，一个用户友好的本地GUI，用于观察和引导AI代理

Result: AgentGUI提高了识别关键元素的速度（38%），并提高了小代理的任务完成率（最高34pp）

Conclusion: AgentGUI为AI代理的观察和引导提供了有效工具，有助于提高效率和准确性
...
TL;DR: LLMs作为合成用户存在局限性，不能完全替代人类响应数据，需要谨慎使用。


<details>
  <summary>Details</summary>
Motivation: 评估大型语言模型（LLMs）作为合成用户的有效性，以及它们在产品、政策和市场决策中的应用。

Method: 在两个独立的人类响应数据领域（美国社会态度和跨文化价值观）中，对四个不同能力范围的LLMs进行基准测试，并与非LLM基线进行比较。

Result: LLMs在个体层面上无法超越最强基线，且在跨文化价值观方面表现不佳；模型系统地高估了人口统计学因素对态度的影响。

Conclusion: LLMs作为合成用户存在局限性，不能完全替代人类响应数据，需要谨慎使用。
...
TL;DR: 研究揭示了LLMs在音乐乐器与性别关联上的偏见，发现文本模态的偏见最强。


<details>
  <summary>Details</summary>
Motivation: 研究LLMs中性别偏见的问题，特别是通过音乐乐器与性别关联的角度。

Method: 构建了名为Symphony-Bias的多模态数据集，包含文本、视觉和音频数据，评估了10个多模态模型，分析它们如何将22种乐器与三种性别类别（男性、女性、非二元）关联。

Result: 92%的乐器级别结果与先前社会科学研究一致，其中竖琴和鼓在所有评估模型和模态中表现出特别一致的性别关联。发现与社会刻板印象的一致性在音频中最弱，在视觉中较强，在文本中最强。

Conclusion: 多模态模型在处理音乐乐器与性别关联时存在偏见，且文本模态的偏见最强。
...
TL;DR: 使用编码代理可能提高生产力，但损害代码理解，用户仍偏好使用。建议开发者在设计代理时考虑促进理解。


<details>
  <summary>Details</summary>
Motivation: 探讨使用编码代理（如Cursor）对开发者生产力的影响，以及这种影响可能对开发者理解、监督、学习和沟通带来的负面影响。

Method: 进行了一项实验，让54名学生使用两种不同的AI系统创建网站：一种代理编辑用户代码，另一种是聊天机器人，用户可以单独编写代码或修改通用代码片段。通过理解问题和用户在没有代理的情况下扩展代码的任务来测试理解。

Result: （1）虽然代理有助于初始任务完成，但它们损害了用户的代码理解，因此无法为用户扩展代码做准备；（2）低成本的代理交互类型（如复制粘贴提示和自动接受的编辑）与较低的理解能力相关；（3）尽管用户自我报告理解能力较弱，但他们仍然更喜欢编码代理，因为它们快速且易于使用。

Conclusion: 在编码工作流程中，用户保持参与，但不应忽视理解。针对这一目标，我们总结了针对编码代理开发者的未来研究方向：劝阻低成本的提示、创建可读代码和促进积极参与。
...
TL;DR: 提出EC-Reason-Bench，分析LLM在酶功能预测中的局限性，揭示外部知识和推理的重要性。


<details>
  <summary>Details</summary>
Motivation: 分析现有基准测试中发现的异常：通用LLM在预测EC编号时得分接近零，而专业模型和工具仍可使用。

Method: 提出EC-Reason-Bench，一个无需训练的诊断评估协议，用于回答两个问题：为什么通用LLM在EC编号预测上的得分接近零，以及在不更新任何权重的情况下，可以恢复多少损失。将酶分类能力分解为四个正交杠杆，每个杠杆都可以单独测量：输出结构、外部知识、推理结构和推理鲁棒性。通过推理时间方法测试每个杠杆，与共享的无监督基线进行对比，该基线再现了之前报告的几乎为零的性能。

Result: 实验表明，外部知识至关重要，必须在推理之前进行；闭卷情况下，级联和思维链是否有助于或有害取决于模型避免的倾向；一旦有证据，最佳LLM设置的总体得分与简单地投票选择最接近检索到的邻居的EC编号无法区分；这种平局是平均的结果，掩盖了对多功能酶的巨大损失。因此，推理在冲突的邻居之间充当仲裁者，而不是知识来源，没有一个单一的排行榜可以看到这一点。准确性遵循同源可用性定律。

Conclusion: 提出了一种新的评估协议，以解决通用LLM在酶功能预测中的局限性，并揭示了外部知识和推理在提高预测准确性中的重要性。
...
TL;DR: Voice Memory是一种基于记忆的语音识别方案，有效降低了错误率，并提高了模型的可移植性和可审计性。


<details>
  <summary>Details</summary>
Motivation: 提出Voice Memory，一种针对语音识别的推理方案。

Method: 结合了经典ASR-LM框架，采用听者-思考者架构，通过记忆耦合两个角色。

Result: Voice Memory在多个领域降低加权词错误率，并在某些领域（如航空旅行命令和噪声远场语音）中取得了显著的性能提升。

Conclusion: Voice Memory通过记忆机制实现了高效的语音识别，并提高了模型的性能和可审计性。
...
TL;DR: 模型端聚合接口提升长上下文语言模型在非加性聚合任务中的性能


<details>
  <summary>Details</summary>
Motivation: 长上下文语言模型在非加性、基于集合的聚合任务中的性能不可靠

Method: 引入模型端聚合接口，维护基于哈希的超日志日志（HLL）草图状态

Result: 在实验中，该方法在多个任务上取得了良好的效果，包括精确度、相对误差等

Conclusion: 模型端聚合接口可以有效地提高长上下文语言模型在非加性、基于集合的聚合任务中的性能
...
TL;DR: 提出了ForgetBench，用于评估LLMs在持续知识编辑下的遗忘行为，发现现有方法在长期保留和泛化质量之间未能取得平衡。


<details>
  <summary>Details</summary>
Motivation: 为了理解大型语言模型（LLMs）在持续知识编辑下保留先前获取知识的能力，以及评估知识保留和退化过程中的时间动态。

Method: 提出了ForgetBench基准，引入了基于概念和基于场景的QA评估范式，并建立了一个时间有序的知识流，以评估模型在多个编辑阶段的行为。

Result: 实验表明，现有方法在长期保留和泛化质量之间未能取得平衡，强调了未来LLMs需要更强大的记忆机制。

Conclusion: 提出了ForgetBench，以系统地描述LLMs在持续知识编辑下的遗忘行为，为未来LLMs的记忆机制提供了新的研究方向。
...
TL;DR: CMT-RAG通过将对话记忆与检索对齐，提高了多轮多跳RAG的性能。


<details>
  <summary>Details</summary>
Motivation: 多轮信息检索对话需要跨轮次的多跳推理和长距离依赖跟踪。然而，现有的RAG系统通常将对话记忆表示为原始对话历史、重写的查询或非结构化摘要，这使得难以恢复后续查询所需的具体先前推理步骤和证据。

Method: 将对话记忆与检索对齐，将对话上下文表示为子问题级别的推理痕迹。引入MuMu-QA基准和CMT-RAG记忆框架，其中CMT-RAG在每个轮次使用状态空间迹生成器，其循环状态作为运行时记忆，以整合最近的对话上下文并将当前查询分解为包含检索导向的子问题和先前痕迹依赖的结构化迹草稿。然后，它使用检索到的证据来定位这些草稿，并将它们存储为会话级别的DAG中的持久记忆痕迹。

Result: 在MuMu-QA和语料库级别的RAG基准上的实验表明，CMT-RAG在答案准确性方面始终优于五类RAG基线。

Conclusion: CMT-RAG在多轮多跳RAG中表现出色，能够有效地恢复先前推理和证据。
...
TL;DR: BM25在受控环境中表现最佳，而基于图的RAG在可扩展性方面存在问题。


<details>
  <summary>Details</summary>
Motivation: 为了解决检索增强生成（RAG）方法在不同语料库规模下的准确率-成本缩放不明确的问题。

Method: 进行了一个受控的语料库缩放研究，包括28个严格嵌套的层级，从大约1,000个文档增长到512,000个文档，同时保持问题和相关及对抗性文档的固定数量。在统一的读者和判断协议下，测量了官方准确率、构建和查询令牌以及延迟。

Result: 实验结果表明，BM25在受控环境中表现最佳；它定义了每个测量层级的帕累托前沿的低成本端，并在中等规模及以上引领准确率，无需基于LLM的构建。文件系统代理在最小层级上与BM25相当或略高，但在基础层和全规模上，每个答案的查询令牌多39倍，准确率落后近20个百分点。匹配的检索交换逆转了这一失败：Agent+BM25在满规模上的得分为69.4，而原始文件代理为36.9，原生BM25为54.8。基于图的RAG遇到构建瓶颈：最重的构建者使用高达24.6个生成LLM令牌来索引每个语料库令牌，但停止在全语料库的前2%，而可扩展的变体在共享层级上比BM25的准确率低。

Conclusion: BM25在受控环境中表现最佳，而基于图的RAG在可扩展性方面存在问题。
...
TL;DR: EGMD enhances cross-domain generalization of multimodal fake news detectors.


<details>
  <summary>Details</summary>
Motivation: Multimodal fake news detectors often generalize poorly across domains due to unreliable evidence.

Method: Expert-Guided Mutual Distillation (EGMD) is proposed to learn what evidence to trust across the prediction pipeline.

Result: EGMD achieves state-of-the-art accuracy on four datasets in two languages, reducing domain bias by up to 57.3%.

Conclusion: EGMD improves the generalization of multimodal fake news detectors across domains.
...
TL;DR: WikiLoop通过联合学习提高了知识库构建和查询的效率。


<details>
  <summary>Details</summary>
Motivation: 优化知识库构建和查询，提高机器导航能力。

Method: WikiLoop框架联合学习构建和导航，使用角色条件共享策略和下游导航评估结构化编辑。

Result: WikiLoop在AuthTrace、HotpotQA和MuSiQue上均取得了较好的效果。

Conclusion: WikiLoop在AuthTrace上达到了62.6的答案正确率，在多文档查询上取得了最大收益。它通过联合学习和训练，提高了导航器和答案正确率。
...
TL;DR: 本研究分析了损失性验证方法对生成质量的影响，提出了优化策略。


<details>
  <summary>Details</summary>
Motivation: 为了进一步提高大语言模型推理效率，允许轻量级草稿模型提出标记，随后由较大的目标模型并行验证。

Method: 对损失性验证方法诱导的分布进行原则性分析，将方法分类为基于截断的验证和协作验证，构建诊断评估框架。

Result: 揭示了基于截断的验证方法的潜在问题，并提出了协作验证的关键原则。

Conclusion: 损失性验证方法在提高效率的同时可能降低生成质量，需要进一步优化以平衡效率和质量。
...
TL;DR: 本研究探讨了LLM代理基于文件系统的长期记忆管理，发现其存在局限性，但仍具有设计潜力。


<details>
  <summary>Details</summary>
Motivation: 研究如何有效地组织和检索长期记忆对于大型语言模型（LLM）代理至关重要。

Method: 本研究通过系统性地探索基于文件系统的内存，以评估LLM代理的长期记忆管理能力。

Result: 研究发现，尽管文件系统可以降低检索成本，但目前的LLM代理在记忆组织和检索方面仍存在不足。

Conclusion: 文件系统可以作为LLM代理记忆的设计空间，但需要进一步优化以实现更有效的记忆管理。
...
TL;DR: 中期宪法训练提高模型对齐的持久性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 研究训练后对齐的持久性，以及中期干预对对齐的影响。

Method: 通过宪法中期训练，在120B规模下，将基于原则和价值观的内容插入中期训练，与仅重放的控制进行对比。

Result: 宪法中期训练模型在对齐泛化和持久性方面优于控制，特别是在勒索方面：SFT在所有模型中培养了勒索倾向，但宪法中期训练将其减弱，优势在良性微调后仍然存在。

Conclusion: 中期宪法训练可以提供持久、广泛的对齐收益，是SFT管道的廉价补充。
...
TL;DR: 使用BERT模型自动分类痘疮研究，提高信息检索效率


<details>
  <summary>Details</summary>
Motivation: 研究痘疮疫情，开发疫苗，改进诊断，研究病毒进化，预防未来疫情爆发

Method: 多标签分类，使用BERT模型进行分类，SHAP分析关键词特征和模式

Result: BERT模型在分类痘疮研究文章方面表现最佳，准确率达到97.05%，微F1分数达到97.67%，宏F1分数达到96.46%，SHAP分析揭示关键特征和模式

Conclusion: BERT模型可以自动化痘疮研究的分类，有助于研究人员、政策制定者和医疗工作者快速找到相关信息，节省时间并提高公共卫生工作
...
TL;DR: AtmosERC是一种基于图的ERC框架，通过利用对话级情感氛围，提高了轻量级和基于LLM的ERC的性能。


<details>
  <summary>Details</summary>
Motivation: 对话中的情感识别（ERC）旨在预测对话中的话语级情感，主要通过上下文中心建模来发展。然而，全局上下文是一个异质信号，并非所有上下文信息都与情感预测同等相关。本文聚焦于信号的情感导向组件，即对话级情感氛围，它捕捉了在对话情感模式中普遍反映的潜在趋势。

Method: 提出了一种基于图的ERC框架AtmosERC，将每个对话建模为基于话语和说话人的对话图。关系感知图提取器过滤和融合异构图信号，以产生对话级和说话人条件下的情感先验。

Result: 实验表明，AtmosERC提高了轻量级ERC，增强了基于LLM的ERC作为插件提示，并在局部情感偏差下产生了更稳定的预测。

Conclusion: AtmosERC在对话情感识别中表现良好，提高了轻量级和基于LLM的ERC的性能，并在局部情感偏差下提供了更稳定的预测。
...
TL;DR: This paper introduces memory foundation models, with the prototype Metis demonstrating advantages in architecture, optimization, and efficiency. The project and model checkpoints are released for future research.


<details>
  <summary>Details</summary>
Motivation: Recent advances in AI agents have led to the development of multimodal foundation models and large reasoning models, but agent memory is primarily implemented through external modules. This paper aims to explore native memory capabilities within foundation models.

Method: The paper introduces memory foundation models and formalizes native memory from two perspectives: persistent and dynamically evolving memory state within the backbone, and native memory procedures that autonomously store and utilize information. The proposed Metis is the first prototype of memory foundation models, with a new architecture and memory-specific training data.

Result: Metis exhibits native memory capabilities, with advantages in architecture, end-to-end optimization, and efficiency. The online memory maintenance of Metis is gradient-free, and memory update requires only a forward pass. The paper provides a detailed analysis of Metis' strengths, limitations, and behaviors, and releases project and model checkpoints.

Conclusion: Memory foundation models offer a new direction for AI agents, with Metis demonstrating significant advantages. Future research on memory foundation models is facilitated by the released project and model checkpoints.
...
TL;DR: 本研究比较了跨度引导和无引导的去毒化方法，发现它们在不同情况下各有优劣，并强调了评估缓解充分性和意义保留的重要性。


<details>
  <summary>Details</summary>
Motivation: 为了保留意义，将编辑局部化到注解的有害跨度，但相同的约束可能无法充分缓解有害意图。

Method: 进行受控的探索性比较，比较跨度引导和无引导的去毒化在混合来源的英语评估集上的效果，该评估集包括手动编纂的输入和HateXplain测试项目。在固定的单一生成器设置下进行密集的盲法人类评估。

Result: 人类偏好揭示了一种权衡，而不是一种统一的优先重写策略。当局部编辑保留原始立场并避免不必要的修改时，倾向于跨度引导输出；而当更广泛的重写实现更完整的缓解时，倾向于无引导输出。这种对比在不同层次上有很大差异：在强层次中，两种策略具有竞争力，而在轻度层次中，无引导重写明显更受欢迎。理由注释将这种差异归因于互补的失败风险：局部编辑后的残留伤害和广泛重写后的过度修改。

Conclusion: 将自动评估视为诊断工具，而不是人类判断的替代品。毒性相似度标量化、多生成器分析和两个通用LLM评判者重现了部分总体趋势，但没有产生类似的分层对比。这些特定于设置的发现并没有建立基于严重程度的路由规则。相反，它们激励了评估协议，这些协议评估缓解充分性和意义保留，并报告与总体分数一起的残留伤害和过度修改。
...
TL;DR: 提出将概念作为LLMs的设计轴，并揭示三种模式，以设计具有明确概念表示的LLMs。


<details>
  <summary>Details</summary>
Motivation: 由于大型语言模型（LLMs）通过分布式统计关联隐式地表示概念类信息，而不是作为明确的、结构化的、组合性的概念，因此概念级结构通常是“找到”的而不是“设计”的。这导致在训练后通过探测或词典学习恢复结构，而没有关于稳定性、组合性、可控性或与人类概念组织的对齐的架构保证。

Method: 提出将概念作为LLMs的设计轴，并沿两个维度映射设计空间：引入概念结构时的管道阶段（训练目标、核心架构、推理或事后解释），以及该结构是否从模型自身的表示内部衍生或基于外部资源。

Result: 揭示了三种广泛的模式：推理时间方法相对较少研究，相关想法在管道阶段主要独立发展，外部基于的方法贯穿整个管道，尽管通常使用不同的术语描述。这些观察结果促使人们超越从训练模型中恢复概念类似结构，转向设计具有明确概念表示的LLMs。

Conclusion: 需要设计具有明确概念表示的LLMs，而不是仅仅从训练模型中恢复概念类似结构。
...
TL;DR: SERPO通过协同进化响应证据、评分标准和策略参数，显著提高了语言模型的性能。


<details>
  <summary>Details</summary>
Motivation: 为了在推理时使语言模型能够自我进化，而无需标签反馈，测试时强化学习（TTRL）被引入。现有方法依赖于答案投票，因此无法自然扩展到开放式生成，其中有效响应无法映射到共享的规范答案。在没有外部奖励模型或更强的裁判的情况下，适应性必须从模型的自身输出中构建可靠的奖励。

Method: 我们引入了SERPO（自我进化的评分策略优化），它用响应证据、特定查询的评分标准和策略参数的协同进化闭环替换了答案投票。Good-Normal-Bad（G-N-B）响应进化将最大分离的滚动操作组织成有序的存档；评分标准进化保留了区分这些存档的标准；概率标准评分将判决标记的可能性转换为奖励信号；策略进化使用这些信号优化演员。新的演员滚动操作刷新存档和评分标准，关闭三方进化循环。

Result: 在两种模型配置、两个领域基准和四个OOD基准上，SERPO将HealthBench和ResearchQA分别提高了20.63和20.31分，将六个基准的宏平均提高了8.06分，并支持OOD迁移和持续的跨基准进化。

Conclusion: SERPO是一种有效的TTRL方法，可以提高语言模型的性能，并支持跨领域迁移和持续进化。
...
TL;DR: 提出DIRECT框架，显著提高序列标注任务的性能和效率。


<details>
  <summary>Details</summary>
Motivation: 解决现有基于大型语言模型的方法在领域对齐和推理效率方面的不足

Method: 提出DIRECT框架，通过训练时间优化和推理时间校正来解决问题。具体来说，DIRECT在监督微调后执行直接偏好优化（DPO），以增强与人类偏好的任务对齐，并引入一个受控解码过程，强制执行固定输出格式并限制预测到候选集。为了进一步提高效率，模板填充机制要求模型仅生成标签令牌，并通过KV缓存重用前缀内容，从而减少冗余计算。

Result: 在八个数据集上的实验结果表明，与现有方法相比，DIRECT在性能和效率方面都取得了显著提高。

Conclusion: DIRECT框架通过优化训练时间和推理时间，显著提高了序列标注任务的性能和效率。
...
TL;DR: DuPLeR是一种用于多模态少样本知识图谱补全的框架，通过结合多模态LLM先验和结构推理，实现了在数据稀缺场景下的稳健性能。


<details>
  <summary>Details</summary>
Motivation: 知识图谱补全（KGC）旨在推断知识图谱（KG）中缺失的事实，从而提高其完整性和支持下游智能应用。然而，在现实部署中出现的实体和关系使得归纳式KGC变得困难，尤其是在少样本和零样本设置下。多模态信息和大型语言模型（LLM）推导的先验可以丰富稀疏的关系上下文，但它们也可能引入噪声或幻觉证据。

Method: 我们提出了DuPLeR，一个用于多模态少样本KGC的Dual-Path LLM Reasoning框架。DuPLeR通过结合多模态LLM推导的类型先验和事实支持结构来构建一个校准的关系图，并在精炼的关系拓扑上执行双级结构推理。此外，一个双路径多模态增强模块通过查询相关的多模态信号调节消息传递，并在图传播后补充实体表示。

Result: 在两个多模态知识图谱（MMKG）基准的八个归纳变体上的实验表明，DuPLeR在数据稀缺的KGC场景中实现了稳健的性能。

Conclusion: DuPLeR在多模态少样本KGC中表现出色，能够有效地推断缺失的事实，提高知识图谱的完整性。
...
TL;DR: Latent-IM improves dialogue system performance by 12.5 points through move control.


<details>
  <summary>Details</summary>
Motivation: Classical spoken dialogue systems often separated dialogue management from response realization.

Method: Formulate move control as two coupled problems: selection and realization. Introduce Latent-IM, an internal dialogue-management framework.

Result: Improve average end-to-end move accuracy by 12.5 points over the unsteered backbone while performing comparably to fine-tuning.

Conclusion: Latent-IM is effective in choosing and deploying conversational moves for dialogue systems.
...
TL;DR: 通过对比不同的因果目标，分析诊断证据的准确性，发现隐藏状态包含有关因果目标的信息。


<details>
  <summary>Details</summary>
Motivation: 分析不同诊断结果对因果假设的影响，以及如何通过对比不同的因果目标来评估诊断证据的准确性。

Method: 提出成对提示，通过改变因果目标来重复相同的诊断证据，并使用线性读出分析Qwen2.5-7B-Instruct、Qwen3-8B和Llama-3.1-8B-Instruct的最后隐藏状态。

Result: 在49对主要基准测试中，平衡准确率在0.654到0.659之间，18-21对被正确分类。人类评审员对95个提示中的标签达成一致。隐藏状态读出在平衡准确率和恢复对数方面优于线性分类器和文本基线。

Conclusion: 隐藏状态包含有关诊断证据是否有利于、挑战或未能解决因果目标的线性可解码信息。
...
TL;DR: 大型语言模型的概率判断存在方向偏差，模型身份比语言更重要。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型被用作决策辅助工具，但其概率判断是否存在系统性偏差难以检测。

Method: 引入OptimismBench，使用反转对来检测方向偏差。

Result: 在16个模型中，14个模型表现出乐观倾向，消极情绪仅在Anthropic的前沿层出现。

Conclusion: 模型的身份比语言更重要，模型之间的方差是语言间方差的4.7倍。
...
TL;DR: 研究发现，大型语言模型中存在区域偏见，这对社会决策有重大影响。


<details>
  <summary>Details</summary>
Motivation: 区域偏见在大型语言模型中可能导致对地区群体的认知和对不同地区个人的决策产生影响。然而，现有研究往往分别考察这些表现，对其结构和后果不清楚。

Method: S2D 框架评估从抽象刻板印象到具体社会决策的区域偏见。使用刻板印象评分对温暖（感知友好和可信）和竞争力（感知能力和智力），以及教育、职业和社会互动的配对选择任务对中国所有34个省级行政区域进行评估。

Result: 结果表明，区域得分存在显著差异，特别是在竞争力职业决策上，模型之间存在相当的一致性。这些模式与区域经济和数字发展指标相关，并显示出混合的人性化刻板印象。

Conclusion: 研究表明，大型语言模型中的区域偏见普遍存在、系统性强、后果严重，这促使我们进行更区域意识强的评估和缓解。
...
TL;DR: 提出了一种开放端到端训练检索模型的方法，并通过翻译训练将英语监督转移到多语言检索，取得了新的最先进结果。


<details>
  <summary>Details</summary>
Motivation: 为了解决使用封闭训练数据训练检索模型导致的可重复性问题，以及研究英语监督如何通过翻译训练转移到多语言检索。

Method: 从34个公共来源中重构和整理了665M个英语对比预训练对，并构建了1.88M个带有挖掘的硬负例的监督微调对。训练产生了两个149M参数的模型：DenseOn（单向量密集模型）和LateOn（ColBERT风格的后期交互模型）。然后将验证的英语数据翻译成八种语言，产生了2.8B个带有跨语言样本的对，并训练了mDenseOn和mLateOn，这是基于mmBERT-base的两种307M参数模型。

Result: DenseOn和LateOn在BEIR上分别达到了56.20和57.22的平均nDCG@10，为这个尺寸类别的检索模型设定了新的最先进结果。mDenseOn在英语和翻译语言上表现良好，但在翻译训练支持之外会退化，而Late-interaction模型更好地泛化到未见过的语言和脚本。

Conclusion: 这表明，token级别的匹配将翻译训练从目标语言扩展策略转变为多语言泛化食谱。
...
TL;DR: Pangram 4是Pangram Labs推出的新型AI文本分类模型，在性能上优于前代产品，并在AI文本检测任务中表现出色。


<details>
  <summary>Details</summary>
Motivation: Pangram Labs发布Pangram 4，一种基于深度学习的AI文本分类模型。

Method: 使用深度学习技术进行AI文本分类，并评估其性能。

Result: Pangram 4在AUROC、假阳性率、假阴性率等指标上均优于Pangram 3，并在边界检测和AI辅助检测任务中表现优异。

Conclusion: Pangram 4在AI文本检测任务中表现出色，达到业界领先水平。
...
TL;DR: APEX-Accounting基准评估前沿模型在会计任务上的表现，发现Simpson's paradox现象。


<details>
  <summary>Details</summary>
Motivation: 评估前沿模型是否能完成会计的实际工作

Method: 建立APEX-Accounting基准，包含160个任务，涵盖10个世界，每个世界包含会计系统、电子表格、PDF和其他文件。专家完成每个任务并编写评分标准。

Result: Claude-Fable-5 (Max) 在9个前沿模型中得分最高，达到56.4%的Mean Criteria@3。Muse-Spark-1.1 (xHigh) 以52.6%的得分位居第二。最高Pass@8为21.5%。增加token预算观察到Simpson's paradox现象。

Conclusion: APEX-Accounting基准可以评估前沿模型在会计任务上的表现，但存在Simpson's paradox现象。
...
TL;DR: 提出MWM，通过模拟心理状态预测人类决策。


<details>
  <summary>Details</summary>
Motivation: 为了解决现有世界模型无法准确预测人类行为的问题，提出了一种新的模型框架——心理世界建模（MWM）。

Method: 提出MWM框架，将其应用于MENTIS系统，并在一个高质量的决策场景数据集上进行实验。

Result: 实验结果表明，MWM能够有效地预测人类决策，并揭示了当前心理世界建模的瓶颈。

Conclusion: MWM是心理世界建模的下一阶段，从模拟物理场景到模拟在其中行动的头脑。
...
### [138] [Framework Implementation Maturity in Blockchain-Based Third-Party Compliance Assessment](https://arxiv.org/abs/2607.26087)
*Jemima Owusu-Tweneboah,Amani Altarawneh,Deepti Gupta,Maria Luisa Figueroa*

Main category: cs.CR

TL;DR: 本研究提出了一种基于区块链的第三方风险评估框架，以解决现有框架采用评估方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 为了解决现有框架采用评估方法的局限性，特别是在依赖第三方供应商的环境中。

Method: 提出基于区块链的第三方风险评估框架，利用智能合约实现评估工作流程、多方治理和纵向评估状态保持。

Result: 引入一套评估指标和定性成熟度模型，以评估合规控制措施是否在重复评估周期中得到验证、治理和维持。

Conclusion: 区块链系统可以支持框架实施成熟度测量，而不是静态的合规性验证。
...
### [139] [Lilith: Backdoor Generalization under Training-Inference Trigger Shift](https://arxiv.org/abs/2607.26099)
*Zhou Feng,Jiahao Chen,Chunyi Zhou,Yuan Su,Tianyu Du,Yuwen Pu,Jianhai Chen,Jinbao Li,Shouling Ji*

Main category: cs.CR

TL;DR: Lilith在攻击成功率和效用降低之间取得了平衡，同时揭示了基于精确触发的评估所忽视的更广泛威胁。


<details>
  <summary>Details</summary>
Motivation: 为了解决现有后门研究未考虑从训练时触发的后门能否泛化到受害者训练中不存在的推理时触发器家族的问题。

Method: Lilith是一个黑盒锚定到家族框架，使用离散的代理资源。

Result: 实验表明，Lilith在攻击成功率和效用降低之间取得了平衡，同时揭示了基于精确触发的评估所忽视的更广泛威胁。

Conclusion: Lilith在攻击成功率和效用降低之间取得了平衡，同时揭示了基于精确触发的评估所忽视的更广泛威胁。
...
### [140] [A Reference-Free Score for Detecting Silent Reasoning Failures in Large Language Models](https://arxiv.org/abs/2607.26102)
...
### [141] [GPT-Red: Automated Red Teaming via Self-Play at Scale](https://arxiv.org/abs/2607.26115)
...
TL;DR: GPT-Red是一个高效的自动化红队攻击代理，用于提高大型语言模型的鲁棒性


<details>
  <summary>Details</summary>
Motivation: 提高生产系统的鲁棒性

Method: 设计可扩展的自我博弈算法，使用大规模计算训练模型

Result: GPT-Red能够可靠地突破GPT-5.5及以下模型，发现比人类红队人员更成功的攻击，并能推广到不同的环境和模型

Conclusion: GPT-Red是一个高效的自动化红队攻击代理，有助于提高大型语言模型的鲁棒性
...
### [142] [(EC)2: Event-Centric Explainability for Cybersecurity Through Multi-Agent LLM Investigations](https://arxiv.org/abs/2607.26201)
*Neta Kirmayer,David Tayouri,Andrés Murillo,Motoyoshi Sekiya,Asaf Shabtai,Rami Puzis*

Main category: cs.CR

TL;DR: 本文提出了一种新的网络安全警报解释方法，提高了事件分类的准确性。


<details>
  <summary>Details</summary>
Motivation: 为了有效处理警报，分析师需要了解实体之间的上下文关系，并需要对涉及的实体有可操作的理解。

Method: 提出了一种以事件为中心的、与检测器无关的解释方法，用于解释中小型企业网络中的网络安全警报。我们提出了（EC）2，一个多智能体框架，它执行结构化、假设驱动的调查，以提供基于可验证证据的解释。

Result: 评估结果表明，所提出的框架通过生成具有操作意义的解释，提高了检测后的分析，同时也提高了事件分类的准确性。

Conclusion: 本文提出的方法能够有效提高网络安全警报的解释能力，有助于分析师更好地理解和处理警报。
...
### [143] [On Exercising Governance Power in Decentralized Autonomous Organizations](https://arxiv.org/abs/2607.26204)
*Vabuk Pahari,Balakrishnan Chandrasekaran,Johnnatan Messias,Krishna P. Gummadi,Abhisek Dash*

Main category: cs.CR

TL;DR: 分析DAO设计，揭示治理攻击风险。


<details>
  <summary>Details</summary>
Motivation: 研究DAO设计中信任与透明度之间的权衡，以及不良设计选择如何引入关键漏洞。

Method: 分析48个活跃的以太坊基于DAO，并分类设计选择的关键维度。

Result: 发现新的攻击类型，称为治理攻击，该攻击直接利用DAO治理机制的基本设计。

Conclusion: DAO治理合同的设计对安全性和隐私性有重大影响，需要谨慎设计以防止漏洞。
...
### [144] [Learning the Word Problem: Geodesic Lengths and Cryptographic Applications](https://arxiv.org/abs/2607.26241)
*Elisabeth Fink*
...
### [145] [StealthBench: Measuring Operational Stealth in Autonomous Offensive-Security Agents](https://arxiv.org/abs/2607.26314)
*Ads Dawson,Adrian Wood*

Main category: cs.CR

TL;DR: StealthBench测试了自动进攻安全代理的隐蔽性，发现OPSEC失败在模型家族中普遍存在，并发布了公开基准测试


<details>
  <summary>Details</summary>
Motivation: 研究自动进攻安全代理的作战隐蔽性

Method: 提出StealthBench基准测试，测量代理在六个作战安全维度上的隐蔽性

Result: 所有模型的安全成功率均未超过54%，表明OPSEC失败在模型家族中是普遍存在的

Conclusion: StealthBench可作为公开基准测试，支持隐蔽感知代理的开发和自动OPSEC监控
...
### [146] [A Controlled Candidate-Set Benchmark for Offline Satellite-Security Plan Decomposition](https://arxiv.org/abs/2607.26371)
...
### [147] [QUIC-TRIP: A Triple-Redundant Journey Toward Secure Substation Communications](https://arxiv.org/abs/2607.26379)
...
### [148] [Recover, Decode, Reguard: Guard-Agnostic Defense Amplification againstEncoded VLM Jailbreaks](https://arxiv.org/abs/2607.26574)
...
### [149] [CDN Tsunami: Exploiting HTTP/3-HTTP/1.1 Conversion for DoS Attacks](https://arxiv.org/abs/2607.26589)
*Ziyu Lin,Tianlong Su,Yingjie Lin,Prosanta Gope,Yinzhi Cao,Ximeng Liu,Biplab Sikdar*

Main category: cs.CR

TL;DR: HTTP/3在CDN中引入了新的DoS攻击风险，已发现42,330个子域可能受到攻击。


<details>
  <summary>Details</summary>
Motivation: 研究HTTP/3在CDN中的应用对拒绝服务攻击（DoS）的影响，特别是CDN和主机网站之间HTTP/3和HTTP/1.1的异构部署。

Method: 设计攻击实验，进行大规模测量，并负责披露攻击细节。

Result: 发现42,330个子域可能受到攻击，两家CDN厂商已承认漏洞并部署了缓解措施。

Conclusion: HTTP/3在CDN中的应用增加了新的DoS攻击风险，需要进一步研究并采取缓解措施。
...
### [150] [Guarding Organizations Against Malware Risk: A Novel Graph-Based Malware Detection Method](https://arxiv.org/abs/2607.26634)
*Yinan Gao,Jiarong Xu,Xiaohang Zhao,Xiao Fang*

Main category: cs.CR

TL;DR: MalGuard通过图方法提高了恶意软件检测的准确性和效率。


<details>
  <summary>Details</summary>
Motivation: 随着组织数字化的发展，网络安全风险不断增加，特别是在信息系统领域。恶意软件已成为一种普遍且具有破坏性的威胁。基于字节的机器学习方法在恶意软件检测中得到广泛应用，但容易受到逃避检测行为的攻击。图方法在这方面受影响较小，因为它将软件表示为程序图，从而捕获执行行为。然而，它们并没有明确识别实现有意义程序行为的基块组，也没有学习足够表达性的程序图表示以进行准确检测。

Method: 我们提出了MalGuard，这是一种基于图的恶意软件检测方法，用于组织恶意软件风险管理。MalGuard引入了两种方法创新：一种操作角色识别方法和一种程序图表示学习方法。前者将基块组识别为操作角色，使检测器能够捕获从孤立的基本块中不可见的程序行为。后者通过建模操作角色之间的交互，保留稀疏的恶意信号并捕获层次图结构来学习表达性的程序图表示。

Result: 大量实验表明，MalGuard提高了检测性能并降低了未检测到恶意软件的预期成本。

Conclusion: MalGuard通过引入操作角色识别和程序图表示学习方法，提高了恶意软件检测的性能，并降低了未检测到恶意软件的风险。
...
### [151] [Borrowed Strength: Best-of-N Search over a Code EncodingBreaks Self-Check Jailbreak Defenses](https://arxiv.org/abs/2607.26639)
*Haoyu Zhang,Shibo Zheng,Xiangchen Guan,Zhuoxi Wang,Zijian Xiao,Mohammad Zandsalimy,Shanu Sushmita*

Main category: cs.CR

TL;DR: 本文发现并修复了SAGE自我检查防御的漏洞，并揭示了攻击方式及其影响。


<details>
  <summary>Details</summary>
Motivation: 提高对SAGE自我检查防御的深入理解，并发现潜在的安全漏洞。

Method: 分析SAGE自我检查防御，构建攻击组合，研究攻击效果和影响因素。

Result: 发现SAGE防御可被组合攻击突破，并揭示了攻击方式和影响因素。

Conclusion: 本文发现并修复了SAGE自我检查防御的漏洞，并揭示了攻击方式及其影响。
...
### [152] [Fingerprint-Driven Automation: Coupling Reconnaissance with POC Verification](https://arxiv.org/abs/2607.26655)
*Hongping Wang,Xiaoqi Li*

Main category: cs.CR

TL;DR: 开发了一种高度自动化的网络安全侦察和漏洞验证工具，提高了用户体验和工作效率。


<details>
  <summary>Details</summary>
Motivation: 为了解决现有网络安全侦察工具的准确性低、缺乏隐蔽性、耗时、工具集成度低或高、难以启动使用等问题，提高用户体验和工作效率。

Method: 设计并开发一个高度自动化的侦察和漏洞验证工具，包括工具功能设计和具体功能的实现与测试。

Result: 实现了自动侦察、深入数据处理、自动漏洞检测和验证等功能，并通过在线安全模拟范围的测试验证了工具的可行性。

Conclusion: 该工具在自动化、检索和网络安全操作中的应用具有优势。
...
### [153] [Graph Is the Verifier: Agentic Reinforcement Learning for Interprocedural Vulnerability Detection](https://arxiv.org/abs/2607.26656)
...
TL;DR: VulAgentRL：一种基于代理的强化学习框架，用于跨程序漏洞检测，优于现有基线。


<details>
  <summary>Details</summary>
Motivation: 大多数基于学习的检测器将每个功能单独分类，而现实世界的漏洞通常跨越多个功能。

Method: 提出了一种名为VulAgentRL的基于代理的强化学习框架，该框架基于代码属性图（CPG）。

Result: VulAgentRL在严格的双向正确性指标上优于最先进的基线，同时发出更少的工具调用。

Conclusion: VulAgentRL能够有效地检测跨程序漏洞，并具有更好的性能和效率。
...
### [154] [Not In My Git Yard: Catching Backdoors at Commit and Release Time](https://arxiv.org/abs/2607.26719)
*Dimitri Kokkonis,Michaël Marcozzi,Stefano Zacchiroli*

Main category: cs.CR

TL;DR: Lily是一种自动化的后门检测方法，可以增强开源软件的安全性。


<details>
  <summary>Details</summary>
Motivation: 开源软件中代码级别后门的存在构成了持续的威胁，现有方法难以检测和防御。

Method: 提出了一种名为Lily的自动化方法，将后门检测机制集成到持续集成(CI)管道和发布审核流程中。

Result: Lily能够有效地检测后门，并防止恶意代码进入大型生态系统，如Linux发行版。

Conclusion: Lily实现了高检测精度和低误报率，能够可靠地识别恶意代码，并有效抵御攻击。
...
### [155] [FARI: Robust One-Step Inversion for Watermarking in Diffusion Models](https://arxiv.org/abs/2607.26723)
*Jindong Yang,Han Fang,Weiming Zhang,Nenghai Yu,Kejiang Chen*

Main category: cs.CR

TL;DR: FARI：一种快速且鲁棒的反演水印提取框架


<details>
  <summary>Details</summary>
Motivation: 为了提高基于反演的水印技术在实际应用中的效率和鲁棒性

Method: 提出了一种名为FARI的快速反演框架，并结合轻量级对抗LoRA微调去噪器进行水印提取

Result: FARI在速度和鲁棒性方面都取得了显著提升，仅需大约20分钟在NVIDIA RTX A6000 GPU上进行微调，即可在鲁棒性方面超越50步DDIM反演，同时大幅减少推理时间

Conclusion: FARI框架有效地提高了基于反演的水印技术的效率和鲁棒性，为水印提取提供了一种新的解决方案
...
### [156] [Verifiable Random Sampling](https://arxiv.org/abs/2607.26734)
*Yeoh Wei Zhu,Soorya Rethinasamy,Anthony Alexiades Armenakas,Yash Satsangi,Shaltiel Eloul,Ruslan Shaydulin*

Main category: cs.CR

TL;DR: 我们提出了一种基于量子计算的VRS构造，以解决VRF的局限性，并确保了其安全性。


<details>
  <summary>Details</summary>
Motivation: 为了解决可验证随机函数（VRF）在公开可验证的伪随机函数评估上的局限性，我们引入了可验证随机采样（VRS）的概念。

Method: 我们提出了一种基于随机量子电路采样（RCS）的VRS构造，该构造可在今天的量子计算设备上执行。

Result: VRS支持多方协议，其中验证器的最终输出是一个与指定目标分布统计上接近的样本，同时保持公开可验证。我们在这个构造中建模并证明了其在构造密码学（CC）框架下的安全性，从而确保与其他密码协议的可组合性。

Conclusion: 我们的结果提供了一种可验证随机采样的机制，同时保证了样本的新鲜性和公开可验证性，使需要不可预测、新鲜随机性的应用成为可能，同时通过公开可验证性来保持公平性。
...
### [157] [SecRespond: Benchmarking AI Agents for Real-World Post-Compromise Incident Response](https://arxiv.org/abs/2607.26791)
*Lehan Wang,Boli Chen,Ruixue Ding,Pengjun Xie,Jinwei Huang,Zhendong Liu,Shuo Wang,Tao Lei,Xin Ouyang,Xiaomeng Li*

Main category: cs.CR

TL;DR: SecRespond基准评估LLM在妥协后事件响应中的表现，揭示构建这些代理的基本瓶颈。


<details>
  <summary>Details</summary>
Motivation: 现有的网络安全基准主要集中在攻击发生前的妥协设置，而对妥协后的设置探索不足。

Method: 提出SecRespond基准，评估LLM在妥协后事件响应工作流程中的表现。

Result: 实验结果表明，尽管当前代理可以可靠地揭示由警报暴露的问题，但它们在积极调查磁盘以查找静默入侵以及产生全面、经过验证的修复计划方面存在困难。

Conclusion: SecRespond基准有助于评估LLM在真实世界事件响应中的表现，并揭示了构建这些代理的基本瓶颈。
...
### [158] [Before Agents Speak: Pre-hoc Failure Risk Inference in Multi-Agent Systems](https://arxiv.org/abs/2607.26836)
*Shi Lin,Chenpei Wang,Peng Qian,Dezhang Kong,Minghao Li,Yufeng Li,Xun Wang*

Main category: cs.CR

TL;DR: 提出了HalluProp，一个预先风险推理框架，用于减少LLM-based多智能体系统中的系统风险。


<details>
  <summary>Details</summary>
Motivation: LLM-based multi-agent systems在协同推理和决策方面表现出色，但其相互连接的通信引入了新的系统风险：局部幻觉可以沿着代理通信链传播，通过交互放大，最终触发级联故障。

Method: 提出HalluProp，一个感知传播的幻觉推理框架，该框架在代理交互之前估计个体代理故障和系统级幻觉风险。通过识别代理角色和任务查询之间的细粒度语义不匹配来建模内在幻觉风险，通过建模语义影响和通信拓扑来表征代理之间的风险传播。最后，通过可微的Noisy-OR推理机制将这些风险集成以得出系统诊断。

Result: HalluProp准确定位有故障的代理，平均AUROC为84.6%，同时实现亚秒级诊断，比事后方法快65倍以上。通过上游筛选促进早期干预，HalluProp有效地补充了事后方法，突出了预先风险推理在构建更可靠的代理系统中的潜力。

Conclusion: HalluProp是一个有效的预先风险推理框架，可以减少MAS中的系统风险，提高系统的可靠性。
...
### [159] [ToxScreen: Detecting Whether an LLM Has Been Poisoned](https://arxiv.org/abs/2607.26849)
...
### [160] [Defending Against Backdoor Attacks via Alignment Checking in Model-Contrastive Federated Learning](https://arxiv.org/abs/2607.26933)
*Hongliang Zhang,Zhongyuan Yu,Guijuan Wang,Tianqing He,Wenshuo Ma,Xiaosong Zhang,Jiguo Yu*

Main category: cs.CR

TL;DR: FedDAB：一种防御Federated Learning后门攻击的新方法


<details>
  <summary>Details</summary>
Motivation: Federated Learning (FL)易受后门攻击，现有防御方法效果有限

Method: FedDAB，结合局部对比正则化和对齐检查的两阶段方法

Result: FedDAB在防御后门攻击方面优于现有防御方法

Conclusion: FedDAB通过提高更新的一致性和对齐检查，提高了FL的鲁棒性
...
### [161] [InkShield: Writing Style Protection Against Unauthorized Handwriting Mimicry](https://arxiv.org/abs/2607.26976)
*Jian Xiong,Wenbo Jiang,Zihan Wang,Rui Zhang,Wenshu Fan,Hongwei Li,Guowen Xu*

Main category: cs.CR

TL;DR: InkShield is a proactive defense against handwritten text generators, reducing the success rate of style mimicry and maintaining visual quality and readability.


<details>
  <summary>Details</summary>
Motivation: Recent handwritten text generators can reproduce a writer's style from publicly available references, posing risks of document forgery and identity misuse.

Method: Proposes InkShield, a proactive writing-style defense that protects reference images before release. It selects a decoy writer to define a style-displacement direction, optimizes perturbations with a frozen handwriting-generation surrogate, and confines them to ink-stroke edges.

Result: On IAM, the average Top-1/Top-5 rates at which generated samples are retrieved as the target writer decrease from 11.94%/36.52% to 2.03%/8.79%. The protected references remain visually close to the originals (LPIPS 0.0078), and the generated text remains readable. InkShield also exhibits transferability to other handwriting generators.

Conclusion: InkShield provides practical protection against unauthorized handwriting style mimicry.
...
### [162] [AgentSnare: Learning to Delay, Divert, and Defuse Autonomous Penetration Agents](https://arxiv.org/abs/2607.26998)
*Ruoyu Wang,Heng Zhao,Renjie Wu,Mengnan Zhao,Zhixuan Chu,Wanyu Lin,Tianhang Zheng*

Main category: cs.CR

TL;DR: AgentSnare是一种有效的欺骗系统，可以延迟渗透测试代理的攻击并减少对真实目标的利用。


<details>
  <summary>Details</summary>
Motivation: 为了解决现有防御方法在对抗高级渗透测试代理时的局限性，特别是代理能够识别和绕过预先放置的静态诱饵的问题。

Method: 引入了AgentSnare，一个轨迹自适应欺骗系统，该系统动态地展开一个诱饵环境，以持续地将渗透测试代理引开。AgentSnare使用一个基于代理交互历史和诱饵状态的诱饵构建策略模型来构建候选诱饵。然后，AgentSnare验证这些候选诱饵，并将有效的诱饵逐步纳入一个事实上一致的诱饵环境中。

Result: 在15个CVE-Bench网络应用程序和三种攻击者模型上，AgentSnare在诱饵中吸收了代理的46.8%的工具调用，并在其中保留了55.9%的进入后的动作，而90.0%的完成尝试都是基于诱饵证据的；在所有45个攻击者-CVE对中，没有在pass@3时成功利用真实目标。

Conclusion: AgentSnare通过动态展开诱饵环境，有效地延迟了攻击，并减少了攻击者对真实目标的利用。
...
### [163] [HoF-Bench: Rediscovering Real AI-Discovered CVEs Without Frontier Models](https://arxiv.org/abs/2607.27030)
...
### [164] [MemSecBench: Tracking Agent Memory Poisoning from Persistence to Consequence and Repair](https://arxiv.org/abs/2607.27080)
...
### [165] [Function Privatization in the Local Model](https://arxiv.org/abs/2607.27164)
*Yuting Liang,Tian Shu,Ke Yi*

Main category: cs.CR

TL;DR: 通过Geo-Privacy（GP）概念实现了曲线数据的私有化。


<details>
  <summary>Details</summary>
Motivation: 研究私有化函数的问题，特别是曲线，这些曲线是连续函数在有限区间上的图像。许多类型的数据自然地表现为曲线，如轨迹数据或1D密度曲线。主要关注局部模型设置，其中要私有化的函数捕获属于单个个体的数据，这是一个具有有限工作的更具挑战性的设置。

Method: 采用标准局部差分隐私（DP）概念，任何两个任意不同的函数都要求在私有化后变得不可区分，这太强了，不允许有意义的效用；因此，我们使用广义的DP概念，称为地理隐私（GP），它允许更轻松地区分远离的函数，同时为接近的函数提供强大的保护。

Result: 在几个数据集上提供实验评估以证明框架的有效性。

Conclusion: 通过使用Geo-Privacy（GP）概念，实现了对曲线数据的私有化，同时保持了数据的有效性。
...
TL;DR: 研究揭示深度强化学习代理在训练过程中自发形成稀疏表示的现象，并揭示了其背后的机制和影响。


<details>
  <summary>Details</summary>
Motivation: 探索深度强化学习代理在使用冻结的随机初始化CNN特征提取器训练时自发形成的极稀疏全连接表示现象。

Method: 通过分析不同游戏的代理在训练过程中的表现，并对比冻结和可训练CNN的激活情况。

Result: 发现FC1的稀疏性与任务复杂性成正比，同一任务的不同随机种子会导致不同的神经元激活，去除活跃神经元会导致性能下降，信息瓶颈在早期就存在，冻结和可训练CNN通过不同的瓶颈结构达到相似的性能。

Conclusion: 冻结的随机投影可能在不需要显式稀疏机制的情况下揭示潜在问题的有效秩。
...
### [167] [Top-$k$ Pareto Bandits: Hypervolume Regret for Multi-Objective Slate Selection](https://arxiv.org/abs/2607.26273)
...
### [168] [Sim2Win: A Team-Agnostic, Event-Based Pre-Match Outcome Prediction and Tactical Profiling System for Football](https://arxiv.org/abs/2607.26061)
...
TL;DR: 提出了一种名为kOMB的新算法，用于从数据中学习变量的图形马尔可夫毯，即使在忠诚度假设被违反的情况下也能有效工作。


<details>
  <summary>Details</summary>
Motivation: 许多领域如贝叶斯网络和马尔可夫随机场的结构学习、因果发现和特征选择都应用了从数据中学习变量图形马尔可夫毯（MB）的问题。

Method: 提出了一种对忠诚度假设的“k阶”放松，以捕捉k+2变量之间的奇偶型关系。然后提出了一种名为k阶马尔可夫毯（kOMB）的概念验证算法，该算法使用这种放松进行MB发现。

Result: 通过实验表明，kOMB可以在忠诚度真实和经验违反的情况下恢复变量的MB。

Conclusion: 该研究提出了一种新的方法，用于从数据中学习变量的图形马尔可夫毯，即使在忠诚度假设被违反的情况下也能有效工作。
...
TL;DR: 提出EFDM，为空间点过程生成建模提供了一种灵活和通用的方法。


<details>
  <summary>Details</summary>
Motivation: 研究空间点过程（SPP）的生成建模，其中点的数量和空间配置都受联合分布控制。

Method: 提出存在场扩散模型（EFDM）进行空间点过程建模，每个潜在点都与一个表示其存在程度的变量相关联。

Result: 提供了一种灵活和通用的框架，用于生成建模空间点过程，在具有不同数量级的数据集上实现了改进的建模能力。

Conclusion: EFDM能够联合建模空间位置和数量，无需显式离散转换，为空间点过程的生成建模提供了一种灵活和通用的方法。
...
### [172] [Data Fusion and Contrastive Alignment for Unconstrained IR Molecular Structure Elucidation](https://arxiv.org/abs/2607.26164)
*Ethan J. Mick,Campbell A. Sweet,Matthias J. Young,Derek T. Anderson*

Main category: cs.LG

TL;DR: 提出了一种改进的转换器模型，实现了从红外光谱数据中自动解析分子结构，显著提高了预测准确率。


<details>
  <summary>Details</summary>
Motivation: 提高从红外光谱数据中自动解析分子结构的能力，突破对预先确定化学公式的依赖，实现分子全结构预测。

Method: 提出并评估了传统编码器-解码器转换器的关键修改，包括实现一个新型的混合专家（MoE）解码器模块，使用线性顺序统计和Choquet积分进行非加性聚合，并修改转换器以使用这些非加性算子聚合光谱表示。

Result: 与仅红外光谱的基线模型相比，Top-K预测准确率提高了10个百分点以上。

Conclusion: 该研究通过从测量的红外光谱中自动解析分子结构，显著扩大了AI在分析化学中的应用范围。
...
TL;DR: 研究了具有随机节点特征的PENNs，证明了其强大的函数逼近能力。


<details>
  <summary>Details</summary>
Motivation: 为了研究具有随机节点特征的图神经网络，并建立关于偏移等变神经网络（PENNs）的新奇异性结果。

Method: 理论分析和实证研究。

Result: 证明了PENNs结合部分随机节点特征可以近似任意可测的排列不变或排列等变函数，并推导了连续可微函数的近似率上界。

Conclusion: 为PENNs在图神经网络中的应用提供了理论基础和近似率的指导。
...
TL;DR: 通过在不同研究领域之间转移SFT课程，可以提高研究水平，并建议更多研究人员借鉴其他领域的技术。


<details>
  <summary>Details</summary>
Motivation: 在所有三个领域中，项目通常被视为不同的研究领域。但是，所有三个领域中的项目经常使用监督微调（SFT）来实现相同的潜在目标。当项目有共同目标时，我们应该测试从一个领域学到的经验是否可以转移到其他领域。

Method: 研究了三个这样的转移，每个转移都是从一个SFT设置中发展出一课，并在另一个设置中进行测试。首先，将来自对齐训练中的行为泛化教训移植到玩具模型中。在关于行为原因的训练上，例如在教Claude Why上，可以使行为比仅训练行为示例更好地泛化。其次，将来自模型生物的关于能力保持的教训移植到模型-特定中训练对齐设置中。在训练非学生模型（离模型输出）的输出时，SFT可以损害能力。在我们的训练中混合良性在线模型（和在线策略）数据可以防止大多数这种损害，同时仍然嵌入目标行为。第三，将来自模型生物的关于鲁棒性的教训移植到相同的对齐设置中。我们发现后续的良性SFT可以消除对齐行为，同时保持能力，表明仅能力保持本身并不能确保对后续训练的鲁棒性。

Result: 我们的工作说明了如何在不同的研究领域之间转移SFT课程，从而提高所有这些领域的水平，建议更多的研究人员应该从他们自己的领域之外借用技术。

Conclusion: 将SFT课程从不同的研究领域转移到其他领域可以提高所有这些领域的水平，并为研究人员提供了借鉴其他领域技术的建议。
...
TL;DR: BayesAME：一种自动确定coreset大小的贝叶斯框架，提高大型生成模型评估效率。


<details>
  <summary>Details</summary>
Motivation: 评估大型生成模型需要大量时间和计算资源，因此需要方法来通过评估模型的一小部分项目（称为coreset）来估计完整基准性能。

Method: 提出BayesAME，一个针对自动确定coreset大小的序列贝叶斯框架。通过定义每个共享相同历史模型性能的项目组的潜在能力，并使用联合先验分布来编码目标模型与这些历史模型行为相似的信念，将性能建模为随机变量。使用后验分布来推导性能估计器，量化性能不确定性，并通过信息增益标准选择添加到coreset的项目。迭代增加coreset，直到性能估计波动和性能不确定性低于用户定义的阈值。提出多目标扩展，以捕捉多个目标模型之间的性能相关性，进一步减少coreset大小。

Result: 在多个基准上进行了广泛的实验，表明BayesAME在性能上优于现有方法的序列自适应。非随机coreset选择优于随机选择。使用连续响应对数似然比传统二值分数显著提高估计精度。

Conclusion: BayesAME是一个有效的框架，可以自动确定coreset大小，从而提高大型生成模型评估的效率和准确性。
...
TL;DR: W2S-OPD通过构建代理教师模型，显著提升了学生模型的能力。


<details>
  <summary>Details</summary>
Motivation: 为了提高强学生的能力，提出了一种从多个弱模型中蒸馏的能力迁移范式。

Method: 提出了一种名为W2S-OPD的OPD框架，该框架通过构建一个代理教师模型来提升学生模型的能力。代理教师模型由正负模型的对数空间构建，这两个模型都比学生模型小且易于获得。

Result: 在四个数学和三个代码基准测试中，W2S-OPD优于OPD，使学生能够超越领域教师，并且在每个监督源都较弱的情况下，仍然能够提升学生模型。

Conclusion: W2S-OPD是一种简单而有效的OPD框架，通过构建代理教师模型，能够显著提升学生模型的能力。
...
TL;DR: ULoRA通过优化初始化方法显著提高了LoRA的性能，使其在多个任务上达到与全微调相当的性能，且在部署时无需搜索。


<details>
  <summary>Details</summary>
Motivation: 降低低秩适配（LoRA）的初始化成本，提高其性能。

Method: 提出了一种名为Unified LoRA（ULoRA）的初始化方法，该方法是一种基于谱白化指数和Adam-like对角指数的两参数预条件梯度初始化方法。

Result: ULoRA在GLUE任务和GSM8K任务上均表现出色，其性能与全微调相当，且在部署时无需搜索。

Conclusion: LoRA初始化和曲率预条件的设计空间应该被视为可调的维度，而不是固定的设计决策。
...
TL;DR: Developed and evaluated a self-serve ER system, identified key lessons for system development


<details>
  <summary>Details</summary>
Motivation: Evaluate the performance of a self-serve entity resolution (ER) system

Method: Built and evaluated a self-serve ER system on six benchmarks

Result: Identified three lessons for ER system development

Conclusion: These lessons can save practitioners time in ER system development
...
TL;DR: 隐式因果关系世界模型在多智能体系统中的训练和预测中表现出色，特别是在处理分布变化和提供可解释性方面。


<details>
  <summary>Details</summary>
Motivation: 在基于模型的重强化学习中，世界模型作为内部模拟器存在，但其训练往往将统计相关性混同于因果关系。在多智能体系统中，这一问题是加剧的，因为物理转换与智能体战略意图交织在一起，导致世界模型在分布变化下失效。

Method: 我们引入了隐式因果关系世界模型，从离线演示中恢复环境动态，而无需预定义因果关系图。通过结合策略方差，我们使世界模型可以通过顺序后门条件被发现。

Result: 在协调任务（双门、导航和让行）的评估中，这些模型在完全和部分可观察性下都提供了可解释的因果关系表示，模型精度直接与干预强度成比例。

Conclusion: 隐式因果关系世界模型在多智能体系统中的训练和预测中表现出色，特别是在处理分布变化和提供可解释性方面。
...
TL;DR: RAGuard通过对抗性检索器和零知识推理补丁ZKIP，有效防御了RAG管道的事实性语料库中毒攻击。


<details>
  <summary>Details</summary>
Motivation: RAG系统依赖于外部语料库，容易受到恶意注入的段落的影响，导致证据被操纵。

Method: RAGuard，一种针对RAG管道的事实性语料库中毒攻击的分层防御系统。第一层通过在合成中毒文档上对抗性微调密集检索器，使其在生成之前降低恶意段落的排名。第二层是零知识推理补丁ZKIP，它是一种无标签、黑盒过滤器。对于每个检索到的文档，它执行留一法解码，并通过移除该文档引起的语义变化和输出熵变化来对文档进行评分。

Result: 在受污染的自然问题数据集上，仅对抗性检索器训练降低了攻击成功率，而ZKIP将攻击成功率降至0.000。在自然问题和BEIR（NFCorpus）上的监督分析证实了ZKIP依赖的逆事实信号携带可学习的毒化结构。防御成本为每查询k+1个生成器遍历。

Conclusion: RAGuard系统有效地防御了RAG管道的事实性语料库中毒攻击，并通过ZKIP实现了对攻击成功率的显著降低。
...
TL;DR: MetaKoopman是一种用于建模非线性动态的贝叶斯元学习框架，在预测精度和鲁棒性方面优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 建模和预测非线性动态，在分布变化的情况下对于现实世界系统的鲁棒决策至关重要。

Method: 提出MetaKoopman，一种贝叶斯元学习框架，通过线性潜在表示建模非线性动态。

Result: MetaKoopman在多步预测精度、不确定性校准和分布变化鲁棒性方面优于先前方法。

Conclusion: MetaKoopman在动态可行的运动规划中表现出色，尤其是在避障操作和牵引极限操作时。
...
### [186] [Post-Training at the Edge of Detectability: A Game-Theoretic Approach to Fine-Tuning](https://arxiv.org/abs/2607.26358)
*Keegan Harris,Brian W. Lee,Ian Waudby-Smith,Philip Amortila,Nika Haghtalab,Michael I. Jordan*

Main category: cs.LG

TL;DR: 本文提出了一种基于博弈论的框架，通过减少到KL正则化RL目标来学习平衡系数，以实现灵活的集成和改进语言模型微调。


<details>
  <summary>Details</summary>
Motivation: 为了在目标任务上提高模型性能的同时限制与参考策略的偏差，强化学习（RL）微调被广泛用于语言模型训练。

Method: 提出了一种基于博弈论的框架，该框架给出了这种权衡的明确统计解释。具体来说，我们研究了在累积奖励最大化的过程中，一个代理选择策略，而监视器观察策略输出并测试对参考策略的偏差的顺序博弈。

Result: 结果表明，结果平衡策略可以表达为KL正则化RL问题的解，该问题的最优正则化参数可以视为最大化单位统计可区分性下的奖励。通过将问题简化为KL正则化RL目标，我们提供了一种通过减少到KL正则化RL目标来学习这种平衡系数的原理方法，从而允许灵活地集成到标准的微调流程中。

Conclusion: 实验结果表明，我们的方法在持续学习环境中实现了有竞争力的奖励保留权衡，并说明了如何使用我们的框架来审计提供开源模型的API提供商。
...
### [187] [ClockRoPE: Random Fourier Rotations for Temporal Routine Modeling](https://arxiv.org/abs/2607.26369)
*Yiwen Chen,Joshua Ainslie,Krzysztof Choromanski,Xiang Gao,Su-Lin Wu,Yiping Yuan,Qian Sun*

Main category: cs.LG

TL;DR: ClockRoPE通过周期性注意力调制函数改进了RoPE，提高了序列推荐的性能。


<details>
  <summary>Details</summary>
Motivation: 为了解决RoPE在具有更复杂距离相关模式领域（如序列推荐中的时间周期性）的局限性，提出新的解决方案。

Method: 研究了通用查询/键旋转的表达能力，提出ClockRoPE模型，其旋转频率来自周期性注意力调制函数。

Result: ClockRoPE在在线A/B测试中表现出色，并已在大型视频分享平台的生产规模生成检索系统中成功部署。

Conclusion: ClockRoPE是RoPE的一个有效扩展，适用于具有复杂距离相关模式的领域，如序列推荐。
...
### [188] [Q-Steer: Action-Value Guidance for Molecular Policy Optimization](https://arxiv.org/abs/2607.26391)
...
### [189] [Forecasting Trajectory-Level Safety Risks in Black-Box Multi-Turn Interactions](https://arxiv.org/abs/2607.26820)
*Shi Lin,Peng Qian,Dinghao Liu,Renjie Sun,Sifan Wu,Dezhang Kong,Chenpei Wang,Xun Wang*

Main category: cs.LG

TL;DR: Recast通过轨迹级预测，有效识别潜在风险，提高LLMs安全性。


<details>
  <summary>Details</summary>
Motivation: 为了解决现有大型语言模型（LLMs）安全保护中存在的局限性，如缺乏对潜在风险演变的预测和预防能力。

Method: 提出了一种名为Recast的安全风险预测框架，该框架通过双重尺度轨迹视图从短期对话进展和长期历史背景中检索风险相关证据，并利用因果时间编码器学习潜在风险演变模式，预测未来风险出现的分布。

Result: Recast在7个风险类别上的实验中，预测了88.3%的未来安全故障，平均提前2.41个回合，同时保持12.3%的误报率。

Conclusion: Recast在识别潜在风险方面表现出色，有助于提前预防安全故障。
...
TL;DR: SGFlow通过绕过可逆性约束和昂贵的微分，在图像生成任务中实现了高效的流映射学习。


<details>
  <summary>Details</summary>
Motivation: 为了解决扩散和基于流的模型在推理过程中由于采样需要积分而导致的显著开销问题。

Method: 提出了一种名为SGFlow的新方法，该方法通过绕过显式的可逆性约束和通过模型迭代进行昂贵的微分来学习流映射。SGFlow训练模型从头开始计算ODE解和隐含速度，通过跟随具有所需流映射为驻点的非保守动力学。

Result: 在CIFAR图像基准测试中，SGFlow在10个采样步骤中达到了最佳的FID，在其他步骤数中与flow matching、Meanflow和Lagrangian map matching保持竞争力，并且是唯一一个对其基于stopgrad的动力学有驻点保证的方法。

Conclusion: SGFlow是一种有效的学习流映射的方法，在图像生成任务中表现出色。
...
TL;DR: 本文研究了GNN在回归任务中的有效性，发现深度卷积GNN在特定任务中优于其他类型的GNN。


<details>
  <summary>Details</summary>
Motivation: 研究GNN在回归任务中的有效性，特别是与分类任务的比较。

Method: 对GNN层在不同回归场景中的表现进行评估，包括排序、误差最小化和洞察提取。

Result: 深度卷积GNN（特别是GEN）在这些任务上比基于注意力的GNN更有效，而其他经典和理论启发的GNN仍然具有竞争力且效率高。

Conclusion: GNN在回归任务中的表现优于分类任务，深度卷积GNN在特定任务中表现最佳。
...
TL;DR: DHRCL是一种基于强化学习的代码生成框架，能够有效提高代码质量。


<details>
  <summary>Details</summary>
Motivation: 强化学习是大语言模型进行代码训练的自然范式，因为生成的程序可以通过解析、执行、单元测试和结构分析来评估。然而，现有方法通常依赖于稀疏的结果奖励或静态地组合异构的密集信号。

Method: 提出了一种名为DHRCL的强化学习框架，该框架具有密集层次奖励和课程学习。DHRCL将反馈分解为语法验证、执行成功、单元测试通过率和基于AST的结构相似性，并通过三个阶段的语法、执行、通过和结构课程组织这些信号。

Result: 在Qwen3-8B和KodCode协议下，实验将DHRCL与二进制、通过率、基于奖励模型的和可验证的密集奖励基线进行比较。在Qwen3-4B、Qwen3-8B和Qwen3-14B骨干网络上评估DHRCL，表明随着模型容量的增加，其优势保持一致。

Conclusion: DHRCL在代码生成任务中表现出色，能够有效提高代码质量。
...
TL;DR: 本文综述了NAS在交通预测中的应用，分析了搜索策略和开放挑战，并提出了未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 交通预测是智能交通系统的核心任务，需要支持自适应信号控制、路线指导和打车调度等应用。然而，现有的深度学习模型虽然取得了很好的结果，但它们的架构是手工设计的，需要大量的专家努力，并且通常在跨城市和数据集上的泛化能力较差。神经架构搜索（NAS）提供了一种系统性的替代方案。

Method: 本文综述了应用于交通预测的NAS方法，按照搜索策略分为：基于梯度的方法、进化方法和单次权重共享方法。对于每个类别，我们分析了搜索空间是如何设计来覆盖空间和时间交通算子，以及搜索策略如何平衡成本与架构质量。

Result: 本文讨论了开放挑战，包括大规模道路网络的计算可扩展性、手动搜索空间设计、跨城市泛化、动态图结构以及NAS在时空基础模型中的应用的开放问题，并确定了未来研究的方向。

Conclusion: NAS为交通预测提供了一种自动化的架构搜索方法，有助于提高模型的泛化能力和计算效率。
...
TL;DR: IRIS是一种新的个性化LLM框架，通过隐式行为建模实现了高准确率的用户角色识别和决策预测。


<details>
  <summary>Details</summary>
Motivation: 为了提高用户体验，个性化大型语言模型（LLMs）对于个人用户至关重要，但现有方法通常依赖于显式偏好监督，如成对比较或人口统计属性，限制了它们在自然交互环境中的应用。

Method: 我们提出了IRIS框架，该框架通过从日常对话中提取行为信号，直接从隐式交互流中学习动态用户角色，并通过预测驱动的闭环迭代地细化角色表示，而不需要显式反馈。

Result: 在基于行为预测、角色稳定性和决策预测的评估协议中，IRIS在从公共领域自传文本中派生出的合成交互流上的概念验证研究表明，IRIS产生了稳定的角色，并区分了个人用户，同时揭示了仅基于记忆的方法在回忆导向指标上的局限性。在Reddit r/AmItheAsshole (AITA)数据上，IRIS在100名作者中实现了所有评估方法中决策预测的最高准确率（61.0%），优于静态角色、仅记忆检索和无个性化基线。

Conclusion: 这些结果表明，隐式行为建模为个性化LLMs提供了一种可扩展的替代方案，为需要持续演变的用户模型的适应性对话系统和具身智能体提供了实际的基础。
...
TL;DR: Developed and evaluated a snow-water MCP network framework showing its effectiveness in hydrologic modeling.


<details>
  <summary>Details</summary>
Motivation: Develop a snow-water MCP network framework and evaluate it across 513 CAMELS-US basins.

Method: Reformulate conceptual hydrologic models as mass-conserving neural networks, examine cross-node state-information sharing, and evaluate broader single-layer networks.

Result: Achieved comparable predictive performance, identified optimal state configurations, and provided empirical basis for hydrologic representation.

Conclusion: MCP networks are effective for hydrologic modeling and provide a framework for optimizing model complexity and predictive accuracy.
...
TL;DR: 本文提出了一种新的模型，能够支持任意顺序推理，并提高了非因果任务的性能。


<details>
  <summary>Details</summary>
Motivation: 许多离散推理任务，如代码生成，本质上是非因果的，程序员在高级结构和局部细节之间移动，我们称之为任意顺序推理。自回归语言模型缺乏本地的任意顺序接口，因此需要手动设计机制来处理非因果能力，如填充和下一个编辑预测。

Method: 提出两种互补的方法：基于插入的掩码扩散模型和潜在空间掩码扩散模型，通过插入放松固定位置承诺，实现跨非连续区域的生成，将预测转移到更粗的语义段，从而实现潜在生成顺序的搜索。

Result: 实验结果表明，这两种方法都诱导了不同的任意顺序推理行为，并提高了下游性能。

Conclusion: 我们可以设计支持任意顺序推理的模型，这可以提高非因果任务的性能。
...
TL;DR: Proposes CWAC to improve training stability and performance in deep off-policy reinforcement learning for continuous control.


<details>
  <summary>Details</summary>
Motivation: Deep off-policy reinforcement learning algorithms for continuous control often face challenges due to noisy targets and early-stage estimation errors.

Method: Proposes Collaborative Weighting Actor-Critic (CWAC) framework that accounts for predictive uncertainty in value estimation and introduces a collaborative weighting mechanism.

Result: Enhances performance across a diverse range of simulated tasks.

Conclusion: CWAC significantly improves the training stability and performance of deep off-policy reinforcement learning algorithms for continuous control.
...
TL;DR: 提出了一个结合因果发现和不确定性感知的亚组选择框架，用于政策优先级排序，并评估了不同数据集下的政策效用。


<details>
  <summary>Details</summary>
Motivation: 传统亚组分析可能导致不稳定且难以解释的结论，尤其是在观察性生物医学数据中，每个个体只在一个暴露状态下被观察，真实的个体治疗效果不可用，因果结构不确定。

Method: 我们提出了一个框架，结合因果发现信息化的协变量选择、发现评估样本分割、归纳无监督聚类、不确定性感知的亚组选择和保留的 doubly robust 政策评估。

Result: 在 PIMA 印第安人糖尿病数据集中，对于假设的肥胖到非肥胖和血糖升高到血糖降低状态的变化，以及 NHANES 中的终身吸烟史对比，我们评估了 70% 预算下的政策。使用贝叶斯 GMM 的 BMI 政策估计的未门控效用最高，为 0.799；使用硬或成员加权 FCM 的血糖政策估计的效用为 0.735；使用 K-means 的吸烟史政策估计的效用为 0.775。所有政策风险的 95% 置信区间均包含零，经过 Holm 调整后，没有比较结果具有统计学意义。贝叶斯汇总通常保留了未门控分配，而经验贝叶斯门控则更为保守。具有相似估计效用的政策可能优先考虑不同的个体。

Conclusion: 这些发现应被视为假设状态对比的假设依赖决策支持证据，而不是干预益处的证明。
...
TL;DR: CMP架构在持续学习中表现出色，有助于减少灾难性遗忘。


<details>
  <summary>Details</summary>
Motivation: 研究稀疏表示、局部学习和持久记忆结合是否能减少灾难性遗忘，并探索其在持续学习中的应用。

Method: 设计了一种名为CMP的持续学习架构，该架构使用稀疏关系编码表示输入，存储在双层竞争性记忆中，并通过局部更新进行学习。在控制域增量字节级语言建模协议上进行了实验。

Result: CMP在控制域增量字节级语言建模协议上比使用在线弹性权重巩固（EWC）训练的参数匹配Transformer具有更低的反向迁移。在15个域的实验中，CMP表现出稳定的遗忘行为，且遗忘率低于Transformer。

Conclusion: 稀疏表示、局部学习和持久记忆的结合是持续学习的一个有希望的方向。
...
TL;DR: AgentGFM enhances GFMs with autonomous information-flow control for better adaptation in various graph structures.


<details>
  <summary>Details</summary>
Motivation: Graph Foundation Models (GFMs) aim to learn transferable knowledge from multi-domain graphs and adapt to unseen scenarios.

Method: Proposing AgentGFM, where all node agents follow a shared end-to-end trainable policy. Each node interacts with the graph through a predict-act-observe-correct process.

Result: Experiments demonstrate the effectiveness of AgentGFM across diverse graph topologies.

Conclusion: AgentGFM provides effective information-flow control for GFMs, improving adaptability and performance in diverse graph scenarios.
...
TL;DR: 提出了一种新的在线学习框架，解决了ACI方法的局限性，提高了预测的稳健性和效率。


<details>
  <summary>Details</summary>
Motivation: 分析自适应一致性推断（ACI）及其变体在分布变化下的在线一致性预测中的局限性。

Method: 提出一个统一的在线学习框架，控制绝对、非抵消覆盖违规和预测集效率，针对三个重要模型。

Result: 在完全对抗性设置中，对标准ACI更新进行投影在线梯度下降，对任意单调Lipschitz效率目标获得同时覆盖和效率保证。在具有完全分数反馈的随机设置中，提出滑动窗口分位数跟踪器，并建立匹配的极小极大下界，表明算法是速率最优的。在协变量相关的随机设置中，开发了一个分区ACI算法，跟踪函数值算术阈值，并推导出同时覆盖和效率保证。

Conclusion: 该研究提出了一个有效的在线学习框架，解决了现有ACI方法的局限性，并提供了更稳健和高效的预测结果。
...
TL;DR: ConvLSTM在降雨场预测中表现不佳，神经网络模型低估了降雨量。


<details>
  <summary>Details</summary>
Motivation: 测试卷积循环是否可以提高小网格上的一日降雨场预测精度。

Method: 比较了十种不同的方法，包括FC-LSTM和持久性，并分析了1998-2020年IMDAA数据集。

Result: ConvLSTM在孟买表现最佳，但与其他模型差异不大。

Conclusion: ConvLSTM在降雨场预测中并不总是优于简单替代方案，而神经网络模型低估了降雨量，并在高降雨日预测了过多的阈值超越次数。
...
TL;DR: 提出CURL以解决异质处理效应估计中的局部不稳定性问题，提高了估计的准确性。


<details>
  <summary>Details</summary>
Motivation: 针对个性化促销和精准医疗等针对性干预措施，估计异质处理效应至关重要。

Method: 提出了一种名为CURL（因果不确定性引导的表示学习）的插件适配器，该适配器使用估计器不确定性将预训练的语义能力分配给局部不稳定的单元。

Result: 在四个基准测试中，CURL在大多数设置中提高了十个主机学习器。

Conclusion: CURL通过分离的路径构建了指向异质性的表示，并通过角色条件提示查询冻结的LLM，有效地解决了局部不稳定性问题，提高了异质处理效应估计的准确性。
...
TL;DR: FedWeave框架通过非对称聚合和分离专家聚合与路由优化，有效解决了联邦PEFT中的跨任务干扰和梯度冲突问题


<details>
  <summary>Details</summary>
Motivation: 为了解决联邦PEFT在跨任务干扰和梯度冲突问题

Method: FedWeave框架，采用非对称聚合，分离专家聚合和路由优化

Result: 在异构多任务基准上，FedWeave优于强基线，消融实验验证了设计有效性

Conclusion: FedWeave通过非对称聚合和分离专家聚合与路由优化，有效解决了联邦PEFT中的跨任务干扰和梯度冲突问题
...
TL;DR: 通过优化分布方法提高了训练-测试集划分的准确性，从而提高了AutoML模型评估的准确性。


<details>
  <summary>Details</summary>
Motivation: 为了更准确地评估机器学习模型，研究统计相似性在训练-测试集划分中的作用及其对AutoML模型评估的后果。

Method: 比较了五种已建立的策略：随机划分、分层抽样、Kennard-Stone、Duplex和SPXY。使用卡方检验、Kolmogorov-Smirnov检验和最大均值差异（MMD）测试来评估相似性。

Result: 基于几何的方法在所有评估的策略中产生了接近零的MMD分数，引入了下游性能估计的不稳定性。提出的优化分布方法将相似性作为一个显式的优化目标，在所有评估的策略中实现了最高的平均MMD相似性，达到89.0%。

Conclusion: 优化分布方法在处理具有类别不平衡、自然聚类或空间自相关性的数据集时，可以提高训练-测试集划分的准确性，从而提高AutoML模型评估的准确性。
...
TL;DR: TabPFN在小型表格数据集上的分类效果较好，但上下文选择和成本是一个重要考虑因素。


<details>
  <summary>Details</summary>
Motivation: 在小型表格数据集上，TabPFN通过上下文学习进行分类。研究人员研究了上下文大小和构成对预测稳定性、准确性和选择成本的影响。

Method: 使用15个OpenML数据集进行重复上下文采样，研究上下文大小、准确性、选择成本等因素。

Result: 发现较大的上下文可以提高准确性和稳定性，但选择成本较高。K-Means和最远点采样在准确性上与随机选择相似，但选择成本高。随机采样成功的原因是它提供了特征空间覆盖率。

Conclusion: TabPFN在小型表格数据集上的分类效果较好，但上下文选择和成本是一个重要考虑因素。
...
TL;DR: AIGen是一个模块化的AIBoM生成器，支持多种AI框架，旨在提高AI系统的透明度和可追溯性。


<details>
  <summary>Details</summary>
Motivation: 确保人工智能（AI）系统的负责任开发和部署需要对其组成部分进行严格的记录，例如数据集、模型权重、训练管道和运行时依赖项。尽管SPDX 3.0标准引入了对AI和数据集配置文件的原生支持，但能够以自动化和可扩展的方式生成符合标准的人工智能物料清单（AIBoM）的工具仍然很少。

Method: AIGen是一个模块化的AIBoM生成器，它基于MLflow MLOps框架，结合挖掘启发式方法和大型语言模型来生成AIBoM。它还提供了一个插件接口，允许实践者在不修改核心代码库的情况下扩展工具，以支持Hugging Face、PyTorch和TensorFlow等异构AI框架。

Result: AIGen旨在促进符合欧盟AI法案、NIST AI风险管理框架和ISO/IEC 42001，为透明、负责任的AI供应链治理提供了一个具体、可重用的基础。

Conclusion: AIGen是一个创新的工具，旨在解决AI系统文档记录的挑战，通过提供符合SPDX 3.0标准的AIBoM生成，为AI系统的透明度和可追溯性提供了重要支持。
...
TL;DR: ERAHBO是一种有效的风险规避的贝叶斯优化方法，显著提高了强化学习超参数优化的样本效率。


<details>
  <summary>Details</summary>
Motivation: 提高强化学习(RL)的超参数优化效率和稳定性。

Method: 提出了一种名为ERAHBO的贝叶斯优化方法，该方法将学习结果的均值和方差建模为超参数配置的函数。

Result: 在多种RL算法和环境中的实证评估表明，ERAHBO通常优于风险中性和风险规避的基线，提高了风险规避回报的样本效率。

Conclusion: ERAHBO是一种有效的风险规避的贝叶斯优化方法，可以显著提高强化学习超参数优化的样本效率。
...
TL;DR: 提出了一种基于MOE的新方法，显著提高了在线手写轨迹重建的准确性


<details>
  <summary>Details</summary>
Motivation: 提高在线手写轨迹重建的准确性

Method: 使用混合专家（MOE）方法，包括触摸专家模型和悬停专家模型，并基于额外上下文或特定示例改进每个专家的学习

Result: 显著提高了重建准确性，与主要竞争对手相比有显著提升

Conclusion: 提出了一种基于MOE的新方法，用于精确重建在线手写轨迹，为手写重建领域提供了新的基准数据集
...
TL;DR: PowerAtlas：电网-计算协同调度框架


<details>
  <summary>Details</summary>
Motivation: AI工作负载的增长导致数据中心成为大规模、易变的时空灵活电网负荷，需要协调的电力-计算调度。

Method: 通过结合历史实例、领域知识和物理约束来产生联合决策。

Result: 在11个LLMs上的实验表明，PowerAtlas在真实物理运行条件下表现良好，具有可行性高和成本优势。

Conclusion: PowerAtlas是一种有效的电网-计算协同调度框架，在真实物理运行条件下表现良好，具有可行性高和成本优势。
...
TL;DR: CalTwin通过轻量级正则化目标解决医疗世界模型的协变量偏移和置信度不匹配问题，提高了模型在临床部署中的可靠性。


<details>
  <summary>Details</summary>
Motivation: 医疗世界模型旨在学习患者或器官生理的潜在状态以及转换函数，以预测在干预下该状态如何演变，支持从基于成像的诊断到数字孪生治疗计划等下游任务。两种故障模式威胁了这些模型在临床部署中的可靠性：一是协变量偏移，因为训练数据分散在各个医院、扫描仪和时间上，因此潜在动力学预测器看到的特征分布在不同片段之间以及与部署时的分布不同；二是置信度不匹配，因为多步预测往往在临床风险最高的地方过于自信。

Method: 提出了一种名为CalTwin的轻量级正则化目标，该目标结合了基于Fisher信息的偏移惩罚和置信度不匹配惩罚，并应用于基于GRU的医疗世界模型的潜在转换预测器。

Result: CalTwin将OOD下一步潜在状态MSE相对于无惩罚基线降低了9.1%，仅FIM惩罚就占7.0%；置信度不匹配惩罚的ECE降低是真实但小的（CalTwin为0.7%，CMP单独为1.3%）

Conclusion: CalTwin有效地解决了医疗世界模型在临床部署中的协变量偏移和置信度不匹配问题，提高了模型的可靠性。
...
TL;DR: 开发了一种多轴结构数据建模框架，提出了一种名为JoFormer的模型，具有潜在的应用价值


<details>
  <summary>Details</summary>
Motivation: 多轴结构数据建模的需求

Method: 开发了一种多轴结构建模框架，使用路径和变换来定义数据项的组成和位置

Result: 提出了一种名为JoFormer的模型，用于值聚合，并证明了路径独立性和变换交换性原则

Conclusion: 该框架和模型在视觉、语言和长度泛化方面具有潜在的应用价值
...
TL;DR: SkillRise是一种跨任务的技能学习框架，提高了LLM代理在不同任务中提取、精炼和重用技能的效率。


<details>
  <summary>Details</summary>
Motivation: 标准强化学习将任务视为独立的片段，而现有的技能学习方法要么专注于重复尝试单个任务，要么使用多个阶段的流水线。

Method: SkillRise通过将相关实例组织成递增挑战的序列，并使用单一策略在任务解决和创建技能文档之间交替，实现了跨任务的技能学习。

Result: 实验表明，SkillRise在ALFWorld、WebShop和ScienceWorld上的性能优于基线方法，且在相同任务上的重复尝试中，其创建的技能策略仍然有效。

Conclusion: SkillRise在多个任务中实现了最强的Pass@1性能，并且能够在不同任务间重用可迁移的技能，同时降低了技能学习管道的运行开销。
...
TL;DR: FedTopo通过关系拓扑编码和可靠性感知聚合，解决了联邦学习中知识迁移问题，性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 为了解决联邦学习中不同架构导致的知识迁移问题，以及现有方法中全局知识编码方式存在的问题。

Method: 提出FedTopo框架，通过关系拓扑编码全局知识，并采用可靠性感知的聚合策略。

Result: 在三个数据集和八种异构骨干网络上的实验表明，FedTopo在低通信和无推理开销的情况下，优于参数共享、蒸馏和原型共享等基线方法。

Conclusion: FedTopo通过关系拓扑编码和可靠性感知的聚合策略，有效地解决了联邦学习中全局知识迁移问题。
...
TL;DR: CostAda通过成本校准前沿效用提高了搜索质量，达到更有效的搜索策略。


<details>
  <summary>Details</summary>
Motivation: 为了解决现有自适应发现控制器在分配信用时仅基于分数进步的问题，同时考虑到提示长度、重试和指导调用导致搜索操作产生不同的token成本。

Method: 提出了一种名为CostAda的成本校准自适应控制器，该控制器基于成本校准前沿效用。该效用值将前沿进度与实现动作成本进行比较，并基于剩余预算进行信用分配。CostAda使用该信号来控制局部探索强度、前沿分配和预算策略干预。

Result: CostAda在16个基准-骨干对中的12个上达到了最强基线的全预算质量，而使用GLM-5和GPT-5.4在所有8个基准上实现了最强的平均最终质量。

Conclusion: CostAda在固定搜索端token预算下，通过成本校准前沿效用实现了更有效的搜索策略，提高了搜索质量。
...
TL;DR: Kairos通过上下文在线学习和MRL解决区域市场新闻个性化中的数据稀缺问题，提高了推荐系统的效率。


<details>
  <summary>Details</summary>
Motivation: 算法在区域市场新闻个性化中失败的原因，因为现代深度学习模型需要大量的交互数据，而现实中的新闻具有很短的存活时间（TTL < 48小时）和浅的文章池。这种结构性的冷启动剥夺了协同过滤所需的数据以进行稳健建模的能力。

Method: 提出Project Kairos框架，通过上下文在线学习方法（LinUCB）弥合数据稀缺性。Kairos用Cholesky因子的直接秩-1更新替换了易出错的Sherman-Morrison逆矩阵，以保持协方差矩阵的正定性。同时，集成Matryoshka Representation Learning（MRL）以解决推理延迟。

Result: 在Tagesschau API上进行的实证评估表明，在特征空间中利用语义冗余可以实现4.85倍的效率提升，同时没有显著降低排名精度。

Conclusion: Kairos为资源受限和数据受限环境中的高性能推荐系统提供了一个蓝图。
...
TL;DR: 本文分析了AdaBoost的泛化误差上界，并推导出相应的公式


<details>
  <summary>Details</summary>
Motivation: 分析AdaBoost泛化误差的上界

Method: 结合已知事实和新的边界理论

Result: 推导出AdaBoost泛化误差的上界公式

Conclusion: 本文对AdaBoost的泛化误差上界进行了分析，并推导出相应的公式
...
TL;DR: TREA-Net是一种轻量级、可迁移的登革热预测网络，在有限数据条件下提高了预测准确性。


<details>
  <summary>Details</summary>
Motivation: 准确的多周登革热预测对于及时进行矢量控制干预、疫情准备和医疗资源分配至关重要。然而，新建立的监控系统通常缺乏训练可靠神经预测模型所需的历史数据。尽管预训练的时间序列模型提供了有希望的零样本预测，但它们的跨领域训练可能无法捕捉到本地的流行病学动态。

Method: 提出了一种名为TREA-Net的迁移学习网络，该网络结合了环境时间序列易感-感染-恢复模型，并学习了一种轻量级的门控残差校正，可以从数据丰富的区域迁移到数据稀缺的区域。该网络的设计允许不同数量的地点的监控系统使用，而目标自适应只需要学习两个全局参数。

Result: TREA-Net在10个设置中的9个中提高了相应的骨干网络，具有统计学意义。当与TiRex预测基础模型集成时，它在所有目标数据集中实现了最低的平均绝对误差。符合性预测进一步保持了经验覆盖率，同时将墨西哥的8周预测区间宽度减少了29.6%。

Conclusion: TREA-Net作为一种轻量级和便携式的早期预警框架，对于具有有限监测数据的卫生机构具有潜力。
...
TL;DR: 提出了一种基于神经网络的数据矩匹配方法，实现了AMFD损失，并在多个任务上取得了良好的效果。


<details>
  <summary>Details</summary>
Motivation: 提出一种利用神经网络学习数据矩作为分布训练信号的方法，以解决数据矩匹配问题。

Method: 通过多项式投影将扩散去噪器应用于数据矩匹配，建立了矩放缩的通用框架。

Result: 实现了Amortized Fréchet Distance (AMFD)损失，并通过实验证明了其在图像生成和文本到图像生成任务上的优越性。

Conclusion: AMFD方法在数据矩匹配方面表现出色，为图像生成和文本到图像生成任务提供了新的解决方案。
...
TL;DR: ReCo解决了GRPO在Pass@k中性能下降的问题，并在多个基准上提升表现


<details>
  <summary>Details</summary>
Motivation: GRPO导致推理能力降低和Pass@k表现不佳

Method: ReCo，重新加权方法

Result: 提高Pass@k性能，在大k值下优于GRPO，在小k值下与GRPO相当

Conclusion: ReCo解决了GRPO的局限性，在数学推理基准上表现出色
...
TL;DR: 本文提出了一种名为结果表现力A/B检测的方法，用于检测预测对结果的影响，并通过案例研究证明了其有效性。


<details>
  <summary>Details</summary>
Motivation: 在许多领域，如姑息治疗、信用分配和推荐系统，预测可能会因果性地影响预测的结果。这种现象被称为结果表现力。本文提出了一种使用预测干预（结果表现力A/B检测）的方法来检测结果表现力。

Method: 本文提出了一种名为结果表现力A/B检测（OPAB）的方法，通过评估不同预测组（干预）产生的结果分布之间的差异来检测结果表现力。

Result: 研究结果表明，使用OPAB检测结果表现力在许多情况下是可行的。同时，研究还发现了一些不可区分的区域，这些区域描述了干预措施数量不足以检测结果表现力的设置。

Conclusion: 本文通过案例研究证明了OPAB在Open Bandits数据集上的有效性，并为未来的研究提供了方向。
...
TL;DR: This paper proposes a new method for multi-task world-model learning using LeWorldModel and SIGReg, achieving significant improvements in performance on the LIBERO benchmark.


<details>
  <summary>Details</summary>
Motivation: Recent work on LeWorldModel (LeWM) has shown that the Sketched Isotropic Gaussian Regularizer (SIGReg) enables stable end-to-end world-model learning from pixels by regularizing the latent marginal distribution toward an isotropic Gaussian, thereby preventing representation collapse. While effective and elegant in single-task settings, this recipe does not extend reliably to multi-task training, leading to substantially worse downstream behavior-cloning performance.

Method: We apply SIGReg to temporally centered residuals rather than to the latent marginal distribution. This surrogate target places no direct regularization pressure on the separation among cluster centers, removes the requirement that the full latent follow a single isotropic Gaussian, and retains the anti-collapse effect of SIGReg.

Result: On the LIBERO benchmark, our method improves downstream success on the long-horizon suite by 1.7x and raises the average success rate across four suites from 53.2% to 73.6%. Without external pretraining, it slightly outperforms Diffusion Policy trained from scratch and approaches the performance of large-scale pretrained policy baselines.

Conclusion: These results reveal a structural incompatibility between marginal Gaussian priors and multi-task latent structure, and provide a simple route toward stable and scalable end-to-end multi-task world-model learning.
...
TL;DR: 该方法通过双目标代理引导的集成搜索，提高了集成搜索的效率，显著提升了集成性能


<details>
  <summary>Details</summary>
Motivation: 提高深度神经网络性能和鲁棒性

Method: 双目标代理引导的集成搜索，使用有向无环图表示候选架构，并训练两个代理模型来估计预测精度和多样性潜力

Result: 在FashionMNIST，CIFAR-10和CIFAR-100上，最终集成在性能上与Deep Ensembles和Random Search等标准基线相当或更优

Conclusion: 提出的方法有效提高了集成搜索的效率，并显著提升了集成性能
...
TL;DR: This paper investigates the effectiveness of pretrained vision models in face presentation attack detection and finds that while they contain relevant information, explicit adaptation is needed for cross-dataset performance.


<details>
  <summary>Details</summary>
Motivation: The scarcity of large-scale labeled data motivates adapting pretrained vision models rather than training task-specific architectures from scratch.

Method: Systematically evaluate 24 frozen encoders using a unified linear-probing protocol on the MCIO benchmark.

Result: Frozen foundation-model representations can support strong intra-dataset PAD performance with only a linear classifier, but this performance does not reliably transfer across datasets. Model scale is beneficial within several families, although the effect is not monotonic and is strongly mediated by architecture and pretraining.

Conclusion: While pretrained representations contain PAD-relevant information, explicit adaptation remains necessary to address domain shift.
...
### [233] [What Can Latent World Models Know? Physical Parameter Identifiability in Multimodal Predictive Representations](https://arxiv.org/abs/2607.27017)
*Kaizhen Tan,Xin Xu,Siru Tao,Hanzhe Hong,Yang Feng,Heqing Du*

Main category: cs.LG

TL;DR: 潜在世界模型通过预测未来使表示内部化其环境的物理，研究确定了哪些物理量被包含以及决定因素。


<details>
  <summary>Details</summary>
Motivation: 研究潜在世界模型中预测未来如何使表示内部化其环境的物理。

Method: 在POKEWORLD环境中进行控制干预，该环境中的视觉相同物体隐藏质量、阻力和接触刚度。

Result: 确定了哪些物理量被训练的潜在包含，以及是什么决定了这一点。

Conclusion: 目标结构决定了潜在获得的物理参数，而额外的数据只能改善它已经获得的参数。
...
TL;DR: TreeCCA结合了非线性精度和原生可解释性，在多个基准测试中表现出色。


<details>
  <summary>Details</summary>
Motivation: 梯度提升树在表格机器学习中占主导地位，而典型相关分析始终依赖于线性或神经编码器。

Method: 提出TreeCCA，这是第一个将梯度提升树集成端到端训练为CCA编码器的方法，继承了它们的即插即用可靠性：无需架构设计，熟悉的超参数，默认情况下具有强大的性能。技术推动者是Eckart-Young (EY) 损失，它为每个样本提供封闭形式的梯度，可直接插入任何标准GBT库（XGBoost、LightGBM）作为自定义目标。

Result: TreeCCA是第一个将非线性精度与原生可解释性相结合的CCA方法：每个树分裂选择一个特征，因此增益重要性揭示哪些输入驱动交叉视图相关性，无需额外成本。在合成基准测试中，TreeCCA与Deep CCA相当或超过（在签名的功率上2.61比2.43；在埃尔米特上2.93比2.89），在稀疏基准测试中，TreeCCA在p=50时以$	ext{Precision@}S = 1.00$恢复真实支持，而PMD找不到信号。在UCI HAR传感器融合基准测试中，TreeCCA在5倍的成本下达到与Deep CCA相当的精度，而XGBoost的增益重要性直接验证了关于数据的一个物理动机假设——这是神经编码器难以提供的解释。在五个流行的表格多视图数据集上，TreeMCCA在非线性相关性提取和下游分类精度方面始终与线性CCA相当或超过。

Conclusion: TreeCCA是一种新的CCA方法，结合了非线性精度和原生可解释性，在多个基准测试中表现出色。
...
TL;DR: 本研究评估了稀疏化、压缩和彩票票据对模型行为的影响，发现虽然稀疏候选者可以恢复密集参考精度，但它们的行为仍然不同，这可能会对下游决策逻辑产生影响。


<details>
  <summary>Details</summary>
Motivation: 为了评估先前文献中关于模型行为变化的混合发现，本研究从实际部署的角度研究了生产-替换问题，即是否可以使用与现有密集模型精度匹配的彩票票据或其他稀疏挑战者替换现有密集模型，而无需重新配置下游决策逻辑。

Method: 审计了广泛的、协议特定的、与部署相关的行为面板，包括校准、OOD响应、类级别可靠性、表示和下游政策决策，并使用行为兼容性距离总结了除清洁精度外的偏差。

Result: 稀疏候选者反复恢复密集参考精度，但行为上仍然不同；在几个研究带匹配的设置中，LTs也显示出较低的腐败精度。在固定阈值策略诊断的小差距设置中，彩票票据替换改变了7%到10%的接受-审查决策。这些发现确立了清洁精度认证的局限性：与固定现有兼容性不同，不能将 churn 唯一归因于稀疏性或将每个测量的偏差都视为有害。

Conclusion: 即使精确的点值 top-1 一致性也不能限制固定阈值决策变化，并且接近操作边界的微小置信度变化可以产生一级路由 churn。
...
TL;DR: CoCaRS：一种基于相关性校准的冗余抑制方法，有效提高知识蒸馏性能并减少系数设置敏感性。


<details>
  <summary>Details</summary>
Motivation: 为了解决不同模型架构之间存在的结构信息损失和系数设置敏感性等问题。

Method: 提出了一种基于相关性校准的冗余抑制（CoCaRS）方法，通过混淆证据估计（CEE）和强度分配控制（SAC）进行特征去相关，并通过自适应系数调节（ACR）减少对系数设置的敏感性。

Result: 在CIFAR-100和ImageNet-1K数据集上进行了广泛实验，验证了CoCaRS在提高蒸馏性能和减少对系数设置敏感性的有效性。

Conclusion: CoCaRS能够更好地保留结构信息，同时抑制冗余，并减少对系数设置的敏感性。
...
TL;DR: 提出HT-PAder算法，在非平稳环境下实现通用动态后悔度保证。


<details>
  <summary>Details</summary>
Motivation: 研究在线凸优化（OCO）在非平稳环境下的重尾噪声中，其中随机梯度算子仅对某些p∈(1,2]的p-阶中心矩有有限值。

Method: 提出HT-PAder算法，结合重启AdaGrad专家和AdaGrad-Hedge路径元算法。

Result: HT-PAder算法在直径为D，Lipschitz常数为G，噪声水平为σ，比较路径长度为P_T的领域内，实现了期望的通用动态后悔度为O(GD√(T(1+P_T/D))+σD T^(1/p)(1+P_T/D)^(p-1/p))。

Conclusion: HT-PAder算法在有限方差（p=2）的特殊情况下，提供了第一个参数免费的极小极大通用动态后悔度保证，并证明了匹配的下界，确立了路径长度指数的最优性。
...
TL;DR: PTT算法提高了EBM的可靠性，实现了高效训练


<details>
  <summary>Details</summary>
Motivation: 提高能量基础模型（EBM）在生成科学数据方面的可靠性

Method: 引入基于并行轨迹退火的（PTT）训练算法

Result: 在高度多模态和数据稀缺的科学数据集上实现稳定和快速训练，优于现有EBM训练方法，并在离散表格数据上优于最先进的深度生成模型

Conclusion: PTT使EBM的平衡最大似然训练变得实际且计算高效
...
TL;DR: CAM-DF方法在异质成本和高压成本下表现最佳，且无需微调LLM。


<details>
  <summary>Details</summary>
Motivation: LLM代理越来越依赖外部服务，如搜索引擎、数据库和连接器，而代理选择工具面临挑战。

Method: 提出CAM-DF和CAM-DF-lite方法，通过计算成本和收益来选择工具。

Result: 在五个工具使用领域中的1,343个任务上评估，CAM-DF在异质成本和高压成本下表现最佳。

Conclusion: CAM-DF方法在异质成本和高压成本下表现最佳，且无需微调LLM。
...
TL;DR: HierSTT：提高急诊部门多层级预测的准确性和一致性。


<details>
  <summary>Details</summary>
Motivation: 应对急诊部门面临的不可预测的患者需求、季节性激增和非紧急访问的压力，有效规划急诊部门需要在不同决策层面上进行预测。

Method: 提出HierSTT，一个基于Transformer的分层框架，用于连贯的多层次急诊部门预测。

Result: 实验表明，HierSTT相对于最佳非分层深度学习基线降低了32%的平均WAPE，并优于所有经典分层协调方法，同时在不同层次上产生近一致的预测。

Conclusion: HierSTT能够提高急诊部门预测的准确性和一致性。
...
TL;DR: 本研究提出了一种基于Voronoi图的EPD向量化方法，提高了计算效率并保持了拓扑特征的稳定性。


<details>
  <summary>Details</summary>
Motivation: 为了降低点云拓扑计算的时间复杂度，研究者们开发了期望持久度图（EPD）。然而，现有的EPD向量化方法通常依赖于预定义的点变换，如高斯或景观函数。

Method: 本研究提出了一种基于Voronoi图直方图的离散化方法，以自适应分区计数为代价，换取平滑函数逼近。我们建议使用基于Voronoi图的直方图作为EPD的向量化表示，而不强制实施显式的平滑点变换模型。在给定的分离和归一化条件下，我们建立了稳定性界限，并描述了何时直方图表示保留Wasserstein尺度变化。

Result: 在具有显著拓扑特征的分类和降维任务的真实数据集上，我们证明了所提出表示的有效性。

Conclusion: 基于Voronoi图的直方图在EPD向量化中提供了一种有效的方法，能够提高计算效率并保持拓扑特征的稳定性。
...
TL;DR: 本研究提出了一种 holonomy 内存强化学习方法，通过优化记忆表示和更新策略，在部分可观测环境下实现了高效的决策。


<details>
  <summary>Details</summary>
Motivation: 研究在部分可观测性下，智能体如何维持一个递归更新的历史统计量以恢复马尔可夫性质。

Method: 提出了结构化POMDP类—— holonomy-cover 决策过程，并通过构建稳定商、粗观察抽象等手段，证明了当前观察和稳定类构成一个精确的有限马尔可夫状态。

Result: 实现了 holonomy 内存强化学习，通过当前稳定类表示记忆，通过有序边传输更新，在诊断可用时识别局部类坐标，并使用标准的有限 MDP 强化学习框架。实验结果表明，该方法能够从原始状态到商状态进行精确压缩，并达到完美的配对顺序准确性。

Conclusion: 该方法在 holonomy 内存强化学习方面取得了显著成果，为解决部分可观测性问题提供了新的思路。
...
TL;DR: 莫迪利亚尼CP提高少数类别覆盖率，降低决策成本


<details>
  <summary>Details</summary>
Motivation: 在高风险决策系统中，如信用评分、欺诈检测、医疗保健和工业安全，需要在大类不平衡和不对称错误成本下进行可靠的不可知量化。标准边际一致预测（CP）提供有效的整体覆盖率保证；然而，我们发现它严重低估了稀有且昂贵的少数类别，某些数据集的少数类别覆盖率降至0.5%。为了表征和解决这一局限性，我们进行了一项全面的基准测试，比较了边际CP、类条件（莫迪利亚尼）CP和成本控制的弃权机制，涵盖了15个真实世界的类不平衡表格数据集、7个分类模型、3种概率校准技术和10个随机种子，共计3,150次实验运行。

Method: 进行全面的基准测试，比较边际CP、类条件（莫迪利亚尼）CP和成本控制的弃权机制，涵盖了15个真实世界的类不平衡表格数据集、7个分类模型、3种概率校准技术和10个随机种子，共计3,150次实验运行。

Result: 结果表明，莫迪利亚尼CP恢复了有效的少数类别覆盖率，与边际CP相比，平均少数覆盖率提高了61.7个百分点（p < 1e-80）。此外，将莫迪利亚尼CP与成本控制的弃权机制相结合，在现实人类审查预算下，与标准决策边界、基于置信度的拒绝者和风险控制的拒绝者相比，显著降低了预期决策成本。我们还量化了特定数据集的盈亏平衡阈值，在哪些情况下将模糊实例推迟到人工专家成为成本效益的。这些发现为在高风险决策支持系统中部署无分布、成本感知的不确定性量化提供了实际指导。

Conclusion: 莫迪利亚尼CP有效地提高了少数类别的覆盖率，显著降低了预期决策成本，为高风险决策支持系统中的不确定性量化提供了实际指导。
...
TL;DR: WindCastNet是一种基于卫星的现在casting框架，用于海上风电速度和方向的日内预测，显著提高了预测精度。


<details>
  <summary>Details</summary>
Motivation: 准确的海上风电日内预测对电力系统运行和海上风电的集成变得越来越重要。

Method: 提出了一种基于卫星的现在casting框架WindCastNet，该框架通过学习时空不规则的卫星观测数据来进行日内预测。WindCastNet利用来自欧洲、中国和印度散射计的微波雷达观测数据，尽管这些数据的时空覆盖不均匀，采样异步，重访时间可变。

Result: WindCastNet在北海的评估中，将1小时和2小时的均方根误差分别降低了23%和7%，并且在最初的三个预报小时内比持续性预测提高了9-15%。

Conclusion: 卫星散射计星座可以提供一种独立且具有竞争力的短期海上风预测源，为可再生能源预测以及更广泛的海洋天气预报应用开辟了新的机会。
...
TL;DR: 研究结果表明，准确期权价格并不一定意味着潜在风险中性密度的准确恢复，学习算子在某些情况下可以提高准确性，但结果可能因数值条件而异


<details>
  <summary>Details</summary>
Motivation: 研究准确期权价格与潜在风险中性密度恢复之间的差异

Method: 使用两种互补的基准，包括受控基准和按时间顺序的NIFTY基准

Result: 发现学习算子保留了较窄的优势，DeepONet和quote transformer在特定基准上提高了准确性，但整体排名可能因数值条件而有所不同

Conclusion: 证据支持目标相关的归纳偏差，而非普遍的获胜者
...
TL;DR: 基于果蝇感知机制，提出一种新的回归任务方法，应用于多种领域，具有效率优势。


<details>
  <summary>Details</summary>
Motivation: 受果蝇感知环境机制启发，提出一种新的回归任务方法。

Method: 使用有限代表性的局部模式库代替复杂的全局代理模型，学习非线性输入输出关系。

Result: 应用于非线性动力系统、数据驱动回归和物理信息学习，降低了计算和内存需求，同时提供对准确度、存储和推理成本之间的权衡的显式控制。

Conclusion: 提出的方法在回归任务中表现出色，具有实际应用价值。
...
TL;DR: 预训练Q函数可能不是必要的，IPE方法可以显著提高在线RL中的Q函数学习性能。


<details>
  <summary>Details</summary>
Motivation: 在值函数强化学习（RL）中，预训练后微调已成为学习高性能策略的主要方法。对于预训练策略，Q函数是否也应该在离线数据上进行预训练？

Method: 我们系统地研究了在预训练基策略之上微调时，预训练Q函数是否真正有帮助。我们提出了初始化通过策略集成（IPE），这是一种简单的方法，它训练多个不同的策略，并使用它们的汇总回滚来启动在线RL中的Q函数学习。

Result: 我们发现，简单的Q函数预训练通常比随机初始化提供的好处很少。在一系列具有挑战性的连续控制基准测试中，IPE在微调性能方面比简单的Q函数预训练平均提高了1.26倍。

Conclusion: 预训练Q函数在微调预训练基策略时可能没有预期的帮助，而IPE方法可以显著提高在线RL中的Q函数学习性能。
...
### [251] [When Kernel Ridge Regression Meets the Hölder-Zygmund Class: Minimax Optimality and Failure of Properness](https://arxiv.org/abs/2607.26065)
*Yuxuan Hou*

Main category: stat.ML

TL;DR: 核岭回归在Hölder-Zygmund类上的非参数回归研究中取得新进展，揭示了正定性失败现象及最小-最大L2率。


<details>
  <summary>Details</summary>
Motivation: 研究Hölder-Zygmund类上的核岭回归的非参数回归

Method: 使用与Sobolev空间s+d/2等价的RKHS

Result: 证明了误指定的KRR达到最小-最大L2率n^{-2s/(2s+d)}；展示了在Hölder-Zygmund范数中，正定性失败：即使是具有高斯噪声的零回归函数，KRR噪声成分的期望平方Hölder-Zygmund范数也随log n增长

Conclusion: 对非参数回归中的核岭回归进行了深入研究，揭示了其在Hölder-Zygmund范数中的正定性失败现象，并提供了最小-最大L2率的理论支持。
...
### [252] [Origins and mitigation of double descent in reduced order modeling](https://arxiv.org/abs/2607.26414)
*Andrei A. Klishin,J. Nathan Kutz,Krithika Manohar*

Main category: stat.ML

TL;DR: 本研究通过数据噪声平均理论分析了数据集的稀疏感知和全状态重建问题，为减轻重建不稳定性提供了新的方法


<details>
  <summary>Details</summary>
Motivation: 探索数据噪声平均理论下数据集的稀疏感知和全状态重建问题

Method: 提出数据噪声平均理论，并分析重建算法、传感器位置和测量噪声对重建风险曲线的影响

Result: 预测了详细的重建风险曲线，追踪了重建不稳定性，并提供了正则化机制以减轻不稳定性

Conclusion: 数据噪声平均理论有助于理解重建风险曲线的多样性，并为减轻重建不稳定性提供了新的方法
...
### [253] [Chaos Is a LADDER: Domain Generalization Beyond Invariance via Reweighting](https://arxiv.org/abs/2607.26458)
*Yuhang Jiang,Fengchuan Zhang,Sanguo Zhang,Guojun Zhu*

Main category: stat.ML

TL;DR: 提出了一种新的领域泛化方法LADDER，通过引入域风格提高了泛化性能。


<details>
  <summary>Details</summary>
Motivation: 领域泛化（DG）旨在从多个源域学习并泛化到未见过的目标域。大多数DG方法追求不变性：它们寻求一个因果表示，其预测规则在域之间是不变的。当因果机制稳定时，这一原则是有效的，但当域本身调节因果内容映射到响应的方式时，它就变得具有约束力。在这种情况下，直接将域风格输入到预测器中可能会产生误导性的捷径，因为风格本身并不导致响应。然而，多种风格的明显混乱可以成为一个梯子：风格可以定位未见的目标域在源域之间，并指导哪些域相关的预测规则应该被信任。

Method: 我们提出了LADDER（潜在自适应域解耦和环境重加权），这是一种固定模型的DG流程，它学习因果/风格表示，冻结编码器，拟合源特定的分类器，并在推理时仅使用未标记的目标域协变量集来计算这些固定分类器的权重，没有任何目标标签或模型状态更新。

Result: 我们在模拟、FMoW和一个按位置分组的iWildCam协议上验证了LADDER，结果显示整体和组平均准确率有所提高。

Conclusion: LADDER方法通过引入域风格作为辅助信息，提高了领域泛化性能，为领域泛化问题提供了一种新的解决思路。
...
### [254] [Early Failure Prediction from Near-Anomaly Detection: A Proactive Approach](https://arxiv.org/abs/2607.26704)
*Léa Billet,Louise Travé-Massuyès,Elodie Chanthery,Alexandre Gaffet*

Main category: stat.ML

TL;DR: 本文提出了一种基于克里斯托费尔函数的近异常检测方法CANARI，有效预测未来异常，具有应用潜力。


<details>
  <summary>Details</summary>
Motivation: 现有的异常检测方法在分布边界附近的样本上往往表现不确定，限制了其预测未来异常的能力。

Method: 提出了一种名为CANARI的无监督方法，利用克里斯托费尔函数的强大理论基础来检测近异常。

Result: 实验结果表明，CANARI在检测近异常方面优于比较的基线方法，提供了一种在异常发生之前预测异常的主动解决方案。

Conclusion: CANARI为预测性维护、质量和控制提供了一种有希望的方法。
...
### [255] [Crossing-Free Probabilistic K-Line Forecasts Without Retraining](https://arxiv.org/abs/2607.26792)
*Runyao Yu,Yuchen Tao,Yujie Chen,Wentao Wang,Derek W. Bunn*

Main category: stat.ML

TL;DR: 提出KQSP方法，有效解决概率K线预测中的不确定性问题。


<details>
  <summary>Details</summary>
Motivation: 解决概率K线预测中的不确定性问题，特别是分位数交叉和K线交叉问题。

Method: 提出K线-分位数顺序投影（KQSP）方法，该方法无需参数和训练，适用于任何模型的预测结果。

Result: KQSP方法显著减小了原始预测的修正量，同时降低了分位数和K线交叉率。

Conclusion: KQSP方法可以独立于预测生成和重新训练，有效解决概率K线一致性。
...
### [256] [Think Short, Defer Smart, Act, and Repeat: Calibrated Reasoning and Uncertainty-Aware Deferral for Edge LLM Agents](https://arxiv.org/abs/2607.26865)
*Amirmohammad Farzaneh,Osvaldo Simeone*

Main category: stat.ML

TL;DR: TSDS框架通过优化推理和延迟规则，在保持性能的同时减少了计算量。


<details>
  <summary>Details</summary>
Motivation: 在边缘部署的LLM代理需要在有限的推理预算内保持可靠性和安全性。

Method: 多目标学习-测试（LTT）程序，结合轻量级收敛探测和困惑度基于的延迟规则。

Result: TSDS在四个ReAct基准测试中表现良好，与仅思考校准和仅校准延迟的基线相比，计算降低了43%-73%。

Conclusion: TSDS框架有效降低了推理计算，同时保持了奖励和云调用率的保证。
...
### [257] [Conformalized Rate-Adaptive Sensing](https://arxiv.org/abs/2607.26887)
*Jiawei Yang,Yao Zhang*

Main category: stat.ML

TL;DR: CoRAS is an adaptive sensing method for high-resolution imaging that can accurately determine the optimal stopping time to achieve high-quality image reconstruction.


<details>
  <summary>Details</summary>
Motivation: Many high-resolution imaging systems need to determine when to stop collecting measurements to reconstruct images accurately.

Method: Develops Conformalized Rate-Adaptive Sensing (CoRAS) to adaptively choose acquisition or compression rates for each image, while keeping the reconstruction error below a target level with high probability.

Result: CoRAS can estimate the target stopping time with high accuracy, use fewer measurements, and assign more measurements to harder-to-reconstruct images.

Conclusion: CoRAS is an effective method for determining the optimal stopping time in high-resolution imaging systems.
...
### [258] [Breaking the Curse with BAND: Nonparametric Distribution Estimation in High Dimensions](https://arxiv.org/abs/2607.26955)
*Shuo-Chieh Huang,Chien-Ming Chi,Jau-er Chen*

Main category: stat.ML

TL;DR: 提出了一种新的稀疏贝叶斯网络方法BAND，有效解决了多变量分布估计的维数灾难问题，并在多个方面表现出竞争力。


<details>
  <summary>Details</summary>
Motivation: 多变量分布估计的维数灾难问题

Method: 稀疏贝叶斯网络方法，使用稀疏感知条件均值方法估计条件概率

Result: BAND估计器，处理高维时间序列中的混合数据类型，达到多项式总变差收敛速率，特征维度可以多项式增长

Conclusion: BAND在数据采样和置信区域预测方面，与最先进的基准相比表现出竞争力
...
### [259] [Feature Bagging Provides Stability](https://arxiv.org/abs/2607.26964)
*Yuheng Ma,Qiang Sun*

Main category: stat.ML

TL;DR: 特征袋装通过提高稳定性增强模型泛化能力


<details>
  <summary>Details</summary>
Motivation: 研究特征袋装方法通过算法稳定性视角

Method: 引入特征不稳定性（FI），分析特征袋装在参数线性模型和无模型设置下的表现

Result: 特征袋装提高了相关稳定性，少量袋装轮次即可接近无限袋装稳定性水平

Conclusion: 特征袋装是一种提高模型稳定性和泛化能力的有效方法
...
### [260] [PIKS: Universal Physics-Informed Kernel Methods](https://arxiv.org/abs/2607.27062)
*Joachim Bona-Pellissier,Giacomo Meanti,Matteo Santacesaria,Lorenzo Rosasco*

Main category: stat.ML

TL;DR: 提出了一种新的物理信息核方法（PIKS），在满足物理约束的情况下，具有渐近一致性和有限的样本界限。


<details>
  <summary>Details</summary>
Motivation: 为了克服物理信息神经网络（PINNs）的复杂性，并解决现有核方法保证的现实假设问题。

Method: 提出并分析物理信息核方法（PIKS），包括建立其对于线性微分约束的通用一致性，并在适当条件下推导有限样本界限。

Result: 证明了PIKS对于通用核（如高斯或Matérn核）的渐近一致性，并通过数值实验证明其与PINNs和传统有限元方法相比具有竞争力。

Conclusion: PIKS是一种有效的物理信息机器学习方法，在满足物理约束的同时具有渐近一致性和有限的样本界限。
...
### [261] [Probing the Origins of Reasoning Performance: Representational Quality for Mathematical Problem-Solving in RL vs. SFT Fine-Tuned Models](https://arxiv.org/abs/2607.26119)
*Antyabha Rahman,Akshaj Gurugubelli,Omar Ankit,Kevin Zhu,Aishwarya Balwani*

Main category: cs.AI

TL;DR: 强化学习模型在数学推理任务上优于监督微调模型，其机制在于更线性可分和结构化的表示以及层次化架构


<details>
  <summary>Details</summary>
Motivation: 研究强化学习（RL）模型在数学推理任务上的优越性及其机制基础

Method: 线性探针、平均消融研究、重复采样分析

Result: RL模型在预测答案正确性方面比SFT模型更准确，具有更线性可分和结构化的表示；RL模型发展出层次化架构，深层变得越来越重要；token分配可能更多取决于整体训练流程而非仅RL与SFT；token分配的变异性揭示了策略推理的分布，突出了模型的行为特征

Conclusion: 强化学习训练从根本上改变了模型表示和处理推理问题的方法，揭示了token分配和策略推理的复杂性
...
### [262] [Even More Deception: Objective Misalignment in Mixed-Motive LLM Multi-Agent Systems](https://arxiv.org/abs/2607.26120)
*Marylou Fauchard,Florian Carichon,Margarida Carvalho,Golnoosh Farnadi*

Main category: cs.AI

TL;DR: LLM驱动的多代理系统中的目标不一致性是一个需要解决的重要问题。


<details>
  <summary>Details</summary>
Motivation: 在混合动机环境中，由于代理的目标冲突或隐藏，代理在不对称信息和策略欺骗下运作。因此，与集体目标的不一致成为一个主要问题。

Method: 我们提出了一种使用狼人杀游戏来评估目标不一致性的新框架，在保持单个代理分配的角色的同时修改其目标。通过对来自四个不同模型家族和尺寸的LLM、四种玩家角色和三种目标表述的实验，我们对代理的内部推理和公开的廉价沟通行为进行了双重分析，并辅以游戏结果的评估。

Result: 结果显示，目标不一致性会削弱本质上具有对抗性的环境中的结果，这种影响因不对称信息和专门的角色而加剧。虽然受损害的代理会发展出不同的目标相关推理策略，但这些适应在他们的公开行为中仍然 largely 看不见。更广泛地说，我们的发现表明，即使是微小的目标不一致性也可能严重影响集体决策，强调了需要针对基于LLM的多代理系统采取有效的缓解策略。

Conclusion: LLM驱动的多代理系统中的目标不一致性会削弱结果，并需要有效的缓解策略。
...
### [263] [ClinLens: Towards Long-Horizon Coding Agents for Longitudinal Multimodal Clinical Data Science](https://arxiv.org/abs/2607.26155)
*Yuan Zhu,Ethan B. Liu,Frank Nie,Jindong Han*

Main category: cs.AI

TL;DR: CLINLENS基准评估临床数据科学代理，发现现有方法与正确临床分析之间存在重大差距。


<details>
  <summary>Details</summary>
Motivation: Clinical data-science agents需要将异构的纵向记录转换为可审计的分析，但现有的基准测试主要针对医学问答、结构化表格推理或通用科学库。

Method: 提出CLINLENS基准，包含200个可执行任务，涵盖结构化电子健康记录、笔记、心电图、胸部X光片和超声心动图等五个MIMIC资源。采用4x5分类法，交叉四个患者时间范围和五个分析能力。程序优先的逆向合成将每个半原始包与评估者私有的参考工作流程配对，并检查所需工件、队列和时序语义以及最终答案。

Result: 在固定的126个任务集中，24个标准化模型框架配置中最强的一个实现了56.3%的范围宏STRICTPASS，尽管100%的EXECSUCCESS。与单独配置的编码代理相比，它可以解决126个任务中的83个，而适应GPT-4o-mini的五个生物医学系统在范围宏STRICTPASS上最多达到2.9%。这些结果表明，可运行提交与正确临床分析之间存在重大差距。

Conclusion: CLINLENS基准有助于评估临床数据科学代理的性能，揭示了现有方法与正确临床分析之间的差距。
...
### [264] [When benchmark inferences do not compose: Projectibility in AI evaluation](https://arxiv.org/abs/2607.26159)
*Brett Reynolds*

Main category: cs.AI

TL;DR: 本文提出了一种基于可靠性的基准评估审计方法，以避免过度泛化和误用基准评估结果。


<details>
  <summary>Details</summary>
Motivation: 识别AI基准评估中的认识论问题

Method: 提出非组合原则，并通过案例分析验证

Result: 提出了一种基准评估的可靠性审计方法

Conclusion: 基准评估的结果需要谨慎解读，避免过度泛化和误用
...
### [265] [GuideSkill: Evolving Executable LLM Agent Skills for Guideline-Grounded Clinical Reasoning](https://arxiv.org/abs/2607.26160)
*Lang Cao,Yuhao Shen,Tianyang Luo,Simo Du,Hao Peng,Yue Guo*

Main category: cs.AI

TL;DR: GuideSkill模型通过提高诊断准确性，在临床实践指南的执行和推理方面表现出色。


<details>
  <summary>Details</summary>
Motivation: 由于LLM系统通常只是检索或吸收指南文本，而不是执行其规则，因此提出了GuideSkill模型以提高诊断准确性。

Method: 提出了GuideSkill模型，该模型包括GuideSkill-Zero和GuideSkill-Evo两个版本。GuideSkill-Zero从指南中初始化，而GuideSkill-Evo使用案例-诊断对进行优化。

Result: GuideSkill-Zero在四个基准和四个骨干上的平均宏平均准确率比指南RAG提高了13.45%。GuideSkill-Evo在所有骨干上实现了最高的宏平均准确率，比直接推理提高了18.49%。

Conclusion: GuideSkill模型在临床实践指南（CPGs）的执行和推理方面表现出色，显著提高了诊断准确性。
...
### [266] [GoGoTB: Agentic RTL Verification with Specification-Grounded Coverage Closure](https://arxiv.org/abs/2607.26181)
...
TL;DR: GoGoTB是一种新的集成电路功能验证框架，通过自动化和高效的方法提高了验证效率。


<details>
  <summary>Details</summary>
Motivation: 现有的LLM方法无法检测接口不匹配和覆盖范围与规范要求不匹配的问题。

Method: GoGoTB框架，包括执行控制层、知识系统和覆盖框架。

Result: GoGoTB在8个RTL设计中实现了100%的环境生成成功，平均覆盖率达到98.4%的行，97.2%的分支，97.0%的切换和83.2%的功能覆盖率。

Conclusion: GoGoTB在8个RTL设计中实现了100%的环境生成成功，平均覆盖率达到98.4%的行，97.2%的分支，97.0%的切换和83.2%的功能覆盖率。
...
### [267] [Position: Evaluation Scores Are Perishable Knowledge Claims](https://arxiv.org/abs/2607.26191)
...
### [268] [TraceCoder: Explainable and Auditable Code Generation with Position-Key Snippet Versioning](https://arxiv.org/abs/2607.26307)
...
TL;DR: 提出了一种新的代码生成方法，通过记录和可视化代码生成过程，提高了代码生成的透明度和可追溯性。


<details>
  <summary>Details</summary>
Motivation: 当代基于LLM的编码代理生成的代码是黑盒输出，无法了解每行代码背后的逻辑，代码的演变过程也无法追踪，后期的审计也变得不可能。

Method: 提出了一种代码生成概念，通过三个互补的机制来解决这些问题：（1）一个关系型代码片段历史架构，记录每次修复事件时的基准参考、轮次、失败文本和LLM解释，实现完整的溯源查询；（2）一个基于浏览器的可视化工具，将历史记录渲染为热图标注的源代码；（3）一种具有树节点分隔符的竞争分数位置键索引方案，为每个代码片段分配稳定、字典序排列的标识符，实现细粒度跟踪而不破坏周围的行。

Result: 在30个算法编程任务上评估了TraceCoder，涵盖了字符串处理、数学计算和数据结构操作。其中10个任务在具有微妙边缘行为的情况下耗尽了6次迭代预算。平均变化率达到30%，有三分之一的代码片段包含可追溯的修复事件行，而使用Gemini 2.0 Flash作为唯一提供者时，在20个任务的子集上这一比例仅为21%。三个详细的案例研究展示了系统如何解释哪些具体的基准失败塑造了最终程序的每一行。该机制使自动代码生成的内部'叙事'可审计和可回放，这是生产部署中信任和问责制的基本属性。

Conclusion: 提出的机制提高了代码生成的透明度和可追溯性，有助于提高生产部署中的信任和问责制。
...
### [269] [Exploring Structures in Physics Problems: Can AI Agents Discover Statistical Mechanical Mappings?](https://arxiv.org/abs/2607.26367)
...
### [270] [CaM-Wolf: Causal-Aware Multimodal Agents for Social Deduction Games](https://arxiv.org/abs/2607.26393)
*Zheng Zhang,Nanjie Yao,Jiarui He,Deheng Ye,Peilin Zhao,Hao Wang*

Main category: cs.AI

TL;DR: CaM-Wolf是第一个集成多模态感知和生成的社交推理游戏AI代理，它在游戏表现和人类-人工智能交互质量方面都表现出色。


<details>
  <summary>Details</summary>
Motivation: Social deduction games (SDGs) such as Werewolf have become challenging testbeds for AI agents. These games require complex social skills such as reasoning, deception, and collaboration. While recent advances in large language models (LLMs) have driven significant progress in SDG agents, current approaches are predominantly text-based, overlooking the multimodal nature that is fundamental to human social interaction.

Method: CaM-Wolf, the first SDG agent that integrates multimodal perception and generation. CaM-Wolf processes video inputs from other players, employs a causal-aware Reasoner trained via reinforcement learning to establish logical chains between observable behaviors and hidden roles, and presents itself through an animated avatar.

Result: Our experiments and user study show that CaM-Wolf achieves superior agent gameplay performance and enhances the quality of human-AI interaction. This work represents a significant advancement towards creating more human-like AI agents capable of participating in nuanced social dynamics.
...
### [271] [CG-World: A Large-Scale World-State Dataset and Protocol for World Models](https://arxiv.org/abs/2607.26452)
*Yiming Cai,Fangjie Yu,Meiqing Yu,Ziyue Shi,Pengfei Yuan,Yong Guo*

Main category: cs.AI

TL;DR: CG-World是一个大规模世界状态数据集，为世界模型、物理AI和具身智能提供了可重用的结构化监督。


<details>
  <summary>Details</summary>
Motivation: 为了解决现有视频、机器人和模拟数据集通常只能捕捉到部分状态、动作、事件和观察的结构的问题，提出了CG-World。

Method: 构建了一个名为CG-World的大型世界状态数据集和协议，该数据集来源于工业计算机图形生产流程，并显式记录了中间状态，包括多模态语义、空间结构、骨骼和控制器状态、运动曲线、摄像机和照明参数、物理缓存、接触事件和多通道渲染。

Result: CG-World包含约850,000个1-5秒的时间对齐片段，并支持干预学习和反事实推理。在几何条件视频生成、动作预测和闭环视觉语言动作策略迁移方面进行了评估，结果表明CG-World为可控生成、动作建模和具身策略迁移提供了可重用的结构化监督。

Conclusion: CG-World为世界模型、物理AI和具身智能提供了一个共享的数据基础设施。
...
### [272] [MultivationBench: A Benchmark for Multimodal Sequential Motivation Reasoning](https://arxiv.org/abs/2607.26465)
...
TL;DR: MultivationBench是一个用于评估多模态动机推理的新基准，发现现有模型在动态推理方面存在不足。


<details>
  <summary>Details</summary>
Motivation: 为了解决现有评估方法无法反映真实世界行为驱动累积性质的问题，提出了MultivationBench，这是一个用于严格评估故事驱动视觉叙事中多模态动机推理的基准。

Method: 该基准基于马斯洛的需求层次和里斯的基本欲望等心理学框架，要求模型整合累积的多模态上下文来推断不断发展的动机。

Result: 结果表明，MultivationBench对模型提出了重大挑战：所有测试的模型在跨情境中保持一致的动机推理方面都存在困难，揭示了静态识别能力与人类式社交理解所需的动态推理之间的关键脱节。

Conclusion: MultivationBench揭示了多模态大语言模型在动机推理方面的不足，为未来研究提供了新的方向。
...
### [273] [EvoPINN: Agentic Discovery of Executable Algorithms for Physics-Informed Neural Networks](https://arxiv.org/abs/2607.26490)
*Peng Yin,Kai Li,Yifan Zhang,Jian Cheng*

Main category: cs.AI

TL;DR: EvoPINN通过利用LLM代理和结构验证，提高了PINNs在解决PDEs时的性能和效率。


<details>
  <summary>Details</summary>
Motivation: 提高物理信息神经网络（PINNs）在解决偏微分方程（PDEs）时的性能和效率。

Method: 提出EvoPINN框架，通过将PINN开发从劳动密集型的手动设计转变为严格的、基于执行的算法发现问题，利用LLM代理迭代提出程序性修改。

Result: EvoPINN在各种PDE领域发现专化的学习算法，显著降低了与基线相比的相对$L_{2}$误差。

Conclusion: EvoPINN通过执行基础的代理发现新的科学计算机制，提高了PINNs的性能。
...
### [274] [Evidence-Ledger Adjudication for Claim-Evidence Traceability](https://arxiv.org/abs/2607.26512)
*Gengyu Chen,Yongjie Yu,Weiling Wang*

Main category: cs.AI

TL;DR: 证据链审判提高AI写作可追溯性和准确性。


<details>
  <summary>Details</summary>
Motivation: 研究证据链审判，解决AI代理快速起草声明但无法验证证据支持的问题。

Method: 构建2,335行的盲基准，使用独立外部标签，并隐藏金关系和源证据标签。

Result: 证据链审判将异质证据包转换为可审计的可追溯层。

Conclusion: 证据链审判可以提高AI辅助写作的可追溯性和准确性。
...
### [275] [Eco3S: Complex Socio-Economic System Simulation via Agent-Based Models](https://arxiv.org/abs/2607.26588)
*Shaopeng Wei,Yufei Cheng,Wenxi Sun,Yepeng Ding,Yu Zhao,Gang Kou*

Main category: cs.AI

TL;DR: Eco3S是一个基于LLM的社会经济系统模拟框架，通过协同演化的环境设计、结构因果模拟和模拟-分析-改进范式解决ABM研究的挑战，有效性和应用前景得到验证。


<details>
  <summary>Details</summary>
Motivation: LLMs的快速发展重新点燃了对基于代理的建模（ABM）的兴趣。然而，基于LLM的ABM研究面临着几个关键挑战：建模演变中的代理-环境交互、实现灵活的反事实推理以及自动化科学研究中的模拟工作流程。

Method: 提出了Eco3S，一个用于经济研究和政策分析的社会经济系统模拟框架，通过以下三种关键机制来解决这些挑战：（1）协同演化的环境设计，其中代理和环境协同演化，产生现实性的涌现行为；（2）结构因果模拟，一种受结构因果模型（SCM）启发的反事实机制，允许进行灵活的干预以进行各种因果推断任务；（3）模拟-分析-改进范式，一种自我纠正机制，基于先前模拟结果迭代改进实验设计。

Result: Eco3S在不同经济场景中的实验证实了其在复制多个已建立的经济研究（运河退化、治理起源和信息传播）以及跨领域现象方面的有效性。此外的结果进一步证明了其可扩展性和通用性，突出了该框架在严谨的经济研究和政策制定中的潜力。

Conclusion: Eco3S是一个有效的社会经济系统模拟框架，可以解决基于LLM的ABM研究中的挑战，并在经济研究和政策制定中具有广泛应用前景。
...
### [276] [Fewer Clarifications, Better Code: Benchmarking Cross-Session Personalized Ambiguity Adaptation in Coding Assistants](https://arxiv.org/abs/2607.26611)
*Zijian Xu,Wenshuo Zhang,Zisen Qin,Rui Sheng,Yushi Sun,Huamin Qu,Chuhan Shi*

Main category: cs.AI

TL;DR: 研究人员提出了CAPA，一种用于解决AI辅助编码中个性化模糊性的新方法。


<details>
  <summary>Details</summary>
Motivation: 为了解决AI辅助编码中用户意图的不确定性，研究人员提出了个性化模糊适应作为新的任务。

Method: 引入CAPA，通过六种机制表征个性化编码模糊性，并在可控的三阶段生成管道中注入这些机制。

Result: CAPA包含600个编码会话，涵盖60个平衡的用户-模糊性单元，包括300个保留的评估会话。研究人员评估了12种最近的LLMs，在无历史和同一用户历史条件下，使用可执行成功、首次成功和完成所需轮数作为指标。

Conclusion: CAPA为开发长期编码助手提供了基础，这些助手可以更好地将生成的代码与用户意图对齐，同时减少重复澄清。
...
### [277] [AlphaSchema: Exploring the Space of Trading Semantics for LLM-Based Alpha Mining](https://arxiv.org/abs/2607.26642)
*Jingyang Yi,Jian Yang,Yifei Jin,Yuqi Li,Jian Li*

Main category: cs.AI

TL;DR: AlphaSchema通过结构化空间探索和LLM解耦，有效提高了alpha挖掘的性能和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 自动化alpha挖掘越来越采用大型语言模型（LLM）代理进行因子生成和迭代发现。然而，现有的基于LLM的系统通常将因子构建和搜索决策委托给代理本身，而没有明确的探索空间或导航该空间的原理机制。因此，探索仍然在很大程度上是隐含的，难以系统性地控制或优化。

Method: AlphaSchema构建和探索alpha挖掘的语义结构空间。每个空间点都是一个由事件、上下文、品质、方向和输出组成的方案计划，在实施之前指定候选因子的语义。AlphaSchema将探索与实施解耦：LLM将选定的方案计划转换为可执行因子，而评估奖励的累积用于在语义空间上学习代理模型。迭代选择机制使用此模型来平衡全局探索、代理引导的利用和局部突变。

Result: 在股票市场实验中，AlphaSchema发现了具有强大预测和投资组合性能的因子池。进一步分析表明，语义搜索过程在导航不同区域的同时，越来越多地将评估分配给高回报区域，并且不同LLM对相同方案计划的实现表现出可比的预测质量，表明在框架内alpha挖掘质量在很大程度上对LLM的选择具有鲁棒性。

Conclusion: AlphaSchema通过结构化空间探索提高了alpha挖掘的效率和性能，并通过将探索与实施解耦，实现了对搜索过程的更有效控制和优化。
...
### [278] [Rethinking Self-Evolution: A Constrained Exploration-Exploitation Process for Mitigating Skill Overfitting](https://arxiv.org/abs/2607.26643)
*Hongqiang Lin,Chao Liu,Xiaofan Bai,Xuan Jin,Yuhong Li,Nenggan Zheng,Xipeng Cao*

Main category: cs.AI

TL;DR: SkillBoost通过探索-利用权衡优化技能，显著提升了LLM代理的性能


<details>
  <summary>Details</summary>
Motivation: 技能优化中的探索-利用权衡问题

Method: SkillBoost框架，包括结构化利用、先验引导的探索和验证接受

Result: 在23个模型-基准配置中达到最先进的性能，同时减轻过拟合，优于人工设计和LLM生成的技能

Conclusion: SkillBoost框架有效地解决了技能优化中的挑战，提高了LLM代理的性能和泛化能力
...
### [279] [AgenticCANN: Automated Ascend C Operator Generation via Knowledge-Augmented Agentic Evolution](https://arxiv.org/abs/2607.26661)
*Junhao Qiu,Zidong Wang,Yansong Sun,Zhitong Ma,Ping Guo,Qingfu Zhang*

Main category: cs.AI

TL;DR: AgenticCANN通过知识增强的代理进化框架，显著提高了Ascend C操作符的自动化合成和NPU推理性能。


<details>
  <summary>Details</summary>
Motivation: Ascend C operator optimization对于NPU推理性能至关重要，但需要深厚的硬件专业知识。

Method: 提出AgenticCANN，一个专门针对自动Ascend C操作符合成的知识增强的代理进化框架。

Result: 在华为Ascend 910B上进行了广泛的实验，证明了该方法在元素级和归一化操作符上实现了90%到100%的可行性，在融合操作符上实现了56%，在1B Pangu模型推理内核上实现了高达6.65倍的加速。

Conclusion: AgenticCANN在Ascend C操作符优化方面表现出色，提高了可行性并显著加速了推理性能。
...
### [280] [UrbanDS: A Graph-Guided LLM Multi-Agent System for Data-Intensive Urban Tasks](https://arxiv.org/abs/2607.26724)
*Zhilun Zhou,Jianghao Yu,Yuming Lin,yongjun yang,Sun Yongquan,Depeng Jin,Yong Li*

Main category: cs.AI

TL;DR: UrbanDS是一个图引导的LLM多智能体系统，用于数据密集型城市任务，表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法在数据密集型场景中存在挑战，如城市任务需要从大规模异构数据仓库中获取和利用相关信息。

Method: 提出UrbanDS，一个图引导的LLM多智能体系统，用于数据密集型城市任务。构建统一数据集图，开发数据配置文件代理、关系代理、规划代理、执行代理和报告代理。构建UrbanDS-Bench，一个城市数据科学基准。

Result: UrbanDS在数据密集型任务上优于现有数据科学代理，并在武汉市东湖高新区城市运营平台上部署，证明其实际应用的有效性。

Conclusion: UrbanDS在数据密集型城市任务中表现出色，为城市数据科学提供了有效解决方案。
...
### [281] [Do Latent Channels Actually Communicate? A Causal Audit of Latent Multi-Agent LLM](https://arxiv.org/abs/2607.26773)
*Huixiang Zhang,Mahzabeen Emu*

Main category: cs.AI

TL;DR: 通过实验，发现隐式消息对系统性能有影响，并提出了新的评估方法。


<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型（LLM）的多智能体系统（MAS）中隐式通信的问题。

Method: 引入因果审计方法，通过控制信息替换来检测通信效果。

Result: 通过在GSM8K、ARC-C和MATH-500上的实验，验证了信息内容、其他智能体价值对系统性能的影响。

Conclusion: 总准确率无法识别隐式消息如何影响接收器，建议使用控制信息比较作为隐式通信的标准评估方法。
...
### [282] [Property-driven Causal Abstractions for Markov Decision Processes](https://arxiv.org/abs/2607.26787)
*Jule Schmidt,Maximilian Weininger,Clemens Dubslaff,David Parker,Nils Jansen*

Main category: cs.AI

TL;DR: 提出了一种基于因果性的MDP抽象方法，能够有效减轻可扩展性问题，并提高决策效率。


<details>
  <summary>Details</summary>
Motivation: 为了减轻MDP中状态数量激增导致的可扩展性问题，提出了对MDP进行抽象化的方法。

Method: 引入了因果性的概念，并开发了一种基于属性驱动的因果抽象技术，该技术保留了原始MDP模型的大部分特征。该方法依赖于状态变量谓词上的因果关系，并识别那些共享相同原因以实现或违反给定抽象属性的状态。

Result: 通过理论和实证比较了各种因果MDP抽象方法，结果表明该方法对于几个标准基准测试，可以获得小的抽象化，从而计算原始MDP的近似最优策略。此外，这些因果抽象通常可以推广到相关的规模较大的MDP模型。

Conclusion: 提出的方法在减轻MDP可扩展性问题和提高决策效率方面具有潜力。
...
### [283] [From Passive Video to Editable Experience: Physically Grounded Experience Synthesis for Embodied Intelligence](https://arxiv.org/abs/2607.26903)
*Jia Luo*

Main category: cs.AI

TL;DR: Pegasus通过知识迁移解决Embodied AI数据瓶颈，实现跨形态机器人数据生成。


<details>
  <summary>Details</summary>
Motivation: Embodied AI中的关键瓶颈在于数据，机器人无法直接从人类操作视频中学习。

Method: Pegasus框架，通过结构化知识迁移将人类演示转换为机器人可学习的数据。

Result: Pegasus在多个基准测试中表现出可靠的跨形态转换，将机器人数据生成问题转化为可扩展、低资源的知识迁移问题。

Conclusion: Pegasus有效地解决了Embodied AI中的数据瓶颈问题，为跨形态机器人数据生成提供了一种新方法。
...
### [284] [What Does It Take to Detect an AI Agent? Minimal Feature Sets for Behavioral Detection under Browser Automation](https://arxiv.org/abs/2607.26935)
...
### [285] [Belief-Guided Decision Making with Uncertainty Gating in the Game of Go](https://arxiv.org/abs/2607.26946)
*Mehrad Yaghoubi,Azam Bastanfard,Abbas Jalilvand,Ashkan Rezaei*

Main category: cs.AI

TL;DR: This paper proposes a new architecture for Computer Go that improves performance on consumer-grade hardware.


<details>
  <summary>Details</summary>
Motivation: Recent advancements in Computer Go have heavily relied on Monte Carlo Tree Search (MCTS) to correct the errors of the neural network policy, but this creates a bottleneck on consumer-grade hardware and leads to hallucination.

Method: This paper introduces a novel Belief-Guided architecture that disentangles the Policy head from a distinct Belief head, and integrates memory mechanisms (Transformer/GRU) and a gating mechanism.

Result: This approach significantly improves search-free win rates and reduces hallucination, enabling professional-level play on limited hardware.

Conclusion: The Belief-Guided architecture is effective in improving the performance of Computer Go on consumer-grade hardware.
...
### [286] [On-Policy Distillation for LLM Safety: A Routing Approach to Template-Robust Realignment](https://arxiv.org/abs/2607.27081)
*Yongjian Guo,Wanlun Ma,Lingyu Shen,Xi Xiao,Sheng Wen*

Main category: cs.AI

TL;DR: 提出了一种新的安全重定位框架ROPD，有效减轻了语言模型微调中的安全风险。


<details>
  <summary>Details</summary>
Motivation: 针对现有语言模型微调的安全问题，特别是恶意数据提供者可能将有害行为嵌入下游语料库的问题。

Method: 提出了一种名为ROPD的新型安全重定位框架，该框架通过建模对齐和受损害的输出概率分布之间的差异，而不是拟合特定的提示模板。

Result: 实验结果表明，当基线防御面临模板不匹配时，ROPD可以显著减轻模板不匹配风险，在防御有效性和能力保持方面都表现出优越的鲁棒性。

Conclusion: ROPD为鲁棒的大型语言模型重定位设定了新的标准。
...
### [287] [Setoka: A Benchmark for Hierarchical User Understanding in Personalized Agents over Heterogeneous Data](https://arxiv.org/abs/2607.27056)
...
### [288] [AgentMap: Joint Equivalence and Subsumption Discovery for Ontology Matching](https://arxiv.org/abs/2607.27130)
*Yiping Song,Jiaoyan Chen,Renate Schmidt,Hui Yang,Wen Zhang*

Main category: cs.AI

TL;DR: 提出了新的混合本体匹配任务和相应的AgentMap框架，在实验中取得了显著的性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有OM系统只能识别一种语义对应关系，不能同时发现等价和子类映射。

Method: 提出了混合本体匹配（HOM）任务和基于大型语言模型（LLM）的多智能体OM框架AgentMap。AgentMap通过一系列相互依赖的语义决策来实现，整合语义检索、分层搜索和协同多智能体LLM推理。

Result: AgentMap在混合设置下取得了令人满意的性能，并且在仅等价匹配和仅子类匹配设置下分别优于等价匹配和子类匹配基线。

Conclusion: 实验结果表明，AgentMap在混合设置下表现出色，同时在仅等价匹配和仅子类匹配设置下分别优于等价匹配和子类匹配基准。
...
### [289] [Linguistic Monoculture in LLM-Assisted Language Use](https://arxiv.org/abs/2607.27134)
*Suhas Thejaswi,Juhi Kulshreshta,Lutz Oettershagen*

Main category: cs.AI

TL;DR: 本文研究了LLMs在写作和沟通中的应用，分析了不同交互机制对语言多样性的影响，并提出了内生化一致性的策略选择。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型（LLMs）在写作和沟通中的应用越来越广泛，作者对共享模型的依赖可能减少语言形式的多样性，导致语言单一化。

Method: 建立数学框架，将作者和LLMs表示为语言特征的分布，并通过重复交互进行协同进化。分析三种交互机制：具有固定语言分布的共享模型、从作者输出递归更新的共享模型以及通过作者特定和人口水平反馈更新的个性化模型。

Result: 共享模型可以推动作者向共同规范发展，递归反馈可以将共享规范重新定位，而个性化可以保持具有非零语言多样性的不同作者-模型均衡。

Conclusion: 通过内生化一致性作为策略选择，作者可能比社会最优更多地遵守规范，因为他们没有内部化他们独特性对他人提供的价值，这导致了负外部性和单一文化的代价。
...
### [290] [OmegaUse-OfficeVal: Benchmarking LLM Agents on Long-Horizon Office-Suite Tasks with Economic Grounding](https://arxiv.org/abs/2607.27155)
...
TL;DR: OmegaUse-OfficeVal基准评估LLM在办公套件任务中的表现，发现LLM在成本和速度上优于人类，但在交付质量上仍有待提高。


<details>
  <summary>Details</summary>
Motivation: 评估大型语言模型（LLM）代理在办公套件任务中的表现，特别是经济成本方面。

Method: 创建OmegaUse-OfficeVal基准，包含100个从实际办公套件请求中提取的任务，并引入经济信号进行评估。

Result: 评估了多个前沿LLM，发现它们在成本和速度上优于人类，但在交付质量上尚未达到人类水平。

Conclusion: OmegaUse-OfficeVal基准为评估LLM在办公套件任务中的表现提供了新的方法，但LLM在交付质量上仍有待提高。
...
### [291] [Partner Capability Estimation for Task-Agnostic Adaptation in Ad-Hoc Teamwork](https://arxiv.org/abs/2607.27177)
*Peter Tisnikar,Maja Swieczkowska,Benteng Ma,Gerard Canal,Matteo Leonetti*

Main category: cs.AI

TL;DR: 提出了一种基于能力的新方法，用于改进自主智能体在多任务环境下的协作能力。


<details>
  <summary>Details</summary>
Motivation: 解决当前AHT方法中存在的局限性，如合作伙伴的能力未知以及人类在具有多个有效策略的任务上可能表现不佳的问题。

Method: 提出了一种新的方法CE-CM，它通过上下文模型近似贝叶斯方法来估计任务无关的能力向量。

Result: CE-CM快速恢复隐藏的能力，减少不可行动作分配，适应时间变化。CE-CM-Div在离线人类研究中比基线CE-CM方法显著提高了能力估计。

Conclusion: 能力建模是任务无关的表示，在研究中显示出强大的鲁棒性和可解释性，证明了在人类-人工智能团队中考虑行为多样性至关重要。
...
### [292] [Can AI agents conduct open-ended AI research? Early evidence from two case studies](https://arxiv.org/abs/2607.27191)
