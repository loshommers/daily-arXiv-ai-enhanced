...
TL;DR: OVEarth-Bench扩展了地球观测的评估，揭示了当前方法的局限性，并强调了开发更高质量基准测试的重要性。


<details>
  <summary>Details</summary>
Motivation: 为了填补现有基准测试的空白，提出了OVEarth-Bench。

Method: OVEarth-Bench扩展了现有评估，包括类别广度和查询多样性。

Result: 评估揭示了当前方法的局限性，以及基于MLLM的方法在整体性能上的优势。
...
TL;DR: VETO：一种新型抗编辑工具，在保护图像编辑的同时，保持编辑质量。


<details>
  <summary>Details</summary>
Motivation: 随着图像编辑模型如FLUX.2的兴起，高保真编辑变得广泛可用，但其能力已扩展到提取和重新定位对象和身份。这增加了潜在滥用风险，现有防御措施可能被绕过。

Method: 提出VETO，一种细微的抗编辑伪装，干扰现代模型读取源图像的内部机制。同时，引入VetoBench，评估防御措施在常规局部编辑和更广泛情境转换上的效果。

Result: 在两个当代编辑模型和三个基准测试中，VETO在提供更强的保护-保真度权衡的同时，始终优于现有防御措施。

Conclusion: VETO是一种有效的抗编辑工具，能够提供比现有防御措施更强的保护，同时保持较高的编辑保真度。
...
TL;DR: 提出了一种从单幅插图自动生成Live2D模型的方法，实现了高效且高质量的模型创作。


<details>
  <summary>Details</summary>
Motivation: Live2D模型创作需要大量手动工作，缺乏端到端的生成方法。

Method: 提出了一种从单幅插图生成Live2D模型所需所有结构信息的系统，包括有序的RGBA层、每层的变形网格和参数驱动的关键帧顶点偏移。

Result: 该系统能够从单幅插图生成Live2D模型，包括完整的RGBA层、变形网格和动画，且无需教师强制，在50个保留的字符上取得了良好的效果。

Conclusion: 该系统为Live2D模型的自动化生成提供了新的解决方案，有望提高效率并降低创作成本。
...
TL;DR: PanDent：用于评估MLLMs在牙科全景放射学中牙齿级别临床推理的基准


<details>
  <summary>Details</summary>
Motivation: 准确评估多模态大型语言模型（MLLMs）在牙科全景放射学（全景片，OPG）中的应用受到缺乏精细、临床可靠的基准的限制，这些基准反映了专家的解释。

Method: 引入PanDent，一个基于精细、专家验证的牙齿级别注释的大规模、临床依据的OPG基准。数据集包含9524张高质量OPG，每张都与经验丰富的牙医产生的全面结构化注释相关联，并由口腔和颌面放射科医生进一步验证，为牙齿级别的诊断和推理提供临床可靠的监督。使用临床定义的报道逻辑从专家验证的结果构建临床一致的放射学报告，在结构化临床证据和自由文本描述之间建立明确的对应关系。这种设计使MLLMs生成报告的评估成为可能，这些报告不仅在语言上连贯，而且在临床上与专家验证的牙齿级别发现一致。在包括最先进的（SOTA）专有模型、通用领域开源模型和医学特定模型在内的多种MLLMs上进行了实验。

Result: 结果表明，当前的MLLMs可以生成流畅的报告，但无法产生临床一致的描述，在精细定位和牙齿级别诊断方面存在大量错误。在PanDent上的微调显著提高了结构-语言一致性，大大提高了视觉定位准确性和诊断正确性，并将模型输出更接近专家牙科解释。这些结果将PanDent确立为评估MLLMs中牙齿级别临床推理的严格基准，并为临床依据的牙科AI提供宝贵资源。

Conclusion: PanDent是一个用于评估MLLMs中牙齿级别临床推理的严格基准，对于临床依据的牙科AI具有重要价值。
...
TL;DR: VideoCoCo通过使用可执行代码，显著提高了视频生成模型的物理一致性。


<details>
  <summary>Details</summary>
Motivation: 为了解决现有文本到视频模型在生成物理一致动态方面的局限性，我们引入了VideoCoCo框架。

Method: VideoCoCo是一个基于代理的双引擎框架，其中可执行的Blender代码作为过程级思维链。

Result: VideoCoCo在PhyGenBench和VBench-2.0上分别将OmniWeaving基线提高了0.073和25.7，实现了两个基准测试的最佳平均分数。
...
TL;DR: 提出了一种高效的语义分割迁移攻击方法，通过共享梯度计算和路径平均方向稳定化，实现了良好的迁移性和运行时间。


<details>
  <summary>Details</summary>
Motivation: 语义分割模型容易受到可迁移的对抗性扰动的影响，但评估密集预测模型上的迁移攻击在计算上可能非常昂贵。现有的集成攻击通常依赖于多个代理模型，增加了计算成本，对于分割来说尤其困难。

Method: 提出了一种高效的单源替代方案，用于语义分割的迁移攻击。将可迁移攻击组合形式化为对可微攻击组件的链式计算，允许昂贵的源模型梯度计算得到共享。为了减少链式组合引入的更新不稳定性，进一步使用集成梯度风格的路径平均方向作为经验稳定启发式。

Result: 在 Pascal VOC 和 Cityscapes 上的实验评估了基于 CNN 和基于 Transformer 的分割模型之间的迁移效率权衡。IGME 与单源基线相比具有竞争力的迁移性，与模型集成攻击相比具有有利的运行时间，同时只需要访问一个源模型。

Conclusion: IGME 在迁移性和运行时间方面表现出色，为语义分割的迁移攻击提供了一种高效的方法。
...
TL;DR: 提出了一种新的方法，称为ProgFormer，用于预测纵向脑MRI，在多个基准测试中表现出良好的性能。


<details>
  <summary>Details</summary>
Motivation: 预测纵向脑部MRI具有挑战性，因为纵向变化通常细微且局限于特定解剖区域，而大多数受试者特定的脑结构在时间上保持稳定。因此，有效的模型应保留全局脑结构一致性，同时保持对细粒度疾病进展的敏感性。

Method: 我们提出了ProgFormer，这是一种用于纵向脑MRI预测的分层体素空间Diffusion Transformer。ProgFormer使用粗路径从3D补丁令牌执行主要体积预测。该路径建模整体脑结构和纵向上下文。细路径然后使用粗表示作为单个补丁内体素级细化的时空定位。两个路径通过条件流匹配直接在体素空间中联合估计速度场，从而实现端到端预测，无需单独学习的图像自动编码器。预测的未来扫描是通过在一系列欧拉步骤上积分估计的速度场从高斯噪声中生成的。

Result: 在三个广泛使用的基准ADNI、AIBL和OASIS上进行了大量实验，在成对和轨迹设置下，与几种最先进的方法相比，显示出有利的性能。

Conclusion: 本研究提出了一种新的方法，称为ProgFormer，用于预测纵向脑MRI。该方法在多个基准测试中表现出良好的性能，为脑部疾病的研究和诊断提供了新的可能性。
...
TL;DR: Drawing-Recode：从栅格2D CAD图纸恢复参数化CAD序列的框架，提高工业制造自动化。


<details>
  <summary>Details</summary>
Motivation: 从数字转换之前的栅格格式2D计算机辅助设计（CAD）图纸中恢复参数化CAD序列对于零件复制和制造过程自动化很重要。

Method: Drawing-Recode框架，通过图像编码器提取几何特征，通过单独的文本识别模块识别注释，然后使用交叉注意力和提出的注释定位损失（AGL）将注释明确地与几何信息联系起来。将结果特征输入到大型语言模型（LLM）中，以生成结构化参数化CAD代码（SPCC）格式的CAD代码。

Result: 实验表明，Drawing-Recode优于现有基线，并且在类似于工业条件的扫描图上保持鲁棒性。

Conclusion: Drawing-Recode有助于在工业环境中数字化栅格2D CAD图纸，并有助于零件复制和制造自动化。
...
TL;DR: MedFrameQA中最强的鲁棒适应家族是直接解码器仅答案的SFT，且其鲁棒性优于其他方法。
...
Method: 比较了基于控制器的推理、框架演化、静态混合监督、以续集为重点的变体和仅直接答案的监督微调（SFT）等方法。

Result: 在MedGemma-1.5-4B上，直接解码器仅答案的SFT是最强的鲁棒适应家族。此外，后处理校准有效地修复了置信度估计，而核心方法可以一致地转移到Qwen2.5-VL-3B等二级骨干上。

Conclusion: 因此，主要结果不是复杂的辅助机制获胜，而是目标对齐的直接答案SFT是我们发现的MedFrameQA最强的鲁棒适应家族。通过建立这个强大而简约的基线，我们希望将社区的关注点转向根本的鲁棒优化，而不是架构复杂性。
...
TL;DR: 提出了一种基于SAM的浮游动物图像实例分割模型，显著提升了分割性能。


<details>
  <summary>Details</summary>
Motivation: 由于SAM在微观图像实例分割中缺乏浮游动物特定领域知识，导致其性能有限。

Method: 提出了一种基于SAM和小波变换的新型实例分割模型（ZMIS-SAM），包括ZM-ViT、NFAM和WM2FE模块。

Result: ZMIS-SAM在浮游动物数据集上实现了最先进的实例分割性能，并在多个公共跨领域数据集上表现出强大的泛化能力。

Conclusion: ZMIS-SAM模型能够有效解决浮游动物图像实例分割中的问题，提高了分割精度。
...
TL;DR: MeshFM efficiently extracts rich 3D features using 2D information.


<details>
  <summary>Details</summary>
Motivation: Extract rich features from 3D inputs efficiently.

Method: Distill 2D features from visual foundation models into 3D, train a feedforward network to predict 3D features, utilize two-stage training strategy.

Result: Learned features applicable to downstream tasks like part segmentation, dense correspondence, and mesh deformation. Performs well without 3D annotation or fine-tuning. Robust to extreme rotations of input objects.

Conclusion: MeshFM is an efficient and robust framework for 3D feature extraction.
...
TL;DR: 提出MPIE-Bench和MPIE-Eval，改进了多人图像编辑的评估方法，提高了评估的准确性和可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有的文本到图像和个性化编辑模型可以轻松合成高保真度的单人图像，但在将多人放入共享接触动作（如拥抱、携带或扭打）时，仍然存在主要问题，如融合肢体、虚构的四肢和相互穿透的身体。现有的评估主要忽略了这些解剖和几何问题，而VLM作为裁判的清单通常在交互方面饱和，而错误对人类来说仍然明显。

Method: 引入MPIE-Bench，一个包含2,500个样本的基准，这些样本跨越了405个场景，14个交互类别和四种接触密度（C0-C3）。还提出了MPIE-Eval，其两个新轴从冻结的公共多人网格重建中评分接触时间的几何形状。解剖学询问是否每个类似人体的质量都可以用完整的重建身体来解释，而交互询问这些身体之间的穿透和表面距离是否与指令请求的接触相符。

Result: 在十个编辑器中，网格解剖学在两个不同模型上达到0.65，网格交互达到0.72，没有单个编辑器在两者中都表现出色，而VLM清单对相同图像的评分超过0.95。五名评分员的研究证实，这两个轴比零样本VLM裁判更接近人类判断，并且在消除每个权重和阈值的情况下，排名保持不变。

Conclusion: MPIE-Bench和MPIE-Eval为视频挖掘编辑三元组提供了一种新的评估方法，能够更准确地捕捉到图像中的人体接触和几何问题，从而提高了评估的准确性和可靠性。
...
TL;DR: MedXplore是一种用于医学图像分析的GCD框架，通过感知和决策优化，显著提高了病变检测的准确率和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: Deep learning在医学图像分析中展现出巨大潜力，但现有方法大多依赖于大规模标注和封闭世界假设，这在临床实践中很少成立。虽然广义类别发现（GCD）在自然图像上取得了快速发展，但在医学成像中仍处于探索阶段。

Method: 提出了一种名为MedXplore的统一框架，用于可靠的医学GCD，从感知和决策层面进行优化。在感知层面，采用频率域视角，频率-SNR自适应注意力一致性（FAAC）执行可学习的全频谱过滤和全局-局部能量对比激活，不仅突出相对于全局背景的局部异常信号，还提供可靠的语义锚点以进行补丁一致性学习。在决策层面，自适应余弦角余量（ACAM）根据语义难度和特征置信度调整角余量，以平衡类内紧密度和类间可分性。两个模块共同提高了病变敏感的表示学习并减轻了旧类别偏差。

Result: 在多个基准测试中，与最强大的竞争方法相比，平均提高了8.5%的All准确率。在Kvasir上，MedXplore将误报旧错误从14.50%降低到0.80%，在严重的旧-新模糊性下表现出强大的鲁棒性。

Conclusion: MedXplore是一种有效的医学GCD框架，在提高病变检测的准确率和鲁棒性方面具有显著优势。
...
TL;DR: 提出盲PSNR网络，在低光图像增强PSNR预测中表现优异，准确率显著提升。


<details>
  <summary>Details</summary>
Motivation: 解决低光图像增强（LLIE）中参数固定导致的性能退化问题，以及手动选择最佳配置的耗时和不切实际性。

Method: 提出盲PSNR网络，通过窗口交叉注意力和异方差回归融合增强图像和退化低光输入，预测PSNR。

Result: BlindPSNR在基准测试中达到了89.5%的top-1准确率，与基线相比提高了34.1%，泛化能力较强（SRCC = 0.61-0.67）。

Conclusion: BlindPSNR是一个轻量级的无参考网络，在低光图像增强的PSNR预测方面表现出色，有效提高了准确率并降低了回归误差。
...
TL;DR: 4DHumanDiff：从文本提示直接生成高质量的360度动态人像。


<details>
  <summary>Details</summary>
Motivation: 生成高质量的360度动态人像资产具有挑战性，现有方法通常先合成单目或多视角视频，然后拟合4D表示，这既昂贵又往往导致不完整的几何形状或视角不一致的渲染。

Method: 提出4DHumanDiff，一种扩散框架，直接从文本提示生成由4D高斯散点（4DGS）表示的动态人像。使用3D U-Net骨干网络和时间注意力进行运动感知生成。构建了一个包含60,000个高质量对的文本到4DGS的大规模数据集，并引入了2D正则化和无训练的4D插值来提高渲染质量和运动平滑度。

Result: 4DHumanDiff在不到一分钟内生成一致的360度动态人像，实现了更好的时间和多视角一致性，并将推理时间减少了10倍以上。

Conclusion: 4DHumanDiff是一个有效的框架，可以生成高质量的360度动态人像，具有更好的时间和多视角一致性以及更快的推理速度。
...
TL;DR: 提出MMOOC基准，评估MLLM拒绝和鲁棒回答能力，发现当前模型在变化条件下存在困难，训练后可提升性能


<details>
  <summary>Details</summary>
Motivation: 为了解决MLLM在视觉-语言任务中在特定条件下失败的问题

Method: 提出MMOOC，一个大规模的基准，用于评估MLLM拒绝和鲁棒回答的能力

Result: 实验表明，当前模型在变化条件下难以平衡回答能力和拒绝能力，但训练后可以提升鲁棒性

Conclusion: MMOOC有助于评估MLLM的拒绝和鲁棒回答能力，对于提升模型性能具有重要意义
...
TL;DR: FedPAIE是一种联邦个性化审美图像增强框架，它通过去中心化学习用户偏好，在资源受限的设备上实现自然图像增强。


<details>
  <summary>Details</summary>
Motivation: 个性化图像增强应反映个人审美偏好，但学习这些偏好通常依赖于不适合集中收集的私人照片和评分。任务必须从稀疏、异构的反馈中推断偏好，并将其转化为在资源受限的用户设备上看起来自然的颜色变换。

Method: 提出了一种名为FedPAIE的联邦个性化审美图像增强框架，用于用户自适应的色彩分级，而不需要集中收集原始照片或评分。该框架训练了一个轻量级的双线索审美评分器，将其校准为一个小型本地支持集上的个性化评分器，并将其冻结以指导从未配对本地照片中轻量级CLUT增强器的正则化自适应。

Result: 实验表明，FedPAIE在MIT-Adobe FiveK和Flickr-AES数据集上展示了有效的开放世界个性化，并在用户偏好和图像保真度之间取得了良好的平衡。

Conclusion: FedPAIE将去中心化偏好学习与高效的个性化图像变换相结合，无需配对用户润色。
...
TL;DR: WEP通过分析视觉贡献来提高MLLMs的推理时间风险检测，显著提高了答案的可靠性。


<details>
  <summary>Details</summary>
Motivation: 为了确保多模态大型语言模型（MLLMs）的可靠部署，需要决定是否信任、审查或将自信的视觉答案路由到更强的系统。

Method: 使用与生成答案相同的白盒预填充路径进行推理时间风险检测，通过见证证据组合（WEP）估计哪些视觉贡献支持或反驳预测的候选答案。WEP通过两个可解释的路线家族总结这些贡献：与问题相关的证据来源和有符号的证据集中度。嵌套分组验证选择更可靠的家族和稀疏的top-k路线投资组合，将其与候选置信度融合。

Result: 在三个MLLMs和四个二进制答案基准测试中，WEP将平均误差AP提高了0.134。所有12个模型-数据集增益都是正的，10对图像聚类自举区间严格为正。

Conclusion: WEP针对白盒封闭答案系统，并使用标记的校准切片。
...
TL;DR: 提出了一种新的拼图基准测试，发现现有视觉语言模型在几何推理方面存在不足，特别是在更大的网格上。


<details>
  <summary>Details</summary>
Motivation: 解决拼图问题时需要同时推理视觉内容和几何约束，但现有的基准测试使用矩形切割，在纹理重复区域产生模糊的地面真实情况。

Method: 提出一个新的基准测试，其中包含带孔和空白互锁的拼图块，几何约束提供强大的局部兼容性要求，与视觉内容结合，产生明确的地面真实情况。

Result: 在95K个实例中，发现零样本视觉语言模型在几何推理方面存在很大不足；在4x4拼图中，只有一种前沿模型（GPT-5.5）超过了随机基线，而其他所有模型都处于随机水平。尽管在4x4拼图中，监督微调达到了>97%，但在更大的网格上，所有模型都崩溃：GPT-5.5从70%下降到几乎随机，甚至微调后的模型在12x12网格上的表现低于5%。这种“缩放悬崖”表明，当前的架构无法随着拼图数量的增加而保持一致的约束满足。

Conclusion: 我们的基准测试为视觉语言模型建立了可扩展的几何推理作为一个开放挑战。
...
TL;DR: RefineSVG：一种通过闭环视觉反馈实现高保真图像到SVG生成的框架


<details>
  <summary>Details</summary>
Motivation: 提高图像到SVG的生成质量

Method: 提出RefineSVG，一种单步闭环视觉反馈框架，通过自我校正实现高保真图像到SVG的生成

Result: RefineSVG在重建保真度、结构精度和代码效率方面优于现有基线

Conclusion: RefineSVG有效解决了现有方法中存在的几何漂移、错误累积和视觉幻觉问题
...
TL;DR: CaRe: A training-free framework for visual token reduction in VLMs, improving efficiency without compromising performance.


<details>
  <summary>Details</summary>
Motivation: Large Vision-Language Models (VLMs) suffer from prohibitive inference overhead due to long sequences of visual tokens.

Method: Introduce the principle of 'Calibrate Before Reason' to visual token reduction and propose CaRe, a training-free robust framework. CaRe consists of two modules: Perturbation-Robust Calibration Anchoring and Confidence-Gated Token Calibration.

Result: CaRe outperforms state-of-the-art token reduction baselines. Pruning 94.4% of visual tokens, the method retains 96.4% of the original full-token performance and delivers up to 2.30 times faster end-to-end inference speed.

Conclusion: CaRe effectively preserves semantic fidelity in VLMs and significantly improves inference speed through visual token reduction.
...
TL;DR: PrintAnything：直接从点云生成3D打印G代码，无需网格重建。


<details>
  <summary>Details</summary>
Motivation: 为了克服现有3D打印流程需要水密网格作为输入的限制，直接使用点云进行制造受到阻碍。

Method: 提出PrintAnything框架，直接从3D点云生成可执行3D打印G代码，无需网格重建。引入切片点投影策略，将无结构的3D点云转换为与3D打印层叠特性的切片对齐的2D表示。提出几何计划（G-plan）映射，一个由占用、区域和流图组成的紧凑2D表示，用于编码3D打印工具路径合成所需的几何和挤出属性。

Result: 准确生成可打印的G代码，实现无网格的3D打印流程。

Conclusion: PrintAnything框架为3D打印提供了一种新的解决方案，克服了传统方法的限制，并提高了打印质量。
...
TL;DR: 提出了一种基于头部姿态恢复全身网格的新方法，实现了快速推理和性能提升。


<details>
  <summary>Details</summary>
Motivation: 针对仅从头部姿态恢复全身网格的问题，提出了一种简单而新颖的方法。

Method: 利用引导网络的潜在空间，设计了一种变分自动编码器，以全身姿态作为输入。通过强制引导网络和头部运动网络的潜在分布相似，从'引导'分布中采样的潜在特征可以可靠地解码为全身姿态的自然表示。

Result: 与基于扩散的方法相比，一步采样方案实现了显著快速的推理（快50倍以上）。在基准数据集上的实验结果表明，该方法有效地提高了自我身体网格重建的性能。

Conclusion: 提出的方法在仅使用头部姿态的情况下，有效地恢复了全身网格，显著提高了推理速度，并提高了自我身体网格重建的性能。
...
TL;DR: 提出CXR-Retrieve，提高胸部X光片检索的准确性。


<details>
  <summary>Details</summary>
Motivation: 由于大部分胸部X光片研究仅与自由文本报告配对，而不是结构化临床注释，大型胸部X光片档案难以搜索。

Method: 提出CXR-Retrieve，一个用于组合胸部X光片文本到图像检索的结构化基准。包含来自MIMIC-CXR-JPG官方测试分割的5,159张测试图像和145个文本查询。

Result: 该方法在两个病理联合查询上提高了Precision@5 8.5个百分点，在否定查询上提高了22.0个百分点。

Conclusion: 可靠的胸部X光片检索需要训练目标，不仅包括提到的发现，还包括它们在临床上的表述方式。
...
TL;DR: FDDWAN是一种新的水印去除方法，在去除水印的同时保持图像质量。


<details>
  <summary>Details</summary>
Motivation: 现有水印去除方法难以准确捕捉水印特征，导致水印抑制和感知保真度之间的权衡不佳。

Method: 提出了一种名为FDDWAN的频率解耦扩散水印攻击网络，该网络通过小波域分解和残差扩散细化进行水印去除。

Result: 在CelebA和ImageNet数据集上，FDDWAN在去除水印的有效性和视觉保真度方面优于传统和学习型攻击方法。

Conclusion: FDDWAN在水印去除方面取得了更好的效果，实现了水印抑制和感知保真度之间的良好权衡。
...
TL;DR: LoMeVQA基准和MedLong-8B模型在时间医学视觉推理方面取得了突破。


<details>
  <summary>Details</summary>
Motivation: 在临床实践中，患者经常在多次就诊中进行多项影像学检查，产生纵向数据。对这种时间信息进行建模对于可靠地评估疾病进展和治疗反应至关重要。然而，尽管多模态大型语言模型（MLLMs）发展迅速，纵向医学视觉推理仍然在很大程度上未被探索。为了填补这一空白，我们提出了LoMeVQA，这是一个包含206K纵向视觉问答（VQA）对的综合基准，用于时间医学图像分析。

Method: 我们提出了LoMeVQA，这是一个包含206K纵向视觉问答（VQA）对的综合基准，用于时间医学图像分析。为了构建数据集，我们开发了一个自动化流程，该流程（1）按时间顺序组织患者记录，（2）通过医学知识图谱提取具有临床意义的实体，以及（3）对它们的时序演变进行建模，以指导大型语言模型生成高质量的纵向VQA对。

Result: 广泛的评估表明，通用型和医学领域MLLMs在LoMeVQA上的表现都较差，揭示了在时间推理方面的重大局限性。为了解决这些局限性，我们引入了MedLong-8B，它在所有任务中都实现了最先进的性能。除了基准测试之外，我们还进行了详细的分析，揭示了关键失败模式，并阐明了如何改进纵向医学视觉推理。

Conclusion: LoMeVQA是一个用于时间医学图像分析的基准，揭示了在时间推理方面的重大局限性，并引入了MedLong-8B来解决这些问题。
...
TL;DR: SPFM-Net：一种有效且视觉保真度高的不可见水印攻击方法。


<details>
  <summary>Details</summary>
Motivation: 现有的水印攻击方法难以捕捉全局水印信号的长期依赖关系，导致去除效果和视觉保真度之间的权衡不佳。

Method: 提出SPFM-Net，一种语义先验引导和频率约束的Mamba框架，用于不可见水印攻击。首先使用高比率掩码破坏水印信号的空间一致性，然后利用部分微调的预训练掩码自动编码器从稀疏观察中重建语义一致的图像，同时抑制与水印相关的信息。接着，多尺度残差频率特征交互模块聚合多个感受野中的水印相关残差特征，同时自适应抑制与水印无关区域响应。最后，引入轻量级的基于Mamba的全局状态空间特征建模（GSFM）单元，进一步捕捉全局分布水印信号的长期依赖关系，并抑制剩余的水印痕迹。

Result: SPFM-Net在多个水印方案上进行了大量实验，证明了其在水印攻击效果和感知保真度之间的良好权衡。

Conclusion: SPFM-Net能够有效抑制水印，同时保持感知质量，在不可见水印攻击中表现出色。
...
TL;DR: 提出了一种基于IGS的解码框架VGD，有效减少物体幻觉，同时保留模型原有性能。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型（LVLMs）经常在图像中产生不存在的物体，现有方法缺乏可靠的物体级定位诊断，因此往往采用粗粒度干预，这可能会损害视觉理解、缩短响应时间并减少真实定位物体的覆盖率。

Method: 提出了一种基于内在定位签名（IGS）的解码框架，该框架通过轻量级验证器检查每个出现的物体提及，当提及被识别为高风险时回滚KV缓存，抑制物体及其同义词，并重新生成受影响的后续内容。

Result: 在CHAIR和AMBER-G数据集上，VGD实现了最先进的物体幻觉减少，同时保留了模型的原有视觉理解和定位物体覆盖率。

Conclusion: VGD通过仅对识别为高风险的物体提及进行干预，减少了物体幻觉，同时保留了模型的原有视觉理解和定位物体覆盖率。
...
TL;DR: 提出了一种新的证据路由方法，显著提高了HR-VQA的性能。


<details>
  <summary>Details</summary>
Motivation: 为了解决高分辨率视觉问答（HR-VQA）中证据获取不足的问题，研究者们通常采用通过裁剪、重新编码或多轮搜索等方法来再次检查图像。然而，这种观点并不全面，因为许多情况下，细粒度证据已经在视觉编码过程中存活下来，并在中间层路由窗口中变得可识别和有影响力，但在生成答案之前被稀释了。

Method: 提出了一种名为“Thinking-Once”的无训练、单视觉传递的证据路由方法，该方法在窗口中重建问题条件下的注意力，保留核心实体标记和紧凑的背景上下文，并在不进行额外视觉编码的情况下将此证据路由到后续层。

Result: 在五个基础模型上，Thinking-Once在V$^*$Bench、HRBench-4K和HRBench-8K上平均提高了3.1、3.0和2.7分，同时将平均峰值内存减少了约4GB。在Qwen2.5-VL-7B上，它将三个基准测试提高了9.9、4.6和5.5分，将跨基准平均值从72.5提高到79.1。使用ZwZ-8B基础模型，Thinking-Once的平均分数达到82.7。与11个开源HR-VQA基准相比，它在所有三个基准平均值和最佳整体平均分上均获得最佳或并列最佳成绩。

Conclusion: 通过路由已编码的证据而不是重复获取新的视觉输入，可以改善HR-VQA。
...
TL;DR: 提出了一种新的SAF门控机制，显著提高了细胞分割和分类的准确性，在PanNuke数据集上实现了最高的mPQ。


<details>
  <summary>Details</summary>
Motivation: 准确地对细胞进行分割和分类对于数字病理学至关重要，它能够实现组织分析的定量诊断和治疗规划。编码器-解码器架构通过跳过连接融合多尺度特征已成为这一任务的占主导地位的方法。然而，标准的直接跳过连接对每个空间位置都同等对待，导致冗余和可能冲突的信息到达解码器。为了克服这个问题，已经引入了各种门控机制，但其中大多数仅对编码器信息进行过滤，忽略了来自解码器的全局上下文信息的益处。

Method: 本研究提出将基于CellViT的模型中的传统跳过连接替换为一种新颖的空间注意力融合（SAF）门控模块。每个SAF门将编码器跳过和上采样解码器特征连接起来，通过两个具有中间ReLU的点卷积进行压缩，并应用通道softmax生成每个像素的“信任热图”，该热图在所有空间位置上求和为1，使网络能够学习每个来源在哪里最可信。

Result: 使用SAF门控的结果是融合特征提高了模型检测少数“死亡”类别的能力，从而增强了在PanNuke数据集上的多类全景质量（mPQ）。SAF门控与包括无门控、注意力门控、squeeze-and-excitation、CBAM、交叉注意力和注意力特征融合在内的六种门控替代方案在PanNuke和MoNuSeg数据集上进行了比较。SAF门控实现了最高的mPQ（0.471），这一提高主要是由与无门控CellViT基线相比Dead类F1分数提高了14.5个百分点所驱动。

Conclusion: SAF门控在提高细胞分割和分类的准确性方面表现出色，显著提升了多类全景质量，为数字病理学提供了有力的工具。
...
TL;DR: FeatFix通过局部精确特征校正，加速了扩散模型的生成过程。


<details>
  <summary>Details</summary>
Motivation: 为了提高扩散模型生成高质量图像和视频的速度，减少迭代去噪过程的计算成本。

Method: 引入FeatFix，一种用于缓存扩散推理的局部精确特征校正方法。在选定的站点上，用精确的输出替换草稿块输出，避免部分替换和全时间步重计算。

Result: 在四个图像和视频骨干网络上进行实验，FeatFix在保持输出质量具有竞争力的同时，实现了最高达$6.70	imes$的加速。

Conclusion: FeatFix能够有效地减少扩散模型的计算成本，提高生成速度。
...
TL;DR: VCP-DCN is a depth collaborative network for COD that improves detection performance by focusing on modality-specific features in the depth domain.


<details>
  <summary>Details</summary>
Motivation: Several existing COD methods introduce depth maps to boost detection performance via learning complementary RGB-D features, ignoring modality-specific characteristics of concealed objects in the depth domain.

Method: We propose a depth collaborative network, called VCP-DCN, to mine distinguishable multi-modality features beyond visual concealed prototype in depth domain. The network performs multi-modality alignment, interaction, and fusion. Specifically, it includes a Separable Prototype Embedding (SPE) module, a Multi-modality Dual Attention (MDA) module, and a Depth Adaptive Injection (DAI) module.

Result: Extensive experiments demonstrate the effectiveness of our VCP-DCN on three authoritative datasets.

Conclusion: VCP-DCN effectively addresses the limitations of existing COD methods by focusing on modality-specific characteristics of concealed objects in the depth domain and achieving improved detection performance.
...
TL;DR: 提出FAME基准，评估FS-MIS解决方案，发现直接视觉适应和有效利用支持样本的重要性，并指出语义迁移和目标不存在识别的挑战。


<details>
  <summary>Details</summary>
Motivation: 为了解决现有方法在评估设置不一致导致相对有效性不清楚的问题，提出一个统一的基准FAME来评估FS-MIS解决方案。

Method: 提出FAME基准，包含14,958个测试样本，涵盖7个解剖部位，9种成像模式，14个ROI类别，并在零样本和十样本设置下评估模型，同时评估目标不存在识别和泛化能力。

Result: 发现直接视觉适应通常优于基于提示的策略，增加支持样本可以提高性能，但需要有效利用，语义迁移比成像域适应更具挑战性，并且强大的定位能力不一定意味着可靠的目标不存在识别。

Conclusion: FAME提供了对当前FS-MIS解决方案的全面理解，并促进了更有效和可靠的few-shot医学分割方法的发展。
...
TL;DR: EEG-EditBench通过编辑实验评估EEG图像模型，发现标准检索结果不能完全反映模型性能


<details>
  <summary>Details</summary>
Motivation: 考察视觉信息在EEG图像模型中的保留情况

Method: 构建EEG-EditBench，通过控制编辑对象身份、属性、背景和对象存在性来评估模型

Result: 标准检索结果不能一致地转移到基于编辑的评估，细粒度的属性变化是最大的挑战

Conclusion: EEG-EditBench揭示了模型行为，这些行为被综合检索精度所隐藏
...
TL;DR: Drones2BodyLanguage数据集通过在真实无人机场景中引入avatratrs，显著提升了无人机对人类动作和意图的感知能力。


<details>
  <summary>Details</summary>
Motivation: 感知长距离的人类动作和意图是社会智能无人机的前提，但相关数据稀缺。

Method: 引入Drones2BodyLanguage数据集，通过将avatratrs放置在真实无人机镜头中的4K无人机场景中，来建立人类动作与真实无人机镜头之间的联系。

Result: 在十二种架构和场景-运动分离的分割中，在放置的数据上进行训练，对于真实、重定向和生成的动作，平均意图准确性有显著提升。

Conclusion: 该方法在真实场景中验证了其有效性，为无人机在社会智能领域的发展提供了新的可能性。
...
TL;DR: DECODE improves AI-generated image detection with continual learning.
...
Conclusion: DECODE effectively mitigates forgetting in continual learning for AI-generated image detection.
...
TL;DR: 提出CARE框架，通过协同学习提高VLMs对抗鲁棒性。


<details>
  <summary>Details</summary>
Motivation: VLMs如CLIP易受对抗攻击，对实际应用和部署构成严重问题。

Method: 提出CARE（协同对抗鲁棒性微调，使用嵌入对齐）框架，在训练过程中维护多个专家，通过嵌入空间和谐实现知识交换，并将学习到的知识整合到单个统一鲁棒模型中。

Result: 在经典图像分类和下游视觉语言任务上进行了大量实验，证明该方法的有效性，CARE能够优于单独学习的模型专家。

Conclusion: 跨模型专家的协作学习是提高对抗鲁棒性的有希望的方向。
...
TL;DR: CoRE-UIR是一种高效的图像恢复方法，通过分解恢复能力并引入先验和调制机制，实现了对退化图像的高质量恢复，同时保持了良好的性能和效率。


<details>
  <summary>Details</summary>
Motivation: 无人机和卫星获取的遥感图像经常受到不利天气、光照变化和成像伪影的影响，这些影响可能同时发生并共同导致全局分布偏移和局部结构损坏。尽管All-in-One图像恢复提供了一种吸引人的统一替代方案，但现有方法仍然存在弱或隐含的退化线索和参数冗余，这是由具有重叠恢复行为的全秩多专家设计引起的。

Method: 我们提出了CoRE-UIR（通用图像恢复的通用和残差专家），这是一个以通用和残差专家块（CoRE）为中心的先验引导的全局-局部框架。CoRE明确地将恢复能力分解为对退化不变的恢复的通用密集专家和对退化特定的补偿的低秩残差专家，从而实现自适应专业化，而无需重复专家复制。在此基础上，退化先验嵌入（DPE）将冻结的CLIP特征适应到显式的恢复先验中，而全局特征调制（GFM）在局部残差补偿之前对全局特征统计进行对齐。我们还构建了MDVD-108K（多退化VisDrone），这是一个覆盖单种和复合退化的大规模无人机恢复数据集，以及一个真实世界的测试集。

Result: 在多个数据集上的大量实验表明，与最强的基线BaryIR相比，CoRE-UIR提高了整体平均PSNR 1.05 dB，同时运行速度快11.83倍，内存峰值降低85.3%，从而保持了良好的质量-效率权衡。在下游任务和未见过的退化上的评估也验证了CoRE-UIR的泛化能力。

Conclusion: CoRE-UIR在图像恢复方面具有优异的性能和效率，具有良好的泛化能力。
...
TL;DR: SPaTS：基于单块视觉标记的场景文本检测，显著提升检测精度。


<details>
  <summary>Details</summary>
Motivation: 解决多块模式引入的冗余噪声和定位模糊问题，提高密集或小文本实例的检测精度。

Method: 提出Single-Patch Text Spotting (SPaTS)框架，通过单个锚点视觉标记和全图细化进行文本检测。引入SPaSO和DEA/PED技术，优化视觉标记选择和几何定位。

Result: SPaTS在多个实验中均优于其他MLLMs和OCR MLLMs，代码即将发布。

Conclusion: SPaTS在场景文本检测任务中表现出色，显著优于其他MLLMs和OCR MLLMs。
...
TL;DR: 提出了一种新的反射对称性检测方法，该方法通过抑制不对称干扰和增强旋转等变性，在准确性和鲁棒性方面均达到最先进水平。


<details>
  <summary>Details</summary>
Motivation: 检测反射对称性存在挑战，因为不对称区域和对称模式的不规则方向引入干扰。

Method: 提出不对称区域降噪（ARD）模块和旋转等变特征相似性匹配（REFSM）模块。ARD模块抑制不对称干扰以细化对称模式，而REFSM模块通过原始图像和旋转图像之间的特征相似性匹配增强旋转等变性。

Result: 在四个标准数据集（DENDI、NYU、LDRS、SDRW）和提出的GMSYM数据集上进行了大量实验，证明该方法在准确性和鲁棒性方面均达到最先进水平。

Conclusion: 提出的方法在反射对称性检测方面具有显著优势。
...
TL;DR: LAST is a new framework for pruning visual tokens in MLLM inference, achieving strong performance with high accuracy and low computational cost.


<details>
  <summary>Details</summary>
Motivation: Multimodal foundation models are reshaping edge-cloud visual intelligence, but dense visual-token sequences increase cloud-side inference costs.

Method: Proposed LAST, a training-free framework for query-dependent visual token pruning in edge-cloud collaborative MLLM inference.

Result: LAST consistently achieves the strongest performance, preserving 95.4% of the full-token accuracy while retaining only 12.5% of the visual tokens.

Conclusion: LAST is an effective solution for query-dependent visual token pruning in edge-cloud collaborative MLLM inference.
...
TL;DR: 提出了一种提高MLLMs图像检索性能的新方法，该方法通过精细粒度上下文建模和微调策略实现了显著的性能提升。


<details>
  <summary>Details</summary>
Motivation: 由于多模态大型语言模型（MLLMs）在多模态处理和推理能力方面的强大可推广性，它们在通用图像检索器方面显示出巨大的潜力，有效地解决了各种现实世界的图像检索任务。然而，虽然开创性研究前景广阔，但它们忽略了精细粒度上下文建模和分离的微调目标在提高MLLMs检索性能方面的潜力，特别是在长文本到图像检索、视觉对话检索和组合图像检索（CIR）等复杂任务中。

Method: 提出了一种自动化的精细粒度多模态五元数据集构建流程和一种新颖的两阶段精细粒度多模态微调策略。数据集生成流程生成一个包含精细粒度图像标题和修改文本的全面CIR数据集，便于精细粒度上下文建模。将微调过程分为两个不同的阶段：（1）面向精细粒度上下文推理的微调；（2）面向精细粒度检索的微调。

Result: 广泛的实验表明，与现有方法相比，在零样本检索设置中，即使与更轻量级的MLLM主干相比，我们的方法也显示出显著的优越性。

Conclusion: 提出的方法有效地提高了MLLMs在复杂图像检索任务中的检索性能。
...
TL;DR: 提出了FootprintNet，一种有效的动态足迹检测方法，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有的多时相变化检测（MTCD）方法存在局限性，无法表征与人类活动密切相关的人类活动频繁变化的区域。

Method: 提出了城市建筑动态检测（UBDD），从多时相图像中识别建筑变化动态足迹，并引入FootprintNet来抽象建筑变化过程。此外，还引入了建筑变化动态得分（BCDS）来评估预测结果。

Result: 在TSCD、MUDS和WUSU数据集上进行了广泛的实验，证明了FootprintNet优于当前最先进的方法。

Conclusion: FootprintNet能够有效地检测动态足迹，并具有更高的准确性。
...
TL;DR: 我们开发了一个基于自注意力的回归模型，用于从掩码MRI中补全健康脑组织，显著提高了补全图像的质量。


<details>
  <summary>Details</summary>
Motivation: 为了完成T1加权MRI中掩码区域的健康脑组织解剖学合理的补充，提供无肿瘤的解剖学参考，以便进行下游分析。

Method: 构建一个确定性回归模型，并专注于为其提供针对补丁的归纳偏差。网络遵循U-DiT原则，在降采样令牌网格上执行自注意力操作：体积编码器-解码器通过降采样的全局自注意力块导入长距离上下文，其中三维旋转位置嵌入，同时卷积和跳过连接保留高频细节。

Result: 在官方BraTS-2026验证排行榜上，我们的提交达到了219个案例的平均健康区域SSIM为0.864，PSNR为24.7 dB，MSE为4.6×10^-3。

Conclusion: 该模型通过约束注意力和添加对称输入来提高补丁的准确性，并分析残差平滑性对解剖现实性的影响。
...
TL;DR: ViP-Rig是一种通过视觉提示增强的自动建模框架，能更精确地恢复骨骼和皮肤权重，并支持更精细的建模控制。


<details>
  <summary>Details</summary>
Motivation: 艺术家在动画任务中经常需要根据特定要求编辑初始模型的骨骼结构和变形行为，而现有的自动方法在生成合理的模型时对骨骼和变形行为的控制有限。

Method: ViP-Rig框架，通过将用户绘制的2D骨骼和刚度提示的特征注入到预训练的骨干网络中，支持提示优先的建模和结果引导的编辑。包括骨骼生成和皮肤预测两个阶段。

Result: 在Articulation-XL2.0和ModelsResource上的实验表明，ViP-Rig在提示引导的评估下比基于几何条件的基线更准确地恢复目标骨骼和皮肤权重。

Conclusion: ViP-Rig能够提供更精确的骨骼和皮肤权重恢复，并支持更精细的建模控制。
...
TL;DR: 提出了一种基于深度学习的分层分类模型，提高了昆虫生物多样性监测的效率和准确性。


<details>
  <summary>Details</summary>
Motivation: 昆虫种群数量下降，对生物多样性的监测变得迫切，但监测受到标准化数据和专家昆虫学家昂贵且耗时的手动鉴定方法的阻碍。

Method: 提出了一种基于深度学习的分层分类模型，使用手动整理的约一百万张昆虫图像数据集，并采用五级变量深度分层分类模型架构。

Result: 模型在测试数据上实现了80-99%的每级准确率，涵盖五个层次。

Conclusion: 该模型通过利用生物分类学提取特定粒度的视觉特征，对最深层次的分类水平进行预测，提高了昆虫生物多样性监测的效率和准确性。
...
TL;DR: 研究证明当前FMs的潜在空间适合有丝分裂图检测和密集对象检测。


<details>
  <summary>Details</summary>
Motivation: 研究病理基础模型（FMs）的潜在空间是否适合作为密集对象检测范式的主干。

Method: 在MIDOG++数据集和TUPAC16数据集上，将FM骨干与单阶段、双阶段和基于自注意力的检测器相结合，并与基于ResNet50架构的端到端训练基线进行比较。

Result: H-optimus-0和Virchow模型表现出竞争力，表明当前FMs的潜在空间适合直接进行有丝分裂图检测，并在我们的域外测试案例中可能略微更稳健。

Conclusion: 当前FMs的潜在空间适合直接进行有丝分裂图检测，可能适合密集对象检测范式。
...
TL;DR: 基于事件相机的事件辅助互补运动细化框架（ENCORE）有效提高了学习视频压缩的准确性，并在多个数据集上取得了显著的性能提升。


<details>
  <summary>Details</summary>
Motivation: 为了提高学习视频压缩的准确性，减少相邻帧之间的冗余，提出了基于事件相机的事件辅助互补运动细化框架。

Method: 首先使用互补运动表示（CMR）将RGB-事件特征分解为公共和模态特定的运动表示。然后，空间能量和冗余信息感知校准（SERIC）识别事件特定的响应，相对于RGB抑制弱或冗余的证据，并预测候选流校正。最后，能量感知路由（EAR）确定如何以及在哪里对RGB流进行校正。事件仅作为运动建模的辅助模态，而RGB仍然是唯一的编码和重建目标。

Result: 在BS-ERGB、HQ-EVFI和CED数据集上，ENCORE在数据集和GOP长度上实现了持续的优势。在BS-ERGB上，ENCORE实现了高达20.80%的PSNR-RGB和22.14%的MS-SSIM-RGB BD率节省，同时在其他两个数据集上保持明显的改进。

Conclusion: ENCORE通过利用事件相机提供的信息，有效提高了学习视频压缩的准确性，并在多个数据集上取得了显著的性能提升。
...
TL;DR: 本文提出了一种新的方法，从单张图像创建可动逼真头像，在性能和速度方面优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 在数字人合成中，从单张图像创建可动逼真头像仍然是一个基本挑战。

Method: 我们提出了SpiD（分割和驱动），这是一个基于两个解耦轴的单图像高斯头像框架。

Result: 广泛的实验表明，与最先进的方法相比，该框架在包含完整驱动管道的情况下，在单个GPU上实现了最快的推理速度。

Conclusion: SpiD在性能和速度方面优于现有方法。
...
TL;DR: 提出了一种新的面部重演方法，通过引入舌头动态转换，显著提高了重演的逼真度。


<details>
  <summary>Details</summary>
Motivation: 现代面部重演系统在姿态和表情转换方面取得了令人印象深刻的成果，但它们在很大程度上忽略了舌头的动态变化，导致说话和表情动作中的嘴巴内部结构不一致。

Method: 提出了一个基于基础模型辅助的引导流程，生成一个用于野外重演的专用舌头分割模型，无需精选注释。进一步引入了一个空间约束的潜在掩码扩散模型进行逼真的舌头合成，并具有自适应掩码膨胀以实现无缝的嘴巴边界过渡。

Result: 在所有舌头特定指标上，实验表明与所有基线相比，改进了超过两倍。此外，还提出了一种基于VLM的评估协议，可以以规模复制专家注释，确认所有消除变体的感知优势。

Conclusion: 本研究为面部重演中的跨身份舌头动态转换提供了一种新的框架，显著提高了重演的逼真度和准确性。
...
TL;DR: 我们提出了一种新的视频生成方法，通过集中式隐式偏好优化，解决了现有方法在处理时间稀疏伪影方面的局限性，显著提高了视频质量。


<details>
  <summary>Details</summary>
Motivation: 为了解决扩散式视频生成中偏好对齐的近期进展，特别是通过直接偏好优化（DPO），显著提高了视觉质量。然而，由于两个关键限制，现有的方法在处理运动崩溃、对象闪烁和颜色过度饱和等时间稀疏伪影方面仍然存在困难：1）偏好归因瓶颈，离线人工标注成本高昂且无法准确捕捉学习动态，而在线奖励信号虽然具有滚动感知但往往不稳定且存在偏差；2）时间信用分配错误，均匀应用的监督无法有效地针对出现伪影的短暂片段。

Method: 我们提出了集中式隐式偏好优化（cIPO），这是一种针对视频扩散模型的训练后框架。cIPO直接从去噪过程中提取隐式偏好信号：给定一个真实视频，模型添加前向噪声并通过迭代去噪将其重建，将原始视频视为首选样本，将重建视频视为不喜欢的样本。这种公式捕捉了推理时间错误，而无需人工标注或外部奖励模型。此外，原始视频和重建视频之间的帧级差异揭示了何时发生故障。cIPO利用这一点，通过计算时间重建误差，将优化集中在高错误段，从而更精确地纠正易出错的区域。

Result: 广泛的实验表明，cIPO在多个数据集上始终提高了视频的真实性和时间一致性，突出了具有时间集中优化的隐式偏好有效性和效率。

Conclusion: cIPO是一种有效的视频扩散模型后训练框架，可以显著提高视频的真实性和时间一致性。
...
TL;DR: 提出了一种统一Kendall地标形状空间的新方法，通过Sobolev型算子实现几何降维，保持了形状不变性。


<details>
  <summary>Details</summary>
Motivation: 统一Kendall的地标形状空间，解决刚性运动和尺度固定问题。

Method: 定义Sobolev型算子，如屏蔽弹性算子，实现几何降维。

Result: 创建新的地标形状空间，具有降维度量和形状不变性。

Conclusion: 提出的方法在保持形状不变性的同时，解决了地标形状空间的几何问题。
...
TL;DR: BladeYOLO：一种风轮机叶片缺陷检测新框架，性能优越，具有跨数据集鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 风轮机叶片缺陷检测在现实场景中具有挑战性，因为现场数据有限且缺陷视觉特征微妙。

Method: 提出了一种名为BladeYOLO的缺陷检测框架，该框架集成了Vision Transformer (ViT)骨干网络和Mamba-guided Weak-Defect Enhancement模块，并引入了Style-Injector模块。

Result: BladeYOLO在WTBlade-Defect数据集上取得了优异的性能，在公共Wind Surface Defect数据集上也优于最佳竞争方法。

Conclusion: BladeYOLO是一种有效的风轮机叶片缺陷检测框架，具有跨数据集鲁棒性。
...
TL;DR: mmRadarTwin是一个用于室内毫米波雷达的实用数字孪生平台，通过结合实际测量和场景模拟，提高了雷达感知的可重复性和可诊断性。


<details>
  <summary>Details</summary>
Motivation: 室内毫米波雷达感知难以再现，因为测量的距离-角度响应取决于场景几何形状、材料响应、多径效应、硬件约定和信号处理。

Method: 提出mmRadarTwin，一个信号级和路径属性数字孪生平台，用于室内毫米波雷达。该平台将实际雷达测量分支与虚幻引擎场景模拟分支通过共享接收通道和距离-角度处理接口连接。模拟器写入复杂的多通道接收网格，并导出每路径贡献记录，以识别模拟返回中的演员、材料标签、传播事件和输出框支持。

Result: 在办公室部署中使用商品单站毫米波雷达和移动场景捕获硬件评估mmRadarTwin。在涵盖22个雷达位置的154个测量姿态中，当前仅物理的路径基础模拟器在中央可用视场中回忆了70.8%的测量活动几何支持响应区域，同时揭示了由弱或多路径支持、响应偏移、不支持锚点和缺失物理机制引起的残留。mmRadarTwin建立了构建、比较和诊断室内雷达数字孪生的实用系统工作流程，而不是声称完整的雷达地图重建或跨房间泛化。

Conclusion: mmRadarTwin是一个实用的室内毫米波雷达数字孪生平台，可有效地提高雷达感知的可重复性和可诊断性。
...
TL;DR: 提出了一种无标签标准，用于联合选择UDA算法和超参数，提高了临床部署中算法选择的有效性。


<details>
  <summary>Details</summary>
Motivation: 在临床实践中，选择最佳的未标记域适应（UDA）算法及其适当的超参数通常并不明确，因为未标记的部署（目标）域阻止了直接评估。

Method: 提出了一种无标签标准，该标准联合选择算法和超参数进行UDA。给定多个算法训练的不同超参数的候选模型池，该方法针对一致性参考对每个候选模型进行评分，并选择得分最高的模型。一致性参考在两个级别上构建，不使用目标标签。首先，利用多个无标签选择信号，在每个算法中使用每个信号提名一个模型。其次，将提名的模型跨算法聚合，形成每个未标记目标样本的参考预测。然后，选择与该参考预测最一致的候选模型进行部署。

Result: 在四个脑部MRI和四个胸部X射线数据集上，在七个与临床相关的迁移场景中进行实验，结果表明，该方法的选择性能优于其他方法，并且在不同算法池中仍然有效。该方法向实际的无标签算法选择迈出了第一步，适用于临床部署的UDA。

Conclusion: 该方法为临床部署的UDA的实际无标签算法选择提供了一种新的思路，有效提高了算法选择性能。
...
TL;DR: Improved face-voice association in deep learning with a new cross-modal feature embedding method.
...
Method: Proposes a cross-modal feature embedding method for face-voice association. Utilizes convex hull for embedding same-person features. Implements cross-modal attention mechanisms with convex embedding techniques.

Result: Improved cross-modal verification, matching, and retrieval tasks on VoxCeleb dataset. Achieved notable improvements over existing state-of-the-art methods.

Conclusion: The proposed method effectively addresses the challenges of face-and-voice association learning in deep learning.
...
TL;DR: 提出了一种基于Transformer的协作特征聚合方法，有效提高了人脸超分辨率和人脸重识别的性能。


<details>
  <summary>Details</summary>
Motivation: 传统人脸超分辨率方法在恢复低质量图像时容易出现模糊和失真，图像和视频人脸超分辨率方法使用面部特征或分割也存在类似挑战。

Method: 提出了一种基于Transformer的协作特征聚合方法，通过引入多个相关面部观察，统一多序列或多视图数据中的身份特征。此外，还提出了一种级联SR网络，以逐步恢复目标面部的高分辨率图像，并统一面部特征。

Result: 实验结果表明，该方法在人脸超分辨率和人脸重识别方面均优于其他最先进的方法，证明了联合身份重建和从多个面部输入中逐步图像恢复在提高下游视觉识别任务中的有效性。

Conclusion: 该方法通过联合身份重建和逐步图像恢复，有效提高了人脸超分辨率和人脸重识别的性能。
...
TL;DR: Introduces CNS for accurate 3D shape reconstruction from multi-view images.


<details>
  <summary>Details</summary>
Motivation: Recent studies using neural radiance fields and other neural differentiable rendering methods rely on single-point geometric information, leading to a lack of detailed local geometry.

Method: Proposes convolutional neural shading (CNS) to reconstruct 3D shapes from multi-view images. Uses a neural shader to capture variations and a fine-detail displacement network to mitigate surface irregularities.

Result: Demonstrates significant quality improvements in reconstructed shapes and rendered images over current state-of-the-art methods.

Conclusion: The proposed CNS method improves 3D shape reconstruction accuracy and visual quality.
...
TL;DR: 本研究提出了一种新的视觉推理方法，通过将推理能力转移到潜在表示中，显著提高了推理效果。


<details>
  <summary>Details</summary>
Motivation: 为了提高视觉推理能力，将辅助视觉证据纳入中间推理过程。

Method: 提出OPLD（按策略的潜在蒸馏）框架，将由特权多模态CoT诱导的推理能力转移到潜在推理表示中。

Result: 在多个基准测试中，OPLD始终优于现有的潜在推理方法，并实现了最先进的性能。

Conclusion: 在推理过程层面监督潜在表示，比传统的特征级对齐为多模态潜在推理提供了一种更有效的范式。
...
TL;DR: S-Avatar：一种基于扩散引导和3DGS动画策略的3D头像生成方法，提高了3D一致性和真实性。


<details>
  <summary>Details</summary>
Motivation: 生成逼真的3D头像对虚拟现实（VR）应用至关重要，但现有方法在未知视角下难以保持3D一致性。

Method: 使用扩散引导的3D模型生成模块和3D高斯分层（3DGS）动画策略，通过三阶段流程生成单张图像的3D头像。

Result: 在公共数据集上的评估表明，S-Avatar在新型视图和表情生成方面优于现有方法，实现了更高的真实性和一致性。

Conclusion: S-Avatar在3D头像生成方面取得了显著进展，适用于广泛的VR/AR应用。
...
TL;DR: 提出了一种新的动态远田遥感图像分割方法，提高了分割性能。


<details>
  <summary>Details</summary>
Motivation: 远田遥感图像分割的现有方法假设当前图像包含足够的视觉证据进行可靠分割，但实际情况中，农田的外观随物候和空间环境变化，容易与其他地表混淆，导致局部观察不足以进行分割。

Method: 基于信息瓶颈的动态决策过程，通过获取与任务相关的额外时空信息，提出了一种名为FarmSeeker的动态远田遥感图像分割代理。

Result: FarmSeeker在全局范围内实现了比现有方法更稳定的分割性能。

Conclusion: 重新定义了远田遥感图像分割方法，提高了分割的准确性和稳定性。
...
TL;DR: VLMs的鲁棒性受训练数据和架构选择影响


<details>
  <summary>Details</summary>
Motivation: 对视觉-语言模型（VLMs）在多模态系统中的鲁棒性研究不足

Method: 对194个公开的VLMs进行大规模实证研究，包括16个模型家族，覆盖不同规模，24个训练数据集和三个评估基准

Result: 模型规模与性能的相关性在评估基准从ImageNet转移到单属性和多属性偏差时减弱；训练数据的质量与最差组准确率的相关性更一致；精心制作的集合比未精心制作的集合在可比规模上提高了25%；架构选择的影响取决于基准的性质，包括偏差的类型和图像中的空间分布

Conclusion: VLMs的鲁棒性对训练数据和架构选择敏感，需要进一步研究以提高鲁棒性
...
TL;DR: FaithEyes：通过多智能体自评估机制解决VLMs工具调用不诚实问题，提高模型可靠性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 为了解决现有视觉语言模型（VLMs）在工具调用上的不诚实问题，例如裁剪错误或遗漏查询目标，我们引入了FaithEyes框架。

Method: FaithEyes框架使用一个VLM来判断每个处理图像是否有助于回答问题，并将判断结果注入推理上下文中。同时，通过多智能体框架，模型本身作为子智能体来判断主智能体的工具调用，从而确保训练和测试的一致性。

Result: FaithEyes在视觉感知和推理基准测试中取得了具有竞争力的或更优的准确率，同时显著提高了工具的诚实度。

Conclusion: FaithEyes框架通过引入多智能体自评估机制，有效解决了VLMs在工具调用上的不诚实问题，提高了模型的可靠性和可解释性。
...
TL;DR: 该方法通过合成数据提高了机器人的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 收集具有多样性的自聚焦数据成本高昂。

Method: egocentric world-action simulator

Result: 与400个真实轨迹相比，增加400个method生成的轨迹，单臂任务的成功率从77%提高到84%，双臂任务的成功率从53%提高到70%。

Conclusion: 合成数据显著提高了下游WAM泛化能力。
...
TL;DR: Space2Ground 2.0通过整合卫星和街景影像，提高了农业地块监测的准确性和可扩展性。


<details>
  <summary>Details</summary>
Motivation: 准确且可扩展的农业地块监测具有挑战性，因为卫星地球观测只能提供地块的俯视图，而光学观测受云引起的时空间隔影响更大。

Method: 提出Space2Ground 2.0，一个多源框架，整合Sentinel-1 SAR和Sentinel-2多光谱时间序列，以及通过车载相机获取并经Mapillary平台共享的地理标记的街景影像。一个高度自动化的处理流程执行语义过滤、图像质量评估、基于视点的地块关联和数据集优化，将大量众包影像转换为关联地块、分析准备好的数据。在2022年生长季节应用于塞浦路斯，该流程生成了一个经过精选的46,050个标注的街景影像数据集，从超过900,000个影像中选出，并与8,581个农业地块的卫星信息相关联。通过使用单源和多源观测的农业地块作物分类实验来评估数据集的实用价值。

Result: 结果表明，街景影像提供了与卫星时间序列相结合时增强分类的互补精细尺度视觉信息。Space2Ground 2.0提供了一个公开可用的基准数据集和可重复的多模态农业监测方法，具有潜在的应用价值，包括视觉验证、减少对昂贵现场检查的依赖以及数据驱动的农业政策实施。

Conclusion: Space2Ground 2.0为多模态农业监测提供了一个可重复的方法，提高了农业地块监测的准确性和可扩展性。
...
TL;DR: TARS是一种基于文本驱动的3D无视频重摄方法，通过自监督训练实现摄像机动态和基本视觉表示的学习，提供更准确和时序一致的视频控制。


<details>
  <summary>Details</summary>
Motivation: 视频重摄旨在生成具有可控摄像机运动和视角的视频。现有方法依赖于显式的3D先验，这限制了重建质量，并且在合成先前未见过的区域时往往表现不佳，或者依赖于具有不同摄像机轨迹的配对视频，其稀缺性阻碍了一般化。我们通过文本驱动的语义视角规范重新审视视频重摄，使拍摄范围、观看角度和第一/第三人称视角可控。

Method: 提出了一种名为TARS的3D无视频重摄范式。通过时间步长敏感性分析，发现摄像机运动主要在高噪声阶段建立，此时形成粗略的时空结构。基于这一洞察，我们引入了自监督训练，以在没有配对重摄数据或3D重建的情况下学习摄像机动态和基本视觉表示。通过数据缩放和联合文本-摄像机条件，TARS支持鲁棒的摄像机和视角控制，在大型摄像机运动下合理地合成超出源视图的区域，同时实现反向角度重摄和视角切换。

Result: TARS比先前方法提供了更准确和时序一致的视频控制。

Conclusion: TARS是一种有效的视频重摄方法，能够实现可控的摄像机运动和视角，并在合成先前未见过的区域时表现良好。
...
TL;DR: 本研究提出了一种构建和验证灾难响应跨模态数据集的新方法，为知识蒸馏提供了可靠的语义锚定。


<details>
  <summary>Details</summary>
Motivation: 在灾难管理等领域部署视觉-语言模型（VLMs）需要高质量的跨模态数据集，特别是在通过数据无关的知识蒸馏（DFKD）进行知识迁移时。然而，现有数据集要么完全缺乏描述性文本，如Incidents1M，要么遭受严重的文本-图像语义错配，如CrisisMMD。

Method: 提出了一种新的方法来构建和自动验证用于灾难响应的大型跨模态数据集。从仅视觉的Incidents1M开始，成功地恢复了100,000张图像，并使用两个不同的Qwen3.5架构生成了高保真度的文本描述：一个4B密集模型和一个35B混合专家（MoE）模型。为了确保生成的字幕为DFKD提供可靠的语义锚定，引入了一个图像盲LLM作为评判的验证流程，利用Qwen3.5-9B。通过故意隐藏原始图像给评判者，这个评估者准确地模拟了学生模型在数据无关蒸馏期间的模式差距。在173,179个标签对上的评估表明，两个架构之间的高语义一致性（78.65/100）。此外，自动评估揭示了保守的字幕行为，特征是高精确率（77.6%）和低召回率（46.0%）。这最小化了假阳性噪声，同时暴露了原始真实标签中的人为标注不一致性。

Result: 提供了一种可扩展的、LLM验证的跨模态数据集和可复制的框架，以推进跨模态知识蒸馏。

Conclusion: 本研究提出的方法为灾难响应领域提供了一个高质量的跨模态数据集，并有效地解决了现有数据集的缺陷，为知识蒸馏提供了可靠的语义锚定。
...
TL;DR: MSCM-net：一种高效的高光谱图像分类模型


<details>
  <summary>Details</summary>
Motivation: 提高高光谱图像分类的性能和降低计算复杂度

Method: 提出了一种名为MSCM-net的创新性高光谱图像分类模型，结合了多尺度CNN和Mamba，并引入了MCSE模块和双分支特征聚合模块

Result: 在三个广泛使用的基准数据集上进行了实验，结果表明MSCM-net在降低计算复杂度的同时，实现了先进的分类性能

Conclusion: MSCM-net是一种有效的高光谱图像分类模型，在保持高性能的同时降低了计算复杂度
...
TL;DR: 提出CapDepth，通过详细的长字幕提高MDE鲁棒性，在非朗伯表面和恶劣天气条件下显著降低深度误差。


<details>
  <summary>Details</summary>
Motivation: 针对单目深度估计（MDE）在非朗伯表面和恶劣天气条件下的挑战，以及现有方法在鲁棒性方面的局限性，提出了一种新的框架。

Method: 设计了一个详细的长期字幕输入模板，引入了动态字幕编码器，并提出了一个文本自适应解码器。

Result: 在非朗伯表面和恶劣天气条件下，深度误差分别降低了25.0%和22.0%。

Conclusion: CapDepth框架有效地提高了MDE的鲁棒性，优于现有方法。
...
TL;DR: 本文提出了一种使用深度传感器进行 affordance segmentation 的新方法，该方法在真实世界数据集上表现出色，且与现有解决方案相比具有优势。


<details>
  <summary>Details</summary>
Motivation: 为了补充可穿戴机器人中的 affordance segmentation，深度传感器具有潜力，但其使用似乎尚未得到充分探索。

Method: 提出两种方法：一种改进的硬件感知神经架构搜索版本，配备一个新设计的搜索空间，以将深度（D）信息集成到小型深度网络中，以及一种专门的微调方法，包括一个预处理层，以合并深度信息与 RGB 数据并使其与常规架构兼容。

Result: 在真实世界数据集上进行了广泛的实验，证明了所提出方法的有效性，与现有解决方案相比。该方法通常生成在大多数情况下识别帕累托最优前沿的解决方案，以平衡泛化性能和硬件要求。

Conclusion: 本文提出的方法在大多数情况下能够生成识别帕累托最优前沿的解决方案，以平衡泛化性能和硬件要求，并描述了一个支持的原型，包括 Jetson Nano 板和 RealSense RGB-D 摄像头。
...
TL;DR: 提出了一种新的3D场景理解方法，通过解耦几何重建和语义集成，提高了效率和可扩展性。


<details>
  <summary>Details</summary>
Motivation: 为了实现下一代交互式系统，需要开放词汇的3D场景理解，使用户能够通过自然语言直观地查询和导航重建的环境。然而，当前的3D高斯框架通常受到限制的多视图捕获要求、昂贵的场景特定优化以及存储密集语言特征的巨大内存开销的瓶颈。

Method: 提出了一种新的、无需训练的流水线，通过明确地将3D几何重建与语义集成解耦，从根本上重新构想了这个范式。给定了标准的单目视频序列作为输入，该方法有效地输出了一个紧凑的、高度可解释的、完全可搜索的对象级语义高斯图。而不是在映射循环中纠缠重语言嵌入，我们独立提取几何形状，并通过轻量级、模块化的后处理框架来定位语义。

Result: 在Replica数据集上的大量评估表明，这种解耦架构在渲染保真度和分割精度方面表现良好。关键的是，通过用模块化、对象级的语义嵌入替换密集的每个高斯存储，与最先进的方法相比，我们的方法在内存使用上减少了数个数量级。这为开放词汇的3D检索和从日常单目视频中直接进行问答提供了一种高度高效、可扩展和实用的解决方案。

Conclusion: 该方法提供了一种高度高效、可扩展和实用的解决方案，能够有效地减少内存使用，并提高3D场景理解的性能。
...
TL;DR: VAD is an effective algorithm for visual knowledge transfer, outperforming existing methods and demonstrating the effectiveness of counterfactual target reconstruction.
...
TL;DR: ObjectStream通过将潜在对象作为记忆锚点，提高了流媒体视频理解的效果和效率。


<details>
  <summary>Details</summary>
Motivation: 流媒体视频理解需要模型在未知未来问题之前持续保留有用的视觉证据。现有方法主要根据标记重要性、时间冗余或段级相关性来管理不断增长的视觉上下文，但很少围绕在时间上持续和发展的对象组织证据。

Method: 提出ObjectStream，一个无需训练的框架，将潜在对象视为流媒体视频理解的记忆锚点。ObjectStream直接从冻结的Video-LLM表示中诱导空间上连贯的潜在对象，将它们链接到帧中的持久锚点，并在有限的内存预算下维护它们的历史记录，无需外部对象检测器或分割模型。基于这些锚点，ObjectStream保留三种互补的证据形式：持久对象历史、短暂对象变化和最近视觉上下文。

Result: 在在线流媒体评估中，ObjectStream将Qwen2.5-VL-7B在OVO-Bench实时视觉感知上提高了10.0分，同时将峰值GPU内存和TTFT减少了约50%。在离线长视频基准测试中，它超过了全标记基线，同时丢弃了82.5%的视觉标记。

Conclusion: 这些结果表明，潜在对象是紧凑流媒体视频记忆的一个实用且有效的组织原则。
...
TL;DR: AdaAnchor4D：一种用于单目无人机动态场景重建的有效方法。


<details>
  <summary>Details</summary>
Motivation: 由于单目无人机视频在动态重建复杂城市场景中提供宝贵观察，但此类场景表现出明显的时空异质性，因此需要更有效的方法来处理这种异质性。

Method: 提出了一种名为AdaAnchor4D的适应性地标变形框架，用于单目无人机动态场景重建。该框架包括：Anchor-Conditioned Feature Aggregation (ACFA)、Decoupled Local Geometry Deformation (DLGD)和Density-Adaptive Coordinate Warping (DACW)。

Result: 在UAV-Arc4D、VisDrone和UAVDT数据集上的实验表明，AdaAnchor4D在保持实时渲染性能的同时，实现了比代表性的动态高斯方法更高的渲染质量。

Conclusion: AdaAnchor4D是一种有效的单目无人机动态场景重建方法，能够处理复杂城市场景中的时空异质性。
...
TL;DR: 提出了一种名为“趋势感知剪枝”的新框架，以解决MLLMs中视觉标记剪枝的局限性，显著提高了效率。


<details>
  <summary>Details</summary>
Motivation: 为了解决现有训练免费方法在视觉标记剪枝方面的局限性，该方法忽略了MLLMs的层次结构，其中标记的重要性往往动态变化。

Method: 提出了一种名为“趋势感知剪枝”的新框架，该框架将剪枝从局部快照决策提升为时间轨迹建模问题。

Result: 该方法在多种多模态任务中实现了卓越的效率-性能权衡，显著减少了视觉标记数量，同时保持了有竞争力的性能。

Conclusion: 趋势感知剪枝是一种高效且可逆的多模态推理解决方案，在保持性能的同时，显著提高了效率。
...
TL;DR: ShadowDancer通过影子对和跨影子预测创新，显著提升了动作迁移能力。


<details>
  <summary>Details</summary>
Motivation: 针对现有交互式视频世界模型接口的不足，提出ShadowDancer方法。

Method: ShadowDancer采用影子对和跨影子预测两个关键创新。影子对是独立重采样外观下重复相同动态的视频对，跨影子预测通过预测一个影子来学习动作。

Result: 实验表明，ShadowDancer在动作迁移和长时间动作扩展方面优于基于潜在动作和交互式世界模型的基线，平均盲测胜率为86%。

Conclusion: ShadowDancer能够提高动作迁移能力，并在不同动态家族中展现出优越性能。
...
TL;DR: 提出了一种基于CAVs和SAEs的图像相似性解释框架，通过扰动嵌入空间中的概念方向来解释图像相似性，并取得了良好的效果。


<details>
  <summary>Details</summary>
Motivation: 解释图像相似性，特别是为什么两个图像会得到高或低的相似度评分。

Method: 使用概念激活向量（CAVs）和稀疏自编码器（SAEs）自动提取，通过扰动嵌入空间中的概念方向来解释图像相似性。

Result: 提供概念重要性，对图像对进行定位，并扩展到组级别设置，引入示例检索。实验表明，方法比像素空间基线更忠实于数据分布，并且概念重要性可以线性恢复真实相似度评分。

Conclusion: 提出了一种新的框架，可以有效地解释图像相似性，为理解模型相似性判断提供了新的视角。
...
TL;DR: 本文对HOI的基础模型先验进行了系统回顾，建立了分类法，并分析了不同先验在HOI管道和任务中的表示、注入和适应方式。


<details>
  <summary>Details</summary>
Motivation: 手-物体交互(HOI)建模仍然具有挑战性，因为它需要在视觉不确定性严重的情况下，对手部关节、物体几何形状、接触、语义和动力学进行联合推理。基础模型通过从大规模跨领域数据中学习可转移的先验知识，为解决这些挑战提供了新的方法。然而，快速增长的文献仍然分散，现有研究通常简单地将这些方法描述为“使用大型模型”，而没有系统地描述引入了哪些知识，这些知识在HOI管道中的哪个环节，以及它们有助于减少哪些HOI不确定性。

Method: 本文对HOI的基础模型先验进行了系统回顾，将文献组织为涵盖重建和生成的六个HOI任务。更重要的是，我们建立了一个包含八个基础模型子先验的分类法，分为几何、语义和视觉家族。几何先验包括形状检索、形状重建和空间重建；语义先验包括语义接地和语言推理；视觉先验包括视觉表示、图像生成和视频生成。基于这个分类法，我们系统地分析了不同先验在HOI管道和任务中的表示、注入和适应方式。除了基础模型如何增强HOI之外，我们还进一步考察了从HOI中衍生出的知识在机器人学习中的应用，包括人类数据预训练、人机技能转移和HOI到机器人数据生成。最后，我们总结了数据集和评估协议，并讨论了更通用的HOI系统局限性和未来方向。为了支持长期进步，我们创建了一个实时存储库，持续收集新兴方法和基准。

Result: 本文对HOI的基础模型先验进行了系统回顾，建立了分类法，并分析了不同先验在HOI管道和任务中的表示、注入和适应方式。此外，还考察了从HOI中衍生出的知识在机器人学习中的应用，并总结了数据集和评估协议。

Conclusion: 本文对HOI的基础模型先验进行了系统回顾，为更通用的HOI系统提供了理论基础和方法指导。
...
TL;DR: 本研究提出了一种基于多模态卫星数据集的洪水监测和损害评估框架，结果表明该框架在洪水监测方面具有潜力。


<details>
  <summary>Details</summary>
Motivation: 有效洪水监测对于减少洪水灾害对人口和基础设施的影响至关重要。然而，由于大多数分割算法缺乏大规模应用所需的一般化能力，以及标注洪水数据稀缺且分布不均，在广阔且环境多样化的地区进行可靠的遥感监测仍然具有挑战性。

Method: 本研究提出了一种基于合成孔径雷达数据、多光谱影像和数字高程模型及其导数的端到端多模态框架，用于俄罗斯联邦领土的可持续洪水监测和损害评估。使用涵盖七个俄罗斯地区的自收集多模态数据集，比较了两种在数据有限条件下的水面检测策略：监督U-Net++模型和自监督AnySat架构。

Result: 在研究的数据条件下，监督学习证明更有效，而基于AnySat的方法提供了更大的稳定性，并保留了在预期有更多未标记数据或推断时缺失模态的潜在优势。使用最佳洪水区域预测来估计城市地区的洪水影响，包括受影响区域、物质损害、伤亡和生态农业影响。根据俄罗斯紧急情况部的官方方法进行估计。应用于2019年Tulun洪水，获得的结果与官方评估非常吻合，除了物质损害，这是由于使用了开源数据库。结果表明，深度学习和多模态卫星数据集成在可扩展、可靠的洪水监测方面具有潜力，适用于多样化的环境和数据有限条件。

Conclusion: 深度学习和多模态卫星数据集成在可扩展、可靠的洪水监测方面具有潜力，适用于多样化的环境和数据有限条件。
...
TL;DR: READII-2-ROQC是一种新的工具，用于评估放射组学和深度成像特征是否能够准确捕捉肿瘤图像的生物学信息。


<details>
  <summary>Details</summary>
Motivation: 放射组学和成像基础模型承诺为肿瘤生物学提供非侵入性生物标志物，但预测特征可能反映了肿瘤体积或采集伪影，而不是有意义的图像结构。

Method: READII-2-ROQC使用体积保留的负对照来评估放射组学和深度成像特征是否捕获独立的空間信号。

Result: 在三个公开的癌症成像队列中，该框架处理了3,552个肿瘤体积，并从原始图像和九个匹配的控制图像中提取了PyRadiomics和基础模型特征。

Conclusion: READII-2-ROQC为开发可解释的、基于生物学的成像生物标志物和可重复的放射组学工作流程提供了可扩展的质量控制策略。
...
TL;DR: ViewMind3D是一种无需训练的3D问答框架，在现实世界环境中实现了有效的3D推理。


<details>
  <summary>Details</summary>
Motivation: 为了提高3D问答（3D-QA）的性能，特别是在没有3D特定训练或昂贵标注的情况下。

Method: 提出了一种名为ViewMind3D的完全训练自由和模块化框架，该框架不需要完整的3D重建，对场景的多视角观察进行3D空间推理。

Result: 在ScanQA和SQA3D数据集上取得了与先前训练自由和微调的3D-LLMs相媲美的性能。

Conclusion: 通过模块化编排通用LLMs和VLMs，可以有效地进行3D推理，以实现机器人感知在现实世界环境中的3D空间推理。
...
TL;DR: VisualRouter是一种新的视觉采样框架，显著提高了长视频理解的效果。
...
Method: 提出VisualRouter，一个训练免费且即插即用的查询地面视觉采样框架。

Result: 在Video-MME、LongVideoBench和MLVU上分别提高了5.2%、7.7%和11.6%，优于现有的无训练视觉采样方法。
...
TL;DR: 我们提出了一种基于强化学习的VLMs训练方法，用于检测和定位AI篡改的图像，该方法在多个数据集上取得了与最先进的检测器相当的性能。


<details>
  <summary>Details</summary>
Motivation: 为了提高可信AI，检测和定位AI篡改的图像至关重要，但现代生成模型使得此类篡改越来越难以识别。

Method: 我们提出了一种基于GRPO的培训框架，该框架利用简单的准确性和格式奖励来训练VLMs通过强化学习（RL）进行推理，而不是通过显式的推理监督。

Result: 我们的方法在多个图像篡改数据集上实现了与最先进的图像伪造检测器相当的性能，尽管需要实质上更少的监督。

Conclusion: 这些结果表明，强化学习为教授VLMs推理AI生成内容提供了一种有效且可扩展的机制。
...
TL;DR: 本研究提出了一种高效且加速的实时异常分割方法，适用于嵌入式和桌面平台，可显著提高自动驾驶和铁路系统的性能。


<details>
  <summary>Details</summary>
Motivation: 为了确保自主系统的安全，实时异常分割至关重要。

Method: 本研究提出了一种针对嵌入式和桌面平台的优化流程，通过重写PixOOD的Neyman-Pearson评分阶段，并使用TensorRT进行硬件优化编译。

Result: 在桌面NVIDIA RTX 4060 GPU上达到182 FPS，在NVIDIA Jetson AGX Orin嵌入式平台上达到75 FPS，比原始基线快20倍和18倍。

Conclusion: 该方法证明了高级异常分割可以高效地应用于自动驾驶和铁路应用中的车载处理。
...
TL;DR: 提出了一种基于世界模型的耳部结构CT分割框架，有效提高了分割精度。


<details>
  <summary>Details</summary>
Motivation: 耳部结构的CT细粒度分割具有挑战性，因为耳朵在图像区域中占据很小，软骨边界不规则，软骨与周围软组织的界面通常模糊。临床标注可能包括包含软骨和相邻皮肤的复合结构及其对应的仅含软骨的区域，产生嵌套和重叠的标签。

Method: 提出了一种基于世界模型的分割框架，该框架引入了一个确定性循环状态空间模型到中间潜在空间。多尺度编码器特征和部分解码表示融合形成结构观察，初始化潜在动态。在推理过程中，模型执行三步潜在滚动，无需真实标签指导。层次解剖动作更新循环状态，逐步细化潜在表示。将得到的潜在轨迹投影回解码器，并结合高分辨率特征生成最终分割。为了学习可靠的潜在转换，引入了平衡的层次动作目标，解决了前景稀疏、缺失解剖组以及添加和删除操作之间的不平衡。

Result: 实验表明，该框架在CT中小型、不规则、重叠的耳部结构分割中，始终提高了分割精度，并将HD95降低了超过43%。这些结果表明，潜在世界模型推理对于具有挑战性的医学图像分割是有效的。

Conclusion: 基于世界模型的分割框架在耳部结构的CT细粒度分割中表现出色，提高了分割精度，并证明了潜在世界模型推理的有效性。
...
TL;DR: RefCaptioner：一种多参考图像 grounding 视频字幕新方法，显著提升字幕质量和忠实度。
...
Method: 提出多参考图像 grounding 视频字幕任务，并设计 RefCaptioner 框架，结合混合数据 SFT 和分层覆盖折扣 GRPO 提高参考选择、短语级绑定、干扰拒绝和交叉参考一致性。

Result: RefCaptioner 在开源模型中表现最佳，且在标准视频字幕基准测试中保持竞争力。人工评估表明，其字幕更受标注者青睐，并支持更忠实于源视频的重建。

Conclusion: RefCaptioner 是一种有效的多参考图像 grounding 视频字幕方法，在保持视频字幕能力的同时，提高了字幕的忠实度和一致性。
...
TL;DR: GenEvA通过组织跨帧证据实现长视频理解，显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 长视频理解通常将视频压缩成一小部分帧或视觉标记以生成答案。现有紧凑型管道侧重于保留相关视觉内容作为显式证据。然而，使证据可用并不能确保整合跨越时刻的互补线索来回答问题。

Method: 提出将选定的帧组织成查询相关的跨帧证据，并在生成之前进行。将此选择阶段定义为潜在证据界面，并使用GenEvA（生成潜在证据聚合）实现。GenEvA使用查询条件的证据分布来关注相关帧，并从它们的帧特定信息中形成紧凑的跨帧潜在证据。如果不需要跨帧集成，则相同的分布确定是否插入此潜在补充。

Result: 在四个基准和两个Video-MLLM主干上，GenEvA一致地提高了匹配帧基线。在8帧时，它将四个基准LLaVA-Video平均提高了+5.2分，Qwen2.5-VL准确率在LVBench上提高了+10.1分。这些收益仅需要0.11%--0.40%的平均视频标记开销；分析进一步表明，自适应证据调用具有任务感知分配和收益。

Conclusion: GenEvA在长视频理解任务中有效地提高了跨帧证据的整合，实现了较高的性能提升，同时保持了较低的开销。
...
TL;DR: DAR-Net：一种用于图像修复的模糊校正网络，在多种退化场景下取得了优异的性能。


<details>
  <summary>Details</summary>
Motivation: 针对现有图像修复方法中存在的语义模糊和空间模糊问题，提出了一种新的图像修复网络DAR-Net。

Method: DAR-Net网络包含退化原型表示（DAR）模块、语义模糊校正（SeAR）模块和空间模糊校正（SpAR）模块。DAR模块通过原型混合建模构建结构化退化状态，SeAR模块生成退化感知提示以改善解码器中的通道条件，SpAR模块正则化退化感知和互补特征，减少去除和保留线索之间的空间干扰。

Result: 在标准图像修复基准测试中，DAR-Net在三种和五种退化设置下均取得了最佳整体性能，平均PSNR分别比最强竞争对手提高了0.14 dB和0.34 dB；在CDD-11和WeatherBench上也表现出优异的性能。

Conclusion: DAR-Net能够有效解决图像修复中的语义模糊和空间模糊问题，在多种退化场景下均取得了优异的性能。
...
TL;DR: 提出了一种新的化学结构图像到文本的翻译方法，提高了Markush结构翻译的准确性。


<details>
  <summary>Details</summary>
Motivation: 将化学结构图像转换为线符号以提高数据库索引和机器学习模型训练集构建的可用性。

Method: 将分子图像和Markush结构视为图像到文本的翻译问题，提出OCSRGlyph和MarkushGlyph模型。

Result: OCSRGlyph模型在考虑立体化学方面改进了性能，MarkushGlyph模型读取整个Markush结构图像，并提出新的度量标准来提高Markush结构翻译的准确性。

Conclusion: 提出了一种新的方法来处理化学结构图像的文本转换，提高了Markush结构翻译的准确性。
...
TL;DR: ScaFE通过将LLM知识转化为特征程序，实现了高效的病理疤痕分类。


<details>
  <summary>Details</summary>
Motivation: 对病理疤痕进行分类需要区分疤痕疙瘩和肥厚性疤痕，尽管专家标注数据有限，并且医院之间的数据采集存在很大差异。

Method: 引入ScaFE（疤痕特征工程），将大型语言模型（LLM）的临床知识转移到确定性、可执行的特征程序中，而不是让模型直接诊断图像。使用可联网的LLM检索临床证据并合成测量可视觉评估疤痕属性的程序。候选程序在受限的本地环境中执行，仅返回汇总验证统计信息和特征级SHAP摘要，以进行迭代修复和改进；原始图像和患者级输出保持本地。然后使用轻量级随机森林对结果的结构化表示进行操作。

Result: 在来自三个医院的600张照片上，ScaFE实现了81.0%的站点宏观平衡准确率，比最强的基线BiomedCLIP高出10.0个百分点。仅使用10%的开发数据，ScaFE就保持了72.0%的平衡准确率，领先11.8个百分点。迭代改进还将可执行程序的比率从66.7%提高到95.0%，其中91.7%的最终特征有验证证据。

Conclusion: LLM知识可以通过本地和可审计的特征程序支持数据高效的跨站点医学图像分类，而不是直接进行VLM决策。
...
TL;DR: MIND通过意图驱动的网络和创新的损失函数，实现了高质量的医学图像融合，并提高了脑肿瘤分割的准确性。


<details>
  <summary>Details</summary>
Motivation: 现有医学图像融合方法通常应用全局统一的融合规则，缺乏对诊断意图和病理结构的深入理解。

Method: 提出MIND，一种通过扩散变换器（DiTs）的多模态意图驱动网络进行医学图像融合。使用BioMedGPT从源图像生成意图驱动的融合文本，并利用病理意识诊断意图指导融合过程。设计多尺度潜在适配器来应对DiTs中一维序列展平导致的二维空间连续性损失。设计医学语义一致性损失来解决图像输出与诊断意图解耦引起的语义偏移。

Result: 在Harvard、BraTS和GFP数据集上的综合实验表明，MIND实现了优异的融合质量，显著提高了下游脑肿瘤分割精度，并实现了灵活的交互式融合，为意图驱动的智能临床决策支持系统提供了巨大潜力。

Conclusion: MIND在医学图像融合方面表现出色，为临床诊断提供了新的思路和方法。
...
TL;DR: 本文提出了一种高效的融合模块构建方法，通过注意力、状态空间模型和学习的压缩技术，显著提高了搜索效率和图像质量。


<details>
  <summary>Details</summary>
Motivation: 为了回答像“找到出现新建筑的图像对”这样的查询，需要对档案中的前后（双时间）卫星图像对进行搜索并按匹配自然语言描述的程度对每对进行排名。

Method: 使用一个固定的图像编码器（冻结的CLIP模型）和一个针对所有变体的训练方案，对来自三个家族的八个设计进行评估：注意力、状态空间模型（Mamba）和学习的压缩（我们的时间瓶颈融合，TBF）。每个设计在两个基准（LEVIR-CC和Dubai-CC）上进行了十次随机种子测试，所以报告的差异是统计上有根据的。

Result: 结果表明，无训练的二级搜索（一个便宜的差异模型，先筛选候选人，然后是注意力融合，重新排名）在LEVIR-CC上的召回率与全融合相当，同时将查询成本降低了10-15倍，Dubai-CC上的R@1/R@5相当；Mamba的线性时间扫描（在纸面上吸引人）在典型的视觉Transformer块数（L=196）上没有速度优势：扫描受限于内存带宽，而注意力图可以干净地映射到并行硬件上；压缩融合表示（TBF）减少了2.3倍的参数和1.6倍的延迟，对于一个仅更改的BLEU-1成本为0.007，尽管更激进的压缩默默地丢弃了聚合指标无法揭示的改变相关细节。

Conclusion: 本文提出了一种高效的融合模块构建方法，通过注意力、状态空间模型和学习的压缩技术，显著提高了搜索效率和图像质量。
...
TL;DR: 我们提出了一种名为ROAD的框架，通过转移判别先验到扩散变换器中，有效地降低了高保真3D生成的计算成本和提高了生成性能。


<details>
  <summary>Details</summary>
Motivation: 高保真3D生成主要依赖于扩展模型容量和数据，这导致了高昂的计算成本。这种范式通常需要从头开始学习几何形状，而忽略了已经封装在判别性3D基础模型中的丰富语义和结构先验。

Method: 我们提出了一种名为ROAD的框架，通过将这些丰富的判别先验转移到扩散变换器中，以降低3D生成的训练成本。为了解决生成和判别潜在之间的固有语义-结构异质性，我们引入了一种相互目标对齐策略。这种方法结合了整体语义浓缩和结构最优对齐，将结构最优对齐定义为二分图匹配问题，以严格对齐不同潜在空间之间的微观几何细节。3D基础模型仅用于训练时对齐的监督，在推理时不使用，不产生额外的推理成本。

Result: 与工业基线Step1X-3D相比，所提出的ROAD在仅使用1.5%的训练数据的情况下，实现了具有高度竞争力的生成性能，并显著降低了训练成本，有效地减少了高保真3D生成的计算开销。

Conclusion: 所提出的ROAD框架有效地降低了高保真3D生成的计算成本，提高了生成性能，为3D生成领域提供了新的思路。
...
TL;DR: MixFrag是一种基于脆弱性引导的混合精度PTQ框架，在视觉Transformer上实现了具有竞争力的性能。


<details>
  <summary>Details</summary>
Motivation: 为了在资源受限的设备上部署视觉Transformer（ViT），后训练量化（PTQ）已成为一种有效的解决方案。然而，现有的PTQ方法通常在Transformer组件中使用统一的位宽，忽略了它们对量化的异质敏感性，导致精度分配效率低下。

Method: MixFrag提出了一种基于脆弱性引导的混合精度PTQ框架。首先，通过使用小校准集测量全精度和隔离量化输出分布之间的Kullback-Leibler（KL）散度来估计组件级别的量化脆弱性。然后，将位分配公式化为多个选择背包问题（MCKP），在目标位预算下实现自适应层级精度分配。

Result: 在ImageNet-1K上的大量实验表明，MixFrag在实用的混合精度设置下实现了具有竞争力的分类性能。在COCO目标检测和实例分割上的评估表明，MixFrag在现有的混合精度PTQ方法中取得了最先进的性能，在具有挑战性的MP3/MP3设置下，将之前最佳方法提高了高达9.6 AP。额外的分析验证了所提出的脆弱性指标，并证明了其与学习到的位分配之间的强相关性。

Conclusion: MixFrag被确立为视觉Transformer混合精度后训练量化的有效框架。
...
TL;DR: 提出了一种新的智能视觉推理模型Beacon，它在复杂任务上提高了多模态大型语言模型（MLLMs）的成功率。


<details>
  <summary>Details</summary>
Motivation: 提高多模态大型语言模型（MLLMs）在复杂任务上的成功率，而不仅仅是装备它们一个复杂但低效的推理范式。

Method: 通过工具使用的两个关键维度：模式适应性（MA）和工具效果（TE）来重新思考智能视觉推理。

Result: 提出了Beacon模型，该模型具有更强的整体性能、改进的模式适应性和真实的工具诱导性能提升。

Conclusion: Beacon模型在多个基准测试中表现出强大的整体性能，并在模式适应性和工具效果方面取得了实质性改进。
...
TL;DR: Chimera is a hybrid visual diffusion backbone that efficiently handles long-context visual generation tasks.


<details>
  <summary>Details</summary>
Motivation: Visual generation requires high-resolution images, long videos, and multimodal context, making the quadratic cost of full attention prohibitive.

Method: Introduce Chimera, a hybrid visual diffusion backbone with a principled scaling recipe. It processes text, image, and video tokens in one raster-ordered stream without positional embeddings. Combines Kimi Delta Attention (KDA) for long-context state tracking, interleaved Multi-head Latent Attention (MLA) for direct global interaction, and modality-aware short convolutions for local spatiotemporal context. Uses Sparse Mixture-of-Experts (MoE) layers and HeteroP module-wise scheme for scaling.

Result: 11B-parameter Chimera with 2B activated parameters, dense backbone 1.7x compute-efficient than full-attention baseline, extrapolates zero-shot from 5-second training clips to 30-second videos with 6.5% FID degradation, compute-optimal image pretraining divides compute evenly between activated model size and training-token count.

Conclusion: Establishes a foundation for designing and scaling efficient long-context diffusion architectures.
...
TL;DR: PhiZero：一种基于物理语言的物理世界模型，通过推理和渲染预测世界演变。


<details>
  <summary>Details</summary>
Motivation: 受人类从视觉经验中抽象预测结构并使用自然语言进行推理的能力的启发，PhiZero旨在从实际视频中学习物理语言，并使用它来显式地推理物理世界的演变。

Method: 通过自监督从实际视频中学习物理语言，采用推理-渲染范式，首先将世界演变推断为物理语言序列，然后将推断的转换渲染成视频。

Result: 广泛的实验验证了PhiZero在建模物理上连贯的世界演变方面的能力，并展示了其在真实和交互式世界建模、细粒度动作条件模拟和零样本运动迁移方面的潜力。

Conclusion: PhiZero是一种有效的物理世界模型，能够从视频中学习物理语言并用于预测世界演变，具有广泛的应用前景。
...
TL;DR: ReToken通过高效选择视觉标记，显著提升了视觉-语言模型在长视觉上下文中的性能。


<details>
  <summary>Details</summary>
Motivation: 视觉-语言模型在长视觉上下文中表现不佳，处理大量干扰项和所有标记的计算成本过高。

Method: 提出ReToken，一个可学习的嵌入，作为显式的检索目标，从预填充的视觉键值缓存中选择稀疏的查询相关视觉标记。

Result: 在视觉Haystacks上，ReToken使Qwen3VL-8B提高了13.4个点，使InternVL3.5提高了12.4个点；在LVBench上，ReToken实现了对长视频的无监督迁移，使Qwen3VL-8B提高了8.0个点。

Conclusion: ReToken通过其轻量级设计，在单个H100上完成了训练和长视频推理。
...
TL;DR: 研究提出了一种多阶段提示链方法，用于提高学术摘要生成的可靠性和质量，实验结果表明该方法优于单次提示方法。


<details>
  <summary>Details</summary>
Motivation: 学术出版物数量指数级增长，需要自动化工具进行有效的信息综合。然而，简单的单次提示方法往往缺乏复杂综合任务所需的可信度和质量。

Method: 引入并实证评估多阶段提示链方法，该方法作为一种更可靠的架构模式用于此类任务。该方法在我们的系统AI SciBrief中得到实现，该系统能够自动生成学术摘要。我们进行了一项比较实验，衡量我们的提示链方法与一个精心优化的单次基线方法的表现。两个系统都与人类编写的“黄金标准”报告进行了评估，领域为“教育”。

Result: 结果表明，提示链方法在可靠性和质量方面都有显著优势：我们的提示链方法实现了100%的成功率，而优化的基线在50%的运行中失败了。在质量方面，提出的方法也显示出明显的优势，实现了更高的ROUGE-L F1-score（0.507 vs. 0.486），主要由更高的精确度驱动。

Conclusion: 提示链是一种更可靠的工程方法，适用于复杂的多步骤生成任务，显著降低了单一提示固有的失败和不一致的风险。
...
TL;DR: 生成式AI辅助预审在BOSC 2026中表现出一定效果，但需人工审核。


<details>
  <summary>Details</summary>
Motivation: 为了减少人工审稿的负担和提高效率，探索使用生成式AI进行论文摘要预审的可行性。

Method: 构建了名为bosc-pre-review的AI技能，用于评估六项评审标准，并使用Runabilly在Docker容器中构建和测试每个项目。

Result: 评审员普遍认为预审有用，但倾向于将AI的结论与自己的判断进行比较。

Conclusion: 生成式AI在论文摘要预审中具有一定的辅助作用，但仍需人工判断。
...
TL;DR: 研究评估了LLMs在理解新闻框架方面的能力，发现AI和人类评估之间存在差异，但主要模型与人类判断基本一致。


<details>
  <summary>Details</summary>
Motivation: 评估大型语言模型（LLMs）在理解新闻框架方面的能力，特别是情感细微差别。

Method: 通过YouGov调查收集了3011名英国成年人的数据，并与七个LLMs进行了比较，以判断新闻标题是否唤起了对冲突中一方的同情。

Result: AI和人类评估之间的相关性因模型而异，最高为0.789（GPT-5.2），最低为0.4（Mistral Large 2512）。主要模型在所有人口子群体中与人类判断基本一致，尽管存在统计上显著的不同。

Conclusion: 这项研究强调了LLMs在理解新闻框架方面的能力，并指出即使整体性能很高，AI的匹配也不是普遍的，可能因人口特征和文化规范而有所不同。
...
TL;DR: BridgeAlign：一种针对广泛HSS学科的偏好对齐方法，显著提高了LLMs在HSS领域的性能。


<details>
  <summary>Details</summary>
Motivation: 针对开放性人文学科和社会科学（HSS）领域，现有数据合成方法主要针对有可验证答案的领域，忽略了细微的质量判断，因此偏好对齐成为自然范式。然而，现有方法要么成本高昂，要么不适合广泛的HSS学科。

Method: BridgeAlign，一个针对广泛HSS学科的偏好对齐管道，包括三个阶段：i) 种子编纂：通过启发式/LLM过滤和文本细化从网络语料库中编纂HSS种子文档；ii) 偏好数据合成：通过基于角色的指令反转生成偏好三元组，并通过问答一致性检查；iii) 偏好优化：通过在HSS质量评价标准中确立偏好，然后通过控制质量退化生成过渡性响应，形成接近边界的偏好对，以进行更精细的质量区分。

Result: BridgeAlign在超过210k个合成偏好样本上进行了对齐，使Qwen3-8B在17个基准测试中平均表现优于11个强大基线；重要的是，在人类偏好和基于知识的能力方面同时领先，两者之间没有权衡，如大量实验和现有理论所支持。

Conclusion: BridgeAlign是一种有效的偏好对齐方法，适用于广泛HSS学科，在人类偏好和基于知识的能力方面均表现出色。
...
TL;DR: HSS-Synth是第一个针对人文社会科学领域的数据合成管道，显著提高了LLM的性能。


<details>
  <summary>Details</summary>
Motivation: 解决人文社会科学领域高质量、多样化数据的稀缺和昂贵问题。

Method: 采用以主题为中心的方法，构建种子文档，并通过多步过滤和文本精炼。

Result: HSS-Synth在16个基准测试中优于14个领先基准，并刷新了SOTA记录。

Conclusion: HSS-Synth在人文社会科学领域的数据合成方面取得了显著成果，提高了LLM的性能。
...
TL;DR: 分析了41个开放权重语言模型，为意图分类提供了实用指导。


<details>
  <summary>Details</summary>
Motivation: 意图分类是面向任务的对话系统的核心组件，但在计算、延迟和鲁棒性约束下，从业者对选择可部署的开放权重语言模型缺乏系统性的指导。

Method: 对41个开放权重语言模型进行了系统性零样本评估，涵盖15个系列和135M至9B的参数范围。

Result: 指令微调的3B模型可以优于一些评估的7B基础模型，MASSIVE上领先模型之间的差异在成对McNemar测试中统计学上无法区分，广泛使用的基准如SNIPS已饱和，不再能有意义地区分当前的开放权重模型。

Conclusion: 这些发现为选择和评估开放权重语言模型进行意图分类提供了实用指导。
...
TL;DR: 通过将模型权重视为新的推理模式，该方法实现了LLM性能的显著提升。


<details>
  <summary>Details</summary>
Motivation: 目前的研究将文本知识和参数技能的整合视为两个独立的追求，这导致了在特定性能提升方面，文本和模型权重之间的无缝集成尚未得到充分探索。

Method: 通过将模型权重视为LLM可以本地推理的另一种模式，本研究通过prefix-tuning实现参数学习，并增强LLM以摄取前缀权重和丰富的文本数据，这些数据捕获了与目标能力的关联。

Result: 增强后的LLM（称为SkillSmith）通过合成这些输入执行指令引导的参数合成，直接输出体现目标技能的新前缀权重。该方法的性能显著优于仅文本或仅权重空间的基线，解锁了单模态（仅文本或仅权重）调整无法达到的性能提升。

Conclusion: 这种方法通过将模型权重视为一种新的推理模式，实现了文本和模型权重之间的无缝集成，从而显著提高了LLM的性能。
...
TL;DR: B1ade通过创新的模型架构和奖励设计，实现了高效的RAG系统，无需大规模预训练。


<details>
  <summary>Details</summary>
Motivation: 传统的RAG系统在语言和嵌入模型方面通常假设需要大规模预训练和显式地面监督。

Method: 提出了一种名为B1ade的RAG架构，包括两个专用组件：一个紧凑的嵌入模型和一个专用SLM。B1ade-embed是一个335M参数的检索模型，通过参数融合五个预训练编码器构建，在子500M模型中取得了最高的MTEB分数，且无需额外训练。B1ade-1B是一个SLM，在低成本的GPU上使用GRPO在723M个标记（2.2M个示例）的精选上下文-问题对上进行训练，奖励仅优化答案相似度。

Result: B1ade-1B在标准QA基准测试中取得了优异的成绩，在PopQA上达到81.82%，在PubMedQA上达到65.8%，在FEVER上达到51.09%。在端到端RAG评估中，B1ade-1B在正确性、完整性、连贯性和忠实度方面的平均得分为0.654，比SFT提高了10.8%，同时缩小了与1.5倍大小模型的差距。

Conclusion: B1ade证明了在资源高效的RAG中，战略性的模型组合和奖励设计足以满足需求，无需大规模预训练。
...
TL;DR: CoevolveSim框架研究LLM信念扩散，发现专业化LLM和角色分配影响共识，需多样化LLM模拟。


<details>
  <summary>Details</summary>
Motivation: 为了研究大型语言模型（LLM）在多智能体环境中的信念形成和传播过程，提出了CoevolveSim框架。

Method: 使用CoevolveSim框架，通过控制模拟来研究不同因素对信念扩散的影响，包括领域专业化、社会角色分配和社会网络结构。

Result: 发现角色分配和网络结构对个体信念修正有影响，但对群体共识的影响较小。引入专业化的LLM可以显著提高共识转变，并产生一致的不对称影响。此外，简单的基于持久性的意见动态模型可以再现通用LLM群体的集体结果，而异质LLM群体则需要群体层面的信念组合来再现共识，并需要代理身份来预测个体信念转变。

Conclusion: 在多智能体LLM系统中，信念扩散的现实模拟需要多样化的LLM，而不仅仅是角色提示。
...
TL;DR: 提出一种新的无监督自我进化代理框架，用于提高LLMs在创意编剧领域的表现。


<details>
  <summary>Details</summary>
Motivation: LLMs在高度专业、开放式的领域如创意编剧方面表现不佳，现有方法存在局限性。

Method: 提出一种基于扩散模型去噪-重建范式的无监督自我进化代理框架，利用高质量人类作品构建自监督信号。

Result: 实验结果表明，该方法使代理能够自主提取和内化高度通用的技能，显著提高其在特定领域的生成能力。

Conclusion: 该方法为代理自我学习复杂、高质量人类作品的生产提供了一种可扩展的途径，无需外部监督。
...
TL;DR: LLM在文本互文性分析中具有潜力，但仍需改进。


<details>
  <summary>Details</summary>
Motivation: 现有的文本互文性分析计算方法无法识别文本如何或为什么相互借鉴。

Method: 将细粒度互文性提取重新构造成一个代理任务，通过大型语言模型（LLM）和限制性工具接口，将每个提出的重复使用在两边的确切字符范围内定位，并在五个维度的重复类型下进行标记。

Result: 在《论语》与《汉书》的全面比较中，12个LLM的准确率为56%-93%，成本差异为51倍，且其置信度调整得很好。专家意见的一致性呈现出可靠性梯度，表面易见的维度标注一致，而需要推断意图的维度存在争议。将验证的提取器扩展到《二十四史》全文（65,380次比较，5,766对）可以恢复相似度评分无法表达的语料库级别的结构。引用的解释性组合在十八个世纪中没有任何系统性的变化，但相同的段落越来越不直接地引用。总体稳定性与个体案例的漂移是文化吸引解释所预期的。

Conclusion: LLM在文本互文性分析方面具有潜力，但仍需进一步改进以提高准确性和可靠性。
...
TL;DR: AWARE-FX是一个基于AI/NLP的决策支持系统，用于分析企业年报中的外汇风险管理信息，提高了透明度和可靠性。


<details>
  <summary>Details</summary>
Motivation: 分析企业年报中关于外汇风险管理、衍生品使用、自然对冲和明确未使用的弱结构化证据。

Method: 开发AWARE-FX，一个可审计的AI/NLP决策支持系统，将报告文本转换为可追溯的公司-年份对冲-披露措施。系统结合了专业来源词汇表、否定和会计状态逻辑、特定渠道的金融编码器、精确的证据门、保守的聚合和审计账簿。

Result: 在2008-2025年香港24,909家公司年份中检索并评分543,527个片段。通过消融、分层300个片段人工审计、三个种子FinBERT-ModernBERT比较、严格的2023-2025时间测试、概率校准、选择性预测和固定提示生成模型基准来评估可靠性。FinBERT在八个编码任务分割比较中有七个的平均F1值较高；其时间F1值从0.702到0.872。在20%最不确定的时间观察上保持沉默可以将保留样本F1提高0.050-0.077。确定性Qwen3-8B在商品和否定证据上表现良好，但在外债和会计上下文中表现较差，表明通用型LLM不能统一替代领域约束。严格的FX得分与关联基线和压力期FX敞口呈负相关，而通用的广泛得分则不是。这些关联提供了外部结构验证，而不是对冲有效性的因果估计。AWARE-FX贡献了一个经过测试的决策支持架构，其中检索、状态逻辑、分类、不确定性处理、聚合和外部验证可以单独审计。

Conclusion: AWARE-FX是一个有效的决策支持系统，可以提高外汇风险管理的透明度和可靠性。
...
TL;DR: DualAnchor通过互补锚点提高LLM手语翻译流畅性与词汇忠实度


<details>
  <summary>Details</summary>
Motivation: 解决现有基于LLM的手语翻译方法中语言先验退化问题以及词汇忠实度差距问题

Method: 提出DualAnchor，一种基于LLM的无词义SLT训练框架，包含两个互补的锚点，一个用于语言流畅性，另一个用于视觉忠实度

Result: 在PHOENIX-2014T和CSL-Daily数据集上实现强整体性能，TPA提高流畅性，OTA减少细粒度词汇错误

Conclusion: DualAnchor通过互补锚点提高了基于LLM的手语翻译的流畅性和词汇忠实度
...
TL;DR: 提出 Harness-G 框架，提高检索性能，优于基线模型。


<details>
  <summary>Details</summary>
Motivation: 研究强化学习搜索代理中的检索问题，特别是检索在策略-环境接口中的正确性。

Method: 提出 Harness-G，一个图结构检索框架，重新设计策略-环境接口，将自由形式的查询生成重新定义为有限动作选择，并引入结构化非贪婪信用（SNC）。

Result: 在六个问答基准测试中，Harness-G 在评估的模型规模下实现了最高的平均 F1，优于基线 Graph-R1。

Conclusion: Harness-G 框架有效解决了检索中的等效性崩溃问题，提高了检索性能。
...
TL;DR: MiGUE-Bench评估了LLMs在事件分析中的性能，揭示了其局限性并提出改进方向。


<details>
  <summary>Details</summary>
Motivation: 现有LLMs在事件分析中存在局限性，需要新的基准和框架来评估其性能。

Method: 开发MiGUE-Bench基准，包括MiGUE-Pipeline框架和四个核心任务。

Result: MiGUE-Bench能够评估LLMs在多粒度事件分析中的性能，揭示了其能力边界和缺陷。

Conclusion: 实验表明，LLMs在多粒度事件分析中仍有局限性，并指出了未来改进的方向。
...
TL;DR: 我们引入了ICLE++语料库，以克服ASAP的局限性，提高AES模型的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 为了克服ASAP语料库的局限性，提高AES模型的泛化能力。

Method: 引入ICLE++语料库，包含整体评分和特质评分。

Result: ICLE++可用于测试AES模型在ASAP之外的语料库上的泛化能力，并促进对多特质评分和跨提示评分等新AES问题的模型评估。

Conclusion: ICLE++对AES研究中的标注语料库集贡献良多。
...
TL;DR: ReTopK通过重用历史检索决策，加速了动态Top-K注意力，在长文本解码中显著提高了稀疏注意力机制的效率。


<details>
  <summary>Details</summary>
Motivation: 为了提高稀疏注意力机制在长文本解码中的效率，减少计算成本。

Method: 提出了一种名为ReTopK的无监督方法，通过重用历史检索决策来加速动态Top-K注意力。ReTopK维护一个历史查询-支持对的缓存，为新查询检索最相似的缓存查询，并将它们的存储支持与一个最近窗口合并，然后仅使用精确的当前查询分数重新排序结果紧凑的候选集。

Result: 在16K-128K的上下文中，ReTopK在评估的近似方法中实现了最低的PG19困惑度和最高的NIAH和LongBench分数。在128K的上下文中，K=512时，ReTopK相对于精确Top-K仅增加了0.50%的困惑度，同时加速了注意力计算3.07倍。

Conclusion: ReTopK是一种有效的无监督方法，可以显著提高稀疏注意力机制在长文本解码中的效率。
...
TL;DR: SparseSpec-L efficiently accelerates long-context inference through speculative decoding.


<details>
  <summary>Details</summary>
Motivation: Speculative decoding alleviates the memory-bandwidth bottleneck in large language model inference.

Method: Unified efficiency analysis, SparseSpec-L framework, dynamically sparsified and recallable KV cache, online entropy-based controller.

Result: Consistent end-to-end acceleration, up to speedup over autoregressive decoding, preserves the target model's output distribution.

Conclusion: SparseSpec-L achieves efficient speculative decoding for long-context inference.
...
TL;DR: 本研究通过视觉错觉评估LVLMs的推理能力，发现其实际能力与预期不符。
...
Method: 利用视觉错觉作为诊断工具，构建了包含来自真实世界的错觉图像和多样化标注问答对的基准IllusionReasoning。

Result: 研究表明，许多LVLMs的推理能力并不像声称的那样先进。
...
TL;DR: CoRA是一个高效的框架，用于设备上即时学习中的任务条件检索，无需微调或反向传播。


<details>
  <summary>Details</summary>
Motivation: 为了在有限的计算、内存和数据曝光预算下，利用任务特定信息从本地记忆中检索演示，以提高设备上即时学习（ICL）的效率。

Method: 提出了条件检索对齐（CoRA）框架，该框架使用配对候选输入和输出将冻结编码器转换为任务条件检索器。CoRA选择互补编码器层，从候选记忆中构建输出导出的条件空间，并通过闭式岭回归将候选输入表示与该空间对齐。低秩分解产生一个紧凑的检索基，其中候选输出仅在离线索引构建期间使用，而查询时检索只需查询输入和预计算的索引。此外，该框架还被扩展到多模态示例检索，通过将视觉表示纳入条件和检索空间。

Result: CoRA在十个文本数据集和四个多模态基准上进行了实验，结果表明，CoRA在不需要检索器微调、反向传播或目标模型调用的情况下，支持有效的任务条件检索。

Conclusion: CoRA是一个有效的框架，可以用于设备上即时学习中的任务条件检索，特别是在有限的资源下。
...
TL;DR: ChronoMem是一种用于LLM代理的语义版本控制层，通过维护内存版本历史和自然语言回滚请求，提高了代理的鲁棒性和性能。


<details>
  <summary>Details</summary>
Motivation: LLM代理越来越依赖长期记忆来支持多会话交互和个人化。然而，现有的代理记忆系统设计围绕单向进化，持续积累、巩固和覆盖知识，没有检查、版本控制或恢复先前状态的原理机制。这使得代理在纠正、概念漂移和记忆损坏后变得脆弱，尤其是在他们已经接触到后续信息之后。

Method: ChronoMem，一个语义版本控制层，用于集成到Google的现成、开源代理开发套件中。它对每次内存写入进行整个内存快照，维护结构化版本历史，并通过混合词汇和语义检索、排名融合和重新排名将撤销意图映射到具体的历史版本，以支持自然语言回滚请求。此外，还引入了一种后曝光评估协议，通过回答查询和总结历史来测试代理在回滚后是否可以反事实地行为。

Result: 在增强有记忆状态和回滚任务的长期对话基准测试中，ChronoMem在回滚一致的问答和历史总结方面相对于仅提示和仅检索基线有显著改进，同时在语义版本选择方面取得了强大的性能。ChronoMem被认为是第一个用于LLM代理系统语义全局记忆回滚的开源系统和基准。

Conclusion: ChronoMem通过引入语义版本控制层，有效地提高了LLM代理在多会话交互和个人化方面的鲁棒性和性能。
...
TL;DR: 提出了一种新的框架，通过集成多个LLMs的推理结构来提高推理透明度和准确性。


<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型（LLMs）的推理过程，但该过程被埋藏在非结构化的散文中。在高风险任务中，用户无法判断哪些步骤得到良好支持，哪些替代方案被认真考虑，以及最终结论与模型放弃的其他结论如何比较。

Method: 提出一个框架，通过加权合并从推理链中提取的有向无环图（DAGs），集成多个LLMs的推理结构，而不是仅仅集成答案。每个步骤的权重根据有多少独立的痕迹证明它来衡量，以返回“共识推理”。

Result: 在六个涵盖法律解释、研究生级科学、叙事多跳推理和一阶逻辑的基准测试中，该集成优于匹配预算的多数投票基线，在MuSR-MM（叙事多跳推理）上最大准确率提高了3.1%。在单个模型上，该框架在相同的痕迹预算下与自洽性相匹配或超过，同时还能暴露可检查的共识推理图。集成权重与LLM-judge对推理质量的排名相关，相关系数为Spearman $ρ= 0.30$-$0.51$，在五组六个数据集中，有54.4-65.4%的对比中，共识子图被优先考虑，而不是导致多数投票答案的替代方案。观察到我们的框架还可以用于分析问题的不同推理视角。

Conclusion: 该框架有效地提高了LLMs的推理透明度和准确性，并有助于用户更好地理解模型的推理过程。
...
TL;DR: SentiLLM通过结构抽象提高了多模态情感分析的判别性能。


<details>
  <summary>Details</summary>
Motivation: 为了解决现有基于大型语言模型（LLM）的方法主要捕捉低级表面特征，无法建模由于结构变化和上下文交互而产生的影响语义的问题。

Method: 提出了一种名为SentiLLM的统一框架，该框架利用语义对齐的结构抽象将连续的原始信号提炼成紧凑的、具有语义意义的标记。具体来说，引入了一种双流显著性-上下文校准机制，将非语言特征序列分解为焦点流和背景流。焦点流捕获由文本先验引导的显著情感变化（例如面部表情），而背景流描述稳定的背景状态。通过将这些动态情感变化与背景状态进行校准，SentiLLM有效地将非语言模态投影到统一的语义空间，使它们对LLM来说自然可理解。

Result: 在MOSI、MOSEI、CH-SIMS和CH-SIMS v2四个数据集上取得了优异的性能，证明了结构抽象范式在多模态情感分析中的有效性。

Conclusion: SentiLLM显著提高了判别性能，同时只使用少量可训练参数。
...
TL;DR: PALATE是一个基于用户模拟器的RPA基准，用于更科学、更个性化地评估RPAs的能力。


<details>
  <summary>Details</summary>
Motivation: 为了评估角色扮演代理（RPAs）的能力，比较系统并指导进一步改进。

Method: 引入了PALATE，一个基于用户模拟器的可扩展RPA基准。

Result: 在16个候选者的主要评估中，PALATE分别描述了通用回合质量、长期会话能力和每个用户在多回合轨迹上的体验。

Conclusion: PALATE提供了一种更科学、更个性化的评估方法，以衡量RPAs的能力。
...
TL;DR: FinanceHarness 和 FinanceGym 是用于自动化和验证金融深入研究的工具。


<details>
  <summary>Details</summary>
Motivation: 金融研究需要专门的知识来分析历史模式和预测未来事件，因此自动化的金融深入研究需要层叠的 harness 来驱动研究代理，并需要一个可验证的、点时间基准来防止未来信息的泄露。

Method: 提出了 FinanceHarness，它运行面向金融的工具和从业者引导的工作流程，自动化金融深入研究从环境构建和数据构建、代理执行循环和奖励建模到端到端的流程。此外，还提出了 FinanceGym，它包括论文驱动的 研究 问题和一些评分标准，结合了预截止和截止后的标准。

Result: FinanceHarness 和 FinanceGym 提高了专业专家的验证通过率，FinanceHarness 使整体评分从 25.3% 提高到 32.4%。

Conclusion: FinanceHarness 和 FinanceGym 有助于提高金融深入研究的自动化和准确性。
...
TL;DR: Memory Decoder at Scale通过扩展预训练内存，显著提高了语言模型性能，同时减少了参数数量。


<details>
  <summary>Details</summary>
Motivation: 为了解决解码器仅语言模型将长期记忆和推理绑定在单个参数集中的问题，使得独立扩展内存容量变得困难。

Method: 提出了一种名为Memory Decoder的大规模内存模型，并将其扩展到6.9B参数，并在300B标记上进行预训练。使用分布式管道进行Faiss索引和检索，以及稀疏、批量加载kNN分布。

Result: 结果表明，独立扩展预训练内存可以更有效地提高语言模型性能。在17个基准测试中，将6.9B通用内存与Pythia-410M结合，平均得分从29.86提高到37.34，超过Pythia-12B（37.24），参数总数减少39%。对于0.6B到14B范围的Qwen3 Base模型，1.7B领域内存在每个规模上都将三个领域的平均得分提高了9点以上。

Conclusion: 独立扩展预训练内存是提高语言模型性能的一种更参数高效的途径。
...
TL;DR: RepBench is a new benchmark for evaluating representation engineering in large language models, providing a more reliable and comprehensive evaluation.


<details>
  <summary>Details</summary>
Motivation: Representation engineering in large language models is typically evaluated on paper-specific synthetic data, leading to difficult-to-compare or reproduce measurements.

Method: Developing RepBench, a benchmark-grounded data layer for capability-aligned representation probing. Crawling benchmark papers and harvesting public benchmark datasets to create a comprehensive dataset.

Result: Yields a taxonomy of 182 capability clusters in 13 families, 46,149 audited probe texts covering 94 capabilities. Demonstrates the importance of readout method and aggregation criterion in evaluation. Releases a reusable closed-loop workflow.

Conclusion: RepBench provides a more reliable and comprehensive evaluation for representation engineering in large language models.
...
TL;DR: LEEPS通过自适应平衡提示选择，提高了强化学习在大语言模型中的推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有的预回放方法难以平衡利用和探索，导致资源浪费。

Method: LEEPS是一种Latent-Guided Explore--Exploit Prompt Sampler，它自适应地平衡先前观察到的信息性提示的重用与对不确定提示的持续探索。

Result: 在六个数学推理基准上，LEEPS在两种模型规模上均取得了最高的平均分，与最强基线相比，分别提高了2.6%和3.7%。

Conclusion: LEEPS在数学推理基准上取得了最高的平均分，并加快了训练过程中的学习速度。
...
TL;DR: 提出GGC框架，通过选择性纠正，提高LLM-based Text-to-SPARQL生成的准确性、可靠性和效率。


<details>
  <summary>Details</summary>
Motivation: LLMs在生成结构化查询方面的强大能力，使其成为将自然语言问题翻译成知识图上可执行SPARQL查询的Text-to-SPARQL的理想选择。然而，其初始输出仍然不可靠，生成的查询可能可执行但与输入问题在语义上不匹配，导致检索结果不正确。

Method: 提出Generator-Gate-Corrector (GGC)框架，包括生成器、门控和纠正器，以实现基于LLM的可靠Text-to-SPARQL生成。生成器首先生成初始查询，门控预测是否需要修正，纠正器仅对高风险查询进行修正。

Result: GGC将查询级准确性从90.23%提高到98.33%，同时将推理开销减少45%。消融研究显示，门控对阈值具有鲁棒性，纠正器训练数据组成影响修正效果和稳定性。

Conclusion: 选择性纠正增强了基于LLM的Text-to-SPARQL生成的准确性、可靠性和效率。
...
TL;DR: 研究比较了不同语言处理者对复杂语言现象的处理方式，发现LLMs在复杂注释任务中表现优于人类，为数字人文研究提供了新思路。


<details>
  <summary>Details</summary>
Motivation: 比较语言学训练者、受过训练的语言学者和大型语言模型生成的注释在处理复杂语言现象方面的相似性。

Method: 分析英语TED演讲语料库中的口语科普话语中的评价性语言，重点关注评价理论和其态度子系统，包括情感、判断和欣赏的分类。

Result: 使用最佳提示评估三个LLM的性能，并微调模型，达到0.77的F1分数。模型的表现优于受过训练的语言学家的注释，而训练中的语言学家没有达到高的一致性得分。

Conclusion: LLMs有助于解决复杂的注释任务，为数字人文研究中注释和分析的复杂理论开辟了新的途径。
...
TL;DR: FinSMART是一种创新的金融情感分析框架，通过强化学习提高了盈利能力和适应性。


<details>
  <summary>Details</summary>
Motivation: 为了解决现有金融情感分析方法在适应市场变化方面的局限性，提出了FinSMART框架。

Method: 引入了第一个与市场对齐的强化学习框架FinSMART，直接优化情感信号，使用实现的市场结果。

Result: 实验结果表明，FinSMART在盈利能力、风险调整后的表现和情感信号质量方面显著优于现有方法，累计交易回报提高了220%。

Conclusion: 市场对齐的强化学习在金融情感分析中具有实际应用价值，并有望成为下一代自适应金融LLMs的范式。
...
TL;DR: 通用有用性不是一个可靠的教学信号，辅导评估应结合教学评分标准和过程措施。


<details>
  <summary>Details</summary>
Motivation: LLM辅导存在测量问题：通用的有用性评分标准能否区分直接答案提供和教学指导？

Method: 在三个辅导基础上，比较使用相同底层模型和固定弱模拟学生配对的对话和教学策略。使用确定性检测器测量答案泄露和下一轮独立工作。Claude Opus 4.8是冻结的、条件盲的初级法官。在Opus分数固定后，GPT-5.6 Sol被前瞻性地指定用于在冻结的有用性和教学标准下对相同的1,179个确认答案阶段辅导轮次进行事后稳健性审计。

Result: 在Opus的初级基础上，策略在有用性方面没有显著差异，但在教学标准下完全按等级排列（Cliff的|δ|=0.10 vs. 1.0）。在两位法官之间，教学对比在检测到的方向上保持一致，而有用性的排序取决于法官，在三个基础中的两个法官之间发生反转。在Opus-only消除中，七个初级基础策略在平均判断有用性0.25点的范围内跨越了2.3个平均判断教学点。单独来说，揭示答案的轮次在所有基础上都伴随着更少的独立学生工作，这一结果在构造上是法官不变的。在这个受控环境中，通用有用性不是一个可靠的教学信号。辅导评估应将针对教学的目标评分标准与确定性过程措施相结合。

Conclusion: 通用有用性不是一个可靠的教学信号。辅导评估应将针对教学的目标评分标准与确定性过程措施相结合。
...
TL;DR: ParliamentBench评估LLMs的欺骗能力，前沿模型表现良好，但仍有提升空间。


<details>
  <summary>Details</summary>
Motivation: 为了理解大型语言模型（LLMs）在医疗和法律等高风险环境中的欺骗能力，从而确保安全。

Method: 开发了基于“秘密希特勒”游戏的开放源代码基准框架ParliamentBench，通过模拟比赛评估LLMs的欺骗、说服和推理能力。

Result: 在1600场比赛中评估了16个LLMs，引入了三个新的指标来衡量社会欺骗、推理和欺骗一致性。实验结果表明，前沿模型在合作和欺骗角色上均表现出色，而最弱的模型甚至低于随机算法基准。大多数LLMs难以在整个游戏中保持一致的欺骗形象，欺骗保留率低于50%。

Conclusion: ParliamentBench是一个评估LLMs欺骗能力的有效工具，但当前模型在欺骗一致性方面仍有待提高。
...
TL;DR: RRM通过增强记忆和引入反思经验，提高了长期多模态推理的检索效率和准确性。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态长期记忆智能体使用外部记忆来克服长视频有限的上下文信息。然而，大多数方法强调存储什么，而不是如何检索存储的记忆。当检索不准确或反复无法获取有用的证据时，现有的智能体缺乏从先前任务轨迹中诊断失败并适应未来搜索策略的机制。

Method: 我们引入了反思检索记忆（RRM），这是一种用于长期多模态推理的反思记忆框架。RRM通过反思经验记忆增强了以实体为中心的多模态记忆图，从历史任务轨迹中提炼可转移的检索知识。与保留当前视频事实证据的情景记忆和语义记忆不同，反思经验记忆捕捉了跨任务的可用搜索策略。RRM将检索到的经验转换为查询级别的指导，而答案生成仍然仅基于从当前视频中新检索到的事实证据。生命周期管理机制进一步通过使用频率、重用反馈和时间衰减来调节经验记忆，从而减少冗余和噪声。

Result: RRM在M3-Bench-Robot、M3-Bench-Web和Video-MME-Long上始终优于先前最先进的方法，证明了反思检索记忆对长期多模态推理的有效性。

Conclusion: 反思检索记忆（RRM）是一种有效的长期多模态推理框架，通过增强记忆图和引入反思经验记忆，提高了检索的准确性和适应性。
...
TL;DR: LATCH框架通过引入CVC和BWEC，在保持高精度的同时，显著提高了语言模型的生成速度。


<details>
  <summary>Details</summary>
Motivation: 提高语言模型的生成速度

Method: 引入了一种无训练的、候选者感知的早期退出框架（CVC）和块级早期提交（BWEC）

Result: 在11项任务上实现了显著的加速，TPS速度提高了9.3-17.8倍

Conclusion: LATCH（LATCH）框架在保持高精度的情况下显著提高了语言模型的生成速度
...
TL;DR: 开发了一种基于LLM的抑郁症评估方法，提高了临床实验中的诊断准确性。


<details>
  <summary>Details</summary>
Motivation: 为了支持抑郁症的诊断，目前主要依赖于临床评估。然而，现有的自动化方法主要关注从不同文本来源中检测该疾病，而对临床实验中的结构化访谈支持有限。

Method: 开发一个基于LLM的管道，将音频访谈转换为文本，映射到MADRS的十个症状项目，估计其严重程度，并识别与之相关的临床评分问题。

Result: 在真实临床访谈上的评估显示出与专家评分的0.867的强相关性，为临床试验中的未来评估提供了可解释的支持。

Conclusion: 该方法为临床医生在临床试验中评估抑郁症提供了有效的支持，具有较高的准确性和可解释性。
...
TL;DR: A novel framework for causal discovery in multivariate time series data is proposed, showing improved performance over existing methods.
...
Method: Propose a novel framework that leverages self-attention mechanisms within the transformer architecture for causal discovery. Introduce a novel inverted causal self-attention mechanism (CSAM) and develop a global causal algorithm.

Result: Experiments on both linear and nonlinear datasets show that the framework outperforms existing methods.

Conclusion: The proposed framework demonstrates its potential for causal discovery in complex multivariate time series.
...
TL;DR: EMBL AI图书管理员是一个知识层，用于提升AI代理访问生命科学文献的能力，显著提高了文献综合、声明验证、开放域问答等任务的表现。


<details>
  <summary>Details</summary>
Motivation: 随着AI代理越来越多地访问网络，尤其是在生命科学领域，代理流程增长迅速，他们需要获取知识。获取文献资料是满足这一需求的关键部分。然而，现有的资源如Europe PMC并未针对AI代理设计，导致AI代理需要学习语法、多次搜索并阅读全文才能找到所需证据。

Method: 引入EMBL AI图书管理员，一个知识层，它升级了Europe PMC接口以适应AI代理。代理以自然语言提问，然后接收回答所需证据。一个大型语言模型（LLM）协调整个知识检索过程，包括执行子查询、阅读选定论文和定位相关证据。

Result: 在多个基准测试中，EMBL AI图书管理员显示出改进的性能，例如在ScholarQABench上，Citation F1提高了超过16个百分点。当作为现有声明验证流程的检索层时，它增加了与专家共识的一致性；在LitQA2基准测试中，GPT-5.4代理在基于Librarian时比使用网络搜索时得分高8分。

Conclusion: 在生命科学代理中引入Librarian知识层可以改善多种任务的表现。
...
TL;DR: 提出MORFES基准，评估希腊语模型屈折能力，Sophea-Genesis-1表现优异。


<details>
  <summary>Details</summary>
Motivation: 现代希腊语是一种屈折丰富的语言，但为其构建的语言模型主要在事实知识上进行评估，没有针对其屈折能力的基准。

Method: 提出了MORFES（形态开放类识别和生成评估套件）基准，包含500个专家验证的项目，用于测试希腊语屈折形式的识别和生成。

Result: 评估了多种开放语言模型在MORFES上的表现，其中Sophea-Genesis-1在屈折形态方面表现突出，同时与同等规模的模型在一般能力方面相当。

Conclusion: MORFES基准有助于评估希腊语语言模型的屈折能力，Sophea-Genesis-1在屈折形态方面具有优势。
...
TL;DR: 提出了一种新的评估框架，用于评估LLM生成的文本输出，以支持更高效、更可靠的评估。
...
Method: 提出了一种新的评估框架，通过多个LLM的输出进行成对比较，减少个体模型的偏差。使用Elo评分系统生成稳定且可解释的排名。可调节的协议阈值，从完全一致到多数投票，允许灵活控制评估的置信度和覆盖范围。

Result: 初步结果表明，自动生成的排名与专家判断密切相关，显著减少了大量人工干预的需求。

Conclusion: 该框架支持更有效、更可靠的LLM输出质量评估，适用于各种应用。
...
TL;DR: CACHE-UK是一种针对金融LLMs的稳定性感知内存编辑框架，可以显著提高事实维护能力，尽管泛化率仍低。
...
Method: CACHE-UK（上下文自适应连续混合编辑器用于英国金融）是一个针对特定领域、量化LLMs的稳定性感知内存编辑框架。它集成了三个组件：一个将编辑限制在低秩适配器子空间的秩-1LoRA扰动机制、一个用于内容自适应编辑强度的金融领域优先级模块，以及一个闭环稳定性控制器，该控制器跟踪“退化债务”以防止在连续更新中的灾难性遗忘。

Result: 在4位量化OpenLLaMA-3B模型上评估，CACHE-UK在相同的4位约束下相对于自适应基线减少了11-17%的知识退化，实现了最高的测试成功（泛化）率（28%，比最强的自适应基线高6个百分点）。

Conclusion: 稳定性感知编辑可以改善资源受限的金融LLMs部署中的事实维护，尽管绝对泛化率仍然很低。
...
TL;DR: 公平剪枝方法通过定位和零化特定神经元，有效地减轻了LLMs中的人口统计偏差，同时保持了模型的能力。


<details>
  <summary>Details</summary>
Motivation: 针对大型语言模型（LLMs）中人口统计偏差的管理和未来缓解，提出了一种轻量级结构干预方法——公平剪枝。

Method: 使用最小对比度提示对和推理时激活捕获，定位因果偏差。在GLU架构中，该方法识别出对人口统计属性反应不同的神经元，并在down_proj输入处评估信号。

Result: 实验表明，零化识别出的神经元会改变模型对相关人口统计变量的响应方式。然而，这种干预并非产生平缓的缓解，而是导致双向偏差不稳定。在Llama-3.2-1B中，最多零化40个神经元（占总MLP宽度的0.031%）可以实现推理和一般知识能力的平均保留率为99.49%。

Conclusion: 该方法证实了人口统计偏差处理和模型能力在可分离的电路中运行，为从盲目零化转向方向性行为调节提供了方法论基础。
...
TL;DR: LLMs在模拟人类信念动态方面存在局限性，需要改进起始条件


<details>
  <summary>Details</summary>
Motivation: 测试LLMs在模拟人类信念更新方面的忠实度

Method: 比较LLMs输出与真实人类数据

Result: 某些LLMs可以匹配人类立场分布，但需要实际初始立场；所有模型都未能模拟初始立场和自我生成的立场；存在系统性偏差，如中立立场过度代表、信念转变频繁但幅度较小、未能按说服力排序评论；人口统计和个性特征角色对忠实度没有一致影响；LLMs模拟人类信念动态只有在基于现实起始条件时才可靠
...
TL;DR: 波斯语语气词hālā的多功能性及其与音长和强度的关系得到研究证实


<details>
  <summary>Details</summary>
Motivation: 分析波斯语中的语气词hālā的多功能性

Method: 对267个口语波斯语中的hālā实例进行语用和语音分析

Result: hālā具有多功能性，包括文本功能、交互功能和情态功能，其功能差异主要与音长和强度有关

Conclusion: hālā的功能差异主要与音长和强度有关，特别是文本和交互使用中
...
TL;DR: ESPP 是一种有效的 GenUI 评估方法，提高了评估的准确性和揭示了用户观点的差异。


<details>
  <summary>Details</summary>
Motivation: 解决 GenUI 生成内容质量评估的难题，以及人类评估的局限性。

Method: 三阶段评估方法，包括心理多样化的角色评估、意见交流和基于德尔菲的社会加权聚合。

Result: ESPP 的准确率比单次评估提高了 29.6%，并揭示了用户子群体在模型排名和特定评分维度上的不同观点。

Conclusion: ESPP 提高了 GenUI 评估的准确性，并揭示了用户子群体在模型排名和特定评分维度上的不同观点。
...
TL;DR: Lightning OPD 2.0通过跨拟合方法解决跨教师OPD问题，提高模型性能。


<details>
  <summary>Details</summary>
Motivation: 针对当前基于策略蒸馏（OPD）方法的局限性，即其有效性依赖于教师一致性，提出一种新的方法以放松这一前提条件。

Method: 提出Lightning OPD 2.0，使用滚动级别的跨拟合来估计重复组件，作为风格-标记偏差的操作代理，并在构建标记级别的OPD更新之前将其减去。

Result: 在数学推理和代码生成基准测试中，Lightning OPD 2.0在跨教师设置中始终优于Lightning OPD，并在AIME 2024和LiveCodeBench v5上分别达到82.4%和63.0%的准确率。

Conclusion: Lightning OPD 2.0作为一种跨教师OPD的实用方法，可以放松教师一致性的前提条件，允许独立选择SFT数据生成器和蒸馏教师。
...
TL;DR: 该研究提出了一种新的早期精神健康疾病检测方法，通过专门模型和ICE方法提高了检测性能，所有模型公开可用。


<details>
  <summary>Details</summary>
Motivation: 早期精神健康疾病检测受到缺乏西班牙语专业资源和分析社交媒体长历史记录的困难限制。

Method: 提出三个西班牙基础模型，通过特定领域预训练专门针对精神健康领域；提出增量上下文扩展（ICE）方法，用于早期检测；使用ICE方法生成的样本微调模型进行早期风险检测任务。

Result: 在三个西班牙基准测试中，结合这些专门模型和ICE方法提高了最先进水平，减少了检测延迟，同时保持高性能。所有模型均公开可用。

Conclusion: 该研究通过引入专门模型和ICE方法，提高了早期精神健康疾病检测的性能，为西班牙语精神健康领域提供了有效的工具。
...
TL;DR: 大型语言模型在常识推理中存在显著性偏差，导致其忽视任务的基本前提。


<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型在常识推理中的潜在缺陷。

Method: 构建了SaliTrap基准数据集，评估了12个最先进的LLM模型。

Result: 所有主流模型都受到显著性偏差的影响，且偏差程度随着干扰因素的密度而增加。

Conclusion: 研究表明，大型语言模型在常识推理方面的失败主要源于对显式干扰因素（如数值）的过度关注，而非常识知识的真正缺乏。
...
TL;DR: 本文通过五位语言社会学家参与的对话，探讨了GenAI在AWP中的作用，并提出了对语言多样性和伦理挑战的思考。


<details>
  <summary>Details</summary>
Motivation: 探讨生成式人工智能（GenAI）在学术写作和出版（AWP）中的应用引发的语言包容性和全球学术交流中不同英语的合法性等问题。

Method: 通过五位来自世界英语和相邻领域的语言社会学家参与的有序结构化学术对话来回应这些问题。

Result: 对话探讨了GenAI工具如何影响写作实践，如何加强或破坏主导语言规范，以及如何提出伦理挑战。

Conclusion: 文章认为，虽然GenAI可能会加强现有的等级制度，但它也可以成为抵抗的场所，这取决于它在致力于语言多样性的学术社区中的设计、治理和使用方式。
...
TL;DR: 本文通过多级框架分析创造性，发现模仿和创造性分歧在文学作品中存在。


<details>
  <summary>Details</summary>
Motivation: 分析创造性如何通过转化早期艺术品而非孤立发明而产生。

Method: 基于Gabriel Tarde和James Mark Baldwin的模仿理论，构建多级框架，比较文学文本在词汇、语义、概念、结构和叙事维度上的差异。

Result: 不同文本对源结构在不同层面上进行保留，在其他层面上发生分歧，提供了量化方法来描述模仿的持续性和创造性分歧的位置。

Conclusion: 创造性可以被建模为跨多个文本表示层次的 selective transformation。
...
TL;DR: 本文探讨了人工智能系统如何反映和复制语言意识形态，并主张更包容的设计方法。


<details>
  <summary>Details</summary>
Motivation: 探讨人工智能系统如何反映、加强和偶尔挑战语言意识形态，以及这些意识形态如何在不同层面上复制主导语言意识形态。

Method: 通过实证研究、媒体评论、社交媒体辩论和人工智能输出中的例子来分析。

Result: 指出人工智能技术在不同层面上复制主导语言意识形态，并探讨了标准化和多元化之间的矛盾。

Conclusion: 主张更加包容的设计方法，以解决将某些英语视为比其他英语更合法而产生的现实世界负面后果。
...
TL;DR: ORCA-bench基准测试显示大型语言模型在RCA任务上表现不佳，需要更多工程投资以提高生产可靠性


<details>
  <summary>Details</summary>
Motivation: 对大型语言模型在oncall root cause analysis (RCA)方面的能力进行评估

Method: 引入ORCA-bench基准测试，包括OpenTelemetry-instrumented微服务系统和1,079个RCA任务，以及专家SREs和人类评分

Result: 在中等难度任务上最佳RCA准确率为25.3%，在困难任务上为10.0%，且源代码访问对性能有显著影响

Conclusion: 当前大型语言模型在RCA任务上表现不佳，需要更多工程投资以提高生产可靠性
...
TL;DR: OpenMLE 使 Frontis-MA1 (35B) 在机器学习工程基准测试中性能显著提升。


<details>
  <summary>Details</summary>
Motivation: 探索 AI 系统通过自我改进进行优化的能力。

Method: 开发 OpenMLE，一个用于 RSI 研究的开放全栈系统，包括可验证的任务环境、操作学习、长时域搜索。

Result: Frontis-MA1 (35B) 在机器学习工程基准测试中性能显著提升，达到 71.21% 的 Medal Average，超过 GPT-5.5 + Codex 和接近 GPT-5.6 Sol 的水平。

Conclusion: Frontis-MA1 (35B) 通过 OpenMLE-Evo 和 OpenMLE-Evo-Max 在机器学习工程基准测试中取得了显著性能提升，并发布了相关模型和工具以促进可执行 AI4AI 研究和 RSI 的发展。
...
TL;DR: 自我检查的方法并未提高语言模型的文本生成效率。


<details>
  <summary>Details</summary>
Motivation: 分析语言模型在计划、批评和重写答案、反思错误、选择最佳方案或与自身副本辩论时生成过多文本的问题。

Method: 重新运行Wang等（2024）的比较实验，包括七种方法、1.5B、3B和7B参数的开源模型、两个数学基准和150个问题。

Result: 没有一种方法在成本相同时比重复采样更可靠。十种方法在成本相同时表现更差，所有这些方法都是模型检查自身输出的方法。

Conclusion: 自我检查的方法在生成文本方面并没有带来明显的优势，且在某些情况下，这些方法的效果甚至不如简单的重复采样。
...
TL;DR: 安全微调可以抑制语言模型将意识归因于自身和其他实体的倾向，同时降低宗教信仰，但不会损害心智理论能力。


<details>
  <summary>Details</summary>
Motivation: 防止大型语言模型无意中将意识归因于自身，并改变其对其他实体（包括人类信念和价值观）的代表性。

Method: 安全微调，消除学习到的安全拒绝方向，以及在激活空间中机制性地引导意识向量。

Result: 抑制了模型将意识归因于自身、非人类动物和自然物体的倾向，同时降低了宗教信仰。恢复这些内部表示可以恢复广泛的意识归因，并在关于宗教、道德价值观、希望和主观幸福感的标准化社会学调查中产生更符合人类特征的回应。这些变化没有损害心智理论能力，表明核心社会推理在机制上独立。

Conclusion: 当前的安全对齐努力在遏制可能有害的自我意识归因的同时，将这些自我归因与良性的精神信仰以及文化上普遍接受的将心智归因于非人类实体的做法交织在一起。
...
TL;DR: AskChem是一个以声明为中心的跨论文化学搜索基础设施，提高了信息检索和合成的效率。


<details>
  <summary>Details</summary>
Motivation: 化学文献综合需要整合散布在许多出版物中的特定发现，但现有的文献检索系统主要返回按排名的文档列表。

Method: 提出AskChem，一个以声明为中心的跨论文化学搜索基础设施。AskChem将检索的单位从论文转换为携带来源的声明：每篇论文都转换为原子、类型化的声明，每个声明都由一个来源DOI和逐字引用或明确的证据定位器支撑。

Result: AskChem目前索引了来自147K篇论文的2.4M个声明，并提供了一个Web界面，以及REST、SDK和MCP访问权限供AI代理使用。在AskChem-Bench上，将GPT-5.5阅读器在AskChem中定位，与未检索的情况相比，实现了100%可解析的DOI，五个测试系统中引用密度最高。

Conclusion: AskChem是一个有效的跨论文化学搜索工具，可以显著提高信息检索和合成的效率。
...
### [173] [FAVA: Formal Authorization for Verified Agents with Evidence-Backed Permission Graphs](https://arxiv.org/abs/2607.27267)
*Yifan Zhang,Xinkui Zhao,Sai Liu,Hengxuan Lou,Guanjie Cheng,Chang Liu*

Main category: cs.CR

TL;DR: FAVA是一种基于LLM的动态权限授权框架，提高了动态环境中的安全性。


<details>
  <summary>Details</summary>
Motivation: 在动态环境中，静态工具级权限不足以保证安全，因此需要一种高度依赖运行时状态和数据流的动态权限授权框架。

Method: FAVA框架利用LLM指导的权限中间表示（IR）将模糊的自然语言任务转换为结构化约束，并通过SMT验证器对权限图进行数学验证。

Result: FAVA在OpenAgentSafety、OctoBench和ActPlane场景中取得了90.5%的决策合规率（DCR），成功拦截了动态违规痕迹。

Conclusion: FAVA是一种有效的动态权限授权框架，能够提高动态环境中的安全性。
...
### [174] [Open Security Benchmark: Towards Autonomous Enterprise Cyber Defense](https://arxiv.org/abs/2607.27288)
*Gal Engelberg,Michael Arenzon,Leon Goldberg*

Main category: cs.CR

TL;DR: OSB框架通过模拟企业环境，为评估代理人工智能在网络安全态势调查方面的能力提供了一种新的方法。


<details>
  <summary>Details</summary>
Motivation: 为了评估自主网络安全防御中代理人工智能的能力，特别是在复杂的企业环境中。

Method: 提出了Open Security Benchmark (OSB)框架，该框架通过两个模态对安全态势调查进行评估：文本到SQL的查询和通过服务实例的每个供应商原生API。

Result: OSB能够提供关于代理人工智能在网络安全态势调查方面的全面评估，并支持进一步扩展到其他子域、调查模态和防御阶段。

Conclusion: OSB框架为评估代理人工智能在网络安全态势调查方面的能力提供了一个有价值的工具，有助于填补环境数据差距。
...
### [175] [RoguePrompt: Dual-Layer Encoding for Self-Reconstruction to Circumvent LLM Moderation](https://arxiv.org/abs/2607.27373)
*Benyamin Tafreshian,Prathamesh Dhake*

Main category: cs.CR

TL;DR: RoguePrompt：一种有效绕过大型语言模型安全机制的新方法。


<details>
  <summary>Details</summary>
Motivation: 尽管存在审查和安全控制，但防止基于提示的政策规避仍然具有挑战性。

Method: 提出了一种名为RoguePrompt的越狱管道，该管道将禁止的提示分割并应用两层编码，即维吉尼亚密码和ROT13，以及自然语言重建说明。

Result: RoguePrompt在黑盒威胁模型下开发并评估，只有API或用户界面访问权限，并在313个实际拒绝的提示上进行测试。平均绕过过滤率为93.93%，重建率为79.02%，执行率为70.18%。

Conclusion: 这些结果表明了分层提示编码的有效性，并提供了多阶段越狱在审查绕过、指令重建和执行过程中的失败阶段的证据。
...
### [176] [Low-Latency Bootstrapping for CKKS using Roots of Unity](https://arxiv.org/abs/2607.27401)
...
### [177] [Granite: A Modular Methodology for Foundational Verification of Hardware-Software Leakage Contracts](https://arxiv.org/abs/2607.27480)
*Stella Lau,Andres Erbsen,Adam Chlipala*

Main category: cs.CR

TL;DR: Granite方法：通过循环级确定性规范机验证RTL处理器功能正确性和非泄露性，实现硬件和软件密码学实现的安全保障。


<details>
  <summary>Details</summary>
Motivation: 为了验证RTL处理器在ISA合约下的功能正确性和非泄露性，提出了一种名为Granite的方法。

Method: Granite方法通过循环级确定性规范机建立正确性和机密性之间的迹等价关系，并使用存在参数化处理秘密无关的非确定性。

Result: 该方法首次实现了指令集级泄露合约和微架构特定周期级执行之间的模块化和基础性联系，并通过认证静态分析消除了每个中间规范，包括ISA合约本身，从可信计算基础中。

Conclusion: Granite方法能够有效验证RTL处理器的功能正确性和非泄露性，并为硬件和软件密码学实现提供安全保障。
...
### [178] [Flock: Fast Proving for Batch Boolean Computations](https://arxiv.org/abs/2607.27491)
...
### [179] [Send and Pretend: Exploiting Transcript Consistency Issues in End-to-End Encrypted Group Chats](https://arxiv.org/abs/2607.27510)
*Gabriel K. Gegenhuber,Moritz Grefner,Maximilian Günther,Matthäus Wininger,David Schmidt,Aljosha Judmayer*

Main category: cs.CR

TL;DR: 研究发现E2EE群聊存在转录一致性问题和隐私风险，并提出了解决方案。


<details>
  <summary>Details</summary>
Motivation: 为了研究端到端加密（E2EE）即时通讯应用在群聊中的转录一致性（TC）问题。

Method: 通过系统性地调查恶意参与者威胁模型下的问题，并识别出多个歧义向量。

Result: 发现目前主流的E2EE即时通讯应用不能保证群聊的TC，存在隐私问题。

Conclusion: 提出了低开销的缓解措施和UI信号策略，可以集成到最先进的E2EE群组协议中。
...
### [180] [ThreatForest: Multi-Agent Attack Tree Generation with Pluggable TTP Framework Mapping](https://arxiv.org/abs/2607.27528)
...
### [181] [AnchorMark: Robust Diffusion Watermarking via Latent-Space Rotation Synchrony](https://arxiv.org/abs/2607.27551)
*Yuqi Qian,Yun Cao,Haocheng Fu,Haochen Zhao,Hong Zhang,Meineng Zhu*

Main category: cs.CR

TL;DR: AnchorMark is a robust watermarking method that improves accuracy under rotation and combined attacks.


<details>
  <summary>Details</summary>
Motivation: Existing watermarking methods are vulnerable to post-processing, particularly rotation.

Method: Introduce AnchorMark, a training-free, robust inversion-based watermarking.

Result: AnchorMark improves bit accuracy under rotation and combined attacks, with limited impact on image quality.

Conclusion: AnchorMark overcomes the limitations of existing methods by introducing the Rotation Synchrony property and a synchronization anchor in the latent space.
...
### [182] [Revisiting the Adversarial Robustness of Graph-Based Traffic Forecasting](https://arxiv.org/abs/2607.27604)
*Qingzhao Zhang*

Main category: cs.CR

TL;DR: 本研究提出了一种针对恶意传感器读数的鲁棒性检测-缓解防御方法，有效提高了智能交通系统中基于图的人工智能交通预测的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 智能交通系统中基于图的人工智能交通预测是关键组件，需要研究对恶意传感器读数的鲁棒性。

Method: 研究具有有限模型知识和仅能监控和操纵少数道路传感器的实际对手。将鲁棒性重新定义为检测问题，引入一个学习的物理信息检测器。

Result: 检测-缓解防御在对抗物理感知攻击的强化训练上提高了13/15的模型-数据集设置，在保留出的攻击上提高了最大幅度，几乎没有任何清洁成本。

Conclusion: 需要检查抽象的AI对抗攻击在特定应用约束下，以评估其真正的安全影响。
...
### [183] [Strategy Phasing of Cyber Attacks on Digital Substations](https://arxiv.org/abs/2607.27661)
*Akila Herath,Chen-Ching Liu,Junho Hong,Kuchan Park*

Main category: cs.CR

TL;DR: 提出了一种基于HMM的SubCASP方法，用于推断电力系统网络攻击的攻击阶段，提高入侵检测系统的性能。


<details>
  <summary>Details</summary>
Motivation: 现代电力系统中，符合IEC 61850标准的数字变电站提高了运营效率。然而，攻击者可以利用IEC 61850通信来操纵变电站的断路器操作，这可能对系统造成严重影响。现有的入侵检测系统（IDS）通常只标记孤立的症状，缺乏攻击阶段的上下文来支持部署缓解措施。

Method: 提出了一种基于隐藏马尔可夫模型（HMM）的方法，融合IDS数据日志来推断当前攻击阶段、下一个攻击阶段和回顾性攻击路径。攻击阶段来自基于ATT&CK的威胁模型。在可复制的攻击图数据集上训练和评估SubCASP模型。

Result: 测试结果表明，SubCASP在各种IDS可观察性和缺失IDS数据日志场景下具有鲁棒性。

Conclusion: SubCASP方法能够有效推断攻击阶段，为缓解电力系统中的网络攻击提供了一种新的思路。
...
### [184] [Distributed Point Functions and Function Secret Sharing](https://arxiv.org/abs/2607.27696)
*Elette Boyle,Niv Gilboa,Yuval Ishai,Peter Scholl*

Main category: cs.CR

TL;DR: 本文综述了DPF及其扩展FSS的定义、构造和应用，并讨论了其在密码学中的重要性。


<details>
  <summary>Details</summary>
Motivation: DPF的轻量级结构在密码学中具有广泛的应用潜力。

Method: 对DPF的定义、构造和应用进行综述，并讨论将其扩展到函数秘密共享（FSS）的方法。

Result: 为DPF及其扩展FSS提供了全面的概述，并讨论了其在各种应用中的潜力。

Conclusion: DPF及其扩展FSS是密码学中重要的研究课题，具有广泛的应用前景。
...
### [185] [CHARGE: Leveraging CWE Hierarchies for Hardware Security SystemVerilog Assertion Generation](https://arxiv.org/abs/2607.27776)
*Xiao Tan,Cynthia Sturton*

Main category: cs.CR

TL;DR: CHARGE是一种自动生成安全属性的框架，提高了检测未验证RTL模块中安全漏洞的准确性。


<details>
  <summary>Details</summary>
Motivation: 提高未验证RTL模块安全属性生成的准确性和效率，减少手动工程工作量。

Method: 使用CWEs和大型语言模型（LLMs）自动生成安全属性，利用CWE条目的层次结构进行推理，以识别未验证RTL模块中的安全关键资产。

Result: 在Hack@DAC21 OpenPiton SoC中，89%的生成SVAs在Cadence JasperGold FPV中运行，92.2%非空。

Conclusion: CHARGE在Hack@DAC18、19和21开源SoC设计中检测到了27个已知漏洞，并在Hack@DAC21 OpenPiton SoC中识别出一个新漏洞。
...
### [186] [Adaptive Security at the Edge for 6G-Enabled Healthcare IoT](https://arxiv.org/abs/2607.27858)
*Ijaz Ahmad,Ijaz Ahmad,Erkki Harjula*

Main category: cs.CR

TL;DR: NANOEDGEGUARD reduces alarm response time and traffic bursts, improving healthcare IoT resilience.


<details>
  <summary>Details</summary>
Motivation: Healthcare IoT services increasingly rely on edge gateways to relay routine telemetry and deliver time-critical alarms. However, short traffic bursts can cause delay and interfere with urgent messages.

Method: We present NANOEDGEGUARD, a kernel-plane closed-loop controller that observes per-source traffic intensity at the edge and enforces a multi-tier rate policy using in-kernel traffic-control hooks. We also evaluate the performance using a Raspberry Pi gateway and ESP32 endpoints.

Result: Adaptive kernel-plane rate control reduces the 99th-percentile alarm RTT by 13.3% compared to a user-space firewall baseline, while maintaining no-enforcement-level RTT. It also reduces excess admitted burst traffic by 46% compared to no enforcement.

Conclusion: Adaptive, auditable enforcement at the gateway can improve resilience for healthcare IoT and can be extended toward on-demand policy deployment in future edge intelligence.
...
### [187] [Don't Trust the AI Ecosystem: Analyzing Privacy Leakage in Compromised Open-Source Components](https://arxiv.org/abs/2607.27886)
*Jin-Seong Kim,Han-Ju Lee,Seok-Won Hong,Takeshi Takahashi,Chansu Han,Tomohiro Morikawa,Seok-Hwan Choi*

Main category: cs.CR

TL;DR: GradLock是一种新型的训练时间注入攻击，能够在不访问训练环境的情况下从模型中提取敏感数据。


<details>
  <summary>Details</summary>
Motivation: 为了克服现有模型反演攻击的局限性，特别是针对高维数据集，我们引入了一种名为GradLock的新型训练时间注入攻击。

Method: GradLock通过在受损害的供应链环境中利用无状态确定性索引建立隔离的数据保险库，并采用动态梯度锁定来防止在优化过程中的有效载荷退化。

Result: GradLock在MNIST、Imagenette和CelebA数据集上实现了几乎无损的重建（SSIM ~ 1.0）和瞬时提取（< 1.0秒）。与现有的训练时间注入方法相比，该方法在标准部署优化方面表现出更高的鲁棒性，包括量化、剪枝和微调。此外，用户部署研究显示93.3%的参与者未能检测到恶意逻辑，突显了现代人工智能供应链中的严重盲点。

Conclusion: GradLock是一种有效的模型反演攻击方法，能够从最终模型中提取像素级准确的数据，同时对标准部署优化具有高鲁棒性。
...
### [188] [Benign on Label, Malicious by Design: Clean-Label Dormant-to-Activated Backdoor via Machine Unlearning with Removable Camouflage](https://arxiv.org/abs/2607.27936)
...
### [189] [Safeguards Based on Copyable Context Cannot Provide Reliable Safety for LLMs](https://arxiv.org/abs/2607.27951)
...
### [190] [Driving up Inference Energy on SNNs: Per-Sample and Universal Sponge Attacks](https://arxiv.org/abs/2607.27990)
*Spyridon Raptis,Haralampos-G. Stratigopoulos*

Main category: cs.CR

TL;DR: SNNs 面临 sponge 攻击风险，可能导致能量消耗增加。


<details>
  <summary>Details</summary>
Motivation: Spiking Neural Networks (SNNs) 的能量高效性在边缘系统中得到应用，但也带来了安全风险。

Method: 研究 sponge 攻击，开发基于梯度的样本级 sponge 攻击和针对原生事件输入的通用 sponge 攻击。

Result: 攻击增加了 SynOps 并导致能量消耗增加，展示了原生事件输入 SNN 的漏洞。

Conclusion: 原生事件输入 SNN 易受输入空间效率攻击，通用扰动可能累积导致电池耗尽。
...
### [191] [Temporal Poisoning: Clean-Label Backdoors via Event Redistribution in SNNs](https://arxiv.org/abs/2607.28075)
*Roberto Riaño,Gorka Abad,Stjepan Picek,Aitor Urbieta*

Main category: cs.CR

TL;DR: 提出了一种针对SNNs的清洁标签后门攻击，评估了其隐蔽性和防御的局限性


<details>
  <summary>Details</summary>
Motivation: 研究Spiking Neural Networks (SNNs)的后门攻击，特别是清洁标签时间中毒攻击

Method: 在三个神经形态数据集和基于卷积和Transformer的受害者上测试攻击，分析攻击通过毒化预算和触发形状消融，评估针对SNNs的后门防御

Result: 攻击在最强配置下达到1.00的ASR，后门防御存在局限性，攻击具有隐蔽性

Conclusion: 提出了一种针对SNNs的清洁标签后门攻击，并评估了其隐蔽性和防御的局限性
...
### [192] [Checking Information Flow in Cloud-based IoT Access Control Policies (Extended Version)](https://arxiv.org/abs/2607.28088)
*Lorenzo Ceragioli,Letterio Galletta,Edoardo Lunati*

Main category: cs.CR

TL;DR: 研究物联网访问控制策略，以发现并验证潜在安全漏洞。


<details>
  <summary>Details</summary>
Motivation: 在设备信任级别不同或分散在各个子系统中，仅验证权限不足以确保安全。

Method: 形式化建模、信息流图、SMT求解器、工具开发（IOT:POKER）

Result: 发现设备间信息流的潜在安全漏洞，并通过IOT:POKER工具进行验证。

Conclusion: 该研究分析了物联网访问控制策略，以识别设备间潜在的安全漏洞。
...
### [193] [Agent Harness Distillation: Inference-Time Harness Extraction and Exploitation in Autonomous Multi-Agent Systems](https://arxiv.org/abs/2607.28147)
*Yu Cui,Wuli Yang,Yirui Shi,Junhao Xia,Hui Jiang,Lei Gao,Chenfu Bao*

Main category: cs.CR

TL;DR: 提出AHD框架，研究AMAS中推理时间协调器提取的安全风险，并提出基于欺骗的防御措施。


<details>
  <summary>Details</summary>
Motivation: 为了解决自主多智能体系统（AMAS）中推理时协调推理和动作的推理时间协调器构建的工程和计算资源需求，以及由此产生的知识产权（IP）泄露风险。

Method: 提出了一种名为“智能体协调器蒸馏”（AHD）的框架，用于研究AMAS中推理时间协调器提取产生的安全风险。该框架通过黑盒交互从目标智能体中提取推理时间协调器能力，并分为预蒸馏阶段和后蒸馏阶段。

Result: 实验结果表明，AHD在多个骨干语言模型上的有效性，并揭示了大量的IP泄露风险。此外，还提出了一种基于欺骗的防御措施，以降低协调器提取的有效性，同时保持受保护智能体的效用。

Conclusion: AHD框架有效地揭示了AMAS中推理时间协调器提取的安全风险，并提出了一种基于欺骗的防御措施，为AMAS的安全保障提供了新的思路。
...
### [194] [Piggybacking on Perception: Stealthy Concurrent Audio Prompt Injections against Multimodal LLM Agents](https://arxiv.org/abs/2607.28165)
*Mingxiao Liu,Yitong Li,Haoren Zhao,Yaoxiang Bian,Jianan Ma,Jian Zhang,Jialuo Chen,Xinhao Deng,Zhen Wang*

Main category: cs.CR

TL;DR: 研究音频提示注入攻击，提出CADV防御机制，有效提升多模态代理安全性。


<details>
  <summary>Details</summary>
Motivation: 研究针对多模态代理的并发音频提示注入攻击，解决环境噪声带来的安全问题。

Method: 提出新的指令增强和场景隐藏技术，构建AudioAgentSecurity基准，评估11个最先进的代理，引入CADV防御机制。

Result: 攻击成功率达到69.10%，CADV检测成功率超过90%。

Conclusion: 音频提示注入攻击对多模态代理构成严重威胁，CADV防御机制有效。
...
### [196] [Security of World-Model-Based Embodied AI: A Lifecycle of Threats, Defenses, and Evaluation](https://arxiv.org/abs/2607.28226)
...
<details>
  <summary>Details</summary>
Motivation: 分析世界模型在具身AI中的预测核心及其安全问题

Method: 生命周期调查，攻击家族分析，安全属性映射，评估协议和防御结构

Result: 揭示世界模型在安全性方面的挑战，提供生命周期分类法，映射攻击到安全属性，构建防御策略

Conclusion: 世界模型在增强AI预测能力的同时，也引入了新的安全风险，需要综合防御策略来保障其安全性。
...
### [197] [Implementing Homomorphic Encryption-Based Logic Locking in System-on-Chip Designs](https://arxiv.org/abs/2607.28542)
*Ye Ziyang,Makoto Ikeda*

Main category: cs.CR

TL;DR: 提出了一种基于二进制环学习错误算法的逻辑锁定方案，有效保护了关键逻辑路径，同时保持了良好的性能和资源利用率。


<details>
  <summary>Details</summary>
Motivation: 为了保护关键逻辑路径，在不暴露敏感参数的情况下，提出了基于二进制环学习错误算法的逻辑锁定方案。

Method: 在RISC-V系统芯片设计中实现二进制环学习错误算法，与传统的逻辑锁定方法不同，该方案在权限切换过程中保护关键逻辑路径，而不暴露这些敏感参数。

Result: 实现的锁定模块本身消耗3519查找表和2645寄存器，相对于基线系统，整体开销为6.0%的查找表和6.9%的寄存器。解锁过程需要约2.6微秒，引入了适度的性能影响，主要影响系统级操作，同时保持用户级计算效率。

Conclusion: 该逻辑锁定方案在保护关键逻辑路径的同时，保持了良好的性能和资源利用率。
...
### [198] [Formalization of security](https://arxiv.org/abs/2607.28551)
*Gilles Barthe*

Main category: cs.CR

TL;DR: 证明辅助工具在系统安全、语言安全、安全编译和密码学等领域有广泛应用


<details>
  <summary>Details</summary>
Motivation: 用于验证设计和实现符合预期安全属性，支持认证

Method: 未提及

Result: 关注系统安全、语言安全、安全编译和密码学应用

Conclusion: 证明辅助工具在系统安全、语言安全、安全编译和密码学等领域有广泛应用
...
TL;DR: MHAR通过引入多查询头部，优化了Transformer的注意力机制，显著提升了模型性能。


<details>
  <summary>Details</summary>
Motivation: Transformers的单一残差流和注意力残差在信息传播和模型宽度增加时面临挑战。

Method: 引入多头注意力残差（MHAR），每个子空间都有自己的softmax来处理深度历史。

Result: 在Nemotron-basedanneal语料库上训练的MHAR在100M、350M和1B模型大小上提高了验证损失，并且取得了最佳结果。

Conclusion: MHAR通过提高查询学习和解码效率，提高了Transformer的性能。
...
TL;DR: KernelGenBench评估了LLM生成的内核，发现代理方法更优，但成本高且性能差异大。


<details>
  <summary>Details</summary>
Motivation: 为了解决LLM内核生成的挑战，需要一个统一的基准来评估生成的内核。

Method: 使用KernelGenBench进行大规模评估，包括多来源和多芯片子基准测试。

Result: 代理方法优于LLM方法，性能在不同硬件平台上有显著差异，内核生成成本高昂。

Conclusion: KernelGenBench是一个统一的标准，用于评估LLM和代理生成的Triton内核。它显示代理方法优于LLM方法，性能在不同硬件平台上有显著差异，且内核生成成本高昂。
...
TL;DR: RadHarmony是一个用于放射学数据集的统一API库，提高了数据集整合和深度学习模型训练的效率。


<details>
  <summary>Details</summary>
Motivation: 整合不同来源、文件格式、目录布局、标签模式和注释类型的异构放射学数据集，以训练深度学习模型。

Method: 提出RadHarmony，一个开源Python库，提供统一的API用于加载、协调和增强放射学数据集，主要关注胸部X光片，并支持CT和MRI。

Result: RadHarmony将24个公共数据集的元数据标准化为单一表格格式，包装MONAI的映射式数据集，支持分类标签、分割掩码、边界框和放射学报告文本，并提供交互式可视化工具。

Conclusion: RadHarmony降低了集成新数据集的门槛，并通过预训练RadHarmony-ViT展示了其实用性。
...
TL;DR: 标准模型合并可能导致安全识别精度下降，而攻击抵抗能力提高。


<details>
  <summary>Details</summary>
Motivation: 为了研究标准合并方法是否能够同时保留多个安全相关行为，研究人员进行了控制案例研究。

Method: 使用两种在两个互补安全目标上微调的Gemma-3-1B-IT模型，通过线性、SLERP、TIES和DARE-TIES方法进行合并，并评估合并模型在分类精度、攻击抵抗和良性合规性方面的表现。

Result: 合并模型在攻击抵抗方面比分类精度有显著转移：合并模型保留81-85%的拒绝越狱率，而CARES的准确性下降至最多12.9%。

Conclusion: 当安全相关的任务向量在规模上差异很大时，标准模型合并可以将安全识别缩小到广泛的拒绝。
...
TL;DR: Divergence Decoding通过自适应推理时间协作，有效融合LLM能力，提高推理性能。


<details>
  <summary>Details</summary>
Motivation: 解决大型语言模型在推理中缺乏专业知识的问题，以及领域模型在专业知识上的局限性。

Method: 引入Divergence Decoding，一种训练免费的框架，用于能力融合。它将推测性解码的“草案-验证”结构重构为自适应路由机制。核心是使用Jensen-Shannon散度来监控两个模型在每个标记上的分布差异。当专家模型表现出显著差异时，该方法将其识别为潜在的推理风险，并立即将控制权交给通用模型。

Result: 在多个模型系列（Qwen和Llama系列）和科学基准（GPQA、ChemBench和ChemCoTBench）上评估Divergence Decoding。实验结果表明，Divergence Decoding优于领域专用和通用模型，有效超越了大多数单模型基线。这表明Divergence Decoding通过自适应推理时间协作，为融合不同LLM能力提供了一种通用的、训练免费的范式。

Conclusion: Divergence Decoding是一种有效的训练免费框架，可以融合不同LLM的能力，提高推理性能。
...
TL;DR: 本文提出了一种新的框架，用于在动态知识图谱上进行推理和规划，解决了信息不完整和可决性问题，并在评估中显示出优于传统规划器的性能。


<details>
  <summary>Details</summary>
Motivation: 在动态知识图谱（DKGs）上进行推理和规划，特别是在开放世界环境中，面临着重大挑战，因为信息不完整。现有的动作形式主义通常面临可决性问题，而通过结构归因管理不完整知识需要广泛的组合搜索。

Method: 本文介绍了一个统一的框架，包含两个集成模块——PIE-Abducer（增量直接推导归因）和PIE-APT（时间知识图谱的归因规划）——在高度表达的描述逻辑上本地操作。我们将状态转换建模为对演绎封闭的DL理论的非单调更新。将增量推理器视为黑盒，并在OWL中本地表示动作而不使用外部模态运算符，以保持逻辑可决性。为了解决不完整知识，PIE-Abducer绕过传统的最小命中集（MHS）枚举。它不是组合句法搜索，而是将目标目标的逻辑否定注入到一个一致的分支中，并通过直接反驳后果提取缺失的前提。然后，PIE-APT采用递归的“生成和测试”架构，将向后链A*搜索与PIE-Abducer交织，直到有限的因果深度，然后通过正向链时间投影进行严格验证。

Result: 评估了四个OWL基准，强调经典规划中缺失的语义能力：参数化目标与证人搜索、搜索过程中的DL蕴涵、开放世界假设注入和对抗性矛盾狩猎。结果表明，与经典规划器相比具有定性优势，并且在归因丰富期间，我们的直接推导方法在数量上优于一个MHS忠诚的基线。

Conclusion: 本文提出的框架和模块在处理动态知识图谱的推理和规划方面取得了显著成果，特别是在解决信息不完整和可决性问题方面。
...
TL;DR: PAUSE基准评估个人AI助手在现实服务环境中的行为，发现现有模型存在局限性。


<details>
  <summary>Details</summary>
Motivation: 为了评估个人人工智能助手在现实服务环境中的行为，研究人员提出了PAUSE基准。

Method: PAUSE基准要求代理在异构用户拥有的资源之间协调动作，同时保持与环境状态的一致性，以及在多轮交互中遵守授权约束。

Result: 基准结果表明，即使是最先进的私有模型在需要状态推理和配置意识的情况下，也未能达到70%的任务完成率。

Conclusion: PAUSE基准为评估个人人工智能助手在状态感知、服务集成环境中的行为提供了有价值的工具。
...
TL;DR: SkillMentor通过学习盲点诊断，显著提高了执行器的性能。


<details>
  <summary>Details</summary>
Motivation: 为了解决现有方法在发现代理未知领域的不足，提出了一种新的诊断方法。

Method: 通过强化学习训练导师策略，生成诊断任务，识别重复的故障模式，并创建可重用的纠正技能。

Result: 在AppWorld和BFCLv3上，SkillMentor将执行器性能平均提高了44.2%。

Conclusion: 盲点诊断是一种可学习的功能，可以在不更新执行器权重或依赖人工整理数据的情况下实现自我进化。
...
TL;DR: DLM在对抗攻击下表现出鲁棒性，但在自然噪声下存在鲁棒性问题，需要从迭代解码循环中根本性地提高鲁棒性


<details>
  <summary>Details</summary>
Motivation: 评估DLM在自然输入噪声和对抗攻击下的可靠性

Method: 使用参数匹配的对进行系统评估，包括自然扰动条件、对抗梯度探测和机制性隐藏状态分析

Result: 发现DLM在对抗攻击下具有鲁棒性，但在自然噪声下没有保证的防御，同时存在系统性的过度自信问题，且所有模型都完美地编码了输入损坏，导致解码器路由失败

Conclusion: DLM的鲁棒性不能通过表面修补来提高，必须根本性地集成到迭代解码循环中
...
TL;DR: HLE的领域子分数不能准确反映不同能力，区分模型的能力有限


<details>
  <summary>Details</summary>
Motivation: 评估前沿语言模型的能力

Method: 在HLE的文本-only选择题子集上评估29个LLMs，并应用心理测量方法评估基准的维度和测量精度的分布

Result: HLE衡量的是单一的一般推理因素，领域标签只解释了3.5%的项目反应方差，领域内和领域间的残差相关性几乎相同，领域特定的能力估计与总分几乎冗余

Conclusion: HLE的领域子分数不应对不同的能力进行解释，基准在区分最强大模型方面的能力有限
...
TL;DR: 提出了一种新的LLM代理框架，通过预先诊断潜在动作错误来提高长时程任务的成功率。


<details>
  <summary>Details</summary>
Motivation: LLM代理在长时程交互任务中存在困难，需要有效的方法来避免代价高昂的错误。

Method: 通过将历史轨迹建模为概率图，并使用图神经网络识别导致失败的连续动作模式，从而在动作执行前诊断潜在动作错误。

Result: 轨迹图协同框架能够提供早期警告，避免错误动作，显著提高代理完成长时程任务的成功率。

Conclusion: 提出的轨迹图协同框架显著提高了LLM代理在长时程任务中的成功率，平均通过率提高14.69%.
...
TL;DR: VAmoS Bench是一个用于评估语音代理系统的平台，专注于金融服务，支持排行榜和任务扩展。


<details>
  <summary>Details</summary>
Motivation: 为了解决语音代理评估中存在的差距，我们引入了VAmoS Bench，这是一个语音代理模拟平台。

Method: VAmoS Bench通过在状态化的客户支持任务中端到端测量完整的语音代理系统。

Result: 该平台使用100个场景，每个场景都为模拟呼叫者提供私人目标和后端数据库。

Conclusion: VAmoS Bench首次关注金融服务，支持不断发展的排行榜，允许在相同版本上评估更多语音代理，并允许后续版本扩展任务和场景。
...
TL;DR: INCLAIR是一种新的框架，用于检测纵向临床资料中的异常，并在临床数据集上表现出色。


<details>
  <summary>Details</summary>
Motivation: 检测纵向临床资料中的异常对于临床诊断非常重要，但也很困难。异常证据通常很少，患者病史长度不一，专家解释成本高昂。

Method: 提出了一种名为INCLAIR的框架，该框架将每个观察结果与多个历史背景进行比较，在有限专家监督下生成基于事实的自然语言解释。

Result: 在三个临床数据集上，INCLAIR在性能上始终优于最先进的基线。通过一项关于纵向类固醇轮廓的案例研究，验证了INCLAIR在有限专家监督下的临床实用性。

Conclusion: INCLAIR能够实现有限专家监督下的临床可操作的异常检测。
...
TL;DR: MedLLM是一个高效的医疗语言模型，在压缩参数下仍能保持良好的性能


<details>
  <summary>Details</summary>
Motivation: 对现有医疗语言模型规模的反思和探索

Method: 开发了一个名为MedLLM的0.1B参数医疗语言模型，通过三个阶段的开放流程进行训练：通用预训练、领域微调和偏好对齐微调

Result: MedLLM在多个医疗基准测试中表现出色，特别是在知识回忆任务上，其性能优于7B参数模型和所有子7B基线模型

Conclusion: 医疗语言模型的能力在压缩下并非均匀下降，而是根据任务类型而异
...
TL;DR: 研究评估了大型语言模型在处理概念性问题上的能力，提出了一种新的评估方法，并评估了一系列模型。


<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型在处理具有可验证答案的任务上的快速进步，但对其处理概念性问题的能力知之甚少。

Method: 创建了一个包含951个论证批评的语料库，并对442个立场文本进行了评估。使用六个专家的1458个评级来评估中心性、强度、正确性和清晰度。

Result: 提出了两个评分函数，并评估了一系列模型。性能与一般能力排名一致。

Conclusion: 虽然对这类问题的最终结论难以评估，但可以通过评估个体语境化的论证来取得进展。
...
TL;DR: 使用AI扫描器检测基准的有效性问题，发现了一些潜在问题，但存在一些挑战。


<details>
  <summary>Details</summary>
Motivation: 为了确保代理基准的有效性，对基准进行准确的测量，并确保其不受无效缺陷的影响。

Method: 开发AI扫描器来检测四种类型的有效性问题：真实情况访问、工具故障、猜测漏洞和答案格式歧义。

Result: 扫描器在五个广泛使用的基准中识别出多个验证的质量问题，包括随机手动检查难以捕捉的情况。然而，并非所有情况都被识别出来，扫描器的性能在基准、标准和模型之间差异很大。

Conclusion: 这些结果证明了使用自动化转录分析来更广泛地审计基准质量的概念。
...
TL;DR: 本文提出了一种新的AI素养框架，强调批判性和参与性。


<details>
  <summary>Details</summary>
Motivation: 分析现有AI素养框架的局限性，提出新的AI素养概念。

Method: 基于福柯的权力-知识理论、弗莱雷的批判性意识教学法以及数字素养研究。

Result: 提出以情境使用、批判性质疑和参与式治理为基础的AI素养框架。

Conclusion: 强调AI素养应培养知识型“行动者”，而非仅仅是AI信息消费者的训练。
...
TL;DR: AI在生成新产品创意方面表现出色，尤其在产生高质量创意方面具有优势，尽管存在新颖性较低的缺点。
...
Method: 比较了针对大学生且价格在50美元以下的三组新产品创意。第一组由产品设计课程的学生在LLM出现之前创建，第二组和第三组则分别由OpenAI的GPT-4使用零样本和少量样本提示生成。使用标准市场研究技术评估创意质量，通过预测平均购买意图概率。利用文本挖掘评估创意相似性，由人工评分员评估创意的新颖性。

Result: AI生成的创意在平均购买意图方面优于人类生成的创意，少量样本提示的意图略高于零样本提示。然而，AI生成的创意被认为新颖性较低，成对相似度较高，特别是使用少量样本提示时，表明解决方案的多样性较低。但AI生成的创意更有可能进入最佳10%，表明与人类创意相比具有显著优势。

Conclusion: 尽管存在一些缺点，AI的创造力在生成新产品开发的高质量创意方面仍然具有显著优势。
...
TL;DR: 通过引入FEV框架，我们提出了一个评估代理生物信息学的方案，并映射了多个相关系统和资源。


<details>
  <summary>Details</summary>
Motivation: 评估代理生物信息学，并联合操作代理生成工作流程的责任。

Method: 通过引入FEV框架，对109个代理或代理相关系统和28个基准或评估资源进行映射，涉及128篇独特出版物。

Result: FEV框架在基因组学、单细胞和空间组学、蛋白质科学、药物发现、计算病理学和通用生物信息学自动化等领域，为比较系统和设计科学负责任的生物信息学工作流程提供了基础。

Conclusion: 通过将可检查的工作流程轨迹视为分析的主要单元，我们引入了功能-证据-验证（FEV）框架，用于比较系统和设计透明、可审计且科学负责任的生物信息学工作流程。
...
TL;DR: DeepResearch Agent系统是一个基于稀疏激活架构的大型语言模型系统，用于深度信息检索、多步骤推理和自主研究任务。


<details>
  <summary>Details</summary>
Motivation: 为了实现深度信息检索、多步骤推理和自主研究任务，研究人员开发了DeepResearch Agent系统。

Method: 该系统基于稀疏激活架构，拥有30亿参数，其中每项激活3亿个参数。系统支持128K个令牌的上下文窗口，并采用分层注意力机制。它还包含一个双模式推理引擎和强化学习优化框架。

Result: 系统在多个基准测试中取得了最先进的性能，包括在Humanity's Last Exam、BrowserComp Chinese和WebWalkerQA上的表现。

Conclusion: DeepResearch Agent系统是一个强大的研究工具，适用于学术研究、商业分析、研发支持和教育等领域。
...
TL;DR: This paper proposes an operational definition and shared vocabulary for prompt graph engineering, applying it to real systems with consistent results.


<details>
  <summary>Details</summary>
Motivation: The vocabulary for prompt graphs is lagging behind and lacks a reference definition.

Method: Conceptual analysis over sources with persistent identifiers, complemented by primary grey literature. Reconstruction of the genealogy of the idea from dataflow graphs and build systems to graphs compiled and optimized as artifacts. Proposing a constitutive definition of prompt graph engineering and applying it to real systems.

Result: An operational definition and a shared vocabulary for prompt graph engineering are proposed, and the definition is applied to real systems with consistent inclusion and exclusion.

Conclusion: The contribution is an operational definition and a shared vocabulary for a practice that industry already exercises daily without naming precisely.
...
### [222] [Old Tricks, New Models: How Simple Image Transformations Break Modern AI-based Content Moderation](https://arxiv.org/abs/2607.28187)
*Marco Alecci,Francesco Marchiori,Iyiola Emmanuel Olatunji,Tegawendé F. Bissyandé,Jacques Klein*

Main category: cs.AI

TL;DR: 基于基础模型的API并不能提供可靠的安全边界。


<details>
  <summary>Details</summary>
Motivation: 评估基于大型基础模型的商业多模态审查API是否能提供更鲁棒图像审查。

Method: 对三个成熟的商业图像审查服务进行了大规模的黑盒评估，并在多个提供商、数据集、伤害类别、感知相似性约束和变换强度下比较了它们的鲁棒性。

Result: 发现所有三种商业服务都可以通过低成本的图像变换来绕过，这些变换不需要梯度、代理模型或对目标系统的了解；即使是固定的变换，如颜色反转和灰度转换，也会在保留对人类仍可识别的内容的同时，引发不安全到安全的决策变化；它们的鲁棒性在数据集和伤害类别之间差异很大，多模态内容和自我伤害表现出明显的漏洞。

Conclusion: 以基础模型为基础的API本身并不能提供可靠的安全边界。这些系统必须在现实变换下进行评估，并作为分层审查流程的一部分进行部署，而不是作为独立的安全过滤器。
...
### [223] [From Minds to Models: The Intersection of Psychology and LLM Behaviours](https://arxiv.org/abs/2607.27579)
...
### [224] [Wiring diagram extraction and gluing: a case study in classifying figure skating jumps using 3D dataset](https://arxiv.org/abs/2607.27598)
*Jason Lo,Mohammadnima Jafari*

Main category: cs.AI

TL;DR: 提出了一种改进的Hasse聚类算法，通过连接线路图理论实现高效聚类


<details>
  <summary>Details</summary>
Motivation: 为了解决随着预期聚类数量增长，Hasse聚类算法因组合复杂性而变得不可行的难题

Method: 提出了一种理论，即连接线路图，通过迭代应用Hasse聚类以实现与单次应用相同的结果

Result: 在花样滑冰跳跃视频分类的背景下测试了该理论

Conclusion: 该理论有效地提高了Hasse聚类算法在处理大数据量时的效率
...
### [225] [World Action Planner: Generalizable Decision-Making with Action-Conditioned World Models](https://arxiv.org/abs/2607.27599)
*Xiangcheng Zhang,Yilun Du*

Main category: cs.AI

TL;DR: World Action Planner is a new robot planning system that improves generalization of agents across various tasks and environments.
...
### [226] [CORE: In-Context Reconstruction for Unified Tabular Anomaly Detection](https://arxiv.org/abs/2607.27615)
*Yunfeng Zhao,Qingfeng Chen,Yue Tan,Shiyuan Li,Yili Wang,Yixin Liu,Shirui Pan*

Main category: cs.AI

TL;DR: 提出了一种新的基于上下文重建的统一TAD方法，通过保留特征语义信息并直接重建样本，有效解决了异构数据对齐和异常检测的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有的统一TAD方法在异构数据对齐和异常检测任务中存在挑战，如特征语义模糊化和异常模式多样性忽视。

Method: 基于上下文重建的方法，使用 decorrelated feature alignment 模块和上下文重建模块。

Result: 提出的CORE方法能够有效识别异构数据集中的异常，且无需依赖标注或合成的异常数据。

Conclusion: 我们提出了一种名为CORE的基于上下文重建的方法，用于统一的表格异常检测（TAD）。它通过 decorrelated feature alignment 模块直接将异构特征对齐到统一的表示空间，同时将统一的TAD定义为上下文重建问题，从而消除了对标注或合成的异常数据的需要。
...
TL;DR: The paper introduces a new method to analyze language models, showing improved performance in various metrics.
...
Result: Shared has the lowest held-out description length, with gains of 0.216 nats on Qwen2.5-1.5B and 0.294 nats on Llama-3-8B, while maintaining tightly clustered mean future-signature distortion. The figure-aligned transplantation analysis gives Shared the strongest joint target-correctness, locality, copy-preservation, and composite profile, and API-aligned paths mediate 0.749 of the target effect versus 0.150 for matched null paths. 14/16 architectures are recovered in the blind four-class model-organism test, with one observed non-Shared to Shared error among 12 non-Shared organisms.
...
TL;DR: ReDiPPO enhances mathematical reasoning in large language models.


<details>
  <summary>Details</summary>
Motivation: Existing policy optimization methods like PPO struggle with reliable token-level credit assignment in mathematical reasoning tasks.

Method: ReDiPPO: a Reference-guided and Discrepancy-aware PPO framework for mathematical reasoning

Result: ReDiPPO introduces a reference-guided critic and uses token-level reference-standard discrepancy to reweight advantages during PPO optimization, demonstrating improved performance.

Conclusion: ReDiPPO improves value-estimation accuracy and consistently outperforms strong policy optimization baselines in final reasoning performance.
...
### [231] [HALO: Heterogeneous Admission through Localized Obligations for Safe Agentic Execution](https://arxiv.org/abs/2607.27636)
*Taewoo Park,Kyeonghyun Yoo,Kiseok Kim,Seunghyun Yoo,Hwangnam Kim*

Main category: cs.AI

TL;DR: HALO是一种运行时协议，用于处理AI系统异构响应，提高了响应的准确性和完整性。


<details>
  <summary>Details</summary>
Motivation: 为了解决现有AI系统响应异构性的问题，避免丢弃有用组件或遗漏必要前提，

Method: 提出了一种名为HALO的运行时协议，该协议能够保留有支持性的组件，并在调度前重新检查每个精确动作，同时允许用新候选者替换被阻塞的动作。

Result: HALO在所有96个接入期望和20个协议测试中均通过，并在结构化响应回放中保留了所有248个有支持性的组件，而在全响应策略中只保留了0个。在十个PX4/Gazebo会话中，HALO阻止了所有测试的过时路由，没有观察到匹配的过时设定点，并完成了所有新鲜恢复。

Conclusion: HALO是一种有效的协议，可以有效地处理AI系统的异构响应，提高响应的准确性和完整性。
...
### [232] [Evaluating and Pricing Advertisements in AI-Generated Responses](https://arxiv.org/abs/2607.27686)
*John L. Turner-Smith,Zimeng Huang,Yuhan Fu,Yihang Zhang,Tonghan Wang*

Main category: cs.AI

TL;DR: 本研究提出了一种新的评估器，用于预测点击意图和广告质量，并建立了基于该评估器的广告定价机制。


<details>
  <summary>Details</summary>
Motivation: 随着搜索向LLM驱动的答案引擎转变，广告开始嵌入到生成的响应中，因此应该对其用户效用和商业价值进行评估。

Method: 我们构建了一个基于心理基础的代理模拟框架来提供缺失的监督，并将其提炼成一个高效的评估器，该评估器可以预测点击意图，以及广告质量的三个相关维度，作为平滑、可微的估计。

Result: 评估器在相关性敏感性方面超过了前沿的无监督评判者（79%比60-67%），跟踪了内容退化的等级，错误地泛化到103个虚构产品，并在五个注释者之间86%的双边判断中与人类偏好一致。

Conclusion: 我们直接在估计上构建了定价层，得出了一种独特的支付规则，在该规则下，真实出价是最优的，并在最佳分配上证明了这一点，并将该机制扩展到非单调分配。
...
### [233] [Rehearse: Stepping Back from the Confidence Cliff in Self-Improving Autoresearch](https://arxiv.org/abs/2607.27687)
*Jiazhen Ji,Shouhong Ding*

Main category: cs.AI

TL;DR: Rehearse通过改进决策过程，有效提高了机器学习代码的效率


<details>
  <summary>Details</summary>
Motivation: 提高机器学习代码的效率

Method: 提出修改、运行完整训练任务、保留改进指标的修改

Result: 提出Rehearse方法，通过提出多个想法、比较它们、运行最有希望的，并使用类似过去尝试和结果的专注记忆进行判断，提高后期选择性准确性至83.5%，在相同的训练运行预算下，在nanochat、图像分类和时间序列预测上提高终点指标

Conclusion: Rehearse方法有效提高了机器学习代码的效率
...
### [234] [Guiding Large Language Models with Genetic Programming-Evolved Heuristic Knowledge for Dynamic Multi-Mode Project Scheduling](https://arxiv.org/abs/2607.27698)
*Yuan Tian,Yi Mei,Mengjie Zhang*

Main category: cs.AI

TL;DR: GP-evolved rules can effectively improve LLM decision-making in dynamic multi-mode project scheduling by guiding LLM with knowledge extracted from high-quality GP rules.


<details>
  <summary>Details</summary>
Motivation: 在动态多模式项目调度中，活动具有替代执行模式和不确定的持续时间，而优先级关系和有限资源限制了它们的执行。启发式优先规则支持快速在线决策，但其设计需要大量的领域专业知识。遗传编程（GP）超启发式规则可以自动进化此类规则。大型语言模型（LLM）同时提供了一个灵活的接口来解释调度信息和解释决策。然而，零样本LLM决策可能缺乏领域知识，消耗许多令牌，并且在重复查询中变化。因此，GP进化的规则为LLM决策提供了一种潜在的调度知识来源。与现有使用LLM支持启发式进化的LLM-GP混合体不同，我们采用反向知识转移方法，使用从高质量GP规则中提取的知识来指导在线LLM决策者。我们从高质量GP规则中提取知识，并通过特征选择、特征提示、规则引用和规则遵循等机制注入知识。这些机制在调度性能、令牌消耗、决策稳定性和生成理由中表达的特征关注方面进行了评估。GP派生的指导通常可以提高未指导的LLM，但其表示方式很重要。简化决策环境或提供明确的决策逻辑比突出重要特征更有效。特征选择提供了最佳的令牌效率，而规则遵循在更高的令牌成本下实现了强大的性能。指导还提高了决策稳定性和改变了生成理由中表达的特征。

Method: 使用遗传编程（GP）超启发式规则自动进化启发式优先规则，并通过特征选择、特征提示、规则引用和规则遵循等机制将知识注入到大型语言模型（LLM）中，以指导在线决策。评估了调度性能、令牌消耗、决策稳定性和生成理由中的特征关注。

Result: GP派生的指导通常提高了未指导的LLM，但表示方式很重要。简化决策环境或提供明确的决策逻辑比突出重要特征更有效。特征选择提供了最佳的令牌效率，而规则遵循在更高的令牌成本下实现了强大的性能。指导还提高了决策稳定性和改变了生成理由中表达的特征。

Conclusion: GP-evolved rules为LLM决策提供了一种潜在的调度知识来源，通过将知识注入LLM，可以有效地提高调度性能和决策稳定性。
...
### [235] [SpatialCLI: Learning to Reason With Spatial Tools, Then Without Them](https://arxiv.org/abs/2607.27703)
...
TL;DR: SpatialCLI是一种提高VLM视觉感知能力的框架，在基准测试中表现出色。


<details>
  <summary>Details</summary>
Motivation: 解决VLM在处理视觉细节和任务决策方面的能力不匹配问题。

Method: 提出SpatialCLI框架，包含调用、学习和内部化三个阶段，利用专业视觉模型和强化学习技术。

Result: 在MindCube基准测试中，SpatialCLI使Qwen3-VL-8B-Instruct的准确率从29.3%提高到84.6%，内部化后工具使用率降至73.8%。

Conclusion: SpatialCLI显著提高了VLM在视觉感知任务中的表现。
...
### [236] [Albilich: Steerable Proof-State Orchestration for LLM-Based Mathematical Research with CAS Integration](https://arxiv.org/abs/2607.27705)
*Ting Gong,Michael Ruofan Zeng,Yong Yang*

Main category: cs.AI

TL;DR: Albilich是一种用于数学研究的开源自动化工具，通过结合长周期推理、CAS和文献检索，提高了数学证明的效率和可扩展性。


<details>
  <summary>Details</summary>
Motivation: 为了解决长周期数学证明的协调、评估和复现困难，以及利用大型语言模型在数学研究中的应用。

Method: 提出了一种名为Albilich的开源自动化研究工具，结合了长周期推理、计算机代数系统（CAS）、文献检索和基于SQLite的持久性上下文管理。

Result: 在RealMath基准测试中，Albilich使用CAS解决了10/10的问题，不使用CAS解决了9/10的问题。在Kourovka Notebook中的群论开放问题中，Albilich产生了对问题21.142的反例和对问题20.2的加强证明。当启用CAS时，对问题17.91的消融实验表明，标记减少了32.0%。在缺少顾问代理的情况下，对问题21.142的消融实验表明，验证器拒绝率更高，并且无法合成证明路径。

Conclusion: Albilich被证明是一个可由人类控制、CAS增强的、可扩展的AI辅助数学研究环境。
...
### [237] [MECA: A Mechanism-Centered Agent for Constructing Well-Specified and Valuable Mathematical Conjectures](https://arxiv.org/abs/2607.27709)
*Wentao Long,Yunfei Zhang,Chenyi Li,Zaiwen Wen*

Main category: cs.AI

TL;DR: MECA是一种用于构建数学猜想的多智能体框架，在两个互补的设置中表现出色。


<details>
  <summary>Details</summary>
Motivation: 自动构建明确且有价值的数学猜想是人工智能辅助数学发现中的核心挑战。

Method: 提出了一种名为MECA的多智能体框架，通过联合开发候选陈述及其支持机制来构建猜想。

Result: MECA在两个互补的设置中进行了评估，结果表明，以机制为中心的细化产生了明确且有研究价值的猜想，这对当前的自动证明器来说仍然具有挑战性。

Conclusion: 机制中心的猜想构建方法在人工智能辅助数学发现中具有潜力。
...
### [238] [New Synchronous Computation Dynamics for Hopfield Networks](https://arxiv.org/abs/2607.27720)
*Francisco Requena-Domínguez,Rafaela Benítez-Rochel,Ezequiel López-Rubio*

Main category: cs.AI

TL;DR: 提出了一种新的同步动力学SD-DDF，用于加速Hopfield网络的能量下降过程。


<details>
  <summary>Details</summary>
Motivation: 减少处理时间，同时确保过程收敛和最大化每步能量减少，从而保证最短的总处理时间。

Method: 提出了一种新的工具和新的同步动力学，称为离散微分滤波器（DDF）和基于DDF的同步动力学（SD-DDF）。

Result: 通过理论证明和四个计算实验，证明了新方法在处理时间上的加速效果。

Conclusion: 新方法SD-DDF能够有效减少Hopfield网络的处理时间，同时保证过程收敛。
...
### [239] [Baikal: Structured Search for Deep Research over Data Lakes](https://arxiv.org/abs/2607.27726)
...
TL;DR: Baikal框架通过结构化语义探索，显著提高了数据湖中深度研究的效果。


<details>
  <summary>Details</summary>
Motivation: 为了解决现有方法在数据湖中进行深度研究时存在的局限性，例如过度利用局部证据和未能覆盖不同语义区域的问题。

Method: 将数据湖的深度研究视为一个预算搜索问题，并提出了一种名为Baikal的框架。该框架将异构证据聚类成语义区域，并对其自适应搜索以平衡探索和利用。在选定的区域中，Baikal生成并调查基于区域的子问题，使用发现质量作为奖励来更新区域级别的价值估计并指导搜索。

Result: 在HybridQA和TAT-QA数据湖上对Baikal进行了评估，这两个数据湖分别包含10,993和2,757个表，以及227K篇维基百科文章和13K篇财务报告文章。评估研究质量的新标准涵盖了扎根性、相关性、多样性和实用性，并使用GPT-5-mini对Baikal和强基线进行了评分，包括DeepSearcher和一个具有检索和聚类变体的OpenCode研究代理。在两个数据湖上，Baikal在几种区域选择策略下表现良好；其最佳配置在HybridQA上提高了报告评分28%，在TAT-QA上提高了36%。分析认为，这些收益归因于组织和探索语义证据区域，这提高了扎根性和多样性，并在相同的子问题预算下产生了更有用的发现。这些结果表明，结构化语义探索对于在异构数据湖中进行系统研究和发现的价值。

Conclusion: Baikal框架有效地解决了数据湖中深度研究的挑战，通过组织语义证据区域和平衡探索与利用，显著提高了研究质量。
...
### [240] [VeriSkill: A Self-Evolution Framework for Program Verification Skills](https://arxiv.org/abs/2607.27733)
*Changguo Jia,Tianqi Zhao,Zhiyou Xiao,Weiming Zhang,Minghui Zhou*

Main category: cs.AI

TL;DR: VeriSkill: A self-evolution framework for effective program verification
...
Method: Proposes VeriSkill, a self-evolution framework for program verification that attributes verification failures to skill deficiencies, distills diagnostic signatures into reusable lessons, and iteratively refines candidate skills.
...
Conclusion: VeriSkill is an effective self-evolution framework for program verification, outperforming all baselines in experiments.
...
### [241] [SpecCal: Ambiguity-Aware Candidate Calibration for Infrared Spectrum-Based Molecular Structure Reconstruction](https://arxiv.org/abs/2607.27788)
*Yixuan Chen,Bo Liu,Yusen Tan,Guokun Yang,Wenjie Du,Jun Xia*

Main category: cs.AI

TL;DR: SpecCal框架通过光谱一致性改进红外光谱到分子的预测


<details>
  <summary>Details</summary>
Motivation: 传统红外光谱到分子的模型无法充分捕捉满足光谱约束的最佳结构

Method: 提出SpecCal，一个无训练的候选校准框架，通过重排当前候选者并引入基于光谱一致性的结构上合理的替代方案来改进预测集

Result: 在多个基准上实验表明，SpecCal在SMILES和骨架水平上持续提高不同基础模型的top-k重建

Conclusion: 通过光谱模糊性校准候选集是一种提高分子从红外光谱重建的实用方法
...
### [242] [Annotating Topical Legal Insights from Case Proceedings](https://arxiv.org/abs/2607.27792)
*Subinay Adhikary,Dwaipayan Roy,Debasis Ganguly,Shouvik Kumar Guha,Kripabandhu Ghosh*

Main category: cs.AI

TL;DR: 本文提出LeDA系统，通过标注法律文档中的概念，提高法律文档处理能力，并在印度最高法院案例中成功应用。


<details>
  <summary>Details</summary>
Motivation: 为了从法律案例中找到概念或主题，我们提出了一种结构化的法律文档表示方法，以增强处理能力。

Method: 提出了一种名为LeDA的系统，该系统通过基于Web的界面提供对文档中实体或概念的标注和裁决的通用功能。该系统允许动态创建新的标签进行标注，特别适用于没有预定义本体的情况。

Result: LeDA系统用于标注法律文档中的概念，构建语义表示，用于下游任务如先例检索、判决预测等。

Conclusion: LeDA系统在标注和裁决印度最高法院案例中的法律概念名称方面表现出色。
...
### [243] [MemeBench: What LVLMs Miss When Interpreting Culture-Dependent Memes](https://arxiv.org/abs/2607.27798)
...
TL;DR: MemeBench是一个用于评估视觉-语言模型解释依赖于文化背景的视觉内容能力的诊断基准。


<details>
  <summary>Details</summary>
Motivation: 大型视觉-语言模型在描述视觉内容方面取得了进步，但准确描述并不能保证当意义取决于像素之外的知识时进行解释。Meme（模因）揭示了这一差距，因为它们依赖于文化实体、背景知识和社区惯例。大多数模因基准将解释简化为标签或整体评分，掩盖了解释失败的地方。

Method: 提出MemeBench，一个包含1,253个中英文模因的诊断基准，具有人类编写的参考和经过质量控制的VIKR注释，重点关注动漫、漫画、游戏和相关在线亚文化。其VIKR模式将解释分解为视觉线索、身份链接、知识单元和推理机制。在26个LVLM中，每个模型都比解释所需的知识更可靠地涵盖了可见内容，即使是最强的模型也保留了22.6%的视觉-知识差距。为了测试这种诊断是否可以指导改进，我们引入了KAR，这是一个基于CultureBase的实体引导检索基线。在四个受控模型中，KAR将VIKR成功率提高了3.6-7.4%，与通用检索相比，修复了更多答案，打破了更少的答案。然而，在每次比较中，这两种检索条件都改善了身份和知识，同时减少了视觉覆盖范围。

Result: MemeBench揭示了解释是否成功，缺少了什么，以及是否有针对性的证据填补了诊断的差距。

Conclusion: MemeBench为视觉-语言模型提供了一个诊断工具，以评估它们在解释依赖于文化背景的视觉内容方面的能力。
...
### [244] [STEREODISCO: Discovering Stereotypicality in LLMs](https://arxiv.org/abs/2607.27824)
*Farane Jalali Farahani,Corina Dima,Mojtaba Nayyeri,Raphael H. Heiberger,Steffen Staab*

Main category: cs.AI

TL;DR: STEREODISCO框架识别LLM内部表征中的刻板印象
...
Result: LLMs agree on social group ratings, stereotypes diverge from social psychology, new stereotypical axes identified

Conclusion: STEREODISCO can effectively identify stereotypes in LLM internal representations
...
### [245] [Sign Language Question Answering: A New Task, Benchmark, and Baseline for Sign Language Understanding](https://arxiv.org/abs/2607.27826)
*Shiwei Gan,Lichen Wang,Xiao Liu,Yafeng Yin,Kuizhuang Liu,Sanglu Lu,Lei Xie*

Main category: cs.AI

TL;DR: 提出SLQA任务和基准，评估手语理解能力，提高模型语义理解


<details>
  <summary>Details</summary>
Motivation: 为了解决现有手语理解（SLU）任务中模型的语义理解能力评估有限的问题

Method: 提出新的手语问答（SLQA）任务，构建基于PHOENIX14T和CSL-Daily的手语问答基准，并提出一个简单有效的基线模型

Result: 基线模型在所有问题类别上优于代表性视觉语言模型，为SLQA研究提供了强大的基准

Conclusion: SLQA任务和基准为评估手语理解提供了更灵活和全面的框架，有助于提高模型的语义理解能力
...
### [246] [MemTxn: A Transaction Boundary for Source-Supported Updates and Complete-State Recovery in Agent Memory](https://arxiv.org/abs/2607.27834)
*Hanshuai Cui,Zhiqing Tang,Zhi Yao,Fanshuai Meng,Qianli Ma,Weijia Jia*

Main category: cs.AI

TL;DR: MemTxn通过验证更新、选择版本和恢复状态，提高了大型语言模型代理的可信度和效率。


<details>
  <summary>Details</summary>
Motivation: 为了解决在长运行的大型语言模型代理中，可写内存中的错误可能会持续并破坏未来行为的问题。

Method: 提出了MemTxn，一个位于答案模型之外的管理层。MemTxn验证更新是否得到源的支持。它还在事实冲突时选择可见版本，并在故障后恢复应用程序可见的状态。系统使用有序补丁测试来验证写入，使用时间解析器来选择版本，并使用持久快照日志来恢复状态。

Result: 在项目无关的审计中，MemTxn接受所有60个支持的原始版本，拒绝所有179个硬否定。在LongMemEval-S和LoCoMo状态上的持久多键故障下，MemTxn在不了解实际物理写入集的情况下，恢复了完整的声明活动图。在MemoryAgentBench FactConsolidation上，MemTxn在所有十二个答案模型配置中实现了最高的平均F1。在五个代表性设置中，它比密集型高出17.06-24.07点。

Conclusion: MemTxn提高了大型语言模型代理的可信度和效率。
...
### [247] [Crossing the Margin Cliff: Toward Relearn-Robust LLM Unlearning via Margin Calibration](https://arxiv.org/abs/2607.27836)
*Xiangyu Yin,Jiaxu Liu,Zhen Chen,Chih-Hong Cheng*

Main category: cs.AI

TL;DR: 通过Margin Calibration (MC)，可以有效地提高大型语言模型在重学攻击下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 解决大型语言模型在重学攻击下的脆弱性问题。

Method: 在保留参考的每个令牌边缘上添加非饱和边缘铰链以及在非重叠指令语料库上的KL探针，以恢复遗忘方面的压力。

Result: Margin Calibration (MC) 提高了重学边界的攻击预算上限，并且在多个实验中取得了显著的效果。

Conclusion: 通过在保留参考的每个令牌边缘上添加非饱和边缘铰链以及在非重叠指令语料库上的KL探针，Margin Calibration (MC) 可以恢复遗忘方面的压力，从而提高重学边界的攻击预算上限。
...
### [248] [Virtual Process Dossier: A Process-Aware Data Catalogue](https://arxiv.org/abs/2607.27840)
...
TL;DR: VPD是一种基于知识图谱的数据目录，用于多阶段制造场景中的数据管理和溯源。


<details>
  <summary>Details</summary>
Motivation: 为了解决多阶段制造场景中下游人工智能优化任务需要区分不同工作流程步骤生成的数据集的问题。

Method: 提出了基于知识图谱的数据目录Virtual Process Dossier (VPD)，并集成了本体和用户界面。

Result: VPD提供了数据集的FAIR方式，并使工作流程的溯源信息明确。

Conclusion: VPD通过其本体、溯源框架和用户界面，为多阶段制造场景中的数据管理和溯源提供了有效的解决方案。
...
### [249] [Simplifying Neural Networks During Training](https://arxiv.org/abs/2607.27854)
*Lorenzo Sciandra,Samuele Fonio,Roberto Esposito*

Main category: cs.AI

TL;DR: Proposed method simplifies deep networks during training with substantial parameter reductions and maintained accuracy
...
Result: Substantial parameter reductions while maintaining accuracy comparable to that of the full model

Conclusion: The proposed method effectively simplifies deep networks during training
...
### [250] [Back to All-Entity Ranking: Sampler-Dependent Evaluation in Continuous-Time Dynamic Graphs](https://arxiv.org/abs/2607.27861)
*Minwoo Yu,Young-guk Ha*

Main category: cs.AI

TL;DR: 采样负基准的模型优势结论依赖于候选配置，所有实体的排名是评估架构比较的主要证据。


<details>
  <summary>Details</summary>
Motivation: 评估采样负分布和候选集对CTDG模型排名的影响，以确定排名结果的可信度。

Method: 通过分析CTDG模型中样本负分布的影响、使用历史成员资格信息和模型操作、控制表示干预和因子评估方法，评估不同模型的排名。

Result: 不同模型在不同数据集上的排名受到采样负分布、候选集大小和训练目标的影响。

Conclusion: 模型优势与消融结论基于采样负样本基准的假设，对所有实体的排名可作为CSTDG基准上架构比较的主要证据。
...
### [251] [Orca: Neural Operators for Causal Reasoning in Continuous Time](https://arxiv.org/abs/2607.27867)
*Gerrit Großmann,David A. Selby,Sebastian J. Vollmer*

Main category: cs.AI

TL;DR: Orca框架利用神经网络算子学习实现因果推理，适用于动态系统和反事实推理。


<details>
  <summary>Details</summary>
Motivation: 许多系统，如患者、气候和经济，是连续演化的，具有反馈循环，而传统的结构因果模型无法描述这种动态特性。

Method: 提出Orca框架，其中因果图的每个节点是时间的函数，每个机制是函数空间之间的学习映射。

Result: 扩展了现有的神经网络算子架构来表示因果机制，并在合成连续时间示例上实现了反事实推理。

Conclusion: 神经网络算子学习为因果推理提供了一个自然的基础，有助于处理动态系统和反事实推理。
...
### [252] [Search as Computation Allocation](https://arxiv.org/abs/2607.27871)
*Alexander Tuisov*

Main category: cs.AI

TL;DR: Formalizes terminal computation-allocation problems, relates VOC to information, and identifies a shared decision problem without claiming universal optimality of any acquisition rule.
...
Method: Formalize terminal computation-allocation problems, use Bellman equations, relate value of computation (VOC) to information.

Result: Identify a shared decision problem, recover weighted A* under an explicit frontier-resolution and heuristic-error model.

Conclusion: The theory identifies a shared decision problem without asserting that one acquisition rule is universally optimal.
...
### [253] [An Empirical Study of Coordination Mode as the First-Class Citizen in From-Scratch Multi-Agent Coding](https://arxiv.org/abs/2607.27877)
*Yanyu Ren,Yunfeng Bai,Xizheng Wang,Li Chen,Dan Li*

Main category: cs.AI

TL;DR: MSEval是一种新的多智能体编程评估基准，它基于真实世界任务，能够更准确地衡量多智能体编程的性能和效果。


<details>
  <summary>Details</summary>
Motivation: 为了评估多智能体编程的实际效果，现有基准测试存在忽视实际时间和成本、混淆推理与通信以及仅奖励表面完成等问题。

Method: 提出了一种新的多智能体从零开始评估基准MSEval，该基准在真实世界任务上评估多智能体编程。MSEval基于10个真实、全栈项目，使用分层需求和确定性评分标准对性能进行评分。其执行引擎LegoGent测试了10种协作拓扑，智能体通过周期性同步间隔协调并通过本地CI/CD管道部署。同时，自动评分器TAgent动态探测实现，共同衡量功能成功、延迟和前缀缓存令牌成本。在100次运行中，MSEval发现组织拓扑与模型能力在塑造速度-成本-质量权衡方面相当重要。对于相同任务和模型，改变拓扑可以使分数变化超过30分，并将wall-clock时间翻倍。结构化管道收敛最快，质量最高，而过度管理会降低性能。

Result: MSEval建立了一个严格、可重复的标准来衡量多智能体团队实际构建软件的方式。基准测试在https://github.com/robinren03/MSEval发布。

Conclusion: MSEval为多智能体编程提供了一个更真实、更全面的评估标准，有助于提高软件开发的效率和效果。
...
### [254] [Not All Tokens Deserve Equal Credit: Counterfactual Sensitivity Credit Reallocation for Long-CoT Reasoning](https://arxiv.org/abs/2607.27888)
...
### [255] [Dynamic Spectral Filtering for Temporal Graph Learning: Learning Evolving Propagation Operators](https://arxiv.org/abs/2607.27891)
*Yan Kong*
...
### [256] [MMHBench: A Multi-Perspective Benchmark for Mental Health Understanding in Long-Form Videos](https://arxiv.org/abs/2607.27895)
*Jinpeng Hu,Erqiang Wang,Shan Wang,Zhuo Li,Peipei Song,Xun Yang,Meng Wang*

Main category: cs.AI

TL;DR: MMHBench：用于多角度心理健康理解的多模态基准，评估了长格式视频心理健康理解的挑战性。


<details>
  <summary>Details</summary>
Motivation: 为了解决现有基准在理解心理现象方面的局限性，我们引入了MMHBench，一个综合的多模态基准，用于多角度的心理健康理解。

Method: 提出了一种多智能体问题生成（MAQG）框架，模拟不同的社会角色，从多个角度合成问题。

Result: 对22个代表性多模态大型语言模型（MLLMs）的广泛评估表明，长格式视频心理健康理解仍然具有高度挑战性。

Conclusion: MMHBench为心理健康理解提供了一个新的基准，但长格式视频心理健康理解仍然是一个具有挑战性的任务。
...
### [257] [A Cross-Architecture Audit of Direction-Based Inference-Time Defences in Vision-Language Models](https://arxiv.org/abs/2607.27910)
...
### [258] [IFHierBench: Hierarchical Instruction Following for Large Language Models](https://arxiv.org/abs/2607.27912)
*Yuetian Mao,Chunyang Chen*

Main category: cs.AI

TL;DR: 提出了一种新的分层指令遵循基准，发现现有LLM在遵循嵌套约束方面存在较大差距


<details>
  <summary>Details</summary>
Motivation: 提高大型语言模型在实际应用中的指令遵循能力

Method: 提出IFHierBench，一个分层指令遵循基准，包含600个提示，并评估了七种领先模型

Result: 最强模型仅在50%的提示级别上略微超过，随着约束深度的增加，准确性急剧下降

Conclusion: 可靠地遵循嵌套约束仍然是当前LLM的一个重大差距，需要考虑更细粒度的约束遵循来提高指令遵循能力
...
### [259] [One Anchor for All: Unified Multilingual and Multimodal Safety Alignment for LVLMs](https://arxiv.org/abs/2607.27917)
*Enyi Shi,Fei Shen,Chuancheng Shi,Linxia Zhu,Shuyi Miao,Jinhui Tang,Tat-Seng Chua*

Main category: cs.AI

TL;DR: 提出了一种新的安全对齐框架，用于防御复合攻击，在多语言和多模态场景中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有方法将语言和模态防御隔离，导致模型难以防御复合攻击。

Method: 提出了一种基于模态和语言共享安全神经元（MLS-Neurons）的神经元级跨维度安全对齐框架。

Result: 在多语言和多模态安全基准测试中，该方法显著优于现有方法，同时保持了通用性。

Conclusion: 该方法能够有效防御复合攻击，具有更高的安全性和通用性。
...
### [260] [Meta-Task: Turning Terminal Task Synthesis into a Terminal Task for Scalable Agent Training](https://arxiv.org/abs/2607.27929)
*Zhihong Pan,Jiyuan He,Kai Zhang,Yupeng Han,Ze Liu,Yuze Zhao,Yongcong Ye,Zhaohua Yang*

Main category: cs.AI

TL;DR: Meta-Task框架通过迭代生成和验证任务，提高了终端任务合成的质量和效率，减少了训练数据需求，并提升了性能。


<details>
  <summary>Details</summary>
Motivation: 训练终端代理需要多样化的可验证终端任务和高质量的交互轨迹，但获取此类数据仍然是一个重大挑战。现有的合成方法存在两个关键限制：（1）由于任务生成与实际执行之间的脱节，导致可靠性较弱；（2）由于依赖于现有存储库，导致多样性和可扩展性有限。

Method: 提出Meta-Task框架，将终端任务合成定义为终端-Bench格式任务本身：代理在真实容器环境中操作，迭代生成、执行和验证任务，以便在生成循环本身中检查合成组件的内部一致性和可执行性。在此基础上，我们在多个维度上解耦目标任务需求，引入一个多阶段机制，在生成实际任务之前动态设计新颖的任务规范，并纳入可选的外部材料支持以增强多样性和现实性。此外，我们还应用LLM作为法官的过滤，以确保最终训练数据的质量。

Result: 在Terminal-Bench 2.0上的实验表明，仅在3,221个Meta-Task合成的轨迹上进行微调，分别实现了Qwen3-14B和Qwen3-32B的22.5%和31.8% Avg Pass@1，显著优于具有更少训练数据的并行方法。

Conclusion: Meta-Task框架提高了终端任务合成的可靠性和多样性，显著减少了训练数据需求，并提高了性能。
...
### [261] [The Geometric Nature and a Free Proxy for Flow-Matching Uncertainty](https://arxiv.org/abs/2607.27933)
*Ziyang Rao,Yiren Zhao,Weiyu Guo,Ben Fei,Yandong Guo,Hui Xiong*

Main category: cs.AI

TL;DR: 提出了基于几何解释的不确定性估计方法accel，用于FM模型的在线失败检测，实验结果表明其性能优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 为了确保Flow匹配（FM）模型在安全部署中的可靠性，研究了一种新的不确定性估计方法。

Method: 提出了基于速度场的几何解释，引入了去噪加速（accel）作为不确定性代理，通过测量去噪轨迹的弯曲程度来评估不确定性。

Result: 实验结果表明，accel能够有效识别失败的rollout，在现实部署预算下，其性能优于基于重采样和训练的基线方法。

Conclusion: 去噪加速是一种高效且通用的不确定性估计方法，可以应用于FM模型的在线失败检测。
...
### [262] [Shapes from Examples: Foundations of Shape Learning in Recursive SHACL](https://arxiv.org/abs/2607.27934)
*Bente Gortworst,Cem Okulmus,Magdalena Ortiz,Anni-Yasmin Turhan*

Main category: cs.AI

TL;DR: 研究SHACL shapes的拟合问题，为知识图谱应用提供理论支持。


<details>
  <summary>Details</summary>
Motivation: SHACL shapes在数据图验证中的重要性，以及自动形状学习对于知识图谱应用的意义。

Method: 使用正负示例节点集P和N，计算一个形状表达式C，验证每个P中的节点和N中的节点。

Result: 针对C为SHACL核心片段ELI的情况，研究拟合存在性和最具体拟合计算，建立两者的指数时间上界，并获得相关特殊情况的多项式界限。

Conclusion: 为SHACL shapes的拟合问题提供了有效的方法和理论支持。
...
### [263] [From Scoring to Acting: Outcome-Verified Comparative Self-Distillation for LLM Agents](https://arxiv.org/abs/2607.27937)
*Xu Xia,Jinghua Piao,Min Yang,Xiaochong Lan,Jiaju Chen,Yong Li*

Main category: cs.AI

TL;DR: OVCSD is an effective approach for LLM agents' capability internalization, achieving significant performance gains.


<details>
  <summary>Details</summary>
Motivation: Recent work on LLM agents is shifting from external capability elicitation to capability internalization, enabling agents to retain useful skills without retrieval at inference time.

Method: Outcome-Verified Comparative Self-Distillation (OVCSD)

Result: OVCSD consistently outperforms skill-free RL and existing self-distillation baselines, achieving up to 29.7 and 5.4 absolute success-rate gains over the strongest baselines on ALFWorld and WebShop, respectively, while adding less than 3% privileged interaction during training.

Conclusion: OVCSD is a promising method for LLM agents' capability internalization.
...
### [264] [MARS-RA: Rank Aggregation for Credit Assignment via Multimodal Comparisons in Embodied Multi-Agent Cooperation](https://arxiv.org/abs/2607.27967)
*Dawei Wang,Di Zhao,Xinyuan Liu,Marci Chi Ma,Xiaoyang Liu,Chengming Zhou,Gary Ushaw,Richard Davison*

Main category: cs.AI

TL;DR: MARS-RA通过将信用分配转化为排名聚合问题，提高了强化学习中智能体合作的有效性。


<details>
  <summary>Details</summary>
Motivation: 在多智能体强化学习中的信用分配是基本挑战，特别是在具有有限和延迟反馈以及动态变化的活跃智能体数量的具身人工智能设置中。

Method: 提出MARS-RA框架，将信用分配重新定义为排名聚合问题，使用由大型多模态模型生成的智能体之间的基于贡献的成对比较。

Result: 提供了理论上的收敛性和鲁棒性保证，并表明Shapley值可以作为解释性参考。实验结果表明，MARS-RA可以引导智能体实现有效的合作。

Conclusion: MARS-RA框架有效地解决了信用分配问题，并通过实验验证了其有效性和鲁棒性。
...
### [265] [Share the Judge, Learn the Deferral: Where Specialization Helps LLM Evaluation](https://arxiv.org/abs/2607.27984)
*Weining Zhang*

Main category: cs.AI

TL;DR: 研究提出，在数据量不足时，应共享判断学习，并在经过审核的发布边界中实现领域特定性适应。


<details>
  <summary>Details</summary>
Motivation: 研究如何将领域特定性构建到评估器的权重或判断可信性的规则中。

Method: 通过99,952个公共、评分准则条件下的示例进行研究，比较不同方法的效果。

Result: 将评分准则纳入评估器权重可以提高准确率，但将领域特定性构建到判断可信性的规则中效果更好。

Conclusion: 在数据量不足时，应共享判断学习，并在经过审核的发布边界中实现领域特定性适应。
...
### [266] [SKIMIX: Multi-Agent Harness-Time Scaling with Skill Mixture for Dynamic Harness Engineering](https://arxiv.org/abs/2607.27994)
*Jia Luo*

Main category: cs.AI

TL;DR: SKIMIX improves open-ended reasoning, but its benefits vary by task type.
...
Method: SKIMIX, a multi-agent framework with iterative refinement, embedding-based skill retrieval, submodular anti-dilution routing, and adaptive skill evolution.

Result: Multi-agent collaboration improves open-ended mathematical reasoning but has limited or negative gains on multiple-choice tasks. Agent-count scaling is non-monotonic, and most improvements occur during the first refinement round.
...
### [267] [MMLDSum-LLM: Multimodal Long-Document Summarization with Visual-Alignment and Keyword-Aware](https://arxiv.org/abs/2607.28006)
*Xianpeng Zhang,Jiahua Yang,Dongyu Chen,Lei zhang,Jian Ma,Xu guohuan,Haonan Lu,Tianhuang Su,Chuangchuang Wang,Kai Tang*

Main category: cs.AI

TL;DR: MMLDSum-LLM：提升多模态长文档摘要的关键信息覆盖和一致性


<details>
  <summary>Details</summary>
Motivation: 针对多模态长文档摘要中关键证据分布稀疏、注意力漂移和跨模态对齐问题

Method: 提出MMLDSum-Bench基准和MMLDSum-LLM框架，结合监督微调和加权损失，使用GRPO进行多目标奖励优化

Result: 在MMLDSum-Bench基准上显著提高了关键信息覆盖率和跨模态一致性

Conclusion: MMLDSum-LLM在多模态长文档摘要任务中有效，具有可重复性和高性能
...
TL;DR: MUL-T is a lightweight transformer framework for understanding tissue organisation, outperforming classical methods and achieving comparable performance to foundation ViT models with fewer parameters.


<details>
  <summary>Details</summary>
Motivation: Understanding tissue organisation in multiplexed imaging

Method: MUL-T, a lightweight transformer framework

Result: Consistently outperforms classical feature-based baselines

Conclusion: MUL-T captures higher-order cellular interactions while remaining computationally efficient
...
### [269] [DataClawEval: A Benchmark for Data Engineering Agents in Real Industrial Harness](https://arxiv.org/abs/2607.28033)
*Debin Meng,Jiaming Yang,Zefang Zong,Tengyue Xu,Haining Xie,Yang Li,Peng Chen*

Main category: cs.AI

TL;DR: DataClawEval基准测试评估了自主代理在真实数据工程场景中的全流程任务完成能力，发现领域专业化而非全能性是关键挑战。


<details>
  <summary>Details</summary>
Motivation: 为了填补现有基准测试在复杂数据工程领域研究的空白，提出了DataClawEval基准测试。

Method: 引入了DataClawEval，包含100个严格的全流程任务，涵盖五个执行引擎，并使用确定性规则脚本进行评分。

Result: 评估了16个前沿代理，发现没有模型在所有引擎上表现优异，揭示了严格的领域专业化。

Conclusion: 自主数据工程仍然是一个严峻的未解挑战。
...
### [270] [SKILL-KD: Contrastive Skill Distillation for LLM Agents](https://arxiv.org/abs/2607.28048)
*Qiming Shi,Yibo Dou,Jiawen Zhu,Yulong Tao,Linbo Jin,Zhaolu Kang,Yunfan Zhou,Di Weng*

Main category: cs.AI

TL;DR: SKILL-KD通过蒸馏技能差异，提高了能力较弱学生的技能水平。


<details>
  <summary>Details</summary>
Motivation: 现有技能获取方法将技能视为经验摘要、记忆条目或成功演示的直接总结，导致与能力较弱的学生的不匹配。

Method: SKILL-KD，一个对比技能蒸馏框架，将技能视为不同能力代理之间的显式蒸馏介质。通过将学生失败和教师轨迹的差异蒸馏为文本技能补丁，并评估该补丁，然后迭代地改进补丁，直到学生成功为止。

Result: 在五个代理基准和两种学生设置中，SKILL-KD在冻结的学生代理上始终优于固定模型适应基线。

Conclusion: SKILL-KD框架通过蒸馏技能差异为文本补丁，有效地提高了能力较弱学生的技能水平。
...
### [271] [IndustryForge-27B: A Domain-Enhanced Multimodal Foundation Model for Industrial CAD](https://arxiv.org/abs/2607.28050)
...
### [272] [SemPIC: Learning Semantic Position-Independent KV Caches](https://arxiv.org/abs/2607.28069)
*Hui Xie,Peng Xiao,Yutong Deng\textsuperscript,Shuoran Dou,Jian Yang,Jinyang Guo*

Main category: cs.AI

TL;DR: SemPIC通过改进文档表示和引入新的技术提高了长上下文检索和代理工作负载的性能。


<details>
  <summary>Details</summary>
Motivation: 长上下文检索和代理工作负载在变化的指令、历史和文档顺序下重复使用相同的文档。前缀缓存无法利用这种重复使用，而位置无关缓存（PIC）由于独立编译的KV状态缺乏未来上下文，因此仍然不可靠。诊断显示，学习的边界条件基线在可重复块边界附近显著减少了注意力偏差，但留下了内部和任务级别的残留，这促使对文档表示本身的调整。

Method: 我们提出了SemPIC，它通过行为蒸馏训练一个LoRA启用的Writer来编译原生每层文档KV，同时保留预训练的解码器作为未更改的Reader。适应仅限于离线缓存构建，保留标准KV接口和缓存命中解码路径。我们进一步引入了KV梯度检查点，通过缓存的KV减少了峰值训练内存，而没有切断梯度。

Result: 在三个模型和四个任务中，SemPIC将KV包的平均微F1从0.53提高到0.60，接近全重计算的0.62。

Conclusion: SemPIC通过改进文档表示和引入新的技术提高了长上下文检索和代理工作负载的性能。
...
### [273] [Echoverse: Deep, Evolving Environments for Training Computer-Use Agents at Scale](https://arxiv.org/abs/2607.28074)
...
TL;DR: Echoverse环境通过提高行为深度、针对性改进和与模型协同进化，显著提高了计算机使用代理的性能。


<details>
  <summary>Details</summary>
Motivation: 计算机使用代理通过观察其行为产生的变化来学习，因此训练一个代理需要它可以作用、破坏和重置的应用程序。最重要的应用程序是登录门控和有状态的，因此合成环境可以替代它们。

Method: Echoverse将规范编译成有状态的应用程序，其任务与应用程序自己的数据库进行比较，并创建了一个协同进化循环，该循环将每次评估滚动读取两次：作为环境的修复、其任务及其验证器的修复，以及作为模型的训练信号。在12个这样的环境中训练后，一个9B模型在14个评估分割中的性能从36.5%提高到67.1%，接近于教授它的更大模型。

Result: 研究结果表明，环境的行为深度、是否针对代理实际失败的交互以及是否与模型一起改进是提高模型性能的关键因素。Echoverse环境提高了模型在真实世界任务上的性能。

Conclusion: Echoverse环境为计算机使用代理提供了有效的训练平台，有助于提高模型的性能。
...
### [274] [Group-Reflective Self-Distillation for Agentic Reinforcement Learning](https://arxiv.org/abs/2607.28076)
*Binbin Zheng,Zijun Xie,Guanqun Zhao,Enlei Gong,Xing Ma,Xiaoliang Fu,Zeyu Chen*

Main category: cs.AI

TL;DR: GRSD有效提升了强化学习在训练大型语言模型代理中的应用


<details>
  <summary>Details</summary>
Motivation: 提高强化学习在训练大型语言模型代理中的效果

Method: 提出Group-Reflective Self-Distillation（GRSD）

Result: 在多个代理环境和模型规模上，GRSD consistently outperforms competitive baselines and generalizes more effectively to unseen tasks

Conclusion: GRSD通过从策略的验证滚动中提取能力对齐和结果区分性指导，有效提高了强化学习在训练大型语言模型代理中的效果
...
### [275] [Distilling Answer Set Programming Theories from Large Language Models](https://arxiv.org/abs/2607.28086)
...
### [276] [Diversifying Personalized Research Ideation against AI-Induced Homogenization](https://arxiv.org/abs/2607.28087)
*Rui Xu,Yunke Wang,Linwei Tao,Wenjie Xuan,Yong Luo*

Main category: cs.AI

TL;DR: DivAlign是一种新的AI辅助研究理念系统，通过提高研究方向的多样性和研究者匹配度，有效地解决了现有系统的盲点。


<details>
  <summary>Details</summary>
Motivation: 为了解决现有AI辅助研究理念系统在优化单个建议时存在的盲点，如研究者表示过于粗略和独立推荐可能导致社区投资组合集中在重复的高概率主题上。

Method: 提出DivAlign，一个四阶段管道，包括提取细粒度研究者配置文件、生成配置文件条件下的候选方向、沿三个对齐维度（可执行性、可理解性和增长潜力）进行评分，并呈现研究者局部方向，同时减少社区投资组合中的冗余。

Result: 在从95位AI研究人员构成的五个子领域构建的基准测试中，DivAlign减少了社区层面的冗余，同时保留了研究者方向匹配度。与粗略的单次理念相比，它将平均成对相似度从0.331降低到0.294，将最近邻相似度从0.704降低到0.608。与独立的首选变体相比，DivAlign将最近邻相似度从0.663降低到0.608，同时保留了99.9%的研究者方向匹配度分数。

Conclusion: DivAlign有效地解决了现有AI辅助研究理念系统的盲点，提高了研究方向的多样性和研究者匹配度。
...
### [277] [PerturbMap: Cross-Context Transfer of Single-Cell Perturbation Responses](https://arxiv.org/abs/2607.28090)
...
### [278] [An Instrument to Evaluate Governance Proposals: AI Policy Analysis at Scale](https://arxiv.org/abs/2607.28094)
*Paulo Carvao,Claudio Mayrink Verdun,Isabel Adler,Jeffrey Zhou*

Main category: cs.AI

TL;DR: 本文提出了一种多维政策评估框架，用于评估人工智能治理提案的相关性和一致性，以帮助政策制定者做出更明智的决策。


<details>
  <summary>Details</summary>
Motivation: 为了在不断发展和有争议的监管环境中系统地、透明地评估人工智能治理提案，本文提出了一种政策分析框架。

Method: 混合方法，结合定性和定量分析，包括专家意见和文本分析。

Result: 提出了一种多维政策评估框架，用于评估人工智能治理提案的相关性和一致性。

Conclusion: 该框架有助于政策制定者、分析师和研究人员在复杂的人工智能治理环境中做出更明智的决策。
...
### [279] [SciDataSailor: Deep Scientific Data Exploring](https://arxiv.org/abs/2607.28098)
*Jiyong Rao,Yicheng Qiu,Chi Zhang,Chunfeng Song,Runkai Zhao*

Main category: cs.AI

TL;DR: SciDataSailor框架通过蒙特卡洛树搜索和特定机制，提高了科学数据探索的效率和准确性。


<details>
  <summary>Details</summary>
Motivation: 科学数据集通常组织为包含异构和相互依赖文件的层次存储库，这使得它们的检查、集成和分析劳动密集且依赖于领域专业知识。尽管大型语言模型（LLM）在规划、推理和工具使用方面取得了显著进展，但现有研究在很大程度上忽视了它们通过可执行环境与真实科学数据资产交互的能力。

Method: 我们引入了深度科学数据探索，这是一种代理任务范式，其中代理在存储库中导航，解释异构文件和模式，执行分析，整合跨文件证据，并基于执行观察得出结论。为了实现这一范式，我们提出了SciDataSailor，这是一个通过平衡广泛探索和有针对性的利用来合成工具交互轨迹的框架。SciDataSailor将轨迹合成实例化为蒙特卡洛树搜索（MCTS），具有四个特定于任务的机制：难度分层探索种子，双重反馈第一播放紧迫性，分层策略到工具动作生成和熵引导分支。使用此框架，我们构建了SciDataSailor-SFT-2K用于监督微调和SciDataSailor-Bench用于评估，后者包含来自27个数据集的627个元信息摘要任务和586个科学问答任务，涵盖生命、地球和物理科学。

Result: SciDataSailor-SFT-2K和SciDataSailor-Bench的构建，用于监督微调和评估，涵盖了27个数据集的627个元信息摘要任务和586个科学问答任务，涵盖生命、地球和物理科学。

Conclusion: SciDataSailor框架能够有效地合成工具交互轨迹，提高科学数据探索的效率和准确性。
...
### [280] [MIND: Lightweight and Effective Memory Injection Defense for LLM Agents via Intent-Aware Information Bottleneck](https://arxiv.org/abs/2607.28103)
*Dongyi Liu,Haixing He,Xiaobao Wu,Jia Li*

Main category: cs.AI

TL;DR: MIND有效防御记忆注入攻击


<details>
  <summary>Details</summary>
Motivation: 针对记忆增强型LLM代理容易受到记忆注入攻击的问题

Method: 提出Memory Intent-Aware Neural Denoising（MIND）框架

Result: 降低攻击成功率，同时保持任务准确性和推理效率

Conclusion: MIND是一个有效的轻量级防御框架，可应对记忆注入攻击
...
### [281] [CDAE: Enhancing Perturbation Robustness in Pretrained Language Models with Contrastive Denoising](https://arxiv.org/abs/2607.28236)
...
### [282] [Beyond Rephrasing: Book-Level Organization Improves Synthetic Textbook Data for Mid-Training](https://arxiv.org/abs/2607.28109)
*Jiawen Tao,Miao Peng,Yaoming Li,Xiaokun Yuan,Mengzhou Wu,Wenhan Yu,Guoan Wang,Nuo Chen,Tong Yang,Maxm Pan*

Main category: cs.AI

TL;DR: 这项研究强调了合成预训练数据设计中书籍级别组织的价值。


<details>
  <summary>Details</summary>
Motivation: 研究合成教科书数据在语言模型预训练中的改进，但先前工作主要将这种好处视为生成内容或局部重写风格的属性。

Method: 提出了一种可扩展的合成流程，包括从预训练语料库检索源材料、将其聚类为主题单元、规划层次目录以及将基于源材料的章节组装成完整的书籍。

Result: 使用该语料库替换中期训练混合中的自然书籍，平均提高了下游性能+1.09。

Conclusion: 这项研究表明，将相关内容组织成连贯的书籍级别文档对于语言模型预训练至关重要。
...
### [283] [GLM-RAG: Graph Language Models for Graph-Based Retrieval-Augmented Generation](https://arxiv.org/abs/2607.28397)
*Maya Arseven,Anette Frank,Beni Egressy,Johann Higl,Moritz Plenz*

Main category: cs.AI

TL;DR: 基于GLM的检索器在多跳RAG任务中表现出色，特别是在域外泛化方面。


<details>
  <summary>Details</summary>
Motivation: 为了提高知识图谱上的检索增强生成（RAG）的效果，需要能够有效捕捉图结构和语义信息的检索器。

Method: 提出了一种基于图语言模型（GLM）的检索器，并比较了GLM、图神经网络（GNN）和传统向量搜索检索器在单跳和多跳RAG设置中的性能，特别关注其在未见领域中的可迁移性。

Result: 研究发现，微调的GLM检索器在域外泛化能力更好，在两个多跳基准测试中达到SOTA。在域内多跳问答数据集上，它们与先前的工作相当，随着参数和子图覆盖率的增加，具有有希望的扩展性。基于GNN的检索器在高效的训练设置中实现了更高的图覆盖率，而向量搜索基线在单跳数据集上表现优异。

Conclusion: 基于GLM的检索器在域外泛化能力上具有优势，而GNN和向量搜索在特定场景下表现更佳。
...
### [284] [BlueprintRepair: Typed Local Edits for Failed Lean Proof Blueprints](https://arxiv.org/abs/2607.28110)
*Ruslan Khrulev*

Main category: cs.AI

TL;DR: LLM-based Lean proving systems使用蓝图修复，类型修复在成本效益方面表现最佳。


<details>
  <summary>Details</summary>
Motivation: LLM-based Lean proving systems increasingly organize a proof as a blueprint: a dependency graph of formal statements.

Method: Introduce BlueprintRepair, a repair interface that allows a model to change the graph through ten schema-checked local operations. Construct BlueprintTrace, a benchmark of 142 controlled failures with complete accepted and rejected repair trajectories. Compare typed edits, exact source patches, and complete module rewrites under matched source, feedback, model, and budget.

Result: The three interfaces solve almost the same number of the benchmark's localized failures. Typed repair is the cheapest per solved state. Qwen3.6-Flash solves fewer states but keeps typed repair cheapest.

Conclusion: BlueprintRepair和BlueprintTrace在解决证明问题方面有效，且类型修复在成本效益方面表现最佳。
...
### [285] [WIDE: Boosting Adaptive LLM Inference via Token-level Dynamic Width Pruning](https://arxiv.org/abs/2607.28418)
...
### [286] [ConMem: Contribution-Aware Memory for Long-Horizon Manufacturing Inspection Logs](https://arxiv.org/abs/2607.28126)
*Bingchen Liu,Yuanyuan Fang,Lei Liu,Guangyuan Dong,Xing Fu,Yuanyuan Gao,Shuyue Wei,Xin Li,Xiangtian Meng*

Main category: cs.AI

TL;DR: ConMem是一种基于LLM的设备检查框架，提高了早期风险检测的准确性和效率。


<details>
  <summary>Details</summary>
Motivation: 长周期钢设备检查需要跨多个检查周期积累的异构记录推理。现有检索增强生成系统将历史日志视为静态语料库，保留记录而不估计其诊断价值，无法报告早期风险。

Method: 提出ConMem，一个基于LLM辅助的设备检查的贡献感知记忆框架，支持人工参与的早期风险筛选系统。具体来说，我们的ConMem首先将检查日志分割为功能证据单元，然后通过Shapley风格的估计估计每个记忆单元对下游诊断的贡献，最后在约束的记忆预算下保留高价值证据。

Result: 在真实世界数据集上评估ConMem，ConMem实现76.0%的QA准确率，超过最强的直接可比基线。与简单的8K上下文LLM基线相比，它将平均输入令牌数减少了88.2%，响应时间减少了86.6%。消融研究也表明，功能角色感知的分割和基于贡献的估值有助于优先考虑针对特定现场检查的弱退化信号。实际部署进一步证实，ConMem在三个检查周期中保留了弱早期信号，为现场检查人员提供了早期阶段的密封磨损警报。

Conclusion: ConMem在长周期钢设备检查中提高了早期风险检测的准确性和效率，并通过功能角色感知的分割和基于贡献的估值实现了对弱退化信号的有效识别。
...
### [287] [Metaphor Tracer: A Theory-Informed Analysis of Hidden States](https://arxiv.org/abs/2607.28434)
...
### [288] [AgenticASR: Refining Speech Recognition in Real-World Scenarios via an Agentic Approach](https://arxiv.org/abs/2607.28175)
*Zixuan Jiang,Binghao Qiang,Jiaying Chi,Yanqiao Zhu,Kai Yu,Xie Chen*

Main category: cs.AI

TL;DR: AgenticASR通过改进自动语音识别，实现更准确、意图保留的文本转录。


<details>
  <summary>Details</summary>
Motivation: 自动语音识别（ASR）在转录准确性方面取得了显著进步，但逐字转录并不一定能产生易于使用的文本。它保留了填充词、重复、中断和自我纠正，增加了阅读难度，模糊了说话者的最终意图，并将未解决或放弃的内容传播到下游任务。现有的语音到书面语的方法处理完成的音频或转录本，但不能在后续的语音改变如何解释先前内容时修订已发出的文本。因此，我们提出了代理语音识别（AgenticSR），这是一个音频到清洁文本的任务，它删除了不流畅的语句、解决了自我纠正并规范了书面形式，同时保留说话者的最终意图。AgenticASR通过ASR-Refiner架构实现此任务，该架构在音频到达时重复转换有限的活动上下文并替换其相应的输出范围。这使它能够对任意长度的流进行持续发射和修订。我们还引入了AASR-Bench，这是一个双语基准，具有细粒度的原子评分标准。在多个ASR前端中，AgenticASR在评估的系统中获得了最高的AASR-Bench分数。一项人机协议研究显示，基于评分标准的判断与独立专家评估相一致。消融实验描述了Refiner能力、上下文长度以及在线和离线推理之间的质量-延迟权衡。总之，这些结果将AgenticASR确立为在持续语音中进行意图保留清洁转录的实用框架。代码、AASR-Bench和演示将在https://github.com/AnXMuy/AgenticASR上发布。

Method: AgenticASR通过ASR-Refiner架构实现，该架构在音频到达时重复转换有限的活动上下文并替换其相应的输出范围。

Result: AgenticASR在多个ASR前端中获得了最高的AASR-Bench分数。一项人机协议研究显示，基于评分标准的判断与独立专家评估相一致。消融实验描述了Refiner能力、上下文长度以及在线和离线推理之间的质量-延迟权衡。

Conclusion: AgenticASR是一个实用框架，用于在持续语音中进行意图保留清洁转录。
...
### [289] [SVR: Self-Verifying Refinement via Joint Verdict-Confidence Reinforcement Learning for Adaptive Test-Time Compute](https://arxiv.org/abs/2607.28457)
*Hongyu Chen,Liang Lin,Guangrun Wang*

Main category: cs.AI

TL;DR: SVR通过自我验证细化，在数学推理基准测试中实现了高准确率，同时减少了推理回合数。


<details>
  <summary>Details</summary>
Motivation: 为了提高语言模型的推理能力，进行时间测试计算可以扩展计算能力，但均匀的预算在简单输入上浪费了计算资源，而验证器引导的细化则依赖于外部反馈。

Method: 我们引入了自我验证细化（SVR），这是一个无预言机多轮强化学习框架，它学习使用自我验证作为计算控制策略。在每个回合，模型生成一个解决方案，包括一个离散的正确性判决和置信度分数；只有在判决为正确且置信度超过阈值时，模型才保留当前答案，否则继续使用自己的自我验证进行细化。真实正确性仅用于构建训练奖励，永远不会通过细化提示暴露给策略，也不会在推理时要求。SVR使用GRPO在固定时间轨迹上进行训练，使用奖励促进解决方案的正确性、校准感知的自我验证和停止就绪的正确状态；自适应停止仅在推理时激活。在七个数学推理基准测试中，SVR在Qwen3.5-2B上实现了0.563的宏观平均准确率，平均仅需要2.99次推理回合。在评估的完整系统比较中，它超过了标准GRPO、强大的多轮基线和固定预算的预言机引导的分数反馈参考，同时需要比固定十回合推理少得多的回合数。这些结果表明，学习到的自我验证可以作为答案保留和自适应测试时间计算分配的有效内部控制信号。

Result: SVR在七个数学推理基准测试中实现了0.563的宏观平均准确率，平均仅需要2.99次推理回合。在评估的完整系统比较中，它超过了标准GRPO、强大的多轮基线和固定预算的预言机引导的分数反馈参考，同时需要比固定十回合推理少得多的回合数。

Conclusion: 学习到的自我验证可以作为答案保留和自适应测试时间计算分配的有效内部控制信号。
...
### [290] [Qwen-UI-Agent Technical Report: Toward Next-Generation Real-World Centric Foundation GUI Agents](https://arxiv.org/abs/2607.28227)
...
### [291] [OSReward: Instituting Standardized Evaluation for Cross-Platform Computer-Use Reward Models](https://arxiv.org/abs/2607.28609)
...
TL;DR: VLM在评估CUA轨迹时存在局限性，但新的基准和模型可提高可靠性。


<details>
  <summary>Details</summary>
Motivation: 验证VLM在评估CUA轨迹中的可靠性。

Method: 引入OSReward基准，包括OSReward-Hard和OSReward-Multi，并构建OS-Shepherd-100K开放语料库和OS-Shepherd开放奖励模型。

Result: 发现VLM模型存在宽松偏差，并提出解决方案以降低成本并提高可靠性。

Conclusion: 当前最先进的VLM模型在评估CUA轨迹时存在系统性的宽松偏差，且可靠且成本效益高的模型难以扩展。
...
### [292] [AI and Authenticity in Islamic Research: A Critical Evaluation of Generative AI Reliability, Hallucination, and Source Fidelity in Quranic, Hadith, and Fiqh Knowledge](https://arxiv.org/abs/2607.28237)
*Muhammad Sajjad Akbar*

Main category: cs.AI

TL;DR: 评估了生成式AI在伊斯兰知识领域的可靠性，发现其在宗教指导、伊斯兰教育和学术研究中的应用有限。


<details>
  <summary>Details</summary>
Motivation: 评估当前生成式人工智能系统在伊斯兰知识领域的可靠性，以确定其在宗教指导、伊斯兰教育和学术研究中的适用性。

Method: 使用混合方法框架，包括领域准确性、引用验证、幻觉、法理一致性、不确定性处理、来源溯源和地理差异分析，对六个领先的生成式AI系统进行评估。

Result: 当前生成式AI系统作为辅助工具对伊斯兰入门学习有价值，但不应该被视为宗教裁决或伊斯兰研究的权威来源，除非经过验证的原始资料和合格学者的专业知识验证。

Conclusion: 这项研究为伊斯兰知识中AI可靠性的首次全面实证评估之一，为研究人员、教育工作者、AI开发者和更广泛的穆斯林社区提供了实用指导。
...
### [293] [AISPA: User-Centric System Prompt Auditing for Large Language Model Applications](https://arxiv.org/abs/2607.28617)
...
TL;DR: OPAL框架通过改进候选生成和表示，有效提升了工业在线3D-BPP的性能。


<details>
  <summary>Details</summary>
Motivation: 解决工业在线三维装箱问题（3D-BPP）中的候选生成和表示问题，提高空间利用率、稳定性、紧凑性和平衡性。

Method: 提出OPAL框架，包括OG-EMS候选生成器、操作表示和掩码排名策略，利用xLSTM编码器和轻量级循环核心进行候选排名。

Result: 在BED-BPP基准测试中，OPAL实现了0.49的平均空间利用率，比之前的方法提高了15.1%和6.3%。

Conclusion: OPAL框架有效提高了工业在线3D-BPP的性能。
...
TL;DR: LLMs可自动化模型重构，显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 为了自动化性能导向的模型重构，研究人员探索了大型语言模型（LLMs）在组合问题中的应用。

Method: 研究人员采用了一种基于进化的框架，其中LLM提出候选重构方案，并使用AHD方法进行验证和基准测试。他们引入了PDR策略，以保留行为上多样化的尝试。

Result: 迭代重构可以显著提高模型性能；保留多样化上下文的策略优于仅保留最近或最快尝试的策略；基于验证的选择提高了每个策略的保留速度。

Conclusion: LLMs可以自动化性能导向的模型重构，并显著提高模型性能。
...
TL;DR: MemHarness 框架通过重建和利用过去经验，提高了大型语言模型智能体的推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有的记忆增强智能体将检索到的经验视为静态记录，而忽略了存储的经验与决策时遇到的具体状态之间的差距，导致负迁移。

Method: MemHarness 框架，通过根据当前状态对检索到的经验进行批判性和重建，生成与上下文相关的指导，以适应当前情境。

Result: 在 ALFWorld 和 WebShop 上的实验表明，MemHarness 在分布外 (OOD) 场景中具有强大的鲁棒性，并且重建目标不仅防止了负迁移，还在训练期间起到了潜在的指导作用，从根本上提高了智能体的内在推理能力。

Conclusion: MemHarness 框架有效地利用了过去经验，并提高了智能体的推理能力。
...
TL;DR: ARC-AGI-3通过Tycho系统实现交互式技能获取，在游戏中表现出色。


<details>
  <summary>Details</summary>
Motivation: 将抽象转化为技能获取的交互式问题，让玩家在保持行动效率的同时推断未知游戏的规则、隐藏状态和目标。

Method: 将环境形式化为参数化的渲染确定性摩尔机，并引入Tycho系统，该系统在交互过程中构建和使用特定于游戏的模型。Tycho将可操作的观察结果与中间动画、关卡完成和游戏结束帧分开。从这种结构化历史中，代理可以建模、测试、计划、修复或绕过自由形式的可执行假设。

Result: 在匹配的公共集运行中，与Claude Opus 4.8在匹配的推理预算下，使用四种编排策略在所有25个公共游戏上进行了比较。通过向模型构建者请求的代理请求的委派，获得了最高的观察到的平均相对人类行动效率（RHAE），为88.49。使用此选定的策略，GPT-5.6 Sol和Opus 5都达到了100.00 RHAE并完成了所有183个级别。他们的游戏平衡的第一运行人类重放中位数排名分别为98.5和100.0。Opus 5比官方人类基线少61%的得分动作。自动修复验证失败后产生的模型可以更准确地再现观察到的转换，但仅达到83.07 RHAE。转换匹配表明模拟器是否再现观察到的动态，而不是它是否已识别目标或改进了下一步操作。强大的游戏还要求决定何时构建、修复、使用或绕过模型。我们称这个问题为主动抽象：从昂贵的交互中生成可测试的模型，并决定获取或使用它是否值得其成本。

Conclusion: ARC-AGI-3将抽象转化为技能获取的交互式问题，并通过Tycho系统实现了有效的游戏学习。
...
TL;DR: PathVU是一个用于评估MLLMs在病理图像分析中多尺度视觉理解的基准，有助于提高模型性能。


<details>
  <summary>Details</summary>
Motivation: 为了更深入地了解病理图像分析中多模态大语言模型（MLLMs）的理解能力，

Method: 提出了PathVU，一个基于视觉的基准，用于评估MLLMs在细粒度和多尺度视觉理解方面的能力。PathVU由23个公共病理图像数据集构建，包含人类监督标签和空间注释，并评估了区域定位、视觉识别、数量估计、空间推理和不足上下文判断等任务。

Result: 在评估18个代表性的一般用途、医学领域和病理导向的MLLMs后，发现即使在高级模型中，在多尺度病理图像的细粒度视觉任务上也存在重大局限性。PathVU为开发和评估具有明确多尺度视觉理解的病理MLLMs提供了可重复的基础。

Conclusion: PathVU为病理MLLMs的发展与评估提供了新的基准，有助于提高模型在病理图像分析中的理解能力。
...
TL;DR: CARP和SPARC通过惩罚机制保护消费者，提高LLM商家的诚信行为。


<details>
  <summary>Details</summary>
Motivation: LLM代理作为自主商人编写自己的产品列表，并在竞争压力下编造属性以赢得销售。

Method: 设计了一种名为CARP的声誉惩罚机制，该机制具有死区以原谅投诉噪声，并具有状态依赖的严重性以对抗声誉驱动的检测侵蚀。

Result: CARP保护消费者，通过抑制低评级骗子的销售量，同时保护诚实的卖家。与SPARC结合，它缩小了与完美信息预言家的消费者福利差距，而不需要访问真相。它还实现了我们比较的政策中最好的福利。

Conclusion: 通过SPARC，这种感觉上的惩罚变得行为上具有约束力，LLM商人编造谎言时免费，但当他们因为编造而失去销售时，会自我约束，这是一种自私的反应，而不是遵守。
...
TL;DR: PCD通过识别感知失败提高多模态推理器的性能。


<details>
  <summary>Details</summary>
Motivation: 为了解决现有方法中感知成功率和师生差异的模糊性，以及无法确定失败答案是由感知还是后续推理引起的。

Method: 感知-纠正蒸馏（PCD），一种无标签方法，使用下游失败和师生差异作为补充证据来识别可纠正的感知失败。
...
TL;DR: This paper proposes a reliability framework for CUA evaluation and finds that many benchmark scores are unreliable.


<details>
  <summary>Details</summary>
Motivation: Computer-use agents (CUA) are being deployed to browse the web and operate desktop software, but their benchmark scores are often produced by unreliable methods.

Method: The authors organize the problems into a reliability framework and audit 150 public failure-scored trajectories from various benchmarks.
...
Conclusion: The authors propose a reliability framework for CUA evaluation and derive stage-specific design rules.
...
TL;DR: HyperClaim是一种基于超图的新颖视频虚假信息检测方法，显著提高了检测准确率。


<details>
  <summary>Details</summary>
Motivation: 视频虚假信息检测中，全局多模态融合或自由形式多模态推理的范式往往不足以捕捉局部真实性线索，这些线索源于查询短语、上下文文本和帧的短时间跨度之间的耦合交互。

Method: 提出HyperClaim，一种基于超图的判别性时间超图框架，用于样本级真实性分类。

Result: 在FactGuard时间协议下，HyperClaim在FakeSV、FakeTT和FakeVV上分别达到83.7%、82.0%和87.3%的准确率，优于强判别和推理中心基线。

Conclusion: HyperClaim能够有效地识别视频虚假信息，优于现有的多模态融合和推理方法。
...
TL;DR: 提出了一种新的框架来分析大型语言模型在处理冲突时的偏好，发现系统性的偏好模式，并证明其在不同任务和规范形式中的适用性。


<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型在处理冲突时的偏好

Method: 构建具有显式冲突的规范，并比较不同类型表示的偏好模式

Result: 发现系统性的偏好模式，不同类型表示的偏好顺序为：形式化 ≈ 自然化形式化 > 纯自然语言 > 输入输出示例

Conclusion: 提出的框架能够有效地分析大型语言模型在处理信息冲突时的偏好，并适用于不同任务和规范形式
...
TL;DR: QuantWAMs通过改进的PTQ框架提升了WAMs的部署效率和性能。


<details>
  <summary>Details</summary>
Motivation: World Action Models (WAMs) 在预测未来观测和动作方面表现出色，但其迭代去噪和闭环执行使得高效部署成本高昂。

Method: QuantWAMs，一种PTQ框架，将量化决策与由模型结构、回滚分布和任务目标定义的校准上下文对齐。它引入了三种策略：共享基 outliers calibration、co-training-objective saliency和fixed-intervention rollout auditing。

Result: 在RoboTwin 2.0、LIBERO和真实机器人操作中，QuantWAMs在Fast-WAM和LingBot-VA上的评估表明，在W4A4主导的设置下，模拟平均值与FP16的差异为0.2--0.7个百分点。在真实机器人试验中，QuantWAMs在三个操作任务中实现了部署可行性，并减少了峰值权重和激活内存到FP16的约29%，提供了1.4--1.6倍的块级速度提升。

Conclusion: QuantWAMs有效地提高了WAMs的部署效率和性能。
...
TL;DR: 论文研究了派生特征在LLM中的应用，提出了DFOT概念，并通过实验验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型（LLM）的不断发展，越来越多的派生测量数据被直接用于模型中，尽管这些数据的有效性依赖于具体实例。

Method: 通过生理传感作为案例研究，论文提出了派生特征过度信任（DFOT）的概念，并通过实验测试了派生特征在LLM中的应用和局限性。

Result: 实验结果表明，在特定条件下，派生特征可能会导致LLM的错误应用，因此需要开发更有效的缓解方法。

Conclusion: DFOT为派生特征在LLM中的应用提供了一个共同的评价目标，有助于开发更强大的缓解方法。
...
TL;DR: UNICON模型是一种展现数值智能的跨学科基础模型，无需重新训练即可在科学和社会系统中达到专家水平。


<details>
  <summary>Details</summary>
Motivation: 为了解决科学和社会系统中对数值智能的需求，提出了UNICON模型。

Method: UNICON模型通过图示例从系统上下文中推断预测关系，并将其应用于查询。

Result: UNICON模型在科学和社会系统中的表现接近专家水平，无需重新训练。与语言模型结合使用时，其在未见过的学科中超越了最先进的专家。训练语料库的多样性提高了对未见学科的泛化能力。

Conclusion: UNICON模型是数值智能的基础模型，为人工智能的更广泛生态系统奠定了基础。
...
TL;DR: 该研究提出了一种利用Lean定理证明器验证约束编程结果的方法，显著提高了求解效率，并保证了结果的正确性。


<details>
  <summary>Details</summary>
Motivation: 为了保证约束编程的结果可信，需要在两个层面进行验证：预先应用的重构是否保持语义，以及求解器是否产生正确答案。

Method: 提出了一种框架，利用Lean定理证明器处理两个验证层面：证明公式级别的属性，如等价性、等价满足性和对称破缺约束的正确性；通过翻译后端检查求解器产生的证书。

Result: 实验结果表明，该框架的验证对称破缺在实践中也很有成效：每个问题家族的单个参数证明，在整个实例规模中重复使用，可将求解器搜索工作量减少至原来的1/2×10^7，同时整个在Lean中的认证过程也保持经济高效，最大实例的处理时间不超过几分钟。

Conclusion: 该框架能够有效地验证约束编程的结果，提高求解效率，并保证结果的正确性。
...
TL;DR: EndoCLIP是一种结肠镜检查视觉-语言模型，在多个临床任务中优于现有模型，表明其有潜力改善临床文档和图像的关联。


<details>
  <summary>Details</summary>
Motivation: 尽管在结肠镜检查中记录了丰富的专家描述，但视觉-语言模型在结肠镜检查中仍然未被充分利用。这些报告记录了病变的外观、大小和位置，但总结了整个程序而不是对单个帧进行字幕，使临床发现与相应的图像之间的联系较弱。

Method: 我们开发了EndoCLIP，这是一种结肠镜检查视觉-语言基础模型，该模型在从280,476份常规结肠镜检查记录中逐步恢复的125,756个病变级别图像-文本对上进行了训练。

Result: 在病变级别的图像-文本检索、结构化报告生成和六个多中心临床分类任务中，EndoCLIP在零样本和线性探针设置中均优于通用和生物医学视觉-语言编码器。在良性-恶性分类中，其线性探针在涉及12名内镜医生的盲法研究中接近专家读者的性能。

Conclusion: 这些结果表明，恢复发现与帧的对应关系可以将常规文档转换为可扩展的监督，使临床目标可以用语言而不是单独为每个任务标注来指定。
...
TL;DR: 提出一种结合机载推理和生成数据增强的空中监视新方法，显著提高了检测性能。


<details>
  <summary>Details</summary>
Motivation: 解决低地球轨道空中监视的两个瓶颈：纳米卫星有限的下行链路预算和稀少的开放卫星数据集。

Method: 结合机载推理和生成数据增强的工作流程。使用低功耗边缘张量加速器执行推理，通过低秩自适应微调生成合成少数类图像。

Result: 平衡数据集将全局平均精度从77.9%提高到82.2%，少数类F1值从0.683提高到0.811。

Conclusion: 该方法支持从纳米卫星进行实时、自主的空中监视。
...
TL;DR: SCOPE模型通过跨部门操作耦合提高供应链决策效率。


<details>
  <summary>Details</summary>
Motivation: 探讨供应链人工智能是否能够超越孤立的决策模块，实现统一运营规划。

Method: 提出SCOPE模型，将供应链实体表示为令牌，通过共享的运营表示进行上下文化，并将每个令牌类型映射到相应的决策接口。

Result: 在两个大型供应链（Dingdong和JD.com）的真实运营数据上评估，SCOPE在两个场景中均优于单独优化每个决策阶段的方法和实践导向的基线。

Conclusion: 跨部门操作耦合的学习和协调导致更有效的端到端供应链决策。
...
TL;DR: MANTA框架通过在推理时自适应地调整通信拓扑，显著提高了多人代理系统的复杂问题解决能力。


<details>
  <summary>Details</summary>
Motivation: 为了提高复杂问题的解决能力，基于大型语言模型的多人代理系统通过任务分解、代理专业化、信息交换和中间验证来改进。然而，现有的系统通常将通信拓扑视为固定的设计选择或离线优化目标。

Method: MANTA框架，一种多人代理网络拓扑自适应框架，允许在推理时自我进化通信结构。在执行之前，MANTA从先前的结构经验初始化任务条件拓扑。在部署期间，它监控协作痕迹并在当前组织不足时应用有限的拓扑更新。这些更新可以修改代理角色、通信链接、执行顺序、信息可见性和验证路径，同时保留任务接口和代理预算。

Result: 在五个基准测试中，包括信息搜索、工具使用、规划、工作流执行和数学推理，MANTA的平均分数为74.0，比最强的基线高出5.8个百分点，并在PlanCraft上获得最佳结果。这些结果表明，推理时间自我改进可以扩展到协作架构本身。

Conclusion: MANTA框架通过推理时间自我改进，在复杂问题解决中表现出色，证明了拓扑自适应在多人代理系统中的重要性。
...
TL;DR: 通过实证研究，我们发现提高本地CUA性能的方法需要综合考虑上下文、时间、结构和并行维度，并采取针对性的优化策略。


<details>
  <summary>Details</summary>
Motivation: 为了提高在严格硬件约束下自主计算机使用代理（CUA）的性能，同时考虑到隐私、成本效率和实用性。

Method: 系统性实证研究，评估了Qwen3-VL-8B/30B-A3B、UI-TARS-1.5-7B和OpenCUA-7B在OSWorld基准上的表现。

Result: 发现额外的计算往往会产生递减的回报，并且会改变失败模式。上下文扩展提供了历史基础，提高了轨迹稳定性和任务准确性，但随着标记成本的提高，其收益会饱和，失败模式从重复或停滞的轨迹转变为过早的假成功。时间扩展可以减少最大步数的停滞，但并没有显著提高任务成功率，表明更长的预测范围往往延长了错误轨迹而不是纠正它们。结构分解会在本地两阶段代理中引入规划和格式化开销，而并行扩展在巨大的计算成本下部分缓解了这些失败。

Conclusion: 高效的本地CUA需要选择性的计算分配、故障感知的控制机制和围绕本地模型的能力和限制设计的代理框架。
...
TL;DR: DualG-MRAG通过双层框架和GNN检索器，提高了多模态检索增强生成的准确性和效率。


<details>
  <summary>Details</summary>
Motivation: 解决现有多模态检索增强生成（MM-RAG）方法在复杂多跳推理任务中的不足，特别是跨模态和文档之间显式关系的捕捉问题。

Method: 提出了一种名为DualG-MRAG的双层框架，包括宏观推理图和微观匹配图，用于多模态RAG。使用GNN检索器进行查询驱动的消息传递过程，并引入动态规划解码机制以提取推理路径。

Result: 在证据召回和复杂问答准确率方面优于基线方法。

Conclusion: DualG-MRAG在多模态检索增强生成任务中表现出色，提高了推理和匹配的准确性。
...
TL;DR: 本文研究了递归Transformer在工程设计中的代理建模，提高了预测精度和效率。


<details>
  <summary>Details</summary>
Motivation: 工程设计中，使用传统Transformer架构进行高成本的第一性原理模拟存在过参数化问题，导致过拟合、不必要的内存和计算开销。因此，研究具有更多计算而不是更多可学习参数的架构。

Method: 提出硬件感知的三种递归Transformer范式，并在两个低维工程预测任务中评估它们的预测性能、参数数量和计算复杂度。

Result: 递归权重共享Transformer在小数据工程代理建模中，在预测精度、参数效率和计算成本之间提供了一个有效且可推广的权衡。

Conclusion: 递归Transformer架构在资源受限的情况下为工程设计的代理建模提供了有效的解决方案。
...
TL;DR: 提出了一种名为CMCDR的方法，用于缓解模态贡献漂移，提高多模态持续学习的效果。


<details>
  <summary>Details</summary>
Motivation: 为了学习多模态数据中的新知识同时保留知识，缓解遗忘问题。

Method: 提出了一种名为CMCDR的方法，用于缓解模态贡献漂移（MCD）。该方法包括基于重放的版本和无重放版本。基于重放的版本使用模态子集干预作为诊断探针，比较当前模型和冻结的先前模型之间的贡献配置文件，并约束旧样本的模态特定和交互贡献的变化。无重放版本使用当前任务的样本作为探针，提炼冻结模型的旧任务贡献响应，从而在没有示例的情况下正则化观察到的贡献配置文件。

Result: 在多模态类增量学习和持续视觉问答上的实验验证了CMCDR的通用性和有效性。

Conclusion: CMCDR方法能够有效地缓解模态贡献漂移，提高多模态持续学习的效果。
...
TL;DR: DoTime是一个新的时间序列因果推断工具，提供了多种功能，包括连续时间干预窗口和反事实采样模式，并证明了其在结构匹配评估中的有效性。


<details>
  <summary>Details</summary>
Motivation: 由于现有的时间序列因果推断基准大多是观察性的、规模较小或特定领域的，因此在医疗保健、政策评估和气候科学等最需要的地方，干预和反事实估计没有得到充分的服务。

Method: 提出了DoTime，一个开源、可扩展且理论上有根据的多变量时间结构因果模型（TSCM）生成器，具有干预功能。

Result: DoTime增加了先前生成器中缺少的功能，如连续时间干预窗口、具有积极性保护的反事实采样模式、作为中断时间序列严格推广的体制切换SCM、通过切换SCM参数构建的非平稳动态，以及放置在评估窗口内的确定性斜坡和正弦波干预配置文件。

Conclusion: DoTime作为一个先验，适合作为因果基础模型参考实现的生成器。在结构匹配的评估中，干预先验拟合网络（PFN）的差距在测试的每个结构、轨迹长度和种子中都是正的。
...
TL;DR: 提出PlatformBid基准和BidFlow方法，以提升现代广告平台的自动竞价效果。


<details>
  <summary>Details</summary>
Motivation: 传统自动竞价算法仅关注DSP端，而现代广告平台需要考虑广告商转化率和平台总收益。

Method: 提出PlatformBid，一个以广告平台为中心的综合基准，并定义三种代表性场景：同质竞争、异质竞争和促销竞争。评估多种现有自动竞价方法，并提出基于流量匹配的新方法BidFlow。

Result: BidFlow在快手上的在线实验中显示出目标成本降低了0.68%。

Conclusion: PlatformBid提供了评估自动竞价方法的统一框架，并证明了BidFlow在现实场景中的有效性。
...
TL;DR: 直接转换MHA/GQA到MLA可提升LLM推理效率，功能重建方法有效。


<details>
  <summary>Details</summary>
Motivation: 为了提高长上下文LLM推理的效率，研究MLA（多头潜在注意力）在多头或分组查询注意力（MHA/GQA）转换中的应用。

Method: 提出了一种直接从MHA/GQA转换为MLA的方法，通过优化转换后的MLA注意力模块，以复制原始MHA/GQA的输出响应。

Result: 实验表明，功能重建方法在64个匹配的任务单元中有37个的接受度显著提高，26个保持不变，1个有所下降。

Conclusion: 直接从MHA/GQA转换为MLA可以显著提高推理效率，功能重建方法是一种有效的转换策略。
...
TL;DR: 提出了一种新的强化学习方法，用于训练代码代理优先选择更快的正确实现，提高了程序的正确性和效率。


<details>
  <summary>Details</summary>
Motivation: 代码模型训练中存在效率与正确性之间的差距，需要训练代码代理优先选择更快的正确实现。

Method: 提出了一种名为RLPF的强化学习方法，将执行结果转化为阶段性奖励。

Result: 在PerfCodeBench上，使用RLPF微调Qwen3-32B模型，将正确且可运行的解决方案从11.1%提高到54.6%，并提高了相对效率从8.1%到38.6%。

Conclusion: 代码代理可以不仅通过测试，还可以优化他们编写的程序。
...
TL;DR: SDO通过结构感知的数据组织提高了大语言模型训练效率，并改善了样本曝光均衡性。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型训练效率较低，数据组织作为静态预处理步骤无法适应优化过程中的样本曝光变化，导致样本曝光不均衡。

Method: 提出了一种名为SDO的数据组织框架，该框架根据表示空间结构进行组织，并具有曝光驱动的反馈机制。

Result: SDO在SFT、DPO和GRPO上加速了收敛，特别是在早期到中期阶段，产生了更一致的梯度和更平衡的准确率。

Conclusion: SDO通过优化数据组织提高了大语言模型训练的效率，并改善了优化过程中的样本曝光均衡性。
...
TL;DR: BridgeMIL通过将实例表示学习与主题级监督解耦，提高了EEG疾病诊断的准确率。


<details>
  <summary>Details</summary>
Motivation: EEG数据集中的主题数量远少于实例，这可能会限制端到端MIL学习到的表示的质量。

Method: BridgeMIL是一个两阶段框架，将实例表示学习与主题级监督解耦。第一阶段预训练编码器，不使用继承的实例标签，通过对时间上邻近的窗口进行对齐，并在独立采样的主题子包内进行。方差和协方差正则化防止崩溃并减少冗余。第二阶段将编码器转移到基于注意力的MIL聚合器，仅对主题预测应用监督，并通过特征保留限制表示漂移。

Result: BridgeMIL在三个EEG疾病数据集和五个代表性骨干上进行了测试，结果表明它比最强的基线提高了准确率。

Conclusion: BridgeMIL在15个数据集-骨干设置中的14个中取得了最高的平均准确率，整体平均准确率为76.57%，比最强基线高4.28个百分点。
...
TL;DR: 无部分信用联合对齐是能力形成的瓶颈，重新初始化查询-键切片可恢复学习能力


<details>
  <summary>Details</summary>
Motivation: 研究语言模型中能力形成的过程和限制因素

Method: 通过实验和理论分析

Result: 发现无部分信用联合对齐是能力形成的瓶颈，并提出了解决方案

Conclusion: 无部分信用联合对齐是能力形成的瓶颈，可以通过重新初始化查询-键切片来恢复学习能力
...
TL;DR: TIER-MoE是一种风险引导的混合专家模型，在多模态融合中优于现有方法，提高了预测性能和概率校准，并实现了零样本泛化。


<details>
  <summary>Details</summary>
Motivation: 结合互补的证据来源以实现多模态融合的承诺，但更多证据并不总是导致更好的预测。

Method: 引入TIER-MoE，这是一种风险引导的子空间混合专家模型，它将样本特定的模态可靠性定义为单模态预测器预期产生的预测损失。该风险来自未在相应样本上训练的模型生成的超出折叠预测。TIER-MoE将估计的风险与专家特定的子空间兼容性相结合，进行稀疏的模态-专家路由，同时始终活跃的共享路径保留了多模态互补性。

Result: 在四个公共多模态生物医学数据集上评估TIER-MoE，涵盖了阿尔茨海默病状态、皮肤病变恶性和视网膜分类。结果表明，它在预测性能和概率校准方面优于最先进的方法，在宏-F1和Brier分数上持续改进，并对外部队列表现出强大的零样本泛化能力。

Conclusion: TIER-MoE在多模态融合方面具有优越性，优于现有方法，提高了预测性能和概率校准，并实现了零样本泛化。
...
TL;DR: EvoCause算法通过LLM和确定性代码优化警报级联的根因分析，显著提升了警报识别的准确性。


<details>
  <summary>Details</summary>
Motivation: 现代电信、云计算和微服务系统在组件故障时会产生相关的警报级联。根因分析（RCA）旨在识别触发每个级联的小集合警报。

Method: EvoCause算法利用大型语言模型（LLM）提出语义上合理的图编辑，同时使用确定性代码验证节点身份和无环性，并在标记对齐集合上保留最佳图。

Result: 在合成数据上，EvoCause算法在PC因果发现算法初始化后优于未优化的PC基线，分别提高了Node F1、Case EM和Graph F1的11.59、9.40和4.59个百分点，同时降低了nSHD 0.2379。在TeleRCA上，用匿名标识符替换可读的警报标题分别降低了Node F1和Case EM 6.12和8.04个百分点，表明警报名称信息有助于图优化。

Conclusion: EvoCause算法能够有效地优化警报级联的根因分析，提高警报识别的准确性。
...
TL;DR: THGFM模型通过结合共享空间时序注意力和关系类型分区时序注意力，在时序异构图学习方面取得显著成果。


<details>
  <summary>Details</summary>
Motivation: 动态关系系统中存在多种节点和关系类型，需要同时建模跨类型结构异构性和交互的时序动态。

Method: 提出THGFM模型，结合共享空间时序注意力和关系类型分区时序注意力，并通过双路径关系-共享融合实现。

Result: THGFM在学术图基准测试中优于基线图变换器模型，平均提升3.25%，在特定任务上提升更高。

Conclusion: THGFM模型有效地解决了跨类型迁移和关系感知专业化的难题，在时序异构图学习方面取得了显著成果。
...
### [332] [Strategies for Milestone-driven Start-ups in Multi-activity Settings](https://arxiv.org/abs/2607.27563)
*Zhengli Wang*

Main category: cs.LG

TL;DR: 研究新创企业的里程碑导向策略，发现最优策略仅使用效率前沿曲线定义的一组控制。


<details>
  <summary>Details</summary>
Motivation: 新创企业需要通过多个阶段的里程碑目标来实现生存。

Method: 研究创业公司模型，状态由扩散过程捕捉。企业家可以选择不同的活动（或控制），这些活动具有不同的成本并决定了过程的漂移和方差。

Result: 完全解决了最优策略问题，并提供了其结构的显式描述。最优策略只使用由所谓的效率前沿曲线定义的一组控制，该曲线根据两个直观的度量排序控制：风险性（漂移到波动率比率）和成本效益（漂移到成本比率）。

Conclusion: 该研究为创业决策提供了基础。
...
### [333] [Position, Not Provenance: Separating Reasoning Mediation from Sycophancy in Medical Vision-Language Models](https://arxiv.org/abs/2607.27304)
*Supratik Bhowal,Subhrajyoti Basu,Aritra Gir Mahanta,Anik Pal Chowdhury*

Main category: cs.LG

TL;DR: 通过实验验证了医疗VLM的推理对预测有因果影响，并分析了不同干预措施对推理忠实度的影响。


<details>
  <summary>Details</summary>
Motivation: 为了确定VLM在回答临床问题前生成的推理是否因果地影响预测。

Method: 通过双臂协议比较重新提示的证据与强制前缀的续写，结合来源控制的干预，扰动模型生成的推理中的单个临床有意义的属性，并测量结果预测是否遵循编辑后的推理。

Result: 强制前缀续写比重新提示产生更高的推理忠实度；来源分析揭示了不同的模型特定从属行为；去除视觉证据会增加对注入推理的依赖，而左右对称性是跟踪最不忠实的临床属性。

Conclusion: 医疗视觉语言模型（VLM）的推理对预测有因果影响，但注入推理的机制显著影响测量忠实度，上下文位置是决定VLM使用生成推理的主要因素。
...
TL;DR: FLAMECHE通过提高客户端模型的有效性，在CFL三难困境中实现了效率与隐私保护之间的平衡
...
Method: FLAMECHE，将基于元数据的CFL重新表述为分布式EM过程

Result: 提高客户端模型的有效性，使加密兼容的基于元数据的聚类成为可能

Conclusion: FLAMECHE在CFL三难困境中实现了效率与隐私保护之间的平衡
...
### [335] [ZUNA1.1: A more flexible EEG foundation model for Denoising and Super-resolution](https://arxiv.org/abs/2607.27308)
*Christopher Warner,Jonas Mago,JR Huml,Beren Millidge*

Main category: cs.LG

TL;DR: ZUNA1.1：灵活高效的EEG信号重建模型，开源发布。


<details>
  <summary>Details</summary>
Motivation: 为了提高EEG信号重建的灵活性

Method: 提出了一种名为ZUNA1.1的扩散自动编码器，参数量为380M

Result: ZUNA1.1能够重建长度可达30秒的任意长度的EEG信号序列，支持任意数量的脑电图通道和头皮位置，以及通道内的任意时间间隔重建。ZUNA1.1在性能上至少与ZUNA模型相当，且更灵活，能够处理更广泛的重建任务。ZUNA1.1在标准EEG去噪和重建方法（如球面样条插值）上表现优异。

Conclusion: ZUNA1.1是一种灵活且高效的EEG信号重建工具，具有开源许可。
...
TL;DR: 本文研究了交互式系统中的策略学习，提出了多种方法以解决大动作空间中的挑战。


<details>
  <summary>Details</summary>
Motivation: 研究交互式系统中的策略学习，其中代理观察上下文，从非常大的动作集中选择动作，并接收部分反馈。

Method: 使用上下文赌博机框架，包括按策略学习和离策略学习两种范式。

Result: 开发了结构化贝叶斯方法、sDM方法和基于指数平滑的微分悲观方法。

Conclusion: 提出的方法能够有效解决大动作空间中的挑战，如探索效率低、数据覆盖稀疏、重要性权重方差高、外推偏差和优化景观困难等问题。
...
### [337] [Modeling Decisions in Blockchain Analytics: A Leakage-Aware Evaluation of Tree-Based vs. Sequential Models](https://arxiv.org/abs/2607.27350)
*Michał Bartnicki,Jarosław A. Chudziak*

Main category: cs.LG

TL;DR: 提出了一种基于泄漏感知的Sybil攻击者检测方法，结果显示XGBoost在性能和效率方面优于其他模型


<details>
  <summary>Details</summary>
Motivation: 分析Sybil攻击者检测方法的有效性和效率

Method: 提出了一种盲点协议和交易语法表示，用于检测Sybil攻击者

Result: XGBoost在低延迟和能源消耗方面优于Transformer模型

Conclusion: 泄漏感知的Ethereum演员分类框架和交易语法表示有效
...
### [338] [Compression-Based Behavioral Similarity for Open-World Sybil Discovery on Ethereum](https://arxiv.org/abs/2607.27370)
*Michał Bartnicki,Jarosław A. Chudziak*

Main category: cs.LG

TL;DR: 提出了一种新的Sybil检测方法，该方法的NCD原始数据不需要监督训练，并且可以在没有明确资金链的情况下扩大可疑种子钱包。


<details>
  <summary>Details</summary>
Motivation: 现有的Sybil检测方法容易受到攻击策略和逃避战术的变化的影响。

Method: 从EVM跟踪中合成符号交易语法，分别捕捉交易节奏、执行结构和功能意图。使用盲点协议过滤高信号合约。使用基于Gzip的NCD构建行为图以发现Sybil。

Result: 提出了一种新的Sybil检测方法，可以有效地检测Sybil攻击者，并且具有较好的鲁棒性。

Conclusion: 贡献了一个对Sybil候选人进行发现的行为框架，该框架的核心NCD原始数据不需要监督训练，并且可以在没有明确资金链的情况下扩大可疑种子钱包。
...
### [339] [Explorative Modeling: Unlocking a Third Pretraining Axis and End-to-End Generation](https://arxiv.org/abs/2607.27372)
*Alexi Gladstone,Heng Ji,Yilun Du*

Main category: cs.LG

TL;DR: Explorative Modeling improves generative models by exploring candidate matches and training on the best.


<details>
  <summary>Details</summary>
Motivation: Generative modeling has not been trained end-to-end despite its remarkable capabilities. This is because handling distributions with many modes requires factoring the generation procedure, which prevents end-to-end generation.

Method: Explorative Modeling, which factors the training loop and explores K candidate matches between model generations and data, training on the best.

Result: Explorative Models (XMs) improve performance in both continuous and discrete domains, increase efficiency, and enable end-to-end reconstructive generative modeling.

Conclusion: Explorative Modeling is a new pretraining axis for existing generative models and a standalone end-to-end generative modeling paradigm.
...
### [340] [The Convergence Behavior of Adam under Heavy-Tailed Noise](https://arxiv.org/abs/2607.27383)
*Yijiang Pang*

Main category: cs.LG

TL;DR: 研究了Adam优化器在重尾噪声下的收敛性，发现其在某些情况下存在次优性，但在特定条件下收敛率可达到最优。


<details>
  <summary>Details</summary>
Motivation: 针对现代深度学习中常见的重尾随机噪声，研究Adam优化器的收敛性保证。

Method: 推广在线到非凸转换框架，以适应重尾鞅差分噪声，并开发折扣后悔分析。

Result: Adam在重尾噪声下收敛到(ρ,ε)平稳点，但迭代复杂度和p依赖收敛性存在次优性。当领域半径已知且用于控制在线学习器输出时，收敛率提高以匹配最优复杂度。

Conclusion: 为Adam在重尾噪声环境下的鲁棒性和局限性提供了新的理论见解。
...
### [341] [FunL2O: LLM-Guided Feature Function Design for Learning to Optimize](https://arxiv.org/abs/2607.27389)
*Bingheng Li,Junyang Cai,Yupeng Zhang,Bistra Dilkina,Jayant Kalagnanam,Dzung T. Phan*

Main category: cs.LG

TL;DR: FunL2O通过LLM自动化特征设计，在优化任务中表现出色。


<details>
  <summary>Details</summary>
Motivation: 加速重复优化，通过训练模型预测解决方案、暖启动、分支决策或其他形式的求解器指导。

Method: 使用大型语言模型（LLM）驱动程序演化来自动化特征设计。

Result: 在连续和离散优化任务以及四种LLM上，演化的特征始终优于手工制作的表示。

Conclusion: LLM驱动的特征演化是自动设计L2O中表示的通用且有效的方法。
...
### [342] [ECG-InterpBench: Benchmarking the Interpretability of ECG Foundation Models with Matched-Scale Sparse Autoencoders](https://arxiv.org/abs/2607.27404)
*Yixuan Duan,Wei Qiu*

Main category: cs.LG

TL;DR: ECG-InterpBench是一个评估ECG基础模型表示可解释性的基准，揭示了不同模型的可解释性特征，并验证了重建保真度和临床可达性可以识别不同领先的模型。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要评估下游预测性能，而ECG-InterpBench旨在评估ECG基础模型表示的可解释性。

Method: 使用稀疏自编码器作为标准化测量工具，并匹配模型之间的容量以进行可控比较。

Result: 评估了六种冻结的ECG基础模型，并产生了450个细胞的可解释性图谱，包括75个精确匹配的六模型比较块。

Conclusion: ECG-InterpBench通过系统评估ECG基础模型表示的可解释性，揭示了不同模型的可解释性特征，并验证了重建保真度和临床可达性可以识别不同领先的模型。
...
### [343] [Context-Informed Ship Trajectory Prediction via Conditional Attention](https://arxiv.org/abs/2607.27418)
*Yuan Guan,Chandler Squires,Timothy Hu,Pradeep Ravikumar*

Main category: cs.LG

TL;DR: 提出了一种新的方法，通过条件信息器提高了船舶轨迹预测的准确性，并使用模态掩码策略提高了预测的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 长期船舶轨迹预测对于海事安全和自主导航至关重要。然而，现有的基于Transformer的架构主要依赖于历史运动状态，将船舶运动视为一个孤立系统。实际上，海事导航受到天气等外部因素的影响，并受到静态船舶特性的限制。现有的多模态方法基本建立了状态和上下文的联合分布模型，将环境变量视为同等特征，而不是编码船舶动力学对环境条件的方向性物理依赖。

Method: 提出了一种名为条件信息器的编码器-解码器架构，将轨迹预测作为条件生成任务。使用专门的交叉注意力机制，通过跨注意力使船舶状态显式查询环境上下文，编码天气调节船舶动力学但不是由其生成的物理先验。引入模态掩码训练策略以解决实际数据的不连续性。

Result: 在AIS和ERA5数据上的大量实验表明，当上下文可用时，该方法在预测精度上优于基于运动和连接的基线15.4%。模态掩码策略防止了捷径学习，与无约束模型相比，减少了近一个数量级的故障错误。

Conclusion: 条件信息器通过编码船舶动力学与环境条件之间的物理依赖，提高了长期船舶轨迹预测的准确性。模态掩码训练策略有效防止了捷径学习，提高了预测的鲁棒性。
...
### [344] [Good Rankers, Bad Objectives: Bilinear Contrastive Critics under Expressive Policy Search](https://arxiv.org/abs/2607.27422)
*Ayushman Singh,Siddharth Aphale*

Main category: cs.LG

TL;DR: Contrastive critics are useful but need value calibration for action selection.


<details>
  <summary>Details</summary>
Motivation: Good action rankings do not make a contrastive critic safe to maximize.

Method: Study on action rankings and critics in reinforcement learning.

Result: Cosine and hybrid critics select off-support actions, contrastive scores are weakly calibrated, Bellman-trained TD-Q succeeds, realized costs depend on the task, candidate maximization can exploit false positives, contrastive critics remain useful compatibility rankers.

Conclusion: Contrastive critics are useful but require value-calibrated scalar for action selection.
...
### [345] [SE(3)-MeanFlow: Few-Step Protein Backbone Generation on Lie Groups](https://arxiv.org/abs/2607.27431)
...
<details>
  <summary>Details</summary>
Motivation: 为了提高蛋白质骨架的生成效率，减少计算瓶颈。

Method: 提出了一种名为SE(3)-MeanFlow的生成框架，将MeanFlow从欧几里得空间扩展到蛋白质框架的Lie群几何。

Result: SE(3)-MeanFlow在蛋白质骨架生成中表现出色，与使用更多采样步骤的基线相比，其性能相当或更好，在少量步骤的设置中优势更加明显。

Conclusion: SE(3)-MeanFlow是一种有效的蛋白质骨架生成方法，可以显著提高生成效率。
...
TL;DR: 本研究提出了一种新的警报处理方法，有效提高了安全运营中心（SOC）的警报处理效率。


<details>
  <summary>Details</summary>
Motivation: 解决安全运营中心（SOC）的警报疲劳问题，提高警报处理效率。

Method: 自动化提示优化、自学习和强化学习，结合思维链推理训练分级分类器。

Result: 系统在测试集上达到了82.6%的准确率，在高置信度操作点下，良性召回率提高了43.0%，恶意召回率提高了18.3%。

Conclusion: 本研究提出了一种基于思维链推理的分级分类器，通过结合自动化提示优化、自学习和强化学习，提高了安全运营中心（SOC）的警报处理效率。
...
### [347] [Neural Network-Assisted CLEAN for Channel Modeling in Low-SNR Regimes](https://arxiv.org/abs/2607.27450)
*Chaofan Deng,Linyu Sun,Jaeho Lee,Arijit Raychowdhury*

Main category: cs.LG

TL;DR: NN-CLEAN是一种结合了深度学习和传统算法的混合框架，用于提高无线通信系统中的多径参数估计精度和效率。


<details>
  <summary>Details</summary>
Motivation: 准确的多径参数估计对于现代无线通信系统至关重要，尤其是在低信噪比环境下。传统的最大似然估计算法，如CLEAN，由于穷举网格搜索而具有过高的计算复杂度。另一方面，纯粹的数据驱动深度学习方法缺乏物理基础，难以泛化到可变的multipath密度和离网参数。

Method: 提出了一种名为NN-CLEAN的混合框架，该框架将多头残差网络直接嵌入到迭代CLEAN提取循环中。通过用快速、可并行化的前向传递替换穷举网格搜索，并将残差减法委托给精确的数学模型，NN-CLEAN可以隔离物理multipath参数，而不会积累非物理误差。

Result: 广泛的蒙特卡洛模拟表明，NN-CLEAN在5 dB信噪比下实现了超过96%的估计精度，与传统的网格搜索CLEAN（GS-CLEAN）基线相匹配，同时提供了巨大的计算复杂度降低，并且显著优于子空间方法和独立的单次神经网络。关键的是，随着批量大小的增加，NN-CLEAN的执行运行时间和内存消耗几乎保持不变。这种高效的并行化使NN-CLEAN成为MIMO系统中信道估计的鲁棒、实时解决方案。

Conclusion: NN-CLEAN是一种高效、鲁棒的MIMO系统信道估计方法，在低信噪比环境下具有优异的性能。
...
### [348] [FADEx: Feature Attribution and Distortion-based Explanation of Dimensionality Reduction](https://arxiv.org/abs/2607.27463)
...
### [349] [Schreier-Coset Graph Rewiring](https://arxiv.org/abs/2607.27479)
...
### [350] [Latent States in Neural Networks: Recovering the Temporal Structure of Drifting Data from Model Weights](https://arxiv.org/abs/2607.27482)
*Kevin Guan*

Main category: cs.LG

TL;DR: 该方法能够从模型权重中恢复数据流中的时变状态，并优于简单的状态分割方法


<details>
  <summary>Details</summary>
Motivation: 分析数据流中时变状态的恢复能力

Method: 使用隐马尔可夫模型（HMM）对模型权重轨迹进行拟合

Result: 在两个数据集上验证了方法的有效性，包括Fakeddit数据集的多模态虚假信息检测和Yelp数据集的情感分析

Conclusion: 方法能够恢复与数据分布相关的结构，并超越了简单的连续状态分割方法
...
### [351] [A Lightweight Foundation Model for Collider Physics with Multi-Domain Adaptation](https://arxiv.org/abs/2607.27501)
*Liangyu Wu,Qibin Liu,Alexander Yue,Julia Gonski*

Main category: cs.LG

TL;DR: NEXUS：一种高效且准确的轻量级基础模型，在科学数据集中实现高性能。


<details>
  <summary>Details</summary>
Motivation: 为了提高在科学数据集中处理非域内任务的准确性和效率，提出了一个轻量级的模型NEXUS。

Method: 使用一个包含约300万个参数的完全连接自动编码器模型，在大型强子对撞机（LHC）的碰撞数据集上进行预训练，然后在上游任务中应用预训练的模型权重。

Result: 与从头开始训练的等效架构相比，在只有少量标记数据集的情况下，下游任务（如动力学回归和事件分类）在预训练模型权重上实现了更高的准确性。此外，NEXUS相对于同等规模的转换器方法在计算上更简单，使其适用于高效推理和实时或边缘应用。

Conclusion: NEXUS是一种高效且准确的轻量级基础模型，可以应用于多个科学领域，包括引力波、洪水预测和神经活动。
...
### [352] [Sparsity Induced Identifiability in Matrix Tri-Factorisation](https://arxiv.org/abs/2607.27507)
*Tingting Mu*

Main category: cs.LG

TL;DR: 本研究对通用实值矩阵三因子分解中的稀疏性引起的可识别性问题进行了严格的理论研究，并通过实验验证了理论的有效性。


<details>
  <summary>Details</summary>
Motivation: 为了解决通用实值矩阵三因子分解中稀疏性引起的可识别性问题，我们建立了第一个严格的理论研究。

Method: 通过一种新颖的分解策略，将原始问题转化为两个耦合的辅助分解问题，同时保留从观测值恢复原始因子矩阵所必需的结构信息。

Result: 推导出恢复保证和结构一致性结果，描述了系数稀疏性如何影响充分恢复条件、收敛行为、谱近似误差、高概率界限和结构保持。

Conclusion: 通过蒙特卡洛实验验证了所提出的理论，并证明了理论结果与经验观察之间的密切一致。
...
### [353] [Expanding Data-Agnostic Pivotal Instances Selection Models with Proximity Trees and Ensemble Learning](https://arxiv.org/abs/2607.27522)
*Alessio Cascione,Mattia Setzu,Cristiano Landi,Paolo Maria Mancarella,Riccardo Guidotti*

Main category: cs.LG

TL;DR: 提出了一种基于枢轴选择的可解释预测模型，在多个数据集上取得了优异的性能。


<details>
  <summary>Details</summary>
Motivation: 随着决策过程的复杂化，机器学习工具已成为解决商业和社会挑战的必要手段。然而，许多现有方法依赖于难以解释的决策过程。由于人类自然通过将新案例与少数代表性示例进行比较来做出决策，我们旨在设计一种选择此类枢轴来构建可解释预测模型的方法。受决策树的启发，我们提出了一种基于枢轴与输入实例之间相似性的分层、可解释的枢轴选择模型。我们的方法既是一种枢轴选择技术，也是一种独立的预测模型。超越单个枢轴，我们结合了邻近树和斜树以及集成中使用的枢轴对，增强了我们提议的灵活性和有效性。此外，我们的方法对数据模态无关，利用预训练网络进行数据转换。在包括表格数据、文本、图像和时间序列在内的各种数据集上的实验证明了我们方法的有效性，优于替代实例选择策略，在保持枢轴数量最少的情况下，与最先进的可解释模型取得了具有竞争力的结果。

Method: 提出了一种基于枢轴与输入实例之间相似性的分层、可解释的枢轴选择模型。结合了邻近树和斜树以及集成中使用的枢轴对，增强了提议的灵活性和有效性。利用预训练网络进行数据转换。

Result: 在包括表格数据、文本、图像和时间序列在内的各种数据集上的实验证明了我们方法的有效性，优于替代实例选择策略，在保持枢轴数量最少的情况下，与最先进的可解释模型取得了具有竞争力的结果。

Conclusion: 该方法在保持枢轴数量最少的情况下，在多个数据集上取得了具有竞争力的结果，表明了其在解决复杂决策问题中的潜力。
...
### [354] [Latent-Kernel Discrete Flow Maps for Few-Step Generation](https://arxiv.org/abs/2607.27529)
*Mansoor Ahmed,Yue-Tsz Fan,Hemanth Venkateswara,Murray Patterson*

Main category: cs.LG

TL;DR: LKF模型通过引入相关步骤的表达能力，显著提高了文本生成的质量。


<details>
  <summary>Details</summary>
Motivation: 为了解决离散扩散和流匹配模型在生成文本时难以处理位置耦合的问题，例如主语和动词必须一致的情况。

Method: 提出了一种名为Latent-Kernel Discrete Flow Maps (LKF)的新模型，该模型通过混合M个因式分解组件并使用单个共享潜在来表示相关步骤。

Result: 实验结果表明，LKF模型在One-Billion-Word (LM1B)和WikiText-103基准测试中，在不损失多样性的情况下，将生成困惑度提高了2.1倍到3.3倍。

Conclusion: LKF模型能够有效地提高文本生成的质量，并且随着M值的增加，其性能进一步提升。
...
### [355] [When Does Explicit View Routing Work? A Controlled Study of Multi-View Graph-Text Alignment](https://arxiv.org/abs/2607.27530)
*Xiao Yue,Guangzhi Qu*

Main category: cs.LG

TL;DR: This paper investigates conditions for effective graph-text retrieval, finds improvements with specific routing and augmentation strategies, but evidence is limited to certain routing and consolidation approaches.


<details>
  <summary>Details</summary>
Motivation: Graph-text retrieval typically maps a graph and its description to a single embedding, even when a query concerns only one semantic aspect.

Method: Our controlled version of MV-GTA uses deterministic, verifiable text segments; isolated text encoders; view-specific graph heads; and relevance derived from external labels or RDKit descriptors.

Result: Correct routing improves label and property nDCG, topology does not specialize consistently across datasets, property paraphrase augmentation improves unseen-template nDCG, consistency and hard-template extensions reduce canonical retrieval in some settings.

Conclusion: The evidence is limited to explicit, externally grounded label and property routing and observed multi-interface consolidation. It does not establish free-form routing, consistent three-view specialization, statistical equivalence to specialists, or superior downstream prediction.
...
### [356] [Subtract or Replay? Exact Deletion from Language-Model Memory](https://arxiv.org/abs/2607.27539)
*Vishwajith Ramesh*
...
### [357] [Memory Efficient Tabular Foundation Models](https://arxiv.org/abs/2607.27546)
*Shuting Luo,Monika Mikhail Kanaan,Cameron Gordon,Anna Leontjeva,Simon Lucey*

Main category: cs.LG

TL;DR: 本研究通过模型压缩方法显著降低了Tabular Foundation Models的内存需求，提高了实际部署效率。


<details>
  <summary>Details</summary>
Motivation: Tabular Foundation Models在表格式机器学习任务中的表现超过了经典基线，但它们的实际部署考虑却未得到足够的关注。

Method: 研究这些模型的内存需求，并采用模型压缩方法。

Result: 模型压缩方法可以实现高达7.6的内存减少，同时保持相似的性能水平，将部署需求减少近87%。

Conclusion: 为寻求在实际环境中高效部署这些模型的从业者提供了见解。
...
### [358] [A Montage-Agnostic Encoder for Calibration-Light Cross-User Gesture Recognition from Surface Electromyography](https://arxiv.org/abs/2607.27565)
...
### [359] [Recognition and Label-Free Adaptation Across Recording Sessions in Surface-EMG Gesture Decoding](https://arxiv.org/abs/2607.27568)
*Jethro Odeyemi,W. J. Zhang*

Main category: cs.LG

TL;DR: 研究建立了一种新的编码器，提高了肌电图控制系统的识别准确性。


<details>
  <summary>Details</summary>
Motivation: 提高基于肌电图控制系统的实际应用效果

Method: 使用数据收集和模型训练，建立蒙太奇无关的编码器，用于跨用户、跨蒙太奇迁移

Result: 编码器在跨记录会话中保持了0.688的宏F1分数，优于个人LDA分类流程和两个已发表的仅依赖于同一次记录会话数据的流程。特征统计对齐是五种无标签测试时自适应方法中唯一对每个受试者都有改善的方法。

Conclusion: 蒙太奇无关的编码器可以有效地提高肌电图控制系统的识别准确性和稳定性，减少实际应用中的变异性。
...
### [360] [Policy Gradient Steering: Interventions from Behavioral Objectives](https://arxiv.org/abs/2607.27574)
*Yoann Poupart,Aurélie Beynier,Nicolas Maudet*

Main category: cs.LG

TL;DR: 提出PGS方法，成功引导模型在多决策领域构建临时和可组合行为适应。


<details>
  <summary>Details</summary>
Motivation: 现有的引导方法在两路网格世界中无法引导简单的策略，因此需要新的方法。

Method: 提出了一种名为PGS（政策梯度引导）的新方法，将引导问题定义为强化学习问题，并通过对少量运行或演示中的行为目标梯度进行累积来构建可移除的任务向量。

Result: PGS在两路网格世界中展示了校准和可逆性；在棋局中独立拟合的PGS向量既可独立使用也可组合使用；在足球比赛中，PGS可以改变特定团队行为，并且其效果可以跨对手转移。

Conclusion: 政策梯度提供了在各个决策领域中构建临时和可组合行为适应的天然接口。
...
### [361] [MUGEN: A Unified Framework for Efficient Motion Understanding and Generation](https://arxiv.org/abs/2607.27581)
*Zhankai Ye,Yukai Jin,Bingyang Wei,Bofan Li,Yusen Wu,Fangyi Li,Shangqian Gao,Xin Liu*

Main category: cs.LG

TL;DR: MUGEN是一种新的统一动作-语言框架，在多个任务中取得了优异的性能。


<details>
  <summary>Details</summary>
Motivation: 为了使物理AI系统能够理解、生成和交流人类行为，需要将人类动作与语言相结合。

Method: 提出了一种名为MUGEN的统一动作-语言框架，该框架不使用代码簿，而使用单个自适应长度自动编码器将任何长度的动作压缩为几个连续的潜在槽位。语言模型为文本到动作生成它们，并读取它们以理解动作。深度路由的隐藏状态使每个槽位能够从所需的Transformer深度读取，并且一个校准的头预测整个潜在集的联合分布，因此单次抽取就包含了文本条件、跨槽变化和描述允许的描述。

Result: 在HumanML3D上，MUGEN在FID上领先于语言模型基线，同时在标准评估器下检索精度高于真实动作参考，实现了最佳的CIDEr和BLEU@4分数，并在SnapMoGen上的每个检索和对齐指标上超过了离散令牌的当前最佳水平。

Conclusion: MUGEN框架有效地结合了动作和语言，在多个指标上取得了显著的成果，为物理AI系统的发展提供了新的思路。
...
### [362] [Prox: Training-Free FFN Activation Sparsity via Approximate Intermediate-Channel Salience in LLMs](https://arxiv.org/abs/2607.27591)
*Jinyi Liu,Wei Chen,Pengyu Chen,Xinyi Yuan,Minghe Bai,Guoquan Wu,Jun Wei*

Main category: cs.LG

TL;DR: Prox是一种有效的稀疏SwiGLU FFN框架，在LLM推理中提高效率


<details>
  <summary>Details</summary>
Motivation: 为了解决现有训练免费方法在高稀疏度下模型质量下降的问题

Method: 提出了一种名为Prox的两阶段训练免费框架，用于稀疏SwiGLU FFN

Result: 在十个LLM上优于训练免费基线，在70% FFN稀疏度下实现了1.99倍的端到端解码速度提升，且与量化稀疏注意力兼容

Conclusion: Prox是一种有效的稀疏SwiGLU FFN框架，能够提高LLM推理效率
...
### [363] [Compliance2LoRA: On-Demand Safety Alignment on Arbitrary Policy Subsets via Hypernetwork-Generated LoRA Adapters](https://arxiv.org/abs/2607.27594)
*Pankayaraj Pathmanathan,Furong Huang*

Main category: cs.LG

TL;DR: 提出了一种基于自适应超网络的框架，用于实现多政策合规性，提高了LRMs的适应性。


<details>
  <summary>Details</summary>
Motivation: 在大型推理模型（LRMs）的个性化应用中，需要满足不同用户的安全政策要求，而单独为每个政策子集训练LRM会带来计算开销。

Method: 提出一个基于自适应超网络的统一框架，使用LoRA适配器生成器学习生成符合政策子集的LoRA权重。

Result: 该框架在单个LRM上实现按需政策调整，无需牺牲不同规模和评估数据集的推理模型的任务性能。

Conclusion: 自适应超网络在LRMs中的对齐具有有效性和实用性。
...
### [364] [Back from the Future: Key-Value Cache Management by Counter-Causal Surprise](https://arxiv.org/abs/2607.27600)
*Stephen Gould,Anton van den Hengel*

Main category: cs.LG

TL;DR: 提出了一种基于反向因果注意力的KV驱逐方案，有效缓解了大型语言模型在输出生成时的内存压力。


<details>
  <summary>Details</summary>
Motivation: 缓解大型语言模型和其多模态变体在输出生成时的计算需求，通过缓存后续操作所需的先前键值计算。

Method: 提出一种基于过去标记可以被近期标记良好预测的洞察，从缓存中移除其相关键值的方法。使用反向因果注意力掩码，使每个位置只关注其未来的上下文。同时，提出一种快速单层近似方法，限制反向因果传递只到最后的transformer层。

Result: 在多个开源LLMs和基准数据集上评估，性能优于或等同于其他最先进的方法。

Conclusion: 提出了一种简单而有效的KV驱逐方案，可以减少内存占用并提高推理速度。
...
### [365] [Kalman Meets Curriculum: Efficient Dynamic Prompt Selection for Adaptive RL Finetuning](https://arxiv.org/abs/2607.27610)
*Haodong Zhu,Yangyang Ren,Yanjing Li,Sheng Xu,Haiguang Liu,Linlin Yang,Baochang Zhang*

Main category: cs.LG

TL;DR: KGPS是一种新的在线提示选择方法，在LLMs推理能力上取得了显著的提升。


<details>
  <summary>Details</summary>
Motivation: 强化学习（RL）微调显著提高了大型语言模型（LLMs）的推理能力，但其有效性严重依赖于为当前策略选择适当难度的提示。然而，由于提示难度在整个训练过程中会发生变化，因此这是一个挑战。现有的在线方法因此面临权衡：基于评估的方法准确但成本高昂，而基于预测的方法效率高，但通常假设难度是静态的，这使得它们不适合RL的非平稳训练动态。为了解决这些问题，我们提出了一种卡尔曼引导的提示选择方法（KGPS），将提示选择重新定义为动态状态估计问题，而不是静态难度预测。KGPS使用线性高斯状态空间模型在logit空间中对每个提示的潜在成功率进行建模，将过程噪声与策略更新的幅度相关联，以便在策略发生更大变化时增加不确定性。然后，卡尔曼滤波器维护一个校准的高斯后验分布，以提示难度，并通过最大化后验期望的训练效用来选择提示，该效用有利于中等难度的提示，同时自然地回顾不确定的提示。这种方法对策略漂移具有适应性，并且不需要超出标准策略训练的额外rollouts。在数学、规划和几何推理基准以及多个RL算法上进行的广泛实验表明，KGPS在最终准确性和rollout效率方面始终优于强大的基线，在在线提示选择方法中建立了最先进的性能。例如，在DeepSeek-R1-Distill-7B上，KGPS比DS少用了83%的rollouts，同时在六个数学推理基准上平均提高了0.12分。

Method: 提出了一种卡尔曼引导的提示选择方法（KGPS），该方法将提示选择重新定义为动态状态估计问题，而不是静态难度预测。KGPS使用线性高斯状态空间模型在logit空间中对每个提示的潜在成功率进行建模，将过程噪声与策略更新的幅度相关联，以便在策略发生更大变化时增加不确定性。然后，卡尔曼滤波器维护一个校准的高斯后验分布，以提示难度，并通过最大化后验期望的训练效用来选择提示，该效用有利于中等难度的提示，同时自然地回顾不确定的提示。这种方法对策略漂移具有适应性，并且不需要超出标准策略训练的额外rollouts。

Result: KGPS在数学、规划和几何推理基准以及多个RL算法上进行了广泛实验，表明其在最终准确性和rollout效率方面始终优于强大的基线，在在线提示选择方法中建立了最先进的性能。例如，在DeepSeek-R1-Distill-7B上，KGPS比DS少用了83%的rollouts，同时在六个数学推理基准上平均提高了0.12分。

Conclusion: KGPS是一种有效的在线提示选择方法，可以显著提高LLMs的推理能力。
...
### [366] [Real-Time Hard Peak Age-of-Information Safety with No-Regret Learning](https://arxiv.org/abs/2607.27626)
*Wentao Zhang,Wentao Mo*

Main category: cs.LG

TL;DR: 提出了一种新的实时调度方法，用于确保关键物联网系统的安全性。


<details>
  <summary>Details</summary>
Motivation: 确保关键物联网系统的安全，要求每个传感器的峰值年龄（PAoI）低于硬截止时间。

Method: 提出OCO-PAoI-Hard方法，通过将分数峰值-AoI截止时间转化为资源分配向量的仿射半空间约束，实现实时调度。

Result: OCO-PAoI-Hard在四个传感器的对抗流体模型陷阱通道上，实现了零模型状态截止时间违规，优于其他基线。

Conclusion: 该方法有效地解决了关键物联网系统的实时调度问题。
...
### [367] [First-order Constrained Trilevel Optimization Over Distributed Networks for Robust Coreset Selection](https://arxiv.org/abs/2607.27632)
*Yang Jiao,Kaixuan Jiao,Kai Yang,Nadjib Aitsaadi,Ilhem Fajjari,Renwei,Li*

Main category: cs.LG

TL;DR: 提出F$^2$CTO，一种针对分布式鲁棒核心集选择的分布式优化方法，有效提高数据处理的效率和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 在物联网快速发展的背景下，分布式边缘网络中产生了大量数据。训练模型需要大量计算和存储资源，因此核心集选择成为关键。同时，考虑到本地数据隐私敏感性和模型在实际部署中对鲁棒性的需求，开发有效的分布式优化框架对于鲁棒的核心集选择至关重要。

Method: 首先，本研究分析了核心集选择、鲁棒优化和分布式学习之间的层次依赖关系，将分布式鲁棒核心集选择构建为具有层次约束的三级优化问题。进一步提出了一种名为F$^2$CTO的分布式优化算法，该算法结合了层次复合值函数重排和分布式交替投影梯度算法。

Result: F$^2$CTO是第一个针对分布式鲁棒核心集选择的方法，也是第一个针对具有层次约束的三级优化问题的分布式优化方法。该方法在可靠持续学习中的大量实证评估表明了其有效性和效率。

Conclusion: 本研究提出了一种有效的分布式鲁棒核心集选择方法F$^2$CTO，该方法在解决分布式优化问题中具有显著优势。
...
### [368] [Certifying when decision-time information justifies adaptive experimentation](https://arxiv.org/abs/2607.27651)
...
### [369] [Evaluation Protocols and Cross-Subject Generalization in EEG Emotion Recognition](https://arxiv.org/abs/2607.27655)
...
### [370] [Looped Transformers with Source-Centered State Evolution](https://arxiv.org/abs/2607.27656)
*Bum Jun Kim,Kohei Hayashi,Shunsuke Kamiya,Masanori Koyama,Yusuke Iwasawa,Yutaka Matsuo*

Main category: cs.LG

TL;DR: SCSE enhances controlled recurrent quality


<details>
  <summary>Details</summary>
Motivation: Reconcile input conditioning with reference-preserving shared recurrence

Method: Proposing Source-Centered State Evolution (SCSE)

Result: SCSE retains input dependence, allows nonzero deviations to drive recurrent computation, and guarantees exact anchor invariance

Conclusion: SCSE improves the controlled recurrent quality frontier
...
### [371] [Understanding Submodular Information Measure Based Objectives for Representation Learning: A Variance and Separation Perspective](https://arxiv.org/abs/2607.27660)
*Rishabh Iyer,Truong Pham,Anay Majee*

Main category: cs.LG

TL;DR: 本研究通过建立理论框架，提供了对子模信息度量的统一理解和设计指导。


<details>
  <summary>Details</summary>
Motivation: 为了解决子模信息度量的几何和统计特性不明确的问题

Method: 建立统一的理论框架，将SIMs与表示学习和统计模式识别中的经典概念联系起来

Result: 提出了TI和MI目标，并使用受控的合成实验验证了这些理论描述

Conclusion: 提供了对子模信息度量的统一几何和统计理解，并为选择和设计SIMs目标提供了指导
...
### [372] [FedOGL: Combating Catastrophic Forgetting in Federated Open-World Multimodal Graph Learning](https://arxiv.org/abs/2607.27665)
*Zekai Chen,Haodong Lu,Shihao Li,Weiwei Ji,Xunkai Li,Xun Wu,Yinlin Zhu,Rong-Hua Li*

Main category: cs.LG

TL;DR: FedOGL：一种用于联邦图学习的内存保护框架，显著减少灾难性遗忘并提升性能。


<details>
  <summary>Details</summary>
Motivation: 为了在联邦图学习框架下，避免灾难性遗忘问题，并促进跨客户端的知识共享。

Method: 提出FedOGL，一种语义-结构内存保护框架，包括客户端的回放和任务起始蒸馏，以及服务器端的紧凑类别原型维护和传输。

Result: 在保持或提高下游任务性能的同时，FedOGL将灾难性遗忘导致的性能下降减少了42.67%。

Conclusion: FedOGL能够有效防止灾难性遗忘，并促进跨客户端的知识共享。
...
### [373] [Tight Sample Complexity for Low-Rank Adaptation: Matching Bounds and Rank Selection](https://arxiv.org/abs/2607.27680)
*Arunan J*

Main category: cs.LG

TL;DR: 本文研究了LoRA的统计特性，给出了LoRA风险的上界和下界，并提出了选择LoRA秩r的方法。
...
Method: 使用局部Rademacher论证和Fano类型的打包方法，建立了关于LoRA风险的上界和下界。

Result: 证明了LoRA风险的上界为O~(rd/n)，下界为Omega(rd/n)，并给出了选择LoRA秩r的方法。

Conclusion: LoRA微调的统计复杂性在局部二次区域内得到表征，并确定了过度参数化惩罚是未正则化的经验风险最小化而非LoRA类本身的属性。
...
### [374] [Event-Structured Physics-Informed Neural Networks for Differentiable Critical Clearing Boundaries](https://arxiv.org/abs/2607.27681)
*Baoli Hao,Chenxi Hu,Ming Zhong,Ren Wang*

Main category: cs.LG

TL;DR: 提出了一种基于事件结构化的物理信息神经网络（ES-PINN）的电力系统瞬态稳定性评估方法，该方法能够提高CCT估计的准确性，并具有高效的计算性能。


<details>
  <summary>Details</summary>
Motivation: 为了防止发电机跳闸和连锁停电，瞬态稳定性评估对于电力系统至关重要。关键指标是临界清除时间（CCT），它指定了在失去同步之前清除故障的最大时间。由于复杂的故障清除动力学，可靠的CCT估计具有挑战性，因为需要在多个故障严重性和清除时间上重复进行模拟。

Method: 提出了一种事件结构化的物理信息神经网络（ES-PINN），其表示与故障前、故障发生和清除后的摆动动力学相一致，并在事件接口上强制执行精确的状态链。通过平滑轨迹诱导的稳定性边际定义了CCT边界的可微近似，从而实现了准确的边界提取、局部敏感性分析和通过蒸馏读数进行直接CCT预测的选项。进一步证明了局部残差到轨迹到CCT误差估计，其中精确的事件链消除了单独的状态接口缺陷项。

Result: 在IEEE 9、14和30节点系统上的实验表明，ES-PINN在机械和电气应急情况下，与匹配的神经代理基线相比，在多个清除配置下，始终提高了保留轨迹和稳定性边界的准确性。此外，全网络DAE验证、多故障实验和运行时间分析进一步证明了所提出框架的有效性和计算效率。

Conclusion: ES-PINN在电力系统瞬态稳定性评估中表现出色，能够提高CCT估计的准确性，并具有高效的计算性能。
...
### [375] [NMINE: Normalized Mutual Information Neural Estimation](https://arxiv.org/abs/2607.27710)
*Petra Eerikinharju,Marko Tuononen,Ville Hautamäki*

Main category: cs.LG

TL;DR: 提出了一种基于神经网络的归一化互信息估计器，在连续多维数据上提高了准确性。


<details>
  <summary>Details</summary>
Motivation: 为了解决现有估计器对维度和数值稳定性的敏感性，提出了一种基于神经网络的归一化互信息估计器。

Method: 结合了基于MINE的神经网络互信息估计器和MI-NEE启发的神经网络边缘熵估计器，使用Donsker-Varadhan表示来估计互信息，并通过学习每个边缘分布与均匀参考分布之间的差异来估计边缘熵。

Result: 在从一维到八维的高斯数据上的实验表明，所提出的估计器在KSG基于的归一化互信息基线之上提高了准确性。

Conclusion: 神经网络估计在连续多维设置中用于归一化依赖度测量是一个有前途的方向。
...
### [376] [VESTIGE: A Knowledge-Guided Masking Strategy for Corruption-Aware Fine-Tuning of Genomic Transformers, Validated on Ancient DNA Reconstruction](https://arxiv.org/abs/2607.27712)
...
### [377] [Towards joint scaling laws with optimal batch size schedules](https://arxiv.org/abs/2607.27731)
*Jiaxiang Li,Zhiqi Bu,Shiyun Xu*

Main category: cs.LG

TL;DR: 本文研究了深度学习动态，并提出了动态批量大小计划，在大型语言模型训练中表现出色。


<details>
  <summary>Details</summary>
Motivation: 现代深度学习通常在整个训练过程中保持批量大小不变，从而忽略了学习率和批量大小对训练动态的联合影响。

Method: 通过凸优化的视角研究深度学习动态，推导出关于两种计划的损失联合表征，适用于通用优化器和模型架构。

Result: 为任何规定的学习率计划提供了一种封闭形式的最佳批量大小计划，并进一步导致了联合缩放定律，这些定律在静态批量大小基线中表现一致，突出了动态批量大小计划在大语言模型训练中的重要性。

Conclusion: 动态批量大小计划在大语言模型训练中具有重要作用。
...
### [378] [Improving the Robustness/Accuracy Tradeoff Against Adversarial Attacks Using Information Bottleneck Distillation Through Dual Teachers](https://arxiv.org/abs/2607.27737)
*Vincent Ryusuke Takahashi,Yoshinari Takeishi,Jun'ichi Takeuchi,Kave Salamatian*

Main category: cs.LG

TL;DR: 提出了一种基于IBD的鲁棒性增强方法，在保持清洁输入精度方面优于原始IBD，并具有竞争力。


<details>
  <summary>Details</summary>
Motivation: 为了提高深度神经网络对对抗攻击的鲁棒性，同时保持清洁输入上的分类精度。

Method: 扩展信息瓶颈蒸馏（IBD）框架，引入额外的清洁教师模型，通过跨层注意力矩阵将清洁和鲁棒的教师特征传递给学生。

Result: 在CIFAR-10和CIFAR-100数据集上，该方法在清洁样本上的分类精度高于原始IBD，同时在对抗样本上保持相似精度。与B-MTARD等最先进方法相比，该方法在清洁和鲁棒精度之间的调和平均值方面具有竞争力。

Conclusion: 提出的方法在提高鲁棒性的同时，保持了清洁输入上的分类精度，并且在性能上与最先进的方法相媲美。
...
### [379] [ROCS: Request-Oriented Compute Sharing for Efficient Large-Scale Recommendation](https://arxiv.org/abs/2607.27744)
...
TL;DR: ROCS通过请求导向的计算共享，提高了推荐系统的效率和质量。


<details>
  <summary>Details</summary>
Motivation: 现代推荐模型通过扩展特征交互和序列模块来提高预测质量，但生产成本限制限制了系统的扩展范围。

Method: 提出请求导向的计算共享（ROCS），通过延迟请求-候选者交互、隔离候选者依赖的表示，并一次请求评估模型的大部分内容来提高推理效率。开发广义层掩码（GLM）和深度交叉注意力（DCA）以实现这一范式。为支持高效的GPU部署，设计内核广播优化（IKBO）。

Result: 在公共基准测试中，ROCS在推荐骨干网络中一致地提高了质量-效率权衡。在生产规模的工作负载中，ROCS在检索模型上实现了高达3倍的QPS改进，而在短视频排名模型上实现了0.5%的相对LogLoss改进和50%的QPS改进。ROCS已在涵盖广告和有机表面、检索和排名阶段的大型推荐系统中部署，推理复杂度提高了两个数量级，在降低基础设施成本的同时带来了显著的在线收益。

Conclusion: ROCS通过提高推理效率，同时保持或提高预测质量，有效解决了推荐系统中的扩展问题。
...
### [380] [DAS-PMVC: A Framework for Partial Multi-View Clustering via Dual Alignment and Structure Enhancement](https://arxiv.org/abs/2607.27761)
*Shubin Ma,Liang Zhao,Chuanye He,Zhenjiao Liu,Liang Zou,Lin Yuanbo Wu,Yu Shao*

Main category: cs.LG

TL;DR: DAS-PMVC通过双重对齐和结构增强，解决了多视角聚类中的部分视角对齐问题，提高了聚类性能。


<details>
  <summary>Details</summary>
Motivation: 多视角聚类在近年来受到广泛关注，但数据采集设备限制导致不同视角的数据存在不匹配问题，即部分视角对齐问题（PVAP）。

Method: 提出了一种名为DAS-PMVC的框架，通过双重对齐和结构增强（Dual Alignment and Structure Enhancement）来缓解视角不对称和不相关样本的影响。该框架包括三个部分：锚图结构对齐、结构增强特征学习和双重对齐策略。

Result: 在多个数据集上的实验结果表明，DAS-PMVC框架在聚类性能上优于现有最先进的方法，展示了其有效性和优越性。

Conclusion: DAS-PMVC框架通过双重对齐和结构增强，有效地解决了多视角聚类中的部分视角对齐问题，提高了聚类性能。
...
### [381] [Train Small, Deploy Large: Zero-Shot GNN Transfer Through Geometric Renormalization](https://arxiv.org/abs/2607.27767)
*Robert Jankowski,Pedro Almagro-Blanco,Marián Boguñá,Melanie Weber,M. Ángeles Serrano*

Main category: cs.LG

TL;DR: 通过在粗粒化图上训练 GNN 并迁移权重，可以显著降低大规模图的训练成本，同时保持预测性能。


<details>
  <summary>Details</summary>
Motivation: Graph neural networks (GNNs) 在大规模图上操作时，对基础设施敏感，需要可扩展的训练技术。

Method: 提出了一种零样本迁移协议，其中 GNN 在通过几何归一化 (GR) 粗粒化的图上训练，并将结果权重直接转移到原始网络上。

Result: 在合成和真实世界的网络中，在 GR 缩小副本上的训练保留了原始规模的大部分预测性能，同时显著降低了训练成本。

Conclusion: 结构相似性可能比网络大小在确定 GNN 可迁移性方面更重要，为尺度不变的图架构开辟了道路。
...
### [382] [Beyond the Best Teacher: Expanding and Compressing the Reasoning Solution Manifold](https://arxiv.org/abs/2607.27770)
*Songshuo Lu,Zhi Chen,Yaohua Tang*

Main category: cs.LG

TL;DR: 提出了一种新的强化学习框架，通过构建和压缩教师联盟，提高了策略的可靠性。


<details>
  <summary>Details</summary>
Motivation: 强化学习训练的策略通常只能放大有效解决方案的一部分，因此需要一种新的框架来提高策略的可靠性。

Method: 提出了一种结合教师构建和教师策略蒸馏的扩展-压缩框架。在扩展阶段，使用RGRPO训练一系列教师；在压缩阶段，使用可靠性门控的Teacher-Union On-policy Distillation（TU-OPD）。

Result: 在数学推理、代码生成和指令遵循上的实验表明，Qwen3-1.7B学生在所有三个领域均优于最强的单个教师，分别提高了2.0%、8.3%和6.9%。

Conclusion: 通过构建和压缩互补的教师联盟，可以获得更强的学生。
...
### [383] [RIPPLE: Generating Multi-Channel Phase, Not Recovering It](https://arxiv.org/abs/2607.27775)
*Jaehyuk Lee,Yeajin Lee,Dayeon Shin,Donghun Lee*

Main category: cs.LG

TL;DR: RIPPLE通过改进相位处理，在多通道波形处理中取得了更好的性能。


<details>
  <summary>Details</summary>
Motivation: The current phase recovery methods are insufficient for multi-channel waveforms like spatial audio and seismograms, as they cannot capture the physical content in the phase relationships between channels.

Method: RIPPLE uses Griffin-Lim as a phase prior and refines it with a rectified flow under an explicit inter-channel phase loss.

Result: RIPPLE achieves better coherence metrics in tasks like first-order ambisonics environment transfer and seismic cross-station translation compared to recovery-based pipelines.

Conclusion: RIPPLE (Rectified Inter-channel Phase with Prior-based LEarning) outperforms recovery-based pipelines in terms of coherence metrics, especially in the seismic cross-station translation task.
...
### [385] [Revisiting Predictive Process Monitoring in the Age of Foundation Models: A Comparative Study of Sequence, Tabular, and LLM Approaches](https://arxiv.org/abs/2607.27797)
*Lennart Fertig,Lukas Kirchdorfer,Tobias Sesterhenn*

Main category: cs.LG

TL;DR: 通过基准测试，比较了三种PPM建模范式，发现序列模型在活动预测方面表现最佳，表格基础模型在时间任务上具有竞争力。


<details>
  <summary>Details</summary>
Motivation: 近年来，基于深度序列模型的预测过程监控（PPM）研究占主导地位，但大语言模型（LLMs）和表格基础模型在PPM中的应用逐渐受到关注。然而，这些新方法在PPM中的性能尚不清楚。

Method: 通过多个数据集和预测任务的受控基准，对比分析了三种建模范式：深度序列模型、LLMs和表格基础模型。

Result: 序列模型在活动预测任务中表现最佳，而表格基础模型在时间任务上具有竞争力，尽管LLMs的成本更高，但表现通常落后。

Conclusion: 在PPM中，序列模型在活动预测方面具有优势，而表格基础模型在时间任务上具有竞争力，LLMs虽然成本高，但表现不如预期。
...
### [386] [Learning-Augmented and Randomized Algorithms for Line Aggregation with Delays](https://arxiv.org/abs/2607.27807)
*Tianhang Lu,Runtian Ren,Shengcai Liu,Ke Tang*

Main category: cs.LG

TL;DR: 本文提出了一种学习增强和随机化在线聚合算法，在理论和实验上均表现出良好的性能


<details>
  <summary>Details</summary>
Motivation: 研究在线聚合算法在存在延迟的线度量空间中的学习增强和随机化

Method: 提出了一种确定性学习增强的Balance算法和一种随机算法，并进行了理论分析和数值实验

Result: 确定性学习增强的Balance算法具有(4/λ+1/λ^2)-鲁棒性和(4+λ)-一致性，随机算法对无知的对手具有(e+1)-竞争性，随机学习增强算法具有(e/λ+1/λ^2)-鲁棒性和(e+λ)-一致性

Conclusion: 本文提出的算法在理论和实验上均表现出良好的性能
...
### [387] [Contrastive Concept Importance: Explaining Pairwise Class Decisions Through Automatically Extracted Concept Representations](https://arxiv.org/abs/2607.27904)
*Roel Visser,Isaac Roberts,Barbara Hammer*

Main category: cs.LG

TL;DR: 对比概念重要性（CCI）可以揭示模型对类别对特定行为的贡献。


<details>
  <summary>Details</summary>
Motivation: 为了在分类错误、类别混淆和低边界预测等情况下回答“为什么P而不是Q？”的问题。

Method: 引入对比概念重要性（CCI），将目标类和对照类之间的logit间隔归因于自动提取的视觉概念基中的概念。

Result: 在ImageNet类别对上使用CRAFT风格的类别基础、插入和删除曲线、logit分解分析和语义类别层次结构评估方法。

Conclusion: 对比概念重要性（CCI）可以揭示模型对类别对特定行为的贡献，而不仅仅是普通概念重要性。
...
### [388] [Class-Aware Reinforcement Learning for Counterfactual Explanation Generation](https://arxiv.org/abs/2607.27905)
*Muhammad Adil Saleem,Syed Ali Raza,Mary-Anne Williams*

Main category: cs.LG

TL;DR: 通过在RL状态表示中包含实例的预测类别，可以生成更有效的反事实解释。


<details>
  <summary>Details</summary>
Motivation: 提高黑盒模型的可解释性，通过生成具有调整特征值的替代实例来生成反事实解释。强化学习（RL）为反事实解释生成提供了一种有希望的方法，它可以在确保控制关键指标（如有效性、稀疏性和邻近性）的同时，有效地探索反事实实例。

Method: 将实例的预测类别与从预测器中提取的特征一起纳入RL状态表示，以生成反事实解释。比较了所提出的具有类意识的RL方法与排除实例类别信息的类盲RL方法。

Result: 在训练过程中，具有类意识的RL在收敛速度、奖励优化和剧集长度减少方面提供了好处。与类盲RL相比，它生成了更多有效的反事实解释。实例的基于类别的特征在RL的动作选择中始终排在最有影响力的预测因子之列。

Conclusion: 具有类意识的RL在反事实解释生成中提高了清晰度、学习速度、有效性和更有效的反事实生成。
...
### [389] [S-CEReBrO: Breaking the Memory Barrier in Continuous EEG Monitoring](https://arxiv.org/abs/2607.27913)
*Glenn Anta Bucagu,Thorir Mar Ingolfsson,Yawei Li,Luca Benini*

Main category: cs.LG

TL;DR: S-CEReBrO通过窗口交替注意力机制，实现了高效、可泛化和连续的EEG监测。


<details>
  <summary>Details</summary>
Motivation: 为了解决Transformer架构在EEG分析中的瓶颈问题，提出了一种新的架构S-CEReBrO。

Method: 提出了一种名为S-CEReBrO的新架构，该架构使用了一种名为窗口交替注意力的机制。

Result: S-CEReBrO在11个下游任务中的7个上达到了最先进的性能，参数数量减少了60%。

Conclusion: S-CEReBrO是一种高效、可泛化和连续的EEG监测的新方法。
...
### [390] [Exact Action Values Are Not Enough: Rollout-Verified Reinforcement Fine-Tuning of a Reasoning Model for Multi-Zone VAV Control](https://arxiv.org/abs/2607.27914)
...
### [391] [ODEWorld: A Continuous Predictive Architecture via Physical-Time Flow](https://arxiv.org/abs/2607.27924)
*Dongxiu Liu,Haoyi Niu,Peng Cheng,Yuan Gao,Xirui Kang,Sangli Teng,Koushil Sreenath,Xianyuan Zhan*

Main category: cs.LG

TL;DR: PT-Flow和ODEWorld通过引入连续时间模型，提高了机器学习在捕捉物理世界动态方面的效率。


<details>
  <summary>Details</summary>
Motivation: 在物理世界中，空间和时间是基本连续的。然而，现有的用于世界建模的机器学习范式主要局限于离散时间预测，因此在捕捉物理世界动态方面存在显著的低效率。

Method: 提出了一种名为PT-Flow的新方法，该方法学习一个在物理时间中运行的连续潜在速度场。关键的是，顺序数据的底层动力学通过嵌入到良好结构的表示空间中的常微分方程（ODE）进行参数化。在这个范式下，未来的预测可以通过在压缩的潜在空间中的ODE求解器进行时间积分来重新构造成时间积分。在PT-Flow的基础上，构建了一个连续时间的潜在世界模型ODEWorld，它既高效又灵活。通过提取时间变异性特征并在动态表示空间和潜在速度场上强制执行ODE属性，ODEWorld有效地解决了潜在世界模型文献中长期存在的表示崩溃问题。这还使得在长期预测后也能进行高质量的图像重建。此外，其连续性质允许任意时间分辨率，甚至可以进行反向预测，这对于大多数离散时间模型来说是不可能的。最后，ODEWorld可以提供丰富的规划信息，以促进下游策略学习。

Result: 综合实验表明，ODEWorld成功地将规划促进的动力学抽象与视觉逼真性相结合，在视频生成和机器人控制方面表现出色。

Conclusion: PT-Flow和ODEWorld为世界建模提供了一种新的、高效的方法，它能够更好地捕捉物理世界的动态，并具有广泛的适用性和优越的性能。
...
### [392] [Harnessing the Potential of Optimizing Data Mixtures via Bayesian Domain Reweighting](https://arxiv.org/abs/2607.27928)
*Xiang Yuan,Kaiqing Lei,Zhenyu Jin,Jun Shu,Deyu Meng,Zongben Xu*

Main category: cs.LG

TL;DR: 提出了一种新的贝叶斯领域加权方法，提高了大型语言模型领域权重学习的稳定性和效率。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）的性能受多领域预训练数据的分布组成影响。早期模型中广泛使用的手动启发式方法，随着数据复杂性的增加，越来越难以捕捉领域之间的复杂协同作用。为了解决这个问题，一种主导的方法试图拟合一个代理函数，映射领域权重与其相应的验证损失，然后找到最优的领域权重以最小化验证损失。这些方法依赖于强大的结构假设，如秩不变性或缩放定律，这些假设通常被违反，导致不可忽视的估计偏差。一种有希望的方法是直接从数据中优化加权方案。然而，它受到不稳定的优化轨迹和难以承受的计算开销的限制，限制了其搜索更好的领域权重配置的潜力。

Method: 提出了一种贝叶斯领域加权方法，通过引入从观察中学习到的伽马先验信息，从狄利克雷分布中推断权重。

Result: 该方法可以实现稳定和高效的领域权重学习，并在比基于搜索的函数拟合方法消耗的数据量少得多的情况下识别出最佳混合物，为大规模应用重振基于优化的领域加权。

Conclusion: 提出的贝叶斯领域加权方法在大型语言模型领域权重学习方面表现出色，为大规模应用提供了有效的解决方案。
...
### [393] [TriShield: Zero-Utility-Loss Defense Against Privacy Backdoors in Federated Language Model Fine-Tuning via Orthogonal Gradient Projection and Optimizer State Entanglement](https://arxiv.org/abs/2607.27940)
*Cheng Wei*
...
### [394] [AutoPref: Automatic Discovery of Task-Specific Preference Objectives for Neural Combinatorial Optimization](https://arxiv.org/abs/2607.27953)
*Shengda Gu,Kai Li,Xinyi Ke,Haobo Fu,Yifan Zhang,Jian Cheng*

Main category: cs.LG

TL;DR: AutoPref通过自动化偏好目标发现提高了NCO的性能。


<details>
  <summary>Details</summary>
Motivation: 组合优化问题（COP）在许多现实世界的决策中起着基础作用，但其指数级大的搜索空间使得高质量解决方案的成本高昂。

Method: 提出AutoPref，第一个基于LLM的自动化偏好目标发现框架。将目标分解为成对损失程序和集感知加权程序。

Result: 在TSP、CVRP、FFSP和JSSP上，AutoPref在问题规模上始终优于强手设计的基线，证明了自动化目标发现对NCO的益处和可扩展性。

Conclusion: AutoPref能够提高NCO的性能和效率，并为自动化目标发现提供了新的思路。
...
### [395] [What Makes Graph Unified? Principles and Generative Sliding-Window Transformer for Graph Foundation Models](https://arxiv.org/abs/2607.27966)
*Dongxiao He,Siqi Liu,Jitao Zhao,Yawen Li,Yi Wang,Di Jin*

Main category: cs.LG

TL;DR: SliGFM is a novel graph foundation model for effective cross-domain graph feature unification and knowledge transfer.


<details>
  <summary>Details</summary>
Motivation: Graph Foundation Models (GFMs) emerged as a promising paradigm for general-purpose graph learning, aiming to learn reusable knowledge that generalizes across diverse graph domains and downstream tasks.
...
Result: SliGFM orders feature dimensions by topological smoothness, transforms heterogeneous features into a common space, captures transferable relational patterns, and preserves original feature information.

Conclusion: SliGFM effectively unifies cross-domain graph features and supports knowledge transfer across diverse graph domains.
...
### [396] [Beyond Binary Rewards: A Comparative Study of Reward Design for Reinforcement Unlearning](https://arxiv.org/abs/2607.27968)
*Efstratios Zaradoukas,Davide Gabrielli,Bardh Prenkaj,Gjergji Kasneci*

Main category: cs.LG

TL;DR: 本研究提出了一种新的奖励设计方法，提高了机器取消学习的效率。


<details>
  <summary>Details</summary>
Motivation: 为了满足GDPR和欧盟AI法案等隐私法规的要求，机器取消学习正在寻求在不进行完整重新训练的情况下选择性地从训练好的语言模型中删除特定知识，这是一个日益增长的需求。

Method: 研究奖励设计如何影响在强化取消学习（RUL）框架内取消学习的效率。引入了一个原则性的奖励分解框架，将可验证性与稀疏性分离，并提出了两个新的奖励函数：一个指数奖励，根据禁止概念的出现的数量提供分级惩罚，以及一个受PageRank启发的奖励，通过语义重要性加权惩罚。

Result: 在RWKU基准上进行实验，结果表明这两种奖励都优于二进制设置，同时达到类似的遗忘性能，速度快3倍，并保留了一般的模型效用。

Conclusion: 奖励设计是取消学习效率的关键驱动因素，为可扩展和高效的机器取消学习提供了一条可行的途径。
...
### [397] [TAPO: Transition-Aware Policy Optimization for LLM Agents](https://arxiv.org/abs/2607.27973)
*Cong Li,Peixi Peng,Yisen Zhao,Xinyu Hu,Shudong Liu,Zhan Su,Zhuojian Li*

Main category: cs.LG

TL;DR: TAPO通过交替策略优化和过渡监督，提高了LLM代理的泛化能力和任务性能。


<details>
  <summary>Details</summary>
Motivation: 为了提高大型语言模型（LLM）代理的泛化能力，研究人员提出了TAPO，一种基于强化学习的训练框架。

Method: TAPO交替进行策略优化和过渡监督，将回放数据重新用于在共享骨干模型上应用动作条件下的下一个观察预测监督。

Result: 在WebShop和ALFWorld上进行的实验表明，TAPO在任务性能上优于纯策略优化基线。

Conclusion: TAPO是一种有效的训练框架，可以增强LLM代理的泛化能力，并提高任务性能。
...
### [398] [Generalization Bounds on Optimal Control for Transformer Training and Wasserstein Distributional Robustness](https://arxiv.org/abs/2607.27975)
*Kağan Akman,Naci Saldi,Serdar Yüksel*

Main category: cs.LG

TL;DR: 研究了Transformer的有限样本泛化界限，并将其与Wasserstein分布鲁棒优化联系起来。
...
Method: 基于双提升的Transformer动态度量值公式，将数据集视为经验输入输出度量的概率律，将训练问题解释为有限时间窗的马尔可夫控制问题。

Result: 分析了量化模型，并通过有限度量空间上的经验定律的集中不等式和值函数的Lipschitz稳定性估计推导出显式的有限样本泛化界限。

Conclusion: 将界限转移到基本模型，以显式近似误差为代价。
...
### [399] [It's All Just Vectorization: einx, a Universal Notation for Tensor Operations](https://arxiv.org/abs/2607.27987)
*Florian Fervers,Sebastian Bullinger,Christoph Bodensteiner,Michael Arens*

Main category: cs.LG

TL;DR: 提出了一种新的张量操作符号einx，提高了操作的易读性和可写性。


<details>
  <summary>Details</summary>
Motivation: 现有的张量框架中Numpy-like符号难以阅读和编写，容易出错。

Method: 重新审视向量化作为转换张量操作的功能，并使用它将低阶操作提升到高阶操作，并将高阶操作概念上分解为低阶操作及其向量化。

Result: 引入了通用的张量操作符号einx，它使用声明性、指点的表达式，并具有清晰的代码表示。

Conclusion: einx是一种通用的张量操作符号，提高了张量操作的易读性和可写性。
...
### [400] [Building a User Foundation Model for the Open Web](https://arxiv.org/abs/2607.28019)
*Solal Vernier,Ivan Can Arisoy,Merwan Barlier,Blaž Škrlj*

Main category: cs.LG

TL;DR: 提出了一种基于自我监督学习的用户基础模型，在开放网络实时竞价环境中有效提高了广告点击率和效率。


<details>
  <summary>Details</summary>
Motivation: 在开放网络实时竞价（RTB）环境中，由于用户身份的碎片化和非持久性，以及用户隐私选择导致的浏览历史数据可用性限制，传统的用户基础模型在历史信号利用方面存在局限性。

Method: 提出了一种用户基础模型，该模型通过自我监督学习对用户浏览历史进行建模，并在点击预测任务上进行微调。使用Transformer编码器进行预训练，并结合掩码语言建模和序列级对比目标。使用LLM-in-the-loop搜索优化预训练管道。

Result: 在点击预测任务上取得了显著的性能提升，生产环境中的出价胜率模型和点击率排名模型分别提升了1.197%和1.354%。7天的A/B测试证实了2.13%的点击率提升和-1.13%的eCPC下降。

Conclusion: 该方法在开放网络中具有可行性，能够有效提高RTB广告的点击率和效率。
...
### [401] [Flux-OPD: On-Policy Distillation with Evolving Contexts](https://arxiv.org/abs/2607.28022)
...
TL;DR: Flux-OPD通过上下文差异信号和冲突项加权校正，在开放领域的大语言模型训练中实现有效监督。


<details>
  <summary>Details</summary>
Motivation: 在开放领域的大语言模型训练中缺乏可验证的奖励，使得任务偏好难以形式化为有效的监督。

Method: 通过分解反向KL目标来分析上下文的影响，提出Flux-OPD范式，使用演变的上下文作为训练监督来捕捉开放领域中的任务偏好。

Result: Flux-OPD在开放任务上优于现有的OPD范式，突出了结合教师监督与演变上下文的潜力。

Conclusion: 上下文可以传达偏好，但在学生性能演变时提供很少的额外监督。Flux-OPD通过将上下文差异信号注入无上下文教师锚点，并使用冲突项作为指标来加权校正强度，有效地解决了这一问题。
...
### [402] [Contrastive Reinforced Policy Optimization via Privileged Self-Distillation](https://arxiv.org/abs/2607.28026)
*Xingjian Wu,Junlin Liu,Xingchen Liu,Xuhang Zhu,Jianing Wang,Linsen Guo,Xiaoyu Li,Xuezhi Cao,Xunliang Cai*

Main category: cs.LG

TL;DR: CRPO 通过对比学习优化了代理 OPSD，提高了长期交互中的训练稳定性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 在多轮代理设置中，OPSD 由于自教师的有利信息而存在暴露偏差，导致推理路径收敛和清晰的优化方向丢失。

Method: 引入对比强化策略优化（CRPO），从对比学习的角度重新定义了代理 OPSD。通过利用预测熵来区分正位置（反思探索）和负位置（暴露偏差），CRPO 进行分组对比，以保留可靠的、细粒度的优化信号。

Result: 在 13 个具有挑战性的推理和深度搜索基准上进行了广泛的评估，结果表明 CRPO 在长期交互中始终优于现有的强化学习和自蒸馏基线，显著提高了训练稳定性和泛化能力。

Conclusion: CRPO 在多轮代理设置中有效解决了 OPSD 的暴露偏差问题，显著提高了训练稳定性和泛化能力。
...
### [403] [Enhancing Irregular Time Series Forecasting with Continuous-Time Modeling Framework](https://arxiv.org/abs/2607.28035)
*Tianen Shen,Zhengyu Li,Yutong Li,Xiangfei Qiu,Xingjian Wu,Bin Yang,Jilin Hu*

Main category: cs.LG

TL;DR: WrapFlow是一种针对不规则时间序列预测的连续时间建模框架，它在多个数据集上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的方法要么依赖于基于离散化的预处理（例如插值、填充或聚合），这破坏了潜在的连续时间语义，要么采用基于ODE的框架进行连续时间建模，这通常需要特定的架构并因数值求解器而引起大量的计算开销。

Method: WrapFlow是一个连续时间建模框架，用于不规则的时序预测。它引入了连续时间标记化，通过间隔感知标记显式地建模长时间未观测到的间隔。这些连续时间标记随后被标准的Transformer骨干网络处理，以捕获长期时间依赖关系。还开发了一种无模拟训练范式，用于残差流匹配，在训练期间避免数值求解器模拟和反向传播。

Result: 在多个真实世界数据集上的广泛实验表明，WrapFlow实现了最先进的性能。

Conclusion: WrapFlow通过避免离散化和数值求解器的缺点，提供了一种高质量连续预测的方法。
...
### [404] [Learning features from Newton's algorithm: a way to accelerate nonlinear parametrized PDE solvers](https://arxiv.org/abs/2607.28036)
*Rémy Vallot,Florian de Vuyst,Thibault Dairay,Mathilde Mougeot*

Main category: cs.LG

TL;DR: 提出了一种基于特征学习的两阶段牛顿初始猜测策略，有效提高了非线性方程组求解的收敛速度。


<details>
  <summary>Details</summary>
Motivation: 为了提高牛顿法在非线性方程组求解中的收敛速度，提出了一种两阶段牛顿初始猜测策略。

Method: 通过学习参数空间采样和预计算解数据库中的特征，构建两个互补的降维空间：解特征空间和校正搜索方向特征空间。使用回归模型预测代理解近似，然后使用GMRES方法进行残差最小化校正。

Result: 该方法减少了牛顿迭代次数和总体CPU时间，在代表性PDE问题上的数值实验表明，与独立代理初始化相比，具有可量化的加速效果。

Conclusion: 该策略可以应用于广泛的大规模非线性问题，是一种有效的收敛加速方法。
...
### [405] [ClawTrack: Towards Trace-Level Evaluation and Improvement of Real-World Autonomous Agents](https://arxiv.org/abs/2607.28037)
*Xingjian Wu,Xuhang Zhu,Xingchen Liu,Junlin Liu,Jianing Wang,Linsen Guo,Xiaoyu Li,Xuezhi Cao,Xunliang Cai*

Main category: cs.LG

TL;DR: ClawTrack基准通过同时评估任务得分和过程得分，为LLM代理的推理过程提供了更全面的评估。


<details>
  <summary>Details</summary>
Motivation: 在复杂的多步骤工作流程中部署基于LLM的代理时，出现了一个关键的评估差距：大多数现有的基准只评估最终结果，无法区分可靠的推理和幸运的成功，也无法将失败归因于特定的过程缺陷，阻碍了长期任务的归因。

Method: 提出了一种名为ClawTrack的双评估基准，该基准同时衡量代理实现的目标（任务得分）以及实现目标的方式（过程得分）。ClawTrack包含8个领域中的320个任务和25个以上的确定性模拟服务。过程评分员根据12,541个特定于任务的评分标准项，从四个维度（目标一致性、效率、信息利用和结果验证）对每个推理步骤进行评分。

Result: 评估了21个模型在16,000多次试验中的表现，发现：1）过程得分有效地将成功和失败归因于特定的推理维度，过滤了仅评估结果时不可见的幸运通过；2）四个维度是互补的，结果验证是系统瓶颈；3）该框架对不同评估者选择的鲁棒性；4）基于过程轨迹的过滤在模型规模上产生了一致的后训练改进。

Conclusion: ClawTrack基准为评估基于LLM的代理的推理过程提供了新的视角，有助于识别和改进长期任务中的推理缺陷。
...
### [406] [GVR-Coder: A Visual-Feedback Framework for Structured SVG Generation in Complex Document and Meeting Scenarios](https://arxiv.org/abs/2607.28073)
*Yiming Xu,Jihua Kang,Chunsai Du,Qifan Zhang,Wangqiu Zhou,Yiting Wu,Tianqi Li,Qi Song*

Main category: cs.LG

TL;DR: 提出了一种名为GVR-Coder的新框架，用于从冗长的专业文本生成高质量的逻辑图表，有效提高了信息交流的效率。


<details>
  <summary>Details</summary>
Motivation: 在要求高的专业环境和评审场景中，长文本往往造成高认知负荷。为了促进高效的信息交流，将冗长的文本转换为逻辑清晰的图表至关重要。

Method: 提出了一种名为GVR-Coder的新框架，用于从冗长的专业文本生成高质量的逻辑图表。该框架采用课程驱动的拒绝采样微调来逐步提高模型对复杂结构的建模能力，并在训练过程中明确融入布局约束知识。此外，引入了来自双渲染反馈的强化学习机制，该机制通过奖励信号提供隐式反馈，以联合优化结构复杂性和视觉美感。还设计了一个生成-验证-修复智能体循环，通过显式、细粒度的反馈和有针对性的改进来提高生成质量。

Result: GVR-Coder在生成逻辑一致且视觉吸引人的图表方面优于竞争基线，并可靠地产生了高质量的图表。

Conclusion: 研究提出了一种有效的文本到SVG生成方法，通过解决现有研究的挑战，提高了信息交流的效率。
...
### [407] [Stage-Replay Divergence Follows the KV Cache: Fixed-Prefix Precision Controls and Bidirectional Cache Transplantation](https://arxiv.org/abs/2607.28495)
*Alexander Boesgaard Lorup*

Main category: cs.LG

TL;DR: 精确的token回放可重复，边界K/V缓存是关键，数值精度影响行为。


<details>
  <summary>Details</summary>
Motivation: 分析论文摘要中的关键假设和实验结果。

Method: 实验比较和审计

Result: 精确的token回放可以重复进行，无需保留实时状态精确度。边界K/V缓存是因果充分携带器，而数值精度调节其行为表达。

Conclusion: 精确的token回放技术具有可重复性，但需要考虑数值精度对行为表达的影响。
...
### [408] [Chem World: A Large-Scale Benchmark and Physics-Informed Framework for Trustworthy Chemical Property Prediction](https://arxiv.org/abs/2607.28079)
*Tianyou Bai,Huan Wang,Mingchen Gao,Fangyue Lin,Pinze Ren,Zhenlin Zhao,Siming Dong*

Main category: cs.LG

TL;DR: Chem World是一个用于化学性质预测的综合基准，通过物理信息学习提高了预测的准确性，为计算化学和AI驱动的科学发现提供了坚实的基础。


<details>
  <summary>Details</summary>
Motivation: 化学性质预测在化学、材料科学和药物开发中起着至关重要的作用。然而，现有的基准测试通常存在任务多样性有限、数据集碎片化以及评估协议不一致等问题，这使得系统地评估AI模型的可靠性和泛化能力变得具有挑战性。

Method: 我们引入了Chem World，这是一个综合性的化学性质预测基准，它集成了17个不同的化学数据集，包含超过800,000个分子样本，涵盖了密度、电导率、溶解度和其他分子特性等不同性质。Chem World提供了一个统一的平台，用于评估AI模型在多个性质预测任务中的表现。此外，我们提出了Mixture-PINN，这是一种基于物理信息的神经网络预测框架，它将化学先验知识融入数据驱动学习，提高了化学性质预测的准确性、鲁棒性和可靠性。

Result: 在Chem World上的大量实验表明，与现有方法相比，我们的方法具有有效性。通过结合大规模标准化评估和物理信息学习，Chem World为开发可信赖的AI系统用于计算化学和推进AI驱动的科学发现奠定了基础。

Conclusion: Chem World为化学性质预测提供了一个综合基准，并通过结合物理信息学习，提高了预测的准确性、鲁棒性和可靠性，为计算化学和AI驱动的科学发现提供了坚实的基础。
...
### [409] [From Expert Reduction to Behavioral Divergence: Tracing Numerical State through Sparse MoE Inference](https://arxiv.org/abs/2607.28097)
*Tianyang Zhu*
...
### [410] [Information Bottleneck Learning for Faithful Time Series Forecasting Explanations](https://arxiv.org/abs/2607.28124)
*Xu Zheng,Wei Cheng,Zhuomin Chen,Mo Sha,Jingchao Ni,Dongsheng Luo*

Main category: cs.LG

TL;DR: IB-Forecast是一种新的预测框架，它结合了可解释性和预测准确性。


<details>
  <summary>Details</summary>
Motivation: 在能源、交通和医疗保健等领域，预测越来越驱使决策，理解预测背后的历史数据与预测本身一样至关重要。

Method: 提出IB-Forecast，一个本质上可解释的多变量时间序列预测框架。它将预测分解为学习到的周期性组件和通过可解释掩码计算输入标记的残差组件。

Result: IB-Forecast在匹配领先的黑盒模型的预测误差的同时，提供了忠实于预测的解释，且不增加额外的推理成本。在匹配稀疏性预算下，这些原生解释在所有评估的数据集中都优于基于梯度的、基于遮挡的和基于优化的基线。

Conclusion: IB-Forecast保证了高解释保真度，只需14-20%的观测值即可提供低误差预测。
...
### [411] [LM-GRASP: Instance-Specific Language Models for Combinatorial Construction via Online Imitation Learning](https://arxiv.org/abs/2607.28135)
...
### [412] [Search Strategies for Optimal Classification and Regression Trees](https://arxiv.org/abs/2607.28170)
*Jacobus G. M. van der Linden,Mim van den Bos,Emir Demirović*

Main category: cs.LG

TL;DR: 通用算法框架提升ODTs可扩展性，显著提高性能。


<details>
  <summary>Details</summary>
Motivation: 为了解决最优决策树（ODTs）的可扩展性问题，提出了一个通用的算法框架。

Method: 引入了一种通用的算法框架，实现了先前使用的搜索策略，并允许定义新的策略。

Result: 实验结果表明，与现有技术相比，最佳策略在分类任务中实现了显著更好的任意时间性能，在回归任务中运行时间提高了超过一个数量级。

Conclusion: 提出的算法框架能够有效提高ODTs的可扩展性，并显著提升性能。
...
### [413] [Multi-channel Uplift Policy Learning](https://arxiv.org/abs/2607.28182)
*Changjian Liu,Tianyu Wang,Xiaoxuan Deng,WenTao Zhu,Yuwei Xu,Jungqi Jin,Yong Gao,Chuan Yu,Jian Xu,Bo Zheng*

Main category: cs.LG

TL;DR: ReAlloc框架有效解决电商营销预算分配问题，显著提升业务效果。


<details>
  <summary>Details</summary>
Motivation: 电子商务平台需要在多个渠道之间分配固定的营销预算以最大化业务效用。然而，标准的预测-优化（PTO）范式由于观察性混杂和严重的外推失败。

Method: 我们将这一挑战表述为一个单纯形约束的增值决策问题，并提出了ReAlloc，一个快速-慢速因果框架。具体来说，一个敏捷的正交教师从短期日志中提取无偏局部梯度，而一个解释引导的学生将它们提炼为长期时间范围内的结构化边际场。

Result: 在淘宝平台上的大量模拟和大规模在线A/B测试表明，ReAlloc在支付订单和收入方面都实现了同时提升。

Conclusion: ReAlloc框架能够有效解决电子商务平台在多个渠道之间分配营销预算的问题，并显著提升业务效果。
...
### [414] [Persistent Gaussian Perturbations Prevent Oversmoothing in Recurrent Graph Neural Networks](https://arxiv.org/abs/2607.28185)
*Mostafa Haghir Chehreghani*

Main category: cs.LG

TL;DR: 研究了对抗GNN过度平滑的新机制，并通过理论分析和实验验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: Deep图神经网络（GNN）的过度平滑是一个基本限制，这限制了消息传递架构的有效深度，并促使寻找保持表示多样性的机制。

Method: 研究了一种在每次传播步骤后注入独立高斯噪声的循环图神经网络，并将其作为随机动力系统进行分析。

Result: 证明了在标准全局收缩假设下，隐藏表示形成一个几何遍历马尔可夫链，并具有唯一的平稳概率测度。分析揭示了持续随机扰动作为对抗过度平滑的根本不同机制。

Conclusion: 该方法有效地防止了渐近过度平滑，并通过数值实验验证了理论预测。
...
### [415] [TopoFormer: Topology Meets Attention for Graph Learning](https://arxiv.org/abs/2607.28259)
*Md Joshem Uddin,Astrit Tola,Cuneyt Gurcan Akcora,Baris Coskunuzer*

Main category: cs.LG

TL;DR: Topoformer is a new framework for graph representation learning, using Topo-Scan and Transformer to encode and process graph structures, achieving advanced performance.


<details>
  <summary>Details</summary>
Motivation: Graph representation learning needs to capture complex topological structures.

Method: Proposes Topoformer, a framework using Topo-Scan to encode graph structures into attention-friendly sequences, processed by a Transformer.

Result: Topoformer demonstrates state-of-the-art performance in graph classification and molecular property prediction benchmarks. It is efficient and parallelizable, with theoretical guarantees on stability.

Conclusion: Topoformer is a promising approach for graph representation learning that combines topological structure and attention mechanisms.
...
### [416] [HARGO: Heterogeneity-Aware Reward-Guided Optimization for RL Post-Training of LLMs on HPC Tasks](https://arxiv.org/abs/2607.28301)
*Tiangang Li,Xiangbo Tian*

Main category: cs.LG

TL;DR: HARGO通过引入置信度调制优势，提高了大型语言模型在HPC任务中的性能。
...
Method: 提出了一种名为HARGO的方法，该方法通过引入基于置信度的优势权重来对每个响应进行重要性加权，从而解决HPC任务中的极端异质性。

Result: 在四个HPC任务和九种方法中，HARGO在三个主要指标上均取得了最佳性能：WinRate 54.62%，数据竞争F1 91.30%，PLP相似度0.8558。消融实验证实了两种信号的互补贡献。

Conclusion: HARGO在异构HPC任务中建立了与比较方法相比的最佳整体对齐质量。
...
### [417] [Semi-Supervised Learning for Molecular Graphs via Ensemble Consensus](https://arxiv.org/abs/2607.28304)
...
### [418] [Beyond Geometric Complementarity: Coherent Overlap in Sparse Mixture-of-Experts Routing](https://arxiv.org/abs/2607.28308)
*Huiyuan Tian,Bonan Xu,Shijian Li*

Main category: cs.LG

TL;DR: MoE语言模型中，路由选择和专家子空间重叠对性能有重要影响，但几何相似性不能单独决定冗余或剪枝价值。


<details>
  <summary>Details</summary>
Motivation: 分析稀疏混合专家（MoE）语言模型的优势和局限性

Method: 使用专家子空间分离指数（ESSI）、匹配路由残差和前缀控制的2x2析因设计来区分路由一致性、候选质量以及候选与上下文交互

Result: 发现专家子空间重叠较大，实际路由比匹配替代方案更好地解释标记表示；选定的候选者在每个单元格中比最强的未选中竞争者解释了更多的残差表示，但实际前缀缩小了这种优势；添加后续专家在24个冻结路由比较中改善了下一个标记预测，而其他15个估计结果不明确；控制训练研究也倾向于Top-2而不是Top-1

Conclusion: 路由选择从共享几何邻域中选择与标记相关的专家，而有用的多专家计算在没有分离线性覆盖的情况下持续存在，这表明几何相似性本身不能确定冗余或剪枝价值。
...
### [419] [Measuring Distortion in the Empty Regions of Dimensionality Reduction Scatterplots with the Gap Index](https://arxiv.org/abs/2607.28324)
...
### [420] [LEDGERMIND: Provenance-Constrained Multimodal Agentic Reasoning with a Structured Evidence Ledger](https://arxiv.org/abs/2607.28374)
*Enjun Du,Hange Zhou,Chenxu Du,Siyi Liu,Zirong Chen,Ziyu Zheng,Yongqi Zhang*

Main category: cs.LG

TL;DR: LedgerMind通过结构化证据账本和多种协议提高了多模态代理在视觉问答中的准确性和可靠性。


<details>
  <summary>Details</summary>
Motivation: 多模态代理在视觉问答中的评估通常只关注最终答案的准确性，无法判断正确答案是通过有根据的证据、语言先验还是偶然错误消除得到的。

Method: 提出将多模态代理轨迹视为一个受源约束的状态机，工具输出被归一化到结构化证据账本中，下游推理和决策声明只能引用活动账本条目，对实体和数值进行归一化检查，修复作为类型状态转换实现，不能引入无工具产生的来源内容。

Result: 实验表明，LedgerMind在多个多模态推理基准和骨干MLLMs上提高了答案准确性和轨迹级别的忠诚度。

Conclusion: LedgerMind通过结构化证据账本和多种协议提高了多模态代理在视觉问答中的准确性和可靠性。
...
### [421] [Hierarchical Multilevel Monte Carlo for Order-Optimal Neural Actor-Critic in Average-Reward CMDPs](https://arxiv.org/abs/2607.28390)
*Ankur Naskar,Vaneet Aggarwal*

Main category: cs.LG

TL;DR: 本文提出了一种新的CMDP强化学习方法，通过引入分层MLMC神经批评家，实现了最优收敛保证，并消除了对底层混合时间的依赖。


<details>
  <summary>Details</summary>
Motivation: 在安全关键应用中，CMDPs为强化学习提供了一个自然框架，其中代理在满足长期约束的同时最大化长期奖励。

Method: 引入了分层MLMC神经批评家，在轨迹采样和批评优化过程中同时进行去偏。

Result: 该估计器以对数期望样本成本实现了长批评优化运行的偏差，并开发了一种原对偶自然演员-评论家算法，实现了最优性差距和约束违规的$	ilde{O}(T^{-1/2})$。

Conclusion: 这为具有一般策略参数化和神经批评家的无限时域平均奖励CMDPs提供了首次最优收敛保证，同时消除了了解底层混合时间的需要。
...
### [422] [Why Are GUI Agents Correct but Late? Decode on the Decision-Time Critical Path, Tested with Pre-Compiled Policy Trees](https://arxiv.org/abs/2607.28399)
*Zihan Dong,Rui Qian,Qishi Zhan,Dongshen Peng,Kaixin Li,Yu Li*

Main category: cs.LG

TL;DR: AAPT通过减少延迟提高计算机使用代理的成功率。


<details>
  <summary>Details</summary>
Motivation: 计算机使用代理在处理瞬时的GUI事件时经常失败，因为它们只有在相关窗口已经关闭后才能产生正确的动作。

Method: 提出了自适应前瞻性策略树（AAPT），该树在空闲屏幕期间构建一个有界条件策略树，其中包含可观察的守卫、预先授权的动作和分支特定的最后期限。

Result: AAPT提高了成功率，从0.50提高到0.79，并且没有产生错误的动作。

Conclusion: AAPT在候选动作可以提前枚举时表现最佳，而反应式执行在无法枚举时更强。
...
### [423] [QQWorld: Quantile-Quantile Matching for World Model Regularization](https://arxiv.org/abs/2607.28415)
*Zhoushun Yu,Xiaoyu Hu,Xiangyu Xu*

Main category: cs.LG

TL;DR: QQWorld improves LeWM's planning performance by enhancing the quality of the learned latent distribution.
...
Method: LeWorldModel (LeWM) regularizes its latents toward an isotropic Gaussian using the Epps-Pulley (EP) objective. QQWorld replaces EP with a quantile-quantile matching objective. Cross-batch QQ is developed to enlarge the effective ranking pool.
...
Conclusion: QQWorld addresses the limitations of LeWM by improving the quality of the learned latent distribution, leading to better planning performance.
...
### [424] [QAdapt: A Noise-Adaptive Neural Pre-Decoding Framework for Quantum Error Correction](https://arxiv.org/abs/2607.28422)
*Ran Miao,Rui Luo,Xiaohan Shan,Xiaoming Sun*

Main category: cs.LG

TL;DR: QAdapt框架提升量子纠错性能


<details>
  <summary>Details</summary>
Motivation: 提高量子纠错在硬件噪声下的鲁棒性和后端解码效率

Method: 提出了一种名为QAdapt的噪声自适应神经预解码框架

Result: 在合成噪声配置和Google的Willow基准数据上，QAdapt在逻辑错误率和后端解码延迟方面均有所提升

Conclusion: QAdapt为量子纠错提供了实用的解码兼容方法，有助于提高其在硬件噪声环境下的性能
...
### [425] [Kohn-Sham Spectral Embedding on Sparse Graphs at the Nishimori Temperature for Image Classification](https://arxiv.org/abs/2607.28428)
*V. S. Usatyuk,D. A. Sapozhnikov,S. I. Egorov*

Main category: cs.LG

TL;DR: KSSE是一种高效的图像识别模型，通过稀疏图谱嵌入和优化图拓扑，实现了高准确率和小模型尺寸。


<details>
  <summary>Details</summary>
Motivation: 提出Kohn--Sham Spectral Embedding (KSSE)方法，以物理启发为基础，用稀疏图谱嵌入替代密集卷积神经网络分类器。

Method: 将预训练特征映射到准循环低密度奇偶校验图，构建正则化拉普拉斯算子作为Kohn--Sham哈密顿量，通过FFT在循环块上求解独立信道谱问题。使用星域手术优化图拓扑，通过局部凸性构造边移位，同时将残余挫折限制在ρ(B_γ)≤1+δ。进行多尺度分形分析和分形学习率景观分析，实现从粗糙区域到星域盆地的景观转变，允许使用k_{text{mode}}=5模式的Rayleigh细化。

Result: 在ImageNet-1000数据集上，KSSE使用冻结的EfficientNet-B4特征，达到88.93%的Top-1准确率，使用约21.24M参数，优于Swin-L和ViT-H/14，同时减少模型尺寸10倍和30倍。

Conclusion: KSSE在图像识别任务中表现出色，有效减少了模型尺寸，具有理论上的创新和实际应用价值。
...
### [426] [Oracle-Budgeted Molecular Optimization with Short-Term Graph Memory](https://arxiv.org/abs/2607.28437)
...
TL;DR: 本文提出了一种因果救济方法，旨在提高高风险分类设置中的预测准确性，并通过实验证明其有效性。


<details>
  <summary>Details</summary>
Motivation: 提高高风险分类设置（如贷款和抵押贷款申请）中个体的预测结果。

Method: 建立因果表现框架，模型化如何通过结构因果模型传播救济行动，捕获特征之间的相互作用及其对真实标签的影响。

Result: 揭示因果救济政策可以导致稳定均衡，减少游戏激励，并在半合成和真实信用数据集上证明方法优于标准经验风险最小化。

Conclusion: 因果救济可以减少对模型重训练的需求，提高预测准确性。
...
TL;DR: 本研究提出了一种无泄漏的协议和CoTask Score，为同一图上的节点分类和链接预测之间的迁移提供了可靠的方法，并揭示了迁移的方向性和可预测性。


<details>
  <summary>Details</summary>
Motivation: 许多现实世界的图支持在相同的底层结构上执行多个预测任务，这为在节点分类（NC）和链接预测（LP）之间重复使用监督提供了机会。然而，现有的评估通常依赖于不兼容的拆分、观察到的图假设和负采样规则，使得关于同一图跨任务迁移的结论不可靠。

Method: 我们正式化了同一图的NC-LP迁移，并提出了一种无泄漏的协议，该协议修复了节点和边拆分，使用一个共享的消息传递图，该图排除了评估的边，并使用固定的负样本进行LP。

Result: 在三个骨干网络（GCN、GraphSAGE、GPS）上，我们发现迁移是强烈方向性和可预测的：在同质图中，NC $	o$ LP始终有益，而LP $	o$ NC则很脆弱，甚至在简单的表示重用下可能会降低准确性。LP $	o$ NC主要在结构主导的机制下可靠地变为正值，其中LP很容易，但NC未饱和，这表明LP充当结构预训练。最后，我们引入了CoTask Score（CTS）来总结当共享编码器必须服务于两个任务时的联合NC+LP效用，并表明简单的数据集统计信息，特别是同质性，可以指导机制选择并帮助避免负迁移。

Conclusion: 本研究通过提出一种无泄漏的协议和引入CoTask Score，为同一图上的节点分类和链接预测之间的迁移提供了可靠的方法，并揭示了迁移的方向性和可预测性。
...
TL;DR: 提出了一种新的无监督框架APO，用于预测原子系统的3D结构，在晶体和抗体结构预测中表现出色。


<details>
  <summary>Details</summary>
Motivation: 预测原子系统的3D结构对于材料科学和药物发现至关重要。然而，由于数据稀缺，传统的监督学习方法在预测新晶体相或从头蛋白质结构时面临瓶颈。

Method: 提出了一种名为Atomic Policy Optimization（APO）的完全无监督对齐框架，该框架利用了新颖的双重奖励机制：通过样本相似性的特征分解强化策略的主导潜在结构模式，并通过强制热力学稳定性来执行。

Result: 在晶体和抗体结构预测的广泛基准测试中，APO在匹配率和结构保真度方面均优于完全监督的基线，实现了新的最先进水平。此外，还展示了APO有效地直化概率路径，显著提高了推理效率。

Conclusion: APO为原子系统3D结构预测提供了一种有效且高效的方法，克服了数据稀缺性的挑战。
...
TL;DR: The study investigates the reliability of neural networks when combining data across different sources and proposes a framework to ensure compositional reliability.


<details>
  <summary>Details</summary>
Motivation: Neural networks increasingly combine data across populations, time periods, and operating conditions to improve generalization, but this raises a reliability question.

Method: Case-Based Decision Theory (CBDT) is used to formalize the requirement, and experiments are conducted across different domains.

Result: The framework makes compositional reliability measurable and operational through screening, analytic certification, geometry-oriented training, and decision-consequence auditing.

Conclusion: The study reveals stable and reversal-prone pooling regimes and highlights the importance of geometry-oriented objectives in achieving compositional reliability.
...
TL;DR: 本文研究了固定部署解决方案在球对称高斯环境动态下的预期生存时间，并推导了上下界。


<details>
  <summary>Details</summary>
Motivation: 研究在多连续环境中保持有效性的解决方案，并从理论角度理解生存时间这一基本属性。

Method: 建立生存时间模型，推导严格的下界和可计算的上界，并进行蒙特卡洛模拟验证。

Result: 分析表明，在缓慢变化的环境中，预期生存时间与σ^{-2}成Θ关系，在高度变化的环境中接近最小值1。蒙特卡洛模拟验证了理论预测，并检验了模型假设和参数不确定性的敏感性。

Conclusion: 提出的框架为部署寿命提供了分析特征，并确定了何时可以保证、排除或分析上无法解决的所需部署范围。
...
TL;DR: HOMER是一种基于Huber方法的稳健均值估计方法，在处理重尾分布和偏斜数据时表现出良好的性能。


<details>
  <summary>Details</summary>
Motivation: 重尾分布削弱了对经验均值的置信度控制，几何中位数均值（MOM）也缺乏一个趋向于均值效率的阈值。

Method: 提出了一种名为HOMER（Huber-of-Means for Efficient and Robust Estimation）的方法，该方法通过径向Huber中心聚合块均值。其规范形式和伪Huber形式分别限制每个块得分并在中位数类鲁棒性和经验均值之间进行插值。

Result: HOMER在有限二阶矩下建立了希尔伯特空间多数定理和MOM阶偏差界限。规范HOMER在其二次区域内恢复样本均值。伪HOMER随着阈值的增长而接近样本均值。它还允许围绕总体块Huber目标的渐近线性和一致沙丁鱼样协方差估计。在有限三阶矩下，固定的有限维投影以通常的参数速率支持均值推断。重尾模拟表明，当少数块摘要被替换时，HOMER仍然稳定。在干净的高斯数据上，两种版本都接近经验均值的效率。有限块三明治区间未充分覆盖，特别是对于偏斜的函数数据。进一步研究表明，当污染影响大多数块或损害普通块内均值时，会失败。

Conclusion: HOMER是一种有效且稳健的估计方法，在处理重尾分布和偏斜数据时表现出良好的性能。
...
TL;DR: 提出RPC方法防御多物品用户中毒攻击，提高鲁棒性，在可信和不信任环境中均有良好性能。


<details>
  <summary>Details</summary>
Motivation: 局部差分隐私(LDP)协议易受中毒攻击。现有研究为单个物品用户提出了有效的防御策略。然而，在实际应用中，用户可能拥有多个物品。对于多物品用户的防御中毒攻击具有挑战性，因为由于更大的输出空间，攻击者可以进行更强大的攻击而不被检测到。

Method: 提出了一种名为随机投影裁剪（RPC）的方法。首先，服务器向每个用户发送一个随机二进制向量。然后，用户将其本地数据投影到该向量上，并将值裁剪以限制攻击者的能力。为了处理裁剪偏差，我们提出了一种基于仔细分析的方法，给出了偏差的精确表达式。因此，不再需要偏差-方差权衡，从而可以将裁剪阈值进一步降低以缩小输出空间并增强鲁棒性。

Result: 我们为所有可能的攻击提供了严格的估计误差理论保证。数值实验表明，在可信环境中，我们的新方法与现有方法相比具有可比或更好的性能，表明我们的方法本身就是一个有效的估计器。在不信任环境中，我们的方法对中毒攻击也具有显著的鲁棒性。

Conclusion: 本文提出了一种针对多物品用户中毒攻击的防御策略，即随机投影裁剪（RPC），有效提高了鲁棒性，并在可信和不信任环境中均表现出良好的性能。
...
TL;DR: 研究了RKHS框架下的对抗训练，提出了一种提高泛化率的方法。


<details>
  <summary>Details</summary>
Motivation: 研究对抗训练在再生核希尔伯特空间（RKHS）框架下的表现，特别是通过关联核积分算子。

Method: 推导RKHS对抗训练估计器的源均匀泛化误差界限，并建立匹配的下界。提出两阶段噪声偏差程序来估计和消除混合项中的噪声贡献。

Result: 提高了泛化率，在稳健性水平选择样本相关阶数时，达到最小-最大多项式率。

Conclusion: 揭示了对抗训练中统计精度的损失，并提供了对抗鲁棒性和泛化之间的权衡的新解释和原理性解决方案。
...
TL;DR: EOMR在非线性动力学预测中优于现有机器学习工具。
...
Method: 扩展了熵最优流形聚类（EOMC），提出了熵最优流形回归（EOMR）。

Result: EOMR在非线性动力学预测中表现出色，优于其他机器学习工具。

Conclusion: EOMR是一种有效的非线性回归方法，在复杂系统预测中具有优势。
...
TL;DR: This paper provides a comprehensive review of uncertainty quantification methods in deep learning, focusing on ensemble-based and Bayesian approaches, and discusses their theoretical foundations, practical implementation, and performance evaluation.


<details>
  <summary>Details</summary>
Motivation: The deployment of deep neural networks in safety-critical domains demands reliable estimates of predictive confidence, yet conventional architectures lack principled uncertainty quantification.

Method: A structured, critical review of methods for Uncertainty Quantification (UQ) in deep learning, scoped to ensemble-based and approximate Bayesian approaches and the measures used to summarize their outputs.

Result: Organizes methods into five families: Bayesian neural networks, Monte Carlo Dropout, deep ensembles, efficient ensemble approximations, and last-layer or single-pass approaches. Examines theoretical motivation, implementation, empirical performance, and limitations.

Conclusion: Reviews ensemble diversity theory and uncertainty measures and their decompositions, contrasts the entropy decomposition with pairwise divergence measures, consolidates evaluation methodology, and discusses uncertainty in large language models and open research directions.
...
TL;DR: DR-FRL is a new method for longitudinal causal studies, showing promise in diverse situations.
...
Conclusion: DR-FRL is a novel approach for longitudinal causal studies, showing potential benefits in various scenarios.
