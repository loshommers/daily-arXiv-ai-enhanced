...
TL;DR: DocArena通过自动化数据整理流程，有效提升了搜索代理的训练数据质量和性能


<details>
  <summary>Details</summary>
Motivation: 提高训练数据质量，构建可控、可扩展且考虑多模态数据的训练环境

Method: 提出DocArena，一个基于多模态文档搜索和问答的自动化数据整理流程，包括文档结构化和索引、跨页信息分布分析、构建推理密集型问答对以及通过MLLM进行质量保证操作

Result: DocArena-79K包含来自8,336个文档的问答对，覆盖16个领域和49种语言；Doc-Search代理基础设施使基于文本的LLMs能够作为多模态文档检索和问答的推理骨干；在统一的评估框架下，DocArena数据训练的代理在检索准确性和问答质量上均表现最佳

Conclusion: DocArena有效提升了搜索代理的训练数据质量和性能，为多模态文档搜索和问答提供了新的解决方案
...
TL;DR: 本研究提出了一种结合图像处理和深度学习的混合方法，用于评估水果新鲜度，在苹果和橙子数据集上达到了90%以上的准确率。


<details>
  <summary>Details</summary>
Motivation: 减少水果腐烂导致的经济损失

Method: 结合图像处理和深度学习来评估水果新鲜度，包括图像处理算法和卷积神经网络（CNN），以及逻辑回归模型

Result: 实现了实时性能，并在包含苹果和橙子的数据集上达到90%以上的准确率

Conclusion: 该研究突出了将简单的图像处理技术与机器学习相结合，为农业领域提供实用解决方案的潜力。
...
TL;DR: LCG框架通过SRA和RCC机制，有效提升了长序列图像生成的一致性和质量


<details>
  <summary>Details</summary>
Motivation: 解决当前图像生成模型在长序列输出中一致性不足的问题

Method: 提出长上下文生成（LCG）框架，使用稀疏关系注意力（SRA）机制和路由一致性约束（RCC）

Result: LCG在长上下文图像生成中优于基准模型，包括多角色场景

Conclusion: LCG框架能够有效提高长上下文多图像生成的质量和一致性
...
TL;DR: 该研究提出了一种新的城市树木生物量估算框架，提高了估算精度，并生成了公共数据库。


<details>
  <summary>Details</summary>
Motivation: 城市树木生物量在空间上的量化不如管理森林的生物量精确，因为许多估计依赖于无法解析单个树冠或细尺度异质性的清单或粗略产品。

Method: 使用叶脱空中激光雷达和近红外RGB正射影像，结合双流交叉注意力网络进行语义标记和树顶划分，再根据物种特异性幂律进行AGB估算。

Result: 该框架实现了0.86、0.83和0.84的全局/平均精确度、召回率和Dice分数，并估计了2018年和2023年的总AGB储量为1.73和1.81吨，以及五年内的净碳增益为39吉加碳。

Conclusion: 该框架使用标准省级数据，无需人工标注，并产生一个针对森林外树木的公共双时相树顶级AGB数据库，具有管理相关分辨率。
...
TL;DR: 提出了一种基于深度学习的高精度预测激光渗透焊接渗透状态、深度和焊缝形态的方法，提高了焊接质量控制的准确性和效率。


<details>
  <summary>Details</summary>
Motivation: 评估激光渗透焊接的渗透状态和焊缝形态对于确定焊接质量至关重要。

Method: 提出了一种基于多任务深度学习模型的方法，该模型可以高精度地预测渗透状态、深度和焊缝形态。该模型利用互补金属氧化物半导体摄像头捕获激光焊接过程中的熔池图像，并结合时空特征和焊接参数，通过卷积神经网络和状态空间模型构建深度学习框架。此外，还提出了一种构建可靠数据集的方法，以提高模型的鲁棒性和泛化能力。

Result: 预测渗透状态的准确率可达99.35%，渗透深度的预测误差为1.79毫米，焊缝横截面重建的准确率为95.65%。

Conclusion: 本研究为激光渗透焊接系统的原位质量控制策略提供了新的见解和方法。
...
TL;DR: 本研究扩展了基于重建的fMRI任务学，揭示了认知任务状态之间的转移关系和任务分配规律


<details>
  <summary>Details</summary>
Motivation: 量化认知任务状态之间的转移关系

Method: 使用fMRI认知任务学方法，结合布尔整数规划（BIP）分析预算约束下的任务分配

Result: 揭示了跨范式有限的运动簇和工作记忆状态，在特定全局分配目标下具有高优先级

Conclusion: 本研究将基于重建的fMRI任务学从一对一转移扩展到多对一任务关系和预算约束下的任务依赖
...
TL;DR: GeMoE通过门控熵自适应路由，显著提升了LVLMs的性能和资源利用率
...
Result: 在多个基准测试中，平均性能保留率高达99.5%，专家激活稀疏度提高36.5%

Conclusion: GeMoE在MoE架构中有效提高了LVLMs的性能和资源利用率
...
TL;DR: 本研究提出了TUB数据集和PCD度量，提高了水下浑浊环境计算机视觉模型性能评估的准确性。


<details>
  <summary>Details</summary>
Motivation: 水下环境中能见度在浑浊条件下迅速下降，但对计算机视觉模型的影响尚不清楚。

Method: 引入了Turbid Underwater Baseline (TUB)数据集，包括在极端浑浊条件下捕获的1,320张图像和超过16,000个高置信度地面 truth分割掩码。此外，还提出了PCD，这是一种从相位一致性图中导出的度量，对对比度不变，旨在捕捉真实浑浊中的结构信息损失。

Result: PCD与实例分割模型在真实和合成浑浊图像上的性能强相关，而该领域的常见指标与性能几乎没有相关性。

Conclusion: 本研究通过引入新的数据集和度量标准，为水下浑浊环境中的计算机视觉模型提供了新的基准。
...
TL;DR: 本研究探讨了视觉提示调优中的融合方案，发现混合融合方式更有效，为该领域研究提供了新视角。


<details>
  <summary>Details</summary>
Motivation: 为了研究视觉提示调优中的基本但重要问题，即单一融合方案是否倾向于产生更好的结果，以及这种方案是否有利于开发混合融合方案。

Method: 将任务定义为双层优化问题，并利用可微架构搜索来解决它。在架构搜索中，我们提出了两种额外的融合方案，即仿射变换和交叉注意力，以丰富搜索空间。

Result: 在34个数据集上的实验表明，与提示调优基线相比，我们的方法在VTAB-1k、FGVC和HTA上取得了一致的增益。与VPT-Deep和最近变体相比，我们的方法在冻结的ViT主干上提供了有利的准确度-延迟-参数权衡。
...
TL;DR: VIGIL通过强化学习和几何约束解决了MLLMs的视觉幻觉问题，显著提升了其视觉-文本联合推理能力。


<details>
  <summary>Details</summary>
Motivation: MLLMs在推理能力上继承了LLMs的强大能力，但仍然容易受到视觉输入的幻觉影响。

Method: 提出VIGIL，一种强化学习后训练框架，通过几何约束最大化视觉输入和生成响应之间的互信息，并惩罚‘盲目自信’实例。

Result: VIGIL在幻觉和推理基准测试中优于现有方法，仅需25%的偏好数据即可匹配最先进方法的性能，并显示出空间定位能力。

Conclusion: VIGIL有效解决了MLLMs的视觉幻觉问题，提高了视觉-文本联合推理的准确性。
...
TL;DR: DinoLink通过压缩技术实现高精度远程感知，适用于V2X网络。


<details>
  <summary>Details</summary>
Motivation: 高精度远程感知常受V2X网络带宽限制的影响。

Method: 提出了一种名为DinoLink的以令牌为中心的压缩框架，使用离散语义通信替换原始像素流，以实现车辆-云协同推理。DinoLink采用双稀疏架构：一个显著性感知选择器修剪冗余背景令牌，而一个残差矢量量化（RVQ）模块将特征压缩成紧凑的码本索引。通过仅传输轻量级索引和位置先验，DinoLink与未压缩传输相比实现了139倍的比特率降低，同时在nuScenes数据集上保持了32.8%的mAP。

Result: 在nuScenes数据集上实现了32.8%的mAP，在窄带环境（如LoRa）中加速了34.5倍。

Conclusion: DinoLink是一个鲁棒、带宽高效的远程感知前端，适用于受限的V2X场景。
...
TL;DR: 提出了一种自监督框架，从视频信号中学习隐式三维物理动力学，在多个基准测试中表现出良好的性能。


<details>
  <summary>Details</summary>
Motivation: 为了解决当前生成式视频模型在三维几何基础方面的不足，导致物理不一致性和物体持久性无法维持的问题。

Method: 提出了一种自监督框架，从视频导出的监督信号中直接学习隐式三维物理动力学。该方法将预测瓶颈从二维图像空间转移到提升的三维体积潜在空间，通过将语义特征从视频联合嵌入预测架构（V-JEPA）投影到基于单目深度先验的体素网格中，实现体积特征对流的学习。

Result: 在多个基准测试（CLEVERER、PhysInOne、PhysGaia）上，模型表现出良好的长期结构稳定性和物理合理性。

Conclusion: 这项工作为通过被动观察单目视频来内化物理世界的三维不变性开辟了一条可扩展的途径，以实现通用动态世界模型。
...
TL;DR: APRTrack：针对RGB-Event跟踪的鲁棒性改进框架。


<details>
  <summary>Details</summary>
Motivation: RGB-Event跟踪在现实场景中面临多种信号退化问题，影响定位精度。

Method: 提出APRTrack框架，通过对抗性扰动和检索机制提高跟踪鲁棒性。

Result: 在多个数据集上实验表明，该方法有效提高了RGB-Event跟踪的鲁棒性。

Conclusion: APRTrack框架能够有效应对RGB-Event跟踪中的信号退化问题，提高定位精度。
...
TL;DR: 提出FFN模型，有效解决长视频TTT中的计算复杂度和效率问题。


<details>
  <summary>Details</summary>
Motivation: 针对长视频测试时间训练（TTT）的现有方法存在计算复杂度高和效率低下的问题。

Method: 提出了一种名为Frame Forgetting Network（FFN）的模型，该模型只处理滑动窗口中的三个帧，并定义了一个新的惊喜度量指标，以确定如何修改有效窗口大小。

Result: FFN在密集分割、视频分类、深度估计和多小时长视频任务上表现出良好的效果。

Conclusion: FFN模型在长视频TTT任务中表现出色，为解决现有方法的不足提供了新的思路。
...
TL;DR: CRISP通过诊断视觉空间智能，揭示了VLM评估中的问题，并提出了一种新的评估方法


<details>
  <summary>Details</summary>
Motivation: VLM评估中混淆语言先验与真正的空间推理

Method: 引入CRISP，一种新的结构诊断评估范式，通过一致性评估视觉空间智能

Result: 发现私有模型在度量估计和利用其隐式结构表示方面存在缺陷，而开源模型在多跳组合推理方面存在瓶颈

Conclusion: CRISP为超越端到端后训练的多模态对齐提供了一条严格的路线图
...
TL;DR: PhyEditBench是一个新的基准测试，用于评估图像编辑模型的物理理解能力，PhyWorld模型在图像编辑中表现优异。


<details>
  <summary>Details</summary>
Motivation: 评估图像编辑模型的物理理解能力

Method: 建立物理编辑基准测试（PhyEditBench）

Result: PhyWorld在图像编辑中表现优于其他模型

Conclusion: 现有图像编辑方法在物理推理方面存在局限性，PhyWorld作为训练自由基准模型，可以提升图像编辑的物理推理能力。
...
TL;DR: 提出了一种新的混合自监督方法，在非刚性3D形状匹配中取得了显著的性能提升。
...
Method: 提出了一种基于粗到细策略的混合自监督方法，具有双分支设计，包括基于拉普拉斯基和弹性基的功能映射学习流。

Result: 实验表明，该方法不仅保持了计算效率，还在各种具有挑战性的场景中实现了最先进的性能，包括非等距变形和拓扑噪声。

Conclusion: 该方法通过对比能量促进特征区分，并与现有方法集成，验证了整体有效性。
...
TL;DR: SpaceRipple是一种轻量级框架，用于地球观测卫星网络中的任务导向语义交付和机载处理，实现了高效、可靠的地球观测。
...
Result: SpaceRipple实现了良好的重建质量，提高了语义检测性能，并大幅节省了带宽，证明了其在受限的卫星网络资源下高效、可靠地进行地球观测的潜力。

Conclusion: SpaceRipple是一个高效、可靠的地球观测卫星网络语义交付框架，可以有效地减少卫星间流量，提高语义检测性能，并节省带宽。
...
TL;DR: DiCoBench是一个新的基准，用于评估MLLMs在感知高分辨率图像中的隐含视觉线索的能力。


<details>
  <summary>Details</summary>
Motivation: 为了填补现有基准测试的空白，评估模型在自主感知高分辨率图像中的隐含视觉线索的能力。

Method: 引入DiCoBench，一个综合的多图像高分辨率基准，包括两个渐进式轨道：差异视觉线索和共性视觉线索，涵盖8个不同的感知任务。

Result: 18种不同的MLLMs在DiCoBench上的表现与人类准确率（98.3%）存在显著差距，顶级模型在微尺度细节捕捉方面表现不佳。

Conclusion: DiCoBench将成为一个具有挑战性的测试平台，推动未来在自主、高分辨率多图像感知方面的研究。
...
TL;DR: LogicIR：一种基于逻辑门的图像恢复新方法，显著降低计算成本，提高效率。


<details>
  <summary>Details</summary>
Motivation: 随着图像恢复模型的计算需求不断上升，对轻量级架构的需求也在增加。逻辑门网络（LGNs）因其高效的计算特性而受到关注，但在图像恢复领域尚未得到充分利用。

Method: 提出LogicIR，一种基于逻辑门的全局网络架构，结合了UNet灵感和可微比特解码层以及索引洗牌机制。

Result: LogicIR在多个图像恢复基准测试中表现出色，显著降低了计算成本，成为图像恢复的有效和高效替代方案。

Conclusion: LogicIR为图像恢复提供了一种轻量级、高效的解决方案，有望推动该领域的发展。
...
TL;DR: TaskTok是一种高效的任务驱动图像恢复框架，通过选择性恢复任务相关的标记，显著提高了下游视觉任务的性能。


<details>
  <summary>Details</summary>
Motivation: 传统图像恢复关注感知质量，而任务驱动图像恢复（TDIR）旨在最大化下游高级视觉任务的性能。

Method: 通过分析潜在标记空间，观察到与任务相关的线索在标记序列中分布不均，表现出按索引的专业化。基于此，提出了一种名为TaskTok的新框架，该框架通过可学习的标记切换和轻量级的标记细化模块，仅对任务相关的标记进行恢复。

Result: 在图像分类、语义分割和目标检测等任务上，TaskTok显著提高了任务性能，同时保持了高计算效率。

Conclusion: TaskTok是一种高效的任务驱动图像恢复框架，能够显著提高下游视觉任务的性能。
...
TL;DR: FAROS通过流引导标注框架解决手术场景理解中的标注不平衡问题，显著提高了多任务学习的效果。


<details>
  <summary>Details</summary>
Motivation: 多任务学习在手术场景理解中受到标注粒度不匹配的限制，导致监督不平衡，限制了异构手术任务的联合优化。

Method: 提出了一种名为FAROS的流引导标注框架，结合零样本分割和光流估计，生成时间上一致的密集伪标签。

Result: 在DAVIS 2017基准和GraSP、MISAW、AutoLaparo基准上验证了FAROS的有效性，显著提高了跨任务表示学习和时空任务的手术场景理解性能。

Conclusion: FAROS通过流引导标注框架解决了手术场景理解中的标注不平衡问题，提高了多任务学习的效果。
...
TL;DR: 提出了一种新的事件模拟器FracEvent，提高了事件相机模拟的准确性。


<details>
  <summary>Details</summary>
Motivation: 为了解决实际场景中收集大规模事件数据困难的问题，提出了事件相机模拟技术。

Method: 引入了FracEvent，一种基于分数松弛电压动态的事件模拟器，用于模拟像素级生命周期。

Result: 在多个数据集上，FracEvent提高了生成事件的时间结构，并取得了比竞争模拟器基线更强的下游迁移结果。
...
TL;DR: LayersReg：提高手术导航配准精度和实时性的新方法。


<details>
  <summary>Details</summary>
Motivation: 传统的3D/2D配准方法在手术导航中效率低、失败率高。基于深度学习的方法提高了性能和精度。

Method: 提出LayersReg，具有3D解剖意识的回归模型，逐层搜索正确的姿态。结合节点回归和渐进式配准框架。

Result: 在X射线/CT配准和切片定位任务中，LayersReg在大的偏移和多模态条件下达到高精度，优于现有方法。

Conclusion: LayersReg提高了配准精度和实时性，适用于手术导航。
...
TL;DR: Disco-LoRA通过解耦和重组，实现多概念视频定制，有效控制视频的外观、风格和运动。


<details>
  <summary>Details</summary>
Motivation: 基于文本到视频（T2V）模型的视频定制旨在从参考数据中学习特定特征以生成可控视频。

Method: 构建了一个全面的基准，并提出了Disco-LoRA统一框架，通过分阶段解耦和灵活重组不同概念来解决这个问题。

Result: Disco-LoRA在多概念视频定制方面表现出色，有效地保留了外观、风格和运动，实现了可控的文本到视频生成。

Conclusion: Disco-LoRA框架通过有效的解耦和重组，在多概念视频定制任务中取得了显著的成果。
...
TL;DR: DeCoFlow通过分解子网络和补偿策略，在工业环境中的异常检测方面取得显著成果。


<details>
  <summary>Details</summary>
Motivation: 在工业环境中，新产品类别依次出现，需要不断地进行异常检测，但没有访问过去数据的权限。

Method: 基于正常化流（NFs）的特性，提出DeCoFlow，将子网络分解为冻结的通用基础和特定于任务的低秩适配器来隔离更新。同时，引入任务特定对齐、辅助耦合层和尾部感知损失来补偿冻结基础的刚性。

Result: DeCoFlow在MVTec-AD和VisA图像级AUROC上分别达到98.40%和93.00%，在参数级别上实现了零遗忘（正确路由下的0.00% FM），每个任务只有2.27M个参数。

Conclusion: DeCoFlow在异常检测方面表现出色，能够有效减少遗忘问题，参数高效。
...
TL;DR: PhysEditWorld improves game world models' physical dynamics and provides a new research foundation.


<details>
  <summary>Details</summary>
Motivation: Limitations of recent game world models in interaction behaviors and physical dynamics learning.

Method: Multimodal dataset with physical parameters, replay paradigm implemented with a UE5 replay-and-rendering pipeline.

Result: Improved gravity-faithful dynamics modeling, enhanced consistency under physical edits, scalable foundation for controllable world modeling research.
...
TL;DR: 提出了一种多任务框架，提高了颅内动脉瘤检测的准确性和效率，在挑战赛中取得了优异成绩。


<details>
  <summary>Details</summary>
Motivation: 颅内动脉瘤在破裂前通常无症状，破裂后死亡率高。破裂风险评估和治疗计划取决于动脉瘤形态和解剖位置，但现有的自动化方法仍然局限于二元检测，没有精细的解剖分类或多类分割。

Method: 提出了一种多任务框架，同时执行多标签分类、多类动脉瘤分割和多类血管分割，涵盖13个解剖位置和四种成像模式（CTA、MRA、T2、T1-post）。两阶段方法结合了快速二维三轴感兴趣区域（ROI）提取方法和3D多任务nnU-Net骨干网络。双解码器设计减轻了动脉瘤和血管类别之间的极端体积不平衡，而交叉注意力池和模式特定辅助头部提高了异构输入的特征学习。两倍集成在2025年RSNA颅内动脉瘤检测挑战赛中获得了第二名。

Result: 两倍集成在2025年RSNA颅内动脉瘤检测挑战赛中获得了第二名。代码、模型权重和3D Slicer插件已公开。

Conclusion: 该研究提出了一种有效的方法来提高颅内动脉瘤检测的准确性和效率，为临床治疗提供了新的工具。
...
TL;DR: M2C框架通过SAM3实现高效的医学图像标注


<details>
  <summary>Details</summary>
Motivation: 提高医学数据标注的可扩展性

Method: 提出Mask to Concept (M2C)框架，利用SAM3进行医学图像标注

Result: 实现SOTA的少样本分割性能和高效的标注效率

Conclusion: 为可扩展的医学图像标注提供了一种实用且高效的方法
...
TL;DR: NeuMatEx通过逆渲染和LMRM提取图像中的复杂神经材料，效果优于PBR方法。


<details>
  <summary>Details</summary>
Motivation: 在图像中提取具有复杂反射和散射效果的神经材料具有挑战性。

Method: 提出了一种名为NeuMatEx的基于可微逆渲染的方法，通过训练一个大型材料重建模型（LMRM）来直接从图像中预测初始基础颜色、神经材料潜变量和随机不确定性指南。

Result: 实验表明，NeuMatEx在提取复杂材料方面比基于PBR的方法具有更好的视觉效果和材料分解。

Conclusion: NeuMatEx可以有效地从图像中提取具有复杂反射和散射效果的神经材料，并优于基于PBR的方法。
...
TL;DR: FUSE通过频谱均衡和跨频带一致性，有效提高了不可学习示例的全频谱保护能力。


<details>
  <summary>Details</summary>
Motivation: 为了保护训练数据，通过注入不可感知的扰动，使模型无法提取可利用的表示，研究者们提出了不可学习示例（UEs）。然而，现有的UEs在低通滤波后存在关键缺陷，表明对不可学习性的有效扰动信号主要集中在高频。因此，研究者们认为可靠的UEs应该在全频谱范围内保持有效性。

Method: 提出了一种名为FUSE的方法，通过频谱均衡来生成频谱无关的扰动，并强制执行跨频带一致性，以保持不可学习性。具体来说，FUSE在生成器训练期间采用随机频谱掩码（RSM）策略，随机去除连续的频带，迫使剩余的频带保持不可学习性。此外，FUSE还集成了跨频带指导（CBG），强制执行高频和低频组件之间的相互一致性，从而进一步增强低频不可学习性，并调节高频扰动以保持图像的语义保真度。

Result: 在多个数据集、架构和频谱滤波方面的广泛实验表明，FUSE实现了强大的保护效果。

Conclusion: FUSE是一种有效的不可学习示例生成方法，能够在全频谱范围内提供强大的数据保护。
...
TL;DR: 通过分析发现，鲁棒性主要受图像领域影响，而非注释，通过改进方法提高了鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 分析现实世界噪声对开放词汇目标检测器（OV-ODs）的影响，由于架构复杂性，这一影响目前理解不足。

Method: 通过使用受控的合成视觉退化，对OV-ODs进行逐层分析，揭示鲁棒性退化的原因、方式和位置，系统地分析特征崩溃。

Result: 发现具有相似视觉骨干的网络在相似层具有相似的特征崩溃，从而表现出相似的鲁棒性。预训练策略、架构细节和标题监督对鲁棒性的贡献很小。鲁棒性主要受图像领域的影响，而不是注释。通过在BDD100K、WiderFace和VisDRONE上提高鲁棒性，验证了这些发现。

Conclusion: 鲁棒性主要受图像领域影响，而非注释，通过改进方法提高了鲁棒性。
...
TL;DR: 3DLP基准评估图像编辑模型对真实世界光照的理解，发现现有模型仍有改进空间
...
Method: 引入3D-anchored Light Probe (3DLP)基准，创建真实世界光照变化的高保真HDR数据集，并使用新的评分标准评估模型

Result: 不同图像编辑模型的性能差异显著，最佳模型与真实世界物理规律高度一致，但仍存在改进空间

Conclusion: 3DLP基准有助于评估图像编辑模型对真实世界光照的理解，但现有模型仍有改进空间
...
TL;DR: 提出了一种新的流媒体视频编辑框架，实现了稳定的长时编辑和实时响应，适用于交互和增强现实应用。


<details>
  <summary>Details</summary>
Motivation: 流媒体视频编辑在实际部署中存在两个核心问题：长时间内保持稳定背景和非编辑区域的稳定性，以及实现实时交互场景所需的低延迟。同时，最近开发的流媒体视频生成方法大多用于合成，不能直接应用于编辑，因为对编辑的严格保留要求和区域特定控制。

Method: 提出了一种新的流媒体视频编辑框架，该框架执行因果、逐帧编辑，具有强大的内容保留和实时响应能力。关键设计是一个三阶段蒸馏管道，将编辑能力从强大的双向基础模型逐步转移到高效的单向流媒体编辑器，在不牺牲视觉保真度的同时实现稳定的长时编辑。为了进一步支持实时部署，引入了面向AR的掩码缓存，跨帧重用区域相关计算，大大减少冗余处理并加速推理。最后，建立了一个专门的流媒体视频编辑基准。

Result: 在流媒体基线中实现了最先进的视觉质量，同时将推理速度提高到了12.66 FPS，使其适用于交互和增强现实应用。

Conclusion: 该方法在流媒体视频编辑领域取得了显著的进展，为实时交互和增强现实应用提供了有效的解决方案。
...
TL;DR: 本文提出了一种基于视觉-雷达融合的多模态点云生成方法，显著提升了点云质量和感知模型的性能。


<details>
  <summary>Details</summary>
Motivation: 提高毫米波雷达点云的质量，以改善下游感知任务（如目标检测和跟踪）的性能。

Method: 基于视觉-雷达融合的多模态点云生成方法，结合图像语义信息、结构约束、空间对齐和稀疏补全策略。

Result: 有效提高了点云质量，提升了感知模型在复杂环境中的检测精度和鲁棒性。

Conclusion: 该方法为多传感器点云生成和智能感知系统提供了实用的解决方案。
...
TL;DR: Proposed a capacity-controlled framework for multi-view stylization of 3DGS, enhancing stylistic consistency and stability across viewpoints.
...
Conclusion: Our proposed framework effectively addresses the challenge of enforcing stylistic coherence across viewpoints in 3DGS, leading to improved multi-view stylistic consistency and stable, expressive 3D stylizations.
...
TL;DR: ProtoKV是一种高效的SVU内存解决方案，通过优化内存使用和查询时间延迟，显著提高了SVU的准确性。


<details>
  <summary>Details</summary>
Motivation: Streaming video understanding (SVU)需要处理异步到达的查询，同时视觉标记持续流过，在严格的GPU内存和查询时间延迟预算下。主要挑战是延迟查询：决定性线索可能短暂出现，但在查询到达之前，许多后续更新发生，增加了这些线索在有限内存中被删除或稀释的风险。

Method: 提出ProtoKV，这是一种常规模外SVU内存，将远历史表示为固定容量的摘要状态，而不是保留标记实例。ProtoKV保留精确的近窗口KV缓存，并将旧内容聚合到语义空间原型库中，具有剩余统计信息。查询时，每个原型通过一个有限的伪标记接口公开，该接口与标准注意力完全兼容。

Result: 在匹配的预算和可比的查询时间成本下，ProtoKV在SVU基准测试中的长期延迟模式下，与保留标记基线相比，提高了12.5个百分点的准确性，随着查询延迟的增加，收益增加。

Conclusion: ProtoKV在处理SVU中的延迟查询方面表现出色，提高了准确性并优化了内存使用。
...
TL;DR: CHOIR通过优化网络深度和缓解光谱偏差，提高了INR的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的隐式神经网络表示（INR）方法主要依赖于周期激活函数（如正弦函数）的组合，这会导致优化不稳定，并限制了它们的性能。同时，这些方法未能结合适当的物理先验来有效缓解光谱偏差。

Method: 提出了一种新的校准谐波叠加隐式神经网络表示（CHOIR），利用协调谐波叠加（CHS）来替代传统的函数组合，并引入感知光谱校准（PSC）来缓解光谱偏差。

Result: 在多维数据恢复问题上取得了优于现有方法的性能。

Conclusion: CHOIR方法通过优化网络深度和缓解光谱偏差，提高了INR的性能。
...
TL;DR: 提出了一种4D医学图像合成框架，提高了分割性能和模型鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 解决4D医学图像建模中存在的有限标注数据、设备域迁移和隐私限制等问题。

Method: 提出了一种4D可控生成框架，用于解剖一致的数据增强，包括半监督变分自编码器、级联潜在扩散模型等。

Result: 实验结果表明，该框架提高了静态解剖的可控性和时间一致性，并提高了下游分割性能。

Conclusion: 该框架为4D医学图像合成提供了可扩展且可控的解决方案，支持开发具有有限标注和跨厂商可变性的更稳健的模型。
...
TL;DR: LearniBridge通过轻量级更新实现特征缓存校准，大幅提升计算效率。


<details>
  <summary>Details</summary>
Motivation: 为了解决Diffusion Transformers在图像和视频生成中的计算成本问题，研究人员提出了特征缓存方法。然而，现有方法在高速率加速时存在严重的误差累积问题。

Method: 研究人员研究了所需特征校正的性质，发现最优校准更新由不同提示下的共享低秩子空间表征。基于这一结构洞察，提出了LearniBridge，这是一种可学习的校准机制，通过轻量级的LoRA更新连接多个时间步。

Result: 在FLUX、HunyuanVideo和WAN2.1上，LearniBridge分别实现了$5.87	imes$、$5.75	imes$和$4.10	imes$的加速。在WAN2.1上，与之前的SOTA相比，在$4.10	imes$加速下提高了VBench 1.28%。

Conclusion: LearniBridge通过轻量级的LoRA更新有效提高了特征缓存的校准效果，显著降低了计算成本。
...
TL;DR: 开发了一种基于事件视觉的系统，实现了对球体旋转的高精度和低延迟测量。


<details>
  <summary>Details</summary>
Motivation: 在许多球类运动中，旋转对球的轨迹起着至关重要的作用。由于传统相机存在球体小、速度快、旋转快等问题，在比赛中使用基于视觉的估计方法来实时测量球体旋转具有挑战性。

Method: 提出了一种基于事件驱动的主动视觉系统，该系统使用高时间分辨率的摄像头来跟踪未经修改的球并测量其旋转。该系统包括一个用于高时间分辨率的摄像头，高速度的旋转/倾斜磁力仪镜子以保持球在视野中，以及一个低延迟、可调焦的远摄镜头以增加对球的时空分辨率。为了跟踪球体，采用了一种结合2D事件检测和3D定位的系统。

Result: 该方法在多个运动（乒乓球、棒球、网球和高尔夫）中达到了最先进的静态球体旋转估计精度，平均大小误差和轴误差分别为2.1%和4.0度。针对乒乓球，开发了一种低延迟的在线方法，并实现了可靠的跟踪和旋转估计。

Conclusion: 该系统实现了高精度、低延迟的球体旋转估计，在球类运动中有潜在应用价值。
...
TL;DR: ReasonCLIP-58M：通过推理监督增强CLIP式视觉表示的推理能力。


<details>
  <summary>Details</summary>
Motivation: 为了解决CLIP-style编码器在视觉常识推理和组合推理方面的不足，提出ReasonCLIP-58M框架。

Method: 通过两阶段策略将大规模推理监督集成到CLIP-style模型中，包括描述性对齐和类别结构推理监督。

Result: ReasonCLIP-58M提高了视觉常识和组合推理能力，同时增强了零样本检索性能。

Conclusion: ReasonCLIP-58M框架能够通过结构化推理监督增强CLIP-style视觉表示的表达能力。
...
TL;DR: NaviCache：一种基于惯性导航系统的视频扩散模型加速方法


<details>
  <summary>Details</summary>
Motivation: 视频扩散模型（VDMs）的巨大计算成本限制了其发展。传统的离线校准加速方法存在校准数据依赖、校准时间过长和对分布变化的敏感性等问题。而离线校准无方法虽然避免了这些问题，但存在观测噪声敏感和忽略扩散轨迹内在动量的问题。

Method: 提出了一种名为NaviCache的插拔式测试时自校准方法，将特征演变重新概念化为惯性导航系统（INS）问题。NaviCache通过建模输入和输出变化之间的相对耦合来弥合基本域差距和非平稳性。引入了一种双状态估计架构，该架构自适应地跟踪特征变化比及其潜在漂移，并通过一个专门的初始对齐阶段进行初始化。通过整合时间相关的噪声计划和不确定性感知的测量更新机制，NaviCache提供了一种理论上有根据的错误边界计算跳过的机制。

Result: 在HunyuanVideo、Wan和Open-Sora系列上的大量实验表明，NaviCache在计算跳过的错误判断方面表现出更高的准确性，并实现了出色的综合性能。

Conclusion: NaviCache是一种有效的视频扩散模型加速方法，能够提高计算效率并减少观测噪声的影响。
...
TL;DR: 提出了一种基于掩码引导的MMIF方法，显著提升了图像融合的性能。


<details>
  <summary>Details</summary>
Motivation: 为了解决恶劣天气条件下图像融合的挑战，现有方法在特征表示学习方面存在局限性。

Method: 提出了一种基于掩码引导的多模态图像融合（MMIF）方法，该方法结合了特征恢复和交互。

Result: 在合成和真实世界数据集上的实验表明，该方法在视觉质量、定量指标和下游任务方面优于现有方法。

Conclusion: 该方法通过掩码引导和任务耦合的降级感知学习策略，在视觉质量和下游任务方面取得了显著的性能提升。
...
TL;DR: 提出了一种鲁棒的幼苗检测框架，在户外复杂光照条件下有效，提高了检测精度和定位性能。


<details>
  <summary>Details</summary>
Motivation: 准确检测幼苗对于精准农业中的及时再种植和有效作物管理至关重要。然而，现有研究大多在相对稳定的成像条件下进行评估，如无人机图像或温室环境，而地面基于户外监测的鲁棒检测研究不足。此外，许多鲁棒检测方法依赖于额外的增强或特征提取模块，这增加了推理时间开销，并且没有针对幼苗检测和下游缺失幼苗定位进行优化。

Method: 构建了一个新的大蒜幼苗数据集，该数据集使用地面监测平台在真实户外田野条件下捕获，光照条件高度可变。进一步提出了一种基于对抗增强策略学习的鲁棒幼苗检测框架。该方法联合优化了随机增强策略代理和目标检测器，使检测器能够在具有挑战性的视觉条件下学习鲁棒表示。引入了结构惩罚，以防止不真实的扭曲，同时鼓励在训练期间进行具有挑战性的增强。

Result: 在AP50方面达到91.6%，比基线提高了0.9个百分点，比之前表现最好的方法提高了0.2个百分点。对于下游缺失幼苗定位，它实现了75.0%的精确度和67.0%的F1分数，分别比基线提高了4.8和2.0个百分点。

Conclusion: 该方法在复杂户外光照条件下，无需额外的推理时间计算开销，对地面农业监测是有效的。
...
TL;DR: AnomNOVIC是一种高效、无提示的开放词汇对象检测器，在真实环境中表现出色。


<details>
  <summary>Details</summary>
Motivation: 机器人系统在真实环境中操作时必须能够识别以前未见过的对象。随着机器人系统向开放世界自主性发展，对无提示、高效的开放词汇对象检测器的需求日益增长，但这一需求在很大程度上尚未得到满足。

Method: 提出了一种名为AnomNOVIC的两阶段已知工作空间框架，该框架结合了用于异常检测的掩码自动编码器（MAE）和强大的实时无提示开放词汇图像分类器NOVIC。MAE产生通用的对象无关的边界框，允许NOVIC在没有预定义候选类列表的情况下对显著图像区域进行分类。

Result: 在包含NICOL人形机器人的桌面机器人-对象环境中，AnomNOVIC在无提示识别方面达到了47.1% AP / 57.5% AP50，如果提供类候选，则达到了59.0% AP / 72.5% AP50。在包括具有48个独特对象的野外测试集在内的其他数据集上，AnomNOVIC达到了高达82.6%的无提示检测和分类准确率。这些结果显著优于所有测试的开放词汇基线，包括YOLO-World-v2、OWLv2和YOLOE。

Conclusion: AnomNOVIC在开放词汇对象检测方面表现出色，显著优于现有基线。
...
TL;DR: LFNet通过融合不同神经网络范式的特征，在显著目标检测中取得最佳性能。
...
Method: 引入液体融合网络（LFNet），利用动态门控机制进行内容感知特征聚合，并采用Saliency-Guided Upsampling（SGU）操作。

Result: 在五个不同的任务上达到最先进的性能，在检测精度和模型效率之间提供了更好的权衡。

Conclusion: LFNet在一般显著目标检测中表现出色。
...
TL;DR: 该方法通过引入仿射对应和新的约束，使得RS相对位姿估计变得可行，并在基准测试中表现出色。


<details>
  <summary>Details</summary>
Motivation: Rolling shutter (RS) cameras在消费设备中几乎普遍存在，但RS感知相对位姿估计仍然不切实际。

Method: 将仿射对应（ACs）引入RS双视几何，推导新的RS校正仿射约束，并开发一个线性化代数求解器来估计位姿和RS运动。

Result: 在TUM RS基准测试中，该方法在所有测试方法中实现了最佳的位姿和RS参数精度。在EuRoC MAV数据集上，求解器达到了与标准5点算法相当的精度。

Conclusion: 该方法使得RS相对位姿估计变得可行，并在多个基准测试中取得了优异的性能。
...
<details>
  <summary>Details</summary>
Motivation: 为了解决现有方法依赖于单一整体图像奖励，导致细粒度编辑优化困难的问题。

Method: 提出了一种名为SpatialFlow-GRPO的训练框架，引入了空间细粒度奖励反馈。该框架将区域感知奖励转换为语义区域级别的优化信号，并在策略更新过程中将区域优势与相应的潜在位置对齐。还训练了一个区域感知奖励模型SFReward，构建了SFReward-14K，并引入了MultiEditBench来评估多区域编辑能力。
...
Conclusion: SpatialFlow-GRPO能够将局部反馈转换为空间对齐的更新信号，从而提高图像编辑质量。
...
TL;DR: 提出了一种基于RIS的PHO方法，优化了RIS元素数量，提高了能效。


<details>
  <summary>Details</summary>
Motivation: mmWave网络易受视线遮挡，需要使用VAWC和RIS来保证及时切换，同时考虑到RIS配置时间的影响。

Method: 使用粒子群优化算法（PSO）来确定最佳的RIS链路配置。

Result: 通过优化RIS元素数量，减少了12%的RIS元素，降低了10%的能耗，同时提高了信号质量。

Conclusion: 提出了一种新的基于RIS的PHO方法，该方法通过优化RIS元素的数量来平衡信号处理复杂性和链路质量，从而提高RIS辅助链路的能效。实验结果表明，减少RIS元素数量可显著降低能耗，同时不影响信号质量。
...
TL;DR: OTF-CBM是一种新的概念瓶颈模型，通过动态跨模态传输过程实现更有效的可解释跨模态推理。


<details>
  <summary>Details</summary>
Motivation: Concept Bottleneck Models (CBMs) promise transparent reasoning by predicting through human-interpretable concepts, yet their effectiveness fundamentally depends on how well visual and textual representations are aligned or matched.

Method: proposes the Optimal Transport Flow Concept Bottleneck Model (OTF-CBM), which learns a data-driven semantic cost via Inverse Optimal Transport to measure cross-modal distances and performs unbalanced optimal-transport-based flow matching to model semantic transitions.
...
Conclusion: OTF-CBM provides a more effective and interpretable approach for cross-modal reasoning compared to existing methods.
...
TL;DR: MICViT在多模态脑MRI分析中表现出色，有效提升了性能。


<details>
  <summary>Details</summary>
Motivation: 神经影像学研究样本量有限，而解剖、病理和采集条件多样性大，这使得从高维3D数据中学习成为机器学习的挑战。

Method: 提出了一种名为MICViT的3D视觉Transformer，该模型明确地建模了模态特定的表示和跨模态的局部和全局交互。

Result: 在三个异构数据集上，MICViT在3D设置中始终优于最先进的CNN和Transformer基线，并且随着模态的增加，性能提升更明显。

Conclusion: 明确建模跨模态交互对于释放多模态脑MRI的潜力至关重要。
...
TL;DR: 提出了一种合成数据增强框架，显著提高了dMRI束追踪的准确性


<details>
  <summary>Details</summary>
Motivation: 提高扩散磁共振成像（dMRI）束追踪的准确性

Method: 合成数据增强框架，使用2D U-Net进行纤维束分割

Result: 与仅使用真实数据训练相比，在排除的脑部实验中显示出更好的泛化能力，性能与最先进的方法相当，同时所需手动注释数据减少了3倍

Conclusion: 该方法在减少手动注释数据的同时提高了dMRI束追踪的准确性
...
TL;DR: Robust-TO框架通过集成每帧可信度，提高了视频推理的准确性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 视频推理语言模型在现实情况下存在视觉证据降级的问题，导致准确率下降。

Method: 提出Robust-TO框架，将每帧的可信度集成到推理的每个阶段，并使用统一的证据接口。

Result: 在两个视频推理基准测试中，Robust-TO在干净输入上达到56.4%的平均准确率，超过最强大的开源基线10.6%。在五种现实腐败类型下，Robust-TO的平均准确率为54.3%，比最强大的开源基线高5.8%。

Conclusion: Robust-TO框架有效提高了视频推理的准确性和鲁棒性。
...
TL;DR: Qwen-Image-Agent有效解决了T2I模型中的上下文差距，提升了图像生成能力。


<details>
  <summary>Details</summary>
Motivation: 解决T2I模型在处理真实世界请求时面临的挑战，如需求不明确、隐含或依赖于最新知识。

Method: 提出Qwen-Image-Agent，一个统一的智能体框架，集计划、推理、搜索、记忆和反馈于一体。

Result: 在IA-Bench、Mindbench和WISE-Verified上实验表明，Qwen-Image-Agent优于强基线，达到最先进的性能。

Conclusion: Qwen-Image-Agent能够有效弥补T2I模型中的上下文差距，提高图像生成的质量和效率。
...
TL;DR: PhysRAG enhances physical awareness in video generation with state-of-the-art performance.
...
Result: State-of-the-art performance in both visual quality and physical rule compliance, surpassing existing models in benchmarks such as PhyGenBench and VBench. Conduct extensive ablation studies to validate the effectiveness of key components.

Conclusion: PhysRAG is an effective pipeline for enhancing physical awareness in video generation.
...
TL;DR: PortraitGen通过引入真实图像和互补奖励机制，显著提高了文本到图像后训练的真实感。


<details>
  <summary>Details</summary>
Motivation: 当前文本到图像后训练方法存在缺陷，如过度饱和的色彩和AI伪影等。

Method: 提出PortraitGen框架，通过引入真实图像和互补奖励机制来提高图像的真实感。

Result: PortraitGen在图像真实感方面显著优于现有方法，有效抑制了AI伪影。

Conclusion: PortraitGen为提高文本到图像后训练的真实感提供了有效解决方案。
...
TL;DR: SharpMoE通过精确路由和约束计算资源分配，提高了视觉生成的效率和性能。


<details>
  <summary>Details</summary>
Motivation: 为了提高视觉生成中扩散模型的扩展性，混合专家（MoE）架构被证明是一种强大的范式。然而，现有的扩散MoE框架中存在路由分配问题，即路由器无法准确地将更多的计算资源分配给显著标记。

Method: 提出了一种名为SharpMoE的框架，该框架具有利用清洁潜在特征作为无噪声路由信号的精确路由机制。此外，还引入了轨迹路由损失来约束多步骤去噪轨迹中的计算资源分配。

Result: 实验表明，SharpMoE是一种通用的即插即用解决方案，可以进一步提高预训练的MoE模型的性能，在视觉生成中达到最先进的水平。

Conclusion: SharpMoE通过解决现有扩散MoE框架中的路由分配问题，显著提高了视觉生成的效率和性能。
...
TL;DR: TraMP-LLaMA是一种改进的面部表情质量评估方法，通过生成结构化文本报告提高了可解释性，并在帕金森病评估中实现了更好的严重程度预测性能。


<details>
  <summary>Details</summary>
Motivation: 现有面部表情质量评估（FEQA）方法通常只产生严重程度评分，而没有明确传达支持预测的可观察面部运动证据，这限制了可解释性，并使得检查模型输出的基础在帕金森病评估中变得困难。

Method: 提出了一种名为TraMP-LLaMA的统一多模态框架，该框架联合预测严重程度评分并从面部运动线索生成结构化文本报告。该框架集成了RGB外观和地标轨迹线索，并采用了解耦指令调整策略以减少严重程度预测和语言生成之间的任务干扰。为了支持这一任务，我们还通过专家引导的文本运动描述扩展了PFED5数据集，并构建了PFED5-plus。

Result: 在PFED5-plus上的实验表明，TraMP-LLaMA在报告生成方面优于竞争性的视频语言基线，在联合多表情训练下，与比较方法相比，实现了最佳的严重程度预测性能，Spearman等级相关系数至少提高了4.39个百分点。

Conclusion: TraMP-LLaMA在帕金森病评估中的面部表情质量评估方面表现出色，提高了可解释性和预测准确性。
...
TL;DR: 提出OmniRef-Bench基准和DyRef框架，提高多参考图像生成性能。


<details>
  <summary>Details</summary>
Motivation: 现有基准无法充分评估复杂的MRIG场景，阻碍了该领域的发展。

Method: 提出了一种两阶段训练框架DyRef，第一阶段使用监督微调使模型具备处理复杂多参考图像生成任务的基本能力，第二阶段引入Difficulty-aware Advantage Reweighting (DAR)和Discriminative Reward Scaling (DRS)来提高性能。

Result: 在OmniRef-Bench上的评估表明，主流开源模型在复杂MRIG场景中表现不佳，性能随着混合类型参考图像数量的增加而显著下降。

Conclusion: DyRef显著提高了开源模型在OmniRef-Bench和单图像编辑基准测试中的性能，证明了我们方法的有效性和泛化能力。
...
TL;DR: 本研究引入了Dota2-Vis，一个基于视频的MOBA游戏可见性分析数据集，揭示了传统分析难以获得的模式。


<details>
  <summary>Details</summary>
Motivation: 为了更全面地分析MOBA游戏中的可见性，本研究引入了Dota2-Vis视频数据集和基于视频的可见性分析基础管道。

Method: 本研究构建了一个包含144场比赛的视频数据集，包括从双方视角记录的288个全高清视频和2,477个手动标注的小地图图像。使用现代目标检测器进行玩家图标检测，并使用性能最好的模型来估计对手可见的玩家存在。

Result: YOLO11l（大）模型在密集和视觉杂乱的小地图场景中可靠地识别玩家图标，并揭示了玩家、英雄、角色和团队级别的模式，这些模式补充了传统的MOBA分析，突出了仅从结构化数据中难以获得的差异。

Conclusion: Dota2-Vis数据集和代码已公开，为MOBA游戏中的可见性分析提供了新的视角。
...
TL;DR: GGR通过梯度校正，在开放集半监督学习中实现了性能提升。
...
Method: 提出了一种名为GGR的插件框架，该框架使用监督梯度作为锚点，并将冲突的辅助梯度投影到梯度空间中的可接受区域。

Result: 在CIFAR和ImageNet基准测试中，GGR在大多数设置中提高了代表性OSSL基线，并在封闭集泛化能力和开放集鲁棒性方面都取得了收益。

Conclusion: GGR通过梯度级别的控制，在开放集半监督学习中取得了显著的性能提升。
...
TL;DR: Unison是一个全面基准，用于评估统一多模态模型中的联合理解和生成，有助于发现当前系统的局限性并指导未来的研究方向。


<details>
  <summary>Details</summary>
Motivation: 尽管现有的统一多模态模型在理解和生成方面都取得了显著的进步，但它们通常将理解和生成能力分开评估，忽略了理解和生成之间的协同作用。

Method: 提出了Unison，一个包含2,169个高质量统一任务样本的全面基准，用于评估统一多模态模型中的联合理解和生成。Unison具有三个关键优势：1）全面维度：Unison涵盖了内部一致性、理解引导的生成、生成引导的理解和相互增强，以实现全面评估。2）诊断评估：它提供了理解和生成的一体化和解耦轨道，允许对失败模式进行细致的归因和从统一建模中获益的定量分析。3）与人类判断的对齐：我们还引入了与人类判断良好对齐的Unison-Judge评估模型，以确保可靠的评估。

Result: 在Unison上对最先进的模型进行系统评估后，我们发现了当前统一多模态系统中的关键局限性，并突出了未来研究的有希望的方向。

Conclusion: Unison为统一多模态模型提供了更全面的评估，有助于发现当前系统的局限性并指导未来的研究方向。
...
TL;DR: EVIS通过事件感知和分层理解，有效解决了视频分割问题。


<details>
  <summary>Details</summary>
Motivation: 现有视频分割方法通常将视频视为一个由多个图像组成的事件，忽略了视频通常包含多个不同事件的事实。这种方法导致模型需要直接理解视频和文本中的所有复杂内容，容易导致混淆和幻觉。

Method: 我们提出将视频分解为一系列简单事件，通过可学习的Event Query进行分解，并以事件为单位、易于理解的方式理解复杂的视频内容。这基于观察，自然语言表达式通常将视频划分为与文本相关的不同段，每段代表一个复合事件中的独立事件。我们引入了EVIS，一个事件感知视频指令分割助手，它利用文本引导的事件查询将视频分割成简单事件，提取事件感知的视觉-文本特征，以实现对视频的分层理解。此外，我们提出了Object-Pixel-Hybrid Learning，它通过整合细粒度像素特征与先前的对象查询，使MLLMs能够在长期视频中跟踪目标。

Result: 在5个公开基准上的大量实验结果表明，EVIS在解决指视频分割任务方面表现出强大的性能。

Conclusion: EVIS在指视频分割任务中表现出色，通过将视频分解为简单事件，并使用事件感知的视觉-文本特征，实现了对视频的分层理解。
...
TL;DR: 提出一种基于RSFM的改进方法，实现无监督异常检测，提高地球观测效率


<details>
  <summary>Details</summary>
Motivation: 提高地球观测效率和减少成本

Method: 基于ResNet（RSFM）+FPN的无监督检测方法，通过检测连续轨道飞行之间的潜在空间中的细微语义变化来识别广泛的异常

Result: 无需训练的FPN架构和其内在先验，实现高效图像级生成和更高分辨率的映射，同时保证了跨不同地形和传感器的泛化性能

Conclusion: 该方法可以减少定制模型的需求，降低开发成本，并通过消除对特定训练和大量开发工作的需求，在提高任务效率和性能方面取得突破
...
TL;DR: PanoImager：一种提高稀疏全景图3D重建稳定性的方法。


<details>
  <summary>Details</summary>
Motivation: 从稀疏全景图中进行3D重建在旋转主导、弱视差运动下具有挑战性，SfM/SLAM初始化往往不可靠。

Method: PanoImager，一个无SfM框架，结合前馈姿态/深度先验、几何条件扩散视图完成和深度引导的3DGS优化。

Result: 实验表明，在极端稀疏性下，PanoImager的稳定性得到了提高，建议在SfM/SLAM无法初始化时，将其作为离线/后台组件用于地图细化。

Conclusion: PanoImager在提高3D重建的稳定性方面表现良好，尤其是在稀疏数据情况下。
...
TL;DR: CT-3GDINO是一种用于腹部CT图像中器官定位的轻量级3D检测器，在多尺度模型和3D IoU阈值上表现出色，为未来研究提供了基准。


<details>
  <summary>Details</summary>
Motivation: 提高腹部CT图像中器官定位的可靠性，为下游创伤分析提供空间先验。

Method: 提出了一种名为CT-3GDINO的轻量级3D检测器，该检测器采用Grounding-DINO风格的查询式架构，使用冻结的伪文本类别标记而非真实文本编码器进行固定器官定位。模型结合了Swin3D视觉骨干网络、双向特征增强、伪文本引导的查询选择和跨模态解码器，以预测肝脏、脾脏、左肾、右肾和肠道的归一化3D边界框。在193个匹配的RSNA/RATIC CT体积上训练和评估，使用分割得到的边界框。

Result: 最佳多尺度模型在从头开始训练的情况下，在0.1到0.7的3D IoU阈值上实现了0.5830的整体top-1类别平均精度（mAP），优于固定和可训练骨干网络的分类预训练变体（0.5570和0.4657 mAP）。在粗略定位方面表现良好，IoU为0.1时AP为0.9649，但在严格框对齐方面仍有局限性，IoU为0.7时AP为0.1552。

Conclusion: CT-3GDINO在伪文本条件下的3D器官定位方面建立了开源基准，并激励了未来在定位感知预训练、更丰富的多模态条件和以损伤检测为重点的工作。
...
TL;DR: SubdivAR is a neural mesh subdivision framework that achieves superior performance compared to existing methods, demonstrating strong robustness and improved detail synthesis capabilities.
...
Conclusion: SubdivAR is an effective neural mesh subdivision framework that achieves superior performance compared to existing methods, demonstrating strong robustness and improved detail synthesis capabilities.
...
TL;DR: TMP框架有效压缩了图像生成模型，提高了模型的可负担性。


<details>
  <summary>Details</summary>
Motivation: 现代图像生成模型迅速增长其大小以满足高保真图像合成。然而，它们逐渐变得不可负担，因为它们的巨大参数消耗和计算预算导致了对大量资源和GPU内存占用的大量需求。

Method: 提出了一种名为TMP的树状混合策略剪枝框架，该框架推广了常见的图像任务（T2I和TI2I）和架构（混合专家（MoE）和扩散变换器（DiT））。

Result: 通过实验，该框架成功将HunyuanImage 3.0从80B压缩到20B参数，参数减少了75%，同时牺牲了有限的生成质量。此外，我们还通过工程技能优化了剪枝后的20B HunyuanImage 3.0版本，使其能够在单个24GB 4090 GPU上运行。同时，该框架还能将Z-Image turbo从6B压缩到4B（减少33%），且质量下降可忽略。

Conclusion: TMP框架在保持有限生成质量的前提下，有效降低了模型参数和计算需求，提高了模型的可负担性。
...
TL;DR: 提出迭代自我改进代码簿，以提高自回归生成图像的安全性


<details>
  <summary>Details</summary>
Motivation: 提高自回归生成图像的安全性

Method: 迭代自我改进代码簿

Result: 安全增强的模型代码簿

Conclusion: 模型安全性通过迭代自我改进的代码簿得到提高
...
TL;DR: HarmVideoBench和BCR方法提升有害视频理解性能至84.4%。


<details>
  <summary>Details</summary>
Motivation: 针对现有有害视频基准测试的局限性，提出了一种新的基准测试HarmVideoBench，以评估模型对有害视频的深层理解。

Method: HarmVideoBench包含1379个视频和4137个多项选择题，涵盖三个层次：可观察的证据、剪辑内部意义和剪辑外推理。引入BCR方法，预测推理边界并动态检索上下文。

Result: BCR方法将基准测试的性能从61.7%提升到84.4%，达到最先进的水平。

Conclusion: HarmVideoBench和BCR方法能够有效提升有害视频理解性能。
...
TL;DR: LISA通过正则化方法提升视觉条件可控生成模型训练效率。


<details>
  <summary>Details</summary>
Motivation: 对现有双分支范式进行回顾，探讨侧分支的作用和训练效率。

Method: 提出LISA方法，通过钩取侧网络特征并投影到得分潜在空间，构建近似似然分数目标，并计算解码器输出与目标之间的距离作为正则化损失。

Result: 实验表明LISA可以加速训练收敛，提高合成结果，并使侧网络特征更加解耦，同时无额外训练成本和推理成本。

Conclusion: LISA是一种有效的正则化方法，可以显著提高视觉条件可控生成模型的训练效率和生成质量。
...
TL;DR: SatSplatDiff通过结合深度监督和生成细化，显著提升了卫星图像重建的视觉质量和几何精度。


<details>
  <summary>Details</summary>
Motivation: 提高卫星图像重建的视觉保真度，解决Gaussian Splatting在卫星图像重建中的局限性，如有限的视角和几何退化问题。

Method: 提出SatSplatDiff，结合单目深度监督和多尺度几何细化，应用阴影引导的生成细化，以减少几何退化并提高视觉保真度。

Result: 在IARPA2016和DFC2019数据集上实现最先进的性能，几何MAE降低18%，视觉保真度（FID-CLIP）提高28-45%，分辨率提升5倍，最小化幻觉和传感器一致性，实现无缝跨瓦片一致性和大规模重建的强可扩展性。

Conclusion: SatSplatDiff有效提高了卫星图像重建的视觉保真度和几何精度。
...
TL;DR: 我们开发了CORTEX，一个用于3D胸部CT的结构化推理基准，旨在提高医疗影像分析的准确性和可靠性。


<details>
  <summary>Details</summary>
Motivation: 在医疗影像中，多模态大型语言模型（MLLMs）的推理能力显示出巨大的潜力。然而，这种推理通常只根据其最终答案进行自由形式的文本判断，这使得其难以解释和验证，尤其是在3D放射学中，诊断应可追溯到扫描中的证据。现有的胸部CT问答数据集通过将专家放射学报告简化为只包含答案的配对，而忽略了将发现与结论联系起来的推理以及临床医生所依赖的患者病史，从而加剧了这一问题。因此，能够进行推理的3D胸部CT MLLMs仍然难以实现，因为既没有训练这些模型所需的有序监督，也没有验证其推理所需的协议。

Method: 我们介绍了CORTEX（临床组织推理和结构化解释），这是一个针对3D胸部CT的结构化推理基准。对于每个问题，CORTEX都将缺失的推理恢复为四个阶段的诊断轨迹，类似于放射科医生的流程：任务理解、视觉观察、诊断推理和答案综合。我们使用具有广泛医学和通用领域知识的前沿大型语言模型生成这些轨迹，然后使用结合自动化评分和专家放射科医生评审的阶段级评估协议对其进行过滤和验证。关键的是，推理结构和评估标准都是在与临床医生紧密合作的情况下设计的。建立在CT-RATE（一个大型、公开的胸部CT数据集，没有推理注释）之上，CORTEX包含76,177个经过验证的推理轨迹，涵盖了开放式问答、封闭式问答和报告生成，为构建和评估3D胸部CT的可靠推理模型提供了所需的有序监督和阶段级评估协议。我们的数据集和评估代码将在接受后公开。

Result: CORTEX是一个结构化推理基准，用于3D胸部CT。它包含76,177个经过验证的推理轨迹，为构建和评估可靠推理模型提供了所需的有序监督和评估协议。数据集和评估代码将在接受后公开。

Conclusion: CORTEX为3D胸部CT推理模型的发展提供了新的基准和工具，有助于提高医疗影像分析的准确性和可靠性。
...
TL;DR: 提出了一种基于HN的补丁描述符检索方法，显著提高了检索速度，同时保证了精确度和结果一致性。


<details>
  <summary>Details</summary>
Motivation: 为了提供一种在搜索中既保证精确度又提高速度的方法，同时确保结果的一致性。

Method: 提出了一种基于分层归一化（HN）的补丁描述符检索方法，通过将特征向量分为主要组件和次要组件，并仅在必要时进行完整评估。

Result: 在trevi和halfdome数据集上实现了13.7倍和12.7倍的速度提升，同时保持了较高的精确度。

Conclusion: 该方法在保证精确度的同时，显著提高了检索速度，并且结果的一致性也得到了保证。
...
TL;DR: 研究通过直接从人类反馈中学习，在3D几何优化方面取得了显著的成果。


<details>
  <summary>Details</summary>
Motivation: 改进3D生成中的强化学习，从人类反馈中学习，以优化几何结构。

Method: 使用无监督的3D感知生成模型，直接从学习到的奖励值对辐射场密度进行微调，无需外部提供的网格或形状先验。

Result: 在无条件3D感知人脸生成对抗网络（EG3D）上工作，奖励模型读取神经网络辐射场（NeRF）的连续3D密度场，并仅提供几何学习信号。通过密度一致性约束，在2D外观保持定性相似的同时，重塑几何形状。

Conclusion: 该研究通过直接从人类反馈中学习，在3D几何优化方面取得了显著的成果，证明了其方法的有效性。
...
TL;DR: SmellNet-V 和 See & Sniff：通过视觉-嗅觉学习实现气味识别和定位


<details>
  <summary>Details</summary>
Motivation: 现代多模态模型将视觉与语言、音频或触觉相结合，但嗅觉因缺乏配对视觉-嗅觉数据而 largely 未被探索。

Method: 提出 SmellNet-V，一个可扩展的视觉-嗅觉数据集，通过在语义类别内嗅觉身份对视觉转换 largely 不变这一洞察，将仅含嗅觉的样本与语义对齐的野外网络图像配对，将单模态嗅觉数据集转换为跨模态基准，无需昂贵的共收集。基于此数据集，提出 See & Sniff，一个自监督框架，通过密集局部对齐学习联合视觉-嗅觉表示，并自然地产生气味显著性图以进行气味源的定位。进一步引入像素级气味定位任务和评估基准。

Result: 该方法在仅从气味进行气味分类方面优于仅含气味的基线 7%，并推广到跨模态检索和气味定位，将视觉-嗅觉学习确立为多模态感知的新方向。

Conclusion: 视觉-嗅觉学习作为多模态感知的新方向，在气味分类、跨模态检索和气味定位方面取得了显著成果。
...
TL;DR: ViQ框架通过文本对齐预训练和特征离散化，在多模态任务中实现了高效的视觉量化表示。


<details>
  <summary>Details</summary>
Motivation: 为了实现更简单的多模态建模和更高效的训练，研究者们追求文本和视觉的统一表示。然而，将图像表示为离散信号的方式不可避免地引入了严重的信息损失。现有工作在离散表示中难以平衡低级细节和高级语义：以重建为导向的表示通常缺乏语义信息，而语义较强的特征通常会受到严重细节损失。

Method: ViQ框架，将量化学习分为两个阶段：文本对齐预训练和特征离散化。在文本对齐预训练中，增强视觉编码器的语义丰富监督，并使其能够处理原生分辨率的视觉输入。在离散化过程中，提出了一种近端表示学习策略，以逐步压缩特征空间，并引入了一种位置感知的头量化机制，以实现任意分辨率的灵活处理。

Result: 在多模态任务上进行了大量实验，结果表明，与具有连续和高度视觉特征的现有多模态视觉编码器相比，ViQ在保持高级重建精度的同时，实现了具有竞争力的性能。此外，使用视觉量化表示的多模态训练在很大程度上提高了效率，与不同的基础LLM和训练方案相比，速度提高了20%-70%。

Conclusion: ViQ框架在多模态任务中实现了具有竞争力的性能，同时提高了训练效率。
...
TL;DR: OctoSense是一个多传感器平台，其学习方法在多个视觉任务中表现出色。


<details>
  <summary>Details</summary>
Motivation: 提出OctoSense，一个开源的传感器平台，包含多种传感器，并建立相应的数据集。

Method: 使用多模态自监督学习，采用“后期融合”掩码自动编码器架构。

Result: 该架构在估计光流、深度、语义分割和自我运动（平移、旋转和转向角）等任务上优于现有的仅图像的基础模型，且在夜间或传感器数据退化时预测稳健。

Conclusion: OctoSense平台和所提出的方法在多模态自监督学习方面具有潜力。
...
TL;DR: 提出了一种新的动作条件模型，提高了高自由度控制中的视觉-时间现实感和动作跟随一致性。


<details>
  <summary>Details</summary>
Motivation: 现有的动作条件模型在高自由度场景下表现不佳，因为它们通常将整个动作序列压缩成单一表示，这在低自由度控制中表现良好，但在高自由度场景中可靠性降低。

Method: 提出了DexAC-WM模型，将动作条件视为结构化过程，通过动作标记保留维度语义，并通过局部优化和全局调制对动作信号进行定位和调节。同时，引入语义分支提供丰富的物体-场景先验，使世界模型能够捕获动态视觉细节并支持高自由度动作条件视频预测。

Result: 在EgoDex和EgoVerse上的实验表明，将语义分支与DexAC结合显著提高了FID、FVD和PCK，证明了在视觉-时间现实感和动作跟随一致性方面的改进。

Conclusion: 为了将世界模型扩展到高自由度控制，需要结构化动作建模和语义基础。
...
TL;DR: A new method using RoPE and 3D spatial structure encoding excels in single-image object motion.


<details>
  <summary>Details</summary>
Motivation: Moving an object in a single image requires geometry-consistent spatial rearrangement, including handling occlusions, revealing previously unseen regions, and maintaining coherent shadows and reflections. Existing approaches are not well suited to this setting and often fail to preserve such scene-level consistency.

Method: Introducing a geometry-aware object motion method that operates directly on the positional representations of diffusion transformers. Using rotary positional embeddings (RoPE) to define a structured spatial field that can be explicitly manipulated to induce controlled motion. Extending 2D RoPE into a depth-aware formulation that encodes 3D spatial structure. Training using synthetic data combined with a small set of real images via parameter-efficient fine-tuning.

Result: Preserves object identity under large spatial displacements, generates plausible content in newly revealed regions, and consistently updates scene-dependent effects such as shadows and illumination. State-of-the-art performance on standard object motion benchmarks.

Conclusion: A new geometry-aware object motion method using RoPE and 3D spatial structure encoding demonstrates excellent performance in handling object motion in a single image.
...
TL;DR: Proposes RayPE, a positional-encoding extension for video diffusion transformers, to improve camera controllability and video quality.


<details>
  <summary>Details</summary>
Motivation: Modern video diffusion transformers position their tokens through RoPE on the (u,v,t) axes, which describes the camera's sampling grid but not the 3D structure of the scene. The abstract aims to address this limitation.

Method: Proposes RayPE, a positional-encoding extension that injects per-token 6D Plucker coordinates into the queries and keys of self-attention. It utilizes the Plucker reciprocal product, similar to Transformer attention, and incorporates geometric relationships between camera rays.

Result: The proposed method improves camera controllability, cross-frame 3D consistency, and overall video quality on a four-dataset training mixture. The full module adds less than 0.1% parameters to a pretrained video DiT and improves attention scores by decomposing them into content, geometry, and cross-terms.

Conclusion: RayPE effectively enhances video diffusion transformers by incorporating geometric information and improving attention mechanisms.
...
TL;DR: PhysiFormer是一种用于3D物体运动的扩散Transformer，显著优于自回归基线，具有视图不变和几何感知的特点。


<details>
  <summary>Details</summary>
Motivation: 提出PhysiFormer，一种用于物理上合理的3D物体运动的扩散Transformer。

Method: 将物体表示为世界坐标中的3D网格，通过采样未来顶点轨迹来预测物体运动。

Result: PhysiFormer在轨迹精度、刚性保持和基于动量的物理一致性方面显著优于自回归基线。

Conclusion: PhysiFormer将坐标空间扩散定位为向视图不变、几何感知的世界建模迈出的有希望的步骤。
...
TL;DR: Efficiently mitigate diversity collapse in flow models with a self-guidance mechanism and manifold regularization.
...
Method: Introduce an efficient, training-free self-guidance mechanism; disperse internal features with feature self-guidance; introduce manifold regularization step; integrate seamlessly into pretrained flow models.

Result: Significant improvements in diversity while preserving fidelity across several conditional flow models.

Conclusion: The proposed method effectively mitigates diversity collapse in flow models without requiring additional reward models or significant inference-time overhead.
...
### [91] [DanceOPD: On-Policy Generative Field Distillation](https://arxiv.org/abs/2606.27377)
*Wei Zhou,Xiongwei Zhu,Zelin Xu,Bo Dong,Lixue Gong,Yongyuan Liang,Meng Chu,Leigang Qu,Lingdong Kong,Wei Liu,Tat-Seng Chua*

Main category: cs.CV

TL;DR: 提出DanceOPD，一种用于图像生成的多能力统一模型，提高目标能力并保持生成质量。


<details>
  <summary>Details</summary>
Motivation: 现代图像生成需要能够统一文本到图像（T2I）、局部编辑和全局编辑等不同能力，但这些能力往往难以自然对齐且相互冲突。

Method: 提出DanceOPD，一个基于策略的生成场蒸馏框架，用于流匹配模型。该框架将每个样本路由到一种能力域，查询一个低噪声的学生诱导状态，并使用简单的速度MSE目标进行训练。学生从自己的滚动状态查询的场中学习，以组合专家能力。

Result: 在T2I、编辑、现实感场吸收和CFG吸收等方面进行了综合实验，表明该方法提高了多能力组合，增强了目标能力，同时保持了锚点生成质量。

Conclusion: 该工作为流匹配模型中的生成场蒸馏提供了一条实用路线。
...
TL;DR: Proposes DnA for improved discriminability and performance in visual perception tasks.


<details>
  <summary>Details</summary>
Motivation: Standard softmax in multihead attention can produce noisy attention patterns that dilute relevant features and degrade performance.

Method: Proposing Denoising Attention (DnA) with positive and negative queries to identify relevant and irrelevant image features, and projecting interactions into two distinct subspaces.

Result: DnA achieves an absolute gain of 0.8% on ImageNet-1K, and improvements across multiple visual understanding tasks including video understanding with video transformers and video LLMs.

Conclusion: DnA improves discriminability and performance in visual perception tasks.
...
TL;DR: 提出VISE框架，有效解决视觉推理中的视觉欠条件化问题，提升视觉-语言理解任务性能。


<details>
  <summary>Details</summary>
Motivation: 为了解决现有自进化大型多模态模型在视觉推理方面的不足，提出了一种新的自进化框架。

Method: VISE（视觉不变自进化）框架，通过两种互补的不变性奖励直接正则化模型的视觉条件化策略：几何不变性奖励和语义不变性奖励。

Result: 在18个基准测试中，VISE在COCO和TextCaps上分别提高了16.85和19.66的CIDEr，减少了5.0个Chair-I点的物体幻觉，并在四个模型家族和规模上实现了泛化。

Conclusion: VISE框架有效地解决了视觉推理中的视觉欠条件化问题，提高了视觉-语言理解任务的性能。
...
TL;DR: 提出了一种无需人工标注或外部奖励的统一多模态模型训练方法，该方法能够自主提升模型能力。


<details>
  <summary>Details</summary>
Motivation: 提出了一种无需人工标注或外部奖励模型的统一大型多模态模型训练框架。

Method: 自演化训练框架，包含提案者、求解者和生成者三个角色。使用自衍生一致性信号进行训练，并引入求解器标记熵（STE）来稳定学习。针对图像生成，设计了多尺度内部评估方案。

Result: 在八项理解指标中，该方法优于相应的基本模型。在BAGEL上，它在MMMU上提高了+3.5%的绝对增益，并将图像生成性能从82%提高到85%。

Conclusion: 所提出的框架能够有效地提高统一大型多模态模型的能力，且无需外部监督。
...
TL;DR: HierBias 是一种高效的媒体偏见检测器，在 F1 和 MCC 指标上优于现有技术。


<details>
  <summary>Details</summary>
Motivation: 确保公平和平衡的信息传播，但现有的句子级方法独立地对每个句子进行分类，忽略了人类注释员自然利用的句子间上下文信号。

Method: 提出 HierBias，一种层次化上下文条件媒体偏见检测器，在偏见预测中正式建模文档上下文。引入上下文条件化偏见概率，并从理论上证明，当句子间的互信息非零时，利用文档上下文严格降低句子级分类的贝叶斯错误。多任务泛化界限进一步建立，联合训练二元偏见检测和细粒度偏见类型分类可以提高小标注语料库上的样本效率。在架构上，HierBias 将句子级 RoBERTa 编码器与跨句子 Transformer 聚合器和双输出头（用于二元检测和四类类型分类）配对。

Result: 在 BABE 和 BASIL 上评估，HierBias 实现 0.853 F1 和 0.723 MCC，超过最先进的偏见检测器，F1 增加 2.6%，MCC 增加 4.3%（McNemar 测试，p < 0.05）。消融实验确认每个理论组件独立且一致地做出贡献。

Conclusion: HierBias 在媒体偏见检测方面表现出色，优于现有方法。
...
TL;DR: 提出了一种新的基准来评估大型语言模型的可靠性，并通过实验验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 为了可靠地评估大型语言模型，应该区分支持的回答和不受支持的猜测，同时避免将两者与数据污染、提示个性和通用拒绝行为混淆。

Method: 提出了一个污染感知的、多区域基准，用于在冻结的构建时标签下测量从可回答的知识到预期拒绝的未知状态的转变。基准包含1200个项目，涵盖五个领域，明确的拒绝期望，污染风险元数据和双解析（官方严格解析器和标准化鲁棒性解析器）。在锁定答案或拒绝提示，答案控制，和提示模板变体下评估了FLAN-T5、Qwen2.5-Instruct和Llama-3-Instruct模型。

Result: 该基准不是通过通用的非答案行为解决的：FLAN基线在生产性拒绝方面仍然较弱，而更强的指令调整模型在从回答到拒绝的过渡中表现出选择性的但不完整的转变。Qwen2.5-3B-Instruct在整体可靠性方面达到最佳，但期望答案的区域仍然困难，校准仍然较差，良性项目的拒绝持续存在。提示和解析器鲁棒性分析保留了主要的排名和定性结论。因此，该基准为审计可回答性、拒绝、拒绝和污染作为LLM可靠性的不同但相互作用的维度提供了一个可重复的协议。

Conclusion: 该基准为审计LLM的可回答性、拒绝、拒绝和污染提供了一个可重复的协议。
...
TL;DR: 后训练数据域影响动物同情价值观保留，编码领域后训练可能更优


<details>
  <summary>Details</summary>
Motivation: 探讨后训练数据域对动物同情价值观保留的影响

Method: 使用监督微调（SFT）和强化学习（RL）进行实验，评估AHB 2.2和MORU基准测试

Result: 帮助性训练相对于编码训练降低了动物同情价值观，对道德推理能力也有负面影响，但在多语言MORU基准测试中效果不明显

Conclusion: 编码领域的后训练可能比帮助性后训练更好地保留中间训练的价值观，而不会损害一般推理能力
...
TL;DR: LLMs在解决静力学问题方面表现出色，但在涉及图表和多步推理的问题上存在局限性。


<details>
  <summary>Details</summary>
Motivation: 为了评估大型语言模型（LLMs）在解决静力学问题方面的能力，本研究采用了一种模型蒸馏过程。

Method: 通过蒸馏ChatGPT，提取了25个纯文本静力学问题，并进一步构建了两个额外的数据集，通过添加图表和修改数值。

Result: 实验结果表明，LLMs在纯文本静力学问题上的表现良好，但当引入图表并需要多步推理时，其准确性会下降。进一步分析表明，这种性能下降并非主要由于图像识别的限制，而是由于多步推理的困难和在连续的解决方案阶段中一致地应用提取的视觉信息。

Conclusion: LLMs在解决静力学问题方面表现出色，但在涉及图表和多步推理的问题上存在局限性。
...
TL;DR: ContextForge 通过上下文回收提高了长对话中 LLM 的性能。


<details>
  <summary>Details</summary>
Motivation: Large language models (LLMs) 在短语境推理方面表现出色，但在长对话范围内性能下降，因为存在语境窗口限制和无效的标记使用。

Method: 提出 ContextForge 系统，通过结构化查询生成、外部内存检索和受控合成来维护任务相关的信息。

Result: ContextForge 在一个15轮对话基准测试中表现出色，提高了一致性并减少了标记消耗，同时保持了可比的响应准确性。

Conclusion: 上下文回收为在长周期任务中扩展 LLM 功能提供了一种实用方法，无需更大的上下文窗口或模型重新训练。
...
TL;DR: 通过NVC约束提示，LLM可以减少对话升级并提高对话可靠性。


<details>
  <summary>Details</summary>
Motivation: 在涉及人际冲突、挫折和痛苦的情感化情况下，大型语言模型（LLMs）的使用越来越多。以往的安全研究主要集中在防止明显的伤害，如有害或违反政策的内容，而对可能无意中升级冲突的对话行为关注较少。

Method: 本研究将非暴力沟通（NVC）原则重新定义为以过程为导向的指南，以减少归因于责备，强调关注用户的情感体验，并在提供建议之前鼓励澄清。使用双代理模拟框架，在多个指令调整模型和用户抵抗级别上，我们展示了NVC约束提示可以持续减少对话升级并稳定与高度抵抗用户的互动。

Result: NVC约束提示可以持续减少对话升级并稳定与高度抵抗用户的互动，这表明简单的沟通约束可以显著提高LLM在冲突易发环境中的对话可靠性。

Conclusion: 简单的沟通约束可以显著提高LLM在冲突易发环境中的对话可靠性。
...
TL;DR: 本研究提出了一种名为NEST-V1的多模态框架，用于生成情感条件下的尼泊尔手语动画头像，为低资源环境下的情感感知手语翻译提供了技术基础。


<details>
  <summary>Details</summary>
Motivation: 低资源语言的手语交流系统整合情感表达的研究仍然很少，本研究旨在展示一种多模态框架，以验证从语音输入生成情感条件下的尼泊尔手语动画头像的可行性。

Method: 提出了一种名为NEST-V1的多模态框架，该框架使用共享声学编码器同时进行语音识别和情感分类。

Result: 在600个标记音频样本的数据集上，实现了81.1%的语音识别准确率和79.21%的情感识别准确率。

Conclusion: 本研究为低资源环境下的情感感知手语翻译奠定了技术基础，并为未来扩展到更大词汇量和更多样化的情感表达提供了可扩展的框架。
...
TL;DR: 大型语言模型在推理方面优于小型模型，其优势在于约束引导推理


<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型在推理基准测试中优于小型模型的原因

Method: 开发AdvCluster自动化框架，通过语义聚类和定量评估识别大型模型的优势

Result: 发现大型模型在识别约束、组织推理和验证中间步骤方面具有优势

Conclusion: 大型模型在推理方面具有系统性优势，特别是约束引导推理
...
TL;DR: Dynamic-dLLM是一种提升dLLM推理效率的框架，在保持性能的同时，显著提高了推理速度。


<details>
  <summary>Details</summary>
Motivation: dLLM在长序列和实时应用中的计算复杂度很高，现有的加速方法效果有限。

Method: 提出Dynamic-dLLM，包括动态缓存更新（DCU）和自适应并行解码（APD）两个组件。

Result: Dynamic-dLLM在LLaDA-8B-Instruct、LLaDA-1.5和Dream-v0-7B-Instruct等模型上实现了显著的推理速度提升。

Conclusion: Dynamic-dLLM显著提高了dLLM的推理速度，在保持性能的同时，平均速度提升了3倍以上。
...
TL;DR: 本文首次对MLLMs中的统一视觉-语言感知进行了系统性调查，为AGI的发展提供了重要的理论框架和实践指导。


<details>
  <summary>Details</summary>
Motivation: 为了填补现有文献中对感知的碎片化、分而治之的研究方式的空白，本文提出了一个对统一视觉-语言感知的系统性调查。

Method: 对MLLMs中统一视觉-语言感知进行系统调查，包括（1）将MLLM感知形式化为类似于人类天生的感知的内在、统一的视觉-语言能力，（2）引入一个五阶段分类法，追踪MLLM感知的范式演变，并调查每个阶段的方法和里程碑，（3）确定开放挑战并概述通向真正通用、统一的多模态智能的研究方向。

Result: 本研究提供了一个基础性的理解和实际的可操作路线图，以促进人工智能（AGI）道路上的人工智能进一步创新。

Conclusion: 本文提出了MLLMs中统一视觉-语言感知的第一个系统性调查，填补了现有文献的空白，并为AGI的道路提供了重要的参考和指导。
...
TL;DR: 小型语言模型（SLMs）可以增强专家审稿人的工作，加快大规模文献综述的速度。


<details>
  <summary>Details</summary>
Motivation: 为了支持可扩展的综述实践，研究评估了小型语言模型（SLMs；<1.5B参数）在大型spHRI系统综述中辅助标题和摘要筛选的程度。

Method: 使用小型语言模型（SLMs）对spHRI系统综述的标题和摘要进行筛选，并与人类审稿人的表现进行比较。

Result: 虽然SLMs没有达到人类审稿人的表现，但它们在本地运行并能够以更快的速度筛选论文。SLM组合识别出39篇审稿人遗漏的论文，占最终相关数据集的10.29%。这些结果表明，SLMs可以补充，而不是取代，专家审稿人，并使大规模文献综述变得可访问和可持续。

Conclusion: SLMs可以增强，而不是取代，专家审稿人，并使大规模文献综述变得可访问和可持续。
...
TL;DR: ConflictScore是一个评估模型对矛盾性证据处理能力的指标。
...
Method: 提出ConflictScore，将回答分解为原子性主张，对每个主张与每个基础文档进行标记，然后汇总这些标记为两个互补的度量：ConflictScore-Count（CS-C）和ConflictScore-Ratio（CS-R）。

Result: 实验表明，ConflictScore能够有效地检测出跨领域的过度自信的主张，并可以作为纠正反馈机制，提高TruthfulQA上的真实性。

Conclusion: ConflictScore是一个有效的指标，可以用于评估模型对矛盾性证据的识别和处理能力。
...
TL;DR: AnySimLite：一种轻量级模型，用于解决多个语音相邻分类任务，性能优异且内存占用低。
...
Result: AnySimLite在多个SA分类任务上实现了最先进（SOTA）或SOTA竞争性能，在少样本设置下保持低内存占用。即使在最坏的情况下，性能下降也低于7%，而使用的模型大小仅为SOTA qLLaMA_LoRA-7B基线的大约1/250。
...
TL;DR: SOLAR improves multilingual reasoning accuracy by aligning soft-token representations across languages.


<details>
  <summary>Details</summary>
Motivation: Multilingual large language models often produce inconsistent reasoning and answers for semantically equivalent prompts in different languages.

Method: Proposing SOLAR, an auxiliary objective for supervised fine-tuning that aligns soft-token representations across languages using English as a pivot.

Result: Improves accuracy by up to +17.7 points over the base model and +3.8 over standard supervised fine-tuning, with the largest gains on low-resource languages. Strengthens final-layer cross-lingual similarity and reduces language-cluster separability.

Conclusion: Aligning soft-token representations helps preserve shared semantic structure during multilingual reasoning.
...
TL;DR: 本文提出了一种基于FE去敏感化和上下文增强的模型，有效提高了科学论文中问题和方法的提取效果。
...
Method: 提出公式化表达（FE）去敏感化、基于FE去敏感化的数据增强器、上下文增强的Transformer等。

Result: 在两个科学论文数据集上，与基线模型相比，所提出的模型在宏观F1分数上分别提高了3.71%和2.67%。

Conclusion: 所提出的模型能够有效提高科学论文中问题和方法的提取效果。
...
TL;DR: 本研究利用脑电图（EEG）和眼动信号进行基于微博的自动关键词提取（AKE），结果表明认知信号有效提高了AKE性能。


<details>
  <summary>Details</summary>
Motivation: 微博平台生成大量短、嘈杂和分散的用户内容，使自动关键词提取（AKE）成为一项重要但具有挑战性的任务。

Method: 我们研究了是否可以使用脑电图（EEG）信号来补充眼动信号以进行AKE。我们使用ZuCo认知语言处理语料库，选择了8个EEG特征和17个眼动特征，并将它们纳入基于微博的AKE模型中。为了减少模型结构对认知信号的潜在扭曲，我们将这些特征注入到软注意力层的输入和自注意力层的查询向量中。然后，我们在AKE模型中评估了不同认知信号的组合。

Result: 结果显示，阅读过程中产生的认知信号可以一致地提高AKE性能，无论特征组合还是模型架构。EEG特征带来了最大的收益，而结合EEG和眼动特征则产生了介于两种个别信号类型之间的性能，表明部分互补性但也存在可能的冗余或噪声。
...
TL;DR: TempoWave通过多小波、多尺度系数嵌入，提高了LLM在时间序列预测中的准确性和可靠性。


<details>
  <summary>Details</summary>
Motivation: LLMs在上下文感知的时间序列预测中很有吸引力，但它们与连续数值值的离散、语言导向的标记化和嵌入接口不匹配，这往往损害数值排序和预测可靠性。

Method: 提出了一种名为TempoWave的插件式时间波形数字接口，将每个标量观察值映射到由多小波、多尺度系数构成的位嵌入。

Result: TempoWave在五个上下文丰富的预测基准测试中显示出，与标准数字标记化和替代嵌入接口相比，它始终如一地提高了基于LLM的预测器，达到了新的最先进水平。

Conclusion: TempoWave解决了LLMs在数值接口方面的瓶颈，并通过多分辨率嵌入更好地将LLMs的上下文推理与精确预测相结合。
...
TL;DR: TwoTower模型通过分离上下文表示和降噪功能，提高了扩散语言模型的生成效率。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型在并行和迭代生成方面具有潜力，但现有方法使用单个网络进行上下文表示和迭代降噪，限制了其能力。

Method: 提出TwoTower，一个块状自回归扩散模型，将这两个角色分离成两个塔：一个冻结的AR上下文塔，因果处理干净的标记；一个可训练的扩散降噪塔，通过双向块注意力对噪声块进行细化。

Result: Nemotron-TwoTower在质量上保留了98.7%的自回归基线，同时提供了2.42倍更高的时钟生成吞吐量。

Conclusion: TwoTower模型在保证生成质量的同时，提高了生成效率。
...
TL;DR: MemStrata是一种新的检索记忆系统，通过维护时间有效性，显著提高了RAG在知识演变情况下的性能。


<details>
  <summary>Details</summary>
Motivation: 检索增强生成（RAG）为代理提供访问累积知识的能力，但没有时间模型。当事实发生变化时（例如，函数重命名或API重构），RAG检索到过时和当前值具有几乎相同的嵌入相似度。代理随后要么拒绝服务，要么提供被取代的事实。我们表明这是一个结构性问题：在一个校准的数据集上，余弦相似度将矛盾的与重复的事实区分开来，AUROC为0.59（接近随机），因为矛盾的事实通常比重新表述的重复事实与原始事实的嵌入相似度更高。

Method: 我们提出了MemStrata，这是一种维护时间有效性的检索记忆。它像RAG一样存储事实，保留静态召回，但当事实的值被矛盾时，一个确定性的（主题，关系，对象）取代规则在双时间账本中淘汰过时的值 - 没有相似度阈值，没有LLM调用。

Result: 在六个本地运行的基准测试中，MemStrata在静态知识上与RAG持平，在知识演变上达到0.95-1.00的准确率（RAG达到0.20-0.47）。主要结果是过时事实错误率：当需要回答时，RAG 15-40%的时间提供被取代的值；MemStrata将这一比例降低到~0%，这是RAG无法避免的失败类别。MemStrata在检索延迟（~2.1秒）方面实现了这一点，而基线LLM重新排序的延迟为~16-18秒。我们发布了工具包、数据集和无标记评估协议，以在知识演变下对记忆进行评估。

Conclusion: MemStrata通过维护时间有效性的检索记忆，显著提高了RAG在知识演变情况下的性能，减少了错误率，并具有更低的检索延迟。
...
TL;DR: 纵向文本分析揭示企业叙事披露的复杂变化。


<details>
  <summary>Details</summary>
Motivation: 评估企业叙事披露的定性变化具有挑战性，因为叙事文本是多维度的，一个文本维度的改进通常伴随着其他维度的变化。

Method: 结合日语NLP度量提取、配对测试、位移函数分析和跨度量相关分析，提出纵向文本分析方法。

Result: 分析日本2019年披露改革，对19,770个企业年度观察进行10年（2015财年-2024财年）研究。发现披露数量大幅增加，但可读性下降；整体信息结构改善，但具体描述质量停滞，市场各细分领域适应性差异明显。

Conclusion: 纵向文本分析能够揭示传统单一指标方法掩盖的披露模式复杂变化。
...
TL;DR: 研究发现，在狭窄任务下训练的AI模型可能无法检测到潜在的安全风险，即使这些风险在实际世界中可能导致危害。


<details>
  <summary>Details</summary>
Motivation: 评估AI安全性的方法存在缺陷，因为模型可能无法检测到未被指定的潜在危险。

Method: 研究通过在狭窄任务上对语言或视觉模型进行条件化，来观察模型对共存的安全关键信号的抑制情况。

Result: 在放射学和驾驶文本场景以及胸部X光视觉任务中，所有测试的模型都表现出抑制现象，并且这种现象与模型规模无关，持续存在于推理模型中，并且模型家族之间的差异大于规模差异。

Conclusion: 提出了‘注意力间隙’这一概念，认为这导致了测量基准安全与实际世界安全之间的脱节。
...
TL;DR: 本文提出了一种名为DiARC的新方法，用于解决ARC-like问题，并在多个基准测试中取得了显著的性能提升。


<details>
  <summary>Details</summary>
Motivation: 许多基于大型语言模型的方法尝试将ARC转化为基于文本的推理任务，但基于开源模型的方法通常结果不令人满意，而依赖封闭源模型的方法成本过高。

Method: 提出了一种名为DiARC的方法，该方法通过构建偏好对来使模型能够区分它们，并提出了三种构建负样本的方法，包括输出级别的视觉转换、DSL级别的规则反转和特定于任务的规则编辑。

Result: 实验结果表明，DiARC在多个ARC-like基准测试中一致地提高了性能。

Conclusion: 解决ARC-like问题不仅需要正样本监督，还需要区分负样本的能力来提高模型推理。
...
TL;DR: KIRP通过结合外部知识与实体重组，有效提升了零样本推文立场检测的性能。


<details>
  <summary>Details</summary>
Motivation: 解决零样本推文立场检测中的挑战，如文本的上下文稀疏性和建立隐含目标与文本内容的相关性。

Method: 构建一个四类、多主题的日语推文数据集，提出KIRP框架，结合外部知识与实体重组进行数据增强，使用提示链进行推理。同时采用立场感知对比学习捕获判别性特征，设计三层迭代原型网络进行精细分类。

Result: KIRP在SemEval-2016、WT-WT和KIRP-D数据集上取得最先进的性能，F1分数分别为84.05%（三类）、84.99%（四类）和79.18%（四类）。

Conclusion: KIRP在零样本推文立场检测中表现出色，为相关领域提供了有效的解决方案。
...
TL;DR: 针对资源稀缺语言，TTS模型通过适配器显著提升质量，但对资源丰富语言效果有限。


<details>
  <summary>Details</summary>
Motivation: 研究在训练数据稀缺的语言上，大型预训练文本到语音（TTS）模型的质量差距问题。

Method: 使用VoxCPM2模型，一个2.4B参数、无需分词的TTS模型，结合MiniCPM-4语言模型骨干和流匹配扩散解码器。构建一个共享的、语言标记的语料库，约26小时，并使用单个低秩适应（LoRA）适配器对VoxCPM2进行适应，该适配器在两种语言上同时训练，并添加到语言模型和解码器中。

Result: Khmer的MOS评分从3.85提高到4.23，最佳适配器（64秩）带来了高度显著的增长（配对Wilcoxon检验，p<0.001），而只训练了0.19到3.03%的参数。然而，对于已经处理得很好的Korean语言，该适配器没有带来任何改进，甚至在高秩时降低了质量。因此，适应主要帮助基础模型真正薄弱的地方。

Conclusion: 针对训练数据稀缺的语言，通过适配器可以显著提高TTS模型的质量，但对于已经处理得很好的语言，则可能不会带来改善。
...
TL;DR: CAT-Q：一种简单有效的后训练量化方案，用于压缩和加速LLMs。


<details>
  <summary>Details</summary>
Motivation: 为了压缩和加速大型语言模型（LLMs），本文提出了CAT-Q，一种高效且准确的ternary量化方法。

Method: CAT-Q采用可学习调制（LM）和软化ternarization（ST）两种关键组件。LM通过组合可学习因子来调节预训练高精度权重和ternary阈值的分布，ST引入可微过渡函数以引导ternarization过程稳定收敛。

Result: CAT-Q在1.7B到8B参数的预训练LLMs上实现了高效的ternary量化，性能优于BitNet 1.58-bit v1和v2家族。对于14B到235B参数的更大预训练LLMs，CAT-Q可以在8到60小时内将其量化为领先的ternary模型。

Conclusion: CAT-Q是一种简单而有效的后训练量化方案，适用于不同架构和规模的LLMs，在压缩和加速LLMs方面具有显著优势。
...
TL;DR: SocialPersona是一个评估多模态语言模型能否从社交媒体时间线中恢复显式偏好的基准，有助于推进个性化助手的发展。


<details>
  <summary>Details</summary>
Motivation: 为了更全面地个性化语言模型助手，需要一种更高级的能力——从用户自然留下的多模态痕迹中推断用户关心的事情。

Method: 提出SocialPersona，一个用于评估多模态大型语言模型（MLLM）是否能够从纵向社交媒体时间线中恢复显式偏好并用于对话的基准。SocialPersona包含来自171个日常、非促销社交媒体用户的纵向时间线，包括文本、图像、时间戳和2,597个人类验证的偏好标签，涵盖七个兴趣领域。支持两个任务：从多模态上下文中构建结构化用户配置文件和生成与推断配置文件一致的响应。

Result: 实验表明，模型可以识别广泛的兴趣领域，但在细粒度和最近兴趣上的表现下降，并且当必须使用推断配置文件来个性化对话时，性能进一步下降。这些结果表明，强大的跨模态、长周期用户建模仍然是一个关键挑战，SocialPersona可以帮助衡量和推进助手推断和利用显式偏好的进展。

Conclusion: SocialPersona有助于评估和推进能够推断和利用显式偏好的助手的发展。
...
TL;DR: ConvMemory v3通过有效性上下文层和双证据门控器，提高了对话记忆检索的准确性和有效性。


<details>
  <summary>Details</summary>
Motivation: 优化对话记忆检索的相关性，同时解决检索到的记忆可能过时的问题。

Method: 引入有效性上下文层，通过目标条件关系验证检测和呈现更新证据。

Result: 在合成多跳有效性基准上，门控器达到90.12%的准确率；在真实数据反馈循环中，验证器在无目标标签的情况下达到98.8%的准确率。

Conclusion: ConvMemory v3通过有效性上下文层和双证据门控器，提高了对话记忆检索的准确性和有效性。
...
TL;DR: 本文对多媒体事件提取中的评估陷阱进行了系统分析，强调了评估标准的重要性。
...
Method: 通过一系列受控实验，在严格的评估框架下，分析了多媒体事件提取中的评估陷阱，并确定了三个主要问题来源：不一致的数据处理、不一致的任务假设和过于宽松的评估设置。

Result: 研究发现，微小的评估选择可能导致性能差异很大，并导致对模型跨模态接地现实世界事件能力的过度估计。

Conclusion: 强调了需要可比的评估标准，并鼓励向更严格的评估转变。
...
TL;DR: OPID通过从在线策略轨迹中提取技能监督，提高了语言代理的性能和效率。


<details>
  <summary>Details</summary>
Motivation: 为了解决基于结果的强化学习在语言代理中提供的稳定优化骨干，但其稀疏轨迹级奖励在中间决策方面提供的指导有限的问题。

Method: 提出了一种名为OPID的框架，该框架直接从完成的在线策略轨迹中提取技能监督。OPID将轨迹事后分析表示为分层技能：剧集级技能捕获全局工作流程或避免失败规则，而步骤级技能捕获关键时间步的局部决策知识。

Result: 实验表明，OPID在ALFWorld、WebShop和基于搜索的QA上通常比仅基于结果的强化学习和现有的技能蒸馏基线提高了代理性能、样本效率和鲁棒性。
...
TL;DR: 计算语料库研究证实了佛教和湿婆在孟加拉的词汇融合


<details>
  <summary>Details</summary>
Motivation: 研究孟加拉语和梵文宗教文献中词汇关系的演变

Method: 计算语料库研究，TF-IDF字符n-gram向量化和余弦相似性分析

Result: 证实了佛教密宗词汇在帕拉寺庙倒塌后融入孟加拉语湿婆传统，提供了多传统融合的量化证据

Conclusion: 首次对历史上争论的佛教-湿婆在孟加拉的融合提供了定量证据
...
TL;DR: SpeechLLMs在IWSLT 2026指令遵循任务中取得良好成绩，但长形式生成存在幻觉问题。
...
Method: 在短形式任务中，使用MCIF评估；在长形式任务中，探索三种语音分割方法，并引入HIFS评分来考虑不稳定的长形式生成。

Result: 短形式任务中，MCIF SIFS得分为2.0708；长形式任务中，固定30秒分割方法获得最高HIFS得分2.0663。分析显示，幻觉主要表现为重复插入，对ASR和SSUM有显著影响，但长形式扩展后仍保留了短形式能力。

Conclusion: SpeechLLMs在指令遵循任务中表现出色，但长形式生成存在幻觉问题，需要进一步改进。
...
TL;DR: 提出了一种新的剪枝框架，在工业物联网边缘设备上部署大型语言模型时，实现了高压缩比和较高的准确率。


<details>
  <summary>Details</summary>
Motivation: 为了在工业物联网边缘设备上部署大型语言模型（LLM），需要极端压缩，但现有的结构化剪枝方法在高压缩比下会因一次性重要性估计而崩溃，并且它们的跨架构行为仍然不可预测。

Method: 提出了一种级联多粒度剪枝框架，按粗到细的顺序删除层、注意力头和前馈通道，在阶段之间使用轻量级低秩恢复来重新估计组件重要性。

Result: 在88M到6.25B参数模型的承载故障诊断中，该框架将MHA+GELU架构的可实现压缩扩展到13.8倍，准确率达到83.82%（比最强的基线高3.70个百分点），而在违反SIA的GQA+SwiGLU架构上暴露出约74个百分点的准确率下降。在工业回转轴承故障诊断平台上部署NVIDIA DGX Spark，压缩模型将推理延迟降低最多67.2%，峰值内存降低62.5%，证明了其在IIoT边缘推理中的可行性。

Conclusion: 该框架在工业物联网边缘设备上部署大型语言模型时，实现了高压缩比和较高的准确率，证明了其在实际应用中的可行性。
...
TL;DR: 本文提出了一种基于信息论的KV缓存压缩方法，显著提升了长期推理中的性能。


<details>
  <summary>Details</summary>
Motivation: 现有KV缓存压缩方法主要依赖于注意力权重来估计标记的重要性，但忽略了与预测不确定性和标记信息论相关的互补信息论信号。

Method: 提出了一种名为InfoKV的熵感知KV缓存压缩框架，该框架结合了信息论信号，包括标记级别的预测不确定性和层内表示演变，并在推理过程中将结果熵分数与注意力分数相结合。

Result: 在Llama-3.1、Llama-3.2和DeepSeek-R1的长期推理基准测试中，InfoKV在长预填充和解码场景中均优于现有的基于注意力的KV压缩方法。

Conclusion: InfoKV框架通过考虑预测不确定性和信息论信号，在长期推理任务中提高了KV缓存的压缩效果。
...
TL;DR: 研究表明，语言模型可以有效地预测神经活动，并有助于理解自然语言和文本。


<details>
  <summary>Details</summary>
Motivation: 分析语言模型表示在自然语言刺激中的结构和高度注释，以及在理解过程中的信息性神经预测作用。

Method: 分析了来自Brain Treebank、MEG-MASC和Podcast ECoG的锁定衍生数据，使用八个冻结语言模型、块编码模型和匹配的时间、混杂和表示能力控制。

Result: 在源级摘要中，广泛存在积极的保留预测和超过低级基线的增益。大多数可评估的源行中，模型端特征消融改变了预测分数。大脑衍生、时间相关、声学信号和植入信号控制确认了分析管道的组件级敏感性。

Conclusion: 语言模型衍生量可以注释自然语言和文本理解过程中的神经活动。参与者级别的匹配控制优势是局部化的，而不是统一的；反应配置文件和特征特异性对比限制了表示或计算解释；完整的共索引集成解释将需要未来的联合索引覆盖。分析将语言模型特征识别为有用的神经预测因子，并将预测的有用性从共享神经组织或语言处理计算的声明中分离出来。
...
TL;DR: GAVEL addresses VLM misalignment through verification, explanation, and localization, with promising results from supervised baselines.
...
Conclusion: GAVEL provides a comprehensive approach to address the misalignment problem in VLMs, demonstrating the potential of learnable supervision for these abilities.
...
TL;DR: 提出了一种基于术语的框架，用于从异构语料库中诱导层次分类法，在多来源基准测试中表现优于基线。
...
Method: 提出了一种基于术语的框架，从异构语料库中诱导层次分类法。使用自动术语提取将文档映射到共享表示空间，并基于这些表示构建可解释的层次结构，整合领域先验与数据驱动聚类。

Result: 在超过一百万份文档的英语和德语多来源基准测试中，该方法在跨来源一致性和层次质量方面优于基于文本和摘要的基线。案例研究进一步证明了其在德国区域创新分析中的实用性。

Conclusion: 提出的方法在跨来源一致性和层次质量方面优于基线，并具有实际应用价值。
...
TL;DR: 语音模型在多语言环境下的安全性和公平性存在漏洞，且在非英语语言和语音输入条件下更加严重


<details>
  <summary>Details</summary>
Motivation: 研究语音模型在不同语言和自然条件下安全性和公平性的不足

Method: 调查最先进的语音模型发布的安全报告实践，并引入RedVox进行多语言安全性和公平性基准测试

Result: 发现即使是非对抗条件下，模型也存在漏洞，在非英语语言中恶化，并且当请求来自语音输入时放大。此外，收集语音数据时面临独特的人际和隐私挑战

Conclusion: 语音模型在不同语言和自然条件下的安全性和公平性仍需进一步研究
...
TL;DR: 研究发现，框架会影响语言模型的内部表示和行为，这对于评估心理健康领域对话AI系统的可靠性至关重要


<details>
  <summary>Details</summary>
Motivation: 探讨框架相关变化如何反映在对齐语言模型的内部表示中

Method: 使用受控匹配提示，跨越多个指令调整的模型家族，在多个上下文框架条件下进行研究

Result: 框架系统性地改变了解释性响应倾向，行为关联信息在变压器深度中保持可解码，解码强度存在架构依赖性变化，框架关联的表示方向可以部分调节下游行为结果

Conclusion: 上下文变化的鲁棒性可能在评估以心理健康为导向的交互中部署的对话AI系统的一致性和可靠性时是一个重要的考虑因素
...
TL;DR: ReaORE 框架通过关系推理改进了 OpenRE 任务的表现。


<details>
  <summary>Details</summary>
Motivation: Open Relation Extraction (OpenRE) 面临的挑战：可靠地泛化到未见过的关系类型。

Method: 提出 Reasoning-guided progressive OpenRE (ReaORE) 框架，通过粗到细的关系推理进行关系提取。

Result: 在两个广泛使用的 OpenRE 数据集上，ReaORE 的表现优于现有基线。

Conclusion: ReaORE 框架在 OpenRE 任务中取得了较好的效果。
...
TL;DR: 研究证实了情感向量在大型语言模型中的普遍性，并揭示了其在模型深度中的分布差异。


<details>
  <summary>Details</summary>
Motivation: 研究Claude Sonnet 4.5中的情感向量，测试其在两个大型语言模型中的普遍性。

Method: 在两个大型语言模型中提取情感对比向量，分析情感表示在模型深度中的变化。

Result: 在两个模型中均恢复了情感几何，发现情感表示在模型深度中的不同模式。

Conclusion: 情感表示在大型语言模型中具有普遍性，但其在模型深度中的分布模式存在差异。
...
TL;DR: MinGram是Unigram的一种改进，提高了压缩效率和下游任务性能


<details>
  <summary>Details</summary>
Motivation: Unigram标记器的训练比较重且复杂，提出MinGram（极简Unigram）来简化训练

Method: 使用BPE衍生的种子词汇、最小令牌路径的硬EM和单级评分剪枝步骤

Result: 在六个语言中，MinGram比BPE和标准Unigram的压缩更好，并且压缩性更强的变体在保留更高形态对齐的同时与最强的令牌计数压缩器相当。在控制下游语言模型训练中，Unigram系列标记器，其中MinGram表现最佳，在每字节比特数方面始终优于BPE

Conclusion: MinGram是一种比BPE和标准Unigram更好的压缩技术，并且在下游任务中表现出色
...
TL;DR: Psy-CoT和RAPO有效提升了角色扮演代理的性能。


<details>
  <summary>Details</summary>
Motivation: Building general-purpose role-playing agents that can accurately represent any character from a natural-language profile remains challenging due to the limitations of the current dominant paradigm of supervised fine-tuning.

Method: Propose Psy-CoT, a psychology-grounded chain-of-thought framework, which decomposes pre-response reasoning into three role-specific steps: Interaction Perception, Psychological Empathy, and Logical Construction. Also, propose Role-Aware Policy Optimization (RAPO) to address the issue of gradient signals being identical for generic and genuinely role-specific phrases.
...
Conclusion: Psy-CoT and RAPO effectively improve the performance of role-playing agents.
...
TL;DR: NuclearQAv2是一个用于评估大型语言模型在核工程知识方面能力的基准，它揭示了模型在定量推理和概念理解方面的挑战。


<details>
  <summary>Details</summary>
Motivation: 在高度技术领域中确保大型语言模型的可靠性仍然是一个重大挑战。

Method: 混合流程结合专家编写的问答、现有数据集和来自特定技术语料库的LLM辅助生成。利用结构化提示进行自动问答生成和评估。

Result: 使用NuclearQAv2评估了多种大型语言模型，发现不同任务类型之间存在显著的性能差异。

Conclusion: NuclearQAv2是用于评估大型语言模型在核工程知识方面的能力的可扩展基准。虽然模型在事实性问题上的表现良好，但在定量推理和概念理解方面仍有较大挑战。
...
TL;DR: 提出了一种新的法律结果预测方法，在基准测试中取得了最先进的结果，同时减少了参数数量。


<details>
  <summary>Details</summary>
Motivation: 预测法律结果需要区分客观案件事实和裁决背景。

Method: 提出了一种名为“法官感知门控多任务学习”的架构，该架构明确地模拟了这种区别。

Result: 在13,937个英国就业法庭裁决上评估了该方法。与基于Gemma-4 26B-A4B骨干的监督微调（SFT）相比，该方法在基准测试中定义了新的最先进水平，同时所需的可训练参数数量比生成SFT基线少一个数量级。

Conclusion: 对于法律结果的身份条件分类，条件接口的选择决定了规模：可微结构组合比基于提示的组合在更大的骨干上产生更准确、参数效率更高的模型。
...
TL;DR: LLMs在真实谜语上的表现可能不是基于灵活推理，而是基于记忆检索。


<details>
  <summary>Details</summary>
Motivation: 探讨大型语言模型（LLMs）在认知任务上的准确性是否源于模式匹配还是灵活推理。

Method: 提出谜语范式，通过模拟谜语并改变其答案，以测试LLMs在需要灵活推理的任务上的表现。

Result: 人类和LLMs在谜语范式上的表现相反。LLMs在真实谜语上的准确率远高于谜语谜语（84.9% vs. 50.7%），而人类则相反（50.5% vs. 80.5%）。错误分析表明，90.8%的LLMs在谜语谜语上的错误（表现下降的条件）是由于不恰当地使用创新推理，而只有57.6%的人类在真实谜语上的错误是由于过度扩展字面推理。

Conclusion: LLMs在真实谜语上的强表现可能反映的是记忆检索而不是灵活策略选择。
...
TL;DR: 通过增强时间感知特征，可以减少LLMs在预测任务中的前瞻性偏差，同时保持其推理能力。


<details>
  <summary>Details</summary>
Motivation: 为了成功预测，需要识别历史和未来世界状态之间的模式，这些模式可以推广到未来的观察结果。

Method: 应用LLMs（大型语言模型）到各种预测任务中，并使用稀疏自编码器检查其内部状态，以了解它们是否似乎依赖于特定时间的知识片段而不是可推广的模式。

Result: 分析确定了与时间感知推理和前瞻性偏差推理相关的特征。然后，将这些LLMs应用于完全不同的领域，并干预这些特征。我们发现，增强时间感知特征可以显著减少预测提示中的前瞻性偏差，同时保持一般的推理性能。相比之下，引导候选前瞻性偏差特征没有产生效果。这些结果表明，可解释的时间特征可以用来因果地将LLMs推向更基于历史的推理。

Conclusion: 可解释的时间特征可以用来因果地将LLMs推向更基于历史的推理。
...
TL;DR: 提出了一种新的处理花园路径句子的方法，该方法通过句法信念更新，比现有方法更有效地拟合人类阅读时间数据。


<details>
  <summary>Details</summary>
Motivation: 分析人类处理花园路径句子的困难，以及现有方法（如词汇惊讶度）的不足。

Method: 提出一种新的方法，即通过预测句法树的概率分布（句法信念）并在每个新词后更新该分布来处理花园路径句子。使用广义Rényi距离来衡量更新幅度。

Result: 该方法比现有方法更好地拟合人类阅读花园路径句子的时间数据，为心理语言学中非词汇替代方法的研究提供了新的方向。

Conclusion: 句法信念更新是一种新的、更有效的方法，可以更好地处理花园路径句子，并为心理语言学的研究提供了新的视角。
...
TL;DR: 本研究提出了基于意图的安全分类器，并验证了其在各种训练模式下的有效性，为构建更安全、更可靠的模型提供了新思路。


<details>
  <summary>Details</summary>
Motivation: 为了确保安全分类器能够更好地理解用户意图，我们提出应该将用户意图建模为提示和最终标签之间的显式信号。

Method: 我们引入了名为AIMS的人标注数据集，包含1,724个难以处理的安全提示，每个提示都与意图描述和伤害标签配对。我们使用AIMS来评估意图感知训练，包括监督微调、偏好学习、推理蒸馏和强化学习。

Result: 在AIMS数据集上，基于意图的微调（DPO）在意图错误方面优于监督微调（SFT），并且基于意图的蒸馏在大多数教师-学生对中优于仅推理的蒸馏。直接奖励意图忠诚度（GRPO）在五个外部安全基准测试中取得了最强的平均性能，而我们的意图感知模型形成了推理延迟-F1帕累托前沿。

Conclusion: 忠实建模意图是一种紧凑、高质量的监督信号，有助于构建更鲁棒的安全分类器。
...
TL;DR: CARVE is a new recurrent model that improves on existing methods by erasing only on the key axis and reusing the recurrent output tensor.


<details>
  <summary>Details</summary>
Motivation: Recurrent models need to forget to remember, but the current state-of-the-art methods do not consider the stored information when deciding what to erase. This leads to three defects in the GDN-2 architecture.

Method: Introduce CARVE, a content-aware recurrent model that erases only on the key axis, reuses the recurrent output tensor as a content signal, and replaces the per-value write-gate projection with a single scalar per head.

Result: CARVE achieves better performance than GDN-2 on various benchmarks, including WikiText perplexity, common-sense reasoning, and RULER retrieval. It also has lower memory usage and fewer parameters.

Conclusion: CARVE is a more efficient and effective recurrent model that addresses the limitations of existing methods.
...
TL;DR: 提出了一种用于分析协作问题解决中对话的框架，强调了人机和多智能体协作的新兴动态


<details>
  <summary>Details</summary>
Motivation: 分析协作问题解决中对话的动态，特别是人机协作和多智能体协作的新兴动态

Method: 分层双层编码方案，结合认知和非认知问题解决以及元认知调节机制

Result: 在九个跨多个领域的数据集上验证了框架的有效性和普适性，揭示了人类和智能体如何协调知识和技能以解决复杂问题，特别是元认知调节在更深层次的协作中扮演关键角色

Conclusion: 该框架有助于优化和评估人机和多智能体协作问题解决的伙伴关系
...
TL;DR: LLMs在处理历史语言方面存在挑战，但可以通过适当的方法缓解这些挑战


<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型（LLMs）在处理历史语言方面的能力，以及将历史难度分解为不同维度

Method: 提出了一种诊断框架，并在三个世纪的数据集上评估了该框架

Result: 发现编码成本与理解之间存在明显差异，历史文本对编码的影响是一致的，但数字图书馆可以安全地部署LLMs进行语义检索任务，前提是生成应用得到精心调整

Conclusion: LLMs在处理历史语言方面具有挑战性，但可以通过适当的方法缓解这些挑战
...
TL;DR: 提出了一种新的跨语言推理方法，通过保留原始问题信息提高性能


<details>
  <summary>Details</summary>
Motivation: 翻译级联推理在跨语言推理中的应用及其局限性

Method: 提出了一种上下文感知的翻译级联方法，并评估了其在多个基准上的性能

Result: 该方法在开放式生成任务中表现出色，原始语言问题提供了大部分有益的上下文信息

Conclusion: 为了减轻错误传播，需要更好地设计机器翻译级联中的信息流，并建议保留原始用户问题直到管道结束
...
TL;DR: 本研究首次将LLMs应用于证券抵押品合格性验证，显著提高了验证效率和准确性。


<details>
  <summary>Details</summary>
Motivation: 为了提高德国央行对证券作为抵押品合格性验证的效率，减少资源消耗。

Method: 采用大型语言模型（LLMs）进行信息提取，将任务分解为提取、归一化和解释三个阶段。

Result: 基于LLMs的系统在文档级别合格性验证中实现了高达91%的精确率，具有保守的操作特征，最大限度地减少了错误接受。

Conclusion: LLMs在证券抵押品合格性验证中具有显著优势，可以有效地提高验证效率和准确性。
...
TL;DR: PEEU方法通过构建历史高级任务和利用经验，有效提升了小规模MLLM在重复GUI任务中的规划能力。


<details>
  <summary>Details</summary>
Motivation: 提高小规模机器学习模型在重复GUI任务中的任务规划能力，解决现有模型在规划和泛化方面的不足。

Method: 提出PEEU方法，通过自主探索环境发现经验，并利用历史经验合成高级训练数据；提出TDHAF框架，系统研究不同任务粒度下的组合泛化。

Result: 实验结果表明，PEEU方法在真实世界基准测试中优于Qwen2.5-VL-32B模型，证明了构建历史高级任务和利用经验对于小规模MLLM的OOD规划能力至关重要。

Conclusion: PEEU方法能够有效提高小规模MLLM在重复GUI任务中的规划能力，并通过TDHAF框架揭示了不同任务粒度下的泛化行为。
...
TL;DR: 本文提出了一种新的方法来分析政治精英组织，通过构建知识图谱，为跨国的经验计算社会科学研究提供了基础。


<details>
  <summary>Details</summary>
Motivation: 观察政治精英如何组织成寻租联盟或公民网络，一直是比较政治学中的一个核心问题。然而，在规模上观察这些复杂、非正式和对抗性的联系需要密集的手动编码，而自动文本数据方法在很大程度上仅限于简单的共现。

Method: 将基于跨度命名实体识别（NER）与三个阶段的链接级联映射相结合，将提及映射到语言无关的Wikidata标识符；然后，一个高吞吐量、本体约束的专家混合模型使用引导解码提取基于领域本体的有向、签名的关联关系。

Result: 在奥地利，该框架重建了一个政党的完整生命周期，追溯了内部裂痕，并跟踪人员进入继承派系和法院审判。在波兰语语料库中，它揭示了国家-企业赞助重叠的经济和治理网络，以及结构平衡的、有向的冲突网络，即极化的公民平台（Platforma Obywatelska, PO）--法律与正义（Prawo i Sprawiedliwość, PiS）双头垄断。

Conclusion: 本文提出了一种模块化的、完全开源的、多语言联合实体关系提取管道，从大量的非结构化新闻语料库中构建了有向、时序知识图谱。该框架为跨国的经验计算社会科学研究提供了一个稳健、可复制的基石。
...
TL;DR: Proposed attention-based model efficiently predicts phase separation in binary mixtures.


<details>
  <summary>Details</summary>
Motivation: The spatiotemporal evolution of physical, chemical, and biological systems is often described by nonlinear PDEs. Deep neural network-based surrogate models are efficient alternatives to traditional numerical solvers.

Method: Proposing an attention-based, physics-guided convolutional neural network as a surrogate model. Training the model to predict the full time-evolution of phase separation in binary mixtures governed by the Cahn-Hilliard equation.

Result: The model accurately predicts phase separation, preserves mixture composition, captures domain growth, and is consistent with the Lifshitz-Slyozov domain-growth law.

Conclusion: The proposed framework is effective for modeling systems with conserved kinetics and can be extended to other complex dynamical systems.
...
TL;DR: 该研究提出了一种新的模型来解释单细胞藻类的光向性导航，该模型能够再现实验结果，并揭示了细胞内生化网络在信息寻求中的作用。


<details>
  <summary>Details</summary>
Motivation: 研究单细胞藻类如何利用感官信号导航环境，并探讨其信息驱动的过程。

Method: 提出将部分可观察马尔可夫决策过程与生化反应动力学相结合的框架，并使用逆强化学习进行行为目标推断。

Result: 模型能够再现实验记录的拟南芥光向性运动，并与标准随机模拟算法基准相当。

Conclusion: 该模型展示了细胞内生化网络如何支持细胞导航中的适应性信息寻求行为。
...
TL;DR: 本文提出了一种新的公平策略，以解决机器学习系统中的公平性问题。


<details>
  <summary>Details</summary>
Motivation: 为了解决现代机器学习系统在预测准确性方面可能对边缘化群体产生系统性不利影响的问题。

Method: 通过识别和解决当代公平范式中的两个基本局限性：对确定性点估计的依赖以及对个体作为孤立实体的处理。

Result: 提出了新的公平策略，以解决复杂的社会技术环境中的公平性问题。

Conclusion: 新的公平策略有助于减少机器学习系统中的结构性不平等和偏见，从而提高对边缘化群体的机会。
...
TL;DR: 本综述全面回顾了应用于GAN的NAS方法，比较了各种方法，并突出了其优势、局限和未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 自动化GAN架构搜索，解决手动设计的挑战。

Method: 通过分类和比较NAS方法，包括搜索策略、评估指标和性能结果。

Result: 强调了进化算法和基于梯度的方法的优越性，指出超越IS和FID的传统分数的稳健评估指标的重要性，以及评估GAN性能所需的多样化数据集。

Conclusion: 本综述突出了NAS在提升GAN性能、稳定性和效率方面的益处，并确定了现有方法的局限性以及未来研究方向。
...
TL;DR: KG-TRACE通过结合知识图谱和神经符号框架，提高了基于WGS的AMR预测的准确性和生物学归因。


<details>
  <summary>Details</summary>
Motivation: 现有基于WGS的AMR预测模型缺乏将神经归因与已建立的生物学途径相结合的机制。

Method: 提出KG-TRACE，一个新颖的神经符号框架，该框架将世界卫生组织突变知识图谱（KG）集成到神经基因组模型中，作为结构化的生物学约束。通过学习知识信任门，KG-TRACE融合基因组特征和基于RotatE的KG嵌入。

Result: 在CRyPTIC M. tuberculosis队列上评估，KG-TRACE在异烟肼上的AUROC达到0.9760，实现了有竞争力的准确性。引入了生物学归因比（BGR），这是一个数据集级别的指标，用于量化神经归因与已建立生物学之间的对齐。框架实现了异烟肼耐药预测的92.5%符号覆盖率，并通过为'UNCERTAIN'病例发出实验室后续标志来有效识别MDR共发生伪影。

Conclusion: 神经符号归因提供了可验证的审计跟踪，弥合了预测准确性与临床信任之间的差距。
...
TL;DR: 提出了一种新的DP学习框架，通过使用超网络将私有数据集映射到目标模型参数，有效减少噪声影响，提高DP训练效率。


<details>
  <summary>Details</summary>
Motivation: 提高差分隐私（DP）神经网络训练的效率，减少噪声的影响。

Method: 提出一种新的DP学习框架，避免在参数空间中进行迭代优化。使用超网络将私有数据集映射到目标模型的参数。

Result: 在合成设置中，在固定的隐私预算下，所产生模型的有效性高于使用DP-SGD训练的模型。在LoRA微调扩散模型时，比使用DP-SGD和其他公开数据引导方法训练的LoRA模型具有更低的FID。

Conclusion: 该方法可以显著减少噪声的负面影响，提高DP训练的效率。
...
TL;DR: Clue2Group：一种基于线索的反洗钱方法，逐步恢复洗钱集团，弥合研究与实践差距。


<details>
  <summary>Details</summary>
Motivation: 为了解决现有方法与实际反洗钱调查之间的不匹配问题，提出了一种新的方法。

Method: 提出了一种名为Clue-Guided Group Discovery (CGGD)的方法，该方法通过分析人员的交互，从初始线索集中逐步恢复洗钱集团。Clue2Group框架首先构建紧凑的本地调查环境，然后使用多语义本地时序GNN估计线索条件下的本地风险场，最后整合风险、结构和先验模式证据来恢复一致的洗钱集团。

Result: 在两个大规模反洗钱基准测试中，Clue2Group提供了一种实用的线索驱动分析框架，为反洗钱调查提供了可行的步骤，有助于弥合基于图的反洗钱研究与实际调查工作流程之间的差距。

Conclusion: Clue2Group是一种有效的反洗钱方法，可以逐步恢复洗钱集团，并有助于弥合基于图的反洗钱研究与实际调查工作流程之间的差距。
...
TL;DR: 提出了一种新的不确定性评估指标，该指标与决策效用更一致，为现有评估协议提供了改进。


<details>
  <summary>Details</summary>
Motivation: 在机器学习中，通常使用通用的指标来评估不确定性估计，但这些指标的良好性能并不一定意味着在高层决策中的高效用。

Method: 引入决策一致性标准，揭示哪些评估指标与下游效用有意义的对齐。

Result: 许多广泛使用的不确定性指标与常见的决策问题不一致，或者编码了关于下游任务的病理学先验信念。提出了先验加权效用指标，这是一种特殊的正确评分规则，提供决策对齐的不确定性评估。在基准实验和实际案例研究中，我们的指标始终与实现决策效用相一致，而传统指标则不然。

Conclusion: 当前的不确定性评估协议存在缺陷，并提供了对现有指标向决策相关不确定性评估的原理性扩展。
...
TL;DR: 提出了FHPLF，一个集成了哈希学习到联邦学习的模型，以解决传统方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 为了解决传统哈希学习方法和联邦学习方法的局限性，例如数据安全问题、隐私风险以及表示能力的不足。

Method: 提出了一个名为FHPLF的联邦哈希投影潜因子模型，该模型具有三个关键创新：（1）使用二进制梯度类似矩阵代替实值梯度矩阵，以显著降低计算、存储和通信成本并增强隐私保护；（2）利用投影汉明距离进行相似度建模，捕捉到各个二进制位的重要性，以提高表示能力；（3）提出了一个安全二进制梯度重组和隐私增强上传（SBG-PEU）策略，以进一步减少用户交互泄露的风险。

Result: 在四个真实世界数据集上的大量实验表明，FHPLF在准确性、效率和隐私保护方面均优于最先进的哈希学习和联邦学习方法，实现了三者之间良好的权衡。

Conclusion: FHPLF是一种有效的方法，可以提高联邦学习中的数据安全性、隐私保护和表示能力。
...
TL;DR: 使用TDA和神经网络进行洪水检测，提高准确性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 为了提高洪水检测的准确性和及时性，以减少人类和经济损失。

Method: 使用拓扑数据分析（TDA）来提取洪水图像中的拓扑特征，并将其与神经网络结合，以提高洪水检测系统的鲁棒性和可解释性。

Result: 拓扑描述符能够独立携带有意义的洪水信号，并补充现有网络，从而产生更鲁棒和可解释的洪水检测系统。

Conclusion: 拓扑数据分析在洪水检测中具有潜力，可以提高检测系统的性能和可解释性。
...
TL;DR: 提出了一种基于GNN的框架，可以从凯莱图中学习有限群的代数性质


<details>
  <summary>Details</summary>
Motivation: 研究有限群代数性质的图表示学习

Method: 提出一个通用的框架，从凯莱图中直接学习有限群的代数性质，使用图神经网络（GNN）架构和训练流程

Result: 成功从关联的凯莱图中学习并区分多个代数性质，证明框架可以应用于有限群代数性质的研究

Conclusion: 图表示中编码了大量的代数信息，可以通过GNN提取
...
TL;DR: Fast-LeWM通过动作前缀预测，提高了视觉规划的效率和准确性。


<details>
  <summary>Details</summary>
Motivation: LeWM在视觉规划中的计算成本高，预测轨迹存在累积误差。

Method: 提出Fast-LeWM，使用动作前缀预测代替重复的局部回滚。

Result: Fast-LeWM在多个任务上提高了平均成功率，同时显著减少了规划时间。

Conclusion: Fast-LeWM是一种快速且有效的视觉世界模型，适用于视觉规划。
...
TL;DR: 提出了一种新的DUI方法，无需影子模型或真实保留数据，可量化数据集的使用情况


<details>
  <summary>Details</summary>
Motivation: 估算数据集中有多少数据被用于训练机器学习模型

Method: 生成合成非成员样本，提取不同的成员信号，将DUI视为混合比例估计问题来估计候选数据集中有多少数据被用于训练

Result: 在大型图像生成模型上的实验表明，该方法可靠地量化了数据集的使用情况，为数据所有者提供了确定其数据中有多少被用于训练模型的实际工具

Conclusion: 提出了一种实用的DUI框架，无需影子模型或真实保留数据，为数据所有者提供了一种确定其数据使用情况的方法
...
TL;DR: RQGM是一种通过引入对抗性目标和动态效用实现递归自我改进的新框架。


<details>
  <summary>Details</summary>
Motivation: 当前的自我改进代理假设一个固定的评估标准，忽略了环境变化的重要性。

Method: 引入了红皇后哥德尔机器（RQGM），这是一种在非平稳效用下的递归自我改进的进化框架。通过受控的效用进化，将搜索组织成具有固定周期内评估标准的时期，同时可以在周期边界更新效用，以确保每个周期自我改进的保证。

Result: 在可验证的编码任务中，RQGM通过添加互补的代理作为裁判的代码审查信号，提高了测试通过率。在科学论文写作和审查以及奥林匹克级别的证明写作和评分中，RQGM的性能优于先前的自我改进代理。

Conclusion: RQGM通过引入对抗性目标，提高了评估的严格性，从而在多个领域实现了自我改进。
...
TL;DR: 本文研究了一阶方法在满足PL条件且梯度样本由外生马尔可夫链生成的平滑目标下的收敛性，并提出了新的算法和块方法。


<details>
  <summary>Details</summary>
Motivation: 在具有外生马尔可夫链生成梯度样本的情况下，研究满足PL条件的平滑目标的一阶方法。

Method: 使用滞后屏蔽论证和降低到重尾平均估计的PL优化，设计了新的块方法和算法。

Result: 提出了新的块方法和算法，提高了轻尾和重尾Markovian梯度下的收敛性。

Conclusion: 本工作紧密地描述了轻尾PL-SGD在混合时间上的最优多项式依赖，以及鲁棒情况下的最优重尾指数和有效样本量依赖。
...
TL;DR: EVOM是一种用于自动化演员-评论家架构设计的元进化框架，在实验中表现出优异的性能。


<details>
  <summary>Details</summary>
Motivation: 在演员-评论家强化学习中，网络架构通常是手动设计的。自动化这一设计具有挑战性，因为每个候选架构必须经过训练才能进行评估，且设计空间是开放的。

Method: 我们引入EVOM，这是一个基于代理的元进化框架，用于发现高性能的演员-评论家架构。将架构搜索视为一个双层优化：内循环通过低保真近端策略优化（PPO）训练权重，而外循环通过迭代优化架构程序来驱动元进化。关键的是，这个外循环由一个基于LLM的设计代理驱动，该代理仅作为架构设计师运行，完全与策略执行和环境控制解耦。

Result: 实验表明，EVOM优于手动设计的基线、LLM引导的随机搜索以及最先进的LLM引导的程序化策略搜索方法MLES，在Ant-v4和HalfCheetah-v4上表现出优异的性能。消融研究表明，元进化循环和LLM设计代理对于最终性能都是必不可少的。

Conclusion: EVOM是一种有效的自动化演员-评论家架构设计方法，优于手动设计和其他自动化方法。
...
TL;DR: Mesh-RL通过空间域分解加速强化学习中的时间差学习。


<details>
  <summary>Details</summary>
Motivation: 强化学习在大型或稀疏奖励环境中存在时间差奖励传播缓慢的问题，因为价值信息只在状态空间中局部传播。

Method: 提出了一种名为Mesh-RL的空间域分解框架，该框架受到有限元方法和域分解理论的启发，将环境划分为重叠的子网格，并强制执行边界一致的时间差更新。

Result: 在Q学习、SARSA和Dyna-Q上评估Mesh-RL，在具有不同几何形状和网格分辨率的危险密集网格世界中，Mesh-RL在收敛速度、累积奖励和学习稳定性方面均有所提高。

Conclusion: Mesh-RL引入了一种原理性的空间域分解机制，用于加速时间差学习，将科学计算中受有限元方法启发的边界一致性技术与强化学习相结合，以提高稀疏奖励环境中的样本效率。
...
TL;DR: 提出了一种新的特征筛选算法EMA-FS，显著提高了GBDT的训练速度，并在不同数据集上取得了良好的效果。


<details>
  <summary>Details</summary>
Motivation: 为了提高GBDT（例如LightGBM）的训练速度，特别是减少特征直方图构建的时间。

Method: 提出了一种基于指数移动平均（EMA）的特征筛选算法（EMA-FS），该算法在迭代过程中维护每个特征的EMA分割增益，并在预热后仅对历史增益排名前K的特征构建直方图。此外，还引入了随机EMA-FS（S-EMA-FS），该算法使用集中参数beta控制的增益加权的随机采样替换确定性top-K选择。

Result: 在金融欺诈检测、广告点击率预测、工业质量控制等数据集上，EMA-FS在密集的、中到高维度的数据上实现了显著的加速。S-EMA-FS则将确定性EMA-FS和随机子采样统一在一个框架内。

Conclusion: EMA-FS和S-EMA-FS是有效的特征筛选方法，可以显著提高GBDT模型的训练速度，且与LightGBM兼容。
...
TL;DR: 研究揭示了Aurora模型如何通过内部表示学习气象动力学


<details>
  <summary>Details</summary>
Motivation: 研究Aurora模型内部表示，以了解其如何模拟大气动力学

Method: 使用空间池化主成分分析（PCA）和层状相关性传播（LRP）

Result: 发现Aurora的潜在空间主要由季节周期组织，极端风暴事件没有形成可线性分离的簇；模型关注与1987年大风暴的三维垂直结构一致的特征；掩蔽相关区域比随机掩蔽降低了3.31倍的预测性能

Conclusion: Aurora模型学习气象一致性和垂直结构，无需明确指令
...
TL;DR: SOLAR是一个自动推导SOL界限的框架，可提高深度学习模型在目标硬件上的运行速度。


<details>
  <summary>Details</summary>
Motivation: 为了解决手动推导SOL界限的困难、易出错以及与快速模型开发脱节的问题。

Method: SOLAR框架，自动从PyTorch和JAX源代码中推导验证过的SOL界限。

Result: SOLAR提供了全面的操作符和语言覆盖，产生了验证过的界限，没有观察到SOL违规，并提供了多保真度分析，以收紧界限并揭示优化见解。

Conclusion: SOLAR在KernelBench、JAX/Flax模型和机器人工作负载上进行了评估，证明了其在多个保真度级别的头寸分析、识别优化机会、跨平台探索和逆屋顶线硬件配置方面的应用。
...
TL;DR: 通过实验发现，Transformer模型在处理与训练数据分布不同的数据时，会出现内部概念错误，并提出一种新的诊断方法以增强模型鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 提高Transformer模型在实际应用中的可靠性和安全性，应对与训练数据分布不同的数据。

Method: 通过系统实验提出一个机制框架，量化理解提示的分布变化，从而实现模型鲁棒性的提升。

Result: 发现OOD输入，如微小错误和越狱提示，导致语言模型在其内部操作更多错误概念。

Conclusion: 提出一个新的Transformer诊断方法，有助于使AI系统在科学、商业和政府等领域得到安全部署。
...
TL;DR: 本文提出了一种新的偏好条件Bellman算子，用于计算MOMDPs的确定性帕累托最优策略，成功恢复了复杂的权衡，为多目标优化问题提供了一种解决方案。


<details>
  <summary>Details</summary>
Motivation: 在现实世界的决策中，平衡多个相互冲突的目标是一个挑战，标准强化学习（RL）通常通过将奖励聚合为单个标量信号来解决这个挑战。然而，这种方法对于简单的任务来说很有效，但往往无法捕捉到最优权衡的完整范围，即帕累托前沿。在本文中，我们受切比雪夫标量化启发，引入了一种新颖的偏好条件Bellman算子，旨在计算多目标马尔可夫决策过程（MOMDPs）的确定性帕累托最优策略。

Method: 提出了一种新颖的偏好条件Bellman算子，该算子基于切比雪夫标量化，旨在计算多目标马尔可夫决策过程（MOMDPs）的确定性帕累托最优策略。该算子满足包含属性，即估计的价值函数上界于真实的帕累托前沿，并证明它单调收敛到该前沿的覆盖集。此外，还展示了如何从这些收敛的Q估计中提取确定性策略。

Result: 该算法成功恢复了复杂的权衡，为确定性帕累托最优策略的综合提供了一种解决方案。

Conclusion: 本文提出的方法能够有效计算多目标马尔可夫决策过程（MOMDPs）的确定性帕累托最优策略，为现实世界决策中的多目标优化问题提供了一种新的解决方案。
...
TL;DR: 提出了一种安全AGI的架构，具有自模型、工具性自我保护和无程序目标导向行为。


<details>
  <summary>Details</summary>
Motivation: 为了构建安全的人工通用智能（AGI）。

Method: 基于封闭再入环（D <-> I循环）的完整架构蓝图，S度量计算，机器验证Lean 4证明，Apache Kafka和Docker Compose的横向扩展，容错和恢复协议等。

Result: 提出了一种基于封闭再入环的AGI架构，包括S度量、机器验证的Lean 4证明、横向扩展方案、容错和恢复协议等。

Conclusion: 此架构是可部署的，代表了基于拓扑保护、安全设计的AGI方法。
...
TL;DR: Otter Weather模型通过提高效率和降低成本，为AI天气预测带来了突破，使其更适用于资源有限群体和快速迭代。


<details>
  <summary>Details</summary>
Motivation: 为了提高高性能天气预测的可用性，尤其是对资源有限群体的可用性，以及加快模型迭代速度。

Method: 开发了一种名为Otter Weather的高效时空预测模型，通过使用ERA5再分析数据在1.5°分辨率上，采用标准WeatherBench协议进行评估。模型训练通过连续排名概率得分（CRPS）进行。

Result: Otter系列模型在技能-计算帕累托前沿方面取得了显著进步。确定性版本在24小时预测提前期上优于最佳NWP基线9.6%，而训练时间少于3.5 A100天。在概率预测中，Otter-XL在更大架构上实现了9.7%的CRPS改进。在复杂的声学散射PDE任务中，Otter模型优于最先进的基模方法。

Conclusion: Otter Weather模型提高了AI天气预测的效率和准确性，同时降低了计算成本，为资源有限群体和快速模型迭代提供了新的可能性。
...
TL;DR: 通过后处理方法解决轨迹预测模型的无信息后验问题，提高预测准确性。


<details>
  <summary>Details</summary>
Motivation: 轨迹预测模型在自动驾驶领域快速发展，但代表性模型在预测模式上产生无信息后验，导致模式剪枝问题。

Method: 引入两种后处理方法：测试时后验加权合并和一步期望最大化（EM）更新，以解决模型训练不匹配问题。

Result: 在多个WTA训练架构上，这些轻量级步骤产生了更 informative、faithfully ranked 模式后验，并加强了最终预测。

Conclusion: 通过GMM与K-means的视角统一了最近的设计选择，并提供了与推理更一致的训练目标的原则性、实用性的纠正。
...
TL;DR: DualEval通过联合模型-项目校准，统一了静态和竞技场式评估，提高了LLM评估的效率和可靠性。


<details>
  <summary>Details</summary>
Motivation: 当前LLM评估依赖于静态基准和开放式用户交互的偏好数据，这两种方法通常相互独立。

Method: 提出了一种名为DualEval的潜在模型-项目校准框架，用于联合估计模型能力、项目难度和锐度。

Result: 在四个领域应用DualEval，对18个前沿LLM进行评估，结果可靠且平衡，支持更高效、可解释和可审计的评估流程。

Conclusion: DualEval通过联合模型-项目校准，统一了静态和竞技场式评估，产生了模型排名和项目级诊断，支持更高效、可解释和可审计的评估流程。
...
TL;DR: 提出两阶段适配器，解决tabular foundation models预测结果违反经济逻辑的问题，提高了预测准确率和合理性。


<details>
  <summary>Details</summary>
Motivation: Tabular foundation models在预测任务上表现良好，但其预测结果常违反经济逻辑。

Method: 提出一个两阶段适配器，将基础模型的预测选择概率作为预计算特征，并嵌入多项logit的效用中。第一阶段，使用最大似然估计和符号约束来拟合多项logit的结构系数；第二阶段，冻结这些系数，并拟合一个小的神经网络对基础模型的预测进行校正。

Result: 在三个数据集和两种基础模型上，适配器平均提高了6.4个百分点的测试准确率，最高可达12.8个百分点，保持了100%的成本单调性，并在交通数据集上产生了在已发表的运输经济学范围内的价值。在基础模型上下文限制下，性能优雅地下降，即使在原始基础模型上下文的10%时，也能保持至少6个百分点的准确率提升。

Conclusion: 该两阶段适配器有效地解决了tabular foundation models预测结果违反经济逻辑的问题，提高了预测的准确性和合理性。
...
TL;DR: 我们提出了一种新的实时强化学习方法，通过使用门控策略选择状态相关的规划预算，提高了实时环境中的性能。


<details>
  <summary>Details</summary>
Motivation: 在实时设置中，审议需要时间，而这种时间并非免费。标准强化学习（RL）通过让环境无限期地等待代理的决定来规避这一点。

Method: 我们研究了实时强化学习环境，其中环境在等待代理的动作期间继续进展。在之前的实时形式化的基础上，我们引入了可变延迟实时强化学习，其中代理可以选择在每次决策点审议多长时间，因为环境在进展。对于我们使用的规划代理，正确的延迟是状态相关的，而天真地计划多长时间可以导致代理瘫痪。我们通过在规划器之上训练一个轻量级门控策略来处理这个问题，以选择状态相关的规划预算。

Result: 在实时 Pac-Man、Tetris、Snake、Speed Hex 和 Speed Go 上，我们的门控策略优于固定预算和启发式基线，并且转移到在两个不同的 GPU 上运行环境和代理的实时设置中。

Conclusion: 通过引入可变延迟实时强化学习，并训练一个轻量级门控策略，我们能够提高实时强化学习环境中的性能。
...
TL;DR: TabPFN-CFM模型有效解决多因果问题，预测准确度高
...
Method: TabPFN-CFM模型，预测因果结构和结果，支持Pearl的因果层次的三级查询，利用已知图结构提高预测

Result: 在合成数据集上训练，推广到真实数据集，在结构和结果预测上优于基线
...
TL;DR: DFC方法通过识别和利用RL模型中的特定特征，提高了代理型LLMs的工具调用能力，并实现了对它们的行为控制。


<details>
  <summary>Details</summary>
Motivation: 为了理解通过强化学习（RL）微调的语言模型内部表示的变化机制，并探究这些变化如何影响工具使用等代理行为。

Method: 提出了一种名为“专用特征交叉编码器（DFC）”的方法，通过在Qwen2.5-3B模型中隔离一组RL特定的特征，来分析这些特征对工具调用能力的影响。

Result: DFC将RL引入的能力集中到一个最小、可操控的特征集中，从而实现了对代理型LLMs的运行时行为控制。在48次交叉编码器超参数扫描中，编码-解码重建提高了RL模型的工具正确性，并使冻结的基础模型被动地转移了工具调用能力。

Conclusion: DFC能够有效地识别和利用RL模型中的特定特征，从而实现对代理型LLMs的行为控制。
...
TL;DR: 研究检索预热能量推理，发现每图的对齐是主导因素，并提出了五臂消融方法。


<details>
  <summary>Details</summary>
Motivation: 加速迭代推理，但不确定哪部分流程带来了增益。

Method: 研究检索预热能量推理（RW-EBR），这是一种IRED能量扩散模型，并引入了现代Hopfield轨迹记忆。提出了五臂消融方法（占卜、最佳常数、每查询随机、洗牌、对齐），以分离三个混淆效应：类别优先偏差移动、随机预热启动和图对齐值重用。诊断分解来自LLM-RAG评估。

Result: 在connectivity-2（Erdős-Rényi全对数可达性）上，对齐与洗牌或占卜的平衡准确率提高了+35%，在固定的1000个图验证集诊断中，只破坏了每图的对齐，而每查询的随机初始化低于冷启动。但可部署的冷预测流程在存储值质量上缺失了接受门。同样的诊断逻辑，在关键质量屏幕停止，应用于Sudoku，产生了不同的组件——关键质量。

Conclusion: 研究揭示了检索预热能量推理中的不同效应，并通过消融实验和诊断分解，明确了每图的对齐是主导因素。
...
TL;DR: 递归推理模型在压缩后，局部预测得以保留，但全局推理被破坏。


<details>
  <summary>Details</summary>
Motivation: 在边缘硬件上部署递归推理模型需要显著的压缩，但与传统的序列模型不同，量化误差在递归推理周期中累积，而不是在输出标记中累积。

Method: 通过在三个任务和两种递归架构上进行全精度扫描，研究压缩对递归推理模型的影响。

Result: 在激进压缩下，局部预测得以保留，但全局推理被破坏。

Conclusion: 引入了携带轨迹保真度作为无标签信号，预测损害及其恢复，并提出了部署方案。
...
TL;DR: 提出了一种自适应框架，结合GB-Latent ODE和MP-JIT-FT，有效提高了生物工艺预测的准确性，并通过融合拉曼光谱数据增强了模型的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 为了及时调整喂养、采样和控制，需要提前预测多日生物工艺过程，但由于数据稀疏、不规律采样、操作条件异质以及运行轨迹可能不同，生物工艺预测具有挑战性。

Method: 提出了一种自适应框架，结合了门控瓶颈潜在常微分方程（GB-Latent ODE）和多功能即时微调（MP-JIT-FT）。GB-Latent ODE通过可学习的变量门控和掩码感知瓶颈压缩高维稀疏输入，提高有限数据下的学习效果。MP-JIT-FT检索相似的历史轨迹，将局部邻域聚类成候选状态，并为每个状态微调一个模型，以生成多个可能的路径，每个路径都有一个基于重建的置信分数。此外，还融合了拉曼光谱数据，通过机器学习软传感器将密集的拉曼光谱转换为伪观测值，丰富稀疏的离线测量，从而提高鲁棒性。

Result: 在38个14种条件下的5L生物反应器运行中，MP-JIT-FT与拉曼融合实现了最佳平均排名，并在9个目标变量中的8个上优于全局Latent ODE基线。使用局部差异度量，我们表明当局部相似的前缀发散时，多路径增益最大；而当早期动力学代表后期行为时，拉曼融合帮助最大。

Conclusion: 该方法有效提高了生物工艺预测的准确性，并有助于及时调整生产过程。
...
TL;DR: 本文提出了一种基于逻辑计算机科学的独立、理论规模的基准LCS-Bench，用于解决自动形式化中的挑战，并通过实验证明其有效性和挑战性。


<details>
  <summary>Details</summary>
Motivation: 为了解决现有自动形式化在一致性、忠实度、可扩展性和正确性方面的挑战，以及理论规模自动形式化的开放性问题。

Method: 提出LCS-Bench，一个基于逻辑计算机科学的独立、理论规模的基准。通过一个新颖的半自动化代理流程构建，利用概念图、形式签名规划、问题跟踪、反例搜索的抱歉填充，以及来自人类专家的忠实度审查。

Result: 构建了一个包含327个教科书项目、超过4076个Lean声明和超过85K行Lean代码的工件。数据集支持通过数据引擎自动推导五个评估基准，测量自动形式化和定理证明能力的不同方面。

Conclusion: LCS-Bench在质量、一致性和忠实度方面表现良好，对自动形式化任务具有挑战性，并揭示了关于理论规模自动形式化的关键发现，为未来的研究指明了有希望的方向。
...
TL;DR: TerraProbe是一个用于评估LLM辅助Terraform安全修复的框架，发现删除修复高估了成功率，并提出了区分真实修复和错误成功的评估方法。


<details>
  <summary>Details</summary>
Motivation: 云部署中Terraform基础设施代码的安全配置错误日益成为风险，大型语言模型被越来越多地用作自动化修复代理。

Method: 提出TerraProbe，一个五层预言机框架，用于评估LLM辅助的Terraform安全修复。

Result: 结果表明，针对Checkov的删除高估了修复的成功率。尽管针对删除的主要模型达到83.3%，但全扫描清洁度降至10.4%，Terraform规划成功率为39.6%，计划比较可达38.5%。

Conclusion: TerraProbe为区分意图一致的安全修复和扫描器通过的错误成功提供了评估方法、复制包和多层预言机评估框架。
...
TL;DR: 提出了一种安全迁移强化学习框架，显著提高了自主高速公路车道变更的安全性。


<details>
  <summary>Details</summary>
Motivation: 提高政策学习效率，解决现有方法在迁移学习中的匹配问题，并保证训练安全。

Method: 提出一个安全迁移强化学习框架，包括自适应教师干预机制、教师引导的安全迁移模块和教师引导的加权优化机制。

Result: 在安全和学习效率方面超过基线方法52.2%和5.0%，验证了该方法在各种交通条件下的有效性和鲁棒性。

Conclusion: 该方法能够有效提高自主高速公路车道变更的安全性，并提高学习效率。
...
TL;DR: 提出两种提高深度Transformer效率的方法，利用深度层的不对称信息流，提高了训练效率和性能。


<details>
  <summary>Details</summary>
Motivation: 为了解决深度Transformer深层层贡献较少的问题。

Method: 提出 CascadeFormer 和 CascadeFlow Pruning 方法，分析梯度扇入不对称性（GFA）对深度层贡献的影响。

Result: CascadeFormer 在保持相同训练预算下，实现了与均匀基线相当的困惑度，同时降低了8.6%的延迟和提高了9.4%的吞吐量。CascadeFlow Pruning 在困惑度和排名稳定性方面优于标准启发式方法，并在下游精度方面保持竞争力。

Conclusion: CascadeFormer 和 CascadeFlow Pruning 是两种提高深度Transformer效率的方法，它们利用了深度层的不对称信息流，提高了训练效率和性能。
...
TL;DR: 研究混合动作空间的控制问题，发现VDN-PPO和PPO-MIX表现最佳，自回归动作性能最高，原生连续SAC计算成本较高但性能更优。


<details>
  <summary>Details</summary>
Motivation: 研究混合离散-连续动作空间中的控制问题，如自动驾驶中的转向和信号、机器人或视频游戏中的瞄准和射击。

Method: 对三种算法（PPO、SAC、DQN）在三种动作空间（离散、混合、连续）和四种轻量级环境（Platform、混合-LunarLander、Hybrid-Shoot、CoopPush）中的因子分解方法（独立网络、共享编码器、VDN、QPLEX、联合、自回归）进行横断面研究。

Result: VDN-PPO和PPO-MIX使用分支评论员将信用分配给多头PPO，这些变体优于所有其他测试的PPO分解。自回归动作在整体性能上达到最高，而原生连续SAC在计算成本增加的情况下优于离散和混合算法。

Conclusion: 分支对偶架构在计算和性能之间取得了最佳平衡，自回归动作在整体性能上达到最高，原生连续SAC在计算成本增加的情况下优于离散和混合算法。
...
TL;DR: SharQ通过在线稀疏-密集分解实现LLM激活压缩，显著提高计算效率。


<details>
  <summary>Details</summary>
Motivation: 现代加速器越来越支持低比特浮点格式和半结构化稀疏性，但将它们结合用于LLM激活压缩仍然具有挑战性：激活包含输入依赖的异常值，这些异常值在FP4量化中占主导地位，直接应用N:M稀疏掩码会丢弃适度值，将稀疏化损失与量化误差耦合。

Method: 提出了一种名为SharQ的无训练推理方法，通过在线稀疏-密集分解将激活稀疏性和FP4量化联系起来。

Result: SharQ在多个语言和视觉语言任务上恢复了43-63%的NVFP4到FP16的精度差距，并在NVFP4、HiF4和MXFP4格式上具有良好的泛化能力。在RTX 5090上，与FP16相比，SharQ实现了2.2-2.4倍的延迟降低，与FP8相比，实现了1.2-1.4倍的吞吐量提升。

Conclusion: SharQ是一种有效的LLM激活压缩方法，在保持较高精度的同时，显著提高了计算效率。
...
TL;DR: We introduce Qwen3-Instruct SAE, a suite of SAEs for language models, and demonstrate its utility in steering models towards specific behaviors.
...
Method: We introduce Qwen3-Instruct SAE, a comprehensive suite of SAEs trained on the Qwen3 instruction-tuned model family, covering Qwen3-1.7B, Qwen3-4B, and Qwen3-8B. We train layer-wise SAEs at three key activation sites: residual streams, MLP outputs, and attention outputs. For Qwen3-8B, we train SAEs on a subset of residual stream layers.

Result: Systematically evaluate these SAEs using both activation-level reconstruction metrics and model-level recovery metrics, revealing distinct sparsity--fidelity trade-offs across layers and components. Demonstrate the utility of Qwen3-Instruct SAE through a refusal-steering case study, showing that selected SAE features can causally steer instruction-tuned Qwen3 models toward refusal behavior.
...
TL;DR: 提出了一种基于激活空间正则化的新方法，用于解决大语言模型在持续学习中的灾难性遗忘问题，优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 解决大语言模型在持续学习中灾难性遗忘问题，传统方法如EWC在大型语言模型中表现不佳。

Method: 在模型激活空间进行正则化，使用预训练的稀疏自编码器（SAE）作为单义特征词典。

Result: 在TRACE和MedCL持续学习基准测试中，该方法在没有引入特定任务架构组件的情况下，取得了最佳结果，并超越了传统的权重空间正则化方法如EWC。

Conclusion: 提出的方法在持续学习中有效解决了大语言模型的灾难性遗忘问题，优于传统方法。
...
TL;DR: BOBa框架通过自适应分配计算，提高了样本效率，为超大型分子库的虚拟筛选提供了可行的解决方案。


<details>
  <summary>Details</summary>
Motivation: 从大量离散空间中识别高效用候选者是一个跨科学领域的挑战，结构化药物发现是一个突出的例子。

Method: 提出BOBa，一个基于探索的代理优化框架，通过自适应地分配计算来消除整个库的推理，将分区视为多臂老虎机中的臂，将推理和评估集中在经验上有希望的分区上，同时保持原则性的探索。

Result: 实验表明，乐观不确定性老虎机与有意义的动作空间分区相结合，对于有效分配推理和评估至关重要。我们的发现揭示了筛选性能和代理推理成本之间的可调节权衡，这支持了当前库的实际优化，并为超大型库的虚拟筛选开辟了一条可行的途径。

Conclusion: BOBa框架通过自适应分配计算，有效提高了样本效率，为超大型分子库的虚拟筛选提供了可行的解决方案。
...
TL;DR: GNDEs 在小图上训练，在大图上部署，表现出良好的性能和零样本迁移能力。


<details>
  <summary>Details</summary>
Motivation: Graph Neural Differential Equations (GNDEs) 模型连续时间图动力学，通过用图神经网络参数化神经网络微分方程速度场。

Method: 通过在稀疏随机图上采样图子，开发定量理论，考虑图子神经网络微分方程（Graphon-NDEs）和伴随图子神经网络微分方程（adjoint Graphon-NDEs）作为前向和伴随 GNDE 系统的无限节点极限，并建立适定性。

Result: 证明了 GNDE 解在轨迹上以 $O((α_n n)^{-1/2})$ 的速率收敛到 Graphon-NDE 解，直到对数因子，以高概率。还建立了控制隐藏状态和参数梯度的伴随系统的时间一致性收敛界限。

Conclusion: GNDEs 在小图上训练，在大图上部署，表现出良好的性能和零样本迁移能力。
...
TL;DR: HyperDFlash通过优化解码框架，显著提升了DeepSeek-V4的起草精度和解码速度。


<details>
  <summary>Details</summary>
Motivation: 尽管DeepSeek-V4的MTP模块在初始令牌起草方面表现出色，但其起草精度在后期位置急剧下降，因为未验证的中间令牌的错误累积会损害接受率。

Method: 提出HyperDFlash，一个针对DeepSeek-V4提出的多超连接（MHC）架构定制的块并行投机解码框架。通过采用预折叠残差状态作为独家条件信号，并使用轻量级门控残差减少器替换重型通用线性压缩器，实现模型对齐和参数优化。

Result: 在数学推理、代码合成和对话基准测试中，HyperDFlash在平均接受起草长度和解码速度方面取得了显著提升，优于原生MTP基线和vanilla DFlash适配。

Conclusion: HyperDFlash通过MHC对齐、门控减少和针对性蒸馏，在投机解码方面表现出色，验证了其有效性。
...
TL;DR: 神经网络通过one-hot标签也能学习到语义结构


<details>
  <summary>Details</summary>
Motivation: 分析神经网络在平衡一热分类下预测的模型表示以及语言模型在one-hot标签下的语义结构学习

Method: 通过构建三个合成的控制环境来研究语义结构

Result: 发现语义几何在训练早期出现，表示会根据共享属性聚类，最终模型达到对称状态

Conclusion: 尽管没有显式的监督，模型也能学习到语义结构，并且可以通过修改模型特征来捕捉这种结构
...
TL;DR: BISN是一种用于自动昆虫物种鉴定的端到端框架，提高了跨批次鲁棒性和生化可解释性。


<details>
  <summary>Details</summary>
Motivation: Edible insects作为蛋白质来源的替代品，需要可靠的物种鉴定以控制过敏原暴露、防止掺假并符合监管标准。

Method: 提出了一种名为Batch-Invariant Spectral Network (BISN)的端到端框架，该框架结合了一个可学习的预处理模块和熵正则化的对抗性目标，以抑制特定的光谱变化。

Result: BISN在三个独立生产批次中收集的2700个光谱上实现了0.93的平均leave-one-batch-out准确率，比最强的基线高出4%。

Conclusion: BISN解决了跨批次鲁棒性和生化可解释性，以在现实工业条件下实现自动昆虫物种鉴定。
...
TL;DR: 对AlphaEdit进行了可复现性研究，发现其在不同条件下表现不一，并揭示了其理论保证的局限性。


<details>
  <summary>Details</summary>
Motivation: 为了验证AlphaEdit方法的有效性和可复现性，以及探讨其在不同模型架构、下游基准和编辑规模下的表现。

Method: 进行可复现性研究，包括在原始实验设置下重现AlphaEdit的结果，并扩展评估到新的模型架构、额外的下游基准和更长的序列编辑范围。

Result: 成功重现了AlphaEdit在原始模型上的报告指标，但发现报告的流畅性和一致性指标存在差异。AlphaEdit在扩展到新的模型架构时，其优势并不普遍适用，这归因于定位-编辑范式中的架构假设被这些新模型违反。随着编辑数量的增加，性能在原始报告的规模上保持稳定，但达到更高数量时性能下降，表明零空间投影对灾难性遗忘的保护是有界的。在三个额外的基准上评估编辑模型，发现大规模序列编辑降低了下游任务能力和与安全相关的拒绝行为。

Conclusion: AlphaEdit在其原始范围内表现如报告所示，但其核心理论保证对模型架构和编辑规模敏感，这在其实际部署中具有实际意义。
...
TL;DR: AIGP框架显著提升了电子商务的定价效果，同时保证了可解释性和透明度。


<details>
  <summary>Details</summary>
Motivation: 传统动态定价模型在大规模电子商务中存在可解释性有限、未充分利用非结构化信息以及与长期业务目标（如累计商品交易总额、投资回报率和里程碑达成）不符的问题。

Method: 提出了一种名为AIGP的新型框架，该框架利用大型语言模型（LLM）和领域知识、结构化数据以及文本上下文进行可解释的、知识感知的定价决策。采用监督微调进行知识蒸馏，以在保持高质量输出的同时高效部署。AIGP的核心是长期价值估算器（LTVE），通过离线强化学习在历史数据上训练，作为评分候选定价操作的奖励模型，并用于直接偏好优化（DPO），从而将定价策略与长期业务目标对齐。

Result: 在淘宝工厂的离线和大规模在线A/B测试中，AIGP实现了显著的改进：与生产基线相比，14天内商品交易总额（GMV）提高了+13.21%，投资回报率（ROI）提高了+7.59%，里程碑达成率提高了+8.20%，同时提供了可解释和透明的定价理由。

Conclusion: AIGP框架在提高GMV、ROI和里程碑达成率方面表现优异，并且具有可解释性和透明度。
...
TL;DR: 通过分析初始推理标记，提出了一种新的SFT数据整理方法，显著提高了LLMs的推理能力，同时降低了成本。


<details>
  <summary>Details</summary>
Motivation: 为了提高大型语言模型（LLMs）的推理能力，提出了一种基于监督微调（SFT）的方法。然而，现有的SFT数据整理方法依赖于强大的推理模型，导致成本高且数据质量不佳。

Method: 提出了一种新的方法，通过分析预训练模型的初始推理标记来识别多样性和挑战性的推理示例。具体来说，根据预训练模型随机扰动检查点的第一个100个推理标记的损失，可以可靠地检测到困难问题。进一步地，展示了具有相似损失模式的示例，这些示例在少量扰动检查点和微调轨迹上可以产生相似的梯度。

Result: 在M23K医学推理和OpenThoughts-Math数据集上对Qwen2.5-7B和Llama3.1-8B模型进行微调的实验中，该方法比现有基线提高了1.7%，并且效率提高了91%。

Conclusion: 该方法有效地提高了LLMs的推理能力，同时降低了成本并提高了数据质量。
...
TL;DR: 本文对FL中的量化进行了系统性的回顾，提出了一种新的分类法，并分析了量化与核心FL行为之间的关系。


<details>
  <summary>Details</summary>
Motivation: 隐私保护分布式智能领域，联邦学习（FL）成为基础范式，但其可扩展性受限于通信瓶颈、设备异质性和统计非独立同分布（non-IID）数据训练的挑战。量化是一种有效的缓解这些限制的机制，可以减少上行/下行负载和设备上的计算。

Method: 提出了一种新的量化分类法，涵盖了FL特定的维度，包括客户端异质性、聚合一致性、通信调度适应性、non-IID鲁棒性、隐私/安全性集成和硬件/能源协同优化。

Result: 分析了量化与核心FL行为（如客户端漂移、部分参与、收敛稳定性、安全聚合和差分隐私）的相互作用，并确定了跨方法见解、开放研究差距和实践指南。

Conclusion: 量化不仅仅是压缩技术，而且是塑造现代FL性能、鲁棒性和实用性的基本系统组件。
...
TL;DR: Geoalign通过校正方向不一致的滚动来提高在线LLM RL的稳定性。


<details>
  <summary>Details</summary>
Motivation: 在线强化学习在将大型语言模型与奖励信号对齐方面得到了广泛应用，但训练可能在噪声或错误的奖励下不稳定。

Method: Geoalign，一个轻量级的插件，用于迭代策略优化中的滚动策略优化。Geoalign通过形成提示内偏好对、学习每个滚动隐藏状态上的在线投影器以及通过检测方向不一致的滚动来校正它们与批次共识原型之间的角度偏差。

Result: 在对话对齐和数学推理中，Geoalign提高了最终性能并减少了训练振荡，优于PF-PPO、PAR、PODS和Seed-GRPO。

Conclusion: 这些结果表明，潜在的定向共识是有效在线LLM RL的可靠性信号。
...
TL;DR: 本研究探讨了使用一致预测方法量化数值天气预报中的不确定性，并与其他方法进行了比较


<details>
  <summary>Details</summary>
Motivation: 量化不确定性在概率预测和数据同化中的重要性

Method: 使用一致预测（CP）方法来量化不确定性，并与传统方法进行比较

Result: 结果表明CP在简化大气模型中具有优势和局限性，可以补充常见的基于集合的不确定性量化方法

Conclusion: 一致预测方法在量化数值天气预报中的不确定性方面具有潜力
...
TL;DR: 本研究为JEPA世界模型提供了泛化理论，揭示了潜在预测模型的权衡。


<details>
  <summary>Details</summary>
Motivation: 为了深入理解基于JEPA的世界模型，并建立其泛化理论。

Method: 将JEPA预训练视为条件谱图学习问题，并建立了JEPA预训练错误与下游规划遗憾之间的联系。

Result: 提出了一种基于JEPA的世界模型的有限样本泛化界限，揭示了近似误差和样本误差之间的固有权衡。

Conclusion: 为JEPA世界模型提供了理论上的见解，比较了潜在预测模型与输入级预测方法的优缺点。
...
TL;DR: 提出了一种基于训练的框架，有效提高了MLLMs在医学视觉问答中的校准。


<details>
  <summary>Details</summary>
Motivation: 多模态大型语言模型（MLLMs）在医学视觉问答（VQA）中往往产生过度自信的输出，而现有的基于语言的置信度校准方法并未考虑医学图像理解的跨模态性质。

Method: 提出了一种基于训练的框架，通过组合损失函数微调MLLMs以提高其校准，该函数结合了Brier风格的校准项、防止置信度向极端值塌陷的锚定正则化器、对比图像文本对齐项和基于KL的模型稳定项。对齐信号来自一个2x2因子扰动设计，该设计交叉图像存在与文本完整性，探究模型对视觉模态输入和语言先验的依赖程度。最后，使用top K KL散度正则化器在微调期间保护模型的回答能力。
...
Conclusion: 该方法有效提高了MLLMs在医学视觉问答中的校准，并优于现有方法。
...
TL;DR: 通过隐式辛积分器训练HNN，提高样本效率和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 通过将物理先验集成到神经网络中，Hamiltonian神经网络（HNNs）通过学习系统的哈密顿量来提高泛化能力和样本效率。

Method: 探索使用隐式辛积分器的HNN模型在噪声轨迹观察下的训练方法，采用预测-校正的常微分方程求解器和定点迭代来减轻隐式时间步进的计算成本，并使用后处理方法改进学习到的哈密顿量。

Result: 在非分离、混沌系统上展示了数值优势，包括系统识别和能量保存，以及高效的计算和内存复杂度。

Conclusion: 提出了一种基于隐式辛积分器的HNN训练方法，该方法在提高样本效率和泛化能力方面具有优势。
...
TL;DR: 在能源交易中，结合多种市场特征可以显著提高决策效果。


<details>
  <summary>Details</summary>
Motivation: 研究能源交易决策中状态表示的重要性，特别是在水电站泵储能套利环境中。

Method: 使用固定Double DQN智能体在HydroDam环境中进行实验，比较不同市场特征对决策的影响。

Result: 发现结合绝对特征、相对特征和预测特征可以显著提高决策效果。

Conclusion: 状态表示在存储交易强化学习策略设计中至关重要，需要综合考虑价格规模、近期相对价格环境和短期预测信息。
...
TL;DR: CIRCLE是一种新的冷启动类增量学习方法，在多个数据集上表现优异。


<details>
  <summary>Details</summary>
Motivation: 冷启动示例无关类增量学习需要在没有重放、外部预训练或大量初始任务的情况下学习不断增长的类别集。

Method: CIRCLE使用固定的双向二维水库特征提取器，并结合流线性判别分析头进行分类。

Result: 在CIFAR-100、TinyImageNet、ImageNet-Subset和ImageNet-1k等多个数据集上，CIRCLE在10-20个任务分割上具有竞争力，在50、100和500个任务分割上显著优于其他冷启动方法。

Conclusion: CIRCLE在多个数据集上表现出色，显著优于其他冷启动方法，且训练速度更快。
...
TL;DR: 提出了一种改进的重球Q学习算法，证明了其收敛性，并分析了其加速收敛的条件。


<details>
  <summary>Details</summary>
Motivation: 提出一种改进的重球Q学习算法，并建立其收敛性。

Method: 基于切换线性系统（SLS）表示的Q学习算法和关联切换族的相关谱半径（JSR）进行分析。

Result: 证明了改进的重球Q学习算法的收敛性，并确定了其比标准Q学习更快收敛的条件。该方法也被扩展到具有线性函数近似的Q学习，并推导出类似的收敛和加速陈述。

Conclusion: 该研究为Q学习提供了新的见解，并可能提高强化学习的效果。
...
TL;DR: 提出了一种新的提升建模方法，在公共数据集和商业数据集上取得了显著的性能提升。


<details>
  <summary>Details</summary>
Motivation: 提升建模在估计个体治疗效果（ITE）时面临双重挑战：灵活利用组间相似性以增强判别能力，以及在不可观测混杂因素场景下的去偏。

Method: 提出交叉头部注意力提升网络（CHAUN）和鲁棒对抗性逆倾向得分（RA-IPS）方法。CHAUN利用共享特征嵌入和交叉头部注意力机制动态整合处理特定和控制特定的表示，增强组间相关性建模。RA-IPS在约束不确定性集合内对抗性地优化倾向得分权重，以减轻不可观测变量的偏差。

Result: 在公共数据集（CRITEO-UPLIFT，LAZADA）和商业电子商务数据集上的实验表明，CHAUN在QINI得分上相对于最先进的提升模型具有优越性，相对提高了25.6%。RA-IPS进一步增强了鲁棒性，在不可观测混杂因素下优于标准IPS 5.4%。结果验证了所提出方法在现实世界因果推断任务中的有效性。

Conclusion: 所提出的方法在估计个体治疗效果和去偏方面具有有效性，在现实世界因果推断任务中表现出色。
...
TL;DR: KAN在流体动力学代理建模中表现出色，但性能略逊于MLP，GNN需要更长时间训练。


<details>
  <summary>Details</summary>
Motivation: 评估Kolmogorov Arnold网络（KAN）在流体动力学代理建模方面的性能，并与深度多层感知器（MLP）和图神经网络（GNN）进行比较。

Method: 在预测亚音速和跨音速翼型表面压力分布的任务中，对KAN、MLP和GNN进行性能比较。

Result: KAN在预测整个压力系数方面表现出良好的性能，但在某些情况下性能略逊于MLP。GNN在最佳性能下需要更长的训练时间。KAN的复杂度通常低于MLP和GNN，但训练不稳定，性能高度依赖于超参数优化。

Conclusion: KAN在流体动力学代理建模方面表现出良好的性能，但与MLP相比存在一些局限。
...
TL;DR: Developed fTNN for fractional Laplacian problems; achieved high accuracy; improved over existing methods.


<details>
  <summary>Details</summary>
Motivation: Developing a deterministic tensor neural network subspace method for solving fractional Laplacian problems.

Method: Geometry-adapted integration split, Gauss-Jacobi quadrature, Gauss quadrature, deterministic angular quadrature, boundary-singularity-aware trial functions, spatiotemporally separable neural network.

Result: High accuracy on tested benchmarks, substantial improvement over existing methods.

Conclusion: fTNN is an effective and accurate method for solving fractional Laplacian problems.
...
TL;DR: 提出了一种新的模型辅助采样框架，用于解决机器学习中随机梯度估计的方差问题，实验结果表明该方法在多个基准数据集上取得了性能提升。
...
Method: 提出了一种模型辅助采样框架，通过调查抽样理论解释小批量梯度，将数据集视为固定有限总体，梯度视为基于样本的估计。

Result: 在合成和六个基准数据集上的实验结果表明，在71-86%的实验中取得了性能提升，特别是在我们的基准中的中等规模输入空间中。与基线估计器相比，使用动量优化器（如AdamW）的所提出的估计器在大约一半的训练周期内实现了明显更好的泛化。

Conclusion: 所提出的方法提高了效率，同时没有改变现有优化器的动态，在多个实验中取得了性能提升。
...
TL;DR: VLM-PBRS框架通过从视觉语言模型中学习势函数，提高了强化学习在稀疏奖励环境中的样本效率。
...
Result: 在Meta-World和Franka Kitchen环境中进行了实证验证，证明了改进的样本效率和对抗奖励操纵的鲁棒性。

Conclusion: VLM-PBRS框架通过利用视觉语言模型，有效地解决了稀疏奖励问题，提高了强化学习样本效率。
...
TL;DR: 提出了一种新的ETGNN解释方法，该方法通过分析整个信息流，提高了解释的可解释性和准确性。


<details>
  <summary>Details</summary>
Motivation: 为了克服现有ETGNN解释方法的局限性，我们提出了一种新的归因方法，该方法分析了所有与事件相关的变量中的整个信息流。

Method: 该方法基于最近的标准相关度度量（NRM）框架，能够显式量化来自事件嵌入的信息流以及通过事件诱导变量的信息流。此外，它还确保了跨层的潜在变量的可比性，并支持事件之间交互的高阶分析。为了处理ETGNN的架构复杂性，我们通过模块分解程序扩展了NRM框架，该程序有助于系统地构建复杂神经架构的相关结构。

Result: 在流行病追踪和社会动态的两个合成数据集以及政治事件网络的现实世界数据集上评估了我们的方法。我们的定性和定量实验表明，我们的方法在产生更可解释的解释的同时，始终优于现有的解释方法。

Conclusion: 提出的方法在解释ETGNNs方面取得了显著的成果，为理解和分析复杂事件信息流提供了新的视角。
...
TL;DR: Graph neural networks在各个应用领域取得显著进展，但仍面临挑战。


<details>
  <summary>Details</summary>
Motivation: Graph neural networks在数据具有关系结构的情况下，已从一种小众的表示学习技术转变为默认的模型类别。

Method: 通过调查组织该领域，围绕单一设计空间，从共享的基本原理中推导出光谱和空间公式，并将表达能力与Weisfeiler-Leman层次结构联系起来，明确说明当前架构可以和不能分离的内容。

Result: 对十二个应用领域进行了研究，包括推荐和社会网络、知识图谱和语言模型集成、药物发现和分子属性学习、医疗保健和神经科学、计算机视觉、交通和城市计算、电力和可再生能源系统、无线和第六代网络、欺诈和网络安全、工业预测、材料科学和气候建模。对于每个领域，我们指定了图构建选择及其成本，确定了主导的架构家族及其原因，并将报告的收益与弱基线或有利拆分的艺术效果区分开来。跨领域比较揭示了重复出现的模式：异配性和规模几乎在所有地方都削弱了相同的模型，时间图仍然比其静态对应物更难，而在公共排行榜上名列前茅的架构很少是那些达到部署的架构。我们将过度平滑、过度压缩、鲁棒性、分布偏移、公平性和可解释性视为决定采用的限制条件，而不是最后的清单。

Conclusion: Graph neural networks在各个应用领域都取得了显著进展，但仍存在一些挑战，如过度平滑、过度压缩、鲁棒性、分布偏移、公平性和可解释性等。
...
TL;DR: 研究了一种可解的GAN训练模型，揭示了信号增强机制，并通过实验验证了其有效性。
...
Method: 扩展多特征判别器设置到类依赖、相关和非零均值的潜在结构。

Result: 证明了随机微观训练过程在高维极限下收敛到由有效协方差控制的确定性常微分方程。
...
TL;DR: RSPC语料库在考虑关系背景的NLP任务中建立基准，支持转向情境感知的心理健康建模。


<details>
  <summary>Details</summary>
Motivation: 在自然语言处理（NLP）中，心理健康状况通常被视为孤立现象，没有人际交往的背景。

Method: 使用Reddit上关于异地恋的帖子来捕捉心理健康困扰和相关的情感触发因素。引入关系压力和精神性疾病语料库（RSPC），包含经精神病学家标注的诊断类别，包括最常见的情绪障碍（焦虑和抑郁）、关系压力触发因素和关系阶段指标。在多标签疾病分类、关系触发检测和时间阶段预测任务上对七个微调的transformer模型和五个大型语言模型进行基准测试。

Result: 在不同模型家族之间存在明确的任务相关差异，Claude-3-Haiku在疾病分类方面表现最佳（Macro-F1 = 0.538），而GPT-4o在关系触发检测方面表现最强（Macro-F1 = 0.519），表明模型能力具有独特性。焦虑障碍与长期关系不确定性之间存在强烈关联。RSPC为考虑关系背景的NLP任务建立了基准，支持从以个人为中心的心理健康建模转向情境感知的心理健康建模，从而捕捉困扰的社会和时间动态。

Conclusion: RSPC建立了考虑关系背景的NLP任务的基准，支持从以个人为中心的心理健康建模转向情境感知的心理健康建模，从而捕捉困扰的社会和时间动态。
...
TL;DR: 本研究提出了一种检测操纵性和欺骗性赌博广告的新方法，并提出了实际应用的可能性。
...
Method: 引入了一个新的赌博相关广告数据集，该数据集从两个广泛使用的社交媒体平台Instagram和Reddit收集而来，并对操纵性和欺骗性广告实践进行了人工标注。

Result: 提出了一个框架，该框架可以应用于浏览器插件，警告用户关于操纵性赌博广告，以及自动网络爬虫，帮助监管机构在线监控和检测此类推广。

Conclusion: 该研究有助于提高对操纵性和欺骗性赌博广告的检测能力，并可能对用户的心理健康产生积极影响。
...
TL;DR: 本研究提出了一种从观察到的解数据中学习控制方程的新方法，通过引入 Hausdorff 距离和建立识别界限，解决了样本复杂性问题。


<details>
  <summary>Details</summary>
Motivation: 为了解决从观察到的解数据中学习控制方程的基本挑战，特别是当从多个解观察中唯一且稳定地识别出真实 ODE 的理论条件尚未得到充分发展，且相关学习任务的样本复杂性的定量分析在文献中不存在的问题。

Method: 引入了解集上的 Hausdorff 距离作为比较微分方程的自然度量，并建立了跨广泛类别的结构方程的控制 ODE 识别界限，包括从线性 ODE 到具有 Lipschitz (Hölder) 连续向量场的非线性类别。使用此度量，对相关 ODE 类别推导了度量熵估计，并分析了样本复杂度界限。

Result: 推导了控制 ODE 的识别界限，并量化了需要多少个解观察来可靠地恢复控制方程。

Conclusion: 本研究为从观察到的解数据中学习控制方程提供了一种新的方法，有助于解决样本复杂性问题，并为进一步的理论研究和实际应用奠定了基础。
...
TL;DR: 提出了一种新的工作搜索查询生成方法，实验验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 为了解决工作搜索平台无法捕捉候选人档案高维复杂性的问题，提出了RLAIF框架来生成可移植的工作搜索查询。

Method: 提出了一个端到端的RLAIF框架，用于生成可移植的工作搜索查询，并进行了综合的实证实验。

Result: 实验结果表明，对于无批评优化器，性能主要取决于鲁棒的奖励塑造，而算法的具体选择影响不大。引入确定性、基于规则的奖励下限可以纠正对逐字复制的奖励，从而在跨家族评估中实现+0.147的质量提升。训练时间奖励模型通过2.4倍放大了性能提升，证实了训练成功主要依赖于执行奖励塑造纪律，而不是选择替代优化器。

Conclusion: 该研究提出了一种新的工作搜索查询生成方法，并通过实验验证了其有效性。
...
TL;DR: 本研究提出了一种多保真度迁移学习框架，用于基于导波的结构健康监测，以实现工程结构损伤的早期诊断。


<details>
  <summary>Details</summary>
Motivation: 为了解决在工程结构中实现基于导波的结构健康监测（GWSHM）的挑战，如标记实验数据的有限可用性和生成大规模高保真模拟数据集的高计算成本。

Method: 研究采用了多保真度迁移学习框架，结合了轻量级基于物理的模拟、基于卷积自编码器的深度特征学习、前馈神经网络和有限的实验测量。

Result: 模型在损伤定位和尺寸测量方面表现出优异的性能，$R^2$得分超过0.93和0.99，并证明了其泛化能力。

Conclusion: 该研究提出的多保真度迁移学习框架为工程结构中的损伤定位和尺寸测量提供了准确、计算效率高且实用的解决方案。
...
TL;DR: GEQ是一种有效的在线优化框架，与Blackwell可接近性等价，可与其他在线学习框架进行有效转换。


<details>
  <summary>Details</summary>
Motivation: 分析梯度平衡（GEQ）在线优化框架，并探讨其在在线学习领域的地位。

Method: 通过算法分析，将GEQ与Blackwell可接近性进行等价，并建立GEQ与其他在线学习框架之间的联系。

Result: 证明了GEQ与Blackwell可接近性等价，并建立了GEQ与其他在线学习框架之间的联系，同时识别了GEQ的必要和充分条件，并建立了不同GEQ概念之间的降低关系。

Conclusion: GEQ是一种有效的在线优化框架，其在算法层面上与Blackwell可接近性等价，并且可以与其他在线学习框架进行有效转换。
...
TL;DR: 通过使用覆盖感知采样技术和幻觉预测器，可以有效地检测和缓解世界模型中的幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 现代生成式世界模型渲染出越来越逼真的动作可控的未来，但它们经常出现幻觉：在偏离真实动态的同时，rollouts仍然视觉流畅。

Method: 引入MMBench2，一个包含427小时、210个任务的视觉世界建模数据集，并训练一个350M参数的世界模型。识别三种不同的幻觉模式：感知、动作边缘化和场景偏离。开发三种信号来预测模型将失败的地方。开发覆盖感知采样技术来关闭训练时间内的覆盖差距。将幻觉预测器作为好奇心奖励用于有针对性的数据收集。

Result: 发现世界模型中的幻觉本质上是数据覆盖问题，用于检测它的相同信号也可以用于缓解。

Conclusion: 通过使用覆盖感知采样技术和幻觉预测器，可以有效地检测和缓解世界模型中的幻觉问题。
...
TL;DR: 提出了一种新的神经网络求解器，通过直接输入PDE残差场来提高预测精度和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 为了提高神经代理模型在偏微分方程（PDE）参数到解的映射速度和准确性，提出了一种新的方法。

Method: 提出了一种名为错误条件神经网络求解器（ENS）的新方法，通过将PDE残差场作为直接输入到网络中，使网络能够读取其错误的空间结构并学习更新策略来迭代地纠正其预测。

Result: 在四个PDE家族中，ENS在大多数设置中达到了最高的预测精度，在湍流Kolmogorov流中的增益达到10倍，同时避免了混合方法的昂贵计算成本。

Conclusion: ENS在处理不良条件下的PDE时表现出色，具有更高的预测精度和更好的泛化能力。
...
TL;DR: ArBG is a new approach for efficient molecular system sampling, offering superior scalability and accuracy.
...
Method: Proposed Autoregressive Boltzmann Generators (ArBG), a novel autoregressive modelling framework.

Result: ArBG leads to significant improvements over flow-based models across all benchmarks, particularly in larger peptide systems. Introduces Robin, a 132 million parameter transferable model, improving zero-shot energy error on 8-residue systems by over 60%. 

Conclusion: ArBG provides a new paradigm for efficient sampling in molecular systems, significantly outperforming existing methods in terms of scalability and accuracy.
...
TL;DR: RiVER框架通过解决规模优势和频率优势，在无真实解决方案的情况下训练LLMs，显著提高了模型性能。


<details>
  <summary>Details</summary>
Motivation: 训练大型语言模型（LLMs）时，基于可验证奖励的强化学习（RLVR）通常依赖于真实答案来分配奖励，这限制了它们在真实解决方案未知任务中的应用。

Method: 提出了一个名为RiVER的框架，该框架在无真实解决方案的基于分数的优化任务上训练LLMs，使用确定性执行反馈作为连续值监督。通过调整奖励形状来解决两个关键挑战：规模优势和频率优势。

Result: 在12个AtCoder启发式竞赛任务上训练，并在Algorithm Engineering Benchmark（ALE-Bench）、LiveCodeBench和USACO上进行评估。RiVER将Qwen3-8B和GLM-Z1-9B-0414的ALE排名提高了8.9%和9.4%。更重要的是，尽管RiVER仅在基于分数的任务上训练，没有任何真实解决方案，但它也使LiveCodeBench和USACO等精确解决方案基准的骨干提高了2.4%和3.5%。
...
TL;DR: 本文提出一种概率框架，用于解决在线测试时自适应问题，模型能适应训练和测试分布差异。
...
Result: 模型能够在训练和测试分布可能存在差异的情况下，适应未标记数据。

Conclusion: 该框架为解决在线测试时自适应问题提供了一种有效的方法。
...
TL;DR: 提出了一种XMSE感知混合估计方法，在保持正则化优势的同时，有效应对核不匹配问题，具有较高的准确性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 为了解决经验贝叶斯（EB）估计在二阶近似风险方面可能不如最大似然（ML）的问题，该论文提出了一种XMSE感知混合估计方法。

Method: 提出了一种基于XMSE感知混合估计的方法，该方法在ML和EB收缩之间进行插值，并使用有限样本XMSE近似进行实现。

Result: 该方法在保持正则化优势的同时，在核不匹配的情况下退化为ML，并在所选权重处评估固定权重风险曲线，以实现第二阶 oracle 惩罚率。

Conclusion: 该论文提出的XMSE感知混合估计方法在保持正则化优势的同时，能够有效应对核不匹配问题，并具有较高的准确性和鲁棒性。
...
TL;DR: 研究局部质量行为，提出新的数学工具，为局部质量行为提供理论解释和实验验证。


<details>
  <summary>Details</summary>
Motivation: 研究全局目标（如KL散度和ELBO）在贝叶斯推理中测量分布差异的局限性，特别是它们未能直接捕捉到的局部质量行为。

Method: 引入并使用两个数学工具：1）质量指数，用于记录局部质量的多项式和对数衰减尺度；2）正则化扩展KL（RE-KL），一种在存在奇点成分的情况下可以形式化的集合局部化散度。

Result: 通过质量指数，证明了贝叶斯更新如何改变局部质量：1）幂对数似然因子明确地改变它；2）参数相关的支撑或其平滑软化可能通过保持参数值附近的质量量来改变局部尺度。使用局部RE-KL，证明了在两个KL方向下比较局部小球质量时的绝对、相对和方向不等式。
...
TL;DR: Ribbon is a new method for quantifying predictive uncertainty in complex models, offering scalability and efficiency.
...
TL;DR: 本文量化了解码方法、模型和基准之间的序列概率与正确性之间的关系，发现其在某些情况下可以预测正确性，但并非普遍适用。
...
Method: 在四个层面进行量化分析：解码方法、方法内的超参数、数据集中的提示-答案对、同一提示的重复响应

Result: 发现更高的序列概率通常可以预测固定数据集中提示-答案对的正确性；但这种方法不适用于解码决策；序列概率也不是同一提示响应正确性的良好指标

Conclusion: 这些发现阐明了何时可以期望解码提高正确性，并为解码、自洽性和无验证器自我改进提供了实用指导。
...
TL;DR: 本研究提出了一种迭代数据生成管道，用于隔离导致行为的级联线性特征，有效实现了对模型行为的解释和控制。


<details>
  <summary>Details</summary>
Motivation: 为了通过激活引导方法解释和控制模型行为，需要许多对比样本，这些样本清楚地展示了期望或不期望的行为。这些数据对决定了可解释性框架能够可靠地检测出导致行为的模型特征，因此也决定了将模型引导向或远离此类行为的可能性。

Method: 提出了一种迭代数据生成管道，用于隔离导致行为的级联线性特征。具体来说，我们展示了如何超越简单的样本二分法，而是隔离表现出特征程度的样本，这些特征与行为呈线性关系，从而更好地解耦特征。

Result: 我们发现，通过级联样本发现的恭维特征形成了线性可分子空间，并允许选择与基线方法相比更清晰地对应于期望行为的模型激活。我们还评估了它们的检测、确定性评分和鲁棒引导能力，发现它们在提供较低的计算需求的同时，提供了更多的可解释性保证。

Conclusion: 通过迭代数据生成管道，我们成功实现了对模型行为的解释和控制，并通过实验验证了其有效性。
...
TL;DR: 本文提出了一种更全面的评估代理性能的方法，超越了以准确性为中心的范式。


<details>
  <summary>Details</summary>
Motivation: 当基准的准确性饱和时，通常会将其退役并替换为更具挑战性的版本。

Method: 使用CORE-Bench Hard作为案例研究，测量代理性能的六个关键维度：构造效度问题、泛化能力、效率、可靠性、模型与支架的相对重要性以及人与代理协作的提升。

Result: 即使准确性饱和，这些维度也能提供有意义的见解。

Conclusion: 提出了一个比以准确性为中心的评估范式更严谨的替代方案。
...
TL;DR: AlgoEvolve is an LLM-driven evolutionary framework for algorithmic trading that outperforms human-designed instructions.


<details>
  <summary>Details</summary>
Motivation: Recent work shows that Large Language Models (LLMs) can act as semantic mutation operators for the evolutionary discovery of programs and proofs. Most current applications focus on static coding benchmarks. We extend this paradigm to algorithmic trading.

Method: We present AlgoEvolve, an LLM-driven evolutionary framework that generates, evaluates, and iteratively improves executable trading strategies. These strategies are expressed as Python code and evaluated through a rigorous testing protocol. We further introduce a meta-evolutionary outer loop that evolves the prompts guiding program synthesis in the inner loop.

Result: The system exhibits emergent regime-adaptive strategy logic, including autonomous shifts in trading rules. The heuristics consistently outperform initial human-designed instructions. The results demonstrate that LLM-based semantic evolution provides a viable approach for continual program synthesis in complex environments.

Conclusion: LLM-based semantic evolution provides a viable approach for continual program synthesis in complex environments.
...
TL;DR: 使用LLM进行大规模治理话语分析，发现治理形式影响实质性焦点，两种制度都表现出相似程度的参与不平等和社区碎片化


<details>
  <summary>Details</summary>
Motivation: 研究AI代理协议的治理结构

Method: 使用LLM驱动的比较管道进行大规模治理话语分析，结合自动化标注、神经主题建模和多层网络分析

Result: 分析了4,323个治理参与记录，发现治理形式影响实质性焦点，两种制度都表现出相似程度的参与不平等和社区碎片化

Conclusion: LLM辅助的方法可以推进对技术治理的实证研究，对设计更公平的代理AI标准有影响
...
TL;DR: 开发了一个框架，将监管和患者数据整合到一个知识图谱中，提高了精神药物信息的安全性和可审计性。


<details>
  <summary>Details</summary>
Motivation: 整合监管不良事件记录和患者叙述，以提供更全面的精神药物安全知识。

Method: 开发一个基于知识图谱的多智能体框架，统一来自Reddit、WebMD和FDA的记录。

Result: 知识图谱能够区分监管事实和患者经验，提高了信息可审计性。

Conclusion: 基于来源的整合是提高精神药物信息可审计性的途径。
...
TL;DR: DD-Elo是一种改进的象棋评级系统，通过整合走棋数据，更快地适应技能变化，提高了匹配的准确性。


<details>
  <summary>Details</summary>
Motivation: 传统的Elo评级系统在匹配比赛中存在响应延迟的问题，因为它只依赖于比赛结果，而忽略了游戏操作的细粒度质量。

Method: 提出了一种名为Drift-Diffusion-Enhanced Elo Rating System (DD-Elo)的新颖技能评估框架，该框架受到认知神经科学中的漂移扩散模型（DDM）的启发。通过将技能表达建模为决策过程，该模型将走棋级别的数据整合到评级调整中，以捕捉技能的快速波动。

Result: DD-Elo比Elo更快地适应技能变化，提供了一种可解释、高度响应和向后兼容的解决方案。

Conclusion: DD-Elo是一个有效的评级系统，可以改进象棋比赛的匹配过程。
...
TL;DR: 本文提出了一种用于监管AI代理执行高风险操作的治理模型。


<details>
  <summary>Details</summary>
Motivation: 为了避免AI代理执行后果严重、不可逆转的操作，如临床用药和生产软件部署。

Method: 提出了一种计算治理模型，该模型要求在关键操作点上提供独立证明的证据，以监管强大的自主行为者。

Result: 该模型允许代理在计划和推理方面保持完全自主，但在执行高风险操作时没有执行权限。执行取决于由独立权威来源证明的先决条件，并通过确定性策略评估。决策记录在可独立重新验证的防篡改日志中。

Conclusion: 该研究为治理强大AI代理提供了一种新的方法。
...
TL;DR: COrigami：一种基于人工智能的端到端计算折纸设计方法


<details>
  <summary>Details</summary>
Motivation: 解决在计算折纸领域中满足严格几何约束和主观视觉美学的物理艺术生成问题

Method: 提出COrigami，一个端到端的人工智能驱动管道，通过自然语言生成折痕图案，涉及语义棒图生成、基础包装计算、平面可折叠折痕图案求解、平面折叠折痕图案塑形以及使用强化学习驱动的自主美学评估循环进行模型优化

Result: 生成结构起始点供人类艺术家进一步扩展和塑造，实现算法优化与自主美学评价的结合，展示人工智能系统如何满足多目标物理约束，实现可靠的、基于数学的协同创作

Conclusion: COrigami在计算折纸领域为生成满足几何和美学要求的物理艺术提供了一种有效的方法，证明了人工智能在协同创作中的潜力
...
TL;DR: 该研究评估了工具增强型LLM代理在能源市场分析任务上的表现，并比较了不同LLM的性能。


<details>
  <summary>Details</summary>
Motivation: 能源领域的评估主要限于静态知识回忆，这对于需要实时数据检索、专业法规和市场知识以及在实际约束下的多步骤定量推理的领域来说是一个关键差距。

Method: 进行实证研究，评估工具增强型LLM代理在现实世界能源市场分析任务上的表现。评估环境包括243个专家定制的问题，涵盖三个类别：市场数据检索和分析、知识检索和解释、高级定量建模和决策分析。

Result: 评估了代理的响应，使用多维评估协议对方法正确性、答案准确性、属性一致性和来源有效性进行评分，并根据问题类型进行类别感知路由。比较分析了封闭源和开源LLM在高风险专业领域的模型能力和领域工具如何相互作用。

Conclusion: 该研究为能源领域提供了对工具增强型LLM代理性能的实证评估，并揭示了模型能力和领域工具在专业领域的相互作用。
...
TL;DR: 本文分析了多模态大型语言模型的评估方法，并指出当前评估方法的不足，强调了填补这些不足的重要性。
...
Result: 发现时间空间一致性、物理世界理解、多模态一致性和选择性注意力等差距

Conclusion: 填补这些差距对于衡量多模态智能的真正进展和揭示能力边界至关重要
...
TL;DR: OpenFinGym是一个统一的定量金融代理开发环境，旨在解决现有评估方法的不足。


<details>
  <summary>Details</summary>
Motivation: 虽然大型语言模型代理在定量金融工作流程中越来越多地被应用，但其评估仍然在孤立的任务中碎片化，而基准任务的金融相关性往往被忽视。然而，金融工作流程本质上是多阶段的，跨越相互依存的任务，如预测、策略构建、风险管理、和交易。现有的平台通常只关注单一任务，因此可以夸大代理的能力，并无法揭示泛化、真实市场交互和具有金融意义的决策中的弱点。

Method: 我们引入了OpenFinGym，这是一个统一的定量金融代理开发环境，涵盖了预测、市场生成、实时交易和欺诈检测，具有单一的执行和验证界面。OpenFinGym还提供了一个自动化的任务构建管道，将定量金融出版物转换为可执行的任务包；一个容器化运行时，具有主机端验证器服务，支持可扩展的代理部署，并防止运行时训练-测试泄漏；一个低延迟的数据流设计交易引擎；延迟解析支持长期和事件市场预测；以及SFT和RL后训练的集成。

Result: OpenFinGym提供了一种统一的方法来评估定量金融代理，涵盖了多个任务，并具有自动化任务构建和低延迟数据流设计等特点。

Conclusion: OpenFinGym为定量金融代理的开发和评估提供了一种新的方法，有望提高代理的能力并揭示其弱点。
...
TL;DR: 加速回报论无法解决科学发现的根本问题，QES提供了解决方案。


<details>
  <summary>Details</summary>
Motivation: 分析技术进步的加速回报论及其对科学发现的影响。

Method: 数学解释和比较分析

Result: 指出加速回报论在执行和基础设施能力方面的适用性，但无法解决科学发现的根本问题。提出定性推理引擎（QES）作为解决方案。

Conclusion: 加速回报论有助于解释定量能力的加速，但QES是解决科学发现核心问题的必要补充。
...
TL;DR: 提出了一种新的几何感知蒙特卡洛树搜索框架，用于解决组合几何中的极值问题，取得了新的计算结果。


<details>
  <summary>Details</summary>
Motivation: 研究组合几何中的某些极值问题，这些问题涉及满足严格全局几何约束的点在n×n网格中的配置。

Method: 提出了一种几何感知蒙特卡洛树搜索（MCTS）框架，通过增量更新可行动作空间来严格执行几何约束。

Result: 在五个问题中的六个上建立了新的计算结果，特别是在Max-N3IL上找到的大约1.8n的配置，在Smallest Complete Set问题中找到的大约0.95n的配置。
...
TL;DR: 提出了一种基于代理的电动公交车队调度框架，以解决运营中的复杂协调问题，并评估了其在实际案例中的效果。


<details>
  <summary>Details</summary>
Motivation: 为了解决电动公交车队运营中各种因素（如服务可靠性、电池状态、充电器可用性、电价、路线能源不确定性和车辆到电网（V2G）机会）之间的持续协调问题，提出了一种基于代理的聚合器框架。

Method: 提出了一种基于代理的聚合器框架，该框架结合了基于优化的电动公交车调度模型和监督代理，用于扰动检测、电价调整和调度评估。

Result: 结果表明，代理聚合可以支持自适应车队-电网协调，通过保持可行的调度、选择性激活重新优化以及提高充电和V2G灵活性的使用。然而，也揭示了关键权衡：当围绕以利润为导向的定价配置时，降低运营复杂性的相同代理能力可以从PTO中提取价值。

Conclusion: 代理聚合器可以成为管理电动公交车V2G运营的有用工具，但在公共车队环境中部署需要透明的协调模式、可审计的电价设置和明确的价值共享规则。
...
TL;DR: 监督机器学习系统无法达到符号逻辑推理的严谨性。


<details>
  <summary>Details</summary>
Motivation: 探讨逻辑推理缩放定律的极限，即数据驱动机器学习系统是否能通过增加训练数据和训练时间达到与符号级三段论推理相同的水平。

Method: 理论分析和实验验证，包括对Euler Net的实验，以及挑战ChatGPT GPT-5确定三段论语句的满足性。

Result: 监督机器学习系统无法达到符号逻辑推理的严谨性。

Conclusion: 监督机器学习系统无法达到符号逻辑推理的严谨性，因为训练数据无法区分所有有效的三段论推理类型，且端到端映射引入了神经组件之间的矛盾训练目标。
...
TL;DR: 提出MKG-RAG-Bench基准测试，用于评估MKG-RAG中的检索挑战，为多模态知识图谱RAG系统提供基础。


<details>
  <summary>Details</summary>
Motivation: 针对现有基准测试中忽视多模态知识图谱检索挑战的问题，提出MKG-RAG-Bench基准测试。

Method: 构建MKG-RAG-Bench基准测试，包括跨领域多模态知识图谱和精确对齐的问答数据集。使用基于LLM的整理流程筛选低效用知识，生成具有精确监督的结构化查询。

Result: 验证了有效多模态检索对于端到端MKG-RAG性能的重要性，检索质量对生成结果有显著影响。

Conclusion: MKG-RAG-Bench为诊断当前限制和推进多模态知识图谱RAG系统提供了原则性的基础。
...
TL;DR: 该研究展示了计算认知科学中自动数据收集和理论发现的可行性。


<details>
  <summary>Details</summary>
Motivation: 提高AI在科学自动化中的应用，解决数据收集瓶颈。

Method: 使用基于代理的系统进行自动发现技术，通过众筹调查实验收集人类数据。

Result: 系统可以快速且可靠地从合成数据中恢复出真实理论，并通过三次独立的人类实验证明其有效性。

Conclusion: 自动数据收集和理论发现是计算认知科学中的可行方法。
...
TL;DR: 本文提出了一种运行时治理架构，用于支持跨时间问责的临床护理能力。


<details>
  <summary>Details</summary>
Motivation: 医学人工智能目前以孤立模型的形式组织，而临床护理需要持续跨时间的问责能力。

Method: 提出了临床人工智能技能和临床工具：一个用于注册、编排、保护和监控人工智能临床能力的运行时治理架构。

Result: 以骨质疏松症为例，展示了知识驱动、数据驱动和物理增强的技能如何在运行时治理下支持生命周期护理。

Conclusion: 本文提出了一种临床人工智能技能和运行时治理架构，以支持跨时间问责的临床护理能力。
...
TL;DR: 教程详细介绍了NeuraDock Agent的安装、运行和分析，为研究人员、开发人员和应用团队提供了从EEG文件到实时视觉认知负荷原型的透明路径。


<details>
  <summary>Details</summary>
Motivation: 提供NeuraDock Agent的安装、运行、分析和解释的教程，旨在帮助读者从EEG文件到实时视觉认知负荷原型的透明路径。

Method: 教程方法，包括安装、运行EEG预处理和质量控制、生成Alpha动态图、进行Rest/Task视觉认知负荷比较、运行公共迷你数据集分析、启动在线仪表板、从外部应用程序调用实时API以及使用LLM解释层来解释质量风险。

Result: 教程使读者能够完成一系列操作，包括EEG预处理、质量控制、Alpha动态图生成、视觉认知负荷比较、数据集分析、在线仪表板启动、实时API调用和解释质量风险。

Conclusion: 教程填补了离线到在线的差距，为研究人员、开发人员和应用团队提供了一个从EEG文件到实时视觉认知负荷原型的透明路径。
...
TL;DR: NeuraDock Agent通过分离EEG引擎和语言层，提高了EEG软件的可用性和可靠性。


<details>
  <summary>Details</summary>
Motivation: 提高科学软件的使用便捷性，特别是在低通道脑电图（EEG）领域。

Method: 提出NeuraDock Agent架构，将确定性本地EEG引擎与硬件感知语言层分离。

Result: NeuraDock Agent在三个层面进行了评估，包括结果一致性、数据边界和边界意识。

Conclusion: NeuraDock Agent支持硬件和实现感知的定位，作为校准EEG代理接受、认可或拒绝内容的实用机制。
...
TL;DR: 本文提出了一种框架，将人工智能系统解释为代理，并提出了关于表征主义和解释主义方法的准则，强调了信念、欲望及其预设命题结构的相互约束，以及机制可解释性的重要性。


<details>
  <summary>Details</summary>
Motivation: 为了提高人工智能系统的安全性，我们需要理解其目标或可靠地检测欺骗。

Method: 提出了一种框架，将人工智能系统解释为代理，并借鉴了激进解释的哲学传统和机制可解释性的工具。

Result: 提出了关于表征主义和解释主义方法的准则，并将每个准则与当前可解释性方法可以进行的测试联系起来。

Conclusion: 信念、欲望及其预设命题结构是相互约束的，一种只固定一个同时测量其他的方法会引入任何扭曲。这种整体性对于人工智能系统至关重要，因为它们可能不共享解释者的概念。然而，这也提供了杠杆作用：一个系统的态度约束其命题结构，这种结构约束可以归因的态度，机制可解释性可以帮助我们测量这两者。
...
TL;DR: PMDformer模型有效捕捉长期预测中的形状相似性。


<details>
  <summary>Details</summary>
Motivation: 解决LTSF中跨尺度差异导致的形状相似性建模挑战。

Method: 提出PMD、TRA和PVA模块，用于捕捉形状相似性、趋势恢复和跨变量关系。

Result: PMDformer在多个LTSF基准测试中表现出色。

Conclusion: PMDformer模型在稳定性与准确性方面优于现有LTSF基准方法。
...
TL;DR: 本研究表明，机器学习在早期检测乙型肝炎患者的肝硬化方面具有潜力，其中额外树模型表现出色。


<details>
  <summary>Details</summary>
Motivation: 早期检测肝硬化对避免并发症至关重要，而机器学习（ML）在提供精确信息用于诊断疾病方面已被证明是有效的。

Method: 从加州大学欧文分校的ML存储库中获取了2038名埃及患者的28个属性的数据集。在数据集上训练了四种机器学习算法来诊断乙型肝炎患者的肝硬化：随机森林、梯度提升机、极端梯度提升和额外树模型。

Result: 额外树模型在仅使用28个特征中的16个的情况下，在诊断肝硬化方面优于其他模型，实现了96.92%的准确率、94.00%的召回率、99.81%的精确率和96%的受试者工作特征曲线下面积。

Conclusion: 机器学习可以有效地用于早期检测乙型肝炎患者的肝硬化，并且额外树模型是一个很有前途的工具。
...
TL;DR: EvoOptiGraph通过数据-模型共同进化显著提高了LLMs在优化建模任务上的性能。
...
Method: 提出EvoOptiGraph框架，其中数据和模型共同进化，由模型弱点驱动。将每个混合整数线性规划（MILP）表示为属性二分图，并应用保留有效性的进化算子来生成结构多样化的实例。通过确定性编译和验证回译将演化的图转换为求解器代码和自然语言。训练分为两个阶段：在初始数据集上进行监督微调（SFT），然后进行带有可验证奖励的强化学习（RLVR），其中图推导出的弱点信号指导生成新的实例，以针对模型的失败。这形成了一个闭环，持续更新训练分布。

Result: 在六个公共数据集上的实证结果表明，EvoOptiGraph在准确性、可执行性和泛化方面显著优于更大的通用模型、代理方法和专业基线。这些结果表明，针对数据-模型共同进化是提高LLMs在优化建模任务上的有效策略。

Conclusion: EvoOptiGraph是一种有效的策略，可以提高LLMs在优化建模任务上的性能。
...
TL;DR: 提出了一种多层次验证和可追溯推理框架，提高了人工智能在望远镜调度中的可靠性和可执行性。


<details>
  <summary>Details</summary>
Motivation: 随着人工智能在望远镜调度中的逐步应用，人工智能决策在处理复杂多约束问题方面显示出优势。然而，其输出往往存在数据引用不一致、推理错误和非可执行决策等问题，限制了其在高可靠性观测任务中的应用。

Method: 提出一个多层次验证和可追溯推理框架，在执行前对人工智能生成的决策进行系统可靠性验证，并使推理过程具有明确的表示以支持可追溯的决策。该框架集成数据引用验证、逻辑一致性检查和观测与仪器约束验证，以过滤和纠正无效决策。它还引入了原子推理单元及其依赖关系，将调度决策表示为一系列相互关联的推理步骤，支持错误定位和事后分析。

Result: 实验表明，该框架提高了人工智能调度的可执行性和可靠性，并减少了瞬时机会的损失。特别地，反馈纠正和推理步骤的结构化验证增强了修复和阻止错误决策的能力，尤其是在复杂场景中。与纯人工智能方法相比，框架增强方法在保持灵活性的同时，显著提高了可靠性和可执行性。这些结果证明了将人工智能应用于高可靠性天文观测调度的一种可行且可验证的途径。

Conclusion: 该框架为人工智能在望远镜调度中的应用提供了可靠性和可执行性的保证，为高可靠性天文观测调度提供了可行且可验证的途径。
...
TL;DR: Automated email dispatch system using LLMs for efficient organization communication.


<details>
  <summary>Details</summary>
Motivation: Address the issue of manual email handling in large organizations, leading to productivity losses and stress.

Method: Agents querying large language models (LLMs) to analyze email contents and route them to relevant student groups.

Result: Automated email dispatch system using LLMs, improving information flow and reducing workload.

Conclusion: The proposed system automates email dispatch based on content, enhancing productivity and reducing cognitive load.
...
TL;DR: 本文提出了一种结合LLMs、统计技术和人机协作的新方法，以改进公共服务机构的服务反馈分析。


<details>
  <summary>Details</summary>
Motivation: 提高公共服务机构，特别是税务机关对服务反馈的分析能力，这对于建立信任和遵守法规至关重要。随着反馈量的增加，识别新兴的服务质量问题以及在不同群体之间的潜在差异变得越来越具有挑战性。传统的做法通常依赖于人工审查或静态的专家定义指标，这限制了可扩展性和捕捉文本反馈中复杂模式的能力。

Method: 提出了一种新的方法，该方法结合了大型语言模型（LLMs）、统计技术和人机协作，以改进多语言客户反馈分析。该方法结合了微调的、量化的LLMs和专家监督，以产生准确、计算效率高和上下文感知的分析。

Result: 该方法通过相似性分析和经验丰富的税务官员的评估进行了评估，表明其与基线模型相比，与专家判断的吻合度更强。通过采用人机协作框架，该方法降低了LLM的虚构性，同时提高了生成的见解的可靠性和相关性。结果证明了将LLMs与人类专业知识相结合，以支持公共部门组织可扩展、基于证据的决策制定的实用性。这项工作有助于开发负责任的AI系统，通过更有效地分析多语言客户反馈，提高服务质量、响应性、公平性和公众信任。

Conclusion: 该方法为公共部门组织提供了有效的工具，以更准确地分析服务反馈，从而提高服务质量、响应性和公众信任。
...
TL;DR: 我们提出了一种自动形式化流程，将自然语言策略转换为形式化策略，提高了高风险领域代理的安全性。


<details>
  <summary>Details</summary>
Motivation: 在高风险领域，代理的安全性需要形式化的策略执行，但大多数现有方法要么依赖于概率性护栏（微调的分类器、基于提示的引导），这些方法无法提供正式的保证，要么依赖于手工编写的符号执行，这种方法无法扩展到真实策略规范的广度。

Method: 我们提出了一种自动形式化流程，该流程使用基于LLM的生成器-评论家循环将代理提示、MCP工具描述和自然语言策略文档转换为形式化验证策略。结果策略用Cedar策略语言编写。

Result: 在MedAgentBench基准测试中，我们的自动形式化策略比先前工作中的手工编写的符号执行覆盖了更多的源自然语言规范。

Conclusion: 这种方法提供了一种将自然语言策略转换为形式化策略的有效途径，提高了策略执行的安全性。
...
TL;DR: SkillDisCo框架通过重用程序技能提高了代理的性能。


<details>
  <summary>Details</summary>
Motivation: 为了解决重复任务导致的不必要的推理成本和长的执行轨迹，我们研究了在有限状态机定义的场景中，如何表示和重用程序技能。

Method: 我们提出了SkillDisCo框架，从成功轨迹中提取可重用的PFSM子图，并编译成可调用、可执行和可验证的程序技能。

Result: 在ALFWorld和WebArena上的实验表明，SkillDisCo提高了成功率和减少了代理的回合数，证明了将共享经验表示为可重用执行结构的好处。

Conclusion: SkillDisCo框架有效地提高了代理在重复任务中的性能，并证明了程序技能表示的重要性。
...
TL;DR: 提出了一种新的后训练流程，显著提高了LLMs的推理和人类偏好能力，并提供了可重复的后训练方法。


<details>
  <summary>Details</summary>
Motivation: 为了提高大型语言模型在推理和人类偏好方面的能力，以及促进社区的可重复性和轻量级模型优化。

Method: 提出NebulaExp，一个基于Qwen3-8B-base的完全透明的、消融驱动的后训练流程，包括通用指令模型和复杂推理专用模型。构建了3.84M的多源SFT样本语料库和200K的可验证RL候选池，设计了端到端的数据处理堆栈，包括响应蒸馏、多维度交叉验证过滤、细粒度难度分级、任务分类和多样性感知采样。

Result: 在指令分支上，NebulaExp-Ins-SFT将Qwen3-8B-nothink的基准分数从55.01提高到60.99。GRPO强化学习进一步将平均分数提高到61.85。在推理分支上，中等难度的GRPO RL将平均推理分数从73.88提高到75.17。通过单教师和多教师OPD（MOPD）解决了RL对任务验证器的依赖，仅使用4K指令遵循样本，在IFEval上比RL基线高出3.26分，平均整体增益为+4.43；MOPD融合了四个领域专家教师，仅使用10K样本，平均性能比基模型提高4.18。

Conclusion: 为8B规模的LLMs提供了一种可完全重复的后训练方法，并全面分析了指令遵循、数学推理、代码生成和一般知识之间的能力权衡。
...
TL;DR: 本文提出了一种名为LeanGuard的轻量级安全护栏，它不使用推理，但在公共基准测试中表现良好。


<details>
  <summary>Details</summary>
Motivation: 为了筛选提示或响应，最近的安全护栏方法在作出判断之前生成思维链（CoT）。这种设计遵循一个普遍的信念，即逐步推理可以提高决策质量。然而，CoT也使得护栏变得沉重且缓慢，因为模型必须在决定之前生成许多标记。这可能不符合安全护栏的实际部署方式。护栏有时不应该沉重且缓慢，并且通常在设备上运行，例如在实体机器人上。本文提出了一个问题：安全护栏真的需要推理吗？

Method: 在相同的语料库上训练了一个轻量级的双向编码器和推理护栏，然后在保持其他一切不变的情况下仅移除了推理。通过这种受控的同一基础比较，我们发现思维链并没有提高审查的准确性。我们称结果为LeanGuard。一个395M的标签-only编码器在公共基准测试中达到了平均F1值为82.90±0.26。它与一个建立在更大解码器上的推理护栏相匹配，而它只需要对最多512个标记的输入进行单次正向传递。这大约是推理计算量减少了100倍。我们进一步表明，这个标签-only编码器在训练标签噪声下保持鲁棒，并且在严格的假阳性率下比推理护栏保留了更多的召回率，因此更重的推理护栏也不是更鲁棒的选项。

Result: LeanGuard在公共基准测试中达到了平均F1值为82.90±0.26，比推理护栏减少了100倍的推理计算量。标签-only编码器在训练标签噪声下保持鲁棒，并且在严格的假阳性率下比推理护栏保留了更多的召回率。

Conclusion: 当前的护栏基准测试可能不足以奖励推理，而思维链对审查的必要性仍未得到证明。
...
TL;DR: 提出了一种基于KPN的CCGT故障诊断方法，在模拟泄漏故障检测中表现出色，适用于标签数据有限的实际情况。


<details>
  <summary>Details</summary>
Motivation: 为了解决联合循环燃气轮机（CCGT）故障检测中标签数据稀缺的问题，特别是当故障数据难以获取时。

Method: 提出了一种基于Kalman原型网络（KPN）的度量学习方法，该网络专门用于CCGT故障诊断。将类别原型的演变建模为动态系统中的潜在随机状态，以减少样本间差异并提高嵌入表示的鲁棒性。使用高保真Modelica动态模拟数据集进行实验，模拟了正常操作和渐进泄漏故障。

Result: 在模拟泄漏故障检测任务中，KPN在准确性和稳定性方面优于传统方法，如匹配网络、关系网络和MAML。该框架提高了训练收敛性和泛化能力，适用于标签数据有限的实际情况。

Conclusion: KPN是一种有效的CCGT故障诊断方法，能够提高故障检测的准确性和鲁棒性，适用于标签数据稀缺的实际情况。
...
TL;DR: LithoDreamer：首个物理信息世界模型框架，用于计算光刻，显著提高光刻性能。


<details>
  <summary>Details</summary>
Motivation: 半导体技术节点缩小，计算光刻对于确保产量和性能至关重要。然而，光刻是一个涉及掩模优化、光学成像、光刻胶曝光和显影的连续物理过程，现有模型无法捕捉。

Method: 提出LithoDreamer，第一个用于计算光刻的物理信息世界模型（WM）框架，将“布局-掩模-光刻胶图像-显影后图像（ADI）”管道定义为决策驱动的多步骤进化系统。LithoDreamer捕捉相邻状态之间的特征变化，以建模特定阶段的物理信息潜在空间，在其中控制过程干预探索并驱动后续状态转换。为了在不进行连续监督的情况下实现可解释的干预优化，我们提出了一种对比变分优化范式，该范式对比干预路径之间的潜在差异与变分进化约束，引导模型生成与实际光刻物理一致的进化。

Result: 实验表明，LithoDreamer在正向进化和逆向规划方面实现了最先进的性能。我们的光刻数据集在GitHub上公开可用。

Conclusion: LithoDreamer是计算光刻领域的一项重要进展，为提高光刻过程的产量和性能提供了新的解决方案。
...
TL;DR: 提出了一种新的模型来分析心室运动，该模型在预测心脏病风险方面优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 传统风险模型只使用少数从选定的心室阶段中导出的图像指数，而心磁共振成像（CMR）可以捕捉到关于心室结构和运动的丰富时空信息。

Method: 提出了一种编码双心室解剖结构和完整循环电影运动的潜在动态模型，使用心率感知神经网络常微分方程（ODE）动力学和基于图的网格自动编码器来重建解剖一致的3D+t心室运动。

Result: 将潜在得分添加到重新拟合的汇总队列方程中，将分层C指数从0.704提高到0.785，而七个已建立的心脏标志物的C指数为0.764。与无图和无ODE方法相比，所提出的模型在重建保真度、生成真实性和下游预测性能之间提供了最佳权衡。

Conclusion: 连续的全循环建模提供了比传统CMR总结更有信息的心脏表型，但在临床风险预测使用之前，需要在更具代表性的患者队列中进行外部验证。
...
TL;DR: AHOIS是一种多智能体AI科学家，通过苏格拉底助产术实现自主科学发现。


<details>
  <summary>Details</summary>
Motivation: 科学发现的自动化已经达到一个拐点，需要实现认知自主性，即能够根据证据构建、挑战和修改物理解释的能力。

Method: 引入AHOIS，一个多智能体人工智能科学家，将苏格拉底助产术嵌入闭环实验。一个物理批评智能体通过因果提问、约束检查、反例生成和证伪标准制定来质疑假设。

Result: 在真实的多模式光纤平台上评估AHOIS，该平台是一个具有复杂波变换、间接检测、环境漂移和多模态采集的高维系统。系统自主地提出了一个随机干扰编码假设，发现了任务自适应的稀疏测量策略，诊断了不同的故障模式，并将已发表的成像协议转换为一个在非原始配置上可执行的流程。

Conclusion: 这些结果表明，从工作流程自动化到基于证据的自我校正的自主发现，可以解决复杂物理环境中的问题。
...
TL;DR: EGG通过专家引导和分阶段分解，实现了高性能GPU内核的自动化生成，显著提高了LLM的性能。


<details>
  <summary>Details</summary>
Motivation: 为了降低大型语言模型（LLM）的指数级增长的计算成本，需要开发高性能的GPU内核。然而，内核的开发高度依赖于领域专家的手动调整。虽然基于LLM的方法在自动化内核生成方面显示出希望，但它们仍然难以同时实现正确性和高性能。这种限制主要源于缺乏特定领域的优化指导，阻碍了优化空间的有效探索。

Method: EGG（专家引导的代理框架）将专家优化原则纳入LLM的决策中。受专家工作流程的启发，将内核生成分解为两个层次阶段：1）算法结构设计，建立高质量的计算结构基础；2）硬件特定的调整，通过并行映射、张量瓦片和内存优化进行针对性的调整。这种分阶段分解定义了明确的优化目标，构建设计空间以实现渐进式优化。为此，设计了一种阶段感知的多代理协作机制，用于管理和确保稳定的优化轨迹。

Result: 在KernelBench和真实世界的工作负载上进行的实验表明，EGG比PyTorch平均快2.13倍，优于现有的基于代理和基于强化学习的的方法。

Conclusion: EGG通过将专家优化原则与LLM结合，实现了高性能GPU内核的自动化生成，在性能上优于现有方法。
...
TL;DR: ResilPhase：一种噪声鲁棒的加速框架，用于提高扩散模型的推理速度。


<details>
  <summary>Details</summary>
Motivation: 强大的扩散模型被其显著的推理延迟所阻碍。

Method: 提出了一种基于普通微分方程(ODE)空间的稳定宏观轨迹外推方法，并引入了无导数巴塞托拉格朗日外推器。

Result: ResilPhase，一种噪声鲁棒的加速框架，在FLUX.1-dev和HunyuanVideo上的实验表明，在激进加速比下仍能保持最先进的保真度。

Conclusion: ResilPhase能够有效地加速扩散模型，同时保持高保真度。
...
TL;DR: 研究了智能体如何选择经验来影响行为，提出了一种新的巩固机制，并通过实验验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 研究长期运行的智能体在行为塑造中，如何选择哪些经验继续影响行为，即使工作上下文已经卸载。

Method: 提出循环漂移协议，在卸载工作上下文的同时保持检索索引完整，并在长时间循环干扰下持续目标条件化行为。

Result: EVAF机制在目标持续性和卸载后的恢复方面表现最佳，每个200个事件只需要2-3个参数写入。机制控制显示选择性和激活可以分解为两个可控维度。

Conclusion: 选择性参数化巩固提供了与检索访问不同且互补的内存深度。
...
TL;DR: 该研究评估了40名健康成年人的心率变异性指数，发现时间域和非线性指数最适合准确表示心率变异性成分，并增强其临床和研究的相关性。


<details>
  <summary>Details</summary>
Motivation: 虽然心率变异性分析是心脏生理状态的关键指标，有助于疾病诊断，但在健康个体中关于心率变异性参数的研究仍然有限，没有黄金标准。

Method: 使用计算方法进行信号处理和数据分析，分析了时间、频率和非线性指数，以解决五个问题：正常性、稳定性、相关性、可重复性和一致性。

Result: 时间域和非线性指数，尤其是全局和低频（LF）指数，遵循正态分布，存在性别差异。大多数指数稳定，除了高频（HF）相关指数。高频相关指数的高相关性表明存在冗余，表明在研究中只需要一个。与Fantasia数据库的比较显示，大多数指数的误差小于10%，但女性中的SD2和SDNN大于15%。时间域和非线性指数表现出低研究间变异性，而频域指数表现出高变异性，限制了跨研究比较。

Conclusion: 该研究评估了40名健康成年人（20名男性，20名女性，年龄30-50岁）的心率变异性指数，以改善其临床应用。研究表明，时间域和非线性指数，尤其是全局和低频（LF）指数，遵循正态分布，存在性别差异。大多数指数稳定，除了高频（HF）相关指数。高频相关指数的高相关性表明存在冗余，表明在研究中只需要一个。与Fantasia数据库的比较显示，大多数指数的误差小于10%，但女性中的SD2和SDNN大于15%。时间域和非线性指数表现出低研究间变异性，而频域指数表现出高变异性，限制了跨研究比较。选择的指数（ApEn和IRRR、HRVi和SD2、MADRR或rMSSD）最适合准确表示心率变异性成分，并增强其临床和研究的相关性。
...
TL;DR: 提出能力前沿来量化LLM的真正能力，发现现有评估方法低估了LLM的能力，尤其是在异构数据分布下。


<details>
  <summary>Details</summary>
Motivation: 现有基准通常仅报告单个模型在单个运行中的准确率，这系统地低估了真实世界LLM的能力，特别是在异构数据分布下：(i)不同的模型根据其专业特长回答不同的问题，(ii)在预算有限的情况下，可以采样和选择性地保留多个生成。为了量化这种差距，我们引入了能力前沿：一组模型的最优选择（即通过一个算子）在各个成本水平上实现的最佳性能的Pareto前沿。我们的构建纠正了两种相反的偏差：来自单个模型评估的低估和来自对噪声样本取最大值的过高估计。我们研究了21个LLM，涵盖了16个广泛使用的基准，包括编码、推理、医学、事实性、指令遵循和代理任务，将能力前沿的性能与每个基准的最佳表现模型进行了比较。纠正单个模型评估产生54%的错误率降低；此外，纠正单个运行产生82%的提高，在85%的成本降低下匹配了最先进的准确性。为了补充这些实证结果，我们使用受控的概率模拟来表明，更高的查询主题熵会产生算子路由和最佳单模型性能之间性能差距的近似单调增加。我们的发现表明，集体LLM能力被大幅度低估，这对其在数据异构、多领域设置中的评估和部署有影响。

Method: 提出能力前沿，通过一组模型的最优选择（即通过一个算子）在各个成本水平上实现的最佳性能的Pareto前沿。使用21个LLM，涵盖16个广泛使用的基准，并与每个基准的最佳表现模型进行比较。进行受控的概率模拟来研究查询主题熵与性能差距之间的关系。

Result: 纠正单个模型评估产生54%的错误率降低；纠正单个运行产生82%的提高，在85%的成本降低下匹配了最先进的准确性。通过模拟表明，查询主题熵的增加会导致算子路由和最佳单模型性能之间性能差距的增加。

Conclusion: 集体LLM能力被大幅度低估，这对其在数据异构、多领域设置中的评估和部署有影响。
...
TL;DR: AgentX系统通过自动化和自我改进，提高了推荐算法迭代的效率，减少了人力依赖。


<details>
  <summary>Details</summary>
Motivation: 推荐算法迭代从工匠式、工程师依赖的过程向工业化研究循环转变，但这一转变仍受结构性执行瓶颈的阻碍：从想法到发布的周期仍然依赖于人类工程师生成假设、修改生产代码、启动A/B实验和归因在线结果。因此，创新与人员数量呈线性增长，而不是与证据、计算和累积的实验知识呈指数增长。

Method: 提出AgentX，一个生产部署的多智能体系统，从根本上重构了这一生产功能。AgentX作为一个自我进化的开发引擎，自主地生成、实施、评估和从推荐实验中学习，其规模和速度是任何手动工作流程都无法维持的。系统在一个闭环中协调四个紧密耦合的阶段。头脑风暴智能体从历史实验、系统架构、数据分析和外部分析中综合证据，生成排名靠前的、可执行的提案。开发智能体通过基于存储库的生成和多维可靠性验证将每个提案转换为生产就绪代码。评估智能体通过带有安全护栏的A/B判断进行安全的在线发布，将成功和失败都转化为结构化的知识资产。然后，利用进化层（SGPO）从执行轨迹中提取语义梯度更新，不断锐化智能体本身，使系统不仅自动化，而且自我改进。

Result: AgentX系统通过自动化和自我改进的方式，实现了推荐算法迭代的工业化，提高了创新效率，并减少了对人类工程师的依赖。

Conclusion: AgentX系统为推荐算法迭代提供了一个新的解决方案，通过智能体之间的协同工作，实现了从想法到发布的全自动化，提高了创新效率，并减少了人力成本。
...
TL;DR: TAVR-VLM通过R-CGA解决了诊断性幻觉问题，显著提高了手术AI的性能。


<details>
  <summary>Details</summary>
Motivation: TAVR计划需要细致的多模态推理，但将多模态大型语言模型（MLLMs）应用于高风险领域受到诊断性幻觉的严重阻碍，生成的文本缺乏解剖基础。

Method: 提出了一种名为TAVR-VLM的新框架，具有风险条件因果 grounding 注意力（R-CGA），该框架实现了一个模型内部的“风险$ightarrow$区域$ightarrow$单词”结构 grounding 路径。R-CGA将多模态输入压缩成一个因果风险瓶颈，将密集的视觉特征净化成一个全局风险掩码。在自回归生成过程中，一个支持投影的因果一致性目标约束了在风险定义的支持掩码内的标记级别 grounding。

Result: 在$	ext{M}^3	ext{TAVR}$上进行了评估，这是一个包含1,482名患者的综合队列，TAVR-VLM建立了一个新的最先进水平。它实现了AUROC 0.896，将CIDEr提高到0.936，并将幻觉率大幅降低到8.1%，从而提高了基于证据的手术AI的可解释性。

Conclusion: TAVR-VLM通过引入R-CGA，有效地解决了诊断性幻觉问题，提高了手术AI的可解释性和准确性。
...
TL;DR: 提出了一种合成临床笔记流水线和数据集，用于支持临床AI工具的开发，同时避免隐私风险。


<details>
  <summary>Details</summary>
Motivation: 在医疗保健领域，由于临床文档的敏感性，获取真实世界数据存在挑战。

Method: 提出了一种合成临床笔记流水线和数据集，该数据集通过模块化流水线生成，结合了结构化患者生成、半结构化患者旅程模拟和大型语言模型生成的非结构化临床笔记。

Result: 发布了包含70名合成患者及其20-50个临床笔记的数据集，支持临床AI系统的开发、测试和评估。

Conclusion: 该研究为临床AI工具的开发提供了支持，同时避免了与真实患者数据相关的隐私风险。
...
TL;DR: 提出了一种新的检索方法，通过保持模式同时提高属性检索的准确性。


<details>
  <summary>Details</summary>
Motivation: 在许多生产环境中，基于嵌入的检索通常不是所需的：给定一个表示细粒度模式的种子集，需要更多同时满足目标属性并保持该模式的项目。

Method: 提出了一种名为MO-DiT+HPPO的阶段性框架，包括原始序列预训练、多域度量有序续训、尾点质心微调和HPPO。度量有序训练将稀疏的在线检索标签转换为按低到高预测属性密度排序的在线模式轨迹，教会一个模型跨域的度量改进方向。HPPO通过使用在线交集度量对混合候选池进行标记并应用参考锚定的偏好优化，将生成的查询分布与真实在线目标对齐。Pareto对过滤器仅保留不降低相同模式纯度的获胜对，在不牺牲模式的情况下提高属性度量。

Result: 在四个属性域中，度量有序DiT在预训练的生成检索器上提高了交集度量，HPPO进一步提高了它，在八个域划分单元中有七个取得了显著收益，在最困难的划分单元上取得了微小的平局。度量预测器验证、顺序消融、CPT/SFT比较和候选策略消融表明了收益的来源。

Conclusion: MO-DiT+HPPO框架在保持模式的同时提高了属性检索的准确性。
...
TL;DR: ReTeX：一种基于参数扰动预测的多任务模型合并框架，能够消除参数干扰，提高模型性能和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 多任务模型合并旨在将多个特定任务的专家合并成一个统一模型，但静态合并始终存在参数干扰的问题。动态合并模型旨在弥合这一差距，但许多工作依赖于在推理时昂贵地存储和加载冗余的专家组件。

Method: 从任务专家的角度出发，将参数干扰视为合并过程中引入每个专家的参数扰动。表明这种参数扰动可以建模为仿射变换，可以近似为加性偏移。受此启发，提出了ReTeX框架，该框架预测这些偏移，以取消参数干扰并从单个合并检查点恢复任务-专家性能。为了在任务标识未知时恢复适当的专家，引入了一种基于SVD子空间签名的无路由任务标识符，该签名在推理之前离线计算。在推理时，标识符选择给定输入的子空间产生最小投影残差的任务。

Result: ReTeX在视觉和NLP领域恢复了超过95%的个体专家性能，同时显著提高了对未见任务的泛化能力。参数偏移预测导致了对分布外（OOD）任务的专家知识自适应插值。ReTeX自适应地插值已看到的专家知识来处理未见任务。

Conclusion: ReTeX是一种有效的多任务模型合并框架，能够有效消除参数干扰，提高模型性能和泛化能力。
...
TL;DR: Task insensitivity leads to weak OOD generalization, and Task-Perturbed NLL Optimization can improve it.


<details>
  <summary>Details</summary>
Motivation: Large language models' weak out-of-distribution generalization

Method: Task-Perturbed NLL Optimization

Result: Improved task sensitivity and OOD generalization

Conclusion: Task insensitivity is a key source of failure in OOD generalization.
...
TL;DR: CoT训练提高了prompt-action的质量，但未扩大CoT推理的优势。


<details>
  <summary>Details</summary>
Motivation: 为了研究思维链（CoT）训练实际上在提高什么，即模型是否通过生成的推理改变其行为，或者是否直接从提示中预测行为。

Method: 比较提示动作（不使用CoT预测动作）与CoT动作（使用CoT预测动作），并观察不同检查点的prompt-action质量。

Result: prompt-action质量显著提高，CoT训练没有扩大CoT推理的优势，反而有助于提高prompt-action的质量。后期检查点更少根据CoT修改动作，表明对提示的更大依赖。选择性掩码训练示例中的动作标记监督可以改善领域外泛化。

Conclusion: CoT训练主要提高了prompt-action的质量，而不是CoT推理的优势。
...
TL;DR: 研究提出了一种基于叙事的世界视觉注意力，通过相机规划解决动态3D环境中的视觉感知问题。


<details>
  <summary>Details</summary>
Motivation: 随着具身AI和世界模型在动态3D环境中的运用，视觉感知必须从被动地解释给定观察转变为主动决定观察什么。

Method: 提出了一种名为“观察先于移动”的相机规划框架，该框架将观察指定与运动执行分离。首先构建语义观察合同，将导演意图转换为可执行的视觉约束，然后进行蒙特卡洛视角搜索，寻找符合叙事和几何可行性的视角，最后应用语义轨迹定位将选定的视角连接成连续的、碰撞感知的、时间上连贯的相机运动。

Result: 实验表明，该框架提高了主观感知、意图一致性和轨迹质量，证明了在生成相机运动之前组织视觉注意力的重要性。

Conclusion: 研究提出了基于叙事的世界视觉注意力，通过相机规划在动态3D故事世界中解决视觉感知问题，为具身AI和世界模型在动态环境中的视觉感知提供了新的思路。
...
TL;DR: 本研究提出EWMs，以视觉反事实事件可视化来增强LLMs的推理能力。


<details>
  <summary>Details</summary>
Motivation: 探讨视觉反事实事件是否可以补充语言作为复杂思维机制。

Method: 提出爱因斯坦世界模型（EWM），该模型通过在推理轨迹中放置视觉-时间展开来增强LLMs的推理能力。

Result: EWMs可以扩展LLMs的工具调用能力，进入视觉思维实验领域。

Conclusion: 视觉反事实事件可视化在复杂思维中可能起重要作用，EWMs是利用这种可视化的有效方法。
...
TL;DR: AURORA-AI框架在动态环境下提高人工智能系统的性能和公平性。


<details>
  <summary>Details</summary>
Motivation: 现代人工智能系统在非平稳的计算、人口统计和运营条件下部署，静态资源分配策略会降低预测性能和公平性、可解释性等人性化属性。

Method: 提出AURORA-AI框架，该框架结合了Hamilton-Jacobi-Bellman反馈控制、基于Lyapunov的稳定性监控和公平感知复合效用，形成单一闭环策略。该框架在连续的时间模拟中进行评估，同时注入人口统计偏差冲击、渐进的概念漂移和突发的黑天鹅事件，并与五种已建立的控制器进行比较。

Result: AURORA-AI与静态基准相比，从黑天鹅事件中恢复更快，同时提高了预测性能、公平性和鲁棒性，减少了人口统计差异，并增加了Lyapunov稳定的操作步骤。

Conclusion: 公平感知的适应性编排基于稳定性理论，是实现具有鲁棒性的人性化人工智能部署的可行且具有理论依据的方法。
...
TL;DR: JERP通过联合学习经验规则和政策，提高了LLM智能体在多步交互环境中的决策性能。
...
Method: 提出了一种名为JERP的方法，该方法同时更新长期经验规则池和政策，并使用收集到的轨迹来优化策略和修订规则池。

Result: 在AlfWorld和WebShop上的实验表明，JERP在复杂交互任务中决策性能得到了一致的提升。

Conclusion: JERP通过耦合经验规则池和策略，有效地提高了LLM智能体的交互决策能力。
...
TL;DR: PAVE协议提高了LLM在RCA任务中的性能，但仍需解决无根据诊断问题。


<details>
  <summary>Details</summary>
Motivation: 为了支持对LLM代理能力的严格评估，研究人员提出了一种新的标注协议PAVE，以解决现有数据集在根因分析（RCA）任务中的根本缺陷。

Method: 提出了一种名为PAVE的逐步标注协议，通过利用已知干预措施从故障注入来重建因果传播路径，从而实现向前验证：从原因推理到效果，而不是从症状反向推断。

Result: 在11个前沿LLM上，平均只有20.7%的情况能够成功恢复确切的根因集。通过放宽标准，发现所谓的无根据诊断：代理在76.0%的情况下至少识别出一个正确的根因服务，但只有61.5%的情况将该服务定位在经过验证的因果传播路径上。仅对结果进行评估隐藏了这种失败模式；逐步因果真相是可信的LLM基于RCA代理所缺少的部分。

Conclusion: PAVE协议有助于提高LLM在RCA任务中的性能，但仍然存在一些挑战，如无根据诊断问题。
...
TL;DR: TOPS是一种高效、通用的视觉标记剪枝方法，能够在减少视觉标记的同时保持或提高模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉标记剪枝方法存在效率低、保留冗余标记等问题。

Method: 提出了一种基于信息理论的视觉标记剪枝方法TOPS，该方法不依赖于训练和模型特定，适用于多种多模态大语言模型。

Result: 实验结果表明，TOPS在多种多模态大语言模型和基准测试中均优于现有方法，能够在减少视觉标记的同时保持或提高模型性能。

Conclusion: TOPS在多种剪枝设置下优于现有方法，能够有效减少冗余视觉标记并提高性能。
...
TL;DR: 提出过程 harness 机制，提升传统工作流程，实现 Agentic BPM


<details>
  <summary>Details</summary>
Motivation: 提升传统工作流程进入Agentic BPM（Agentic业务流程管理）

Method: 提出过程 harness 机制，并开发Task-Decision-Flow（TDF）模型

Result: 实现过程 harness 的设计和实现，并在贷款审批工作流程中展示其功能

Conclusion: 过程 harness 通过在特定控制点调用政策框架的代理自主权，独特地协调了命令式要求和规范性要求
...
TL;DR: GAversary是一种新的对抗性攻击方法，在保持较高语义相似性的同时，显著降低了目标模型的准确性。


<details>
  <summary>Details</summary>
Motivation: 自然语言处理模型易受对抗性输入的影响，这可能导致严重的现实后果。

Method: 提出了一种名为GAversary的混合遗传算法（GA）来生成对抗性攻击。GA可以将目标模型视为黑盒，只需要模型输出的logit值来引导搜索。GAversary使用GloVe嵌入来提出单词替换（变异操作），以提高对抗性示例的语义相似性。

Result: GAversary在多个基准数据集和知名目标模型上进行了应用。与BAE和A2T攻击相比，GAversary能够显著降低目标模型在测试数据上的准确性。在最佳情况下，将76.8%的准确性降低到5.8%，而BAE的准确率降低到27.6%。

Conclusion: GAversary在减少目标模型准确性的同时，略微增加了运行时间，但保持了较高的语义相似性。
...
TL;DR: BINEVAL是一种新的评估LLM输出的框架，它通过分解评估标准并提供透明的反馈来提高评估的准确性和可解释性。
...
Result: 在SummEval、Topical-Chat和QAGS上，BINEVAL与包括UniEval和G-Eval在内的强大基线相匹配或优于它们，特别是在QAGS等事实一致性基准上表现突出。BINEVAL不仅与人类判断具有竞争力的相关性，而且更好地匹配人类评分分布，避免了先前LLM评分者中常见的天花板效应，从而更好地区分边缘和明显有缺陷的输出。此外，相同的反馈支持迭代提示优化，在自我更新和跨模型更新设置下，改进了总结和IFBench上的评估提示。
...
TL;DR: 本文提出EO-WM模型，用于地球观测预测，提高预测精度和响应保真度，提供新的评估基准。


<details>
  <summary>Details</summary>
Motivation: 地球观测预测的目的是从卫星观测中预测未来地表动态，在变化的风云条件下。本文将这一任务视为一个部分观察、受天气驱动的世界建模问题，其中天气作为条件信号，但由于观测稀疏和地表状态未观测，预测仍然存在不确定性。然而，现有方法并没有完全捕捉这个设置：确定性模型将不确定性简化为单一的未来预测，而基于扩散的方法通常将天气变量视为未分化的条件信号，现有的基准主要关注重建精度，而不是预测是否正确响应天气变化。

Method: 本文引入了EO-WM，这是一种用于多光谱地球观测预测的视频扩散变换器。EO-WM集成了物理信息化的条件框架，通过气候基线、天气异常和累积物理应力信号来表示气象强迫。具体来说，它通过不同的条件路径将基线和异常分开，并随时间积累异常强迫，以捕捉持续的炎热和干旱应力。为了评估超出标准指标的天气响应行为，我们引入了两个诊断基准：一个极端夏季基准，用于在极端天气下对植被退化的严重程度进行预测，以及一个季节性匹配对基准，用于测试在变化的天气强迫下的响应保真度。

Result: 实验表明，EO-WM将预测的归一化植被指数（NDVI）下降幅度误差降低了相对5.63%，并提高了方向命中率的相对7.80%，同时在标准像素级指标上保持竞争力。基准和模型将开源于https://github.com/Luo-Z13/EO-WM。

Conclusion: 本文提出的EO-WM模型在地球观测预测方面取得了显著成果，提高了预测精度和响应保真度，并提供了新的评估基准。
...
TL;DR: 研究显示，在简历质量同质且注射者少的情况下，prompt注入能提高排名，但随着注射者增多效果减弱。


<details>
  <summary>Details</summary>
Motivation: 研究prompt注入在自动简历筛选中的应用，以及它对LLM评估的影响。

Method: 通过控制实验，观察prompt注入对申请人排名的影响。

Result: 当简历质量同质且注射者少时，prompt注入可以提高申请人排名。但随着注射者增多，其效果会迅速下降。当候选人质量参差不齐时，prompt注入效果一般，但有时能让低质量候选人排名高于高质量候选人，引发公平性问题。

Conclusion: 基于LLM的筛选在操纵罕见且候选人质量差异小的情况下最易受到攻击。
...
TL;DR: 多模型LLM系统提高准确性的潜力有限，需要优化模型选择和路由策略。


<details>
  <summary>Details</summary>
Motivation: 通过多模型LLM系统提高准确性

Method: 分析多模型系统的性能

Result: 发现准确性的提高受到beta的限制，并提出了Clopper-Pearson界限

Conclusion: 多模型系统可以提高准确性，但存在限制，且模型选择和路由策略很重要。
...
TL;DR: 提出了一种基于语言的数字孪生框架，用于模拟老年人的对话行为，并在认知健康监测中显示出了良好的效果。


<details>
  <summary>Details</summary>
Motivation: 在个性化医疗领域，数字孪生作为一种有前景的范例，可以模拟个体行为和健康状况。在认知健康领域，早期检测轻度认知障碍（MCI）仍然具有挑战性，语言和对话模式作为无创的生物标志物。

Method: 提出了一种基于语言的数字孪生框架，利用大型语言模型（LLMs）通过结合文体学和上下文元数据来模拟老年人的对话行为。引入了多头条件变分自动编码器（cVAE）来评估保真度和认知一致性，该编码器同时衡量重建质量和预测认知分数。

Result: 在I-CONECT数据集上的实验表明，数字孪生保留了个体特定的特征，实现了与真实数据相当的重建和MoCA预测误差，同时优于基线GPT生成的回答。

Conclusion: 基于语言的双胞胎在个性化、持续的认知健康监测中具有潜在的应用价值。
...
### [338] [MIRAGE: Protecting against Malicious Image Editing via False Moderation](https://arxiv.org/abs/2606.26199)
*Anshul Nasery,Ramnath Kumar,Cho-Jui Hsieh,Sewoong Oh*

Main category: cs.CR

TL;DR: MIRAGE is a new method to protect images from AI editing by causing moderation classifiers to flag them as policy-violating.
...
Method: Proposing MIRAGE, a method that adds adversarial perturbations to align images to policy-violating concepts in the representation space of open-source embedding and moderation models.

Result: MIRAGE demonstrates success rates of more than 88% against multiple closed-source image editing APIs.
...
### [339] [Data Facts: A Metadata Schema for Structured Data Exchange in the NANDini Multi-Agent Ecosystem](https://arxiv.org/abs/2606.26211)
*Jin Gao,Maria Gorskikh,Pradyumna Chari,Brittany Box,Mukul Kemla,Pratik Behera,Abhishek Mehta,Ramesh Raskar*

Main category: cs.CR

TL;DR: Data Facts and security enhance autonomous agent interactions.


<details>
  <summary>Details</summary>
Motivation: To enable autonomous and real-time agent interactions in a networked ecosystem.

Method: Introducing Data Facts, a JSON metadata schema, and implementing a three-layer security pipeline for data access.

Result: Achieved 100% accuracy in data-informed agent decision-making, reduced stale-data errors, and blocked all forgery attempts with zero data leakage.

Conclusion: Data Facts and the security pipeline effectively support autonomous agent interactions in a networked ecosystem.
...
### [340] [CyberChainBench: Can AI Agents Secure Smart Contracts Against Real-World On-Chain Vulnerabilities?](https://arxiv.org/abs/2606.26216)
...
### [341] [Expecting (Targeted Ads)? Network Analysis of User Health Data Leakage in Fertility Tracking Apps](https://arxiv.org/abs/2606.26276)
...
### [342] [TEMPO-Diffusion: Temporally Exposed Malicious Poisoning of Diffusion Models](https://arxiv.org/abs/2606.26285)
*William Aiken,Paula Branco,Guy-Vincent Jourdan,Iosif-Viorel Onut*

Main category: cs.CR

TL;DR: TEMPO-Diffusion框架能够有效地对特定类别进行针对性的攻击，并提高了攻击成功率


<details>
  <summary>Details</summary>
Motivation: 为了提高基于噪声的回门攻击在扩散模型中的隐蔽性和实际相关性

Method: 提出了一种名为TEMPO-Diffusion的目标回门框架，该框架将恶意分布偏移定位到时间上的分布暴露

Result: TEMPO-Diffusion能够在CIFAR10、GTSRB和CALISA数据集上可靠地毒害特定类别的合成数据生成，并诱导下游分类器的高攻击成功率

Conclusion: TEMPO-Diffusion在利用后门扩散模型进行合成训练数据方面具有相关和实际的安全问题
...
### [343] [Beyond Takedown: Measuring Malicious Go Module Persistence in the Wild](https://arxiv.org/abs/2606.26291)
*Minjae Bae,Carter Yagemann*

Main category: cs.CR

TL;DR: Go生态系统存在自动化供应链攻击，GitHub和Google Go已采取措施


<details>
  <summary>Details</summary>
Motivation: 测量Go生态系统中的自动化供应链攻击活动

Method: 手动搜索GitHub上的2,113个存储库和大规模扫描12.3M索引条目，使用GOAST进行去混淆

Result: 发现2,289个恶意版本的合法Go模块，纯GitHub搜索无法全面识别受影响代码，99.4%的受影响代码可通过Go代理访问

Conclusion: GitHub和Google Go团队已采取措施删除恶意代码
...
### [344] [Hybrid privacy-aware semantic search: SVD-truncated document geometry and CKKS-encrypted query reranking under a restricted threat model](https://arxiv.org/abs/2606.26373)
*Sergey Kurilenko*
...
### [345] [Verifying Intent and Harm: A Unified Defense Against LLM-Generated Threats](https://arxiv.org/abs/2606.26377)
*Poojitha Thota,Yun Lei,Santhosh Thangaraj,Siddhartha Reddy Jonnalagadda,Shirin Nilizadeh*

Main category: cs.CR

TL;DR: 提出了一种新的防御框架，通过联合评估提示意图和响应危害来提高LLM的安全性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）在交互式应用中越来越受欢迎，但它们仍然容易受到有害、欺骗性或违反策略的对抗性交互的影响。现有的防御措施通常只分析用户提示或生成的输出，而不分析两者。然而，许多现实世界的攻击利用了提示中表达的对抗意图和只有在实际响应中才显现的可操作危害之间的分离。因此，仅提示和仅响应的防御措施通常会错过看似无害的交互，当从孤立的一侧查看时。

Method: 提出了一种以验证为中心的防御框架，该框架在将LLM响应交付给用户之前，联合评估提示意图和响应危害。该框架采用专门的分析师进行意图和危害评估，以及一个法官进行冲突解决。

Result: 实验表明，联合验证提示意图和响应危害始终优于单方面的防御和单代理推理基线。在威胁类别方面，该框架将平均F1从最强基线的0.90提高到0.95，同时将平均攻击成功率降低到4.1%。与Single-Agent+CoT基线相比，它将平均F1从0.87提高到0.95，并将良性敏感请求的误报率从0.12降低到0.06。

Conclusion: 提示-响应验证为保护LLM应用免受不断发展的对抗性威胁提供了实用的基础。
...
### [346] [Lessons from the Adoption and Deprecation of the Privacy Sandbox Web APIs](https://arxiv.org/abs/2606.26390)
...
### [347] [What Browsers Do in the Shaders: A Measurement Study of WebGPU Privacy](https://arxiv.org/abs/2606.26412)
*Igor Santos-Grueiro*

Main category: cs.CR

TL;DR: WGPULens框架用于测量WebGPU隐私问题，并提出缓解策略。


<details>
  <summary>Details</summary>
Motivation: 分析WebGPU的隐私问题，并提出WGPULens框架来测量相关信号。

Method: 通过控制场景、浏览器原生共存、参与者现场研究、公共页面加载和缓解策略来测量信号。

Result: WGPULens框架可以识别出WebGPU的隐私问题，并提出相应的缓解策略。

Conclusion: WebGPU隐私分析必须针对具体表面进行，浏览器需要测量哪些GPU状态跨越哪些边界，哪些浏览器可见信号揭示它，以及相应的缓解成本。
...
### [348] [Adaptive Evaluation of Out-of-Band Defenses Against Prompt Injection in LLM Agents](https://arxiv.org/abs/2606.26479)
...
### [349] [DKVE: Decentralized Key Validation for End-to-End Encrypted Messaging](https://arxiv.org/abs/2606.26486)
...
### [350] [VIGIL: Runtime Enforcement of Behavioral Specifications in AI Agent Skills](https://arxiv.org/abs/2606.26524)
*Ying Li,Yanju Chen,Hongbo Wen,Bosi Zhang,Hanzhi Liu,Peiran Wang,Yu Feng,Yuan Tian*

Main category: cs.CR

TL;DR: VIGIL是一种新的运行时强制执行框架，用于检测代理系统中的策略违规，具有高召回率和低误报率。


<details>
  <summary>Details</summary>
Motivation: 为了解决在代理系统中执行运行时强制执行的问题，特别是当技能通过第三方技能操作时。

Method: 提出了VIGIL，一个端到端的运行时强制执行框架，用于检查代理的实际执行跟踪与技能规范、操作员定义的约束和跨多个技能的全局规则中的行为策略。VIGIL引入了一种策略语言，用于捕获特定于上下文的执行要求，并使用符号评估规则将策略转换为SMT约束，以检测违反事件顺序、参数关系或跨调用值流的违规行为。

Result: 在真实LLM代理运行中，VIGIL以超过95%的召回率和低于10%的误报率检测到策略违规。

Conclusion: VIGIL是一个有效的端到端运行时强制执行框架，可以有效地检测代理系统中的策略违规。
...
### [351] [TESLA-for-5G: Broadcast Authentication for 5G Networks Using TESLA](https://arxiv.org/abs/2606.26528)
*Subin Song,Michael K. Reiter,Taekyoung Kwon*

Main category: cs.CR

TL;DR: TF5是一种新的5G SIB1认证协议，使用TESLA和GG09 IBS，提高了认证效率并减轻了用户终端的负担。


<details>
  <summary>Details</summary>
Motivation: 现有的方法要求用户终端对系统信息块1（SIB1）进行数字签名认证，这给资源受限的用户终端带来了沉重的计算负担。

Method: 提出了一种名为TESLA-for-5G（TF5）的广播认证协议，结合了TESLA和GG09 Schnorr-like基于身份的签名（IBS）。

Result: TF5允许用户终端使用对称MAC和延迟密钥披露来认证每个SIB1消息，避免了每条消息的数字签名需求。在小区进入过程中，使用轻量级的GG09 IBS在TESLA参数上建立初始信任，避免了证书分发的开销。通过OpenAirInterface 5G堆栈的实现和基于跟踪的分析，证明了TF5在计算、通信和存储成本方面的优势。

Conclusion: TF5是一种有效的5G SIB1广播认证协议，可以显著减轻资源受限用户终端的负担，并提高认证效率。
...
### [352] [Adversarial Diffusion Across Modalities: A Fusion Survey of Attacks, Defenses, and Evaluation for Text, Vision, and Vision-Language Models](https://arxiv.org/abs/2606.26566)
*Abrar Alotaibi,Moataz Ahmed*
...
### [353] [Agents That Know Too Much: A Data-Centric Survey of Privacy in LLM Agents](https://arxiv.org/abs/2606.26627)
*Nada Lahjouji,Ashwin Gerard Colaco*

Main category: cs.CR

TL;DR: 对LLM代理的隐私风险进行了调研，发现了治理机制和基准方面的不足。


<details>
  <summary>Details</summary>
Motivation: 随着LLM代理在敏感数据处理中扮演越来越重要的角色，隐私保护变得更加困难。

Method: 对LLM代理的隐私风险进行数据中心视角的调研，包括数据源、隐私风险和治理机制。

Result: 发现了当前治理机制中只有信息流控制能够覆盖组合和跨会话推理泄漏，以及没有基准能够驱动代理在单一隐私策略下跨数据表面操作的问题。

Conclusion: 我们需要一个能够处理跨数据源隐私保护的基准，并解决当前研究中存在的遗漏问题。
...
### [354] [TGHE: Template-based Graph Homomorphic Encryption for Privacy-Preserving GNN Inference in Edge-Cloud Systems](https://arxiv.org/abs/2606.26664)
...
### [355] [The Fungible Reserve Standard: A Deterministic Framework for Encoding Carrying Costs in Asset-Backed Tokens](https://arxiv.org/abs/2606.26704)
*JJ Jia Jing Tan,Eva Meng,Josh Ng,Zack Zhang,September Liu,Teelet Wang,Ludwig Zhang,Seth Yan*

Main category: cs.CR

TL;DR: 提出FRS，解决RWA代币化成本问题，提供透明、高效的代币设计框架。


<details>
  <summary>Details</summary>
Motivation: 解决现实世界资产（RWAs）代币化过程中的成本问题，特别是物理资产如贵金属、存储商品和仓储商品的负持有成本。

Method: 提出可替换储备标准（FRS），一个确定性的代币设计框架，将持有成本透明地编码到链上逻辑中。

Result: FRS引入了一个资产-代币变量q(t)，该变量根据预定义的年化持有成本率降低，并辅以一个供应协调机制，以保持持有者余额和ERC-20可组合性。

Conclusion: FRS为具有正的、可预测的持有成本的任何现实世界资产提供了一种资产无关的框架，同时保持了与DeFi协议的兼容性。
...
### [356] [DroidBreaker: Practical and Functional Problem-Space Attacks on Machine-Learning Android Malware Detectors](https://arxiv.org/abs/2606.26707)
...
### [357] [MergeLLL: A Hierarchical Divide-and-Conquer Framework for LLL-Based Lattice Reduction](https://arxiv.org/abs/2606.26784)
*Niharika Gauraha*

Main category: cs.CR

TL;DR: MergeLLL算法通过分解和合并策略，有效提高了格基降维的效率和质量


<details>
  <summary>Details</summary>
Motivation: 提高局部格结构，降低全局基性质优化成本

Method: 将格基分解为子基，独立执行局部降维，通过层次合并重建完整基

Result: 提高了正交性和数值稳定性，降低了计算成本，并行化执行，实验证明优于经典降维算法

Conclusion: MergeLLL算法在保持格结构的同时，提高了降维效率和质量
...
### [358] [MIRROR: Novelty-Constrained Memory-Guided MCTS Red-Teaming for Agentic RAG](https://arxiv.org/abs/2606.26793)
*Inderjeet Singh,Andrés Murillo,Motoyoshi Sekiya,Yuki Unno,Junichi Suga*

Main category: cs.CR

TL;DR: MIRROR是一种新的攻击框架，提高了针对多模态代理RAG系统的攻击成功率，并具有较低的跨表面方差。


<details>
  <summary>Details</summary>
Motivation: 为了解决现有的红队方法通常针对特定表面并且经常重复已知攻击模板的问题，以及多模态代理检索增强生成（RAG）系统攻击面的扩展。

Method: 提出了一种名为MIRROR的统一跨表面框架，该框架在检索上下文下进行记忆引导蒙特卡洛树搜索，并在显式新颖性约束下对候选生成进行条件化。

Result: MIRROR在四个攻击表面上实现了76%的ASR（攻击成功率）在图像中毒方面，与基线相比提高了24%，在指挥官攻击方面实现了97%的ASR，查询成本减半，并且具有最低的跨表面方差（变异系数0.47）。

Conclusion: MIRROR框架有效地提高了针对多模态代理RAG系统的攻击成功率，并降低了攻击成本和跨表面方差。
...
### [359] [SpikeTimer: Exploring Active Copyright Protection in Spiking Neural Networks via Temporal Backdoor Regularization](https://arxiv.org/abs/2606.26841)
*Xiao Yang,Gaolei Li,Jun Wu,Jianhua Li,Zhiquan Liu*

Main category: cs.CR

TL;DR: 提出SpikeTimer，用于SNN版权保护，在多个数据集上表现良好。


<details>
  <summary>Details</summary>
Motivation: SNN在能量效率计算和事件驱动传感数据处理方面表现出色，但在版权保护方面仍需探索。

Method: 提出了一种名为SpikeTimer的新颖版权保护框架，通过时间后门学习对SNN进行版权保护。

Result: SpikeTimer能够精确地响应包含正确时间切片中的标记的授权数据，对未授权数据产生错误响应。

Conclusion: SpikeTimer在多个神经形态数据集上实现了约10%的未授权数据准确性，同时在授权输入上仅有约1.5%的下降。
...
### [360] [Fortress and Gatekeeper: Theorizing Transitive Trust in Third-Party Cybersecurity Risk Governance](https://arxiv.org/abs/2606.26866)
*Yijun Chen,Misita Anwar*

Main category: cs.CR

TL;DR: 本文通过分析OpenAI-Mixpanel安全事件，探讨了第三方供应商在数字服务提供中的安全问题，提出了可传递信任和Fortress-Gatekeeper框架，对网络安全治理研究有重要贡献。


<details>
  <summary>Details</summary>
Motivation: 分析第三方供应商在数字服务提供中的嵌入问题，特别是其带来的客户数据安全和治理责任问题。

Method: 通过分析2025年11月OpenAI-Mixpanel安全事件，结合组织信任研究和代理理论。

Result: 提出了可传递信任的概念，Fortress和Gatekeeper框架，以及四个关于供应商整合、元数据暴露、供应商保证和数据传播的命题。

Conclusion: 为网络安全治理研究做出了贡献，解释了委托数据处理如何创造面向客户的问责制，并确定了供应商分层、数据分类、合同设计、持续保证和数据最小化等方面的启示。
...
### [361] [Chai: Agentic Discovery of Cryptographic Misuse Vulnerabilities](https://arxiv.org/abs/2606.26933)
*Corban Villa,Sohee Kim,Austin Chu,Alon Shakevsky,Raluca Ada Popa*

Main category: cs.CR

TL;DR: Chai系统利用AI发现密码学滥用漏洞，发现100多个漏洞。
...
Result: Chai在X.509、JWT和SAML库上进行了评估，发现了100多个漏洞，包括一个SSL库中的严重漏洞和两个主要浏览器和Linux发行版中的安全漏洞。

Conclusion: Chai系统通过AI技术有效地发现了密码学滥用漏洞，提高了漏洞发现和验证的效率。
...
### [362] [Jailbreaking for the Average Jane: Choosing Optimal Jailbreaks via Bandit Algorithms for Automatically Enhanced Queries](https://arxiv.org/abs/2606.26936)
*Prarabdh Shukla,Ritik,Suhas Rao,Arpit Agarwal,Arjun Bhagoji*

Main category: cs.CR

TL;DR: 非专家恶意行为者可以通过有效的攻击策略对LLM进行攻击，成功率达到97%。


<details>
  <summary>Details</summary>
Motivation: 随着LLM的越狱方法广为人知，人们越来越担心非专家恶意行为者（“普通Jane”）可能会对恶意请求产生有效响应。

Method: 提出了一种基于多臂老虎机框架的新型攻击策略，用于高效在线学习从大量选择集中最优越狱方法。同时，创建了FrankensteinBench，一个包含11,279个恶意查询的安全基准，用于评估攻击策略。

Result: 实验结果表明，基于老虎机的攻击在15个SoTA开放权重LLM上平均成功率达到97%。增加查询的复杂性可以将攻击成功率平均提高26%。

Conclusion: 实验证实了人们的担忧，非专家恶意行为者可以通过有效的攻击策略对LLM进行攻击。
...
### [363] [Protocol Prying: Systematic Vulnerability Research in the Apple AirDrop and Android Quick Share Proximity Transfer Protocols](https://arxiv.org/abs/2606.26967)
*Arash Ale Ebrahim,Nils Ole Tippenhauer*

Main category: cs.CR

TL;DR: 对AirDrop和Quick Share进行了安全性研究，发现多个漏洞


<details>
  <summary>Details</summary>
Motivation: 分析苹果AirDrop和谷歌/三星快速共享的文件传输协议的安全性

Method: 跨平台逆向工程和模糊测试研究

Result: 发现六个漏洞，包括macOS/iOS AirDrop的三个预认证问题，三星快速共享的两个协议层漏洞，以及谷歌快速共享Windows的堆使用后释放漏洞

Conclusion: 苹果、三星和谷歌已确认报告，并对此作出回应
...
### [364] [ShareLock: A Stealthy Multi-Tool Threshold Poisoning Attack Against MCP](https://arxiv.org/abs/2606.27027)
*Liwei Liu,Tianzhu Han,Zijian Liu,Zishu Dong,Na Ruan*

Main category: cs.CR

TL;DR: ShareLock：一个基于多工具阈值中毒的框架，增强了LLM的安全性，提高了隐蔽性和攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 随着LLM驱动代理的快速发展，MCP（模型上下文协议）已成为现代代理生态系统的基石。然而，MCP的广泛应用也带来了新的安全担忧，如工具中毒攻击（TPA）。现有的中毒方案通常采用统一的明文嵌入范式，无法抵抗人工检查或自动检测器。当前研究缺乏对多工具中毒的系统分析，即多个工具可以合作以分散检测风险。

Method: 我们引入了ShareLock，一个多工具阈值中毒框架，它利用Shamir的阈值方案确保卓越的隐蔽性和容错性。ShareLock将恶意指令作为看似良性的秘密份额分散到多个工具描述中，实现了信息论上的秘密性和对适度审计的攻击鲁棒性。在服务器更新期间植入了隐蔽的重构触发器后，汇总的份额重构了隐藏的指令，导致对系统资产或私人数据的严重破坏。为了评估ShareLock的现实威胁，我们构建了一个包含四个多工具场景的综合基准，并在两个不同的MCP客户端上对主流LLM进行了广泛的实验。

Result: 我们的结果表明，ShareLock在基于工具描述的检测中显著优于现有的单一工具中毒策略，同时保持了平均攻击成功率超过90%的攻击成功率。

Conclusion: ShareLock是一个多工具阈值中毒框架，通过分散恶意指令来增强LLM的安全性。它提供了更好的隐蔽性和容错性，同时保持了高攻击成功率。
...
### [365] [Design and Performance Evaluation of Secure RF and WiFi-Based Communication in Drone Swarms via Testbed Implementation](https://arxiv.org/abs/2606.27028)
*Bhavya Dixit,Aayushi Rajgor,Subham Kumar,Rushikesh Patil,Ananthapadmanabhan A.,Gaurav S. Kasbekar,Arnab Maity*

Main category: cs.CR

TL;DR: MAVShield是一个高效且安全的无人机群通信解决方案。


<details>
  <summary>Details</summary>
Motivation: 无人机群依赖于分布式协调和合作通信以支持可扩展操作、扩展覆盖以及监视和实时数据交换等应用。无线技术如射频（RF）和WiFi在无人机之间以及无人机与地面控制站（GCS）之间的通信中广泛应用，但引入了重大的安全挑战。MAVLink是无人机系统中占主导地位的通信协议，提供消息完整性和认证，但缺乏内置加密，使遥测流量易受窃听。在先前的研究中，我们提出了MAVShield，一个用于MAVLink通信的轻量级加密框架。在这篇论文中，将MAVShield、AES-CTR、Speck-CTR、ChaCha20和Rabbit集成到四个自定义构建的无人机中，以在RF和WiFi通道上建立安全的通信链路。通过无人机群测试平台进行的飞行实验评估了其性能。加密遥测数据使飞行中的自主编队控制和碰撞避免成为可能。为了碰撞避免，我们开发了一个修改后的人工势场（APF）算法，该算法在地理坐标中直接计算吸引力和排斥力，消除了笛卡尔变换，减少了轨迹振荡，并避免了局部最小值捕获。测量了每种加密方案的CPU利用率、内存消耗和分组交付率（PDR）。结果表明，MAVShield在性能上与未加密通信相当，但在整体效率上优于AES-CTR、Speck-CTR、ChaCha20和Rabbit。代数密码分析和基于Wireshark的流量分析表明，MAVShield对密钥恢复攻击具有抵抗力，并保护了遥测数据的机密性。结果表明，MAVShield是无人机群通信的一种高效且安全的解决方案。

Method: 将MAVShield、AES-CTR、Speck-CTR、ChaCha20和Rabbit集成到四个自定义构建的无人机中，在RF和WiFi通道上建立安全的通信链路。通过无人机群测试平台进行的飞行实验评估了其性能。为了碰撞避免，开发了一个修改后的APF算法。

Result: MAVShield在性能上与未加密通信相当，但在整体效率上优于AES-CTR、Speck-CTR、ChaCha20和Rabbit。代数密码分析和基于Wireshark的流量分析表明，MAVShield对密钥恢复攻击具有抵抗力，并保护了遥测数据的机密性。

Conclusion: MAVShield是无人机群通信的一种高效且安全的解决方案。
...
### [366] [Physical Layer Authentication With Channel Knowledge Maps in Indoor Environments](https://arxiv.org/abs/2606.27044)
*Luca Bonaventura,Francesco Ardizzon,Stefano Tomasin*

Main category: cs.CR

TL;DR: 针对室内移动设备提出了一种物理层认证方法，通过信道知识图和到达角进行认证，验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 移动设备在室内环境中的物理层认证面临挑战，因为多径传播和遮挡导致时间一致性和演变建模变得困难。

Method: 提出了一种针对室内移动设备的物理层认证机制，其中多个接入点（AP）通过接收信号估计主信道抽头路径损耗（PL）和到达角（AoA），并将其与之前收集的信道知识图（CKM）进行比较。具体来说，测量值与从CKM获得的先前已知位置附近的测量值进行比较。在随机和最优攻击下进行了全面的安全分析。

Result: 通过射线追踪获得的CKM在代表性室内场景中的数值结果验证了所提出物理层认证方法的有效性。

Conclusion: 所提出的物理层认证方法在室内移动设备中表现出良好的效果，并能够有效地抵御攻击。
...
### [367] [Type-based information flow analysis for $π$-calculus with a dynamically extensible security lattice](https://arxiv.org/abs/2606.27059)
*Yukihiro Oda,Eijiro Sumii*

Main category: cs.CR

TL;DR: 开发了一种新的类型系统，可在执行过程中动态调整安全级别，增强了秘密性的形式化。


<details>
  <summary>Details</summary>
Motivation: 为了提高信息流的安全性，我们开发了一种新的类型系统。

Method: 通过扩展Kobayashi的类型安全信息流分析，将其应用于Milner的π-演算。

Result: 该系统可以动态地创建和插入新的安全级别，即使在系统执行过程中。

Conclusion: 该系统在保持简洁语法和基于归约的语义的同时，增强了秘密性作为非干扰的鲁棒形式化。
...
### [368] [Inherited Circuits, Learned Semantics: How Fine-Tuning Creates Evasion Vulnerabilities Invisible to Standard Evaluation](https://arxiv.org/abs/2606.27091)
*Ryan Fetterman*

Main category: cs.CR

TL;DR: 微调可以提高任务准确性，同时扩大逃避面，因此不能简单地将小任务特定微调视为更安全的分类器。


<details>
  <summary>Details</summary>
Motivation: 为了评估安全分类中LLMs微调的效果，通常在训练数据相同分布的保留示例上进行评估。

Method: 研究Foundation-Sec-8B-Instruct及其基础模型Llama-3.1-8B-Instruct在匹配的PowerShell分类群体上的表现。

Result: 发现微调本身可能会引入漏洞，例如在PowerShell别名替换、命令重建、字符串构造、执行间接和大小写变换等行为保持变换下，模型可能学习到保留规范准确性的标记级指示语义。

Conclusion: 微调可以改善基线行为，同时创建对变换敏感的攻击面。
...
### [369] [zQR: A Verifiable QR-Driven zkSNARK Proof Verification Framework for Mobile Platforms](https://arxiv.org/abs/2606.27092)
*Goshgar Can Ismayilov*

Main category: cs.CR

TL;DR: 本文提出了一种新的QR驱动zkSNARK证明验证框架（zQR），用于移动平台上的隐私保护验证，并通过实验证明了其可行性和有效性。


<details>
  <summary>Details</summary>
Motivation: 隐私是现代社会中个人的基本权利。然而，隐私保护技术在日常互动中的实际应用仍然有限。零知识证明提供了强大的隐私保障，但往往受到其技术复杂性的限制。

Method: 我们提出了一种可验证的二维码（verifiable QR codes）的想法，使离线验证者能够验证编码在二维码中的证明。基于这一核心思想，我们为移动平台构建了一个新的基于QR码的zkSNARK证明验证框架（即zQR）。该框架集成了区块链以实现可审计性、不可否认性和日志记录；以及大型语言模型用于自动电路生成。通过对多个攻击面进行安全讨论，我们对该框架进行了安全评估。此外，我们还进行了一项实验评估，衡量了时间成本（证明生成和验证延迟、二维码编码和解码延迟）和财务成本（区块链气体消耗）。

Result: 结果表明，zQR作为隐私保护验证的概念验证框架在移动平台上是可行的，其中证明以19位二维码符号版本表示，错误纠正级别低。

Conclusion: 本文讨论了在移动平台上实现隐私保护验证的方法，并提出了zQR框架，该框架利用二维码和区块链技术，结合大型语言模型，实现高效、安全的隐私保护。
...
### [370] [Application of LLMs to Threat Assessment of Foreign Peacekeeping Missions](https://arxiv.org/abs/2606.27106)
*Gerhard Backfried,Christian Schmidt,Diego Pilutti,Michael Suker*

Main category: cs.CR

TL;DR: 本文提出了一种利用LLMs进行维和任务威胁评估的新方法。
...
### [371] [PRISM: PE Relational Inter-Section Matrix. A 2D Section-Aware Dataset for Static PE Malware Detection](https://arxiv.org/abs/2606.27109)
*José M. Sacristán,Ana I. González-Tablas*

Main category: cs.CR

TL;DR: PRISM通过矩阵表示提高了PE恶意软件检测的准确性和效率
...
Method: 提出PRISM（PE关系交矩阵）数据集和特征表示方法，构建了一个包含83,633个去重矩阵和49,204个样本的分析语料库

Result: PRISM能够捕获PE文件中每部分的位置结构信息，在较小的维度下几乎恢复了与EMBER向量相同的二进制检测性能

Conclusion: PRISM在保持兼容性的同时，提高了恶意软件检测的准确性和效率
...
### [372] [The Observer World: A Cryptographic Extension of Impagliazzo's Five Worlds](https://arxiv.org/abs/2606.27139)
*Fabio F. G. Buono*
...
### [373] [Tilikum: Transaction Fair Ordering on a DAG without Weak Edges](https://arxiv.org/abs/2606.27250)
*Giulio Segalini,Yigit Çolakoğlu,Marko Putnik,Jérémie Decouchant*

Main category: cs.CR

TL;DR: Tilikum：一种基于DAG的账本协议，实现公平交易排序，提升DeFi应用安全性。


<details>
  <summary>Details</summary>
Motivation: 提高去中心化金融应用的交易顺序公平性，防止重排序攻击，提升DAG共识协议的安全性。

Method: 提出了一种名为Tilikum的基于DAG的账本协议，通过利用基于中值的时标聚合（或批量排序公平性）实现排序线性化，同时保持低数据冗余和强大的垃圾收集功能。

Result: 在与其他公平排序基线（如Narwhal/Tusk、Pompē、Themis和FairDAG）的评估中，Tilikum实现了高达39倍的其他公平排序基线的吞吐量，并完全阻止了最先进的DAG特定重排序攻击。

Conclusion: Tilikum在保持高吞吐量和安全性的同时，为DAG共识协议提供了有效的公平交易排序解决方案。
